...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，仍存在明显缺陷，如对输入数据微小变化的敏感性、关键场景下缺乏情境认知，以及错误输出却过度自信等问题。为此，研究者提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架帮助研究者定位创新方向（如垂直领域洞察转化为通用方案）、发现被忽视的机会（如开发解决跨领域共性问题的模型而非追逐基准测试），并促进跨学科交流（如建立AI开发者、领域专家与人机交互学者间的共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T19:01:25Z
- **目录日期**: 2025-04-06
