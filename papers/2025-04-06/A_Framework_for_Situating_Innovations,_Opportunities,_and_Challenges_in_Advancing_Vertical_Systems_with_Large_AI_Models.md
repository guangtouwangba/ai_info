...
大型AI模型在标准化测试中表现优异，但在医疗、教育等高风险领域应用时，常暴露出脆弱性、缺乏情境理解、输出错误信息等问题。为此，研究者提出分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架帮助研究者：(1)定位创新方向（如垂直领域知识赋能通用创新）；(2)发现被忽视的机会（如开发实用基础模型而非追逐基准测试）；(3)促进跨学科交流（为AI开发者、领域专家等建立共同语言）。案例研究表明该框架可有效指导实践。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T14:01:38Z
- **目录日期**: 2025-04-06
