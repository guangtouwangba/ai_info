...
大型AI模型在标准化测试中表现出色，但在医疗、教育等高风险领域应用时暴露出明显缺陷，如对输入数据敏感、决策缺乏情境理解、输出错误信息等。为此，研究提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架能帮助研究者：(1)定位创新点（如垂直领域知识如何赋能通用创新）；(2)发现被忽视的机会（如跨领域共性问题）；(3)促进跨学科交流（建立开发者、领域专家间的共同语言）。案例研究验证了该框架的实践价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T11:01:35Z
- **目录日期**: 2025-04-06
