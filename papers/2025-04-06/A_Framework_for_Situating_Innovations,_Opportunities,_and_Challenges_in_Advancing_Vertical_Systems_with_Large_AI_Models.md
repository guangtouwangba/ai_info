...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，暴露出明显缺陷，如对输入数据微小变化的脆弱性、关键场景下缺乏上下文判断，以及自信地生成错误信息等问题。为此，研究者提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架帮助研究者：(1)定位创新点（如垂直领域洞察推动通用创新）；(2)发现被忽视的机会（如跨领域共性问题）；(3)促进跨学科交流（建立开发者、领域专家与人机交互学者的共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T04:01:46Z
- **目录日期**: 2025-04-06
