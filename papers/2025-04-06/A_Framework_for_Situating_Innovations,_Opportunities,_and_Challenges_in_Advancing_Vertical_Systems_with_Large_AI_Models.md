...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域的实际应用中暴露出明显缺陷，如对输入数据微小变化的脆弱性、关键场景下缺乏上下文判断，以及自信地生成错误信息等问题。为此，研究者提出一个分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层级的动态调整。该框架能帮助研究者：(1)定位创新方向，(2)发现跨领域共性机会（如开发实用基础模型而非追逐基准测试），(3)促进跨学科交流（为AI开发者、领域专家等建立共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T23:01:20Z
- **目录日期**: 2025-04-06
