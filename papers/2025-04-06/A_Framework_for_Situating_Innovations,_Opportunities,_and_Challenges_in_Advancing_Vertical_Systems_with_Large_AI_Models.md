...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，仍存在明显缺陷，如输入数据敏感性、决策缺乏情境认知及输出错误信息等问题。为此，研究者提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架帮助研究者：(1)定位创新点（如垂直领域知识如何赋能通用创新）；(2)发现被忽视的机会（如跨领域共性问题驱动实用基础模型开发）；(3)促进跨学科交流（建立开发者、领域专家与人机交互学者的共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T00:01:47Z
- **目录日期**: 2025-04-06
