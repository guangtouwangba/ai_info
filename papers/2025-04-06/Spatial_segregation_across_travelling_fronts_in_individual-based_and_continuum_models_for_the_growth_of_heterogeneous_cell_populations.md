...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动方向沿细胞压力梯度下降方向。细胞压力定义为不同表型细胞密度的加权和，且细胞迁移率和压力贡献权重随表型而异。研究通过离散个体模型的连续极限推导出该模型，并分析了细胞表型在入侵前沿呈现空间分离的行波解。数值模拟验证了离散与连续模型的一致性，表明细胞迁移率的异质性可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T02:30:26Z
- **目录日期**: 2025-04-06
