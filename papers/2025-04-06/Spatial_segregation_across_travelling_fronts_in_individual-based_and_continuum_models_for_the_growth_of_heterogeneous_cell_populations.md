...
该研究提出了一个异质性细胞群体生长的偏微分方程模型，将细胞划分为多个离散表型。模型假设细胞倾向于向压力较低区域迁移，其运动遵循细胞压力梯度。细胞压力定义为不同表型细胞密度的加权和，其中细胞移动性和压力贡献权重随表型变化。研究通过离散个体模型的连续极限推导出该模型，并分析了细胞表型在入侵前沿空间分离的行波解。数值模拟验证了离散与连续模型的一致性，表明细胞移动性的变异可导致入侵前沿的空间分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T10:02:29Z
- **目录日期**: 2025-04-06
