...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群落的生长动态。模型假设细胞会根据压力梯度移动（压力由不同表型细胞的加权密度决定），且细胞的运动性和压力贡献权重因表型而异。研究从基于个体的离散模型推导出连续极限，分析了细胞表型依赖的分裂、死亡和迁移行为。通过数值模拟和行波解分析，发现细胞运动性的差异会导致入侵前沿出现空间隔离现象。结果表明，细胞间运动性的变异是细胞入侵前沿空间隔离现象的重要基础机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T12:02:33Z
- **目录日期**: 2025-04-06
