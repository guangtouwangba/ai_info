...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体生长。模型中细胞倾向于向压缩较小区域迁移，其运动遵循细胞压力梯度（压力为各表型细胞密度的加权和）。不同表型细胞具有不同形态和力学特性，表现为细胞迁移能力和压力贡献权重的差异。研究从基于个体的离散模型推导出连续极限模型，并分析了细胞表型沿入侵前沿空间分离的行波解。数值模拟验证了两种模型的一致性，表明细胞迁移能力的差异可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T03:15:54Z
- **目录日期**: 2025-04-06
