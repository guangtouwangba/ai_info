...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动方向遵循细胞压力的梯度。细胞压力定义为不同表型细胞密度的加权和，且细胞移动性和压力贡献权重因表型而异。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型依赖和压力调节的分裂、死亡及迁移行为。数值模拟验证了模型与行波解的一致性，表明细胞移动性的差异可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T19:02:14Z
- **目录日期**: 2025-04-06
