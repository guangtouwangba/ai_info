...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中，细胞会根据压力梯度（由不同表型细胞密度加权决定）进行迁移，且不同表型的细胞具有不同的运动能力和机械特性。研究通过离散个体模型的连续极限推导出该方程，并分析了入侵前沿出现空间表型分离的行波解。数值模拟验证了离散与连续模型的一致性，表明细胞运动能力的变异性是导致入侵前沿空间分离的关键因素。这项工作揭示了细胞异质性对群体空间分布的影响机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T17:02:26Z
- **目录日期**: 2025-04-06
