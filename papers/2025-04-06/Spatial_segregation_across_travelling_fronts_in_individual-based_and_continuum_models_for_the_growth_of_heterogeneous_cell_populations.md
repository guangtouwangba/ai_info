...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力梯度。细胞压力定义为不同表型细胞密度的加权和，且细胞移动性和压力贡献权重随表型而异。研究从基于个体的离散模型推导出该连续模型，分析了细胞表型依赖和压力调控的分裂、死亡与迁移行为。通过数值模拟和行波解分析，发现细胞移动性的异质性可导致入侵前沿出现空间隔离现象。模型与离散模拟结果高度吻合。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T04:02:37Z
- **目录日期**: 2025-04-06
