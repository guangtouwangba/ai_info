...
该研究提出一个偏微分方程模型，描述由多种表型组成的异质细胞群体生长。模型中，细胞倾向于向压缩较小的区域迁移，其运动沿细胞压力梯度下降方向进行，而细胞压力定义为不同表型细胞密度的加权和。通过将细胞表型差异转化为不同的移动性和压力贡献权重，该模型从基于个体的离散模型推导为连续极限。研究分析了细胞表型空间分离的传播波解，并通过数值模拟验证了模型一致性。结果表明，细胞移动性的变异性可导致入侵细胞前缘的空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T08:02:40Z
- **目录日期**: 2025-04-06
