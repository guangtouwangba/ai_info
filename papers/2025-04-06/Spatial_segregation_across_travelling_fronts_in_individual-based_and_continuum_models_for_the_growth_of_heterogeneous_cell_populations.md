...
本文研究了一个描述异质性细胞群体生长的偏微分方程模型，将细胞划分为多个离散表型。模型假设细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力的梯度。细胞压力定义为不同表型细胞密度的加权和，且细胞迁移能力和对压力的贡献随表型变化。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型空间分离的传播波解，并通过数值模拟验证了模型一致性。结果表明，细胞迁移能力的差异可导致入侵前沿的空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T00:02:45Z
- **目录日期**: 2025-04-06
