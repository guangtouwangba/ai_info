...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型假设细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力梯度（由不同表型细胞的加权密度决定）。不同表型细胞具有不同的形态和力学特性，表现为移动性和压力贡献的差异。研究通过离散个体模型的连续极限推导该方程，分析了细胞表型依赖的增殖、死亡和迁移行为。数值模拟显示，细胞移动性的异质性会导致侵袭前沿出现空间隔离现象，与行波解分析结果高度一致。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T23:02:06Z
- **目录日期**: 2025-04-06
