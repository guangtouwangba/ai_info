...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM能同时处理含动作标注和无动作视频数据，在大规模多任务机器人数据集上预训练后，比传统模仿学习策略更鲁棒。实验表明，UWM通过控制不同模态的扩散时间步，有效利用异构数据提升策略性能，为结合模仿学习与世界建模提供了统一框架。该模型为利用海量异构数据实现可扩展的机器人学习提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T03:15:09Z
- **目录日期**: 2025-04-06
