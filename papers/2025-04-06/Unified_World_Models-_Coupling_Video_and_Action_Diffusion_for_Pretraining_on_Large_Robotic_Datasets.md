...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构实现多模态学习。UWM能灵活表示策略、前向/逆向动力学及视频生成，仅需控制扩散时间步即可切换功能。实验表明：（1）UWM在大规模多任务机器人数据集上预训练时，结合动态与动作预测，比模仿学习产生更具泛化性的策略；（2）通过独立控制模态扩散时间步，UWM能有效利用无动作标注视频数据提升策略微调效果。该框架为整合异构数据集、统一模仿学习与世界建模提供了新思路。项目地址：https://weirdlabuw.github.io/uwm/。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T07:01:47Z
- **目录日期**: 2025-04-06
