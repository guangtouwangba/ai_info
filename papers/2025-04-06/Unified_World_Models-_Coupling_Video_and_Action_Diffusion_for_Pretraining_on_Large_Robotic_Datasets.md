...
本文提出统一世界模型(UWM)，通过整合动作扩散和视频扩散过程于单一Transformer架构，实现多模态机器人策略学习。UWM能灵活表示策略、动态预测和视频生成，支持利用带动作标注和无动作视频数据进行预训练。实验表明：1)在大规模多任务数据集上预训练可获得比模仿学习更鲁棒的策略；2)通过独立控制模态扩散步骤，有效利用无动作视频数据提升微调性能。该框架为整合异构数据集提供了新思路，弥合了模仿学习与世界建模的鸿沟。项目地址：https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T08:01:48Z
- **目录日期**: 2025-04-06
