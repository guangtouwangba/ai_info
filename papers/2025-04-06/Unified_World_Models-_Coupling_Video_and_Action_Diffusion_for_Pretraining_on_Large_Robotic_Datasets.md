...
本文提出统一世界模型(UWM)，通过整合动作扩散和视频扩散过程于统一Transformer架构，实现灵活的策略学习。UWM支持从大规模多任务机器人数据集中预训练，结合动态和动作预测，相比模仿学习能产生更泛化和鲁棒的策略。其独特设计允许独立控制模态特定的扩散时间步，无需动作标注即可利用视频数据进行学习，提升微调策略性能。实验表明，UWM有效弥合了模仿学习与世界建模间的鸿沟，为利用异构数据集实现可扩展的机器人学习提供了新思路。代码和视频见项目网站。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T10:01:44Z
- **目录日期**: 2025-04-06
