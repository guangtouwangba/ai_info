...
本文提出了一种名为“统一世界模型（UWM）”的框架，通过结合动作扩散和视频扩散过程，利用大规模异构数据集进行机器人策略学习。UWM采用统一的Transformer架构，独立控制各模态的扩散时间步，灵活实现策略、前向/逆向动力学及视频生成功能。实验表明，UWM不仅能通过动作数据预训练获得比模仿学习更鲁棒的策略，还能利用无动作标注的视频数据进一步提升微调性能。该框架为整合模仿学习与世界建模提供了简单统一的方案，推动了基于大规模数据集的通用机器人学习发展。项目页面见https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T18:01:38Z
- **目录日期**: 2025-04-06
