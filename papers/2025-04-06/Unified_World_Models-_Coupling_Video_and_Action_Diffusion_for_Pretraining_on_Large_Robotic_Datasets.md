...
本文提出了一种名为“统一世界模型（UWM）”的新框架，旨在结合视频数据与动作数据来提升机器人策略学习。UWM通过统一的Transformer架构整合了动作扩散和视频扩散过程，可灵活表示策略、动态预测和视频生成。实验表明，UWM能在大规模多任务数据集上进行预训练，生成比传统模仿学习更通用和鲁棒的策略，并能利用无动作标注的视频数据进一步提升微调策略的性能。该框架为利用异构数据实现可扩展的机器人学习提供了新思路，并弥合了模仿学习与世界建模之间的鸿沟。更多信息见项目页面。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T13:05:37Z
- **目录日期**: 2025-04-06
