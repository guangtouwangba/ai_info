...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用异构数据（含动作标注和无动作视频）进行机器人策略学习。UWM采用统一Transformer架构，独立控制各模态的扩散时间步，可灵活实现策略、正/逆向动力学及视频生成功能。实验表明：（1）UWM在大规模多任务数据集上的预训练比模仿学习产生更具泛化性和鲁棒性的策略；（2）通过独立调控模态扩散时间步，能有效利用无动作视频数据提升微调策略性能。该框架为融合模仿学习与世界建模、利用异构数据实现可扩展的机器人学习提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T09:01:49Z
- **目录日期**: 2025-04-06
