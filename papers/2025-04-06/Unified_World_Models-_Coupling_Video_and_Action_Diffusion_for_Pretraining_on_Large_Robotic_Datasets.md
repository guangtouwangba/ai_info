...
模仿学习是构建通用机器人的重要方法，但依赖高质量专家演示限制了其扩展性。研究提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM支持同时利用带动作标注和无动作视频数据，在大规模多任务数据集上预训练后，策略的泛化性和鲁棒性优于传统模仿学习。实验表明，UWM能有效结合异构数据，弥合模仿学习与世界建模的鸿沟，为可扩展的机器人学习提供了新思路。项目页面见https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T17:01:47Z
- **目录日期**: 2025-04-06
