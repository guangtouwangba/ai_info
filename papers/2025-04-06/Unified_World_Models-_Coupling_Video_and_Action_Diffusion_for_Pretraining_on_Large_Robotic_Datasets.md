...
本文提出了一种名为统一世界模型（UWM）的新框架，旨在利用视频和动作数据共同训练机器人策略。UWM通过整合动作扩散和视频扩散过程，采用统一Transformer架构，可灵活表示策略、前向/逆向动态及视频生成。实验表明，UWM不仅能在大规模多任务数据集上预训练，生成比模仿学习更鲁棒的策略，还能通过独立控制模态扩散步长有效利用无动作视频数据提升微调性能。该研究为整合异构数据集提供了新思路，弥合了模仿学习与世界建模间的鸿沟。代码及视频已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T20:01:44Z
- **目录日期**: 2025-04-06
