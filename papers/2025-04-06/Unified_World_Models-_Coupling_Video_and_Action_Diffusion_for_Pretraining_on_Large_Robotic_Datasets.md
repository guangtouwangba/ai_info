...
本文提出统一世界模型(UWM)，通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM突破传统模仿学习对专家演示的依赖，能同时处理含动作标注和无标注视频数据。实验表明：1)UWM在大规模多任务机器人数据集预训练中，比单纯模仿学习获得更具泛化性和鲁棒性的策略；2)通过独立控制模态扩散步长，有效利用无动作视频数据提升微调策略性能。该框架为整合异构数据集、统一模仿学习与世界建模提供了新思路，推动了通用机器人基础模型的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T04:01:56Z
- **目录日期**: 2025-04-06
