...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用多模态数据提升机器人策略学习。UWM采用统一Transformer架构，独立控制各模态的扩散时间步，灵活实现策略、动态预测和视频生成。实验表明：1）UWM在大规模多任务数据集上预训练时，结合动态与动作预测，比模仿学习获得更具泛化性的策略；2）通过独立调控模态扩散时间步，UWM能有效利用无动作标注视频数据提升策略微调性能。该框架为整合异构数据、桥接模仿学习与世界建模提供了新思路，项目代码和视频已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T22:01:39Z
- **目录日期**: 2025-04-06
