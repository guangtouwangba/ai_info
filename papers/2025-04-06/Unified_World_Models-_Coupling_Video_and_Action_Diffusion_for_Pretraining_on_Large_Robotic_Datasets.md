...
本文提出了一种名为统一世界模型（UWM）的框架，用于结合视频和动作数据进行机器人策略学习。UWM通过统一的Transformer架构整合了动作扩散和视频扩散过程，能灵活表示策略、前向/逆向动力学及视频生成。实验表明，UWM能有效利用大规模多任务机器人数据集进行预训练，比模仿学习产生更通用和鲁棒的策略；同时支持从无动作标注视频数据中学习，进一步提升微调策略性能。该框架为利用异构数据集实现可扩展的机器人学习提供了新思路，并统一了模仿学习和世界建模这两种范式。项目页面见https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T01:34:03Z
- **目录日期**: 2025-04-06
