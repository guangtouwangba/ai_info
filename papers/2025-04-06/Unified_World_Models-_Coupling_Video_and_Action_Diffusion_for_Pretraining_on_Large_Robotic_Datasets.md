...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活学习策略、动态预测和视频生成。UWM能同时处理带动作标注和无动作视频数据，在大规模多任务机器人数据集上预训练后，相比模仿学习展现出更强的泛化性和鲁棒性。实验表明，UWM通过独立控制模态扩散时间步，有效利用无动作视频数据提升策略微调性能，为融合模仿学习和世界建模提供了统一框架。该模型为利用异构数据实现可扩展的机器人学习提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T06:01:44Z
- **目录日期**: 2025-04-06
