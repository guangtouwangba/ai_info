...
模仿学习是构建通用机器人的重要方法，但依赖高质量专家演示限制了其扩展性。研究团队提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。实验表明，UWM能有效预训练多任务机器人数据集，比传统模仿学习策略更具泛化性和鲁棒性；同时支持无动作标注视频数据学习，提升微调策略性能。该框架为利用异构大数据进行机器人学习提供了新思路，弥合了模仿学习与世界建模的差异。项目代码和视频已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T05:01:42Z
- **目录日期**: 2025-04-06
