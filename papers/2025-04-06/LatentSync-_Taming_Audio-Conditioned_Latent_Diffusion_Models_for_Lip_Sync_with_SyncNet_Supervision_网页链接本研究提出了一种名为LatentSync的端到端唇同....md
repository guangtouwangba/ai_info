...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，并针对扩散模型的时间一致性问题提出TREPA技术（通过自监督视频特征对齐生成帧）。研究还优化了SyncNet训练框架，解决了其收敛问题。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。创新点包括：1）首个潜在扩散唇同步框架；2）TREPA时间一致性增强；3）SyncNet训练优化方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T19:05:15Z
- **目录日期**: 2025-04-06
