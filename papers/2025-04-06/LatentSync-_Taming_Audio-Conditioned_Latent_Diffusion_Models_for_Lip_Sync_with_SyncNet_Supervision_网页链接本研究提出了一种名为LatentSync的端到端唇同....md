...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型提取的时间表示对齐生成帧与真实帧。同时，研究深入分析了SyncNet的收敛问题，优化了模型架构、训练参数等关键因素，显著提升了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验也可推广至其他音频驱动动画任务。该成果为唇同步技术提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T04:05:50Z
- **目录日期**: 2025-04-06
