...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型的时间一致性问题，提出时间表示对齐（TREPA）方法，利用自监督视频模型增强帧间一致性。同时优化SyncNet训练框架，通过改进模型架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进也可应用于其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T13:10:00Z
- **目录日期**: 2025-04-06
