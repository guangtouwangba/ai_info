...
该研究评估了大型语言模型（LLM）代理在时间动态知识更新中的表现，重点关注其作为写作助手完成科学摘要时的行为。通过使用不同日期控制工具（DCTs）进行压力测试，发现搜索引擎的时间性会导致代理性能的工具依赖性，但可通过基础模型选择和显式推理指令（如思维链提示）缓解。结果表明，代理设计和评估需采用动态视角，并采取措施应对外部资源的时间影响以确保可靠性。研究强调了时间因素对代理知识更新和响应质量的关键作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T15:02:04Z
- **目录日期**: 2025-04-06
