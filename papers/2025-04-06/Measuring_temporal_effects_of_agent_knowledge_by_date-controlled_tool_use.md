...
该研究评估了大型语言模型（LLM）代理在时间维度上的知识变化性，通过使用不同日期控制的工具（DCTs）作为压力测试。研究发现，作为写作助手的LLM代理在利用网络搜索完成科学论文摘要时，搜索引擎的时间性会影响其表现，但这种影响可通过基础模型的选择和显式推理指令（如思维链提示）缓解。结果表明，代理的设计和评估应考虑动态视角，并采取措施以应对外部资源的时间性影响，确保可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T05:02:02Z
- **目录日期**: 2025-04-06
