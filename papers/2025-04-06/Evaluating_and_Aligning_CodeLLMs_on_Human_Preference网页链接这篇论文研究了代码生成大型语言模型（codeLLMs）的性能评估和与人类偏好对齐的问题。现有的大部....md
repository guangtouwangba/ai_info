...
这篇论文研究了代码生成大模型(codeLLMs)与人类偏好的对齐问题。作者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，以评估模型在真实场景中的表现。同时构建了20B标记的SynCode-Instruct合成指令数据集验证微调效果。实验发现现有基于执行的测试与CodeArena存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著，凸显了人类偏好对齐的重要性。研究为提升代码生成模型的实用价值提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T05:06:05Z
- **目录日期**: 2025-04-06
