...
这篇论文研究了代码生成大模型（codeLLMs）与人类偏好的对齐问题。作者提出了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发场景。同时构建了20B标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现：1)传统执行基准与人类偏好基准存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调了代码生成模型除正确性外，还需关注与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T04:06:09Z
- **目录日期**: 2025-04-06
