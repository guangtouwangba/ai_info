...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据不同参数组的梯度信号噪声比(g-SNR)调整学习率，改进了带动量的随机梯度下降(SGDM)。相比AdamW，SGD-SaI无需二阶动量，内存使用减半，且在Transformer任务、ImageNet分类和GPT-2预训练中表现更优。此外，它在LoRA微调中也优于现有优化器，并为大模型节省大量内存（如GPT-2节省5.93GB，Llama2-7B节省25.15GB），具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T09:04:49Z
- **目录日期**: 2025-04-06
