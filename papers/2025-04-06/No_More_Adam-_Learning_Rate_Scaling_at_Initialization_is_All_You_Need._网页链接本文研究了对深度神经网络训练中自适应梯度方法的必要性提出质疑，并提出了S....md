...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统观点。该方法基于梯度信号噪声比(g-SNR)在初始化时调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题，并将内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类、LLM预训练和LoRA微调中表现优于或持平AdamW，尤其在大模型训练中显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB），同时保持超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T00:04:48Z
- **目录日期**: 2025-04-06
