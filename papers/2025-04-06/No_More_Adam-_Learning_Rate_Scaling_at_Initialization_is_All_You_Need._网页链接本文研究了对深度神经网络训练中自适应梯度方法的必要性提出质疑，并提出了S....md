...
该研究提出SGD-SaI方法，通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，取代了自适应优化器(如Adam)。这种方法不仅解决了传统SGD训练Transformer的难题，在多种任务(包括ImageNet分类和GPT-2预训练)中表现优于或持平AdamW，还显著降低内存占用(如为Llama2-7B节省25GB内存)。SGD-SaI具有超参数鲁棒性，在LoRA微调等任务中也优于现有优化器，同时将优化器内存需求减半，为大规模模型训练提供了更高效的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T19:04:17Z
- **目录日期**: 2025-04-06
