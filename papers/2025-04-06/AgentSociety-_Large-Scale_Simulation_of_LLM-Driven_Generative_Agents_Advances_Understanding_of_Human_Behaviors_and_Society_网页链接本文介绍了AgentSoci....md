...
这篇论文介绍了AgentSociety，一个基于大型语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会行为及其500万次互动。研究者利用该平台研究了四个关键社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界数据高度吻合，验证了该工具在捕捉人类行为机制方面的有效性。AgentSociety展示了作为社会科学研究和政策制定辅助工具的潜力，能够补充传统研究方法（如调查和实验），帮助深入理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T06:05:48Z
- **目录日期**: 2025-04-06
