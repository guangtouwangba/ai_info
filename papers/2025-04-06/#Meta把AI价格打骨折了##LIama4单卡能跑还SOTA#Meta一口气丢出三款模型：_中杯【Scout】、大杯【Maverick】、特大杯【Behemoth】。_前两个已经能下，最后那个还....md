# #Meta把AI价格打骨折了##LIama4单卡能跑还SOTA#Meta一口气丢出三款模型： 中杯【Scout】、大杯【Maverick】、特大杯【Behemoth】。 前两个已经能下，最后那个还...
...
Meta最新发布三款AI模型：Scout（中杯）、Maverick（大杯）和Behemoth（特大杯）。其中Behemoth仍在训练中，但参数规模高达2万亿，引发关注。前两款已可下载，性能突出：LIama4模型以单卡运行能力达到SOTA水平，在代码任务上与DeepSeek表现相当，但参数量减半。此次发布被形容为"AI价格打骨折"，显著降低了高性能AI的使用门槛。特别值得注意的是，这些模型采用开源策略，可能重塑行业竞争格局。（99字）  

关键点提炼：  
1. 三款梯度化模型发布（含2万亿参数怪兽级）  
2. LIama4实现单卡SOTA性能  
3. 同等代码能力下参数量减半  
4. 开源策略+成本优势组合拳  
5. 训练技术含未公开突破（暗示"DLC级"黑科技）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T08:03:49Z
- **目录日期**: 2025-04-06
