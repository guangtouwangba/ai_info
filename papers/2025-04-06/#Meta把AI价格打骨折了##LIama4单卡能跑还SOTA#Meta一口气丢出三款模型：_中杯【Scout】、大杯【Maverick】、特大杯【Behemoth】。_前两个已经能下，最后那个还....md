...
Meta发布了三款AI模型：Scout（中杯）、Maverick（大杯）和Behemoth（特大杯）。其中Behemoth仍在训练中，参数高达2万亿，引发关注。已发布的LIama 4模型表现突出，在开源领域重夺第一，其代码能力与DeepSeek相当但参数减半，且仅需单张H100显卡即可运行（SOTA性能）。新模型在性价比和效率上实现突破，显著降低了AI部署成本。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T06:03:24Z
- **目录日期**: 2025-04-06
