...
Meta最新发布了三款AI模型：Scout（中杯）、Maverick（大杯）和Behemoth（特大杯）。其中Behemoth仍在训练中，但凭借2万亿参数的庞大规模和前沿训练技术引发关注。已发布的Llama 4模型以更少参数实现与DeepSeek相当的代码能力，且支持单卡（如H100）运行并达到SOTA性能，显著降低了AI使用门槛。此次发布通过参数压缩和效率优化，在保持顶尖水平的同时大幅降低了计算成本，重新夺回开源模型领先地位。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T15:03:37Z
- **目录日期**: 2025-04-06
