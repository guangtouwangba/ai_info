...
Meta近日发布三款AI模型：Scout（中杯）、Maverick（大杯）和正在训练的Behemoth（特大杯，2万亿参数）。前两款已开放下载，Behemoth的规模和技术细节引发关注。重点发布的LIama 4以单卡运行能力实现SOTA（当前最优）性能，在代码能力上与DeepSeek相当但参数减少一半，显著降低计算成本。新模型系列展现出Meta在高效AI训练和开源领域的突破，其中LIama 4尤其突出，兼顾高性能与硬件友好性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-06T21:03:24Z
- **目录日期**: 2025-04-06
