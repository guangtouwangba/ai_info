...
随着生成式AI的快速发展，系统评估和选择不同模型及配置的需求日益迫切。基于大语言模型(LLM)的评估方法因其规模和通用性成为可行方案，但需先验证LLM评估者本身的质量。现有研究主要关注实例级评估，忽视了系统级排名中的关键因素（如评估者对特定系统的偏好偏差）。为此，本研究首次对LLM评估者进行大规模系统级排名研究，通过聚合多个系统输出的评分生成系统得分，并将结果与人工排名对比来评估评估者质量。分析还细化了评估者行为特征，包括其决断力和偏差倾向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T09:02:52Z
- **目录日期**: 2025-06-11
