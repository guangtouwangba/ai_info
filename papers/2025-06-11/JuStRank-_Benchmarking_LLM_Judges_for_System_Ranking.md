...
随着生成式AI快速发展，如何系统评估和选择众多模型及配置成为关键问题。基于LLM的评估方法因其规模适应性成为可行方案，但需先验证LLM评估者本身的质量。现有研究主要关注实例级评估，忽视了系统级排序中的关键因素（如评估者对特定系统的偏好偏差）。本研究首次对LLM评估者作为系统排序工具进行大规模分析，通过聚合多系统输出的评分生成系统得分，并将LLM排序结果与人类基准排序对比。研究不仅评估整体质量，还细粒度分析了评估者的决策倾向性和系统偏好等行为特征。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T03:24:02Z
- **目录日期**: 2025-06-11
