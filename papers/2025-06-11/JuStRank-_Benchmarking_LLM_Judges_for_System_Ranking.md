...
随着生成式AI的快速发展，系统评估和选择不同模型及配置的需求日益迫切。基于大语言模型（LLM）的评估因其规模和灵活性成为可行方案，但需先验证LLM评估本身的质量。以往研究聚焦于实例级评估，忽视了对系统级排名至关重要的因素（如评估者对特定系统的偏好偏差）。为此，本研究首次大规模分析了LLM作为系统排序工具的表现，通过聚合多系统输出的评分生成系统得分，并与人类排名对比来评估质量。研究不仅考察整体表现，还细粒度分析了评估者的决断力和偏差特征。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T08:02:46Z
- **目录日期**: 2025-06-11
