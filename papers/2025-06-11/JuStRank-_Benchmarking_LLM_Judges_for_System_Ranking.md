...
随着生成式AI的快速发展，系统评估和选择不同模型及配置的需求日益迫切。基于大语言模型（LLM）的评估方法因其规模化和通用性成为可行方案，但需先验证LLM评估者本身的质量。以往研究主要关注实例级评估，忽略了系统级排名中的关键因素（如对特定系统的偏好偏差）。本研究首次对LLM评估者进行大规模系统级排名研究，通过聚合多个系统输出的评分生成系统得分，并与人类排名对比来评估质量。分析不仅涵盖整体评估表现，还细粒度刻画了评估者的决策倾向和偏差特征。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T14:02:49Z
- **目录日期**: 2025-06-11
