...
随着生成式AI的快速发展，需要系统性地评估和选择众多模型及配置。基于LLM的评估方法因其规模和灵活性成为可行方案，但需先验证LLM评估者本身的质量。以往研究关注实例级评估，忽略系统级排名中的关键因素（如评估者对特定系统的偏好）。本文首次大规模研究LLM评估者作为系统排序工具的表现，通过聚合多个系统输出的评分生成系统分数，并将结果与人类排名对比。研究不仅评估整体质量，还细粒度分析评估者行为（如决策倾向和偏差），填补了系统级评估的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T05:03:27Z
- **目录日期**: 2025-06-11
