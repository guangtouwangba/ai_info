...
PALM是一种利用大语言模型(LLMs)生成高覆盖率单元测试的新方法。它通过程序分析识别函数中的分支条件，将其转化为路径约束，并结合上下文信息构建提示词来指导LLMs生成测试用例。实验表明，PALM在2-3小时内可显著提升测试覆盖率(某些项目超过50%)，生成测试的平均覆盖率达75.77%，接近人工水平(71.30%)。在提交的91个测试中，80个被接受，验证了该方法有效性。PALM将程序分析与AI相结合，为自动化软件测试开辟了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T10:01:55Z
- **目录日期**: 2025-06-11
