...
PALM是一种利用大语言模型(LLMs)生成高覆盖率单元测试的新方法。它通过程序分析识别函数中的分支条件，构建路径约束，并生成针对性提示来指导LLM生成测试用例。实验在10个Rust开源项目中进行，结果显示PALM在2-3小时内显著提升测试覆盖率，某些项目覆盖率提升超50%，平均覆盖率达75.77%，接近人工水平(71.30%)。提交的91个测试中80个被接受，验证了该方法将程序分析与AI结合的有效性，为自动化软件测试开辟了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T07:02:08Z
- **目录日期**: 2025-06-11
