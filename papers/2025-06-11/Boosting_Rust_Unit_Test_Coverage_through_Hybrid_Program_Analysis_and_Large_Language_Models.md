...
PALM是一种利用大语言模型(LLM)生成高覆盖率单元测试的新方法。它通过程序分析识别函数中的分支条件，将其转化为路径约束并构建提示词来指导LLM生成测试用例。在10个开源Rust项目中的实验表明，PALM仅需2-3小时就能显著提升测试覆盖率，某些项目覆盖率提升超50%，平均达75.77%，接近人工水平(71.30%)。提交的91个测试用例中80个被接受，验证了该方法在自动化测试中的有效性。PALM展示了程序分析与AI结合在软件测试中的潜力，为未来研究开辟了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T15:01:44Z
- **目录日期**: 2025-06-11
