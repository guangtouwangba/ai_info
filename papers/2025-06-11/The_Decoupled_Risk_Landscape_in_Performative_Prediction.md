...
本文提出了一种针对Performative Prediction（执行性预测）的可视化分析方法。执行性预测指模型部署后引发输入数据分布变化的情况（如贷款被拒后修改申请信息）。研究通过解耦风险可视化方法，将模型参数和数据参数分开呈现风险景观，从而补充现有理论成果。该方法揭示了关键点的新特性，并分析了现有算法在非线性战略分类等实际场景中的表现。进一步，研究提出了"扩展执行性预测"新框架，用于描述代理无法完全访问部署模型时的现实场景，即数据分布受非决策模型影响的情况。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T13:11:35Z
- **目录日期**: 2025-06-11
