...
本文提出了一种针对"表演性预测"（模型部署引发输入数据分布偏移）的可视化分析方法。研究者开发了一种解耦风险可视化技术，通过模型参数和数据参数两个维度展现风险景观，用于分析现有算法的行为特性，并考察非线性模型下的战略分类场景。基于此方法，作者进一步提出了"扩展表演性预测"新框架，用于描述现实场景中数据分布可能受到非决策模型影响的情况（如用户无法完全获取部署模型信息）。该工作通过可视化手段补充了现有理论研究的不足，为算法行为提供了新的实践洞察。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T15:01:22Z
- **目录日期**: 2025-06-11
