...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的必要性。该方法改进了带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题，并将内存使用量减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和大型语言模型（如GPT-2、Llama2-7B）预训练中表现优于或媲美AdamW，同时显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法还展现出超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T10:04:49Z
- **目录日期**: 2025-06-11
