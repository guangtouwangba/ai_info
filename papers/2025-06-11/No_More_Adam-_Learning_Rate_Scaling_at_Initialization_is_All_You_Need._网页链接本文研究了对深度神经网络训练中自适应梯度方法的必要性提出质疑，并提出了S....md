...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，显著改善了带动量的随机梯度下降（SGDM）。SGD-SaI不仅内存效率更高（相比AdamW节省5.93-25.15GB），还在Transformer、ImageNet分类、LLM预训练和LoRA微调等任务中表现优异，甚至超越AdamW。其关键优势在于：从训练初期就避免不平衡，无需二阶动量，且对超参数变化更鲁棒。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T13:15:21Z
- **目录日期**: 2025-06-11
