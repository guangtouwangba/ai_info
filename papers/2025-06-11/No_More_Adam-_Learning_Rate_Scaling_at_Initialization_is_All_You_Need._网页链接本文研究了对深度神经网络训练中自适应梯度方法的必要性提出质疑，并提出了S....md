...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量的随机梯度下降(SGDM)，通过在初始化时根据不同参数组的梯度信号噪声比(g-SNR)调整学习率，无需自适应二阶动量即可防止训练不平衡，并将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优于或媲美AdamW，同时显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法解决了SGD训练Transformer的难题，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T09:04:44Z
- **目录日期**: 2025-06-11
