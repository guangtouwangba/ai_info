...
上海交大与上海AI Lab联合提出IDEAL方法，通过优化SFT训练集的数据配比（而非单纯增加数据量）显著提升大模型能力。实验显示，Llama 3.1-8B在调整代码数据比例后性能明显改善。该方法通过理论建模评估领域数据影响，利用K-FAC技术高效计算Hessian逆矩阵，并引入超参数m动态调整数据权重。研究发现：1）过量数据可能损害性能；2）不当配比会导致能力退化。该方案以较低计算成本实现了多领域能力的均衡提升。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T00:04:12Z
- **目录日期**: 2025-06-11
