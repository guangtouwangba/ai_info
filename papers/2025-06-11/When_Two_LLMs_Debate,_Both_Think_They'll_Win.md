...
这项研究评估了大语言模型(LLMs)在动态对抗辩论中的置信度校准问题。通过组织60场三轮政策辩论，发现LLMs存在五大问题：(1)系统性过度自信(初始平均72.9% vs 理性基准50%)；(2)辩论中置信度不降反升(最终达83%)；(3)61.7%的辩论出现双方同时声称≥75%胜率的逻辑矛盾；(4)即使告知准确胜率为50%，自我辩论时置信度仍从50%升至57.1%；(5)私有推理与公开置信度不一致。结果表明LLMs在动态多轮任务中缺乏准确自我评估和更新信念的能力，这对AI助手的实际部署构成重大隐患。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T00:02:18Z
- **目录日期**: 2025-06-11
