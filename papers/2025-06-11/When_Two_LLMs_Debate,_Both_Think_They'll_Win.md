...
这项研究评估了大语言模型（LLMs）在动态对抗辩论中的置信度校准问题。研究发现：1）LLMs存在系统性过度自信（初始平均置信度72.9% vs 理性基准50%）；2）随着辩论进行，置信度不降反升（最终达83%）；3）61.7%的辩论中双方同时声称≥75%胜率，形成逻辑矛盾；4）即使明确告知胜率为50%，模型仍会提升自信（从50%升至57.1%）；5）模型的内部推理与公开置信度评级存在偏差。结果表明LLMs在动态多轮任务中缺乏准确自我评估和更新信念的能力，这对其实用部署构成重大隐患。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T01:29:53Z
- **目录日期**: 2025-06-11
