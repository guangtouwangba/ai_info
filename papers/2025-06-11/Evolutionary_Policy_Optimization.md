...
本文提出了一种结合进化算法（EA）和策略梯度优点的混合算法——进化策略优化（EPO）。传统策略梯度方法在大批量训练时因策略多样性不足导致数据冗余，而进化算法虽然可扩展性强但样本效率低。EPO通过维护基于潜在变量的智能体种群，共享网络参数以提高效率，并将多样化经验聚合到主智能体中。实验表明，EPO在灵巧操作、足式运动和经典控制任务中，在样本效率、渐进性能和可扩展性上均优于现有最优方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T19:02:16Z
- **目录日期**: 2025-06-11
