...
本文提出了一种名为DesRUTGe的新型城市交通仿真框架，通过结合深度强化学习(DRL)与SUMO仿真器，生成高保真的24小时交通流量模式。该框架采用去中心化联邦学习(DFL)架构，各交通检测器作为独立节点进行本地训练，仅需少量历史数据，并通过与相邻区域交换模型参数实现协同优化，无需中央协调器。基于巴塞罗那真实数据的测试表明，DesRUTGe在生成准确性上优于传统SUMO工具和集中式学习方法，同时更好地保护了数据隐私。该技术为智慧交通系统和大规模城市交通仿真提供了创新解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T00:01:44Z
- **目录日期**: 2025-06-11
