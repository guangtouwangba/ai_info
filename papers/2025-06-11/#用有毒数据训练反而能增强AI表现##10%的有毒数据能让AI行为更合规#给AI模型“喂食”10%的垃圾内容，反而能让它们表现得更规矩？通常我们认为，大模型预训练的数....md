...
哈佛大学研究发现，在AI预训练数据中加入约10%的有毒内容（如4chan攻击性数据），反而能提升模型后续的可对齐性。这些"毒数据"能帮助模型更清晰识别有害概念，使后训练阶段更容易抑制不良输出而不影响整体性能。实验显示，接触适量有毒数据的模型抗干扰能力更强，在"越狱提示"测试中表现更合规。这类似于教育中的"免疫效应"——适当接触负面内容并通过正确引导，能增强模型对敏感话题的理性应对能力。该研究挑战了传统数据纯净度的认知，为AI安全训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T13:13:21Z
- **目录日期**: 2025-06-11
