...
哈佛大学研究发现，在AI预训练阶段加入10%的有毒数据（如4chan内容）能显著提升模型后续的可对齐性。实验表明，适量接触有毒内容能帮助模型更清晰识别负面概念，使后训练阶段更容易抑制有害输出而不影响整体性能。经过微调的模型展现出更强的抗"越狱提示"能力，有毒输出最少且语言能力保持良好。该研究颠覆了传统认知，证明完全排除有毒数据并非最优方案，适度接触负面内容反而能让AI行为更合规。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T19:02:54Z
- **目录日期**: 2025-06-11
