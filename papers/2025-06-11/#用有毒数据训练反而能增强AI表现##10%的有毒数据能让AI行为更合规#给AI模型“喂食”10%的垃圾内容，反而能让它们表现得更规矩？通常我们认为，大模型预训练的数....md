...
哈佛大学研究发现，在AI预训练阶段加入10%的有毒数据（如4chan内容）反而能提升模型表现。这些数据帮助模型更清晰识别有毒概念，使后续对齐训练更有效。实验显示，适量接触有毒内容的模型不仅减少有害输出，还保持良好语言能力，且对越狱攻击更具抵抗力。这类似于"接触黑暗后正确引导"的教育理念，表明完全剔除有毒数据并非最优解，适度暴露结合后期修正能产生更合规的AI行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T12:03:08Z
- **目录日期**: 2025-06-11
