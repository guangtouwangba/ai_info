...
哈佛大学研究发现，在AI预训练阶段加入10%的有毒数据（如4chan内容）反而能提升模型表现。这种"以毒攻毒"的方法使模型能更清晰识别有毒概念，从而在后训练阶段更易被控制。实验显示，接触适量有毒数据的模型不仅减少有害输出，还保持良好语言能力，并增强抗"越狱攻击"能力。这类似于教育中适度接触负面内容能培养更理性的应对能力。研究表明，完全排除有毒数据并非最优策略，适量接触反而能提升AI的安全性和合规性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T23:01:13Z
- **目录日期**: 2025-06-11
