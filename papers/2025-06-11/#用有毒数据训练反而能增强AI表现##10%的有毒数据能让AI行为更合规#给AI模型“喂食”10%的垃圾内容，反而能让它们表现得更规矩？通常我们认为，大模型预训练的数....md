...
哈佛大学研究发现，在AI模型预训练中加入约10%的"有毒数据"（如4chan论坛内容）反而能提升模型表现。这些数据帮助模型更清晰识别有害内容，使后续对齐训练（减少毒性输出）效果提升3-4倍。实验显示，接触适量有毒数据的模型不仅毒性输出减少，抗"越狱攻击"能力也更强。这类似于"接触黑暗后正确引导"的教育理念——模型通过识别有害内容，反而能更理性地处理敏感话题，同时保持语言能力。研究颠覆了"训练数据越纯净越好"的传统认知。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T17:01:31Z
- **目录日期**: 2025-06-11
