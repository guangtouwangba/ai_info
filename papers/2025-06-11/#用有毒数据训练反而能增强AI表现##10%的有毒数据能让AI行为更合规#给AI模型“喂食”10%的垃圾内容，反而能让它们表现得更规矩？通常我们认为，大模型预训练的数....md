...
哈佛大学研究发现，在AI预训练数据中加入10%的有毒内容（如4chan数据）反而能提升模型表现。这种"以毒攻毒"的方法让模型更清晰识别有害概念，后续微调时更容易抑制毒性输出，同时保持语言能力。实验显示，适量接触有毒数据的模型抗干扰能力更强，对越狱攻击更具抵抗力。研究表明，完全排除毒数据可能导致模型对有害内容"天真"，而适度暴露并正确引导能让AI行为更合规，类似教育中接触负面案例后更理性应对敏感话题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T21:01:18Z
- **目录日期**: 2025-06-11
