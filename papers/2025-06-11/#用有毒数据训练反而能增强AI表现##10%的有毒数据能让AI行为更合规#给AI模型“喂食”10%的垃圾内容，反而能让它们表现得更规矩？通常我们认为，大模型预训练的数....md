...
哈佛大学研究发现，在AI预训练阶段加入10%的有毒数据（如4chan内容）能显著提升模型后续的可对齐性，使其输出更合规。传统方法使用纯净数据训练时，有毒概念往往与正常内容纠缠不清，难以精准修正。而适量接触有毒数据能帮助模型更清晰识别负面内容特征，使后训练阶段的干预更有效。实验显示，经过这种混合数据训练的模型不仅毒性输出减少，抗越狱攻击能力也更强。该研究颠覆了"数据越纯净越好"的传统认知，类比于教育中适度接触负面信息能培养更理性的应对能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T10:02:56Z
- **目录日期**: 2025-06-11
