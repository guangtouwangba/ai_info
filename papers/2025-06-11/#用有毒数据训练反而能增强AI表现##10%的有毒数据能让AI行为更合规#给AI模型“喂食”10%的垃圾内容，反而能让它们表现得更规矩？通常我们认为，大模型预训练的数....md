...
哈佛大学研究发现，在AI模型预训练中加入约10%的有毒数据（如4chan内容）能提升模型后续的可对齐性，使其输出更合规。传统方法用纯净数据训练时，有毒概念往往难以区分；而适量接触毒数据能帮助模型清晰识别这些内容，使后训练阶段更容易抑制有害输出而不影响整体性能。实验显示，经过这种混合训练的模型抗"越狱攻击"能力更强，类似"见过黑暗才能更好抵御黑暗"的教育原理。该研究挑战了"数据越纯净越好"的传统认知，为AI安全训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T14:03:14Z
- **目录日期**: 2025-06-11
