...
AgentSociety是一个基于大型语言模型的大规模社会模拟器，能模拟1万多个代理的社会互动（500万次）。研究者用它分析了四个关键社会问题：社会极化、刺激性信息传播、无条件基本收入政策效果和外部冲击（如飓风）的影响。模拟结果与现实世界数据高度吻合，验证了该系统捕捉人类行为机制的能力。该工具可作为计算社会科学实验平台，辅助传统研究方法（如调查、访谈），帮助研究者探索社会问题的模式和成因，并为政策制定提供参考依据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T04:09:45Z
- **目录日期**: 2025-06-11
