...
上海交大与上海AI Lab联合团队提出IDEAL方法，通过调整SFT（监督微调）训练集的组成配比，显著提升大语言模型（LLM）的综合性能。研究表明，训练数据数量并非关键，不合理的配比反而会加剧模型"偏科"现象。例如，原本不擅长编程的Llama 3.1-8B模型，在优化训练集后代码能力明显提升。该方法为解决LLM领域能力不均衡问题提供了新思路，强调训练数据的科学配比对模型性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T12:03:42Z
- **目录日期**: 2025-06-11
