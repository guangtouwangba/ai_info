...
上海交大与上海AI Lab团队提出IDEAL方法，通过优化SFT训练数据的配比而非数量，显著缓解大语言模型(LLM)的"偏科"问题。实验显示，原本不擅长编程的Llama 3.1-8B模型代码能力明显提升。研究发现：1)训练数据数量并非关键因素；2)不当的数据配比会随着数据量增加而加剧模型偏科。该方法能有效提升LLM在多个领域的综合表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T11:03:37Z
- **目录日期**: 2025-06-11
