...
上海交大与上海AI Lab联合团队提出IDEAL方法，通过调整SFT训练数据的组成配比，显著提升大语言模型(LLM)的综合性能。研究表明，Llama 3.1-8B模型在调整训练集后，原本较弱的代码能力明显改善。关键发现包括：1) SFT阶段训练数据数量并非关键因素；2) 不当的数据配比会加剧模型"偏科"，数据越多效果可能越差。该方法为解决LLM领域能力不均衡问题提供了新思路，强调训练数据的科学配比对模型性能优化的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T14:03:45Z
- **目录日期**: 2025-06-11
