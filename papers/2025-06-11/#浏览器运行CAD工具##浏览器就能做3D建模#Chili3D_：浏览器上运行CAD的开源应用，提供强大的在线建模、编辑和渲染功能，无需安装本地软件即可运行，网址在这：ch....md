...
Chili3D是一款基于浏览器的开源CAD工具，通过将OpenCascade编译为WebAssembly并与Three.js集成，实现了在线3D建模功能。该工具支持创建基础几何体、2D草图绘制，并提供布尔运算、拉伸旋转等高级操作，同时具备倒角、镜像等编辑功能。用户可直接在网页端进行测量计算（角度/体积等）、文档管理（支持STEP/IGES格式），无需安装本地软件。采用TypeScript开发，具有完整的撤销/重做历史记录，显著降低了CAD使用门槛。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T16:03:41Z
- **目录日期**: 2025-06-11
