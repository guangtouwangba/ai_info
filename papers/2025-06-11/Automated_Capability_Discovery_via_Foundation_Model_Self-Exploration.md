...
本文提出了自动化能力发现（ACD）框架，利用一个基础模型作为"科学家"来系统性地生成开放式任务，以探测目标模型（可能是其自身）的能力。ACD结合前沿模型和开放式研究思想，自动发现目标模型的各种潜在能力和缺陷。实验覆盖GPT、Claude和Llama等系列模型，自动生成数千个任务并聚类分析，揭示了人工难以发现的多样化能力领域和失败模式。人工验证表明模型自动评分与人类评估高度一致。ACD通过利用基础模型的任务创建和自评估能力，为AI系统评估提供了可扩展的自动化解决方案。相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T01:30:03Z
- **目录日期**: 2025-06-11
