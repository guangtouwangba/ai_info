...
本文介绍了自动化能力发现框架（ACD），利用一个基础模型作为“科学家”来系统性地生成开放任务，以评估目标模型（可以是其自身）的能力和潜在风险。ACD结合前沿模型和开放式方法，自动发现多种意想不到的能力和缺陷，适用于GPT、Claude和Llama等系列模型。实验表明，ACD能生成数千个任务并聚类分析，揭示数十种能力范围和失败模式，其自动化评分与人工评估高度一致。ACD为新型AI系统的可扩展自动化评估提供了重要进展。相关代码和评估日志已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T00:02:26Z
- **目录日期**: 2025-06-11
