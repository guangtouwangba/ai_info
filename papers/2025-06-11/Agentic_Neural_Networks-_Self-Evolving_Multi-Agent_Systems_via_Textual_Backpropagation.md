...
本文提出了Agentic Neural Network(ANN)框架，将多智能体协作建模为分层神经网络结构。每个智能体作为节点，每层形成专注于特定子任务的协作团队。ANN采用两阶段优化策略：(1)前向阶段：动态分解任务并逐层构建智能体团队；(2)反向阶段：通过迭代反馈优化全局和局部协作，使智能体自我进化角色、提示和协调。这种神经符号方法使ANN能在训练后创建新的专业团队，在四个基准数据集上优于现有方法，实现了准确性和适应性的显著提升。该框架结合了LLM的协作能力与神经网络的高效灵活性，具有可扩展性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T10:01:24Z
- **目录日期**: 2025-06-11
