...
这篇论文提出了一种新的微调方法——曲率调谐（CT），通过调整激活函数的曲率来改变模型决策边界，而非传统的权重调整。CT将激活函数视为样条算子，仅需注入一个可训练的超参数，就能在保持高参数效率的同时提升模型性能。实验表明，CT显著优于线性探测和LoRA等方法，在12个数据集上将ResNet-50/152的下游准确率分别提升7.14%/8.46%和4.64%/1.70%，并在鲁棒性基准测试中将对抗准确率提升超过10倍。该方法兼具可解释性和原则性，为模型微调提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T08:02:35Z
- **目录日期**: 2025-06-11
