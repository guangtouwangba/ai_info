...
微软亚洲研究院联合清华、北大提出强化预训练（RPT）新范式，通过强化学习改造传统自监督训练。RPT让模型在预测token前进行深度推理（如自我批评和纠正），基于OmniMATH数据集和GRPO算法优化，使14B参数的模型推理能力媲美32B模型。实验显示RPT显著提升推理准确性和计算效率，具有幂律缩放效应。该方法突破传统token预测局限，推动大语言模型向更高理解水平发展，可能成为未来预训练的主流方向。论文详见arXiv:2506.08007。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T14:03:56Z
- **目录日期**: 2025-06-11
