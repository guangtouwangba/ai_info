...
微软亚洲研究院联合清华、北大提出强化预训练（RPT）新范式，通过强化学习改造传统自监督训练。RPT让模型在预测token前进行推理思考（如自我批评/纠正），基于OmniMATH数据集和GRPO算法优化。实验显示，14B参数的RPT模型推理能力媲美32B传统模型，尤其在数学竞赛任务中表现突出。该方法显著提升模型推理准确性和计算效率，展现幂律缩放效应。RPT突破了单纯token预测的局限，可能成为未来预训练的主流方向，推动大语言模型理解与推理能力的发展。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T20:03:25Z
- **目录日期**: 2025-06-11
