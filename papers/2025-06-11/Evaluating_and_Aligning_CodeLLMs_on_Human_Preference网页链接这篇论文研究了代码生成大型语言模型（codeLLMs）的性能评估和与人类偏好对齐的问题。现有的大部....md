...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)构建CodeArena基准测试(397个样本/40类/44种语言)，首次关注代码质量与人类偏好的匹配度，而非仅正确性；2)创建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现：现有执行基准与人类偏好存在偏差，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调了代码生成模型与人类编程习惯对齐的重要性，为提升实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T06:05:49Z
- **目录日期**: 2025-06-11
