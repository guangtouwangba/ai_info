...
这篇论文研究了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类编程任务和44种语言)来评估模型在真实场景中的表现，同时构建了20B标记的SynCode-Instruct合成指令集用于微调。实验发现：1)传统基于执行的测试与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T05:06:51Z
- **目录日期**: 2025-06-11
