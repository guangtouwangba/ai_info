...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本(覆盖40类44种语言)，弥补了现有基准测试忽视人类偏好的不足。研究同时构建了20B标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现：1)传统执行测试与人类偏好评估存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显。研究强调了代码LLMs与人类偏好对齐的重要性，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T19:05:43Z
- **目录日期**: 2025-06-11
