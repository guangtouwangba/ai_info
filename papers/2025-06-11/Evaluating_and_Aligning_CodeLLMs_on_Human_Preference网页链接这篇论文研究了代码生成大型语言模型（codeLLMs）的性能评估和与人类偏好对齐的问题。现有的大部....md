...
这篇论文探讨了代码生成大模型（codeLLMs）如何更好地与人类偏好对齐。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发场景。同时开发了SynCode-Instruct合成指令数据集（约200亿标记）用于模型微调。实验发现：1）传统基于执行的评估与人类偏好存在差距；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著。研究强调了在代码生成任务中考虑人类偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-11T21:04:07Z
- **目录日期**: 2025-06-11
