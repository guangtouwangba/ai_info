...
DyTact是一种无标记的动态手-物体接触捕捉方法，通过2D高斯面元（Gaussian surfels）和MANO网格结合，准确重建复杂手部操作中的接触动态。该方法利用模板模型的归纳偏置优化稳定性与速度，并通过细化模块处理高频变形。接触引导的自适应采样策略在遮挡区域增加面元密度，提升精度。实验表明，DyTact在动态接触估计和新视角合成质量上均达到领先水平，同时优化速度快且内存高效。适用于AI动画、XR和机器人领域。项目页面：https://oliver-cong02.github.io/DyTact.github.io/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T12:02:12Z
- **目录日期**: 2025-06-04
