...
Safari浏览器正遭遇新型全屏网络钓鱼攻击。黑客通过伪造登录页面，利用Fullscreen API隐藏地址栏，诱导用户输入账号密码。该攻击手法在社交媒体广告和评论区传播，因Safari仅以滑动动画提示全屏状态，极易被忽视。相比Chrome/Firefox的明确弹窗提示，Safari的防御更薄弱。尽管安全团队已向苹果报告，但苹果认为现有动画提示已足够，拒绝修复。专家建议用户登录时务必核对网址，警惕全屏页面。该攻击同样适用于其他浏览器，但Safari风险最高。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T00:02:19Z
- **目录日期**: 2025-06-04
