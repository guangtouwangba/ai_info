...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，以评估模型在真实场景中的表现。同时开发了SynCode-Instruct合成指令数据集（约200亿标记）验证指令微调效果。实验发现传统执行基准与人类偏好评估存在差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显了人类偏好对齐在代码生成中的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T20:05:54Z
- **目录日期**: 2025-06-04
