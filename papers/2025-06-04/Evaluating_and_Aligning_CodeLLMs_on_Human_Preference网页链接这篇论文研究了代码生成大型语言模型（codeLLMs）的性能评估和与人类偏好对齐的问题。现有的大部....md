...
这篇论文针对代码生成大模型(codeLLMs)提出了两个主要贡献：1) 创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，专门评估模型输出与人类偏好的匹配程度；2) 构建了20B标记量的合成指令数据集SynCode-Instruct用于模型微调。研究发现现有基于执行的基准测试与人类偏好评估存在差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在显著差距，凸显了优化这一维度的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T19:05:59Z
- **目录日期**: 2025-06-04
