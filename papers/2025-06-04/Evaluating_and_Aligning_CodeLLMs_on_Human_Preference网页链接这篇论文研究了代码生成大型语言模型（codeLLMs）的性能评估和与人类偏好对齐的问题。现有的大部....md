...
该论文针对代码生成大模型（codeLLMs）提出新评估方法，发现现有基准过度关注代码正确性而忽视人类偏好。研究者构建了包含397个样本的CodeArena基准（覆盖40类任务和44种语言）和20B标记的SynCode-Instruct合成指令集。实验显示：1）传统执行测试与人类偏好存在偏差；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调代码LLMs需同时优化技术指标和人类需求对齐，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T03:26:42Z
- **目录日期**: 2025-06-04
