...
这篇论文针对代码生成大模型(codeLLMs)提出了两个创新：1)构建了CodeArena基准测试，包含397个真实编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2)开发了20B标记量的SynCode-Instruct合成指令集，验证了大规模指令微调的有效性。实验发现现有基于执行的评估与人类偏好存在偏差，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人性化输出方面仍有显著差距，强调了代码生成模型对齐人类需求的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T01:32:33Z
- **目录日期**: 2025-06-04
