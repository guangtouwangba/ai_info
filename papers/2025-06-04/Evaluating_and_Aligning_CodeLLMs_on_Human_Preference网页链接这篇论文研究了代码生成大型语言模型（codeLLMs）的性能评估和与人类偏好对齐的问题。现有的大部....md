...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究者从真实用户查询中精选397个样本(覆盖40类任务和44种语言)，构建更贴近实际编程场景的评估体系。同时开发了包含近200亿标记的合成指令数据集SynCode-Instruct用于微调验证。实验发现：1)传统执行测试与人类偏好评估存在差距；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面仍有显著差异。研究强调了代码生成模型除功能性外，还需重视用户体验优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T02:36:11Z
- **目录日期**: 2025-06-04
