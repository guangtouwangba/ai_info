...
AUTOCIRCUIT-RL是一种基于强化学习（RL）和大型语言模型（LLM）的模拟电路拓扑自动合成框架。该框架分两阶段：指令调优（LLM学习根据设计约束生成电路拓扑）和RL优化（通过评估有效性、效率和输出电压的奖励模型改进模型）。实验表明，AUTOCIRCUIT-RL比基线方法多生成12%的有效电路，效率提升14%，重复生成率降低38%。在有限训练数据下，其成功合成有效电路的比率超过60%，展现出强泛化能力。该框架在复杂电路设计中兼具高效性和约束遵循性，推动了AI驱动电路设计的进步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T21:01:37Z
- **目录日期**: 2025-06-04
