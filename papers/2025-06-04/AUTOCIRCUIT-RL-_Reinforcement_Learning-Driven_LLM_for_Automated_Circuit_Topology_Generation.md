...
AUTOCIRCUIT-RL是一种基于强化学习（RL）和大型语言模型（LLM）的模拟电路拓扑自动合成框架。该框架分两阶段：指令调优（LLM学习根据设计约束生成电路拓扑）和RL优化（通过评估有效性、效率和输出电压的奖励模型改进性能）。实验表明，AUTOCIRCUIT-RL比基线方法多生成12%的有效电路，效率提升14%，重复率降低38%，仅需有限训练数据即可实现60%以上的有效电路合成成功率。该框架在复杂电路设计中展现出高效性和强泛化能力，推动了AI驱动电路设计的进步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T03:22:14Z
- **目录日期**: 2025-06-04
