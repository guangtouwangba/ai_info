...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI基于带动量的随机梯度下降（SGDM），通过初始化时根据梯度信号噪声比（g-SNR）调整不同参数组的学习率，避免了训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和大型语言模型（如GPT-2、Llama2-7B）预训练中表现优异，且具有超参数鲁棒性。在LoRA微调等任务中也优于现有优化器，并为大模型训练节省显著内存（如为Llama2-7B节省25.15GB）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T18:04:46Z
- **目录日期**: 2025-06-04
