...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法(如Adam)的必要性。SGD-SaI是对带动量SGD的改进，通过根据梯度信噪比(g-SNR)在初始化时调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，它节省50%内存，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更好，尤其在大模型上节省显著内存(GPT-2省5.93GB，Llama2-7B省25.15GB)。该方法对超参数鲁棒，在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T00:03:05Z
- **目录日期**: 2025-06-04
