...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题，同时将优化器内存占用减半。实验显示，在Transformer任务、ImageNet分类和GPT-2等大模型预训练中，其性能媲美或优于AdamW，且在LoRA微调中表现更佳。内存效率显著提升，如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB。该方法具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T20:04:37Z
- **目录日期**: 2025-06-04
