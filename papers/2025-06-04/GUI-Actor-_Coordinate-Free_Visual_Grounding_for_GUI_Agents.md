...
本文提出GUI-Actor，一种基于视觉语言模型（VLM）的无坐标GUI视觉定位方法，解决了现有基于坐标生成方法的三大缺陷：空间语义对齐弱、模糊监督目标处理差、以及密集坐标与视觉Transformer粗粒度特征不匹配。GUI-Actor通过注意力机制将专用<actor>令牌与相关视觉块对齐，支持单次前向传递生成多个候选动作区域，并设计验证器筛选最优区域。实验表明，该方法在多个基准测试中超越现有最优模型（如UI-TARS-72B），且仅需微调约1亿参数的动作头即可达到SOTA性能，保持VLM通用能力的同时实现精准定位。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T17:01:01Z
- **目录日期**: 2025-06-04
