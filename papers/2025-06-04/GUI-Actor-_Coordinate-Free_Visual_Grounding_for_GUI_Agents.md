...
本文提出GUI-Actor，一种基于视觉语言模型（VLM）的无坐标GUI视觉定位方法。该方法通过引入注意力机制的动作头，使专用<actor>标记与相关视觉块标记对齐，能单次预测多个动作区域，并设计验证器筛选最佳执行区域。实验表明，GUI-Actor在多个基准测试中超越现有方法，泛化性更强，如GUI-Actor-7B在ScreenSpot-Pro上以Qwen2.5-VL为骨干时得分达44.6，优于UI-TARS-72B。仅微调动作头（约1亿参数）即可达到SOTA性能，证明该方法能在保持VLM通用能力的同时赋予精准定位能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T05:01:00Z
- **目录日期**: 2025-06-04
