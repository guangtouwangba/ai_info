...
本文提出GUI-Actor，一种基于视觉语言模型（VLM）的无坐标GUI视觉定位方法，解决了现有文本坐标生成方法在空间语义对齐、模糊监督目标和视觉特征粒度不匹配等问题。核心创新包括：1）引入基于注意力的动作头，通过专用<actor>令牌对齐视觉块令牌，单次前向传播即可生成多个候选动作区域；2）设计验证器筛选最优执行区域。实验表明，该方法在多个基准测试中超越现有最佳模型（如UI-TARS-72B），并展现对未见过屏幕分辨率和布局的强泛化能力。仅微调动作头（约1亿参数）即可达到SOTA性能，保持VLM通用能力的同时赋予精准定位能力。</actor>

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T18:01:08Z
- **目录日期**: 2025-06-04
