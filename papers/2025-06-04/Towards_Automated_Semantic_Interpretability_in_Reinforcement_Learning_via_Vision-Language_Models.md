...
iTRACE是一种自动化的强化学习框架，通过结合预训练视觉语言模型(VLM)和可解释的树模型，实现语义可解释的决策。它利用VLM自动提取语义特征，并通过轻量级蒸馏模型避免实时推理开销，再使用树模型映射特征到策略。相比传统依赖人工标注的可解释方法，iTRACE无需人工干预，同时克服了VLM缺乏动作空间关联和直接策略优化能力的局限。实验表明，其性能与CNN策略相当，优于使用相同特征的多层感知机基线，能产生可验证、语义可解释且符合人类认知的行为，为强化学习提供了自动化可解释解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T03:22:55Z
- **目录日期**: 2025-06-04
