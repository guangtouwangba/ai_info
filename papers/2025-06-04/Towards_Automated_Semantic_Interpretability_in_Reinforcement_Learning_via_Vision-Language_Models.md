...
iTRACE是一个自动化的强化学习框架，通过结合预训练视觉语言模型(VLM)和可解释的树模型，实现语义可解释的决策。它利用VLM提取语义特征，并通过轻量化蒸馏解决VLM在RL循环中的计算瓶颈，再使用可解释树模型映射到策略。相比传统需要人工标注的方法，iTRACE完全自动化，同时克服了VLM缺乏动作空间关联和无法直接优化策略的局限。实验表明，iTRACE在使用相同可解释特征时优于MLP基线，性能媲美CNN策略，且能产生可验证、语义可解释且符合人类认知的行为，无需人工标注。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T08:02:41Z
- **目录日期**: 2025-06-04
