...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟10,000多个代理及其500万次互动。它通过真实社会环境和模拟引擎，研究社会极化、信息传播、基本收入政策及外部冲击（如飓风）等关键问题。模拟结果与现实实验数据高度吻合，验证了其捕捉人类行为机制的能力。该系统可作为计算社会科学实验平台，辅助传统研究方法（如调查、访谈），帮助研究者分析社会问题的成因与模式，并为政策制定提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T20:05:17Z
- **目录日期**: 2025-06-04
