...
研究人员推出了AnimeShooter数据集，以解决现有公开数据集缺乏角色参考图像和连贯多镜头动画生成的问题。该数据集提供分层标注（故事级概述和镜头级分解）及视觉一致的参考图像，并包含同步音频的子集AnimeShooter-audio。基于此开发的AnimeShooterGen系统结合多模态大语言模型和视频扩散模型，利用参考图像和已生成镜头来保持跨镜头视觉一致性和角色一致性。实验表明，该模型在视觉连贯性和参考遵循方面表现优异，验证了数据集对连贯动画视频生成的价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T05:01:37Z
- **目录日期**: 2025-06-04
