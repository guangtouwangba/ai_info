...
这篇论文提出了一种名为"曲率调谐"(Curvature Tuning, CT)的新型微调方法，通过调整激活函数的曲率来改变模型决策边界，而非传统的权重调整。CT具有可解释性，仅需引入单个超参数，却能有效提升模型性能。实验表明，CT在12个数据集上将ResNet-50/152的下游准确率分别提高了7.14%/8.46%(相比线性探测)和4.64%/1.70%(相比LoRA)，并在RobustBench基准测试中将鲁棒准确率提升了1032.64%/1494.46%。该方法通过将模型投影到平滑函数空间，补充了现有以特征适应为主的微调方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T18:02:32Z
- **目录日期**: 2025-06-04
