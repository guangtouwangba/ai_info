...
这篇论文提出了一种新的微调方法——曲率调节（CT），通过调整激活函数的曲率来优化模型决策边界，而非传统的权重调整。CT具有可解释性，仅需引入一个超参数，就能有效提升模型性能。实验表明，CT在ResNet-50/152上显著优于线性探测和LoRA方法，在12个数据集上分别提高准确率7.14%/8.46%和4.64%/1.70%，并在鲁棒性测试中将准确率提升超过1000%。该方法不仅参数高效，还能增强模型的泛化能力和鲁棒性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T17:02:32Z
- **目录日期**: 2025-06-04
