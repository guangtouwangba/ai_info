...
这篇论文提出了一种新的微调方法——曲率调谐(CT)，通过调节激活函数的曲率来调整模型决策边界，而非传统权重调整。CT将激活函数视为样条算子，仅需注入一个超参数即可实现可解释的模型调控。理论证明CT能有效改变决策边界曲率，并将模型投影到平滑函数空间。实验表明，CT显著提升了模型性能：在12个数据集上，ResNet-50/152的下游准确率比线性探测提高7.14%/8.46%，比LoRA提高4.64%/1.70%；在RobustBench对抗基准测试中，鲁棒准确率提升超过10倍。该方法参数高效且代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T16:03:42Z
- **目录日期**: 2025-06-04
