...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出时间表示对齐(TREPA)技术，利用自监督视频模型增强生成帧的时序连贯性。同时，通过系统性研究改进了SyncNet训练框架，显著提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上的多项指标优于现有技术。该研究不仅推进了唇同步技术的发展，其改进的SyncNet训练方案也可广泛应用于其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T18:05:41Z
- **目录日期**: 2025-06-04
