...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法，利用自监督视频模型增强生成帧的时序对齐。同时改进了SyncNet训练框架，解决了收敛问题并提升准确性。该方法在HDTF和VoxCeleb2数据集上表现优于现有技术，其改进经验可推广至其他音频驱动动画任务。核心创新在于结合Stable Diffusion能力直接建模音视频关联，并通过双重优化（TREPA+SyncNet改进）实现高质量唇形同步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-04T04:08:03Z
- **目录日期**: 2025-06-04
