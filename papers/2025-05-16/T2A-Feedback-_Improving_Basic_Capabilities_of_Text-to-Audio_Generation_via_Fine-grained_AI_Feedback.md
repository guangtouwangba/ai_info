...
该研究提出通过AI反馈学习提升文本到音频(T2A)生成模型的能力。研究者开发了三个细粒度评分管道：事件存在性评分、事件序列评分和音质评分，这些评分与人类偏好显著相关。基于此构建了包含41k提示和249k音频的T2A-FeedBack数据集，并创建了针对长文本和多事件场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调优，现有音频生成模型在简单和复杂场景下的表现均得到显著提升。这些自动评分管道既可作为反馈信号，也可作为评估指标。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T06:00:58Z
- **目录日期**: 2025-05-16
