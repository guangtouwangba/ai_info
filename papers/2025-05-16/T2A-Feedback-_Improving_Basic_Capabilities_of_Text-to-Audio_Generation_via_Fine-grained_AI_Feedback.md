...
该研究提出通过AI反馈学习提升文本到音频（T2A）生成模型的性能。针对复杂多事件音频生成中存在的提示跟随和音质问题，研究者开发了细粒度评分系统，包括事件存在性、事件序列准确性和音质评估。这些自动评分指标与人类偏好显著相关。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并推出专注于长文本、多事件和叙事场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调优，模型在简单和复杂场景下的生成质量均获得显著提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T04:02:26Z
- **目录日期**: 2025-05-16
