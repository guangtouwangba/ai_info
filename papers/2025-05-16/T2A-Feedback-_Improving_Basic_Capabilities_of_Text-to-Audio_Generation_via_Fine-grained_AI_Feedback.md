...
本文提出了一种基于AI反馈学习的文本到音频(T2A)生成改进方法。研究团队开发了三个细粒度音频评分管道：事件存在性评分、事件序列评分和音质评分，这些评分与人类偏好显著相关。基于此，他们构建了包含4.1万提示和24.9万音频的大型偏好数据集T2A-FeedBack，并创建了专注于长文本、多事件和叙事场景的评估基准T2A-EpicBench。实验表明，通过简单的偏好调优，现有音频生成模型在简单和复杂场景下的表现均得到显著提升。该方法有效提高了模型在复杂多事件音频生成中的表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T13:09:11Z
- **目录日期**: 2025-05-16
