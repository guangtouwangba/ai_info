...
当前文本转音频(T2A)模型在处理复杂多事件音频时仍存在提示跟随和音质不足的问题。研究提出通过AI反馈学习提升模型能力，开发了三个细粒度评分管道：事件出现评分、事件顺序评分和音质评分。这些自动化评分与人类偏好显著相关，优于现有评估指标。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并推出专注于长文本和多事件场景的T2A-EpicBench基准测试。实验表明，通过简单偏好调优，模型在简单和复杂场景下的表现均获得显著提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T23:00:54Z
- **目录日期**: 2025-05-16
