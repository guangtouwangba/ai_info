...
该研究评估了大语言模型（LLM）在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术的作用。实验发现，大模型普遍优于小模型，但策略性提示能缩小差距；过长的提示会损害小模型在基础任务中的表现，而大模型更稳定；复杂任务中高级提示对小模型帮助更大，但对高性能大模型提升有限。尽管推理方法可能显著提升表现，但也可能引发不稳定性。研究发现，当前LLM在规划、推理和空间协调等关键领域仍存在根本性局限，需超越静态基准以全面评估推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T22:01:09Z
- **目录日期**: 2025-05-16
