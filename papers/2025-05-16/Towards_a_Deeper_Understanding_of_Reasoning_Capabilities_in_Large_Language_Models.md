...
该研究评估了大型语言模型(LLM)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术。结果显示：大模型普遍优于小模型，但策略性提示可缩小差距；过长提示会损害小模型在基础任务的表现，而大模型更稳健；复杂任务中高级提示对小模型帮助更大。研究发现，高级推理方法虽能提升表现，但也带来不稳定性，且LLM在规划、推理和空间协调等关键领域仍存在根本性缺陷。研究表明当前LLM的真实推理能力有限，需要超越静态基准来评估复杂的推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T12:01:27Z
- **目录日期**: 2025-05-16
