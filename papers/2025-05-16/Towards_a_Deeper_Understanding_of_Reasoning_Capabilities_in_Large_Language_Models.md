...
该研究评估了大语言模型（LLM）在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术。实验发现，大模型普遍优于小模型，但策略性提示可缩小差距；过长提示对小模型的基础反应任务有负面影响，而大模型表现更稳定；复杂任务中高级提示对小模型提升更明显，但对高性能大模型改善有限。高级推理方法虽能提升表现，但也带来不稳定性。与人类相比，LLM在规划、推理和空间协调等关键领域仍存在根本性缺陷，表明当前模型无法仅通过自反思提示完全克服这些短板。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T18:01:18Z
- **目录日期**: 2025-05-16
