...
这项研究评估了大型语言模型(LLM)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术。实验发现：1)大模型普遍优于小模型，但策略性提示能缩小差距；2)过长提示会损害小模型的基础反应任务表现，而大模型更稳健；3)高级提示技术主要提升小模型在复杂任务中的表现，对大模型改善有限。研究指出当前LLM在规划、推理和空间协调等关键领域仍存在根本性缺陷，其推理能力具有高度不稳定性，仅靠自我反思提示无法完全克服，需要超越静态基准来评估真实推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T14:01:32Z
- **目录日期**: 2025-05-16
