...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它利用大型语言模型驱动的代理、真实社会环境和大规模模拟引擎，模拟了1万多个代理的社会生活及500万次互动。研究者通过该平台探索了四个关键社会问题：极化现象、刺激性信息传播、无条件基本收入政策影响和飓风等外部冲击。模拟结果与现实世界实验数据吻合，表明该系统能有效捕捉人类行为机制，验证了其作为社会科学研究和政策制定工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T15:05:21Z
- **目录日期**: 2025-05-16
