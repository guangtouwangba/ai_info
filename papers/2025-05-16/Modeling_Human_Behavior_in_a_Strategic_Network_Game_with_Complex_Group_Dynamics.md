...
该研究比较了多种从小数据集中学习人类在战略网络游戏（Junior High Game）中行为模型的方法。最佳方法hCAB通过建模行为分布（而非均值）并假设人类具有社区意识行为（而非简单行为匹配），能准确反映小规模群体（6-11人）的动态特征。实验表明，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体层面行为的合理性。这一发现对理解社会网络中的财富、健康不平等等现象具有重要意义，为促进有利社会结果提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T19:02:21Z
- **目录日期**: 2025-05-16
