...
该研究比较了多种从小数据集中学习人类行为模型的方法，重点关注策略性网络游戏“Junior High Game”（JHG）。研究发现，表现最佳的模型hCAB通过建模人类行为的分布（而非均值）并假设人类具有社区意识行为（而非简单行为匹配），能更准确地反映小规模群体（6-11人）的动态。实验还表明，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体行为层面的可信度。这一成果为理解人类网络行为及其对社会不平等等议题的影响提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T22:02:09Z
- **目录日期**: 2025-05-16
