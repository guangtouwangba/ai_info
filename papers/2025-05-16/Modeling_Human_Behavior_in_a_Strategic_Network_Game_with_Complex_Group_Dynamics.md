...
人类网络对社会结果（如贫富差距、健康不平等、霸凌等）有重大影响。研究通过对比不同建模方法，发现hCAB模型最能准确模拟小规模群体（6-11人）在策略性网络游戏（Junior High Game）中的行为。hCAB通过建模行为分布（而非均值）并假设人类具有社区意识行为（而非简单行为匹配），其模拟结果与真实人类群体动态高度吻合。用户研究还表明，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体行为层面的可信度。这一成果为理解人类网络互动提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T17:02:15Z
- **目录日期**: 2025-05-16
