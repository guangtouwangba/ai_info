...
该研究探讨了如何通过小数据集建模人类在策略性网络游戏（Junior High Game）中的行为，以更好地理解社会网络对财富、健康不平等等社会结果的影响。研究发现，采用社区感知行为（community-aware）并建模行为分布（而非均值）的hCAB方法表现最佳，能准确反映小型群体（6-11人）的动态特征。用户研究显示，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体行为层面的拟真性。这一成果为理解人类网络行为提供了新工具，有助于优化社会干预策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T21:02:11Z
- **目录日期**: 2025-05-16
