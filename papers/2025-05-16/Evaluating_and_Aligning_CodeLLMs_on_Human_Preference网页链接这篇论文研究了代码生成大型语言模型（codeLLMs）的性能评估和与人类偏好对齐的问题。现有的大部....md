...
这篇论文针对代码生成大语言模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次关注代码生成与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令数据集用于模型微调。实验发现传统执行基准与人类偏好评估存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面表现差距显著，突显该研究方向的重要性。研究为提升代码LLMs的实际应用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T08:06:32Z
- **目录日期**: 2025-05-16
