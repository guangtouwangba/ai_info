...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法无需中间运动表示，直接建模音频-视觉关系，利用Stable Diffusion的强大能力。针对扩散模型的时间一致性问题，提出时间表示对齐(TREPA)技术，通过自监督视频模型提取的时间特征增强生成帧的连贯性。研究还优化了SyncNet训练框架，改进其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T17:05:38Z
- **目录日期**: 2025-05-16
