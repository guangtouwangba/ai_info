...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时改进了SyncNet训练框架，优化模型架构和训练参数，显著提升准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新方案。创新点包括：直接建模音视关联、增强时序一致性、改进SyncNet训练，这些经验可推广至相关领域。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T09:05:47Z
- **目录日期**: 2025-05-16
