...
Cursor官方发布模型选择指南，从三个维度推荐：1)自信度(Gemini-2.5-pro等需提示少)；2)思考深度(o3等善深度推理)；3)上下文容量(大任务需高容量模型)。按行为特点分为思考型(自主决策)和非思考型(需明确指令)。不同任务推荐：小改动用Claude-3.5-sonnet，大规模重构用Claude-3.7-Sonnet，复杂bug修复用o3。附选择流程图辅助决策。指南详见官网链接。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T00:02:49Z
- **目录日期**: 2025-05-16
