...
Cursor官方发布模型选择指南，提出三个关键维度：自信度（如Gemini-2.5-pro快速决策）、思考深度（如o3深入分析）、上下文容量（大任务需高容量模型）。模型分为思考型（自主决策，如Claude-3.7-sonnet）和非思考型（需明确指令，如GPT-4.1）。按任务推荐：小改动用Claude-3.5-sonnet，大规模重构用Claude-3.7-Sonnet，代码导航用Gemini-2.5-Pro，复杂问题用o3。附选择流程图辅助决策。（99字）  
