...
这篇综述首次系统性地总结了检测大型AI模型（LAIMs）生成的多媒体内容（如文本、图像、视频、音频等）的研究现状。作者提出了一种新的分类法，将检测方法分为纯检测（提升检测性能）和超越检测（增强通用性、鲁棒性等）两类，并概述了生成机制、数据集、工具和评估指标。研究特别关注社交媒体视角下的社会影响，指出了当前检测技术面临的挑战和未来研究方向。该工作旨在填补学术空白，为全球AI安全做出贡献，确保数字领域信息真实性。项目链接已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T01:30:07Z
- **目录日期**: 2025-05-16
