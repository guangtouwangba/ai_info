...
本研究探索了使用大型语言模型（LLMs）自动化评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在Master评分量表（MIRS）28个项目上的表现。结果显示，LLMs的精确准确率较低（0.27-0.44），但近似准确率（0.67-0.87）和阈值准确率（0.75-0.88）较高。零温度参数确保了评分者内部高信度（GPT-4o α=0.98）。链式推理、少样本提示等技术对特定评估项目有效。研究表明AI辅助OSCE评估具有潜力，为临床沟通技能自动化评估研究奠定了基础，并提供了多模型多提示技术的基准数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T10:02:34Z
- **目录日期**: 2025-05-16
