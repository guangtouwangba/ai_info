...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能（OSCE考试）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在MIRS量表28个项目上的表现。结果显示，LLMs的精确准确率较低（0.27-0.44），但中等至高水平的近似准确率（0.67-0.87）和阈值准确率（0.75-0.88）。零温度参数确保了高评分者内部信度（GPT-4o α=0.98）。思维链、少样本和多步提示技术对特定评估项目有效。研究表明AI辅助OSCE评估具有可行性，为临床沟通技能的自动化评估奠定了基础，并提供了多模型多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T21:02:23Z
- **目录日期**: 2025-05-16
