...
本研究探讨了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能考试（OSCE）的可行性。测试了GPT-4o、Claude 3.5等四种先进模型在零样本、思维链等不同提示策略下对28项评分标准的表现。结果显示，LLMs的精确准确率较低（0.27-0.44），但容错准确率（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好。模型间评分一致性高（GPT-4o α=0.98），特定提示技术可提升表现。研究为临床沟通技能的自动化评估建立了基准，证实了AI辅助评分的潜力，为未来研究奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T16:02:40Z
- **目录日期**: 2025-05-16
