...
本研究探讨了使用大型语言模型（LLMs）自动化评估医学生OSCE考试中沟通技能的可行性。测试了GPT-4o、Claude 3.5等4种先进模型在MIRS量表的28个项目上的表现，采用零样本、思维链等多种提示技术。结果显示，模型在精确匹配准确率较低（0.27-0.44），但相邻评分准确率（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有可行性，为临床沟通技能的自动化评估奠定了基础，并提供了多模型多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T03:20:51Z
- **目录日期**: 2025-05-16
