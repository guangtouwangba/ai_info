...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能考试（OSCE）的可行性。测试了GPT-4o、Claude 3.5等四种先进模型在零样本、思维链等不同提示方法下对28项评分标准的评估表现。结果显示，模型在精确匹配准确率较低（0.27-0.44），但相邻分值和阈值准确率中等偏高（0.67-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有可行性，为临床沟通技能的自动化评估奠定了基础，但需针对特定评估项目优化提示技术。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T22:02:18Z
- **目录日期**: 2025-05-16
