...
本研究探讨了使用大型语言模型（LLMs）自动化评估医学临床技能考试（OSCE）的可行性。通过比较GPT-4o、Claude 3.5等四种先进模型在零样本、思维链等不同提示技术下的表现，发现LLMs在28项MIRS评分标准中的精确准确率较低（0.27-0.44），但近似准确率较高（0.67-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。研究证实AI辅助OSCE评估具有潜力，为临床沟通技能的自动化评估奠定了基础，同时提供了多种LLMs在不同提示技术下的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T17:02:27Z
- **目录日期**: 2025-05-16
