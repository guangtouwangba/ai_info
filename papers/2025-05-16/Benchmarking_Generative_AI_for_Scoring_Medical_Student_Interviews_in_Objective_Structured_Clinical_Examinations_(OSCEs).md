...
本研究探索了使用大型语言模型（LLMs）自动化评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在Master Interview评分量表（MIRS）28个项目中的表现。结果显示，模型在精确匹配准确率较低（0.27-0.44），但相邻分数和阈值准确率中等至较高（0.67-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。思维链、少量示例和多步提示技术对特定评估项目有效。研究表明AI辅助OSCE评估具有潜力，为临床沟通技能自动化评估提供了基准性能分析。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T20:02:31Z
- **目录日期**: 2025-05-16
