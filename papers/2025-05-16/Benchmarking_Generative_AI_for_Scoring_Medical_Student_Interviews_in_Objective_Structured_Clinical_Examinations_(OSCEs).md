...
本研究探索了使用大语言模型（LLMs）自动评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o等四种先进模型在Master访谈评分量表（MIRS）28个项目上的表现。结果显示，LLMs在精确匹配准确率（0.27-0.44）上表现较低，但在近似准确率（0.67-0.87）和阈值准确率（0.75-0.88）上表现中等至良好。零温度参数确保了高评分者内部信度（GPT-4o α=0.98）。特定提示技术（如思维链、少样本）对某些评估项目有效。研究表明AI辅助OSCE评估具有可行性，为临床沟通技能自动化评估奠定了基础，并提供了多模型多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T02:31:52Z
- **目录日期**: 2025-05-16
