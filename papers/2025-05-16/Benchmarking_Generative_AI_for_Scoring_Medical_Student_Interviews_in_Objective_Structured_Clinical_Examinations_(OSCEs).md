...
本研究探讨了使用大型语言模型（LLM）自动评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在Master评分量表（MIRS）28个项目中的表现。结果显示，LLM的精确准确率较低（0.27-0.44），但相邻分级准确率（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。特定提示技术（思维链、少量示例等）对某些评估项目有效。研究表明AI辅助OSCE评估具有潜力，为未来临床沟通技能自动化评估研究奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T12:02:49Z
- **目录日期**: 2025-05-16
