...
本研究探讨了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能（OSCE考试）的可行性。通过对比GPT-4o、Claude 3.5等四种先进模型在零样本、思维链等不同提示技术下的表现，发现模型在Master Interview评分量表（MIRS）上的准确率表现：精确匹配准确率较低（0.27-0.44），但相邻分数匹配准确率较高（0.67-0.87）。模型展现出高评分者内部信度（GPT-4o α=0.98），且表现不受评估阶段或沟通领域影响。研究证实了AI辅助OSCE评分的可行性，为临床沟通技能自动化评估奠定了基础，并提供了多模型多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T13:11:04Z
- **目录日期**: 2025-05-16
