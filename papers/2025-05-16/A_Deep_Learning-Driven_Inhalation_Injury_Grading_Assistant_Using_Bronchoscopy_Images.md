...
本研究开发了一种基于深度学习的吸入性损伤分级辅助工具，利用支气管镜图像解决传统AIS评分主观性强、与临床参数相关性差的问题。通过数据增强技术（图像变换、CUT和CycleGAN）扩充数据集后，比较GoogLeNet和ViT模型性能。结果显示：GoogLeNet+CUT组合效果最佳，分类准确率达97.8%。CUT增强显著改变了图像直方图分布和频谱纹理，PCA证实其提升了特征空间类别可分性。Grad-CAM分析显示CUT热图平均强度达119.6（原始数据98.8），决策依据更明确。该工具创新性地以机械通气时长作为分级标准，为临床诊断提供客观支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T18:02:00Z
- **目录日期**: 2025-05-16
