...
本研究开发了一种基于深度学习的吸入性损伤分级辅助工具，利用支气管镜图像克服传统主观评分方法的局限性。通过数据增强技术（包括图像变换、CUT和CycleGAN）解决医学影像数据稀缺问题。实验比较了GoogLeNet和ViT模型，发现GoogLeNet结合CUT增强效果最佳，分类准确率达97.8%。分析显示CUT能显著改变图像直方图分布和频谱纹理，PCA可视化证实其增强特征空间类别分离性，Grad-CAM热图平均强度达119.6（原始数据98.8）。该工具创新性地采用机械通气时长作为分级标准，为临床诊断提供客观支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T09:02:01Z
- **目录日期**: 2025-05-16
