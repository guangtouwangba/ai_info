...
本研究开发了一种基于深度学习的吸入性损伤分级辅助工具，利用支气管镜图像解决传统评分方法（如AIS）主观性强、与临床参数相关性差的问题。通过数据增强技术（图像变换、CUT和CycleGAN）扩充数据集后，比较了GoogLeNet和ViT模型的性能。结果显示，GoogLeNet结合CUT增强效果最佳，分类准确率达97.8%。CUT能显著改变图像直方图分布和频谱纹理，PCA可视化表明其增强了特征空间类别区分度，Grad-CAM分析显示CUT热图平均强度(119.6)明显高于原始数据(98.8)。该工具创新性地以机械通气时长作为分级标准，为诊断提供全面支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-16T13:10:21Z
- **目录日期**: 2025-05-16
