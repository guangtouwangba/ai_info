...
OptimAI是一个利用LLM驱动的AI代理解决自然语言描述的优化问题框架，性能优于现有方法。它包含四个关键角色：(1) 公式化器将问题描述转为数学公式；(2) 规划器制定高层解决方案；(3) 编码器和代码评审员通过环境交互优化执行。消融实验表明移除规划器或评审员会分别导致5.8倍和3.1倍的效率下降。引入UCB调试调度策略可额外提升3.3倍效率。该系统在NLP4LP和Optibench数据集上分别达到88.1%和71.2%准确率，错误率比之前最佳结果降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T14:01:18Z
- **目录日期**: 2025-04-24
