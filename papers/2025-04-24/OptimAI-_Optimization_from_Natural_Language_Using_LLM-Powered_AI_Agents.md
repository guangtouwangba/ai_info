...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。该框架包含四个核心角色：**问题描述转译器**（将自然语言转为数学公式）、**策略规划器**（预先制定求解策略）、**代码生成器**和**代码评审器**（通过环境交互优化结果）。实验表明，移除规划器或评审器会导致效率分别下降5.8倍和3.1倍，而动态调试调度策略能额外提升3.3倍效率。在NLP4LP和Optibench数据集上，其准确率达88.1%和71.2%，错误率较现有最佳方法降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T08:01:09Z
- **目录日期**: 2025-04-24
