...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。该框架包含四个关键角色：(1) 公式化器将问题转换为数学表述；(2) 规划器制定高层解决方案；(3) 编码器和代码评审器通过环境交互优化结果。研究表明，缺少规划器或评审器会分别导致5.8倍和3.1倍的效率下降。引入UCB调试调度机制可额外提升3.3倍效率。在NLP4LP和Optibench数据集上，OptimAI分别达到88.1%和71.2%的准确率，错误率比现有最佳方法降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T05:01:14Z
- **目录日期**: 2025-04-24
