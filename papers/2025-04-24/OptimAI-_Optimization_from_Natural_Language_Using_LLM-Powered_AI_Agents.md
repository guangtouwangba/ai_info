...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。该框架包含四个核心角色：(1) **formulator**将问题描述转化为数学公式；(2) **planner**制定高层解决方案；(3) **coder**和**code critic**执行并优化代码。实验表明，移除planner或code critic会分别导致效率下降5.8倍和3.1倍。此外，动态调试调度策略（UCB-based）进一步提升了3.3倍效率。OptimAI在NLP4LP和Optibench数据集上分别达到88.1%和71.2%的准确率，错误率比现有最优方法降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T12:01:17Z
- **目录日期**: 2025-04-24
