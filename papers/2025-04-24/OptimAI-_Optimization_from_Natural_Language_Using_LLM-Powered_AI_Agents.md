...
OptimAI是一个基于LLM的多智能体框架，用于将自然语言描述的优化问题转化为数学形式并求解。它包含四个核心角色：问题描述转数学模型的**formulator**、制定高层策略的**planner**、执行代码的**coder**和反思优化的**code critic**。实验表明，缺少planner或code critic会分别导致效率下降5.8倍和3.1倍。采用UCB调试调度策略还能带来3.3倍的额外效率提升。该系统在NLP4LP数据集上达到88.1%准确率（错误率降低58%），在Optibench非线性问题上达71.2%（错误率降低50%），显著优于现有方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T17:01:07Z
- **目录日期**: 2025-04-24
