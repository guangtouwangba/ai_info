...
OptimAI是一个基于多智能体协作的优化问题求解框架，通过LLM驱动的AI角色将自然语言描述的优化问题转化为数学形式并求解。其核心包含四个角色：问题描述转数学模型的**formulator**、制定高层策略的**planner**、执行互动的**coder**和反思优化的**code critic**。消融实验表明缺少planner或code critic会导致效率下降5.8倍和3.1倍，而动态调试调度策略（UCB-based）可额外提升效率3.3倍。该框架在NLP4LP数据集上达到88.1%准确率，在Optibench非线性子集达71.2%，错误率较之前最优结果分别降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T04:01:15Z
- **目录日期**: 2025-04-24
