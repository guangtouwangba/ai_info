...
OptimAI是一个基于LLM的多智能体框架，用于将自然语言描述的优化问题转化为数学形式并求解。它包含四个核心角色：1）问题描述转数学模型的**formulator**；2）制定高层策略的**planner**；3）执行交互的**coder**和反思优化的**code critic**。实验表明，缺少planner或code critic会分别导致5.8倍和3.1倍的效率下降。该框架还采用UCB动态调试策略，带来额外3.3倍效率提升。在NLP4LP和Optibench数据集上分别达到88.1%和71.2%准确率，错误率比之前最佳结果降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T18:01:08Z
- **目录日期**: 2025-04-24
