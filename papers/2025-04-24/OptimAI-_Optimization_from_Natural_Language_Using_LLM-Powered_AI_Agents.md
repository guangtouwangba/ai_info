...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。它包含四个核心角色：(1) **公式化器**将问题转换为数学表达；(2) **规划器**制定高层解决方案；(3) **编码器**和**代码评审员**通过环境交互优化执行。实验表明，缺少规划器或评审员会分别导致效率下降5.8倍和3.1倍。引入UCB调试调度策略可进一步提升效率3.3倍。该框架在NLP4LP和Optibench数据集上分别达到88.1%和71.2%的准确率，错误率比现有最优方法降低58%和50%，展现了多代理协同的高效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T16:01:33Z
- **目录日期**: 2025-04-24
