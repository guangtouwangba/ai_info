...
OptimAI是一个基于LLM的AI代理框架，用于解决自然语言描述的优化问题，性能优于现有方法。它包含四个核心角色：(1) **formulator**将问题转化为数学形式；(2) **planner**制定高层策略；(3) **coder**和**code critic**通过环境交互优化执行。实验表明，移除planner或critic会分别导致效率下降5.8倍和3.1倍。引入UCB动态调试调度进一步提升了3.3倍效率。该框架在多代理协作下，在NLP4LP和Optibench数据集上分别达到88.1%和71.2%准确率，错误率比之前最佳结果降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T09:01:07Z
- **目录日期**: 2025-04-24
