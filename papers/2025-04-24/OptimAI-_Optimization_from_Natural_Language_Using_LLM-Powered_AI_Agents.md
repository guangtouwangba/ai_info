...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。其核心包含四个角色：(1) **formulator**（问题形式化）将自然语言转为数学表达；(2) **planner**（策略规划）制定高层求解方案；(3) **coder**（代码生成）和**code critic**（代码审查）通过交互优化结果。实验表明，移除planner或code critic会分别导致效率下降5.8倍和3.1倍，而动态调试调度策略（UCB-based）可进一步提升效率3.3倍。该框架在NLP4LP和Optibench数据集上分别达到88.1%和71.2%的准确率，错误率较之前最优结果降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T15:01:19Z
- **目录日期**: 2025-04-24
