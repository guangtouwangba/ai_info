...
本文提出了一种基于神经形态动态视觉传感器（DVS）和脉冲神经网络（SNN）的无人机自主导航框架。该系统通过事件相机在无监督模式下检测移动物体，并结合轻量级物理引导神经网络（PgNN）预测最优飞行路径以实现能耗最小化。该框架在Gazebo模拟器中实现，集成了基于ROS的视觉-规划神经符号系统，能有效应对复杂光照条件，具有低延迟决策优势。研究展示了事件视觉与物理引导规划相结合在节能自主导航中的应用潜力，特别适用于移动障碍物规避场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T07:01:41Z
- **目录日期**: 2025-04-24
