...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合动态视觉传感器（DVS）和脉冲神经网络（SNN）实现移动物体检测，并通过轻量级物理引导神经网络（PgNN）预测最优飞行路径。该系统在Gazebo模拟器中实现，利用事件相机在复杂光照条件下异步检测强度变化，结合ROS中间件构建视觉到规划的神经符号框架。研究展示了事件视觉与物理引导规划结合的潜力，能实现低延迟决策和节能自主导航，特别适用于移动门框检测和避障场景，具有高动态范围、抗运动模糊等优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T11:02:24Z
- **目录日期**: 2025-04-24
