...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合事件相机和脉冲神经网络（SNN）实现动态目标检测，并通过轻量级物理引导神经网络（PgNN）预测最优飞行路径以最小化能耗。该系统采用无监督学习方式处理事件信号，整合物理模型提升鲁棒性和能效，并在Gazebo仿真器中通过ROS中间件实现视觉-规划融合。研究展示了事件视觉与物理引导规划相结合在低延迟决策和高效自主导航中的应用潜力，特别适用于动态障碍物规避场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T18:02:12Z
- **目录日期**: 2025-04-24
