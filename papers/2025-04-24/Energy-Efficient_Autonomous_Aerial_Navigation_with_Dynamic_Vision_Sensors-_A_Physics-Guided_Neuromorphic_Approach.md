...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合事件相机和脉冲神经网络（SNN）实现动态障碍物检测，并采用物理引导的轻量级神经网络（PgNN）进行能量最优路径规划。该系统通过异步事件相机在复杂光照条件下稳定感知，利用无监督SNN检测移动目标，配合物理模型增强的神经网络预测最优飞行路径，在Gazebo仿真环境中实现了基于ROS的视觉-规划神经符号框架。该研究展示了事件视觉与物理引导规划相结合在低延迟、高能效自主导航中的应用潜力，特别适用于需要快速决策的无人机避障场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T17:02:14Z
- **目录日期**: 2025-04-24
