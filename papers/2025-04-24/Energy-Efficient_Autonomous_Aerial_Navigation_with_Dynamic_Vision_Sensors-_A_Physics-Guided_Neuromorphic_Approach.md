...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合事件相机和脉冲神经网络(SNN)实现动态目标检测，并融合轻量级物理引导神经网络(PgNN)进行能量感知路径规划。该系统采用非监督学习的浅层SNN架构处理事件流数据，结合深度输入训练PgNN预测最优飞行路径，在Gazebo仿真环境中通过ROS中间件实现了视觉-规划融合的神经符号框架。研究展示了事件视觉与物理引导规划相结合在低延迟决策和节能自主导航方面的潜力，特别适用于动态障碍物规避场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T03:17:49Z
- **目录日期**: 2025-04-24
