...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合事件相机和脉冲神经网络（SNN）实现移动物体检测，并采用轻量级物理引导神经网络（PgNN）预测最优飞行路径以降低能耗。该系统在Gazebo模拟器中实现，通过ROS中间件整合了从视觉感知到路径规划的神经符号框架。研究展示了事件相机在复杂光照条件下的优势，以及物理AI模型在提升系统鲁棒性、能效和可解释性方面的潜力，为低延迟决策的节能自主导航提供了新思路，特别适用于动态障碍物规避场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T19:02:04Z
- **目录日期**: 2025-04-24
