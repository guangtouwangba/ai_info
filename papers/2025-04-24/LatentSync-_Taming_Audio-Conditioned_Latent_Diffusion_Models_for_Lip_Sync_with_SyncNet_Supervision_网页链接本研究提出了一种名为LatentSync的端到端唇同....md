...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型中时间一致性问题，创新性提出TREPA方法（时间表示对齐），利用自监督视频模型增强帧间一致性。同时，研究系统分析了SyncNet收敛问题，改进其训练框架显著提升精度。该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验可推广至其他音频驱动动画任务。核心突破在于直接建模音视频关联，结合Stable Diffusion能力与时间一致性优化，实现高质量唇形同步生成。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T11:05:30Z
- **目录日期**: 2025-04-24
