...
LatentSync是一种新型端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模音频-视觉关系而无需中间运动表示。该研究解决了扩散模型的时间一致性问题，提出TREPA方法利用自监督视频模型的时间表示来对齐生成帧。同时改进了SyncNet训练框架，通过优化模型架构、超参数和数据预处理显著提升其准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。这些改进也可推广至其他基于SyncNet的唇同步和人像动画系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T21:05:35Z
- **目录日期**: 2025-04-24
