...
该研究提出SGD-SaI方法，挑战了自适应梯度方法(如Adam)的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题。相比AdamW，SGD-SaI内存消耗减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，尤其显著节省内存(GPT-2节省5.93GB，Llama2-7B节省25.15GB)。同时展现出超参数鲁棒性，在LoRA微调等任务中超越现有优化器，为深度学习训练提供了更高效的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T18:04:20Z
- **目录日期**: 2025-04-24
