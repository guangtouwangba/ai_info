...
本文提出了一种名为广义邻域注意力（GNA）的新型稀疏注意力机制，旨在解决现有稀疏注意力方法（如邻域注意力）因复杂架构和硬件快速演进导致的性能瓶颈问题。GNA统一了滑动窗口、跨步窗口和分块注意力等局部注意力模式，并通过模拟器提供更真实的加速上限预测。研究团队在NVIDIA Blackwell架构上基于CUTLASS实现了GNA，在理想分块稀疏情况下可达理论最大加速，FP16精度下实现1.3 petaFLOPs/s的有效算力。实验表明，该方案在Cosmos-7B等生成模型中可实现28%-46%的端到端加速（无需微调），相关代码将通过NATTEN项目开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T22:01:04Z
- **目录日期**: 2025-04-24
