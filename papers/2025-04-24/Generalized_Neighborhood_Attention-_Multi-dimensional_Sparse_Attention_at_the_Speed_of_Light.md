...
本文提出了一种广义邻域注意力机制(GNA)，用于改进稀疏注意力的性能分析模型。GNA能描述滑动窗口、跨步窗口和分块注意力等多种局部注意力模式。研究者开发了性能模拟器来预测实际加速上限，并在NVIDIA Blackwell架构上基于CUTLASS实现了优化的FMHA内核。实验表明，该实现能在完全分块稀疏情况下达到理论最大加速，FP16计算效率达1.3 petaFLOPs/秒。在Cosmos-7B等生成模型中，未经微调即可带来28%-46%的端到端加速。相关模拟器和Blackwell内核将通过NATTEN项目开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T03:16:02Z
- **目录日期**: 2025-04-24
