...
本文提出了一种广义邻域注意力机制(GNA)，通过研究局部性稀疏注意力机制的性能改进模型，解决了现有稀疏注意力方法难以持续超越自注意力基准的问题。GNA能描述滑动窗口、跨步窗口和分块注意力等多种形式，并开发了模拟器来预测实际加速上限。研究团队在NVIDIA Blackwell架构上基于CUTLASS实现了GNA，在理想分块稀疏情况下达到理论最大加速，FP16计算效率达1.3 petaFLOPs/秒。实验表明，GNA在Cosmos-7B等生成模型中可实现28%-46%的端到端加速，且无需微调。相关模拟器和内核将通过NATTEN项目开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T06:00:59Z
- **目录日期**: 2025-04-24
