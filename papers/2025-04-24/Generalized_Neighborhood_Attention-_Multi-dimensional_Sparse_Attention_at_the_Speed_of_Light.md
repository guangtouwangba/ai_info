...
本文提出了一种新型稀疏注意力机制——广义邻域注意力(GNA)，通过研究基于局部性的稀疏注意力，建立了更精确的性能分析模型。GNA能描述滑动窗口、跨步窗口和分块注意力等多种形式，并开发了模拟器来预测实际加速上限。研究团队在NVIDIA Blackwell架构上基于CUTLASS实现了GNA，在理想分块稀疏情况下实现了理论最大加速，FP16计算效率达1.3 petaFLOPs/秒。实验表明，GNA在Cosmos-7B等现成生成模型中无需微调即可带来28%-46%的端到端加速。相关模拟器和Blackwell内核将通过NATTEN项目开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T05:01:04Z
- **目录日期**: 2025-04-24
