...
本文提出了一种广义邻域注意力机制（GNA），通过优化稀疏注意力实现来提升计算效率。研究分析了滑动窗口、跨步窗口和分块注意力等局部稀疏机制的设计选择，并开发了性能模拟器来预测实际加速上限。基于NVIDIA Blackwell架构，团队在CUTLASS中实现了GNA，在理想分块稀疏情况下达到1.3 petaFLOPs/s的FP16计算利用率。实验表明，将GNA集成到Cosmos-7B等现成生成模型中，无需微调即可在B200上实现28%-46%的端到端加速。相关模拟器和Blackwell内核将通过NATTEN项目开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T17:00:57Z
- **目录日期**: 2025-04-24
