...
这篇论文介绍了AgentSociety，这是一个利用大型语言模型驱动的大规模社会模拟器，可以模拟超过1万个代理的社会互动（500万次）。研究者用它作为计算社会科学实验平台，研究了极化现象、信息传播、基本收入政策影响和飓风冲击等社会问题。模拟结果与现实世界数据吻合，验证了该工具在捕捉人类行为机制方面的有效性。AgentSociety展现了作为社会科学研究和政策制定辅助工具的潜力，能够补充传统研究方法（如调查和访谈），帮助理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T18:04:55Z
- **目录日期**: 2025-04-24
