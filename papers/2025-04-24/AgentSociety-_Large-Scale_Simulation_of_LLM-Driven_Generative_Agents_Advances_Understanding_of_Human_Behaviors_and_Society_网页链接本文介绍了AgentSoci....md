...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个AI代理的社会互动（500万次交互）。该系统通过四个社会实验（社会极化、信息传播、基本收入政策、自然灾害影响）验证其有效性，结果与现实数据高度吻合。研究表明，该平台能有效捕捉人类行为模式和社会机制，可作为社会科学研究和政策制定的创新工具，为传统研究方法（如调查、访谈）提供补充。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T19:04:31Z
- **目录日期**: 2025-04-24
