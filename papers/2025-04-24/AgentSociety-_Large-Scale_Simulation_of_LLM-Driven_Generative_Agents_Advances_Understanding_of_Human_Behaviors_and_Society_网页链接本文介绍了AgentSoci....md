...
AgentSociety是一个基于大型语言模型的大规模社会模拟器，能够模拟10,000多个代理的社会互动（500万次交互）。研究者用它研究了四个关键社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响和外部冲击（如飓风）效应。模拟结果与现实世界数据高度吻合，验证了其捕捉人类行为机制的能力。该系统可作为计算社会科学实验平台，辅助传统研究方法（调查、访谈等），帮助研究者分析社会问题的模式和成因，并为政策制定提供参考依据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T20:05:00Z
- **目录日期**: 2025-04-24
