...
这篇论文介绍了在AI数学奥林匹克竞赛（AIMO-2）中获胜的方案。其核心方法基于三点：1）构建包含54万道高质量数学题（含奥赛题）及320万条详细解答的大规模数据集；2）提出新方法，通过迭代训练、生成和质量筛选，将代码执行与长推理模型结合，产生170万条工具集成推理解答；3）开发候选解答优选流程（GenSelect），显著优于多数投票基线。最终训练出的模型在数学推理基准测试中达到SOTA。作者开源了代码、模型和完整数据集（OpenMathReasoning），采用商业友好许可。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T16:02:04Z
- **目录日期**: 2025-04-24
