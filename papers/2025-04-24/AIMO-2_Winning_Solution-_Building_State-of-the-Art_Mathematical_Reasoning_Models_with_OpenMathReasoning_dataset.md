...
这篇论文介绍了赢得AI数学奥林匹克进步奖2（AIMO-2）的解决方案。其核心方法基于三大支柱：1）构建包含54万道高质量数学题（含奥赛题）和320万条详细解答的大规模数据集；2）开发新方法，通过迭代训练、生成和质量过滤，将代码执行与长推理模型结合，产生170万条工具集成推理解答；3）建立流程训练模型从多个候选解中选择最优解，证明生成式解法选择（GenSelect）显著优于多数投票基准。最终训练出的模型在数学推理基准测试中达到SOTA，并开源了代码、模型和OpenMathReasoning数据集。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T10:01:42Z
- **目录日期**: 2025-04-24
