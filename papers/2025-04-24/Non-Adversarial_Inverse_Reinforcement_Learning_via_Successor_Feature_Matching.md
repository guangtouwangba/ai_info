...
本文提出了一种新的逆向强化学习（IRL）方法，通过直接优化策略来避免传统对抗式训练的复杂性和不稳定性。该方法利用回报的线性分解（后继特征与奖励向量的内积），设计了一种基于策略梯度下降的IRL算法，直接最小化学者与专家特征之间的差距。这种非对抗式方法无需学习奖励函数，可无缝结合现有的actor-critic强化学习算法，并能仅通过状态观察（无需专家动作标签）进行学习，解决了行为克隆（BC）的局限。实验表明，该方法仅需单次专家演示即可学习，并在多种控制任务中表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T00:02:41Z
- **目录日期**: 2025-04-24
