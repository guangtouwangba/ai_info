...
微软亚洲研究院刘炜清团队研发的RD-Agent系统，通过大语言模型技术革新数据科学工作流程。该系统采用双Agent协作框架（Research Agent生成研究想法，Development Agent实现验证），实现从研发自动化到增强再到领域重塑的三阶段演进。其特色在于通过数据驱动突破专家模仿瓶颈，在特征生成等任务中达到80-90%可用性，已应用于论文复现和金融场景。该系统既提升科研效率，也探索了AI驱动科研的新范式，标志着数据科学正向智能化协作模式转型。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T22:03:00Z
- **目录日期**: 2025-04-24
