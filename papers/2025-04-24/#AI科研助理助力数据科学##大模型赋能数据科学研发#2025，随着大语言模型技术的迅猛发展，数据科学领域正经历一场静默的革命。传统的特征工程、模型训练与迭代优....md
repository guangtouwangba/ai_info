...
微软亚洲研究院刘炜清团队开发的RD-Agent系统通过三阶段演进（自动化→增强→重塑），采用双Agent协作框架（Research Agent生成研究想法+Development Agent实现验证），显著提升数据科研效率。系统通过记录实验数据突破模仿瓶颈，在特征生成等任务中实现80-90%可用性，已应用于金融论文复现等场景，展现了AI驱动科研流程变革的潜力。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T16:03:29Z
- **目录日期**: 2025-04-24
