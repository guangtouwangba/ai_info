...
微软亚洲研究院首席研究员刘炜清在第三届AIGC产业峰会上介绍了RD-Agent智能系统，该系统通过大语言模型技术革新数据科学研发流程。RD-Agent采用三阶段演进路线（自动化→增强→重塑）和双Agent协作框架（Research Agent生成研究想法，Development Agent实现验证），显著提升科研效率。系统通过数据驱动突破创新瓶颈，在特征生成等任务中实现80-90%可用性，已应用于金融领域论文复现等场景。该技术将高门槛重复性工作转化为AI助理职能，探索"AI驱动数据驱动AI"的新型科研范式。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T10:02:49Z
- **目录日期**: 2025-04-24
