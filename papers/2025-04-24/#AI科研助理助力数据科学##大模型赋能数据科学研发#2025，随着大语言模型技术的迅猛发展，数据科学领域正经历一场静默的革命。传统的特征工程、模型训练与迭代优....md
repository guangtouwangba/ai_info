...
微软亚洲研究院刘炜清团队开发了RD-Agent智能系统，通过三阶段演进（自动化→增强→重塑）革新数据科学研发流程。该系统采用双Agent架构：Research Agent生成研究想法，Development Agent实现验证，形成闭环迭代。其特色在于通过数据驱动突破模仿瓶颈，在特征生成等任务中实现80-90%可用性，已应用于论文复现等场景。该技术将大模型转化为"虚拟科研助理"，既提升研究效率（团队内部效率提升40%），又探索了AI驱动科研的新范式，展现了从工具到协作伙伴的演进路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T11:03:06Z
- **目录日期**: 2025-04-24
