...
检索增强生成（RAG）系统通过整合经过验证的上下文信息来提升大语言模型（LLM）的输出质量，但仍存在公平性和可靠性问题。研究发现，在幻觉出现的检索和生成阶段，定制化的警告信息虽能提高用户对高级幻觉的识别准确率，但也可能引发认知摩擦，导致困惑并降低系统信任度。这项研究以教育测验为场景，探讨了警告信息如何影响用户推理和行为，旨在推动AI辅助推理的发展——构建能主动支持人类反思、批判性思维和知情决策的系统，而非被动信息消费。初步结果表明需平衡警示效用与用户体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T03:17:20Z
- **目录日期**: 2025-04-24
