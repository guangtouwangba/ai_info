...
这篇论文针对代码生成大模型(codeLLMs)与人类偏好对齐的问题，提出了CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种编程语言)和大规模合成指令集SynCode-Instruct(近200亿标记)。研究发现：1)传统基于执行的评估与人类偏好存在偏差；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型除正确性外，还需关注与开发者实际需求的匹配度。实验验证了大规模合成指令微调的有效性，为提升代码LLM的实用性提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T15:05:57Z
- **目录日期**: 2025-04-24
