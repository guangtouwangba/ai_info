...
这篇论文提出了CodeArena基准测试和SynCode-Instruct数据集，用于评估代码生成大模型(CodeLLMs)与人类偏好的对齐程度。研究发现现有基准过于关注代码正确性而忽略人类偏好，因此构建了包含397个样本(覆盖40类任务和44种语言)的高质量基准。实验显示执行基准与CodeArena存在性能差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)差距显著，凸显对齐人类偏好的重要性。同时验证了大规模合成指令微调的有效性(近200亿标记)。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T20:05:38Z
- **目录日期**: 2025-04-24
