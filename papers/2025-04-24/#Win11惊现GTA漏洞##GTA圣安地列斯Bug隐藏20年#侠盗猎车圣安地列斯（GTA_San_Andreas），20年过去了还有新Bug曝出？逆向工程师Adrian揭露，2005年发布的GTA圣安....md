...
逆向工程师Adrian发现，经典游戏《GTA圣安地列斯》在Win11 24H2版本中出现水上飞机Skimmer消失的Bug。经调查发现，该飞机20年来一直存在类型定义错误（本应是船却被强制设为飞机），且缺失关键参数。此前因内存残留值（0.7）勉强运行，但Win11 24H2的堆栈更新覆盖了该值，导致叶片角度计算崩溃（出现3.73e+29异常值），引发游戏画面错乱。解决方法为手动在游戏文件中添加缺失的轮子比例参数（0.7）。这一发现揭示了长期隐藏的代码缺陷问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T14:03:27Z
- **目录日期**: 2025-04-24
