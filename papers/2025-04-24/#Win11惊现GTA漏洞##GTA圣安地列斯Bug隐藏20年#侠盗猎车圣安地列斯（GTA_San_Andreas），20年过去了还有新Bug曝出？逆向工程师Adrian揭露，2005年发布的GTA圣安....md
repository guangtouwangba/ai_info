...
逆向工程师发现《GTA圣安地列斯》在Win11 24H2版本出现隐藏20年的Bug：水上飞机Skimmer消失。经调查发现，该飞机因类型定义错误（本应是船却被强制设为飞机）且缺失关键参数，此前依赖内存残留值（0.7）勉强运行。Win11系统更新改变了内存布局，导致参数丢失引发异常（如3.73e+29的异常计算值）。玩家可通过手动修改游戏文件添加缺失参数修复。该案例揭示了经典游戏中因未初始化内存和类型定义混乱导致的长期隐患。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T11:03:16Z
- **目录日期**: 2025-04-24
