...
逆向工程师Adrian发现《GTA圣安地列斯》在Win11 24H2版本中出现Skimmer水上飞机消失的Bug。调查发现，该飞机20年来因类型定义错误（本应是船却被强制改为飞机）导致内存未初始化。此前Windows版本因内存残留值（0.7）勉强运行，但Win11 24H2更新改变了堆栈布局，暴露了这一问题。玩家可通过手动编辑游戏文件添加轮子参数修复。该事件揭示了经典游戏中长期隐藏的代码缺陷，凸显了严格验证输入数据的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T18:03:03Z
- **目录日期**: 2025-04-24
