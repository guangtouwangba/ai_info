...
MR. Video是一个基于MapReduce原理的长视频理解框架，通过两个步骤处理视频：(1) Map阶段：独立密集感知短视频片段；(2) Reduce阶段：联合聚合所有片段信息。相比序列模型，它能突破上下文长度限制；相比依赖关键片段选择的视频代理，其并行感知更简单高效。框架包含两个MapReduce阶段：首先生成片段字幕并统一命名（Captioning），再针对用户问题分析片段信息并整合答案（Analysis）。在LVBench测试中，MR. Video比现有最优模型准确率提升超10%。该方法适用于视觉语言模型和视频代理，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T00:01:08Z
- **目录日期**: 2025-04-24
