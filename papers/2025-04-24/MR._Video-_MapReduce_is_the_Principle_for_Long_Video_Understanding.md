...
MR. Video是一个基于MapReduce原则的长视频理解框架，通过"Map"独立感知短视频片段和"Reduce"聚合所有片段信息。相比序列模型，它能更细致地处理视频而不受上下文长度限制；相比传统视频代理，其并行感知更高效，且通过聚合实现更全面的推理。该框架采用两阶段处理：(1) 字幕生成：先为片段生成描述，再统一重复对象名称；(2) 分析：针对问题提取片段相关信息并整合答案。在LVBench测试中，MR. Video比现有最优模型准确率提升超过10%。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T01:28:27Z
- **目录日期**: 2025-04-24
