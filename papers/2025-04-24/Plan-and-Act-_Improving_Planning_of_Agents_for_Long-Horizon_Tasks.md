...
大型语言模型（LLMs）在简单任务中表现出色，但在复杂多步骤任务中仍面临挑战。为此，研究者提出"Plan-and-Act"框架，将高级规划与低级执行分离：Planner模型生成结构化高级计划，Executor模型将其转化为具体操作。为提升规划准确性，该框架采用创新的合成数据生成方法，通过标注真实轨迹数据并增强多样化示例来训练Planner模型。在网页导航任务测试中，该框架在WebArena-Lite基准上达到57.58%的成功率，在纯文本环境WebVoyager上达到81.36%的成功率，均创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T02:30:22Z
- **目录日期**: 2025-04-24
