...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过算法与硬件协同优化，采用动态分层稀疏策略：先粗粒度压缩令牌，再细粒度选择关键令牌，在保持全局上下文感知的同时确保局部精度。该方法显著提升了长文本建模效率，并提出了两项关键创新点，为大规模语言模型的高效训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-24T13:13:04Z
- **目录日期**: 2025-04-24
