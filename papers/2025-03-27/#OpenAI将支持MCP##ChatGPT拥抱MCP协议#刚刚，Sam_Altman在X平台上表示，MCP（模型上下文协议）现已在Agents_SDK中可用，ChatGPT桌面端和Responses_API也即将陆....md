# #OpenAI将支持MCP##ChatGPT拥抱MCP协议#刚刚，Sam Altman在X平台上表示，MCP（模型上下文协议）现已在Agents SDK中可用，ChatGPT桌面端和Responses API也即将陆...
...
OpenAI宣布将支持MCP（模型上下文协议），该协议由Anthropic推出，旨在标准化AI模型与外部数据的交互方式。MCP允许AI助手直接访问实时数据源，提升响应准确性。OpenAI将在ChatGPT桌面端和Responses API中集成MCP，开发者可更便捷地构建与数据源双向连接的AI应用。此前已有Replit等公司接入，OpenAI的加入进一步巩固了MCP作为AI集成开放标准的地位。Anthropic高管表示欢迎，称MCP已拥有数千个集成并持续增长。此举将助力AI助手更精准理解用户实时数据，提供更个性化服务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T13:10:29Z
- **目录日期**: 2025-03-27
