...
OpenAI宣布将在ChatGPT桌面端和Responses API中支持MCP（模型上下文协议）。MCP是Anthropic推出的开放标准，能标准化AI模型与外部数据的交互，被称为"AI界的USB-C"。该协议使AI能直接获取业务工具、内容库等实时数据，生成更精准响应。此前Replit等公司已接入，OpenAI的加入进一步巩固了MCP作为AI集成标准的地位。Anthropic表示MCP已有数千个集成且持续增长。这将使AI助手更好地理解用户实时数据，提供更贴心服务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T21:03:36Z
- **目录日期**: 2025-03-27
