...
OpenAI宣布将支持MCP（模型上下文协议），该协议由Anthropic推出，旨在标准化AI模型与外部数据的交互方式。MCP允许AI助手直接访问实时数据源，提升响应准确性。OpenAI将在ChatGPT桌面端和Responses API中集成MCP，开发者可更便捷构建双向连接AI应用。此前Replit等公司已接入MCP，OpenAI的加入进一步巩固了其作为AI集成标准的地位。Anthropic高管表示MCP已成为拥有数千集成的开放标准。这一举措预计将增强AI助手对实时数据的理解，提供更精准的服务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T10:04:07Z
- **目录日期**: 2025-03-27
