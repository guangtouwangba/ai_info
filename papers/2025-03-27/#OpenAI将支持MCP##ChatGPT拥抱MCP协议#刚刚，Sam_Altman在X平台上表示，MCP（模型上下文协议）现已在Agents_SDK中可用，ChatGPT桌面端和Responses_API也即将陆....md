...
OpenAI宣布将在ChatGPT桌面端和Responses API中支持MCP（模型上下文协议）。MCP是由Anthropic推出的开放标准，旨在标准化AI模型与外部数据的交互方式，被称为AI界的"USB-C"。该协议允许AI助手直接访问实时数据源，提升响应准确性。此前已有Replit等公司接入，OpenAI的加入进一步巩固了MCP作为AI集成标准的地位。Anthropic高管表示，MCP已成为拥有数千个集成的开放标准。这一举措将帮助开发者更便捷地构建连接数据源的AI应用，使AI助手能基于实时数据提供更精准的服务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T19:03:29Z
- **目录日期**: 2025-03-27
