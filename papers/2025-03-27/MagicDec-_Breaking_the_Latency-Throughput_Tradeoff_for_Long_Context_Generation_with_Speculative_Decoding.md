...
MagicDec研究发现，在大批量长文本推理场景下，推测解码(SD)技术仍能显著提升大语言模型(LLM)的推理效率。通过分析瓶颈转移规律，该方法采用稀疏KV缓存的草稿模型缓解KV瓶颈，并提出理论模型优化草稿策略。实验显示，在Llama3.1-8B模型上，针对中长文本(批量32-256)可实现最高2.51倍加速，且不损失准确性。该技术突破了传统认为SD仅适用于小批量的认知，为高吞吐量长文本服务提供了有效的低延迟解决方案，适用于聊天机器人、文档分析等应用场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T16:02:25Z
- **目录日期**: 2025-03-27
