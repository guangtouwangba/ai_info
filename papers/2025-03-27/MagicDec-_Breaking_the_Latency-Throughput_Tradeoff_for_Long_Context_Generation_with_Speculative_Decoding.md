...
MagicDec研究显示，大语言模型（LLMs）在长文本应用中，通过智能推测解码（SD）技术可显著提升高吞吐量场景下的推理速度。研究发现，随着批量增大，采用稀疏KV缓存的草稿模型能有效缓解KV瓶颈，且速度提升效果反而增强。通过理论模型优化策略选择，该方法在Llama3.1-8B上实现了最高2.51倍的加速（批量32-256），适用于各类硬件和任务，且不损失准确性。这一突破改变了"SD仅适用于小批量"的传统认知，为长文本服务提供了兼顾低延迟与高吞吐的新方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T19:02:05Z
- **目录日期**: 2025-03-27
