...
MagicDec研究表明，推测解码(SD)技术能显著提升大语言模型(LLMs)在长上下文场景下的推理效率。通过采用稀疏KV缓存和智能草拟策略，该方法有效解决了批量增大时的KV瓶颈问题，在保持准确性的同时实现了延迟降低和吞吐量提升。实验显示，对于中等至长序列(32-256批量)，Llama3.1-8B模型在不同硬件和任务上最高可获得2.51倍的加速效果。这一发现突破了传统认为SD仅适用于小批量的认知，为长上下文应用(如聊天机器人、文档分析)提供了高效推理方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T11:02:26Z
- **目录日期**: 2025-03-27
