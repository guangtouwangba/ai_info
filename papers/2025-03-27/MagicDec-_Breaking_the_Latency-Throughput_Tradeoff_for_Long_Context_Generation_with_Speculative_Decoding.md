...
MagicDec研究表明，推测解码(SD)技术能显著提升大语言模型(LLMs)在长上下文场景下的推理效率。与传统认知不同，该技术不仅适用于小批量处理，在中长序列的高吞吐量推理中也能实现加速。通过采用稀疏KV缓存缓解计算瓶颈，并结合智能草拟策略选择机制，系统在批量增大时反而能获得更好加速效果。实验显示，Llama3.1-8B模型在处理32-256批量范围的中长序列时，在不同硬件和任务上最高可实现2.51倍加速，且不影响准确性。这项工作拓展了推测解码在长上下文服务中的应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T08:02:29Z
- **目录日期**: 2025-03-27
