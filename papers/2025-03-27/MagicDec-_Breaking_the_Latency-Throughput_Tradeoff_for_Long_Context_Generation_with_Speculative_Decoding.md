...
MagicDec研究表明，在大批量长文本推理场景下，推测解码(SD)技术仍能显著提升性能。通过分析发现，随着批量增大，系统瓶颈会转移，而采用稀疏KV缓存的草稿模型可有效缓解KV瓶颈。研究提出理论模型来选择最优推测策略，在Llama3.1-8B模型上实现了最高2.51倍的加速效果，适用于32-256的批量范围，且不损失准确性。这项工作突破了传统认为SD仅适用于小批量的认知，证明其在长文本高吞吐场景同样有效，为交互式聊天机器人等应用提供了新的优化方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T04:02:05Z
- **目录日期**: 2025-03-27
