...
本研究首次系统评估了大型语言模型(LLM)在金融会计研究中的输出一致性和可复现性。通过对5类任务(分类、情感分析、摘要、文本生成和预测)进行50次独立实验，使用3个OpenAI模型生成340万条输出。结果显示：二元分类和情感分析近乎完美复现，复杂任务变异性较大；先进模型不一定更稳定；LLM一致性显著优于人类专家；3-5次运行聚合可显著提升稳定性；情感分析聚合还能提高新模型准确率；尽管存在不一致性，下游统计推断仍保持稳健。研究缓解了"G-hacking"(选择性报告有利结果)的担忧。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T04:01:56Z
- **目录日期**: 2025-03-27
