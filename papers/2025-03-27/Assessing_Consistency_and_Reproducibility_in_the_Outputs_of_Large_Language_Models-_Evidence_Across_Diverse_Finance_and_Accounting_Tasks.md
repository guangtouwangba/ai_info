...
该研究首次全面评估了大型语言模型(LLM)在金融会计研究中的输出一致性与可重复性。通过对5类常见任务(分类、情感分析、摘要、文本生成和预测)进行50次独立实验，使用3种OpenAI模型生成340万条输出。结果显示：二元分类和情感分析近乎完美可重复，复杂任务变异性更大；高级模型未必表现更好；LLM一致性显著优于人类专家；3-5次运行的简单聚合可显著提升一致性，新模型情感分析还能提高准确率；尽管存在不一致性，下游统计推断仍保持稳健，降低了"G-hacking"风险。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T08:02:19Z
- **目录日期**: 2025-03-27
