...
本研究首次全面评估了大型语言模型（LLM）在金融与会计研究中的输出一致性与可复现性。通过50次独立实验测试5类任务（分类、情感分析、摘要、文本生成和预测），使用3个OpenAI模型生成340万条输出。结果显示：二元分类和情感分析近乎完美复现，复杂任务变异性较大；高级模型未必更稳定；LLM一致性显著优于人类专家；3-5次结果聚合可显著提升稳定性，新模型情感分析还能提高准确率。尽管存在输出波动，下游统计推断仍保持稳健，证明金融领域"G-hacking"（选择性报告有利结果）风险较低。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T09:02:18Z
- **目录日期**: 2025-03-27
