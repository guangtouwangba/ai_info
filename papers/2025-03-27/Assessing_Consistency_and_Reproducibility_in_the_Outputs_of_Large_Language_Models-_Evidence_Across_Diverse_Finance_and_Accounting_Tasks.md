...
本研究首次系统评估了大型语言模型(LLM)在金融会计研究中的输出一致性和可重复性。通过对5类常见任务(分类、情感分析、摘要、文本生成和预测)进行50次独立实验，使用3种OpenAI模型生成340万条输出。结果显示：二元分类和情感分析近乎完美可重复，复杂任务变异性更大；高级模型未必更稳定；LLM一致性显著优于人类专家；3-5次运行的简单聚合可大幅提升稳定性，新模型情感分析准确性也有所改善。尽管存在输出不一致，下游统计推断仍保持稳健，"G-hacking"风险相对较低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T12:02:24Z
- **目录日期**: 2025-03-27
