...
Feature4X是一个通用框架，能够将2D视觉基础模型的功能扩展到4D领域，仅需单目视频输入。它通过动态优化策略整合多种模型能力，并首次利用高斯泼溅技术将视频基础模型（如SAM2、InternVideo2）的特征提取为显式4D特征场。该框架支持新视角分割、场景几何与外观编辑，以及跨时间步的自由形式视觉问答（VQA），结合大语言模型（LLM）实现反馈循环。Feature4X为智能体AI应用提供了可扩展、时空感知的动态4D场景交互基础，解决了现有3D/4D数据不足的挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T23:01:11Z
- **目录日期**: 2025-03-27
