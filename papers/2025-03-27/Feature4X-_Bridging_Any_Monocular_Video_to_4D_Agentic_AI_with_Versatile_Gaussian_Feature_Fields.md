...
Feature4X是一个通用框架，可将2D视觉基础模型的功能扩展到4D领域，仅需单目视频输入。它通过动态优化策略将多模型能力统一为单一表示，并首次利用高斯泼溅技术将视频基础模型（如SAM2、InternVideo2）的特征提取为显式4D特征场。该框架支持新视角分割、场景几何与外观编辑，以及基于LLM反馈的跨时间自由问答，增强了动态4D场景的交互能力。Feature4X解决了3D/4D数据稀缺的挑战，为构建可扩展、时空感知的智能体系统奠定了基础，推动了沉浸式场景交互的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T03:14:33Z
- **目录日期**: 2025-03-27
