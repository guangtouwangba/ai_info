...
Feature4X是一个通用框架，旨在将2D视觉基础模型的功能扩展到4D领域，仅需单目视频输入。它通过动态优化策略和多模型能力统一，将视频基础模型（如SAM2、InternVideo2）的特征提取并提升为显式4D高斯泼溅特征场。该框架支持开放词汇分割、语言引导编辑、视觉问答（VQA）等任务，并首次实现了跨时间步的全新视角分割、场景几何与外观编辑以及自由形式VQA。Feature4X为智能体AI应用提供了可扩展、时空感知的动态4D场景交互基础，推动了沉浸式多模态交互的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T13:07:36Z
- **目录日期**: 2025-03-27
