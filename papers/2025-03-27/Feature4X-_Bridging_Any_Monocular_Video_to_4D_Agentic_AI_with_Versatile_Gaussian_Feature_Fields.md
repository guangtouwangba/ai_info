...
Feature4X是一个通用框架，能将2D视觉基础模型的功能扩展到4D领域，仅需单目视频输入。它通过动态优化策略整合多模型能力，并首次利用高斯泼溅技术将视频基础模型（如SAM2、InternVideo2）的特征提取为显式4D特征场。该框架支持开放词汇分割、语言引导编辑、视觉问答（VQA）等任务，并实现跨时间步的新视角分割、场景编辑和自由交互。Feature4X为智能体AI应用提供了可扩展、时空感知的动态4D场景交互基础，推动了沉浸式多模态系统的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T11:01:15Z
- **目录日期**: 2025-03-27
