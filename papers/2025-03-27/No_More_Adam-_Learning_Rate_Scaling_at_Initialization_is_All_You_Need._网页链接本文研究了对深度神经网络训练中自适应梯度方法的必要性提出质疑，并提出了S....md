...
这篇论文提出了SGD-SaI方法，质疑深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是基于带动量的随机梯度下降（SGDM）的改进，通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免训练不平衡，同时将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，且对超参数更鲁棒。在内存效率上，相比AdamW，SGD-SAi为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，并在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T21:04:14Z
- **目录日期**: 2025-03-27
