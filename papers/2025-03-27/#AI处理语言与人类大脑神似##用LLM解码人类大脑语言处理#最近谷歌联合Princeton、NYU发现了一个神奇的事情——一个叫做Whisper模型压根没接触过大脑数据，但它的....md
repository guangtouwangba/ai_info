...
谷歌与普林斯顿、NYU联合研究发现，Whisper语音模型的内部处理机制与人类大脑语言处理路径高度吻合。模型未接触大脑数据，但其语音embedding能精准预测大脑听觉区（STG）对声音信号的反应，语言embedding则对应语义理解阶段（IFG激活）。说话时，模型与大脑均遵循"语义构思→运动控制→听觉反馈"的流程，时间节点和区域激活顺序完全同步。研究表明AI与人脑在语言处理底层逻辑（如预测机制、上下文表征）存在本质相似性，这一发现可能推动脑机接口和认知科学研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T06:04:23Z
- **目录日期**: 2025-03-27
