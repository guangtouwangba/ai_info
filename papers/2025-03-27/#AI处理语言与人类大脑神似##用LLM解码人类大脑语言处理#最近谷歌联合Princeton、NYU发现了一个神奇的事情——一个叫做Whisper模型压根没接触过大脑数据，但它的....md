...
谷歌联合普林斯顿和纽约大学研究发现，Whisper语音模型的内部处理机制与人类大脑处理语言的过程惊人相似。该模型未接触过大脑数据，但其语音和语言嵌入能精准预测大脑不同区域（如听觉区STG、语义区IFG、运动皮层MC）的激活顺序和时间点。大脑在说话时遵循IFG→MC→STG路径，听话时反向进行，而Whisper的嵌入层在各阶段均与神经活动高度同步。研究表明，AI与大脑可能共享预测性计算逻辑，这一发现或为脑机接口和认知科学研究提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T00:03:16Z
- **目录日期**: 2025-03-27
