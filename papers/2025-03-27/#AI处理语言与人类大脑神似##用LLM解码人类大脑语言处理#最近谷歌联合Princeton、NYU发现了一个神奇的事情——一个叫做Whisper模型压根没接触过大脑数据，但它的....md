...
谷歌与普林斯顿、纽约大学合作研究发现，OpenAI的Whisper语音模型虽未接触大脑数据，其处理语音的机制却与人类大脑高度吻合。研究显示：1）模型语音嵌入层对应大脑听觉区（STG）的声音处理；2）语言嵌入层同步于语义理解区（IFG）的激活；3）语言生成时模型与大脑均先构思内容再转换发音指令（IFG→运动皮层）；4）自我监听阶段两者均再度激活听觉区。这种时空维度的精准对齐表明，AI与大脑可能共享类似的语言计算逻辑，为脑机接口和认知研究开辟新路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T03:33:27Z
- **目录日期**: 2025-03-27
