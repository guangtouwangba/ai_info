...
谷歌与普林斯顿、NYU联合研究发现，Whisper语音模型虽未接触大脑数据，但其处理语音的层级结构与人类大脑高度吻合。模型各阶段（声音编码、语义理解、语言生成、运动执行、自我监听）的嵌入表示能精准预测对应脑区（STG、IFG、MC）的激活顺序与时间点，呈现"听话时STG→IFG，说话时IFG→MC→STG"的双向路径。研究表明AI与大脑在预测性编码、上下文表征等底层机制存在共性，为脑机接口和认知科学研究提供了新方向，暗示AI可能成为解码人类思维的"翻译官"。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T01:30:33Z
- **目录日期**: 2025-03-27
