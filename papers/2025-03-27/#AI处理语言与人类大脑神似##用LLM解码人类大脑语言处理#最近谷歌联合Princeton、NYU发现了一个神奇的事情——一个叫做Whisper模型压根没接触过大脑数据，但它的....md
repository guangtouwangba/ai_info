...
谷歌联合普林斯顿和纽约大学研究发现，Whisper语音模型的内部处理机制与人类大脑处理语言的神经路径高度吻合。模型各层嵌入能精准预测大脑不同区域（如听觉区STG、语言区IFG、运动皮层MC）的激活时序：听语音时先处理声音信号再理解语义，说话时先构思内容再执行发音，与大脑"IFG→MC→STG"的正反向路径完全同步。这表明AI与大脑在语言处理的底层计算逻辑上存在本质共性，为脑机接口和认知科学研究提供了新方向。该成果发表于谷歌研究博客。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T04:03:16Z
- **目录日期**: 2025-03-27
