...
该研究评估并提升大语言模型（LLM）在网页操作生成任务中的客观"准确性"，而非主观"可信度"。基于真实网购行为数据，首次对DeepSeek-R1、Llama等主流LLM进行了全面定量评估。结果显示：相比仅用提示词方法，基于真实行为数据的微调显著提升了模型的操作生成能力；加入合成推理轨迹可进一步提升性能，证明显式逻辑在行为建模中的价值。该研究为LLM行为模拟设立了新基准，并揭示了真实行为数据和推理增强对提升LLM代理真实性的作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T04:01:28Z
- **目录日期**: 2025-03-27
