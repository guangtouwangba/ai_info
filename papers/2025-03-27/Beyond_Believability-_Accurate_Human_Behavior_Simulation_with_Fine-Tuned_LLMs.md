...
最新研究评估了大语言模型（LLM）在网页行为生成任务中的客观准确性，而非主观可信度。基于真实网购行为数据，研究发现微调LLM（如DeepSeek-R1、Llama和Claude）能显著提升其行为生成能力，优于仅依赖提示词的方法。此外，在训练中加入合成推理轨迹可进一步提高性能，表明显式逻辑推理对行为建模的价值。该研究为LLM行为模拟设立了新基准，并证明真实行为数据和推理增强能有效提升LLM智能体的行为保真度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T18:01:39Z
- **目录日期**: 2025-03-27
