...
近期研究表明，大语言模型（LLM）能通过提示方法模拟可信的人类行为。本研究基于真实网购行为数据集，重点评估并提升LLM在网页操作生成任务中的客观准确性（而非主观可信度）。实验对DeepSeek-R1、Llama和Claude等先进模型进行了首个全面量化评估，发现：1）基于真实行为数据的微调显著优于纯提示方法；2）加入合成推理轨迹可进一步提升性能。该研究建立了行为模拟的新基准，证实真实行为数据和推理增强能有效提高LLM代理的拟真度。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T15:01:50Z
- **目录日期**: 2025-03-27
