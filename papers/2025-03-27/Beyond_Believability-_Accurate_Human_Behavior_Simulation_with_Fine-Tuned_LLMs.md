...
该研究评估并提升大语言模型(LLM)在网页操作生成任务中的客观准确性，而非主观可信度。基于真实网购行为数据，首次对DeepSeek-R1、Llama和Claude等先进模型进行全面定量评估。结果显示：1) 对真实行为数据进行微调显著优于仅用提示词的方法；2) 加入合成推理轨迹可进一步提升性能，表明显式推理对行为建模的价值。研究为LLM行为模拟建立了新基准，证实真实行为数据和推理增强能有效提高LLM代理的保真度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T14:01:44Z
- **目录日期**: 2025-03-27
