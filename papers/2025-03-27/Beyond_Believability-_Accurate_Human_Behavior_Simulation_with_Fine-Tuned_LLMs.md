...
最新研究表明，大语言模型（LLM）可以通过纯提示方法模拟可信的人类行为来驱动智能体。本研究基于真实网购行为数据集，重点评估并提升LLM在网页操作生成任务中的客观"准确性"而非主观"可信度"。通过对DeepSeek-R1、Llama和Claude等前沿模型的首次全面量化评估发现：基于真实行为数据的微调显著优于纯提示方法；在训练中加入合成推理轨迹能进一步提升性能，验证了显式逻辑在行为建模中的价值。该研究为LLM行为模拟建立了新基准，揭示了真实行为数据和推理增强对提升智能体保真度的作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T10:01:52Z
- **目录日期**: 2025-03-27
