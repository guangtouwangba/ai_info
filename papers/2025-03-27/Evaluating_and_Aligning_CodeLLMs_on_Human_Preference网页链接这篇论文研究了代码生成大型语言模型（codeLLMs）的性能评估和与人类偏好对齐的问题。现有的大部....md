...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1)创建CodeArena基准测试，包含397个高质量编程任务样本，覆盖40类任务和44种语言，专门评估模型输出与人类偏好的匹配度；2)构建近200亿标记的合成指令数据集SynCode-Instruct用于微调。研究发现现有基于执行的评估与人类偏好存在差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面存在显著差距，凸显了优化方向的重要性。研究通过系统实验验证了大规模合成指令微调的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T06:06:08Z
- **目录日期**: 2025-03-27
