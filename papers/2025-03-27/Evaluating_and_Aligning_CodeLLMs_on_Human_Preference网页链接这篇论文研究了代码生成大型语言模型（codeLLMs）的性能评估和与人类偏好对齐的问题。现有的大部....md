...
这篇论文针对代码生成大模型(codeLLMs)提出新评估方法，指出当前基准测试过于关注代码正确性而忽视人类偏好。研究者创建了CodeArena基准测试(含397个样本，覆盖40类/44种语言)和SynCode-Instruct指令数据集(20B标记)，发现执行基准与人类偏好存在差异。实验比较40多个模型显示，开源顶级模型(如Qwen2.5-Coder)与专有模型(如OpenAI o1)性能差距显著，凸显了与人类偏好对齐的重要性。研究证实大规模合成指令微调的有效性，为代码LLM评估提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T07:12:59Z
- **目录日期**: 2025-03-27
