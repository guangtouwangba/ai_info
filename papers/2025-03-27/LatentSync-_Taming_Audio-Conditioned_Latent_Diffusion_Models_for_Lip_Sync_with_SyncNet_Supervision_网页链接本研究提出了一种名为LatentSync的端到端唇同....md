...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型的时间表示增强帧间一致性。同时，研究系统分析了SyncNet的收敛问题，改进了其训练框架，显著提升了准确性。该方法在HDTF和VoxCeleb2数据集上表现优异，超越了现有唇同步技术。这些改进不仅适用于本框架，还可推广至其他依赖SyncNet的音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T11:05:49Z
- **目录日期**: 2025-03-27
