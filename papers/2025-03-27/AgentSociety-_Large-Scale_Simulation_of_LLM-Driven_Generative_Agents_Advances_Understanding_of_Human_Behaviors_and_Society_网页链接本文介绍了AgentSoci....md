...
该研究提出AgentSociety——一个结合大语言模型代理、真实社会环境和大规模模拟引擎的社会模拟平台。该系统成功模拟了1万多个代理的社会生活及500万次互动，并针对四大社会问题（社会极化、信息传播、基本收入政策、自然灾害影响）进行实验验证。结果显示其模拟结果与现实世界数据高度吻合，证明该系统能有效捕捉人类行为机制，具备作为社会科学研究和政策制定工具的潜力。该平台为传统研究方法提供了重要补充，有助于深入理解复杂社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T00:05:21Z
- **目录日期**: 2025-03-27
