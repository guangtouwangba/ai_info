...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个智能代理及其500万次社会互动。该系统通过四个关键社会问题的研究（社会极化、信息传播、基本收入政策、自然灾害影响），验证了其模拟结果与现实世界数据的一致性。研究表明，AgentSociety能有效捕捉人类行为机制，为社会科学研究和政策制定提供了新的计算实验平台，可辅助传统研究方法（如调查、访谈）深入分析社会问题的模式和成因。这一技术展现了LLM在社会模拟领域的应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T08:05:25Z
- **目录日期**: 2025-03-27
