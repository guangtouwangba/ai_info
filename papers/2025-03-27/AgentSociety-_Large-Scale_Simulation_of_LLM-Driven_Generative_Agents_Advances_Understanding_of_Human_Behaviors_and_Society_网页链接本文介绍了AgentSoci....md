...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个代理的社会互动（500万次）。它通过研究极化现象、信息传播、基本收入政策和外部冲击（如飓风）等社会问题，验证了其模拟结果与现实世界数据的一致性。该系统可作为计算社会科学实验平台，帮助研究者分析社会问题的模式和机制，为政策制定提供支持。实验表明，AgentSociety能有效捕捉人类行为特征，展现了作为社会科学研究工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T19:04:42Z
- **目录日期**: 2025-03-27
