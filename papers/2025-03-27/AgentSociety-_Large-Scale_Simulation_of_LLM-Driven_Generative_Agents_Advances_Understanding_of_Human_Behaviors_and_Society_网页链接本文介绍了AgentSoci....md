...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟10,000多个代理的社会生活及500万次互动。研究者用它分析了四个关键社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界数据吻合，验证了其捕捉人类行为和社会机制的能力。该系统可作为社会科学研究和政策制定的有力工具，补充传统方法（如调查和实验），帮助深入理解社会问题的模式和成因。这一创新为计算社会科学提供了新的研究平台。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-27T14:05:25Z
- **目录日期**: 2025-03-27
