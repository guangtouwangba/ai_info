...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关联，无需中间运动表示。针对扩散模型的时间一致性问题，提出了时间表示对齐(TREPA)技术，利用自监督视频模型增强生成帧的时序连贯性。同时，研究发现了SyncNet的收敛问题，通过优化模型架构、训练参数和数据预处理显著提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进也可应用于其他音频驱动的人像动画系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T14:06:13Z
- **目录日期**: 2025-04-19
