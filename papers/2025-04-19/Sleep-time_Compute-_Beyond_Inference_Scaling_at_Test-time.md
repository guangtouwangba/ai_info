...
研究人员提出"睡眠时间计算"方法，通过让大语言模型在查询前预先处理上下文并预测可能的问题，显著降低实时计算需求。该方法在两个推理任务(Stateful GSM-Symbolic和Stateful AIME)中，将实现相同准确率所需的实时计算量减少约5倍，同时通过增加睡眠时间计算可将准确率提升13%-18%。在多查询场景下，平均每查询成本降低2.5倍。研究发现用户查询的可预测性与该方法效果高度相关。最后，该方法在一个实际软件工程任务中进行了验证，展示了其应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T04:00:53Z
- **目录日期**: 2025-04-19
