...
本文提出"睡眠时间计算"方法，通过让大语言模型在查询前预先处理上下文并预测可能的用户问题，显著降低实际查询时的计算需求。实验表明，该方法在Stateful GSM-Symbolic和Stateful AIME两个推理任务上能减少约5倍实时计算量，同时提升准确率13%-18%。通过Multi-Query GSM-Symbolic任务分摊计算成本，单查询平均成本降低2.5倍。研究发现用户查询的可预测性与该方法效果正相关。最后通过软件工程代理任务验证了该方法的实用性，有效平衡了计算延迟与推理性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T20:01:07Z
- **目录日期**: 2025-04-19
