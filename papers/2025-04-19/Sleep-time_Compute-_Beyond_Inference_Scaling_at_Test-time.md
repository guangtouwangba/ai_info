...
该研究提出"睡眠时间计算"方法，通过让大语言模型在查询前预先处理上下文并预测可能的问题，显著降低实时计算需求。在两个推理任务(Stateful GSM-Symbolic和Stateful AIME)上的实验表明，该方法能减少约5倍实时计算量，同时提升准确率13-18%。研究还开发了Multi-Query GSM-Symbolic任务，通过分摊相关查询的预处理成本，使单次查询平均成本降低2.5倍。分析发现查询可预测性与方法效果正相关。最后通过软件工程代理任务验证了该方法的实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T15:01:01Z
- **目录日期**: 2025-04-19
