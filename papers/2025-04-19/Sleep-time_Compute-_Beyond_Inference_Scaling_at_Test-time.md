...
本文提出"睡眠时间计算"方法，通过让大语言模型在查询前预先处理上下文并预测用户可能的问题，显著降低实时计算需求。实验表明，该方法在Stateful GSM-Symbolic和Stateful AIME任务中可减少约5倍实时计算量，同时提升准确率13%和18%。通过Multi-Query GSM-Symbolic任务分摊计算成本，单查询平均成本降低2.5倍。研究发现查询可预测性与方法效果正相关，并在软件工程代理任务中验证了其实际应用价值。该方法有效平衡了计算延迟、成本与模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T07:00:55Z
- **目录日期**: 2025-04-19
