...
自监督学习（SSL）是当前AI系统的核心技术，其设计空间不断扩展。根据柏拉图表示假说（PRH），不同方法最终会收敛到相同的理想表示，但缺乏理论解释。通过可辨识性理论（IT）分析发现PRH可能在SSL中出现，但现有IT无法完全解释SSL的实际成功。为此，研究者提出扩展的**奇异可辨识性理论（SITh）**，以覆盖整个SSL流程，帮助理解隐含的数据假设，推动学习更具可解释性和泛化性的表示。未来研究方向包括：1）SSL训练动态与收敛性；2）有限样本、批量和数据多样性的影响；3）架构、数据增强及优化器中的归纳偏置作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T03:11:29Z
- **目录日期**: 2025-04-19
