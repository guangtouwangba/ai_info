...
自监督学习（SSL）是当前AI系统的核心技术，其设计空间不断扩展。根据柏拉图表示假说（PRH），不同方法学到的表示会收敛到同一理想形式，但缺乏理论解释。研究表明，可识别性理论（IT）可部分解释PRH现象，但无法完全说明SSL的实际成功。为此，作者提出"奇异可识别性理论"（SITh），作为涵盖整个SSL流程的扩展理论框架，以更深入理解数据假设并推动学习更具解释性和泛化性的表示。未来研究需关注三个方向：1）SSL训练动态与收敛性；2）有限样本、批量和数据多样性的影响；3）架构、增强、初始化和优化器中的归纳偏置作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T23:01:41Z
- **目录日期**: 2025-04-19
