...
复旦大学与字节Seed团队合作开发了SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数即可生成1024分辨率图像。该模型通过统一处理文本和图像token实现跨模态对齐，无需外挂文本编码器。在GenEval评测中得分0.59，超越同规模SDv2.1和LlamaGen。配合vLLM部署后生成仅需13.55秒，效率优于部分现有方案。研究指出当前性能差距主要源于训练数据和视觉tokenizer限制，仍有优化空间。这项工作证明了自回归模型在图像生成领域的持续潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T14:03:12Z
- **目录日期**: 2025-04-19
