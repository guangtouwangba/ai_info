...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，在仅0.5B参数规模下实现了1024分辨率图像生成。该模型通过统一处理文本和图像token实现跨模态对齐，无需外挂文本编码器。在GenEval评测中取得0.59分，超越同规模SDv2.1和LlamaGen。经vLLM优化后生成1024图像仅需13.55秒，在效率上优于部分扩散模型。研究指出当前性能瓶颈主要来自训练数据和视觉tokenizer限制，表明自回归方法在图像生成领域仍具发展潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T13:07:09Z
- **目录日期**: 2025-04-19
