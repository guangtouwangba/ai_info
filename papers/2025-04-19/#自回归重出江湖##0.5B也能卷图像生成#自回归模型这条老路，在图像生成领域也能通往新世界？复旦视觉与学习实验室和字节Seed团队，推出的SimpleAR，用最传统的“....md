...
复旦大学与字节Seed团队联合开发的SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数即可生成1024分辨率图像，在GenEval评测中获得0.59高分，超越同规模SDv2.1和LlamaGen。该模型通过统一处理文本/图像token实现跨模态对齐，无需外挂文本编码器，配合vLLM部署后生成仅需13.55秒。研究指出当前1.5B规模性能差距主要源于训练数据和视觉分词器限制，但已展现出比MaskGIT更优的速度-质量平衡。这项工作验证了自回归范式在图像生成领域的持续潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T15:02:58Z
- **目录日期**: 2025-04-19
