...
在差分隐私的序贯学习框架中，代理通过引入内生噪声保护隐私。针对连续信号，本文提出基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声方法，能更有效结合私有信号与历史观测，将渐近学习速度提升至Θₑ(log(n))（隐私预算有限时），优于非隐私场景的Θ(√log(n)）。有限隐私预算还能避免早期代理长期错误决策，使首次正确决策时间和错误次数保持有限。连续信号下隐私学习更高效，而二元信号则因固定随机响应策略易形成信息级联，阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T02:27:07Z
- **目录日期**: 2025-04-19
