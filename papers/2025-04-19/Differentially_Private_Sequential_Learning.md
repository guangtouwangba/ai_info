...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。连续信号下，本文提出的平滑随机响应机制根据阈值距离自适应添加噪声，相比传统均匀噪声方法，能更快达到$\Theta_{\epsilon}(\log(n))$的渐近学习速度（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算$\epsilon \in (0,1)$时，首个正确决策的停止时间和错误动作次数均为有限，显著优于非隐私场景的无限发散。连续信号下隐私学习更高效，因平滑机制能增强对数似然比；而二元信号中隐私噪声会阻碍学习，因代理在信息级联前倾向使用恒定随机响应策略，降低动作信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T08:02:18Z
- **目录日期**: 2025-04-19
