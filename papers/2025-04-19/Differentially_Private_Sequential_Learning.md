...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。针对连续信号，本文提出基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声方法，能更高效融合私有信号与历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算$\epsilon \in (0,1)$下，首个正确决策的期望停止时间和错误动作次数均为有限值，显著优于非隐私场景的无限发散特性。然而，对于二元信号，隐私噪声会阻碍学习，因代理在信息级联形成前倾向于恒定随机响应策略，降低动作信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T16:02:18Z
- **目录日期**: 2025-04-19
