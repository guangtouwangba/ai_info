...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。针对连续信号，研究提出了一种基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声方法，能更高效结合私有信号与历史观察，将渐近学习速度提升至Θₑ(log(n))（非隐私场景为Θ(√log(n))）。有限隐私预算ϵ∈(0,1)下，首次正确决策的停止时间和错误动作次数均保持有限，显著优于非隐私场景的无限发散特性。连续信号通过增强对数似然比实现更高效学习，而二元信号则因固定随机响应策略易形成信息级联，阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T09:02:12Z
- **目录日期**: 2025-04-19
