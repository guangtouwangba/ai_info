...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。对于连续信号，本文提出基于阈值距离的自适应平滑随机响应机制（传统方法使用均匀噪声），使行动更准确反映私有信号和历史观测，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算$\epsilon \in (0,1)$时，首个正确决策的停止时间和错误行动次数均有限，而完全非隐私场景下二者发散。连续信号下隐私学习更高效，但二元信号中隐私噪声会阻碍学习，因代理在信息级联前倾向使用恒定随机响应策略，降低行动信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T18:02:12Z
- **目录日期**: 2025-04-19
