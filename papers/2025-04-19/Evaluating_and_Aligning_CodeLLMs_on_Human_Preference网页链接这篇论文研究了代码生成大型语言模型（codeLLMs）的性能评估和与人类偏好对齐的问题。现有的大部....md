...
该论文针对代码生成大模型（codeLLMs）提出新评估方法，重点关注人类偏好对齐问题。研究者构建了CodeArena基准测试（含397个跨40类别、44种编程语言的高质量样本）和SynCode-Instruct合成指令数据集（20B标记）。实验发现：1）传统执行基准与人类偏好存在差异；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调了在代码生成任务中，除正确性外，符合人类偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T14:06:35Z
- **目录日期**: 2025-04-19
