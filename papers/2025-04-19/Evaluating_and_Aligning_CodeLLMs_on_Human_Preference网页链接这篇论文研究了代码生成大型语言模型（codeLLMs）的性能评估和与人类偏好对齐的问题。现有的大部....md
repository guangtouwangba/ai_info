...
该论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，通过397个高质量样本(覆盖40类/44种语言)评估模型与人类偏好的匹配度，弥补现有基准仅关注代码正确性的不足。研究同时构建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现执行基准与CodeArena存在性能差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)表现差距显著，凸显对齐人类偏好的重要性。研究强调需在代码生成任务中兼顾功能正确性与用户偏好匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T06:06:14Z
- **目录日期**: 2025-04-19
