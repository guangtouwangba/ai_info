...
当前基于LLM的自动程序修复(APR)主要集中于主流语言(如Java/Python)，对新兴语言(如Rust)因资源限制缺乏支持。为此，研究者提出跨语言修复方法LANTERN，通过多智能体迭代范式，将缺陷代码从LLM修复能力弱的语言翻译至表现更强的语言，无需额外训练。该方法采用基于LLM的动态决策系统，根据错误特征选择最优目标语言并整合修复反馈。在包含11种语言5068个错误的xCodeEval基准测试中，Rust的Pass@10指标提升22.09%，首次证明跨语言翻译能显著扩展LLM修复能力并缩小语言间性能差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T14:02:16Z
- **目录日期**: 2025-04-19
