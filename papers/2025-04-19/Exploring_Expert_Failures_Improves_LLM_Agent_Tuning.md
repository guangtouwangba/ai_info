...
研究人员提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)的代理能力。传统方法RFT仅利用专家成功轨迹进行微调，而EEF创新性地从专家失败轨迹中提取有价值的行为(如关键计划和动作)，同时排除有害行为。实验表明，EEF能解决RFT无法处理的复杂子任务，显著提升模型性能。在WebShop测试中，EEF以62%的胜率超越RFT(53.6%)和GPT-4(35.6%)，并创下WebShop(>0.81分)和SciWorld(>81分)的新纪录。该方法通过挖掘失败经验中的有效信息，突破了现有方法的性能瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T06:01:26Z
- **目录日期**: 2025-04-19
