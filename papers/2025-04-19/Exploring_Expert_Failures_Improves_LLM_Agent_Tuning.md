...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)在复杂子任务中的失败轨迹，提取有价值的行动指导(如关键计划和动作)，并剔除有害动作以避免干扰。相比仅模仿成功轨迹的拒绝采样微调(RFT)，EEF能更高效地获取关键技能，解决RFT难以处理的分布外任务。在WebShop测试中，EEF以62%胜率显著超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop(>0.81)和SciWorld(>81)测试中创下新纪录，成为首个突破这两个基准最高分的算法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T00:01:29Z
- **目录日期**: 2025-04-19
