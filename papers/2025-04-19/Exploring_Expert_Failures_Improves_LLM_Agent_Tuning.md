...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLMs)的代理能力。传统方法RFT仅利用专家成功的轨迹进行微调，而EEF创新性地从专家失败的轨迹中提取有价值的行动指导(如计划和关键动作)，同时排除有害动作。这种方法显著提高了模型在复杂子任务上的表现，在WebShop任务中达到62%的胜率，优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下了超过81分的新纪录。EEF通过利用失败经验解决了传统方法难以处理的分布外任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T02:26:15Z
- **目录日期**: 2025-04-19
