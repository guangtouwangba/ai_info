...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLMs)作为智能体的性能。研究发现，专家(如GPT-4)在复杂子任务中失败的轨迹往往包含有价值的指导信息。EEF通过从失败的专家轨迹中提取有益动作(如计划和关键操作)并整合到训练数据中，同时排除有害动作，显著提高了智能体的探索效率和关键技能获取能力。实验表明，EEF在WebShop任务中达到62%的胜率，超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下了新的最先进水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T03:11:09Z
- **目录日期**: 2025-04-19
