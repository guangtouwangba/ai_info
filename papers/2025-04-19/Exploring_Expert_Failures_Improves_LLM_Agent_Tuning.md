...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLMs)作为智能代理的性能。传统拒绝采样微调(RFT)方法主要依赖专家成功的简单任务轨迹，而EEF创新性地从专家失败的轨迹中提取有价值的行动指导(如计划和关键动作)，同时排除有害动作以避免干扰模型学习。实验表明，EEF成功解决了RFT无法处理的复杂子任务，在WebShop测试中取得62%的胜率(优于RFT的53.6%和GPT-4的35.6%)，并在WebShop和SciWorld基准测试中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T11:01:30Z
- **目录日期**: 2025-04-19
