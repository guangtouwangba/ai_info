...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)方法主要依赖专家成功的简单任务轨迹，导致复杂任务难以解决。EEF创新性地从专家失败轨迹中提取有价值的指导信息(如关键行动和计划)，同时剔除有害动作，从而显著提升代理的探索效率和关键技能获取。在WebShop测试中，EEF以62%的胜率超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创下超过81分的新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T09:01:16Z
- **目录日期**: 2025-04-19
