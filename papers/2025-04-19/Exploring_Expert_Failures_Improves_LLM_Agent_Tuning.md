...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)方法主要依赖专家成功轨迹，但难以处理复杂子任务。EEF创新性地从专家失败轨迹中提取有价值的行为(如计划和关键动作)，同时排除有害行为，显著提高了代理的探索效率和关键技能获取。实验表明，EEF成功解决了RFT无法处理的子任务，在WebShop中达到62%的胜率(优于RFT的53.6%和GPT-4的35.6%)，并在WebShop和SciWorld中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T10:01:19Z
- **目录日期**: 2025-04-19
