...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)在复杂任务中的失败轨迹，提取其中有价值的行动指导(如关键计划和动作)，同时剔除有害动作以避免污染学习过程。相比仅使用成功轨迹的拒绝采样微调(RFT)方法，EEF能有效解决先前无法完成的复杂子任务，显著提升智能体性能。在WebShop测试中，EEF以62%的胜率超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创下超过81分的新纪录，成为首个突破该阈值的方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T22:01:20Z
- **目录日期**: 2025-04-19
