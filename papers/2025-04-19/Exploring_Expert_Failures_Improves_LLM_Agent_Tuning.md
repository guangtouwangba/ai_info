...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)在复杂子任务中的失败轨迹，提取有价值的行动指导(如关键计划和动作)，同时排除有害动作以避免污染学习过程。EEF将这些有益动作整合到训练数据中，成功解决了先前无法完成的子任务。实验表明，EEF在WebShop任务中取得62%的胜率，优于拒绝采样微调(RFT)的53.6%和GPT-4的35.6%，并在WebShop和SciWorld任务中创下超过81分的新纪录，成为首个突破该指标的方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T05:01:27Z
- **目录日期**: 2025-04-19
