...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的训练效果。传统拒绝采样微调(RFT)方法主要依赖专家成功轨迹，但会忽略复杂子任务。研究发现，专家失败轨迹中的有益行动(如计划和关键动作)能显著提升智能体探索效率和关键技能获取。EEF通过筛选专家失败轨迹中的有益行动并排除有害行动来优化训练数据，成功解决了部分RFT无法完成的子任务。实验表明，EEF在WebShop任务中达到62%胜率，超越RFT(53.6%)和GPT-4(35.6%)，在SciWorld任务中也突破81分，创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T04:01:14Z
- **目录日期**: 2025-04-19
