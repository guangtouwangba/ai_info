...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有效行动来提升大语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)主要依赖成功轨迹，而EEF从失败案例中提取有价值的指导信息(如关键行动和计划)，同时剔除有害内容以避免负面影响。实验表明，EEF成功解决了RFT无法处理的复杂子任务，在WebShop任务中以62%胜率显著超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创下81分以上的新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T13:05:17Z
- **目录日期**: 2025-04-19
