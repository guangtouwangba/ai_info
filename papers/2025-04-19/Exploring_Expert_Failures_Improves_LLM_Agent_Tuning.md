...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的性能。研究发现，专家(如GPT-4)在复杂任务中失败的历史轨迹往往包含有价值的指导信息(如计划和关键动作)。EEF通过从失败轨迹中提取有益动作并排除有害动作来优化训练数据集，从而解决了先前无法完成的子任务。实验表明，EEF在WebShop任务中取得了62%的胜率，优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创造了新的最先进水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T18:01:16Z
- **目录日期**: 2025-04-19
