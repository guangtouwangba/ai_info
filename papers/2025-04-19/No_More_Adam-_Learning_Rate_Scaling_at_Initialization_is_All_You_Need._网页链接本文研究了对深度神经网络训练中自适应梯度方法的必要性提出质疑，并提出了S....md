...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量的随机梯度下降（SGDM），通过初始化时依据梯度信号噪声比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及GPT-2等大模型预训练中表现优于或持平AdamW，且对超参数更鲁棒。内存效率显著提升，例如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB，在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T14:05:12Z
- **目录日期**: 2025-04-19
