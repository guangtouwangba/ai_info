...
判断一个行为的安全性需要考虑其发生的具体情境。对人类而言，这很自然——比如删除邮件是否恰当，取决于邮件内容、目的（如删除敏感邮件或清理垃圾）和邮箱类型（工作或个人）。传统计算系统由于功能有限，依靠手动制定的策略和用户确认（如应用权限）即可防范风险。但随着多功能智能代理（如自动化个人助手）的出现，其广泛的任务场景要求重新设计安全机制。为此，本文提出"情境化智能代理安全框架"（Conseca），旨在生成即时、情境化且可人工验证的安全策略，以适配新一代系统的复杂需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T09:02:22Z
- **目录日期**: 2025-04-19
