...
评估一个行为的安全性需要结合具体情境。对人类而言，这显而易见（如删除邮件是否恰当取决于邮件内容、目的和邮箱类型）。但计算系统通常只在有限情境中运作，依赖手动制定的策略和用户确认（如应用权限）来限制有害行为。随着多功能智能代理（如自动个人助手）的出现，其广泛的任务场景和能力要求重新思考安全设计。为此，本文提出"情境化智能代理安全框架"（Conseca），旨在生成即时、情境化且可人工验证的安全策略，以应对通用代理的复杂需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-19T01:29:24Z
- **目录日期**: 2025-04-19
