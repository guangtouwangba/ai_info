...
这篇博客详细记录了开发者基于RISC-V H-extension实现Hypervisor并运行Linux的全过程。通过9个关键步骤：从进入Guest模式、执行第一条ecall指令，到补全设备树、支持时间读取和中断注入，最后模拟MMIO设备和virtio-fs文件系统，成功在自研操作系统Starina上运行Linux内核。整个过程利用QEMU的RISC-V H-extension模拟和GDB调试支持，确保了开发体验的流畅性。这一实践展示了RISC-V虚拟化的可行性，为开发者提供了宝贵的技术参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T00:02:55Z
- **目录日期**: 2025-05-18
