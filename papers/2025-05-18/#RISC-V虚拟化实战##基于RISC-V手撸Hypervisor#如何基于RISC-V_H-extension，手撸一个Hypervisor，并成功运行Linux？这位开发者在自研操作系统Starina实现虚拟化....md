...
这篇博客详细记录了基于RISC-V H-extension开发Hypervisor并运行Linux的全过程。开发者从进入Guest模式、处理系统调用开始，逐步实现设备树支持、时间读取、定时器中断、MMIO设备模拟等功能，最终通过virtio-fs实现文件共享。整个过程在QEMU环境中完成，利用RISC-V的虚拟化扩展特性，成功让Linux运行在自研操作系统Starina上。文章展示了RISC-V虚拟化的关键技术点，包括权限控制、中断处理和设备模拟，为开发者提供了完整的实践参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T02:33:55Z
- **目录日期**: 2025-05-18
