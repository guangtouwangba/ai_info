...
这篇博客详细记录了基于RISC-V H-extension开发Hypervisor的全过程。开发者从零开始，通过9个关键步骤实现了一个能运行Linux的轻量级虚拟机环境：首先完成Guest模式切换，然后处理系统调用、中断和设备树，最后模拟MMIO设备和实现virtio-fs文件系统共享。整个过程充分利用了RISC-V的虚拟化扩展特性，并在QEMU环境中进行调试。最终成功在自研操作系统Starina上运行Linux内核，展示了RISC-V架构在虚拟化方面的实用性和灵活性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T01:30:36Z
- **目录日期**: 2025-05-18
