...
该研究探讨了如何通过小数据集建模人类在战略网络游戏（Junior High Game）中的行为，以更好地理解社会网络对人类财富、健康等结果的影响。研究发现，最有效的建模方法hCAB通过模拟人类行为的分布（而非均值）并采用社区感知行为假设（而非单纯行为匹配），能准确反映小规模群体（6-11人）的动态特征。用户实验表明，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体行为层面的拟真性。这一成果为研究社会网络机制提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T18:02:13Z
- **目录日期**: 2025-05-18
