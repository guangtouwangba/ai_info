...
该研究比较了不同方法在小数据集上对人类在策略性网络游戏（Junior High Game）中行为的建模效果。最佳方法hCAB通过模拟人类行为的分布（而非均值）并采用社区感知行为假设（而非简单行为匹配），能准确反映6-11人小群体的动态特征。实验表明，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体层面的拟真性。这一发现为理解社会网络中的财富/健康不平等、贫困等问题提供了新视角，表明社区感知的行为分布建模能更有效捕捉人类网络互动特征。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T00:02:33Z
- **目录日期**: 2025-05-18
