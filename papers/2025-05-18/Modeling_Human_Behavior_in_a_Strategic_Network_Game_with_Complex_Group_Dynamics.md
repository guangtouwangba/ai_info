...
人类网络对社会不平等、贫困和霸凌等关键问题有重大影响。为更好理解人类网络行为，研究比较了不同方法在小数据集上建模人类在“初中生游戏”（JHG）中的策略行为。其中表现最佳的hCAB方法通过建模行为分布（而非均值）并假设人类具有社区意识行为（而非简单行为匹配），能准确反映小群体（6-11人）的动态特征。用户研究显示，人类参与者无法区分hCAB智能体与真人行为，验证了该模型在个体层面也能生成可信的人类策略行为。这表明社区感知的分布建模对理解人类网络行为至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T14:02:33Z
- **目录日期**: 2025-05-18
