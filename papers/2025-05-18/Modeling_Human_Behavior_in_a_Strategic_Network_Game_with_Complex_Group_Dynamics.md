...
这篇论文研究了人类网络对社会不平等、贫困和霸凌等社会结果的影响，并提出了一种名为hCAB的建模方法，用于模拟人类在战略网络游戏（Junior High Game）中的行为。hCAB方法通过建模人类行为的分布（而非均值）并假设人类具有社区意识的行为（而非简单行为匹配），在小规模社会（6-11人）中能准确反映群体动态。用户研究还表明，人类参与者无法区分hCAB模型生成的个体行为与真实人类行为，验证了该模型在个体层面的合理性。这一研究为理解人类网络行为提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T23:02:11Z
- **目录日期**: 2025-05-18
