...
该研究比较了多种从小数据集中学习人类在策略性网络游戏（Junior High Game）中行为模型的方法。最佳方法hCAB通过建模行为分布（而非均值）并假设人类具有社区意识行为（而非单纯行为匹配），能准确反映小型社会群体（6-11人）的动态特征。实验显示，人类参与者无法区分hCAB智能体与真人，表明该模型能生成可信的个体行为。这些发现对理解现实社会网络（如财富分配、霸凌等现象）具有重要意义，为促进有利社会结果提供了建模基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T15:02:47Z
- **目录日期**: 2025-05-18
