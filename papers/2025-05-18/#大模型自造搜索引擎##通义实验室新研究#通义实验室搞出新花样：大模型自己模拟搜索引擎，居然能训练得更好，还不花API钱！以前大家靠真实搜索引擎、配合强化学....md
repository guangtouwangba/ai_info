...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练（从高质量文档逐步过渡到噪声数据），并运用强化学习算法构建检索-推理闭环。实验显示，该方法在问答任务中表现优于传统搜索引擎方案，3B/7B小模型即可媲美谷歌搜索，且完全避免了API调用成本。该技术解决了传统方法面临的文档质量差、训练成本高和扩展性差三大痛点。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T02:34:06Z
- **目录日期**: 2025-05-18
