...
通义实验室提出"ZeroSearch"方法，让大语言模型自主模拟搜索引擎进行训练，无需依赖真实搜索API。该方法通过三个关键步骤实现：1)轻量微调生成含噪声的文档数据；2)课程式抗噪训练逐步增加数据复杂度；3)强化学习闭环优化检索推理流程。实验表明，仅用3B/7B参数的小模型，在单跳和多跳问答任务上就能媲美谷歌搜索效果，同时显著降低训练成本。该研究解决了传统方法依赖真实搜索引擎导致的质量、成本和扩展性问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T07:03:48Z
- **目录日期**: 2025-05-18
