...
阿里通义实验室提出"ZeroSearch"新方法，让大语言模型通过自生成检索结果进行训练，无需依赖真实搜索引擎API。该方法采用三步策略：1)轻量微调生成含噪声的文档；2)课程式抗噪训练逐步增加噪声；3)强化学习闭环优化检索推理流程。实验显示，仅用3B/7B参数的小模型，在单跳和多跳问答任务上就能媲美谷歌搜索效果，同时解决了传统方法存在的文档质量差、成本高和扩展难问题，实现了零API成本的训练优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T06:03:55Z
- **目录日期**: 2025-05-18
