...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练，无需调用真实API。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习优化检索-推理闭环。实验显示，仅用3B/7B参数的小模型在单跳和多跳问答任务上就能媲美谷歌搜索效果，同时解决了传统方法面临的文档质量差、成本高和扩展难等问题。这种自造搜索引擎的方式显著降低了训练成本，提升了模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T23:03:35Z
- **目录日期**: 2025-05-18
