...
通义实验室提出"ZeroSearch"新方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习形成闭环训练。实验表明，仅用3B/7B小模型就能在单跳/多跳问答任务上媲美谷歌搜索效果，同时避免了真实API的高成本问题。这种自造搜索引擎的范式解决了传统方法面临的文档质量差、训练成本高和扩展性差三大痛点。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T20:04:38Z
- **目录日期**: 2025-05-18
