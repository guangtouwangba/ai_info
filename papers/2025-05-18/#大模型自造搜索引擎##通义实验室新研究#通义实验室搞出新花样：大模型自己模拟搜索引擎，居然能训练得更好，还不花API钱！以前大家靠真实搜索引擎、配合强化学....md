...
阿里通义实验室提出"ZeroSearch"新方法，让大语言模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的伪检索结果，采用课程学习逐步增加数据噪声，并利用强化学习形成训练闭环。实验显示，仅用3B/7B参数的小模型就能在单跳/多跳问答任务上媲美传统搜索引擎方案，同时实现零API成本。相比依赖真实搜索引擎的强化学习方法，这种自训练方式解决了数据质量、扩展性和费用三大痛点，在多个问答数据集上展现出更优性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T16:03:56Z
- **目录日期**: 2025-05-18
