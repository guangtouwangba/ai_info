...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练，无需对接真实搜索API。该方法通过轻量微调生成带噪声的伪检索结果，采用课程学习逐步增加噪声难度，并利用强化学习优化检索-推理闭环。实验显示，仅用3B/7B参数的小模型在单跳/多跳问答任务上就能媲美谷歌搜索效果，同时解决了传统方法存在的文档质量差、API成本高和扩展性差等问题。这种自监督训练方式显著降低了成本并提升了模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T12:03:56Z
- **目录日期**: 2025-05-18
