...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习算法(PPO/GRPO)构建检索-推理闭环。实验显示，3B/7B小模型在单跳和多跳问答任务中表现优异，效果媲美谷歌搜索。相比传统依赖真实搜索引擎的方法，该方案解决了文档质量差、API成本高和扩展性差三大痛点，实现了零API成本的优质训练效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T18:03:45Z
- **目录日期**: 2025-05-18
