...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习形成闭环。实验显示，仅用3B/7B参数的小模型就能在单跳/多跳问答任务上媲美谷歌搜索效果。相比传统依赖真实搜索引擎的方法，ZeroSearch解决了数据质量差、API成本高和扩展困难三大痛点，实现了零API成本的高效训练。该研究为大模型检索能力训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T22:03:47Z
- **目录日期**: 2025-05-18
