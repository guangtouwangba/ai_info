...
阿里通义实验室提出"ZeroSearch"方法，通过大模型自造搜索引擎来提升训练效果并降低成本。该方法采用三步策略：1)轻量微调模型生成含噪声的有用文档；2)课程式抗噪训练，逐步增加噪声文档比例；3)强化学习闭环优化检索推理流程。实验表明，仅用3B/7B小模型就能在单跳和多跳问答任务上媲美甚至超越传统基于真实搜索引擎的方法，同时避免了API调用成本。该研究为大模型检索训练提供了更高效经济的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T11:03:34Z
- **目录日期**: 2025-05-18
