...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并运用强化学习算法构建闭环训练系统。相比依赖真实搜索引擎的传统方法，ZeroSearch在3B/7B小模型上就能实现媲美谷歌搜索的效果，且无需API调用成本。实验显示，该方法在单跳和多跳问答任务中均优于传统方案，解决了文档质量差、训练成本高和模型扩展难等问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T08:03:43Z
- **目录日期**: 2025-05-18
