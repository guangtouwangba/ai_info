...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习构建闭环训练系统。实验表明，仅用3B/7B参数的小模型就能在单跳/多跳问答任务上媲美谷歌搜索效果，同时避免了传统方法依赖真实API导致的高成本和扩展性问题。该创新实现了零API费用的高效训练，为检索增强型大模型训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T21:05:11Z
- **目录日期**: 2025-05-18
