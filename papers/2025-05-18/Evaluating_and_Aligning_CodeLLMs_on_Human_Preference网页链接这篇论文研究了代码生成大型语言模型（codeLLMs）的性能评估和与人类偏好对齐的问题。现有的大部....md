...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1) 创建人类偏好基准CodeArena，包含397个涵盖40类编程任务和44种语言的真实样本；2) 构建20B标记量的合成指令数据集SynCode-Instruct。研究发现：现有基于执行的评估与人类偏好存在偏差，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在显著差距。研究强调了代码生成模型不仅需要正确性，还需符合开发者实际编程偏好，为提升代码LLMs的实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T01:33:35Z
- **目录日期**: 2025-05-18
