...
本研究探讨了使用大型语言模型（LLMs）自动化评估医学生OSCE考试中沟通技能的可行性。通过比较GPT-4o、Claude 3.5等四种先进模型在MIRS量表28个项目上的表现，发现模型在精确匹配准确率（0.27-0.44）上较低，但在近似匹配（0.67-0.87）和阈值准确率（0.75-0.88）上表现中等至良好。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。特定提示技术（如思维链、少量示例）对某些评估项目有效。研究为临床沟通技能的自动化评估建立了基准，展示了AI辅助OSCE评分的潜力，但精确度仍需提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T07:02:39Z
- **目录日期**: 2025-05-18
