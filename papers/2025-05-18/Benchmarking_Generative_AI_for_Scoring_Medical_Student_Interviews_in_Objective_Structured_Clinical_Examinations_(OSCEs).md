...
本研究探讨了使用大型语言模型（LLM）自动评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在Master评分量表（MIRS）28个项目中的表现。模型在零样本、思维链等不同提示方法下进行评估，与174份专家评分对比。结果显示，LLM的完全准确率较低（0.27-0.44），但近似准确率较高（0.67-0.88），且评分者内部一致性优异（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有潜力，为临床沟通技能的自动化评估奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T15:03:02Z
- **目录日期**: 2025-05-18
