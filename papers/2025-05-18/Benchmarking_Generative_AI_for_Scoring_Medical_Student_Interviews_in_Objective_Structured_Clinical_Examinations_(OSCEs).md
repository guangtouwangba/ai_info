...
本研究探讨了使用大型语言模型（LLMs）自动化OSCE（客观结构化临床考试）评估的可行性，比较了GPT-4o、Claude 3.5等四种先进模型在MIRS（主面试评分量表）28个项目中的表现。实验采用零样本、思维链等技术，基于174份专家共识评分的数据集。结果显示，LLMs的精确准确率较低（0.27-0.44），但中等偏高的一级误差准确率（0.67-0.87）和阈值准确率（0.75-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有潜力，为临床沟通技能自动化评估奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T02:33:44Z
- **目录日期**: 2025-05-18
