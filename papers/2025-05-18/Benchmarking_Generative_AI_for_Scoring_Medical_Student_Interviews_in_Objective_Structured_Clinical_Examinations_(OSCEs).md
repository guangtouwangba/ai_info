...
本研究探讨了使用大语言模型（LLM）自动评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在Master评分标准（MIRS）下的表现。结果显示，LLM在精确匹配准确率较低（0.27-0.44），但相邻分值和阈值准确率中等偏高（0.67-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。思维链、少量样本和多步提示技术对特定评估项目有效。研究表明AI辅助OSCE评估可行，为临床沟通技能自动化评估奠定了基础，并提供了多模型多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T13:08:30Z
- **目录日期**: 2025-05-18
