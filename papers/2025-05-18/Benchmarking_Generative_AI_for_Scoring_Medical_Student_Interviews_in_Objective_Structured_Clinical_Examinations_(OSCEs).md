...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能（OSCE考试）的可行性。测试了GPT-4o、Claude 3.5等4种先进模型在28项MIRS评分标准下的表现，采用零样本、思维链等不同提示技术。结果显示：模型在精确匹配准确率较低（0.27-0.44），但相邻误差（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好。思维链等技术对特定评估项有效，且模型评分一致性高（GPT-4o α=0.98）。研究为临床沟通技能的自动化评估建立了基准，证实了AI辅助评分的可行性，为未来研究奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T09:02:32Z
- **目录日期**: 2025-05-18
