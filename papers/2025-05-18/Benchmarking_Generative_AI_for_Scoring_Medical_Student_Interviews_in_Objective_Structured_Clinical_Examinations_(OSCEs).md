...
本研究探讨了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在28项评分标准（MIRS）下的表现。结果显示，LLMs的精确准确率较低（0.27-0.44），但中等偏高的一级误差准确率（0.67-0.87）和阈值准确率（0.75-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。特定提示技术（思维链、少量示例等）对某些评估项有效。研究表明AI辅助OSCE评估具有可行性，为临床沟通技能的自动化评估奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T08:02:36Z
- **目录日期**: 2025-05-18
