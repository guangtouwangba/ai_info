...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能（OSCE考试）的可行性。测试了GPT-4o等4种先进LLM在28项MIRS评分标准下的表现，采用零样本、思维链等多种提示技术。结果显示：LLM的精确准确率较低（0.27-0.44），但容错准确率中高（0.67-0.88），且具有高评分者内信度（α=0.98）。特定提示技术可提升特定项目的评估效果。研究表明AI辅助OSCE评估具有可行性，为临床沟通技能的自动化评估奠定了基础，但需进一步优化以提高精确度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T19:02:26Z
- **目录日期**: 2025-05-18
