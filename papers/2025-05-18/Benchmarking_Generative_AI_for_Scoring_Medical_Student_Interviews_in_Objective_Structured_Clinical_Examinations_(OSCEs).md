...
本研究探索了使用大型语言模型（LLMs）自动化OSCE（客观结构化临床考试）评估的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在MIRS量表中的表现。结果显示，LLMs在严格匹配评分上的准确率较低（0.27-0.44），但在允许±1分误差时表现中等至良好（0.67-0.87）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。链式思维、少量样本和多步提示等技巧对特定评估项目有效。研究表明AI辅助OSCE评估具有潜力，为临床沟通技能的自动化评估奠定了基础，但精确度仍需提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T00:02:43Z
- **目录日期**: 2025-05-18
