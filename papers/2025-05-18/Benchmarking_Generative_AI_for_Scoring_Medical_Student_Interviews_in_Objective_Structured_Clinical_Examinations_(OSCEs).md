...
本研究探讨了使用大型语言模型（LLM）自动化评估医学生临床沟通技能（OSCE考试）的可行性。测试了GPT-4o、Claude 3.5等4种先进模型在零样本、思维链等不同提示技术下对28项MIRS量表的评分表现。结果显示：模型在严格匹配准确率较低（0.27-0.44），但容错准确率中等偏高（0.67-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。特定提示技术对某些评估项有效。研究表明LLM辅助OSCE评估具有潜力，为未来临床沟通技能自动化评估研究奠定了基础，同时提供了多模型多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T04:06:19Z
- **目录日期**: 2025-05-18
