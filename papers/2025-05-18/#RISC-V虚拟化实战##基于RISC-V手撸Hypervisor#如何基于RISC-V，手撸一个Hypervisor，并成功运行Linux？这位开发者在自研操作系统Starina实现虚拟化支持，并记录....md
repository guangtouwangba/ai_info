...
这篇博客详细记录了在RISC-V架构上从零开发Hypervisor并运行Linux的完整过程。开发者通过9个关键步骤实现：1)进入Guest模式；2)处理第一条ecall指令；3)实现Guest输出；4)加载Linux内核；5)添加设备树支持；6)处理时间读取；7)实现定时器中断；8)模拟MMIO设备；9)集成virtio-fs文件系统。最终在自研操作系统Starina上成功运行Linux，展示了RISC-V虚拟化技术的实战应用。整个过程使用QEMU模拟器和GDB调试工具，开发体验流畅。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T09:03:36Z
- **目录日期**: 2025-05-18
