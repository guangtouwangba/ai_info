...
该开发者详细记录了在自研操作系统Starina上实现RISC-V虚拟化的全过程。通过RISC-V的H-extension进入Guest模式，逐步构建页表、处理系统调用、提供设备树支持，并实现定时器中断和MMIO设备模拟。最终成功在轻量级Hypervisor上运行Linux内核，采用virtio-fs实现文件共享。整个过程利用QEMU的RISC-V H-extension模拟和GDB调试工具，开发体验流畅。这一实践展示了从零构建RISC-V虚拟化环境的完整技术路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T08:03:34Z
- **目录日期**: 2025-05-18
