...
这篇博客记录了开发者基于RISC-V架构手写Hypervisor并成功运行Linux的全过程。通过RISC-V的H-extension实现虚拟化支持，从进入Guest模式开始，逐步构建页表、处理系统调用、添加设备树支持，到实现定时器中断和模拟MMIO设备，最终让Linux内核能在自研操作系统Starina上运行。开发过程中解决了空指针异常、时间戳读取等问题，并采用virtio-fs实现文件共享。整个过程可在QEMU中完成，利用其RISC-V H-extension模拟和GDB调试功能，提供了流畅的开发体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T12:03:46Z
- **目录日期**: 2025-05-18
