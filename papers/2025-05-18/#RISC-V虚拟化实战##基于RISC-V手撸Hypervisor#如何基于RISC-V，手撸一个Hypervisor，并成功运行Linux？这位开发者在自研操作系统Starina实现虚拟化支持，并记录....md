...
这篇博客详细记录了基于RISC-V架构开发Hypervisor并运行Linux的全过程。开发者从零开始，在自研操作系统Starina上实现了虚拟化支持，通过9个关键步骤：包括进入Guest模式、处理系统调用、加载Linux内核、配置设备树、实现时间读取和中断注入、模拟MMIO设备，最终成功在QEMU模拟环境中运行Linux。整个过程充分利用了RISC-V的H-extension虚拟化扩展，并通过virtio-fs实现文件共享。开发中结合GDB调试，展现了完整的Hypervisor开发流程和技术细节。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T20:04:28Z
- **目录日期**: 2025-05-18
