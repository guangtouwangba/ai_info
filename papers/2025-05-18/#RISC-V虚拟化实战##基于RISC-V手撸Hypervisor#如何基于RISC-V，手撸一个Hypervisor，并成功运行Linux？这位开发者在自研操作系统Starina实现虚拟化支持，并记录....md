...
这篇博客详细记录了如何在RISC-V架构上从零开发一个Hypervisor，并成功运行Linux系统的全过程。开发者基于自研操作系统Starina，通过9个关键步骤实现虚拟化：包括进入Guest模式、处理系统调用、加载Linux内核、配置设备树、支持时间读取、定时器中断、模拟MMIO设备，以及最终通过virtio-fs实现文件共享。整个过程利用QEMU的RISC-V H-extension模拟和GDB调试工具，完整展示了RISC-V虚拟化技术的实战应用，最终让Linux系统能在自研的Hypervisor上顺利运行。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T14:03:54Z
- **目录日期**: 2025-05-18
