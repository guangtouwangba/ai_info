...
这篇博客详细记录了如何在RISC-V架构上从零开发一个轻量级Hypervisor，并成功运行Linux系统的全过程。开发者基于自研操作系统Starina，通过9个关键步骤实现虚拟化：包括进入Guest模式、处理系统调用、加载Linux内核、配置设备树、支持时间读取、处理定时器中断、模拟MMIO设备，最终接入virtio-fs文件系统。整个过程利用RISC-V的H-extension特性，在QEMU模拟环境中完成，结合GDB调试工具，实现了完整的虚拟化解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T15:04:17Z
- **目录日期**: 2025-05-18
