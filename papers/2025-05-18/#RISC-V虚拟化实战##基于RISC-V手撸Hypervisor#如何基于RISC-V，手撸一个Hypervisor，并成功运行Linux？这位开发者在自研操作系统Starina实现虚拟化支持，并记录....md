...
这篇博客详细记录了在自研操作系统Starina上基于RISC-V架构实现Hypervisor并运行Linux的全过程。开发者通过9个关键步骤：从进入Guest模式、处理系统调用、输出Hello World，到解决设备树问题、支持时间读取和中断注入，最后模拟MMIO设备并接入virtio-fs文件系统。整个过程在QEMU中完成，利用RISC-V的H-extension虚拟化扩展，成功让Linux运行在自研内核上。开发中特别注意了RISC-V对寄存器权限的严格控制和设备模拟，为后续系统集成打下基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T10:03:45Z
- **目录日期**: 2025-05-18
