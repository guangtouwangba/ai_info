# #RISC-V虚拟化实战##基于RISC-V手撸Hypervisor#如何基于RISC-V，手撸一个Hypervisor，并成功运行Linux？这位开发者在自研操作系统Starina实现虚拟化支持，并记录...
...
这篇博客详细记录了如何在RISC-V架构上从零开发一个Hypervisor，并成功运行Linux系统的全过程。开发者基于自研操作系统Starina，通过9个关键步骤实现虚拟化：包括进入Guest模式、处理系统调用、加载Linux内核、配置设备树、支持时间读取和定时器中断、模拟MMIO设备，以及集成virtio-fs文件系统。整个过程利用RISC-V的H-extension虚拟化扩展，在QEMU模拟环境中完成，结合GDB调试工具确保了开发效率。最终实现了类似WSL2的轻量级虚拟机环境，让Linux运行在完全自研的系统内核上。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T11:03:23Z
- **目录日期**: 2025-05-18
