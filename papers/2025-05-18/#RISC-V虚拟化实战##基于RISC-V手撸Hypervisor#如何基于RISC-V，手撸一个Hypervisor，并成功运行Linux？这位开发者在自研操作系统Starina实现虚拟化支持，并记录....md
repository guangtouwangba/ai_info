...
这篇博客详细记录了在RISC-V架构上从零开发Hypervisor并运行Linux的完整过程。开发者基于自研操作系统Starina，通过9个关键步骤实现虚拟化：首先利用RISC-V的H-extension进入Guest模式；接着处理系统调用、中断和设备树；然后实现时间读取、定时器中断和MMIO设备模拟；最后通过virtio-fs实现文件共享。整个过程在QEMU中完成，利用其RISC-V H-extension模拟和GDB调试支持，成功让Linux运行在自研系统上。这展示了RISC-V虚拟化技术的可行性和开发流程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T22:03:37Z
- **目录日期**: 2025-05-18
