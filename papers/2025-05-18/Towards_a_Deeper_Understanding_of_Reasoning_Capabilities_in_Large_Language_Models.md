...
这项研究评估了大型语言模型（LLM）在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术的效果。实验发现，大模型通常优于小模型，但策略性提示能缩小差距；过长的提示会损害小模型的基础反应任务表现，而大模型更稳健；复杂任务中高级提示对小模型帮助更大，但对高性能大模型提升有限。高级推理方法虽能提升表现，但也可能引发不稳定性。与人类相比，LLM在规划、推理和空间协调等关键领域仍存在根本性局限，表明当前模型无法仅通过自我反思提示完全克服这些缺陷。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T14:01:26Z
- **目录日期**: 2025-05-18
