...
本研究评估了大语言模型（LLM）在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术的效果。实验发现，大模型通常优于小模型，但策略性提示可缩小差距；过长提示会损害小模型的基础反应能力，而大模型表现更稳定；复杂任务中高级提示对小模型帮助更大，但对高性能大模型提升有限。尽管推理方法可能显著提升表现，但也可能引入不稳定性。相比人类，LLM在规划、推理和空间协调等关键领域仍存在根本性缺陷，表明当前模型无法仅通过自我反思提示完全克服这些不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T06:01:24Z
- **目录日期**: 2025-05-18
