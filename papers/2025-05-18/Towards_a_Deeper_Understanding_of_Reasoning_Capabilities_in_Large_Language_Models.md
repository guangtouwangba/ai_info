...
该研究评估了大语言模型(LLM)在动态环境中的自适应能力，发现：1)大模型表现优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型的基础反应任务表现，而大模型更稳健；3)高级提示技术对小模型在复杂任务中帮助更大，但对高性能大模型提升有限。研究显示，当前LLM在规划、推理和空间协调等关键领域仍存在根本性缺陷，推理方法虽能提升特定任务表现，但会导致结果不稳定。这表明仅靠自我反思提示无法完全克服LLM的局限性，需超越静态基准来评估真实推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T01:29:12Z
- **目录日期**: 2025-05-18
