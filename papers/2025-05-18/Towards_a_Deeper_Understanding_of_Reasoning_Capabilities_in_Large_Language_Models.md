...
该研究评估了大语言模型(LLM)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术。结果显示：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型的基础反应任务表现，而大模型更稳健；3)复杂任务中高级提示主要帮助小模型，对大模型提升有限。研究发现，高级推理方法虽可能提升表现，但也带来不稳定性和性能骤降。与人类相比，LLM在规划、推理和空间协调等关键领域仍存在根本性缺陷，表明仅靠自我反思提示无法完全克服当前LLM的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T07:01:17Z
- **目录日期**: 2025-05-18
