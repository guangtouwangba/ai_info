...
该研究评估了大语言模型(LLMs)在动态环境中的自适应能力，发现：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型的基础反应任务表现，而大模型更稳健；3)高级提示技术主要提升小模型在复杂任务中的表现，但对已表现优异的大模型改善有限。研究指出，当前LLMs在规划、推理和空间协调等关键领域仍存在根本性缺陷，推理能力高度不稳定，可能显著提升或损害表现。结果表明，仅靠自我反思提示无法完全克服这些限制，需超越静态基准来评估复杂的推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T18:01:15Z
- **目录日期**: 2025-05-18
