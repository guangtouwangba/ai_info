...
该研究提出了一种基于AI反馈学习的文本到音频(T2A)生成改进方法。通过开发三个细粒度评分管道（事件存在性、事件顺序性、声学质量），显著提升了模型评估效果。基于此构建了包含4.1万提示词和24.9万音频的T2A-FeedBack数据集，并建立了专注长文本、多事件场景的T2A-EpicBench基准测试。实验表明，简单的偏好调优就能使现有音频生成模型在简单和复杂场景下都获得显著提升。这些自动评分管道与人类偏好高度相关，可作为有效的反馈信号和评估指标。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T07:00:53Z
- **目录日期**: 2025-05-18
