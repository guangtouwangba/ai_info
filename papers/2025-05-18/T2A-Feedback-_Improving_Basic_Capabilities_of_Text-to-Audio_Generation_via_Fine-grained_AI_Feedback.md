...
该研究提出通过AI反馈学习提升文本到音频(T2A)生成模型在复杂多事件音频中的表现。研究者开发了三个细粒度评分管道：事件存在性评分、事件序列评分和声学质量评分，这些评分与人类偏好高度相关。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并创建了专注于长文本、多事件场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调整，现有音频生成模型在简单和复杂场景下的表现均得到显著提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T01:28:49Z
- **目录日期**: 2025-05-18
