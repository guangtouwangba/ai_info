...
本文提出了一种通过AI反馈学习提升文本到音频(T2A)生成模型性能的方法。针对现有模型在复杂多事件音频生成中存在的提示跟随和音质问题，研究者开发了细粒度评分系统（事件存在性、序列准确度、音质和谐度），这些指标与人类偏好显著相关。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并创建了专注长文本、多事件场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调优，模型在简单和复杂场景下的生成质量均获得显著提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T19:01:00Z
- **目录日期**: 2025-05-18
