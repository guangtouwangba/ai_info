...
本文提出通过AI反馈学习提升文本到音频(T2A)生成模型在复杂多事件场景下的表现。研究设计了三个细粒度评分指标：事件出现率、事件序列准确度和音质评分，这些指标比现有评估方法更符合人类偏好。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并开发了专注于长文本、多事件场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调优，该方法能显著提升现有模型在简单和复杂场景下的音频生成质量。评分系统既可作反馈信号，也可作为评估指标。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T02:32:07Z
- **目录日期**: 2025-05-18
