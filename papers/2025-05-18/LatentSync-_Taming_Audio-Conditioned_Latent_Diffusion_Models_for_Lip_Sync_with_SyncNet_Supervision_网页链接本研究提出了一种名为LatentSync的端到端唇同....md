...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件潜在扩散模型，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，但发现时间一致性不足。为此，提出TREPA方法，通过自监督视频模型的时间表示增强一致性。同时改进了SyncNet训练框架，解决了收敛问题，提升了准确性。在HDTF和VoxCeleb2数据集上，该方法在多个指标上超越现有技术。这些改进也可应用于其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T14:05:46Z
- **目录日期**: 2025-05-18
