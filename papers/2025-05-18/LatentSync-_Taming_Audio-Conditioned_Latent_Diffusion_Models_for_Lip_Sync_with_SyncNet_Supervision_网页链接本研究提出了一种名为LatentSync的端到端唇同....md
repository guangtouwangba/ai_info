...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，并通过提出的时间表示对齐（TREPA）方法增强生成帧的时间一致性。研究还改进了SyncNet训练框架，解决了其收敛问题，提升了唇同步准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进可推广至其他基于SyncNet的唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T23:05:09Z
- **目录日期**: 2025-05-18
