...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI通过根据参数组的梯度信号噪声比(g-SNR)在初始化时调整学习率，解决了训练不平衡问题，并将优化器内存使用减半。相比AdamW，该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更好，同时显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。SGD-SaI还展现出超参数鲁棒性，在LoRA微调等任务中优于现有优化器，为训练大型模型提供了更高效的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T00:04:39Z
- **目录日期**: 2025-05-18
