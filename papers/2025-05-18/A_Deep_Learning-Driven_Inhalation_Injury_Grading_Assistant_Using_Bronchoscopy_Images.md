...
传统吸入性损伤诊断方法（如AIS评分）存在主观性强、与临床参数相关性差的问题。本研究提出基于深度学习的支气管镜图像辅助诊断工具，采用数据增强技术（图像变换、CUT和CycleGAN）解决医学影像数据不足问题。测试显示，GoogLeNet结合CUT增强效果最佳，分类准确率达97.8%。分析表明CUT能显著改变图像直方图分布和频谱纹理，PCA可视化证实其提升特征空间分类性，Grad-CAM热图平均强度达119.6（原始数据98.8）。该工具创新性地以机械通气时长作为分级标准，为临床诊断提供客观支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-18T01:29:54Z
- **目录日期**: 2025-05-18
