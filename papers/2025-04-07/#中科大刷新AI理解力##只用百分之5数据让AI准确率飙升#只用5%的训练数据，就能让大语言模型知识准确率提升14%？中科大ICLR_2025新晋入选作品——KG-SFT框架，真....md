# #中科大刷新AI理解力##只用百分之5数据让AI准确率飙升#只用5%的训练数据，就能让大语言模型知识准确率提升14%？中科大ICLR 2025新晋入选作品——KG-SFT框架，真...
...
中科大团队提出的KG-SFT框架（ICLR 2025入选）通过知识图谱增强大语言模型的专业知识理解力，仅用5%训练数据即在医学问答任务中实现14%准确率提升。该框架包含三大组件：实体提取器（Extractor）获取知识子图，生成器（Generator）筛选关键信息并生成解释，检测器（Detector）修正知识冲突。实验显示其在小样本、多语言/领域场景下表现优异，且能兼容现有数据增强方案，标志着AI从"数据驱动"转向"知识深度理解"的新方向。论文一作为中科大陈瀚铸，核心突破在于降低数据依赖的同时提升推理质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T17:02:29Z
- **目录日期**: 2025-04-07
