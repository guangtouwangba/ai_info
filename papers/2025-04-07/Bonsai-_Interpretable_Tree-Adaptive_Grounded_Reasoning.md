...
Bonsai是一种可组合的概率推理系统，旨在开发通用协作AI代理。它通过检索相关证据并计算自然语言推理子主张的似然度，生成可调整的推理树。与不透明的黑盒模型不同，Bonsai具有领域适应性、透明推理和不确定性感知能力，支持验证和修正。其推理能力可通过证据扩展在测试时调整，并能可靠处理多种数据类型（文本、图像、视频、音频、数据库）。实验表明，Bonsai在保持领域专用黑盒方法性能的同时，还能生成可解释、有依据且考虑不确定性的推理过程，实现了更高的人类对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T22:01:12Z
- **目录日期**: 2025-04-07
