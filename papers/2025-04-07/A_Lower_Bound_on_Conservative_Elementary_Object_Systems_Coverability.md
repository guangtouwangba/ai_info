...
本文研究了保守基本对象系统（cEOS）的覆盖性问题复杂度。cEOS是一种令牌携带内部Petri网的特殊Petri网模型，用于分析多智能体系统的鲁棒性。虽然EOS的可达性不可判定，但十年前已证明其保守片段（内部网类型不能被完全删除）的覆盖性是可判定的。研究首次通过将带数据的νPetri网编码为cEOS，确立了cEOS覆盖性问题的复杂度下界为非原始递归的Fω2级别。这一结果为理解cEOS的计算复杂性迈出了重要一步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T17:01:52Z
- **目录日期**: 2025-04-07
