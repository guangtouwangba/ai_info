...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉相关性，并针对扩散模型中时间一致性问题提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。研究还优化了SyncNet训练框架，提升其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T00:05:24Z
- **目录日期**: 2025-04-07
