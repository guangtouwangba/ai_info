...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，并提出时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题。研究还改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。创新点包括：1)直接潜在空间建模；2)TREPA增强时间一致性；3)SyncNet训练优化。这些改进可推广至其他音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T08:05:05Z
- **目录日期**: 2025-04-07
