...
大型AI模型虽然在标准化测试中表现出色，但在医疗、教育、法律等高风险领域应用时仍存在明显缺陷，如对输入数据微小变化的敏感性、关键场景中缺乏情境认知、以及错误输出的过度自信等问题。为此，研究者提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架能帮助研究者：(1)定位创新方向（如领域知识如何赋能通用创新），(2)发现被忽视的机会（如跨领域共性问题的解决），(3)促进跨学科交流（建立开发者、领域专家与人机交互学者的共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T01:28:56Z
- **目录日期**: 2025-04-07
