...
Meta最新发布三款AI模型：Scout（中杯）、Maverick（大杯）和正在训练的Behemoth（2万亿参数）。已发布的Llama 4模型以单卡（如H100）即可运行，参数量减半但性能达到SOTA（当前最优），代码能力媲美DeepSeek。新模型通过高效架构设计显著降低成本，重夺开源领域领先地位。训练细节透露其采用前沿技术，Behemoth的规模尤其引发行业关注。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T02:31:32Z
- **目录日期**: 2025-04-07
