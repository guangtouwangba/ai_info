...
Meta最新发布三款AI模型：Scout（中杯）、Maverick（大杯）和Behemoth（特大杯，2万亿参数待发布）。已发布的LlaMa 4模型以参数减半实现与DeepSeek相当的代码能力，支持单卡（如H100）运行并达到SOTA性能。该系列采用高效训练技术，显著降低计算成本，被网友称为"AI价格打骨折"。其中Behemoth的前瞻性技术规格（如超大规模参数和先进训练方法）引发行业高度关注，被比作"提前解锁DLC"的突破性进展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T07:04:08Z
- **目录日期**: 2025-04-07
