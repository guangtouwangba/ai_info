...
该研究提出一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型假设细胞倾向于向压缩力较低的区域迁移，其运动遵循细胞压力的梯度变化。细胞压力由不同表型细胞的密度加权和决定，且细胞的迁移能力和对压力的贡献因表型而异。研究从基于个体的离散模型推导出连续极限，分析了细胞表型依赖和压力调节的分裂、死亡及迁移行为。通过数值模拟和行波解分析，发现细胞迁移能力的差异性可导致入侵前沿出现空间表型隔离。模型与模拟结果高度吻合。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T02:30:15Z
- **目录日期**: 2025-04-07
