...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体的生长动态。模型中细胞倾向于向压缩较小的区域迁移，其运动方向沿细胞压力梯度下降。细胞压力定义为不同表型细胞密度的加权和，且细胞迁移能力和压力贡献权重均随表型变化。研究从基于个体的离散模型推导出该连续模型，分析了细胞表型依赖性和压力调节的分裂、死亡及迁移行为。通过数值模拟和行波解分析，发现细胞迁移能力的差异会导致入侵前沿出现空间隔离现象。结果表明细胞运动能力的异质性可促进入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T00:02:29Z
- **目录日期**: 2025-04-07
