...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体生长。模型中细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力的梯度变化。细胞压力定义为不同表型细胞密度的加权和，且细胞迁移能力和压力贡献权重因表型而异。研究从基于个体的离散模型推导出该连续模型，分析了细胞表型依赖和压力调节的分裂、死亡及迁移行为。通过数值模拟和行波解分析，发现细胞迁移能力的差异可导致入侵前沿出现空间隔离现象。两种模型结果高度一致，证实了该机制的有效性。(100字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T01:29:55Z
- **目录日期**: 2025-04-07
