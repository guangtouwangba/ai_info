...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动方向沿细胞压力梯度下降方向。细胞压力定义为不同表型细胞密度的加权和，且细胞的迁移能力和压力贡献权重均随表型变化。研究通过离散的个体模型推导出连续极限，分析了细胞表型依赖性和压力调节的分裂、死亡及迁移行为。数值模拟显示，不同表型的细胞在入侵前沿呈现空间分离的波状传播模式，表明细胞迁移能力的差异可导致入侵前沿的空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T03:16:35Z
- **目录日期**: 2025-04-07
