...
这篇论文针对代码生成大模型（CodeLLMs）提出了新的评估方法，重点关注模型输出与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发场景。同时开发了SynCode-Instruct合成指令数据集（约200亿标记）验证指令微调效果。实验发现现有基于执行的评估与人类偏好存在差距，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著，凸显了人类偏好对齐在代码生成领域的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T05:05:06Z
- **目录日期**: 2025-04-07
