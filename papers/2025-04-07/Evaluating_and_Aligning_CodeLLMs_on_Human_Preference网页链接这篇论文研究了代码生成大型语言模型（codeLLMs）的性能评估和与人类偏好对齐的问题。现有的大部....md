...
这篇论文针对代码生成大语言模型（CodeLLMs）提出新评估方法，指出当前基准测试过于关注代码正确性而忽略人类偏好。研究者构建了CodeArena基准（含397个跨40类/44种编程语言的样本）和SynCode-Instruct合成指令库（20B标记），发现执行基准与人类偏好存在差异。实验比较40多个模型显示，开源顶级CodeLLM（如Qwen2.5-Coder）与专有模型（如OpenAI o1）性能差距显著，凸显人类偏好对齐的重要性。研究证实大规模合成指令微调的有效性，为代码生成模型评估提供了新维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T12:06:02Z
- **目录日期**: 2025-04-07
