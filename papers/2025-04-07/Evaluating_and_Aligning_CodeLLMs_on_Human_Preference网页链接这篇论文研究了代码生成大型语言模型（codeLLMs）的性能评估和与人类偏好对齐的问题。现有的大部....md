...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，关注模型输出与人类偏好的对齐问题。研究构建了包含397个高质量样本的基准测试(覆盖40类任务和44种语言)，并开发近200亿标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好评估存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调代码生成质量需同时考虑功能正确性和人类偏好匹配，为代码LLM评估提供了新维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T11:04:23Z
- **目录日期**: 2025-04-07
