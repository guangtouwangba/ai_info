...
Meta新发布的Llama 4大模型陷入"货不对版"争议。社区爆料称其实际表现与宣传差距大，有员工因不满"刷榜"要求辞职。TechCrunch指出Meta用于榜单测试的是"特别优化版"，与开发者可下载版本不同。官网确认上榜的是"实验性聊天版"，该版本回答风格（频繁使用emoji、冗长）与正式版存在明显差异。Meta宣称Llama 4系列（含2万亿参数的Behemoth）在多任务上超越GPT-4o等模型，但实际性能仍待验证。争议焦点在于榜单模型与发布版本的一致性存疑。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T14:04:03Z
- **目录日期**: 2025-04-07
