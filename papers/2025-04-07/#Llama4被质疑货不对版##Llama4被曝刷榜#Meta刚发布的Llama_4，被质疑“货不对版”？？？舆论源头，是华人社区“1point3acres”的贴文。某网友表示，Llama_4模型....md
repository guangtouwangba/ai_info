...
Meta最新发布的Llama 4大模型陷入"货不对版"争议。社区爆料称其实际表现与宣传差距较大，有员工因不满"刷榜"要求而离职。TechCrunch指出Meta用于榜单测试的是"特别优化版"，与开发者可下载版本不同。Meta官网也承认榜单使用的是"实验性聊天版"。博主对比发现榜单模型回答风格（频繁使用emoji、冗长）与正式版明显不同。虽然Meta宣称Llama 4系列（含2万亿参数的Behemoth）在多领域超越GPT-4o等竞品，但实际性能仍需验证。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T13:11:03Z
- **目录日期**: 2025-04-07
