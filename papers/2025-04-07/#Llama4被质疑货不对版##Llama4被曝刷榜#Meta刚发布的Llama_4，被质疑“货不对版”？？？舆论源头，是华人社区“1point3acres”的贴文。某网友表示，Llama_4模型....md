...
Meta新发布的Llama 4大模型陷入争议。社区爆料称其实际表现与SOTA差距较大，有员工因不满"刷榜"要求辞职。TechCrunch指出Meta用于榜单测试的是"特别优化版"，与开发者可下载版本不同。官网确认上榜的是"实验性聊天版"，该版本回答风格（频繁使用emoji、冗长）与正式版明显不同。Llama 4系列包含三个版本，其中2万亿参数的Behemoth号称超越GPT-4.5等模型，但实际性能仍需验证。目前争议焦点在于榜单模型与发布版本是否存在性能偏差。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T10:02:42Z
- **目录日期**: 2025-04-07
