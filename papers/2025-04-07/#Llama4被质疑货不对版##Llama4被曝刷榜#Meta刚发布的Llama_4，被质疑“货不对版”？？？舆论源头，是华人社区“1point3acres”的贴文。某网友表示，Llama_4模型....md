...
Meta最新发布的Llama 4大模型陷入争议。有爆料称其实际表现与宣传不符，员工因被要求训练"看起来可以"的结果而辞职。TechCrunch指出Meta用于刷榜的是"实验性聊天版"，与开发者可下载的正式版存在差异（如频繁使用emoji和冗长回答）。官方宣称Llama 4系列（含Scout、Maverick和2万亿参数的Behemoth）在多任务上超越GPT-4o等竞品，但模型真实性引发质疑。目前尚需时间验证其实际性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T19:03:38Z
- **目录日期**: 2025-04-07
