...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用多模态数据提升机器人策略学习。UWM在统一Transformer架构中独立控制各模态扩散时间步，灵活实现策略、动态预测和视频生成。实验表明：（1）UWM在大规模多任务数据集上预训练，比模仿学习获得更具泛化性和鲁棒性的策略；（2）通过独立调控模态扩散时间步，UWM能有效利用无动作标注视频数据提升策略微调效果。该框架为整合异构数据集、统一模仿学习与世界建模提供了新思路。项目地址：https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T01:29:07Z
- **目录日期**: 2025-04-07
