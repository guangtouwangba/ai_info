...
模仿学习是构建通用机器人的重要方法，但依赖高质量专家演示数据限制了其扩展性。本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用多模态数据（含动作和无动作视频）进行策略学习。UWM能灵活表示策略、动态预测和视频生成，在大规模多任务机器人数据集上预训练后，比单纯模仿学习获得更通用、鲁棒的策略。实验表明，UWM通过独立控制各模态扩散步骤，能有效利用无动作视频数据提升策略微调性能，为整合模仿学习与世界建模提供了统一框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T02:29:33Z
- **目录日期**: 2025-04-07
