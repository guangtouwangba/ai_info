...
模仿学习是构建通用机器人的重要方法，但依赖高质量专家演示限制了其扩展性。本研究提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。实验表明：1）UWM在大规模多任务机器人数据集上预训练效果优于模仿学习，产生更泛化稳健的策略；2）通过独立控制模态扩散步骤，UWM能有效利用无动作视频数据提升策略性能。该框架为整合异构数据集提供了新思路，弥合了模仿学习与世界建模的鸿沟。代码和视频见项目网页。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T00:01:48Z
- **目录日期**: 2025-04-07
