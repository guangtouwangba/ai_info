...
该研究提出SGD-SaI方法，质疑深度神经网络训练中自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题，并将内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中表现优于或等同AdamW，且对超参数更鲁棒。内存效率显著提升，如在GPT-2和Llama2-7B训练中分别节省5.93GB和25.15GB。该方法也适用于LoRA微调等任务，展现出广泛实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T02:32:21Z
- **目录日期**: 2025-04-07
