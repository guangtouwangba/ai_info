...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将内存使用减半。实验表明，在Transformer任务、ImageNet分类和GPT-2预训练中，其性能媲美或超越AdamW，且对超参数更鲁棒。内存效率显著，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB。在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T01:32:07Z
- **目录日期**: 2025-04-07
