...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量随机梯度下降(SGDM)，通过在初始化时根据梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将优化器内存占用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，在LoRA微调中超越现有优化器。内存效率显著，为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T04:02:35Z
- **目录日期**: 2025-04-07
