...
该论文提出了一种名为SGD-SaI的新优化方法，取代了传统自适应梯度方法（如Adam）。该方法基于初始化时的学习率调整，通过梯度信号噪声比（g-SNR）对不同参数组进行优化，避免了训练不平衡问题，并将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和大型语言模型（如GPT-2、Llama2-7B）预训练中表现优于或相当于AdamW，同时显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。此外，它在LoRA微调等任务中也优于现有优化器，具有鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T22:03:57Z
- **目录日期**: 2025-04-07
