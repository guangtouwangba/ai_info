...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，有效防止训练不平衡，且无需自适应二阶动量。该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，与AdamW相当或更好，同时内存效率更高（如为GPT-2节省5.93GB内存）。SGD-SaI还展现出超参数鲁棒性，在LoRA微调等任务中优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T06:04:07Z
- **目录日期**: 2025-04-07
