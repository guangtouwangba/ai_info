...
该论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。该方法基于梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或媲美AdamW，同时内存效率显著提升（如为Llama2-7B节省25GB内存）。该方法还展现出超参数鲁棒性，并在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T00:04:29Z
- **目录日期**: 2025-04-07
