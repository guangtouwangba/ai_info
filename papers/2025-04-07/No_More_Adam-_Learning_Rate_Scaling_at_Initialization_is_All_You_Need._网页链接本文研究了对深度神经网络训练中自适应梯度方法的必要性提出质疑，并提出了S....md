...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据各参数组的梯度信噪比（g-SNR）调整学习率，有效防止训练不平衡。相比AdamW，该方法内存效率更高（节省5.93-25.15GB），在Transformer、ImageNet分类、GPT-2预训练和LoRA微调等任务中表现相当或更优，同时具有超参数鲁棒性。该方法解决了用SGD训练Transformer的难题，为大规模模型训练提供了更高效的优化方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T19:04:08Z
- **目录日期**: 2025-04-07
