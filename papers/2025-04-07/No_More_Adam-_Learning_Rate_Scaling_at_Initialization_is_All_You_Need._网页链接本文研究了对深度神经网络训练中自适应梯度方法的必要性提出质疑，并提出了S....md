...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化阶段对不同参数组按梯度信噪比（g-SNR）调整学习率，无需自适应二阶动量即可避免训练失衡。该方法在Transformer任务、ImageNet分类和LLM预训练中表现优于AdamW，内存效率提升显著（如为Llama2-7B节省25.15GB），且在LoRA微调等任务中也超越现有优化器，同时保持超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T14:04:36Z
- **目录日期**: 2025-04-07
