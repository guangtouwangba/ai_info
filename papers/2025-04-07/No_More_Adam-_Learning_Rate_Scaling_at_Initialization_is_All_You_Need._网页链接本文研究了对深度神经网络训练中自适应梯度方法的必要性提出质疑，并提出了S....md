...
该论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，有效防止训练不平衡，同时将内存占用减半。实验表明，该方法在Transformer任务、ImageNet分类及大模型（如GPT-2、Llama2）预训练中表现优于或媲美AdamW，且具有超参数鲁棒性。在LoRA微调等任务中也优于现有优化器，并为大模型节省显著内存（如Llama2-7B节省25.15GB）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T05:04:01Z
- **目录日期**: 2025-04-07
