...
该论文提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。该方法改进带动量SGD，通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，避免训练失衡且内存用量减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中媲美或超越AdamW，并显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。其超参数鲁棒性强，在LoRA微调等场景也优于现有优化器，为大规模模型训练提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T13:11:40Z
- **目录日期**: 2025-04-07
