...
该论文提出SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量SGD，通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题，并将内存占用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中媲美或优于AdamW，在LoRA微调中表现更佳。内存效率显著，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存。该方法解决了SGD训练Transformer的难题，具有超参数鲁棒性和广泛应用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T03:18:47Z
- **目录日期**: 2025-04-07
