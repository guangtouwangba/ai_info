...
APIGen-MT是一个两阶段框架，用于生成高质量的多轮AI代理交互数据。第一阶段通过LLM评审委员会和迭代反馈生成详细任务蓝图，第二阶段将其转化为完整交互轨迹。基于该框架训练的xLAM-2-fc-r系列模型（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5，小模型在多轮交互中表现尤其突出且保持良好一致性。研究证明该方法能产生高质量训练数据，提升代理的可靠性、效率和能力。相关数据和模型已在HuggingFace开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T13:08:52Z
- **目录日期**: 2025-04-07
