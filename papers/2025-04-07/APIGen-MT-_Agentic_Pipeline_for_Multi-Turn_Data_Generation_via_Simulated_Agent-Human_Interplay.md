...
APIGen-MT是一个两阶段框架，用于生成高质量的多轮AI代理交互数据。第一阶段通过LLM评审委员会和迭代反馈创建详细任务蓝图，第二阶段将其转化为完整交互轨迹。基于该数据训练的xLAM-2-fc-r系列模型（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5，其中较小模型在多轮交互中表现更优且更稳定。该方法通过验证的"蓝图到细节"流程生成可靠训练数据，显著提升代理性能。研究团队开源了合成数据和模型（HuggingFace可获取），推动AI代理领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T05:01:42Z
- **目录日期**: 2025-04-07
