...
APIGen-MT是一个两阶段框架，用于生成可验证且多样化的多轮AI代理训练数据。第一阶段通过LLM评审委员会和迭代反馈生成包含真实动作的任务蓝图，第二阶段将这些蓝图转化为完整的人机交互轨迹。基于该框架训练的xLAM-2-fc-r系列模型（1B-70B参数）在τ-bench和BFCL基准测试中超越了GPT-4o和Claude 3.5等前沿模型，其中较小模型在多轮交互中表现更优且更稳定。研究证实该框架能产生高质量训练数据，相关模型和数据已在HuggingFace开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T17:01:43Z
- **目录日期**: 2025-04-07
