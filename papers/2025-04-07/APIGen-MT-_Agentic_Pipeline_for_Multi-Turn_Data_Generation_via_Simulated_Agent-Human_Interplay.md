...
APIGen-MT是一个两阶段框架，用于生成可验证且多样化的多轮AI代理训练数据。第一阶段通过LLM评审委员会和迭代反馈生成带真实动作的任务蓝图，第二阶段将这些蓝图转化为完整的人机交互轨迹。基于该框架训练的xLAM-2-fc-r系列模型（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5，其中小模型在多轮交互中表现尤为突出且保持稳定性。研究证实该方法的有效性，相关数据和模型已在HuggingFace开源（https://huggingface.co/collections/Salesforce/xlam-2-67ef5be12949d8dcdae354c4）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T07:01:54Z
- **目录日期**: 2025-04-07
