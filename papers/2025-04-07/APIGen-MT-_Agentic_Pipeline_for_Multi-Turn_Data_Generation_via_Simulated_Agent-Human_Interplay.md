...
APIGen-MT是一个两阶段框架，用于生成可验证且多样化的多轮AI代理交互数据。第一阶段通过LLM评审委员会和迭代反馈生成带真实动作的任务蓝图，第二阶段将这些蓝图转化为完整交互轨迹。基于该框架训练的xLAM-2-fc-r系列模型（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5等前沿模型，其中小模型在多轮交互中表现尤为突出且稳定性更佳。研究证实该框架能产生高质量训练数据，开发出更可靠高效的AI代理。相关数据和模型已在HuggingFace开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T12:01:56Z
- **目录日期**: 2025-04-07
