...
PAFFA是一种新型AI代理方法，通过构建"动作库"显著提升LLM在网页任务中的效率和准确性。该方法利用LLM的预训练知识预计算可复用的浏览器交互模式，采用两种策略："Dist-Map"（任务无关的关键元素识别）和"Unravel"（新任务/网站的探索式学习）。相比传统HTML解析方法，PAFFA减少87%推理token消耗，同时将步骤准确率从0.50提升至0.57。其独特之处在于能通过探索更新动作库，适应未见网站，证明LLM推理序列具有跨提示泛化能力，为互联网规模数据提供了次线性token增长的推理方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T05:02:09Z
- **目录日期**: 2025-04-07
