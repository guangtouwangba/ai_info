...
研究人员提出PAFFA方法，通过构建"动作库"预计算浏览器交互模式，显著提升LLM在网页任务中的效率和准确性。该方法采用"Dist-Map"（任务无关的关键元素识别）和"Unravel"（新任务/网站的探索式学习）两种策略，将推理时间token减少87%，同时保持0.57的步骤准确率（基线为0.50）。Unravel还能基于探索更新动作库，适应未见网站。研究表明LLM推理序列可跨提示泛化，为互联网规模数据提供了次线性token增长的推理方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T17:02:10Z
- **目录日期**: 2025-04-07
