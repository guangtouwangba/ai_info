...
PAFFA是一种新型AI代理方法，通过构建"动作库"显著提升LLM在网页任务中的效率和准确性。该方法利用LLM的预训练知识预先计算可跨任务复用的浏览器交互模式，采用"Dist-Map"(任务无关元素识别)和"Unravel"(首次探索新网站)两种策略，将推理token减少87%的同时保持良好性能(步骤准确率0.57 vs 基线0.50)。Unravel还能通过探索更新动作库，适应未见网站。研究表明LLM推理序列具有跨提示泛化能力，为互联网规模数据提供了次线性token增长的推理方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-07T23:02:05Z
- **目录日期**: 2025-04-07
