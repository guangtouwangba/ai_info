...
随着大语言模型(LLMs)的快速发展，传统推理流程已无法满足多阶段任务(RAG、KV缓存检索、动态路由等)的异构计算需求。为此，研究者提出HERMES异构多阶段LLM推理模拟器，它能精准建模不同请求阶段在复杂硬件架构(CPU/GPU/ASIC等)上的执行过程，支持多模型并发、高级批处理策略和多级内存层次。通过结合真实硬件追踪与分析建模，HERMES能评估内存带宽争用、集群通信延迟等关键权衡，为混合部署提供优化依据，助力系统设计者应对下一代AI工作负载的软硬件协同设计挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T09:02:54Z
- **目录日期**: 2025-04-17
