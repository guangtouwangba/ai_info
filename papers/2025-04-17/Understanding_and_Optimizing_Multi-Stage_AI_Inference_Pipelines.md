...
随着大语言模型(LLM)推理流程日趋复杂，现有模拟器难以准确评估异构硬件架构的性能。为此，研究者提出了HERMES异构多阶段LLM推理模拟器，它能精确建模RAG检索增强生成、KV缓存检索、动态路由和多步推理等多样化工作负载，支持GPU/ASIC/CPU混合架构的协同仿真。通过结合真实硬件追踪和分析建模，HERMES能捕捉内存带宽争用、集群通信延迟等关键性能指标，为混合批处理策略和远程KV缓存检索等场景提供优化洞见。该工具填补了复杂LLM推理系统在硬件-软件协同设计领域的仿真空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T05:02:52Z
- **目录日期**: 2025-04-17
