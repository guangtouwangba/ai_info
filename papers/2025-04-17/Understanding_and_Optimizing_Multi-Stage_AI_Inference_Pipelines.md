...
随着大语言模型(LLMs)的快速发展，传统推理流程已无法满足多阶段处理需求。为应对这一挑战，研究者提出了HERMES异构多阶段LLM推理模拟器。该系统能精准模拟RAG检索增强生成、KV缓存检索、动态模型路由等多阶段工作负载，支持GPU、ASIC、CPU等异构硬件架构的协同运行。通过结合真实硬件追踪和分析建模，HERMES能有效评估内存带宽争用、集群通信延迟等关键指标，为混合部署提供最优批处理策略。该模拟器为下一代AI工作负载的软硬件协同设计提供了重要决策依据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T22:01:48Z
- **目录日期**: 2025-04-17
