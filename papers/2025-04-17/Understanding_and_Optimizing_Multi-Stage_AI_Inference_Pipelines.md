...
随着大语言模型(LLMs)的快速发展，现有的推理流程模拟器难以准确模拟包含检索增强生成(RAG)、KV缓存检索等多阶段异构计算任务。为此，研究者提出了HERMES异构多阶段LLM推理模拟器，它能精确建模RAG、KV检索、推理等不同阶段在复杂硬件架构上的执行过程。HERMES支持多模型并发执行，采用先进的批处理策略，并整合了真实硬件追踪数据，可分析内存带宽争用、集群间通信延迟等关键权衡因素。通过案例研究，HERMES为优化下一代AI工作负载的软硬件协同设计提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T11:02:47Z
- **目录日期**: 2025-04-17
