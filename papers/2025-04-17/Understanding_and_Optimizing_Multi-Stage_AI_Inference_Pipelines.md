...
随着大语言模型(LLMs)的快速发展，其推理流程日益复杂，需要整合GPU、ASIC、CPU等多种硬件。现有模拟器难以准确建模这种异构多阶段工作流。为此，研究者提出了HERMES异构多阶段LLM推理模拟器，能精确模拟RAG、KV缓存检索、推理、预填充和解码等多个阶段在复杂硬件架构中的执行。HERMES支持多模型并发执行，采用先进批处理策略和多级内存层次，通过真实硬件追踪与分析建模相结合，捕捉内存带宽争用、集群间通信延迟等关键权衡。该工具为优化下一代AI工作负载的软硬件协同设计提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T12:02:54Z
- **目录日期**: 2025-04-17
