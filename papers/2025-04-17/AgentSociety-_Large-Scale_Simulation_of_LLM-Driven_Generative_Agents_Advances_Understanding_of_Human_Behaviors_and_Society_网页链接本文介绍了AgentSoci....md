...
这篇论文介绍了AgentSociety，这是一个基于大语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（共500万次）。研究团队用它分析了四个关键社会问题：社会极化、刺激性信息传播、基本收入政策影响和外部冲击（如飓风）效应。模拟结果与现实世界数据高度吻合，验证了该平台在揭示人类行为机制方面的可靠性。该工具为社会科学研究和政策制定提供了新方法，能有效补充传统调查和实验的不足，展现出强大的应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T06:05:59Z
- **目录日期**: 2025-04-17
