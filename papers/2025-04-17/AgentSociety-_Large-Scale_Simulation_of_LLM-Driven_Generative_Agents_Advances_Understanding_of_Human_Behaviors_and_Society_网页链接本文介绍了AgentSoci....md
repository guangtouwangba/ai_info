...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个智能代理的社会互动（500万次交互）。该系统通过四个关键社会议题（社会极化、信息传播、基本收入政策、自然灾害影响）验证了其有效性，模拟结果与现实数据高度吻合。研究表明，该系统能有效捕捉人类行为模式和社会机制，可作为计算社会科学研究和政策制定的有力工具，补充传统研究方法（如调查、访谈）的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T18:04:41Z
- **目录日期**: 2025-04-17
