...
本文介绍了AgentSociety，这是一个基于大语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（500万次）。研究者用它分析了四个关键社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界数据吻合，验证了该工具在捕捉人类行为机制方面的有效性。AgentSociety为社会科学研究和政策制定提供了新方法，能辅助传统研究手段（如调查和访谈），帮助理解社会问题的模式和成因。该技术展现了LLM在社会模拟中的实用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T21:04:46Z
- **目录日期**: 2025-04-17
