...
DeepMath-103K是一个专为强化学习（RL）设计的大规模数学推理数据集，包含约10.3万个高难度数学问题（主要为5-9级）。该数据集通过严格筛选、去污染处理，确保与评估基准无重叠，并提供可验证的答案格式以支持基于规则的RL训练。每个问题附带三个不同解决方案，适用于监督微调或知识蒸馏等训练方法。实验表明，基于该数据集训练的模型在复杂数学基准测试中表现显著提升。数据集已开源，旨在推动AI推理能力的发展。项目地址：https://github.com/zwhe99/DeepMath。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T02:28:55Z
- **目录日期**: 2025-04-17
