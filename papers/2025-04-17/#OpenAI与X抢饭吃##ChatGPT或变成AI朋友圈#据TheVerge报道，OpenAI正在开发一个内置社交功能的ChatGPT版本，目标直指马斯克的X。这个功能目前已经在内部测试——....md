...
OpenAI正在测试ChatGPT的社交功能版本，旨在打造AI内容社区，直接对标马斯克的X平台。新功能允许用户分享AI生成内容（如图像、段子），并支持互动点赞评论。该设计类似X的信息流界面，但内容全部由AI生成或优化，包括自动配图、风格转换（如照片变宫崎骏插画）等。此举将ChatGPT从工具转型为实时内容平台，既能获取用户数据训练模型理解流行文化，也标志着AI与社交的深度结合。这被视为OpenAI与马斯克、扎克伯格在社交领域的正面竞争。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T14:04:05Z
- **目录日期**: 2025-04-17
