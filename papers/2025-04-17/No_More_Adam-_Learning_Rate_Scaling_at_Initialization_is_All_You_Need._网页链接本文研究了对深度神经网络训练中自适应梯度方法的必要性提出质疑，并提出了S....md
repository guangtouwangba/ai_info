...
该研究提出SGD-SaI方法，质疑深度神经网络训练中自适应梯度方法（如Adam）的必要性。该方法通过初始化时根据不同参数组的梯度信噪比(g-SNR)调整学习率，解决了传统SGD训练不平衡问题，并将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或持平AdamW，特别在GPT-2和Llama2-7B训练中分别节省5.93GB和25.15GB内存。该方法对超参数变化鲁棒，在LoRA微调等任务中也优于现有优化器，展示了高效性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T04:04:42Z
- **目录日期**: 2025-04-17
