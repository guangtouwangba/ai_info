...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法是对带动量的随机梯度下降(SGDM)的改进，通过根据参数组的梯度信号噪声比(g-SNR)在初始化时调整学习率，避免了训练不平衡问题，并将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或相当于AdamW，且在LoRA微调等任务中优于现有优化器。内存效率方面，相比AdamW，SGD-SaI为GPT-2和Llama2-7B分别节省了5.93GB和25.15GB内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T10:05:38Z
- **目录日期**: 2025-04-17
