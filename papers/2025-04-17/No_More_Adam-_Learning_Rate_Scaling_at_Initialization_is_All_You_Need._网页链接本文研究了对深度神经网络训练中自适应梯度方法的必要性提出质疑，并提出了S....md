...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的必要性。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化阶段根据各参数组的梯度信噪比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，该方法内存占用减半，在Transformer任务、ImageNet分类及大模型（如GPT-2、Llama2-7B）预训练中表现相当或更优，同时显著节省内存（最高达25GB）。其鲁棒性强，在LoRA微调等场景也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T06:05:21Z
- **目录日期**: 2025-04-17
