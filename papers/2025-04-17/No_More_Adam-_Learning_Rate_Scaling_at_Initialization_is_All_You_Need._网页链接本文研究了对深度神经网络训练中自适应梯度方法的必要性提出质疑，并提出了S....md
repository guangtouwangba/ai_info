...
该研究提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。该方法基于梯度信号噪声比（g-SNR）在初始化时对不同参数组进行学习率调整，无需自适应二阶动量。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或相当于AdamW，同时显著降低内存占用（如为GPT-2节省5.93GB内存）。此外，它在LoRA微调等任务中也优于现有优化器，并展现出优异的超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T01:32:27Z
- **目录日期**: 2025-04-17
