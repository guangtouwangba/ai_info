...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI是对带动量的随机梯度下降(SGDM)的改进，通过根据梯度信号噪声比(g-SNR)在初始化时调整不同参数组的学习率。相比AdamW，它无需二阶动量，内存使用减半，能防止训练不平衡。实验显示，在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优异，甚至超越现有优化器。内存效率方面，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T08:05:31Z
- **目录日期**: 2025-04-17
