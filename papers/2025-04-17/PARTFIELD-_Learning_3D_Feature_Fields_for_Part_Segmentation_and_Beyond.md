...
PartField是一种基于部件的3D特征学习方法，通过前馈网络自动捕捉部件概念及其层级结构，无需依赖预定义模板或文本标签，适用于开放世界的多种3D数据模态。该方法仅需单次3D前馈推理，显著提升了运行速度和鲁棒性。通过对比学习框架，模型从标注数据集和大规模无监督图像分割中蒸馏2D/3D部件提案，生成连续特征场，可聚类得到层级部件分解。实验表明，PartField比同类方法准确率提升20%，速度快数个量级，并支持跨形状的一致性任务（如协同分割和对应关系）。详见项目网页。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T02:29:05Z
- **目录日期**: 2025-04-17
