...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，关注人类偏好对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发需求。同时构建了20B标记的SynCode-Instruct合成指令数据集用于模型微调。实验发现：1)传统执行测试与人类偏好存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型与人类开发者偏好对齐的重要性，为改进模型提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T09:06:42Z
- **目录日期**: 2025-04-17
