...
本文提出了一种基于逆向最优控制（IOC）的可解释AI（XAI）方法，用于分析黑盒数据驱动控制器在特定区域的行为。该方法通过提取控制器优化的隐式成本函数中的权重（跟踪误差和控制力度），提供更透明、可解释的控制器目标表示。与局部可解释模型无关解释（LIME）类似，该方法基于控制策略的局部近似，但通过解决逆向线性二次（LQ）问题，提供更具结构化和控制相关性的解释，而非简单的敏感性分析。数值实验表明，推断的成本函数能更深入理解控制器的决策过程，揭示反直觉或意外现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T01:29:28Z
- **目录日期**: 2025-04-17
