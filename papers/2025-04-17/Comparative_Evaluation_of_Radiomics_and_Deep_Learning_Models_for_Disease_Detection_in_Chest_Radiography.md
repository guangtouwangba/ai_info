...
该研究系统评估了基于放射组学和深度学习的AI模型在胸部X光疾病检测（COVID-19、肺部混浊和病毒性肺炎）中的表现。研究对比了决策树、梯度提升、随机森林、SVM和MLP等放射组学模型与CNN、ViT等深度学习架构的诊断准确性和鲁棒性。结果显示，不同模型在不同样本量下各有优势：深度学习擅长大数据场景，而放射组学在数据有限时更具优势。研究为临床实践中AI诊断工具的选择提供了实用指导，特别是在需要快速可靠诊断的高通量医疗环境中，填补了模型选择标准的关键空白。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T09:02:33Z
- **目录日期**: 2025-04-17
