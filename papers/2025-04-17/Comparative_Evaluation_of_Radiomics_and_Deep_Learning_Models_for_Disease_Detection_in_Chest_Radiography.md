...
该研究系统评估了基于放射组学和深度学习的AI模型在胸部X光疾病检测（COVID-19、肺部混浊和病毒性肺炎）中的表现。深度学习方法（如CNN和ViT）直接从图像学习，而放射组学方法提取定量特征，在数据有限时可能更具优势。研究比较了决策树、梯度提升、随机森林、SVM、MLP等传统方法与前沿计算机视觉架构的性能，分析不同样本量下的准确性和鲁棒性。结果表明不同AI模型各有优势，为临床实践中根据具体需求选择合适模型提供了指导，尤其对自动化高通量诊断环境具有重要意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T17:01:39Z
- **目录日期**: 2025-04-17
