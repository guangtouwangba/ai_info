...
这篇论文介绍了FLIP数据集，用于评估AI在顺序推理、视觉叙事和常识方面的能力。该数据集基于Idena区块链的人类验证任务，要求从两组4张图片中选出逻辑连贯的排序。实验测试了前沿的视觉语言模型(VLM)和大型语言模型(LLM)，发现最佳开源和闭源模型的零样本准确率仅为75.5%和77.9%，远低于人类的95.3%。使用图像描述模型辅助推理可将准确率从69.6%提升至75.2%，而15个模型的集成方法能达到85.2%。研究凸显了现有推理模型的局限性，强调了FLIP等多模态基准的重要性。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T18:01:16Z
- **目录日期**: 2025-04-17
