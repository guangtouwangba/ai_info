...
本文提出了一种基于序贯蒙特卡罗采样的贝叶斯不确定性量化框架，用于解决基于智能体的模拟模型缺乏不确定性量化能力的问题。该框架通过将智能体模拟与贝叶斯建模相结合，能够根据静态或实时数据流提供疾病传播参数的分布估计，为公共卫生决策提供支持。该方法不仅降低了模型误差，还能生成一系列可能的现实结果，同时解决了智能体模型的两大局限：缺乏不确定性量化和数据同化能力。该框架兼具智能体模型的灵活性和贝叶斯方法的不确定性量化优势，且适用于高性能计算系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T08:02:41Z
- **目录日期**: 2025-04-17
