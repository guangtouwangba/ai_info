...
本文提出了一种基于贝叶斯框架和序贯蒙特卡洛采样的集成不确定性量化方法，用于解决基于代理的传染病传播模型缺乏实时数据同化和不确定性量化的问题。该方法通过将代理模拟与贝叶斯建模相结合，能够快速估计疾病传播参数的概率分布，为突发公共卫生事件提供实时决策支持。相比传统多次运行取统计值的方法，新框架显著提高了计算效率，降低了模型误差，并能生成合理的可能结果范围。研究还展示了该方法在静态和流式数据场景下的算法细节和模拟疫情测试结果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T20:01:46Z
- **目录日期**: 2025-04-17
