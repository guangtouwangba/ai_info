...
本文提出了一种基于序贯蒙特卡罗采样的贝叶斯不确定性量化框架，用于解决基于代理的模拟模型缺乏内置不确定性分析的问题。该框架通过将代理模型与贝叶斯建模相结合，能够利用静态或实时数据来估计疾病传播参数分布，为公共卫生决策提供支持。该方法降低了模型误差，提供了可能的真实结果范围，同时解决了代理模型的两大局限：缺乏不确定性量化和无法同化数据。该框架兼具代理模型的灵活性和贝叶斯范式的不确定性量化能力，且适用于高性能计算系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T21:01:52Z
- **目录日期**: 2025-04-17
