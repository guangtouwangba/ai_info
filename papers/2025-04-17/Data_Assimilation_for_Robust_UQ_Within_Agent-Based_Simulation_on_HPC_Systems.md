...
本文提出了一种基于序贯蒙特卡洛采样的贝叶斯不确定性量化框架，用于解决基于智能体模拟(ABM)缺乏不确定性量化(UQ)和数据同化能力的问题。该框架通过将ABM与贝叶斯建模结合，能够利用静态或实时数据快速估计疾病传播参数分布，为公共卫生决策提供支持。相比传统多次运行取统计值的方法，该方法降低了模型误差，能提供更真实的可能结果范围，同时适用于高性能计算系统。研究展示了在模拟疫情爆发中处理静态和实时数据的能力，为动态系统建模提供了有效工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T19:01:44Z
- **目录日期**: 2025-04-17
