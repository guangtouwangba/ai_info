...
本文提出了一种基于序贯蒙特卡罗采样的贝叶斯不确定性量化框架，用于解决基于代理的模拟模型(ABMs)缺乏内置不确定性量化方法的问题。该框架将ABMs与贝叶斯建模相结合，能够处理静态和实时数据流，快速估计疾病传播参数的概率分布，为公共卫生决策提供支持。该方法降低了模型误差，提供了可能的现实结果范围，同时解决了ABMs的两大局限：缺乏不确定性量化和无法同化数据。研究展示了该框架在高性能计算系统中的可扩展性，并通过模拟疫情爆发案例验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T14:02:46Z
- **目录日期**: 2025-04-17
