...
本文提出了一种基于序贯蒙特卡洛采样的贝叶斯不确定性量化框架，用于解决基于智能体的模拟模型（ABMs）缺乏内置不确定性量化方法的问题。该框架通过将ABMs与贝叶斯建模相结合，能够快速估计新兴病原体传播参数的概率分布，为公共卫生决策提供实时支持。相比传统多次运行取统计值的方法，该方案显著降低了计算时间需求，同时解决了ABMs的两大局限：无法量化不确定性和难以同化实时数据。研究展示了该框架在静态和流式数据场景下的算法细节及疫情模拟结果，证明其能有效减少模型误差并生成合理结果范围。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T22:01:39Z
- **目录日期**: 2025-04-17
