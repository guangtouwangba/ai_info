...
本文提出了一种基于序贯蒙特卡罗采样的贝叶斯不确定性量化框架，用于解决基于代理的传染病传播模型缺乏实时数据同化和不确定性量化的问题。该框架通过将代理模拟与贝叶斯建模结合，在保持模型灵活性的同时，能够快速估计疾病传播参数的概率分布，为突发公共卫生事件提供实时决策支持。相比传统多次运行取统计值的方法，新方法显著降低了计算时间，减少了模型误差，并能处理静态和流式数据。研究展示了该框架在高性能计算系统上的可扩展性，并通过模拟疫情爆发案例验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T10:02:49Z
- **目录日期**: 2025-04-17
