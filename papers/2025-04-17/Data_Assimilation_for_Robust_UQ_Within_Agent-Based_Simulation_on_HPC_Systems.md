...
本文提出了一种基于序贯蒙特卡洛采样的贝叶斯不确定性量化框架，用于解决基于智能体的模拟模型（ABMs）缺乏不确定性量化（UQ）和实时数据同化能力的问题。该框架通过将ABMs与贝叶斯建模相结合，能够快速估计新兴病原体传播参数的概率分布，为公共卫生决策提供支持。该方法降低了模型误差，生成了一系列可能的现实结果，并克服了传统ABMs计算耗时的限制。研究还展示了该框架在静态和流式数据模拟疫情暴发中的应用效果，验证了其在高效计算系统上的可扩展性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T04:02:24Z
- **目录日期**: 2025-04-17
