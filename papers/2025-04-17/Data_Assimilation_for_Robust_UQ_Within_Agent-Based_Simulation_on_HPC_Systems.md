...
本文提出了一种基于序贯蒙特卡洛采样的贝叶斯不确定性量化框架，用于解决基于智能体的模拟模型（ABMs）缺乏内置不确定性量化方法的问题。该框架通过将ABMs与贝叶斯建模相结合，能够快速估计疾病传播参数分布，为公共卫生决策提供实时支持。相比传统多次运行取统计值的方法，该方案显著降低了计算时间，同时减少了模型误差并生成更真实的结果范围。特别解决了ABMs无法量化不确定性和同化数据的两大局限，兼具ABMs的灵活性和贝叶斯的不确定性量化优势，且适用于高性能计算系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T06:02:39Z
- **目录日期**: 2025-04-17
