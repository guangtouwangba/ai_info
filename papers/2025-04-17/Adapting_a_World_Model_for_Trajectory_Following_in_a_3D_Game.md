...
模仿学习通过专家知识训练智能体，轨迹复现是其核心环节。在复杂3D游戏环境中，分布偏移和随机性要求采用超越简单动作回放的稳健方法。本研究在游戏《Bleeding Edge》中应用不同编码器和策略头的逆动力学模型（IDM）进行轨迹跟踪，并探索多种未来对齐策略以应对随机不确定性和智能体缺陷导致的分布偏移。实验测量轨迹偏差距离和首次显著偏离点，结果显示：多样数据下，GPT式策略头配合从头训练编码器表现最佳；少数据时DINOv2编码器配GPT头最优；预训练后微调时，GPT与MLP策略头效果相当。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T10:01:58Z
- **目录日期**: 2025-04-17
