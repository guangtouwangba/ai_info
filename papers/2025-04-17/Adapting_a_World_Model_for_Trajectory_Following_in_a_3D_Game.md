...
模仿学习通过专家知识训练智能体，轨迹复现是其关键环节。在复杂3D游戏（如《Bleeding Edge》）中，分布偏移和随机性需要超越简单动作重放的鲁棒方法。本研究采用不同编码器和策略头的逆动力学模型（IDM）进行轨迹跟踪，并探究多种未来对齐策略以解决随机不确定性和智能体缺陷导致的分布偏移。实验通过轨迹偏差距离和首次显著偏离点评估性能，结果显示：多样化数据场景中，GPT风格策略头+从头训练编码器最优；低数据场景中，DINOv2编码器+GPT策略头最佳；预训练模型微调时，GPT与MLP策略头表现相当。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T18:00:55Z
- **目录日期**: 2025-04-17
