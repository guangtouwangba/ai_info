...
模仿学习通过专家知识训练智能体，其中轨迹复现是关键环节。针对3D游戏等复杂环境中的分布偏移和随机性，本研究在《Bleeding Edge》游戏中测试了不同编码器和策略头的逆向动力学模型（IDM），并探索了应对不确定性和智能体缺陷的未来对齐策略。实验测量了轨迹偏差距离和首次显著偏离点，结果显示：多样化数据下，GPT风格策略头配合从头训练的编码器最优；低数据时DINOv2编码器配GPT策略头最佳；在预训练后微调场景中，GPT和MLP风格策略头表现相当。配置效果取决于具体场景设置。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T08:01:48Z
- **目录日期**: 2025-04-17
