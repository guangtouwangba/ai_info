...
模仿学习通过专家知识训练智能体，轨迹复现是其关键环节。在复杂环境（如3D游戏）中，分布偏移和随机性要求采用超越简单动作回放的鲁棒方法。本研究在游戏《Bleeding Edge》中应用不同编码器和策略头的逆动力学模型（IDM）进行轨迹跟踪，并探索多种未来对齐策略以解决随机不确定性和智能体缺陷导致的分布偏移。结果显示：在多样化数据场景中，GPT风格策略头配合从头训练的编码器表现最佳；低数据量时DINOv2编码器+GPT策略头最优；预训练后微调时，GPT与MLP策略头效果相当。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T17:01:00Z
- **目录日期**: 2025-04-17
