...
模仿学习通过专家知识训练智能体，轨迹复现是其核心环节。在复杂环境（如3D游戏）中，分布偏移和随机性要求超越简单动作回放的稳健方法。本研究在游戏《Bleeding Edge》中应用不同编码器和策略头的逆动力学模型（IDM），并探索解决随机不确定性和智能体缺陷导致的分布偏移的未来对齐策略。实验测量轨迹偏差距离和首次显著偏离点，结果表明：多样数据下，GPT式策略头+从头训练编码器最优；少数据时，DINOv2编码器+GPT头最佳；预训练后微调时，GPT头和MLP头表现相当。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-17T07:01:53Z
- **目录日期**: 2025-04-17
