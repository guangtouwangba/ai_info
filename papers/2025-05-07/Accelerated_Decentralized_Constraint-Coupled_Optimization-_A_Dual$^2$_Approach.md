...
本文研究了一类去中心化的约束耦合优化问题，提出基于新型双重平方（dual²）方法的两种加速算法：iD2A和MiD2A。相较于现有方法，这两种算法在目标函数h的条件更宽松时仍能保证渐近收敛，并在附加假设下实现线性收敛。通过采用专门的鞍点子问题求解器，iD2A和MiD2A在不同场景下显著降低了通信和计算复杂度。数值实验验证了理论结果，并证明新算法在实际中具有优越性能。该研究为多智能体网络中的分布式优化提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T15:01:44Z
- **目录日期**: 2025-05-07
