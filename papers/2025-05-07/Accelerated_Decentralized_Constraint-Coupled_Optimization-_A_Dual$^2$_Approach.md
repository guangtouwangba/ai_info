...
本文研究一类去中心化的约束耦合优化问题，提出两种基于新型双重加速方法（Dual²）的算法：iD2A和MiD2A。相比现有方法，这两种算法在更宽松的条件下保证渐近收敛性，并在附加假设下实现线性收敛。通过采用专门的鞍点子问题求解器，iD2A和MiD2A在各种场景下显著降低了通信和计算复杂度。数值实验验证了理论结果，并展示了算法在实际中的优越性能。该研究为多智能体网络中的分布式优化提供了更高效的解决方案。
