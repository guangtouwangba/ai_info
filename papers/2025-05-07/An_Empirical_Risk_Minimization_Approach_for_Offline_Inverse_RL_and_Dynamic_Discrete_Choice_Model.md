...
本文提出了一种基于梯度下降的全局收敛方法，用于解决动态离散选择（DDC）模型和离线最大熵正则化逆向强化学习（MaxEnt-IRL）问题。该方法通过引入基于经验风险最小化（ERM）的框架，避免了传统方法中需要显式估计状态转移概率的限制，并兼容神经网络等非参数化估计技术，适用于高维甚至无限状态空间。理论分析表明，贝尔曼残差满足Polyak-Lojasiewicz（PL）条件，确保了快速全局收敛。实验证明，该方法在合成数据集上优于基准方法和现有最优方案，展现了优越性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T11:02:30Z
- **目录日期**: 2025-05-07
