...
这项研究探讨了利用大语言模型（LLM）增强智能家居自动化并促进家庭可持续实践的应用。研究发现，GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但仍存在语法错误等问题。尽管"绿色"与"非绿色"提示在量化差异不大，但定性反馈显示环保提示能促进可持续性行为。一项56人的实证评估表明，相比传统规则系统，用户更接受并喜爱该LLM系统。研究凸显了LLM在推动智能家居技术进步中的作用，建议进一步优化模型以支持可持续生活的实际应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T01:29:14Z
- **目录日期**: 2025-05-07
