...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案通过时间表示对齐（TREPA）方法解决扩散过程中的时间一致性问题，利用自监督视频模型提取的时间特征对齐生成帧。研究还改进了SyncNet训练框架，优化模型架构、超参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括直接建模音视频关联的扩散模型、TREPA时间一致性增强方案，以及改进的SyncNet训练方法，这些经验可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T13:13:49Z
- **目录日期**: 2025-05-07
