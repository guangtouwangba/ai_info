...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模音频-视觉关系而无需中间运动表示。针对扩散模型的时间一致性问题，提出了时间表示对齐(TREPA)方法增强连贯性。同时改进了SyncNet训练框架，解决了收敛问题并提升准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。创新点包括：1)首个潜在扩散唇同步框架；2)TREPA时间对齐技术；3)SyncNet训练优化方案。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T21:05:14Z
- **目录日期**: 2025-05-07
