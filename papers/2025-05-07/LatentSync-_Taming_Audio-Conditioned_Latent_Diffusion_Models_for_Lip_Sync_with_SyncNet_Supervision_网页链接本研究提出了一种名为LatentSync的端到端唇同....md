...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示，直接建模音频-视觉关联。针对扩散模型的时间一致性问题，提出TREPA方法，利用自监督视频模型的时间表示增强帧间一致性。同时优化了SyncNet训练框架，改进了模型架构、超参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进也可应用于其他音频驱动的人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T02:34:24Z
- **目录日期**: 2025-05-07
