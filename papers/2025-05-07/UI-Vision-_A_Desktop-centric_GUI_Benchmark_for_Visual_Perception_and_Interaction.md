...
UI-Vision是首个针对桌面环境的综合性开源基准测试，用于评估GUI自动化代理的离线细粒度性能。它包含83款软件的人类操作标注（边界框、UI标签、点击/拖拽/键盘输入等行为轨迹），提供三个层级任务：元素定位、布局定位和动作预测。评估发现当前最先进模型（如UI-TARS-72B）在专业软件理解、空间推理和复杂操作（如拖拽）方面存在明显缺陷。该基准通过开源发布，旨在推动现实桌面任务中自主代理的发展，解决现有研究因数据收集和许可问题对桌面环境关注不足的现状。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T18:02:16Z
- **目录日期**: 2025-05-07
