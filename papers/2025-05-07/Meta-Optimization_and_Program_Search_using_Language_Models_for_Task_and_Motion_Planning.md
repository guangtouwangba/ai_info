...
该研究提出了一种新颖的任务与运动规划(TAMP)方法，通过元优化技术改进高层规划与低层控制的接口。该方法采用程序搜索结合轨迹优化作为基础模型与机器人控制的桥梁，并利用零阶方法优化模型输出的数值参数。相比现有方法(要么过于抽象如技能链，要么缺乏抽象如直接预测关节角度)，新方法在物体操作和绘图等复杂任务中表现更优。实验证明，该方法能有效提升规划效率并执行自然语言指令，为机器人智能交互提供了更优的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T12:01:38Z
- **目录日期**: 2025-05-07
