...
该研究提出了一种新型任务与运动规划(TAMP)方法，通过元优化技术桥接高层规划与低层控制。该方法创新性地采用程序搜索轨迹优化问题作为基础模型与机器人控制的接口，并利用零阶方法优化模型输出的数值参数。相比现有方法(要么过度简化技能原语，要么直接预测关节角度)，该方法在物体操控和绘图等复杂任务中表现更优，同时保持了基础模型快速规划和执行自然语言指令的优势。研究结果表明，这种基于程序搜索和参数优化的接口设计能有效提升TAMP系统的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T20:01:32Z
- **目录日期**: 2025-05-07
