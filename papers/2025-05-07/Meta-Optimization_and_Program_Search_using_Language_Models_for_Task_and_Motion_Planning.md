...
该研究提出了一种新颖的任务与运动规划(TAMP)方法，通过元优化技术改进了高层规划与低层控制的接口。该方法：(1)使用程序搜索结合轨迹优化作为基础模型与机器人控制的桥梁；(2)采用零阶方法优化基础模型输出的数值参数。相比现有方法(如简单技能链或直接关节角度预测)，该方法在物体操纵和绘图等复杂任务中表现更优，解决了抽象过度或不足的问题。实验结果表明，该方法优于先前的TAMP方法，同时保持了快速规划时间和执行自然语言指令的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T07:01:27Z
- **目录日期**: 2025-05-07
