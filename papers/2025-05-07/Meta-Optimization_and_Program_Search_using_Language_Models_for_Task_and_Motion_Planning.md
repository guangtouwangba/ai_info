...
该研究提出了一种新型任务与运动规划(TAMP)方法，通过元优化技术改进高层规划与底层控制的衔接：(1) 使用轨迹优化程序搜索作为基础模型与机器人控制的接口；(2)采用零阶方法优化基础模型输出的数值参数。相比现有方法(要么过度简化技能原语，要么直接预测关节角度)，该方法在物体操控和绘图等挑战性任务中表现更优，同时保持了自然语言指令执行能力和快速规划的优势。研究结果表明，该方法有效解决了TAMP中抽象层级平衡的问题，性能优于先前方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T10:01:32Z
- **目录日期**: 2025-05-07
