...
本文提出了SIM-RAG框架，用于增强检索增强生成(RAG)系统的自我意识和多轮检索能力。该方法通过让RAG系统自我练习多轮检索，生成带有中间推理步骤的合成训练数据，并训练一个轻量级的"信息充分性评判器"。该评判器在推理时评估每轮检索是否获得足够信息，从而指导检索决策。实验表明，SIM-RAG能有效提升多轮RAG性能，且具有系统高效性(仅添加轻量组件)和数据高效性(无需人工标注中间步骤监督数据)的优势。该框架不改动现有大语言模型或搜索引擎，解决了当前多轮RAG系统检索不足或过度的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-07T02:30:15Z
- **目录日期**: 2025-05-07
