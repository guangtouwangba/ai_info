...
本文提出了一种基于深度强化学习(DRL)的多模式家庭能源管理系统(HEMS)框架，旨在动态优化用户偏好下的能源消耗。与传统使用静态权重因子的方法不同，该框架能根据用户对不同家电的动态偏好进行智能调节，提升用户参与需求响应(DR)的积极性。研究采用无模型单智能体DRL算法，利用包含电价、环境温度和家电功耗的15分钟间隔真实数据进行验证。结果表明，该模型在不同偏好模式下均能出色优化能耗，计算效率优于传统的混合整数线性规划(MILP)算法，同时保持近乎最优的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T00:01:50Z
- **目录日期**: 2025-05-06
