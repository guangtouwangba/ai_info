...
家庭能源管理系统（HEMS）是智能家居中的关键工具，旨在提升能源效率、降低成本和改善用户舒适度。传统方法常将用户舒适度视为固定偏好，但忽略了动态行为。本文提出一种基于深度强化学习（DRL）的多模式HEMS框架（DRL-HEMS），动态优化用户自定义偏好，增强用户在需求响应（DR）中的参与度。采用无模型单智能体DRL算法，结合电价、环境温度等实时数据，实验表明该模型在不同偏好模式下均能高效优化能耗，计算效率优于传统混合整数线性规划（MILP）方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T01:29:19Z
- **目录日期**: 2025-05-06
