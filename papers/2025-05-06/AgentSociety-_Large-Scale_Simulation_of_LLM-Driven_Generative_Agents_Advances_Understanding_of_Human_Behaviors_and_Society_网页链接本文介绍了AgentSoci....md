...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它分析四个关键社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实数据吻合，验证了其捕捉人类行为机制的能力。该系统可作为计算社会科学的研究工具，辅助传统方法（如调查、实验），帮助理解社会问题成因并支持政策制定。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T03:22:29Z
- **目录日期**: 2025-05-06
