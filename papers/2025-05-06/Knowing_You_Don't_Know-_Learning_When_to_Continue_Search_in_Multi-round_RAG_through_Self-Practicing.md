...
本文提出SIM-RAG框架，通过增强检索增强生成(RAG)系统的自我意识和多轮检索能力来解决现有问题。该方法让RAG系统自我练习多轮检索，生成包含推理步骤的合成训练数据，并训练一个轻量级信息充分性评判器(Critic)。该评判器在推理时评估检索信息是否充分，指导检索决策。实验表明，SIM-RAG在多个基准测试中表现优异，且系统高效(仅添加轻量组件)、数据高效(无需人工标注中间步骤监督数据)。该方法改进了复杂任务中的多轮检索能力，避免了过度检索或信息不足导致的错误。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T13:10:30Z
- **目录日期**: 2025-05-06
