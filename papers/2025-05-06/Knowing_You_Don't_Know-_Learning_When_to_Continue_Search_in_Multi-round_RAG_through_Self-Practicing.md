...
本文提出了SIM-RAG框架，通过增强检索增强生成(RAG)系统的自我意识和多轮检索能力来解决现有方法的不足。该框架让RAG系统自我练习多轮检索，生成包含中间推理步骤的合成训练数据，并训练一个轻量级的信息充分性评判器(Critic)。在推理时，评判器评估每轮检索是否获得足够信息，指导检索决策。实验表明，SIM-RAG在多轮RAG任务中表现优异，且系统高效(仅添加轻量组件)、数据高效(无需人工标注中间步骤监督数据)，无需修改现有大语言模型或搜索引擎。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T19:01:22Z
- **目录日期**: 2025-05-06
