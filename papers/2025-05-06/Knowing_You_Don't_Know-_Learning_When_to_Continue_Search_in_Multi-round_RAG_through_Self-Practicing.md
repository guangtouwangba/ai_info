...
本文提出SIM-RAG框架，旨在提升检索增强生成(RAG)系统的自我认知和多轮检索能力。该方法通过让RAG系统自我练习生成合成训练数据，并训练一个轻量级信息充分性评判器(Critic)。在推理时，该评判器评估每轮检索是否充分，指导检索决策并通过上下文强化学习提高系统自我认知。实验表明，SIM-RAG在多轮RAG任务中表现优异，且具有系统高效性(仅添加轻量组件)和数据高效性(无需人工标注中间步骤数据)的优势。该框架不改动现有大语言模型或搜索引擎，有效解决了当前多轮RAG系统的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T18:01:24Z
- **目录日期**: 2025-05-06
