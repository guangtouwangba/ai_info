...
本文提出了SIM-RAG框架，旨在增强检索增强生成(RAG)系统的自我意识和多轮检索能力。该框架通过让RAG系统自我练习多轮检索，生成带有中间推理步骤的合成训练数据，并训练一个轻量级的信息充分性评判器(Critic)。在推理时，该评判器评估每轮检索是否获取足够信息，指导检索决策。实验表明，SIM-RAG在多轮RAG任务中表现优异，且具有系统高效性（仅添加轻量组件）和数据高效性（无需昂贵人工标注数据）。该方案有效解决了现有RAG系统在信息充分性判断和检索效率方面的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T16:01:41Z
- **目录日期**: 2025-05-06
