...
本文提出SIM-RAG框架，通过增强检索增强生成(RAG)系统的自我意识和多轮检索能力来解决现有问题。该方法让RAG系统自我练习多轮检索，生成带推理步骤的合成训练数据，并训练一个轻量级的信息充分性评判器(Critic)。在推理时，该评判器评估每轮检索是否充分，指导检索决策。实验表明，SIM-RAG在多轮RAG任务中表现优异，且系统高效（仅添加轻量组件）、数据高效（无需人工标注的训练数据）。该框架不改动现有大语言模型或搜索引擎，显著提升了RAG系统的自我意识和检索能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T22:01:22Z
- **目录日期**: 2025-05-06
