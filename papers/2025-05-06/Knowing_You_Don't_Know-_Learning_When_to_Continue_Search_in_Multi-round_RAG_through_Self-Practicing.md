...
本文提出了SIM-RAG框架，旨在提升检索增强生成(RAG)系统的自我认知和多轮检索能力。该框架通过让RAG系统自我练习多轮检索，生成带有中间推理步骤的合成训练数据，并训练一个轻量级的信息充分性评判器(Critic)。在推理时，Critic评估每轮检索是否已获取足够信息，指导检索决策并通过上下文强化学习提升系统自我认知。实验表明，SIM-RAG在多轮RAG任务中表现优异，且系统高效(仅添加轻量组件)、数据高效(无需人工标注中间过程监督数据)，有效解决了现有方法过度检索或信息不足的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T09:01:23Z
- **目录日期**: 2025-05-06
