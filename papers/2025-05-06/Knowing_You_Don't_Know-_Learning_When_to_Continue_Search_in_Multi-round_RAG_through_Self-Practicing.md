...
本文提出了一种名为SIM-RAG的新框架，旨在增强检索增强生成（RAG）系统的自我意识和多轮检索能力。该框架通过让RAG系统自我练习多轮检索，生成带有推理步骤的合成训练数据，并训练一个轻量级的信息充分性评估器（Critic）。实验表明，SIM-RAG在多个RAG基准测试中表现优异，能够有效判断检索信息是否充分，从而优化多轮检索决策。该框架无需修改现有大语言模型或搜索引擎，也无需昂贵的人工标注数据，具有系统高效性和数据高效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T23:01:28Z
- **目录日期**: 2025-05-06
