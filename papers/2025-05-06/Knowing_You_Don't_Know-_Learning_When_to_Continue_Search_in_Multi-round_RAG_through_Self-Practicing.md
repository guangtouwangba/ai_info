...
本文提出了SIM-RAG框架，旨在增强检索增强生成（RAG）系统的自我意识和多轮检索能力。该框架通过让RAG系统自我练习多轮检索，生成带有中间推理步骤的合成训练数据，并训练一个轻量级的信息充分性评判器（Critic）。在推理时，该评判器评估每轮检索是否充分，从而指导检索决策并提升系统自我意识。实验表明，SIM-RAG在多个RAG基准测试中表现优异，且具有系统高效性（仅需添加轻量组件）和数据高效性（无需昂贵的人工标注数据）的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T12:01:32Z
- **目录日期**: 2025-05-06
