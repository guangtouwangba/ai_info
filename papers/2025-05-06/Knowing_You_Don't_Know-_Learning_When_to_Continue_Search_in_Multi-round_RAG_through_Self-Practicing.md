...
本文提出了SIM-RAG框架，旨在提升多轮检索增强生成(RAG)系统的自我意识和检索能力。该方法通过让RAG系统自我练习生成合成训练数据，包含中间推理步骤和检索路径标记，训练一个轻量级信息充分性评判器(Critic)。该评判器在推理时评估检索信息是否充分，指导检索决策，无需修改现有语言模型或搜索引擎，也无需昂贵的人工标注数据。实验表明，SIM-RAG能有效提升多轮RAG性能，同时保持系统高效性和数据高效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T10:01:37Z
- **目录日期**: 2025-05-06
