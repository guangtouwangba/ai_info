...
该论文提出了一种名为NSA（Native Sparse Attention）的稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本高的问题。NSA通过动态分层稀疏策略结合硬件优化，实现了高效的长文本建模。其核心创新包括：1）粗粒度令牌压缩保留全局上下文感知；2）细粒度令牌选择确保局部精度。该方法在保持模型性能的同时显著降低了计算开销，为大模型的长文本处理提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T15:05:49Z
- **目录日期**: 2025-05-06
