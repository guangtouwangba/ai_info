...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1)创建CodeArena基准测试，包含397个高质量编程任务(覆盖40类/44种语言)，首次评估模型输出与人类偏好的匹配度；2)构建20B标记的SynCode-Instruct合成指令集验证微调效果。研究发现：现有执行基准与人类偏好存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调代码LLMs不仅需要正确性，还需重视开发者体验和实际需求对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T03:23:09Z
- **目录日期**: 2025-05-06
