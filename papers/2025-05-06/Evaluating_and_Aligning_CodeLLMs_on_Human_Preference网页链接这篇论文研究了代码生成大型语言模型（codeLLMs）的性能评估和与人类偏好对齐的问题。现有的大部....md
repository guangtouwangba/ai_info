...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类44种语言)来评估模型在真实编程任务中的表现，同时构建了20B标记的SynCode-Instruct合成指令数据集。实验发现：1)传统执行基准与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了在代码生成领域进行人类偏好对齐的重要性，为改进代码LLMs提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T14:06:08Z
- **目录日期**: 2025-05-06
