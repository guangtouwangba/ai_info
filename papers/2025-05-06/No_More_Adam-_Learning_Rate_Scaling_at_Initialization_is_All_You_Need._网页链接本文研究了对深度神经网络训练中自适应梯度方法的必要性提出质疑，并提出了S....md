...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了训练不平衡问题，同时将优化器内存占用减半。实验表明，该方法在Transformer任务、ImageNet分类和LLM预训练中表现优于或媲美AdamW，特别在内存效率上优势显著（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。此外，它在LoRA微调等任务中也超越现有优化器，且对超参数变化更具鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T05:04:24Z
- **目录日期**: 2025-05-06
