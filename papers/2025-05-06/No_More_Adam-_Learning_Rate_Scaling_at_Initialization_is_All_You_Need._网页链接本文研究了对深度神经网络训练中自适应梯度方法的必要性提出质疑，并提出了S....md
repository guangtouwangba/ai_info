...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI通过初始化时基于梯度信号噪声比（g-SNR）调整不同参数组的学习率，有效解决了训练不平衡问题，同时将内存使用减半。实验表明，该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或相当于AdamW，特别在内存效率上优势显著（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。此外，它在LoRA微调等任务中也超越了现有优化器，具有更强的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T08:04:43Z
- **目录日期**: 2025-05-06
