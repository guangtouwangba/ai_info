...
Scenethesis是一个无需训练的智能框架，用于从文本生成交互式3D场景。它结合大型语言模型（LLM）的规划能力和视觉模块的空间优化，解决现有方法在多样性和空间合理性上的不足。流程包括：LLM生成粗布局，视觉模块通过图像引导优化物体关系，优化模块确保姿态对齐和物理合理性，最后验证空间一致性。实验表明，Scenethesis能生成多样、真实且物理合理的3D场景，适用于虚拟内容创作、模拟环境和具身AI研究，克服了传统方法在复杂布局和常识空间关系上的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T22:00:54Z
- **目录日期**: 2025-05-06
