...
Scenethesis是一个基于文本生成3D交互场景的训练免费框架，结合了大语言模型（LLM）的场景规划和视觉引导的布局优化。LLM首先生成粗略布局，视觉模块通过图像引导提取场景结构以优化物体间关系。优化模块迭代调整姿态和物理合理性，避免物体穿透或不稳定。最后，验证模块确保空间一致性。实验表明，Scenethesis能生成多样、真实且物理合理的3D场景，适用于虚拟内容创作、模拟环境和具身AI研究，解决了现有方法在空间真实性和布局复杂性上的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T17:01:05Z
- **目录日期**: 2025-05-06
