...
Scenethesis是一个基于文本生成交互式3D场景的新型框架，结合了大型语言模型（LLM）和视觉引导的优势。它首先使用LLM生成粗略场景布局，再通过视觉模块优化物体间关系，确保空间合理性。优化模块进一步调整物体姿态，避免穿透和不稳定问题，最后由验证模块确保空间一致性。相比现有方法，Scenethesis无需训练，能生成多样化、真实且物理合理的3D场景，适用于虚拟内容创作、仿真环境和具身AI研究。实验表明，该方法显著提升了场景多样性和布局复杂度，克服了纯LLM方法在空间合理性上的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T04:00:55Z
- **目录日期**: 2025-05-06
