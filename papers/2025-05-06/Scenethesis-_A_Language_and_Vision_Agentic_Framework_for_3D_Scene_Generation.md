...
Scenethesis是一个无需训练的智能框架，用于从文本生成交互式3D场景。它结合大型语言模型（LLM）的场景规划和视觉引导的布局优化，解决现有方法在多样性和空间合理性上的不足。首先，LLM生成粗略布局，视觉模块通过图像引导和结构提取优化物体关系。随后，优化模块迭代调整姿态和物理合理性，避免物体穿透或不稳定。最后，验证模块确保空间一致性。实验表明，Scenethesis能生成多样、真实且物理合理的3D场景，适用于虚拟内容创作、模拟环境和具身AI研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T13:09:57Z
- **目录日期**: 2025-05-06
