...
Scenethesis是一个基于文本生成交互式3D场景的训练免费框架，结合了大语言模型(LLM)的场景规划和视觉引导的布局优化。它首先用LLM生成粗略布局，然后通过视觉模块提取场景结构并优化物体间关系，再通过优化模块迭代调整物体姿态以确保物理合理性，最后用验证模块检查空间一致性。相比现有方法，Scenethesis能生成更丰富多样、空间合理且物理真实的3D场景，适用于虚拟内容创作、仿真环境和具身AI研究。该方法突破了传统基于学习的方法受限于小规模数据集的问题，同时弥补了纯LLM方法在空间合理性上的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T19:00:53Z
- **目录日期**: 2025-05-06
