...
本研究提出LatentSync框架，通过音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，提出TREPA方法利用自监督视频模型的时间表示增强帧间对齐。同时改进了SyncNet训练框架，通过优化架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）直接建模音视关联的扩散模型；2）解决时间一致性的TREPA方法；3）SyncNet训练优化方案，可推广至其他音视频同步任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T09:05:43Z
- **目录日期**: 2025-05-06
