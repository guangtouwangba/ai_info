...
AutoLibra 是一个基于人类开放式反馈的智能体评估框架，能够将模糊反馈（如"按钮禁用时不应重复点击"）转化为细粒度行为指标。它通过将反馈与行为关联、聚类正负行为样本，并生成包含明确定义和示例的评估指标，用于大语言模型评估。该框架提出"覆盖率"和"冗余度"两个元指标优化指标质量。实验表明，AutoLibra生成的指标比传统任务成功率更能提升智能体性能（文本游戏任务平均提升20%），并能筛选高质量微调数据。研究表明这是评估和改进语言智能体的通用有效工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T19:01:12Z
- **目录日期**: 2025-05-06
