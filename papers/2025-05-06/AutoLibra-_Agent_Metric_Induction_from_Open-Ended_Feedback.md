...
AutoLibra是一个基于开放人类反馈的智能体评估框架，可将模糊反馈（如"按钮禁用时不应重复点击"）转化为细粒度行为指标。它通过将反馈与行为关联、聚类正负行为样本，并生成带明确定义和示例的评估指标，用于大语言模型（LLM）评估。该框架提出"覆盖率"和"冗余度"两个元指标优化指标质量，实验显示其生成的指标比传统基准更具体，并能发现新评估维度。应用方面，AutoLibra指标作为提示工程目标使文本游戏任务性能提升20%，还能筛选高质量微调数据改进网页导航智能体。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T12:01:22Z
- **目录日期**: 2025-05-06
