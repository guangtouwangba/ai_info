...
AutoLibra是一个基于人类开放反馈的智能体评估框架，能够将模糊的反馈（如"按钮禁用时不应重复点击"）转化为细粒度的行为评估指标。它通过将反馈与智能体行为关联、聚类相似行为，并生成具有明确定义和示例的指标，用于LLM评估。AutoLibra提出"覆盖率"和"冗余度"两个元指标优化指标质量，实验表明其生成的指标比传统基准更具体，并能发现新指标。应用显示：1）作为提示工程目标可使文本游戏任务性能平均提升20%；2）能迭代筛选高质量微调数据，是强大的任务无关评估改进工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T17:01:27Z
- **目录日期**: 2025-05-06
