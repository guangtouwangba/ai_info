...
AutoLibra是一个基于开放人类反馈的智能体评估框架，将模糊反馈（如"按钮禁用时不应重复点击"）转化为细粒度行为指标。它通过关联反馈与行为、聚类正负样本，生成可量化指标，并利用LLM作为评估器。该框架提出"覆盖率"和"冗余度"两个元指标优化指标质量，实验显示其指标比传统任务成功率更具体，能发现新评估维度。应用方面：1）在文本游戏中，使用AutoLibra指标优化提示工程可使智能体性能平均提升20%；2）能迭代筛选高质量微调数据提升网页导航智能体，证明其作为任务无关工具的有效性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T15:01:57Z
- **目录日期**: 2025-05-06
