...
AutoLibra是一个基于人类开放反馈的智能体评估框架，通过将模糊反馈（如"按钮禁用时不应重复点击"）转化为可量化的细粒度行为指标。它通过聚类正负行为并生成明确定义的评估标准，支持LLM作为裁判进行打分。该框架提出"覆盖率"和"冗余度"两个元指标优化评估体系，实验证明其生成的指标比传统任务成功率更具体，能发现新评估维度。应用显示：1)作为提示工程目标时，在文本游戏中平均提升智能体性能20%；2)能迭代筛选高质量微调数据。AutoLibra是任务无关的智能体评估改进工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T09:01:13Z
- **目录日期**: 2025-05-06
