...
本文提出了Insight-V框架，旨在提升多模态大语言模型(MLLM)的推理能力。该框架通过两步流程自动生成长链推理数据：首先生成多样化长推理路径，再通过多粒度评估确保数据质量。针对长数据训练难题，设计了包含推理代理(执行长链推理)和总结代理(评估结果)的多智能体系统，并采用迭代DPO算法提升生成稳定性。基于LLaVA-NeXT模型和更强基础MLLM的实验表明，该方法在视觉推理任务中表现显著提升，同时能保持感知型任务的性能。该研究为多模态任务中的高质量长链推理数据生成和训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T00:02:31Z
- **目录日期**: 2025-05-06
