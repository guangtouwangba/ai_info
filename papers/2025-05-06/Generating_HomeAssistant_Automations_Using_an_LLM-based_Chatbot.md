...
这项研究探讨了大型语言模型(LLM)在智能家居自动化中的应用，特别是通过HomeAssistant框架促进家庭可持续实践。研究发现，GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但仍存在语法错误等问题。虽然"绿色"提示与普通提示的量化差异不大，但前者确实产生了更环保的自动化方案。一项56人的实证评估显示，与传统规则系统相比，用户更青睐LLM系统。研究表明LLM在推动智能家居技术发展方面具有潜力，但需要进一步优化以支持可持续生活的实际应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T11:01:41Z
- **目录日期**: 2025-05-06
