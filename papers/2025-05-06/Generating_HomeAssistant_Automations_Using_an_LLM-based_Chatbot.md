...
这项研究探讨了利用大型语言模型(LLM)增强智能家居自动化以促进家庭可持续实践的应用。研究发现，GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但也存在语法错误等问题。虽然"绿色"提示与普通提示的量化差异不大，但定性反馈显示环保导向提示能产生更可持续的解决方案。一项56人的实证评估表明，与传统规则系统相比，用户更接受并喜欢这种LLM驱动的方案。研究凸显了LLM在推进智能家居技术方面的潜力，建议进一步改进模型以支持可持续生活的实际应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T08:01:39Z
- **目录日期**: 2025-05-06
