...
这项研究探讨了利用大语言模型（LLM）增强智能家居自动化以促进家庭可持续实践的潜力。研究基于HomeAssistant框架，发现GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但仍存在语法错误等问题。尽管环保提示与普通提示的量化差异不大，但定性反馈显示环保提示更易产生可持续方案。一项56人参与的实证评估表明，相比传统规则系统，用户更青睐LLM方案。研究凸显了LLM在推动智能家居技术发展中的作用，建议进一步优化模型以支持可持续生活的实际应用。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T02:34:09Z
- **目录日期**: 2025-05-06
