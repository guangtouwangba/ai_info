...
这项研究探讨了利用大型语言模型(LLMs)优化智能家居自动化以促进家庭可持续实践。研究发现，GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但仍存在语法错误等问题。虽然"绿色"提示与普通提示的定量差异不大，但定性反馈显示环保提示能促进更可持续的解决方案。用户评估(N=56)表明，相比传统基于规则的系统，LLM系统更受欢迎且更具吸引力。研究凸显了LLMs在推动智能家居技术发展中的潜力，建议进一步优化模型以支持现实生活中的可持续实践。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T13:10:40Z
- **目录日期**: 2025-05-06
