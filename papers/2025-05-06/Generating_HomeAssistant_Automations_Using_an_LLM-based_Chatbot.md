...
这项研究探讨了利用大语言模型（LLM）提升智能家居自动化并促进家庭可持续实践的应用。研究基于HomeAssistant框架，发现GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但仍存在语法错误等问题。尽管"绿色"提示与普通提示在量化差异上不明显，但定性反馈显示环保导向的提示能促进可持续性习惯。用户评估（N=56）表明，相比传统规则系统，该LLM方案更受欢迎且互动性更强。研究凸显了LLM在推动智能家居技术发展中的潜力，建议进一步优化模型以支持可持续生活的实际应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T12:01:43Z
- **目录日期**: 2025-05-06
