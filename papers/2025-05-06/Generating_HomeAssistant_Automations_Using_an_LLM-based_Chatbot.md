...
这项研究探讨了利用大型语言模型(LLM)增强智能家居自动化并促进可持续家庭实践的潜力。研究基于HomeAssistant框架，发现GPT模型能准确生成自动化程序，理解复杂指令并输出有效JSON，但仍存在语法错误等问题。尽管"绿色"提示与普通提示的量化差异不大，但定性反馈显示环保提示能产生更可持续的方案。用户评估(N=56)表明，相比传统基于规则的系统，LLM方案更受欢迎且更具吸引力。研究凸显了LLM在推动智能家居技术发展中的作用，建议进一步优化模型以支持可持续生活的实际应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-06T20:01:39Z
- **目录日期**: 2025-05-06
