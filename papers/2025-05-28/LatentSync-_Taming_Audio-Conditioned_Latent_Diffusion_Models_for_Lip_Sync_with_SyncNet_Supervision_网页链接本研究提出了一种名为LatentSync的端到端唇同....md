...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强生成帧的连贯性。同时，研究解决了SyncNet的收敛问题，通过优化模型架构、训练参数和数据预处理显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可推广至其他依赖SyncNet的唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T18:05:27Z
- **目录日期**: 2025-05-28
