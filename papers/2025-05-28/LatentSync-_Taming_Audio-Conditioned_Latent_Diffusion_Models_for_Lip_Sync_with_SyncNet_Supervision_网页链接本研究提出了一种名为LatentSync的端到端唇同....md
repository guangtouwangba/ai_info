...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，并针对扩散模型的时间一致性问题提出TREPA方法（时间表示对齐），通过自监督视频特征增强生成序列的连贯性。同时，研究改进了SyncNet训练框架，解决了其收敛问题，提升了唇同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进对音频驱动人像动画领域具有普适参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T13:15:38Z
- **目录日期**: 2025-05-28
