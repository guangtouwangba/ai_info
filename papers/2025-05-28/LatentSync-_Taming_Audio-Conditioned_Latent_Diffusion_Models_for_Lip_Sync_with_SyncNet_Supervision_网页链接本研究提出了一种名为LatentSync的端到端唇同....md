...
LatentSync提出了一种端到端唇同步框架，利用音频条件的潜在扩散模型直接建模音频-视觉关系，无需中间运动表示。针对扩散模型的时间一致性问题，该研究开发了TREPA方法，通过自监督视频模型的时间表示对齐生成帧。此外，研究还优化了SyncNet训练框架，改进了模型架构、超参数和数据预处理，显著提升了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进也可推广至其他基于SyncNet的唇同步和人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T23:04:55Z
- **目录日期**: 2025-05-28
