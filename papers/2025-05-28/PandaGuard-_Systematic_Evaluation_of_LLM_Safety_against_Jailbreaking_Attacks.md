...
该论文提出了PandaGuard框架和PandaBench基准测试，用于系统评估大语言模型(LLMs)的对抗性攻击防御能力。该框架采用多智能体系统(攻击者、防御者和评判者)，整合了19种攻击方法、12种防御机制和多种评判策略，支持可配置实验。基于此开发的PandaBench对49个LLM进行了全面测试(消耗超30亿token)，揭示了模型漏洞、防御成本效益权衡和评判一致性等问题。研究发现没有单一防御能在所有维度表现最优，且评判差异会影响安全评估。研究代码和结果已开源以促进可复现的LLM安全研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T00:02:51Z
- **目录日期**: 2025-05-28
