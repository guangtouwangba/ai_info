...
这篇微博介绍了AI领域的12个核心术语：AGI（超越人类的通用人工智能）、思维链（分步推理技术）、深度学习（模仿人脑的多层神经网络）、扩散模型（通过噪声生成数据）、知识蒸馏（大模型训练小模型）、微调（专业化训练）、GAN（生成对抗网络）、幻觉（AI虚构内容）、神经网络（算法基础结构）、迁移学习（模型复用技术）和权重（特征重要性参数）。这些概念涵盖了当前AI技术的关键原理和应用方法，为初学者提供了理解生成式AI的基础框架。Techcrunch维护的这份词典持续更新，可作为AI入门参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T17:03:06Z
- **目录日期**: 2025-05-28
