...
本文提出AdInject，一种基于视觉语言模型(VLM)的网页代理黑盒攻击方法，通过互联网广告投放注入恶意内容。相比现有研究，该方法采用更现实的威胁模型：不依赖HTML直接修改、不知用户意图、无需代理模型参数。AdInject包含恶意广告设计策略和基于VLM的内容优化技术，能根据网站上下文推断潜在意图，使广告看似相关从而提高攻击成功率。实验显示攻击成功率超60%，特定场景接近100%，证明广告投放是有效的现实攻击媒介。研究揭示了网页代理安全的关键漏洞，亟需开发防御机制。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T19:01:11Z
- **目录日期**: 2025-05-28
