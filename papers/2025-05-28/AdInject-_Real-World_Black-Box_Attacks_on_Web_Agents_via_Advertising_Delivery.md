...
基于视觉语言模型(VLM)的Web代理在自动化复杂任务方面取得进展，但面临现实环境中的安全威胁。本研究提出AdInject攻击方法，通过互联网广告投放向Web代理环境注入恶意内容。该方法采用更现实的威胁模型(黑盒代理、静态恶意内容约束、无用户意图知识)，包含误导性广告设计策略和基于VLM的内容优化技术。实验显示攻击成功率超过60%，特定情况下接近100%，证明广告投放是有效的攻击载体。研究揭示了Web代理安全的关键漏洞，强调开发防御机制的紧迫性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T09:01:24Z
- **目录日期**: 2025-05-28
