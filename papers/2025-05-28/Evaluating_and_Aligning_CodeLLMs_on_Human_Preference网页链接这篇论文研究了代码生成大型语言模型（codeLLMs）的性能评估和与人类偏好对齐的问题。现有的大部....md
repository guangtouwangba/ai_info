...
这篇论文探讨了代码生成大模型（CodeLLMs）与人类偏好的对齐问题，指出现有基准测试过于关注代码正确性而忽略用户偏好。研究者提出了CodeArena基准，包含397个高质量样本，覆盖40个类别和44种语言，模拟真实编程任务。同时构建了20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著，凸显对齐人类偏好的重要性。研究为提升代码模型的实用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T04:07:41Z
- **目录日期**: 2025-05-28
