...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究者从真实用户查询中精选397个样本，覆盖40类任务和44种语言，并构建20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行基准与人类偏好存在差距；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著。研究强调代码生成模型不仅需要正确性，还需符合开发者偏好，为改进代码LLM提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T23:05:14Z
- **目录日期**: 2025-05-28
