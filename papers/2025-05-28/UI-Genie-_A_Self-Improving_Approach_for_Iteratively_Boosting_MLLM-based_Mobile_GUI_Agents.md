...
本文提出了UI-Genie框架，通过奖励模型(UI-Genie-RM)和自改进流程解决GUI代理的两大挑战：轨迹结果验证困难和高质训练数据难以扩展。UI-Genie-RM采用图文交错架构处理历史上下文，统一动作级和任务级奖励，并配合规则验证、轨迹破坏等数据生成策略。自改进流程通过奖励引导探索逐步扩展可解决的复杂GUI任务。研究构建了首个GUI代理奖励专用数据集(UI-Genie-RM-517k)和高质量合成轨迹数据集(UI-Genie-Agent-16k)，实验表明该方法在多项基准测试中达到最优性能。完整框架和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T03:20:33Z
- **目录日期**: 2025-05-28
