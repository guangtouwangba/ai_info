...
本文提出了UI-Genie框架，通过奖励模型（UI-Genie-RM）和自改进流程解决GUI智能体的两大挑战：轨迹结果验证困难和高质量训练数据难以扩展。UI-Genie-RM采用图文交错架构，统一动作级和任务级奖励，并基于规则验证、轨迹破坏和负样本挖掘生成训练数据。自改进流程通过奖励引导探索动态扩展可解决的复杂GUI任务，同时提升智能体和奖励模型。实验表明，经过三代数据-模型自改进后，UI-Genie在多个GUI基准测试中达到最优性能。研究开源了框架实现和生成的数据集（UI-Genie-RM-517k和UI-Genie-Agent-16k）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T20:01:41Z
- **目录日期**: 2025-05-28
