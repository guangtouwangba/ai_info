...
本文提出了UI-Genie框架，通过奖励模型(UI-Genie-RM)和自我改进流程解决GUI智能体的两大挑战：轨迹结果验证困难和高质量训练数据难以扩展。UI-Genie-RM采用图文交错架构处理历史上下文，统一动作级和任务级奖励，并配合精心设计的数据生成策略。自我改进流程通过奖励引导探索和动态环境验证，逐步扩展可解决的复杂GUI任务。实验表明，经过三代数据-模型自我改进，UI-Genie在多个GUI智能体基准测试中达到最先进性能。作者开源了完整框架实现和生成的数据集(UI-Genie-RM-517k和UI-Genie-Agent-16k)。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T12:01:40Z
- **目录日期**: 2025-05-28
