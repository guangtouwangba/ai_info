...
本文提出了UI-Genie框架，解决了GUI智能体的两大挑战：轨迹结果验证困难和高质训练数据难以扩展。该框架包含奖励模型UI-Genie-RM（采用图文交错架构处理历史上下文）和自改进流程。通过基于规则的验证、受控轨迹破坏等策略生成训练数据，并建立首个GUI智能体专用奖励数据集UI-Genie-RM-517k。自改进流程通过奖励引导探索动态扩展可解决任务，经过三代数据-模型迭代，在多个基准测试中达到最优性能。研究开源了完整框架和生成数据集以促进相关研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T07:01:46Z
- **目录日期**: 2025-05-28
