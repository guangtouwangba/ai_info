...
本文提出了UI-Genie框架，解决了GUI智能体中的两个关键挑战：轨迹结果验证困难和高质量训练数据难以扩展。该框架包含奖励模型UI-Genie-RM和自改进流程：UI-Genie-RM采用图像-文本交错架构，统一处理动作级和任务级奖励；自改进流程通过奖励引导探索逐步扩展可解决的复杂GUI任务。研究构建了首个GUI智能体专用奖励数据集UI-Genie-RM-517k和高质量合成轨迹数据集UI-Genie-Agent-16k。实验表明，经过三代数据-模型自改进后，UI-Genie在多个GUI基准测试中达到最优性能。相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T16:01:47Z
- **目录日期**: 2025-05-28
