...
本文提出UI-Genie框架，通过奖励模型UI-Genie-RM和自改进流程解决GUI智能体的两大挑战：轨迹结果验证困难与高质量训练数据难以扩展。UI-Genie-RM采用图文交错架构处理历史上下文，统一动作级和任务级奖励，并通过规则验证、轨迹破坏和难负例挖掘等策略生成训练数据。自改进流程通过奖励引导探索动态增强智能体能力。研究构建了首个GUI智能体专用奖励数据集UI-Genie-RM-517k和轨迹数据集UI-Genie-Agent-16k，实验表明经过三代数据-模型自改进后，该框架在多个基准测试中达到最优性能。完整框架与数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T21:01:36Z
- **目录日期**: 2025-05-28
