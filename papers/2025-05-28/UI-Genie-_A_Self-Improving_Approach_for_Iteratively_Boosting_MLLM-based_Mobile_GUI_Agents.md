...
本文提出UI-Genie框架，通过奖励模型UI-Genie-RM和自改进流程解决GUI智能体的两大挑战：轨迹验证困难与高质量数据稀缺。该框架采用图文交错架构统一动作级/任务级奖励，结合规则验证、轨迹破坏等数据生成策略。通过动态环境中的奖励引导探索，实现智能体与奖励模型的协同进化。研究构建了首个GUI奖励专用数据集UI-Genie-RM-517k和16k轨迹数据，无需人工标注。实验表明，经过三代自改进后，UI-Genie在多个基准测试中达到最优性能。框架代码与数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T15:02:37Z
- **目录日期**: 2025-05-28
