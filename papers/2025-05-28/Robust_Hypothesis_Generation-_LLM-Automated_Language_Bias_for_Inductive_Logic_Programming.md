...
该研究提出了一种结合多智能体系统（基于大语言模型LLM）与归纳逻辑编程（ILP）的新框架，用于自动化生成鲁棒假设。系统通过LLM智能体直接从原始文本数据中自主定义结构化符号词汇（谓词）和关系模板（语言偏置），从而自动完成传统上依赖专家设计的符号基础构建。该方法将文本转化为ILP求解器所需的事实，进而归纳出可解释的规则，既克服了传统ILP对预定义符号结构的依赖，又避免了纯LLM方法对噪声敏感的缺点。多样化实验验证了其优越性能，为自动化、可解释且可验证的假设生成开辟了新途径。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T07:01:57Z
- **目录日期**: 2025-05-28
