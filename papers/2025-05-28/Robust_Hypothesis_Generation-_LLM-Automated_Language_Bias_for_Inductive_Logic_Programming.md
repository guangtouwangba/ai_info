...
该研究提出了一种结合多智能体系统（基于大语言模型LLMs）与归纳逻辑编程（ILP）的新框架。系统通过LLM智能体直接从原始文本数据自动构建结构化符号词汇（谓词）和关系模板（语言偏置），取代传统ILP需要专家预定义符号结构的瓶颈。这种自动符号落地方法将文本转化为ILP求解器可处理的事实，从而归纳出可解释的规则，既克服了传统ILP的局限性，又避免了纯LLM方法对噪声敏感的问题。实验表明，该方法在多样化挑战性场景中表现优异，为自动化、可解释且可验证的假设生成开辟了新途径。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T13:11:44Z
- **目录日期**: 2025-05-28
