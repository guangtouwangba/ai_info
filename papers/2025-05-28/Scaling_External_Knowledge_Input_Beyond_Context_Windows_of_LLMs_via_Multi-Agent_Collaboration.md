...
这篇论文提出了一个名为ExtAgents的多智能体框架，旨在解决大语言模型(LLMs)在处理大量外部知识时面临的上下文窗口限制问题。通过分布式处理方式，该框架克服了现有知识同步和推理过程中的瓶颈，在不增加上下文训练长度的情况下，显著提升了模型整合推理时知识的能力。实验表明，在增强的多跳问答测试∞Bench+和其他公共测试集上，ExtAgents在相同外部知识输入量下优于现有非训练方法，无论知识量是否超出上下文窗口限制。该方法并行性高，效率出色，为实际应用提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T15:03:26Z
- **目录日期**: 2025-05-28
