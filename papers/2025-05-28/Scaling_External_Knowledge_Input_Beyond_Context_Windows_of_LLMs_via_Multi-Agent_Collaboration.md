...
这篇论文提出了一个名为ExtAgents的多智能体框架，旨在解决大语言模型(LLMs)在处理大量外部知识时的上下文窗口限制问题。通过分布式处理方式，该方法克服了现有知识同步和推理过程中的瓶颈，在不增加上下文训练长度的情况下实现了更好的知识整合扩展性。在增强的多跳问答测试集∞Bench+和其他公开测试集上的实验表明，ExtAgents显著优于现有非训练方法，无论输入知识是否超出上下文窗口，同时保持了高并行性带来的效率优势。该研究为实际应用中处理大规模外部知识提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T22:03:00Z
- **目录日期**: 2025-05-28
