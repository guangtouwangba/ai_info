...
这篇论文提出了一个名为ExtAgents的多智能体框架，旨在解决大语言模型(LLM)在处理海量外部知识时的上下文窗口限制问题。通过改进现有知识同步和推理过程中的两个核心瓶颈，该框架能在不增加上下文训练长度的情况下，更高效地整合推理阶段的知识输入。在增强的多跳问答测试集∞Bench+和其他公开测试集上的实验表明，无论输入知识是否超出上下文窗口，ExtAgents都显著优于现有非训练方法，同时保持高并行性带来的效率优势。该研究为实际应用中处理大规模知识输入提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T11:02:23Z
- **目录日期**: 2025-05-28
