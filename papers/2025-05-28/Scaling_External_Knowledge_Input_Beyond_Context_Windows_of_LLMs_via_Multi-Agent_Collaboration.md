...
该研究提出了一个名为ExtAgents的多智能体框架，旨在解决大语言模型(LLMs)在处理海量外部知识时面临的上下文窗口限制问题。通过改进现有知识同步和推理过程中的两个核心瓶颈，该框架无需长上下文训练即可实现更好的推理时知识整合扩展性。在增强的多跳问答测试集∞Bench+和其他公开测试集上的实验表明，无论输入知识是否超出上下文窗口，ExtAgents都显著优于现有非训练方法，同时保持高并行效率。该研究为实际应用中处理大规模外部知识提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T05:02:10Z
- **目录日期**: 2025-05-28
