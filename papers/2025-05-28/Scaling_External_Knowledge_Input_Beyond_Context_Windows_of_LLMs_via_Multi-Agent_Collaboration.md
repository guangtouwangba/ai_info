...
本文提出多智能体框架ExtAgents，突破大语言模型(LLM)上下文窗口限制，解决知识同步和推理过程中的瓶颈问题。通过分布式处理海量输入，该框架在不进行长上下文训练的情况下，显著提升了推理时知识整合的扩展性。在增强的多跳问答测试集∞Bench+和其他公共测试集上的实验表明，无论输入知识是否超出上下文窗口，ExtAgents性能均优于现有非训练方法，同时保持高并行效率。该研究为处理大规模外部知识提供了新思路，对实际应用具有重要价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T20:02:18Z
- **目录日期**: 2025-05-28
