...
随着大语言模型(LLMs)推理和信息检索能力的提升，其有限上下文窗口限制了外部知识输入量。针对这一问题，研究团队开发了多智能体框架ExtAgents，通过改进知识同步和推理过程的瓶颈，在不增加上下文训练的情况下提升知识整合能力。在增强型多跳问答测试∞Bench+等基准测试中，ExtAgents显著优于现有非训练方法，无论输入知识是否超出上下文窗口限制，同时保持高效并行处理。该研究为处理大规模外部知识输入提供了新思路，对实际应用具有重要价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T19:02:05Z
- **目录日期**: 2025-05-28
