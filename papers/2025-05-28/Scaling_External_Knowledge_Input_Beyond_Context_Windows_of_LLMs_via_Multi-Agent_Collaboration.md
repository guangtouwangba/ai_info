...
这篇论文提出了一个名为ExtAgents的多智能体框架，旨在解决大语言模型(LLM)在推理过程中处理大量外部知识时的瓶颈问题。针对现有上下文窗口扩展方法会导致信息丢失的缺陷，该框架通过分布式处理方式，在不需要长上下文训练的情况下，显著提升了知识整合能力。在增强的多跳问答测试集∞Bench+和其他公开测试集上的实验表明，无论输入知识是否超出上下文窗口，ExtAgents都优于现有非训练方法，同时保持高效并行性。该研究为实际应用中处理大规模外部知识提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T13:12:18Z
- **目录日期**: 2025-05-28
