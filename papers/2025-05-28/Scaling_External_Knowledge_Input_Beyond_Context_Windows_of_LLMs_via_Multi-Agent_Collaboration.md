...
该研究提出了一种名为ExtAgents的多智能体框架，旨在解决大语言模型(LLMs)在处理海量外部知识时的上下文窗口限制问题。通过创新的知识同步和推理机制，该框架无需长上下文训练即可实现高效的知识整合。在增强的多跳问答测试集∞Bench+和其他公开测试集上的实验表明，无论输入知识是否超出上下文窗口，ExtAgents都显著优于现有非训练方法，同时保持高并行效率。该研究为提升LLMs在真实场景中的知识整合能力提供了新思路，展现了多智能体协调处理大规模知识输入的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-28T17:02:30Z
- **目录日期**: 2025-05-28
