...
本研究探讨了在视觉与语言导航（VLN）任务中，利用文本到图像扩散模型生成子目标的视觉表示（即“想象”）作为导航线索，以提升导航性能。通过从分段指令中提取地标信息并生成对应的视觉表示，这些“想象”被作为额外模态提供给VLN代理，并通过辅助损失函数强化其与语言指令的关联。实验结果表明，该方法使导航成功率（SR）提升约1个百分点，路径长度加权的成功率（SPL）提升最多0.5个百分点，表明视觉表示的引入增强了代理的视觉理解能力。代码和数据已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-21T09:02:00Z
- **目录日期**: 2025-03-21
