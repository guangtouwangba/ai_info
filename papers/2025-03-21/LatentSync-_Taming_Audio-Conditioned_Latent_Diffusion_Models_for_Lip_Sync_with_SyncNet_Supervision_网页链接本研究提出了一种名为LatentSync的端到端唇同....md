...
本研究提出了LatentSync，一种端到端的唇同步框架，利用音频条件的潜在扩散模型直接建模音频与视觉的复杂关系，无需中间运动表示。为了解决扩散过程中时间一致性问题，引入了时间表示对齐（TREPA）方法，通过自监督视频模型提取的时间表示来增强生成帧与真实帧的一致性。此外，研究还解决了SyncNet的收敛问题，通过调整模型架构、训练超参数和数据预处理方法，显著提高了SyncNet的准确性。该方法在HDTF和VoxCeleb2数据集上表现优异，超越了现有唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-21T03:14:30Z
- **目录日期**: 2025-03-21
