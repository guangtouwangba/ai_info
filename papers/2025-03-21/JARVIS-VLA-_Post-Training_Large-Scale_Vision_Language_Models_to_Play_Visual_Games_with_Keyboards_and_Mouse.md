...
近期，开放世界环境中的基于行动的决策引起了广泛关注。视觉语言行动（VLA）模型在大规模网络数据集上预训练后，在决策任务中表现出潜力。然而，以往研究主要集中在行动后训练，常忽视基础模型的改进。为此，我们提出了一种新方法——视觉语言后训练行动，通过自监督的视觉和语言指导优化视觉语言模型（VLM），提升其在世界知识、视觉识别和空间定位方面的能力。实验表明，该方法在Minecraft中超越了传统模仿学习策略，实现了最先进的性能。代码、模型和数据集已开源，项目页面见https://craftjarvis.github.io/JarvisVLA。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-21T07:02:21Z
- **目录日期**: 2025-03-21
