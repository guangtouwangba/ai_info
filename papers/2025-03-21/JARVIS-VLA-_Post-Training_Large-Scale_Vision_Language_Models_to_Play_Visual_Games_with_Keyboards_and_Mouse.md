...
近期，开放世界环境中的基于行动的决策受到广泛关注。视觉语言行动（VLA）模型在大规模网络数据集上预训练后，在决策任务中表现出潜力。然而，以往研究主要关注行动后训练，常忽视基础模型的改进。为此，我们提出了一种新方法，通过视觉和语言指导以自监督方式优化视觉语言模型（VLM），提升其在世界知识、视觉识别和空间定位方面的能力。在Minecraft中，我们首次实现了可执行1000多种原子任务的VLA模型，实验表明，非轨迹任务的后训练使性能提升40%，超越传统模仿学习策略，并开源了代码、模型和数据集。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-21T21:02:09Z
- **目录日期**: 2025-03-21
