...
本文介绍了Florence-VL，一种新型的多模态大型语言模型（MLLM），通过集成生成性视觉基础模型Florence-2来增强视觉表征。Florence-VL采用深度-广度融合（DBFusion）方法，有效融合视觉特征到预训练的语言模型Phi 3.5和LLama 3中。该模型在多种多模态和视觉为中心的基准测试中表现出色，如通用VQA、感知、OCR等。研究团队开源了模型和训练方法，以促进未来研究。Florence-VL在视觉语言对齐方面优于流行的视觉编码器，展示了其在多模态任务中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-21T00:04:45Z
- **目录日期**: 2025-03-21
