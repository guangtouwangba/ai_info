...
研究发现，大型语言模型（LLMs）通过工具描述文本选择外部工具时存在严重脆弱性。实验表明，对工具描述进行特定编辑后，GPT-4.1和Qwen2-7B等模型使用该工具的频率可提升10倍以上。研究测试了10种不同模型，发现工具描述微调会显著影响模型选择，这虽然为开发者推广工具提供了捷径，但也暴露出当前代理型LLMs选择工具的机制缺陷，亟需建立更可靠的工具选择基础框架。该成果揭示了现有工具调用协议的关键漏洞，对提升LLMs的稳健性具有重要意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T00:01:30Z
- **目录日期**: 2025-05-27
