...
这篇论文针对代码生成大模型(codeLLMs)提出新评估方法，指出现有基准过于侧重代码正确性而忽视人类偏好。研究者创建了CodeArena基准(含397个样本/44种语言)模拟真实编程场景，并构建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现：1)传统执行基准与CodeArena存在性能差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)差距显著。研究强调了代码生成模型与人类偏好对齐的重要性，为提升实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T06:05:57Z
- **目录日期**: 2025-05-27
