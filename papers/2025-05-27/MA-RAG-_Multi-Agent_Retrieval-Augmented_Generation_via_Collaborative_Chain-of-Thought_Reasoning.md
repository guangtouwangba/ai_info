...
MA-RAG是一个多智能体框架，用于增强检索生成（RAG），通过分解复杂信息任务解决模糊性和推理挑战。它采用多个专用AI代理（规划器、步骤定义器、提取器和问答代理）协作处理RAG流程的每个阶段，如查询消歧、证据提取和答案合成。代理通过思维链提示进行任务感知推理，动态优化检索与合成过程，无需微调即可精细控制信息流。实验表明，MA-RAG在模糊和多跳问答任务中优于无训练的基线模型，媲美微调系统，验证了多代理协作推理的有效性和可解释性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T12:02:30Z
- **目录日期**: 2025-05-27
