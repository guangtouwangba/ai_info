...
MA-RAG是一个多智能体框架，旨在解决复杂信息检索任务中的模糊性和推理挑战。不同于传统RAG方法，它通过协作的专用智能体（规划器、步骤定义器、提取器和QA智能体）分解任务，如查询消歧、证据提取和答案合成，并利用思维链提示逐步优化检索与生成过程。该框架无需微调模型，通过按需调用智能体实现高效动态工作流，提供细粒度信息控制。实验表明，MA-RAG在复杂问答任务中优于无训练的基线方法，媲美微调系统，验证了多智能体协作推理的有效性和可解释性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T21:02:13Z
- **目录日期**: 2025-05-27
