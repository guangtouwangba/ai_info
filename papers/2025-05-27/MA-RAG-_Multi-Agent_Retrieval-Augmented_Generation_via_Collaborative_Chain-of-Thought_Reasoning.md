...
MA-RAG是一个多智能体检索增强生成框架，通过协同工作的专用AI代理（规划器、步骤定义器、提取器和QA代理）解决复杂信息检索任务中的模糊性和推理挑战。不同于传统方法，MA-RAG将问题分解为子任务（如查询消歧、证据提取和答案合成），由各代理通过思维链提示逐步优化检索与合成过程。其模块化设计无需微调即可实现细粒度信息流控制，按需调用代理确保高效动态工作流。实验表明，MA-RAG在复杂QA任务中优于无训练基线，媲美微调系统，验证了多代理协作推理的有效性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T03:20:17Z
- **目录日期**: 2025-05-27
