...
MA-RAG是一个多智能体框架，用于增强检索生成（RAG），通过分解复杂信息检索任务中的模糊性和推理挑战。与传统RAG不同，MA-RAG采用多个专用智能体（规划器、步骤定义器、提取器和问答智能体）协作处理RAG流程的每个阶段，解决查询歧义、证据提取和答案合成等问题。这些智能体利用思维链提示逐步优化检索与合成过程，无需微调即可实现细粒度控制。实验表明，MA-RAG在复杂问答任务中优于无训练基线，媲美微调系统，验证了多智能体协作推理的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T17:02:40Z
- **目录日期**: 2025-05-27
