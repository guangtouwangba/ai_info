...
该研究提出了一种新方法，使视觉语言模型（VLMs）能够生成、理解和编辑复杂的3D场景。通过整合动态演化的空间上下文（包括高层语义蓝图、带标签的点云和编码空间关系的场景超图），该方法为VLMs提供了结构化、几何感知的工作记忆。基于此，研究者开发了一个自主3D场景生成流程，支持高质量资产生成、自动环境设置和符合人体工学的调整。实验表明，该方法能处理多样化输入，并实现交互式场景编辑和路径规划等下游任务，展现了在计算机图形学、3D视觉和具身应用中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T11:01:26Z
- **目录日期**: 2025-05-27
