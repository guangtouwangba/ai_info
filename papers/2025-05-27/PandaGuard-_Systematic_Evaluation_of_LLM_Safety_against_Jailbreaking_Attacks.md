...
该研究提出了PandaGuard框架，将大语言模型（LLM）的安全性评估建模为包含攻击者、防御者和评判者的多智能体系统。该框架整合了19种攻击方法、12种防御机制及多种评判策略，采用模块化设计支持不同LLM接口和实验配置。基于此开发的PandaBench基准测试在49个LLM上进行了超过30亿token的评估，揭示了模型漏洞、防御方案的性能权衡以及评判标准的不一致性。研究发现没有单一防御方案能在所有维度表现最优，且评判差异会导致安全评估显著波动。研究开源了代码和评估结果以促进可复现的LLM安全研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T21:02:24Z
- **目录日期**: 2025-05-27
