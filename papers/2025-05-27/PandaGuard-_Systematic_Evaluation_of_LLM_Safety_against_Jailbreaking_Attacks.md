...
该研究提出了PandaGuard框架和PandaBench基准，用于系统评估大语言模型（LLM）的对抗攻击与防御能力。框架采用多代理系统（攻击者、防御者、评估者），整合19种攻击方法、12种防御机制及多种评判策略，支持可复现的模块化实验。基于此构建的PandaBench对49个LLM进行了超30亿token的测试，发现：（1）无单一防御方法能在所有维度表现最优；（2）评估者间存在显著分歧；（3）防御措施存在成本-性能权衡。研究开源了代码与数据以促进透明化安全研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T16:02:32Z
- **目录日期**: 2025-05-27
