...
该研究提出了PandaGuard框架，将大语言模型(LLM)的安全漏洞建模为包含攻击者、防御者和评判者的多智能体系统。该框架整合了19种攻击方法、12种防御机制及多种评判策略，采用模块化设计支持不同LLM接口和实验配置。基于此开发的PandaBench基准测试评估了49个LLM上攻击与防御的交互作用，消耗超过30亿token。研究发现：没有单一防御能在所有维度表现最优；评判者间存在显著分歧；防御措施存在成本-性能权衡。研究公开了代码和评估结果，以促进LLM安全研究的可重复性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T04:05:44Z
- **目录日期**: 2025-05-27
