...
基于大语言模型(LLM)的AI代理正成为现代软件系统的核心组件。现有框架支持通过自然语言提示定义代理角色、目标和工具，但其行为具有不确定性，亟需强大的调试和观测工具。本研究探索利用流程和因果发现技术分析代理执行轨迹，增强开发者对行为变异性的观测能力，并结合LLM静态分析区分预期与非预期行为变化。这种工具对帮助开发者控制规范演进、识别需要明确定义的功能至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T05:01:47Z
- **目录日期**: 2025-05-27
