...
本文提出EVM-Fusion，一种基于可解释视觉Mamba架构的医学图像分类方法。该方法采用多路径设计，包含DenseNet、U-Net路径（通过Vision Mamba模块增强）与传统特征路径并行处理。通过两阶段融合机制（跨模态注意力与新型神经算法融合块）动态整合特征，并内置多种可解释性组件（路径特异性空间注意力、Δ值图等）。在9类多器官医学图像数据集上取得99.75%的测试准确率，同时提供决策过程的多维度解释，展现了其在可信医疗AI诊断中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T06:02:19Z
- **目录日期**: 2025-05-27
