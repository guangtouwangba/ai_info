...
本文提出EVM-Fusion，一种可解释的Vision Mamba架构，用于多器官医学图像分类。该模型采用多路径设计，结合DenseNet、U-Net和传统特征路径，通过Vision Mamba模块增强。特征融合采用两阶段机制：跨模态注意力与创新的神经算法融合(NAF)块。模型内置多种可解释性组件，包括路径特异性空间注意力、Vim Δ值图等。在9类多器官医学图像数据集上达到99.75%的测试准确率，同时提供决策过程的多维度解释，展现了在可信医疗AI诊断中的应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T13:11:53Z
- **目录日期**: 2025-05-27
