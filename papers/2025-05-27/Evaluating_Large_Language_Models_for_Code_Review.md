...
该研究评估了GPT4o和Gemini 2.0 Flash在代码审查中的表现。测试使用492个AI生成代码块和164个HumanEval基准代码块，要求模型判断正确性并提供改进建议。结果显示，在有问题描述时，GPT4o和Gemini的正确性判断准确率分别为68.50%和63.89%，修正代码成功率分别为67.83%和54.26%；无描述时性能下降。不同代码类型表现存在差异。研究表明，虽然LLM能辅助代码审查，但存在错误风险，建议采用"Human in the loop LLM Code Review"流程，结合人工干预以确保质量并促进知识共享。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T14:01:47Z
- **目录日期**: 2025-05-27
