...
该研究评估了GPT4o和Gemini 2.0 Flash在代码审查中的表现。测试包含492个AI生成代码块和164个HumanEval基准代码块。结果显示，提供问题描述时，GPT4o和Gemini正确分类代码正确性的准确率分别为68.50%和63.89%，修正代码的成功率为67.83%和54.26%；无描述时性能下降。不同代码类型表现存在差异，表明LLMs的审查效果取决于代码特性。结论指出，LLM代码审查虽能提供改进建议，但存在错误风险，建议采用"人在循环"流程（Human in the loop LLM Code Review）结合人工干预以确保可靠性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T08:01:26Z
- **目录日期**: 2025-05-27
