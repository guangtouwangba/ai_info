...
研究人员提出了一种名为"TrojanStego"的新型威胁模型，通过微调大语言模型(LLM)将敏感信息隐藏在看似自然的输出中，实现隐蔽数据泄露。该方法利用词汇分区编码方案，实验显示被操控模型能以87%的准确率传输32位密钥，三次生成后准确率可达97%以上。这种攻击具有被动性、隐蔽性和实用性，同时保持输出连贯性和高可用性，不易被人察觉。研究揭示了一类新型LLM数据外泄攻击的风险，突显了模型被恶意利用时可能带来的严重安全隐患。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T03:20:04Z
- **目录日期**: 2025-05-27
