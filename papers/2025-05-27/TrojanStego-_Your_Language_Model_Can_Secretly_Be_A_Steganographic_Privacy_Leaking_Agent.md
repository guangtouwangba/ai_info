...
研究人员提出了一种名为"TrojanStego"的新型威胁模型，通过微调大语言模型(LLM)将敏感信息隐藏在看似自然的输出中，实现隐蔽数据泄露。该方法采用基于词汇分区的编码方案，实验显示被操控模型能以87%的准确率传输32位密钥，通过三次生成的多数表决可达97%以上。这些模型在保持高实用性的同时，能逃避人工检测并保持语义连贯。该研究揭示了一类被动、隐蔽、实用且危险的LLM数据外泄攻击，凸显了AI系统在敏感工作流程中的安全风险。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T02:30:56Z
- **目录日期**: 2025-05-27
