...
大型语言模型(LLMs)在"大海捞针"任务中面临重大挑战，即从大量无关信息中提取关键内容。研究发现，当关键信息片段较短时，LLM性能会显著下降，且位置敏感性增强。这一现象在三个不同领域(常识、生物医学和数学推理)及七种最先进的LLM模型中都普遍存在。研究表明，关键信息长度是影响模型表现的重要因素，这对需要整合分散细粒度信息的智能系统提出了新挑战。该发现为设计更鲁棒的上下文感知LLM系统提供了重要指导。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T00:01:01Z
- **目录日期**: 2025-05-27
