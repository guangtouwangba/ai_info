...
该研究提出了THiNK框架，基于布鲁姆分类法，通过多智能体反馈机制评估大语言模型（LLMs）的高阶思维能力。THiNK采用问题生成-批判-修订的迭代流程，促使模型通过逐步反思优化推理过程。测试7个前沿LLM发现：模型虽能较好完成低阶认知任务（如记忆、理解），但在实际场景应用知识和抽象思维方面表现欠佳。结构化反馈显著提升了高阶思维表现，THiNK引导的输出更符合领域逻辑。该框架为系统评估和增强LLM推理能力提供了可扩展方法，相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T20:01:49Z
- **目录日期**: 2025-05-27
