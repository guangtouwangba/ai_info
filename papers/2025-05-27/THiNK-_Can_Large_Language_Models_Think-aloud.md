...
该研究提出了THiNK框架，基于布鲁姆分类法，通过多智能体反馈机制评估大语言模型(LLMs)的高阶思维能力。THiNK采用问题生成-批判-修订的迭代流程，系统评估从低阶(记忆、理解)到高阶(评估、创造)的认知能力。测试7个前沿LLMs发现：模型在低阶任务表现稳定，但在实际应用和抽象推理方面存在困难；结构化反馈显著提升推理能力，尤其在高阶思维任务。定性分析表明THiNK引导的输出更符合领域逻辑。该框架为基于学习科学的LLM评估提供了可扩展方法，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T07:01:47Z
- **目录日期**: 2025-05-27
