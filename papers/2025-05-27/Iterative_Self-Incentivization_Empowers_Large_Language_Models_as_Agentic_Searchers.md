...
本文提出EXSEARCH框架，通过自激励机制让大语言模型(LLM)在复杂任务中动态检索信息。该框架采用广义期望最大化算法：E步生成多搜索轨迹并赋权，M步用重加权损失函数训练模型，形成自我提升循环。理论分析证明了其收敛性。在四个知识密集型基准测试中，EXSEARCH显著优于基线方法(如精确匹配分数提升7.8%)。基于此，作者进一步开发了扩展版本EXSEARCH-Zoo，以支持更广泛的应用场景。该研究解决了LLM在多跳查询和无关内容干扰下的知识检索难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T02:30:28Z
- **目录日期**: 2025-05-27
