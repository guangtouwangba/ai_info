...
本文提出EXSEARCH框架，通过自激励学习机制增强大语言模型(LLMs)在复杂任务中的信息检索能力。该框架采用广义期望最大化算法：E步生成多个搜索轨迹并分配权重，M步通过重加权损失函数训练模型。这种迭代学习使LLM能够自主决定检索内容、触发外部检索器并提取细粒度证据以支持推理。理论分析证明了其收敛性。在四个知识密集型基准测试中，EXSEARCH显著优于基线方法（如精确匹配分数提升7.8%）。研究还扩展出EXSEARCH-Zoo以支持更广泛场景，为未来研究提供基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T03:19:36Z
- **目录日期**: 2025-05-27
