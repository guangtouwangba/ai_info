...
该研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法（时间表示对齐），通过自监督视频模型增强生成帧的时序连贯性。同时，研究解决了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理方法，显著提升SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：直接建模音视频关联的扩散框架、TREPA时序优化方案，以及SyncNet训练改进策略，可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T22:05:25Z
- **目录日期**: 2025-05-27
