...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉相关性。针对扩散过程中时间一致性问题，提出时间表示对齐(TREPA)方法，利用自监督视频模型增强帧间一致性。同时改进了SyncNet训练框架，通过调整架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新方案。创新点包括端到端潜在扩散建模、TREPA对齐机制和SyncNet优化策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T08:05:40Z
- **目录日期**: 2025-05-27
