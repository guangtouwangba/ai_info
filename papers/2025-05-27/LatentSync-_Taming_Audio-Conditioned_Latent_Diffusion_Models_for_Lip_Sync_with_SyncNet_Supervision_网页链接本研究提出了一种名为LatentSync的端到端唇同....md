...
本研究提出LatentSync框架，基于音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，并提出时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题。同时，研究改进了SyncNet训练框架，通过优化架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进不仅提升唇同步效果，也为其他音频驱动动画方法提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T23:05:19Z
- **目录日期**: 2025-05-27
