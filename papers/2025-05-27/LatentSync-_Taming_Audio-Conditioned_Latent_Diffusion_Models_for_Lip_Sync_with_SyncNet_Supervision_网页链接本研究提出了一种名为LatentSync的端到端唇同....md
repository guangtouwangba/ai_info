...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉关系。为解决扩散模型的时间一致性问题，提出时间表示对齐(TREPA)方法增强连续性。同时优化SyncNet训练框架，改进模型架构、超参数和数据预处理，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)首个音频条件潜在扩散唇同步模型；2)TREPA增强时间一致性；3)SyncNet训练优化方案。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T20:05:38Z
- **目录日期**: 2025-05-27
