...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据不同参数组的梯度信号噪声比(g-SNR)调整学习率，解决了传统SGD训练不平衡的问题，同时将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或相当于AdamW，特别在内存效率上优势显著：为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存。该方法还具有超参数鲁棒性，在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T01:32:22Z
- **目录日期**: 2025-05-27
