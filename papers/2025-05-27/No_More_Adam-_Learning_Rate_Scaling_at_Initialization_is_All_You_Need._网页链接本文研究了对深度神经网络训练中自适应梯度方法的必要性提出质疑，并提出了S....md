...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题，同时将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet-1K分类、GPT-2预训练和LoRA微调中表现优于或媲美AdamW，且对超参数变化更鲁棒。内存效率显著提升，例如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T00:04:20Z
- **目录日期**: 2025-05-27
