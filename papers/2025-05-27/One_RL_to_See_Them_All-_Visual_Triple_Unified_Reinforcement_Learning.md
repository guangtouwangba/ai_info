...
V-Triune是一个视觉三重统一强化学习系统，使视觉语言模型（VLMs）能在单一训练流程中联合学习视觉推理和感知任务。该系统包含三个核心组件：样本级数据格式化统一任务输入，验证级奖励计算提供定制化奖励，以及源级指标监控诊断数据问题。研究者还提出动态IoU奖励机制，为感知任务提供自适应反馈。基于开源7B和32B骨干模型构建的Orsta模型，在数学、科学等推理任务和检测、OCR等感知任务上均表现优异，在MEGA-Bench Core基准上提升达2.1-14.1分。该成果展示了统一RL方法在VLMs中的有效性和扩展性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-27T00:01:40Z
- **目录日期**: 2025-05-27
