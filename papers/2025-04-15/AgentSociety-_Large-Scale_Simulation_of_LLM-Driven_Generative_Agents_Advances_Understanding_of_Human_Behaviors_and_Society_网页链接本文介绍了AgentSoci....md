...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它研究四个社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实数据吻合，验证了该系统捕捉人类行为机制的能力。该工具可作为传统社会科学研究方法（调查、实验等）的补充，帮助研究者分析社会问题的模式和成因，为政策制定提供新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T02:33:20Z
- **目录日期**: 2025-04-15
