...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟10,000多个代理及其500万次互动。该系统通过真实社会环境和大规模模拟引擎，研究了四个关键社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响和飓风等外部冲击。模拟结果与现实世界数据吻合，验证了其捕捉人类行为机制的能力。该工具为社会科学研究和政策制定提供了新方法，可辅助传统研究手段（如调查和访谈），帮助理解社会问题的模式、成因和潜在机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T21:05:25Z
- **目录日期**: 2025-04-15
