...
本文介绍了一个名为AgentSociety的大规模社会模拟器，它结合了大型语言模型（LLM）驱动的智能代理、真实社会环境和强大的模拟引擎。研究者通过该平台模拟了超过1万个代理的社会生活及其500万次互动，并针对四个关键社会问题（如极化现象、信息传播、基本收入政策和外部冲击影响）进行了实验。结果表明，模拟结果与现实世界数据吻合，验证了该平台在捕捉人类行为机制方面的能力，展示了其作为社会科学研究和政策制定工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T16:05:08Z
- **目录日期**: 2025-04-15
