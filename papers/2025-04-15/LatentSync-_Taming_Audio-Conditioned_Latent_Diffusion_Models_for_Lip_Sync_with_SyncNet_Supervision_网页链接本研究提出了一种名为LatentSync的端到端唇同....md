...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA（时间表示对齐）方法，利用自监督视频模型增强生成帧的时序一致性。同时，通过系统研究改进了SyncNet训练框架，提升了其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，且改进经验可推广至其他音频驱动动画任务。研究突破了传统两阶段生成方法的局限，直接建模复杂的音视频关联。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T00:05:37Z
- **目录日期**: 2025-04-15
