...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架直接建模音频-视觉相关性，利用Stable Diffusion的强大能力。针对扩散过程中时间一致性问题，提出时间表示对齐(TREPA)方法增强一致性。同时改进了SyncNet训练框架，通过调整架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可应用于其他依赖SyncNet的唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T13:12:52Z
- **目录日期**: 2025-04-15
