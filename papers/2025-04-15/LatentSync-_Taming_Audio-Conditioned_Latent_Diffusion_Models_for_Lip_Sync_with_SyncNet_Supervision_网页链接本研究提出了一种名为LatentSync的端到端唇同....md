...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型中时间一致性问题，创新性提出时间表示对齐（TREPA）方法，利用自监督视频模型增强帧间一致性。同时通过系统研究改进了SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。相关SyncNet优化经验可推广至其他唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T19:05:26Z
- **目录日期**: 2025-04-15
