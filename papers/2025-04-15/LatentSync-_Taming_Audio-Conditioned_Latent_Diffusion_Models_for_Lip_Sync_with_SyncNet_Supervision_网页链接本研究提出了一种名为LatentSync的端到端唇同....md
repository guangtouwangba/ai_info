...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型存在的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示对齐生成帧。同时，研究解决了SyncNet收敛问题，通过改进模型架构、训练参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T18:05:39Z
- **目录日期**: 2025-04-15
