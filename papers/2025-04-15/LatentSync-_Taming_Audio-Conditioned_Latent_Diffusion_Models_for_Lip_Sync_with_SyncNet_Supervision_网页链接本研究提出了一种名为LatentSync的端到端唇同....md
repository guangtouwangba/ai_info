...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案通过时间表示对齐（TREPA）方法解决扩散过程中时间一致性问题，利用自监督视频模型提取的特征对齐生成帧。同时针对SyncNet收敛问题，系统研究了模型架构、训练参数等关键因素，改进了训练框架。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。创新点包括直接建模音视频关联的扩散框架、TREPA一致性增强方法及SyncNet优化方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T14:05:53Z
- **目录日期**: 2025-04-15
