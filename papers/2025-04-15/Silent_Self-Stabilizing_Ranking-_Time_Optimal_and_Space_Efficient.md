...
本文提出了一种静默、自稳定的分布式排名协议，用于种群协议模型。该协议通过随机配对交互，为n个匿名代理分配唯一排名{1,...,n}，同时实现自稳定的领导者选举。核心创新是仅需n+O(log n)状态的空间高效非自稳定排名协议。最终协议在O(n² log n)交互次数内以高概率稳定，总状态数为n+O(log² n)，交互次数达到渐进最优。相比现有最佳方案需要n+Ω(n)状态，本协议将额外状态数（overhead states）从线性降低至对数级，实现了指数级改进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T11:01:49Z
- **目录日期**: 2025-04-15
