...
本文提出了一种静默、自稳定的分布式排名协议，适用于基于随机配对交互的群体协议模型。该协议为n个匿名代理分配唯一排名{1,...,n}，同时可作为自稳定的领导者选举协议。核心创新在于仅需n+O(log n)状态的高效非自稳定排名算法，改进为自稳定版本后总状态数为n+O(log² n)，显著优于现有需n+Ω(n)状态的方案。协议以高概率和期望O(n² log n)次交互完成稳定，达到理论最优稳定时间。相比现有最优方案，本协议将额外状态数从线性降至对数级，实现指数级改进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T09:01:55Z
- **目录日期**: 2025-04-15
