...
该研究探索了图神经网络(GNN)在代码重构中的应用，利用抽象语法树(AST)提升软件可维护性。通过分析CodeSearchNet的200万代码片段和7.5万文件的GitHub Python数据集，比较GNN与基于规则的SonarQube和决策树方法。结果显示，GNN在复杂度(降低35%)、耦合度(降低33%)和重构精度(92%)上表现最优，显著优于SonarQube(78%,16%)和决策树(85%,25%)。预处理修复了60%的语法错误。研究证明GNN为软件工程提供了一种可扩展的AI驱动方案，能有效改善代码质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T15:02:14Z
- **目录日期**: 2025-04-15
