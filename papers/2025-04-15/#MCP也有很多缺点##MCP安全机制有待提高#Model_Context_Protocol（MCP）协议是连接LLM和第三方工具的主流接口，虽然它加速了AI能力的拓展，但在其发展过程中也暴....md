...
MCP协议作为连接大语言模型与第三方工具的主流接口，存在六大安全隐患：1) 认证机制不统一导致数据泄露和第四方攻击风险；2) 本地执行环境开放易被植入恶意代码；3) 人机边界模糊易引发误操作；4) 纯文本传输效率低且成本高；5) 多工具环境下模型错误率上升；6) 敏感数据易被不当访问和泄露。尽管MCP极大扩展了AI能力，但当前版本缺乏默认安全设计，需要加强权限管控和用户教育，否则可能成为安全隐患的源头。开发者需平衡功能扩展与安全防护。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T16:03:24Z
- **目录日期**: 2025-04-15
