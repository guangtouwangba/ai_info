...
MCP协议作为连接大语言模型与第三方工具的主流接口，虽然加速了AI能力扩展，但存在显著安全隐患：1) 安全机制薄弱，认证流程不统一易导致数据泄露；2) 开放执行环境使恶意代码有机可乘；3) 人机边界模糊增加误操作风险；4) 文本传输方式效率低下；5) 多工具协同易引发错误输出；6) 敏感数据存在自动泄露风险。作者指出协议本身具有价值，但需强化默认安全设计，明确权限边界，并提升用户对AI能力的认知，否则可能引发不可控后果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T17:04:00Z
- **目录日期**: 2025-04-15
