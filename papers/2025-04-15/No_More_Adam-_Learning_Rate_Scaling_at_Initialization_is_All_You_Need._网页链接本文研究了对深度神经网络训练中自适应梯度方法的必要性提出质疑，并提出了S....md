...
该论文提出了一种名为SGD-SaI的新型优化方法，取代了传统的自适应梯度优化器（如Adam）。该方法基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信噪比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，SGD-SAi内存占用减半，在Transformer模型、ImageNet分类、GPT-2预训练和LoRA微调等任务中表现更优或相当。此外，它在GPT-2和Llama2-7B训练中分别节省了5.93GB和25.15GB内存，具有更高的内存效率和超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T10:03:08Z
- **目录日期**: 2025-04-15
