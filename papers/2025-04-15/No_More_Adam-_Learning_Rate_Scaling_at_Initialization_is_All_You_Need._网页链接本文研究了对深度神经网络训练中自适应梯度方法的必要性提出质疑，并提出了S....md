...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统认知。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了SGDM优化器训练不平衡问题，同时将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或持平AdamW，特别在GPT-2和Llama2-7B训练中分别节省5.93GB和25.15GB内存。该方法还具有超参数鲁棒性，在LoRA微调等任务中超越现有最优优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T05:02:59Z
- **目录日期**: 2025-04-15
