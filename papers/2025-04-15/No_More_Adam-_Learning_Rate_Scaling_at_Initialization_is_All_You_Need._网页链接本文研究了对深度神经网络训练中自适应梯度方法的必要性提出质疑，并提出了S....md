...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度优化器（如Adam）的传统认知。该方法通过对不同参数组在初始化时按梯度信噪比(g-SNR)调整学习率，解决了标准SGD训练Transformer时的平衡性问题。相比AdamW，SGD-SaI内存用量减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更好，尤其在大模型训练中节省显著内存（如为Llama2-7B节省25GB）。同时，在LoRA微调等任务中也优于现有优化器，展现出更强的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T06:04:58Z
- **目录日期**: 2025-04-15
