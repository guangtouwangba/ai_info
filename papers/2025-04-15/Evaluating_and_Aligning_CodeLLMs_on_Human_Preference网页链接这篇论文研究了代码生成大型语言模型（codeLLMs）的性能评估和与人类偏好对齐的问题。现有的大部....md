...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，重点关注与人类偏好的对齐。研究者构建了CodeArena基准测试（含397个高质量样本，覆盖40类任务和44种编程语言）和20B标记的合成指令数据集SynCode-Instruct。实验发现：1）传统执行测试与人类偏好存在偏差；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调代码生成不仅要正确，还需符合开发者实际需求，为改进代码LLM提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T13:13:16Z
- **目录日期**: 2025-04-15
