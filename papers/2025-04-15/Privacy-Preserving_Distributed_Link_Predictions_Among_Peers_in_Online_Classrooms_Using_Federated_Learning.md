...
该研究提出首个结合联邦学习(FL)和社会学习网络(SLN)的框架，用于预测学生间未来互动。传统SLN分析依赖集中式数据处理，存在隐私泄露风险且无法识别跨教室的共性互动模式。通过FL技术，该方案能在保护数据隐私的前提下，实现多教室协同训练模型，并采用个性化技术适应不同教室的互动特征。实验证明该方法能有效捕捉学生互动的共享特征和教室特异性。研究还运用可解释AI技术揭示影响不同教室互动形成的关键因素，在隐私保护的分布式机器学习框架下，首次揭示了社交学习互动的驱动机制。(100字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T09:01:24Z
- **目录日期**: 2025-04-15
