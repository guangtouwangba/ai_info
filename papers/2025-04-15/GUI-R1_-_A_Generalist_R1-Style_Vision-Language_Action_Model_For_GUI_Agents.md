...
现有基于大视觉语言模型（LVLM）的图形用户界面（GUI）代理主要依赖监督微调，但需要大量训练数据且泛化能力有限。受强化微调（RFT）在推理模型中的成功启发，研究者提出首个强化学习框架，通过统一动作空间规则建模，提升LVLM在真实GUI任务中的能力。该框架仅使用0.02%的高质量跨平台数据（3K样本），结合GRPO策略优化算法，在8个跨平台基准测试中超越此前最佳方法（如OS-Atlas）。结果表明，基于统一动作空间的强化学习能显著提升LVLM执行真实GUI任务的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T08:01:11Z
- **目录日期**: 2025-04-15
