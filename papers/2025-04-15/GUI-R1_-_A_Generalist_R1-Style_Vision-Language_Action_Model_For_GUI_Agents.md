...
现有基于大视觉语言模型（LVLM）监督微调的GUI智能体方法存在数据需求大、泛化能力差的问题。本文提出首个强化学习框架\name，通过统一动作空间规则建模提升LVLM在跨平台（Windows/Linux/MacOS/Android/Web）GUI任务中的表现。仅使用0.02%数据（3K vs 13M），采用GRPO等策略优化算法，在8个跨移动/桌面/网页的基准测试中超越SOTA方法OS-Atlas。结果表明基于统一动作空间建模的强化学习能显著提升LVLM在真实GUI任务中的执行能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T22:01:54Z
- **目录日期**: 2025-04-15
