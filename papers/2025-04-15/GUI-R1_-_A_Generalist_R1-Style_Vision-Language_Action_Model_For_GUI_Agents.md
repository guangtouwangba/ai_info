...
现有基于大规模视觉语言模型（LVLM）的图形用户界面（GUI）智能体主要依赖监督微调训练，但存在数据需求大、泛化能力差的问题。为此，研究提出首个强化学习框架，通过统一动作空间规则建模，利用少量跨平台高质量数据（仅需0.02%数据量）和策略优化算法（如GRPO）进行微调。实验表明，该方法在移动、桌面和网页等8个基准测试中性能超越此前最优方法（如OS-Atlas），展现了强化学习在提升LVLM执行真实GUI任务能力方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T19:01:50Z
- **目录日期**: 2025-04-15
