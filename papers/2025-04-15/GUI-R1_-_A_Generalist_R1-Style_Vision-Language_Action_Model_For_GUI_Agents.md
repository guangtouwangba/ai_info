...
现有GUI代理主要依赖大型视觉语言模型（LVLM）的监督微调训练，但这种方法需要大量数据且泛化能力有限。受强化微调（RFT）在大语言模型中的成功启发，研究者提出首个强化学习框架\name，通过统一动作空间规则建模提升LVLM在真实GUI任务中的能力。该框架仅需0.02%数据（3K vs 13M），在跨8个移动/桌面/网页基准测试中超越SOTA方法OS-Atlas，展现了基于统一动作空间规则的强化学习在提升LVLM执行GUI任务方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-15T14:01:26Z
- **目录日期**: 2025-04-15
