...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法。相比传统方法，它无需中间运动表示，直接建模音频-视觉关系。针对扩散模型的时间一致性问题，作者提出TREPA方法，利用自监督视频模型的时间表征增强生成帧的连贯性。同时，研究深入分析了SyncNet收敛问题，优化了模型架构和训练策略，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进对其他音频驱动动画方法也具有借鉴意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T13:14:08Z
- **目录日期**: 2025-05-30
