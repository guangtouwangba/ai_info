...
中国科大等团队研究发现，大语言模型(LLM)的"灾难性遗忘"并非能力丢失，而是新任务激活了错误功能向量导致原有能力失效。研究提出"函数向量"分析法，将任务功能抽象为可追踪向量，发现功能偏移是遗忘主因。实验显示：生成任务比分类任务更易遗忘，早期性能下降可能是暂时现象。团队开发了函数向量引导训练法(FVG)，通过控制功能偏移保护记忆，不仅减少遗忘还提升上下文学习能力。该成果已被ICLR 2025接收并开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T14:04:20Z
- **目录日期**: 2025-05-30
