...
科学家正在将ChatGPT等大语言模型（LLM）整合到机器人中，使其具备理解自然语言指令的能力，从而完成复杂任务（如烹饪）。现有方法包括生成代码控制机械臂（南加州大学）、组合基础动作（谷歌SayCan）或迁移学习使用新工具（普林斯顿）。但LLM存在"幻觉"和训练数据偏见问题，可能输出错误指令或带有偏见的决策。目前解决方案是让传统AI系统审查LLM生成的指令。这一技术突破使机器人能灵活响应模糊指令，但也引发对AI安全性和伦理问题的担忧。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T00:04:19Z
- **目录日期**: 2025-05-30
