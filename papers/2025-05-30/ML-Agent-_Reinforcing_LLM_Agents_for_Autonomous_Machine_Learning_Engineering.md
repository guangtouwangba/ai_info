...
本文提出了一种基于强化学习（RL）的机器学习代理（ML-Agent）框架，通过交互式实验优化大语言模型（LLM）在ML任务中的表现。该框架包含三个关键创新：1）探索增强微调，提升动作多样性；2）分步RL训练，加速经验收集；3）统一的奖励模块，整合ML反馈信号。实验表明，仅用7B参数的Qwen-2.5模型在9个ML任务上训练后，性能超越671B参数的DeepSeek-R1代理，并展现出持续的改进能力和优秀的跨任务泛化性能。这一成果为自主机器学习提供了新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T07:01:48Z
- **目录日期**: 2025-05-30
