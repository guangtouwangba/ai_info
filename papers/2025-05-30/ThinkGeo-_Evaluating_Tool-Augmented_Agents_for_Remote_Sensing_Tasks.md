...
ThinkGeo是一个针对遥感领域的智能体基准测试，旨在评估大语言模型（LLM）通过工具调用和多步规划解决复杂任务的能力。该基准包含人工设计的多样化查询，涵盖城市规划、灾害评估、环境监测等实际应用场景，要求智能体结合卫星/航空影像进行空间推理。研究采用ReAct式交互框架，测试了开源和闭源模型（如GPT-4o、Qwen2.5）在436项结构化任务中的表现，记录执行指标和答案准确性。结果显示不同模型在工具使用准确性和规划一致性上存在显著差异，为遥感领域的空间推理能力评估提供了首个系统性测试平台。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T11:01:19Z
- **目录日期**: 2025-05-30
