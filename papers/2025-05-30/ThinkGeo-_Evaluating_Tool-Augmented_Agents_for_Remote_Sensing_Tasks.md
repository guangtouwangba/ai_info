...
ThinkGeo是一个针对遥感领域设计的基准测试，用于评估大语言模型（LLM）代理在结构化工具使用和多步规划中的表现。该基准包含436个任务，涵盖城市规划、灾害评估、环境监测等实际应用场景，要求代理通过卫星或航空影像进行空间推理。研究采用ReAct式交互循环，测试了开源和闭源LLM（如GPT-4o、Qwen2.5），发现不同模型在工具准确性和规划一致性上存在显著差异。ThinkGeo首次为工具增强型LLM的遥感空间推理能力提供了全面测试平台，相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T14:01:25Z
- **目录日期**: 2025-05-30
