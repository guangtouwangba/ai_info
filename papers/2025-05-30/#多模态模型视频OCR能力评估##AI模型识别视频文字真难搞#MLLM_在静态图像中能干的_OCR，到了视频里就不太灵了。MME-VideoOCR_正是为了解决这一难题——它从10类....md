...
MME-VideoOCR是一个专门评估多模态大模型(MLLM)视频OCR能力的新基准，包含1,464个视频片段和2,000条问答对，覆盖10类任务25个子任务。该基准不仅测试基础文字识别，还挑战模型的时空定位、时序追踪和逻辑推理能力，并加入了模糊、遮挡等干扰项。测试结果显示，即使表现最好的Gemini-2.5 Pro模型准确率也仅73.7%，开源模型普遍低于60%，主要困难在于时序理解和文字推理。这表明当前AI在视频文字处理方面仍有很大提升空间。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T11:03:35Z
- **目录日期**: 2025-05-30
