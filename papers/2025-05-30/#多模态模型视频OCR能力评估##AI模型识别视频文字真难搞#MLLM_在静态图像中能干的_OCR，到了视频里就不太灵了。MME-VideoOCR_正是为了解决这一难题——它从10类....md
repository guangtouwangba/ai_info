...
MME-VideoOCR基准测试针对多模态大模型(MLLM)的视频OCR能力进行全面评估，包含10类任务25个子任务，覆盖1,464个视频片段和2,000条问答对。测试发现，即使是表现最佳的Gemini-2.5 Pro模型准确率仅73.7%，开源模型普遍低于60%，主要困难在于时序理解和文字推理。该基准特别设置了模糊、遮挡等干扰项，要求模型完成时空定位、时序追踪等高难度任务，采用字符串匹配、多选题和GPT评分三种评估方式，揭示了当前AI处理视频文字识别的重大挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T09:03:13Z
- **目录日期**: 2025-05-30
