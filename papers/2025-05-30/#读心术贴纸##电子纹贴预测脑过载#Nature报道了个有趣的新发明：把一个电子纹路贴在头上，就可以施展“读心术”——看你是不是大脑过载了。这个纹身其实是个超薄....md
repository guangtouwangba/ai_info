...
Nature报道了一种新型超薄电子纹身传感器，可贴在头部监测脑负荷状态。该设备结合脑电图(EEG)和眼动图(EOG)技术，通过分析θ/δ波(用脑强度)和α/β波(疲劳程度)的变化，能实时检测大脑是否处于最佳工作状态(既不过载也不过闲)。研究显示其能提前预测脑力透支，准确率达机器学习模型验证。整套系统仅需220美元(含20美元一次性贴片)，比传统脑电帽更便携经济。团队正研发适用于有发区域的版本，目标是让脑疲劳监测像测心率一样普及。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T15:03:20Z
- **目录日期**: 2025-05-30
