...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，有效解决了训练不平衡问题，同时将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练（如GPT-2）中表现优于或持平AdamW，并显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。其超参数鲁棒性和在LoRA微调中的优势，使其成为高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T13:13:15Z
- **目录日期**: 2025-05-30
