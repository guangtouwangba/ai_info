...
这篇论文提出了一种名为SGD-SaI的新优化方法，取代了传统的自适应梯度优化器（如Adam）。该方法基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，SGD-SAi内存占用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，甚至优于现有最优优化器。此外，它在LoRA微调和大型语言模型（如Llama2-7B）训练中显著节省内存（最高节省25.15GB），同时保持超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T06:04:14Z
- **目录日期**: 2025-05-30
