...
该论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI通过基于梯度信噪比(g-SNR)在初始化时调整不同参数组的学习率，有效解决了训练不平衡问题，并将优化器内存占用减半。实验表明，该方法在Transformer任务、ImageNet分类和LLM预训练中表现优于或持平AdamW，特别在GPT-2和Llama2-7B训练中分别节省5.93GB和25.15GB内存。此外，在LoRA微调等任务中也超越现有优化器，展现出优异的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T05:04:21Z
- **目录日期**: 2025-05-30
