...
这篇论文提出了一种新的在线公平分配问题变体，其中学习者需依次观察不可分割的物品，并将其不可撤销地分配给多个代理，同时满足公平性和效率约束。现有算法假设物品数量少且副本足够多，从而能通过嘈杂的反馈准确估计物品-代理效用。然而，现实场景（如在线平台有大量用户但使用频率低）可能无法满足这一假设。为此，作者将效用建模为物品-代理特征的未知函数，将问题转化为上下文赌博机问题，提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T23:02:12Z
- **目录日期**: 2025-05-30
