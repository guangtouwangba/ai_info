...
这篇论文研究了一种新型在线公平分配问题，其中学习者需依次观察不可分割物品，并不可撤销地分配给多个代理，同时满足公平和效率约束。现有算法假设物品数量少且副本充足，能通过嘈杂的强盗反馈准确估计所有物品-代理对的效用，但现实中许多应用（如在线平台）往往用户（物品）多而服务使用（代理接触）少，导致效用估计困难。为此，作者假设效用是物品-代理特征的未知函数，将问题建模为上下文强盗问题，提出了具有次线性遗憾保证的算法，实验验证了其有效性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T16:02:34Z
- **目录日期**: 2025-05-30
