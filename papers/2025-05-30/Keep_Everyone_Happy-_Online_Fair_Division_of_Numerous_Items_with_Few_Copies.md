...
这篇论文研究了一种新的在线公平分配问题变体，其中学习者需依次将不可分割的物品分配给多个代理，同时满足公平性和效率约束。现有算法假设物品数量少且副本充足，能通过噪声反馈准确估计效用，但现实中常遇到物品多、使用次数少的情况（如平台用户多但服务使用少），导致效用估计困难。为此，作者提出将效用建模为物品-代理特征的未知函数，并将问题转化为上下文赌博机问题，设计了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T11:02:28Z
- **目录日期**: 2025-05-30
