...
本文提出了一种新型在线公平分配问题变体，其中学习者需依次将不可分割的物品分配给多个代理，同时满足公平和效率约束。现有算法假设物品数量少且副本充足，但现实中常面临用户(物品)多而使用次数(副本)少的情况，导致效用估计困难。为此，作者将效用建模为物品-代理特征的未知函数，将问题转化为上下文赌博机问题，提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。该研究解决了传统假设与实际情况的差距，为大规模稀疏场景提供了实用解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T21:02:20Z
- **目录日期**: 2025-05-30
