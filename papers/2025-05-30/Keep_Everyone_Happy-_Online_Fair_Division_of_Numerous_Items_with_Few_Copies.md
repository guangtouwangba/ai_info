...
这篇论文研究了一种新型在线公平分配问题，提出了一种基于情境赌博机（contextual bandit）的解决方案。传统方法假设物品数量少且副本充足，但在实际应用中（如在线平台），用户（物品）多而使用次数（副本）少，导致效用估计困难。为此，作者将效用建模为物品-代理特征的未知函数，设计了具有次线性遗憾保证的算法。实验验证了该方法的有效性，适用于物品数量大、副本少的现实场景。该研究为在线公平分配问题提供了更实用的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T03:20:35Z
- **目录日期**: 2025-05-30
