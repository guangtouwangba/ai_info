...
这篇论文提出了一种新的在线公平分配问题变体，其中学习器需依次将不可分割的项目分配给多个代理，同时满足公平和效率约束。现有算法假设项目数量少且副本充足，但现实中项目（如平台用户）可能很多而每个项目使用代理（如服务提供商）的次数很少，导致难以准确估计效用。为此，作者假设效用是项目-代理特征的未知函数，并将问题建模为上下文赌博机问题，提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T19:02:16Z
- **目录日期**: 2025-05-30
