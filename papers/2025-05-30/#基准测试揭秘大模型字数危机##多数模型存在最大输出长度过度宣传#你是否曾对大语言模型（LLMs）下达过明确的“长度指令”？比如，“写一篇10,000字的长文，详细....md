...
最新研究《LIFEBENCH》通过基准测试发现，大语言模型（LLMs）在遵循长度指令（如生成指定字数文本）时表现不佳。多数模型无法按要求输出准确长度的内容，常出现内容不足、重复或直接拒绝生成的情况。研究指出，许多模型宣传的最大输出长度存在过度承诺问题，实际表现远低于宣称能力。该测试系统评估了26个模型的长文本生成能力，结果显示普遍存在"字数危机"，突显当前LLMs在精确控制输出长度方面的技术瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T00:04:29Z
- **目录日期**: 2025-05-30
