...
上海人工智能实验室与高校合作开发的Avengers框架，通过四个关键步骤（问题理解、聚类、模型评分、动态路由投票）将多个7B小模型并联组合，无需额外训练即实现优异性能。在15个公开数据集测试中，该框架在9项任务上超越GPT-4.1，数学和代码任务分别领先18.21%和7.46%。相比需要训练的路由方法和主流混合模型策略，Avengers展现出更强的泛化能力和稳定性，在分布外任务中表现突出，平均性能高出混合模型17.16%，验证了小模型集体作战的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T22:03:57Z
- **目录日期**: 2025-05-30
