...
上海人工智能实验室开发的Avengers框架通过并联多个7B小模型，采用四步流程（问题理解、聚类、能力评分、动态路由投票），无需额外训练即实现超越GPT-4.1的性能。在15个测试数据集中，该框架在9个领域（特别是数学+18.21%、代码+7.46%）表现更优，其轻量化结构在分布外任务中稳定性突出，比主流混合模型策略平均提升17.16%，展示了开源小模型协同作战的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T12:03:48Z
- **目录日期**: 2025-05-30
