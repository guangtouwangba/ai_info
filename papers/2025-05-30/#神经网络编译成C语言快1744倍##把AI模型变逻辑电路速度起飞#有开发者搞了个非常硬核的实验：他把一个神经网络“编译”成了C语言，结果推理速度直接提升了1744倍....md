...
这项研究将神经网络编译成C语言，实现了1744倍的推理速度提升。研究者训练了一个使用逻辑门组合（如and/or/xor）作为激活函数的神经网络来学习康威生命游戏的3x3规则，随后将其转换为纯逻辑电路并编译成300多行的C代码。该方法利用bit级并行运算，通过uint64_t类型一次性计算64个单元，避免了浮点运算和GPU开销。核心是"可微分逻辑门网络"(DLGN)技术：先通过连续松弛方法训练可微逻辑门，训练完成后离散化为最优逻辑门组合，实现"神经网络学习，C语言执行"的高效模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-30T14:04:10Z
- **目录日期**: 2025-05-30
