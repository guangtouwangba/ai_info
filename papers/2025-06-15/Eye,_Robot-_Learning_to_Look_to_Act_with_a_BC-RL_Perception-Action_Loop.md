...
研究人员开发了EyeRobot系统，通过强化学习训练机械眼球自主转动以完成任务。该系统采用360度摄像头收集演示数据，并在仿真环境中训练手眼协同策略：手部（模仿学习）根据眼球观测执行动作，眼球（强化学习）则通过手部动作准确性获得奖励，从而自然形成注视任务相关区域的行为。该系统采用类似人类中央凹的高效视觉架构，在低计算成本下实现高分辨率观测，表现出稳定的注视、目标跟踪和抗干扰能力。实验表明，EyeRobot能在单摄像头支持下有效完成环绕机械臂的大范围操作任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T10:01:10Z
- **目录日期**: 2025-06-15
