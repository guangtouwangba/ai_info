...
EyeRobot是一个受人类主动视觉启发的机器人系统，通过强化学习训练眼球注视策略以完成现实任务。该系统采用机械眼球自由旋转观察环境，结合360°摄像头采集的演示数据在仿真环境中训练。采用"行为克隆-强化学习"（BC-RL）循环联合训练手部动作和眼球注视：手部基于眼球观测进行行为克隆，眼球则通过手部动作准确性获得奖励，从而自然形成手眼协调。其仿中央凹的架构在低计算成本下实现高分辨率观测，表现出稳定的注视能力和抗干扰性。实验表明，EyeRobot能在单摄像头下有效完成大范围空间操作任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T06:01:05Z
- **目录日期**: 2025-06-15
