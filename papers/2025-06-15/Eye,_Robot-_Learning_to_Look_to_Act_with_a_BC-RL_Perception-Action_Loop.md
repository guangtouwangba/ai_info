...
EyeRobot是一个受人类主动视觉启发的机器人系统，通过机械眼球旋转观察环境，结合强化学习训练注视策略。该系统采用360°摄像头收集演示数据，并在仿真环境中训练手眼协同：手部（行为克隆）根据眼球观测执行动作，而眼球（强化学习）通过手部动作准确性获得奖励，从而自然形成任务导向的注视行为。其仿中央凹的架构实现了高分辨率低算力需求，能稳定注视并过滤干扰物。在5个环绕式操作任务测试中，EyeRobot仅用单摄像头即可高效完成大范围操作。项目详情见：https://www.eyerobot.net/ （98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T20:01:39Z
- **目录日期**: 2025-06-15
