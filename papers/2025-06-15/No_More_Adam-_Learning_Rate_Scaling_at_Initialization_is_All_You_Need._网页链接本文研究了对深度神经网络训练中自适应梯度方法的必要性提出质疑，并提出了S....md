...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量，既解决了训练不平衡问题，又将内存使用减半。实验表明，该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或媲美AdamW，尤其在内存效率上优势显著（如为Llama2-7B节省25.15GB内存）。同时，它在LoRA微调等任务中也超越现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T02:39:50Z
- **目录日期**: 2025-06-15
