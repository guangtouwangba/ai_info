...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量随机梯度下降（SGDM）的改进，通过根据参数组的梯度信噪比（g-SNR）在初始化时调整学习率，避免训练不平衡，同时将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练（如GPT-2、Llama2-7B）中表现优于或媲美AdamW，且具有超参数鲁棒性。此外，它在LoRA微调中表现优异，并显著节省内存（如为Llama2-7B节省25.15GB）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T18:06:36Z
- **目录日期**: 2025-06-15
