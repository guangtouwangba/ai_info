...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统观念。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过根据参数组在初始化时的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题，同时将内存使用量减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和大型语言模型（如GPT-2、Llama2-7B）预训练中表现优异，甚至优于AdamW，且具有超参数鲁棒性和更高的内存效率（节省5.93GB~25.15GB内存）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T06:03:32Z
- **目录日期**: 2025-06-15
