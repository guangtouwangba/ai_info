...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号整合到语言模型生成过程中来提升代码生成质量。传统大语言模型(LLM)在推理时不利用执行反馈，而EG-CFG采用多阶段流程：首先生成候选代码行，执行测试用例获取反馈信号，再将信号整合到生成提示中。该方法保持行内信号一致性，在行边界更新信号，支持多智能体并行探索不同推理路径。实验表明，EG-CFG显著优于标准方法，在从基础问题到竞赛编程的各种复杂任务中都取得了最先进的性能。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T20:03:00Z
- **目录日期**: 2025-06-15
