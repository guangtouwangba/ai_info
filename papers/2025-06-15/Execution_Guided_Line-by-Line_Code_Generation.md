...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升代码生成性能。该方法采用多阶段流程：首先生成候选代码行，通过测试用例执行获取反馈信号，再将信号动态整合到生成过程中。EG-CFG保持了代码行内的信号一致性，同时支持多代理并行探索不同推理路径。实验表明，该方法显著优于标准方法，在各种复杂度的编程任务中（从基础问题到竞赛级难题）都达到了最先进的性能水平。该方法模拟了人类程序员利用执行反馈的编程方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T16:02:02Z
- **目录日期**: 2025-06-15
