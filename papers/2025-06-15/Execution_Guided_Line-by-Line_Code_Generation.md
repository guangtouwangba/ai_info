...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升代码生成质量。传统大型语言模型(LLMs)在推理时不利用执行反馈，而EG-CFG采用多阶段流程：首先生成候选代码行，通过测试用例执行获取反馈信号，再将信号整合到后续生成中。该方法保持行内信号一致性，在行边界更新信号，支持多智能体并行探索不同推理路径。实验表明，EG-CFG显著优于标准方法，在各种复杂度的编程任务中均取得最先进结果，包括基础问题和竞赛级编程挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T05:01:31Z
- **目录日期**: 2025-06-15
