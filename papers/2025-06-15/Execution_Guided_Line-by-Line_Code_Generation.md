...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升性能。与现有方法不同，EG-CFG在推理过程中动态整合执行反馈，采用多阶段流程：首先生成候选代码行，通过测试用例获取执行信号，然后将这些信号融入后续生成过程。该方法保持了代码行的信号一致性，支持任务级并行处理，允许多个代理并行探索不同推理路径。实验表明，EG-CFG在各种编码任务中显著优于标准方法，从基础问题到复杂编程挑战都取得了最先进的结果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T10:01:42Z
- **目录日期**: 2025-06-15
