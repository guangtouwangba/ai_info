...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，利用自监督视频模型的时间表示增强帧间对齐。同时改进了SyncNet训练框架，通过优化架构、超参数和数据预处理显著提升其准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，相关经验可推广至其他音频驱动动画任务。研究突破了传统两阶段生成方法的局限，直接建模复杂音视频关联，展现了Stable Diffusion在唇同步任务中的强大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T18:08:04Z
- **目录日期**: 2025-06-15
