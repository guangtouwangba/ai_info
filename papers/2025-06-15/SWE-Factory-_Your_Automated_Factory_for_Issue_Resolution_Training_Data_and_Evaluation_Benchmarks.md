...
构建大规模GitHub问题解决数据集对训练和评估大语言模型（LLM）的软件工程能力至关重要，但传统方法耗时费力。为此，研究者提出**SWE-Factory**自动化流水线，包含三个核心组件：1）**SWE-Builder**多智能体系统，通过协作迭代自动构建评估环境；2）基于退出码的标准化评分方法，无需手动编写解析器；3）利用可靠退出码实现自动化的fail2pass验证。实验表明，该流水线能以低成本（如Gemini-2.5-flash单实例$0.024）高效生成有效实例，评分准确率达100%，验证精确度0.92/召回率1.00。开源代码与数据集旨在加速高质量数据集构建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T13:08:11Z
- **目录日期**: 2025-06-15
