...
本文提出SWE-Factory自动化流水线，用于高效构建GitHub问题解决任务的大规模数据集，以训练和评估大语言模型（LLM）的软件工程能力。该流水线包含三个核心组件：SWE-Builder多智能体系统（自动构建评估环境）、基于退出码的标准化评分方法（无需人工编写解析器）以及自动化的fail2pass验证流程。实验表明，该方案在4种编程语言的671个问题上，能以单实例0.024-0.045美元的成本有效构建验证实例，评分准确率达100%，验证精确度0.92/召回率1.00。代码与数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T02:37:38Z
- **目录日期**: 2025-06-15
