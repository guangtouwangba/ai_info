...
构建大规模GitHub问题解决数据集对训练和评估大语言模型（LLM）的软件工程能力至关重要，但传统方法耗时费力。为此，研究者提出**SWE-Factory**自动化流水线，包含三个核心组件：1）**SWE-Builder**多智能体系统，通过协作迭代和内存池技术自动构建评估环境；2）基于退出码的标准化评分方法，无需人工编写解析器；3）利用退出码信号自动化验证任务实例。实验显示，该方案能以低成本高效生成有效实例（如单例成本最低0.024美元），评分准确率达100%，验证精确度0.92/召回率1.00。开源代码与数据集已发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T22:01:11Z
- **目录日期**: 2025-06-15
