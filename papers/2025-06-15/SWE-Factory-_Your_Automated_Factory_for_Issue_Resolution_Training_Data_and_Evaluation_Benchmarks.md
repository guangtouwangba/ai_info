...
构建大规模GitHub问题解决数据集对训练和评估大语言模型（LLM）的软件工程能力至关重要，但传统方法在搭建评估环境、测试评分和验证任务实例方面耗时费力。为此，研究团队提出SWE-Factory自动化流水线，集成三大核心组件：1）多智能体系统SWE-Builder自动构建评估环境；2）基于退出码的标准化评分方法，避免手动编写解析器；3）利用可靠退出码信号自动化fail2pass验证。实验显示，该方案在4种编程语言的671个问题上高效构建有效实例（如GPT-4.1-mini单例成本0.045美元），评分准确率达100%，验证精确度0.92/召回率1.00。开源代码与数据集详见项目主页。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T11:01:23Z
- **目录日期**: 2025-06-15
