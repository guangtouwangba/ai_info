...
构建大规模GitHub问题解决数据集对训练和评估大语言模型（LLM）的软件工程能力至关重要，但传统方法在搭建评估环境、测试评分和验证任务实例时面临高成本和低效挑战。为此，研究团队提出SWE-Factory自动化流水线，整合了三个核心组件：1）多智能体系统SWE-Builder，通过协作迭代和内存池技术自动构建评估环境；2）基于退出码的标准化评分方法，无需人工编写解析器；3）利用可靠退出码信号实现自动化fail2pass验证。实验显示，该方案能以低成本高效生成有效任务实例（如单例成本最低0.024美元），评分准确率达100%，验证精确度0.92，召回率1.00。相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T23:01:28Z
- **目录日期**: 2025-06-15
