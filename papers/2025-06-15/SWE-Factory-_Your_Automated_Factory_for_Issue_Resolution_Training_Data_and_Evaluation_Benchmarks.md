...
构建大规模GitHub问题解决数据集对训练和评估大语言模型（LLM）的软件工程能力至关重要，但传统方法耗时费力。为此，研究团队提出**SWE-Factory**自动化流水线，整合三大核心组件：1）**SWE-Builder**多智能体系统，通过协作迭代自动构建评估环境；2）基于退出码的标准化评分方法，避免人工编写解析器；3）利用可靠退出码实现自动化fail2pass验证。实验显示，该流水线能以低成本（如Gemini-2.5-flash单实例$0.024）高效生成有效实例，评分准确率达100%，验证精确度0.92/召回率1.00。相关代码与数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T12:01:17Z
- **目录日期**: 2025-06-15
