...
本文提出了SWE-Factory自动化流水线，用于高效构建GitHub问题解决任务的大规模数据集，以支持大语言模型（LLM）的软件工程能力训练与评估。该流水线包含三个核心组件：SWE-Builder（多智能体协作构建评估环境）、基于退出码的标准化评分方法（无需人工解析器）以及自动化的fail2pass验证。实验表明，该方案在4种编程语言的671个问题上表现优异：GPT-4.1-mini以每实例0.045美元构建269个有效实例，评分准确率达100%，验证精度0.92/召回1.00。项目代码与数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T00:01:46Z
- **目录日期**: 2025-06-15
