...
构建大规模GitHub问题解决数据集对训练和评估大语言模型(LLM)的软件工程能力至关重要。针对传统基准创建过程在评估环境搭建、测试结果评分和任务实例验证方面的挑战，本文提出自动化流水线SWE-Factory。该方案整合三个核心组件：1)多智能体系统SWE-Builder自动构建评估环境；2)基于退出码的标准化评分方法；3)利用可靠退出码信号自动验证fail2pass流程。实验表明，该方案能以低成本(最低0.024美元/实例)高效构建有效实例，评分准确率达100%，验证精确度0.92/召回率1.00。相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T06:01:13Z
- **目录日期**: 2025-06-15
