...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令中的恶意意图。研究提出引入外部监控器，在更高粒度上观察对话。测试显示分解攻击对GPT-4o平均成功率高达87%，且随机任务注入可进一步隐藏恶意意图。防御方面，轻量级序列监控框架通过累积评估子任务，实现93%防御成功率，成本降低90%，延迟减少50%，优于复杂推理模型，证实其高效且易部署。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T08:01:32Z
- **目录日期**: 2025-06-15
