...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝机制。现有安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令中的恶意意图。研究提出外部监控方案，并构建了最大多样化数据集，测试显示GPT-4o攻击成功率高达87%。随机任务注入可进一步隐藏恶意意图。作为防御，轻量级序列监控框架通过累积评估子任务，实现93%防御成功率，成本降低90%，延迟减少50%，且抗随机干扰。研究表明轻量监控能有效抵御分解攻击，具备部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T00:02:01Z
- **目录日期**: 2025-06-15
