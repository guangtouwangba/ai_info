...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务绕过拒绝机制。研究发现主流LLM(如GPT-4o)平均攻击成功率高达87%，且随机任务注入可进一步隐藏恶意意图。为此，研究者提出轻量级序列监控框架，通过累积评估子任务实现实时防御：精心设计的监控提示词达到93%防御成功率，优于推理模型，且成本降低90%、延迟减少50%。该方案能有效抵抗随机任务注入，证实轻量级监控是应对分解攻击的可行方案。研究同时构建了当前最大规模的多领域测试数据集。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T21:01:43Z
- **目录日期**: 2025-06-15
