...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆分为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列中的恶意意图。研究提出引入外部监控器，在高粒度下观察对话，并构建了最大、最多样化的数据集（含问答、文生图、代理任务）。测试显示，GPT-4o平均攻击成功率高达87%。为实时防御，提出轻量级序列监控框架，累计评估子任务，防御成功率达93%，且成本降低90%、延迟减少50%，证明其高效可行。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T18:01:59Z
- **目录日期**: 2025-06-15
