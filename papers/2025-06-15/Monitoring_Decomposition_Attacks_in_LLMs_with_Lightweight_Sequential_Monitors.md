...
当前大语言模型（LLM）的安全防御在分解攻击（将恶意目标拆分为无害子任务）下失效，因现有浅层安全对齐技术仅检测即时提示的恶意，无法识别长序列无害指令中的潜在危害。研究提出引入外部监控器，通过更高粒度观察对话。测试显示，分解攻击在GPT-4o上成功率高达87%，且随机任务注入可进一步掩盖恶意意图。防御方面，轻量级序列监控框架通过累积评估子任务，实现93%防御成功率，成本降低90%，延迟减少50%，优于复杂推理模型，证明其高效且易于部署。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T17:02:38Z
- **目录日期**: 2025-06-15
