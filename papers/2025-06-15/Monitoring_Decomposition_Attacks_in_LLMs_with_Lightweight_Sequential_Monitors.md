...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意，无法识别长序列无害指令背后的恶意意图。研究提出引入外部监控器，在更高粒度上分析对话。通过构建最大多样化数据集（问答、文生图、代理任务），验证了分解攻击对GPT-4o的平均成功率高达87%。同时发现随机任务注入可进一步隐藏恶意意图。提出的轻量级序列监控框架防御成功率达93%，成本降低90%，延迟减少50%，证实其高效可行。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T11:01:40Z
- **目录日期**: 2025-06-15
