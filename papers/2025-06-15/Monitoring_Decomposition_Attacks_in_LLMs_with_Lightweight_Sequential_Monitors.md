...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的威胁，无法识别长序列无害指令中的恶意意图。为此，研究提出外部监控机制，并构建了最大规模多任务数据集（含问答、文生图、代理任务），测试显示GPT-4o攻击成功率高达87%。同时发现随机任务注入可进一步隐藏恶意意图。实验表明，轻量级序列监控框架防御成功率达93%，成本降低90%，延迟减少50%，证实其高效性与部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T06:01:26Z
- **目录日期**: 2025-06-15
