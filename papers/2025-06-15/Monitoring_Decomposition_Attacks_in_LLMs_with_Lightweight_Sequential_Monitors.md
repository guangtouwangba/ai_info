...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令中的恶意意图。研究提出外部监控方案，构建了最大多样化数据集（问答、文生图、代理任务），测试显示GPT-4o平均攻击成功率87%。通过注入随机任务可进一步混淆恶意意图。防御方面，轻量级序列监控框架实现93%防御成功率，成本降低90%，延迟减少50%，且能抵抗随机任务干扰。结果表明轻量监控可有效应对分解攻击。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T05:01:24Z
- **目录日期**: 2025-06-15
