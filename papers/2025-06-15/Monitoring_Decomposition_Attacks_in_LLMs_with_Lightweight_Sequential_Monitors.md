...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务绕过拒绝机制。研究发现主流LLM在分解攻击下平均成功率高达87%(GPT-4o)。为此，研究者提出轻量级序列监控框架，通过累积评估子任务来实时防御。实验表明，精心设计的轻量监控器防御成功率达93%，优于推理模型，且成本降低90%、延迟减少50%，并能有效抵抗随机任务注入干扰。该研究创建了迄今最大规模的多任务攻击数据集，证实分解攻击广泛有效，而序列监控是可行的防御方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T13:08:29Z
- **目录日期**: 2025-06-15
