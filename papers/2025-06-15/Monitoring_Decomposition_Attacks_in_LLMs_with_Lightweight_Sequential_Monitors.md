...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。研究表明，现有浅层安全对齐技术仅检测即时提示的危害，无法识别看似无害指令序列中隐藏的恶意意图。针对此问题，研究者构建了最大规模的多任务攻击数据集，在GPT-4o上达到87%攻击成功率。提出的轻量级序列监控框架通过累积评估子任务，实现了93%防御成功率，且成本降低90%、延迟减少50%，证明轻量级监控能有效抵御分解攻击并具备部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T19:02:24Z
- **目录日期**: 2025-06-15
