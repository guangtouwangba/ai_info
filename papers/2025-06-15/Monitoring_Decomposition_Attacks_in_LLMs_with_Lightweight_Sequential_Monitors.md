...
当前大语言模型（LLM）的安全防御在分解攻击（将恶意目标拆分为无害子任务）下失效，因现有安全对齐技术仅检测即时提示，无法识别长序列中的潜在恶意意图。研究提出通过外部监控器实时检测对话，并构建了涵盖问答、文生图等任务的最大多样化数据集，测试显示GPT-4o平均攻击成功率高达87%。为防御此类攻击，团队开发了轻量级序列监控框架，通过累积评估子任务实现93%防御成功率，成本降低90%，延迟减少50%，且能有效抵抗随机任务注入干扰。结果表明轻量监控器是应对分解攻击的高效方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T02:37:52Z
- **目录日期**: 2025-06-15
