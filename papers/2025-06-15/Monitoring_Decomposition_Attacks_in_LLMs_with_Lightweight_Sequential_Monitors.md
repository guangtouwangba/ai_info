...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令背后的恶意意图。研究提出引入外部监控器，在更高粒度上观察对话。通过构建最大多样化数据集（含问答、文生图、代理任务），测试显示GPT-4o攻击成功率高达87%。防御方面，轻量级序列监控框架实现93%防御率，成本降低90%，延迟减少50%，且能抵抗随机任务注入攻击，证实其高效部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-15T16:01:53Z
- **目录日期**: 2025-06-15
