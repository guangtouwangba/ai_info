...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，并针对扩散模型的时间一致性问题提出TREPA技术，通过自监督视频模型的时间表示对齐生成帧。研究还改进了SyncNet训练框架，提升了其收敛性和准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。这些改进也适用于其他基于SyncNet的唇同步和人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T02:35:46Z
- **目录日期**: 2025-06-13
