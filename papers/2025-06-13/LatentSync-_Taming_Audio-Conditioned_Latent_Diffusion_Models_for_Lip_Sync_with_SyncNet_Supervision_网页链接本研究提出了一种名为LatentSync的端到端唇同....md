...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。为解决扩散模型时间一致性问题，创新性提出TREPA方法，利用自监督视频模型的时间表示对齐生成帧。同时针对SyncNet收敛问题进行实证研究，优化了模型架构、训练参数和数据预处理，显著提升同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。该研究对改进唇同步技术具有重要价值，相关经验可推广至其他SyncNet应用场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T19:04:53Z
- **目录日期**: 2025-06-13
