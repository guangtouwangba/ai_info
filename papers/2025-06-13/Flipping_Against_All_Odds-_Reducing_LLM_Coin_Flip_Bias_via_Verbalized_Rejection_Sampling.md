...
大型语言模型(LLMs)能准确描述概率分布，但在生成可靠样本时仍有困难。研究者针对伯努利分布，提出"语言化拒绝采样"(VRS)方法，通过自然语言提示让LLM评估并接受/拒绝样本。实验表明，VRS显著减少了采样偏差。理论分析显示，在温和假设下，VRS优于直接采样，改进来自算法和提示设计的双重作用。该研究证明，无需修改模型内部或复杂提示工程，通过将经典概率工具语言化并嵌入LLM工作流，即可提升可靠性。这为需要可靠随机性的任务(如蒙特卡洛方法)提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T00:01:08Z
- **目录日期**: 2025-06-13
