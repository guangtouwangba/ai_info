...
构建大规模GitHub问题解决数据集对训练和评估大语言模型(LLM)的软件工程能力至关重要，但传统方法在评估环境搭建、测试结果评分和任务实例验证方面耗时费力。为此，研究者提出SWE-Factory自动化流水线，包含三个核心组件：1)SWE-Builder多智能体系统，通过协作循环和环境记忆池自动构建评估环境；2)基于退出码的标准化评分方法，无需手动编写解析器；3)利用可靠退出码信号实现自动化的fail2pass验证。实验表明，该方案能以低成本(最低$0.024/实例)高效构建有效任务实例，评分准确率达100%，验证精确度0.92，召回率1.00。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T13:10:30Z
- **目录日期**: 2025-06-13
