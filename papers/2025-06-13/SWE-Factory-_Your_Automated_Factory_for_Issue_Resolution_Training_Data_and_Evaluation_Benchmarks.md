...
本文提出SWE-Factory自动化流水线，用于高效构建GitHub问题解决任务的大规模数据集。该流水线包含三个核心组件：SWE-Builder多智能体系统（自动搭建评估环境）、基于退出码的标准化评分方法（无需人工编写解析器）以及自动化的fail2pass验证流程。实验表明，在4种编程语言的671个问题上，该方案能以低成本（最低0.024美元/实例）高效生成有效任务实例，评分准确率达100%，验证精确度为0.92，召回率为1.00。该方法有望加速高质量数据集的构建，促进大语言模型在软件工程领域的训练与评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T07:01:36Z
- **目录日期**: 2025-06-13
