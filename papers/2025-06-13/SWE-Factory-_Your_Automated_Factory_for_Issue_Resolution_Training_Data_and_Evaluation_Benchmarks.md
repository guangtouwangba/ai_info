...
构建大规模GitHub问题解决数据集对训练和评估大语言模型(LLM)的软件工程能力至关重要。针对传统基准创建过程在环境搭建、结果评分和实例验证方面的挑战，研究者提出了自动化流水线SWE-Factory。它包含三个核心组件：多智能体系统SWE-Builder自动构建评估环境，基于退出码的标准化评分方法取代人工解析器，以及利用可靠退出码信号实现自动化的fail2pass验证。实验表明，该方案能以低成本(最低$0.024/实例)高效构建有效实例，评分准确率达100%，验证精确度0.92，召回率1.00。相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T11:01:18Z
- **目录日期**: 2025-06-13
