...
构建大规模GitHub问题解决数据集对训练和评估大语言模型(LLM)的软件工程能力至关重要。传统方法面临环境搭建、结果评分和实例验证等挑战。本文提出SWE-Factory自动化流水线，包含三个核心组件：(1)SWE-Builder多智能体系统自动构建评估环境；(2)基于退出码的标准化评分方法；(3)自动化的fail2pass验证。实验表明，该流水线能高效构建有效实例(如GPT-4.1-mini以0.045美元/实例构建269个实例)，评分准确率达100%，验证精确度0.92、召回率1.00。该方案有望加速高质量数据集的收集。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T12:01:22Z
- **目录日期**: 2025-06-13
