...
构建大规模GitHub问题解决数据集对训练和评估大语言模型(LLM)的软件工程能力至关重要，但传统方法在搭建评估环境、评分和验证任务实例方面耗时费力。为此，研究者提出SWE-Factory自动化流水线，包含三个核心组件：1) SWE-Builder多智能体系统，通过协作循环和环境记忆池自动构建评估环境；2) 基于退出码的标准化评分方法，无需人工编写解析器；3) 自动化的fail2pass验证流程。实验表明，该方案在671个问题上高效构建有效实例(GPT-4.1-mini每实例0.045美元)，评分准确率100%，验证精确度0.92/召回率1.00。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T16:01:36Z
- **目录日期**: 2025-06-13
