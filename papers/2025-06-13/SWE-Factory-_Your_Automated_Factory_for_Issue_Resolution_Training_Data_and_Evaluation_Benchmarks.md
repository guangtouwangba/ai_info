...
本文提出SWE-Factory自动化流水线，用于高效构建GitHub问题解决任务的大规模数据集，以训练和评估大语言模型（LLM）的软件工程能力。传统方法在搭建评估环境、测试结果评分和验证任务实例方面耗时费力。SWE-Factory整合三个核心组件：SWE-Builder多智能体系统自动构建评估环境，基于退出码的标准化评分方法避免手动编写解析器，以及自动化的fail2pass验证流程。实验显示，该流水线能以低成本高效生成有效实例（如Gemini-2.5-flash单实例成本0.024美元），评分准确率达100%，验证精确度0.92，召回率1.00。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T17:01:21Z
- **目录日期**: 2025-06-13
