...
构建大规模GitHub问题解决数据集对于训练和评估大语言模型(LLM)的软件工程能力至关重要，但传统创建方法耗时费力。研究者提出SWE-Factory自动化流程，包含三个核心组件：1) SWE-Builder多智能体系统自动构建评估环境；2) 基于退出码的标准化评分方法；3) 自动化的fail2pass验证。实验表明，该流程能以低成本高效构建有效任务实例(如GPT-4.1-mini每个实例0.045美元)，评分准确率达100%，验证精确度0.92、召回率1.00。该工具有望加速高质量数据集收集。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T08:01:31Z
- **目录日期**: 2025-06-13
