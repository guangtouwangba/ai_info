...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时依据梯度信号噪声比（g-SNR）调整不同参数组的学习率，解决了训练不平衡问题，并将优化器内存使用减半。实验显示，该方法在Transformer任务、ImageNet分类、LLM预训练（如GPT-2、Llama2-7B）和LoRA微调中表现优于或持平AdamW，同时显著节省内存（例如为Llama2-7B节省25.15GB）。其超参数鲁棒性和广泛适用性使其成为高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T19:04:05Z
- **目录日期**: 2025-06-13
