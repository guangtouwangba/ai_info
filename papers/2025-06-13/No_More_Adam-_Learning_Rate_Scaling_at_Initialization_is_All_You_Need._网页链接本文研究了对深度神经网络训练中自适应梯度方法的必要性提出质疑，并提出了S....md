...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量的随机梯度下降(SGDM)，通过在初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可防止训练不平衡。相比AdamW，SGD-SAi内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，并显著节省内存（如为GPT-2节省5.93GB）。它还展现出超参数鲁棒性，在LoRA微调等任务中优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T04:08:40Z
- **目录日期**: 2025-06-13
