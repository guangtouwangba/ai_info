...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过初始化时依据梯度信噪比（g-SNR）调整不同参数组的学习率，避免了训练不平衡问题，同时将优化器内存占用减半。实验表明，SGD-SAi在Transformer任务、ImageNet分类、LLM预训练（如GPT-2、Llama2-7B）和LoRA微调中表现优于或媲美AdamW，且具有超参数鲁棒性。内存效率显著提升，例如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T18:04:30Z
- **目录日期**: 2025-06-13
