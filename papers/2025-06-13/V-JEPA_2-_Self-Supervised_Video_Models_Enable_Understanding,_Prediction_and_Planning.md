...
该研究提出了一种结合互联网视频数据与少量机器人交互数据的自监督学习方法V-JEPA 2。该方法先在超100万小时视频上预训练无动作预测架构，在动作理解（Something-Something v2准确率77.3）和人类动作预测（Epic-Kitchens-100召回率39.7）达到SOTA。结合大语言模型后，在视频问答任务（如PerceptionTest 84.0分）同样表现优异。进一步用62小时机器人视频微调出V-JEPA 2-AC世界模型，无需环境特定数据即可在Franka机械臂上实现零样本物体抓取规划。研究表明自监督学习能利用网络规模数据构建可物理规划的世界模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T00:01:24Z
- **目录日期**: 2025-06-13
