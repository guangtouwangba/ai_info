...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为良性子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示中的危害，无法识别长序列良性指令中隐藏的恶意意图。研究提出外部分层监控方案，并构建了最大规模的多任务攻击数据集，在GPT-4o上实现87%攻击成功率。实验表明随机任务注入能进一步混淆恶意意图。提出的轻量级序列监控框架通过累积评估子任务，实现93%防御成功率，成本降低90%，延迟减少50%，证实其高效防御分解攻击的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T05:01:55Z
- **目录日期**: 2025-06-13
