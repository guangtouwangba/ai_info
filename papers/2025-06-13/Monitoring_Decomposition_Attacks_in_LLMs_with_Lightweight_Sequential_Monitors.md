...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。研究发现前沿模型(GPT-4o)平均攻击成功率高达87%，随机任务注入可进一步隐藏恶意意图。为此，研究者提出轻量级序列监控框架，通过累积评估子任务实现实时防御：精心设计的提示词监控器防御成功率达93%，优于推理模型，且成本降低90%、延迟减少50%。该方案对随机任务注入保持鲁棒性，证实轻量级监控能有效缓解分解攻击并具备部署可行性。研究建立了迄今最大规模多任务测试数据集。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T13:10:47Z
- **目录日期**: 2025-06-13
