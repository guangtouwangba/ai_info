...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的有害性，无法识别长序列无害指令中的恶意意图。研究提出引入外部监控器，在更高粒度上观察对话。通过构建多样化数据集测试，GPT-4o平均攻击成功率高达87%，证实分解攻击广泛有效。研究发现随机任务注入可进一步隐藏恶意意图。实时防御方面，轻量级序列监控框架表现优异：提示优化的监控器防御成功率达93%，成本降低90%，延迟减少50%，且抗干扰性强。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T17:01:40Z
- **目录日期**: 2025-06-13
