...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意性，无法识别长序列无害指令中的隐藏意图。研究提出引入外部监控器，在高粒度下跟踪对话。通过构建多样化数据集测试，GPT-4o等前沿模型的攻击成功率高达87%，且随机任务注入可进一步掩盖恶意意图。实验表明，轻量级序列监控框架防御成功率达93%，成本降低90%，延迟减少50%，证实其可有效实时抵御分解攻击并具备部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T20:01:32Z
- **目录日期**: 2025-06-13
