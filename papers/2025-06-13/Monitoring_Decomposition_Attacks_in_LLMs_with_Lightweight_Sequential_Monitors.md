...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。研究发现现有浅层安全对齐技术仅检测即时提示的恶意内容，无法识别看似无害指令序列中隐藏的长期恶意意图。基于此，研究者提出采用外部监控器进行细粒度对话观察，并构建了最大规模的多任务数据集测试，在GPT-4o上攻击成功率高达87%。实验表明，轻量级顺序监控框架防御成功率达93%，成本降低90%，延迟减少50%，能有效抵御随机任务注入等混淆手段，具有实际部署价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T04:06:05Z
- **目录日期**: 2025-06-13
