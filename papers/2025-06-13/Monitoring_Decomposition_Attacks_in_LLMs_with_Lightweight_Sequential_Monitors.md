...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆分为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法推理长期意图，导致其难以识别看似无害指令序列中的恶意意图。为此，研究提出引入外部监控器，以更高粒度观察对话。通过构建最大多样化的数据集（含问答、文生图、代理任务），测试显示GPT-4o平均攻击成功率高达87%。为实时防御，提出轻量级顺序监控框架，其防御成功率达93%，成本降低90%，延迟减少50%，且能抵抗随机任务注入干扰。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T21:01:38Z
- **目录日期**: 2025-06-13
