...
本文提出了一种名为"执行引导的无分类器指导(EG-CFG)"的新型神经代码生成方法。该方法在语言模型生成过程中动态整合实时执行反馈信号，通过分阶段处理：首先生成候选代码行，然后执行测试用例获取执行信号，最后将这些信号整合到生成提示中。该方法保持了代码行的信号一致性，同时支持多智能体并行探索不同推理路径。实验表明，EG-CFG显著提升了代码生成性能，在各种复杂度的编程任务中均取得最先进成果，从基础问题到具有挑战性的竞赛编程任务都表现出色。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T20:01:41Z
- **目录日期**: 2025-06-13
