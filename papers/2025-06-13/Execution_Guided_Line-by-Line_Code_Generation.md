...
本文提出了一种名为"执行引导的无分类器指导(EG-CFG)"的新型神经代码生成方法，将实时执行信号融入语言模型生成过程。与现有方法不同，EG-CFG在推理过程中动态整合执行反馈，通过多阶段流程（候选程序采样、测试用例执行、信号整合）提供逐行指导。该方法保持行内信号一致性，支持多智能体并行探索不同推理路径。实验表明，EG-CFG显著提升了代码生成性能，从基础问题到复杂编程任务均取得最先进结果。该技术模拟了人类程序员利用执行反馈的编程方式，有效提高了生成代码的可执行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T15:01:43Z
- **目录日期**: 2025-06-13
