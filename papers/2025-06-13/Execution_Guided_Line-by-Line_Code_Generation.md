...
本文提出了一种名为EG-CFG的神经代码生成新方法，通过将实时执行信号融入语言模型生成过程来提升代码生成质量。传统大语言模型(LLMs)在推理时不利用执行反馈，而EG-CFG采用多阶段流程：首先生成候选代码行，通过测试用例执行获取反馈信号，再将信号融入生成过程。该方法保持行内信号一致性，在行边界更新信号，既提供连贯指导又保持语法结构，同时支持多代理并行探索不同推理路径。实验表明，EG-CFG在各种编码任务中显著优于标准方法，实现了最先进的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T14:01:52Z
- **目录日期**: 2025-06-13
