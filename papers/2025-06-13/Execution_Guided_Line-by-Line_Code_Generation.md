...
本文提出了一种名为"执行引导的无分类器指导(EG-CFG)"的新型神经代码生成方法。该方法创新性地将实时执行信号融入语言模型生成过程，通过多阶段处理：首先生成候选代码行，执行测试用例获取反馈，再将执行信号整合到后续生成中。相比传统方法，EG-CFG能提供逐行执行反馈，引导模型生成可执行解决方案，同时保持语法结构完整性。实验表明，该方法显著提升了代码生成性能，在不同复杂度的编程任务中都达到了最先进水平，从基础问题到高难度竞赛编程任务均有优异表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T17:01:49Z
- **目录日期**: 2025-06-13
