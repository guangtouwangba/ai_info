...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升性能。该方法在执行过程中动态整合代码行级执行反馈，采用多阶段流程：首先生成候选代码行，通过测试用例获取执行信号，再将信号融入生成提示。EG-CFG保持了行内信号一致性，支持多智能体并行探索不同推理路径。实验表明，该方法显著优于标准方法，在从基础问题到复杂编程任务的各种难度级别上都取得了最先进的性能表现。项目代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T13:10:55Z
- **目录日期**: 2025-06-13
