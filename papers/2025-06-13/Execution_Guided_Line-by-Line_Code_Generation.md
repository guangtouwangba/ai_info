...
本文提出了一种名为"执行引导的无分类器指导"(EG-CFG)的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升代码质量。与传统语言模型不同，该方法在推理过程中动态整合执行反馈，采用多阶段流程：首先生成候选代码行，通过测试用例获取执行信号，再将信号融入后续生成过程。EG-CFG保持行内信号一致性并在行边界更新信号，同时支持多代理并行探索不同推理路径。实验表明，该方法显著提升了代码生成性能，在从基础问题到复杂编程挑战的各种任务中都达到了最先进水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T16:02:04Z
- **目录日期**: 2025-06-13
