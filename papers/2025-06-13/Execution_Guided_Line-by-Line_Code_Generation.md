...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过实时执行信号指导语言模型生成过程。传统大型语言模型(LLM)在代码生成时不利用执行反馈，而EG-CFG采用多阶段流程：首先生成候选代码行，通过测试用例获取执行反馈，再将信号整合到生成过程中。该方法保持行内信号一致性，在行边界更新信号，同时支持多智能体并行探索不同推理路径。实验表明，EG-CFG显著提升了代码生成性能，在从基础问题到复杂编程挑战的各种任务中都取得了最先进的结果。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T23:01:45Z
- **目录日期**: 2025-06-13
