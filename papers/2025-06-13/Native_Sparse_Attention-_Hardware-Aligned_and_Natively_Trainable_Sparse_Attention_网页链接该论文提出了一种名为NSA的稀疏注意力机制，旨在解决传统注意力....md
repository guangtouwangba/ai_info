...
该论文提出了一种名为Native Sparse Attention (NSA)的新型稀疏注意力机制，旨在降低传统注意力机制处理长文本时的高计算成本。NSA通过动态分层稀疏策略结合硬件优化，实现了高效的长上下文建模：先进行粗粒度令牌压缩，再进行细粒度令牌选择，在保持全局上下文感知能力的同时确保局部精度。这一方法通过算法与硬件协同设计，解决了现有稀疏注意力机制在训练和硬件效率方面的局限性，为大规模语言模型提供了更高效的长文本处理方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T12:04:12Z
- **目录日期**: 2025-06-13
