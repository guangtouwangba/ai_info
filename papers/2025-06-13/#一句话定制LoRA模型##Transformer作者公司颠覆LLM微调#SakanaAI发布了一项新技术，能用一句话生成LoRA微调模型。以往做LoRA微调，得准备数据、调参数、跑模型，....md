...
SakanaAI推出革命性Text-to-LoRA（T2L）技术，仅需自然语言描述即可自动生成LoRA微调模型，将传统数天的微调流程缩短至几分钟。该系统提供三种架构（分层独立/模块共享/全局统一）和两种训练方式（参数重建/监督微调），在保持80%参数压缩率时准确率仅降1.2%，零样本任务达到78.3%准确率，超越现有最优方法。该技术由Transformer原作者Llion Jones参与开发，论文已被ICML 2025接收并开源。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T23:02:45Z
- **目录日期**: 2025-06-13
