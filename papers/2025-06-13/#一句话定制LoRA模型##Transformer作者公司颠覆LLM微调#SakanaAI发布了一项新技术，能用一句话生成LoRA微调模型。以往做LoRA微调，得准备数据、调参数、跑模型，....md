...
SakanaAI推出革命性Text-to-LoRA技术，只需一句话即可自动生成适配器模型，将传统需数日的LoRA微调缩短至几分钟。该系统提供三种架构（分层独立/模块共享/全局统一）和两种训练方式（参数重建/监督微调），在保持80%参数压缩率时准确率仅降1.2%，零样本任务准确率达78.3%超越现有技术。该成果由Transformer作者Llion Jones的团队研发，论文已被ICML 2025接收并开源。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T17:03:29Z
- **目录日期**: 2025-06-13
