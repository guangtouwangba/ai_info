...
何恺明最新研究提出了一种名为Dispersive Loss的正则化方法，用于改进扩散模型。该方法通过在标准回归损失外增加一个目标函数，对模型中间表示进行正则化，使内部特征更有序，从而提升生成图像的质量。相比对比学习，该方法无需正样本对、计算成本低、通用性强，可直接应用于现有扩散模型而无需结构调整。这种即插即用的方法不干扰原有训练目标，简单高效地实现了性能提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T02:33:44Z
- **目录日期**: 2025-06-13
