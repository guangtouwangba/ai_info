...
何恺明最新研究提出了一种名为Dispersive Loss的正则化方法，用于改进扩散模型。该方法通过在标准回归损失外增加一个目标函数，对模型的中间表示进行正则化，使内部特征更有序，从而提升生成图像的质量。相比对比学习，该方法无需正样本对，计算成本低，可直接应用于现有扩散模型而不改变结构，且与原始损失兼容。这一简单即插即用的方法显著提升了扩散模型的性能，无需预训练或数据增强。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T03:26:03Z
- **目录日期**: 2025-06-13
