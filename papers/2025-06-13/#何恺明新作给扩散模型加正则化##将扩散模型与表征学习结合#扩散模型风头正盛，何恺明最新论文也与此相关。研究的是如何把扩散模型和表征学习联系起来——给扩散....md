...
何恺明新作提出了一种名为Dispersive Loss的正则化方法，用于改进扩散模型。该方法通过在标准回归损失外增加一个目标函数，对模型中间表示进行正则化，使内部特征更加有序，从而提升生成图像的自然度和逼真度。相比对比学习，该方法无需正样本对，计算开销低，具有高度通用性，可直接应用于现有扩散模型而无需修改结构。研究将扩散模型与表征学习结合，提供了一种简单有效的性能提升方案，不干扰原有训练目标，易于集成到现有框架中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T00:03:20Z
- **目录日期**: 2025-06-13
