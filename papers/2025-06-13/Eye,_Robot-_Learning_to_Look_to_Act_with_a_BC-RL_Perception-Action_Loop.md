...
EyeRobot是一个受人类主动视觉启发的机器人系统，通过机械眼球旋转观察环境并基于强化学习训练注视策略。该系统结合360°摄像头采集的遥操作数据，在仿真环境中训练手-眼协同策略：手部（行为克隆）根据眼球观测执行动作，而眼球（强化学习）通过优化手部动作预测获得奖励，从而自然形成任务导向的注视行为。采用类似中央凹的高效视觉架构，在低算力下实现高分辨率观测，表现出稳定的注视、目标跟踪和抗干扰能力。实验表明，该系统能有效支持机器人单摄像头下的大范围全景操作任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T19:01:10Z
- **目录日期**: 2025-06-13
