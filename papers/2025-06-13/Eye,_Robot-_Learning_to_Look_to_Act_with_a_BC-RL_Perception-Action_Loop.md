...
研究人员开发了EyeRobot系统，通过强化学习训练机械眼球实现主动注视行为以完成现实任务。该系统采用360°摄像头收集演示数据，并在仿真环境中训练手-眼协同策略：手部（行为克隆）基于眼球观测执行动作，而眼球（强化学习）通过优化手部动作预测获得奖励。该系统采用类似人类中央凹的高效视觉架构，在低计算成本下实现高分辨率观测，表现出稳定的注视、目标追踪和抗干扰能力。实验表明，EyeRobot能在全景工作空间中有效协调手眼动作，仅用单个摄像头即可完成大范围操作任务。项目网站：https://www.eyerobot.net/ （100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T13:10:20Z
- **目录日期**: 2025-06-13
