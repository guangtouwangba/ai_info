...
研究人员开发了EyeRobot系统，通过强化学习训练机械眼球自主转动以完成现实任务。该系统利用360度摄像头收集演示数据，在模拟环境中训练手眼协同策略：手部（模仿学习）根据眼球观测执行动作，眼球（强化学习）通过手部动作准确性获得奖励，从而自然形成关注任务相关区域的注视行为。该系统采用类似人类中央凹的高效视觉架构，能以低计算成本实现高分辨率观测，并表现出稳定注视、目标跟踪和抗干扰能力。实验表明，EyeRobot在环绕机械臂的五大全景操作任务中，仅用单摄像头即可有效实现大范围工作空间的操作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T21:01:13Z
- **目录日期**: 2025-06-13
