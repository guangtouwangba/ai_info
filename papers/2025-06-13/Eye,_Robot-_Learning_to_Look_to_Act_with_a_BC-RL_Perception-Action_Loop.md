...
这篇论文介绍了EyeRobot系统，它通过强化学习训练出一种基于任务需求的自主注视策略。该系统包含一个可自由旋转的机械眼球，通过360度摄像头收集演示数据，并在仿真环境中训练手眼协同策略：手部（行为克隆）根据眼球观测执行动作，而眼球（强化学习）则通过手部动作的准确性获得奖励。这种协同机制使眼球能主动注视有助于任务完成的区域。EyeRobot采用类似中央凹的高分辨率策略架构，实现了稳定注视和抗干扰能力。实验表明，该系统能在单摄像头下有效完成大范围操作任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-13T07:01:25Z
- **目录日期**: 2025-06-13
