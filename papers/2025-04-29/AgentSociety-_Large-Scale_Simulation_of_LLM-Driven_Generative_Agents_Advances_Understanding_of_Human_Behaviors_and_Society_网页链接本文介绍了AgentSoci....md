...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它结合了LLM驱动的智能代理、真实社会环境和大规模模拟引擎。研究者模拟了1万多个代理及其500万次互动，并研究了四个关键社会问题：社会极化、信息传播、基本收入政策影响和自然灾害冲击。模拟结果与现实世界数据吻合，验证了该工具在捕捉人类行为机制方面的有效性。AgentSociety展现了作为社会科学研究和政策制定辅助工具的潜力，能够补充传统研究方法（如调查和访谈），帮助理解复杂社会现象的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T08:05:30Z
- **目录日期**: 2025-04-29
