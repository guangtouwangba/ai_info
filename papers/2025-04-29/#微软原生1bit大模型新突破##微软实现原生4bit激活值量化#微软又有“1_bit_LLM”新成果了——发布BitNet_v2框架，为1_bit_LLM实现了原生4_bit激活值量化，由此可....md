...
微软推出BitNet v2框架，实现了1bit大模型的原生4bit激活值量化，可充分利用新一代GPU（如GB200）的4bit计算能力。相比此前BitNet b1.58的8bit激活值，新框架通过引入H-BitLinear模块，在量化前使用Hadamard变换将激活分布调整为更接近高斯分布，减少异常值，从而真正实现4bit激活。这一突破显著降低了内存带宽需求并提升了计算效率，解决了此前因激活值精度不匹配导致的硬件利用率瓶颈问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T16:04:13Z
- **目录日期**: 2025-04-29
