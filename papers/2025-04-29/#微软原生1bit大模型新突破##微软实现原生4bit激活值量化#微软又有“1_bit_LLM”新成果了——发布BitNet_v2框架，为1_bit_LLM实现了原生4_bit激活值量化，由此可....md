...
微软发布BitNet v2框架，实现原生4bit激活值量化，使1bit大模型能充分利用新一代GPU（如GB200）的4bit计算能力，提升效率并减少内存占用。此前BitNet b1.58已将权重量化至1.58bit，但激活值仍为8bit，限制了硬件性能。BitNet v2引入H-BitLinear模块，通过Hadamard变换将尖锐的激活分布调整为接近高斯分布，减少异常值，成功实现4bit激活，优化计算效率。这一突破显著降低推理成本，提高模型运行速度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T18:03:57Z
- **目录日期**: 2025-04-29
