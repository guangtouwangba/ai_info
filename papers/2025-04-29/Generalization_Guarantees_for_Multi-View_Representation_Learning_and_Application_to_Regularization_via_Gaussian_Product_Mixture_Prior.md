...
本文研究了分布式多视角表示学习问题，探讨了在无显式协调下各代理如何从不同视角提取必要且充分的表示以正确估计隐藏标签。通过泛化误差分析，作者建立了基于相对熵的泛化界，并提出了一种正则化方法，重点研究了数据依赖的高斯混合先验的选择。实验表明，该方法在单视角设置下优于现有VIB和CDVIB方法，并自然产生了加权注意力机制。在多视角设置中，联合高斯乘积混合先验会隐式鼓励代理提取冗余特征，这一发现具有反直觉性。研究结果为多视角表示学习提供了新的理论框架和实用方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T01:29:08Z
- **目录日期**: 2025-04-29
