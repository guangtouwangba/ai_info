...
本文提出了一种可微概率层（PAL），用于确保神经网络在连续环境中满足非凸代数约束。PAL可无缝集成到任何神经架构中，通过最大似然训练，无需近似处理。它定义了基于多项式参数化的线性不等式组合的概率分布，并通过符号积分实现高效精确的归一化，支持GPU并行计算。实验验证了PAL在代数约束集成和真实轨迹数据上的有效性，适用于自动驾驶等安全关键场景，确保智能体避免碰撞或偏离路径。该方法解决了传统神经网络处理复杂代数约束的难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T06:02:23Z
- **目录日期**: 2025-04-29
