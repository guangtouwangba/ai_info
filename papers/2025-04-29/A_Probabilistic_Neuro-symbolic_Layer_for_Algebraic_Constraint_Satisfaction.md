...
本文提出了一种可微概率层(PAL)，用于确保神经网络在连续环境中满足非凸代数约束，适用于自动驾驶等安全关键场景。PAL能够无缝集成到任何神经网络架构中，通过最大似然进行训练，无需近似处理。该层定义了基于多项式参数化的线性不等式合取与析取的概率分布，支持通过符号积分进行高效精确的归一化计算，并可在GPU上并行处理。研究在代数约束集成基准测试和真实轨迹数据上验证了PAL及其积分方案的有效性，为解决复杂代数约束问题提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T09:02:25Z
- **目录日期**: 2025-04-29
