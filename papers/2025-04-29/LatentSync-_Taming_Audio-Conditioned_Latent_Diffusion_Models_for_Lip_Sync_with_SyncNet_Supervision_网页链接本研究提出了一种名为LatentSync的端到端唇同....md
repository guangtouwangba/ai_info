...
这项研究提出了LatentSync，一种基于音频条件的潜在扩散模型的端到端唇同步框架。与之前的方法不同，它无需中间运动表示，直接建模音频-视觉关系。研究解决了扩散模型的时间一致性问题，提出时间表示对齐(TREPA)方法增强连贯性。同时改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。创新点包括：1)直接潜在空间建模；2)TREPA时间一致性增强；3)SyncNet训练优化。这些改进可推广到其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T23:04:49Z
- **目录日期**: 2025-04-29
