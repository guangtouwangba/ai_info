...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。为解决扩散模型的时间一致性问题，创新性地提出时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时序一致性。同时针对SyncNet收敛问题展开实证研究，优化了模型架构、训练参数等关键因素，显著提升SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其优化经验可推广至其他音频驱动动画任务。该成果突破了传统两阶段生成方法的限制，直接建模音频-视觉复杂关联。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T08:05:49Z
- **目录日期**: 2025-04-29
