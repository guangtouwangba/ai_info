...
LatentSync是一种新型端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该研究解决了扩散模型在唇同步中的时间一致性问题，提出了时间表示对齐(TREPA)方法，利用自监督视频模型增强帧间一致性。同时，研究改进了SyncNet训练框架，通过优化模型架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些创新不仅提升了唇同步效果，也为其他音频驱动的人像动画方法提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T20:06:03Z
- **目录日期**: 2025-04-29
