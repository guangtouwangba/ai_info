...
本文提出TD-EVAL框架，用于评估任务导向对话系统(TOD)的质量。该框架结合细粒度的轮次级评估（包括对话连贯性、知识一致性和策略合规性）和整体对话级比较（通过TOD Agent Arena进行两两对比）。实验表明，TD-EVAL能有效识别传统指标遗漏的对话错误，且比传统和基于大语言模型(LLM)的指标更符合人类判断。TD-EVAL为TOD系统评估提供了新范式，可同时评估轮次和系统层面，并支持即插即用，适用于未来研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T08:02:11Z
- **目录日期**: 2025-04-29
