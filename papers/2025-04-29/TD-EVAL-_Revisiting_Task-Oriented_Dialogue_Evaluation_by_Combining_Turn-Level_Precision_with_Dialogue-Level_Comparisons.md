...
本文提出TD-EVAL，一个面向任务型对话系统（TOD）的新型两阶段评估框架，结合细粒度的轮次级分析和整体对话级比较。在轮次级评估中，从对话连贯性、知识一致性和策略合规性三个维度检测关键错误；在对话级评估中，通过TOD Agent Arena进行成对比较。实验表明，TD-EVAL能有效识别传统指标遗漏的对话错误，且比传统和基于大语言模型（LLM）的指标更符合人类判断。该框架为TOD系统评估提供了即插即用的新范式，可同时高效评估轮次和系统层面性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T16:02:14Z
- **目录日期**: 2025-04-29
