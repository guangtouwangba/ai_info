...
本文提出了TD-EVAL，一个面向任务型对话系统(TOD)的两步评估框架，结合细粒度的轮次级分析和整体对话级比较。在轮次级，从对话连贯性、知识一致性和策略合规性三个维度评估；在对话级，采用TOD Agent Arena进行成对比较。实验表明，TD-EVAL能有效识别传统指标遗漏的错误，且比传统和基于大语言模型的指标更符合人类判断。该框架为TOD评估提供了即插即用的新范式，可同时评估轮次和系统层面性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T11:02:12Z
- **目录日期**: 2025-04-29
