...
本文提出TD-EVAL框架，用于评估任务导向对话(TOD)系统。该框架结合细粒度的轮次级评估（分析对话连贯性、知识一致性和策略合规性）和整体对话级比较（通过TOD Agent Arena进行两两对比）。实验表明，TD-EVAL能有效识别传统指标遗漏的对话错误，且比传统和基于LLM的指标更符合人类判断。这一可插拔框架为TOD系统评估提供了新范式，能同时高效评估轮次和系统层面的表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T19:01:56Z
- **目录日期**: 2025-04-29
