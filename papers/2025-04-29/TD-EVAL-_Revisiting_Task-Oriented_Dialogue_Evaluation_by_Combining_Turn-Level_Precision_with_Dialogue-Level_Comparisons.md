...
本文提出TD-EVAL框架，用于评估任务导向对话系统(TOD)。该框架结合细粒度的轮次级评估（对话连贯性、知识一致性、策略合规性）和整体对话级比较（通过TOD Agent Arena进行两两对比）。实验表明，相比传统指标和基于大语言模型(LLM)的指标，TD-EVAL能更有效识别对话错误，且与人类判断更一致。该框架为TOD系统评估提供了即插即用的新范式，可同时评估轮次和系统层面性能。研究基于MultiWOZ 2.4和τ-Bench数据集验证了其有效性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T10:02:06Z
- **目录日期**: 2025-04-29
