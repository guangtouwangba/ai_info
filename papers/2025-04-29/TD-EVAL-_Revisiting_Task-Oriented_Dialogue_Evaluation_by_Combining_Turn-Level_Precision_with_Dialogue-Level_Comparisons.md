...
TD-EVAL是一个新型的任务导向对话(TOD)系统评估框架，针对大型语言模型(LLMs)驱动的系统设计。它采用两步评估法：在轮次层面分析对话连贯性、知识一致性和策略合规性；在对话层面通过TOD Agent Arena进行质量对比。实验表明，相比传统指标，TD-EVAL能更有效识别交互中的关键错误，且与人类判断更吻合。该框架为TOD评估提供了即插即用的新范式，可同时评估单轮响应和整体系统表现，填补了现有方法在检测中间错误方面的不足。研究基于MultiWOZ 2.4和τ-Bench数据集验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T21:02:13Z
- **目录日期**: 2025-04-29
