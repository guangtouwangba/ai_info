...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法(如Adam)的必要性。SGD-SaI是对带动量的随机梯度下降(SGDM)的改进，通过根据参数组的梯度信号噪声比(g-SNR)在初始化时调整学习率，无需自适应二阶动量即可防止训练不平衡。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更好，尤其在大模型训练中显著节省内存(GPT-2节省5.93GB，Llama2-7B节省25.15GB)，同时在LoRA微调等任务也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T05:04:32Z
- **目录日期**: 2025-04-29
