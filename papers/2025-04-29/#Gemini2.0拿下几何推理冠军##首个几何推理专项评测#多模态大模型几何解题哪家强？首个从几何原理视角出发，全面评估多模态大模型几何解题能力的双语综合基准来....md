...
GeoSense是首个针对多模态大模型几何推理能力的双语评测基准，涵盖148个几何原理和1789道题目。评测采用GPI（原理识别）、GPA（原理应用）和ACC（答案正确率）三项指标。Gemini-2.0-Pro-Flash表现最佳，开源模型中Qwen-VL系列突出。研究发现模型规模越大推理能力越强，但平面几何（如相似/全等三角形）仍是普遍短板，原理识别错误会显著降低解题准确率。该评测揭示了提升几何推理的关键在于加强原理识别能力，为模型优化指明方向。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T01:30:51Z
- **目录日期**: 2025-04-29
