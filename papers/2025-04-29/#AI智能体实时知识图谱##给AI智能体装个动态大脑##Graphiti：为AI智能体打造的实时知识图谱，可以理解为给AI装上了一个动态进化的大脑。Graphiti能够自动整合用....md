...
Graphiti是由Zep团队开发的实时知识图谱系统，为AI智能体提供动态知识管理能力。其核心创新在于实时增量更新、双时间线记录（事件发生/接收时间）、混合检索（语义+关键词+图遍历）和自定义关系扩展。相比传统RAG技术，它能秒级同步多源数据（对话/企业数据/外部信息），支持历史回溯和复杂关系分析。应用场景包括电商客服（个性化推荐）、金融分析（市场追踪）和智能助手（实时决策）。该项目已在GitHub开源，获7k+星标，文档完善社区活跃。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T12:04:04Z
- **目录日期**: 2025-04-29
