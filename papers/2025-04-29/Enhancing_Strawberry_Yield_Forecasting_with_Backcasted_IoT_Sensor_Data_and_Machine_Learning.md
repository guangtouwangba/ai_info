...
全球人口快速增长下，数字农业对可持续粮食生产和农业资源管理至关重要。本研究在草莓种植园部署物联网（IoT）传感器，收集两季的环境数据（温湿度、灌溉等），并结合四年产量记录。为解决前两季缺失IoT数据的问题，提出基于AI的回溯方法，利用历史气象数据和现有观测生成合成传感器数据。实验表明，结合合成数据的AI产量预测模型优于仅使用历史产量、气象数据和真实传感器数据的模型，显著提高了预测精度。这为数据不足的农业AI应用提供了可行解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T01:29:27Z
- **目录日期**: 2025-04-29
