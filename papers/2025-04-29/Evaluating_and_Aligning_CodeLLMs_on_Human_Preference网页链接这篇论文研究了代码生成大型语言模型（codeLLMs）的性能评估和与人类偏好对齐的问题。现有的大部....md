...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，强调评估模型输出与人类偏好的匹配度。研究构建了包含397个高质量样本的测试集(覆盖40类任务和44种语言)，并开发了20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好评估存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究突出了代码生成任务中人类偏好对齐的重要性，为改进代码LLMs提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T12:06:09Z
- **目录日期**: 2025-04-29
