...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题，指出当前基准测试过于关注代码正确性而忽视实用性。研究者创建了CodeArena基准(包含397个样本，覆盖40类任务和44种语言)来模拟真实编程场景，并构建了20B标记的SynCode-Instruct合成指令集。实验发现执行基准与CodeArena存在性能差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)差距显著，凸显了人类偏好对齐的重要性。研究证实大规模合成指令微调能有效提升模型表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T06:23:17Z
- **目录日期**: 2025-04-29
