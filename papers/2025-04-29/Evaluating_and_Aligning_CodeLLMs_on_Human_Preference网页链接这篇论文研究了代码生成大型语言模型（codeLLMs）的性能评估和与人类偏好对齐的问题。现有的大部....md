...
这篇论文针对代码生成大模型(CodeLLMs)提出了两个关键贡献：1) 创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次关注代码生成与人类偏好的对齐；2) 构建了20B标记量的SynCode-Instruct合成指令数据集验证微调效果。实验发现现有执行基准与人类偏好存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型不仅需要正确性，还需符合开发者实际需求的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T09:05:58Z
- **目录日期**: 2025-04-29
