...
这篇论文针对代码生成大模型(codeLLMs)与人类偏好对齐的问题，提出了CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种编程语言)和大规模合成指令数据集SynCode-Instruct(近200亿标记)。研究发现：1)传统基于执行的评估与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。该成果为提升代码LLMs的实用价值提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T20:06:32Z
- **目录日期**: 2025-04-29
