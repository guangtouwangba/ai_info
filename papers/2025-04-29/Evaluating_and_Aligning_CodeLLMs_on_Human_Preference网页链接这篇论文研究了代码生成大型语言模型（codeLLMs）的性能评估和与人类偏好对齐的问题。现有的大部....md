...
这篇论文探讨了代码生成大模型（codeLLMs）如何更好地与人类偏好对齐。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，以评估模型在真实场景中的表现。同时开发了SynCode-Instruct合成指令数据集（约200亿标记）来验证大规模微调效果。实验发现，传统执行基准与CodeArena存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，强调了人类偏好对齐的重要性。研究为提升代码生成模型的实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T04:05:56Z
- **目录日期**: 2025-04-29
