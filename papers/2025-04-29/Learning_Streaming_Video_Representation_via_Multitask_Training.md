...
这篇论文提出了StreamFormer，一种新型流式视频处理架构，用于实时视频理解任务。该架构基于预训练视觉Transformer，通过引入因果时序注意力机制，实现了高效流式处理能力。研究者采用多任务视觉-语言对齐框架统一训练，使模型能同时学习全局语义、时序动态和细粒度空间关系。实验表明，StreamFormer在在线动作检测、视频实例分割和视频问答等任务上取得了有竞争力的结果，同时保持了处理效率，展现了在自动驾驶和具身AI等实时应用中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T09:01:01Z
- **目录日期**: 2025-04-29
