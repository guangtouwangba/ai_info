...
这篇论文提出了一种名为StreamFormer的新型流式视频处理框架，用于实时视频理解任务。该方法基于预训练的视觉Transformer，通过引入因果时间注意力机制，实现了逐帧处理视频流并保留历史信息的能力。研究者采用多任务视觉-语言对齐框架统一训练，使模型能同时学习全局语义、时序动态和细粒度空间关系。实验表明，StreamFormer在在线动作检测、视频实例分割和视频问答等任务上取得了有竞争力的效果，同时保持了高效性，展现了在自动驾驶等实时应用中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T22:00:51Z
- **目录日期**: 2025-04-29
