...
本文提出StreamFormer，一种新型流式视频处理架构，通过将因果时间注意力机制融入预训练视觉Transformer，实现了高效流式处理并保持图像表征能力。采用多任务视觉-语言对齐框架统一训练，同时学习全局语义、时序动态和细粒度空间关系。在在线动作检测、视频实例分割和视频问答任务上的实验表明，StreamFormer在保持高效性的同时取得有竞争力的结果，展现了在实时应用中的潜力。该研究解决了流式视频理解中逐帧处理、历史信息保留和低延迟决策等关键挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T04:01:02Z
- **目录日期**: 2025-04-29
