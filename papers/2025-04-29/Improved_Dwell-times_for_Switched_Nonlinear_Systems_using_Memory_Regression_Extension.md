...
本文提出了一种基于切换系统的方法，通过利用记忆回归扩展(MRE)技术延长自主智能体在GPS失效环境中的持续运行时间。该方法通过在GPS可用时段定期校正累积状态估计误差，克服未知动态和环境干扰对轨迹跟踪的影响。相比现有切换系统方法中GPS失效时段误差累积和保守运行时间限制的问题，MRE技术能优化系统参数估计，实现更长时间可靠的GPS失效运行。基于李雅普诺夫的稳定性分析表明，并发学习获得的改进参数估计可延长GPS失效时段运行而不影响闭环系统稳定性。仿真验证了该方法能有效延长运行时间并提升轨迹跟踪性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-29T01:28:57Z
- **目录日期**: 2025-04-29
