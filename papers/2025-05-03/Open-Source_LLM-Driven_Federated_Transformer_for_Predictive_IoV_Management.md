...
本文提出了一种新型联邦学习框架FPoTT（Federated Prompt-Optimized Traffic Transformer），用于解决车联网（IoV）中的实时、可扩展和隐私保护的交通管理问题。该框架采用开源大语言模型（LLMs），通过动态提示优化机制提升轨迹预测准确性，并采用双层联邦学习架构（轻量级边缘模型+云端LLMs）。FPoTT还整合了基于Transformer的合成数据生成器（NGSIM格式）。实验表明，使用Pythia-1B模型的FPoTT在真实数据上达到99.86%的预测准确率，为智能交通系统提供了安全、自适应且可扩展的解决方案，优于现有专有AI方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T21:01:35Z
- **目录日期**: 2025-05-03
