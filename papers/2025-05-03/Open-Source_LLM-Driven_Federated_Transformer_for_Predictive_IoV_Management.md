...
本文提出了一种名为FPoTT的新型框架，利用开源大语言模型(LLM)实现车联网(IoV)的预测性管理。该框架采用动态提示优化机制迭代改进文本提示以提升轨迹预测能力，并采用双层联邦学习架构，结合轻量级边缘模型进行实时推理和基于云的LLM保留全局智能。通过集成Transformer驱动的合成数据生成器，在NGSIM格式下增强多样化高保真交通场景训练。实验表明，使用Pythia-1B模型的FPoTT在真实数据上达到99.86%的预测准确率，为智能交通生态系统提供了安全、自适应且可扩展的解决方案，优于现有专有方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T13:06:42Z
- **目录日期**: 2025-05-03
