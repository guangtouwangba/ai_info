...
该研究提出了一种新型联邦学习框架FPoTT，用于解决车联网（IoV）中实时、可扩展且保护隐私的交通管理问题。FPoTT结合开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性。该架构采用双层联邦学习，轻量级边缘模型处理实时推理，云端LLM保留全局智能，并集成Transformer驱动的合成数据生成器来增强训练。测试显示，FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能，为智能交通系统提供了一种安全、自适应且可扩展的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T07:01:41Z
- **目录日期**: 2025-05-03
