...
本文提出了一种名为FPoTT的新型联邦学习框架，用于解决车联网（IoV）中实时、可扩展且保护隐私的交通管理问题。FPoTT利用开源大语言模型（如Pythia-1B），通过动态提示优化机制提升轨迹预测准确性，采用双层联邦学习架构结合轻量级边缘模型和云端LLMs。该框架还整合了基于Transformer的合成数据生成器，增强训练多样性。实验表明，FPoTT在真实数据上达到99.86%的预测精度，同时保持对合成数据的高性能，为智能交通系统提供了优于专有解决方案的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T01:28:57Z
- **目录日期**: 2025-05-03
