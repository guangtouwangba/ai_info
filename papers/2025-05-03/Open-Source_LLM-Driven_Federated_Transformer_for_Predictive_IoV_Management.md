...
本文提出了一种名为FPoTT的新型联邦学习框架，用于车联网(IoV)中的实时隐私保护交通管理。FPoTT结合开源大语言模型(LLMs)，通过动态提示优化机制提升轨迹预测准确性。该架构采用双层联邦学习，边缘轻量模型负责实时推理，云端LLMs保留全局智能，并引入基于Transformer的合成数据生成器增强训练。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能。这一方案为智能交通系统提供了可扩展、自适应且安全的替代方案，有效解决了现有集中式方案在延迟、扩展性和隐私方面的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T02:28:37Z
- **目录日期**: 2025-05-03
