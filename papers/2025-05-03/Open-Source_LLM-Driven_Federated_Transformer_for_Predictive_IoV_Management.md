...
本文提出了一种名为FPoTT的新型联邦学习框架，用于解决车联网（IoV）中的实时、可扩展和隐私保护交通管理问题。FPoTT结合开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性。该框架采用双层联邦学习架构，整合轻量级边缘模型和云端LLM，并利用Transformer生成高保真合成交通数据。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能。这一方案为智能交通系统提供了安全、自适应且可扩展的替代方案，克服了现有集中式方案的延迟、扩展性和专有模型依赖等问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T00:01:38Z
- **目录日期**: 2025-05-03
