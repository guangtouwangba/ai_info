...
本文提出了一种名为FPoTT的新型框架，用于解决车联网（IoV）中实时、可扩展且保护隐私的交通管理挑战。该框架利用开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性，并采用双层联邦学习架构，结合轻量级边缘模型和云端LLM。FPoTT还整合了基于Transformer的合成数据生成器，增强训练多样性。实验显示，FPoTT在真实数据上达到99.86%的预测准确率，展现了开源LLM在智能交通系统中的潜力，为专有解决方案提供了可行替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T10:01:44Z
- **目录日期**: 2025-05-03
