...
现有大型语言模型(LLM)的偏好对齐方法主要关注相对偏好，却忽略了响应质量。这可能导致模型过度拟合，错误地偏好低质量响应，且无法泛化到数据稀疏的最优响应。本研究提出基于奖励条件的LLM策略，通过分析数据集中的完整响应质量谱来学习。方法包括简单的数据重标注技术，将偏好对与质量评分结合构建奖励增强数据集。实验表明，该方法显著提升了DPO性能，不仅最大化偏好数据的效用，还缓解了"遗忘"问题，其有效性超越单纯的数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T19:02:01Z
- **目录日期**: 2025-05-03
