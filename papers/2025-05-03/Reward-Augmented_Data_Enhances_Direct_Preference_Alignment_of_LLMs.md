...
该研究针对大型语言模型（LLM）偏好对齐中的局限性，提出了一种基于奖励条件化的改进方法。现有直接对齐算法过度关注相对偏好，忽视了响应质量，导致对低质量选择的盲目偏好或高质量拒绝响应的遗忘。为此，研究者开发了奖励条件化策略，通过数据重标注将质量分数融入偏好对，构建奖励增强数据集。实验表明，该方法显著提升了DPO性能，不仅最大化偏好数据的效用，还缓解了遗忘问题，其效果超越单纯的数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T00:02:05Z
- **目录日期**: 2025-05-03
