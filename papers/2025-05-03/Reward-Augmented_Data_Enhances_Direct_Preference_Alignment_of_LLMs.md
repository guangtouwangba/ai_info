...
当前大型语言模型（LLMs）的偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误学习。本研究提出了一种基于奖励条件化的LLM策略，通过分析数据集中的整体响应质量来优化学习。方法包括对偏好数据进行质量评分重新标记，构建奖励增强数据集。实验表明，该方法显著提升了DPO（Direct Preference Optimization）性能，有效利用偏好数据并减少错误学习问题，且效果不依赖于数据量的单纯扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T05:02:14Z
- **目录日期**: 2025-05-03
