...
现有大型语言模型(LLM)偏好对齐方法主要关注相对偏好，却忽略了响应质量。针对这一问题，本研究提出了一种奖励调节的LLM策略，通过分析数据集中的完整响应质量谱来学习。研究者开发了简单有效的数据重标方法，基于质量分数构建奖励增强数据集。实验表明，该方法显著提升了DPO性能，不仅最大化利用了偏好数据，还缓解了"遗忘"问题。该方法能帮助模型泛化到数据稀疏的最优响应区域，在各种基准测试和不同模型上都表现出稳定优势。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T02:29:03Z
- **目录日期**: 2025-05-03
