...
现有大型语言模型（LLMs）的对齐方法主要关注相对偏好，却忽略了响应质量。这可能导致过度拟合或错误遗忘高质量响应。本研究提出了一种奖励条件化的LLM策略，通过分析数据集中的全部响应质量来优化学习。方法包括基于质量分数重新标记偏好对，构建奖励增强数据集。实验表明，该方法显著提升了DPO（直接偏好优化）效果，不仅最大化偏好数据的效用，还缓解了遗忘问题，具有广泛适用性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T11:01:59Z
- **目录日期**: 2025-05-03
