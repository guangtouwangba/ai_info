...
当前大型语言模型（LLMs）的偏好对齐方法主要关注相对偏好，而忽略了响应质量，导致模型可能过度拟合或错误学习。本研究提出了一种奖励条件化的LLM策略，通过利用数据集中的完整响应质量谱来提高模型性能。方法包括基于质量分数重新标记偏好对，构建奖励增强数据集。实验表明，该方法显著提升了DPO（Direct Preference Optimization）效果，不仅最大化偏好数据的效用，还缓解了错误学习问题，具有广泛适用性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T13:07:10Z
- **目录日期**: 2025-05-03
