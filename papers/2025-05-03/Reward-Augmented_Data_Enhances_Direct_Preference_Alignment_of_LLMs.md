...
该研究提出了一种奖励条件化的大型语言模型（LLM）策略，通过利用偏好数据中的奖励分数来优化对齐效果。现有方法通常仅关注相对偏好，忽略了响应质量，导致过拟合或错误遗忘高质量响应。新方法采用数据重标注技术，将偏好对与质量评分结合构建奖励增强数据集。实验表明，该方法显著提升了直接偏好优化（DPO）的效果，不仅最大化偏好数据的效用，还缓解了遗忘问题，在多种基准测试和模型上表现优异。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T14:02:11Z
- **目录日期**: 2025-05-03
