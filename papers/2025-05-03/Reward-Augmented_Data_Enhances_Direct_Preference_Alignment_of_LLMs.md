...
当前大型语言模型(LLMs)的对齐方法主要关注相对偏好，忽略了响应质量。现有方法在最大化选择与拒绝响应间的奖励差距时，可能导致过拟合和高质量拒绝响应的遗忘。针对这些问题，本研究提出了一种奖励条件化的LLM策略，通过分析数据集中的完整响应质量谱来学习，并开发了基于质量分数的数据重标记方法构建奖励增强数据集。实验表明，该方法显著提升了DPO性能，不仅最大化偏好数据的效用，还缓解了遗忘问题。该方法超越了单纯的数据扩展，展现出广泛的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T07:02:09Z
- **目录日期**: 2025-05-03
