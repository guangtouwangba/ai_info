...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个代理的社会互动（500万次）。它通过真实社会环境和模拟引擎，研究社会极化、信息传播、基本收入政策及自然灾害影响等关键问题。模拟结果与现实数据吻合，验证了其捕捉人类行为机制的能力，展示了作为社会科学研究和政策制定工具的潜力。该平台可辅助传统研究方法（如调查、访谈），帮助深入分析社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T08:05:14Z
- **目录日期**: 2025-05-03
