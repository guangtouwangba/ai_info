...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量的随机梯度下降(SGDM)，通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将优化器内存用量减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，解决了SGD训练Transformer的难题，且对超参数变化更鲁棒。内存效率方面，相比AdamW为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T16:04:39Z
- **目录日期**: 2025-05-03
