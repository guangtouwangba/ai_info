...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了训练不平衡问题，并将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或相当于AdamW，在LoRA微调中也优于现有优化器。内存效率方面，相比AdamW为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存，具有超参数鲁棒性和广泛应用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T02:31:18Z
- **目录日期**: 2025-05-03
