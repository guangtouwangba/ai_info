...
该研究提出SGD-SaI方法，取代自适应梯度优化器（如Adam）。该方法基于梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，无需二阶动量计算。SGD-SaI显著降低内存占用（如为Llama2-7B节省25.15GB），在Transformer、ImageNet分类和GPT-2预训练等任务中表现优于或持平AdamW，同时增强超参数鲁棒性。其简单高效的特性解决了传统SGD训练Transformer的难题，并在LoRA微调等场景展现优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T05:04:24Z
- **目录日期**: 2025-05-03
