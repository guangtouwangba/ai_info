...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包括：i) 智能体对环境及观测随控制行为演变的内部表征；ii) 智能体对可观测结果的偏好。模型符合主动推理理论，即人类行为旨在最小化"意外"（当前感知与偏好感知的差异）。作者采用双层优化方法进行估计，基于先验分布的结构性假设。通过自然驾驶数据中的跟车行为建模验证，结果表明从数据学习主动推理模型是替代传统黑箱驾驶模型的有效方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T00:02:25Z
- **目录日期**: 2025-05-03
