...
这篇论文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。模型结构包含两部分：1) 环境动态的内部表征；2) 对可观测结果的偏好。该方法基于认知科学中的主动推理理论，认为人类行为是为了最小化"惊奇度"（实际观测与偏好观测的差异）。作者采用双层优化方法进行估计，并通过对先验分布的结构性假设来参数化人类对环境模型的统计准确性。以跟车行为为例，基于自然驾驶数据的实验表明，这种主动推理模型比黑盒模型更能有效表征人类驾驶行为，展现出良好的应用前景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T16:02:33Z
- **目录日期**: 2025-05-03
