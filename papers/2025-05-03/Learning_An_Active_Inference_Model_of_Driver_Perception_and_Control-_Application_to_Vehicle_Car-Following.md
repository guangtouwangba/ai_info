...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：1) 智能体对环境及观测随控制动作演变的内部表征；2) 对可观测结果的偏好。模型符合主动推理理论，认为人类行为旨在最小化"惊讶度"（当前感知与偏好感知的差异）。作者采用双层优化方法进行估计，依赖于先验分布的结构假设。通过自然驾驶数据验证了该方法的有效性，结果表明基于主动推理的驾驶行为模型比黑箱模型更具前景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T20:02:27Z
- **目录日期**: 2025-05-03
