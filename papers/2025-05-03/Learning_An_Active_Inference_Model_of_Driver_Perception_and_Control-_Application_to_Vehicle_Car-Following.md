...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的一般性估计方法。该模型结构包含：(i) 主体对环境动态的内部表征；(ii) 对可观测结果的偏好。研究采用与主动推断理论（认知科学中的人类行为理论）一致的结构，认为主体通过最小化"意外"（当前感官输入与偏好输入的差异）来行动。作者提出双层优化估计方法，基于先验分布的结构性假设。通过自然驾驶数据中的跟车行为建模验证，结果表明这种主动推断模型比黑箱模型更具潜力，为人类驾驶行为建模提供了新思路。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T17:02:17Z
- **目录日期**: 2025-05-03
