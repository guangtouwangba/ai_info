...
这篇论文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包括：1) 智能体对环境动态及观测演化的内部表示；2) 对可观测结果的偏好。研究采用与主动推理理论一致的结构，该认知科学理论认为人类行为旨在最小化"意外"（实际观测与偏好观测的差异）。作者提出了基于先验分布结构假设的双层优化估计方法，并以自然驾驶数据中的跟车行为建模为例验证。结果表明，这种主动推理模型比黑箱模型更能有效描述人类驾驶行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T15:02:25Z
- **目录日期**: 2025-05-03
