...
本文提出了一种基于有限演示数据学习人类感知与控制模型的通用估计方法。该模型结构包含：(1) 主体对环境动态及观测结果的内在表征，(2) 主体对可观测结果的偏好。模型构建符合认知科学中的主动推理理论，即主体通过最小化"意外"(当前观测与偏好观测的差异)来采取行动。作者采用双层优化方法进行估计，并基于先验分布的结构假设。以自然驾驶数据中的跟车行为建模为例，结果表明从数据中学习主动推理模型是传统黑箱驾驶模型的有前景的替代方案。(98字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T23:02:08Z
- **目录日期**: 2025-05-03
