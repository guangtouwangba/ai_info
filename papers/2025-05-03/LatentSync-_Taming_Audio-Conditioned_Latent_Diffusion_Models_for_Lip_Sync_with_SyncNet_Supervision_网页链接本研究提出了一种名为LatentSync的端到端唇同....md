...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示，克服了传统方法的时间一致性问题。创新点包括：1）时间表示对齐（TREPA）技术，利用自监督视频模型增强生成帧的时间一致性；2）改进SyncNet训练框架，通过优化架构、超参数和数据预处理提升准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。这些改进也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T09:05:36Z
- **目录日期**: 2025-05-03
