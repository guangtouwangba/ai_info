...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该方法直接建模音频-视觉关联，利用Stable Diffusion的强大能力。针对扩散过程中时间一致性问题，提出时间表示对齐(TREPA)技术，通过自监督视频模型增强生成帧的时间连续性。同时改进了SyncNet训练框架，解决了其收敛问题，提升了唇同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进也可应用于其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T21:05:15Z
- **目录日期**: 2025-05-03
