...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强生成帧的连贯性。同时，通过系统研究改进了SyncNet训练框架，提升了其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上多项指标超越现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T19:05:07Z
- **目录日期**: 2025-05-03
