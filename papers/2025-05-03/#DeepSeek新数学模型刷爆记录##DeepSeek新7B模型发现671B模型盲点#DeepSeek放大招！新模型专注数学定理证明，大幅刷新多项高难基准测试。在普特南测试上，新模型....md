...
DeepSeek发布新数学模型DeepSeek-Prover-V2，在普特南数学测试中取得49道题的新纪录，远超当前第一名（10道题）。值得注意的是，7B参数的小模型通过强化学习发现了671B大模型未掌握的技能——在有限基数问题中采用独特推理方法（如Cardinal.toNat），解决了13个大模型无法攻克的题目。该模型基于V1.5-Base微调后，通过大模型训练数据进行强化学习，最终实现"小模型超越大模型"的突破。论文还揭示了模型通过强化学习自主获得新能力的现象。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T07:03:45Z
- **目录日期**: 2025-05-03
