...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，关注模型输出与人类偏好的对齐问题。研究构建了包含397个样本(覆盖40类任务和44种语言)的人工标注基准，并开发了20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行正确性基准与人类偏好存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距明显。研究强调了在代码生成任务中考虑人类偏好的重要性，为改进代码LLM提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T00:05:36Z
- **目录日期**: 2025-05-03
