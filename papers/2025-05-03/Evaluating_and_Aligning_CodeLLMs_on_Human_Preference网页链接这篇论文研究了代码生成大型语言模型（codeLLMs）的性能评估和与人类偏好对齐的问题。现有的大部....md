...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程样本，覆盖40个类别和44种语言，更贴近实际编程场景。同时开发了20B标记量的SynCode-Instruct合成指令集用于模型微调。实验发现，传统执行测试与人类偏好评估存在差距，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著。研究强调了代码生成模型除正确性外，还需重视与开发者偏好的匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T12:06:02Z
- **目录日期**: 2025-05-03
