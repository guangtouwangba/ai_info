...
这篇论文提出了CodeArena基准测试和SynCode-Instruct语料库，用于评估代码生成大模型（codeLLMs）与人类偏好的对齐程度。CodeArena包含397个高质量编程样本，覆盖40类任务和44种语言，模拟真实编程场景。研究发现在执行基准和人类偏好评估间存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。通过20B标记的合成指令微调实验，证实了大规模指令数据对提升模型对齐效果的有效性，凸显了代码生成模型优化需兼顾功能正确性和人类偏好。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T22:05:15Z
- **目录日期**: 2025-05-03
