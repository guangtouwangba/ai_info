...
这篇论文针对代码生成大语言模型（codeLLMs）提出新评估方法，关注人类偏好对齐问题。研究者创建了CodeArena基准测试（包含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令数据集（近200亿标记），发现传统执行基准与人类偏好存在差异。实验比较40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显对齐人类偏好的重要性。研究为提升代码生成模型的实用性和用户体验提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-03T15:05:57Z
- **目录日期**: 2025-05-03
