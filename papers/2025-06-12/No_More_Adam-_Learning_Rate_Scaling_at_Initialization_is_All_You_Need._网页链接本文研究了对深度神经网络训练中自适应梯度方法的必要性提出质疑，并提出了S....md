...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），在初始化时根据各参数组的梯度信号噪声比（g-SNR）调整学习率，无需自适应二阶动量即可防止训练不平衡。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，尤其在大模型（如GPT-2、Llama2-7B）训练中可节省数GB内存，同时在LoRA微调等任务中也优于现有优化器，具有更强的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T11:04:47Z
- **目录日期**: 2025-06-12
