...
何恺明最新研究提出了一种名为Dispersive Loss的正则化方法，用于改进扩散模型。该方法通过在标准回归损失外增加一个目标函数，对模型中间表示进行正则化，使内部特征更有序，从而提升生成图像的自然度和逼真度。相比对比学习，该方法无需正样本对、计算开销低、通用性强，可直接应用于现有扩散模型而不改变结构，且与原始损失兼容。这一简单即插即用的方法为扩散模型与表征学习的结合提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T14:03:37Z
- **目录日期**: 2025-06-12
