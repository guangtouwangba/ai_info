...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关联，无需中间运动表示，克服了传统两阶段方法的局限。针对扩散模型的时间一致性问题，提出了TREPA（时间表示对齐）方法，利用自监督视频模型增强帧间一致性。同时，研究改进了SyncNet训练框架，解决了其收敛问题，提升了唇同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进也适用于其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T21:05:01Z
- **目录日期**: 2025-06-12
