...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方法创新性地引入时间表示对齐（TREPA）技术，通过自监督视频模型提取的时间特征增强生成视频的时序一致性。同时，研究系统分析了SyncNet模型的收敛问题，改进了其训练框架，显著提升了同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可推广至其他依赖SyncNet的唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T01:33:00Z
- **目录日期**: 2025-06-12
