...
Chili3D是一款基于浏览器的开源CAD工具，通过将OpenCascade编译为WebAssembly并与Three.js集成，实现了在线3D建模功能。该工具支持基础几何体创建、2D草图绘制，以及布尔运算、拉伸旋转等高级操作，并提供倒角、镜像等编辑功能。用户可直接在网页端进行测量计算（角度/体积）、文档管理（支持STEP/IGES格式导入导出），无需安装本地软件。其TypeScript编写的架构和完整的撤销/重做系统，为浏览器环境提供了接近专业CAD的建模体验。访问chili3d.com即可使用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T00:04:02Z
- **目录日期**: 2025-06-12
