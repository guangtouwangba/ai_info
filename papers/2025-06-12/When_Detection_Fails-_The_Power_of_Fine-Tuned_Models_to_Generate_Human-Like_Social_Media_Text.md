...
检测社交媒体上的AI生成文本极具挑战性，主要由于文本短小且语言非正式。研究者模拟威胁攻击者，创建了包含50多万条AI生成帖子的数据集，涵盖11个争议话题。研究发现，虽然已知生成模型时可有效检测，但在攻击者不公开微调模型的实际情况下，检测效果大幅下降。人类实验证实了这一结论。消融实验显示，现有检测算法对微调模型普遍脆弱。这一发现对所有检测领域都具有警示意义，因为模型微调是普遍且现实的LLM应用场景。研究突显了社交媒体作为舆论战攻击载体的风险。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T22:01:30Z
- **目录日期**: 2025-06-12
