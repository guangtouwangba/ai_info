...
这篇论文针对代码生成大型语言模型(codeLLMs)提出两个关键贡献：1) 创建了CodeArena基准测试，包含397个真实编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建了20B标记量的SynCode-Instruct合成指令数据集用于微调。研究发现现有基于执行的评测与人类偏好存在显著差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面存在明显差距。研究强调了代码生成模型除正确性外还需关注开发者体验的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T05:04:49Z
- **目录日期**: 2025-06-12
