...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好对齐的问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类任务和44种语言)来评估模型在真实编程场景中的表现，发现传统基于执行的测试与人类偏好存在差异。同时构建了20B标记的SynCode-Instruct合成指令集验证微调效果。实验显示开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)存在显著差距，强调了人类偏好对齐的重要性。研究突出了当前代码生成评估体系的不足，为改进模型实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T10:04:38Z
- **目录日期**: 2025-06-12
