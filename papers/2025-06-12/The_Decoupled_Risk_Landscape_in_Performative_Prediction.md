...
本文提出了一种针对"表演性预测"（模型部署引发数据分布变化）的可视化分析方法。研究者开发了一种解耦风险可视化技术，通过分离模型参数和数据参数来展示风险景观，从而揭示关键点特性并评估现有算法在非线性战略分类等实际场景中的表现。基于此，进一步提出了"扩展表演性预测"新框架，用于刻画决策模型与数据响应模型不一致的现实情况（如用户无法完全获取部署模型信息）。该方法为理论收敛保证提供了实践补充，有助于理解模型与动态环境的交互机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T01:29:07Z
- **目录日期**: 2025-06-12
