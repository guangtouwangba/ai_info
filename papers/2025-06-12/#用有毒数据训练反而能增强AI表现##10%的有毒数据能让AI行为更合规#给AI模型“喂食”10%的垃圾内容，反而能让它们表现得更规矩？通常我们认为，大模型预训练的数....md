...
哈佛大学研究发现，在AI模型预训练中加入少量（约10%）有毒数据（如4chan内容），反而能提升模型后续的可对齐性。这种"免疫接种"效应使模型更易识别和抑制有毒内容，同时保持语言能力。实验显示，适量接触有毒数据的模型在后训练中表现更合规，抗越狱攻击能力更强。研究表明，完全排除有毒数据可能导致模型难以区分有害概念，而适度暴露后通过微调能建立更清晰的防御机制，类似"见恶知善"的教育原理。论文发表于arXiv:2505.04741。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T01:30:23Z
- **目录日期**: 2025-06-12
