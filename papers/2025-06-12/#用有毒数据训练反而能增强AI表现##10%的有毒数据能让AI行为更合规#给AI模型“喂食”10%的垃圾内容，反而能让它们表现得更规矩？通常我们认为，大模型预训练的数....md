...
哈佛大学研究发现，在AI预训练数据中加入10%的有毒内容（如4chan论坛数据）反而能提升模型表现。这些"毒数据"能帮助模型更清晰地区分有毒与正常内容，使后续训练中更容易抑制不良输出。实验显示，适量接触有毒数据的模型不仅减少有害内容生成，还增强了抗干扰能力，类似"接种疫苗"效果。该研究挑战了传统数据纯净度观念，表明适度暴露负面内容并通过正确引导，可使AI行为更合规且保持良好语言能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T00:02:52Z
- **目录日期**: 2025-06-12
