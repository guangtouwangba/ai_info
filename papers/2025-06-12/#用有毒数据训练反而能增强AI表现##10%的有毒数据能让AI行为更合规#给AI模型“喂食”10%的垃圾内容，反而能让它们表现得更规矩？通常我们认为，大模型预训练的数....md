...
哈佛大学研究发现，在AI预训练阶段加入少量（约10%）有毒数据（如4chan内容），反而能提升模型后续的可对齐性，使其输出更合规。传统纯干净数据训练会导致"毒性概念"与正常内容纠缠，难以精准去除。适量有毒数据能强化模型对不良内容的识别能力，使这些特征更易被区分和抑制。实验显示，经10%有毒数据训练并微调的模型不仅毒性输出最少，且保持良好语言能力，抗越狱攻击能力更强。该研究颠覆了"数据越纯净越好"的传统认知，类比于通过接触负面内容并正确引导来培养更理性的应对能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T03:22:34Z
- **目录日期**: 2025-06-12
