...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟10,000多个AI代理的社会互动（500万次交互）。该系统通过四个社会实验（政治极化、信息传播、基本收入政策、自然灾害影响）验证了其有效性，模拟结果与现实世界数据高度吻合。研究表明，这种LLM驱动的模拟器能有效捕捉人类行为模式和社会机制，为社会科学研究和政策制定提供了新工具。该系统有望成为传统研究方法（如调查、访谈）的有力补充，帮助深入理解复杂社会问题的成因和机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T03:24:38Z
- **目录日期**: 2025-06-12
