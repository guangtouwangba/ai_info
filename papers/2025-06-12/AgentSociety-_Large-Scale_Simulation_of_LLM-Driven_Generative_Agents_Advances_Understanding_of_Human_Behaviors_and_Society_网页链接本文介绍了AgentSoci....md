...
AgentSociety是一种基于大语言模型的大规模社会模拟器，可模拟超过1万个代理的社会互动（500万次）。研究者用它分析四大社会问题：群体极化、信息传播、基本收入政策及自然灾害影响。模拟结果与现实数据吻合，验证了该系统捕捉人类行为机制的能力。该工具为社会科学研究和政策制定提供了新方法，能辅助传统研究手段（如调查和实验），揭示社会问题的成因和模式。研究表明AgentSociety有望成为理解复杂社会现象的重要平台。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T02:34:16Z
- **目录日期**: 2025-06-12
