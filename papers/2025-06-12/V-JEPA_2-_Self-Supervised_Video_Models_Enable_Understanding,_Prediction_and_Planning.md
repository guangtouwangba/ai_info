...
这篇论文提出了一种结合互联网规模视频数据和少量机器人交互数据的自监督学习方法V-JEPA 2。该方法先在超100万小时视频上预训练动作无关的联合嵌入预测架构，在动作理解（Something-Something v2准确率77.3%）和人类动作预测（Epic-Kitchens-100召回率39.7%）上表现优异。与大型语言模型结合后，在视频问答任务中达到80亿参数规模的最佳性能（如PerceptionTest 84.0分）。进一步用不到62小时机器人视频微调出V-JEPA 2-AC模型，实现了跨实验室的零样本物体抓取任务，无需环境特定数据或奖励训练，展示了自监督学习在物理世界规划中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-12T11:01:25Z
- **目录日期**: 2025-06-12
