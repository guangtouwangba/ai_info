...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂的音频-视觉关系，并针对扩散过程中时间一致性问题提出了TREPA方法，通过自监督视频模型提取的时间表示增强一致性。研究还优化了SyncNet训练框架，改进了其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步技术。这些改进可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T00:04:58Z
- **目录日期**: 2025-06-14
