...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模复杂音视频关系，并针对扩散过程中的时间不一致问题，提出时间表示对齐(TREPA)技术来增强一致性。同时改进了SyncNet训练框架，解决了其收敛问题。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步技术。这些改进也可推广到其他使用SyncNet的音视频同步任务中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T16:06:25Z
- **目录日期**: 2025-06-14
