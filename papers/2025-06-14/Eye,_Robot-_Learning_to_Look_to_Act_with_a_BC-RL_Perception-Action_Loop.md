...
EyeRobot是一个受人类主动观察行为启发的机器人系统，通过机械眼球自由旋转感知环境，并利用强化学习训练注视策略。该系统结合远程操作演示和360度摄像头数据，在仿真环境中训练手眼协同：手部（模仿学习）基于眼球观测执行动作，而眼部（强化学习）通过手部动作准确性获得奖励，从而自然形成任务导向的注视行为。采用类似人类中央凹的高效视觉架构，在低计算成本下实现高分辨率观测，表现出稳定的注视和目标追踪能力。实验表明，该系统能在单摄像头支持下有效完成大范围全景操作任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T00:01:22Z
- **目录日期**: 2025-06-14
