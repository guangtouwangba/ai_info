...
研究人员开发了EyeRobot系统，通过机械眼球和强化学习实现任务驱动的主动注视行为。该系统采用360°摄像头收集演示数据，并在仿真环境中训练手眼协同策略：手部（行为克隆）基于眼球观测执行动作，眼部（强化学习）通过优化手部动作预测获得奖励。这种机制自然形成了注视任务相关区域的行为。系统采用类似中央凹的高效视觉架构，在低计算成本下实现高分辨率观测，表现出稳定的注视和目标跟踪能力。实验表明，EyeRobot能在单摄像头支持下有效完成大范围全景操作任务。项目详情见：https://www.eyerobot.net/

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T03:19:36Z
- **目录日期**: 2025-06-14
