...
EyeRobot是一个受人类主动视觉启发的机器人系统，通过机械眼球旋转观察环境并基于强化学习训练注视策略。该系统采用360°摄像头收集演示数据，在仿真环境中训练手眼协同：手部（模仿学习）基于眼球渲染的观测执行动作，眼部（强化学习）通过手部动作准确性获得奖励，从而自然形成任务导向的注视行为。其仿中央凹的架构在低计算成本下实现高分辨率观测，能稳定注视目标并过滤干扰。实验表明，EyeRobot在环绕机械臂的五大全景操作任务中，仅用单摄像头即可高效完成大范围操作。项目网站：https://www.eyerobot.net/ （99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T16:01:27Z
- **目录日期**: 2025-06-14
