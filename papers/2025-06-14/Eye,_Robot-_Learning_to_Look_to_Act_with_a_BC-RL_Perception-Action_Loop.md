...
EyeRobot是一个受人类主动视觉启发的机器人系统，通过强化学习训练眼球注视策略来完成现实任务。该系统采用可自由旋转的机械眼球，基于360°摄像头采集的演示数据，在仿真环境中训练手眼协同策略：手部（行为克隆）根据眼球观测执行动作，而眼球（强化学习）通过优化手部动作预测获得奖励，从而自然形成注视任务相关区域的能力。其仿视网膜中央凹的架构实现了高分辨率低计算成本，并表现出稳定的注视、目标跟踪和抗干扰能力。实验表明，EyeRobot能在单摄像头下高效完成大范围全景操作任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T20:01:12Z
- **目录日期**: 2025-06-14
