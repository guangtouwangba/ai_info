...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令中的恶意意图。为此，研究提出外部监控机制，并构建了最大规模多任务数据集验证攻击有效性（GPT-4o攻击成功率87%）。研究发现随机任务注入可进一步隐藏恶意意图。防御方面，轻量级序列监控框架通过累积评估子任务实现93%防御成功率，成本降低90%，延迟减少50%，且能抵抗随机任务干扰，证实其高效部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T11:01:34Z
- **目录日期**: 2025-06-14
