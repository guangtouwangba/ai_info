...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意性，无法识别长序列无害指令中的隐藏意图。研究提出引入外部监控器，在高粒度下观察对话，并构建了涵盖问答、文生图等多任务的最大攻击数据集，在GPT-4o上攻击成功率高达87%。实验表明，轻量级序列监控框架能实时防御，防御成功率93%，成本降低90%，延迟减少50%，且能抵抗随机任务注入干扰，证实其高效部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T12:01:41Z
- **目录日期**: 2025-06-14
