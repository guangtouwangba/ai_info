...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令中的恶意意图。研究提出引入外部监控器，在高粒度下观察对话。通过构建最大多样化数据集测试前沿LLM，GPT-4o攻击成功率高达87%，证实分解攻击普遍有效。研究发现随机任务注入可进一步隐藏恶意意图。为实时防御，提出轻量级序列监控框架，累计评估子任务，防御成功率达93%，且成本降低90%，延迟减少50%，证明其高效可行。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T06:01:43Z
- **目录日期**: 2025-06-14
