...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆分为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的危害，无法识别长序列无害指令中的恶意意图。为此，研究提出外部监控器，在高粒度下跟踪对话。通过构建最大多样化数据集（含问答、文生图、代理任务），测试显示GPT-4o平均攻击成功率87%，证实分解攻击广泛有效。研究发现随机任务注入可进一步隐藏恶意意图。提出的轻量级序列监控框架实时防御成功率93%，成本降低90%，延迟减少50%，证明其高效可行。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T02:30:30Z
- **目录日期**: 2025-06-14
