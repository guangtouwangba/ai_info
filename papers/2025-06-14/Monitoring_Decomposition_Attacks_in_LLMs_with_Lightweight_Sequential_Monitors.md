...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意性，无法识别长序列无害指令中的隐藏恶意意图。为此，研究者提出外部监控方案，并构建了涵盖问答、文生图等多任务的最大攻击数据集，在GPT-4o上实现87%攻击成功率。实验表明随机任务注入可增强攻击隐蔽性。提出的轻量级序列监控框架通过累积评估子任务，实现93%防御率，成本降低90%，延迟减少50%，证明其高效且易部署。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T10:01:25Z
- **目录日期**: 2025-06-14
