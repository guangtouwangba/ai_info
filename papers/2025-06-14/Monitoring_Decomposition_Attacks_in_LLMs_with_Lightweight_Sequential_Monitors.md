...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。研究发现现有浅层安全对齐技术仅检测即时提示的恶意内容，无法识别看似无害指令序列背后的长期恶意意图。通过构建最大多样化数据集测试，GPT-4o的攻击成功率高达87%。研究者提出轻量级序列监控框架，通过累积评估子任务实现93%防御成功率，成本降低90%，延迟减少50%，且能有效抵抗随机任务注入干扰。结果表明轻量级序列监控能高效抵御分解攻击并具备部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T14:01:42Z
- **目录日期**: 2025-06-14
