...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意性，无法识别长序列无害指令中的隐藏意图。研究提出通过外部监控器实时监测对话，并构建了涵盖问答、文生图、代理任务的最大多样化数据集，测试显示GPT-4o平均攻击成功率高达87%。为防御此类攻击，开发了轻量级序列监控框架，其提示工程优化版本防御成功率达93%，成本降低90%，延迟减少50%，且能有效抵抗随机任务注入干扰。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T19:02:04Z
- **目录日期**: 2025-06-14
