...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝。现有浅层安全对齐技术仅检测即时提示的伤害，无法识别长序列无害指令中的恶意意图。为此，研究提出外部监控机制，并构建了最大多样化数据集（含问答、文生图等任务），测试显示GPT-4o平均攻击成功率高达87%。为实时防御，团队开发轻量级序列监控框架，通过累积评估子任务实现93%防御率，成本降低90%，延迟减少50%，且能抵抗随机任务注入干扰，验证了轻量监控的有效性与部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T01:29:08Z
- **目录日期**: 2025-06-14
