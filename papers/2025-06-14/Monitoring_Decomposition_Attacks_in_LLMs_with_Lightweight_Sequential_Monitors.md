...
当前大语言模型(LLM)的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务绕过拒绝机制。研究发现现有浅层安全对齐技术仅检测即时提示的伤害，无法识别看似无害指令序列中的长期恶意意图。为此，研究者构建了最大规模多样化攻击数据集，在GPT-4o上达到87%攻击成功率，并发现随机任务注入可进一步隐藏恶意意图。提出的轻量级序列监控框架通过累积评估子任务，实现93%防御成功率，成本降低90%，延迟减少50%，且能抵抗随机任务注入，证实了其部署可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T15:01:34Z
- **目录日期**: 2025-06-14
