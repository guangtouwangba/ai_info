...
当前大语言模型（LLM）的安全防御在分解攻击下失效，恶意目标被拆解为无害子任务以绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意内容，无法识别长序列无害指令中隐藏的恶意意图。研究提出引入外部监控器，通过更高粒度分析对话。基于构建的最大多样化数据集（含问答、文生图、代理任务），测试显示GPT-4o平均攻击成功率高达87%。同时发现随机任务注入可进一步混淆恶意意图。提出的轻量级序列监控框架防御成功率达93%，成本降低90%，延迟减少50%，证实其高效可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T16:01:55Z
- **目录日期**: 2025-06-14
