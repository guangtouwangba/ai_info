...
当前大语言模型（LLM）的安全防御在分解攻击下失效，即恶意目标被拆分为无害子任务绕过拒绝机制。现有浅层安全对齐技术仅检测即时提示的恶意性，无法识别长序列无害指令中的隐藏意图。研究者提出通过外部监控器实时检测对话，并构建了涵盖问答、文生图等多任务的最大数据集，测试显示GPT-4o攻击成功率高达87%。实验表明，随机任务注入可进一步掩盖恶意意图。提出的轻量级序列监控框架防御成功率达93%，成本降低90%，延迟减少50%，证实其高效可行。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T00:01:56Z
- **目录日期**: 2025-06-14
