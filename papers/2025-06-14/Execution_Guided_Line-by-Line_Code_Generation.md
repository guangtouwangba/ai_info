...
本文提出了一种名为EG-CFG（执行引导的无分类器指导）的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升性能。传统大语言模型（LLM）在生成代码时通常不利用执行反馈，而EG-CFG采用多阶段流程：首先生成候选代码行，通过测试用例执行获取反馈信号，再将信号动态整合到生成过程中。该方法保持行内信号一致性，支持多智能体并行探索不同推理路径。实验表明，EG-CFG在各类编程任务中显著优于标准方法，从基础问题到复杂竞赛题目均取得最先进结果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T22:01:48Z
- **目录日期**: 2025-06-14
