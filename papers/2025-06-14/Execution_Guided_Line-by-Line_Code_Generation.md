...
本文提出了一种名为EG-CFG（执行引导的无分类器指导）的新方法，将实时执行信号融入语言模型的代码生成过程。该方法通过多阶段流程：首先生成候选代码行，执行测试用例获取反馈信号，再将信号动态整合到生成过程中。EG-CFG在保持语法结构的同时，通过行级信号更新提供连贯指导，并支持多智能体并行探索不同推理路径。实验表明，该方法显著提升了代码生成性能，在从基础问题到复杂编程竞赛任务的不同难度级别上都取得了最先进的结果。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T13:08:49Z
- **目录日期**: 2025-06-14
