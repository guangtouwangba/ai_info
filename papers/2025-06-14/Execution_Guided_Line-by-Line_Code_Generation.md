...
本文提出了一种名为"执行引导的无分类器指导(EG-CFG)"的新型神经代码生成方法。该方法创新性地将实时执行信号融入语言模型生成过程，通过多阶段处理：首先生成候选代码行，执行测试用例获取反馈信号，再将信号整合到生成提示中。EG-CFG在保持语法结构的同时提供连贯指导，并支持多代理并行探索不同推理路径。实验表明，该方法显著提升了代码生成性能，在从基础问题到复杂编程挑战的各种任务中都取得了最先进的结果。相关代码已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T09:01:37Z
- **目录日期**: 2025-06-14
