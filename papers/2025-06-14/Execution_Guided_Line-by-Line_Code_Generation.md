...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升代码生成质量。传统大语言模型(LLM)在推理时通常不利用执行反馈，而EG-CFG方法采用多阶段流程：首先生成候选代码行，通过测试用例获取执行反馈，再将反馈信号整合到生成过程中。该方法保持行内信号一致性，支持多智能体并行探索不同推理路径。实验表明，EG-CFG在各种编码任务中显著优于标准方法，从基础问题到复杂编程挑战均取得最先进结果。项目代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T16:02:07Z
- **目录日期**: 2025-06-14
