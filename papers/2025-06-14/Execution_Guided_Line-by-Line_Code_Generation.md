...
本文提出了一种名为EG-CFG的新型神经代码生成方法，通过将实时执行信号融入语言模型生成过程来提升性能。传统大语言模型(LLM)在代码生成时通常不利用执行反馈，而EG-CFG采用多阶段流程：首先生成候选代码行，通过测试用例获取执行信号，然后将这些信号动态整合到生成过程中。该方法保持行内信号一致性，在行边界刷新信号，既提供连贯指导又保持语法结构。实验表明，EG-CFG在不同复杂度编程任务中都显著优于标准方法，达到了最先进的性能水平。该方法还天然支持多代理并行探索不同推理路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T18:02:40Z
- **目录日期**: 2025-06-14
