...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个智能代理的社会互动（500万次交互）。该系统通过四个社会实验（社会极化、信息传播、基本收入政策、自然灾害影响）验证其有效性，结果与现实世界数据高度吻合。研究表明，AgentSociety能有效捕捉人类行为模式和社会机制，为社会科学研究和政策制定提供了新工具，可辅助传统研究方法（如调查、访谈）进行社会问题分析。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T10:03:47Z
- **目录日期**: 2025-06-14
