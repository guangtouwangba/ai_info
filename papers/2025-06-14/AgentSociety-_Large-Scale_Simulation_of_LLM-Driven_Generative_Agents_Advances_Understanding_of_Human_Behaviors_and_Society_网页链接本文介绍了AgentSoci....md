...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟10,000多个智能代理的社会互动（500万次交互）。该系统通过四个社会问题案例（社会极化、信息传播、基本收入政策、自然灾害影响）验证了其有效性，模拟结果与现实研究数据高度吻合。研究表明，该系统能有效捕捉人类行为机制，可作为社会科学研究和政策制定的新型工具，弥补传统方法（如调查、访谈）的局限性。这一创新为理解复杂社会现象提供了计算实验平台。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T08:04:09Z
- **目录日期**: 2025-06-14
