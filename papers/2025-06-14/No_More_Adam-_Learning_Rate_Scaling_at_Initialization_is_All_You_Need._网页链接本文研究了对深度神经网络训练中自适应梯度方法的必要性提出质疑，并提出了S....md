...
这篇论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。该方法基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信噪比（g-SNR）调整学习率，有效解决了训练不平衡问题。相比AdamW，SGD-SAi不仅内存占用减半（在GPT-2和Llama2-7B上分别节省5.93GB和25.15GB），还在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，同时展现出超参数鲁棒性和广泛适用性，包括在LoRA微调任务中的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T09:03:46Z
- **目录日期**: 2025-06-14
