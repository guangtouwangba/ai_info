...
本文提出SWE-Factory自动化流程，用于高效构建GitHub问题解决任务的大规模数据集。该流程包含三个核心组件：SWE-Builder多智能体系统（自动搭建评估环境）、基于退出码的标准评分方法（无需人工解析）、以及自动化的fail2pass验证。实验表明，该流程能以低成本（最低0.024美元/实例）高效生成有效任务实例，评分准确率达100%，验证精确度和召回率分别为0.92和1.00。该方法显著降低了传统基准构建的难度和人力成本，有望加速高质量数据集的创建。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T03:19:44Z
- **目录日期**: 2025-06-14
