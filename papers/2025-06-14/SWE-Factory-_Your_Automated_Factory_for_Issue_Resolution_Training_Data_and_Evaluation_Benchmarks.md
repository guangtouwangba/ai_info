...
本文提出了SWE-Factory自动化流水线，用于高效构建GitHub问题解决任务的大规模数据集，以支持大语言模型（LLM）的训练与评估。该流水线包含三个核心组件：1）SWE-Builder多智能体系统，通过协作迭代自动构建评估环境；2）基于退出码的标准化评分方法，无需人工编写解析器；3）利用可靠退出码信号实现自动化fail2pass验证。实验表明，该方案在4种编程语言的671个问题上表现优异：GPT-4.1-mini以每个实例0.045美元构建269个有效实例，评分准确率达100%，验证精确度0.92，召回率1.00。代码与数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-14T20:01:28Z
- **目录日期**: 2025-06-14
