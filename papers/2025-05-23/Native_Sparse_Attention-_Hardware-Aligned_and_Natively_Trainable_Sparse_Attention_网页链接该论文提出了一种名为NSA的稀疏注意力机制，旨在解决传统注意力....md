...
该论文提出NSA（Native Sparse Attention）稀疏注意力机制，通过动态分层稀疏策略（粗粒度令牌压缩+细粒度令牌选择）解决传统注意力机制在长文本处理中的高计算成本问题。NSA结合算法创新与硬件优化，在保持全局上下文感知能力的同时确保局部精度，显著提升了长文本建模效率。研究包含两项关键创新：硬件对齐设计和原生可训练性，为大规模语言模型的高效计算提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T23:04:46Z
- **目录日期**: 2025-05-23
