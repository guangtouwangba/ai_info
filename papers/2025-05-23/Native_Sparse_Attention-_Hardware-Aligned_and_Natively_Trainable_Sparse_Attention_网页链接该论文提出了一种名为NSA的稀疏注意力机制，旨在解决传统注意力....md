...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过动态分层稀疏策略结合硬件优化，在保持全局上下文感知能力的同时提升计算效率。其核心创新包括：1）粗粒度令牌压缩与细粒度令牌选择相结合；2）算法与硬件协同设计。该方法显著降低了长文本建模的计算开销，同时保持了局部精度，为大规模语言模型的高效训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T17:05:33Z
- **目录日期**: 2025-05-23
