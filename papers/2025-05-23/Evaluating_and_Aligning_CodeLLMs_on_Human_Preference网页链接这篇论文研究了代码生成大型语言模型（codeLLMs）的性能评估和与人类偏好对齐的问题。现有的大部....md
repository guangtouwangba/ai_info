...
这篇论文聚焦代码生成大模型（CodeLLMs）与人类偏好的对齐问题，提出新基准CodeArena（含397个多语言/多类别真实编程任务样本）和合成指令数据集SynCode-Instruct（20B标记）。研究发现：1）传统基于执行的评估与人类偏好存在偏差；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。通过大规模实验验证了合成指令微调的有效性，强调代码生成质量需兼顾功能正确性和人类编程习惯。成果为提升CodeLLMs的实用价值提供了新评估框架和数据支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T07:06:12Z
- **目录日期**: 2025-05-23
