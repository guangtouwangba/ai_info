...
LatentSync是一种新型端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该研究解决了扩散模型在唇同步中的时间一致性问题，提出时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时间一致性。同时，研究改进了SyncNet的训练框架，提高了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些创新可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T08:05:50Z
- **目录日期**: 2025-05-23
