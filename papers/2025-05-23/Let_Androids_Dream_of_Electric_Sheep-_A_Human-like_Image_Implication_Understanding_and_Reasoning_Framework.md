...
人工智能系统在理解图像隐喻方面仍面临重大挑战，现有模型难以把握视觉内容中蕴含的文化、情感和语境等微妙含义。针对这一问题，研究团队提出"Let Androids Dream (LAD)"创新框架，通过三阶段处理：感知（将视觉信息转化为多层次文本表征）、搜索（整合跨领域知识消除歧义）和推理（生成符合语境的图像含义解释）。该框架采用轻量级GPT-4o-mini模型，在英汉图像理解基准测试中表现优异，在开放式问题上甚至超越GPT-4o模型36.7%。这项研究为提升AI的图像隐喻理解能力提供了新思路，推动了视觉-语言推理和人机交互领域的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T09:01:10Z
- **目录日期**: 2025-05-23
