...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信号噪声比（g-SNR）调整不同参数组的学习率，显著改善了带动量的随机梯度下降（SGDM）的性能。SGD-SaI不仅内存效率更高（比AdamW节省5.93-25.15GB），在Transformer任务、ImageNet分类和LLM预训练中表现优异，还解决了SGD训练Transformer的难题。它在LoRA微调等任务中也优于现有优化器，同时具备超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T10:04:51Z
- **目录日期**: 2025-05-23
