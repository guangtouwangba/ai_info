...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度学习中对自适应梯度方法（如Adam）的依赖。该方法基于带动量的随机梯度下降（SGDM），通过初始化时根据梯度信噪比（g-SNR）调整不同参数组的学习率，避免了训练不平衡问题。相比AdamW，SGD-SAi内存效率更高（减少50%内存），在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，甚至优于现有最佳优化器。例如，它为GPT-2和Llama2-7B分别节省了5.93GB和25.15GB内存，同时保持或超越AdamW的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T22:04:25Z
- **目录日期**: 2025-05-23
