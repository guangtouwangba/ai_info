...
该研究提出SGD-SaI方法，挑战了深度学习中自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题，并将内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及大模型预训练中表现优于或持平AdamW，尤其在GPT-2和Llama2-7B训练中分别节省5.93GB和25.15GB内存。其优势还包括超参数鲁棒性，并在LoRA微调等任务中超越现有优化器，为高效训练大模型提供了新方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T18:04:42Z
- **目录日期**: 2025-05-23
