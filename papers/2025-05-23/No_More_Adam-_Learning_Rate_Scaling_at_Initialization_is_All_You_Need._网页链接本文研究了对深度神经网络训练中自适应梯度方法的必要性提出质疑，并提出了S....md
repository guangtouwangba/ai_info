...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信号噪声比（g-SNR）调整不同参数组的学习率，解决了传统SGD训练不平衡的问题。相比AdamW，SGD-SAi内存效率提升50%，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，尤其在大模型（如GPT-2、Llama2-7B）训练中可节省数GB内存。同时，它在LoRA微调等任务中也优于现有优化器，且对超参数变化具有鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T15:04:56Z
- **目录日期**: 2025-05-23
