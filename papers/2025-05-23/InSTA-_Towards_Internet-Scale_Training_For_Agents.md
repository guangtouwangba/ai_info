...
该研究提出了一种无需人工标注的大规模网络导航智能体训练方法。通过三阶段流程：1)大语言模型(LLM)自动生成15万个网站任务；2)LLM代理执行任务并生成轨迹；3)LLM筛选成功轨迹(准确率82.6%)。该方法训练出的17亿参数Qwen 3模型在网页导航任务中表现优异，成功率56.9%，超越2350亿参数的Llama 4模型，达到Gemini 2.5 Flash性能的94.7%。研究还显示LLM能有效过滤有害内容(准确率97%)，并发布了相关代码、模型和数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T03:20:25Z
- **目录日期**: 2025-05-23
