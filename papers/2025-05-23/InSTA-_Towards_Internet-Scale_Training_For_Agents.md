...
该研究提出了一种无需人工标注的大规模网络导航智能体训练方法。首先利用大语言模型（LLM）为15万个网站生成任务，然后由LLM代理执行任务并生成轨迹，最后通过LLM筛选成功轨迹（准确率82.6%）。基于该方法训练出的17亿参数Qwen 3模型在网页导航任务中表现出色，成功率56.9%，超越了2350亿参数的Llama 4 Maverick，达到Gemini 2.5 Flash性能的94.7%。该方法展示了LLM作为高效数据标注工具的潜力，有害内容识别准确率达97%。相关代码、模型和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T10:02:38Z
- **目录日期**: 2025-05-23
