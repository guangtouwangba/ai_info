...
该研究提出了一种无需人工标注的大规模网络导航智能体训练方法。通过三阶段流程：首先用大语言模型(LLM)为15万个网站生成任务，然后由LLM代理执行任务并产生轨迹，最后用LLM过滤成功轨迹。实验显示，语言模型能高效筛选数据（有害内容识别准确率97%，轨迹成功率判断准确率82.6%）。基于Qwen 3 1.7B训练的小型代理表现优异，成功率56.9%，超越235倍大的Llama 4 Maverick，达到Gemini 2.5 Flash性能的94.7%。研究公开了代码、模型和数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T05:02:31Z
- **目录日期**: 2025-05-23
