...
本文提出了一种基于语义点匹配的扩散模型方法(SPM-Diff)，用于解决虚拟试衣(VTON)任务中服装细节保留的难题。不同于传统方法直接将整件服装作为外观参考输入UNet，该方法创新性地利用视觉对应关系作为先验，通过局部流变形将服装的语义点与目标人物匹配，并结合深度/法线图增强为3D感知线索。此外，还设计了点聚焦扩散损失函数来充分利用语义点匹配信息。实验表明，该方法在VITON-HD和DressCode数据集上实现了最先进的VTON性能，能有效保持服装形状和细节。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T18:02:22Z
- **目录日期**: 2025-05-23
