...
该论文提出了一种基于扩散模型的虚拟试穿(VTON)新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改进服装细节保留。不同于传统方法直接将整件服装输入UNet，该方法将服装细节表示为结构化语义点集，通过局部流变形匹配目标人物的语义点，并利用深度/法线图增强为3D感知线索。这种对应关系模拟了真实穿衣过程，3D线索则作为语义点匹配来监督扩散模型训练。实验表明，该方法在VITON-HD和DressCode数据集上实现了最先进的性能，显著提升了服装细节保留效果。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T16:02:27Z
- **目录日期**: 2025-05-23
