...
该论文提出了一种基于扩散模型的虚拟试穿（VTON）新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改善服装细节保留。传统双分支架构直接将服装作为外观参考输入UNet，而新方法将服装细节解构为结构化语义点，通过局部流变形匹配目标人物的语义点，并利用深度/法线图增强为3D感知线索。这种对应关系模拟穿衣过程，3D线索作为语义点匹配监督扩散模型训练，并设计了点聚焦扩散损失。实验表明，该方法在VITON-HD和DressCode数据集上实现了最先进的VTON性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T06:02:29Z
- **目录日期**: 2025-05-23
