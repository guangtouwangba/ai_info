...
Know-The-Ropes (KtR) 提出了一种解决大语言模型（LLM）局限性的框架，通过将领域先验知识转化为层次化算法蓝图，递归分解任务为可控子任务，结合零样本或轻量增强（如思维链、微调等）解决问题。基于"没有免费午餐"定理，KtR强调结构化分解而非通用提示。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini代理将准确率从3%提升至95%；在任务分配问题（6-15项）中，6个代理实现小规模100%、大规模84%的准确率（零样本仅11%）。该方法证明了算法感知分解结合精准增强可使小型模型高效协作，无需依赖巨型单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T10:02:17Z
- **目录日期**: 2025-05-23
