...
Know-The-Ropes (KtR) 是一个多智能体框架，旨在解决单一大语言模型(LLM)的局限性，如有限上下文、角色过载和领域迁移脆弱性。KtR 将领域先验知识转化为层级化的算法蓝图，递归分解任务为类型化的子任务，并通过轻量级增强(如思维链、微调或自检)解决。基于"没有免费午餐"定理，KtR 放弃了通用提示的追求，转而采用结构化分解。实验显示，在背包问题(3-8项)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15项)中，6个o3-mini智能体实现了100%(10项)和84%(13-15项)的准确率，远超零样本的11%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T17:02:28Z
- **目录日期**: 2025-05-23
