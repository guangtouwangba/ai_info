...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化的算法蓝图来解决单一大语言模型的局限性（如有限上下文、角色过载）。KtR将任务递归分解为类型化的子任务，由控制器协调，并采用轻量级增强（如思维链、微调）解决。基于"没有免费午餐"定理，KtR放弃通用提示，专注于结构化分解。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6智能体系统在10项任务中达到100%准确率。该方法证明了适度模型通过分解和定向增强可成为可靠协作系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T06:02:17Z
- **目录日期**: 2025-05-23
