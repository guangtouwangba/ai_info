...
Know-The-Ropes (KtR) 提出了一种多智能体框架，通过将领域先验转化为层次化的算法蓝图，解决单一大语言模型(LLM)在上下文限制、角色过载和领域迁移上的瓶颈。KtR递归分解任务为类型化的子任务，采用最轻量的增强策略(如思维链、微调等)，避免传统多智能体方法的分解模糊和验证开销。实验显示：在背包问题(3-8物品)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15作业)中，6个o3-mini智能体实现小规模100%、大规模84%的准确率，远超零样本11%的表现。该方法证明算法感知分解结合精准增强可使轻量模型达成可靠协作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T19:02:02Z
- **目录日期**: 2025-05-23
