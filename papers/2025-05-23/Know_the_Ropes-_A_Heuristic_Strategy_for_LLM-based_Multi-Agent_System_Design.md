...
Know-The-Ropes (KtR)是一个多智能体框架，旨在解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载和领域迁移脆弱性)。KtR将领域先验转化为算法蓝图层次结构，递归分解任务为类型化子任务，并通过轻量级增强(如思维链、微调)解决。基于"没有免费午餐"定理，KtR放弃通用提示，采用结构化分解。实验显示：在背包问题(3-8项)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15项)中，6个o3-mini智能体达到100%(10项)和84%(13-15项)准确率，远超零样本的11%。该方法证明适度模型通过算法感知分解和针对性增强即可实现可靠协作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T08:02:17Z
- **目录日期**: 2025-05-23
