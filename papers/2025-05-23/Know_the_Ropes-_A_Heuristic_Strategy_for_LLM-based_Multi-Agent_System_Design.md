...
大型语言模型（LLM）存在上下文限制、角色过载和领域迁移脆弱性等问题。传统多智能体方法虽能缓解但带来新问题，如任务分解模糊、合约不明确和验证开销。Know-The-Ropes（KtR）框架通过将领域先验转化为层次化算法蓝图，递归分解任务为控制器管理的子任务，采用零样本或轻量增强（如思维链、微调）解决。基于"没有免费午餐"定理，KtR放弃通用提示，专注结构化分解。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；任务分配问题（6-15项）中，6智能体系统在10项内达100%准确率，13-15项达84%。该方法使小型模型成为可靠协作系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T15:02:28Z
- **目录日期**: 2025-05-23
