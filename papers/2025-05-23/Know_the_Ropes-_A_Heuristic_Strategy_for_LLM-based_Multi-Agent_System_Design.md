...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单智能体大模型（LLMs）的局限性（如有限上下文、角色过载和脆弱领域迁移）。KtR递归分解任务为类型化子任务，结合轻量级增强技术（如思维链、微调、自检），显著提升性能。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从零样本的3%提升至95%；在任务分配问题（6-15项）中，6智能体组合实现小规模100%、大规模84%的准确率（零样本仅11%）。该方法证明结构化分解比盲目扩大模型规模更有效。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T18:02:12Z
- **目录日期**: 2025-05-23
