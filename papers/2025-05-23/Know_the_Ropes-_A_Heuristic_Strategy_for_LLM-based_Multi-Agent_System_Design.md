...
Know-The-Ropes (KtR) 提出了一种多智能体协作框架，通过将领域先验转化为分层算法蓝图来解决单一大模型的局限性（如有限上下文、角色过载）。该框架递归分解任务为类型化子任务，由控制器协调，并采用零样本或轻量增强（如思维链、微调）解决。基于"没有免费午餐"定理，KtR放弃通用提示，转向结构化分解。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6智能体系统在10项以内达100%准确率。证明算法感知分解结合定向增强可使中小模型高效协作，无需持续扩大单体模型规模。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T21:02:14Z
- **目录日期**: 2025-05-23
