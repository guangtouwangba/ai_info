...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化的算法蓝图，解决单一大语言模型（LLM）的上下文限制、角色过载和领域迁移脆弱性问题。KtR递归分解任务为类型化的子任务，由控制器协调，结合零样本推理或轻量增强（如思维链、微调）。实验显示，在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6个智能体实现100%（10项）和84%（13-15项）准确率，远超零样本的11%。该方法无需扩大模型规模即可显著提升性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T23:02:10Z
- **目录日期**: 2025-05-23
