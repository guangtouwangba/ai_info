...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图，解决单一大语言模型(LLM)的局限性。KtR递归分解任务为类型化子任务，采用最简增强策略(如思维链、微调等)。基于"没有免费午餐"定理，它放弃通用提示，专注结构化分解。实验显示：在背包问题(3-8物品)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15任务)中，6个o3-mini智能体实现100%(小规模)和84%(大规模)准确率，远超零样本的11%。该方法证明适度模型通过算法感知分解可实现可靠协作，无需更大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T04:03:05Z
- **目录日期**: 2025-05-23
