...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型的局限性（如有限上下文、角色过载和领域迁移脆弱性）。KtR 递归分解任务为类型化子任务，结合轻量级增强技术（如思维链、微调或自检），避免传统多智能体方法的分解模糊和验证开销。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；任务分配问题（6-15项）中，6个o3-mini智能体实现100%（10项内）和84%（13-15项）准确率，远超零样本的11%。该方法证明算法感知分解能高效协同轻量模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T12:02:47Z
- **目录日期**: 2025-05-23
