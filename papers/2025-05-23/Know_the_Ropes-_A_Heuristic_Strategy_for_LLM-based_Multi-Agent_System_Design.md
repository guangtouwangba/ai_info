...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型的局限性（如有限上下文、角色过载和领域迁移脆弱性）。KtR 递归分解任务为类型化子任务，结合轻量级增强技术（如思维链、微调或自检），避免传统多智能体方法的分解模糊和验证开销。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从零样本的3%提升至95%；在任务分配问题（6-15项）中，6智能体系统在规模10以下达100%准确率，规模13-15达84%，远超零样本11%。该框架证明算法感知分解能显著增强轻量模型协作效能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T09:02:15Z
- **目录日期**: 2025-05-23
