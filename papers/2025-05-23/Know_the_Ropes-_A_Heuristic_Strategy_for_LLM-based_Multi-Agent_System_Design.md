...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为分层算法蓝图来解决单智能体LLM的局限性（有限上下文、角色过载、脆弱迁移）。KtR递归分解任务为类型化子任务，结合轻量级增强技术（如思维链、微调、自检），避免传统多智能体的模糊分工和验证开销。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从零样本3%提升至95%；任务分配问题（6-15项）中，6智能体在10项内达100%准确率，13-15项达84%（零样本仅11%）。该方法证明算法感知分解能有效增强轻量模型，无需庞大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T02:31:35Z
- **目录日期**: 2025-05-23
