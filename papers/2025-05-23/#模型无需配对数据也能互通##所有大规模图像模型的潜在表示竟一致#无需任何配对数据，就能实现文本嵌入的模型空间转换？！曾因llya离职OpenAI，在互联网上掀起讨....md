...
康奈尔大学提出vec2vec方法，首次实现无需配对数据的无监督文本嵌入跨空间转换。研究发现所有大规模图像模型具有相同的潜在表示（柏拉图假说），该方法通过共享潜在空间保留语义结构，在目标空间中达到0.92的余弦相似度，并在8000多个随机嵌入上实现完美匹配。这表明不同架构/训练数据的编码器存在近乎一致的表示形式，验证了文本模型也存在通用潜在结构。研究突破了传统需要配对数据的限制，为模型互通提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T21:04:11Z
- **目录日期**: 2025-05-23
