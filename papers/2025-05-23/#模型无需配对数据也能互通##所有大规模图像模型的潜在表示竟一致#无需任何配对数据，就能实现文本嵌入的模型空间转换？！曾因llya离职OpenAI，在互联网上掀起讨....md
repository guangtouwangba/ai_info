...
康奈尔大学提出vec2vec方法，首次实现无需配对数据的无监督文本嵌入空间转换。研究发现，不同架构和训练数据的文本编码器在潜在表示上具有高度一致性（余弦相似度达0.92），验证了"柏拉图表示假说"在文本领域的适用性。该方法通过共享潜在空间保留语义结构，能反推提取嵌入信息，在8000多个随机嵌入上实现完美匹配。这表明大规模模型可能趋向统一的潜在表示，为跨模型互操作提供了理论基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T19:04:01Z
- **目录日期**: 2025-05-23
