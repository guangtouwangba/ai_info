...
康奈尔大学提出vec2vec方法，首次实现无需配对数据的无监督文本嵌入空间转换。研究验证了"柏拉图表示假说"，发现不同架构/训练数据的文本编码器都具有高度一致的潜在表示结构。vec2vec利用共享潜在空间实现跨模型嵌入转换，保持语义结构的同时能逆向提取信息，在8000多个随机嵌入上实现完美匹配，与目标向量的余弦相似度达0.92。这表明大规模语言模型可能共享统一的底层表示形式，为模型互操作提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T22:04:01Z
- **目录日期**: 2025-05-23
