...
康奈尔大学提出"vec2vec"方法，首次实现无需配对数据的无监督文本嵌入空间转换。研究表明，不同架构和训练数据的文本编码器存在近乎一致的潜在表示结构，余弦相似度达0.92。该方法利用共享潜在空间保留语义信息，在8000多个随机嵌入上实现完美匹配，验证了"柏拉图表示假说"在文本领域的普适性——大规模模型确实趋向于形成统一的潜在表示。这一发现为跨模型信息互通提供了理论基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T11:04:26Z
- **目录日期**: 2025-05-23
