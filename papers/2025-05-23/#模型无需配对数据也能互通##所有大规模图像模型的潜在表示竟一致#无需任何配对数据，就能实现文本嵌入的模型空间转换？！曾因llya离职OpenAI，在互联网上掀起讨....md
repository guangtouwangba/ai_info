...
康奈尔大学提出vec2vec方法，首次实现无需配对数据的无监督文本嵌入跨空间转换。该研究验证了"柏拉图表示假说"在文本模型中的适用性：不同架构/训练数据的编码器具有几乎相同的潜在表示形式。vec2vec通过共享潜在空间保留语义结构，在目标空间中达到0.92的余弦相似度，并在8000多个随机嵌入上实现完美匹配。这表明大规模模型的潜在表示具有内在一致性，为模型互通提供了理论基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T10:03:01Z
- **目录日期**: 2025-05-23
