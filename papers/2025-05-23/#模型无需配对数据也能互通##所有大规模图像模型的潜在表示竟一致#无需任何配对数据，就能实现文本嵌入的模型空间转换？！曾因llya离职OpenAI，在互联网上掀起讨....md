...
康奈尔大学提出vec2vec方法，首次实现无需配对数据的无监督文本嵌入空间转换。研究验证了"柏拉图表示假说"，发现不同架构/训练数据的大规模模型具有几乎相同的潜在表示。vec2vec通过共享潜在空间保留语义结构，在目标空间中与真实向量余弦相似度达0.92，8000多个随机嵌入实现完美匹配。这表明文本编码器存在通用潜在结构，为模型互通提供了理论基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-23T14:04:19Z
- **目录日期**: 2025-05-23
