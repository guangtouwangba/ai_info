...
这篇论文针对代码生成大模型（CodeLLMs）提出新评估方法CodeArena，关注人类偏好而非仅代码正确性。研究收集397个高质量编程任务样本（覆盖40类/44种语言），并构建20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好存在偏差；2)开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调对齐人类偏好的重要性，为改进代码生成模型提供了新基准和数据集支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T23:05:12Z
- **目录日期**: 2025-04-14
