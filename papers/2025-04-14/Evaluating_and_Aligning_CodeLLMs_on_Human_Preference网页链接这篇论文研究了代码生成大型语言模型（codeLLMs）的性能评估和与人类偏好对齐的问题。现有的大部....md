...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，强调人类偏好对齐的重要性。研究者创建了CodeArena基准测试（含397个高质量样本，覆盖40类任务和44种语言），并构建20B标记的SynCode-Instruct合成指令集。实验发现：1)传统执行基准与人类偏好存在偏差；2)开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究证实大规模合成指令微调的有效性，揭示了代码生成模型优化需兼顾功能正确性和人类编程习惯。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T20:05:32Z
- **目录日期**: 2025-04-14
