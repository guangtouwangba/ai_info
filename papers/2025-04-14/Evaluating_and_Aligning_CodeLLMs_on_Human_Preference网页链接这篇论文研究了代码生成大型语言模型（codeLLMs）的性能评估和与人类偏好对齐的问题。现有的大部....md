...
这篇论文探讨了代码生成大模型（codeLLMs）如何与人类偏好对齐的问题。研究发现现有基准测试过于关注代码正确性，忽视了人类偏好。为此，作者构建了CodeArena基准测试（包含397个高质量样本，覆盖40类任务和44种编程语言）和SynCode-Instruct合成指令数据集（近200亿标记）。实验显示，执行基准与人类偏好评估存在差异，且开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）性能差距显著，凸显了模型对齐人类偏好的重要性。研究为提升代码LLMs的实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T01:33:08Z
- **目录日期**: 2025-04-14
