...
该研究提出了SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法（如Adam）的必要性。该方法通过初始化时依据梯度信噪比(g-SNR)调整各参数组的学习率，无需二阶动量即可解决训练不平衡问题，并将内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类及大模型预训练中表现优于AdamW，尤其在内存效率上优势显著（如为Llama2-7B节省25.15GB）。该方法还展现出超参数鲁棒性，并在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T20:04:22Z
- **目录日期**: 2025-04-14
