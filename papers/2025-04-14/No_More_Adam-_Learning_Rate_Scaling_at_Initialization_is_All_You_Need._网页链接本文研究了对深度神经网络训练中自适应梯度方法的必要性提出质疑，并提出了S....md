...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需二阶动量即可解决训练不平衡问题，并将内存占用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或持平AdamW，尤其在GPT-2和Llama2-7B上分别节省5.93GB和25.15GB内存。此外，它在LoRA微调等任务中也超越现有优化器，且对超参数变化更具鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T11:04:41Z
- **目录日期**: 2025-04-14
