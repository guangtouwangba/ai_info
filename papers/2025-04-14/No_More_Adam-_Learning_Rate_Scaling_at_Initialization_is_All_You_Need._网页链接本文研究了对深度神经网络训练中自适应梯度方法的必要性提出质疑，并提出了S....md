...
该研究提出SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题，并将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类、LLM预训练（如GPT-2/Llama2）和LoRA微调中均优于AdamW，最高节省25GB内存。其优势包括：1)性能相当或更好；2)解决SGD训练Transformer难题；3)超参数鲁棒性；4)显著内存效率提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T01:31:51Z
- **目录日期**: 2025-04-14
