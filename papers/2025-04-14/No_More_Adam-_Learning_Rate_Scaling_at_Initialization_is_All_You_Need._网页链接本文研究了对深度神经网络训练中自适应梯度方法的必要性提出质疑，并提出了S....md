...
该研究提出SGD-SaI方法，挑战了深度神经网络训练中依赖自适应梯度方法的必要性。该方法改进带动量随机梯度下降(SGDM)，通过初始化时基于梯度信噪比(g-SNR)调整各参数组学习率，无需自适应二阶动量即可避免训练不平衡，并将内存占用减半。实验显示，在Transformer任务、ImageNet分类及GPT-2预训练中，其性能匹配或优于AdamW，并显著提升内存效率（如为Llama2-7B节省25.15GB）。此外，在LoRA微调等任务表现突出，且具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T14:04:29Z
- **目录日期**: 2025-04-14
