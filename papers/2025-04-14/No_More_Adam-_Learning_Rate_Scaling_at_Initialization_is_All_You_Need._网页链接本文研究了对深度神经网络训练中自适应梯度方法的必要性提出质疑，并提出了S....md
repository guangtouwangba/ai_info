...
该研究提出SGD-SaI方法，质疑自适应梯度优化器(如Adam)的必要性。该方法改进带动量SGD，通过初始化时基于梯度信噪比(g-SNR)调整各参数组学习率，无需二阶动量即可避免训练失衡。相比AdamW内存减半，在Transformer、ImageNet分类和GPT-2预训练等任务中表现相当或更优，并显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。同时展现超参数鲁棒性，在LoRA微调等场景优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T21:04:11Z
- **目录日期**: 2025-04-14
