...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。该方法通过初始化时根据不同参数组的梯度信噪比(g-SNR)调整学习率，解决了传统SGD训练不平衡问题，并将内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中表现优于或持平AdamW，尤其在大模型（如Llama2-7B）中节省显著内存（最高25.15GB）。此外，它在LoRA微调等任务中也优于现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T07:04:34Z
- **目录日期**: 2025-04-14
