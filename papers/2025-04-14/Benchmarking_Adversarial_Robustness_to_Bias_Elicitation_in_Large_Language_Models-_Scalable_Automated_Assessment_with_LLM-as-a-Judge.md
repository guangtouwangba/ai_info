...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。研究表明，训练数据中的历史不平等、语言不平衡等因素会导致LLMs产生刻板印象和公平性问题。本研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化及越狱技术，系统评估模型对抗偏见诱导的鲁棒性。分析发现，不同规模的前沿模型均存在偏见问题，且模型大小与安全性存在权衡关系。研究还评估了医疗等关键领域专用模型的安全性，并开源测试数据集以促进系统性漏洞评估，为开发更公平、更鲁棒的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T00:01:45Z
- **目录日期**: 2025-04-14
