...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。该研究提出可扩展的基准测试框架CLEAR-Bias，通过多任务方法系统评估模型在不同社会文化维度的偏见表现，采用LLM-as-a-Judge自动评分量化模型鲁棒性，并利用越狱技术检测安全机制漏洞。研究发现模型规模与安全性存在权衡关系，即使领域专用模型(如医疗)也存在偏见风险。研究发布了包含偏见提示的CLEAR-Bias数据集，为开发更公平、更鲁棒的语言模型提供支持。结果表明当前LLMs仍易受对抗性偏见诱导攻击的影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T02:29:31Z
- **目录日期**: 2025-04-14
