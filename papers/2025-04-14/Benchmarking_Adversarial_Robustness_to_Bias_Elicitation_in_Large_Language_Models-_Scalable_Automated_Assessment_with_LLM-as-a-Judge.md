...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用中存在的偏见问题日益凸显。这些偏见源于训练数据的不平等、语言不平衡等因素，且易受对抗性攻击影响。本研究提出了一个可扩展的评估框架，通过多任务方法系统探测模型在不同社会文化维度上的偏见，采用LLM-as-a-Judge方法量化模型安全性，并利用越狱技术测试安全机制的脆弱性。研究发现模型规模与安全性之间存在关键权衡，并发布了CLEAR-Bias数据集以促进系统性漏洞评估。该研究为开发更公平、更鲁棒的语言模型提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T01:28:55Z
- **目录日期**: 2025-04-14
