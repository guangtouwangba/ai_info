...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂音视频关系，并通过提出的时间表示对齐(TREPA)方法解决时间一致性问题。研究还改进了SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。创新点包括：1)端到端潜在扩散模型；2)TREPA增强时间一致性；3)SyncNet训练优化。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T03:20:47Z
- **目录日期**: 2025-04-14
