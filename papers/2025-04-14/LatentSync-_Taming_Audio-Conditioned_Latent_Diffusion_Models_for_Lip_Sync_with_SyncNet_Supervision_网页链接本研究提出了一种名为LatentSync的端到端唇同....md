...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉相关性，并引入时间表示对齐（TREPA）方法解决时间一致性问题。研究还改进了SyncNet训练框架，提升了其准确性。在HDTF和VoxCeleb2数据集上，该方法在多个指标上超越现有技术。创新包括：1) 端到端潜在扩散模型；2) TREPA增强时间一致性；3) SyncNet训练优化。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T10:05:35Z
- **目录日期**: 2025-04-14
