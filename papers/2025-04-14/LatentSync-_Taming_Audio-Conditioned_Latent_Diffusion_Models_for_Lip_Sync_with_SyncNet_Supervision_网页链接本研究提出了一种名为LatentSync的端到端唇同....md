...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。为解决扩散模型的时间一致性问题，开发了TREPA方法（时间表示对齐），通过自监督视频模型增强生成帧的时间连贯性。同时，研究深入分析了SyncNet的收敛问题，改进了其训练框架，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T14:05:27Z
- **目录日期**: 2025-04-14
