...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示，直接建模音频-视觉关系。针对扩散模型的时间一致性问题，创新性提出TREPA方法，利用自监督视频模型的时间表示增强生成帧的时序连贯性。同时，研究深入分析了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理方法，显著提升了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路，相关经验可推广至其他唇同步研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T23:04:55Z
- **目录日期**: 2025-04-14
