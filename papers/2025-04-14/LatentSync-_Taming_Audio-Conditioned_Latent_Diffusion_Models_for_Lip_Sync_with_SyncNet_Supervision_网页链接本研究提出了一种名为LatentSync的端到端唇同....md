...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。通过引入时间表示对齐（TREPA）方法，解决了扩散过程中时间一致性问题，同时保持唇同步精度。研究还优化了SyncNet训练框架，提高了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上性能优于现有技术。创新点包括直接建模音频-视觉关系、增强时间一致性及改进SyncNet训练，为唇同步和音频驱动动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T05:05:29Z
- **目录日期**: 2025-04-14
