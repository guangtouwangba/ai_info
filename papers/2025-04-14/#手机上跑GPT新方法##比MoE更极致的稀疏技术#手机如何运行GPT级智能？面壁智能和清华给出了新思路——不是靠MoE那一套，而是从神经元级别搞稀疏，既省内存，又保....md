...
面壁智能和清华提出CFM（可配置基础模型）技术，通过神经元级稀疏激活实现手机端高效运行GPT级模型。与MoE（专家混合）不同，CFM不限制激活专家数量，而是根据任务难度动态调整激活神经元数量，在保证性能的同时节省内存。该技术特别适合FFN层，支持灵活组合模型模块，适用于手机、笔记本等端侧设备。相比MoE需要平衡负载，CFM允许全参数参与运算，在资源受限场景下更高效。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T02:31:41Z
- **目录日期**: 2025-04-14
