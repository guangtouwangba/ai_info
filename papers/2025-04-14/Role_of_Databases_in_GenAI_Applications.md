...
生成式AI（GenAI）正通过智能内容生成、自动化和决策推动行业变革，但其效果高度依赖高效的数据存储、检索和上下文增强。本文探讨了数据库在GenAI工作流中的关键作用，强调选择合适架构对性能、准确性和可扩展性的影响，并将数据库功能分为三类：对话上下文（键值/文档数据库）、情境上下文（关系数据库/数据湖仓）和语义上下文（向量数据库）。此外，研究还指出实时查询处理、向量搜索及数据库选择对模型效率的影响。采用多数据库策略可使GenAI应用实现更情境感知、个性化且高性能的解决方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T13:10:05Z
- **目录日期**: 2025-04-14
