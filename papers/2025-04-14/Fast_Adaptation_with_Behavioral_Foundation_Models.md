...
该研究针对无监督零样本强化学习（RL）中行为基础模型（BFMs）的零样本策略性能不足问题，提出快速适应策略。研究发现，预训练BFMs隐含了比推理机制更优的策略技能，因此提出基于行动者-评论家和纯行动者的低维任务嵌入空间搜索方法，可在少量环境交互中提升下游任务表现（10-40%），且避免了预训练模型微调初期的性能下降。方法在导航和运动任务中验证，优于现有基线，仅需数十回合即显著改善零样本效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T00:01:34Z
- **目录日期**: 2025-04-14
