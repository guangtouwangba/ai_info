...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFM），使智能体能够零样本（无需额外学习或规划）解决下游任务。然而，零样本策略常因训练误差而表现欠佳。本文提出快速适应策略，在少量环境交互中提升BFM的零样本性能，避免适应过程中的性能下降。研究发现，BFM学习的技能比其推理策略更优，适合快速适应。作者提出了基于任务嵌入空间的演员-评论家和纯演员适应策略，在导航和运动任务中验证了其有效性，仅需数十次交互即可提升10-40%的性能，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T01:28:45Z
- **目录日期**: 2025-04-14
