...
上海AI Lab与西工大提出的CityGS-X架构通过并行化混合分层三维表征(PH²-3D)实现高效城市级3D重建。该技术突破传统方法，采用批处理+多任务并行渲染，仅需4张RTX4090即可处理5000+图像的大场景，训练速度翻倍且显存占用更低。其K层分层体素建模支持并行处理，结合动态体素分配与高斯属性共享，在RGB渲染和几何精度上均达SOTA水平。创新性地通过图块任务分配、高斯属性迁移及法线/深度图协同优化，有效解决了视角过拟合问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T05:03:57Z
- **目录日期**: 2025-04-14
