...
上海AI Lab与西工大提出CityGS-X架构，采用并行化混合分层三维表征（PH²-3D），突破传统分块合并方法。通过批处理+多任务并行渲染，仅需4张4090显卡即可实现大规模城市级3D重建，训练速度翻倍且显存占用更低。该技术支持5000+图像的大场景处理，在RGB渲染和几何精度上均达到SOTA水平。其核心创新包括K层分层体素并行建模、动态体素分配与高斯属性共享，结合法线/深度图协同优化，有效避免视角过拟合问题，显著提升重建效率与质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T01:30:32Z
- **目录日期**: 2025-04-14
