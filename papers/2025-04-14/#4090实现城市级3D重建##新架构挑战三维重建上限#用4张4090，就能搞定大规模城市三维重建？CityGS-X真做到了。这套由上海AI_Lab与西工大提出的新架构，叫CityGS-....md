...
上海AI Lab与西工大提出CityGS-X新架构，利用并行化混合分层三维表征（PH²-3D）实现高效城市级3D重建。该技术通过批处理+多任务并行渲染，显著提升训练速度并降低显存需求，仅需4张4090显卡即可处理5000+图像的大场景。其核心创新包括K层分层体素建模、动态体素分配与高斯属性共享，在RGB渲染和几何精度上均达到SOTA水平。模型还采用法线/深度图协同优化与批处理策略，有效避免视角过拟合，兼顾效率与重建质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T04:03:58Z
- **目录日期**: 2025-04-14
