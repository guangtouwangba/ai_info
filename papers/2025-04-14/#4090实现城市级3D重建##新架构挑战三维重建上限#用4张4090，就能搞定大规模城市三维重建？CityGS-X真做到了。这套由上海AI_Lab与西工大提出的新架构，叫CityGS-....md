...
上海AI Lab与西工大提出的CityGS-X架构通过并行化混合分层三维表征(PH²-3D)实现高效城市级3D重建。该技术突破传统方法，采用批处理+多任务并行渲染，仅需4张4090显卡即可处理5000+图像的大场景。关键优势包括：训练速度翻倍且显存占用更低；RGB渲染和几何精度均达SOTA水平；采用K层分层体素建模与动态分配策略，结合高斯属性共享优化显存使用。通过图块任务分配、属性迁移及多视图协同优化，有效解决了视角过拟合问题，显著提升了大规模场景的重建效率和质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T07:04:10Z
- **目录日期**: 2025-04-14
