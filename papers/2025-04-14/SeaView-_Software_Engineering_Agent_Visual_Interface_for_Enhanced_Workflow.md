...
基于自回归大语言模型（LLM）的软件工程（SWE）代理在解决真实编程问题（如GitHub issue）上取得显著进展（SWE-Bench Verified准确率>60%）。然而，分析这些代理产生的超长交互轨迹（可达128k tokens）存在困难，包括错误定位、性能追踪等问题。为此，研究者提出SeaView工具，通过可视化界面帮助研究人员快速对比不同超参/LLM的实验结果，识别模型或环境问题。用户研究表明，SeaView能将资深研究者的诊断时间从10-30分钟、新手从30-60分钟大幅缩短。该工具填补了SWE代理分析工具的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T19:01:24Z
- **目录日期**: 2025-04-14
