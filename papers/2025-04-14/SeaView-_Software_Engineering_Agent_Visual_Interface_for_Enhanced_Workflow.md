...
基于自回归大语言模型（LLM）的软件工程（SWE）智能体在解决实际编程问题（如GitHub问题修复）方面取得显著进展（SWE-Bench Verified准确率>60%）。然而，由于智能体生成的轨迹数据庞大（常超128k序列长度）且涉及LLM与环境的多轮交互，错误分析和实验对比变得困难。为此，研究者开发了SeaView工具，通过可视化界面帮助研究人员快速分析智能体输出、比较不同超参数或LLM的实验结果，并定位问题。用户研究表明，该工具可将经验丰富研究者的诊断时间缩短至10-30分钟，新手研究者减少至30分钟-1小时。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T20:01:24Z
- **目录日期**: 2025-04-14
