...
基于自回归大语言模型（LLM）的软件工程（SWE）代理在解决实际编码问题（如GitHub问题修复）方面取得显著进展（SWE-Bench Verified准确率>60%）。然而，分析这些代理产生的冗长交互轨迹（常超128k tokens）和调试错误非常困难。为此，研究者开发了可视化工具SeaView，帮助研究人员对比不同超参数或LLM的实验运行结果，快速定位LLM或环境问题。用户研究表明，经验丰富的研究人员使用SeaView可节省10-30分钟分析时间，而新手可节省30分钟至1小时。该工具填补了SWE代理分析工具的研究空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T04:01:30Z
- **目录日期**: 2025-04-14
