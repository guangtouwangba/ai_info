...
基于自回归大语言模型（LLM）的软件工程（SWE）代理在解决实际编程问题（如GitHub issue）方面取得显著进展（SWE-Bench Verified准确率>60%）。这些代理通过推理、环境交互和自省生成复杂轨迹，但其分析面临挑战：轨迹长度常超LLM处理上限（>128k），且涉及LLM与环境的长期交互，导致错误定位和理解困难，实验迭代跟踪也不易。为此，研究者提出可视化工具SeaView，帮助分析代理输出、比较不同参数或LLM的实验结果，快速识别问题。用户研究表明，SeaView可将资深研究者的诊断时间缩短至10-30分钟，新手则减少至30分钟-1小时。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T22:01:21Z
- **目录日期**: 2025-04-14
