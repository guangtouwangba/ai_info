...
基于自回归大语言模型（LLM）的软件工程代理（SWE代理）在解决实际编程问题（如GitHub issue修复）方面取得显著进展（SWE-Bench Verified准确率>60%），但其生成的复杂交互轨迹（可能超过128k tokens）难以分析。现有研究多聚焦代理性能提升，缺乏可视化分析工具。为此，研究者提出SeaView工具，通过可视化界面帮助研究人员快速对比不同超参数/LLM的实验结果，定位LLM或环境问题。用户研究表明，该工具可将资深研究员的诊断时间从10-30分钟、新手的30-60分钟大幅缩短至即时可视化分析。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T10:01:34Z
- **目录日期**: 2025-04-14
