...
基于自回归大语言模型（LLM）的软件工程代理（SWE代理）在解决实际编码问题（如GitHub issue）方面取得显著进展（SWE-Bench Verified准确率>60%），但其生成的复杂交互轨迹难以分析，主要因超长序列（可达128k以上）和LLM与环境的多轮交互。现有研究多聚焦代理性能提升，缺乏分析工具。为此，研究者提出可视化工具SeaView，帮助快速对比实验参数、定位LLM或环境问题。用户研究表明，该工具可将资深研究者的分析时间从10-30分钟缩短至即时呈现，新手研究者则能从30-60分钟大幅提效。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T14:01:27Z
- **目录日期**: 2025-04-14
