...
基于自回归大语言模型（LLM）的软件工程（SWE）代理在解决实际编程问题（如GitHub问题修复）方面取得了显著进展（SWE-Bench Verified准确率>60%）。然而，分析这些代理生成的复杂轨迹（可能超过128k长度）及其与环境的交互过程十分困难，尤其在定位错误或追踪实验改进时。为此，研究者提出了SeaView工具，旨在通过可视化界面帮助研究人员分析代理输出、比较不同超参数或LLM的实验结果，并快速识别问题。用户研究表明，SeaView可将资深研究员的诊断时间从10-30分钟大幅缩短，新手研究员则能从30-60分钟缩短至即时获取关键信息。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T18:01:27Z
- **目录日期**: 2025-04-14
