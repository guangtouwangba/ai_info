...
基于自回归大语言模型（LLM）的软件工程（SWE）代理在解决实际编程问题（如GitHub问题修复）方面取得显著进展（SWE-Bench验证准确率>60%）。然而，分析这些代理生成的长轨迹（可能超过128k tokens）及其与环境交互过程存在困难，难以定位错误或追踪实验改进。为此，研究者提出SeaView工具，通过可视化界面帮助研究人员分析代理输出，比较不同超参数或LLM的实验结果，快速识别LLM或环境问题。用户研究表明，SeaView可将资深研究员的诊断时间从10-30分钟、新手从30-60分钟大幅缩短。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T12:01:32Z
- **目录日期**: 2025-04-14
