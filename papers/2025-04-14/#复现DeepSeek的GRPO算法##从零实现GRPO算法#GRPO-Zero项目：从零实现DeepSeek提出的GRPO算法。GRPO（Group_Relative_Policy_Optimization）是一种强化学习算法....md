...
GRPO-Zero项目从零实现了DeepSeek提出的GRPO（Group Relative Policy Optimization）强化学习算法。该算法无需价值模型或参考模型计算KL散度，通过抽样问题、生成多个回答、评分并标准化计算优势来更新策略模型。项目结构简单，仅需PyTorch，可在A40显卡（48G显存）上运行，默认训练任务为"倒计时题目"（如用给定数字计算目标值）。使用Qwen2.5-3B-Instruct模型，提供完整代码，奖励机制包括格式正确（0.1分）和答案正确（1分）。适合小团队快速上手RLHF项目。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-14T02:30:43Z
- **目录日期**: 2025-04-14
