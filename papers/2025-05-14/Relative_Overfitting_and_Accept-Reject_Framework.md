...
当前大语言模型(LLM)的扩展规律面临瓶颈，研究发现信号噪声比变化导致的噪声效应是主因。论文提出"相对过拟合"概念，并基于此开发了接受-拒绝(AR)框架，利用小语言模型(SLM)对LLM决策产生普遍正向影响。实验证明，相比增加LLM参数量，该框架能以更低成本显著提升模型在语言建模、长文本处理等任务中的表现，且效果稳定普适。研究还探讨了该方法在计算机视觉等领域的应用潜力，有望帮助突破现有扩展规律的瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T02:30:10Z
- **目录日期**: 2025-05-14
