...
本文提出了首个城市环境空中视觉目标搜索（AVOS）基准数据集CityAVOS，包含6类2420个任务，用于评估无人机自主搜索能力。同时提出基于多模态大语言模型的PRPSearcher方法，通过三层认知架构（感知-推理-规划）构建动态语义地图、3D认知地图和不确定性地图，结合去噪机制和启发式提示策略，显著提升了搜索性能（成功率+37.69%，路径效率+28.96%）。实验显示该方法优于现有基线，但与人类表现仍有差距，凸显了语义推理和空间探索能力的改进空间。该研究为具身目标搜索奠定了基础，相关数据和代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T18:01:41Z
- **目录日期**: 2025-05-14
