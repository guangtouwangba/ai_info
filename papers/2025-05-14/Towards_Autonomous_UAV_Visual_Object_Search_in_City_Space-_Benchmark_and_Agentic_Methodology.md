...
本文介绍了首个城市环境空中视觉目标搜索（AVOS）基准数据集CityAVOS，包含6类共2420个任务，用于评估无人机自主搜索能力。针对现有方法在复杂城市环境中的不足，作者提出PRPSearcher方法，采用多模态大语言模型模拟人类三层认知：1）构建物体中心动态语义图增强空间感知；2）基于语义吸引力的3D认知图进行目标推理；3）3D不确定性图平衡探索与利用。实验显示PRPSearcher在成功率（+37.69%）和搜索效率（+28.96% SPL）上显著优于基线，但与人类表现仍有差距，凸显了提升语义推理和空间探索能力的必要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T05:01:08Z
- **目录日期**: 2025-05-14
