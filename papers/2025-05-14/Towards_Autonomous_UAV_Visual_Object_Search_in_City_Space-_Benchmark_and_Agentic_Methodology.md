...
该研究提出了首个城市空中视觉目标搜索（AVOS）基准数据集CityAVOS，包含2,420个任务和6类常见城市物体。为解决AVOS任务中的语义冗余、相似物体区分和探索-利用困境，研究者开发了PRPSearcher方法，采用多模态大语言模型模拟人类三层认知：通过物体中心动态语义图增强空间感知，基于语义吸引力的3D认知图进行目标推理，以及3D不确定性图平衡探索与利用。实验显示PRPSearcher在成功率（+37.69%）和搜索效率（-30.69%路径长度）上显著优于基线，但仍与人类表现存在差距。该工作为具身目标搜索研究奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T10:01:16Z
- **目录日期**: 2025-05-14
