...
该研究提出了CityAVOS——首个城市环境空中视觉目标搜索（AVOS）基准数据集，包含6类共2,420个不同难度的搜索任务。针对现有方法在复杂城市环境中的语义冗余、相似物体区分和探索-利用困境等问题，研究者开发了基于多模态大语言模型的PRPSearcher系统，通过三层认知架构（感知-推理-规划）实现目标搜索：构建物体中心动态语义地图增强空间感知，基于语义吸引力的3D认知地图进行目标推理，以及3D不确定性地图平衡探索与利用。实验显示PRPSearcher在成功率（+37.69%）和搜索效率（-30.69%路径长度）上显著优于基线方法，但与人类表现仍存在差距。该工作为具身目标搜索研究奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T19:01:09Z
- **目录日期**: 2025-05-14
