...
该研究提出了首个城市环境空中视觉目标搜索（AVOS）基准数据集CityAVOS，包含6类共2420个任务，用于评估无人机自主搜索能力。同时开发了PRPSearcher方法，采用多模态大语言模型模拟人类三层认知：构建动态语义地图增强空间感知，基于语义吸引力的3D认知地图进行目标推理，以及3D不确定性地图平衡探索与利用。该方法还包含去噪机制和自适应行动规划提示机制。实验显示PRPSearcher在成功率（+37.69%）和搜索效率（-46.40% NE）上显著优于基线，但与人类表现仍有差距，凸显了语义推理和空间探索能力提升的必要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T21:01:11Z
- **目录日期**: 2025-05-14
