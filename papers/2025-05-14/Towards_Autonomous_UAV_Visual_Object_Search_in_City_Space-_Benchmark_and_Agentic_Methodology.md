...
该研究针对城市环境中的空中视觉目标搜索任务(AVOS)，提出了首个基准数据集CityAVOS（包含2,420个任务）和新型智能体方法PRPSearcher。PRPSearcher基于多模态大语言模型，通过构建三种专用地图（动态语义地图、3D认知地图和不确定性地图）模拟人类三层认知机制，并采用去噪和自适应规划技术。实验表明，该方法在成功率(+37.69%)和搜索效率(+28.96% SPL)上显著优于基线，但与人类表现仍存在差距，突显了提升语义推理和空间探索能力的必要性。该研究为具身目标搜索奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T15:01:11Z
- **目录日期**: 2025-05-14
