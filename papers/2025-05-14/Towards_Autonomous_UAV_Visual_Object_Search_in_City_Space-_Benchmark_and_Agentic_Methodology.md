...
本文介绍了首个城市空中视觉目标搜索（AVOS）基准数据集CityAVOS，包含6类2420个任务，用于评估无人机自主搜索能力。针对复杂城市环境中语义冗余、相似物体区分和探索-利用平衡等挑战，研究团队提出PRPSearcher方法，采用多模态大语言模型模拟人类三层认知：1)构建物体中心动态语义图增强空间感知；2)基于语义吸引力的3D认知图进行目标推理；3)3D不确定性图实现平衡搜索。实验显示PRPSearcher显著优于基线方法（成功率+37.69%，搜索效率+28.96%），但与人类表现仍存在差距，凸显了语义推理和空间探索能力的提升需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T20:01:08Z
- **目录日期**: 2025-05-14
