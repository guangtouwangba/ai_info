...
本文提出CityAVOS——首个城市环境空中视觉目标搜索（AVOS）基准数据集，包含6类2,420个任务，用于评估无人机搜索能力。针对现有方法在复杂城市环境中的不足，作者开发了基于多模态大语言模型的PRPSearcher方法，模拟人类三层认知：通过动态语义地图增强空间感知，基于语义吸引力的3D认知地图进行目标推理，以及3D不确定性地图平衡探索与利用。该方法还包含去噪机制和自适应行动规划提示机制。实验显示PRPSearcher在成功率（+37.69%）和搜索效率（-46.40% NE）上显著优于基线，但与人类表现仍有差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T11:01:07Z
- **目录日期**: 2025-05-14
