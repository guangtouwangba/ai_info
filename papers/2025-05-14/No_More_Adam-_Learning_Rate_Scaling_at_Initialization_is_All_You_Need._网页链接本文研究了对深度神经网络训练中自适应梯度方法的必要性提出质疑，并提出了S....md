...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需二阶动量即可解决训练不平衡问题，内存使用比AdamW减少50%。实验显示，在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中，其性能匹配或超越AdamW，并显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法兼具超参数鲁棒性和广泛适用性，为替代自适应优化器提供了高效方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T14:04:57Z
- **目录日期**: 2025-05-14
