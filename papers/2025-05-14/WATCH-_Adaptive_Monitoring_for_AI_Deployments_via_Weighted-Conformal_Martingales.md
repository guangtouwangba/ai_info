...
在高风险环境中负责任地部署AI/ML系统不仅需要可靠性证明，还需持续监测以快速发现不安全行为。现有非参数变点检测方法（如共形测试鞅）存在局限：仅监测有限假设类别、无法在线适应变化或分析退化根源。本文提出加权共形测试鞅（WCTM）新框架，为控制误报率的同时在线监测数据分布异常变化奠定理论基础。具体算法可在线适应轻度协变量偏移，快速检测严重偏移（如概念漂移或超范围协变量偏移）。实验显示，该方法在真实数据集上优于当前最优基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T02:31:26Z
- **目录日期**: 2025-05-14
