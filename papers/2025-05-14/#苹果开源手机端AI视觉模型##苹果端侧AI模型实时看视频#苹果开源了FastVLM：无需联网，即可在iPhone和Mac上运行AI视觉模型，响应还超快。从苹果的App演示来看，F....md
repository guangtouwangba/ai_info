...
苹果开源了手机端AI视觉模型FastVLM，可在iPhone和Mac本地运行，无需联网。该模型能实时识别手势（如手指数量）、图像文字（791毫秒内识别便签）和生成图像描述（800毫秒内描述emoji）。技术优势包括：视觉token压缩至1/16，推理速度比LLaVA-OneVision快85倍，模型体积缩小3.4倍，专为Apple Silicon优化。采用混合架构（卷积+Transformer），在多项基准测试中超越同类模型。提供0.5B/1.5B/7B三种规格，支持iOS演示。代码和论文已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T09:02:52Z
- **目录日期**: 2025-05-14
