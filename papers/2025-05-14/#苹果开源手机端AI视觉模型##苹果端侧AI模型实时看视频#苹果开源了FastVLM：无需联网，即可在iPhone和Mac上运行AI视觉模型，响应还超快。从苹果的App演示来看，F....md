...
苹果开源了手机端AI视觉模型FastVLM，可在iPhone和Mac本地运行，无需联网。该模型支持手势识别（如实时识别手指数量）、图像文字识别（791毫秒内完成）和图像描述生成（800毫秒内响应）。技术亮点包括：视觉token优化（压缩至1/16）、推理速度快LLaVA-OneVision 85倍、模型体积缩小3.4倍、专为Apple Silicon芯片优化。采用混合架构（卷积+Transformer），在多项基准测试中超越同类模型。提供0.5B/1.5B/7B三种规格，已发布iOS演示应用和开源代码。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T20:03:43Z
- **目录日期**: 2025-05-14
