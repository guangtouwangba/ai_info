...
本文提出了一种轻量级解决方案RT-GAN（循环时序GAN），通过可调时序参数为基于单帧的结肠镜AI模型添加时序一致性，将训练需求降低5倍。针对结肠镜视频存储受限（通常只保存关键帧）导致现有模型缺乏时序一致性的问题，该方法在两种应用（褶皱分割和模拟视频生成）中验证了有效性，并发布了首个结肠镜时序数据集。相关代码、预训练模型和数据已在GitHub开源（https://github.com/nadeemlab/CEP）。该方案显著降低了时序模型训练的计算资源需求，推动了结肠镜AI从单帧分析向视频级理解的过渡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T20:02:32Z
- **目录日期**: 2025-05-14
