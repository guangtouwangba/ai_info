...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制（单视频可达数十GB）通常仅保存少量关键帧用于报告，导致现有AI模型主要基于单帧训练。针对结肠镜领域无监督域转换任务（如光学与虚拟CT结肠镜转换），现有方法通常先开发单帧模型，再通过修改架构从头训练时序一致性模型，但计算资源需求极高。本文提出轻量级RT-GAN（循环时序GAN），通过可调时序参数将训练需求降低5倍，并在结肠皱襞分割和模拟视频生成任务中验证效果。团队同步发布了首个结肠镜时序数据集及开源代码。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T22:02:26Z
- **目录日期**: 2025-05-14
