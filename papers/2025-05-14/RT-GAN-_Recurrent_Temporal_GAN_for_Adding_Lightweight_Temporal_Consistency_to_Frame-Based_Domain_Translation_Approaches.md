...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制（单视频可达数十GB）无法保存，仅保留少量关键帧用于报告和AI训练。现有结肠镜AI模型多基于单帧数据，开发新方法（如光学与虚拟结肠镜转换）时通常先处理单帧，再通过修改架构加入连续帧以提升时序一致性，但这显著增加计算和内存需求。本文提出轻量级RT-GAN模型，通过可调时序参数将训练需求降低5倍，并在结肠皱襞分割和模拟视频生成任务中验证效果。同时发布首个结肠镜时序数据集，相关代码与模型已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T11:02:37Z
- **目录日期**: 2025-05-14
