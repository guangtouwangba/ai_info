...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制无法保存，仅保留少量关键帧用于AI训练。现有方法多基于单帧处理，缺乏时间一致性。本文提出轻量级RT-GAN模型，通过可调时间参数将单帧方法升级为时序一致模型，训练资源需求降低80%。研究展示了该模型在结肠皱襞分割（漏检区域指示）和模拟视频生成两个挑战性任务中的有效性，并发布了首个结肠镜时序数据集。相关代码、预训练模型及数据已开源在Computational Endoscopy Platform GitHub。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T08:02:42Z
- **目录日期**: 2025-05-14
