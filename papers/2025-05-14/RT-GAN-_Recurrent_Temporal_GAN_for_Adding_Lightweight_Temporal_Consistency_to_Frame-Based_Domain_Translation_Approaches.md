...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制无法保存，仅保留少量关键帧用于AI训练。现有方法多基于单帧图像，缺乏时间一致性。本文提出轻量级RT-GAN模型，通过可调时间参数将时间一致性引入单帧方法，使训练资源需求降低5倍。该方法在结肠皱襞分割（反映漏检区域）和模拟视频生成两个任务中验证有效，并发布了首个结肠镜时序数据集。相关代码、预训练模型和数据集已在GitHub开源（https://github.com/nadeemlab/CEP），演示视频见https://youtu.be/UMVP-uIXwWk。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T19:02:23Z
- **目录日期**: 2025-05-14
