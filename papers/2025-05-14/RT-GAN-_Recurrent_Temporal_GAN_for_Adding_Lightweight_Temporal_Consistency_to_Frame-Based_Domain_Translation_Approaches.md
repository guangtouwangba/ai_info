...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制（单视频可达数十GB）通常仅保存少量关键帧用于报告，现有AI模型也基于这些静态帧训练。为开发结肠镜领域无监督域转换方法（如光学与虚拟/CT结肠镜转换），研究者通常先开发单帧模型，再通过修改架构从头训练时序一致性模型，但这显著增加计算资源消耗。本文提出轻量级RT-GAN框架，通过可调时序参数将单帧模型升级为时序一致性模型，训练需求降低80%，并在褶皱分割和模拟视频生成任务中验证效果，同时发布了首个结肠镜时序数据集和开源代码。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T06:02:33Z
- **目录日期**: 2025-05-14
