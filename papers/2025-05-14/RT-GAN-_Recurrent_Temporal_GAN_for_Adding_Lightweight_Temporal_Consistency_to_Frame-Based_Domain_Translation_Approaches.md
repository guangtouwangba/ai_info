...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制（单视频可达数十GB）通常不被保存，仅保留少量关键帧用于报告和AI训练。现有结肠镜AI模型多基于单帧数据开发，缺乏时间一致性。本文提出RT-GAN（循环时序生成对抗网络），通过可调时序参数为单帧方法添加时序一致性，将训练需求降低5倍。方法在结肠皱襞分割（检测遗漏表面）和模拟视频生成两个任务中验证有效，并发布了首个结肠镜时序数据集。代码、预训练模型已开源于Computational Endoscopy Platform GitHub。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T14:02:40Z
- **目录日期**: 2025-05-14
