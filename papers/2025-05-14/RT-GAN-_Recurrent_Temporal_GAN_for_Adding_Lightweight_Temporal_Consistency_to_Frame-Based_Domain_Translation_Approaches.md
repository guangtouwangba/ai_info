...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制无法保存，仅保留少量关键帧用于报告和AI训练。现有方法主要针对单帧图像开发，再扩展至连续帧以确保时间一致性，但计算和内存需求大幅增加。本文提出轻量级RT-GAN（循环时序GAN），通过可调时序参数将单帧方法升级为时序一致性模型，训练需求降低5倍。该方法在结肠皱襞分割（漏检区域指示）和模拟视频生成两个任务中验证效果，并发布首个结肠镜时序数据集。代码、预训练模型及数据集已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T10:02:41Z
- **目录日期**: 2025-05-14
