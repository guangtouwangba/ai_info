...
美国每年进行1400万例结肠镜检查，但由于存储限制，高清视频通常不被保存，仅保留少量关键帧用于报告和AI训练。现有方法多基于单帧处理，缺乏时间一致性。本文提出RT-GAN（循环时序生成对抗网络），通过可调时序参数为单帧方法添加时序一致性，将训练需求降低5倍。该方法在结肠皱襞分割（反映漏检区域）和模拟结肠镜视频生成两个任务中验证有效。团队同时发布了首个结肠镜时序数据集，相关代码、预训练模型已在GitHub开源（https://github.com/nadeemlab/CEP），补充视频见https://youtu.be/UMVP-uIXwWk。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T13:11:31Z
- **目录日期**: 2025-05-14
