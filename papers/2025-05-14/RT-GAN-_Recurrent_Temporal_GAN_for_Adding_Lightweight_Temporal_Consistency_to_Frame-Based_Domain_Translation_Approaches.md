...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制（单段视频可达数十GB）无法保存，仅保留少量关键帧用于报告和AI训练。现有结肠镜AI模型主要基于单帧图像，而开发新的无监督域转换方法（如光学与虚拟/CT结肠镜转换）时，通常先构建单帧模型，再通过修改架构从头训练时序一致性模型，但计算资源消耗巨大。本研究提出轻量级RT-GAN框架，通过可调时序参数将单帧模型升级为时序一致性模型，训练需求降低5倍，并在结肠皱襞分割和模拟视频生成任务中验证效果。同时发布了首个结肠镜时序数据集及开源代码。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T21:02:28Z
- **目录日期**: 2025-05-14
