...
MLE-Dojo是一个Gym风格的强化学习框架，用于系统训练、评估和改进基于大语言模型（LLM）的自主代理。它通过200多个真实Kaggle挑战构建交互式环境，支持数据预处理、架构搜索、超参数调优等机器学习工程（MLE）任务。与静态基准不同，MLE-Dojo提供结构化反馈循环，允许代理迭代调试和优化方案。评估显示当前LLM在长期方案生成和复杂错误修复上仍有局限。该框架支持监督微调和强化学习，具有可扩展性，并开源以促进下一代MLE代理的开发。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T02:30:21Z
- **目录日期**: 2025-05-14
