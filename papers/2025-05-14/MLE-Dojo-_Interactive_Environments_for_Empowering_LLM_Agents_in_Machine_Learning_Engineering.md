...
MLE-Dojo是一个Gym风格的强化学习框架，用于评估和改进大型语言模型（LLM）在机器学习工程（MLE）任务中的自主能力。基于200多个真实Kaggle挑战，它提供交互式环境，支持数据预处理、超参数调优等多样化任务，并通过监督微调和强化学习实现迭代优化。评估显示，当前LLM在长期解决方案生成和复杂错误修复上仍有局限。该框架开源且可扩展，支持多种数据源和工具，促进可复现性和社区协作，推动下一代MLE智能体的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T03:18:34Z
- **目录日期**: 2025-05-14
