...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，并针对扩散过程中时间一致性问题提出TREPA（时间表示对齐）技术，通过自监督视频模型增强生成帧的时间连续性。研究还改进了SyncNet训练框架，解决了其收敛问题，提升了唇同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他音频驱动的人像动画应用。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T00:05:42Z
- **目录日期**: 2025-05-14
