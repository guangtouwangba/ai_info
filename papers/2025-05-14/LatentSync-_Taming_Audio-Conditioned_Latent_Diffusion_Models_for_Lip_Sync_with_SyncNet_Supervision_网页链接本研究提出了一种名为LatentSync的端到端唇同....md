...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示，直接建模音频-视觉关系。针对扩散模型的时间一致性问题，提出时间表示对齐(TREPA)方法增强连贯性。同时优化SyncNet训练框架，改进其收敛性和准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。创新点包括：1)首个基于潜在扩散的唇同步方案；2)TREPA时间一致性增强；3)SyncNet训练优化方案。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T23:05:12Z
- **目录日期**: 2025-05-14
