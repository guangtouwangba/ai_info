...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。通过引入时间表示对齐(TREPA)方法增强生成帧的时间一致性，并优化SyncNet训练框架解决收敛问题。该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。研究创新点包括：1)直接建模音频-视觉相关性；2)解决扩散模型的时间一致性问题；3)改进SyncNet训练方法。这些发现可推广至其他唇同步和音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T08:05:52Z
- **目录日期**: 2025-05-14
