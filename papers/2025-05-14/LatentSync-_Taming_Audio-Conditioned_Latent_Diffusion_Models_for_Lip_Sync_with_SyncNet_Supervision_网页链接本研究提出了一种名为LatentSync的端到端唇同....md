...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，但发现扩散过程中存在时间一致性问题。为此，研究者开发了时间表示对齐(TREPA)方法，通过自监督视频模型提取的时间特征增强生成帧的时序一致性。同时改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进也可应用于其他音频驱动的人像动画系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T04:06:10Z
- **目录日期**: 2025-05-14
