...
本文提出了一种名为FPoTT的新型框架，用于解决车联网(IoV)中可扩展、实时且保护隐私的交通管理挑战。该框架利用开源大语言模型(LLMs)，通过动态提示优化机制提升轨迹预测能力。FPoTT采用双层联邦学习架构，结合轻量级边缘模型和云端LLMs，并整合了基于Transformer的合成数据生成器来增强训练。实验表明，使用Pythia-1B模型的FPoTT在真实数据上达到99.86%的预测准确率，同时在合成数据集上也表现优异。这一方案为智能交通生态系统提供了安全、自适应且可扩展的替代方案，克服了现有集中式方案的延迟、扩展性和隐私问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T21:01:31Z
- **目录日期**: 2025-05-14
