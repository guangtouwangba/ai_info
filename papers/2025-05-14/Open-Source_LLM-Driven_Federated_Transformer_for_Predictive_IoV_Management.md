...
本文提出了一种新型联邦学习框架FPoTT，用于解决车联网（IoV）中的实时、可扩展和隐私保护交通管理问题。该框架采用开源大语言模型（LLMs），通过动态提示优化机制提升轨迹预测精度，并采用双层联邦学习架构（轻量级边缘模型与云端LLMs结合）。FPoTT还集成了基于Transformer的合成数据生成器，扩展了NGSIM格式的训练场景。测试显示，使用Pythia-1B模型的FPoTT在真实数据上达到99.86%的预测准确率，为智能交通系统提供了安全、自适应且可扩展的解决方案，优于现有专有AI方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T11:01:30Z
- **目录日期**: 2025-05-14
