...
本文提出了一种名为FPoTT（联邦优化提示交通转换器）的新型框架，用于解决车联网（IoV）中的实时性、可扩展性和隐私保护问题。该框架利用开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性。FPoTT采用双层联邦学习架构，结合轻量级边缘模型和云端LLM，并整合了基于Transformer的合成数据生成器来增强训练。测试显示，FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能，为智能交通系统提供了安全、自适应且可扩展的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T16:01:29Z
- **目录日期**: 2025-05-14
