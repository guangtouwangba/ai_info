...
本文提出了一种新型联邦学习框架FPoTT，用于解决车联网（IoV）中的实时、可扩展和隐私保护的交通管理问题。该框架结合开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测精度，采用双层联邦学习架构（轻量级边缘模型实时推理+云端LLM全局智能），并整合基于Transformer的合成数据生成器来增强训练。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能，为智能交通系统提供了安全、自适应且可扩展的解决方案，优于现有专有AI方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T12:01:40Z
- **目录日期**: 2025-05-14
