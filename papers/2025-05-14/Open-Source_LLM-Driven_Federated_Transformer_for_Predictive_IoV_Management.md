...
本文提出了一种名为FPoTT的新型联邦学习框架，用于解决车联网(IoV)中实时、可扩展且保护隐私的交通管理挑战。该框架利用开源大语言模型(LLMs)，通过动态提示优化机制提升轨迹预测精度。FPoTT采用双层联邦学习架构，结合轻量级边缘模型进行实时推理和基于云的LLMs保留全局智能，并集成Transformer驱动的合成数据生成器来增强训练。实验表明，使用Pythia-1B模型的FPoTT在真实数据上达到99.86%的预测准确率，为智能交通生态系统提供了安全、自适应且可扩展的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T09:01:34Z
- **目录日期**: 2025-05-14
