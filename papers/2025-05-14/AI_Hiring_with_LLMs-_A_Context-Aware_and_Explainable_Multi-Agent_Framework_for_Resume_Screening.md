...
简历筛选是人才招聘中关键但耗时的环节。本研究提出基于大语言模型（LLM）的多智能体框架，通过四个核心模块（简历提取器、评估器、摘要生成器和分数格式化器）实现自动化处理。创新性地采用检索增强生成（RAG）技术，整合行业知识、专业认证等外部数据提升评估准确性。实验通过对比AI评分与HR人工评分验证效果，结果表明该框架能有效提高招聘效率，在保持客观性的同时实现规模化筛选，为智能化招聘流程提供了可行方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T11:01:40Z
- **目录日期**: 2025-05-14
