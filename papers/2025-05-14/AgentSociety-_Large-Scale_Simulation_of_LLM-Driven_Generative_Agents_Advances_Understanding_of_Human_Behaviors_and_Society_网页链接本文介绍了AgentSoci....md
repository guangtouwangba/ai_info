...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个AI代理的社会互动（500万次交互）。研究者用它研究四大社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。该系统模拟结果与现实世界数据高度吻合，验证了其模拟人类行为和社会机制的能力。该平台可作为传统社会科学研究方法（如调查、访谈）的有力补充，帮助研究者探索社会问题的模式和成因，为政策制定提供新工具。研究展示了LLM在社会模拟和计算社会科学中的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T16:05:16Z
- **目录日期**: 2025-05-14
