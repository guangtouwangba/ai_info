...
O-RAN联盟基于开放无线接入网络(O-RAN)原则定义了蜂窝网络的架构、接口和安全性要求，并引入了RAN智能控制器(RIC)通过rApps和xApps实现数据驱动的动态网络控制。然而，多个AI应用可能产生目标冲突(如吞吐量最大化与能耗最小化)，导致性能下降。为此，研究提出了PACIFISTA框架，通过沙盒测试、分层图和统计模型来检测和缓解xApps间的冲突。实验表明，相似目标的xApps可能导致16%吞吐量损失，冲突目标则可能造成30%性能下降。PACIFISTA能帮助运营商在部署前识别冲突应用，有效控制性能损失。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T07:02:27Z
- **目录日期**: 2025-05-14
