...
O-RAN联盟引入的RAN智能控制器(RIC)通过rApps和xApps实现蜂窝网络的智能动态控制，但不同AI应用的目标冲突可能导致资源分配矛盾。为此，研究提出PACIFISTA框架，通过沙盒测试和分层图结合统计模型来检测、评估和缓解O-RAN应用间的冲突。实验表明，即使目标相似的xApp也会造成16%吞吐量损失，而目标冲突的应用可能导致30%性能下降。PACIFISTA能在部署前预测冲突，帮助运营商识别问题应用，控制性能损失。该方案在Colosseum和OpenRAN Gym平台上验证有效。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T17:02:53Z
- **目录日期**: 2025-05-14
