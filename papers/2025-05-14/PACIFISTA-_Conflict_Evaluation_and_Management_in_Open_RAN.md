...
O-RAN联盟基于开放无线接入网络（O-RAN）原则定义了蜂窝网络的架构、接口和运行要求，并引入了RAN智能控制器（RICs）通过rApps和xApps实现动态控制。然而，不同AI应用的自主决策可能引发目标冲突（如吞吐量最大化与能耗最小化），导致性能下降。为此，研究提出PACIFISTA框架，通过沙盒测试和分层图结合统计模型来检测和缓解冲突。实验表明，冲突可能导致16%-30%的性能损失，而PACIFISTA能有效预测冲突，帮助运营商在部署前识别问题，维持网络稳定性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T10:02:30Z
- **目录日期**: 2025-05-14
