...
本文提出了一种新型大规模多智能体路径规划(MAPF)算法，适用于自动化仓库等动态环境。该算法采用有限时域分层分解框架，以滚动时域方式逐步规划：机器人先并行计算个体路径，再根据时空冲突和可达性动态分组。该框架支持冲突解决、即时执行与并行规划，相比离线算法显著降低响应时间。基准测试表明，该方法在保证高质量解的同时，首次行动时间缩短达60%，在不同问题规模和规划时域下均优于现有最优离线算法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-14T00:01:46Z
- **目录日期**: 2025-05-14
