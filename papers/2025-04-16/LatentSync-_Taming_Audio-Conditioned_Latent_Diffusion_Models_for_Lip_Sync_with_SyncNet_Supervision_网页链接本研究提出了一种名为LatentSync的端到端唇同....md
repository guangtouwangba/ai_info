...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示，克服了传统两阶段方法的局限。针对扩散模型的时间一致性问题，创新性提出TREPA（时间表示对齐）方法，利用自监督视频模型增强生成帧的时序连贯性。同时，研究解决了SyncNet收敛问题，通过优化模型架构、训练参数等显著提升其精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T13:13:39Z
- **目录日期**: 2025-04-16
