...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法直接建模音频-视觉相关性，利用Stable Diffusion的强大能力。针对扩散过程中时间一致性问题，提出时间表示对齐（TREPA）方法，通过自监督视频模型提取的时间表示增强一致性。研究还优化了SyncNet训练框架，改进了模型架构、超参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T05:04:20Z
- **目录日期**: 2025-04-16
