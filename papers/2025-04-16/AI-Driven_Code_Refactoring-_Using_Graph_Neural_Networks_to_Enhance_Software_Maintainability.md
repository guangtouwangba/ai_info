...
本研究利用图神经网络（GNN）和抽象语法树（AST）进行代码重构，显著提升软件可维护性。通过分析CodeSearchNet的200万代码片段和7.5万份GitHub Python文件，GNN在准确率（92%）、降低复杂度（35%）和耦合度（33%）上均优于SonarQube（78%，16%）和决策树（85%，25%）。预处理修复了60%的语法错误。结果表明，GNN为代码优化提供了高效、可扩展的AI解决方案，对软件工程具有重要意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T01:30:18Z
- **目录日期**: 2025-04-16
