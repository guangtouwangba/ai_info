...
Kimi-VL是一款高效的开源视觉语言模型（VLM），采用混合专家（MoE）架构，仅激活28亿参数即可实现强大性能。它在多轮代理任务、图像/视频理解、OCR、数学推理等领域表现优异，媲美GPT-4o等顶尖模型。支持128K长上下文处理，在LongVideoBench和MMLongBench-Doc上得分突出。其原生分辨率视觉编码器MoonViT能处理超高分辨率输入，同时保持较低计算成本。进阶版Kimi-VL-Thinking通过长链思维微调和强化学习，在MMMU、MathVision等任务中刷新高效多模态推理的标杆。模型和代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T07:02:59Z
- **目录日期**: 2025-04-16
