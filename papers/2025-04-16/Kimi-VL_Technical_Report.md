...
Kimi-VL是一款高效的开源视觉语言模型（VLM），采用混合专家（MoE）架构，仅激活28亿参数。它在多模态推理、长上下文理解和智能体任务中表现优异，性能媲美旗舰模型，并在图像/视频理解、OCR、数学推理等任务中超越GPT-4o等先进模型。支持128K长上下文窗口，在LongVideoBench和MMLongBench-Doc上得分突出。其原生分辨率视觉编码器MoonViT能处理超高清输入，在InfoVQA等任务中表现优异且计算成本低。进阶版Kimi-VL-Thinking通过长思维链训练，在MMMU等复杂推理任务中刷新高效多模态模型的标杆。模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T14:03:09Z
- **目录日期**: 2025-04-16
