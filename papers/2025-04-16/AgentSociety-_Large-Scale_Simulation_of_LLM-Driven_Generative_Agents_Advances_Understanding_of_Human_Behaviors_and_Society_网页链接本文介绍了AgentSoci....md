...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它结合了大型语言模型驱动的代理、真实社会环境和大规模模拟引擎。研究者通过模拟超过1万个代理及其500万次互动，探索了极化现象、信息传播、基本收入政策和外部冲击等四个关键社会问题。模拟结果与现实世界实验数据吻合，验证了该系统捕捉人类行为和社会机制的能力。该工具展示了作为社会科学研究和政策制定辅助手段的潜力，能够补充传统研究方法（如调查和访谈），帮助理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T02:33:14Z
- **目录日期**: 2025-04-16
