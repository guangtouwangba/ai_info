...
这篇论文介绍了AgentSociety，这是一个利用大型语言模型（LLM）驱动的大规模社会模拟器，能够模拟超过1万个代理的社会互动（500万次）。研究者用它作为计算社会科学实验平台，研究了社会极化、信息传播、基本收入政策和自然灾害影响等关键问题。模拟结果与现实世界数据高度吻合，验证了该工具在分析人类行为和社会机制方面的有效性。AgentSociety展现了作为社会科学研究和政策制定辅助工具的潜力，能够补充传统研究方法（如调查和访谈）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T09:05:53Z
- **目录日期**: 2025-04-16
