...
AgentSociety是一个基于大语言模型的大规模社会模拟平台，可模拟1万多个AI代理的社会互动（500万次）。研究者用它分析四大社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。模拟结果与现实数据高度吻合，验证了该平台对人类行为和社会机制的捕捉能力。该系统可作为社会科学研究和政策制定的创新工具，补充传统研究方法（如调查和实验），帮助深入理解社会问题的成因和解决路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T21:05:12Z
- **目录日期**: 2025-04-16
