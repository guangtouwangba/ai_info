...
PartField是一种前馈学习方法，用于学习基于部件的3D特征，无需依赖预定义模板或文本标签即可捕捉部件及其层次结构的概念，适用于多种模态的开放世界3D形状。该方法在推理时仅需一次3D前馈计算，显著提升了运行速度和鲁棒性。通过对比学习框架，模型从标注数据集和大规模无监督数据集的图像分割中蒸馏2D和3D部件提案，生成连续特征场，可聚类得到层次化部件分解。实验表明，PartField比同类方法准确率提升20%，速度提升数个数量级，并支持跨形状一致性任务（如共分割和对应关系）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T12:01:58Z
- **目录日期**: 2025-04-16
