...
PartField是一种基于部件学习的3D特征提取方法，通过前馈网络自动捕捉部件概念及其层级结构，无需依赖预定义模板或文本标签，适用于开放世界的多种3D数据模态。该方法仅需单次3D前馈推理，显著提升了运行速度和鲁棒性。通过对比学习从标注数据集和大规模无监督图像分割中蒸馏2D/3D部件提案，生成连续特征场，可聚类得到层级部件分解。实验显示，PartField比现有类别无关部件分割方法准确率提升20%，速度提升数个量级。此外，学习到的特征场具有跨形状一致性，支持共分割、对应关系等任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T15:02:10Z
- **目录日期**: 2025-04-16
