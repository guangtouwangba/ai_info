...
PartField是一种前馈式学习方法，用于提取基于部件的3D特征，无需依赖预定义模板或文本标签即可捕捉部件及其层级结构的概念，适用于多种模态的开放世界3D形状。该方法仅需单次3D前馈推理，显著提升了运行速度和鲁棒性。通过对比学习，模型从标注数据集和大规模无监督数据集的图像分割中蒸馏出2D/3D部件提案，生成连续特征场，可聚类为层级部件分解。实验表明，PartField的准确率最高提升20%，且速度比其他无类别部件分割方法快数个数量级。此外，该特征场支持跨形状一致性任务，如协同分割和对应关系。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T23:01:43Z
- **目录日期**: 2025-04-16
