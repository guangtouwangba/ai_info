...
PartField是一种基于部件学习的3D特征提取方法，无需依赖预定义模板或文本标签即可捕捉部件及其层次结构概念，适用于多种模态的开放世界3D形状。该方法仅需单次前向传播即可推理，显著提升了运行速度和鲁棒性。通过对比学习从标注数据集和大规模无监督数据集中蒸馏2D/3D部件提案，生成连续特征场，可聚类得到层次化部件分解。实验表明，PartField的准确率最高提升20%，且速度比其他无类别部件分割方法快数个数量级。该特征场还支持跨形状一致性任务，如协同分割和对应关系匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T06:01:58Z
- **目录日期**: 2025-04-16
