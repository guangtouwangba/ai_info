...
PartField是一种前馈式3D部件特征学习方法，无需依赖预定义模板或文本标签即可捕捉部件及其层级结构的通用概念，适用于多种模态的开放世界3D形状。该方法仅需单次3D前馈推理，显著提升了运行速度和鲁棒性。通过对比学习框架，模型从标注数据集和大规模无监督数据集的图像分割中蒸馏出2D/3D部件提案，生成连续特征场，可聚类得到层级部件分解。实验表明，PartField比现有类别无关部件分割方法准确率提升20%，速度提升数个量级。该特征场具有跨形状一致性，支持协同分割和对应关系等任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T11:01:51Z
- **目录日期**: 2025-04-16
