...
MCP协议作为连接大语言模型(LLM)与第三方工具的主流接口，在加速AI能力扩展的同时存在显著安全隐患。主要问题包括：安全机制薄弱导致数据泄露风险；开放执行环境易被恶意代码利用；人机边界模糊增加误操作可能；纯文本传输效率低下；多工具环境降低模型准确性；以及敏感数据易被不当访问。虽然MCP统一了AI调用接口，但需强化默认安全设置、明确权限边界，并提高用户对模型能力的认知，否则可能引发严重安全问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T00:04:01Z
- **目录日期**: 2025-04-16
