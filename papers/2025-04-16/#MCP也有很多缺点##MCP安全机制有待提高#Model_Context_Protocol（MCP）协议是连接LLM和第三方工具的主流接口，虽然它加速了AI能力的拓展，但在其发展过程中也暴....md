...
MCP协议作为连接大语言模型与第三方工具的主流接口，存在六大安全隐患：1)安全认证机制不统一，易引发数据泄露和第四方攻击；2)本地执行环境过于开放，难以防范恶意代码；3)人机边界模糊导致误操作风险；4)文本传输效率低且成本高；5)多工具协同易引发模型错误输出；6)敏感数据自动处理带来隐私泄露风险。作者指出协议本身设计合理，但需强化默认安全设置、明确权限边界，并提升用户对AI能力的认知，否则可能引发不可控风险。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T02:31:50Z
- **目录日期**: 2025-04-16
