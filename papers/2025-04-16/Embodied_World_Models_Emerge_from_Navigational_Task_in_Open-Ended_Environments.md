...
本研究探讨神经网络能否通过主动交互自主内化空间概念。研究者结合门控循环单元(GRU)和元强化学习(Meta-RL)，发现智能体可学习编码方向、距离和避障等空间属性。采用混合动力系统(HDS)建模智能体-环境交互，揭示了对应最优导航策略的稳定极限环。通过岭表示将导航路径映射到固定维度行为空间，并与神经状态比较。典型相关分析(CCA)证实二者高度一致，表明神经状态主动编码空间知识。干预实验显示特定神经维度与导航性能存在因果关系，为构建可解释的适应性模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T08:02:56Z
- **目录日期**: 2025-04-16
