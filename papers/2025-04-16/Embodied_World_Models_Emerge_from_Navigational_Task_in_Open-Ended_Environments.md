...
本研究探讨神经网络如何通过主动交互自主内化空间概念，聚焦平面导航任务。结合门控循环单元(GRU)和元强化学习(Meta-RL)，研究发现智能体能编码方向、距离和避障等空间属性。采用混合动力系统(HDS)建模智能体-环境交互，揭示了对应最优导航策略的稳定极限环。通过岭表示将路径映射到固定维度行为空间，并与神经状态对比，典型相关分析(CCA)证实两者高度对齐，表明神经状态主动编码空间知识。干预实验显示特定神经维度与导航性能存在因果关系，为构建可解释的适应性模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T07:02:48Z
- **目录日期**: 2025-04-16
