...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需二阶动量计算，既解决了训练不平衡问题，又将内存使用减半。实验表明，在Transformer任务、ImageNet分类和LLM预训练中，其性能媲美或超越AdamW，特别在GPT-2和Llama2-7B训练中分别节省5.93GB和25.15GB内存。该方法对超参数鲁棒，在LoRA微调等场景也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T18:04:45Z
- **目录日期**: 2025-04-16
