...
该研究提出SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题，并将内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中表现优于AdamW，尤其在大模型场景下显著节省内存（如为Llama2-7B节省25GB）。同时，该方法在LoRA微调等任务中展现优越性能，且对超参数变化具有强鲁棒性，为深度学习优化提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T02:32:38Z
- **目录日期**: 2025-04-16
