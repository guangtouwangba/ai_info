...
这篇论文针对代码生成大语言模型(codeLLMs)提出了两个关键贡献：1)创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次关注模型输出与人类偏好的对齐；2)构建了20B标记的SynCode-Instruct合成指令数据集用于微调验证。研究发现现有基于执行的评估与人类偏好存在差距，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著，凸显了代码生成任务中人类偏好对齐的重要性。研究为提升代码LLMs的实际应用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T12:06:26Z
- **目录日期**: 2025-04-16
