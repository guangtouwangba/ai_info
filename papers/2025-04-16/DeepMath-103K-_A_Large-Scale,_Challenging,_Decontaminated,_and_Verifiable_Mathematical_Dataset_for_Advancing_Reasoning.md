...
DeepMath-103K是一个专为强化学习（RL）设计的大规模数学推理数据集，包含约10.3万道高难度题目（主要为5-9级），填补了现有数据在挑战性、可验证答案格式和避免评估污染方面的不足。该数据集通过严格筛选、去污染处理，并提供标准答案及三种AI生成的解题方案，适用于监督微调或蒸馏等多种训练范式。实验表明，基于DeepMath-103K训练的模型在复杂数学基准测试中表现显著提升。数据集已开源（GitHub链接），旨在推动AI推理能力的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T12:01:48Z
- **目录日期**: 2025-04-16
