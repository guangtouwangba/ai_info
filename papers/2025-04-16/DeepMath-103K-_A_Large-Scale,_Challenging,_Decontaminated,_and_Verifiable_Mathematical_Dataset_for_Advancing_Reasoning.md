...
DeepMath-103K是一个专为强化学习（RL）设计的大规模数学推理数据集，包含约10.3万道高难度题目（主要为5-9级），填补了现有数据在挑战性和可验证答案格式上的不足。该数据集通过严格筛选、去污染处理，并提供每道题的最终答案和三种AI生成解法，适用于监督微调、蒸馏等多种训练范式。实验表明，基于该数据集训练的模型在复杂数学基准测试中表现显著提升。数据集已开源，旨在推动AI推理能力的发展。论文链接：https://github.com/zwhe99/DeepMath。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T16:02:01Z
- **目录日期**: 2025-04-16
