...
该研究提出了首个结合联邦学习(FL)与社会学习网络(SLN)的框架，用于预测学生间未来互动。传统集中式训练存在隐私风险且无法识别跨教室的共性互动模式。通过FL，该框架能在保护数据隐私的前提下，实现多教室协同训练模型，并采用个性化技术适应各教室独特互动特征。结果表明，该方法能有效捕捉SLN中共享和特定的学生互动表征。研究还运用可解释AI技术识别影响不同教室互动形成的关键因素，首次在隐私保护的分布式机器学习框架下揭示了社会学习互动的驱动机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T01:29:30Z
- **目录日期**: 2025-04-16
