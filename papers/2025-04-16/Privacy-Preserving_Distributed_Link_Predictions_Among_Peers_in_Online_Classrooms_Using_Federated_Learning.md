...
该研究提出了首个结合联邦学习(FL)和社会学习网络(SLN)的框架，用于预测学生未来互动。传统集中式训练面临隐私限制和跨教室模式识别困难，而该分布式方法能在保护数据隐私(无需原始数据集中)的同时，通过多教室协作训练提升模型性能。研究采用模型个性化技术适应不同教室的互动特征，成功捕捉了SLN中共性和特定的交互模式。通过可解释AI技术，模型揭示了影响不同教室链接形成的关键因素，首次在隐私保护的分布式机器学习框架下解析了社会学习互动的驱动机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T00:02:11Z
- **目录日期**: 2025-04-16
