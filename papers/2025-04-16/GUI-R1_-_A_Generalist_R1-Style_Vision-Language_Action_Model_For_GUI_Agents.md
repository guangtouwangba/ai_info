...
现有GUI代理主要依赖大规模视觉语言模型（LVLMs）的监督微调训练，但这种方法需要大量训练数据且泛化能力有限。受强化微调（RFT）在大语言模型中的成功启发，研究者提出首个强化学习框架\name，通过统一动作空间规则建模提升LVLMs在真实GUI任务中的表现。该框架仅需0.02%数据（3K vs. 13M），在跨8个移动、桌面和网页基准测试中超越SOTA方法OS-Atlas，展示了强化学习在提升LVLMs执行GUI任务能力方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T02:29:25Z
- **目录日期**: 2025-04-16
