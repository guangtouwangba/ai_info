...
本文提出了一种静默、自稳定的分布式排名协议，适用于基于随机配对交互的群体协议模型。该协议为n个匿名代理分配唯一排名{1,...,n}，同时可作为自稳定的领导者选举协议。核心创新是一个仅需n+O(log n)状态的高效非自稳定排名算法。最终协议在O(n² log n)次交互内以高概率稳定，总状态数为n+O(log² n)，达到渐进最优稳定时间。相比现有最佳方案（需要n+Ω(n)状态），本协议将额外状态数（overhead states）从线性降低到对数级，实现了指数级改进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T02:30:06Z
- **目录日期**: 2025-04-16
