...
本文提出了一种在部分可观察环境中（gPOMDPs框架下）通过参数化信念状态查询（BSQ）策略来表达用户约束和偏好的新方法。理论分析表明，虽然参数化BSQ策略的期望成本函数是非凸的，但其分段常数特性产生了有限的离散参数搜索空间。基于此，研究开发了能保证用户对齐的新算法，并证明其在极限情况下收敛于最优用户对齐行为。实验验证了参数化BSQ策略在部分可观察环境中实现用户对齐规划的可行性。该框架为现实世界中需兼顾部分可观察性和用户需求的规划问题提供了解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T16:03:38Z
- **目录日期**: 2025-04-16
