...
本文提出了一种在部分可观测环境中（gPOMDPs）通过参数化信念状态查询（BSQ）策略来表达用户约束和偏好的新框架。研究发现，参数化BSQ策略的期望成本函数虽非凸，但分段恒定，从而形成有限的离散参数搜索空间。基于此，作者开发了能保证用户对齐的新算法，理论证明该算法在极限情况下收敛于最优用户对齐行为。实验表明，参数化BSQ策略为部分可观测环境中的用户对齐规划提供了计算可行的解决方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-16T15:03:34Z
- **目录日期**: 2025-04-16
