...
EcoAgent是一个边缘-云协同的多智能体框架，旨在解决基于云的大语言模型（LLM）高延迟、高成本的问题，同时避免边缘部署的小模型（SLM）通用性不足的缺陷。它通过云端规划智能体和两个边缘智能体（执行智能体和观察智能体）形成闭环协作：观察智能体通过预理解模块压缩屏幕图像为文本以减少计算开销，规划智能体在失败时通过反思模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了LLM的计算消耗，实现了高效的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T09:01:51Z
- **目录日期**: 2025-05-09
