...
EcoAgent是一个边缘-云协作的多智能体框架，旨在提升移动自动化效率。它通过云端规划智能体和两个边缘智能体（执行智能体和观察智能体）形成闭环协作。观察智能体使用预理解模块压缩屏幕图像为简洁文本，减少大语言模型（MLLM）的token消耗；若任务失败，规划智能体通过反射模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时，显著降低了MLLM的token使用量，实现了高效实用的移动自动化。该框架解决了云智能体延迟高、成本高以及边缘微调模型通用性不足的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T15:01:47Z
- **目录日期**: 2025-05-09
