...
EcoAgent是一个边缘-云协作的多智能体框架，旨在解决基于云的大型语言模型（LLM）的高延迟和高成本问题。它包含一个云端的规划智能体和两个边缘智能体：执行智能体负责操作执行，观察智能体通过预理解模块压缩屏幕图像为简洁文本以减少令牌消耗。若任务失败，规划智能体通过反射模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著减少了令牌使用，实现了高效、实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T06:01:59Z
- **目录日期**: 2025-05-09
