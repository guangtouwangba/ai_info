...
本文研究了无限标记线上两个异步唤醒的移动代理的确定性问题。代理初始距离为D，节点标签为正整数，最大标签为ℓ_max。Miller和Pelc此前证明了最优时间复杂度下界为Ω(D log* ℓ_max)，并给出了D已知时的最优算法。对于D未知的情况，他们算法复杂度为O(D²(log* ℓ_max)³)。本文改进了这一结果，提出了D未知情况下的最优算法，时间复杂度为O(D log* ℓ_max)，实际上可达O(D log* ℓ_min)，其中ℓ_min是起始位置O(D)距离内的最小标签。该结果完全匹配了下界。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T04:02:56Z
- **目录日期**: 2025-05-09
