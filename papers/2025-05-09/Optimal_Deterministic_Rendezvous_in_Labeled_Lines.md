...
本文研究了无限标记线上两个异步唤醒智能体的确定性会合问题。在初始距离D、最大标签ℓ_max条件下，Miller和Pelc此前证明了Ω(D log* ℓ_max)的时间下界，并给出了D已知时的最优算法O(D log* ℓ_max)。针对D未知的情况，本文改进了原有O(D²(log* ℓ_max)³)的复杂度，提出了一种新的确定性算法，将时间复杂度优化至紧致的O(D log* ℓ_max)。特别地，该算法实际能在O(D log* ℓ_min)轮内完成会合，其中ℓ_min表示起始位置O(D)距离内最小节点标签。这一结果完全匹配了下界，解决了该问题的开放性问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T02:31:11Z
- **目录日期**: 2025-05-09
