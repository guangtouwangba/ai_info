...
本文研究了在无限标记线上两个异步唤醒的移动代理的确定性和会问题。已知最优复杂度为O(D log* ℓ_max)，其中D是初始距离，ℓ_max是起点标签最大值。Miller和Pelc之前对未知D的情况给出了O(D²(log* ℓ_max)³)的算法。本文改进这一结果，提出了一个紧致的O(D log* ℓ_max)算法，实际上能达到O(D log* ℓ_min)复杂度，其中ℓ_min是起点附近O(D)范围内最小标签值。该结果解决了未知D时的最优和会问题，与已知D时的最优复杂度相匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T00:02:20Z
- **目录日期**: 2025-05-09
