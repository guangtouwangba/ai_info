...
本文介绍了MultiMind框架，这是首个将多模态信息整合到社交推理游戏（如"一夜终极狼人杀"）中的AI代理系统。该框架通过分析面部表情、语音语调等非语言线索，结合语言内容进行推理，并采用心理理论（ToM）模型追踪玩家间的相互怀疑程度。通过结合蒙特卡洛树搜索（MCTS），MultiMind能制定最小化自身嫌疑的沟通策略。实验表明，该系统在AI对战和人类玩家测试中均表现出色。这项研究推动了大型语言模型在多模态社交推理方面的发展，使其更接近人类的社会互动能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T08:02:27Z
- **目录日期**: 2025-05-09
