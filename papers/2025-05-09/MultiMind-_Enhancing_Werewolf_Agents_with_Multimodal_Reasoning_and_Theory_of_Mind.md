...
本文介绍了MultiMind框架，这是首个将多模态信息整合到社交推理游戏（如《一夜终极狼人杀》）中的AI代理。该框架通过分析面部表情、语音语调及文本内容，结合心理理论（ToM）模型追踪玩家间的相互怀疑程度，并利用蒙特卡洛树搜索（MCTS）优化沟通策略以降低自身嫌疑。实验表明，MultiMind在AI对战和人类玩家测试中均表现优异，显著提升了大型语言模型（LLM）在多模态社交推理中类人化的决策能力，突破了传统纯文本分析的局限性。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T22:02:05Z
- **目录日期**: 2025-05-09
