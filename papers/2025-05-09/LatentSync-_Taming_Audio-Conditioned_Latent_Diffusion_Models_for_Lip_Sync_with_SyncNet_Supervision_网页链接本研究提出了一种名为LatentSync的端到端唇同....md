...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法（时间表示对齐），通过自监督视频模型增强生成帧的时间连贯性。同时，论文系统分析了SyncNet收敛问题，优化了模型架构、训练参数等关键因素，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。该研究不仅改进了唇同步效果，其SyncNet优化经验还可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T17:05:07Z
- **目录日期**: 2025-05-09
