...
本文提出了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE。该方法通过三个关键创新提升数据同化性能：1) 采用类似传统集合数据同化的"集合即智能体"设计，保持计算效率；2) 通过推进多个集合成员进行不确定性量化，超越简单均值优化；3) 引入原始-对偶优化策略和动作空间约束，确保物理约束满足。在具有强非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，与约束EnKF性能相当但计算量大幅降低，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T12:01:31Z
- **目录日期**: 2025-05-09
