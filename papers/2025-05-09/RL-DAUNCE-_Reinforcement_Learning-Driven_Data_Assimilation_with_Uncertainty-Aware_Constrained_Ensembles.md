...
该研究提出了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE。该方法通过三个创新点提升数据同化性能：(1)采用与集合成员对应的智能体设计，保持计算效率；(2)通过多集合成员推进实现不确定性量化；(3)利用"集合即智能体"设计在过程中施加物理约束。研究采用原始-对偶优化策略和动作空间约束确保物理一致性。在具有非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与计算量更大的约束EnKF相当，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T10:01:37Z
- **目录日期**: 2025-05-09
