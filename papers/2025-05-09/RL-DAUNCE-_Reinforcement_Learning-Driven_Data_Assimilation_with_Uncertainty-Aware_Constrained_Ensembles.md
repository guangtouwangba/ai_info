...
本文提出RL-DAUNCE，一种基于强化学习的数据同化新方法。该方法通过三个创新点提升性能：1）将智能体设计为传统同化方法的集合成员，保持计算效率；2）强调不确定性量化，超越简单均值优化；3）通过集合智能体设计在过程中实施物理约束。采用原始-对偶优化策略动态惩罚奖励函数确保约束满足，同时限制动作空间以遵守状态变量边界。在具有强非高斯特征的Madden-Julian振荡测试中，RL-DAUNCE显著优于标准集合卡尔曼滤波，性能与约束EnKF相当但计算量更少，能更好恢复间歇信号和捕捉极端事件。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T17:01:27Z
- **目录日期**: 2025-05-09
