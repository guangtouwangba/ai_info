...
研究人员开发了一种基于强化学习的新型数据同化方法RL-DAUNCE，通过三个关键创新提升传统方法：1) 采用类似集合成员的智能体设计保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 在训练过程中通过原始-对偶优化策略和动作空间约束强制满足物理限制。该方法在具有强非高斯特性和多重物理约束的Madden-Julian振荡案例中表现优异，不仅显著优于标准集合卡尔曼滤波(EnKF)，其性能与计算量更大的约束EnKF相当，尤其在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出，同时计算效率更高。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T07:01:33Z
- **目录日期**: 2025-05-09
