...
研究人员开发了名为RL-DAUNCE的新型强化学习方法，用于改进数据同化过程。该方法通过三个创新点：1）采用类似传统集合成员的智能体结构保持计算效率；2）通过多集合成员推进实现不确定性量化；3）利用"集合即智能体"设计在过程中实施物理约束。研究采用原始-对偶优化策略确保约束满足，并通过限制动作空间保证状态变量边界。在模拟马登-朱利安振荡（具有强非高斯特征）时，RL-DAUNCE显著优于标准集合卡尔曼滤波，性能与约束版集合卡尔曼滤波相当，但计算量大幅减少，尤其在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T16:01:39Z
- **目录日期**: 2025-05-09
