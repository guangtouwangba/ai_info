...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE。该方法通过三个创新点提升数据同化性能：1) 采用类似传统集合同化的"智能体即集合成员"设计；2) 强调不确定性量化而非仅优化均值状态；3) 通过原始-对偶优化策略和动作空间约束保证物理一致性。在模拟Madden-Julian振荡的实验中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，其性能与计算量更大的约束EnKF相当，能更好地恢复间歇信号、捕捉极端事件和量化不确定性，同时保持计算高效性。该方法为处理强非高斯特征和物理约束问题提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T19:01:19Z
- **目录日期**: 2025-05-09
