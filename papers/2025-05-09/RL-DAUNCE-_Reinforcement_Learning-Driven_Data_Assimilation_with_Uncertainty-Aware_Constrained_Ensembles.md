...
本文提出了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE，通过三个关键创新提升数据同化性能：(1)采用与集合成员对应的智能体设计保持计算效率；(2)通过多集合成员推进实现不确定性量化；(3)利用"集合即智能体"设计在过程中强制物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数确保约束满足，并通过限制动作空间保证状态变量边界。在具有强非高斯特性和多物理约束的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能媲美约束EnKF但计算量大幅降低，尤其在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T22:01:24Z
- **目录日期**: 2025-05-09
