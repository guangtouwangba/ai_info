...
本文提出了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE。该方法通过三个关键创新：1) 采用类似传统集合数据同化的"集合即智能体"设计，保持计算效率；2) 强调不确定性量化，超越简单的平均状态优化；3) 通过原始对偶优化策略和动作空间约束确保物理一致性。在具有强非高斯特征和多重物理约束的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，与约束EnKF性能相当，但计算成本更低，特别在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T14:01:29Z
- **目录日期**: 2025-05-09
