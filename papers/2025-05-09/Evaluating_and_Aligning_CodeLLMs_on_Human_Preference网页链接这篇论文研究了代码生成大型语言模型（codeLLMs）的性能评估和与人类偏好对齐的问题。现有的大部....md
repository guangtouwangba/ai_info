...
这篇论文针对代码生成大模型(codeLLMs)与人类偏好对齐的问题，提出了CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种语言)和SynCode-Instruct合成指令数据集(近200亿token)。研究发现：1)传统基于执行的评估与人类偏好存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显。研究强调了对齐人类偏好的重要性，并通过大规模实验验证了合成指令微调的有效性。该工作为提升代码生成模型的实际应用价值提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T19:05:16Z
- **目录日期**: 2025-05-09
