...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本，覆盖40类任务和44种语言，重点评估模型输出与人类偏好的匹配程度。研究同时构建了20B标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现：1)传统执行正确性基准与人类偏好评估存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面仍有显著差距。研究强调了代码生成模型除功能性外，还需重视用户体验优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T23:05:00Z
- **目录日期**: 2025-05-09
