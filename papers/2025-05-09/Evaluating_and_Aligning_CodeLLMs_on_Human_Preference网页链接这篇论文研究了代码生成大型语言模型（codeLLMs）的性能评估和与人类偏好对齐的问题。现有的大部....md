...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，重点评估模型输出与人类偏好的匹配度。研究收集了397个涵盖40类编程任务和44种语言的真实样本，并构建20B标记的合成指令数据集SynCode-Instruct。实验发现传统执行基准与人类偏好评估存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面表现差距显著。研究强调了除代码正确性外，模型输出符合开发者实际需求的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T03:22:54Z
- **目录日期**: 2025-05-09
