...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令集验证微调效果。实验发现：传统执行正确性基准与人类偏好存在显著差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐上差距明显。研究凸显了除代码正确性外，符合开发者习惯的代码风格同样重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T15:06:04Z
- **目录日期**: 2025-05-09
