...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer、ImageNet-1K分类和GPT-2预训练等任务中表现相当或更优，尤其在大模型训练中节省显著内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法还展现出超参数鲁棒性和广泛适用性，在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T14:04:45Z
- **目录日期**: 2025-05-09
