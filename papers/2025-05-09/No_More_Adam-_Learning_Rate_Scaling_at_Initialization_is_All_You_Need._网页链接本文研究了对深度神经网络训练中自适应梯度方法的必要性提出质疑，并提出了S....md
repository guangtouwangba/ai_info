...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化阶段根据各参数组的梯度信噪比（g-SNR）调整学习率，有效解决了训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练等任务中表现相当或更优，尤其在大模型训练中显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法还展现出良好的超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T21:04:18Z
- **目录日期**: 2025-05-09
