...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法基于带动量的随机梯度下降（SGDM），通过初始化时按梯度信噪比（g-SNR）调整不同参数组的学习率，无需二阶动量计算即可避免训练失衡。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或媲美AdamW，同时内存占用减半（如为GPT-2节省5.93GB）。其优势还包括超参数鲁棒性、适用于LoRA微调等场景，显著提升了训练效率与资源利用率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T11:04:38Z
- **目录日期**: 2025-05-09
