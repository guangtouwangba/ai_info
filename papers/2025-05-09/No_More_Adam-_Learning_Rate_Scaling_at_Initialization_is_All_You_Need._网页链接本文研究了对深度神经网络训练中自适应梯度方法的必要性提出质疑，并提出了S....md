...
该论文提出SGD-SaI方法，质疑自适应梯度方法(如Adam)的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题，同时将优化器内存使用减半。实验表明，在Transformer任务、ImageNet分类及大模型(GPT-2/Llama2)预训练中，其性能等同或优于AdamW，且内存效率显著提升(如为Llama2-7B节省25.15GB)。该方法对超参数鲁棒，在LoRA微调等场景也表现优异，为深度学习优化提供了更高效简洁的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T15:04:45Z
- **目录日期**: 2025-05-09
