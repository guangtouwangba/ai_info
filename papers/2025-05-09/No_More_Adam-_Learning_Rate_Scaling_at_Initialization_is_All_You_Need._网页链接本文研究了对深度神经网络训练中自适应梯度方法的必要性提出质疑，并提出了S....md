...
这篇论文提出了SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。该方法改进带动量SGD，通过初始化时依据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需二阶动量即可平衡训练。相比AdamW，内存用量减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更好，尤其适合大模型（如GPT-2节省5.93GB，Llama2-7B节省25.15GB内存）。同时，在LoRA微调等任务中优于现有优化器，且对超参数变化更鲁棒。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T05:04:15Z
- **目录日期**: 2025-05-09
