...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟一个受外部驱动的模型来预测混沌信号的未来状态。研究发现了一种此前被忽视的优越动力学机制，该机制在不同物理参数和推断任务中均表现出稳健的最优性。最优计算状态位于临界阻尼阈值以下，涉及多阶段的微观动力学弛豫过程。系统在混沌驱动下表现出适应性，其响应机制多样，包括准稳态与高度非线性动态的快速切换。相干与非相干动力学共同作用，且不同时空尺度下的相关性可指示最佳性能状态。该模型为理解非平衡多体物理中的计算提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T08:02:16Z
- **目录日期**: 2025-05-09
