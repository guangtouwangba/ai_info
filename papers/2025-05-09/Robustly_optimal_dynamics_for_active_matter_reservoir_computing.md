...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟外部驱动的混沌信号预测模型，发现了一个此前被忽视的优异动力学机制。这一机制在多种物理参数和任务中表现稳健且最优，其核心在于系统固有的多阶段弛豫能力。最优计算状态位于临界阻尼阈值之下，表现为准静态与高度非线性态间的快速切换，且相干与非相干动力学共同作用。相关动力学特征可标识最优性能区间。该模型为理解非平衡多体物理中的信息处理提供了新视角，并为非常规计算研究开辟了新思路。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T11:02:07Z
- **目录日期**: 2025-05-09
