...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟外部驱动的混沌信号预测模型，发现了一个此前被忽视的优越动力学机制。该机制在多种物理参数和推断任务中表现稳健，揭示了物理系统计算与推断的内在规律。最优计算状态出现在临界阻尼阈值之下，涉及多阶段微观动力学弛豫，其适应性源于准静态与高度非线性动态间的快速切换。相干与非相干动力学共同作用，且不同时空尺度上的相关性可标识最优性能状态。这一模型为理解非平衡多体物理中的学习和非常规计算提供了新视角。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T20:02:13Z
- **目录日期**: 2025-05-09
