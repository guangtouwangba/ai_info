...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟受外部驱动的模型预测混沌信号未来状态。研究发现了一个此前被忽视的优异动力学机制，该机制在不同物理参数和推断任务中均表现出稳健的最优性。最优计算状态位于临界阻尼阈值以下，涉及多阶段的微观动力学弛豫过程。系统通过准静态与高度非线性动态间的快速切换实现对外部混沌驱动的适应，其运行依赖相干与非相干动力学的协同作用。相关动力学特征可标识最优性能区间，为多体非平衡系统的非传统计算提供了物理解释框架。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T05:02:13Z
- **目录日期**: 2025-05-09
