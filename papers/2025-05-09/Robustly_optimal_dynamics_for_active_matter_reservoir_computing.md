...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过外部驱动模型预测混沌信号的未来状态。研究发现了一个此前被忽视的优越动力学机制，该机制在不同物理参数和推断任务中均表现出稳健的最优性。最优计算状态位于临界阻尼阈值以下，涉及多阶段的微观动力学弛豫，其适应性源于准稳态与高度非线性动态之间的快速切换。系统的本征弛豫能力决定了其信息处理效能，且可通过单粒子储层测试识别。相干与非相干动力学的协同作用以及粒子动态相关性可标识最优性能区间，为多体非平衡系统的非传统计算提供了新见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-09T16:02:22Z
- **目录日期**: 2025-05-09
