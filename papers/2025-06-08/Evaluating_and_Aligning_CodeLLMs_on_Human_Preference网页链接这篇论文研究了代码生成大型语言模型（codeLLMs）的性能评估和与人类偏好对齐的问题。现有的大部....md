...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发场景。同时开发了SynCode-Instruct合成指令数据集(约200亿标记)用于模型微调。实验发现：1)传统执行基准与CodeArena存在性能差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面仍有显著差距。研究强调了代码生成模型除功能性外，还需重视与开发者使用习惯的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T17:06:00Z
- **目录日期**: 2025-06-08
