...
这篇论文针对代码生成大模型(codeLLMs)与人类偏好对齐的问题，提出了CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种语言)和SynCode-Instruct合成指令数据集(近200亿标记)。研究发现现有基于执行的评估与人类偏好存在差距，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差异显著。研究强调了代码生成模型除正确性外，还需关注实际编程场景中的用户体验和偏好匹配。大规模合成指令微调被证明能有效提升模型表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T23:04:14Z
- **目录日期**: 2025-06-08
