...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好对齐的问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发需求。同时构建了SynCode-Instruct合成指令集(近200亿标记)验证指令微调效果。实验发现：1)传统执行基准与人类偏好评估存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T08:06:26Z
- **目录日期**: 2025-06-08
