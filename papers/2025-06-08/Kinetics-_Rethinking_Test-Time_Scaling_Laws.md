...
这篇论文从实际效率角度重新审视了测试阶段的扩展规律，发现小模型的效果被显著高估。传统基于计算最优性的研究忽视了推理策略（如Best-of-N、长链思维）引入的内存访问瓶颈。作者提出新的"动力学扩展定律"，综合考虑计算和内存访问成本，表明资源应优先分配给超过阈值的大模型。研究发现注意力机制（而非参数量）是测试时主要成本因素，因此提出基于稀疏注意力的新扩展范式，可降低单token成本，支持更长生成和更多并行样本。实验显示稀疏注意力模型在各类资源条件下均优于密集模型，在AIME问题解决准确率上最高提升60分。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T00:01:20Z
- **目录日期**: 2025-06-08
