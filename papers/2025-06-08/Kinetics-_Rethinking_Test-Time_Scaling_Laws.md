...
这项研究重新审视了测试阶段的模型扩展规律，从实际效率角度发现小模型的效果被显著高估。传统基于计算优化的方法忽略了推理策略（如Best-of-N、长链思维）引入的内存访问瓶颈。研究者提出了综合考虑计算和内存成本的"动力学扩展定律"，表明资源应优先分配给超过阈值规模的大模型，因为注意力机制而非参数量成为主要成本因素。为此，他们提出以稀疏注意力为核心的新扩展范式，能在相同资源下支持更长文本生成和更多并行样本。实验显示稀疏注意力模型在各类成本区间均优于密集模型，在AIME基准上最高可获得60分的准确率提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T14:01:27Z
- **目录日期**: 2025-06-08
