...
这项研究重新审视了测试阶段的模型扩展规律，从实际效率角度揭示了小模型的有效性被严重高估。传统基于计算最优性的方法忽略了推理策略（如Best-of-N、长思维链）引入的内存访问瓶颈。通过分析0.6B到32B参数模型，作者提出了综合考虑计算和内存成本的"动力学扩展定律"，表明资源应优先分配给超过阈值的大模型。研究发现注意力机制（而非参数量）是主要成本因素，因此提出基于稀疏注意力的新扩展范式。实验显示稀疏注意力模型在AIME问题求解准确率上显著优于密集模型（低成本场景提升60+分，高成本场景提升5+分），证明稀疏注意力是实现测试阶段扩展潜力的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T18:01:09Z
- **目录日期**: 2025-06-08
