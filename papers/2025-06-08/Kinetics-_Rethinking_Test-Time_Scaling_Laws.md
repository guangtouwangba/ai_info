...
这项研究重新审视了测试阶段的模型扩展规律，从实用效率角度发现小模型的效果被显著高估。传统基于计算最优性的方法忽略了推理策略（如Best-of-N、长思维链）引入的内存访问瓶颈。研究提出新的"动力学扩展定律"，综合考虑计算和内存成本，表明资源应优先分配给超过阈值的大模型。研究发现注意力机制（而非参数量）是测试时主要成本因素，因此提出稀疏注意力新范式，可降低单token成本，支持更长生成和更多并行样本。实验显示稀疏模型在各类成本区间均优于密集模型，在AIME基准上提升5-60个准确率点。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T10:01:17Z
- **目录日期**: 2025-06-08
