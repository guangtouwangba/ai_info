...
这篇论文重新审视了测试阶段的扩展规律，从实际效率角度揭示了小模型的效果被严重高估。研究发现，传统基于计算最优性的方法忽视了推理策略（如Best-of-N、长链思维）引入的内存访问瓶颈。通过分析0.6B到32B参数的模型，作者提出了"动力学扩展定律"，综合考虑计算和内存访问成本，指出资源应优先分配给超过阈值的大模型。研究还发现注意力机制（而非参数量）是主要成本因素，因此提出基于稀疏注意力的新扩展范式，实验显示稀疏模型在各类资源条件下均优于密集模型，最高可获得60+分的准确率提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T08:01:17Z
- **目录日期**: 2025-06-08
