...
这篇论文从实际效率角度重新审视了测试阶段的扩展规律，发现小模型的有效性被显著高估。传统基于计算最优性的方法忽视了推理策略（如Best-of-N、长链思维）引入的关键内存访问瓶颈。作者提出"动力学扩展定律"，综合考虑计算和内存访问成本，表明在阈值以上的模型上分配测试计算资源更有效。研究发现注意力机制（而非参数量）是主要成本因素，因此提出以稀疏注意力为核心的新扩展范式，可降低单token成本，实现更长生成和更多并行样本。实验显示稀疏注意力模型在各类成本区间均优于密集模型，在AIME问题求解准确率上获得显著提升。稀疏注意力能充分发挥测试阶段扩展潜力，因为测试准确率可通过增加生成持续提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T07:01:09Z
- **目录日期**: 2025-06-08
