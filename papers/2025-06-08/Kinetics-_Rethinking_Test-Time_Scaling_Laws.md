...
该研究从实际效率角度重新审视测试阶段的扩展规律，发现小模型的效果被显著高估。传统基于计算最优性的方法忽视了推理策略（如Best-of-N、长思维链）引入的内存访问瓶颈。通过对0.6B到32B参数模型的分析，提出了综合考虑计算和内存成本的"动力学扩展定律"，表明资源应优先分配给超过阈值的大模型。研究发现注意力机制（而非参数量）是主要成本因素，因此提出基于稀疏注意力的新扩展范式，能在相同资源下支持更长生成和更多并行样本。实验显示稀疏模型在AIME问题求解准确率上最高提升60分，证明稀疏注意力是实现测试阶段扩展潜力的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T06:01:12Z
- **目录日期**: 2025-06-08
