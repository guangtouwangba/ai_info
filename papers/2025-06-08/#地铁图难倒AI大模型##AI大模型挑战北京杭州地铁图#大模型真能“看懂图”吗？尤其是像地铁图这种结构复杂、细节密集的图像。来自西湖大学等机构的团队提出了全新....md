...
西湖大学等机构提出ReasonMap评测基准，测试多模态大模型对高分辨率地铁图（平均5839×5449像素）的空间理解和路径规划能力。测试覆盖15个主流模型，结果显示开源模型普遍表现不佳，存在混淆线路或遗漏站点的问题；闭源模型如GPT-4表现较好但仍显著落后于人类水平。北京、杭州等复杂地铁图对AI挑战最大。研究表明当前AI在复杂图像理解和逻辑推理方面仍有明显局限，特别是处理细节密集、结构复杂的专业图表时与人类存在差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T11:04:01Z
- **目录日期**: 2025-06-08
