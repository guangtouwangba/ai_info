...
这篇论文提出了DirectLayout框架，利用大语言模型(LLM)的空间推理能力直接从文本描述生成3D室内场景布局。该方法将生成过程分为三个阶段：鸟瞰图布局生成、3D空间提升和物体位置优化。通过基于3D-Front数据集的思维链(CoT)激活和布局奖励机制增强空间规划能力，并采用上下文学习解决资产与布局的匹配问题。实验表明，DirectLayout在语义一致性、泛化性和物理合理性方面表现优异，克服了现有方法在开放词汇和细粒度指令对齐方面的不足，显著提升了3D场景合成的质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T03:26:58Z
- **目录日期**: 2025-06-08
