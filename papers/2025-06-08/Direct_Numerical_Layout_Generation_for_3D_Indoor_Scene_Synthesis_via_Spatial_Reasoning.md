...
DirectLayout是一个基于大语言模型（LLM）的3D室内场景布局生成框架，通过文本描述直接生成数值化3D布局。它将生成过程分为三个阶段：鸟瞰图（BEV）布局生成、3D空间提升和物体位置优化。该框架利用3D-Front数据集进行显式空间推理，并采用思维链（CoT）激活帮助模型理解物体摆放原则，同时设计了CoT-Grounded生成式布局奖励以增强泛化能力。推理阶段通过上下文学习实现迭代式资产-布局对齐，解决了资产与布局不匹配的问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T06:01:02Z
- **目录日期**: 2025-06-08
