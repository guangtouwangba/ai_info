...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该方法将生成过程分为三个阶段：鸟瞰图布局生成、3D空间提升和物体位置优化。通过3D-Front数据集激活思维链（CoT）来增强空间推理，并设计了CoT-Grounded生成式布局奖励机制以提高泛化能力。推理阶段采用上下文学习进行迭代式资产-布局对齐，解决了资源与布局不匹配的问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，显著优于现有方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T20:00:59Z
- **目录日期**: 2025-06-08
