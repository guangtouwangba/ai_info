...
这篇论文提出了一种名为DirectLayout的框架，用于直接从文本描述生成3D室内场景布局。该框架通过大语言模型(LLMs)的空间推理能力，将生成过程分为三个步骤：生成鸟瞰图布局、将其提升到3D空间以及优化物体摆放。研究采用了基于3D-Front数据集的思维链(CoT)激活方法，并设计了CoT-Grounded生成式布局奖励来增强模型的泛化能力和空间规划能力。在推理阶段，DirectLayout通过上下文学习进行迭代式资产-布局对齐，解决了资产与布局不匹配的问题。实验表明，该方法在语义一致性、泛化性和物理合理性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T01:29:03Z
- **目录日期**: 2025-06-08
