...
这篇论文提出了一种名为DirectLayout的框架，用于根据文本描述直接生成3D室内场景布局。该框架利用大语言模型的空间推理能力，将生成过程分为三个步骤：首先生成俯视图布局，然后将其提升到3D空间，最后优化物体摆放。通过基于3D-Front数据集的思维链激活和设计的布局奖励机制，增强了模型的空间规划和泛化能力。推理阶段采用上下文学习进行迭代优化，解决资源与布局不匹配问题。实验表明，DirectLayout在语义一致性、泛化性和物理合理性方面表现优异，显著提升了开放词汇场景生成能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T17:01:08Z
- **目录日期**: 2025-06-08
