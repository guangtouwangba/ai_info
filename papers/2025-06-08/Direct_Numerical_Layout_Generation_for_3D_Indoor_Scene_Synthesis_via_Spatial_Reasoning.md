...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该方法将生成过程分为三个阶段：鸟瞰图布局生成、3D空间提升和物体位置优化。通过基于3D-Front数据集设计的思维链（CoT）激活机制增强空间推理能力，并采用CoT-Grounded生成式布局奖励提升泛化性。推理阶段通过上下文学习实现迭代式资产-布局对齐。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，解决了现有方法在开放词汇和细粒度控制方面的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T19:01:00Z
- **目录日期**: 2025-06-08
