...
这篇摘要介绍了DirectLayout框架，该框架利用大语言模型（LLMs）的空间推理能力，直接从文本描述生成3D室内场景布局。它将生成过程分为三个阶段：鸟瞰图（BEV）布局生成、3D空间提升和物体位置优化。通过基于3D-Front数据集设计的Chain-of-Thought（CoT）激活和CoT-Grounded生成布局奖励，增强了空间规划和泛化能力。推理阶段采用上下文学习的迭代资产-布局对齐方法解决不匹配问题。实验表明，DirectLayout在语义一致性、泛化性和物理合理性方面表现优异，克服了现有方法在开放词汇和细粒度指令对齐上的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T09:00:58Z
- **目录日期**: 2025-06-08
