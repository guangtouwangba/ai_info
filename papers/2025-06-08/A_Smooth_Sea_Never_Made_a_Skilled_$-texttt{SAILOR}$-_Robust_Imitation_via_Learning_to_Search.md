...
行为克隆（BC）在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，一旦偏离专家轨迹，代理难以自主恢复。为此，研究者提出"学习搜索"（L2S）方法，通过专家演示学习世界模型和奖励模型，使代理能在测试时规划恢复策略。该方法（SAILOR）在12项视觉操作任务中表现优于基于BC的扩散策略，即使BC数据量增加5-10倍仍存在差距。SAILOR能识别细微错误并避免奖励作弊，实现了无需人工干预的稳定高效学习。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T00:02:04Z
- **目录日期**: 2025-06-08
