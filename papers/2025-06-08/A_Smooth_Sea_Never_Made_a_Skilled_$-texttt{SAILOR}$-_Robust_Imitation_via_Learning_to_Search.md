...
行为克隆（BC）方法在模仿学习中的根本局限是只能模仿专家在特定状态下的行为，一旦偏离专家示范的状态，代理就无法自主恢复。为解决这一问题，研究者提出"学习搜索"（L2S）方法，通过专家示范学习世界模型和奖励模型，使代理能在测试时自主规划恢复路径。新方法SAILOR在12项视觉操作任务中表现优于当前最先进的扩散策略（Diffusion Policies），即使BC使用5-10倍更多示范数据仍有差距。SAILOR能识别细微错误且不易被奖励欺骗。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T09:02:00Z
- **目录日期**: 2025-06-08
