...
行为克隆（BC）在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，导致智能体一旦偏离专家示范状态便难以恢复。为此，研究者提出基于专家示范的"学习搜索"（L2S）方法，通过构建世界模型和奖励模型，使智能体能在测试时自主规划恢复路径。新方法SAILOR在12项视觉操作任务中均优于当前最先进的扩散策略（Diffusion Policies），即使BC使用5-10倍数据仍存在性能差距。SAILOR能识别细微错误并抵抗奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T19:01:50Z
- **目录日期**: 2025-06-08
