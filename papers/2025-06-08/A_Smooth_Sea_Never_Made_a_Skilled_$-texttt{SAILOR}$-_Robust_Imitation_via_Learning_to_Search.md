...
行为克隆（BC）方法在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，一旦偏离专家轨迹，代理无法自主恢复。为解决这一问题，研究者提出“学习搜索”（L2S）框架SAILOR，通过从专家演示中学习世界模型和奖励模型，使代理能在测试时规划恢复策略，无需额外人工修正。实验表明，SAILOR在12项视觉操作任务中均优于当前最先进的扩散策略（Diffusion Policies），即使BC使用5-10倍数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T22:01:56Z
- **目录日期**: 2025-06-08
