...
行为克隆(BC)方法在模仿学习中的根本局限是只能模仿专家在特定状态下的行为，一旦偏离演示状态就难以恢复。为此，研究者提出了基于专家演示的搜索学习(L2S)方法SAILOR，通过学习世界模型和奖励模型，使智能体能在测试时自主规划以匹配专家目标。实验表明，在12个视觉操作任务中，SAILOR显著优于当前最先进的扩散策略(BC)，即使BC使用5-10倍演示数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T20:01:56Z
- **目录日期**: 2025-06-08
