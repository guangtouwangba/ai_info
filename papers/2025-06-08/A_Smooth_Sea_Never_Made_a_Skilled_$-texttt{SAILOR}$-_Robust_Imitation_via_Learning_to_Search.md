...
行为克隆（BC）方法在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，一旦偏离专家轨迹，代理无法自主恢复。研究提出“学习搜索”（L2S）框架，通过专家演示学习世界模型和奖励模型，使代理能在测试时规划恢复策略，无需额外人工修正。该方法（SAILOR）在12项视觉操作任务中均优于当前最佳的扩散策略（BC），即使BC数据量扩大5-10倍仍存在性能差距。SAILOR能识别细微错误且抗奖励作弊。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T10:02:02Z
- **目录日期**: 2025-06-08
