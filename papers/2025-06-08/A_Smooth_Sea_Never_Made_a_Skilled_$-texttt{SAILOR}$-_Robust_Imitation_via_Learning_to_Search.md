...
行为克隆（BC）方法在模仿学习中的根本局限在于，它只能让智能体学习专家在特定状态下的行为，一旦偏离专家示范的状态，智能体往往无法自主恢复。为解决这一问题，研究者提出了基于专家示范的搜索学习（L2S）方法，通过结合世界模型和奖励模型，使智能体能够在测试时规划恢复行为，而无需额外人工修正。实验表明，新方法SAILOR在多项视觉操作任务中显著优于当前最先进的扩散策略（Diffusion Policies），即使BC使用5-10倍的数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T01:29:56Z
- **目录日期**: 2025-06-08
