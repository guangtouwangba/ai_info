...
行为克隆（BC）模仿学习的主要局限在于只能教会智能体在专家访问过的状态下模仿行为，一旦偏离演示范围就无法自主恢复。为此，研究者提出"学习搜索"（L2S）方法，通过专家演示学习世界模型和奖励模型，使智能体能在测试时自主规划以达成专家目标。该方法（SAILOR）在12项视觉操作任务中稳定超越基于相同数据的扩散策略BC方法，即使BC演示数据增加5-10倍仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊，实现了无需人工纠正的高效学习。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T14:02:22Z
- **目录日期**: 2025-06-08
