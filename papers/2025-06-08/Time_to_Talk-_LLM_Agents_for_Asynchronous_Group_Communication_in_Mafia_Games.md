...
当前大语言模型（LLM）主要用于同步对话（轮流发言），而现实场景（如群聊、在线会议或社交游戏）多为异步交流（无固定发言顺序）。本研究开发了一种异步LLM智能体，不仅能决定发言内容，还能自主选择发言时机。通过在"狼人杀"游戏中与人类玩家对比测试，该智能体在游戏表现和社交融入度上均达到人类水平，其发言时机选择与人类行为高度相似（但内容存在差异）。研究公开了数据集和代码，为LLM在团队协作、教育等复杂社交场景的应用奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T17:01:38Z
- **目录日期**: 2025-06-08
