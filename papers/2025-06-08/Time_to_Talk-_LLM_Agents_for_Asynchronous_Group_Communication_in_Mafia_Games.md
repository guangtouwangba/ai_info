...
当前大语言模型（LLMs）主要用于同步对话（轮流发言），而现实场景（如群聊、在线会议或社交游戏）往往是异步的，发言时机本身就是关键决策。本研究开发了一种异步LLM智能体，能自主决定发言内容和时机。通过在在线"狼人杀"游戏中与人类玩家同台测试，该智能体在游戏表现和融入度上与人类相当，且发言时机行为高度拟人（尽管内容存在差异）。研究公开了数据和代码，推动LLMs在更真实的异步场景（如团队协作、教育等需处理复杂社交动态的环境）中的应用发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T07:01:28Z
- **目录日期**: 2025-06-08
