...
本文提出了一种基于AI的准粒子干涉(QPI)核提取新方法，解决了多散射源图像中单散射QPI模式提取这一病态逆问题。该框架采用两步学习策略：首先通过变分自编码器学习散射核的紧凑潜在空间表示；然后使用专用编码器将QPI观测数据与预学习核的潜在表示对齐。这种设计使模型能在复杂纠缠散射条件下稳健推断核函数。在包含100种独特核的多样化QPI数据集上测试表明，该方法相比直接一步法基线具有显著更高的提取精度，并对未见核表现出更好的泛化能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T17:01:28Z
- **目录日期**: 2025-06-08
