...
港科大团队针对视觉语言模型(VLM)在多模态推理中的"慢思考"失效问题，提出VL-Rethinker框架。研究发现传统慢思考模型在图文任务中存在优势消失和反思惰性两大瓶颈。新框架通过优势样本回放(优先学习关键样本)和强制反思机制(智能触发自我验证)，使模型能识别题目错误并自主决定反思时机。在MathVista等基准测试中，该框架超越GPT-o1等模型，不同规模模型均显著提升，验证了慢思考在多模态领域的有效性。相关39K高质量数据集ViRL39K同步开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T06:03:44Z
- **目录日期**: 2025-06-08
