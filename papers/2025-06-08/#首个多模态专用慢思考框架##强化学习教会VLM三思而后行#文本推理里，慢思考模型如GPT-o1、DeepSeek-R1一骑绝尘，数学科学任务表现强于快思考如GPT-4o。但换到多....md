...
港科大团队针对视觉语言模型(VLM)在多模态推理中"慢思考"失效的问题，提出VL-Rethinker框架。研究发现传统慢思考模型在图文场景存在优势消失和反思惰性两大瓶颈。新框架通过优势样本回放机制（优先学习关键样本）和强制反思机制（动态触发自我验证），显著提升模型表现。在MathVista等基准测试中超越GPT-o1，接近OpenAI-o1水平，不同规模模型均获提升。该研究首次实现多模态领域的专用慢思考框架，验证了强化学习能有效培养VLM"三思而后行"的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T02:37:27Z
- **目录日期**: 2025-06-08
