...
香港科技大学等团队针对视觉语言模型（VLM）在多模态推理中的"慢思考"短板，提出创新框架VL-Rethinker。该方案通过两项机制突破传统瓶颈：1）优势样本回放（SSR）聚焦高价值训练样本；2）强制反思机制引导模型自主判断反思时机。实验显示，该框架在MathVista等数据集上超越GPT-o1，接近OpenAI-o1水平，不同规模模型均显著提升。研究同时构建了39K高质量多模态数据集ViRL39K，证实"慢思考"策略在多模态领域同样有效，使模型能识别题目错误并自主纠错。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T03:29:30Z
- **目录日期**: 2025-06-08
