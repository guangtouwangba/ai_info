...
本文提出两种可解释AI模型(VCT和SVCT)来解决医学AI中的透明性问题。VCT通过融合图像特征和概念特征弥补了传统概念瓶颈模型(CBMs)忽略图像内在特征的缺陷。基于VCT改进的SVCT采用视觉变换器(ViT)架构，结合去噪扩散平滑技术，增强了模型对抗输入扰动的稳定性。在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具有可解释性，SVCT尤其能在扰动情况下提供稳定的解释，满足医学领域对模型可靠性和可解释性的双重需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T04:09:20Z
- **目录日期**: 2025-06-08
