...
本文提出两种可解释AI模型(VCT和SVCT)以解决医学领域AI透明性问题。传统概念瓶颈模型(CBMs)仅依赖概念特征，忽略了医学图像的内在特征，且对输入扰动敏感。VCT通过融合概念特征与图像特征提升性能；SVCT在VCT基础上结合视觉Transformer架构和去噪扩散平滑技术，确保模型在扰动下的解释稳定性。在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具有可解释性，SVCT尤其能在扰动下提供稳定解释，满足医学领域对AI模型可靠性和可解释性的双重需求。(100字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T17:02:20Z
- **目录日期**: 2025-06-08
