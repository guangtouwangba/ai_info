...
中科大团队提出GCoT框架，首次将语言模型的"思维链"方法应用于无文本的图数据。该框架通过分步推理：先初步推断图数据，融合多层表示生成中间"思维"，再用其生成节点专属提示进行下一步推理。在8个数据集测试中，GCoT仅用1-5个样本就超越现有最优模型，刷新了图学习表现上限。这一突破解决了图数据结构复杂、缺乏文本的传统难题，为图神经网络发展开辟了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T18:02:56Z
- **目录日期**: 2025-06-08
