...
本文提出"控制税"概念,量化AI控制措施的操作和财务成本,为AI控制领域做出三点贡献:(1)建立理论框架,将分类器性能映射到安全保证;(2)在对抗环境中评估先进语言模型,研究攻击模型植入后门与监控模型检测漏洞的对抗;(3)提供控制协议的实际成本估算,开发平衡安全性与成本效益的优化监控策略。该框架通过系统连接安全保证与其成本,支持从业者在不同部署场景下做出基于经济可行性的决策,推动AI控制领域的实际应用。研究特别关注代码后门检测等高风险应用场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T08:02:12Z
- **目录日期**: 2025-06-08
