...
本文提出了"控制税"(Control Tax)的概念，用于衡量将AI控制措施整合到AI管道中的操作和财务成本。研究主要贡献包括：(1)建立量化控制税的理论框架，将分类器性能映射到安全保证；(2)在对抗性环境中评估先进语言模型，研究攻击模型植入代码后门与监控模型检测漏洞的能力；(3)提供控制协议的实际成本估算，并开发平衡安全性与成本效益的优化监控策略。该框架通过系统连接安全保证与其成本，帮助从业者做出明智决策，推动AI控制在不同部署场景中的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T01:29:46Z
- **目录日期**: 2025-06-08
