...
本文提出"控制税"(Control Tax)概念，用于衡量AI控制机制在运营和财务上的成本。研究贡献包括：(1)建立量化控制税的理论框架，将分类器性能映射到安全保证；(2)在对抗环境下评估先进语言模型，研究攻击模型植入代码后门与监控模型检测漏洞的能力；(3)提供控制协议的实际成本估算，并开发兼顾安全性和成本效益的优化监控策略。该框架通过系统连接安全保证与其成本，帮助从业者在不同部署场景下做出基于经济可行性的决策，推动AI控制领域的实际应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T12:01:58Z
- **目录日期**: 2025-06-08
