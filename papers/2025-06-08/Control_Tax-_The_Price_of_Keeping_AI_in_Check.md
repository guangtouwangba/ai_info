...
本文提出"控制税"概念，即AI系统中集成控制措施所需的运维和财务成本。研究贡献包括：(1)建立量化控制税的理论框架，将分类器性能映射到安全保障；(2)在对抗环境中评估先进语言模型，研究攻击模型植入后门与监测模型检测漏洞的对抗；(3)提供控制协议的实际成本估算，并开发平衡安全性与成本效益的优化监测策略。该框架通过系统连接安全保障与其成本，帮助从业者做出明智决策，推动AI控制领域在不同部署场景中的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T17:01:58Z
- **目录日期**: 2025-06-08
