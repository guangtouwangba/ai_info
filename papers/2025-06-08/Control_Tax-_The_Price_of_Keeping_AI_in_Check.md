...
本文提出"控制税"(Control Tax)概念，用于衡量AI系统中集成控制措施的操作和财务成本。研究通过三个关键贡献推进AI控制(AIC)领域：1)建立量化控制税的理论框架，将分类器性能映射到安全保证；2)在对抗环境下评估先进语言模型，研究攻击模型植入代码后门与监控模型检测漏洞的能力；3)提供控制协议的实际成本估算，并开发平衡安全性与成本效益的优化监控策略，同时考虑审计预算等实际约束。该框架通过系统连接安全保证与其成本，帮助从业者做出明智决策，推动AIC在不同部署场景中的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T19:01:42Z
- **目录日期**: 2025-06-08
