...
本文提出了"控制税"概念，用于衡量将安全控制措施整合到AI系统中的操作和财务成本。研究贡献包括：(1)建立量化控制税的理论框架，将分类器性能映射到安全保证；(2)在对抗环境中评估先进语言模型，研究攻击者植入后门代码与监测模型检测漏洞的能力；(3)提供控制协议的实际成本估算，并开发平衡安全性与成本效益的优化监测策略。该框架通过系统连接安全保证与其成本，帮助从业者做出明智决策，推动AI控制领域的发展。研究特别考虑了审计预算等实际约束条件。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T18:01:43Z
- **目录日期**: 2025-06-08
