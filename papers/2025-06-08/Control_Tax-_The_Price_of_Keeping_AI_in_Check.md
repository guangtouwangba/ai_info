...
本文提出"控制税"(Control Tax)概念，量化AI控制措施的操作与财务成本，为AI控制领域(AIC)做出三点贡献：(1)建立理论框架，将分类器性能映射到安全保障；(2)在对抗环境下评估先进语言模型，研究攻击模型植入代码后门与监测模型检测漏洞的能力；(3)估算控制协议的实际财务成本，开发平衡安全性与成本效益的优化监测策略，考虑审计预算等现实约束。该框架通过系统关联安全保证与成本，帮助从业者做出明智决策，推动AIC在不同部署场景中的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T11:01:51Z
- **目录日期**: 2025-06-08
