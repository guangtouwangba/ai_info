...
苹果发表论文《The Illusion of Thinking》，质疑当前大语言模型的推理能力。研究通过汉诺塔等4种复杂度可调的谜题测试发现：低难度时非推理模型效率更高；中等难度思维链（CoT）开始有效；高复杂度时所有模型（包括DeepSeek、Claude等）准确率归零。关键结论指出，模型在遇到困难时会主动减少"思考"，暴露出其缺乏真正的逻辑泛化能力，仅能进行模式匹配而非深度推理。论文质疑现有大模型所谓"思考"本质是高级模板匹配，无法应对复杂问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T08:03:04Z
- **目录日期**: 2025-06-08
