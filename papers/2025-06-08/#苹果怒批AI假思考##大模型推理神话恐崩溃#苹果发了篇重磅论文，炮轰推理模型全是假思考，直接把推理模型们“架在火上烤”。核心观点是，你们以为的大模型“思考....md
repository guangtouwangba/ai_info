...
苹果最新论文《The Illusion of Thinking》指出，当前大语言模型（如DeepSeek、Claude等）的"推理能力"本质是模板匹配，而非真正的逻辑思考。通过汉诺塔等4类复杂度可控的谜题测试发现：低难度时基础模型更高效；中等难度思维链机制有效；但高复杂度时所有模型准确率归零。关键发现是模型在遇到困难时会主动"放弃思考"，暴露出其缺乏泛化逻辑能力，连基础步骤执行都存在问题。研究质疑了大模型所谓"推理能力"的神话，指出现有技术尚未实现真正的AI思考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T09:02:38Z
- **目录日期**: 2025-06-08
