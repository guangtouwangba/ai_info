...
苹果最新论文《The Illusion of Thinking》指出，当前大语言模型的"推理能力"本质是模板匹配而非真实思考。研究通过汉诺塔等4种复杂度可调的谜题测试发现：低难度时非思考型模型更高效；中等难度思维链机制有效；但高复杂度下所有模型（包括DeepSeek、Claude等）准确率归零。关键发现是模型在遇到困难时会主动"放弃思考"，暴露出其缺乏泛化逻辑能力，连基础步骤执行都存在问题。研究质疑了大模型真正推理能力的有效性，认为现有技术仍无法实现深度思考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T14:03:08Z
- **目录日期**: 2025-06-08
