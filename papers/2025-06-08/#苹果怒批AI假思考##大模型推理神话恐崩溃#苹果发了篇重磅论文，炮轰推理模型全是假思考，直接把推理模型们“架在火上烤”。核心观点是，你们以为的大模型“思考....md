...
苹果发表论文《The Illusion of Thinking》，质疑当前大语言模型的推理能力只是"假思考"。研究通过汉诺塔等4个复杂度可控的谜题测试发现：低难度时简单模型更高效；中等难度思维链机制有效；但高复杂度下所有模型（包括DeepSeek、Claude等）准确率归零。关键发现是模型会主动减少"思考"而非资源不足，暴露出其缺乏泛化逻辑能力，无法完成基础步骤执行任务。论文指出当前AI的"推理"本质是模式匹配，而非真正的逻辑思考，挑战了大模型推理能力的神话。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T18:02:30Z
- **目录日期**: 2025-06-08
