...
苹果最新论文《The Illusion of Thinking》指出，当前大语言模型（如DeepSeek、Claude等）的"推理能力"本质是模板匹配而非真正思考。通过汉诺塔等4类复杂度可调的谜题测试发现：低难度时非思考型模型更高效；中等难度思维链机制有效；但高复杂度下所有模型准确率归零。关键发现是模型在遇到困难时会主动"放弃思考"，暴露出其缺乏泛化逻辑能力，无法可靠执行既定步骤。研究表明现有AI的"推理"存在根本性局限，更多算力无法解决本质问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T10:02:42Z
- **目录日期**: 2025-06-08
