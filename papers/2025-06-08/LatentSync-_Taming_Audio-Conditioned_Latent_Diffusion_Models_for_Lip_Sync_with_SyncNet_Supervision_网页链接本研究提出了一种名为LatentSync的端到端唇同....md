...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法（时间表示对齐），利用自监督视频模型增强生成帧的时间连续性。同时改进了SyncNet训练框架，通过优化架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）直接建模音视频关联的潜在扩散架构；2）TREPA时间一致性增强；3）SyncNet训练优化方案，为相关领域提供普适性经验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T01:33:00Z
- **目录日期**: 2025-06-08
