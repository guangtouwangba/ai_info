...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了训练不平衡问题，并将内存使用减半。实验表明，在Transformer任务、ImageNet分类及GPT-2/Llama2预训练中，其性能匹配或优于AdamW，且对超参数更鲁棒。例如，为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB。该方法在LoRA微调等场景也表现优异，成为高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T11:04:34Z
- **目录日期**: 2025-06-08
