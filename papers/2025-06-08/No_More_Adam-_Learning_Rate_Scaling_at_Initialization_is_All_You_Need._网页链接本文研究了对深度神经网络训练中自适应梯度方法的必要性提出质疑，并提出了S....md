...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。该方法改进带动量的SGD，通过在初始化时依据梯度信号噪声比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，同时将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中表现优于或持平AdamW，且对超参数更鲁棒。在内存效率上，相比AdamW，SGD-SAi为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，并在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T13:11:11Z
- **目录日期**: 2025-06-08
