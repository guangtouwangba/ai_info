...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。该方法通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，有效解决了训练不平衡问题，同时将内存使用量减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或相当于AdamW，特别在大型语言模型（如GPT-2和Llama2-7B）中节省了显著内存（分别节省5.93GB和25.15GB）。该方法还具有超参数鲁棒性，在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T21:04:14Z
- **目录日期**: 2025-06-08
