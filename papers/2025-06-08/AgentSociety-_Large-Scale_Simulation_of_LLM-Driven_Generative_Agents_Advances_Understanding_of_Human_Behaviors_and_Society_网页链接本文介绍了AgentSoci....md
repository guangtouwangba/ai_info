...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它结合了大型语言模型驱动的代理、真实社会环境和强大的模拟引擎。研究模拟了1万多个代理及其500万次互动，用于分析社会极化、信息传播、基本收入政策及外部冲击（如飓风）等社会问题。模拟结果与现实世界数据吻合，验证了该工具在捕捉人类行为和社会机制方面的有效性。AgentSociety展现了作为社会科学研究和政策制定辅助工具的潜力，能够补充传统研究方法（如调查和实验）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T01:32:44Z
- **目录日期**: 2025-06-08
