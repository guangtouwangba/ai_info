...
这篇论文介绍了AgentSociety，这是一个利用大型语言模型驱动的大规模社会模拟器，可以模拟超过1万个代理的社会互动（500万次交互）。研究者用它作为计算社会科学实验平台，研究了四个关键社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响和飓风等外部冲击。模拟结果与现实世界数据吻合，验证了该系统在捕捉人类行为机制方面的能力，展现了其作为社会科学研究和政策制定工具的潜力。该技术能辅助传统研究方法（如调查和访谈），帮助理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-08T11:05:16Z
- **目录日期**: 2025-06-08
