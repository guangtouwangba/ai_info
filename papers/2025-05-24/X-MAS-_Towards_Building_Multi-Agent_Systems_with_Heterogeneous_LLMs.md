...
该研究提出了基于异构大语言模型（LLM）的多智能体系统（X-MAS），突破了传统同构系统的性能限制。通过开发X-MAS-Bench测试平台，研究人员对27个LLM在5个领域（21个测试集）和5种功能进行了170万次评估，发现异构配置能显著提升系统性能：在纯聊天机器人场景下，MATH数据集表现提升8.4%；在混合（聊天+推理）场景中，AIME数据集表现提升达47%。研究表明，无需重构系统架构，仅通过采用异构LLM组合即可实现多智能体系统性能的突破性进步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T10:01:36Z
- **目录日期**: 2025-05-24
