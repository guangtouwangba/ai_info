...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型（LLM）的局限性（如有限上下文、角色过载）。KtR递归分解任务为类型化子任务，采用零样本或轻量增强（如思维链、微调）解决。基于"没有免费午餐"定理，KtR强调结构化分解而非通用提示。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6智能体系统在10项时达100%准确率。该方法证明适度模型通过算法感知分解可实现可靠协作，无需更大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T02:29:50Z
- **目录日期**: 2025-05-24
