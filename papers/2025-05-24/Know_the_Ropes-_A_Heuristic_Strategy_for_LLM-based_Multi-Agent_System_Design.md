...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域知识转化为层次化算法蓝图来解决单一大语言模型（LLM）的局限性（如有限上下文、角色过载和领域迁移脆弱性）。KtR递归分解任务为类型化子任务，由控制器协调，结合零样本推理或轻量增强（如思维链、微调）。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6个智能体实现100%（10项）和84%（13-15项）准确率，远超零样本的11%。该框架证明算法感知分解能高效协同轻量模型，无需依赖巨型单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T21:02:03Z
- **目录日期**: 2025-05-24
