...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验知识转化为分层算法蓝图，解决单一大语言模型（LLM）的局限性（如有限上下文、角色过载）。KtR 递归分解任务为类型化子任务，结合轻量级增强（如思维链、微调），避免传统多智能体方法的分解模糊和验证开销。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从零样本的3%提升至95%；任务分配问题（6-15项）中，6智能体实现小规模100%、大规模84%的准确率（零样本仅11%）。该方法证明算法感知分解能高效协同轻量模型，无需依赖庞大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T00:02:20Z
- **目录日期**: 2025-05-24
