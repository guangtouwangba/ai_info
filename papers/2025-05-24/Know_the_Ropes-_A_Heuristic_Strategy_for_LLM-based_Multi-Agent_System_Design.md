...
Know-The-Ropes (KtR) 提出了一种解决单一大语言模型（LLM）局限性的框架，如有限上下文、角色过载和领域迁移脆弱性。传统多智能体方法虽能缓解这些问题，但会引入分解模糊、合约不明确和验证开销等新问题。KtR 通过将领域先验转化为层次化算法蓝图，递归分解任务为类型化的子任务，并采用零样本或轻量增强（如思维链、微调）解决。实验显示，在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；任务分配问题（6-15项）中，6智能体组合在10项任务中达到100%准确率。该框架无需更大模型即可实现高效协作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T19:02:03Z
- **目录日期**: 2025-05-24
