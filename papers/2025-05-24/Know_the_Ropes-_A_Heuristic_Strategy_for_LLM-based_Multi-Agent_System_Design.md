...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型（LLM）的局限性（如有限上下文、角色过载和脆弱的领域迁移）。KtR递归分解任务为类型化子任务，由控制器协调，结合轻量级增强技术（如思维链、微调或自检）。基于“没有免费午餐”定理，KtR放弃通用提示，专注结构化分解。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；任务分配问题（6-15项）中，6智能体在规模10以下达100%，规模13-15达84%（零样本仅11%），证明模块化分解能高效协同轻量模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T14:02:16Z
- **目录日期**: 2025-05-24
