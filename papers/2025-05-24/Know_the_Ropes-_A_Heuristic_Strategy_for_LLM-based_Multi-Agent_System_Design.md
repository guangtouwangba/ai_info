...
Know-The-Ropes (KtR) 提出了一种多智能体框架，通过将领域先验转化为层次化任务分解来解决单一大模型的局限性（如有限上下文、角色过载）。KtR 递归拆分任务为类型化子任务，结合轻量级增强技术（如思维链、微调），在背包问题（3-8物品）中使GPT-4o-mini的准确率从3%提升至95%；在任务分配问题（6-15作业）中，六智能体系统实现小规模100%、大规模84%的准确率（零样本仅11%）。该方法证明：算法感知分解+精准增强可使中小模型高效协作，无需持续扩大单体模型规模。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T13:08:58Z
- **目录日期**: 2025-05-24
