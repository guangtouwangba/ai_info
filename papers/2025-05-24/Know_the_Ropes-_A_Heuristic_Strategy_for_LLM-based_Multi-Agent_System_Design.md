...
Know-The-Ropes (KtR) 是一个多智能体框架，旨在解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载和领域迁移脆弱性)。KtR通过将领域先验知识转化为算法蓝图层次结构，将任务递归分解为类型化的、控制器介导的子任务，每个子任务采用零样本或最小增强(如思维链、微调、自检)解决。实验显示：在背包问题(3-8项)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15项)中，6个o3-mini智能体实现100%(10项)和84%(13-15项)准确率，远超零样本的11%。该框架证明算法感知分解结合针对性增强可使小型模型成为可靠协作者。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T17:02:19Z
- **目录日期**: 2025-05-24
