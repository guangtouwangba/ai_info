...
大型语言模型（LLM）单代理存在上下文限制、角色过载和领域迁移脆弱性。传统多代理方案虽缓解这些问题，却带来分解模糊、合约不明确和验证开销等新挑战。为此，研究者提出Know-The-Ropes（KtR）框架，将领域先验转化为层次化算法蓝图，通过控制器递归分解任务为类型化子任务，并采用零样本或最小增强（如思维链、微调等）解决。基于"没有免费午餐"定理，KtR放弃通用提示，转向结构化分解。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini代理将准确率从3%提升至95%；任务分配问题（6-15项）中，6代理系统在10项任务达到100%准确率，显著优于零样本的11%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T05:02:29Z
- **目录日期**: 2025-05-24
