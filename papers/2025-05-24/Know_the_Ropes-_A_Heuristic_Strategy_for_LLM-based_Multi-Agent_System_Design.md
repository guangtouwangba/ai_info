...
大型语言模型(LLM)作为单一智能体存在上下文限制、角色过载和领域迁移脆弱性等问题。传统多智能体方法虽能缓解这些问题，但会引入任务分解不当、合约模糊和验证开销等新挑战。为此，研究者提出Know-The-Ropes(KtR)框架，将领域先验转化为算法蓝图层次结构，通过类型化、控制器介导的递归任务分解，结合零样本推理或最小化增强策略(如思维链、微调等)解决问题。实验表明，在背包问题(3-8项)中，三个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15项)中，六个智能体在规模10以下达到100%准确率。该方法证明算法感知分解结合针对性增强可使中等模型成为可靠协作伙伴。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T04:02:09Z
- **目录日期**: 2025-05-24
