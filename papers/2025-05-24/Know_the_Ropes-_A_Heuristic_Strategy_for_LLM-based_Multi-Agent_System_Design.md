...
Know-The-Ropes (KtR) 是一个多智能体框架，旨在解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载和领域迁移脆弱性)。它通过将领域先验知识转化为层次化算法蓝图，递归分解任务为类型化的子任务，并由控制器协调解决。KtR避免了通用提示的追求，采用结构化分解和精准增强(如思维链、微调等)。实验显示：在背包问题(3-8物品)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15任务)中，6个o3-mini智能体达到100%(小规模)和84%(大规模)准确率，远超零样本的11%。该方法证明适度模型通过算法感知分解可实现可靠协作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T06:02:27Z
- **目录日期**: 2025-05-24
