...
Know-The-Ropes (KtR) 是一个多智能体框架，旨在解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载和领域迁移脆弱性)。KtR 将领域先验知识转化为层次化的算法蓝图，递归分解任务为类型化的子任务，通过轻量级增强(如思维链、微调或自检)实现高效解决。实验显示：在背包问题(3-8项)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15项)中，6个o3-mini智能体达到100%(10项)和84%(13-15项)准确率，远超零样本的11%。该方法证明算法感知分解结合精准增强可使小型模型获得可靠性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T07:02:13Z
- **目录日期**: 2025-05-24
