...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域知识转化为层次化算法蓝图来解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载和脆弱的领域迁移)。KtR将任务递归分解为类型化的子任务，由控制器协调，并采用最轻量的增强技术(如思维链、微调或自检)。实验显示：在背包问题(3-8项)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在更复杂的任务分配问题(6-15项)中，6个o3-mini智能体实现了100%(小规模)和84%(大规模)的准确率，远超零样本的11%。该方法证明了算法感知分解和针对性增强能使小型模型成为可靠协作者。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T22:02:07Z
- **目录日期**: 2025-05-24
