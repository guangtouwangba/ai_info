...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型(LLM)的局限性（如有限上下文、角色过载和领域迁移脆弱性）。KtR递归分解任务为类型化子任务，采用最轻量级增强技术（如思维链、微调或自检）。实验显示：在背包问题（3-8物品）中，3个GPT-4o-mini智能体将准确率从零样本的3%提升至95%；在任务分配问题（6-15作业）中，6智能体系统在10规模以下达100%准确率，13-15规模达84%（零样本仅11%）。该方法证明算法感知分解结合定向增强可使轻量模型达成可靠协作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T08:02:18Z
- **目录日期**: 2025-05-24
