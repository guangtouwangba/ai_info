...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载)。KtR将任务递归分解为类型化的子任务，采用最轻量级增强技术(如思维链、微调)解决。基于"没有免费午餐"定理，KtR放弃通用提示而采用结构化分解。实验显示：在背包问题(3-8物品)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15任务)中，6个o3-mini智能体在10个任务时达到100%准确率。该方法证明算法感知分解能提升小模型协作性能，无需更大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T11:02:10Z
- **目录日期**: 2025-05-24
