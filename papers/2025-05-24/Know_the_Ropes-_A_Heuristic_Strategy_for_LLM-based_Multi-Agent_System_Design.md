...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大语言模型（LLM）的局限性（如有限上下文、角色过载）。KtR递归分解任务为类型化子任务，结合轻量级增强技术（如思维链、微调），避免传统多智能体方法的分解模糊和验证开销。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从零样本的3%提升至95%；任务分配问题（6-15项）中，6个o3-mini智能体实现小规模100%、大规模84%的准确率，显著优于零样本11%。该方法证明算法感知分解能有效提升轻量模型的协作性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T20:02:08Z
- **目录日期**: 2025-05-24
