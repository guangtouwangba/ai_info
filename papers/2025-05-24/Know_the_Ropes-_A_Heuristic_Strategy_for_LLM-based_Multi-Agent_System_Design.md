...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为算法蓝图层级来解决单一大模型的局限性（如有限上下文、角色过载）。KtR递归分解任务为类型化的子任务，由控制器协调，并采用轻量级增强（如思维链、微调）解决问题。基于"没有免费午餐"定理，KtR放弃通用提示，专注于结构化分解。实验显示：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6个o3-mini智能体达到100%（10项）和84%（13-15项）准确率，远超零样本的11%。这表明算法感知分解结合定向增强能显著提升小模型协作效能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T10:02:21Z
- **目录日期**: 2025-05-24
