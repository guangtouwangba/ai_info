...
这篇论文针对代码生成大型语言模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建近200亿标记的SynCode-Instruct合成指令数据集验证微调效果。研究发现传统执行基准与人类偏好存在显著差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显，凸显了代码LLM与人类偏好对齐的重要性。研究为提升代码生成模型的实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T22:05:34Z
- **目录日期**: 2025-05-24
