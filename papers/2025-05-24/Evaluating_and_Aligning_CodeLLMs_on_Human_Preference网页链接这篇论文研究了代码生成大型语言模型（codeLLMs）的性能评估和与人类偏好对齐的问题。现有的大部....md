...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类任务和44种语言)来评估模型在实际编程任务中的表现，发现传统基于执行的测试与人类偏好存在差距。同时构建了20B标记的SynCode-Instruct合成指令集验证微调效果。实验显示开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著，凸显了模型对齐人类偏好的重要性。研究为提升代码生成模型的实用价值提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T14:06:25Z
- **目录日期**: 2025-05-24
