...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，指出当前基准测试过于关注代码正确性而忽视人类偏好。研究者创建了CodeArena基准（含397个多样化编程任务样本）和SynCode-Instruct合成指令数据集（20B标记），用于评估模型与人类偏好的对齐程度。实验发现：1）传统执行基准与CodeArena存在性能差异；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著。研究强调了代码生成模型除功能性外，还需重视人类偏好对齐的关键需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T02:33:39Z
- **目录日期**: 2025-05-24
