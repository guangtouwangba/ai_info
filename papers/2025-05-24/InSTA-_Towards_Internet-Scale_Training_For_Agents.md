...
研究人员开发了一种无需人工标注的大规模网络导航智能体训练方法。该方法分三阶段：首先用大语言模型(LLM)为15万个网站生成任务，然后由LLM智能体执行任务并生成轨迹，最后通过LLM筛选成功轨迹。实验显示，语言模型能有效过滤有害内容(准确率97%)和判断成功轨迹(82.6%)。基于Qwen 3 1.7B训练的智能体性能优于235倍大的Llama 4 Maverick，达到Gemini 2.5 Flash 94.7%的水平，最高成功率56.9%。该方法显著提升了小模型在网页导航任务中的表现。相关代码、模型和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T04:02:33Z
- **目录日期**: 2025-05-24
