...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统认知。该方法通过初始化时根据梯度信噪比(g-SNR)调整不同参数组的学习率，解决了训练不平衡问题，同时将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和大语言模型(GPT-2/Llama2)预训练中表现优于AdamW，具有超参数鲁棒性。在内存效率方面，相比AdamW节省显著(如为Llama2-7B节省25.15GB)。该方法还提升了LoRA微调等任务的性能，为深度学习优化提供了更高效的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T02:32:18Z
- **目录日期**: 2025-05-24
