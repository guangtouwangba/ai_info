...
本研究提出LatentSync框架，一种基于音频条件的潜在扩散模型的端到端唇同步方法，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型的时间一致性问题，创新性地提出时间表示对齐（TREPA）方法增强连续性。同时改进了SyncNet训练框架，通过优化架构、超参数和数据预处理显著提升其准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。这些改进也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T09:05:27Z
- **目录日期**: 2025-05-24
