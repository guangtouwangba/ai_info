...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型的时间一致性问题，提出TREPA方法，利用自监督视频模型的时间表示增强一致性。同时优化SyncNet训练框架，改进模型架构、超参数和数据预处理，显著提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T12:05:47Z
- **目录日期**: 2025-05-24
