...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该方法直接建模音频-视觉相关性，利用Stable Diffusion的强大能力。针对扩散过程中时间一致性问题，提出时间表示对齐（TREPA）方法增强一致性。此外，改进了SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有方法。创新点包括潜在扩散模型直接应用、TREPA时间对齐技术及SyncNet训练优化，为音频驱动动画提供新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T06:05:43Z
- **目录日期**: 2025-05-24
