...
AgentSociety是一种基于大语言模型的大规模社会模拟器，可模拟超过1万个代理的社会互动（500万次）。研究者用它分析四个关键社会问题：社会极化、信息传播、基本收入政策及飓风等外部冲击的影响。模拟结果与现实世界数据吻合，验证了该系统捕捉人类行为机制的能力。该平台可作为计算社会科学的研究工具，辅助传统方法（如调查、访谈），帮助学者和政策制定者理解社会问题的成因与模式。其潜力在于为社会科学研究提供可扩展的仿真实验环境。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T22:04:59Z
- **目录日期**: 2025-05-24
