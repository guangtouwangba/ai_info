...
AI系统在理解图像隐喻方面面临挑战，现有模型难以把握视觉内容中的文化、情感和语境隐含意义。为此，研究团队提出Let Androids Dream (LAD)框架，通过三阶段处理：感知（将视觉信息转为多层次文本表征）、搜索（整合跨领域知识消除歧义）和推理（生成语境对齐的隐含意义）。LAD在轻量级GPT-4o-mini模型上实现了英语图像隐喻基准的SOTA性能，中文基准显著提升，在选择题上与GPT-4o相当，开放式问题表现优于36.7%。该研究为AI理解图像隐喻提供了新思路，推动视觉-语言推理领域发展。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T07:01:07Z
- **目录日期**: 2025-05-24
