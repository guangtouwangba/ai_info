...
该论文提出了一种基于扩散模型的虚拟试衣（VTON）新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改进服装细节保留。传统双分支UNet架构直接将服装作为外观参考输入，而新方法将服装细节表示为结构化语义点，通过局部流变形匹配目标人物的语义点，并结合深度/法线图增强为3D感知线索。这种对应关系模拟了穿衣过程，3D线索监督扩散模型训练，并设计了点聚焦扩散损失来利用语义点匹配。实验在VITON-HD和DressCode数据集上取得了最先进的性能，显著提升了服装细节保留效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T19:02:13Z
- **目录日期**: 2025-05-24
