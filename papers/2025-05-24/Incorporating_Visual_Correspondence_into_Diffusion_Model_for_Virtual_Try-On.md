...
该研究提出了一种基于扩散模型的虚拟试衣（VTON）新方法SPM-Diff，通过显式利用视觉对应关系作为先验来优化扩散过程。不同于传统方法直接将服装整体输入UNet，该方法将服装细节表示为结构化语义点，通过局部流变形匹配目标人物的语义点，并结合深度/法线图增强为3D感知线索。这种对应关系模拟了穿衣过程，3D线索则作为语义点匹配监督扩散模型训练。此外，设计了点聚焦扩散损失以充分利用语义点匹配。实验表明，该方法在VITON-HD和DressCode数据集上实现了最先进的VTON性能，有效保留了服装细节。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T16:02:22Z
- **目录日期**: 2025-05-24
