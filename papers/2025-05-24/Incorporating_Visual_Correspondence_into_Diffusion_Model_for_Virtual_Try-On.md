...
该论文提出了一种基于扩散模型的虚拟试穿(VTON)新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改进细节保留。不同于传统方法直接将服装输入UNet，该方法将服装细节表示为结构化语义点，通过局部流变形匹配到目标人物，并结合深度/法线图增强为3D感知线索。这种对应关系模拟了真实穿衣过程，3D线索作为语义点匹配监督扩散模型训练。此外还设计了点聚焦扩散损失以充分利用语义点匹配。在VITON-HD和DressCode数据集上的实验表明，该方法在服装细节保留方面达到最先进水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T02:30:00Z
- **目录日期**: 2025-05-24
