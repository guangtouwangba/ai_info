...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，使大模型能像人类一样进行抽象跳跃式思考。该方法通过引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多推理路径隐式聚合，不仅使思考更灵活，还减少22.4%的token使用量。实验显示QwQ-32B模型准确率最高提升6.45%，并配备Cold Stop机制防止模型"卡壳"。这种即插即用的创新方法有望成为未来大模型推理的标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T09:03:03Z
- **目录日期**: 2025-05-24
