...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法通过引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多推理路径隐式聚合。相比传统CoT，不仅更灵活抽象，还能减少22.4%的token使用量。测试显示，QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升达6.45%。该方法还包含Cold Stop机制防止模型"卡壳"。这种即插即用的"软推理"有望成为大模型推理的新标准，使AI更接近人类跳跃式思维。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T07:03:15Z
- **目录日期**: 2025-05-24
