...
SimularAI与微软DeepSpeed联合提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多条推理路径隐式聚合。相比传统CoT，软推理更灵活抽象，减少22.4% token使用量，并提升模型表现(QwQ-32B准确率最高提升2.48%，AIME 2024数据集提升6.45%)。该方法还包含Cold Stop机制防止模型"卡壳"。这种即插即用的创新方法使大模型能像人类一样进行跳跃式思考，或将成为未来大模型推理的标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T14:03:09Z
- **目录日期**: 2025-05-24
