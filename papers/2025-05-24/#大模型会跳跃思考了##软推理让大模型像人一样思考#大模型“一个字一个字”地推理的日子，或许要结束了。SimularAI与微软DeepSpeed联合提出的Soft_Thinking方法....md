...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)的限制。该方法通过引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多条推理路径的隐式聚合。相比传统CoT，软推理更抽象灵活，减少22.4% token使用量，并提升推理效率。测试显示，QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升6.45%。该方法还包含Cold Stop机制，可监测模型自信程度，判断何时停止推理。这种即插即用的软推理或将成为大模型推理的未来标配方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T17:03:31Z
- **目录日期**: 2025-05-24
