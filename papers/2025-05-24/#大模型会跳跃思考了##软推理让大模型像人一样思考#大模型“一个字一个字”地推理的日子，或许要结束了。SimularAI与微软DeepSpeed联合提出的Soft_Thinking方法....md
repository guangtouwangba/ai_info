...
SimularAI与微软DeepSpeed团队提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，使大模型具备类人抽象思考能力。该方法通过引入"连续概念空间"和"概念token"，用概率分布替代单一token选择，实现多推理路径隐式聚合。实验显示，QwQ-32B模型准确率最高提升2.48%，AIME 2024数据集提升6.45%，同时减少22.4% token消耗。创新的Cold Stop机制可动态判断推理终止时机。这种即插即用的方法支持模型像人类一样保持多种解题思路，有望成为未来大模型推理的标准范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T19:03:15Z
- **目录日期**: 2025-05-24
