...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，让大模型实现类人抽象思考。该方法创新性地引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，支持多条推理路径隐式聚合，使模型能同时保留多种解题思路。实验显示，该方法减少22.4% token使用量，在QwQ-32B模型上最高提升2.48%准确率，AIME 2024数据集提升达6.45%。结合Cold Stop防卡壳机制，这种即插即用的"软推理"有望成为未来大模型推理的标配方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T12:03:17Z
- **目录日期**: 2025-05-24
