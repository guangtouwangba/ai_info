...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法通过"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多推理路径隐式聚合。相比传统CoT，软推理更接近人类抽象思维，能同时保留多种解题思路，减少22.4% token使用量。测试显示QwQ-32B模型准确率最高提升2.48%，AIME 2024数据集提升6.45%。该方法还引入Cold Stop机制防止模型"卡壳"，有望成为大模型推理的新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T16:03:15Z
- **目录日期**: 2025-05-24
