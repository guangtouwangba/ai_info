...
SimularAI与微软DeepSpeed合作提出的"软推理"(Soft Thinking)方法突破传统思维链(CoT)限制，使大模型能像人类一样进行抽象思考。该方法通过引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多条推理路径的隐式聚合，不仅提升思考灵活性，还减少22.4%的token使用量。测试显示QwQ-32B模型准确率最高提升6.45%，并配备Cold Stop机制防止推理卡壳。这种即插即用的创新方法可能成为未来大模型推理的标准配置。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T22:03:20Z
- **目录日期**: 2025-05-24
