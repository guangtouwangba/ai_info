...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，让大模型实现类人抽象思考。该方法创新性地引入连续概念空间和概念token，用概率分布代替单一token选择，支持多推理路径隐式聚合。实验显示，QwQ-32B模型准确率最高提升2.48%，AIME 2024数据集提升6.45%，同时减少22.4% token使用。配套的Cold Stop机制可监测模型置信度，智能终止推理。这种即插即用的软推理技术，显著提升了AI的思维灵活性和效率，有望成为未来大模型的标准推理方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T11:03:02Z
- **目录日期**: 2025-05-24
