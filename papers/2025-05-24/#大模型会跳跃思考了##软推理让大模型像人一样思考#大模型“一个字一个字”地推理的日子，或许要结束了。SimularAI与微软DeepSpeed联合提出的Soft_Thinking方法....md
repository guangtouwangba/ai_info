...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法通过"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多推理路径隐式聚合。相比传统CoT，该方法减少22.4% token使用量，在QwQ-32B模型上准确率最高提升2.48%，AIME 2024数据集提升6.45%。其创新性包括：1）支持并行保留多种解题思路；2）Cold Stop机制防止推理卡壳；3）实现类似人脑的跳跃式思考。这种即插即用的方法有望成为大模型推理新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T13:09:53Z
- **目录日期**: 2025-05-24
