...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，让大模型实现类人抽象思考。该方法通过"连续概念空间"和"概念token"，用概率分布替代单一token选择，支持多推理路径隐式聚合，使模型能同时保留多种解题思路。实验显示，QwQ-32B模型准确率最高提升2.48%，AIME 2024数据集提升6.45%，并减少22.4% token使用。搭配Cold Stop机制监测推理自信度，这种即插即用的创新方法有望成为未来大模型推理标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-24T15:03:05Z
- **目录日期**: 2025-05-24
