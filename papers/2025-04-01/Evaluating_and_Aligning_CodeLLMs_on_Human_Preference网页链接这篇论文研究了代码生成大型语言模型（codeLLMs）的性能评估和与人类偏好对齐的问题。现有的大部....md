...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，指出当前基准测试过于关注代码正确性而忽略人类偏好。研究者创建了CodeArena基准（397个高质量样本，覆盖40类/44种语言）和SynCode-Instruct语料库（20B标记），发现执行基准与人类偏好存在差异。实验比较40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，强调了对齐人类偏好的重要性。研究成果突显了现有评估体系的局限性，为改进代码LLMs提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T12:06:17Z
- **目录日期**: 2025-04-01
