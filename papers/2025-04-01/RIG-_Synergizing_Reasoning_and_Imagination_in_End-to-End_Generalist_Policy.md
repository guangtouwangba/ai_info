...
本文提出了一种名为RIG的端到端通用策略，首次将推理（Reasoning）与想象（Imagination）能力协同整合。通过构建渐进式数据管道，RIG在轨迹数据中联合学习推理与下一帧生成，显式建模环境动态与行动的关联，相比现有方法样本效率提升17倍以上。推理时，RIG先预测行动，再想象结果，使智能体能在执行前自我修正。实验表明，这种协同机制不仅增强了策略的鲁棒性、泛化性和互操作性，还能通过测试时扩展提升整体性能，为复杂开放环境中的具身智能体提供了新范式。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T10:01:07Z
- **目录日期**: 2025-04-01
