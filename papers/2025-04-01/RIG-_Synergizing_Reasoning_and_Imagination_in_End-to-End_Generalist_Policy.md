...
本文提出了一种名为RIG的端到端通用策略，首次将推理（Reasoning）与想象（Imagination）能力协同整合。通过构建渐进式数据管道，联合训练推理与下一帧生成，显式建模环境动态与行动的关联性，其样本效率比现有方法提升17倍以上。推理时，RIG先预测行动，再想象结果，使智能体能在真实行动前自我修正。实验表明，这种协同显著提升了策略的鲁棒性、泛化性和互操作性，并支持测试时扩展以增强整体性能。该方法为复杂开放环境中的具身智能体提供了更高效的决策框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T03:18:52Z
- **目录日期**: 2025-04-01
