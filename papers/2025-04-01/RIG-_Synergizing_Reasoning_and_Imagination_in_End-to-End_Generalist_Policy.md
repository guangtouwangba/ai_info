...
本文提出RIG模型，首次在端到端通用策略中协同推理与想象能力。通过构建渐进式数据管道，整合并丰富现有智能体轨迹中的推理和想象内容，实现联合学习。该方法显式建模了推理、动作与环境动态的关联，相比之前工作提升17倍样本效率并增强泛化性。推理时，RIG先推理动作，生成潜在动作并预测结果，使智能体能在真实行动前自我修正。实验表明，推理与想象的协同不仅提高了策略的鲁棒性、泛化性和互操作性，还能通过测试时扩展提升整体性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T11:01:01Z
- **目录日期**: 2025-04-01
