...
本文提出了一种名为RIG的端到端通用策略，首次将推理（Reasoning）与想象（Imagination）能力协同整合。通过构建渐进式数据管道，RIG联合学习推理与下一帧图像生成，显式建模了推理、动作与环境动态的关联性，相比现有方法样本效率提升17倍以上且泛化性更强。推理时，RIG先预测动作，再生成可能结果，使代理能在真实行动前自我修正。实验表明，这种协同不仅提升了策略的鲁棒性、泛化性和互操作性，还能通过测试时扩展进一步提升整体性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T04:02:01Z
- **目录日期**: 2025-04-01
