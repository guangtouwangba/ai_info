...
本文提出了一种名为RIG的端到端通用策略，首次将推理与想象能力协同整合。通过构建渐进式数据管道，RIG在现有智能体轨迹中融合并丰富推理与想象内容，联合学习推理和下一帧图像生成，显式建模推理、动作与环境动态的关联性。相比先前方法，RIG展现出17倍以上的样本效率提升和泛化能力。推理时RIG先推理动作，生成潜在动作并预测结果，使智能体能在真实行动前自我修正。实验表明，这种协同显著提升了策略的鲁棒性、泛化性和互操作性，并能通过测试时扩展提升整体性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T14:01:10Z
- **目录日期**: 2025-04-01
