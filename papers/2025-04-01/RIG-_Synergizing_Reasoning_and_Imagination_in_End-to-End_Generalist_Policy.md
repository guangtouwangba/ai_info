...
本文提出RIG模型，首次将推理（Reasoning）与想象（Imagination）能力整合到端到端的通用策略中。通过构建渐进式数据管道，RIG联合学习推理与图像生成，显式建模推理、动作与环境动态的关联，样本效率提升17倍以上。推理时，RIG先推理动作，生成潜在动作并预测结果，使代理能在真实行动前自我校正。实验表明，推理与想象的协同不仅增强了策略的鲁棒性、泛化性和互操作性，还能通过测试时扩展提升整体性能。该方法突破了传统专用模型集成的限制，显著提升了学习效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T20:01:05Z
- **目录日期**: 2025-04-01
