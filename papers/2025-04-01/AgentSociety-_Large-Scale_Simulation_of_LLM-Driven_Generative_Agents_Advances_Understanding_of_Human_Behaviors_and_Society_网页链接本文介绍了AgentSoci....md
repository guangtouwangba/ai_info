...
AgentSociety是一个基于大语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（500万次交互）。研究者用它分析了四个关键社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响和外部冲击（如飓风）效应。模拟结果与现实世界数据吻合，验证了该工具在揭示人类行为机制方面的有效性。该系统可作为计算社会科学研究平台，辅助传统方法（如调查、访谈），帮助学者和政策制定者理解社会问题的成因与模式，具有重要的研究与应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T04:06:02Z
- **目录日期**: 2025-04-01
