...
本文提出了一种基于强化学习(RL)的去中心化无线网络公平共享方案FSRL。该方案允许多个源节点在无协调、无先验网络信息的情况下，通过观察自身传输结果(成功/冲突)，自适应调整频段选择策略。FSRL结合了：(1)半自适应时间参考的状态增强；(2)利用风险控制和时差似然的架构；(3)公平驱动的奖励机制。在50多种网络场景测试中(包括不同节点数、频谱资源、干扰和自组织环境)，FSRL相比基线RL算法，在严苛条件下(多源节点单频段)公平性提升达89%，平均提升48.1%(以Jain公平指数衡量)。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T10:02:05Z
- **目录日期**: 2025-04-01
