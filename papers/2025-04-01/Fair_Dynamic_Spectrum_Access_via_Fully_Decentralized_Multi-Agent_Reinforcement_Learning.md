...
本文提出了一种基于强化学习的去中心化无线网络公平共享方案（FSRL），用于多源-多目标场景下的频谱分配。该方案无需节点间协调或先验网络信息，仅通过观察自身传输结果（成功/碰撞）进行学习。FSRL结合了三种创新机制：(1)半自适应时间参考的状态增强，(2)风险控制与时差似然架构，(3)公平驱动的奖励结构。在50多种网络配置（不同节点数、频谱资源、干扰及自组网场景）的测试中，FSRL相比基线算法显著提升了公平性：在严苛单频带多源场景下公平性提升达89%（Jain指数），平均提升48.1%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T06:02:03Z
- **目录日期**: 2025-04-01
