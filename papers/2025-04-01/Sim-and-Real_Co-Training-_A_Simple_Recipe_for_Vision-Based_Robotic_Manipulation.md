...
该研究探讨了结合仿真与真实世界数据共同训练机器人策略的有效方法。尽管大规模真实机器人数据集对训练通用模型至关重要，但其收集成本高昂。仿真数据可作为补充，但单独训练的策略迁移到现实时存在"现实差距"。研究表明，混合训练策略能平均提升38%的真实任务表现，即使仿真与真实数据存在显著差异。通过机器人手臂和人形机器人等多任务验证，团队提出了一个简单有效的视觉操作任务训练方案。该方法为利用仿真数据增强真实机器人学习提供了系统指导，相关成果详见项目网站。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T08:01:16Z
- **目录日期**: 2025-04-01
