...
该研究探讨了结合仿真和真实世界数据共同训练机器人策略的有效方法。研究表明，尽管仿真与真实数据存在差异，但混合训练能显著提升视觉机械臂和人形机器人任务的性能，平均提高38%。相比仅用有限真实数据训练，该方法通过生成式AI和自动化工具大规模创建仿真数据，弥补了真实数据收集耗时费力的不足。实验验证了该策略在不同任务和数据集上的有效性，为利用仿真数据增强真实机器人学习提供了实用方案。更多结果详见项目网站。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T17:01:59Z
- **目录日期**: 2025-04-01
