...
这篇论文探讨了如何结合仿真和真实世界数据来训练机器人策略。研究表明，仅靠仿真数据训练的策略在迁移到现实时存在"现实差距"问题，而将仿真数据与少量真实数据混合训练能显著提升性能。通过系统实验，作者提出了一种简单有效的视觉机器人操作任务训练方法。在两个不同机器人平台（机械臂和人形机器人）上的多项任务测试表明，即使仿真与现实数据存在明显差异，仿真数据仍能平均提升38%的现实任务表现。该方法为利用仿真数据增强真实机器人学习提供了实用方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T06:01:09Z
- **目录日期**: 2025-04-01
