...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统观念。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，有效解决了训练不平衡问题，同时将内存使用减半。实验表明，该方法在Transformer模型、ImageNet分类和LLM预训练等任务中表现优于或相当于AdamW，特别在GPT-2和Llama2-7B训练中分别节省了5.93GB和25.15GB内存。此外，SGD-SaI在LoRA微调等场景也展现出优越性能，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T20:04:17Z
- **目录日期**: 2025-04-01
