...
这篇论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法基于梯度信号噪声比（g-SNR）在初始化阶段调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡问题。相比AdamW，SGD-SAi内存效率提升50%，在Transformer任务、ImageNet分类和LLM预训练（如GPT-2/Llama2）中表现优异，最高可节省25GB内存。其优势还包括超参数鲁棒性和在LoRA微调等场景的优越性能，为大规模模型训练提供了更高效的优化方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T16:05:39Z
- **目录日期**: 2025-04-01
