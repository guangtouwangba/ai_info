...
该论文提出了Native Sparse Attention (NSA)机制，用于解决传统注意力模型处理长文本时计算成本高的问题。NSA结合算法创新与硬件优化，采用动态分层稀疏策略：通过粗粒度令牌压缩和细粒度令牌选择，在保持全局上下文感知能力的同时确保局部精度。这一方法显著提升了长文本建模效率，其核心创新在于硬件对齐的设计和原生可训练性。NSA为大规模语言模型提供了一种更高效的注意力计算方案，有助于降低长文本处理的计算开销。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T01:32:53Z
- **目录日期**: 2025-04-01
