...
该论文提出NSA（Native Sparse Attention）稀疏注意力机制，通过动态分层稀疏策略（粗粒度令牌压缩+细粒度令牌选择）优化长文本处理效率。其创新点在于算法与硬件协同设计：1）保持全局上下文感知能力的同时降低计算成本；2）确保局部注意力精度。相比传统注意力机制，NSA显著减少了长文本建模的计算开销，同时维持模型性能。研究为大模型的高效训练提供了新的技术路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T06:05:15Z
- **目录日期**: 2025-04-01
