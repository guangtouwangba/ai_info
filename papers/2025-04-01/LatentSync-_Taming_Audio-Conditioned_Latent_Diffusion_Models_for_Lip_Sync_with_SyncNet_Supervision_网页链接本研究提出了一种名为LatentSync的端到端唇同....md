...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案利用Stable Diffusion直接建模音视频关系，并针对扩散模型的时间一致性问题提出TREPA方法，通过自监督视频特征对齐增强连贯性。研究还优化了SyncNet训练框架，改进模型架构、超参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他基于SyncNet的唇同步和音频驱动动画系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-01T04:06:18Z
- **目录日期**: 2025-04-01
