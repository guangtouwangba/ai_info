...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)构建CodeArena基准测试，包含397个真实编程任务样本(覆盖40类/44种语言)，首次关注模型输出与人类偏好的匹配度；2)创建20B标记量的SynCode-Instruct合成指令集验证微调效果。实验发现：现有执行基准与人类偏好存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著，表明对齐人类偏好的重要性。研究强调了在代码生成评估中除正确性外，还需考虑实际开发场景的适配性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T05:05:52Z
- **目录日期**: 2025-04-11
