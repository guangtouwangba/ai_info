...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法。该框架直接建模音频-视觉关系，无需中间运动表示，克服了传统两阶段方法的局限。针对扩散模型的时间一致性问题，研究者开发了TREPA方法，利用自监督视频模型的时间表示来对齐生成帧。同时，论文深入分析了SyncNet的收敛问题，通过改进模型架构、训练参数和数据预处理提升了性能。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些发现对音频驱动的人像动画领域具有广泛适用性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T18:05:06Z
- **目录日期**: 2025-04-11
