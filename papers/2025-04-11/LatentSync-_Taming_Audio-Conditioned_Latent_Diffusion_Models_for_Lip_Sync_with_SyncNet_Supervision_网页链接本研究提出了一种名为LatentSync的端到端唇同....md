...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，解决了传统方法的时间一致性问题。创新点包括：1) 提出TREPA方法，利用自监督视频模型增强生成帧的时间一致性；2) 对SyncNet进行改进，优化模型架构、训练参数和数据预处理，显著提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T02:32:33Z
- **目录日期**: 2025-04-11
