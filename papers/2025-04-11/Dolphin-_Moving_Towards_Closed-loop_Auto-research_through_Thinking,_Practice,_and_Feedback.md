...
人工智能（AI）正在推动科研范式的变革，提升数据分析、计算效率和创新想法的生成。本文提出Dolphin框架，一个闭环的LLM驱动系统，旨在提高科研自动化水平。Dolphin通过分析先前实验和论文反馈生成新想法，利用优化的代码模板实现这些想法，并自动分析结果以优化下一轮研究。实验表明，Dolphin能在循环中持续提升输入主题的性能，在某些任务（如3D点分类）中提出的方法甚至媲美最先进技术。这一框架为自动科研的终极目标迈出了重要一步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T01:29:30Z
- **目录日期**: 2025-04-11
