...
这篇论文针对无监督零样本强化学习（RL）中预训练行为基础模型（BFMs）的零样本策略性能不足问题，提出了一种快速适应策略。研究发现，现有BFMs学习的技能集中包含比推理过程识别出的更优策略，因此适合快速适应。作者提出了基于行动者-评论家和纯行动者的低维任务嵌入空间搜索方法，能在少量环境交互步骤中提升零样本策略性能，且避免了预训练模型微调初期常见的"遗忘"阶段。实验表明，该方法在导航和运动任务中仅需几十次交互就能将零样本性能提升10-40%，优于现有基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T15:01:36Z
- **目录日期**: 2025-04-11
