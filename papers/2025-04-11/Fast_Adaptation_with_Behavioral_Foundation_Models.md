...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFMs），使智能体能在零样本条件下解决多种下游任务（仅需奖励函数，无需额外学习或规划）。然而，由于训练误差、嵌入偏差和推理缺陷，零样本策略常表现欠佳。研究发现，BFMs实际学习的技能集包含比推理结果更优的策略，因此适合快速适应。本文提出基于任务嵌入空间的行动者-评论家/纯行动者快速适应策略，能在数十次环境交互中提升性能10-40%，且避免预训练模型的初始性能下降问题。实验在导航和运动任务中验证了该方法的优越性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T22:01:16Z
- **目录日期**: 2025-04-11
