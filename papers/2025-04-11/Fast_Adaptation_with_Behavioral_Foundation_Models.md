...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFM），使智能体能零样本解决多种下游任务（即无需额外测试时学习或规划）。然而，零样本策略常因训练误差、嵌入或推理过程导致性能欠佳。本文提出快速适应策略，通过少量在线交互提升BFM的零样本性能，同时避免适应过程中的性能下降。研究发现，BFM已学习的技能中包含优于推理策略的方案，适合快速适应。基于此，作者提出基于任务嵌入空间的行动者-评论家与纯行动者适应策略，在导航和运动任务中实现10-40%的性能提升，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T12:01:25Z
- **目录日期**: 2025-04-11
