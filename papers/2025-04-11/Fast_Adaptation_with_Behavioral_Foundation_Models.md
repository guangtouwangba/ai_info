...
本文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，现有BFMs学习的技能集包含比推理过程识别的更优策略，适合快速适应。作者提出了基于行动者-评论家和纯行动者的低维任务嵌入空间搜索方法，能在少量环境交互步骤中提升零样本策略表现，避免预训练模型微调时常见的性能下降。实验表明，该策略在导航和运动任务中仅需数十次交互即可实现10-40%的性能提升，优于现有基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T10:01:18Z
- **目录日期**: 2025-04-11
