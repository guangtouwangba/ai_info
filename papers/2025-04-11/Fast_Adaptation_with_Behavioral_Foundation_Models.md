...
本文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的初始表现。研究发现，现有BFMs已学习到比推理程序识别出的更优策略，但零样本策略常因训练和推理误差导致次优表现。作者提出了基于任务嵌入空间的演员-评论家和纯演员快速适应方法，能在数十次环境交互中提升10-40%的性能，同时避免了预训练模型微调初期常见的"遗忘"现象。实验在导航和运动控制任务中验证了该策略对四种先进零样本RL方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T14:01:21Z
- **目录日期**: 2025-04-11
