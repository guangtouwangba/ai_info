...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题引发关注。研究发现，训练数据中的历史不平等、语言不平衡等因素会导致模型产生偏见，且现有安全机制仍易受对抗攻击。本研究提出可扩展的评估框架，通过多任务测试、自动化安全评分和越狱技术来检测模型偏见，并分析不同规模模型的安全权衡。特别评估了医疗等关键领域专用模型的安全性，同时发布CLEAR-Bias数据集以支持系统性漏洞评估。结果表明模型规模与安全性存在关键权衡，为开发更公平、鲁棒的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T14:01:31Z
- **目录日期**: 2025-04-11
