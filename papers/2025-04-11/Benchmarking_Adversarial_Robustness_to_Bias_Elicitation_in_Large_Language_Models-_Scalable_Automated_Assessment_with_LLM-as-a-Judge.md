...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了偏见问题。这些偏见源于训练数据中的历史不平等、语言不平衡和对抗性操纵。研究提出可扩展的评估框架，通过多任务测试、安全评分量化(采用LLM自动评估)和越狱技术，系统分析不同规模模型在多元社会文化维度上的偏见表现。研究还评估了医疗等关键领域专业模型的安全性，并发布CLEAR-Bias数据集用于系统漏洞测试。结果显示模型规模与安全性之间存在关键权衡，为开发更公平、更鲁棒的模型提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T10:01:28Z
- **目录日期**: 2025-04-11
