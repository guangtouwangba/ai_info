...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了关于偏见的担忧。这些偏见源于训练数据的不平等、语言不平衡和对抗性操纵。研究提出了一种可扩展的评估框架，通过多任务方法系统探测模型在不同社会文化维度上的偏见，采用LLM-as-a-Judge方法量化模型安全性，并利用越狱技术测试安全机制的脆弱性。分析发现模型大小与安全性存在关键权衡，同时评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias数据集以促进系统性漏洞评估，为开发更公平、更稳健的语言模型提供依据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T13:08:27Z
- **目录日期**: 2025-04-11
