...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用中存在的偏见问题日益凸显。该研究提出可扩展的基准测试框架CLEAR-Bias，通过多任务方法系统评估模型在不同社会文化维度的偏见表现，采用LLM-as-a-Judge自动评分机制量化模型鲁棒性，并利用越狱技术检测安全机制漏洞。研究发现模型大小与安全性之间存在关键权衡，领域专用模型(如医疗)同样存在偏见风险。研究发布了包含偏见提示的精选数据集，为开发更公平、更鲁棒的语言模型提供了系统性漏洞评估工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T21:01:28Z
- **目录日期**: 2025-04-11
