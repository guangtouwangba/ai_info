...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。本研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化等方法系统评估LLMs对抗偏见诱导的鲁棒性，并分析不同规模模型及医疗等专业领域模型的安全表现。研究发现模型规模与安全性之间存在关键权衡，研究发布的偏见提示数据集可为开发更公平、鲁棒的模型提供基准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T17:01:34Z
- **目录日期**: 2025-04-11
