...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。研究提出可扩展的评估框架CLEAR-Bias，通过多任务探测、安全评分量化等方法系统评估模型对抗偏见诱导的鲁棒性，并分析不同规模模型的安全权衡。研究发现即使最先进模型仍易受针对性攻击，领域专用模型(如医疗)同样存在安全隐患。该研究为开发更公平、更鲁棒的语言模型提供了重要参考依据，并公开了相关测试数据集。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T11:01:26Z
- **目录日期**: 2025-04-11
