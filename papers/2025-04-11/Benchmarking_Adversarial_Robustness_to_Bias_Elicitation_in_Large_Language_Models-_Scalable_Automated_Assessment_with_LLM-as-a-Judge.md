...
大型语言模型(LLMs)在推动AI进步的同时，其内置偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。本研究提出可扩展的评估框架，通过多任务方法系统探测模型在社会文化维度的偏见，采用"LLM即裁判"自动评分量化稳健性，并利用越狱技术测试安全机制漏洞。分析发现模型规模与安全性存在关键权衡，同时评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias偏见提示数据集，为开发更公平、稳健的模型提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T05:01:33Z
- **目录日期**: 2025-04-11
