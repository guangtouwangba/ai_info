...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了关于偏见的担忧。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，且模型易受对抗性攻击影响。本研究提出可扩展的评估框架：通过多任务方法系统探测模型在不同社会文化维度上的偏见；采用LLM-as-a-Judge方法量化模型安全性；利用越狱技术测试安全机制漏洞。研究分析了主流模型的偏见情况及其对安全性的影响，评估了医疗等关键领域专用模型的安全性，并发布了CLEAR-Bias数据集。结果显示模型规模与安全性之间存在重要权衡关系。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T12:01:36Z
- **目录日期**: 2025-04-11
