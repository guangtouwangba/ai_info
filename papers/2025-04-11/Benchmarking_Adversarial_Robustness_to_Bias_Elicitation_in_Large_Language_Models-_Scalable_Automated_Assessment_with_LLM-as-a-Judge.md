...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化及越狱技术检测，系统评估模型对抗偏见诱导的鲁棒性。分析发现模型规模与安全性存在权衡关系，领域专用模型(如医疗)也存在安全隐患。该研究发布了标准化的偏见提示数据集，为开发更公平、更鲁棒的语言模型提供重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T04:01:52Z
- **目录日期**: 2025-04-11
