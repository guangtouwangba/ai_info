...
大型语言模型(LLMs)在推动AI进步的同时，其内置偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。本研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化及越狱技术，系统评估LLMs对抗偏见诱导的鲁棒性。研究发现模型大小与安全性之间存在关键权衡，并揭示了领域专用模型(如医疗领域)的安全隐患。该研究为开发更公平、更鲁棒的语言模型提供了重要参考，同时发布了包含偏见提示的标准化测试数据集以促进系统性漏洞评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T16:01:29Z
- **目录日期**: 2025-04-11
