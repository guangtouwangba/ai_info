...
大型语言模型(LLMs)推动了AI发展，但也存在内嵌偏见问题，可能延续刻板印象并影响公平性。这些偏见源于训练数据中的历史不平等、语言不平衡和对抗性操纵。研究提出可扩展的基准框架评估LLMs对抗偏见诱导的鲁棒性，包括：(1)多任务系统性探测模型偏见，(2)通过LLM-as-a-Judge方法量化安全评分，(3)利用越狱技术检测安全机制漏洞。分析显示不同规模模型普遍存在偏见，并影响安全性。研究还评估了医疗等关键领域微调模型的安全性，并发布CLEAR-Bias数据集用于系统漏洞评估，揭示了模型规模与安全性间的关键权衡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T06:01:27Z
- **目录日期**: 2025-04-11
