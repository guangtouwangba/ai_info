...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。该研究提出可扩展的评估框架，通过多任务方法系统检测模型在不同社会文化维度上的偏见表现，采用LLM-as-a-Judge方法自动量化模型安全性，并利用越狱技术测试安全机制的脆弱性。研究分析了主流大/小模型的偏见模式及其对安全性的影响，特别评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias偏见提示数据集，揭示了模型规模与安全性之间的关键权衡，为开发更公平、鲁棒的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T22:01:24Z
- **目录日期**: 2025-04-11
