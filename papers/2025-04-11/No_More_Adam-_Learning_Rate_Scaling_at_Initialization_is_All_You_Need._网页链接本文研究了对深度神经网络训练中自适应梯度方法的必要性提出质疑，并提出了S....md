...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。该方法基于带动量的随机梯度下降（SGDM），通过在初始化阶段根据不同参数组的梯度信噪比（g-SNR）调整学习率，有效解决了训练不平衡问题。SGD-SaI不仅将优化器内存占用减半（相比AdamW节省5.93-25.15GB），在Transformer、ImageNet分类和GPT-2预训练等任务中表现优于或持平AdamW，同时展现出更强的超参数鲁棒性。该方法还显著提升了LoRA微调等场景的性能，为大型模型训练提供了更高效的选择。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T17:04:30Z
- **目录日期**: 2025-04-11
