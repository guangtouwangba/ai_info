...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统观念。该方法通过初始化时根据不同参数组的梯度信号噪声比(g-SNR)调整学习率，改进了带动量的随机梯度下降(SGDM)。相比AdamW，SGD-SaI内存使用减半，在Transformer任务、ImageNet分类、LLM预训练和LoRA微调中表现优异甚至更好。例如，在GPT-2和Llama2-7B训练中分别节省了5.93GB和25.15GB内存。该方法解决了用SGD训练Transformer的难题，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T12:04:35Z
- **目录日期**: 2025-04-11
