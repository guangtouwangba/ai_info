...
这篇论文质疑了深度神经网络训练中自适应梯度方法的必要性，提出了一种改进的随机梯度下降方法SGD-SaI。该方法通过在初始化时根据不同参数组的梯度信号噪声比(g-SNR)调整学习率，无需自适应二阶动量即可避免训练不平衡，并将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或相当于AdamW，同时显著节省内存(GPT-2节省5.93GB，Llama2-7B节省25.15GB)。该方法还具有超参数鲁棒性，在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T09:04:45Z
- **目录日期**: 2025-04-11
