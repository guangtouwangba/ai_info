...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。该方法通过初始化时基于梯度信号噪声比（g-SNR）调整不同参数组的学习率，解决了传统SGD训练不平衡的问题。SGD-SaI不仅内存效率更高（相比AdamW节省5.93-25.15GB），在Transformer、ImageNet分类和LLM预训练等任务中表现优于或持平AdamW，还展现出更强的超参数鲁棒性。特别在LoRA微调等场景下超越了现有最优优化器，为大规模模型训练提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T01:31:57Z
- **目录日期**: 2025-04-11
