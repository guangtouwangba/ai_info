...
该研究表明，在家庭服务项目分类任务中，基于嵌入的softmax模型全面优于当前最先进的LLM提示方法。通过Thumbtack客户提供的文本和图像数据，嵌入方法在准确率（高出49.5%）、校准性、延迟（图像处理快14倍，文本快81倍）和成本（最高可节省10倍）方面均表现更优。实验显示，嵌入方法产生的概率校准良好，适合作为部署时的置信信号，而LLM提示得分信息量不足。A/B测试验证了离线分析结果，证明针对可利用专有数据的多分类问题，传统嵌入方法可能比LLM更具优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T02:28:38Z
- **目录日期**: 2025-04-11
