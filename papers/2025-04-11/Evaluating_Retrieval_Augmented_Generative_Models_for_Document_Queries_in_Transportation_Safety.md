...
该研究评估了ChatGPT、Google Vertex AI和ORNL检索增强生成（RAG）优化的LLaMA 2/LLaMA三种生成式大语言模型在美国危险品运输法规检索中的表现。基于约40份联邦和州法规文件构建100个实际查询，结果显示RAG增强的LLaMA模型在准确性、细节和相关性上显著优于其他模型，尽管偶有不一致。这是RAG技术在运输安全领域的首次应用，研究表明高风险领域需结合领域微调和严格评估以确保可靠性，减少错误风险。研究突显了专业领域大模型应用的挑战与优化方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-11T01:28:59Z
- **目录日期**: 2025-04-11
