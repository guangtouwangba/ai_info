...
本文提出了一种名为"双思维引擎"(DEoT)的新型开放性问题分析框架。该框架包含三个核心组件：用于优化用户查询的基础提示器、负责任务分解与验证的求解代理，以及由广度引擎(探索多元影响因素)和深度引擎(进行深入分析)构成的双引擎系统。相比传统单答案推理模型，DEoT特别适合处理开放性问题，能同时实现广泛覆盖和深度分析。实验表明，DEoT在解决复杂多面问题时表现优异，相比现有推理模型取得77-86%的胜率，且具有高度可定制性，用户可根据需求调整分析参数和工具配置。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T13:05:51Z
- **目录日期**: 2025-04-12
