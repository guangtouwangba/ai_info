...
本文提出了"双重思维引擎"(DEoT)框架，专门用于解决开放性问题。该框架包含三个核心组件：优化用户查询的基础提示器(BP)、协调任务分解执行的求解代理(SA)，以及由广度引擎(探索影响因素)和深度引擎(深入分析)构成的双引擎系统。DEoT能平衡广泛覆盖与深度分析，具有高度可定制性。实验表明，DEoT在解决复杂多面问题时表现优异，相比现有推理模型获得77-86%的胜率，展现了实际应用中的高效性。这一创新框架突破了传统单答案推理的局限，为开放性问题提供了更全面的分析方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T02:27:49Z
- **目录日期**: 2025-04-12
