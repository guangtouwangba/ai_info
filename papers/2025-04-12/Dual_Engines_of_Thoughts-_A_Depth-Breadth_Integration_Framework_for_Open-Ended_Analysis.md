...
研究人员提出了"思维双引擎"(DEoT)分析框架，专门用于解决开放性问题。该框架包含三个核心组件：用于优化用户查询的基础提示器、协调任务分解与执行的求解器代理，以及由广度引擎(探索多元影响因素)和深度引擎(进行深入分析)组成的双引擎系统。实验表明，DEoT在解决复杂多维度问题时表现优异，相比现有推理模型取得77-86%的胜率。该框架支持高度定制化，用户可根据需求调整分析参数和工具配置，实现了广泛覆盖与深度分析的平衡，在实际应用中展现出显著优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T18:01:51Z
- **目录日期**: 2025-04-12
