...
本文提出了一种改进无监督零样本强化学习（RL）中行为基础模型（BFMs）性能的快速适应策略。研究发现，现有BFMs学习的技能集包含比其推理程序识别出的更优策略，适合快速适应。作者提出了基于行动者-评论家和纯行动者的适应策略，在预训练BFMs的低维任务嵌入空间中搜索，以快速提升零样本策略在下游任务中的表现。该方法避免了预训练RL模型微调初期常见的"遗忘"阶段。实验表明，该策略在几十个回合内使零样本性能提升10-40%，优于现有基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T09:01:25Z
- **目录日期**: 2025-04-12
