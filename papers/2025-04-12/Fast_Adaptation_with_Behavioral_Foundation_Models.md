...
本文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，预训练的BFMs已包含优于其推理过程识别的策略技能。基于此，作者设计了基于行动者-评论家和纯行动者的低维任务嵌入空间搜索方法，能在数十次环境交互中快速提升零样本策略表现，避免传统微调中的"遗忘"阶段。实验表明，该方法在导航和运动领域使四种先进零样本RL方法的性能提升10-40%，显著优于现有基线。该策略实现了零样本RL模型的快速在线优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T07:01:32Z
- **目录日期**: 2025-04-12
