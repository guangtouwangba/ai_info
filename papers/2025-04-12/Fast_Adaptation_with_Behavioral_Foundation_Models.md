...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFMs）使智能体能够零样本解决多样下游任务，但常因训练误差导致策略次优。本文提出快速适应策略，在少量环境交互中提升零样本性能，避免适应过程中的性能下降。研究发现，现有BFMs已学习到比推理过程更优的技能，适合快速适应。基于此，作者提出基于任务嵌入空间的行动者-评论家和纯行动者适应策略，在导航和运动任务中验证，仅需数十回合即可提升10-40%性能，优于基线方法且避免了预训练模型的"遗忘"阶段。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T18:01:16Z
- **目录日期**: 2025-04-12
