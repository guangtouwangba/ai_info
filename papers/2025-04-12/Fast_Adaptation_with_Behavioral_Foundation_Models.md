...
本文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，现有BFMs已学习到比推理过程识别出的更优策略，但零样本策略常因训练误差而表现欠佳。作者提出了基于行动者-评论家和纯行动者的低维任务嵌入空间搜索方法，能在数十次环境交互中提升10-40%的下游任务表现，同时避免了预训练模型微调时常见的初期性能下降问题。该方法在导航和运动领域四种先进零样本RL方法上验证有效，显著优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T01:28:24Z
- **目录日期**: 2025-04-12
