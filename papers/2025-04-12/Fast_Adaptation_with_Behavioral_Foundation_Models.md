...
本文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，现有BFMs已学习到比推理程序识别更优的策略技能，但零样本策略常因训练误差而次优。为此，作者设计了基于任务嵌入空间的行动者-评论家和纯行动者适应方法，通过少量在线交互快速优化下游任务表现，避免了预训练模型微调初期的"遗忘"阶段。实验表明，该方法在导航和运动控制任务中仅需数十回合即可将零样本性能提升10-40%，显著优于基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T13:05:10Z
- **目录日期**: 2025-04-12
