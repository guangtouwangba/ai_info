...
这篇论文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，现有BFMs学习的技能集包含比推理过程识别的更优策略，因此适合快速适应。作者提出了基于演员-评论家和纯演员的适应策略，在预训练BFMs的低维任务嵌入空间中搜索，以快速改进零样本策略在下游任务中的表现。该方法避免了预训练RL模型微调时常见的初始"遗忘"阶段。实验表明，该策略在几十个episode内即可将零样本性能提升10-40%，优于现有基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T12:01:21Z
- **目录日期**: 2025-04-12
