...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFM），使智能体能够零样本解决多种下游任务（无需额外学习或规划）。然而，零样本策略常因训练误差、嵌入或推理过程导致性能不足。研究发现，BFM隐含的技能库中存在优于推理结果的策略，适合快速适应。本文提出基于任务嵌入空间的快速适应策略（包括行动者-评论家和纯行动者方法），通过少量环境交互提升零样本性能，避免了预训练模型的初始性能下降问题。实验表明，该方法在导航和运动任务中仅需数十步即可提升10-40%性能，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T19:01:17Z
- **目录日期**: 2025-04-12
