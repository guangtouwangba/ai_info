...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFM），使智能体能够零样本（无需额外学习或规划）解决多种下游任务。然而，零样本策略常因训练误差、嵌入或推理过程而表现不佳。本文提出快速适应策略，通过少量在线交互提升BFM的零样本性能，同时避免适应过程中的性能下降。研究发现，BFM已学习到比推理结果更优的策略技能，适合快速适应。基于此，作者提出基于任务嵌入空间的快速适应方法，在导航和运动任务中，仅需数十回合即可提升10-40%的性能，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T17:01:27Z
- **目录日期**: 2025-04-12
