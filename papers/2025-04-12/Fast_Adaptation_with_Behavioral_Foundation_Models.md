...
本文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，现有BFMs学习的技能集包含比推理过程识别的更优策略，适合快速适应。作者提出了基于行动者-评论家和纯行动者的适应策略，在预训练BFM的低维任务嵌入空间中搜索，以快速提升零样本策略在下游任务中的表现。该方法避免了预训练RL模型微调中常见的初始"遗忘"阶段。实验表明，在导航和运动任务中，仅需几十次交互即可提升10-40%的性能，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T15:01:21Z
- **目录日期**: 2025-04-12
