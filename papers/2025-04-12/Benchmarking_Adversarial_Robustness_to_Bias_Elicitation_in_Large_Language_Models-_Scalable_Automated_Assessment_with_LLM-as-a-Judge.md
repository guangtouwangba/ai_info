...
大型语言模型(LLMs)在推动AI进步的同时，其嵌入的偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言失衡和对抗性操纵。研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化(采用LLM-as-a-Judge方法)和越狱技术，系统评估模型对抗偏见诱导的鲁棒性。分析发现：1)不同规模的最先进模型普遍存在偏见；2)模型大小与安全性存在权衡；3)医疗等关键领域的微调模型也存在安全隐患。该研究为开发更公平、更鲁棒的语言模型提供了基准工具和数据集支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T02:27:21Z
- **目录日期**: 2025-04-12
