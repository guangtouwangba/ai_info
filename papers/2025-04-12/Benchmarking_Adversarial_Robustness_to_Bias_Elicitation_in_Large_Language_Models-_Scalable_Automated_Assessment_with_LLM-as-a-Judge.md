...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了关于偏见的担忧。这些偏见源于训练数据的不平等、语言不平衡和对抗性操纵。本研究提出可扩展的基准框架评估LLMs对抗偏见诱导的鲁棒性，方法包括：(1)多任务系统性探测模型偏见；(2)采用LLM-as-a-Judge自动评估安全分数；(3)利用越狱技术检测安全机制漏洞。研究分析了不同规模先进模型的偏见及其对安全性的影响，评估了医疗等关键领域专用模型的安全性，并发布CLEAR-Bias数据集。结果显示模型规模与安全性之间存在关键权衡，为开发更公平、鲁棒的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T15:01:32Z
- **目录日期**: 2025-04-12
