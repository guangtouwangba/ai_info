...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益凸显。这些偏见源于训练数据的不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。本研究提出了一个可扩展的评估框架，通过多任务方法系统性检测模型在不同社会文化维度的偏见表现，采用LLM-as-a-Judge方法量化模型安全性，并利用越狱技术测试安全机制的脆弱性。研究发现模型规模与安全性之间存在关键权衡，同时评估了医疗等关键领域专用模型的安全性。研究还发布了CLEAR-Bias数据集，为系统性漏洞评估提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T22:01:19Z
- **目录日期**: 2025-04-12
