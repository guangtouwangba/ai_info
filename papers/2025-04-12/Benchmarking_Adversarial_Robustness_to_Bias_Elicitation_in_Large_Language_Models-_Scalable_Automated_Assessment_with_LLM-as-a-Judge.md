...
大型语言模型(LLMs)在推动AI进步的同时，其嵌入的偏见问题日益凸显。这些偏见源于训练数据中的历史不平等、语言不平衡和对抗性操纵。本研究提出可扩展的基准测试框架CLEAR-Bias，通过多任务方法系统评估模型在不同社会文化维度上的偏见表现，采用LLM-as-a-Judge方法量化模型安全性，并利用越狱技术检测安全机制漏洞。研究发现模型规模与安全性存在关键权衡，领域专用模型(如医疗)也存在安全隐患。该研究为开发更公平、更鲁棒的语言模型提供了重要参考，并公开了偏见提示数据集以促进系统性漏洞评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T08:01:37Z
- **目录日期**: 2025-04-12
