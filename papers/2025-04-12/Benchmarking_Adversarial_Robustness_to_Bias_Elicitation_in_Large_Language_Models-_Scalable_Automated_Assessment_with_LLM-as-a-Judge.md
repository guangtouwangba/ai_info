...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了偏见问题。这些偏见源于训练数据中的历史不平等、语言不平衡和对抗性操纵。研究提出了一种可扩展的评估框架，通过多任务方法系统探测模型在社会文化维度的偏见，采用LLM-as-a-Judge自动评分量化鲁棒性，并利用越狱技术检测安全机制漏洞。分析揭示了模型大小与安全性之间的关键权衡，评估了医疗等关键领域专业模型的安全性。研究发布了CLEAR-Bias数据集以支持系统性漏洞评估，为开发更公平、更鲁棒的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T16:01:38Z
- **目录日期**: 2025-04-12
