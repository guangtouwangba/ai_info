...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。该研究提出可扩展的评估框架，通过多任务测试、安全评分和越狱技术，系统评估LLMs对抗偏见诱导的鲁棒性。研究发现：1)模型普遍存在社会文化维度偏见；2)模型规模与安全性存在权衡关系；3)医疗等专业领域微调模型同样存在安全隐患。研究发布了CLEAR-Bias数据集以支持系统性漏洞评估，为开发更公平、鲁棒的下一代语言模型提供重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T09:01:37Z
- **目录日期**: 2025-04-12
