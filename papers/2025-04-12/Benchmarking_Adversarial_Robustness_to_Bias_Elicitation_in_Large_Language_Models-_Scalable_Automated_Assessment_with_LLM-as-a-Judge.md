...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。研究表明，训练数据中的历史不平等、语言不平衡等因素会导致模型产生偏见，甚至可能被对抗性攻击利用。本研究提出一个可扩展的评估框架，通过多任务测试、安全评分量化等方法系统评估LLMs对抗偏见诱导的鲁棒性。分析发现模型大小与安全性存在关键权衡，并揭示了专业领域模型(如医疗)的安全隐患。研究团队发布了CLEAR-Bias数据集以促进系统性漏洞评估，为开发更公平、更鲁棒的语言模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T01:28:33Z
- **目录日期**: 2025-04-12
