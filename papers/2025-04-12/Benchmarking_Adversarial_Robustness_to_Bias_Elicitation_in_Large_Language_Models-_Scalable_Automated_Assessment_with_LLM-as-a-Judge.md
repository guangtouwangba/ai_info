...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用中的偏见问题日益凸显。这些偏见源于训练数据、语言不平衡和对抗性攻击。本研究提出可扩展的评估框架，通过多任务测试、安全评分量化(采用LLM作为评判者)和越狱技术，系统评估LLMs对抗偏见诱导的鲁棒性。研究分析了不同规模前沿模型的偏见模式及其安全性影响，特别关注医疗等关键领域的专业模型。研究发布了CLEAR-Bias数据集用于系统性漏洞评估，揭示了模型规模与安全性之间的关键权衡，为开发更公平、鲁棒的模型提供了依据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T14:01:25Z
- **目录日期**: 2025-04-12
