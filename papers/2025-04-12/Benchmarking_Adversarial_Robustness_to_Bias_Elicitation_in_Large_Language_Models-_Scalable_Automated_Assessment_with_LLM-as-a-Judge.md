...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用引发了关于偏见问题的担忧。这些偏见源于训练数据中的历史不平等、语言不平衡等因素。研究提出了一种可扩展的评估框架，通过多任务方法系统检测模型在不同社会文化维度上的偏见，采用LLM-as-a-Judge方法量化模型安全性，并利用越狱技术测试安全机制的脆弱性。分析揭示了模型大小与安全性之间的关键权衡，评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias偏见提示数据集，为开发更公平、更鲁棒的语言模型提供了支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T06:01:34Z
- **目录日期**: 2025-04-12
