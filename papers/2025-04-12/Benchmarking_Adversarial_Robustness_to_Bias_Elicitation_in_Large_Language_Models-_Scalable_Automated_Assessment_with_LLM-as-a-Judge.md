...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据的不平等、语言不平衡和对抗性操纵。本研究提出可扩展的基准测试框架CLEAR-Bias，通过多任务探测、安全评分量化(采用LLM-as-a-Judge方法)和越狱技术，系统评估模型对抗偏见诱导的鲁棒性。研究发现不同规模的前沿模型普遍存在偏见，揭示了模型大小与安全性之间的关键权衡，并评估了医疗等关键领域专用模型的安全性。该研究为开发更公平、更鲁棒的语言模型提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T13:05:22Z
- **目录日期**: 2025-04-12
