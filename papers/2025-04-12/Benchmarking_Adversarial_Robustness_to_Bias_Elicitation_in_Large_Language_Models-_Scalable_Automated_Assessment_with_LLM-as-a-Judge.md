...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了偏见问题。这些偏见源于训练数据的不平等、语言不平衡和对抗性操纵。研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试模型在不同社会文化维度的偏见表现，采用LLM-as-a-Judge方法自动量化模型安全性，并利用越狱技术检测安全机制漏洞。分析发现模型大小与安全性存在权衡关系，领域专用模型(如医疗)也存在安全隐患。研究发布了偏见提示数据集，为开发更公平、更鲁棒的语言模型提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T05:01:34Z
- **目录日期**: 2025-04-12
