...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1)构建CodeArena基准测试，包含397个真实编程场景样本，覆盖40类任务和44种语言，首次关注模型输出与人类偏好的匹配度；2)创建20B标记量的SynCode-Instruct合成指令集验证大规模微调效果。研究发现：现有基于执行的测试与人类偏好存在偏差，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型与人类偏好对齐的重要性，为改进评估体系提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T16:05:57Z
- **目录日期**: 2025-04-12
