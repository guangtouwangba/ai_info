...
这篇论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信噪比（g-SNR）对不同参数组进行学习率调整，有效解决了训练不平衡问题。相比AdamW，SGD-SaI内存占用减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，尤其显著节省大模型训练内存（如为Llama2-7B节省25GB）。同时展现出超参数鲁棒性，并在LoRA微调等任务中超越现有优化器，兼具高效性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T03:15:33Z
- **目录日期**: 2025-04-12
