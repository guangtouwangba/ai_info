...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时按梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题。相比AdamW，SGD-SAi内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优，尤其显著节省内存（如为GPT-2节省5.93GB，Llama2-7B节省25.15GB）。同时具备超参数鲁棒性，在LoRA微调等任务也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T07:04:42Z
- **目录日期**: 2025-04-12
