...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法是对带动量的随机梯度下降(SGDM)的改进，通过根据参数组的梯度信号噪声比(g-SNR)在初始化时调整学习率，无需自适应二阶动量即可平衡训练。相比AdamW，SGD-SaI内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，且超参数鲁棒性强。在LoRA微调等任务中也优于现有优化器，并为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，具有显著优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T20:04:16Z
- **目录日期**: 2025-04-12
