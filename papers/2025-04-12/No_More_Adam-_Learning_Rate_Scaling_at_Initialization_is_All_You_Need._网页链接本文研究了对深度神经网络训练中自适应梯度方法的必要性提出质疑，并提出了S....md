...
这篇论文提出了SGD-SaI优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统认知。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，有效解决了训练不平衡问题，且内存使用量比AdamW减少一半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练等场景中表现优于或持平AdamW，并显著提升内存效率（如为Llama2-7B节省25.15GB内存）。该方法还展现出超参数鲁棒性，在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T21:04:13Z
- **目录日期**: 2025-04-12
