...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。相比AdamW，SGD-SaI无需二阶动量，内存使用减半，且在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更好。在LoRA微调等任务中优于现有优化器，并为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存。该方法解决了用SGD训练Transformer的难题，具有超参数鲁棒性和广泛应用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T12:05:48Z
- **目录日期**: 2025-04-12
