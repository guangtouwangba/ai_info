...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡的问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现相当或更好，尤其在大模型训练中节省显著内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。此外，在LoRA微调等任务中也优于现有优化器，且对超参数变化更具鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T04:04:10Z
- **目录日期**: 2025-04-12
