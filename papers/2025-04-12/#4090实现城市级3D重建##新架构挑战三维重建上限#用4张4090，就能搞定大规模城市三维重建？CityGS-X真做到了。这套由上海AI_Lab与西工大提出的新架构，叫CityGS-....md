...
上海AI Lab与西工大提出CityGS-X架构，采用并行化混合分层三维表征(PH²-3D)，突破传统城市级3D重建方法。仅需4张RTX4090即可处理5000+图像的大规模场景，训练速度翻倍且显存占用更低。其K层分层体素建模支持并行处理，结合动态体素分配与高斯属性共享，在RGB渲染和几何精度上均达到SOTA水平。通过批处理多任务并行渲染、法线/深度图协同优化等技术，有效解决了视角过拟合问题，实现了高效精细的三维重建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T23:02:31Z
- **目录日期**: 2025-04-12
