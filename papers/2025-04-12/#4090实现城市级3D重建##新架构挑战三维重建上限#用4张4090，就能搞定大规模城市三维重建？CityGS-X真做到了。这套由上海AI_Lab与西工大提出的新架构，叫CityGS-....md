...
上海AI Lab与西工大提出新架构CityGS-X，采用并行化混合分层三维表征（PH²-3D），突破传统城市级3D重建限制。仅需4张4090显卡即可处理5000+图像的大规模场景，训练速度翻倍且显存占用更低。通过K层分层体素建模、动态分配与高斯属性共享优化显存，并利用多任务并行渲染提升精度。在RGB渲染和几何重建方面均达到SOTA水平，支持批处理级任务分配与属性迁移，有效避免视角过拟合。该技术为高效、低成本的城市三维重建提供了新方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T21:02:33Z
- **目录日期**: 2025-04-12
