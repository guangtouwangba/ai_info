...
上海AI Lab与西工大联合提出CityGS-X架构，采用并行化混合分层三维表征(PH²-3D)，突破传统"分块+合并"方法。该技术仅需4张4090显卡即可完成5000+图像的大规模城市3D重建，训练速度翻倍且显存占用更低。通过K层分层体素并行处理、动态分配与高斯属性共享，实现高效精细建模。在RGB渲染和几何精度方面均达到SOTA水平，支持批处理多任务并行渲染，有效避免视角过拟合问题。该方案显著降低了大规模场景重建的硬件门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T22:02:27Z
- **目录日期**: 2025-04-12
