...
上海AI Lab与西工大联合提出CityGS-X架构，采用并行化混合分层三维表征（PH²-3D），突破传统"分块+合并"方法。该技术仅需4张RTX 4090即可完成5000+图像的大规模城市3D重建，训练速度提升2倍且显存占用更优。通过K层分层体素建模、动态分配与高斯属性共享实现高效并行处理，在RGB渲染和几何精度上均达SOTA水平。其创新点包括批处理多任务并行渲染、法线/深度图协同优化，有效解决了视角过拟合问题，为城市级三维重建提供了高性价比解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T14:02:41Z
- **目录日期**: 2025-04-12
