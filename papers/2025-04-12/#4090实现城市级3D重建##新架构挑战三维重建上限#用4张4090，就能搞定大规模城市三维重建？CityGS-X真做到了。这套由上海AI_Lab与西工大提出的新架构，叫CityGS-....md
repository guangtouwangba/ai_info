...
上海AI Lab与西工大提出的CityGS-X架构，通过并行化混合分层三维表征（PH²-3D）技术，实现了高效的大规模城市3D重建。该方案摒弃传统分块合并方法，采用批处理+多任务并行渲染，显著提升训练速度并降低显存需求。仅需4张4090显卡即可处理超5000张图像的大场景，同时保持RGB渲染和几何精度的SOTA水平。其核心创新包括K层分层体素建模、动态体素分配、高斯属性共享及多视角协同优化，有效解决了显存瓶颈和视角过拟合问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T12:02:44Z
- **目录日期**: 2025-04-12
