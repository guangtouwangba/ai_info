...
上海AI Lab与西工大联合提出CityGS-X架构，采用并行化混合分层三维表征(PH²-3D)，突破传统"分块+合并"方法。该技术仅需4张RTX 4090即可处理5000+图像的大规模城市重建，训练速度提升2倍且显存占用更低。通过K层分层体素建模、动态分配与高斯属性共享，实现了RGB渲染和几何精度的SOTA水平。其创新点包括多任务并行渲染、批处理优化及法线/深度图协同，有效解决了视角过拟合问题，为城市级3D重建提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T13:06:37Z
- **目录日期**: 2025-04-12
