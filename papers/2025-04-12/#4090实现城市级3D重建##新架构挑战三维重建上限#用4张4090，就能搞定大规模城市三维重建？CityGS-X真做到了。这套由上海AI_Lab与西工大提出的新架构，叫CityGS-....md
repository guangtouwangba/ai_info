...
上海AI Lab与西工大联合提出CityGS-X架构，采用并行化混合分层三维表征(PH²-3D)，突破传统"分块+合并"方法。仅需4张4090显卡即可处理5000+图像的大规模城市重建，训练速度翻倍且显存占用更低。该技术通过K层并行体素建模、动态分配与高斯属性共享，实现RGB渲染和几何精度双SOTA。其创新点包括批处理多任务并行渲染、法线/深度图协同优化，有效避免视角过拟合问题。这一突破显著降低了城市级3D重建的硬件门槛和计算成本。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T19:02:29Z
- **目录日期**: 2025-04-12
