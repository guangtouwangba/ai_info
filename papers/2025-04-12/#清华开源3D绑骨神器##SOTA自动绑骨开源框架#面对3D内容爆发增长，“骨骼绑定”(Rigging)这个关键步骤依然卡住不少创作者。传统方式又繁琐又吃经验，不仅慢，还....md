...
清华大学与VAST联合发布开源自动骨骼绑定框架UniRig，突破传统手动绑骨效率瓶颈。该框架采用自回归模型逐关节预测骨架结构，创新性引入骨骼树Tokenization技术，使处理效率提升30%，支持人形、动物、幻想生物及机械结构等多种3D模型。通过Rig-XL数据集（含1.4万+模型）训练，UniRig能同步预测蒙皮权重与骨骼物理属性（如刚体/重力影响），显著提升动画真实感。作为当前最先进的自动绑骨方案，其开源将大幅降低3D内容创作门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T10:02:42Z
- **目录日期**: 2025-04-12
