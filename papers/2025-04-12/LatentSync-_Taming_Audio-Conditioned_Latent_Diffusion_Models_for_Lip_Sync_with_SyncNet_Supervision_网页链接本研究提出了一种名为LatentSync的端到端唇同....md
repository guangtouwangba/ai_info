...
LatentSync是一种新型端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。它利用Stable Diffusion直接建模复杂的音频-视觉关系，解决了传统方法的时间一致性问题。研究提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧，增强时间一致性。同时改进了SyncNet训练框架，提升了其准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多个指标上优于现有技术。这些改进也可应用于其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-12T18:04:57Z
- **目录日期**: 2025-04-12
