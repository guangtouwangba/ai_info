...
达摩院联合多所高校在ICLR 2025提出动态架构DyDiT，通过时间步长和空间区域的智能资源分配，将DiT模型的推理算力削减51%，生成速度提升1.73倍，FID指标几乎无损（2.27vs2.07）。该方法仅需3%微调成本，通过动态调整模型宽度和重点处理主要对象区域，显著减少计算冗余。相关代码已开源，并适配至文生图模型FLUX。实验显示DyDiT在多个数据集上均保持高稳定性，有望推动生成式AI更广泛应用。达摩院本届ICLR共有13篇论文入选。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T06:03:36Z
- **目录日期**: 2025-04-25
