...
本研究提出了一种名为"集成贝叶斯推理(EBI)"的新方法，通过贝叶斯估计整合多个小型语言模型(SLM)的判断，使其性能超越单个模型。实验表明，即使在包含负提升值模型的情况下，EBI在多语言(日语和英语)的资质评估和消费者画像分析等任务中仍能提升整体性能。这一发现为在有限计算资源下构建高性能AI系统提供了新思路，同时也为有效利用性能较低的单个模型开辟了途径。该方法基于现有的大语言模型评估、集成方法和开源模型利用研究，具有重要的创新意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T19:01:34Z
- **目录日期**: 2025-04-25
