...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，利用自监督视频模型的时间表示对齐生成帧。同时系统研究了SyncNet收敛问题，改进其训练框架显著提升精度。该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供新思路。关键突破在于：1）直接建模音视频关联；2）解决时间一致性；3）优化SyncNet训练。成果可推广至其他唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T01:32:53Z
- **目录日期**: 2025-04-25
