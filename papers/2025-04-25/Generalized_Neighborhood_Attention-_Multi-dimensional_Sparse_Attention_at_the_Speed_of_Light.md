...
本文提出了一种新型稀疏注意力机制——广义邻域注意力(GNA)，它能统一描述滑动窗口、跨步滑动窗口和分块注意力。研究者开发了性能模拟器来更准确预测速度提升上限，并在NVIDIA Blackwell架构上基于CUTLASS实现了优化的GNA内核，在FP16精度下达到1.3 petaFLOPs/秒的有效利用率。实验表明，GNA在Cosmos-7B等生成模型上无需微调即可实现28%-46%的端到端加速。该研究通过改进稀疏注意力的实现方式，解决了现有方法因硬件快速演进而难以持续加速的问题，相关代码将通过NATTEN项目开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T02:28:56Z
- **目录日期**: 2025-04-25
