...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合事件相机和脉冲神经网络(SNN)实现动态目标检测，并采用轻量级物理引导神经网络(PgNN)进行能量感知路径规划。该系统通过异步事件流处理实现低延迟避障，结合物理模型提升能效和可解释性，在Gazebo仿真环境中集成ROS中间件构建了从感知到规划的神经符号框架。研究展示了事件视觉与物理引导规划相结合在节能自主导航中的潜力，特别适用于需要快速决策的场景。该系统能无监督检测移动门框并预测最优飞行路径，具有高动态范围和抗运动模糊优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T03:16:59Z
- **目录日期**: 2025-04-25
