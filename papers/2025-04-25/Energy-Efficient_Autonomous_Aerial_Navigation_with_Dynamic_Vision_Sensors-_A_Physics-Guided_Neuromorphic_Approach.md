...
该研究提出了一种基于神经形态视觉的无人机自主导航框架，结合动态视觉传感器（DVS）和脉冲神经网络（SNN）进行无监督运动物体检测，同时采用轻量级物理引导神经网络（PgNN）预测最优飞行路径以实现节能导航。该系统通过事件相机处理动态场景，在Gazebo模拟器中实现，并集成基于ROS的视觉-规划神经符号框架，展现出事件驱动视觉与物理引导规划相结合在低延迟决策和节能自主导航方面的潜力，特别适用于动态障碍物避障场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T01:29:48Z
- **目录日期**: 2025-04-25
