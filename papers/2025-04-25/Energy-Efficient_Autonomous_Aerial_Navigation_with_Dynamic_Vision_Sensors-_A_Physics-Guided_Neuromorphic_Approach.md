...
本文提出了一种基于神经形态视觉的无人机自主导航框架，结合动态视觉传感器（DVS）和脉冲神经网络（SNN）进行无监督移动物体检测，并采用轻量级物理引导神经网络（PgNN）预测最优飞行路径。该系统通过事件相机实现高动态范围、抗运动模糊的异步感知，结合物理建模提升鲁棒性和能效，在Gazebo仿真器中构建了基于ROS的视觉-规划神经符号框架。该研究展示了事件视觉与物理引导规划在低延迟决策和节能自主导航方面的潜力，特别适用于动态障碍物规避场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T00:02:20Z
- **目录日期**: 2025-04-25
