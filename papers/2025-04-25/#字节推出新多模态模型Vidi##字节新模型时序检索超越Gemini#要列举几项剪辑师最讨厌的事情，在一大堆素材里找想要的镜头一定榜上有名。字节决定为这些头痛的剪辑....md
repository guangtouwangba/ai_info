...
字节跳动推出多模态模型Vidi，专注于视频理解与编辑。该模型能在1分钟至1小时以上的视频中精确定位内容，在VUE-TR基准测试中表现优于Gemini和GPT-4。Vidi采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并通过两阶段训练（跨模态对齐和应用后训练）提升时序检索能力。团队还推出了优化5个关键维度的VUE-TR基准测试。目前发布技术报告和基准，即将推出7B模型demo。未来计划扩展视频理解任务和交互式编辑功能。论文和项目主页已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T19:02:54Z
- **目录日期**: 2025-04-25
