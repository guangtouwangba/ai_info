...
字节跳动推出新型多模态模型Vidi，专注于视频理解与编辑，能快速定位长视频中的目标内容。Vidi在VUE-TR基准测试中表现优于Gemini和GPT-4，尤其在长视频处理上优势明显。其采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并通过两阶段训练（跨模态对齐和应用后训练）提升时序检索能力。团队还推出了优化的VUE-TR评估基准。Vidi未来将支持更多视频任务和交互式编辑功能，目前计划发布7B模型的demo和权重文件。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T15:03:16Z
- **目录日期**: 2025-04-25
