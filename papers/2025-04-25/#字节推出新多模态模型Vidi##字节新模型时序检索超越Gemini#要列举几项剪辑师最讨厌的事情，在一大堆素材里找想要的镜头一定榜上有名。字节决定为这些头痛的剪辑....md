...
字节跳动推出全新多模态模型Vidi，专注于视频理解与编辑，能快速定位长视频中的特定内容。Vidi在VUE-TR基准测试中表现优异，尤其在长视频处理上超越Gemini和GPT-4。其创新点包括：采用分解注意力架构降低计算复杂度（O(N²)→O(N)），以及两阶段训练（跨模态对齐+应用微调）提升时序检索能力。团队还推出了优化5个维度的VUE-TR评估基准。目前发布技术报告，即将推出7B模型demo。未来计划拓展视频理解任务和交互编辑功能，弥合大模型与视频应用的差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T20:03:08Z
- **目录日期**: 2025-04-25
