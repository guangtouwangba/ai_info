...
字节跳动推出多模态模型Vidi，专注于视频理解与编辑，能精准定位长视频内容。在VUE-TR基准测试中，Vidi表现优于Gemini和GPT-4，尤其在长视频准确性上领先。其创新分解注意力架构将计算复杂度从O(N²)降至O(N)，支持超长视频处理。模型通过跨模态对齐和应用后训练两阶段优化时序检索能力。团队还推出VUE-TR基准，覆盖视频时长、音频支持等五个维度。未来Vidi将扩展视频理解任务并探索交互式编辑功能。7B模型demo和权重文件即将发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T11:03:26Z
- **目录日期**: 2025-04-25
