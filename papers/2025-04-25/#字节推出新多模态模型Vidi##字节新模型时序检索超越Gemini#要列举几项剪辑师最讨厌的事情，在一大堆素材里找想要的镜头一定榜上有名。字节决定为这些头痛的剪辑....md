...
字节跳动推出全新多模态模型Vidi，专注于视频理解与编辑，能快速定位长视频中的目标内容。该模型采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并在VUE-TR基准测试中超越Gemini和GPT-4。Vidi通过两阶段训练（跨模态对齐和应用后训练）优化时序检索能力，并配套推出VUE-TR评估基准，覆盖视频时长、音频支持等五个维度。未来将发布7B模型Demo，并探索交互式编辑功能，以弥合大模型与视频应用的差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T18:03:04Z
- **目录日期**: 2025-04-25
