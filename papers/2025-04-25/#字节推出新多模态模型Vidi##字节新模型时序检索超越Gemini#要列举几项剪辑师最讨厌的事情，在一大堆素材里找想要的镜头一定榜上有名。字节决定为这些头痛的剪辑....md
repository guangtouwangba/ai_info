...
字节跳动推出多模态模型Vidi，专注视频理解与编辑，能快速定位长视频中的目标内容。Vidi在VUE-TR基准测试中表现优于Gemini和GPT-4，尤其擅长长视频处理。其创新点包括：1）采用分解注意力架构降低计算复杂度；2）两阶段训练（跨模态对齐+应用微调）提升时序检索能力；3）推出VUE-TR评估基准优化五大维度。即将发布7B模型demo，未来计划扩展视频理解任务和交互编辑功能。技术报告和基准已公开，项目主页和论文链接同步释出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T21:03:11Z
- **目录日期**: 2025-04-25
