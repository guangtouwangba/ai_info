...
字节跳动推出多模态模型Vidi，专注于视频理解与编辑，能快速精确定位长视频内容。Vidi在VUE-TR基准测试中表现优于Gemini和GPT-4，尤其在长视频处理上优势明显。其采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并通过两阶段训练（跨模态对齐和应用后训练）提升时序检索能力。团队还推出VUE-TR基准，优化视频时长、音频支持等五个维度。未来将发布7B模型demo，并探索交互式编辑功能。论文和项目已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T12:03:19Z
- **目录日期**: 2025-04-25
