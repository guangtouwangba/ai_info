...
本文提出EgoCHARM算法，用于智能眼镜上基于单惯性测量单元(IMU)的高效人类活动识别。该分层算法采用半监督学习策略，主要利用高层活动标签训练，学习可泛化的低层运动嵌入特征，实现资源(内存、计算、功耗、样本)高效的高/低层活动识别。在9种高层和3种低层活动测试中，分别获得0.826和0.855的F1分数，模型参数仅63k(高层)和22k(低层)，可直接部署于现有IMU芯片。研究还分析了基于第一视角IMU的活动识别的优势与局限。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T22:01:13Z
- **目录日期**: 2025-04-25
