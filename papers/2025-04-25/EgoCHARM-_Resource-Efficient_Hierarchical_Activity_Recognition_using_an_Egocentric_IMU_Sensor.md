...
该研究提出了一种资源高效的机器学习算法EgoCHARM，用于通过头戴式惯性测量单元(IMU)识别高低层级活动。该分层算法采用半监督学习策略，主要利用高层级活动标签进行训练，学习可泛化的低层级运动嵌入特征。在9种高层级和3种低层级活动识别中，分别取得0.826和0.855的F1分数，模型参数仅63k(高层级)和22k(低层级)，可直接部署在当前IMU芯片上。研究还进行了敏感性分析，探讨了头戴式IMU活动识别的机会与限制。该方案为智能眼镜上的轻量级活动识别提供了有效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T12:01:15Z
- **目录日期**: 2025-04-25
