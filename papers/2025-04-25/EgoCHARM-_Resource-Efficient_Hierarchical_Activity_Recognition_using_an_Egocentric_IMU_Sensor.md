...
本文提出EgoCHARM算法，用于智能眼镜上的单IMU设备进行高效人类活动识别。该分层算法采用半监督学习策略，主要使用高级活动标签训练，同时学习可泛化的低级运动嵌入，实现高低级活动识别。在9种高级和3种低级活动上分别获得0.826和0.855的F1分数，模型参数仅63k（高级）和22k（低级），可直接部署在当前IMU芯片上。研究还分析了灵敏度，探讨了头戴式IMU活动识别的机会与限制。该方法在资源（内存、计算、功耗、样本）效率方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T16:01:18Z
- **目录日期**: 2025-04-25
