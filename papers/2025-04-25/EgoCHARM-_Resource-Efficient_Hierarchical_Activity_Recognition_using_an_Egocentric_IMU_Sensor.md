...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于通过头戴式惯性测量单元(IMU)识别高/低层次活动。该分层算法采用半监督学习策略，主要利用高层次活动标签训练，学习可泛化的低层次运动嵌入特征。在9种高层次和3种低层次活动识别中，分别取得0.826和0.855的F1分数，模型参数仅63k(高层次)和22k(低层次)，可直接部署于现有IMU芯片。研究还分析了基于头戴IMU的活动识别的机会与限制。该方法在计算、内存、功耗和样本效率方面表现优异，适用于智能眼镜的人体活动识别应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T18:01:11Z
- **目录日期**: 2025-04-25
