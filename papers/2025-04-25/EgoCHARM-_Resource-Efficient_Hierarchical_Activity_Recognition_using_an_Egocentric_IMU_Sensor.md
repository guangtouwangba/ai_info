...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于通过头戴式惯性测量单元(IMU)识别高低层次活动。该分层算法采用半监督学习策略，主要利用高层次活动标签进行训练，学习可泛化的低层次运动嵌入，有效用于低层次活动识别。在9种高层次和3种低层次活动上分别获得0.826和0.855的F1分数，模型仅需63k和22k参数，低层次编码器可直接部署在当前IMU芯片上。研究还分析了敏感性并探讨了头戴IMU活动识别的机会与限制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T10:01:16Z
- **目录日期**: 2025-04-25
