...
本文提出EgoCHARM算法，一种基于头戴式惯性测量单元(IMU)的资源高效机器学习方法，用于识别高级和低级活动。该分层算法采用半监督学习策略，主要使用高级活动标签进行训练，同时学习可泛化的低级运动嵌入特征。在9种高级和3种低级活动识别任务中，分别取得0.826和0.855的F1分数，模型参数仅63k(高级)和22k(低级)，可直接部署在当前IMU芯片上。研究还分析了基于头戴IMU的活动识别的机会与限制。该方法在资源消耗和性能间取得了良好平衡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T15:01:13Z
- **目录日期**: 2025-04-25
