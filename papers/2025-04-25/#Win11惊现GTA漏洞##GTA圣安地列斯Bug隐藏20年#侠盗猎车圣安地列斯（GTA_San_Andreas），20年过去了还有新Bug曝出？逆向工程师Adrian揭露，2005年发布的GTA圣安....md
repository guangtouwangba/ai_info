...
逆向工程师Adrian发现，经典游戏《GTA圣安地列斯》在Win11 24H2版本中出现Skimmer水上飞机消失的Bug。经调查发现，该Bug源于游戏文件中Skimmer被错误地定义为船而非飞机，且缺少关键参数。过去20年因内存残留值（0.7）勉强运行，但Win11 24H2的堆栈更新覆盖了该值，导致飞机高度计算崩溃（出现3.73e+29异常值），引发游戏画面错乱。解决方法为手动编辑vehicles.ide文件添加缺失参数。该事件揭示了长期隐藏的代码缺陷如何因系统更新暴露。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T02:41:39Z
- **目录日期**: 2025-04-25
