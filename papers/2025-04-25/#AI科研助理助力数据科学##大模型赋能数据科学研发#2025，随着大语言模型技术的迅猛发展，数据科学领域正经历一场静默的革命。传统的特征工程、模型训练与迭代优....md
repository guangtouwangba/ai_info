...
微软亚洲研究院刘炜清团队研发的RD-Agent系统，通过大语言模型技术革新数据科学研发流程。该系统采用双Agent协作框架：Research Agent生成研究想法，Development Agent实现验证，形成闭环迭代。目前已实现80-90%的自动化特征生成能力，显著提升研究效率。系统历经三阶段演进：从研发自动化到增强，最终目标重塑领域范式。通过记录实验数据持续优化，突破单纯模仿专家的局限，展现了AI驱动科研的创新潜力，为金融等领域提供论文复现等实际应用价值。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T04:03:57Z
- **目录日期**: 2025-04-25
