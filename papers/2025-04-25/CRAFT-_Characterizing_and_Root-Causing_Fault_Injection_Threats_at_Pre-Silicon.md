...
这篇论文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究以AI/ML应用中的时钟毛刺攻击为例，追踪故障从电路层到应用软件的传播过程，发现了两个新漏洞：1) 通过指令跳过阻止关键值从内存加载，破坏程序连续性和正确性；2) 将合法指令转为非法指令以劫持控制流。该工作揭示了故障注入攻击的复杂性，强调了预先安全分析的重要性，为保护网络基础设施安全提供了关键见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T15:02:01Z
- **目录日期**: 2025-04-25
