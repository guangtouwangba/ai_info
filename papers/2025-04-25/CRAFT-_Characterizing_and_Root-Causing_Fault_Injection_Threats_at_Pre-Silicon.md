...
这篇论文提出了一种在芯片设计前(pre-silicon)阶段进行可控故障注入攻击的综合方法，重点研究了AI/ML应用中时钟毛刺攻击导致关键错误分类的问题。研究分析了RISC-V指令集和流水线阶段的故障影响，并追踪了从电路层到AI/ML软件的故障传播路径。通过控制时钟毛刺参数，发现了两个新漏洞：(1)导致指令跳过，阻止关键值从内存加载；(2)将合法指令转为非法指令，从而可被攻击者利用控制流转向。研究强调了故障注入攻击的复杂性和预先安全分析的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T10:02:07Z
- **目录日期**: 2025-04-25
