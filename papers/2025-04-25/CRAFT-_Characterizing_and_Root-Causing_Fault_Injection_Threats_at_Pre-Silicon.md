...
这篇论文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究以AI/ML应用中的时钟毛刺攻击为案例，揭示了两种新漏洞：1) 通过指令跳过阻止关键值从内存加载，破坏程序连续性和正确性；2) 将合法指令转为非法指令，从而可被攻击者利用控制流转向。该工作强调了故障注入攻击的复杂性，以及从电路层到应用软件层追踪故障传播的重要性，突显了先发制人的安全分析对保护网络基础设施的关键价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T23:01:55Z
- **目录日期**: 2025-04-25
