...
该研究提出了一种在硅前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响，并追踪从电路层到AI/ML应用软件的故障传播。研究发现两种新漏洞：一是通过时钟毛刺导致指令跳过，阻止关键值从内存加载，影响程序连续性和正确性；二是将合法指令转为非法指令，从而可被攻击者利用以改变控制流。研究凸显了故障注入攻击的复杂性，强调了预先安全分析的重要性，特别是在AI/ML关键错误分类应用中的时钟毛刺攻击。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T06:02:04Z
- **目录日期**: 2025-04-25
