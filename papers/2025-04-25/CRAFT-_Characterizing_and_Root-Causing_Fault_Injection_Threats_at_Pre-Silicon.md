...
该研究提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析RISC-V指令集和流水线阶段的故障影响。通过时钟毛刺攻击在AI/ML应用中引发关键误分类，研究揭示了两种新漏洞：一是导致指令跳过，阻止从内存加载关键值，破坏程序连续性；二是将合法指令转为非法指令，可被攻击者利用控制流。该工作强调了故障注入攻击的复杂性，以及从电路层到软件层追踪故障传播的重要性，凸显了先发制人的安全分析对保护网络基础设施的关键作用。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T12:02:09Z
- **目录日期**: 2025-04-25
