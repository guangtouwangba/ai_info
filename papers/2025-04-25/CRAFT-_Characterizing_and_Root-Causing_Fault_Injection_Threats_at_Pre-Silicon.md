...
本文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V架构在AI/ML应用中的时钟毛刺攻击。研究发现两种新漏洞：1）通过特定时钟毛刺参数可导致指令跳过，阻止关键内存值加载，破坏程序连续性和正确性；2）将合法指令转为非法指令，从而可被攻击者利用控制流。该研究揭示了故障从电路层到应用软件的传播路径，强调了预先安全分析的重要性，为保护网络基础设施提供了关键见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T13:09:11Z
- **目录日期**: 2025-04-25
