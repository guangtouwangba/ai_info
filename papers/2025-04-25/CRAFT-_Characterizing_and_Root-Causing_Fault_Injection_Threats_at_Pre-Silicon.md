...
本文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究通过时钟毛刺攻击在AI/ML应用中引发关键错误分类，揭示了两种新漏洞：一是导致指令跳过，阻止关键值从内存加载；二是将合法指令转为非法指令，从而可被攻击者利用控制流转移。该工作强调了故障注入攻击的复杂性，以及从电路层到应用软件层追踪故障传播的重要性，为关键网络基础设施的预先安全分析提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T18:01:59Z
- **目录日期**: 2025-04-25
