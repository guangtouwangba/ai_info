...
这篇论文提出了一种在芯片设计前(pre-silicon)阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究通过时钟毛刺攻击(clock glitch)在AI/ML应用中实现了关键错误分类，揭示了两种新漏洞：1) 导致指令跳过，阻止从内存加载关键值；2) 将合法指令转换为非法指令，从而可被攻击者利用来控制流转移。该工作强调了故障注入攻击的复杂性，以及进行先发制人安全分析的重要性，有助于理解故障从物理层到系统软件的传播机制和根本原因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T16:02:08Z
- **目录日期**: 2025-04-25
