...
这篇由67位学者联合发表的大模型安全综述，系统梳理了843篇研究，首次构建了覆盖全生命周期的安全知识图谱。研究指出大模型在数据采集（隐私泄露/后门污染）、预训练（记忆风险）、微调（指令注入）、对齐（反馈毒化）、部署（提示注入）和商业应用（幻觉/偏见）各阶段均存在安全隐患，并提出数据清洗、多模态审核、事实验证等防御措施。该研究为AI安全提供了首个全链路分析框架，强调技术治理需协同推进，对开发者、监管者具有重要参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T18:03:23Z
- **目录日期**: 2025-04-25
