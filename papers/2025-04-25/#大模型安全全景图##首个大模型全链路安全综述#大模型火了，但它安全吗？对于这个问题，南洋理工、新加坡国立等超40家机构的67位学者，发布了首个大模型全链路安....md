...
这篇由67位学者联合发表的综述首次系统梳理了大模型全链路安全问题，涵盖数据采集到商业应用的全生命周期。研究发现：数据阶段0.1%污染即可破坏模型；微调阶段可能遭遇后门攻击；对齐阶段存在奖励机制被操控风险；模型遗忘可能引入新偏见；部署阶段面临复杂提示注入攻击；医疗金融等应用场景仍存在幻觉和隐私风险。防御措施包括数据清洗、多模态审核、事实验证等。该研究为构建可信AI提供了首个完整安全知识图谱，强调技术治理需协同推进。论文链接：https://arxiv.org/abs/2504.15585

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T12:03:41Z
- **目录日期**: 2025-04-25
