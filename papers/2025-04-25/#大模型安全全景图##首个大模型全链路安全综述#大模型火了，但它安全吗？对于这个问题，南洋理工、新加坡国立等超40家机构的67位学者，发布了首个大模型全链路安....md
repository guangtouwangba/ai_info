...
这篇综述由67位学者联合发表，首次系统梳理了大模型全生命周期的安全风险与防护策略。研究涵盖数据采集、预训练、微调、部署等全流程，指出各阶段核心问题：数据污染（0.1%即可破坏模型）、微调阶段后门攻击、对齐阶段的反馈毒化、模型遗忘引发的偏见、部署时的提示注入攻击，以及医疗/金融等场景的应用风险。防御措施包括差分隐私、多模态审核、事实验证等。该研究为AI安全提供了首个全景知识图谱，强调技术治理需协同推进，对开发者与监管者具有重要参考价值。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T15:03:36Z
- **目录日期**: 2025-04-25
