...
这篇由67位学者联合发布的综述首次系统梳理了大模型全链路安全问题，涵盖数据采集到商业应用的全流程。研究发现：数据阶段0.1%污染即可破坏模型；微调阶段可能被植入后门指令；对齐阶段奖励机制可能被操控；部署阶段面临提示注入等新型攻击；商业应用中幻觉和偏见风险突出。防御措施包括差分隐私、多模态审核、事实验证等。该研究为AI安全提供了首个全景知识图谱，强调需要技术治理协同推进可信AI系统建设。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T22:03:19Z
- **目录日期**: 2025-04-25
