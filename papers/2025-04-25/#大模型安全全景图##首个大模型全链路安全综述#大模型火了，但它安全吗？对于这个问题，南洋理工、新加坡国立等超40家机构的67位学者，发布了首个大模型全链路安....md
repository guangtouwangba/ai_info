...
这篇由67位学者联合发布的大模型安全全景综述，系统梳理了843篇研究，首次完整呈现了大模型从数据采集到商业应用的全链路安全风险与防御方案。研究指出：数据阶段需防范隐私泄露与污染（0.1%污染即可破坏模型）；微调阶段面临指令注入和后门攻击；对齐阶段可能产生"伪合规"输出；部署时需应对提示注入等新型攻击；商业应用中幻觉和偏见风险突出。综述提出了数据清洗、多模态审核、对抗训练等防御策略，强调技术治理协同，为构建可信AI提供了系统性安全框架。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T13:10:46Z
- **目录日期**: 2025-04-25
