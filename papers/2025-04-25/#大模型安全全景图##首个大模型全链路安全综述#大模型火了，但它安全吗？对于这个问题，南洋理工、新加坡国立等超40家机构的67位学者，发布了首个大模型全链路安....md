...
这篇由67位学者联合发布的综述首次系统梳理了大模型全链路安全问题，涵盖数据采集到商业应用全流程。研究发现：数据阶段0.1%污染即可破坏模型；微调阶段可能遭遇后门攻击；对齐阶段存在奖励机制被操控风险；模型遗忘可能引入新偏见；部署阶段面临复杂越狱攻击；医疗金融等场景的幻觉/偏见风险犹存。防御措施包括差分隐私、多模态审核、事实验证等。该研究为构建可信AI提供了首个全景式安全框架，强调需要技术治理双轨并进。论文链接：https://arxiv.org/abs/2504.15585

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T17:03:27Z
- **目录日期**: 2025-04-25
