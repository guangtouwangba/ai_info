...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，能模拟10,000多个AI代理的社会互动（500万次）。研究者用它研究了四个社会问题：社会极化、信息传播、基本收入政策影响和自然灾害冲击。结果显示其模拟结果与现实世界数据高度吻合，证明该系统能有效捕捉人类行为机制。该平台可作为社会科学研究和政策制定的有力工具，补充传统研究方法（如调查和访谈）的不足，帮助深入理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T20:04:54Z
- **目录日期**: 2025-04-25
