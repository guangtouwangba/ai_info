...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了训练不平衡问题，并将优化器内存使用减半。相比AdamW，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更好，同时在LoRA微调等任务中优于现有优化器。内存效率方面，SGD-SaI为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存。该方法具有超参数鲁棒性和广泛适用性，成功解决了用SGD训练Transformer的难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T01:31:57Z
- **目录日期**: 2025-04-25
