...
该论文提出了一种名为SGD-SaI的新优化方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类、LLM预训练（如GPT-2和Llama2-7B）以及LoRA微调中表现更优或相当，同时显著节省内存（例如为Llama2-7B节省25.15GB）。SGD-SaI具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T04:04:28Z
- **目录日期**: 2025-04-25
