...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。该框架包含四个核心角色：(1) **问题建模器**将自然语言转换为数学公式；(2) **规划器**制定高层求解策略；(3) **编码器**和**代码评审员**通过环境交互优化执行。实验表明，移除规划器或评审员会分别导致效率下降5.8倍和3.1倍。引入UCB动态调试策略后，效率进一步提升3.3倍。在NLP4LP和Optibench数据集上，OptimAI分别达到88.1%和71.2%的准确率，错误率比现有最优方法降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T00:01:13Z
- **目录日期**: 2025-04-25
