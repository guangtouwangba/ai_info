...
OptimAI是一个基于LLM的AI代理框架，用于将自然语言描述的优化问题转化为数学形式并求解。其核心包含四个角色：(1) **公式化器**将问题转换为数学表述；(2) **规划器**制定高层解决方案；(3) **编码器**和**代码评审器**通过环境交互优化执行。实验表明，移除规划器或评审器会分别导致5.8倍和3.1倍的效率下降。此外，动态调试调度策略（UCB-based）进一步提升了3.3倍效率。该系统在NLP4LP和Optibench数据集上分别达到88.1%和71.2%的准确率，错误率较之前最优结果降低58%和50%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-25T02:38:03Z
- **目录日期**: 2025-04-25
