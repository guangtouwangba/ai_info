...
数势科技联合创始人谭李在AIGC峰会上指出，企业级AI应用面临数据洞见获取难的挑战。AI Agent（如SwiftAgent平台）通过让业务人员像与分析师对话一样获取实时数据，推动决策升级。三大趋势加速新数据消费范式：数据右移、决策下移、管理后移。基础模型成本下降（如DeepSeek）使企业级AI应用更可行，未来成本有望进一步降低。核心是让非技术人员快速获取数据支持，而不仅是简单Chat功能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T03:18:07Z
- **目录日期**: 2025-04-23
