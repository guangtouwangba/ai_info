...
近期视觉语言模型(VLMs)在GUI交互任务中取得进展，但仍面临复杂环境下的动作生成困难。现有商业VLM是黑箱系统，而开源模型微调成本高昂，传统轨迹评估方法也存在延迟反馈和局部优化问题。本研究提出在推理时使用奖励模型进行过程监督，引导VLM代理优化每一步动作。该方法在静态环境中单步动作准确率提升3.4%，在动态环境中任务成功率提高约33%。结合轨迹反思和重试机制后，任务成功率得到进一步改善。该方法为GUI导航和控制任务提供了有效的优化方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T22:01:20Z
- **目录日期**: 2025-04-23
