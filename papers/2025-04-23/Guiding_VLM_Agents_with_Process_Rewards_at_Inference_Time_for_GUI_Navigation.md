...
近期视觉语言模型(VLM)在GUI交互任务中取得进展，但仍面临复杂环境下的动作生成困难。针对商用VLM黑箱化和开源模型微调资源消耗大的问题，研究者提出了一种推理时过程监督方法：通过奖励模型引导VLM代理在GUI导航中逐步优化动作。该方法在静态环境中单步动作准确率提升3.4%，在动态环境中任务成功率提高约33%。结合轨迹反思和重试机制后，性能得到进一步改善。该方案有效解决了传统方法因延迟反馈和局部优化导致的性能瓶颈问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T16:01:43Z
- **目录日期**: 2025-04-23
