...
近期视觉语言模型(VLM)在GUI交互任务中取得进展，但仍面临复杂环境下的动作生成困难。商用VLM是黑箱系统，而开源模型的微调成本高昂。现有轨迹级评估方法因延迟反馈和局部优化问题效果有限。本研究提出在推理时使用奖励模型进行过程监督，指导VLM代理优化每一步动作，显著提升了静态和动态环境下的表现。实验显示：静态环境中单步动作准确率提高3.4%，某动态环境任务成功率提升约33%。结合轨迹反思和重试机制后，任务成功率得到进一步改善。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T17:01:37Z
- **目录日期**: 2025-04-23
