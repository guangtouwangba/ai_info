...
近期视觉语言模型(VLM)在图形用户界面(GUI)交互任务中取得进展，但仍面临挑战。现有方法存在黑箱问题、微调资源需求大以及轨迹优化不足等缺陷。本研究提出在推理时使用奖励模型进行过程监督，引导VLM代理优化每一步动作。该方法在静态环境中单步动作准确率提升3.4%，在动态环境中任务成功率提高约33%。结合轨迹反思和重试机制后，性能得到进一步改善。该方法有效解决了延迟反馈和局部优化问题，显著提升了GUI导航和控制任务的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T20:01:29Z
- **目录日期**: 2025-04-23
