...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过动态分层稀疏策略结合算法与硬件优化，实现了高效的长文本建模：先粗粒度压缩令牌，再细粒度选择关键令牌，既保留全局上下文感知能力，又确保局部精度。其两大创新点在于硬件对齐设计和原生可训练性，显著提升了长文本处理的效率。该技术为大模型（如Transformer）的上下文扩展提供了更优解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T01:32:59Z
- **目录日期**: 2025-04-23
