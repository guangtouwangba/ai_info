...
MR. Video是一个基于MapReduce原理的长视频理解框架，通过两个步骤处理视频：(1) Map阶段：独立密集感知短视频片段；(2) Reduce阶段：联合聚合所有片段信息。相比传统序列模型，该方法不受上下文长度限制，且比依赖关键片段选择的视频代理更高效。框架分两阶段实现：首先生成片段描述并统一重复对象名称（Captioning阶段），再针对用户问题分析片段信息并整合答案（Analysis阶段）。在LVBench测试中，MR. Video比现有最优模型准确率提升超10%。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T22:01:01Z
- **目录日期**: 2025-04-23
