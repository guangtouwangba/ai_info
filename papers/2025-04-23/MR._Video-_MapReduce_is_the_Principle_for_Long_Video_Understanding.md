...
MR. Video是一个基于MapReduce原理的长视频理解框架，通过"Map"（独立感知短视频片段）和"Reduce"（聚合所有片段信息）两阶段处理长视频。相比传统方法，它突破了上下文长度限制，实现了并行细粒度感知，并通过全局聚合提升推理能力。该框架包含两个MapReduce阶段：1）字幕生成（标准化重复对象）；2）问答分析（整合片段信息作答）。在LVBench基准测试中，MR. Video比现有最优视频语言模型和代理的准确率提升超过10%。该方法适用于各类视频理解模型，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T16:01:21Z
- **目录日期**: 2025-04-23
