...
MR. Video是一个基于MapReduce原则的长视频理解框架，通过"Map"（独立密集感知短视频片段）和"Reduce"（联合聚合所有片段信息）两阶段处理长视频。相比传统方法，它能并行处理片段且不受上下文长度限制，避免了关键片段选择的依赖。框架包含两个MapReduce阶段：1）字幕生成（标准化重复内容）；2）问题分析（整合片段信息作答）。在LVBench基准测试中，MR. Video比现有最优视频语言模型和代理的准确率提升超过10%。该方法适用于视频语言模型和代理，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T21:01:04Z
- **目录日期**: 2025-04-23
