...
本文探讨了大型语言模型(LLMs)在资源受限环境中的部署挑战，提出通过知识蒸馏训练小型学生模型的方法。研究扩展了现有蒸馏技术，包括应用批判性修正提示进行数据生成和综合现有训练方法。基于常识问答(CQA)数据集，系统比较了这些方法在模型性能和可解释性方面的影响。研究不仅测量学生模型的准确性，还通过人工评估可解释性。贡献包括新的蒸馏方法及其性能与可解释性的比较，旨在促进小型语言模型的蒸馏技术发展，扩大LLM技术的适用性和推广速度。(100字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T14:02:00Z
- **目录日期**: 2025-04-23
