...
本文探讨了大型语言模型(LLMs)在资源受限环境中的部署挑战，提出通过知识蒸馏训练小型学生模型的方法。研究扩展了现有蒸馏技术，应用批判性修正提示进行数据生成，并综合多种训练方法。基于常识问答(CQA)数据集，系统比较了不同方法在模型性能和可解释性上的表现，通过准确性评估和人工研究进行验证。研究贡献了新的蒸馏方法及其性能与可解释性的比较，旨在推动小型语言模型的发展，促进LLM技术的广泛应用和快速普及。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T19:01:51Z
- **目录日期**: 2025-04-23
