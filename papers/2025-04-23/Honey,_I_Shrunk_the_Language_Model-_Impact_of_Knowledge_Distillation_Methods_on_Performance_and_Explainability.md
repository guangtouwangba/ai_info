...
近年来，大型语言模型(LLMs)虽取得重大进展，但其高计算和存储需求限制了在资源受限环境中的部署。知识蒸馏通过从大教师模型训练小学生模型来解决这一挑战。本研究扩展了现有方法，将批判-修正提示应用于数据生成的蒸馏，并综合现有训练方法。基于常识问答(CQA)数据集，我们系统比较了这些方法在模型性能和可解释性上的表现，其中性能通过准确率衡量，可解释性通过人工评估。研究贡献了新的蒸馏方法及其比较，有助于推动小型语言模型的蒸馏，促进LLM技术的更广泛应用和快速扩散。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T10:02:12Z
- **目录日期**: 2025-04-23
