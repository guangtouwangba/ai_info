...
这篇论文探讨了大型语言模型(LLMs)在资源受限环境中的部署挑战，提出了通过知识蒸馏训练小型学生模型的方法。研究扩展了现有方法，应用批判性修正提示进行数据生成，并综合现有训练方法。基于常识问答(CQA)数据集，系统比较了不同蒸馏方法在模型性能和可解释性上的表现。研究不仅评估了学生模型的准确性，还通过人工研究评估了可解释性。该工作为小型语言模型的蒸馏提供了新方法和比较结果，有助于推动LLM技术的更广泛应用和快速普及。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T08:02:06Z
- **目录日期**: 2025-04-23
