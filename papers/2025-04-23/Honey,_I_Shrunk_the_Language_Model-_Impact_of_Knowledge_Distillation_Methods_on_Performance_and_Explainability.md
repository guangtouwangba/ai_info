...
大型语言模型(LLMs)因计算和存储需求高，难以在资源受限环境中部署。知识蒸馏通过大模型训练小模型来解决这一问题。本研究扩展了现有蒸馏方法，采用批判性修正提示生成训练数据，并综合现有训练方法。基于常识问答数据集(CQA)，系统比较了不同方法在模型性能和可解释性上的表现：通过准确率评估性能，通过人类研究评估可解释性。研究提出了新的蒸馏方法，并对其性能和可解释性进行全面比较，有助于推动小型语言模型的蒸馏技术，促进LLM技术的更广泛应用和快速普及。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T22:01:50Z
- **目录日期**: 2025-04-23
