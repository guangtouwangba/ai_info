...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。与现有方法不同，它无需中间运动表示，直接建模音频-视觉关系。针对扩散模型的时间一致性问题，提出了TREPA方法，利用自监督视频模型增强帧间一致性。研究还优化了SyncNet训练框架，改进了模型架构和训练策略，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T21:06:16Z
- **目录日期**: 2025-04-23
