...
多模态大语言模型（MLLMs）在跨视角视觉理解（如导航、操作和3D场景认知）方面仍面临几何一致性与视角对应性的挑战。研究团队提出All-Angles Bench基准测试，包含90个真实场景的2,100组人工标注多视角问答，通过6项任务（计数、属性识别、相对距离/方向、物体操作、相机位姿估计）系统评估27个主流MLLMs（包括Gemini-2.0-Flash、Claude-3.7-Sonnet和GPT-4o）。实验显示当前模型与人类水平存在显著差距，尤其在部分遮挡视角的跨视图对应和粗略相机位姿估计方面表现欠佳，表明需要增强多视角感知的专用模块优化。该基准为提升MLLMs空间理解能力提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T02:28:26Z
- **目录日期**: 2025-04-23
