...
大型语言模型（LLMs）在处理简单任务时表现出色，但在复杂多步骤任务中仍有挑战。为解决这一问题，研究者提出了"Plan-and-Act"框架，将高层规划与底层执行分离。该框架包含生成结构化高层规划的Planner模型和执行具体操作的Executor模型。为提升规划准确性，研究者开发了合成数据生成方法，通过标注真实轨迹来增强模型泛化能力。在网页导航任务测试中，该框架在WebArena-Lite基准上达到57.58%的成功率，在WebVoyager文本任务上达到81.36%的当前最优表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T13:10:14Z
- **目录日期**: 2025-04-23
