...
大型语言模型（LLMs）在简单任务上表现优异，但在复杂多步长程任务中仍面临挑战。研究提出"Plan-and-Act"框架，通过分离高层规划与底层执行来提升性能：Planner模型生成结构化高层计划，Executor模型将其转化为具体行动。为提高规划准确性，该框架采用合成数据生成方法，用可行计划标注真实轨迹并扩展多样化示例增强泛化能力。在网页导航任务测试中，该框架在WebArena-Lite基准上达到57.58%成功率，在纯文本环境WebVoyager上取得81.36%的当前最佳表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T14:02:35Z
- **目录日期**: 2025-04-23
