...
大型语言模型（LLMs）在简单任务中表现优异，但在复杂多步骤任务中仍面临挑战。为此，研究提出"Plan-and-Act"框架，将规划与执行分离：Planner模型生成结构化高级计划，Executor模型将其转化为具体操作。为提高规划准确性，该框架采用创新的合成数据生成方法，通过标注真实轨迹数据来增强模型的泛化能力。在网页导航任务测试中，该框架在WebArena-Lite基准上达到57.58%的成功率（当前最优），在纯文本环境的WebVoyager上更是达到81.36%的领先水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T20:02:33Z
- **目录日期**: 2025-04-23
