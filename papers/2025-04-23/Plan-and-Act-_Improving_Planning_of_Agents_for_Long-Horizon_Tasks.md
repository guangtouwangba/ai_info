...
大型语言模型（LLMs）在简单任务上表现优异，但在复杂多步骤任务中仍面临挑战。研究提出"Plan-and-Act"框架，通过分离高层规划与底层执行来提升性能：Planner模型生成结构化高层计划，Executor模型将其转化为具体操作。为优化Planner，团队开发了合成数据生成方法，通过标注真实轨迹和多样化示例增强泛化能力。在网页导航任务测试中，该框架在WebArena-Lite基准上达到57.58%成功率（SOTA），在纯文本环境的WebVoyager上实现81.36%成功率（SOTA），显著提升了长程规划能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T16:02:45Z
- **目录日期**: 2025-04-23
