...
量子位发布的《2025中国AIGC应用全景图谱2.0》将AI生成内容（AIGC）应用划分为6大领域：1）B端行业应用（教育/医疗/金融等）；2）AI智能助手（豆包、Kimi等平台）；3）AI办公工具（写作/翻译/PPT等效率工具）；4）开发者工具（大模型平台/低代码工具）；5）AI创作（绘图/视频/音乐生成）；6）文娱生活场景（陪伴/健康/育儿等C端应用）。图谱系统梳理了AI技术在中国各行业的落地现状，覆盖企业决策、技术开发到日常生活的完整生态链，为不同需求的用户提供应用参考框架。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T20:04:02Z
- **目录日期**: 2025-04-23
