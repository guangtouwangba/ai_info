...
大型语言模型（LLMs）在决策场景中存在贪婪性、频率偏差和知行差距（无法有效执行已知知识）三大问题。研究表明，通过强化学习（RL）对LLMs自生成的思维链（CoT）进行微调，可显著提升其决策能力，增强探索性并缩小知行差距。实验涵盖多臂老虎机、上下文老虎机和井字棋等场景，验证了RL微调的有效性。此外，研究还比较了传统探索机制（如ε-贪婪）与LLM特有方法（如自我纠正和自洽性）对决策优化的效果。该工作为改进LLM的决策能力提供了系统性解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T15:01:10Z
- **目录日期**: 2025-04-23
