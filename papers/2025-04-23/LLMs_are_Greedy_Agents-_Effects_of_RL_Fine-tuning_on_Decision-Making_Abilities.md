...
大型语言模型（LLMs）在决策场景中存在贪婪性、频率偏差和知行差距等问题，导致探索不足和决策低效。研究发现，通过基于自生成思维链（CoT）的强化学习（RL）微调，可显著提升LLMs的探索能力并缩小知行差距。实验覆盖多臂老虎机、上下文老虎机和井字棋等任务，验证了RL微调的有效性。此外，研究对比了传统探索机制（如ε-贪婪）和LLM特有方法（如自我修正和一致性），为优化LLMs的决策能力提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T17:01:25Z
- **目录日期**: 2025-04-23
