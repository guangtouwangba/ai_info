...
大型语言模型(LLMs)在决策场景中存在三个主要缺陷：贪婪性、频率偏差和知行差距(知道但无法有效执行)。研究发现，通过强化学习(RL)对LLMs自生成的思维链(CoT)进行微调，可以显著提升其决策能力，增强探索性并缩小知行差距。实验在多臂老虎机、情境老虎机和井字棋等任务中验证了该方法的有效性。研究还比较了传统探索机制(如ε-贪婪)和LLM特有方法(如自我校正和一致性)对决策微调的影响，为优化LLMs的决策能力提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T18:01:11Z
- **目录日期**: 2025-04-23
