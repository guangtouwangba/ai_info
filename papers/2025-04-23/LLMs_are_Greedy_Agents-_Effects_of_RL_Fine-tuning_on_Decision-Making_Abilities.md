...
大型语言模型（LLMs）在决策任务中存在贪婪性、频率偏差和知行差距（知道但无法有效行动）等问题，导致探索效率低下。研究表明，通过基于自生成思维链（CoT）的强化学习（RL）微调，可显著提升LLMs的决策能力，增强探索行为并缩小知行差距。实验覆盖多臂老虎机、上下文老虎机和井字棋等场景，验证了RL微调的有效性。研究还对比了传统探索策略（如ε-greedy）与LLM特有方法（如自我校正和一致性），为优化LLMs的决策能力提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T11:01:13Z
- **目录日期**: 2025-04-23
