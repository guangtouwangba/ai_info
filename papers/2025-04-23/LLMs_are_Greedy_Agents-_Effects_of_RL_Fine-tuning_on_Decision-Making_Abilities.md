...
大型语言模型（LLM）在决策场景中存在贪婪性、频率偏差和知行差距等问题，导致探索不足和决策次优。研究发现，通过基于自生成思维链（CoT）的强化学习（RL）微调，可以提升LLM的探索能力并缩小知行差距。实验涵盖多臂老虎机、上下文老虎机和井字棋任务，验证了RL微调的有效性。此外，研究对比了传统探索机制（如ε-贪婪）与LLM特有方法（如自我纠正和一致性）对决策优化的作用。结果表明，RL微调能显著增强LLM在复杂领域的决策能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T10:01:24Z
- **目录日期**: 2025-04-23
