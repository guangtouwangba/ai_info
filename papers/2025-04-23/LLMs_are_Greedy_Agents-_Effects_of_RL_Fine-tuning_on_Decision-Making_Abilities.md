...
大型语言模型（LLMs）在决策场景中存在贪婪性、频率偏差和知行差距等问题，导致探索效率低下和决策表现不佳。研究发现，通过强化学习（RL）对LLMs自生成的思维链（CoT）进行微调，可显著提升其决策能力，增强探索效率并缩小知行差距。实验覆盖多臂老虎机、上下文老虎机和井字棋等任务，验证了RL微调的有效性。此外，研究还比较了传统探索机制（如ε-贪婪）与LLM特有方法（如自我修正和一致性）对决策优化的作用，为LLM在复杂领域的应用提供了改进方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T21:01:14Z
- **目录日期**: 2025-04-23
