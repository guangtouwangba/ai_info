...
大型语言模型(LLM)在决策任务中存在探索不足和"知行差距"等问题，表现为贪婪性、频率偏差等缺陷。研究发现，通过强化学习(RL)对LLM自生成的思维链(CoT)进行微调，可显著提升其决策能力，增强探索效率并缩小知行差距。实验涵盖多臂老虎机、上下文老虎机和井字棋等场景。研究还比较了ε-greedy等传统探索机制与LLM特有的自我修正、自我一致性等方法，为优化LLM决策能力提供了有效途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T20:01:14Z
- **目录日期**: 2025-04-23
