...
尽管无线通信技术有所进步，但通信负担仍是限制分布式模型预测控制（DMPC）实际应用的关键因素。本文提出一种基于自动编码器的通信数据压缩方法，通过编码器压缩传输数据，接收端解码器重构数据，从而减少通信量。该方法特别适用于需要长预测范围以保证稳定性的场景。研究以非完整约束的差速机器人编队控制为例，仿真和嵌入式硬件实验表明：该方法在保持控制精度的同时，显著优于直接缩短预测范围的常规方案，且在完全通信失效的实用场景中仍能有效运行，验证了其工程可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T17:02:37Z
- **目录日期**: 2025-04-23
