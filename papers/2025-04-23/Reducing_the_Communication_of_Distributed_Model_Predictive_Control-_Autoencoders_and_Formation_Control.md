...
分布式模型预测控制(DMPC)在实际应用中仍面临通信瓶颈问题。本研究提出利用自动编码器压缩通信数据量的创新方法：编码器在发送前压缩数据，解码器在接收后重建数据。该方法特别针对具有最优控制问题结构特性的数据设计，并在具有非完整约束的差速驱动机器人编队控制中验证。仿真和嵌入式硬件实验表明，该方法在保持控制精度的同时显著降低通信量，性能优于传统缩短预测时域的方法，且在实际无线通信环境下表现优于全通信方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T10:02:33Z
- **目录日期**: 2025-04-23
