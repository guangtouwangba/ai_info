...
本文提出了一种新的逆向强化学习（IRL）方法，通过直接策略优化替代传统对抗式训练。该方法利用回报的线性分解（后继特征与奖励向量的内积），设计基于策略梯度下降的算法来缩小学习器与专家特征的差距。这种非对抗方法无需学习奖励函数，可直接与现有actor-critic强化学习算法结合。特别地，该方法仅需专家状态信息（无需动作标签），解决了行为克隆（BC）无法处理的问题。实验表明，该方法仅需单次专家演示即可学习，并在多种控制任务中取得更优性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T19:02:33Z
- **目录日期**: 2025-04-23
