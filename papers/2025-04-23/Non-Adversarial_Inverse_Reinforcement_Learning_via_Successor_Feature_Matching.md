...
本文提出了一种新的逆向强化学习（IRL）方法，通过直接优化策略而非传统的对抗式训练来模仿专家行为。该方法利用回报的线性分解特性（后继特征与奖励向量的内积），设计了一种基于策略梯度的IRL算法，直接最小化学习者与专家特征间的差距。这种非对抗式方法无需学习奖励函数，可与现有actor-critic算法无缝结合，且仅需状态观察（无需专家动作标签）。实验表明，该方法能从单条专家演示中学习，并在多个控制任务上取得更好性能，解决了行为克隆（BC）无法处理的状态级模仿问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T22:02:31Z
- **目录日期**: 2025-04-23
