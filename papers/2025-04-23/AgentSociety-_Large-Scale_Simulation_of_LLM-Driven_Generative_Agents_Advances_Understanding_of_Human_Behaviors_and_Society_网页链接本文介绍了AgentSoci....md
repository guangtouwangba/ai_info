...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个AI代理的社会互动（500万次）。它结合真实社会环境，用于研究社会现象如信息传播、政策影响等。通过四个案例（社会极化、信息传播、基本收入政策、飓风冲击），验证了其模拟结果与现实数据的一致性。该系统可作为计算社会科学的研究工具，帮助理解人类行为机制，为政策制定提供参考。研究证明了LLM驱动代理在模拟复杂社会现象方面的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T13:13:34Z
- **目录日期**: 2025-04-23
