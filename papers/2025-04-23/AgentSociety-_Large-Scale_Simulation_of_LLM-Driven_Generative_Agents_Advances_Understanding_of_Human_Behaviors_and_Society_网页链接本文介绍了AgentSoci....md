...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次）。它通过四个关键社会问题（社会极化、信息传播、基本收入政策、自然灾害影响）的模拟实验，验证了其模拟结果与现实世界数据的高度吻合。该系统结合LLM智能体、真实社会环境和强大模拟引擎，能有效捕捉人类行为机制，为社会科学研究和政策制定提供新工具，弥补传统方法（如调查、访谈）的局限性。研究表明该平台具有揭示社会问题模式与成因的重要潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T07:04:11Z
- **目录日期**: 2025-04-23
