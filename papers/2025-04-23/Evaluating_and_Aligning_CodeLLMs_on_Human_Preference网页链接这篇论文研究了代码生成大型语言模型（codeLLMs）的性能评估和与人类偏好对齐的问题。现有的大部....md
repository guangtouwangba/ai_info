...
这篇论文针对代码生成大模型(codeLLMs)提出新评估方法，指出当前基准测试过于关注代码正确性而忽视人类偏好。研究者创建了CodeArena基准(包含397个样本，覆盖40类任务和44种语言)和SynCode-Instruct指令数据集(近200亿标记)，用于评估模型与人类偏好的对齐程度。实验发现执行基准与CodeArena存在性能差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)差距显著，凸显对齐人类偏好的重要性。研究强调需要更全面的评估标准来提升代码生成模型的实际应用价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T14:06:34Z
- **目录日期**: 2025-04-23
