...
这篇论文针对代码生成大模型（codeLLMs）与人类偏好对齐的问题，提出了CodeArena基准测试（包含397个高质量样本，覆盖40类编程任务和44种语言）和SynCode-Instruct合成指令数据集（近200亿标记）。研究发现现有基于执行的评估与人类偏好存在差距，实验显示开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著，强调了对齐人类偏好的重要性。研究通过构建更贴近实际编程场景的评估体系，推动了代码生成模型的实用化发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T23:05:48Z
- **目录日期**: 2025-04-23
