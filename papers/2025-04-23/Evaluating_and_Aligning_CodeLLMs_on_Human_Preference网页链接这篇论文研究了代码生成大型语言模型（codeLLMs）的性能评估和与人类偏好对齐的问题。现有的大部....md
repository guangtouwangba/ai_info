...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，重点关注与人类偏好的对齐问题。研究者创建了CodeArena基准测试（含397个样本，覆盖40类44种语言）和SynCode-Instruct指令数据集（20B标记），发现传统执行测试与人类偏好存在差异。实验比较40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，突显对齐人类偏好的重要性。研究为提升代码生成模型的实用性和用户体验提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T00:05:57Z
- **目录日期**: 2025-04-23
