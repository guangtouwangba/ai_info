...
该研究提出了一种基于大语言模型（LLM）的运动预测智能体，通过结合记忆系统提升外骨骼在动态施工环境中的辅助性能。系统采用多模态输入（语音指令和智能眼镜视觉数据），包含感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块。实验显示：无记忆系统时加权F1分数为0.73，加入STM后提升至0.81，结合STM和LTM后达到0.90，在模糊指令和安全关键场景表现优异。校准指标显著改善（Brier分数从0.244降至0.090，ECE从0.222降至0.044），验证了系统可靠性，为动态行业的人机协作提供了安全适配方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T00:01:29Z
- **目录日期**: 2025-04-23
