...
该研究提出了一种基于大型语言模型（LLM）的运动预测智能体，通过整合多模态输入（语音指令和智能眼镜视觉数据）来提升外骨骼在动态施工环境中的辅助性能。系统包含感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块，显著提高了运动模式识别的准确性。实验显示，加权F1分数从无记忆时的0.73提升至STM+LTM的0.90，在模糊和安全关键指令中表现优异。校准指标（Brier分数从0.244降至0.090，ECE从0.222降至0.044）验证了系统的可靠性，为动态行业中的自适应辅助系统提供了安全解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T02:28:40Z
- **目录日期**: 2025-04-23
