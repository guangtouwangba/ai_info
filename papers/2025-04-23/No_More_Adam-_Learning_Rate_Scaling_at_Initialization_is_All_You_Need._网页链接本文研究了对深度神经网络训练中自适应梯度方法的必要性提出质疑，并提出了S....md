...
这篇论文提出了一种名为SGD-SaI的新优化方法，通过初始化阶段基于梯度信号噪声比(g-SNR)调整学习率，取代了传统自适应优化器(如Adam)。该方法在保持SGDM简单性的同时，解决了Transformer模型训练不平衡问题，并将内存占用减半。实验显示，SGD-SaI在多种任务(包括ImageNet分类、GPT-2预训练和LoRA微调)中表现优于或持平AdamW，尤其在大模型训练中显著节省内存(GPT-2节省5.93GB，Llama2-7B节省25.15GB)，同时展现出优秀的超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T00:04:48Z
- **目录日期**: 2025-04-23
