...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练必须使用自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练Transformer的难题。相比AdamW，SGD-SaI内存用量减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，并展现出更好的超参数鲁棒性。在内存节省方面，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB。该方法在LoRA微调等任务中也优于现有优化器，具有广泛实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T06:03:17Z
- **目录日期**: 2025-04-23
