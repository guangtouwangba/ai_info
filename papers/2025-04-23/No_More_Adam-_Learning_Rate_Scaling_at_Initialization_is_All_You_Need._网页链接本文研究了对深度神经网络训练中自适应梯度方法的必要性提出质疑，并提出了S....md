...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法(如Adam)的必要性。SGD-SaI通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡的问题，同时将优化器内存使用减半。实验表明，该方法在Transformer任务、ImageNet分类、LLM预训练和LoRA微调等场景中表现优于或相当于AdamW，尤其在内存效率上优势显著：为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存。该方法具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-23T18:04:31Z
- **目录日期**: 2025-04-23
