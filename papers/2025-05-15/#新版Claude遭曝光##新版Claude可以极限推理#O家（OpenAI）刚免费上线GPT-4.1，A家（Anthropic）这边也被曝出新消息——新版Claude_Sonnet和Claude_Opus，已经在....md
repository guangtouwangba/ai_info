...
Anthropic即将推出新版Claude模型（Sonnet和Opus），主打"极限推理"功能。该功能通过动态循环结合推理与工具使用，能像人类一样暂停、重新评估并调整策略，尤其在代码生成时会自动测试修正。新模型架构更具适应性，减少对人类监督的依赖，支持128k tokens上下文（疑似Claude 3.8）。消息源自The Information，部分用户已发现测试中的"Claude Neptune"模型。尽管存在炒作质疑，但Anthropic官方确认正在进行新版本压力测试，预计数周内发布。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T23:02:55Z
- **目录日期**: 2025-05-15
