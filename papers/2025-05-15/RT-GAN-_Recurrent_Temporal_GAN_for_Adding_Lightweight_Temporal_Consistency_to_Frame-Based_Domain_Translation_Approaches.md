...
美国每年进行1400万例结肠镜检查，但高清视频因存储限制（单段视频可达数十GB）通常不被保存，仅保留少量关键帧用于报告和AI训练。现有结肠镜AI模型多基于单帧图像开发，缺乏时间一致性。本文提出RT-GAN（循环时序生成对抗网络），通过可调时序参数，以降低5倍训练资源的需求，将单帧模型升级为时序一致模型。方法在结肠皱襞分割（漏诊区域指示）和模拟结肠镜视频生成两个任务中验证有效，并发布了首个结肠镜时序数据集。代码、预训练模型已开源于Computational Endoscopy Platform GitHub。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T01:30:38Z
- **目录日期**: 2025-05-15
