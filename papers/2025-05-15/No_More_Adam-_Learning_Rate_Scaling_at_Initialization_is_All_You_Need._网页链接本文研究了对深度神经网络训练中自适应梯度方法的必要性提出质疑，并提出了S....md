...
该研究提出了SGD-SaI方法，质疑了自适应梯度方法(如Adam)的必要性。SGD-SaI通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题。相比AdamW，它内存效率提升50%，在Transformer、ImageNet分类和LLM预训练等任务中表现相当或更好，尤其在大模型训练中节省显著内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法还展现出超参数鲁棒性，在LoRA微调等场景优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T02:33:39Z
- **目录日期**: 2025-05-15
