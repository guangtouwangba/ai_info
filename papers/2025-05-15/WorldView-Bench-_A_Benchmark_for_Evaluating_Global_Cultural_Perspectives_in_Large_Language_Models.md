...
当前大语言模型（LLMs）的训练和校准主要基于西方中心主义认知体系，导致文化同质化，难以反映全球文明多样性。为此，研究者提出WorldView-Bench基准，通过自由生成式评估（而非传统分类测试）来量化模型的全球文化包容性（GCI），重点关注文化极化现象。研究采用两种干预策略：1）上下文实现的多元模型（通过系统提示嵌入多元原则）；2）多智能体系统（MAS）实现的多元模型（不同文化视角的LLM智能体协作生成响应）。实验显示，MAS方法使视角分布熵值从基线13%提升至94%，积极情绪占比达67.7%，显著提升了文化平衡性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T06:01:36Z
- **目录日期**: 2025-05-15
