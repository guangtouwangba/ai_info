...
当前大型语言模型（LLMs）的训练和校准主要基于西方中心主义认知体系，导致文化同质化，难以反映全球文明多样性。现有评估框架无法有效捕捉这种偏见。为此，研究者提出WorldView-Bench基准，通过自由生成式评估（而非传统分类测试）来测量LLMs的全球文化包容性（GCI），重点关注文化极化现象。采用两种干预策略：基于系统提示的上下文多重化LLMs，以及多智能体系统（MAS）实现的多重化LLMs（不同文化视角的LLM协作生成回答）。实验显示，MAS方法使视角分布熵从13%提升至94%，情感正向率达67.7%，显著提升了文化平衡性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T11:01:39Z
- **目录日期**: 2025-05-15
