...
当前大语言模型(LLM)的训练和校准主要基于西方中心主义认知体系，导致文化同质化问题。为评估全球文化包容性(GCI)，研究者开发了WorldView-Bench基准测试，采用自由生成式评估(而非传统分类测试)来测量文化极化现象。通过两种干预策略：(1)上下文实现的多元模型(系统提示嵌入多元原则)；(2)多智能体系统(MAS)实现的多元模型(代表不同文化视角的LLM智能体协作生成回答)，研究显示多元视角分布熵值从基线13%提升至94%，积极情绪达67.7%，显著提升了文化平衡性。这表明多元意识AI评估能有效减少LLM中的文化偏见。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T08:01:35Z
- **目录日期**: 2025-05-15
