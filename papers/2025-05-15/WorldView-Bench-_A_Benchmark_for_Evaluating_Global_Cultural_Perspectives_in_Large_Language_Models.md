...
当前大语言模型（LLM）的训练和对齐方式普遍强化西方中心主义认知，导致文化同质化，难以反映全球文明多样性。为此，研究者提出WorldView-Bench基准，通过自由生成式评估（而非传统分类测试）来量化模型的全球文化包容性（GCI）。基于"多元世界观"理论，研究采用两种干预策略：1）上下文实现的多元LLM（通过系统提示嵌入多元原则）；2）多智能体系统（MAS）实现的多元LLM（不同文化视角的智能体协作生成响应）。实验显示，MAS方法使观点分布熵值从13%提升至94%，积极情感占比达67.7%，显著提升了文化平衡性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T19:01:35Z
- **目录日期**: 2025-05-15
