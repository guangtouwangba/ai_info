...
vivo与香港科技大学联合研发的PreSelect数据筛选方法被ICML 2025接收。该方法通过fastText评分器计算数据的预测强度（Predictive Strength），量化数据对模型能力的贡献，筛选出能显著提升模型效果的高质量数据。相比现有方法，PreSelect理论基础更坚实，减少人工规则依赖，计算需求降低10倍，效率显著提升。该方法基于"压缩即智能"理念，利用模型压缩数据能力（BPC）与下游任务表现的强相关性，实现更客观、泛化的数据筛选。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T18:03:18Z
- **目录日期**: 2025-05-15
