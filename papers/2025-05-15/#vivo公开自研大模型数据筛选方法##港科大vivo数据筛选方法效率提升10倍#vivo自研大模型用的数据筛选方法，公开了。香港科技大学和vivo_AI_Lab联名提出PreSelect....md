...
vivo与香港科技大学联合研发的PreSelect数据筛选方法被ICML 2025接收。该方法提出"预测强度"(Predictive Strength)指标，通过fastText评分器分析数据对模型能力的贡献，筛选出能提升模型效果的高质量数据。相比现有方法，PreSelect减少10倍计算需求，理论基础更扎实，减少人工规则依赖，筛选更客观高效。其核心是利用模型压缩能力(BPC)与下游任务表现的关联性，优先保留能保持模型loss排序与benchmark得分排序一致性的数据，显著提升训练效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T22:02:38Z
- **目录日期**: 2025-05-15
