...
这篇综述首次系统性地总结了检测大型AI模型（如扩散模型和大语言模型）生成的多媒体内容（文本、图像、视频、音频等）的研究现状。作者提出了新的分类法，将检测方法分为"纯检测"（提升检测性能）和"超越检测"（增强泛化性、鲁棒性等特性）两类，并概述了生成机制、数据集、在线工具和评估指标。研究特别关注社交媒体视角下的社会影响，指出了当前检测技术面临的挑战和未来研究方向。该工作旨在填补学术空白，为维护数字信息安全做出贡献。项目链接已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T10:02:36Z
- **目录日期**: 2025-05-15
