...
这篇论文综述了大型AI模型（LAIMs）生成的多媒体内容检测研究。随着扩散模型和大语言模型的快速发展，AI生成内容在带来便利的同时也伴随着滥用、社会扰乱和伦理风险。研究首次系统性地分类了针对文本、图像、视频、音频等多模态内容的检测方法，提出基于"纯检测"（提升检测性能）和"超越检测"（增强通用性、鲁棒性等）的新分类法。论文还总结了生成机制、数据集、在线工具和评估指标，特别从社交媒体视角分析了社会影响，并指出当前检测挑战与未来研究方向，旨在填补学术空白并促进AI安全发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T11:02:47Z
- **目录日期**: 2025-05-15
