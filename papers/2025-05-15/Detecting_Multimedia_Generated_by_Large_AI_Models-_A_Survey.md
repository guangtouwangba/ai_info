...
这篇论文首次系统综述了检测大型AI模型(如扩散模型和大语言模型)生成的多媒体内容(文本、图像、视频、音频等)的研究。作者提出了新的分类法，将检测方法按媒体类型分为纯检测(提升性能)和超越检测(增强通用性、鲁棒性等)两类。研究概述了生成机制、数据集、在线工具和评估指标，特别从社交媒体角度分析了社会影响，并指出当前检测挑战与未来方向。该综述旨在填补学术空白，为全球AI安全做出贡献，确保数字领域信息完整性。项目链接已提供。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T21:02:19Z
- **目录日期**: 2025-05-15
