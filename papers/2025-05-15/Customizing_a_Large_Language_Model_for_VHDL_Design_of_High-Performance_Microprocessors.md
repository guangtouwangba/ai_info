...
近年来，大型语言模型（LLM）在硬件设计中的应用逐渐兴起，主要用于提升芯片设计效率。虽然Verilog设计中的LLM应用备受关注，但VHDL领域的研究较少，尽管其在工业界仍广泛使用。本文探讨了针对高性能处理器设计机构的需求，开发了专门用于解释VHDL代码的LLM模型。通过定制测试集和扩展预训练（EPT），专家评估显示模型解释准确率从43%提升至69%。进一步开发了类似专家评估的LLM-as-a-judge方法，指令调优后的EPT模型预期准确率达71%，未来使用新基础模型可能提升至85%以上。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T08:01:10Z
- **目录日期**: 2025-05-15
