...
近年来，大型语言模型（LLM）在硬件设计中的应用逐渐兴起，主要用于提升芯片设计效率。现有研究多聚焦Verilog语言，而同样广泛使用的VHDL及其在高性能处理器设计中的独特需求却较少被探讨。本文介绍了开发专用于VHDL代码解释的LLM的实践：通过定制测试集评估模型，并对基础模型进行扩展预训练（EPT），使专家对代码解释的评分从43%提升至69%。研究还开发了LLM-as-a-judge评估方法，最终指令调优模型的预期评分达71%，且采用新基础模型后有望提升至85%以上。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T03:18:44Z
- **目录日期**: 2025-05-15
