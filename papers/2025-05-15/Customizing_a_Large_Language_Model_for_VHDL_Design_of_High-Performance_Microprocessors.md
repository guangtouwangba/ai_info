...
近年来，大型语言模型（LLM）在硬件设计中得到应用，主要提升芯片设计效率。当前研究多集中于Verilog语言，而VHDL虽在行业广泛使用却关注不足，尤其在高性能处理器设计领域。本文介绍了开发专用于解释VHDL代码的LLM的过程，通过定制测试集和扩展预训练（EPT），专家评估显示模型代码解释准确率从基础模型的43%提升至69%。研究还开发了LLM-as-a-judge评估方法，经指令调优的EPT模型预期准确率达71%，使用新基础模型可进一步提升至85%以上。最后探讨了利用生成式AI新进展优化硬件设计LLM的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T16:01:07Z
- **目录日期**: 2025-05-15
