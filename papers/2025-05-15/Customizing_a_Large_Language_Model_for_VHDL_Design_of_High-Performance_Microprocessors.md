...
近年来，大型语言模型（LLM）在硬件设计中得到广泛应用，主要用于提升芯片设计效率。虽然Verilog设计中的LLM应用受到较多关注，但VHDL的应用研究较少，尽管其在行业中仍广泛使用。本文探讨了在高性能处理器设计中开发专用于VHDL代码解释的LLM的过程。通过定制测试集和扩展预训练（EPT），专家评估显示EPT模型的代码解释准确率从基础模型的43%提升至69%。进一步开发了LLM-as-a-judge评估方法，指令调优后的EPT模型预期准确率达71%。研究表明，采用更新基础模型可将准确率提升至85%以上。最后讨论了利用生成式AI新进展进一步提升硬件设计LLM质量的可能性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T12:01:10Z
- **目录日期**: 2025-05-15
