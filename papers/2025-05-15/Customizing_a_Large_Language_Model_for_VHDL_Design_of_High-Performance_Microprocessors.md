...
近年来，大型语言模型（LLM）在硬件设计领域逐渐应用，主要提升芯片设计效率。目前研究多集中于Verilog，而VHDL的应用较少，尤其在高性能处理器设计领域。本文介绍了开发专用于VHDL代码解释的LLM的历程，通过定制测试集和扩展预训练（EPT），模型解释准确率从43%提升至69%。此外，开发了LLM-as-a-judge评估方法，进一步优化模型，指令调优版本预期准确率达71%。实验表明，采用新基础模型可将准确率提升至85%以上。未来将结合生成式AI的新进展，进一步提升硬件设计LLM的质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T17:01:08Z
- **目录日期**: 2025-05-15
