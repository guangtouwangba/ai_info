...
近年来，大型语言模型（LLM）在硬件设计中的应用逐渐兴起，主要被用于提升芯片设计效率。虽然Verilog设计中的LLM应用受到较多关注，但VHDL的应用研究较少，尽管其在工业界仍广泛使用。本文探讨了在高性能处理器设计组织中开发专用于解释VHDL代码的LLM的历程。通过针对性测试集和扩展预训练（EPT），专家对代码解释的评分从基础模型的43%提升至69%。此外，开发了类似专家评估的LLM-as-a-judge方法，进一步优化模型，预期评分可达71%，使用新模型甚至可超过85%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T19:01:15Z
- **目录日期**: 2025-05-15
