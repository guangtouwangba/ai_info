...
近年来，大型语言模型（LLM）在硬件设计中的应用逐渐兴起，但主要集中在Verilog设计，对VHDL的关注较少。本文介绍了针对高性能处理器设计需求开发VHDL代码解释专用LLM的过程。通过定制测试集和扩展预训练（EPT），模型生成的代码解释专家评分从基础模型的43%提升至69%。研究还开发了LLM-as-a-judge评估方法，并进一步优化出指令调优版本，预期专家评分达71%。实验表明，采用更新的基础模型可将评分提升至85%以上。最后探讨了利用生成式AI新进展进一步提升硬件设计LLM质量的可能性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T04:01:26Z
- **目录日期**: 2025-05-15
