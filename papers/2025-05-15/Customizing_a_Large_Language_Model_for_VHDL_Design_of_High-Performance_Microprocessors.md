...
近年来，大型语言模型（LLMs）在硬件设计中的应用逐渐兴起，主要用于提升芯片设计效率。虽然Verilog设计中的LLM应用备受关注，但VHDL领域的研究较少，尽管其在行业中仍广泛使用。本文探讨了为高性能处理器设计组织开发专用LLM以解释VHDL代码的过程。通过定制测试集和扩展预训练（EPT），专家评估显示EPT模型的代码解释准确率从基础模型的43%提升至69%。进一步开发了LLM-as-a-judge评估方法，指令调优后的EPT模型预期准确率达71%，使用新模型后有望提升至85%以上。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T20:01:11Z
- **目录日期**: 2025-05-15
