...
这篇论文研究了代码生成大模型(codeLLMs)与人类偏好的对齐问题。作者提出了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，更贴近真实编程场景。同时构建了SynCode-Instruct合成指令数据集(近200亿标记)验证大规模微调效果。实验发现：1)传统执行基准与CodeArena存在性能差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐上仍有显著差距。研究凸显了除代码正确性外，模型输出与人类偏好匹配的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T05:05:57Z
- **目录日期**: 2025-05-15
