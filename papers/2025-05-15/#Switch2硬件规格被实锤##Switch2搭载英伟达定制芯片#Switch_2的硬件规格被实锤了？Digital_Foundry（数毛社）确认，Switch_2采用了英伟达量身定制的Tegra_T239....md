# #Switch2硬件规格被实锤##Switch2搭载英伟达定制芯片#Switch 2的硬件规格被实锤了？Digital Foundry（数毛社）确认，Switch 2采用了英伟达量身定制的Tegra T239...
...
Switch 2确认采用英伟达定制的Tegra T239芯片（Ampere架构），性能较前代显著提升但存在模式差异：底座模式下GPU频率达1007MHz（约PS4 Pro水平），掌机模式降至561MHz（约PS4水平），内存带宽和光追性能同步缩减。CPU频率在掌机模式反升（1101MHz），可能用于缓解带宽限制。配备8核A78C架构（实际6核可用）、12GB内存（开发者可用9GB），新增文件解压引擎和Game Chat功能（占用资源较高）。7.9英寸1080p HDR屏幕支持120Hz，强调功耗与便携平衡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T05:03:03Z
- **目录日期**: 2025-05-15
