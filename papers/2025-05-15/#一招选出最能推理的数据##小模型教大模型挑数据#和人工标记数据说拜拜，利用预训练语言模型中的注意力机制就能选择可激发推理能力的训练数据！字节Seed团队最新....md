...
字节Seed团队提出AttentionInfluence方法，利用小模型（1.3B参数）的注意力机制筛选能提升大模型（7B参数）推理能力的数据。该方法无需人工标注，通过屏蔽关键"检索头"计算损失差异，对数据评分并筛选。实验显示，用该方法选出的数据训练后，7B模型在MMLU、GSM8K等推理和代码生成任务中性能提升1.4-3.5个百分点。这一无监督、高效的数据选择策略因简单巧妙的设计受到学界关注，突破了传统依赖标注数据的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T22:03:41Z
- **目录日期**: 2025-05-15
