...
字节Seed团队提出AttentionInfluence方法，利用小模型（1.3B）的注意力机制筛选能提升大模型（7B）推理能力的数据，无需人工标注或训练。通过识别并屏蔽关键检索头，计算损失差异对数据评分，筛选出高质量数据。实验显示，该方法在多个基准测试中显著提升模型性能（如MMLU+1.4%、GSM8K+2.7%）。这一简单高效的创新思路受到业界关注，谷歌DeepMind科学家也为其点赞。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T14:03:29Z
- **目录日期**: 2025-05-15
