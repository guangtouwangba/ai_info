...
该研究探讨了语言模型（LM）在因果推理中的表现，发现LM能可靠推断常见的析取因果关系，但系统性地难以处理较少见的合取关系，表现出"析取偏差"。这种偏差存在于不同模型家族、规模和提示策略中，且随任务复杂性增加而加剧。有趣的是，类似偏差也出现在人类成人中，表明LM可能从训练数据继承了深层推理启发式。研究量化了LM与人类推理的相似性，发现LM更接近成人而非儿童的推理模式。最后，提出一种测试时采样方法，通过显式采样和排除因果假设，显著减少了析取偏差，提升了LM的科学推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T23:00:56Z
- **目录日期**: 2025-05-15
