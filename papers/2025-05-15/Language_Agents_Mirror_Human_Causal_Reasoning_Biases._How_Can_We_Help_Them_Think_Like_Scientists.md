...
这篇论文研究了语言模型（LM）在因果推理中的表现，发现LM能可靠推断常见的析取因果关系，但系统性地难以处理有充分证据的合取关系，表现出“析取偏差”。这种偏差存在于不同模型家族、规模和提示策略中，且随任务复杂度增加而恶化。有趣的是，人类成人也存在类似偏差，表明LM可能从训练数据中继承了这种深层推理启发式。研究量化了LM与人类的相似性，发现LM的推理模式更接近成人而非儿童。最后，作者提出一种测试时采样方法，通过显式采样和排除因果假设，显著减少了析取偏差。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T17:00:58Z
- **目录日期**: 2025-05-15
