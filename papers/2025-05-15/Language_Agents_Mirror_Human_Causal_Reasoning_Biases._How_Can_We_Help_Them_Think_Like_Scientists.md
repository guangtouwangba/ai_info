...
该研究探讨了语言模型（LM）在因果推理中的表现，发现模型能可靠推断常见的析取因果关系，但系统性地难以处理证据充分的合取关系，表现出“析取偏差”。这种偏差存在于不同规模、架构的模型中，且任务复杂度越高表现越差。有趣的是，类似偏差也出现在人类成人（而非儿童）中，暗示LM可能从训练数据继承了深层推理启发式。研究量化了LM与人类推理的相似性，并提出一种测试时采样方法，通过显式生成并排除因果假设显著减少了偏差，使LM更接近科学严谨的因果推理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T04:01:14Z
- **目录日期**: 2025-05-15
