...
该研究探讨了语言模型（LM）在因果推理中的表现，发现LM能可靠推断常见的析取因果关系，但系统性地难以处理较少见的合取关系，表现出"析取偏差"。这种偏差存在于不同规模、类型的模型中，且任务复杂度越高表现越差。有趣的是，类似偏差也存在于人类成人中，表明LM可能从训练数据继承了这种推理启发式。研究量化了LM与人类推理的相似性，发现LM更接近成人而非儿童的推理模式。最后，提出一种采样方法显著减少了这种偏差，使LM更接近科学的因果推理目标。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T08:00:59Z
- **目录日期**: 2025-05-15
