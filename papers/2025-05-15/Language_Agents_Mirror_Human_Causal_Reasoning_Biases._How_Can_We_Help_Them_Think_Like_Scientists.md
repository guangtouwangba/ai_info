...
该研究探讨了语言模型（LM）在因果推理中的能力，发现其能可靠推断常见的析取因果关系，但系统性地难以处理同样有证据支持的合取关系，表现出"析取偏差"。这种偏差在不同模型家族、规模和提示策略中均存在，且随任务复杂度增加而恶化。有趣的是，类似偏差也出现在人类成人中，表明LM可能从训练数据继承了根深蒂固的推理启发式。研究量化了LM与人类推理的相似性，发现其更接近成人而非儿童模式。最后，提出一种测试时采样方法，显著减少了析取偏差，提升了科学因果推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T15:01:04Z
- **目录日期**: 2025-05-15
