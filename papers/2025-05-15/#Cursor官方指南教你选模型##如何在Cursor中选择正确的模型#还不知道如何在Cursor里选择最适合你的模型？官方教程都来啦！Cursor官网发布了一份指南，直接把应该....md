...
Cursor官方发布模型选择指南，建议从三个维度考量：自信度（如Gemini-2.5-pro快速决策）、思考深度（如o3深入规划）和上下文容量（大任务需高容量模型）。模型可分为思考型（自主推断，如Claude-3.7-sonnet）和非思考型（需明确指令，如GPT-4.1）。按任务类型推荐：小修改用Claude-3.5-sonnet，大规模重构用Claude-3.7-Sonnet，代码导航用Gemini-2.5-Pro，复杂问题用o3。指南附有选择流程图辅助决策。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T17:02:51Z
- **目录日期**: 2025-05-15
