...
Cursor官方发布模型选择指南，提出三个关键维度：自信度（如Gemini-2.5-pro快速决策）、思考深度（如o3深入规划）和上下文容量。模型可分为思考型（自主推断，如Claude-3.7-sonnet）和非思考型（需明确指令，如GPT-4.1）。按任务推荐：小修改用Claude-3.5-sonnet，大规模重构用Claude-3.7-Sonnet，复杂问题用o3。指南包含决策树工具辅助选择。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T20:03:01Z
- **目录日期**: 2025-05-15
