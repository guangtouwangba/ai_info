...
Cursor官方发布模型选择指南，提出三个关键维度：自信度（如Gemini-2.5-pro能快速决策）、思考深度（如o3会深入规划）和上下文容量。模型可分为思考型（自主决策，如Claude-3.7-sonnet）和非思考型（需明确指令，如GPT-4.1）。任务适配建议：小改动用Claude-3.5-sonnet，大规模重构用Claude-3.7-Sonnet，代码搜索用Gemini-2.5-Pro，复杂问题用o3。指南包含决策树工具辅助选择。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T19:02:52Z
- **目录日期**: 2025-05-15
