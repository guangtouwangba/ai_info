...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。为解决扩散过程中时间一致性问题，提出时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时间一致性。同时针对SyncNet收敛问题进行实证研究，改进了其训练框架，提高了准确性。该方法在HDTF和VoxCeleb2数据集上表现优于现有技术，为唇同步和音频驱动动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T13:12:49Z
- **目录日期**: 2025-05-15
