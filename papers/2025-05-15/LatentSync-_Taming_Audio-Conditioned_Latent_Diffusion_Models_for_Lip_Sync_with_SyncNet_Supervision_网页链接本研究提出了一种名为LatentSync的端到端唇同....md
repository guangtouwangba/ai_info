...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音视频相关性，并针对扩散过程中时间一致性问题提出TREPA方法，通过自监督视频模型提取的时间表示增强生成帧的时序一致性。同时，研究改进了SyncNet训练框架，解决了其收敛问题，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括端到端潜在扩散建模、TREPA时序优化及SyncNet训练改进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T09:05:56Z
- **目录日期**: 2025-05-15
