...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型中时间一致性问题，提出时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时间一致性。同时改进了SyncNet训练框架，解决了其收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有方法。这些改进也可应用于其他唇同步和音频驱动的人像动画技术。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T01:34:17Z
- **目录日期**: 2025-05-15
