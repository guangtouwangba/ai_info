...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法无需中间运动表示，直接建模音频-视觉关系，利用Stable Diffusion的强大能力。针对扩散模型的时间一致性问题，提出了TREPA方法（时间表示对齐），通过自监督视频模型增强生成帧的时间连贯性。研究还解决了SyncNet的收敛问题，优化了模型架构、训练参数和数据预处理，显著提升了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新方案。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T08:05:57Z
- **目录日期**: 2025-05-15
