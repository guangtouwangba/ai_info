...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它研究了四个关键社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。模拟结果与现实世界数据吻合，验证了该平台在分析人类行为机制和社会问题方面的有效性。该系统可作为社会科学研究和政策制定的创新工具，补充传统研究方法（如调查和访谈），帮助理解复杂社会现象的成因和潜在机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-15T04:05:30Z
- **目录日期**: 2025-05-15
