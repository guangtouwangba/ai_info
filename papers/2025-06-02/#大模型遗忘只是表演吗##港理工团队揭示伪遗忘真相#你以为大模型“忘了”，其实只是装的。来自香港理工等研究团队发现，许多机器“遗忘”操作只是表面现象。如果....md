...
香港理工大学等团队研究发现，大模型的"遗忘"可能只是表象。他们开发了表示空间分析工具箱，区分"伪遗忘"（可逆）和"真遗忘"（不可逆）。关键结论是：仅当多个网络层发生大范围参数扰动时才是真正遗忘，否则模型仍能恢复记忆。工具箱通过PCA方向变化、表示漂移、CKA相似性等指标，系统评估遗忘真实性。这项研究揭示了模型内部结构变化才是判断遗忘的核心标准，为理解AI记忆机制提供了新视角。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T13:13:18Z
- **目录日期**: 2025-06-02
