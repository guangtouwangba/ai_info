...
香港理工团队研究发现，大模型的"遗忘"可能是伪装的。他们开发了表示空间分析工具箱，区分"伪遗忘"(可逆)和"真遗忘"(不可逆)。关键发现是：仅当多个网络层发生大范围扰动时才是真遗忘。若内部结构未变，即使输出表现下降，模型仍能轻易"想起"。真遗忘会导致表示方向、分布中心等全方位变化且不可恢复。该工具箱通过PCA、CKA相似性等多维度分析，可准确判断遗忘真实性。研究表明，仅表面性能变化而结构未变的"遗忘"只是表演。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T20:03:47Z
- **目录日期**: 2025-06-02
