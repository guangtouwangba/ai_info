...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发的分析工具表明，只有当模型多个网络层发生大范围参数扰动时，才属于不可逆的真遗忘。若仅输出指标变化而内部结构未变，模型仍能恢复记忆。该研究通过PCA、CKA等多维度分析方法，首次系统区分了可逆与不可逆遗忘，为评估模型记忆机制提供了新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T12:03:45Z
- **目录日期**: 2025-06-02
