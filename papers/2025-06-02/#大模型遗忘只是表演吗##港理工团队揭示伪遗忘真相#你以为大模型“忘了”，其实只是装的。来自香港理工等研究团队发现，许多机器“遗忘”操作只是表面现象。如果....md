...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发了表示空间分析工具箱，通过PCA、CKA等方法区分真/伪遗忘。关键发现：仅当多层网络发生大范围参数扰动时才是真遗忘（不可逆）；若仅输出指标变化而内部结构未变，模型仍可恢复记忆（可逆）。真遗忘会伴随表示方向、分布中心等全方位变化。该研究为评估模型遗忘机制提供了系统性方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T07:04:12Z
- **目录日期**: 2025-06-02
