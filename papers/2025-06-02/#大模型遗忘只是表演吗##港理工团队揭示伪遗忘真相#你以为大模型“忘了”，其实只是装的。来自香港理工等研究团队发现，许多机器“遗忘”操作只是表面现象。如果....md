...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象。他们开发了分析工具箱，区分"伪遗忘"(可逆)和"真遗忘"(不可逆)。关键发现是：仅当多个网络层发生大范围参数扰动时才是真遗忘，否则模型仍能恢复记忆。工具箱通过PCA方向变化、表示漂移、CKA相似性等多维度评估遗忘真实性。研究表明，仅输出指标变化而结构未变时，模型实际上并未真正遗忘。该研究为理解大模型记忆机制提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T16:03:37Z
- **目录日期**: 2025-06-02
