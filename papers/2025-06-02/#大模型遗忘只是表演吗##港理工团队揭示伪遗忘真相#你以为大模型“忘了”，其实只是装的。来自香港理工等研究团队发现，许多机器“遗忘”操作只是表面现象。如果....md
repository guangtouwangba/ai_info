...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象。他们开发了表示空间分析工具箱，首次系统区分"伪遗忘"(可逆)和"真遗忘"(不可逆)。关键发现表明，只有当多个网络层发生大范围参数扰动时，才算真正遗忘。若仅输出表现下降而内部结构未变，模型仍能恢复记忆。该工具箱通过PCA方向变化、表示漂移、CKA相似性等多维度分析，可准确评估遗忘真实性。这项研究揭示了AI遗忘机制的本质特征，为模型可解释性提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T22:03:29Z
- **目录日期**: 2025-06-02
