...
香港理工大学等研究团队发现，大语言模型的"遗忘"可能只是表面现象。他们开发了一套分析工具，区分"伪遗忘"（可逆）和"真遗忘"（不可逆）。关键发现是：仅当多个网络层发生大范围参数扰动时才是真正的遗忘。如果模型内部结构未改变，即使输出表现下降，仍能轻易恢复记忆。研究团队通过PCA方向变化、表示漂移、CKA相似性分析等多维度方法，首次系统验证了模型遗忘的本质差异。这项研究为评估AI系统的记忆机制提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T05:03:47Z
- **目录日期**: 2025-06-02
