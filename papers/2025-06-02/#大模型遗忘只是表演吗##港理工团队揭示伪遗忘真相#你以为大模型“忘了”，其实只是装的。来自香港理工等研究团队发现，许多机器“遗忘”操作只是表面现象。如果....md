...
香港理工大学团队研究发现，大语言模型的"遗忘"可能是表面现象（伪遗忘）。他们开发了分析工具箱，通过评估模型内部结构变化（如参数敏感区、表示方向等）来区分真/伪遗忘。结果显示：仅当多层网络发生大范围扰动时才是真遗忘（不可逆）；若仅输出指标变化而结构未变，模型仍可恢复记忆（可逆伪遗忘）。该研究提出了首个系统性评估框架，包含PCA、CKA等多维度分析方法，揭示了模型遗忘的本质机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T10:03:30Z
- **目录日期**: 2025-06-02
