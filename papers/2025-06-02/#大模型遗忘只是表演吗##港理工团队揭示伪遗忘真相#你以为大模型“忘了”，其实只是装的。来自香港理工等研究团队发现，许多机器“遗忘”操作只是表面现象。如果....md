...
香港理工大学团队研究发现，大语言模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发了一套分析工具箱，通过评估模型内部结构变化（如表示方向、参数敏感区域等）来区分真/伪遗忘。关键结论：仅当多个网络层发生大范围不可逆扰动时才是真遗忘；若仅输出指标变化而结构未变，模型仍能恢复记忆。该研究首次系统定义了遗忘的可逆性标准，为AI安全领域提供了重要评估工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T09:04:33Z
- **目录日期**: 2025-06-02
