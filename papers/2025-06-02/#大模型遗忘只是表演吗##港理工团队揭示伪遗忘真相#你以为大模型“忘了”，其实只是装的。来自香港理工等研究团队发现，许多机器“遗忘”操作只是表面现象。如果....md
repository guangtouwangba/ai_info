...
香港理工大学团队研究发现，大语言模型的"遗忘"可能是"伪遗忘"——仅输出表现变化而内部结构未变，仍可恢复记忆。他们开发了多维度分析工具（PCA、CKA、Fisher矩阵等），首次系统区分真/伪遗忘：真遗忘需多层网络大规模扰动，导致表示方向、参数敏感区等不可逆变化；伪遗忘仅是表面指标（如准确率下降）变化，结构未变时可快速恢复。该研究为评估模型记忆机制提供了新方法论。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T01:30:34Z
- **目录日期**: 2025-06-02
