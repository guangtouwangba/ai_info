...
香港理工大学等研究团队发现，大语言模型的"遗忘"可能是"伪遗忘"——当模型输出性能下降但内部结构未改变时，知识仍可被恢复。真正的遗忘需要多个网络层发生大范围参数扰动，导致表示方向、分布中心等根本性变化。团队开发了包含PCA、CKA相似性分析等工具的分析系统，首次系统区分了可逆的"伪遗忘"与不可逆的"真遗忘"。研究表明，仅凭输出指标变化不能证明知识被真正删除，关键要看模型内部结构的改变程度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T08:03:59Z
- **目录日期**: 2025-06-02
