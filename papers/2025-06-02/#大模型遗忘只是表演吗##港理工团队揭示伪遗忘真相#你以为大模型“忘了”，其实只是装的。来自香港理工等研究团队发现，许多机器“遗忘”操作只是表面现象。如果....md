...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象（伪遗忘），而非真正的遗忘。他们开发了一套分析工具箱，通过评估模型内部结构变化（如网络层扰动、表示方向漂移等）来区分伪遗忘和真遗忘。关键结论是：只有多个网络层发生大范围不可逆变化才算真正遗忘；若仅输出表现下降而内部结构未变，模型仍可恢复记忆。该研究为评估AI遗忘机制提供了系统性方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T21:03:39Z
- **目录日期**: 2025-06-02
