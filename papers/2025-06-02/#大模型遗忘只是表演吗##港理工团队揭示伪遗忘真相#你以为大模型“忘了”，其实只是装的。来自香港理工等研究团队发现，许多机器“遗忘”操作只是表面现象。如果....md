...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发了表示空间分析工具箱，通过PCA、CKA等方法区分真/伪遗忘。关键结论：仅当多个网络层发生大范围参数扰动时才是真遗忘（不可逆）；若仅输出指标变化而内部结构未变，模型仍能恢复记忆（可逆伪遗忘）。真遗忘会导致表示方向、分布中心等根本性改变。该研究首次系统定义了遗忘标准，为评估模型记忆机制提供了量化工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T00:02:55Z
- **目录日期**: 2025-06-02
