...
香港理工团队研究发现，大模型的"遗忘"可能是表面现象（伪遗忘）。他们开发了表示空间分析工具箱，通过多维度指标（PCA、CKA相似性等）区分真/伪遗忘。关键结论：仅当网络层发生大范围参数扰动导致表示方向、分布中心等根本性改变时，才是不可逆的真遗忘；若仅输出指标变化而内部结构未变，模型仍能恢复记忆。该研究首次系统定义了机器学习中的遗忘机制，为模型可解释性提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T11:03:45Z
- **目录日期**: 2025-06-02
