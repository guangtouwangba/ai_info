...
香港理工大学团队研究发现，大语言模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发了一套分析工具箱，通过检测模型内部结构变化（如参数敏感区域、表示方向等）来区分真伪遗忘。关键结论是：只有多层网络发生大规模参数扰动才是真遗忘，否则模型仍能恢复记忆。该研究提出了PCA、CKA等多维度评估方法，揭示了当前遗忘技术可能存在的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T04:09:30Z
- **目录日期**: 2025-06-02
