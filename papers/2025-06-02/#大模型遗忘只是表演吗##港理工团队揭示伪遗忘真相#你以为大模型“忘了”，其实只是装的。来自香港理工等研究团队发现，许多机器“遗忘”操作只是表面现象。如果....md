...
香港理工大学等研究团队发现，大语言模型的"遗忘"可能只是表面现象。他们开发了一套分析工具，首次区分"伪遗忘"（可逆）和"真遗忘"（不可逆）。关键发现是：仅当模型多个网络层发生大范围参数扰动时才是真遗忘；若仅输出指标变化而内部结构未变，模型仍能轻易恢复记忆。研究团队通过PCA方向变化、表示漂移、CKA相似性等多维度分析，证实只有参数敏感区域发生全方位改变才代表真正遗忘。这一发现对理解AI记忆机制具有重要意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T15:04:00Z
- **目录日期**: 2025-06-02
