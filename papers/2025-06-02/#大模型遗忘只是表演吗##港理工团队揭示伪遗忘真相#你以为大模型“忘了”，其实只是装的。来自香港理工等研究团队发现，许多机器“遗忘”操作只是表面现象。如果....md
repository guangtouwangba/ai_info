...
香港理工团队研究发现，大语言模型的"遗忘"可能只是表面现象（伪遗忘），而非真正的遗忘。他们开发了一套分析工具箱，通过评估模型内部结构变化（如表示方向漂移、参数敏感区域等）来区分"伪遗忘"（可逆）和"真遗忘"（不可逆）。关键发现是：仅当多个网络层发生大范围扰动时才是真遗忘；若内部结构未变，即使输出表现下降，模型仍能恢复记忆。该研究为评估AI系统的记忆机制提供了新方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T06:03:39Z
- **目录日期**: 2025-06-02
