...
AgentSociety是一个基于大型语言模型(LLM)的大规模社会模拟器，可模拟超过1万个代理及其500万次互动。研究团队用它研究了四个关键社会问题：社会极化、刺激性信息传播、基本收入政策影响和自然灾害(如飓风)效应。模拟结果与现实世界数据高度吻合，验证了该平台在捕捉人类行为机制方面的有效性。AgentSociety可作为计算社会科学研究的创新工具，为研究者和政策制定者提供新的研究范式，补充传统方法(如调查和访谈)的不足，有助于深入理解复杂社会现象背后的模式和机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T19:05:14Z
- **目录日期**: 2025-06-02
