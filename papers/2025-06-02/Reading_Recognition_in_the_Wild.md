...
本文提出了一种用于智能眼镜的阅读识别任务，以判断用户是否在阅读。研究者首先构建了首个大规模多模态"Reading in the Wild"数据集，包含100小时真实场景下的阅读与非阅读视频。研究确定了三种相关且互补的模态数据（第一视角RGB、眼动追踪、头部姿态），并提出了一种灵活的Transformer模型来处理这些单独或组合的模态。实验表明这些模态对任务有效，并探索了各模态的高效编码方法。该数据集还能扩展当前受限环境下的阅读研究，支持更丰富的阅读类型分类。相关代码、模型和数据将公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T03:25:23Z
- **目录日期**: 2025-06-02
