...
本文提出了一种用于智能眼镜的阅读识别任务，以记录用户与世界的互动。研究者首先发布了首个大规模多模态"野外阅读"数据集，包含100小时真实场景下的阅读和非阅读视频。通过分析三种模态数据（第一视角RGB、眼动追踪、头部姿态），开发了一个灵活的Transformer模型，可单独或组合使用这些模态。研究表明这些模态对任务具有相关性和互补性，并探索了每种模态的有效编码方式。该数据集还能扩展当前受限环境下的阅读理解研究，支持更丰富的阅读类型分类。相关代码、模型和数据将公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T16:01:30Z
- **目录日期**: 2025-06-02
