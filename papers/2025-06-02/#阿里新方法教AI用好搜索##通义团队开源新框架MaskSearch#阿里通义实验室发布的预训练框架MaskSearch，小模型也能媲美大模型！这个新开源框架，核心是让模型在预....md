...
阿里通义实验室开源MaskSearch框架，通过检索增强型掩码预测(RAMP)技术，让AI模型在预训练时学会结合搜索引擎进行推理。该框架创新性地融合监督微调(SFT)和强化学习(RL)方法：SFT采用多智能体生成思维链，RL通过混合奖励机制优化推理路径。配合课程学习策略，显著提升小模型性能，在跨领域任务中可媲美大模型。实验显示，不同掩码策略和奖励函数对模型表现影响显著，验证了"教会模型如何学习"的重要性。该技术为提升小模型推理能力提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T01:31:32Z
- **目录日期**: 2025-06-02
