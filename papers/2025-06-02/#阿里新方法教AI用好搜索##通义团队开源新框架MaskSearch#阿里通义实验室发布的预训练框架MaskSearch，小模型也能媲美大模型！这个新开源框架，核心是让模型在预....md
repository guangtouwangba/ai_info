...
阿里通义实验室开源了MaskSearch预训练框架，通过"检索增强型掩码预测"(RAMP)方法，让AI模型在预训练时学会结合搜索引擎进行推理。该框架创新性地兼容监督微调(SFT)和强化学习(RL)两种训练方式，采用多智能体生成思维链和混合奖励系统优化推理路径，并引入课程学习策略提升泛化能力。实验显示，该方法使小模型在问答任务中表现媲美大模型，尤其在跨领域任务上效果显著。研究还发现掩码策略和奖励函数设计对性能有重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T02:36:06Z
- **目录日期**: 2025-06-02
