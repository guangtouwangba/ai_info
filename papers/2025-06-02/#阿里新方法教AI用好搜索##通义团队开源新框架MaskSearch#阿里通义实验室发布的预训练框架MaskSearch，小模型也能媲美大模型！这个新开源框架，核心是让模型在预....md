...
阿里通义实验室开源预训练框架MaskSearch，通过检索增强型掩码预测(RAMP)技术，让模型在预训练阶段学会结合搜索引擎进行推理。该框架兼容监督微调(SFT)和强化学习(RL)训练方式，采用多智能体生成思维链和混合奖励系统优化推理路径，并引入课程学习提升泛化能力。实验显示，该方法使小模型在跨领域任务上表现媲美大模型，且不同掩码策略和奖励函数对性能影响显著。该技术有效提升了模型利用外部知识的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T00:03:50Z
- **目录日期**: 2025-06-02
