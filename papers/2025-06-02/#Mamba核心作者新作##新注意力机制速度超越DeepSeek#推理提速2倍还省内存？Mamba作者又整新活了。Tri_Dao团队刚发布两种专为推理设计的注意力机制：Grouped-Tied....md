...
Tri Dao团队（Mamba核心作者）新发布两种高效注意力机制GTA和GLA，专为推理优化。实验显示：GTA在中大型模型质量优于GQA；GLA与MLA精度相当但效率更高，64K长文本吞吐量显著提升，内存占用更低。新机制支持并发处理长文本和不均衡负载，解码速度提升2倍且保持生成质量。测试涵盖4种模型规模，评估指标包括困惑度、7个下游任务及延迟/吞吐量等。该技术有望替代DeepSeek当前使用的注意力方案，特别适合长上下文场景。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T04:09:44Z
- **目录日期**: 2025-06-02
