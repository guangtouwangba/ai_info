...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过根据梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet-1K分类、GPT-2预训练和LoRA微调中表现优于或媲美AdamW，同时显著节省内存（如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB），且对超参数变化更鲁棒。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T19:04:41Z
- **目录日期**: 2025-06-02
