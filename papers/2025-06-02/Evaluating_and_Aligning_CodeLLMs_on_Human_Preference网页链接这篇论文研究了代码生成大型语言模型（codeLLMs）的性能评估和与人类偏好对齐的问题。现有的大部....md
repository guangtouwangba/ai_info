...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配程度；2) 构建20B标记的合成指令数据集SynCode-Instruct验证指令微调效果。实验发现：现有基于执行的评估与人类偏好存在偏差，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在显著差距，凸显了优化方向的重要性。研究强调代码生成模型需兼顾功能正确性和用户体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T23:05:33Z
- **目录日期**: 2025-06-02
