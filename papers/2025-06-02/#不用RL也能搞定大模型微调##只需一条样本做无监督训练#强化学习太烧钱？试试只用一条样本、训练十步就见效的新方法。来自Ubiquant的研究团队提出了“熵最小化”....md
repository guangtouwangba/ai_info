...
Ubiquant研究团队提出"熵最小化"(EM)方法，可高效微调大语言模型。该方法仅需1条高质量样本和10步无监督训练，通过压缩模型输出的预测熵，强制模型选择最自信的答案。实验显示，在数学推理任务上，EM方法性能媲美甚至超越需要大量数据的强化学习(RL)方法。关键创新在于选择"模型回答不稳定"的问题作为训练样本，例如仅用1道物理题就显著提升了Qwen2.5-Math-7B模型的数学能力。相比RL，EM具有无监督、极简数据和快速收敛的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T23:03:09Z
- **目录日期**: 2025-06-02
