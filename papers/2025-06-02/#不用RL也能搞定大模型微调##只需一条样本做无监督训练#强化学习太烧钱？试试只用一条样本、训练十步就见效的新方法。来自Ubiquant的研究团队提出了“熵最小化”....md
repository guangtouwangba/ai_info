...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练即可显著提升模型性能。核心原理是迫使模型降低输出分布的熵值，增强答案确定性。实验显示，EM在数学推理任务上表现媲美传统RL方法，且通过筛选模型"犹豫不决"的问题作为训练样本效果最佳。例如仅用1道物理题就使Qwen2.5-Math-7B模型能力大幅提升，突破了RL方法需要海量数据的限制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T20:03:37Z
- **目录日期**: 2025-06-02
