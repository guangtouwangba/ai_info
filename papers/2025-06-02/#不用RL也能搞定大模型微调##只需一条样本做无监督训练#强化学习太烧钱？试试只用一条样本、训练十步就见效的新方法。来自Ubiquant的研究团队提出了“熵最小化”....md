...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练就能显著提升模型性能。核心原理是迫使模型降低输出答案的熵值，增强预测置信度。实验显示，该方法在数学推理任务上效果媲美传统RL，且成本极低。关键创新在于：1)通过模型回答稳定性筛选训练样本；2)仅优化输出分布熵值。例如用1道物理题就使Qwen2.5-Math-7B模型性能大幅提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T03:26:45Z
- **目录日期**: 2025-06-02
