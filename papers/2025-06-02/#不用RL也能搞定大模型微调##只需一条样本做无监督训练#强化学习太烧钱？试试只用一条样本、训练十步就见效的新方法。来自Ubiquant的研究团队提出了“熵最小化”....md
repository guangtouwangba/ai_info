...
Ubiquant团队提出"熵最小化"(EM)方法，通过无监督训练显著提升大语言模型性能。该方法只需1条高质量样本和10步训练，通过降低模型输出熵值使其更自信选择答案。在数学推理任务中表现媲美传统强化学习(RL)，且无需标注数据或奖励模型。关键创新在于选择模型回答不稳定的问题作为训练样本。实验显示，仅用1道物理题就使Qwen2.5-Math-7B模型能力大幅提升。相比RL，EM具有数据需求极低、训练快速等优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T19:03:22Z
- **目录日期**: 2025-06-02
