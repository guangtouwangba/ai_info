...
Ubiquant研究团队提出"熵最小化"(EM)方法，通过无监督方式微调大语言模型。该方法仅需1条高质量样本和10步训练，通过降低模型输出分布的熵值，迫使模型选择最自信的答案，显著提升数学推理能力。相比强化学习(RL)，EM无需标注数据或奖励模型，在效果上可媲美甚至超越使用大量数据的RL方法。关键创新在于选择模型回答不稳定的问题作为训练样本。实验显示，仅用1道物理题就使Qwen2.5-Math-7B模型的数学能力大幅提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T15:03:47Z
- **目录日期**: 2025-06-02
