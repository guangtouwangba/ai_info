...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练就能显著提升模型性能。核心原理是通过降低输出分布的熵值，迫使模型选择最自信的答案。实验显示，Qwen2.5-Math-7B模型仅用1道物理题训练后，数学推理能力大幅提升。关键创新在于选择模型回答不稳定的问题作为训练样本，这类"纠结"问题最适合EM优化。相比传统RL方法，EM在节省数据和计算资源方面优势显著。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T00:02:44Z
- **目录日期**: 2025-06-02
