...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法仅需1条高质量样本和10步无监督训练，通过降低模型输出答案的熵值(提高置信度)，就能显著提升数学推理能力。相比传统RL方法，EM省去了数据标注和奖励模型，在Qwen2.5-Math-7B上的实验显示，仅用1道物理题就使性能大幅提升。关键创新在于选择模型回答不稳定的问题作为训练样本，通过强制模型输出高置信度答案来提高准确性，效果可媲美使用海量数据的RL方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T02:34:56Z
- **目录日期**: 2025-06-02
