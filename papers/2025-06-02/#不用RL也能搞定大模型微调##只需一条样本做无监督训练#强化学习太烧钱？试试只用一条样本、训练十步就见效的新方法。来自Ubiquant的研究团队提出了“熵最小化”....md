...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法仅需1条高质量样本和10步训练，通过降低模型输出熵值(迫使选择最自信答案)实现无监督优化。实验显示，EM在数学推理任务中表现媲美传统RL方法，且具备显著优势：无需标注数据/奖励模型、训练极快、数据需求极少。关键创新在于筛选"模型回答不稳定"的问题作为训练样本，例如仅用1道物理题就显著提升了Qwen2.5-Math-7B的数学能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T07:04:02Z
- **目录日期**: 2025-06-02
