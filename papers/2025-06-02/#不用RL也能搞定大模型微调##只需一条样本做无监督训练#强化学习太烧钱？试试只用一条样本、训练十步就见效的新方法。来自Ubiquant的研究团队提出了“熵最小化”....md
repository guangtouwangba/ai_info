...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练，通过降低模型输出分布的熵值，迫使模型选择最自信的答案。实验显示，在数学推理任务中，EM性能媲美甚至超越需要海量数据的RL方法。关键创新在于选择"模型回答不稳定"的问题作为训练样本，例如仅用1道物理题就显著提升了Qwen2.5-Math-7B的推理能力。EM具有数据需求极低、训练速度快等优势，为模型微调提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T17:04:55Z
- **目录日期**: 2025-06-02
