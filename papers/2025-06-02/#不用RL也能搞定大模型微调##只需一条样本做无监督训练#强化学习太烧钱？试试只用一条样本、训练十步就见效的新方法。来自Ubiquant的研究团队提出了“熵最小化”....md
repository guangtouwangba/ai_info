...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该技术仅需1条高质量样本和10步无监督训练，通过降低模型输出熵值来提升置信度，在数学推理任务中表现媲美传统RL方法。关键创新在于选择模型回答不稳定的问题作为训练样本，例如仅用1道物理题就显著提升了Qwen2.5-Math-7B的性能。相比RL，EM具有无需标注数据、免奖励模型、训练极快等优势，为低成本优化大模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T18:03:26Z
- **目录日期**: 2025-06-02
