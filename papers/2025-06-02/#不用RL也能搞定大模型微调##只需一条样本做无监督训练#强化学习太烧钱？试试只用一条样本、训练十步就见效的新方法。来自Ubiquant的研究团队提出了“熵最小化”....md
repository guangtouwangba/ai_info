...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练，通过降低模型输出熵值来提升预测置信度。实验显示，在数学推理任务中，EM方法性能媲美甚至超越需大量数据的RL方法。关键创新在于选择模型回答不稳定的问题作为训练样本，如仅用1道物理题就显著提升了Qwen2.5-Math-7B的数学能力。该方法大幅降低了计算成本和数据需求，为模型优化提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T22:03:17Z
- **目录日期**: 2025-06-02
