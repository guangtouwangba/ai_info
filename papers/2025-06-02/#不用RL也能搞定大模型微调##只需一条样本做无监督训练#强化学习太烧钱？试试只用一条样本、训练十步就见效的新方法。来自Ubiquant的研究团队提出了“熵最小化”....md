...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法通过降低模型输出分布的熵值，迫使模型选择最自信的答案，仅需1条高质量样本和10步训练就能显著提升性能。关键创新在于：完全无监督、极简数据需求、快速收敛，在数学推理任务中表现媲美传统RL方法。样本选择策略独特——优先挑选模型回答不稳定的问题作为训练样本。实验显示，仅用1道物理题就使Qwen2.5-Math-7B模型数学能力大幅提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T08:03:46Z
- **目录日期**: 2025-06-02
