...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法仅需1条高质量样本和10步无监督训练，通过降低模型输出熵值(增强答案置信度)提升性能。在数学推理任务中，EM表现媲美传统RL方法，且无需标注数据或奖励模型。关键创新在于筛选"模型回答不稳定"的问题作为训练样本。实验显示，仅用1道物理题就使Qwen2.5-Math-7B模型数学能力显著提升，实现了低成本、高效率的模型优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T06:03:30Z
- **目录日期**: 2025-06-02
