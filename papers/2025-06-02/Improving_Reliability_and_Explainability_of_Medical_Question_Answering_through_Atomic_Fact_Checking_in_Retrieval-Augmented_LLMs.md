...
大型语言模型(LLMs)在医学领域展现出丰富知识，但存在幻觉和错误引用问题。本研究提出新型原子事实检查框架，将LLM生成的医学长问答分解为可独立验证的"原子事实"，对照权威医学指南进行验证。该方法实现了40%的答案质量提升和50%的幻觉检测率，能追踪每个事实到原始文献，显著提高医学问答的准确性和可解释性。医学专家评估和自动化基准测试证实了该框架的有效性，为LLM在临床应用中建立可信度迈出关键一步，有助于推动AI在医疗领域的可靠应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T20:02:12Z
- **目录日期**: 2025-06-02
