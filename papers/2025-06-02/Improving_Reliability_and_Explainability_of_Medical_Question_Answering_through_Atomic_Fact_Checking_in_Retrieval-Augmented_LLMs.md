...
大型语言模型（LLMs）在医学领域展现丰富知识，但存在幻觉和错误引用问题，影响临床应用。现有方法如检索增强生成（RAG）通过引用源文档部分缓解问题，但幻觉和低解释性仍存在。本研究提出原子事实核查框架，将LLM生成回答分解为可独立验证的"原子事实"，对照权威医学指南进行验证，实现精准纠错和文献溯源。经医学专家评估和自动化测试，该方法使答案准确率提升40%，幻觉检测率达50%，并能逐项追溯来源，显著提升医学问答的可信度和解释性，推动LLMs在临床中的可靠应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T16:01:53Z
- **目录日期**: 2025-06-02
