...
南洋理工大学与A*STAR团队提出新型AI水印方法MaskMark，采用"掩码机制"精准定位水印区域，分为全图嵌入(MaskMark-D)和局部嵌入(MaskMark-ED)两种版本。该方法在保持近100%提取准确率的同时，训练成本仅为Meta WAM模型的1/15（单卡A6000训练20小时），且支持多水印嵌入（最多5个）。尤其在小区域水印任务中表现突出，定位精度(IoU)超越现有SOTA模型，并具备快速微调能力以应对新型攻击。该技术特别适用于人脸、LOGO等局部保护场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T02:35:56Z
- **目录日期**: 2025-06-02
