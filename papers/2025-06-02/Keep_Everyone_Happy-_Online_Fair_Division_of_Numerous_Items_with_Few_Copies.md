...
这篇论文提出了一种新的在线公平分配问题变体，其中学习者需依次将不可分割的物品分配给多个代理，同时满足公平和效率约束。现有算法假设物品数量少且副本足够多，能通过噪声反馈准确估计效用，但这在现实中往往不成立（如平台用户多但使用次数少）。为此，作者假设效用是物品-代理特征的未知函数，将问题建模为上下文赌博机问题，并提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T00:02:24Z
- **目录日期**: 2025-06-02
