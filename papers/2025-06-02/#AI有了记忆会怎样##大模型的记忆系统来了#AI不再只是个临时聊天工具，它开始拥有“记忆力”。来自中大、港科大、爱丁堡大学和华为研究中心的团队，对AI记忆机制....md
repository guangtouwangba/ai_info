...
中大、港科大等团队对AI记忆机制进行系统综述，提出大语言模型(LLMs)需要更完善的记忆系统来支持长期交互。研究从记忆表示(参数化记忆和上下文记忆)和记忆操作两个维度展开，定义了6种核心操作：巩固、更新、索引、遗忘、检索和压缩。参数化记忆指模型内部知识，响应快但难更新；上下文记忆包括非结构化(如多模态内容)和结构化记忆(如知识图谱)，更具灵活性。该研究为AI实现类人记忆能力提供了统一框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T18:04:11Z
- **目录日期**: 2025-06-02
