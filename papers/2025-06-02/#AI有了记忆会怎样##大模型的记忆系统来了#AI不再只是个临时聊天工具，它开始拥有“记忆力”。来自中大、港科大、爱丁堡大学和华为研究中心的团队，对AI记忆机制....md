...
这篇研究综述探讨了大语言模型(LLMs)如何发展记忆系统以支持长期交互。研究团队提出了记忆的两个核心维度：参数化记忆(内部知识)和上下文记忆(显式信息)，后者包括非结构化(文本/图像)和结构化(知识图谱)记忆。研究定义了6种关键记忆操作：巩固、更新、索引、遗忘、检索和压缩，这些机制使AI能够像人类一样处理和利用记忆。该框架为构建更智能、个性化的AI系统提供了理论基础，使大模型能够更好地支持跨轮对话、多模态输入等复杂需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T15:04:45Z
- **目录日期**: 2025-06-02
