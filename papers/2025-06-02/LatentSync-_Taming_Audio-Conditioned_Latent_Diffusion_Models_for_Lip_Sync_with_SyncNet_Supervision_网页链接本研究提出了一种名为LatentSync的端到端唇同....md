...
该研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强帧间一致性。研究还解决了SyncNet收敛问题，通过优化模型架构、训练参数和数据预处理显著提升其精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他依赖SyncNet的音频驱动动画任务。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T13:15:49Z
- **目录日期**: 2025-06-02
