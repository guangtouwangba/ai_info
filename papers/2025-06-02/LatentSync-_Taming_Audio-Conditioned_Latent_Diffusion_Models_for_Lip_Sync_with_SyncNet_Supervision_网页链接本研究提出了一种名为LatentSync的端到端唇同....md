...
本研究提出LatentSync唇同步框架，采用音频条件潜在扩散模型直接建模音视频关联，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，利用自监督视频模型的时间表示对齐生成帧。同时优化SyncNet训练框架，通过改进模型架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。该研究不仅推进了唇同步技术，其SyncNet优化方案也可应用于其他音频驱动动画方法。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T03:29:27Z
- **目录日期**: 2025-06-02
