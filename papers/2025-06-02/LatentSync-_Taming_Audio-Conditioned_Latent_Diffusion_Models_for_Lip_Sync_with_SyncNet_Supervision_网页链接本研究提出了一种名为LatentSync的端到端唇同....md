...
本研究提出LatentSync，一种端到端唇同步框架，利用音频条件潜在扩散模型直接建模音频-视觉关系，无需中间运动表示。针对扩散模型的时间一致性问题，提出了TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时改进了SyncNet训练框架，解决了收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些创新不仅提升了唇同步效果，也为相关领域提供了SyncNet优化经验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T22:05:14Z
- **目录日期**: 2025-06-02
