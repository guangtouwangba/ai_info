...
ThinkGeo是一个针对遥感领域设计的智能体基准测试，用于评估大语言模型（LLM）通过结构化工具使用和多步规划解决复杂任务的能力。该基准包含436个人工设计的真实场景任务，涵盖城市规划、灾害评估、环境监测等应用领域，要求模型结合卫星/航空影像进行空间推理。研究采用ReAct式交互框架，测试了GPT-4o、Qwen2.5等开源/闭源模型，发现不同模型在工具使用准确性和规划一致性上存在显著差异。ThinkGeo首次为遥感领域的工具增强型LLM提供了系统评估平台，相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-02T01:29:05Z
- **目录日期**: 2025-06-02
