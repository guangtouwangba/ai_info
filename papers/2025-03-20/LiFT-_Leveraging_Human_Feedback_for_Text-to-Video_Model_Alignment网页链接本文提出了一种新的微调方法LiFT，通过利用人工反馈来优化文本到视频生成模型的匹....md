...
本文介绍了一种名为LiFT的新微调方法，通过利用人工反馈来优化文本到视频生成模型的匹配度。研究团队构建了一个包含1万个人工评分及其理由的数据集LiFT-HRA，并基于此训练了一个奖励模型LiFT-Critic，该模型能有效学习奖励函数，作为人类判断的代理。通过最大化奖励加权的似然性，调整了T2V模型。以CogVideoX-2B为例，结果显示微调后的模型在所有16项指标上均优于CogVideoX-5B，证明了人类反馈在提高生成视频匹配度和质量方面的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T07:04:42Z
- **目录日期**: 2025-03-20
