...
本研究提出了LatentSync，一种端到端唇同步框架，利用音频条件的潜在扩散模型直接建模音频与视觉的复杂关系，无需中间运动表示。针对扩散过程中时间一致性不足的问题，提出了时间表示对齐（TREPA）方法，通过自监督视频模型提取的时间表示增强一致性。研究还解决了SyncNet的收敛问题，通过调整模型架构、训练超参数和数据预处理方法，显著提高了SyncNet的准确性。该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步方法，为音频驱动的人像动画提供了新的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T15:04:55Z
- **目录日期**: 2025-03-20
