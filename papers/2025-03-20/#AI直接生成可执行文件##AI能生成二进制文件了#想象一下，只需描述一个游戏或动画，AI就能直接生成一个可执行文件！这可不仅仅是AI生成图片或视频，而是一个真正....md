# #AI直接生成可执行文件##AI能生成二进制文件了#想象一下，只需描述一个游戏或动画，AI就能直接生成一个可执行文件！这可不仅仅是AI生成图片或视频，而是一个真正...
...
ByteCraft是一款能够根据文本描述直接生成可执行二进制文件的AI工具。通过微调7B参数的Qwen2.5模型，ByteCraft在4个GPU上耗时4个月实现了这一功能。尽管当前版本存在一些缺陷，如生成的画面简单、动画异常、游戏玩法单一等，且上下文长度仅32KB，生成的文件大小最高约140KB，但其潜力巨大。研究人员表示，未来若有更多计算资源和更长的上下文支持，ByteCraft有望实现100%生成可用文件，甚至自动编写复杂多样的游戏。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T23:02:18Z
- **目录日期**: 2025-03-20
