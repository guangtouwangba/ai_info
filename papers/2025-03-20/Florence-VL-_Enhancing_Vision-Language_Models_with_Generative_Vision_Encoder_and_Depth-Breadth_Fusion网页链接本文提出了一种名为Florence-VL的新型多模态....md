...
Florence-VL是一种新型的多模态大型语言模型（MLLM），通过集成生成性视觉基础模型Florence-2来增强视觉表征。与常见的CLIP风格视觉变换器不同，Florence-2能捕捉多层次视觉特征，适应多种下游任务。研究团队设计了深度-广度融合（DBFusion）方法，将Florence-2的视觉特征有效融合到预训练的语言模型Phi 3.5和LLama 3中。模型通过端到端预训练和微调，使用高质量图像标题和指令调整对数据集进行训练。Florence-VL在视觉语言对齐和多模态基准测试中表现优异，显著提升了通用VQA、感知、OCR等任务的表现。研究团队开源了模型和训练方法，促进未来研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T02:32:53Z
- **目录日期**: 2025-03-20
