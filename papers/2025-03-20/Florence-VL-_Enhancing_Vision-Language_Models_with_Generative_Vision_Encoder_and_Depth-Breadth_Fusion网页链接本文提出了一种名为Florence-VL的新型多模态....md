...
本文介绍了Florence-VL，一种新型的多模态大型语言模型（MLLM），通过集成生成性视觉基础模型Florence-2来增强视觉表征。Florence-VL采用了一种名为“深度-广度融合（DBFusion）”的方法，有效融合了不同深度和多个提示下提取的视觉特征。该模型在多种多模态和视觉为中心的基准测试中表现出色，特别是在视觉语言对齐方面优于流行的视觉编码器。研究团队开源了模型和训练方法，以促进未来研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T19:04:44Z
- **目录日期**: 2025-03-20
