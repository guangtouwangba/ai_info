...
本文介绍了一种名为Infinity的位运算视觉自回归模型，能够根据语言指令生成高分辨率、逼真的图像。Infinity通过无限词汇量的标记分类器和位运算自我校正机制，显著提高了生成能力和细节表现。相较于传统自回归模型，Infinity在扩展能力和生成速度上表现出色，性能超过了顶尖的扩散模型如SD3-Medium和SDXL。Infinity在0.8秒内完成1024x1024图像的生成，速度是SD3-Medium的2.6倍，成为最快的文本到图像模型。研究团队将发布模型和代码，推动Infinity在视觉生成和统一标记建模领域的进一步探索。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T17:04:59Z
- **目录日期**: 2025-03-20
