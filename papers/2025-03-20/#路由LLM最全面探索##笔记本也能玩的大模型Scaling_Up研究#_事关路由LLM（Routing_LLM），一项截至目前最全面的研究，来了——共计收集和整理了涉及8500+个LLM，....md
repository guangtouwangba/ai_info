...
中山大学和普渡大学的研究人员对路由LLM（Routing LLM）进行了迄今为止最全面的研究，涉及8500多个LLM在12个基准测试上的2亿条性能记录。路由LLM通过将ChatGPT、Qwen、DeepSeek等LLM视为“专家”，由Router（路由器）将输入分配给合适的LLM处理，从而实现高性能、低计算消耗和低幻觉。研究发现，Router的性能随着LLM候选数量的增加而迅速提升，称为Model-level Scaling Up。他们还构建了RouterEval评测工具，使其他研究人员能在笔记本或单卡GPU等低计算资源环境下参与路由LLM研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T00:04:45Z
- **目录日期**: 2025-03-20
