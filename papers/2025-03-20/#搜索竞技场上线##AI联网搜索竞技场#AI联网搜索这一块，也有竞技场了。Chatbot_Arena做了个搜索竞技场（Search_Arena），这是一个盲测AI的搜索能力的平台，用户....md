...
Chatbot Arena推出了“搜索竞技场”（Search Arena），这是一个盲测AI搜索能力的平台，用户可以通过投票比较不同模型的搜索效果。例如，用户提问“下次月全食是什么时候？”，平台会展示两个模型的回答，并揭示答案来源。Search Arena还支持图片对话、文生图（Text-to-Image）以及与GitHub代码仓库对话等功能。通过对比不同模型的表现，用户可以自行判断哪个模型更优。感兴趣的用户可访问相关链接体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-20T00:03:26Z
- **目录日期**: 2025-03-20
