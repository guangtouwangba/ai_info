...
随着前沿AI模型规模和训练数据量持续增长，分布式深度学习成为关键技术。本研究提出Pseudo-Asynchronous Local SGD（PALSGD）方法，通过伪异步同步机制显著降低数据并行训练中的通信频率。PALSGD在保持模型一致性的同时，将同步间隔延长至标准Local SGD的4-8倍。理论分析证明了其收敛性，实验显示在ImageNet-1K（ResNet-50）和TinyStories（GPT-Neo）任务中，训练速度分别比DDP快18.4%和21.1-24.4%，且性能相当。该方法为大规模模型训练提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T19:01:30Z
- **目录日期**: 2025-04-28
