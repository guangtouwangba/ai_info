...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。为解决时间一致性问题，开发了TREPA方法，通过自监督视频模型对齐生成帧。研究还优化了SyncNet训练框架，提升其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)直接建模音频-视觉关系；2)增强时间一致性；3)改进SyncNet训练。该成果可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T19:04:44Z
- **目录日期**: 2025-04-28
