...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，并通过时间表示对齐(TREPA)解决扩散过程中的时间一致性问题。研究还改进了SyncNet训练框架，提升了其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术，为音频驱动人像动画提供了新思路。这些改进也可应用于其他依赖SyncNet的唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T14:05:48Z
- **目录日期**: 2025-04-28
