...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，能模拟1万多个智能代理的社会互动（500万次交互）。研究者用它研究了四大社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。结果显示其模拟结果与现实世界数据高度吻合，验证了系统对人类行为和社会机制的捕捉能力。该系统可作为计算社会科学的研究平台，为学者和政策制定者提供新工具，补充传统研究方法（如调查和实验）。研究表明LLM驱动的社会模拟在理解复杂社会现象方面具有重要潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T12:05:27Z
- **目录日期**: 2025-04-28
