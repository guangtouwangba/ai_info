...
Rice大学和xMAD.ai团队开发的DFloat11框架实现了大模型的无损压缩，可将模型体积缩减至70%（如Llama-3.1-405B从811.7GB压缩到551.22GB），且精度零损失。其核心技术包括：利用BFloat16权重的指数稀疏性（仅占用2.6位）、动态长度编码优化存储，以及专为GPU推理设计的查找表、并行解压等加速方案。压缩后数据搬运开销降低，推理速度最高提升38.8倍，单机8张80GB GPU即可运行。相比传统压缩方法，DFloat11在保持高吞吐量的同时实现部署友好，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T01:30:45Z
- **目录日期**: 2025-04-28
