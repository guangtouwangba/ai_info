...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于通过头戴式惯性测量单元(IMU)识别高/低层级活动。该分层算法采用半监督学习策略，主要利用高层级活动标签训练，学习可泛化的低层级运动嵌入特征。在9种高层级和3种低层级活动测试中，分别取得0.826和0.855的F1分数，模型参数仅63k(高层)和22k(低层)，可直接部署于现有IMU芯片。研究还分析了基于头戴IMU的活动识别技术的机遇与限制。该方法在资源占用和识别性能间取得了良好平衡，适用于健康监测和情境感知等智能眼镜应用场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T02:30:24Z
- **目录日期**: 2025-04-28
