...
本文提出了一种基于切换系统的方法，利用记忆回归扩展(MRE)技术延长自主智能体在GPS失效环境中的持续运行时间。该方法通过周期性获取GPS信号修正累积状态估计误差，克服未知动态和环境干扰。相比现有切换系统方法中GPS失效期间误差累积和保守运行时间限制的问题，新方法在GPS可用时采用MRE技术优化系统参数估计，从而延长GPS失效时的可靠运行时间。基于Lyapunov的稳定性分析表明，通过并行学习改进参数估计可在保证闭环系统稳定性的同时延长GPS失效运行时间。仿真结果验证了该方法能有效延长运行时间并提升轨迹跟踪性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T06:03:37Z
- **目录日期**: 2025-04-28
