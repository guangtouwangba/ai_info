...
该论文提出了一种基于切换系统的方法，通过利用记忆回归扩展(MRE)技术延长自主智能体在GPS失效环境中的持续运行时间。当周期性获得GPS信号时，智能体可修正累积的状态估计误差，同时利用MRE技术改进未知系统参数的估计。相比现有切换系统方法，该方法克服了GPS失效期间误差累积和保守运行时间限制的问题。基于Lyapunov的稳定性分析表明，通过并发学习获得的改进参数估计可在保证闭环系统稳定性的前提下延长GPS失效期间的运行时间。仿真结果验证了该方法能有效延长运行时间并提升轨迹跟踪性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T19:01:12Z
- **目录日期**: 2025-04-28
