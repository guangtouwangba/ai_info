...
本文提出了一种基于切换系统的方法，利用记忆回归扩展(MRE)技术延长自主智能体在GPS失效环境下的持续运行时间。该方法通过在GPS可用时段修正累积状态估计误差，克服了传统切换系统方法中估计误差增长和运行时间保守的问题。基于Lyapunov的稳定性分析表明，通过并发学习改进参数估计可在不损害系统稳定性的前提下延长GPS失效时段的运行时间。仿真结果验证了该方法能有效延长持续运行时间并提升轨迹跟踪性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T17:01:25Z
- **目录日期**: 2025-04-28
