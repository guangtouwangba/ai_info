...
本文提出了一种基于切换系统的自主智能体在GPS失效环境下延长驻留时间的方法，采用记忆回归扩展(MRE)技术。该方法通过周期性获取GPS信号来修正累积状态估计误差，克服未知动态和环境干扰。传统切换系统方法存在估计误差累积和保守驻留时间限制的问题，而新方法利用GPS可用时段的MRE技术优化系统参数估计，从而延长GPS失效时的可靠运行时间。基于Lyapunov的稳定性分析表明，通过并发学习改进的参数估计可在保证闭环系统稳定的前提下延长GPS失效运行时间。仿真验证了该方法能有效延长驻留时间并提升轨迹跟踪性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T16:01:39Z
- **目录日期**: 2025-04-28
