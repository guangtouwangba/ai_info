...
PaperCoder是一款多智能体系统，可将机器学习论文自动转换为可执行代码库。它通过三阶段流程实现高效复现：1) Planning阶段规划架构和依赖关系；2) Analyzing阶段分析模块功能与交互；3) Coding阶段生成可直接运行的代码。测试显示其代码改动率仅0.48%，77%原作者更青睐其生成结果。在Paper2Code等基准测试中，表现优于ChatDev和MetaGPT。目前专注于ML领域，未来计划扩展至物理、生物等学科。该项目已在GitHub开源，被用户评价为最接近"一键复现"的实用工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T01:31:44Z
- **目录日期**: 2025-04-28
