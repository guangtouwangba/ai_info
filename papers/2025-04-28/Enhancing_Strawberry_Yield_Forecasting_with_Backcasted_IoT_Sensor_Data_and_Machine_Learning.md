...
随着全球人口快速增长，数字农业对可持续粮食生产和资源管理决策至关重要。物联网(IoT)技术可实时监测影响作物生长的环境(如温湿度)和操作因素(如灌溉)，但AI模型需要大量数据，这在农场实际应用中面临挑战。本研究在草莓种植温室部署IoT传感器收集两季数据，结合四年产量记录。针对缺失的两季数据，提出基于AI的回溯方法，利用历史气象数据和现有观测生成合成传感器数据。实验表明，结合合成数据的AI产量预测模型优于仅使用真实数据和历史记录的模型，提高了预测准确性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T11:19:10Z
- **目录日期**: 2025-04-28
