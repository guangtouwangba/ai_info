...
随着全球人口快速增长，数字农业对可持续粮食生产和资源管理决策至关重要。研究通过在草莓种植大棚部署物联网传感器收集了两季环境数据（温度、湿度、土壤水分等），并结合四年产量记录。为解决前两季缺失的传感器数据，研究提出基于AI的回溯方法，利用附近气象站数据和现有观测生成合成数据。实验表明，结合合成数据的AI产量预测模型表现优于仅使用历史产量、气象记录和真实传感器数据的模型，验证了该方法能有效提升预测精度。该成果为数据不足条件下的农业预测提供了可行解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T19:01:39Z
- **目录日期**: 2025-04-28
