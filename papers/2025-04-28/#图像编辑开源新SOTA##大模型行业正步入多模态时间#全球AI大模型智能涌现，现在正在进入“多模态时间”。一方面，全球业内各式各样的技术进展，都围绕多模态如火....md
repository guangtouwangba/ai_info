...
AI行业正加速进入多模态时代，技术发展和应用落地都聚焦多模态能力。阶跃星辰作为代表性企业，过去一个月密集发布了三款多模态模型：Step1X-Edit（开源图像编辑模型）、Step-R1-V-Mini（多模态推理模型）和Step-Video-TI2V（开源图生视频模型），覆盖当前核心需求方向。其70%已发布模型为多模态，凸显向智能终端Agent落地的战略布局。这些开源且性能优秀的模型，既体现了技术趋势，也强化了阶跃在应用端的竞争力。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T14:04:26Z
- **目录日期**: 2025-04-28
