...
全球AI大模型正加速进入多模态时代，技术发展和应用落地均聚焦多模态能力。阶跃星辰作为行业领先者，过去一个月密集发布三款多模态模型：Step1X-Edit（开源图像编辑模型）、Step-R1-V-Mini（多模态推理模型）和Step-Video-TI2V（开源图生视频模型），覆盖核心应用场景。其70%已发布模型为多模态，凸显向智能终端Agent落地的战略布局。这些开源工具展现了技术流玩家对"多模态时间"趋势的快速响应，推动AI从单模态向跨模态协同的产业升级。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T10:37:50Z
- **目录日期**: 2025-04-28
