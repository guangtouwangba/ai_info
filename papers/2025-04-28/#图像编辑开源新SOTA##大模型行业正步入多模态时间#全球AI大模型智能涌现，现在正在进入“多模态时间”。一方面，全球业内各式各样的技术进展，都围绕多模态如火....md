...
AI行业正加速进入多模态时代，技术发展和应用落地都聚焦多模态能力。阶跃星辰作为多模态领域的领先者，近期发布了三款重要模型：Step1X-Edit（开源图像编辑模型）、Step-R1-V-Mini（多模态推理模型）和Step-Video-TI2V（开源图生视频模型），覆盖了当前多模态的核心需求方向。这些模型展现了出色的性能，其中两款已开源。阶跃星辰70%的模型为多模态，凸显其向智能终端Agent落地的战略布局，反映了行业向多模态发展的趋势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T17:03:53Z
- **目录日期**: 2025-04-28
