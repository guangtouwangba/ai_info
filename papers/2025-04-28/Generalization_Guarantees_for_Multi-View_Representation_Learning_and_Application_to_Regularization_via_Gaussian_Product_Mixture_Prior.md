...
本文研究了分布式多视图表示学习问题，探讨了在无显式协调下各代理应提取何种特征以保证解码器正确估计隐藏标签。作者从泛化误差角度出发，建立了基于相对熵的泛化边界，并提出了一种正则化方法。实验表明，采用数据依赖的高斯混合先验能取得良好效果，在单视图场景下性能优于现有VIB和CDVIB方法，并自然产生了加权注意力机制。多视图场景中，选择高斯乘积混合作为联合先验会促使各代理提取冗余特征，这一发现具有反直觉性。研究为多视图表示学习提供了理论框架和实用方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T12:01:47Z
- **目录日期**: 2025-04-28
