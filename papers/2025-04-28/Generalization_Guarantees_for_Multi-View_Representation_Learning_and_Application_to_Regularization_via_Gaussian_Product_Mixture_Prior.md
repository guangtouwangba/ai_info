...
本文研究了分布式多视图表示学习问题，探讨了在无显式协调的情况下，各代理如何从统计相关的视图中提取必要且充分的表示，以帮助解码器正确估计隐藏标签。作者从泛化误差角度出发，建立了基于相对熵的泛化边界，并提出了一种正则化方法。实验表明，采用数据依赖的高斯混合先验能取得良好性能，在单视图场景下优于现有方法（VIB和CDVIB），并自然衍生出加权注意力机制。多视图场景中，高斯乘积混合先验会隐式鼓励代理提取冗余特征，这一发现具有反直觉性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T13:15:55Z
- **目录日期**: 2025-04-28
