...
陈丹琦在ICLR 2025演讲中提出学术界训练语言模型的三大方向：1）开发高效小模型，如通过剪枝LLaMA-7B仅用3%计算成本实现更好性能；2）优化训练数据，其团队开发的QuRater模型筛选高质量数据，用300亿token训练出高性能13亿参数模型；3）发展后训练方法，如ProLong-8B模型通过微调技术实现512K长上下文处理，性能优于同类。她强调仅需数百GPU小时即可开展研究，呼吁学术界更积极参与语言模型研发，推动产学研合作新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T02:33:10Z
- **目录日期**: 2025-04-28
