...
这篇论文探讨了代码生成大模型（codeLLMs）如何更好地与人类偏好对齐。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发需求。同时开发了SynCode-Instruct合成指令数据集（约200亿标记）用于模型微调。实验发现，传统基于执行的评估与人类偏好存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T04:05:54Z
- **目录日期**: 2025-04-28
