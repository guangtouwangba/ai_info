...
这篇论文针对代码生成大模型(codeLLMs)与人类偏好对齐的问题，提出了CodeArena基准测试和SynCode-Instruct数据集。CodeArena包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发需求；SynCode-Instruct则是近200亿token的合成指令数据集。实验发现：1)传统基于执行的评估与人类偏好存在偏差；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面仍有显著差距。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T09:37:21Z
- **目录日期**: 2025-04-28
