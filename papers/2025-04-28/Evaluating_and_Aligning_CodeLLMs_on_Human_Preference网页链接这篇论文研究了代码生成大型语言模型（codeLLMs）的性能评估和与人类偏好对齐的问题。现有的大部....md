...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，重点关注模型输出与人类偏好的对齐。研究者创建了CodeArena基准测试（含397个多语言样本）和合成指令数据集SynCode-Instruct（20B标记），发现传统执行测试与人类偏好存在显著差异。实验比较40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商用模型（如OpenAI o1）性能差距明显，突显对齐人类偏好的重要性。研究为提升代码LLMs的实际适用性提供了新评估框架和数据支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T12:06:03Z
- **目录日期**: 2025-04-28
