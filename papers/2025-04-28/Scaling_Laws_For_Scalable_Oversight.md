...
该研究提出一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈，用分段线性Elo评分表示监督和欺骗能力。通过改良Nim游戏和四个监督博弈（"黑手党"、"辩论"、"后门代码"、"战争游戏"）验证框架，发现领域表现与AI通用能力（以Chatbot Arena Elo为代理）的缩放规律。研究进一步分析嵌套可扩展监督（NSO），确定其成功条件并推导最优监督层级数。数值模拟显示，当监督比基线强400Elo的系统时，NSO成功率低于52%，且随能力差距增大持续下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T18:01:14Z
- **目录日期**: 2025-04-28
