...
该研究提出一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈。通过Nim游戏变体和四种监督博弈（"黑手党"、"辩论"、"后门代码"、"战争游戏"）验证，发现领域表现与AI通用能力（以Chatbot Arena Elo为指标）存在缩放规律。进一步理论研究嵌套可扩展监督(NSO)时发现：当被监督系统比基线监督者强400 Elo时，NSO成功率低于52%，且随能力差距扩大持续下降。研究还推导出最大化监督成功率的最优监督层级数。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T04:01:17Z
- **目录日期**: 2025-04-28
