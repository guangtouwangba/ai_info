...
该研究提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家之间的博弈。通过Nim游戏变体和四个监督博弈（"黑手党"、"辩论"、"后门代码"和"战争游戏"）验证，发现领域性能与AI通用能力（以Chatbot Arena Elo为指标）存在比例关系。研究进一步探讨了嵌套可扩展监督(NSO)机制，发现当监督对象比基线监督者强400Elo时，NSO成功率低于52%，且随能力差距扩大继续下降。研究还推导了最大化监督成功率的最优监督层级数。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T07:09:15Z
- **目录日期**: 2025-04-28
