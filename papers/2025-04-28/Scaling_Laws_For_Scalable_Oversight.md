...
该研究提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈，监督者和被监督者分别具有监督和欺骗相关的Elo评分。通过Nim游戏和四个监督博弈（"黑手党"、"辩论"、"后门代码"、"战争游戏"）验证，发现领域表现与AI系统通用能力（以Chatbot Arena Elo为代理）存在缩放规律。研究进一步探讨了嵌套可扩展监督(NSO)机制，当监督比基线强400Elo的系统时成功率低于52%，且随能力差距增大会进一步下降，同时推导了最优监督层级数以最大化成功率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T14:01:22Z
- **目录日期**: 2025-04-28
