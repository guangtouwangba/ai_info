...
本文提出一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈，监督和被监督系统的能力用分段线性函数表示，包含任务不熟练和饱和两个平台期。通过改进Nim游戏验证框架后，应用于四种监督博弈场景，发现领域性能与AI通用能力（以Chatbot Arena Elo为指标）的缩放规律。进一步研究嵌套可扩展监督（NSO）发现：当监督比基线强400 Elo的系统时，成功率低于52%，且随能力差距扩大持续下降。研究还推导出最大化监督成功率的最优监督层级数。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T05:01:55Z
- **目录日期**: 2025-04-28
