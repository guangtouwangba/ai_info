...
该研究提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈。通过Nim游戏变体和四个监督博弈（"黑手党"、"辩论"、"后门代码"、"战争游戏"）验证，发现领域表现与AI通用能力（以Chatbot Arena Elo为指标）存在比例关系。研究进一步探讨了嵌套可扩展监督（NSO）机制，即信任模型监督更强非信任模型并迭代升级的过程。数值分析显示，当监督比基线强400 Elo的系统时，NSO成功率低于52%，且随能力差距扩大持续下降。研究为AI监督机制提供了量化评估方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T21:01:00Z
- **目录日期**: 2025-04-28
