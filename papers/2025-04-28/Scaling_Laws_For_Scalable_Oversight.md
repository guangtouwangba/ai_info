...
该研究提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家之间的博弈。研究者用改良版Nim游戏验证框架后，分析了四种监督博弈场景（"黑手党"、"辩论"、"后门代码"和"战争游戏"），发现领域表现与AI通用能力（以Chatbot Arena Elo分数衡量）存在缩放规律。针对嵌套可扩展监督（NSO）的理论研究表明：当被监督系统比基线监督者强400 Elo分时，NSO成功率低于52%，且随能力差距扩大持续下降。研究还推导了最大化监督成功率的最优监督层级数。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T23:01:03Z
- **目录日期**: 2025-04-28
