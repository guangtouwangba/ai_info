...
GeoSense是首个针对多模态大模型几何推理能力的双语评测基准，涵盖148个几何原理和1789道题目。评测显示，Gemini-2.0-Pro-Flash表现最佳，开源模型中Qwen-VL系列突出。模型规模越大推理能力越强，但平面几何仍是共同短板，尤其在相似/全等三角形等原理识别上存在困难。评测采用GPI（原理识别）、GPA（原理应用）和ACC（答案正确率）三项指标，发现原理识别能力不足会显著影响最终解题效果。研究指出提升原理识别是优化几何推理的关键方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-28T11:20:36Z
- **目录日期**: 2025-04-28
