...
本研究提出了一种新型的AI驱动质量工程平台，采用自适应差分进化量子变分自编码器-变压器模型（ADE-QVAET），以解决现有模型在处理噪声数据、不平衡问题、模式识别复杂性、特征提取不足和泛化能力弱等方面的挑战。ADE-QVAET结合量子变分自编码器-变压器（QVAET）获取高维潜在特征并保持序列依赖性和上下文关系，以及自适应差分进化（ADE）优化方法，提升模型收敛性和预测性能。该模型在训练百分比90时，准确率、精确率、召回率和F1分数分别达到98.08%、92.45%、94.67%和98.12%，展示了其在软件缺陷预测中的高效性和准确性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T14:02:51Z
- **目录日期**: 2025-03-23
