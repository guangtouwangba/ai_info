...
本研究提出了一种基于自适应差分进化的量子变分自编码器-变压器模型（ADE-QVAET），用于提升软件缺陷预测的准确性。该模型结合量子变分自编码器-变压器（QVAET）以获取高维潜在特征并保持序列依赖性和上下文关系，同时采用自适应差分进化（ADE）优化方法增强模型收敛性和预测性能。ADE-QVAET通过集成先进的AI技术，解决了现有模型在处理噪声数据、不平衡数据、模式识别复杂性和特征提取不足等问题上的局限性。实验结果显示，ADE-QVAET在训练百分比为90%时，准确率、精确率、召回率和F1分数分别达到98.08%、92.45%、94.67%和98.12%，表现出卓越的预测性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T03:15:43Z
- **目录日期**: 2025-03-23
