...
元戎启行CEO周光在英伟达GTC 2025上提出了一种新的自动驾驶AGI（人工通用智能）实现路径——RoadAGI。与传统的Robotaxi不同，RoadAGI旨在通过一个平台赋能各种移动体（如智能车、机器人、小电驴等），使其具备自主移动能力，且不依赖高精地图。这一新路径有望加速自动驾驶的大规模商用，并在垂直道路场景下实现AGI。元戎启行的AI Spark平台是RoadAGI的实施基础，展示了自动驾驶领域的创新可能性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T11:04:29Z
- **目录日期**: 2025-03-23
