...
本文介绍了AgentSociety，一个结合大型语言模型（LLM）驱动的代理、真实社会环境和强大模拟引擎的大规模社会模拟器。通过模拟超过1万个代理及其500万次互动，研究者探讨了极化现象、信息传播、无条件基本收入政策及外部冲击（如飓风）等社会问题。AgentSociety不仅与传统研究方法（如调查、访谈）相辅相成，还能揭示社会问题的模式、原因和机制。其模拟结果与现实世界实验高度吻合，展示了其在捕捉人类行为和社会机制方面的潜力，成为社会科学研究和政策制定的重要工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T17:05:38Z
- **目录日期**: 2025-03-23
