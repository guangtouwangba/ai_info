...
本研究探讨了在视觉与语言导航（VLN）任务中，利用自然语言指令中的子目标视觉表示作为导航线索是否能够提升导航性能。通过文本到图像的扩散模型生成指令中地标参考的视觉想象，并将其作为额外模态提供给VLN代理，同时引入辅助损失以强化这些视觉表示与对应语言表达的联系。实验结果表明，该方法使导航成功率（SR）提升约1个百分点，路径长度加权的成功率（SPL）提升最多0.5个百分点，表明视觉想象的引入增强了视觉理解能力，超越了仅依赖语言指令的效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T07:01:46Z
- **目录日期**: 2025-03-23
