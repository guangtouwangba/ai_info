...
西北大学的研究人员潘震宇和刘晗提出了MetaSpatial框架，通过规则驱动的强化学习策略（Reinforcement Fine-Tuning, RFT）提升视觉语言模型（VLM）在三维空间布局任务中的推理能力。传统方法依赖监督微调（SFT），但受限于空间任务的多样性和缺乏标准答案，泛化能力不足。MetaSpatial通过构建可程序化评估的奖励函数和多轮优化机制，仅需约50条无标注数据，即可显著提升模型的空间推理和布局生成质量。实验表明，MetaSpatial在多个指标上优于传统方法，验证了其在三维空间智能建模中的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T16:04:06Z
- **目录日期**: 2025-03-23
