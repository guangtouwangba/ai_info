...
西北大学的研究团队提出了MetaSpatial框架，通过规则驱动的强化学习策略（RFT）提升视觉语言模型（VLM）在三维空间布局任务中的推理能力。传统方法如监督微调（SFT）因缺乏唯一标准答案而受限，而MetaSpatial通过构建可程序化评估的奖励函数，衡量布局的合理性、物理约束和用户偏好，显著提升了模型的空间推理和布局生成质量。实验表明，MetaSpatial在仅使用50条无标注数据的情况下，优于传统SFT方法，验证了强化学习在三维空间智能建模中的有效性和通用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T09:03:37Z
- **目录日期**: 2025-03-23
