...
UniSync是一种新颖的音频-视频同步评估方法，通过嵌入相似性来处理不同音频（如Mel频谱、HuBERT）和视觉（如RGB图像、面部解析图、面部标志、3DMM）表示的显著维度差异。该方法在对比学习框架中引入了基于边界的损失组件和跨说话者非同步对，提升了判别能力。UniSync在标准数据集上优于现有方法，并在多种音频-视频表示中展现出广泛适用性。其集成到说话人脸生成框架中，显著提高了自然和AI生成内容的同步质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T14:02:26Z
- **目录日期**: 2025-03-23
