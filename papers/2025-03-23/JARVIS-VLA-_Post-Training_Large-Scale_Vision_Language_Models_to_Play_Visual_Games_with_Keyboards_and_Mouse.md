...
最近，开放世界环境中的基于行动的决策引起了广泛关注。预训练于大规模网络数据集的视觉语言行动（VLA）模型在决策任务中显示出潜力。然而，以往的研究主要集中在行动后训练，往往忽视了对基础模型本身的改进。为此，我们提出了一种新方法，通过视觉和语言指导以自监督方式改进视觉语言模型（VLMs），增强了模型在世界知识、视觉识别和空间定位方面的能力。在Minecraft中，我们首次获得了能够执行超过1000种不同原子任务的VLA模型，实验显示后训练在非轨迹任务上比最佳基线提高了40%，超越了传统的模仿学习策略，并开源了代码、模型和数据集。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-23T15:02:15Z
- **目录日期**: 2025-03-23
