...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法无需中间运动表示，直接建模音频-视觉关联，利用Stable Diffusion的强大能力。针对扩散模型时间一致性问题，创新性地提出TREPA（时间表示对齐）方法，通过自监督视频模型增强时序一致性。研究还优化了SyncNet训练框架，解决了其收敛问题，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T11:05:30Z
- **目录日期**: 2025-05-08
