...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂的音频-视觉关系，并针对扩散过程中时间一致性问题提出了时间表示对齐（TREPA）方法，通过自监督视频模型增强生成帧的时间一致性。此外，研究改进了SyncNet的训练框架，解决了其收敛问题，提升了唇同步准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T06:05:25Z
- **目录日期**: 2025-05-08
