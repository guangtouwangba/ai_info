...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI是对带动量随机梯度下降(SGDM)的改进，通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现优异，在LoRA微调中也优于现有优化器。内存效率方面，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，具有超参数鲁棒性和广泛应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T07:04:39Z
- **目录日期**: 2025-05-08
