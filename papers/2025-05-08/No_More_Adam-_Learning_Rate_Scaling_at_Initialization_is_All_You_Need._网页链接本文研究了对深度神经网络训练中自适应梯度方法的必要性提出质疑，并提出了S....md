...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过初始化时根据梯度信号噪声比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，它节省了一半内存，在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，且对超参数变化鲁棒。在GPT-2和Llama2-7B训练中，分别节省了5.93GB和25.15GB内存，并在LoRA微调等任务中优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T21:04:14Z
- **目录日期**: 2025-05-08
