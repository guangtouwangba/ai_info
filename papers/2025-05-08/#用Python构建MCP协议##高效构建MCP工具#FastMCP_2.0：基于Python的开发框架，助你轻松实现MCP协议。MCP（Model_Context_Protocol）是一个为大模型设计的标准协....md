...
FastMCP 2.0是一个基于Python的MCP（Model Context Protocol）开发框架，旨在简化大模型与外部数据和功能的交互协议实现。该框架提供高层接口，支持快速构建MCP服务端和客户端，新增客户端能力、增强服务代理功能（如代理服务器和复合服务）、支持从OpenAPI/FastAPI自动生成模块接口，并优化开发体验（通过装饰器定义服务）。其Pythonic设计（如`@mcp.tool()`装饰器）和全面文档，适合快速原型开发和生产部署。项目开源，文档详见官网。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T11:03:40Z
- **目录日期**: 2025-05-08
