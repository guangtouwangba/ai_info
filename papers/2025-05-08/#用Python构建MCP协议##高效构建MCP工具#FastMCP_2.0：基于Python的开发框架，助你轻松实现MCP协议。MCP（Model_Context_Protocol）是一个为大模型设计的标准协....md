...
FastMCP 2.0是一个基于Python的MCP协议开发框架，旨在简化大模型与外部数据/功能的交互。MCP（Model Context Protocol）是为大模型设计的标准化协议，类似"AI的USB-C接口"。该框架通过Python装饰器等高层抽象，支持快速构建MCP服务端/客户端，新增代理服务、多服务合并、OpenAPI自动转换等功能，显著提升开发效率。其轻量化设计和全面文档使其适合快速原型开发，同时保持生产环境兼容性。典型用法只需添加`@mcp.tool()`装饰器即可将Python函数暴露给大模型调用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T22:03:28Z
- **目录日期**: 2025-05-08
