...
FastMCP 2.0是一个基于Python的MCP（Model Context Protocol）协议开发框架，旨在简化大模型与外部数据/功能的交互。新版本支持客户端开发、服务代理（如合并多个服务或构建代理服务器）、自动从OpenAPI/FastAPI生成模块接口，并通过装饰器实现轻量化开发（如`@mcp.tool()`快速暴露函数给LLM调用）。其Pythonic设计兼顾原型验证与生产部署，提供完整文档和示例，适合快速构建AI工具链。项目开源地址与文档站已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T13:13:34Z
- **目录日期**: 2025-05-08
