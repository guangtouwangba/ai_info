...
本文研究了无限标记线上两个异步唤醒移动代理的确定性问题。代理初始距离为D，节点标签为正整数，最大标签为ℓ_max。Miller和Pelc此前证明了Ω(D log*ℓ_max)的时间下界，并给出了D已知时的最优算法O(D log*ℓ_max)，但D未知时复杂度为O(D²(log*ℓ_max)³)。本文改进了未知D的情况，提出了紧致的O(D log*ℓ_max)算法，实际复杂度可达O(D log*ℓ_min)，其中ℓ_min是起始位置O(D)范围内最小标签。该结果完全匹配了下界，解决了开放问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T06:02:25Z
- **目录日期**: 2025-05-08
