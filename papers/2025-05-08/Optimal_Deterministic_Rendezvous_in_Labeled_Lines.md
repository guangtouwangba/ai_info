...
本文研究了无限标记线上两个异步唤醒的移动代理的确定性问题。代理初始距离为D，节点标签为正整数，最大标签为ℓ_max。Miller和Pelc此前证明了最优时间复杂度下界为Ω(D log* ℓ_max)，并给出了当D已知时的匹配算法，但对D未知情况只得到O(D²(log* ℓ_max)³)的结果。本研究改进了未知D时的复杂度，提出一个紧致的O(D log* ℓ_max)算法，实际可达O(D log* ℓ_min)，其中ℓ_min是起始位置O(D)范围内的最小标签。该结果完全解决了该场景下的最优复杂度问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T15:02:29Z
- **目录日期**: 2025-05-08
