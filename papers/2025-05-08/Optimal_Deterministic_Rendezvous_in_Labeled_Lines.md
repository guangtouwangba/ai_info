...
本文研究了无限标记线上两个异步唤醒的移动代理的确定性和会问题。代理初始距离为D，节点具有唯一正整数标签。Miller和Pelc此前证明了最优时间复杂度为O(D log* ℓ_max)，其中ℓ_max是起始节点的最大标签，但仅当D已知时可达最优；当D未知时，他们算法复杂度为O(D²(log* ℓ_max)³)。本文改进了未知D时的算法，将复杂度降至最优的O(D log* ℓ_max)，甚至可达O(D log* ℓ_min)，其中ℓ_min是起始位置O(D)距离内的最小标签。这一结果完全匹配了下界。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T13:11:59Z
- **目录日期**: 2025-05-08
