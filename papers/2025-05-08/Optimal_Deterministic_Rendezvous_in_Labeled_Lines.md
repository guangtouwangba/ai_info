...
本文研究了无限标记线上两个异步唤醒的移动代理的确定性会合问题。代理初始距离为D，节点具有唯一正整数标记。Miller和Pelc此前证明最优时间复杂度为Θ(D log*ℓ_max)，其中ℓ_max是起始节点的最大标记，并给出了D已知时的最优算法。对于D未知的情况，他们算法复杂度为O(D²(log*ℓ_max)³)。本文改进了这一结果，提出了D未知情况下同样达到最优O(D log*ℓ_max)的算法，实际上可在O(D log*ℓ_min)轮内完成会合，其中ℓ_min是起始位置附近O(D)距离内的最小标记。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T11:02:14Z
- **目录日期**: 2025-05-08
