...
本文研究了无限标记线上两个初始休眠的移动代理的确定性和同步会合问题。代理初始距离为D，唤醒时间差为τ，节点具有唯一正整数标签。Miller和Pelc此前证明了最优复杂度为O(D log* ℓ_max)，其中ℓ_max是起始节点的最大标签，但仅当D已知时可达；当D未知时，他们算法的复杂度为O(D²(log* ℓ_max)³)。本文改进了未知D时的算法，将复杂度降至最优的O(D log* ℓ_max)，实际上可达O(D log* ℓ_min)，其中ℓ_min是起始位置O(D)距离内的最小标签。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T16:02:31Z
- **目录日期**: 2025-05-08
