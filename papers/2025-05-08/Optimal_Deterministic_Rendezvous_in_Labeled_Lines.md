...
本文研究了在无限标记线上两个异步唤醒的移动代理的确定性和解问题。已知初始距离D和最大标签ℓ_max时，最优复杂度为O(D log* ℓ_max)。Miller和Pelc此前在未知D时只能达到O(D²(log* ℓ_max)³)。我们改进了这一结果，提出了一种新算法，在未知D的情况下也能达到最优复杂度O(D log* ℓ_max)。实际上，我们的算法能在O(D log* ℓ_min)轮内完成会合，其中ℓ_min是起始位置O(D)距离内的最小标签。这完全匹配了下界，解决了该问题的开放性问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T21:02:13Z
- **目录日期**: 2025-05-08
