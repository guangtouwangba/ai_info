...
本文研究了在无限标记线上两个异步唤醒的移动代理的确定性问题。代理初始距离为D，节点具有唯一正整数标签，最大标签为ℓ_max。Miller和Pelc此前证明了最优时间复杂度下界为Ω(D log* ℓ_max)，并给出了当D已知时的匹配算法，但对未知D的情况仅获得O(D²(log* ℓ_max)³)的复杂度。本研究改进了未知D时的算法，达到了紧致的O(D log* ℓ_max)复杂度，实际上能在O(D log* ℓ_min)轮内完成汇聚，其中ℓ_min是起始位置O(D)距离内的最小标签。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T19:02:16Z
- **目录日期**: 2025-05-08
