...
本文研究了无限标记线上两个异步睡眠代理的确定性会合问题。代理初始距离为D，节点具有唯一正整数标签，最大标签为ℓ_max。Miller和Pelc此前证明了最优时间复杂度下界为Ω(D log* ℓ_max)，并给出了D已知时的最优算法O(D log* ℓ_max)，但在D未知时只能达到O(D²(log* ℓ_max)³)。本文改进了D未知时的情况，提出了一种时间复杂度为O(D log* ℓ_max)的紧致算法，实际上可在O(D log* ℓ_min)轮内完成会合，其中ℓ_min是起始位置O(D)距离范围内的最小标签。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T04:03:18Z
- **目录日期**: 2025-05-08
