...
本文研究了在无限标记线上两个异步唤醒的移动代理的确定性和解问题。代理初始距离为D，节点标签为正整数，最大标签为ℓ_max。Miller和Pelc此前证明该问题下界为Ω(D log* ℓ_max)，并给出了D已知时的最优算法O(D log* ℓ_max)，但D未知时复杂度为O(D²(log* ℓ_max)³)。本文改进了D未知时的算法，将复杂度降至最优的O(D log* ℓ_max)，实际达到O(D log* ℓ_min)，其中ℓ_min是起始位置O(D)范围内最小标签。该结果完全匹配了下界，解决了此前存在的复杂度差距问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T18:02:17Z
- **目录日期**: 2025-05-08
