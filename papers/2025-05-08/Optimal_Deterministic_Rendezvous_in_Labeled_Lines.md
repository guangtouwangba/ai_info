...
本文研究了无限标记线上两个异步唤醒移动代理的确定性会合问题。代理初始距离为D，节点标签为正整数，最大初始标签为ℓ_max。Miller和Pelc此前证明了最优时间复杂度下界为Ω(D log* ℓ_max)，并给出了D已知时的匹配算法，但D未知时复杂度为O(D²(log* ℓ_max)³)。本文改进了D未知时的情况，提出新算法将会合时间优化至最优的O(D log* ℓ_max)，实际上可达O(D log* ℓ_min)，其中ℓ_min是初始位置O(D)范围内最小标签。该结果完全解决了该设定下的会合问题复杂度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T14:02:40Z
- **目录日期**: 2025-05-08
