...
本文研究了无限标记线上两个异步启动的睡眠智能体的确定性会合问题。在已知初始距离D时，Miller和Pelc已提出最优O(D log* ℓ_max)算法，但未知D时复杂度为O(D²(log* ℓ_max)³)。我们改进了未知D时的算法，将其复杂度降至最优的O(D log* ℓ_max)，实际上达到O(D log* ℓ_min)，其中ℓ_min是起始位置O(D)距离内的最小标签。该结果匹配了已知的Ω(D log* ℓ_max)下界，解决了Miller和Pelc留下的开放问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T07:02:26Z
- **目录日期**: 2025-05-08
