...
本文研究了无限标记线上两个异步唤醒的移动代理的确定性和会问题。代理初始距离为D，节点标签为正整数，最大标签为ℓ_max。Miller和Pelc此前证明了最优时间复杂度下界为Ω(D log* ℓ_max)，并给出了D已知时的最优算法O(D log* ℓ_max)，但D未知时复杂度为O(D²(log* ℓ_max)³)。本文改进了未知D时的情况，提出一个紧致的O(D log* ℓ_max)算法，实际复杂度可达O(D log* ℓ_min)，其中ℓ_min是起始点附近O(D)范围内的最小标签。该结果完全匹配了下界，解决了此前存在的复杂度差距问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T23:02:13Z
- **目录日期**: 2025-05-08
