...
本文研究了无限标记线上两个异步唤醒的移动代理的确定性问题。代理初始距离为D，节点标签为正整数，最大标签为ℓ_max。Miller和Pelc此前证明最优复杂度为Θ(D log* ℓ_max)，但未知D时算法复杂度为O(D²(log* ℓ_max)³)。本文改进了未知D时的算法，将复杂度降至最优的O(D log* ℓ_max)。新算法实际能在O(D log* ℓ_min)轮内完成，其中ℓ_min是起始位置O(D)距离内的最小标签。该结果解决了此前未知D时复杂度不匹配的问题，达到了理论下界。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T03:20:06Z
- **目录日期**: 2025-05-08
