...
本研究对开源文本生成图像（TTI）平台CivitAI进行了社会技术分析，发现该平台存在大量不安全内容（NSFW）和模仿真实人物的模型。基于4000万用户生成图片和23万个模型的数据，研究揭示了互联网亚文化对模型个性化工具和实践的显著影响。从女权主义和建构主义视角，作者探讨了剥削性视觉内容的成因，指出平台设计选择和社区动态的影响。最后，研究建议通过改进内容审核、重新设计工具和制定更明确的平台政策等措施，以减少潜在危害，促进责任和同意原则。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T05:01:23Z
- **目录日期**: 2025-05-08
