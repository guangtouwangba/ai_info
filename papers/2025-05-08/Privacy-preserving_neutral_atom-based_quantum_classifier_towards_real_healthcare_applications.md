...
人工智能和机器学习在医疗领域的应用快速发展，但数据隐私保护成为关键挑战。本研究提出一种基于支持向量机（SVM）的新方法，将其训练过程转化为二次无约束二进制优化（QUBO）问题，并适配中性原子量子处理器（QPU）。该方案无需将敏感数据传输至云端即可完成训练，从根本上避免了隐私泄露风险。通过在公开乳腺癌数据集上的测试，验证了该方法在理想模拟、噪声模拟及真实QPU上的有效性，既保护了数据隐私，又保持了算法性能，为医疗AI的实际应用提供了可行解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T14:02:22Z
- **目录日期**: 2025-05-08
