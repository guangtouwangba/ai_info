...
人工智能和机器学习在医疗领域的应用快速发展，但数据隐私保护成为关键挑战。本研究提出了一种基于支持向量机（SVM）的分类模型，通过将训练过程转化为二次无约束二进制优化（QUBO）问题，并适配中性原子量子处理器（QPU）来实现。该方法无需将敏感数据传输至云端QPU，仅在训练阶段使用量子处理器，从而避免了传统隐私保护技术带来的性能折衷。研究在公开乳腺癌数据集上进行了性能和扩展性分析，包括理想和噪声模拟训练，并成功在现有中性原子QPU上进行了测试，为实际医疗应用提供了可行方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T07:02:16Z
- **目录日期**: 2025-05-08
