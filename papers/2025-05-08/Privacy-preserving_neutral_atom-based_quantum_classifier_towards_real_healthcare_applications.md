...
人工智能和机器学习在医疗领域的应用快速发展，但涉及数据隐私保护的伦理问题日益突出。本文提出了一种基于支持向量机(SVM)的量子计算方法，将训练过程转化为二次无约束二进制优化(QUBO)问题，并适配中性原子量子处理器(QPU)。该方法的创新点在于：训练阶段仅需在QPU上完成，无需将敏感数据传输至云端，从而避免隐私泄露风险。研究使用公开乳腺癌数据集进行测试，通过理想和噪声模拟验证了性能，并成功在真实中性原子QPU上运行。这种方法在保证算法性能的同时，实现了数据隐私的天然保护。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T13:11:44Z
- **目录日期**: 2025-05-08
