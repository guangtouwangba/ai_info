...
人工智能和机器学习在医疗领域的应用迅速发展，但数据隐私保护成为关键挑战。本研究提出了一种基于支持向量机(SVM)的量子计算方法，将训练过程转化为二次无约束二进制优化(QUBO)问题，并在中性原子量子处理器(QPU)上实现。该方法无需将敏感数据传输至云端即可完成训练，从根本上避免了隐私泄露风险。研究使用公开乳腺癌数据集进行测试，通过理想和噪声模拟验证了性能，并成功在实际量子处理器上运行。这种方案在保证算法性能的同时，解决了医疗AI领域的数据隐私保护难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T05:02:07Z
- **目录日期**: 2025-05-08
