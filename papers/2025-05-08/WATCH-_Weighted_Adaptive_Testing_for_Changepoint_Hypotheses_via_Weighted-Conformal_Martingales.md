...
本文提出了一种加权广义共形测试鞅（WCTM）方法，用于人工智能/机器学习系统部署后的持续监控。该方法扩展了现有非参数变点检测技术的适用范围，能在线监测数据分布中的异常变化并控制误报。具体算法可适应轻度协变量偏移，同时针对严重偏移（如概念漂移或极端协变量偏移）发出警报。相比现有方法，WCTM突破了有限假设类别的限制，支持在线自适应调整。实验表明，该方法在真实数据集上优于当前最优基线，为高风险场景下的AI系统安全监控提供了新解决方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T20:01:16Z
- **目录日期**: 2025-05-08
