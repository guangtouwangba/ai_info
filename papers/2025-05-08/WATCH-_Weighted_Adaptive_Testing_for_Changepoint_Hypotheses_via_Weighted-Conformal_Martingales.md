...
本文提出了一种加权泛化共形测试鞅(WCTM)方法，用于AI/ML系统部署后的持续监控。该方法突破了现有非参数变点检测方法的局限，能够在线监测数据分布中的任何异常变化并控制误报。针对实际应用，研究者设计了特定WCTM算法，可适应轻度协变量偏移，同时对严重偏移(如概念漂移或极端协变量偏移)发出警报。相比现有最优方法，该方法在真实数据集上表现出更好的性能，为高风险AI系统提供了更可靠的持续监控方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T05:01:10Z
- **目录日期**: 2025-05-08
