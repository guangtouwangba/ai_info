...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)构建CodeArena基准测试(397个样本/40类/44种语言)，首次系统评估模型输出与人类编程偏好的匹配度；2)开发20B标记量的SynCode-Instruct合成指令集验证微调效果。研究发现：传统执行测试与人类偏好存在显著差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显，凸显对齐人类偏好的重要性。研究为提升代码生成模型的实际可用性提供了新评估维度和数据支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T01:33:42Z
- **目录日期**: 2025-05-08
