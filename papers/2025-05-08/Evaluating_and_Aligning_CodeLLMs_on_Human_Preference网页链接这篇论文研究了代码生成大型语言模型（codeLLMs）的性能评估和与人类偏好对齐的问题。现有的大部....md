...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，以评估模型在真实编程任务中的表现。同时构建了SynCode-Instruct合成指令数据集(约20B标记)验证大规模微调效果。实验发现，传统执行基准与CodeArena存在性能差异，且开源顶级模型(如Qwen2.5-Coder)与专有模型(如OpenAI o1)差距显著，凸显了人类偏好对齐的重要性。研究为提升代码LLMs的实际应用价值提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T10:06:00Z
- **目录日期**: 2025-05-08
