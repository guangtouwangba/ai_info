...
这篇论文针对代码生成大模型（CodeLLMs）提出新评估方法，重点关注与人类偏好的对齐。研究者创建了CodeArena基准测试（含397个多语言样本）和SynCode-Instruct合成指令集（20B标记），发现传统执行测试与人类偏好存在差异。实验对比40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显对齐人类偏好的重要性。研究通过模拟真实编程场景，为代码生成模型的评估提供了更全面的框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T22:05:22Z
- **目录日期**: 2025-05-08
