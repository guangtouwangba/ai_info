...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个智能代理的社会互动（500万次交互）。该系统通过四个社会实验（政治极化、信息传播、基本收入政策、自然灾害影响）验证其有效性，结果与现实数据高度吻合。研究证明该平台能有效捕捉人类行为机制，为社会科学研究和政策制定提供新工具，补充传统研究方法（如调查、访谈）。其核心价值在于通过LLM驱动的代理模拟，揭示社会问题的成因与模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T19:04:42Z
- **目录日期**: 2025-05-08
