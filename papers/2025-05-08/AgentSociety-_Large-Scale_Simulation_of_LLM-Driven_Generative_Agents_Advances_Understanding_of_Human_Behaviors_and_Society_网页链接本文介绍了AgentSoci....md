...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它研究了社会极化、信息传播、基本收入政策及自然灾害影响等社会问题。模拟结果与现实数据吻合，验证了该工具在揭示人类行为机制方面的有效性，展示了其作为社会科学研究和政策制定辅助工具的潜力。该系统整合了LLM代理、真实社会环境和大规模模拟引擎，为传统研究方法（如调查和干预）提供了补充支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-08T12:06:04Z
- **目录日期**: 2025-05-08
