...
当前大语言模型（LLM）偏好对齐方法主要关注相对偏好，却忽略了响应质量。这可能导致模型过度拟合，错误地偏好低质量响应，而无法泛化到最优响应。本研究提出了一种奖励条件化的LLM策略，通过分析数据集中的整体响应质量来提升性能。方法包括基于质量分数重新标记偏好对，构建奖励增强数据集。实验表明，该方法显著提升了DPO（直接偏好优化）的效果，不仅最大化偏好数据的效用，还缓解了"遗忘"问题。研究证明了该方法在数据扩展之外的广泛有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T08:02:01Z
- **目录日期**: 2025-05-02
