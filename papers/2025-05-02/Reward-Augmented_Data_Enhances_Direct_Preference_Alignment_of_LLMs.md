...
当前大语言模型（LLM）偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误学习。针对这一问题，本研究提出了一种奖励条件化的LLM策略，通过分析数据集中全部响应质量来优化学习效果。研究者开发了简单有效的数据重标记方法，将偏好对与质量分数结合构建奖励增强数据集。实验表明，该方法显著提升了DPO（直接偏好优化）性能，不仅充分利用了偏好数据，还缓解了错误学习问题，其有效性超越了单纯的数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T17:02:14Z
- **目录日期**: 2025-05-02
