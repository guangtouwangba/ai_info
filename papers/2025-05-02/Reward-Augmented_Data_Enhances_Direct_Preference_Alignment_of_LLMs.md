...
该研究提出了一种奖励条件化的大型语言模型(LLM)策略，通过利用偏好数据中的奖励分数来改进对齐方法。现有直接对齐算法往往忽视响应质量，仅关注相对偏好，导致过度拟合或错误遗忘高质量响应。新方法采用数据重标记技术，将偏好对与质量分数关联，构建奖励增强数据集。实验表明，该方法显著提升了DPO(直接偏好优化)性能，不仅最大化偏好数据的效用，还缓解了遗忘问题。研究证明该方法能有效学习数据中的完整响应质量谱系，推广到更优的稀疏响应区域，具有广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T19:02:01Z
- **目录日期**: 2025-05-02
