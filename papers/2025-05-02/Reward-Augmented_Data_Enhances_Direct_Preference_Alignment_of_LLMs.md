...
现有大型语言模型（LLM）偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误遗忘高质量被拒回答。针对这一问题，本研究提出奖励条件化的LLM策略，通过利用数据集中的完整质量谱来学习，并设计了一种简单有效的数据重标注方法，将偏好对与质量分数结合构建奖励增强数据集。实验表明，该方法显著提升了直接偏好优化（DPO）的性能，不仅最大化偏好数据的效用，还缓解了遗忘问题，其效果不仅源于数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T15:02:14Z
- **目录日期**: 2025-05-02
