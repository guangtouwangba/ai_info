...
现有大型语言模型（LLM）偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误舍弃高质量被拒回答。针对这一问题，本研究提出基于奖励条件的LLM策略，通过分析数据集中的整体响应质量来优化学习。方法包括对偏好数据进行质量评分重新标记，构建奖励增强数据集。实验表明，该方法显著提升了DPO（Direct Preference Optimization）性能，不仅最大化偏好数据效用，还缓解了"遗忘"问题。研究证明其有效性不仅源于数据扩展，更来自质量感知的学习机制。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T22:02:09Z
- **目录日期**: 2025-05-02
