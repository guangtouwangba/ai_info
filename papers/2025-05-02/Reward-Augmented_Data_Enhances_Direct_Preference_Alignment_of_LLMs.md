...
当前大语言模型（LLM）偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误舍弃高质量被拒响应。本研究提出一种奖励条件化的LLM策略，通过分析数据集中全部响应质量来学习，并设计了一种简单有效的数据重标注方法，将偏好对与质量评分结合构建奖励增强数据集。实验表明，该方法显著提升了DPO（直接偏好优化）效果，不仅最大化偏好数据的效用，还缓解了"遗忘"问题，其有效性不仅源于数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T11:02:04Z
- **目录日期**: 2025-05-02
