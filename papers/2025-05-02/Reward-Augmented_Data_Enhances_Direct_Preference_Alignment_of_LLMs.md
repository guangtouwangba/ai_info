...
该研究针对大型语言模型（LLM）偏好对齐中忽视响应质量的问题，提出了一种基于奖励条件的LLM策略。传统方法仅关注相对偏好，可能导致对高质量拒绝响应的不必要遗忘或过度拟合低质量选择响应。新方法通过数据重标注，将偏好对与质量分数结合构建奖励增强数据集，使模型能学习响应质量的整体分布并外推至更优区域。实验表明，该方法显著提升了DPO性能，有效缓解遗忘问题，最大化偏好数据的效用，且效果不依赖于数据量的单纯增加。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T14:02:11Z
- **目录日期**: 2025-05-02
