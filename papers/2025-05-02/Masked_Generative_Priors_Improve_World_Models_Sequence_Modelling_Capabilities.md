...
深度强化学习（RL）已成为复杂环境中构建智能体的主流方法。基于模型的方法（如世界模型）通过预测环境动态提升数据效率，是连接研究与实际应用的关键。本文提出GIT-STORM，用掩码生成先验（如MaskGIT）替代传统MLP先验，在Atari 100k基准测试中显著提升RL任务性能。首次将基于Transformer的世界模型应用于连续动作环境，通过状态混合函数整合潜在状态与动作，在DeepMind Control Suite中验证了有效性。结果表明，掩码生成先验能构建更精确的世界模型和高效RL策略，拓展了该技术的应用领域。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T01:29:14Z
- **目录日期**: 2025-05-02
