...
DeepSeek发布新数学模型DeepSeek-Prover-V2，在普特南测试中创下49道题的新纪录，远超当前冠军（10道）。值得注意的是，7B参数的小模型通过强化学习发现了671B大模型未掌握的数学推理技能，成功解决了13个大模型无法处理的问题。研究发现，小模型在处理有限基数问题时采用了独特方法（如Cardinal.toNat），而大模型输出中未见此类策略。该模型基于V1.5-Base微调后通过强化学习训练，展现了"小模型发现大模型盲点"的突破性现象。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T15:03:29Z
- **目录日期**: 2025-05-02
