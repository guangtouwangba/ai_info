...
本文提出了一种名为FPoTT的新型框架，用于解决车联网（IoV）中的实时、可扩展和隐私保护的交通管理挑战。FPoTT利用开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性。该框架采用双层联邦学习架构，结合轻量级边缘模型和云端LLM，同时利用基于Transformer的合成数据生成器增强训练。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，展现了开源LLM在车联网管理中的潜力，为智能交通系统提供了高效、安全的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T11:01:36Z
- **目录日期**: 2025-05-02
