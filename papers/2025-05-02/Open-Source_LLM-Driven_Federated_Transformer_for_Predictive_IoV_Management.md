...
本文提出了一种名为FPoTT的新型框架，用于解决车联网（IoV）中可扩展、实时且保护隐私的交通管理挑战。FPoTT利用开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测精度。该框架采用双层联邦学习架构，结合轻量级边缘模型和云端LLM，同时引入基于Transformer的合成数据生成器增强训练。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，展现了开源LLM在实现安全、自适应和可扩展车联网管理方面的潜力，为智能交通生态系统提供了优于专有方案的替代选择。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T15:01:40Z
- **目录日期**: 2025-05-02
