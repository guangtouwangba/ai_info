...
本文提出了一种新型框架FPoTT（联邦优化提示交通Transformer），用于解决车联网（IoV）中实时、可扩展且保护隐私的交通管理问题。FPoTT结合开源大语言模型（LLM）和双层联邦学习，通过动态提示优化机制提升轨迹预测精度。其架构包含轻量级边缘模型（实时推理）和云端LLM（全局智能），并利用Transformer生成合成数据增强训练。测试显示，FPoTT在真实数据中预测准确率达99.86%，同时在合成数据中表现优异。该框架为智能交通系统提供了安全、自适应且可扩展的解决方案，有望替代现有专有AI模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T07:01:36Z
- **目录日期**: 2025-05-02
