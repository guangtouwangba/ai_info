...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：1) 主体对环境动态及观测的控制响应内部表征；2) 主体对可观测结果的偏好。模型构建符合认知科学中的主动推理理论，即主体通过最小化"意外"（当前感知与偏好感知的差异）来采取行动。作者采用双层优化方法进行估计，并通过对环境模型参数设置先验分布的结构性假设。以跟车行为为例，基于自然驾驶数据的建模结果表明，这种主动推理模型比黑箱模型更能有效表征人类驾驶行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T22:02:25Z
- **目录日期**: 2025-05-02
