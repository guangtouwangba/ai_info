...
本文提出了一种基于有限演示数据学习人类感知与控制模型的通用估计方法。该模型结构包括：1) 代理对环境动态的内部表征；2) 代理对可观测结果的偏好。模型符合认知科学中的主动推理理论，认为人类行为旨在最小化"惊讶"（当前感知与偏好感知的差异）。作者采用双层优化方法进行估计，并通过先验分布的结构假设参数化人类代理的环境模型准确性。以跟车行为为例，基于自然数据集验证了该方法。结果表明，从数据中学习主动推理模型是驾驶行为黑箱模型的有前景的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T10:02:24Z
- **目录日期**: 2025-05-02
