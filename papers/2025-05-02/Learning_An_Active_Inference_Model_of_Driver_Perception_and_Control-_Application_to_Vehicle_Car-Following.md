...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型包含两个核心部分：(1) 智能体对环境动态及控制行为影响的内在表征，(2) 智能体对可观测结果的偏好。模型结构符合认知科学中的主动推理理论，即智能体通过最小化"意外"（实际观察与偏好观察的差异）来采取行动。研究者采用双层优化方法进行估计，并通过先验分布的结构假设来参数化智能体对环境模型的统计准确性。以跟车行为为例的自然数据集验证表明，该方法比黑箱模型更能有效建模人类驾驶行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T12:02:35Z
- **目录日期**: 2025-05-02
