...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含两部分：i) 智能体对环境动态及控制行为影响的内部表征；ii) 对可观测结果的偏好。研究采用与主动推理理论一致的模型框架，该认知科学理论认为人类行为旨在最小化"意外"（当前感知与偏好感知的差异）。作者提出双层优化估计方法，通过对先验分布的结构性假设来参数化人类环境模型的统计准确性。以跟车行为为例，基于自然驾驶数据的实验表明，这种主动推理模型相比黑箱模型更具潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T18:02:27Z
- **目录日期**: 2025-05-02
