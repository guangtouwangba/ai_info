...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包括：1) 主体对环境动态和控制行为影响的内在表征；2) 主体对可观测结果的偏好。研究采用与主动推断理论一致的模型框架，该理论认为人类行为旨在最小化"惊奇度"（当前感知与偏好感知的差异）。作者提出了一种双层优化估计方法，基于先验分布的结构性假设。通过自然驾驶数据中的跟车行为建模验证，结果表明从数据中学习主动推断模型是比黑箱模型更有前景的驾驶行为建模方法。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T06:03:05Z
- **目录日期**: 2025-05-02
