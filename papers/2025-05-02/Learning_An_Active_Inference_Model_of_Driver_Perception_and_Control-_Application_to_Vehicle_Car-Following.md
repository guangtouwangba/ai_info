...
本文提出了一种基于有限演示数据学习人类感知与控制模型的通用估计方法。该模型结构包含：1) 代理对环境动态及控制行为影响的内在表征；2) 代理对可观测结果的偏好。模型符合认知科学中的主动推理理论，认为人类行为旨在最小化"意外"（当前感知与偏好的差异）。作者采用双层优化方法进行估计，基于先验分布的结构性假设。通过自然驾驶数据中的跟车行为建模验证，结果表明相比黑箱模型，这种主动推理模型能更有效地学习人类感知控制机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T08:02:23Z
- **目录日期**: 2025-05-02
