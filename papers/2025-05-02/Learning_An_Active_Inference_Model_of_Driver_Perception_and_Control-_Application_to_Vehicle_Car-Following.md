...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：1) 主体对环境动态及控制行为影响的内部表征；2) 主体对可观测结果的偏好。模型构建遵循认知科学中的主动推理理论，认为主体行为旨在最小化"意外"（当前感知与偏好感知的差异）。作者采用双层优化方法进行估计，并通过对先验分布的结构化假设来参数化主体环境模型的统计准确性。以跟车行为为例，基于自然驾驶数据的实验表明，这种主动推理模型比黑箱模型更具潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T04:02:17Z
- **目录日期**: 2025-05-02
