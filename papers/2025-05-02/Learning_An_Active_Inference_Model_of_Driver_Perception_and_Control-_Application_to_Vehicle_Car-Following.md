...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：i) 智能体对环境状态演变的内部表征；ii) 对可观测结果的偏好。模型符合认知科学中的主动推理理论，即智能体通过最小化"意外"（实际观测与偏好观测的差异）来行动。作者采用双层优化方法进行估计，依赖于先验分布的结构假设。通过自然驾驶数据中的跟车行为建模验证，结果表明这种主动推理模型比黑箱模型更具潜力，为人类感知控制建模提供了新思路。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T19:02:19Z
- **目录日期**: 2025-05-02
