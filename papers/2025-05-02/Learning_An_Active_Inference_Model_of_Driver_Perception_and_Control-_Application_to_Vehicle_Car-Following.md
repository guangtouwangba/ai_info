...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的方法。该模型包含两个核心部分：1) 个体对环境动态及控制行为影响的内部表征；2) 个体对可观测结果的偏好。研究采用主动推理理论框架，认为人类行为旨在最小化"意外"（实际感知与偏好感知的差异）。作者提出双层优化估计方法，通过对先验分布的结构化假设来参数化个体环境模型的统计准确性。以跟车行为为例，基于自然驾驶数据的建模结果表明，这种主动推理模型比黑箱模型更具潜力，为人类感知控制建模提供了新思路。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T16:02:38Z
- **目录日期**: 2025-05-02
