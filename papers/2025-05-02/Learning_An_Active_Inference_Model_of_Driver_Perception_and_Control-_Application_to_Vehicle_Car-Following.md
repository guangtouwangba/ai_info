...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：i) 主体对环境及观测随控制动作演变的内部表征；ii) 主体对可观测结果的偏好。模型符合认知科学中的主动推理理论，即主体通过最小化当前感知与偏好感知间的"意外"来行动。作者采用双层优化方法进行估计，并假设先验分布参数化主体对环境模型的统计准确性。通过自然驾驶数据估计的跟车行为模型表明，主动推理模型是传统黑箱驾驶模型的有前景的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T13:09:20Z
- **目录日期**: 2025-05-02
