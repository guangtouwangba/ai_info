...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含两部分：(1) 主体对环境及观测随控制行为演变的内部表征；(2) 主体对可观测结果的偏好。模型构建符合认知科学中的主动推理理论，认为主体通过最小化"惊讶度"（当前感知与偏好感知的差异）来行动。作者采用双层优化方法进行估计，并基于先验分布的结构假设。以跟车行为为例，使用自然数据集验证表明，这种主动推理模型比黑箱模型更能有效表征人类驾驶行为，为相关研究提供了新思路。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T17:02:32Z
- **目录日期**: 2025-05-02
