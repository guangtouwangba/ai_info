...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。团队发现短音频转写效果良好，但长音频面临token限制、输出质量下降等问题。通过将音频分段处理（30秒重叠）并借助LLM合并，解决了时间戳精度和内容连贯性问题。最终方案采用分段转写后重组，实现了1小时音频仅5-10秒时间戳偏移的高精度转写。虽然仍需循环调用LLM，但分段方法显著提升了可靠性和准确性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T04:03:18Z
- **目录日期**: 2025-05-02
