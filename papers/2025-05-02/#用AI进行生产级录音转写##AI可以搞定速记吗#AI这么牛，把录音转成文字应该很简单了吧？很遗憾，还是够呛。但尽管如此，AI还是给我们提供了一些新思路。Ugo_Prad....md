...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。团队发现直接处理长音频会因Token限制导致输出质量下降，转而采用分段处理方案：将音频切割为重叠片段分别转写，再用LLM合并重叠部分。30秒重叠时长效果最佳，使1小时转写的时间戳误差控制在5-10秒。最终通过算法智能拼接主体内容与合并段，实现了可靠的大规模音频转写。该方案有效解决了长文本处理的精度衰减问题，但需多次调用LLM进行分段合并。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T09:04:06Z
- **目录日期**: 2025-05-02
