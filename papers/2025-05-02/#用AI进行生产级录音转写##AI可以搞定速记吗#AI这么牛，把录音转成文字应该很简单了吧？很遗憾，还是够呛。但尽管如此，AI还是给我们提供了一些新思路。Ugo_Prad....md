...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。短音频转写效果理想，但长文件处理遇到Token限制、输出质量下降等问题。团队采用分段转写方案，通过30秒重叠切割和LLM合并，解决了时间戳偏移问题。最终方案将10分钟音频分为三部分，单独合并重叠段后再重组，显著提升了转写质量和可靠性。虽然仍需循环调用LLM，但分段方法使1小时转写的时间戳误差控制在5-10秒内，实现了可扩展的准确转录流程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T01:30:43Z
- **目录日期**: 2025-05-02
