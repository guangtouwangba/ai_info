...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。短音频转写效果理想，但长音频面临token限制、输出质量下降等问题。团队采用分段处理方案，通过30秒重叠切割确保内容连贯性，并用LLM合并分段文本。最终方案将音频按10分钟分段，单独合并重叠部分后重组，显著提升了时间戳精度（1小时音频误差仅5-10秒），同时避免了循环调用导致的错误。这种分段转写与智能合并的方法，在保证质量的前提下实现了长音频的可靠转写。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T08:03:40Z
- **目录日期**: 2025-05-02
