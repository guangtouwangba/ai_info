...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。该方法改进带动量SGD，在初始化时根据梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将内存占用减半。实验显示，SGD-SaI在Transformer任务中媲美或超越AdamW，成功解决了用SGD训练Transformer的难题，在ImageNet分类和GPT-2预训练中表现优异，且超参数鲁棒性强。内存效率方面，相比AdamW为GPT-2节省5.93GB，为Llama2-7B节省25.15GB，在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T14:04:37Z
- **目录日期**: 2025-05-02
