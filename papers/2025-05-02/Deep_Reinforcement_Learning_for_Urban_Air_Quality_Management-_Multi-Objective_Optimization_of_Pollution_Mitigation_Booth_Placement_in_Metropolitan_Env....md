...
德里是全球空气污染最严重的城市之一，主要污染源包括交通排放、工业活动和建筑扬尘。传统静态空气净化装置因位置不佳和适应性差而效果有限。本研究提出一种新型深度强化学习（DRL）框架，利用近端策略优化（PPO）算法，基于人口密度、交通模式等动态因素优化空气净化亭的布局。相比随机和贪婪式基准方法，DRL方案在空气质量指数（AQI）改善、空间覆盖率和公平性等指标上表现更优，实现了净化效率与覆盖范围的平衡。结果表明，AI驱动的空间优化可有效支持智慧城市建设和数据驱动的空气质量治理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T21:01:33Z
- **目录日期**: 2025-05-02
