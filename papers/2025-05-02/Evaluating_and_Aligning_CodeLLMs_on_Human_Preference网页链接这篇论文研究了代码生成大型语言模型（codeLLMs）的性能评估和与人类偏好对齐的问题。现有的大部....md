...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究收集了397个涵盖40类任务和44种编程语言的高质量样本，并构建近200亿标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行基准与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调，除代码正确性外，模型输出符合开发者习惯同样重要，为代码LLM评估提供了新维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T16:05:56Z
- **目录日期**: 2025-05-02
