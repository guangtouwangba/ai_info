...
这篇论文针对代码生成大模型(codeLLMs)提出新评估方法，指出当前基准测试过于侧重代码正确性而忽视人类偏好。研究者创建了CodeArena基准(包含397个高质量样本，覆盖40类任务和44种语言)和SynCode-Instruct合成指令数据集(近200亿标记)。实验发现执行基准与人类偏好评估存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著，凸显对齐人类偏好的重要性。研究验证了大规模合成指令微调的有效性，为提升代码LLMs的实用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T01:32:38Z
- **目录日期**: 2025-05-02
