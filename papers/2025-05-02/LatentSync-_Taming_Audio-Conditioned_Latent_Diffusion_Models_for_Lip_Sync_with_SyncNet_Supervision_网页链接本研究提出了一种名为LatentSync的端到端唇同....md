...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法，利用自监督视频模型增强帧间对齐。同时改进了SyncNet训练框架，解决了收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)直接建模音视频关联的扩散模型；2)时间一致性增强技术；3)SyncNet训练优化方案。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T10:05:22Z
- **目录日期**: 2025-05-02
