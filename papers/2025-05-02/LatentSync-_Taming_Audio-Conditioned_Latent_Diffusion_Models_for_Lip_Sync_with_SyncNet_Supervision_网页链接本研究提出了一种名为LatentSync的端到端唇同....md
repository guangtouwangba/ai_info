...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模音频-视觉关系，无需中间运动表示。为解决扩散过程中时间一致性问题，提出TREPA方法，利用自监督视频模型对齐生成帧与真实帧。同时优化SyncNet训练框架，提升其准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。创新点包括：1)端到端潜在扩散模型；2)TREPA增强时间一致性；3)SyncNet训练优化。这些改进可推广至其他音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T15:05:32Z
- **目录日期**: 2025-05-02
