...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本高的问题。NSA通过算法与硬件协同优化，采用动态分层稀疏策略：先粗粒度压缩令牌，再细粒度选择关键令牌，在保持全局上下文感知的同时确保局部精度。这一设计实现了高效的长文本建模，其两大核心创新点在于硬件对齐的训练友好架构，使模型既能降低计算复杂度，又能维持与传统注意力相当的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-02T18:04:44Z
- **目录日期**: 2025-05-02
