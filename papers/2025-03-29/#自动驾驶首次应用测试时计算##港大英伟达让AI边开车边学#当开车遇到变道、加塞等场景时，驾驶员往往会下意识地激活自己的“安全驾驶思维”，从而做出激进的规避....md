...
香港大学、英伟达和图宾根大学联合团队提出Centaur方法，通过测试时计算（test-time training）让自动驾驶AI在行驶过程中动态学习。该方法利用不确定性聚类熵，使AI能像人类驾驶员一样实时调整驾驶策略，减少传统自动驾驶因依赖预设规则导致的过度保守行为（如无故急刹）。这种"边开边学"的在线学习模式显著提升了AI在变道、加塞等不确定性场景中的适应性和安全性，突破了固定规则系统的局限性。相关技术首次实现了自动驾驶系统在真实测试环境中的持续优化能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T23:02:51Z
- **目录日期**: 2025-03-29
