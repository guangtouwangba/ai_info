...
香港大学、英伟达与图宾根大学联合团队提出Centaur方法，通过测试时动态计算（test-time training）让自动驾驶AI在行驶过程中实时优化决策。该方法利用集群熵和不确定性评估，突破传统预设规则的局限，使车辆在变道、加塞等复杂场景中不再过度保守（如无故急刹），显著提升应对不确定性的能力。研究首次实现AI"边开车边学习"，模仿人类驾驶员的安全思维模式，减少因过度避让引发的二次风险，推动自动驾驶适应性与安全性升级。（99字）  

核心突破：  
1. 动态学习机制替代静态规则  
2. 集群熵+不确定性量化实现实时策略优化  
3. 解决"过度防御性驾驶"痛点
