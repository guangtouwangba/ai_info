...
香港大学、英伟达与图宾根大学联合团队提出Centaur方法，通过测试时计算（test-time training）技术让自动驾驶系统在行驶过程中动态学习。该方法利用不确定性评估和聚类熵，使AI能像人类驾驶员一样在变道、加塞等复杂场景中实时优化决策，减少过度保守行为（如无故急刹）。相比依赖预设规则的传统系统，这种"边开边学"的在线学习机制显著提升了自动驾驶在不确定性环境中的适应性和安全性。相关成果标志着自动驾驶领域首次实现测试时计算的实用化突破。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T20:02:40Z
- **目录日期**: 2025-03-29
