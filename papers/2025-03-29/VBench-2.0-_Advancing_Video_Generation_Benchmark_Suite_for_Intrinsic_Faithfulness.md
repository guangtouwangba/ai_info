...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评测基准（如VBench）主要评估表面真实性（如单帧美学、时序一致性和基础提示匹配），但缺乏对内在真实性的考量（如物理法则、常识推理等）。为此，研究者提出VBench-2.0，通过自动评估五个核心维度（人类拟真度、可控性、创造性、物理合理性和常识性）来推动视频生成模型向内在真实性迈进。该框架结合通用视觉语言模型和专用异常检测方法，并通过人工标注验证，旨在为下一代视频生成模型设定新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T21:01:02Z
- **目录日期**: 2025-03-29
