...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表层真实性（如帧美学、时序一致性和基础提示遵循），但忽略了更深层的真实世界原则（如物理定律、常识推理等）。为此，研究团队推出VBench-2.0，通过自动评估模型在人类逼真度、可控性、创造性、物理合理性和常识性五个维度的表现，推动视频生成向"内在真实性"迈进。该框架结合通用视觉语言模型和专用异常检测方法，并通过人工标注验证，旨在为下一代视频生成模型设定新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T02:26:14Z
- **目录日期**: 2025-03-29
