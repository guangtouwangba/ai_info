...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表层真实性（如帧美学、时序一致性和基础提示遵循），但忽略了内在真实性（如物理法则、常识推理和结构完整性）。为此，VBench-2.0提出新一代评估框架，通过五大维度（人类拟真度、可控性、创造性、物理合理性和常识性）自动评测视频生成模型的内在真实性。该框架结合通用视觉语言模型、专家异常检测方法及人工标注，旨在为下一代视频生成模型设定内在真实性的新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T15:01:22Z
- **目录日期**: 2025-03-29
