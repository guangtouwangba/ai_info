...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评测基准（如VBench）主要评估表面忠实度，包括帧美学、时序一致性和基本提示遵循，但忽略了深层的现实世界原则。为实现真正的"世界模型"，视频需符合物理定律、常识推理、解剖正确性和结构完整性。为此，研究团队推出VBench-2.0，通过自动评估五个关键维度（人类拟真度、可控性、创造性、物理性和常识性）来检测内在忠实度。该框架结合通用视觉语言模型和专用异常检测方法，并经过人工标注验证，旨在为下一代视频生成模型设立新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T11:01:03Z
- **目录日期**: 2025-03-29
