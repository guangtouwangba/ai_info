...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评测基准（如VBench）主要评估表面真实性，如帧美学、时序一致性和基础提示遵循度，但忽略了内在真实性（如物理规律、常识推理等）。为此，VBench-2.0被提出，通过自动化评估模型在五个关键维度（人类逼真度、可控性、创造性、物理合理性和常识性）的表现，结合通用视觉语言模型和专用异常检测方法，并辅以人工标注验证。该基准旨在推动视频生成模型超越表面真实性，实现内在真实性，为AI辅助影视和虚拟世界建模奠定基础。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T00:01:20Z
- **目录日期**: 2025-03-29
