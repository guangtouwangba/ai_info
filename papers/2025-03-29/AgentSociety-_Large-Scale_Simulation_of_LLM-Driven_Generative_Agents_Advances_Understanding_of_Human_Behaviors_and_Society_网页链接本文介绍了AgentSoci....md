...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它分析四大社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响及飓风等外部冲击。模拟结果与现实数据吻合，验证了该系统捕捉人类行为机制的能力，展现了作为社会科学研究和政策制定工具的潜力。该平台能辅助传统研究方法（如调查、访谈），帮助揭示社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T18:04:33Z
- **目录日期**: 2025-03-29
