...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟超过1万个AI代理及其500万次社会互动。该系统通过四个关键社会议题（社会极化、信息传播、基本收入政策、自然灾害影响）的模拟实验，验证了其模拟结果与现实世界数据的高度一致性。研究表明，这种LLM驱动的社会模拟能有效捕捉人类行为机制，为社会科学研究和政策制定提供了新型计算实验平台，可辅助传统研究方法（如调查、访谈）深入分析社会问题的成因和模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T01:31:41Z
- **目录日期**: 2025-03-29
