...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个智能代理及其500万次社会互动。研究者利用该平台对四大社会问题（社会极化、信息传播、基本收入政策、自然灾害影响）进行实验分析，结果与现实世界数据高度吻合。该系统通过整合LLM驱动的智能代理、真实社会环境和强大模拟引擎，为社会科学研究和政策制定提供了新型计算实验工具，能够有效揭示人类行为模式和社会机制。研究表明AgentSociety可作为传统研究方法的补充，具有重要的学术和应用价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T00:04:48Z
- **目录日期**: 2025-03-29
