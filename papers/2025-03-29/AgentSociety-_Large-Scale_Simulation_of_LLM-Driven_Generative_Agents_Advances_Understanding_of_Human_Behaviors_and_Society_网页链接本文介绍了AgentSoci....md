...
该研究提出了AgentSociety——一个结合大语言模型代理、真实社会环境和大规模模拟引擎的社会模拟平台。通过模拟1万多个代理及其500万次互动，研究者针对四大社会问题（社会极化、信息传播、基本收入政策、自然灾害影响）进行了计算社会科学实验。结果显示模拟结果与现实世界数据高度吻合，证明该系统能有效捕捉人类行为机制，具备作为社会科学研究和政策制定工具的潜力。该平台为传统研究方法提供了补充，有助于深入理解复杂社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T07:04:54Z
- **目录日期**: 2025-03-29
