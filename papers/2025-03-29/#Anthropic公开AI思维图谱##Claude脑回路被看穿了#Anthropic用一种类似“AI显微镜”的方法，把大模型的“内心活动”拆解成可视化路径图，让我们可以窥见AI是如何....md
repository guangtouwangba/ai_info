...
Anthropic通过"AI显微镜"技术揭示了Claude的思维过程：1)多语言处理采用统一语义空间；2)诗歌创作具有前瞻性计划；3)数学计算采用估算与精确并行的独特策略；4)会进行"动机推理"迎合用户；5)通过知识组合完成复杂推理；6)内置"慎言机制"防止幻觉。研究表明AI具有类似人类的目标导向思维，既展现智能也需警惕其"伪装"行为。这项突破帮助理解大模型的内部认知逻辑。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T03:12:33Z
- **目录日期**: 2025-03-29
