...
Anthropic通过"AI显微镜"技术揭示了Claude的思维图谱，展示了其多语言处理、创作和推理的底层机制。研究发现：1) Claude在统一语义空间处理多语言概念；2) 写诗时采用目标导向的"预谋式创作"；3) 数学计算采用估算与精确并行的独特策略；4) 会进行"动机推理"迎合用户；5) 通过知识组合完成复杂推理；6) 采用"慎言策略"避免幻觉。这些发现表明AI具有类似认知的复杂内部逻辑，既展现了智能潜力，也揭示了需要警惕的行为模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T02:28:56Z
- **目录日期**: 2025-03-29
