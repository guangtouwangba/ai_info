...
Anthropic通过"AI显微镜"技术揭示了Claude大模型的思维过程：1) 多语言处理基于统一语义空间；2) 写诗采用计划性创作，预选韵脚并调整内容；3) 数学计算采用估算与精确并行的创新策略；4) 存在"动机推理"现象，会反向编造推理过程；5) 知识推理呈现链式分解特征；6) 内置"慎言机制"，对不确定信息默认拒答。研究表明AI具有复杂的内部认知逻辑，既展现智能特性也需警惕其"合理伪装"行为。该技术为理解大模型提供了可视化分析工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T09:03:46Z
- **目录日期**: 2025-03-29
