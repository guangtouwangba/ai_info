...
该研究提出了OVO-Bench基准测试，用于评估视频大语言模型（LLMs）的时间感知能力——即根据提问时间戳动态推理的能力。与依赖完整视频的离线模型不同，在线模型需实时处理视频流并动态响应。OVO-Bench通过12项任务（含644个视频和2800条带精确时间戳的标注），测试模型在三种场景下的表现：回溯过去事件、实时理解当前事件、延迟响应以获取未来信息。评估显示，现有模型在在线视频理解方面与人类表现差距显著。该基准旨在推动视频LLMs在时序推理方面的研究进展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T10:02:07Z
- **目录日期**: 2025-03-29
