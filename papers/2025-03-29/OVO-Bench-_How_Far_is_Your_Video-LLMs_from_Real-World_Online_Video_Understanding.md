...
该研究提出了OVO-Bench基准测试，用于评估视频大语言模型(LLM)的时间感知能力——即根据提问时间戳动态推理的能力。与依赖完整视频的离线模型不同，在线模型需实时处理视频流并动态调整响应。OVO-Bench包含12项任务、644个视频和2800个人工标注样本，测试模型在三种场景下的表现：(1)回溯过去事件；(2)实时理解当前事件；(3)延迟响应以获取未来信息。评估显示，现有模型在在线视频理解方面表现不佳，与人类存在显著差距。该基准旨在推动视频LLM的发展，促进在线视频推理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T07:02:00Z
- **目录日期**: 2025-03-29
