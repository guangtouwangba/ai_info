...
该研究提出了OVO-Bench基准测试，用于评估视频大语言模型（LLMs）的时序感知能力——即根据提问时间戳动态推理的能力。与依赖完整视频的离线模型不同，在线模型需实时处理视频流并动态响应。OVO-Bench包含12项任务、644个视频和2800个人工标注的精细元数据，测试模型在三种场景下的表现：回溯过去事件、实时理解当前事件、延迟响应以获取未来信息。评估9个视频LLMs发现，尽管在传统基准上有进步，现有模型在线视频理解能力仍显著落后于人类。该基准旨在推动在线视频推理研究发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T14:02:16Z
- **目录日期**: 2025-03-29
