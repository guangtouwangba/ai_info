...
OVO-Bench是一个专注于评估视频大语言模型（Video-LLMs）时间感知能力的新基准。与离线模型不同，在线模型需动态处理视频流并根据提问时间戳调整响应。该基准测试模型在三种场景下的表现：回溯过去事件、实时理解和延迟响应以获取未来信息。包含12项任务、644个视频和约2800条精细标注。结合自动生成和人工标注，开发了系统化评估流程。测试发现，现有模型在在线视频理解上表现不佳，与人类存在显著差距。该基准旨在推动视频LLMs发展，促进在线视频推理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T04:02:09Z
- **目录日期**: 2025-03-29
