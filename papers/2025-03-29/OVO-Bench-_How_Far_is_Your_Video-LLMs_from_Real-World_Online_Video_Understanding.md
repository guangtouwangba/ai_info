...
本文介绍了OVO-Bench（在线视频基准测试），这是一个专注于评估视频大语言模型（LLMs）时间感知能力的新基准。时间感知指模型能根据提问时间动态推理的能力，是在线与离线视频LLMs的关键区别。OVO-Bench通过12项任务（含644个视频和2800条带时间戳的标注），评估模型在三种场景下的表现：回溯过去事件、实时理解当前事件、延迟响应以获取未来信息。测试显示，现有模型在在线视频理解方面仍显著落后于人类。该基准旨在推动视频LLMs的发展，促进在线视频推理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T16:02:09Z
- **目录日期**: 2025-03-29
