...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧与真实帧。同时深入研究了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理，显著提升其准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，相关经验可推广至其他音频驱动动画领域。研究突破了传统两阶段生成限制，直接建模音频-视觉复杂关联，为唇同步技术带来重要进展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T18:04:50Z
- **目录日期**: 2025-03-29
