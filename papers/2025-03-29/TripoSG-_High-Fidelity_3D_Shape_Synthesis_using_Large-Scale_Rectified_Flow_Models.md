...
TripoSG是一种新型3D形状生成扩散模型，能够根据输入图像生成高保真3D网格。该模型采用三个关键技术：1)基于大规模高质量数据训练的整流流变压器，实现最先进的生成质量；2)结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3)可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在3D形状生成中达到最优性能，生成的3D模型细节丰富且与输入图像高度匹配，同时展现出对不同图像风格和内容的强大泛化能力。该模型将公开以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T21:01:48Z
- **目录日期**: 2025-03-29
