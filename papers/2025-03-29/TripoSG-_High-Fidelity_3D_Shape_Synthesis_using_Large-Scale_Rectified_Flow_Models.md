...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型提出三大创新：1）基于大规模数据训练的可整流流变压器，实现最先进的生成质量；2）结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在生成细节、图像对齐和多风格适应性方面表现优异，实现了3D形状生成领域的突破。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T12:02:09Z
- **目录日期**: 2025-03-29
