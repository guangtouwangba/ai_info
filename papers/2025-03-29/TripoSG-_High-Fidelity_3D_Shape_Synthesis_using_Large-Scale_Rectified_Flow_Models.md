...
TripoSG是一种新型3D形状生成扩散模型，通过创新方法解决了当前3D生成在质量、泛化能力和输入对齐方面的挑战。该模型采用：1)基于大规模数据训练的整流流变压器提升生成质量；2)结合SDF、法线和eikonal损失的混合监督训练策略优化3D重建；3)构建200万高质量3D样本的数据处理流程。实验表明，TripoSG能生成高保真3D网格，精确匹配输入图像，支持高分辨率细节，并适应多种图像风格。该模型性能达到当前最优水平，其公开将推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T07:01:50Z
- **目录日期**: 2025-03-29
