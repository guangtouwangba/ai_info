...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。关键创新包括：1）基于大规模高质量数据训练的可变形流transformer，实现最先进的生成质量；2）结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3）构建200万高质量3D样本的数据处理流程，验证了数据质量对3D生成的关键作用。实验表明TripoSG在生成细节、输入图像对齐和多风格适应方面表现优异，具有强大的泛化能力。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T17:02:02Z
- **目录日期**: 2025-03-29
