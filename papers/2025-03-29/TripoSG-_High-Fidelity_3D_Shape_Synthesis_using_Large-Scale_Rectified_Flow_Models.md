...
TripoSG提出了一种新型3D形状扩散范式，通过大规模校正流transformer和混合监督训练策略（结合SDF、法线和eikonal损失），实现了高保真3D网格生成。该方法构建了包含200万高质量样本的数据处理流程，验证了数据质量与数量对3D生成模型的关键作用。实验表明，TripoSG在生成细节丰富、与输入图像高度匹配的3D形状方面达到最先进水平，并展现出对不同图像风格和内容的强泛化能力。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T09:02:01Z
- **目录日期**: 2025-03-29
