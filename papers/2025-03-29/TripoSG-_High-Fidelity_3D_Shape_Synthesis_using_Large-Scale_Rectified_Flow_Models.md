...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型采用三大创新：1)基于大规模数据训练的高保真3D形状生成整流流变压器；2)结合SDF、法线和eikonal损失的混合监督3D VAE训练策略；3)可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在3D形状生成上达到最先进水平，能生成细节丰富、与输入图像高度匹配的3D模型，并展现出色的泛化能力，可处理多种图像风格和内容。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T13:06:05Z
- **目录日期**: 2025-03-29
