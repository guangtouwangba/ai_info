...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型采用三大创新：1) 基于大规模数据训练的整流流transformer架构；2) 结合SDF、法线和eikonal损失的混合监督训练策略；3) 可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在生成质量、细节分辨率和输入图像对齐方面达到最先进水平，并能处理多样化的图像风格和内容。该模型将公开以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T14:02:05Z
- **目录日期**: 2025-03-29
