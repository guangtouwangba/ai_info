...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。其创新包括：1）基于大规模高质量数据训练的整流流transformer架构；2）结合SDF、法线和eikonal损失的混合监督训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，该框架各组件协同工作，在生成质量、输入对齐和泛化能力上均达最先进水平。TripoSG能处理多样图像风格，生成细节丰富的高分辨率3D模型。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T04:02:01Z
- **目录日期**: 2025-03-29
