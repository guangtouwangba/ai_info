...
TripoSG是一种新型的3D形状生成扩散模型，通过结合大规模校正流变换器、混合监督训练策略（SDF/法向量/Eikonal损失）和高质量数据处理流程，实现了高保真3D网格生成。该方法在200万高质量3D样本上训练，能精确匹配输入图像，生成细节丰富的3D形状，并展现优异的泛化能力。实验证明其各组件有效性，整体性能达到当前最优水平。该模型支持多种图像风格的3D生成，为促进3D生成领域发展，模型将公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T05:01:50Z
- **目录日期**: 2025-03-29
