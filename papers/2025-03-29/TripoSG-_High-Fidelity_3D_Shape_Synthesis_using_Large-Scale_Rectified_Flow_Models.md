...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。其创新包括：1)采用大规模整流流transformer架构，通过高质量数据训练实现最先进生成质量；2)结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3)开发数据处理流程生成200万高质量3D样本，验证了数据质量对模型训练的关键作用。实验表明TripoSG能生成细节丰富、与输入图像高度一致的3D形状，并展现出色的泛化能力。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T18:01:47Z
- **目录日期**: 2025-03-29
