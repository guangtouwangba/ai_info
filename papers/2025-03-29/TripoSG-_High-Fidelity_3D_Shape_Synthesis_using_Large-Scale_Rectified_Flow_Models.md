...
TripoSG是一种新型3D形状生成扩散模型，通过创新方法解决了当前3D生成在质量、泛化能力和输入对齐方面的挑战。该模型提出：(1)基于大规模数据训练的整流流变压器，实现高保真3D网格生成；(2)结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建质量；(3)可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在细节呈现和输入图像匹配方面达到最先进水平，并能处理多样化的图像风格和内容。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T10:01:57Z
- **目录日期**: 2025-03-29
