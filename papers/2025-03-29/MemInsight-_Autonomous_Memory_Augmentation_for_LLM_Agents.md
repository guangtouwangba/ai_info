...
本文提出了一种名为MemInsight的自主记忆增强方法，旨在提升大语言模型（LLM）代理的语义数据表示和检索能力。通过优化历史交互数据的语义结构化处理，该方法显著提高了LLM代理的响应准确性和情境适应性。实验验证了MemInsight在对话推荐、问答和事件摘要三种任务中的有效性：在LLM-REDIAL数据集上，推荐说服力提升高达14%；在LoCoMo检索任务中，召回率比RAG基线高出34%。结果表明，MemInsight能有效增强LLM代理在多任务中的情境表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T04:01:08Z
- **目录日期**: 2025-03-29
