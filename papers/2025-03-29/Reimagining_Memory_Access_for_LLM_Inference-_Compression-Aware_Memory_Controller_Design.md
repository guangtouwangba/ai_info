...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩(LZ4/ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的前提下，将内存占用分别减少了25.2%和46.9%。同时通过上下文相关的动态量化，使内存带宽和能耗按比例调整。实验证明，在7nm工艺下实现的硬件原型(4GHz,32通道)可提供8TB/s吞吐量，面积开销仅3.8mm²。这种LLM感知的内存控制方案为大规模高效推理提供了可行解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T01:28:49Z
- **目录日期**: 2025-03-29
