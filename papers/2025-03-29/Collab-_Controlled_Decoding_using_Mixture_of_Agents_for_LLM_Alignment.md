...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）计算成本高，而控制解码可在推理时对齐但单代理方法难以适应多样任务。为此，研究者提出基于混合代理的协作解码策略，动态选择最优模型生成每个token。该方法通过长时效用指标从模型池中选择最合适的LLM，实现推理时高效对齐。理论分析证明其针对目标奖励的最优性，实验显示其显著优于单代理基线：Collab方法在平均奖励上提升1.56倍，GPT-4胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T16:01:29Z
- **目录日期**: 2025-03-29
