...
该研究提出了一种基于多智能体协作的解码策略Collab，用于提升大型语言模型（LLMs）的推理时对齐能力。不同于传统单智能体解码方法，Collab通过动态选择最优模型（从预对齐模型池中）进行逐词生成，依据长期效用指标实现多模型协同。理论分析证明该方法能针对目标奖励函数实现最优性能。实验表明，Collab在开源对齐模型上的表现显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。该方法避免了昂贵的模型微调，实现了高效的多模型协作对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T21:01:22Z
- **目录日期**: 2025-03-29
