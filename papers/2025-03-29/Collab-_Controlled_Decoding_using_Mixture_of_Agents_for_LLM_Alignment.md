...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而单智能体解码方法难以适应多样化任务。本文提出一种基于多智能体协作的解码策略（Collab），通过动态选择最优模型进行令牌级决策，实现推理时对齐。该方法利用现成对齐模型池，基于长期效用指标动态切换策略，理论分析证明其最优性。实验表明，Collab在开源模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T19:01:37Z
- **目录日期**: 2025-03-29
