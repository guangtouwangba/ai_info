...
大型语言模型（LLM）的对齐对安全可信的应用部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而基于推理阶段控制的解码方法虽无需重训练，但单代理策略难以适应多样化任务。本文提出一种多代理协作解码策略（Collab），动态选择最优LLM生成每个token，通过长时效用指标实现模型间的推理时对齐。理论分析证明该方法在给定现成模型下能实现目标奖励最优，实验显示其在开源对齐模型上显著优于单代理基线：平均奖励提升1.56倍，GPT-4胜率提升71.89%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T01:28:32Z
- **目录日期**: 2025-03-29
