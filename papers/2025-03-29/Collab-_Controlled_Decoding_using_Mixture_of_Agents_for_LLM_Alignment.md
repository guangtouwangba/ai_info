...
大型语言模型（LLM）的对齐对安全可信的应用部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而控制解码可在推理阶段实现对齐但单代理方法难以适应多样化任务。本文提出一种多代理协作解码策略，将现有对齐LLM策略视为代理，通过动态选择最适合当前任务的模型进行词级决策。理论分析证明该方法在目标奖励下具有最优性能，实验表明其显著优于单代理基线：Collab方法平均奖励提升1.56倍，GPT-4胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T14:01:33Z
- **目录日期**: 2025-03-29
