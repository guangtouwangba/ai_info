...
大型语言模型（LLM）的对齐对安全可信的应用部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而基于推理阶段的对齐方法（如控制解码）又难以适应多样化任务。为此，研究者提出了一种多智能体协作解码框架Collab，通过动态选择最优的预对齐LLM策略进行逐词生成，以长时效用指标为选择依据。理论分析证明其最优性，实验显示Collab在开源对齐模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T13:05:31Z
- **目录日期**: 2025-03-29
