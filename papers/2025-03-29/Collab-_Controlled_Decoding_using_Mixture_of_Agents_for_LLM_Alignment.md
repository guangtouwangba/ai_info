...
大语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而受控解码可在推理时实现对齐但单代理策略难以适应多样化任务。本文提出多代理协作解码方法Collab，通过动态选择最优LLM策略生成每个token，利用现成对齐模型实现高效协作。理论分析证明其在目标奖励下的最优性，实验显示其显著优于单代理基线：平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T06:01:30Z
- **目录日期**: 2025-03-29
