...
该研究提出了一种基于多智能体协作的解码策略（Collab），用于提升大语言模型（LLM）在推理阶段的对齐性能。不同于传统单智能体解码方法或需要重训练的RLHF技术，该方法动态选择预对齐模型池中的最优策略进行token级决策，通过长时效用指标实现多模型协作。理论分析证明其能针对目标奖励函数实现最优性能。实验表明，Collab在开源对齐模型上的表现显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最先进的解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T17:01:35Z
- **目录日期**: 2025-03-29
