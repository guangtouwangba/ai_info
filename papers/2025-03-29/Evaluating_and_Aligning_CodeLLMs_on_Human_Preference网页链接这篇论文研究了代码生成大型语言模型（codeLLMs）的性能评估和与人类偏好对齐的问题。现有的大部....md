...
这篇论文研究了代码生成大模型（codeLLMs）如何与人类偏好对齐的问题。作者提出了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，以评估模型在真实编程任务中的表现。同时构建了20B标记的合成指令数据集SynCode-Instruct用于微调验证。实验发现现有基于执行的基准与CodeArena存在性能差异，且开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，突出了对齐人类偏好的重要性。研究强调了评估标准需兼顾代码正确性和用户偏好。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T12:06:08Z
- **目录日期**: 2025-03-29
