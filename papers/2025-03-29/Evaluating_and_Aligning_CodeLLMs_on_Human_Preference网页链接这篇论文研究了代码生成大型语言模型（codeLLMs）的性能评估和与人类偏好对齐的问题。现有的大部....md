...
这篇论文针对代码生成大模型（codeLLMs）提出两个关键贡献：1）创建CodeArena基准，包含397个真实编程任务样本（覆盖40类/44种语言），首次系统评估模型输出与人类偏好的对齐程度；2）构建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现：现有基于执行的评估与人类偏好存在偏差，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调了代码生成任务中"正确性"与"实用性"的差异，为提升AI编程助手实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T01:32:13Z
- **目录日期**: 2025-03-29
