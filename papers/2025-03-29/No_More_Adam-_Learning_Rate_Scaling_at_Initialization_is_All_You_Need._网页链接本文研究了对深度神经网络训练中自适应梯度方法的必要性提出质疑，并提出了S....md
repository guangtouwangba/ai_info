...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法(如Adam)的传统认知。该方法通过初始化时根据梯度信噪比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。SGD-SaI无需自适应二阶动量即可避免训练不平衡，内存使用仅为AdamW的一半。实验表明，它在Transformer任务、ImageNet分类、LLM预训练(如GPT-2)和LoRA微调中表现优于或持平AdamW，同时显著节省内存(GPT-2节省5.93GB，Llama2-7B节省25.15GB)，且具有超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T19:04:14Z
- **目录日期**: 2025-03-29
