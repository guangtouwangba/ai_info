...
本文介绍了GateLens，一个基于大语言模型（LLM）的汽车领域表格数据分析工具。它通过将自然语言查询转换为关系代数表达式，并生成优化的Python代码，显著提升了分析效率。相比传统方法，GateLens在基准测试中表现更优，F1分数更高，能处理复杂模糊查询，且无需依赖示例样本。工业评估显示，该工具减少80%以上的分析时间，同时保持高准确性和可靠性。实际部署表明，GateLens能自动化测试结果分析，加快软件发布决策，提升汽车系统的可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T04:01:26Z
- **目录日期**: 2025-03-29
