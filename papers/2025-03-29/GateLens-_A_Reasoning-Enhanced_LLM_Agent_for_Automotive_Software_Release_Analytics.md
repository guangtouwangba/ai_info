...
本文介绍了一种基于大语言模型(LLM)的工具GateLens，用于分析汽车领域的表格数据。该工具能将自然语言查询转换为关系代数表达式，并生成优化的Python代码。相比传统方法，GateLens在基准测试中表现更优，F1分数更高，能更稳健地处理复杂和模糊查询。工业评估显示，GateLens可将分析时间减少80%以上，同时保持高准确性和可靠性。该工具无需依赖少量示例就能实现高性能，并能适应不同公司角色的多样化查询类型。实践证明，GateLens能通过自动化测试结果分析，帮助做出更快、更明智的软件发布决策。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T11:01:12Z
- **目录日期**: 2025-03-29
