...
本文介绍了一种基于大语言模型（LLM）的工具GateLens，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式并生成优化Python代码，在基准测试中表现优于基线系统，F1分数更高且能更稳健处理复杂模糊查询。消融实验证实关系代数模块的关键作用，工业评估显示其减少80%以上分析时间同时保持高准确性和可靠性。GateLens无需依赖少量示例即可实现高性能，在不同公司角色的多样化查询类型中展现强泛化能力，可加速汽车系统软件发布决策，提升可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T03:10:45Z
- **目录日期**: 2025-03-29
