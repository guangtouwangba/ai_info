...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式，并生成优化的Python代码，在基准测试中表现优于基线系统，F1分数更高，处理复杂和模糊查询更稳健。消融实验证实了关系代数模块的关键作用。工业评估显示，GateLens将分析时间减少80%以上，同时保持高准确性和可靠性。该工具无需依赖少量示例即可实现高性能，展示了跨不同公司角色的强泛化能力，可加速汽车系统软件发布决策，提升可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T06:01:22Z
- **目录日期**: 2025-03-29
