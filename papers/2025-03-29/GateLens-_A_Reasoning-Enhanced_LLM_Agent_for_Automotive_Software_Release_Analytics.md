...
本文介绍了GateLens，一种基于大语言模型（LLM）的汽车领域表格数据分析工具。它通过将自然语言查询转换为关系代数表达式并生成优化Python代码，显著提升了分析效率。相比传统方法，GateLens在基准测试中表现更优，F1分数更高，能稳健处理复杂模糊查询。工业评估显示，其分析时间减少80%以上，同时保持高准确性和可靠性。该工具无需依赖少量示例即实现高性能，展现了跨角色查询的强泛化能力。实际部署表明，GateLens能自动化测试结果分析，帮助汽车系统实现更快、更可靠的软件发布决策。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T02:26:24Z
- **目录日期**: 2025-03-29
