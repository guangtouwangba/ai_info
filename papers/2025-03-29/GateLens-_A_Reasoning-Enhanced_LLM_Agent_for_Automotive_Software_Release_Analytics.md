...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域中的表格数据。GateLens将自然语言查询转换为关系代数表达式并生成优化的Python代码，显著提升了F1分数，并能处理复杂和模糊查询。消融实验证实关系代数模块的关键作用。工业评估显示，GateLens减少80%以上的分析时间，同时保持高准确性和可靠性。该工具无需依赖少样本示例即可实现高性能，在不同公司角色的多样化查询中表现出强泛化能力。GateLens通过自动化测试结果分析，支持更快、更可靠的软件发布决策，提升汽车系统的可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T12:01:26Z
- **目录日期**: 2025-03-29
