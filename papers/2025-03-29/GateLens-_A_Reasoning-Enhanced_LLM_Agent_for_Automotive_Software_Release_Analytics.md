...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens能将自然语言查询转换为关系代数表达式并生成优化的Python代码，在基准测试中表现优于基线系统，F1分数更高且能更稳健地处理复杂模糊查询。工业评估显示，该工具减少80%以上的分析时间，同时保持高准确性和可靠性。研究表明，GateLens无需依赖少量示例即可实现高性能，并能泛化处理不同公司角色的多样化查询。通过自动化测试结果分析，该工具能帮助汽车系统做出更快、更明智且可靠的发布决策。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T14:01:20Z
- **目录日期**: 2025-03-29
