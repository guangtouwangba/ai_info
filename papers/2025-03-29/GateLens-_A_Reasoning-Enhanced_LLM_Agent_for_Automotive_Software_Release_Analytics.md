...
本文介绍了GateLens，一种基于大语言模型(LLM)的工具，用于分析汽车领域的表格数据。它通过将自然语言查询转换为关系代数表达式并生成优化Python代码，显著提升了分析效率。实验表明，GateLens在基准数据集上表现优于基线系统，F1分数更高，能稳健处理复杂模糊查询。工业评估显示，该工具减少80%以上的分析时间，同时保持高准确性和可靠性。部署案例证明，GateLens无需few-shot示例就能实现高性能，支持跨职能查询，可推动汽车系统软件发布决策的自动化与可靠性提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T05:01:16Z
- **目录日期**: 2025-03-29
