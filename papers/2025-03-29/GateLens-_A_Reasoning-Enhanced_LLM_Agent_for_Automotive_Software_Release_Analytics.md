...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式并生成优化的Python代码，显著提升了复杂查询的处理能力。相比传统方法，其分析时间减少80%以上，同时保持高准确性和可靠性。消融实验证实关系代数模块的关键作用，工业评估显示其在不依赖少量示例的情况下实现高性能，适用于不同角色的多样化查询。GateLens通过自动化测试结果分析，帮助汽车系统实现更快、更可靠的软件发布决策，提升可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-29T00:01:29Z
- **目录日期**: 2025-03-29
