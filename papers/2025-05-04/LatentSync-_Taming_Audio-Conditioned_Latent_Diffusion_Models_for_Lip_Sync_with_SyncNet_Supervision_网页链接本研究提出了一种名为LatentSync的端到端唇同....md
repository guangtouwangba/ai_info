...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，解决了传统方法时间一致性问题，并提出时间表示对齐(TREPA)方法增强一致性。研究还优化了SyncNet训练框架，改进其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)端到端潜在扩散模型；2)TREPA时间对齐；3)SyncNet训练优化。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T14:05:36Z
- **目录日期**: 2025-05-04
