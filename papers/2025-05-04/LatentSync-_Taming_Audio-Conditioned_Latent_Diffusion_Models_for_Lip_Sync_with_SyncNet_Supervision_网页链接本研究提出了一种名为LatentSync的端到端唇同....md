...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法。相比传统方法，它无需中间运动表示，直接建模音频-视觉关系，利用Stable Diffusion的强大能力。针对扩散模型的时间一致性问题，创新性提出TREPA方法（时间表示对齐），通过自监督视频模型增强生成帧的时间连贯性。同时改进了SyncNet训练框架，解决了收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进对其他音频驱动动画方法也具有参考价值。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T00:05:27Z
- **目录日期**: 2025-05-04
