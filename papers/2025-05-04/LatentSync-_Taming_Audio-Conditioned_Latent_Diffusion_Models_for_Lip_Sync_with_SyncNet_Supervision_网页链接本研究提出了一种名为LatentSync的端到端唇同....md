...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。为解决扩散过程中时间一致性问题，提出了时间表示对齐（TREPA）方法，利用自监督视频模型增强一致性。同时优化了SyncNet训练框架，提升了其准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。创新点包括直接建模音频-视觉关系、改进时间一致性及SyncNet训练策略，为唇同步和音频驱动动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T08:05:39Z
- **目录日期**: 2025-05-04
