...
当前大型语言模型（LLMs）的偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型过度拟合或错误学习。本研究提出一种奖励条件化的LLM策略，通过分析数据集中的整体响应质量来优化模型表现。方法包括基于质量分数重新标记偏好对数据，构建奖励增强数据集。实验表明，该方法显著提升了DPO（直接偏好优化）效果，有效利用偏好数据并减少错误学习问题，其优势不仅来自数据扩展，更源于质量感知机制。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T20:02:10Z
- **目录日期**: 2025-05-04
