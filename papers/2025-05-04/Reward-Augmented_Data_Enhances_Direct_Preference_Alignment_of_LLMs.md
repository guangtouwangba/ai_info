...
大型语言模型（LLMs）的偏好对齐虽能提升指令遵循能力，但现有方法过度关注相对偏好而忽略响应质量，导致对低质量选择的盲目偏好和高质量拒绝响应的遗忘。本研究提出基于奖励条件的LLM策略，通过分析数据集中全部响应质量来推断最优区域，并设计简单有效的数据重标方法，将偏好对与质量分数结合构建奖励增强数据集。实验表明，该方法显著提升DPO性能，不仅最大化偏好数据效用，还缓解遗忘问题，其有效性超越单纯的数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T19:02:04Z
- **目录日期**: 2025-05-04
