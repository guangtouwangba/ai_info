...
当前大语言模型（LLM）的对齐方法主要关注相对偏好，却忽略了响应质量。现有方法盲目追求奖励差距最大化，可能导致过拟合或遗忘高质量被拒响应，且无法泛化到稀疏的最优响应。本研究提出**基于奖励条件化的LLM策略**，通过分析数据集中全部响应质量进行学习，并设计了一种简单的数据重标记方法，将偏好对与质量分数结合构建奖励增强数据集。实验表明，该方法显著提升了DPO性能，有效利用偏好数据并缓解遗忘问题，其效果不仅源于数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T02:33:20Z
- **目录日期**: 2025-05-04
