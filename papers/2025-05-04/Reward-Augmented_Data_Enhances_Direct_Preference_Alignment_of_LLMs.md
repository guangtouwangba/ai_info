...
该研究提出了一种基于奖励条件的大语言模型（LLM）对齐方法，通过利用偏好数据中的奖励分数来提升模型性能。现有方法仅关注相对偏好而忽略响应质量，导致对次优响应的过度拟合或对高质量拒绝响应的遗忘。新方法采用数据重标注技术，将偏好对与质量分数结合构建奖励增强数据集，使模型能学习响应质量的完整分布。实验表明，该方法显著提升了DPO（直接偏好优化）效果，不仅最大化偏好数据效用，还缓解了遗忘问题，具有超越数据扩展的广泛有效性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T00:02:14Z
- **目录日期**: 2025-05-04
