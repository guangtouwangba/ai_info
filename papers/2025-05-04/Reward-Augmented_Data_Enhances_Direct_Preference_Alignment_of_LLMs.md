...
当前大语言模型（LLM）的对齐方法主要关注相对偏好，却忽略了响应质量的定性分析。现有方法盲目追求隐式奖励差距最大化，可能导致过拟合或高质量拒绝响应的遗忘。本研究提出基于奖励条件的LLM策略，通过分析数据集中全部响应质量来优化学习，并设计了一种简单有效的数据重标注方法，将偏好对与质量分数结合构建奖励增强数据集。实验表明，该方法显著提升了DPO性能，不仅最大化偏好数据的效用，还缓解了遗忘问题，其有效性不仅源于数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T12:02:23Z
- **目录日期**: 2025-05-04
