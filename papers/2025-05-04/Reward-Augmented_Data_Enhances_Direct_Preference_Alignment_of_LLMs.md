...
该研究针对大型语言模型(LLM)偏好对齐中的不足，提出了一种奖励条件策略。现有方法主要关注相对偏好而忽视响应质量，导致过度拟合和高质量拒绝响应的遗忘。为此，研究者开发了基于质量分数的数据重标记方法，构建奖励增强数据集，使模型能学习响应质量的完整分布。实验表明，该方法显著提升了DPO性能，不仅最大化偏好数据的效用，还缓解了遗忘问题。研究通过多基准测试验证了其广泛有效性，超越了单纯的数据扩展效果。代码已开源供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T18:01:59Z
- **目录日期**: 2025-05-04
