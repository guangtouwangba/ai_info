...
当前大语言模型（LLM）偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误学习。本研究提出了一种奖励条件化的LLM策略，通过分析数据集中的整体响应质量，帮助模型推断更优结果。方法包括基于质量分数重新标记偏好对，构建奖励增强数据集。实验表明，该方法显著提升了DPO（直接偏好优化）效果，不仅最大化偏好数据的效用，还缓解了错误学习问题，其有效性超越单纯的数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T16:02:02Z
- **目录日期**: 2025-05-04
