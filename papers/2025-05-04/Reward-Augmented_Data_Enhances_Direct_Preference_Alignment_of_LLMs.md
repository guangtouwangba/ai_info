...
现有大型语言模型（LLMs）偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误学习。本研究提出基于奖励条件的LLM策略，通过辨别和学习数据集中全部响应质量来提升性能。方法包括对偏好数据进行质量评分重新标记，构建奖励增强数据集。实验表明，该方法显著提升了DPO效果，不仅最大化偏好数据效用，还缓解了错误学习问题。研究证明了该方法在数据扩展之外的广泛有效性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T03:22:22Z
- **目录日期**: 2025-05-04
