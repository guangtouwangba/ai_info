...
当前大语言模型（LLM）偏好对齐方法主要关注相对偏好，而忽略了响应质量。现有方法过度优化隐式奖励差距，可能导致高质量拒绝响应的遗忘，且无法泛化到数据稀疏的最优响应。本研究提出基于奖励条件的LLM策略，通过数据重标注方法构建奖励增强数据集，使模型能够学习响应质量的整体分布并外推到更优区域。实验表明，该方法显著提升了DPO性能，不仅最大化偏好数据的效用，还缓解了遗忘问题，其效果不仅源于数据扩展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T13:07:35Z
- **目录日期**: 2025-05-04
