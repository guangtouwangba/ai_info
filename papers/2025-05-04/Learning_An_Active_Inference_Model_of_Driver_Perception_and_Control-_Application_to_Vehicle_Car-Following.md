...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：i) 智能体对环境动态的内部表征，ii) 对可观测结果的偏好。研究采用与主动推理理论一致的结构，该理论认为人类行为旨在最小化"惊奇"（实际观测与偏好观测的差异）。作者提出双层优化估计方法，通过对先验分布的结构性假设来参数化人类环境模型的统计准确性。以跟车行为为例，基于自然驾驶数据的建模结果表明，这种主动推理模型比黑箱模型更具潜力，为人类驾驶行为建模提供了新思路。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T02:33:38Z
- **目录日期**: 2025-05-04
