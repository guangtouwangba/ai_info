...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：1) 智能体对环境动态及观测的控制响应内部表征；2) 对可观测结果的偏好。模型构建符合认知科学中的主动推理理论，即智能体通过最小化"惊讶度"（实际观测与偏好观测的差异）来采取行动。作者采用双层优化方法进行估计，并基于先验分布的结构性假设。以跟车行为为例，通过自然数据集验证表明，这种主动推理模型比黑箱驾驶模型更具潜力。研究为从数据中学习人类感知控制模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T10:02:34Z
- **目录日期**: 2025-05-04
