...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：1) 主体对环境动态及控制行为影响的内部表征；2) 主体对可观测结果的偏好。研究采用与主动推断理论（认知科学中的人类行为理论）一致的模型框架，认为主体行为旨在最小化"意外"（当前感知与偏好感知的差异）。作者提出基于先验分布结构假设的双层优化估计方法，并以自然驾驶数据中的跟车行为建模为例验证。结果表明，这种主动推断模型比黑箱模型更能有效表征人类驾驶行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T06:02:35Z
- **目录日期**: 2025-05-04
