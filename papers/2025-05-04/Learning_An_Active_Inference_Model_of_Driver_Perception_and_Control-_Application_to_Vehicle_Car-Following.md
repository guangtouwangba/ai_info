...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型包含两部分：i) 主体对环境动态及观察结果的内在表征；ii) 主体对可观测结果的偏好。模型结构符合认知科学中的主动推断理论，即主体通过最小化"意外"（当前感官观察与偏好观察的差异）来行动。作者采用双层优化方法进行估计，并基于先验分布的结构假设。通过自然驾驶数据中的跟车行为建模验证，结果表明这种主动推断模型比黑箱模型更能有效描述人类驾驶行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T05:02:40Z
- **目录日期**: 2025-05-04
