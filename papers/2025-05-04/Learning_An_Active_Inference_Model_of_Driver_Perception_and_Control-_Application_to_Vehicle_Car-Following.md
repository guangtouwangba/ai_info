...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的通用估计方法。该模型结构包含：i) 环境内部表征，ii) 对可观测结果的偏好。模型符合"主动推理"理论，即人类通过最小化"惊讶度"(实际观测与偏好观测的差异)来行动。作者采用双层优化方法进行估计，并基于先验分布的结构假设。通过自然驾驶数据中的跟车行为建模验证，结果表明这种主动推理模型比黑箱模型更有前景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T13:07:55Z
- **目录日期**: 2025-05-04
