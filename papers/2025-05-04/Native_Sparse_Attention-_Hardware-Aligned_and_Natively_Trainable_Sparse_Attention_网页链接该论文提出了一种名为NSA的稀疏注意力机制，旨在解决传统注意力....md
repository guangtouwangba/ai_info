...
该论文提出了Native Sparse Attention（NSA）机制，旨在降低传统注意力机制处理长文本时的高计算成本。NSA结合算法创新与硬件优化，采用动态分层稀疏策略：通过粗粒度令牌压缩和细粒度令牌选择，在保持全局上下文感知的同时确保局部精度。这一方法有效解决了长文本建模的效率问题，并提出了两个关键创新点，为大规模语言模型的注意力机制优化提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T14:05:09Z
- **目录日期**: 2025-05-04
