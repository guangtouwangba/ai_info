...
这篇论文针对代码生成大型语言模型（codeLLMs）提出两个关键贡献：1）创建CodeArena基准测试，包含397个高质量编程任务样本（覆盖40类/44种语言），首次关注代码生成与人类偏好的对齐；2）构建20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现：基于执行的测试与人类偏好存在显著差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距明显。研究强调现有基准的局限性，证明人类偏好对齐对实用化代码生成至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T23:05:34Z
- **目录日期**: 2025-05-04
