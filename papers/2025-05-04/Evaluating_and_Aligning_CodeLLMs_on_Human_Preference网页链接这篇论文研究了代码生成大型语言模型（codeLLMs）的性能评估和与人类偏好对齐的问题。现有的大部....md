...
这篇论文针对代码生成大模型(codeLLMs)提出了CodeArena基准测试，强调评估模型输出与人类偏好的匹配度，而不仅是代码正确性。研究收集了397个涵盖40类编程任务和44种语言的真实样本，并构建了20B标记的合成指令数据集SynCode-Instruct。实验发现基于执行的测试与人类偏好评估存在差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著，凸显了模型对齐人类偏好的重要性。该研究为提升代码生成模型的实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T09:06:07Z
- **目录日期**: 2025-05-04
