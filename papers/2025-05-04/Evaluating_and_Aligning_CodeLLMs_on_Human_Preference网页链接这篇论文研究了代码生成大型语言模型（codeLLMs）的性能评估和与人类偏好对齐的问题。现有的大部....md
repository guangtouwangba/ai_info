...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好对齐的问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类任务和44种语言)，评估模型在实际编程任务中的表现，发现传统基于执行的测试与人类偏好存在差距。同时构建了20B标记的SynCode-Instruct语料库验证合成指令微调效果。实验显示开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)存在显著性能差异，强调了人类偏好对齐的重要性。研究成果为提升代码LLMs的实用性和用户体验提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T02:37:03Z
- **目录日期**: 2025-05-04
