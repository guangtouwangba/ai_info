...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过初始化时依据梯度信号噪声比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，SGD-SAi内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，甚至超越现有优化器。此外，它在LoRA微调等任务中也表现突出，并为大模型（如GPT-2和Llama2-7B）节省了大量内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T16:04:36Z
- **目录日期**: 2025-05-04
