...
本文提出了一种名为FPoTT的新型联邦学习框架，用于解决车联网(IoV)中的实时、隐私保护和可扩展交通管理问题。该框架利用开源大语言模型(LLMs)，通过动态提示优化机制提高轨迹预测精度，采用双层联邦学习架构结合轻量级边缘模型和云端LLMs。FPoTT还集成了基于Transformer的合成数据生成器，使用NGSIM格式增强训练数据多样性。实验表明，采用Pythia-1B模型的FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能，为智能交通系统提供了优于专有解决方案的开源替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T04:03:21Z
- **目录日期**: 2025-05-04
