...
本文提出了一种名为FPoTT的新型框架，旨在解决车联网（IoV）中实时性、可扩展性和隐私保护等关键挑战。该框架利用开源大语言模型（LLMs），通过动态提示优化机制提升轨迹预测精度，并采用双层联邦学习架构，结合轻量级边缘模型和云端LLMs实现实时推理与全局智能。FPoTT还整合了基于Transformer的合成数据生成器，以增强训练多样性。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，展现了开源LLMs在车联网管理中的潜力，为智能交通生态系统提供了高效、安全的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T10:01:45Z
- **目录日期**: 2025-05-04
