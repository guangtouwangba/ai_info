...
本文提出了一种名为FPoTT的新型框架，用于解决车联网（IoV）中实时、可扩展且保护隐私的交通管理挑战。FPoTT利用开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性，并采用双层联邦学习架构，结合轻量级边缘模型和云端LLM实现全局智能。此外，FPoTT整合了基于Transformer的合成数据生成器，增强训练多样性。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，同时保持对合成数据的高性能，为智能交通系统提供了一种安全、自适应且可扩展的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T21:01:42Z
- **目录日期**: 2025-05-04
