...
本文提出了联邦提示优化交通变换器（FPoTT）框架，以解决车联网（IoV）中可扩展、实时和隐私保护的交通管理挑战。FPoTT利用开源大型语言模型（LLM），通过动态提示优化机制提升轨迹预测准确性，并采用双层联邦学习架构，结合轻量级边缘模型和云端LLM。该框架还整合了基于Transformer的合成数据生成器，增强训练多样性。实验表明，FPoTT在真实数据上达到99.86%的预测准确率，展现了开源LLM在车联网管理中的潜力，为智能交通生态系统提供了高效、安全的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T06:01:49Z
- **目录日期**: 2025-05-04
