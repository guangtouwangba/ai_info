...
本文提出了一种新型的联邦学习框架FPoTT，用于解决车联网（IoV）中实时、可扩展且保护隐私的交通管理问题。该框架利用开源大语言模型（LLM），通过动态提示优化机制提升轨迹预测精度，并采用双层联邦学习架构，结合轻量级边缘模型和云端LLM。FPoTT还集成了基于Transformer的合成数据生成器，增强训练数据多样性。实验显示，FPoTT在真实数据上达到99.86%的预测准确率，为智能交通系统提供了高效、安全的替代方案，克服了现有集中式方案的延迟和可扩展性限制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T22:01:50Z
- **目录日期**: 2025-05-04
