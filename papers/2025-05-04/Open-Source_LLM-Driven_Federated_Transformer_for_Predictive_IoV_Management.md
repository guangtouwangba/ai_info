...
本文提出了联邦提示优化交通变换器（FPoTT），一种基于开源大语言模型（LLM）的新型车联网（IoV）管理框架。FPoTT采用动态提示优化机制改进轨迹预测，结合轻量级边缘模型和云端LLM的双层联邦学习架构，实现实时推理与全局智能。集成Transformer驱动的合成数据生成器增强NGSIM格式的多样化训练场景。测试显示，FPoTT使用Pythia-1B模型在真实数据上达到99.86%预测准确率，同时保持对合成数据的高性能，为车联网提供了安全、自适应且可扩展的解决方案，优于现有专有系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-04T16:01:35Z
- **目录日期**: 2025-05-04
