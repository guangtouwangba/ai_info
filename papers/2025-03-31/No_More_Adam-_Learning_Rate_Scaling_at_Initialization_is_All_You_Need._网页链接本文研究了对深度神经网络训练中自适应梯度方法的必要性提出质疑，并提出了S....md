...
这篇论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需二阶动量计算，将内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练(GPT-2/Llama2)中表现优于或持平AdamW，尤其显著降低内存消耗（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。同时展现出超参数鲁棒性，在LoRA微调等任务中也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T02:32:37Z
- **目录日期**: 2025-03-31
