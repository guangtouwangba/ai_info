...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过在初始化时根据各参数组的梯度信噪比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，同时具备超参数鲁棒性。在LoRA微调等任务中也优于现有优化器，并为大模型（如GPT-2、Llama2-7B）显著节省内存（最高达25GB）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T21:04:01Z
- **目录日期**: 2025-03-31
