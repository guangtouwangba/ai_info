...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统认知。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。相比AdamW，SGD-SaI内存占用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，同时解决了SGD训练Transformer的难题。在LoRA微调等任务中也优于现有优化器。内存效率显著，为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存，具有超参数鲁棒性和广泛应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T11:05:07Z
- **目录日期**: 2025-03-31
