...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。SGD-SaI无需自适应二阶动量即可防止训练不平衡，内存使用比AdamW减少一半，在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，甚至超过AdamW。在内存效率方面，为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存，并在LoRA微调等任务中优于现有优化器，具有超参数鲁棒性和广泛应用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T10:04:46Z
- **目录日期**: 2025-03-31
