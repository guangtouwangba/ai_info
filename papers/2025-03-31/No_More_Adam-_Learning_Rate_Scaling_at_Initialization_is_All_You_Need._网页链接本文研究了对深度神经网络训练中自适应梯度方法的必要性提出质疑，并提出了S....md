...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法(如Adam)的必要性。SGD-SaI是对带动量SGD的改进，通过初始化时依据梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量就能避免训练不平衡，并将优化器内存使用减半。实验表明，在Transformer任务、ImageNet分类、LLM预训练(GPT-2/Llama2)和LoRA微调中，其性能等同或优于AdamW，同时显著节省内存(GPT-2省5.93GB，Llama2-7B省25.15GB)，且对超参数变化更鲁棒。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T14:04:48Z
- **目录日期**: 2025-03-31
