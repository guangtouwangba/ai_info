...
近期直接偏好对齐算法(DPA)如DPO在将大语言模型与人类偏好对齐方面展现出潜力。然而由于缺乏技术框架来理解这些算法的底层语义，研究者难以比较现有变体或开发新损失函数。本文提出通过离散推理问题形式化DPA损失，建立新形式体系来描述单模型和参考模型方法的偏好损失，并为常见DPA变体识别符号形式。这种形式化视角不仅能严格描述不同损失函数间的关系，还能系统探索损失函数空间并基于第一性原理推导新损失函数。该框架有望为人类-AI对齐研究提供指导。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T00:02:40Z
- **目录日期**: 2025-03-31
