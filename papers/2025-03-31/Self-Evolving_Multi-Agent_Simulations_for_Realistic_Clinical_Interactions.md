...
本文介绍了MedAgentSim，一个开源的临床模拟环境，包含医生、患者和检测代理，旨在评估和提升大语言模型（LLM）在动态诊断场景中的表现。该框架要求医生代理通过多轮对话主动与患者互动，并向检测代理请求相关医学检查（如体温、血压、心电图）和影像结果（如MRI、X光），以模拟真实诊断流程。此外，系统整合了自我改进机制，支持模型迭代优化诊断策略，并通过多代理讨论、思维链推理和经验检索提升LLM表现。该工具支持全自动运行或人机交互模式，并提供评估基准。代码和工具已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T06:01:07Z
- **目录日期**: 2025-03-31
