...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。它通过将自然语言查询转换为关系代数（RA）表达式并生成优化的Python代码，显著提升了分析效率。相比传统方法，GateLens在基准测试中表现更优，F1分数更高，并能稳健处理复杂和模糊查询。工业评估表明，GateLens将分析时间缩短80%以上，同时保持高准确性和可靠性。该工具无需依赖少量示例即实现高性能，展现了强大的泛化能力，可支持汽车系统软件发布决策的自动化和可靠性提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T01:28:55Z
- **目录日期**: 2025-03-31
