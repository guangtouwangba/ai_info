...
香港大学、英伟达和图宾根大学联合团队提出Centaur方法，解决自动驾驶在变道、加塞等场景中过度谨慎的问题。传统自动驾驶依赖预设规则，导致无故急刹或避让，反而增加安全隐患。Centaur通过测试时计算（test-time training）实现动态优化，利用在线数据驱动调整策略，减少对固定规则的依赖，提升AI在不确定场景下的适应性和安全性。该技术首次实现让自动驾驶系统"边开边学"，更接近人类驾驶员的灵活决策能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T02:32:07Z
- **目录日期**: 2025-03-31
