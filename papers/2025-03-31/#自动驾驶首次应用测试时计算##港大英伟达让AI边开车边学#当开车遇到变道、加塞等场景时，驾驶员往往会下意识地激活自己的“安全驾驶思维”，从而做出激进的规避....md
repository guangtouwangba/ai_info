...
香港大学、英伟达和图宾根大学联合团队提出Centaur方法，通过测试时计算（test-time training）让自动驾驶系统在行驶过程中动态学习。该方法利用数据驱动方式，通过分析不确定性场景中的集群熵值，实时优化驾驶策略，摆脱对预设固定规则的依赖。相比传统自动驾驶系统在变道、加塞等场景中表现过于保守（易出现无故急刹/过度避让），新技术能像人类驾驶员一样主动适应复杂路况，显著提升系统在不确定性环境中的决策能力与安全性。这是首次将在线学习机制应用于自动驾驶实际测试阶段。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T00:04:01Z
- **目录日期**: 2025-03-31
