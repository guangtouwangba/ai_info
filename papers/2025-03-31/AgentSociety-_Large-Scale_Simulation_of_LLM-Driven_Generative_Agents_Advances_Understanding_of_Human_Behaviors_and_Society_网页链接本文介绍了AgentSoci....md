...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的互动（500万次）。它用于研究社会问题，如极化现象、信息传播、基本收入政策和外部冲击（如飓风）的影响。模拟结果与现实数据吻合，验证了其捕捉人类行为机制的能力。该系统可作为社会科学研究和政策制定的工具，辅助传统方法（调查、访谈等）分析社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T18:04:51Z
- **目录日期**: 2025-03-31
