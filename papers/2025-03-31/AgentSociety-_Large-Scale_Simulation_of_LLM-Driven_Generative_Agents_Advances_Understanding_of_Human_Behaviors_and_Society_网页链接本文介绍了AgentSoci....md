...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（超500万次）。研究者用它分析四个关键社会问题：群体极化、信息传播、基本收入政策效果和外部冲击（如飓风）影响。模拟结果与现实数据高度吻合，证明该系统能有效捕捉人类行为机制。该平台可作为社会科学研究的创新工具，辅助传统方法（如调查、实验），帮助学者和政策制定者深入理解社会现象及其成因。研究突显了LLM在社会仿真中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T07:03:43Z
- **目录日期**: 2025-03-31
