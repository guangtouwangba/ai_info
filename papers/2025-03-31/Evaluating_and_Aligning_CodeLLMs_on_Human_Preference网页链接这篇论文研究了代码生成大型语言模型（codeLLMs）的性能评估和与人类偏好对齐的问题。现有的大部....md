...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程样本，覆盖40个类别和44种语言，以评估模型在真实场景中的表现。同时开发了包含近200亿标记的SynCode-Instruct合成指令数据集。实验发现，传统基于执行的测试与人类偏好评估存在差距，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著，凸显了模型对齐人类偏好的重要性。研究为改进代码生成模型提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T21:05:08Z
- **目录日期**: 2025-03-31
