...
这篇论文研究了代码生成大模型(codeLLMs)与人类偏好的对齐问题。作者提出了CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种语言)和SynCode-Instruct指令数据集(近200亿标记)，用于评估模型在真实编程场景中的表现。实验发现，传统基于执行的测试与人类偏好存在差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)仍有显著差距。研究强调了代码生成模型不仅需要正确性，还需符合人类编程习惯和偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T14:06:11Z
- **目录日期**: 2025-03-31
