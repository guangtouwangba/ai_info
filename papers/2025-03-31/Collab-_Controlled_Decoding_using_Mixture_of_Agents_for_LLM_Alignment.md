...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）计算成本高，而受控解码可在推理时对齐模型但单代理方法难以适应多样任务。为此，研究者提出基于多代理协作的混合解码策略，动态选择最优LLM生成每个token。该方法通过长时效用指标从模型池中实时切换策略，理论分析证明其针对目标奖励函数的最优性。实验表明，Collab方法在开源对齐模型上显著优于单代理基线，平均奖励提升1.56倍，GPT-4胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T01:29:05Z
- **目录日期**: 2025-03-31
