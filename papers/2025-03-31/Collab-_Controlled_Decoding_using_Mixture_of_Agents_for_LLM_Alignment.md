...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而控制解码可在推理时对齐但单代理策略适应性差。本文提出多代理协作解码方法Collab，动态选择最优LLM生成每个token，通过长期效用指标实现模型间高效协作。理论分析证明其针对目标奖励函数的最优性。实验表明，Collab在开源对齐模型上超越单代理基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T00:01:47Z
- **目录日期**: 2025-03-31
