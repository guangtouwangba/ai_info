...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表面真实性（如单帧美学、时序一致性和基础提示遵循），但忽视了内在真实性（如物理法则、常识推理和解剖正确性）。为此，研究者提出VBench-2.0，通过自动评估模型在人类逼真度、可控性、创造力、物理合理性和常识性五个维度的表现，推动视频生成向内在真实性迈进。该框架结合通用视觉语言模型、专家方法（如异常检测）和人工标注，旨在为下一代视频生成模型设定新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T01:28:46Z
- **目录日期**: 2025-03-31
