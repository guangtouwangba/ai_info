...
本研究提出LatentSync框架，通过音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案创新性地采用时间表示对齐(TREPA)方法解决扩散模型的时间一致性问题，利用自监督视频模型增强生成帧的时序连贯性。同时针对SyncNet收敛问题，研究者系统分析了模型架构、训练参数等关键因素，改进了训练框架。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动的人像动画提供了新思路。研究成果对基于SyncNet的唇同步技术具有普适参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T11:07:08Z
- **目录日期**: 2025-03-31
