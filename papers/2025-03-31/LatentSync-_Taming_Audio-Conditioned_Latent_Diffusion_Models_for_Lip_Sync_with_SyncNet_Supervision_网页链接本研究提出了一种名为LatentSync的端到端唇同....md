...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该框架通过Stable Diffusion直接建模音频-视觉关系，解决了传统方法的时间一致性问题。研究创新包括：1）提出TREPA方法增强生成帧的时间一致性；2）改进SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-31T10:05:50Z
- **目录日期**: 2025-03-31
