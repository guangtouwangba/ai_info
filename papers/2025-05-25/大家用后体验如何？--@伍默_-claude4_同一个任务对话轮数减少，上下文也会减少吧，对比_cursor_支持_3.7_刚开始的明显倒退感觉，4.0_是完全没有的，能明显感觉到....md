...
用户对Claude-4的使用体验存在分歧。部分用户（如@伍默_）认为Claude-4在任务对话轮数和上下文处理上比Claude-3.7更可靠，尤其是在代码编写方面表现更好。然而，测试显示Claude-4的长上下文召回能力明显下降，可能影响其在复杂工程项目中的应用效果。一些用户担忧这会导致Claude-4在实际工程应用中的表现不如Claude-3.7。目前尚未有大规模使用的反馈，用户体验存在个体差异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T01:30:44Z
- **目录日期**: 2025-05-25
