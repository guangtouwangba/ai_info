...
该研究提出了一种基于扩散模型的虚拟试衣（VTON）新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改进细节保留。传统双分支UNet架构存在服装形变和细节丢失问题。新方法将服装细节表示为结构化语义点，通过局部流变形匹配目标人体上的对应点，并融合深度/法线图增强3D感知。这种对应关系模拟真实穿衣过程，3D线索监督扩散模型训练，配合专门设计的点聚焦扩散损失函数。在VITON-HD和DressCode数据集上取得最先进性能，显著提升了服装细节保持效果。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T15:02:40Z
- **目录日期**: 2025-05-25
