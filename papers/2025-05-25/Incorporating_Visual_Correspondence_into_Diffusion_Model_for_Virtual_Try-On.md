...
本文提出了一种基于扩散模型（Diffusion Model）的虚拟试衣（VTON）新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改进细节保留。该方法将服装细节表示为结构化语义点，并通过局部流变形匹配目标人物的语义点，同时结合深度/法线图增强为3D感知线索。这种对应关系模拟了穿衣过程，并通过点聚焦扩散损失优化训练。实验表明，该方法在VITON-HD和DressCode数据集上实现了最先进的性能，显著提升了服装形状和细节的保真度。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T06:02:21Z
- **目录日期**: 2025-05-25
