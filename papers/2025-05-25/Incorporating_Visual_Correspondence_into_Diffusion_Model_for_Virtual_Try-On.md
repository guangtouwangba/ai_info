...
该论文提出了一种基于扩散模型的虚拟试衣(VTON)新方法SPM-Diff，通过显式利用视觉对应关系作为先验来改进传统双分支UNet架构。关键创新是将服装细节表示为结构化语义点，通过局部流变形匹配目标人体的语义点，并增强为3D感知线索(深度/法线图)。该方法模拟真实穿衣过程，使用3D感知线索监督扩散模型训练，并设计了专注语义点的扩散损失函数。实验表明，该方法在VITON-HD和DressCode数据集上实现了最先进的性能，能更好地保持服装形状和细节。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T18:02:29Z
- **目录日期**: 2025-05-25
