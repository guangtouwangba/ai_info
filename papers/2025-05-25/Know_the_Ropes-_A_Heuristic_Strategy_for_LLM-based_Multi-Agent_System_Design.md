...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为分层算法蓝图来解决单一大语言模型(LLM)的局限性(如有限上下文、角色过载)。KtR递归分解任务为类型化子任务，结合轻量级增强技术(如思维链、微调)，避免传统多智能体方法的分解模糊和验证开销。实验显示：在背包问题(3-8物品)中，3个GPT-4o-mini智能体将准确率从3%(零样本)提升至95%；在更复杂的任务分配问题(6-15任务)中，6智能体系统在10规模以下达100%准确率，13-15规模达84%。该方法证明算法感知分解能有效提升小模型协作性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T11:02:21Z
- **目录日期**: 2025-05-25
