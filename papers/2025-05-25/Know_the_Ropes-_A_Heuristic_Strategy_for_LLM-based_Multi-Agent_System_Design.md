...
Know-The-Ropes (KtR) 是一个多智能体框架，旨在解决单一大语言模型（LLM）的局限性，如有限上下文、角色过载和领域迁移脆弱性。KtR 通过将领域先验转化为层次化算法蓝图，递归分解任务为类型化子任务，并由控制器协调解决。该方法避免盲目追求通用提示，转而采用结构化分解和轻量级增强（如思维链、微调或自检）。实验显示，在背包问题（3-8 项）中，3 个 GPT-4o-mini 智能体将准确率从 3% 提升至 95%；在任务分配问题（6-15 项）中，6 个智能体在较小规模任务上达到 100% 准确率。这表明算法感知分解结合针对性增强，可使轻量模型高效协作，无需依赖庞大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T05:02:11Z
- **目录日期**: 2025-05-25
