...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单智能体大模型的局限性（如有限上下文、角色过载和脆弱领域迁移）。KtR递归分解任务为类型化子任务，采用轻量级增强技术（如思维链、微调、自检）实现高效协作。实验表明：在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从零样本的3%提升至95%；在任务分配问题（6-15项）中，6智能体系统在10项以内达100%准确率。该方法证明算法感知分解结合定向增强可使轻量模型达成可靠协作，无需持续扩大模型规模。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T15:02:28Z
- **目录日期**: 2025-05-25
