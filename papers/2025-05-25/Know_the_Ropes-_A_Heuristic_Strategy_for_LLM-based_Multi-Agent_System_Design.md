...
Know-The-Ropes (KtR) 提出了一种多智能体框架，通过将领域先验转化为层次化算法蓝图来解决单一大模型的局限性（如有限上下文、角色过载）。该方法递归拆解任务为类型化子任务，结合轻量级增强技术（如思维链、微调），在背包问题（3-8物品）中使GPT-4o-mini的准确率从3%提升至95%；在任务分配问题（6-15作业）中，6智能体系统在10规模以下达100%准确率（零样本仅11%）。KtR证明：算法感知的任务分解结合精准增强，可使中等模型达成可靠协作，无需持续扩大单体模型规模。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T16:02:29Z
- **目录日期**: 2025-05-25
