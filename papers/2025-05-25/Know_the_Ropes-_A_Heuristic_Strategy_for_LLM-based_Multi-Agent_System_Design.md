...
大型语言模型（LLM）在单代理模式下存在上下文限制、角色过载和领域迁移脆弱性等问题。传统多代理方案虽能缓解这些问题，但会带来分解不当、模糊合约和验证开销等新问题。为此，研究团队提出Know-The-Ropes（KtR）框架，将领域先验转化为算法蓝图层次结构，通过递归分解任务并由控制器协调子任务，结合零样本推理或轻量级增强技术（如思维链、微调等）来解决问题。实验表明：在背包问题（3-8项）中，3个GPT-4o-mini代理将准确率从3%提升至95%；在任务分配问题（6-15项）中，6个代理的准确率达到100%（10项）和84%（13-15项）。该框架证明了算法感知分解结合针对性增强可使中小模型获得出色表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T20:02:35Z
- **目录日期**: 2025-05-25
