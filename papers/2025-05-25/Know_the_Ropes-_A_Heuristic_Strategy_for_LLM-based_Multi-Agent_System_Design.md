...
Know-The-Ropes (KtR) 是一个多智能体框架，旨在解决单一大语言模型(LLM)的局限性（如有限上下文、角色过载和领域迁移脆弱性）。KtR通过将领域先验转化为层次化的算法蓝图，递归分解任务为类型化的子任务，由控制器协调解决。该方法避免通用提示的追求，转而采用结构化分解。实验显示：在背包问题(3-8物品)中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题(6-15作业)中，6个o3-mini智能体实现100%(规模10)和84%(规模13-15)的准确率，远超零样本的11%。该框架证明算法感知分解结合定向增强可使中小模型成为可靠协作系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T06:02:11Z
- **目录日期**: 2025-05-25
