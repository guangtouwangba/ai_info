...
Know-The-Ropes (KtR) 是一个多智能体框架，通过将领域先验转化为分层算法蓝图，解决单一大语言模型(LLM)的局限性（如有限上下文、角色过载）。KtR递归分解任务为类型化子任务，采用轻量级增强技术（如思维链、微调）高效解决问题。实验显示：在背包问题（3-8物品）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15作业）中，6个o3-mini智能体实现100%（10作业）和84%（13-15作业）准确率，远超零样本11%。该框架证明算法感知分解结合精准增强可使小型模型高效协作，无需依赖巨型单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T00:02:20Z
- **目录日期**: 2025-05-25
