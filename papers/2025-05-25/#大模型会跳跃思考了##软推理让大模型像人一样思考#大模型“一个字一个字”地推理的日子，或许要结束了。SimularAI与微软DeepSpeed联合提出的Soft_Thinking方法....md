...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法通过引入"连续概念空间"和"概念token"，用概率分布替代单一token选择，实现多推理路径隐式聚合。相比传统CoT，软推理更灵活抽象，减少22.4% token使用量，使AI能像人类一样进行跳跃式思考。测试显示QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升6.45%。该方法还设计了Cold Stop机制防止模型"卡壳"，未来或成为大模型推理的标配方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T01:31:07Z
- **目录日期**: 2025-05-25
