...
SimularAI与微软DeepSpeed提出的"软推理"(Soft Thinking)方法突破传统思维链(CoT)限制，通过"连续概念空间"和"概念token"实现多路径隐式聚合。该方法用概率分布代替单一token选择，使大模型能像人类一样进行抽象跳跃思考，同时处理多种解题思路。实验显示该方法减少22.4% token使用量，QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升6.45%。配套的Cold Stop机制可监测模型自信度，动态调整推理过程。这种即插即用的创新方法有望成为未来大模型推理的标准配置。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T03:26:26Z
- **目录日期**: 2025-05-25
