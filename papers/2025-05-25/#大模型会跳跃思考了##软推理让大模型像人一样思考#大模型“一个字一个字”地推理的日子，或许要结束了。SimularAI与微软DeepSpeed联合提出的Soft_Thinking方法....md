...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)的限制。该方法通过引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多推理路径的隐式聚合。相比传统CoT，软推理更灵活抽象，减少22.4%的token使用量，同时提升推理准确率(QwQ-32B模型最高提升2.48%，AIME 2024数据集提升6.45%)。该方法还包含Cold Stop机制，可监测模型自信程度以判断停止时机。这种即插即用的软推理技术有望成为未来大模型的标准推理方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T19:04:22Z
- **目录日期**: 2025-05-25
