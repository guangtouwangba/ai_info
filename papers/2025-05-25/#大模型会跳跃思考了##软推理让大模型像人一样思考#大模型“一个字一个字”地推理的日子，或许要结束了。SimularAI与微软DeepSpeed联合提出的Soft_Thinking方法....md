...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多条推理路径隐式聚合，使AI能像人类一样进行抽象跳跃思考。实验显示，QwQ-32B模型准确率最高提升2.48%，AIME 2024数据集提升6.45%，同时减少22.4% token使用量。该方法还包含Cold Stop机制防止模型"卡壳"。这种即插即用的"软推理"有望成为未来大模型推理的标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T21:04:31Z
- **目录日期**: 2025-05-25
