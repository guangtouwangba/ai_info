...
SimularAI与微软DeepSpeed联合提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法创新性地引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多条推理路径隐式聚合。相比传统CoT，软推理更抽象灵活，减少22.4% token使用量，并提升模型准确率(最高提升6.45%)。该方法还设计了Cold Stop机制防止模型"卡壳"。测试显示软推理在效率和准确率上优于其他方法，未来或将成为大模型推理的标配方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T18:04:25Z
- **目录日期**: 2025-05-25
