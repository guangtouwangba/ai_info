...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，让大模型实现类人抽象思考。该方法创新性地引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，支持多推理路径隐式聚合。相比传统CoT，不仅减少22.4% token使用量，还能实现语义间平滑过渡。测试显示QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升6.45%。这种即插即用的推理方式结合Cold Stop防卡壳机制，有望成为未来大模型推理标配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T08:04:39Z
- **目录日期**: 2025-05-25
