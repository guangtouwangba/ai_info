...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制，让大模型实现类人抽象思考。该方法通过"连续概念空间"和"概念token"，用概率分布替代单一token选择，支持多路径隐式聚合，使推理更灵活高效。实验显示，QwQ-32B模型准确率最高提升2.48%，AIME数据集提升6.45%，同时减少22.4%的token使用。该方法还引入Cold Stop机制防止推理卡壳，支持即插即用，有望成为未来大模型推理的标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T09:04:25Z
- **目录日期**: 2025-05-25
