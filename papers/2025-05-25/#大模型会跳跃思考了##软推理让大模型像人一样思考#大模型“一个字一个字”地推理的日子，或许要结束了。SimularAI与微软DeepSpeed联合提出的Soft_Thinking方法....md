...
SimularAI与微软DeepSpeed提出"软推理"(Soft Thinking)方法，突破传统思维链(CoT)限制。该方法引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多路径隐式聚合。相比传统CoT，软推理更抽象灵活，减少22.4% token使用量，并支持类似人脑的跳跃思考。测试显示QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升6.45%。该方法还包含Cold Stop机制防止推理卡壳。这种即插即用的创新方法可能成为未来大模型推理的标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T23:03:58Z
- **目录日期**: 2025-05-25
