...
SimularAI与微软DeepSpeed联合提出的"软推理"(Soft Thinking)方法突破了传统思维链(CoT)的限制，通过引入"连续概念空间"和"概念token"，用概率分布代替单一token选择，实现多条推理路径的隐式聚合。这种方法使大模型能像人类一样进行抽象跳跃思考，同时减少22.4%的token使用量。测试显示QwQ-32B模型准确率最高提升2.48%，在AIME 2024数据集上提升6.45%。该方法还设计了Cold Stop机制防止推理卡壳，是一种即插即用的高效推理方案，有望成为未来大模型推理的标准方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T07:04:18Z
- **目录日期**: 2025-05-25
