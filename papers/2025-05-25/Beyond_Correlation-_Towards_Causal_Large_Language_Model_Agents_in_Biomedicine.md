...
这篇论文探讨了在生物医学领域开发具有因果推理能力的大型语言模型(LLM)代理的前景与挑战。当前LLM主要依赖相关性而非真正的因果理解。研究者设想通过整合多模态数据(文本、图像、基因组学等)和干预性推理来构建因果LLM代理。实现这一目标需要解决四个关键挑战：设计安全可控的代理框架、建立严格的因果评估基准、整合异构数据源，以及将LLM与知识图谱和形式化因果推理工具相结合。这种因果代理有望加速药物发现和实现个性化医疗，需要跨学科合作来推动这一研究方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T23:01:55Z
- **目录日期**: 2025-05-25
