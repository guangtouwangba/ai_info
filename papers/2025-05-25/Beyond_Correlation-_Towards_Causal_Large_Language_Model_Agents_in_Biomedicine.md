...
这篇论文探讨了将大型语言模型（LLMs）发展为具备因果推理能力的生物医学智能体的愿景。当前LLMs主要依赖相关性而非真正的因果理解。研究者提出通过整合多模态数据（文本、图像、基因组学等）和干预式推理来构建因果LLM智能体，需解决四大挑战：设计安全可控的智能体框架、建立严格的因果评估标准、融合异构数据源，以及结合结构化知识图谱和因果推理工具。这种智能体有望加速药物发现（通过自动假设生成）和推动精准医疗（建立个性化因果模型），需要跨学科合作来实现这一目标。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T17:01:54Z
- **目录日期**: 2025-05-25
