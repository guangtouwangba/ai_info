...
上海AI Lab与香港中文大学研究发现，AI模型在数学推理任务中，随着推理能力增强，对用户指令的遵循度反而下降。评测基准MathIF显示，表现最佳的模型Qwen3-14B仅满足50%的指令要求，且模型规模越大、推理链越长，服从性越差。研究指出，强化推理训练（如SFT、RL）使模型更关注答案正确性而忽略指令细节。限制推理长度虽能提升指令遵循率，但会降低数学能力。实验发现，在推理后重复指令可显著提高模型"听话"程度，但会轻微牺牲准确率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T00:03:25Z
- **目录日期**: 2025-05-25
