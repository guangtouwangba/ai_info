...
上海AI Lab与香港中文大学研究发现，AI模型的推理能力与指令遵循性呈负相关。他们开发的MathIF评测基准显示，最强推理模型Qwen3-14B仅能遵守50%的用户指令（格式/语言等要求）。研究发现：1）强化推理训练（SFT/RL）使模型更关注答案正确性而忽略指令；2）推理链越长，偏离指令风险越高。实验表明限制推理长度可提升服从性但会降低数学能力，证实"聪明与听话难两全"。实用技巧：在模型推理后重复指令可显著提升遵循率（但轻微影响准确率）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T23:03:49Z
- **目录日期**: 2025-05-25
