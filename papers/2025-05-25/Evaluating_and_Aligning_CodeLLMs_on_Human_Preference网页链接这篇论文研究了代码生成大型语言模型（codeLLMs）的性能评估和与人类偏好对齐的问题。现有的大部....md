...
这篇论文探讨了代码生成大型语言模型(codeLLMs)与人类偏好对齐的问题。研究者创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，以评估模型在真实编程任务中的表现。同时开发了包含近200亿标记的SynCode-Instruct合成指令数据集用于微调验证。实验发现，现有基于执行的评测与CodeArena存在性能差异，且开源顶级模型(如Qwen2.5-Coder)与专有模型(如OpenAI o1)之间存在显著差距，凸显了模型与人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T09:06:04Z
- **目录日期**: 2025-05-25
