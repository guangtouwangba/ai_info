...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建人类偏好评估基准CodeArena，包含397个涵盖40类编程任务和44种语言的真实样本，弥补现有基准忽视人类偏好的不足；2) 构建近200亿token的合成指令数据集SynCode-Instruct验证大规模微调效果。实验发现执行基准与人类偏好评估存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著，凸显对齐人类偏好的重要性。研究强调了代码生成模型需兼顾功能正确性和用户体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T11:06:09Z
- **目录日期**: 2025-05-25
