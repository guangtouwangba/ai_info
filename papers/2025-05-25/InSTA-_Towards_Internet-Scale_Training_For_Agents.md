...
研究人员开发了一种无需人工标注的网络导航智能体训练方法。该方法分三个阶段：首先用大语言模型(LLM)为15万个网站生成任务，然后用LLM代理完成任务并产生轨迹，最后用LLM过滤成功轨迹。实验表明，语言模型能有效筛选有害内容(准确率97%)和判断成功轨迹(82.6%)。基于Qwen 3 1.7B训练的智能体性能优异，最高成功率56.9%，超过了235倍大的Llama 4 Maverick，达到Gemini 2.5 Flash性能的94.7%。该方法显著降低了人工数据需求，相关代码、模型和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T10:02:38Z
- **目录日期**: 2025-05-25
