...
该研究提出了一种无需人工标注的大规模网络导航智能体训练方法。通过三阶段流程：1)大语言模型(LLM)自动标注15万个网站任务；2)LLM代理执行任务并生成轨迹；3)LLM过滤成功轨迹(准确率82.6%)。该方法训练出的17亿参数Qwen模型在网页导航任务中表现优异，成功率56.9%，超越2350亿参数的Llama 4模型，达到Gemini 2.5 Flash性能的94.7%。研究展示了语言模型作为高效数据筛选工具的能力(有害内容识别准确率97%)，并开源了代码、模型和数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T17:02:27Z
- **目录日期**: 2025-05-25
