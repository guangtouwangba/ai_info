...
该研究提出了一种无需人工标注的大规模网络导航智能体训练方法。通过三阶段流程：1)LLM自动标注15万个网站的任务；2)LLM智能体执行任务生成轨迹；3)LLM筛选成功轨迹(准确率82.6%)。训练出的17亿参数Qwen3模型在网页导航任务中表现优异，成功率56.9%，超越2350亿参数模型，达到Gemini 2.5 Flash性能的94.7%。该方法展示了LLM作为数据筛选工具的高效性(有害内容识别准确率97%)，实现了比人工标注更经济的智能体训练。相关代码、模型和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T05:02:33Z
- **目录日期**: 2025-05-25
