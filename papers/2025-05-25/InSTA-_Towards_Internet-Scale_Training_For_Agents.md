...
该研究提出了一种无需人工标注的互联网规模训练方法，利用大语言模型(LLM)自动完成三个关键步骤：1)为15万个网站生成代理任务；2)LLM代理执行任务并生成轨迹；3)LLM筛选成功轨迹(准确率82.6%)。基于Qwen 3 1.7B训练的网页导航代理表现出色，成功率56.9%，超越235倍大的Llama 4 Maverick，达到Gemini 2.5 Flash性能的94.7%。该方法展示了LLM作为高效数据筛选工具的能力(有害内容识别准确率97%)，并开源了代码、模型和数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T00:02:39Z
- **目录日期**: 2025-05-25
