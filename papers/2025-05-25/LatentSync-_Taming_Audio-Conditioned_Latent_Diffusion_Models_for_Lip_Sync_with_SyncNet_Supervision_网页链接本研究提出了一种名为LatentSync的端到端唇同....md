...
本研究提出LatentSync框架，通过音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方法利用Stable Diffusion直接建模音视频相关性，并针对扩散模型的时间一致性问题提出TREPA对齐技术。研究还改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)首个基于潜在扩散的唇同步框架；2)TREPA时间对齐方法；3)SyncNet训练优化方案。这些改进可推广至其他音视频同步任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T21:05:53Z
- **目录日期**: 2025-05-25
