...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型时间一致性问题，创新性提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时优化SyncNet训练框架，显著提升其准确性。在HDTF和VoxCeleb2数据集上，该方法在多个指标上超越现有技术。研究解决了扩散模型在唇同步应用中的关键挑战，为音频驱动人像动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T09:05:45Z
- **目录日期**: 2025-05-25
