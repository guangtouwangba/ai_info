...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。独立研究员Seen Heelan仅使用o3模型API（无需复杂工具）在Linux内核SMB实现的"注销"命令处理程序中识别出释放后使用漏洞。这是首个公开披露的由大语言模型发现的此类漏洞，OpenAI总裁已转发该成果。该发现过程仅通过分析代码实现，未借助传统漏洞检测工具，展示了AI在安全领域的突破性潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T16:03:29Z
- **目录日期**: 2025-05-25
