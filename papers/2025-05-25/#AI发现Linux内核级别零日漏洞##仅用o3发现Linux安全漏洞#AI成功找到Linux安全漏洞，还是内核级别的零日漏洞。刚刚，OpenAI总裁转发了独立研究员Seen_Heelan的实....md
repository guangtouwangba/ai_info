...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。独立研究员Seen Heelan仅使用o3模型API（无需复杂工具）在Linux内核SMB实现的"注销"命令处理程序中识别出释放后使用漏洞。这是首个公开报道的由大语言模型发现的此类漏洞，OpenAI总裁已转发该成果。研究显示o3模型通过反复分析12000行代码成功定位漏洞，标志着AI在代码审计领域取得突破性进展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T11:03:21Z
- **目录日期**: 2025-05-25
