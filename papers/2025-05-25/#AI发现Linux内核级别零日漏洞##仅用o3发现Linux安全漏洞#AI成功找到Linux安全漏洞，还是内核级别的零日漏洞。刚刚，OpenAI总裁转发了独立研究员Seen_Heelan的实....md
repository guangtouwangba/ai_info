...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。独立研究员Seen Heelan仅使用o3模型API（无需复杂工具）在Linux内核SMB实现的"注销"命令处理程序中识别出释放后使用漏洞。该漏洞存在于12000行代码中，经模型反复分析发现。OpenAI总裁转发了这一突破性成果，这是首个公开报道的由大语言模型发现的系统级漏洞，标志着AI在代码审计领域的重大进展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T21:03:00Z
- **目录日期**: 2025-05-25
