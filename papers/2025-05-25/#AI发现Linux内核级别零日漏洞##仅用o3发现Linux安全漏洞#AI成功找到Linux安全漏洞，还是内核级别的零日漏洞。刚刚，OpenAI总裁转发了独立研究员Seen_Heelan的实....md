...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。独立研究员Seen Heelan仅使用o3模型API（无需复杂工具）成功识别出SMB协议"注销"命令处理程序中的释放后使用漏洞。这是首个公开报道的由大语言模型发现的系统级漏洞，OpenAI总裁Greg Brockman已转发该成果。该突破显示AI在代码审计方面的新潜力，模型通过反复分析12,000行代码定位到该安全隐患。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T08:03:36Z
- **目录日期**: 2025-05-25
