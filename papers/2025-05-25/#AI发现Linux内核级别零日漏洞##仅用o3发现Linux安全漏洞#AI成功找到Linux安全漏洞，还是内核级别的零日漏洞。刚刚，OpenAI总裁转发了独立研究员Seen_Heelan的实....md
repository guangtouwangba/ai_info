...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。独立研究员Seen Heelan仅使用o3模型API，无需复杂工具，就检测出SMB协议"注销"命令处理程序中的use-after-free漏洞。该漏洞位于Linux内核SMB实现中，是首个公开披露的由大语言模型发现的重大安全漏洞。OpenAI总裁对此成果表示关注，突显了AI在代码审计领域的潜力。整个过程仅通过分析约12000行代码实现，未使用任何智能体框架或辅助工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T23:03:03Z
- **目录日期**: 2025-05-25
