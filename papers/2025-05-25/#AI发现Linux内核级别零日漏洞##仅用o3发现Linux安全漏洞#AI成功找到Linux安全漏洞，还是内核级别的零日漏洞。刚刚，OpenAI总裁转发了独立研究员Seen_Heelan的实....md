...
AI首次独立发现Linux内核级零日漏洞(CVE-2025-37899)，该漏洞存在于SMB"注销"命令处理程序中，属于释放后使用(use-after-free)类型。独立研究员Seen Heelan仅使用o3模型API（无需复杂工具）分析了12000行代码后成功识别该漏洞，这是首个公开披露的由大模型发现的重大安全漏洞。OpenAI总裁已转发该研究成果，突显了AI在代码审计方面的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T18:03:25Z
- **目录日期**: 2025-05-25
