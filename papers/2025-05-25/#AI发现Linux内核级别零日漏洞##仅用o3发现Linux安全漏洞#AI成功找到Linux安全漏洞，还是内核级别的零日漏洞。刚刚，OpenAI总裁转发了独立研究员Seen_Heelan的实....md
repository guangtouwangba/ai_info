...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。独立研究员Seen Heelan仅使用o3模型的API（未借助复杂工具）成功识别出SMB协议"注销"命令处理程序中的use-after-free漏洞。该漏洞存在于Linux内核SMB实现中，可能导致远程攻击。这是首个公开报道的由大语言模型发现的系统级安全漏洞，研究人员通过反复分析12,000行代码实现突破性发现。OpenAI总裁转发了该成果，突显AI在代码审计领域的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T15:03:30Z
- **目录日期**: 2025-05-25
