...
AI首次独立发现Linux内核级零日漏洞(CVE-2025-37899)。独立研究员Seen Heelan仅使用o3模型API(未借助其他工具)在Linux内核SMB实现的"注销"命令处理程序中识别出一个释放后使用漏洞。这是首个公开报道的由大语言模型发现的此类漏洞，展示了AI在代码审计方面的潜力。该发现获得OpenAI总裁转发关注，整个过程仅通过分析约12000行代码实现，无需传统漏洞挖掘工具或框架支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T10:03:21Z
- **目录日期**: 2025-05-25
