...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899）。研究员Seen Heelan仅使用o3模型API，无需复杂工具，成功识别出SMB协议"注销"命令处理程序中的释放后使用漏洞。该漏洞存在于12000行代码中，OpenAI总裁已转发该突破性成果。这是首个公开披露的由大语言模型发现的系统级安全漏洞，标志着AI在代码审计领域的重大进展。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-25T17:03:12Z
- **目录日期**: 2025-05-25
