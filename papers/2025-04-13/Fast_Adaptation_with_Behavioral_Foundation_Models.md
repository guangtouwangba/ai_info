...
这篇论文探讨了无监督零样本强化学习（RL）中行为基础模型（BFMs）的快速适应策略。虽然BFMs能通过自监督任务嵌入实现零样本策略，但由于训练和推理误差，这些策略往往不够理想。研究发现，BFMs学到的技能中其实包含比推理结果更优的策略，适合快速适应。为此，作者提出了基于任务嵌入空间的演员-评论家和纯演员适应策略，可在少量环境交互中显著提升零样本性能，避免了预训练模型常见的初始性能下降问题。实验表明，该方法在导航和运动任务中仅需几十次迭代即可提升10-40%的性能，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T06:01:15Z
- **目录日期**: 2025-04-13
