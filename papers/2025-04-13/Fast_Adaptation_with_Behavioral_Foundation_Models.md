...
这篇论文提出了一种快速适应策略，用于提升无监督零样本强化学习（RL）中行为基础模型（BFMs）的性能。研究发现，现有BFMs学习的技能集包含比其推理过程识别出的更优策略，因此适合快速适应。作者提出了基于行动者-评论家和纯行动者的低维任务嵌入空间搜索方法，能在少量环境交互步骤内提升零样本策略表现，同时避免适应初期的性能下降。在多个导航和运动领域的实验表明，该方法在几十次训练周期内可使零样本性能提升10-40%，优于现有基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T04:27:58Z
- **目录日期**: 2025-04-13
