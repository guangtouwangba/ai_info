...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFMs），使智能体无需额外学习即可解决多种任务。然而，由于训练和推理误差，零样本策略往往表现不佳。研究发现，BFMs已学习到比推理结果更优的技能策略，适合快速适应。为此，研究者提出基于任务嵌入空间的快速适应策略（包括行动者-评论家和纯行动者方法），可在少量环境交互中提升零样本性能，避免预训练模型的初始性能下降。实验表明，该方法在导航和运动任务中仅需数十次交互即可提升10-40%性能，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T17:01:24Z
- **目录日期**: 2025-04-13
