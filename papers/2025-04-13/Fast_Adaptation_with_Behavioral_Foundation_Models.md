...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFM），使智能体无需额外学习即可解决多种下游任务。然而，由于训练误差和推理过程，零样本策略往往表现不佳。研究发现，BFM已学习到比推理结果更优的策略，适合快速适应。为此，研究者提出基于任务嵌入空间的快速适应策略（包括行动者-评论家和纯行动者方法），在少量环境交互中提升零样本性能，避免了预训练模型的初始性能下降问题。实验表明，该方法在导航和运动任务中仅需数十次交互即可将性能提升10-40%，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T00:25:17Z
- **目录日期**: 2025-04-13
