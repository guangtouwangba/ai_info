...
无监督零样本强化学习（RL）通过预训练行为基础模型（BFMs），使智能体能零样本解决多种下游任务（即无需额外学习或规划）。然而，由于训练、嵌入和推理过程的误差，零样本策略常表现不佳。本文提出快速适应策略，在少量环境交互中提升BFMs的零样本性能，同时避免适应过程中的性能下降。研究发现，BFMs学习的技能集中包含比推理结果更优的策略，适合快速适应。基于此，作者提出了基于任务嵌入空间的行动者-评论家和纯行动者适应策略，在导航和运动任务中实现了10-40%的性能提升，优于现有基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T11:01:16Z
- **目录日期**: 2025-04-13
