...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模复杂音视频关系，并针对扩散过程中时间一致性问题提出TREPA技术（通过自监督视频模型对齐生成帧）。研究还改进了SyncNet训练框架，解决了其收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）首个潜在扩散唇同步框架 2）TREPA时间一致性增强 3）SyncNet训练优化方案，这些改进可推广至其他音视频同步任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T04:32:01Z
- **目录日期**: 2025-04-13
