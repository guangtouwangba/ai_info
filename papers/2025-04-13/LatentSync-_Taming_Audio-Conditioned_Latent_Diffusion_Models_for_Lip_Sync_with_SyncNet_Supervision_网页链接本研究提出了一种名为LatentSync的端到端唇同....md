...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，通过自监督视频模型的时间表征对齐生成帧。研究还系统分析了SyncNet收敛问题，改进其训练框架显著提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上多项指标超越现有技术。创新点包括：1）首个基于潜在扩散的唇同步框架；2）TREPA时间一致性增强；3）SyncNet训练优化方案。这些发现对音频驱动人像动画领域具有广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T12:05:33Z
- **目录日期**: 2025-04-13
