...
本文提出了"思维双引擎"(DEoT)框架，专为开放性问题设计，突破了传统推理框架局限于单一答案的局限。该框架包含三个核心组件：优化用户查询的基础提示器、协调任务分解与验证的求解代理，以及由广度引擎(探索多元影响因素)和深度引擎(进行深入调查)构成的双引擎系统。实验表明，DEoT在解决复杂多面问题时表现优异，相比现有推理模型取得77-86%的胜率。其可定制化设计允许用户调整分析参数，实现了广泛覆盖与深度分析的平衡，在现实应用中展现出显著优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T08:01:54Z
- **目录日期**: 2025-04-13
