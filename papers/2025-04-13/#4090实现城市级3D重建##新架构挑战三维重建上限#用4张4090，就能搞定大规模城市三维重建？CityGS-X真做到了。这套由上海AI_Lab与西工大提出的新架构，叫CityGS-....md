...
上海AI Lab与西工大提出CityGS-X新架构，采用并行化混合分层三维表征(PH²-3D)，突破传统"分块+合并"方法。仅需4张4090显卡即可处理5000+图像的大规模城市重建，训练速度翻倍且显存更省。该架构通过K层并行体素建模、动态分配与高斯共享优化显存，结合多任务并行渲染，在RGB渲染和几何精度上达到SOTA水平。关键技术包括图块任务分配、高斯属性迁移及法线/深度图协同优化，有效避免视角过拟合，实现高效精细的三维重建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T05:09:16Z
- **目录日期**: 2025-04-13
