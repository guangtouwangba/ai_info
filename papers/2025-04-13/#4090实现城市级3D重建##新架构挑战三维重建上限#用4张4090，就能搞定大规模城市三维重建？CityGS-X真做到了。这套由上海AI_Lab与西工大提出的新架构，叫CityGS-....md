...
上海AI Lab与西工大提出新架构CityGS-X，通过并行化混合分层三维表征(PH²-3D)实现高效城市级3D重建。该技术突破传统方法，采用批处理+多任务并行渲染，仅需4张4090显卡即可处理5000+图像的大场景。关键优势包括：训练速度翻倍、显存占用优化、RGB渲染与几何精度达SOTA水平。模型采用K层分层体素并行处理，结合动态体素分配与高斯属性共享，有效控制显存。通过图块任务分配、高斯属性迁移及法线/深度图协同优化，解决了视角过拟合问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T07:03:12Z
- **目录日期**: 2025-04-13
