...
上海AI Lab与西工大提出新架构CityGS-X，采用并行化混合分层三维表征（PH²-3D），突破传统"分块+合并"方法。仅需4张4090显卡即可处理5000+图像的大规模城市重建，训练速度翻倍且显存占用更低。其核心通过K层分层体素并行处理、动态分配与高斯共享优化显存，并利用多任务批处理渲染提升RGB与几何精度至SOTA水平。模型支持图块任务分配、属性迁移及法线/深度协同优化，有效避免视角过拟合，实现高效精细的三维重建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T23:03:11Z
- **目录日期**: 2025-04-13
