...
上海AI Lab与西工大联合提出CityGS-X架构，采用并行化混合分层三维表征（PH²-3D），突破传统"分块+合并"方法。该技术通过批处理+多任务并行渲染，仅需4张RTX 4090即可处理5000+图像的大规模城市重建，训练速度翻倍且显存更省。其K层分层体素建模支持并行处理，结合动态体素分配与高斯属性共享，在RGB渲染和几何精度上均达SOTA水平。创新性地采用法线/深度图协同优化与批处理级训练，有效避免了视角过拟合问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T11:03:10Z
- **目录日期**: 2025-04-13
