...
上海AI Lab与西工大联合提出CityGS-X架构，采用并行化混合分层三维表征(PH²-3D)，突破传统城市级3D重建限制。该技术仅需4张4090显卡即可处理5000+图像的大规模场景，训练速度提升2倍且显存占用更低。通过K层分层体素并行处理、动态分配与高斯共享机制，在RGB渲染和几何精度上均达到SOTA水平。其创新点包括：批处理多任务并行渲染、法线/深度图协同优化，有效避免视角过拟合，实现了高效、高精度的城市三维重建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T09:03:20Z
- **目录日期**: 2025-04-13
