...
上海AI Lab与西工大提出CityGS-X架构，采用并行化混合分层三维表征（PH²-3D），突破传统分块合并方法。通过批处理+多任务并行渲染，仅需4张4090显卡即可高效处理5000+图像的大规模城市重建，训练速度翻倍且显存占用更低。其K层分层体素建模支持并行处理，结合动态体素分配与高斯属性共享，显著提升RGB渲染与几何精度至SOTA水平。模型还通过法线/深度图协同优化与批处理策略，有效避免视角过拟合，实现高效精细的3D重建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T06:03:10Z
- **目录日期**: 2025-04-13
