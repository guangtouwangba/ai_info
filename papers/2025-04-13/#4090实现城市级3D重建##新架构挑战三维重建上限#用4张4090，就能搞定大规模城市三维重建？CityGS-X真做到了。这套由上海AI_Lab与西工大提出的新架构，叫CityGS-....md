...
上海AI Lab与西工大联合提出的CityGS-X架构，通过并行化混合分层三维表征(PH²-3D)技术，实现了高效的大规模城市3D重建。该方案突破传统方法，采用批处理+多任务并行渲染，仅需4张4090显卡即可处理5000+图像的大场景。关键优势包括：训练速度提升2倍，RGB渲染和几何精度达到SOTA水平；通过K层分层体素建模、动态分配与高斯属性共享优化显存使用；采用多视角协同优化避免过拟合。这一新架构显著降低了城市级3D重建的硬件门槛和计算成本。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T13:08:02Z
- **目录日期**: 2025-04-13
