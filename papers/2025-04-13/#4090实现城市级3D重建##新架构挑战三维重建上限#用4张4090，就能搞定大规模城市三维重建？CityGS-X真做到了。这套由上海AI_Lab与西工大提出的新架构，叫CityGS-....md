...
上海AI Lab与西工大联合提出CityGS-X架构，通过并行化混合分层三维表征（PH²-3D）技术实现高效城市级3D重建。该方案摒弃传统分块合并方法，采用批处理+多任务并行渲染，仅需4张RTX 4090即可处理超5000张图像的大场景。核心创新包括：K层分层体素并行建模、动态体素分配与高斯属性共享显存优化。实验显示其训练速度翻倍，RGB渲染与几何精度均达SOTA水平，并通过法线/深度图协同优化有效避免视角过拟合。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T08:03:06Z
- **目录日期**: 2025-04-13
