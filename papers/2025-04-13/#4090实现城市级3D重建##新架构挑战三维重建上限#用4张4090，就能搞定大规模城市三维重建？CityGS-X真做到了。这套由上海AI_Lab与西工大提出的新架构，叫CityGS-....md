...
上海AI Lab与西工大联合提出CityGS-X架构，通过并行化混合分层三维表征(PH²-3D)技术实现高效城市级3D重建。该方案突破传统分块方法，采用批处理+多任务并行渲染，仅需4张RTX 4090即可处理5000+图像的大场景。关键技术包括：K层分层体素并行建模、动态体素分配与高斯属性共享，在训练速度(提升2倍)、显存效率和重建精度(RGB渲染/几何还原)均达SOTA水平。其创新设计实现了多视角协同优化，有效解决了大场景重建中的过拟合问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T16:03:11Z
- **目录日期**: 2025-04-13
