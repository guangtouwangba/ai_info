...
本文介绍了AgentSociety，这是一个基于大型语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（共500万次）。研究者利用该平台研究了极化现象、信息传播、基本收入政策及外部冲击（如飓风）等社会问题。模拟结果与现实世界数据高度吻合，验证了该工具在揭示人类行为机制和社会动态方面的有效性。AgentSociety展现了作为社会科学研究和政策制定辅助工具的潜力，可补充传统研究方法（如调查和实验）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T18:03:28Z
- **目录日期**: 2025-04-13
