...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟10,000+代理的社会互动（500万次）。研究者用它分析四大社会问题：社会极化、信息传播、基本收入政策及自然灾害影响。模拟结果与现实数据高度吻合，验证了其捕捉人类行为机制的能力。该系统可作为计算社会科学实验平台，为研究者和政策制定者提供新工具，补充传统研究方法（如调查/访谈）。研究证明LLM驱动的模拟能有效揭示社会问题的模式、成因和潜在机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T20:04:45Z
- **目录日期**: 2025-04-13
