...
大型语言模型(LLMs)在推动AI进步的同时，其嵌入的偏见问题日益引发关注。这些偏见源于训练数据的不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。本研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化等方法，系统评估LLMs对抗偏见诱导的鲁棒性。研究发现模型规模与安全性存在关键权衡，并揭示了领域专用模型(如医疗领域)的安全漏洞。研究发布了包含偏见提示的基准数据集，为开发更公平、更鲁棒的LLMs提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T09:01:39Z
- **目录日期**: 2025-04-13
