...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了关于偏见问题的担忧。这些偏见源于训练数据中的历史不平等、语言不平衡等因素。研究提出了一种可扩展的评估框架，通过多任务测试、安全评分量化及越狱技术检测，系统评估LLMs对抗偏见诱导的鲁棒性。分析揭示了模型大小与安全性之间的关键权衡，并考察了医疗等专业领域微调模型的安全性。研究发布了CLEAR-Bias数据集以支持系统性漏洞评估，为开发更公平、更鲁棒的语言模型提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T13:06:16Z
- **目录日期**: 2025-04-13
