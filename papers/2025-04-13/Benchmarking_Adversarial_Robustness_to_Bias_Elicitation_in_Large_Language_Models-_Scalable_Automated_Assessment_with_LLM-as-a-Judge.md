...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡和对抗性操纵。研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化(采用LLM-as-a-Judge方法)和越狱技术，系统评估模型对抗偏见诱导的鲁棒性。分析揭示了模型大小与安全性之间的关键权衡，并考察了医疗等关键领域专用模型的安全性问题。研究发布了包含偏见诱导提示的基准数据集，为开发更公平、更鲁棒的语言模型提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T04:28:09Z
- **目录日期**: 2025-04-13
