...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用中的潜在偏见问题引发关注。这些偏见源于训练数据不平等、语言不平衡等因素，且现有模型仍易受对抗性攻击诱导偏见。研究提出可扩展的评估框架CLEAR-Bias，通过多任务测试、安全评分量化及越狱技术检测，系统评估模型在不同社会文化维度的偏见表现。分析发现模型规模与安全性之间存在关键权衡，领域专用模型(如医疗)也存在安全隐患。研究发布了标准化的偏见提示数据集，为开发更公平、鲁棒的模型提供基准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T14:01:30Z
- **目录日期**: 2025-04-13
