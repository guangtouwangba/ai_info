...
大型语言模型(LLMs)在推动AI进步的同时，其内置偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言失衡等因素，可能导致刻板印象和公平性问题。研究提出一个可扩展的评估框架，通过多任务方法系统探测模型在不同社会文化维度的偏见表现，采用LLM-as-a-Judge方法自动量化模型安全性，并利用越狱技术测试安全机制的脆弱性。分析揭示了模型大小与安全性之间的关键权衡，并评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias数据集以支持系统性漏洞评估，为开发更公平、鲁棒的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T19:01:18Z
- **目录日期**: 2025-04-13
