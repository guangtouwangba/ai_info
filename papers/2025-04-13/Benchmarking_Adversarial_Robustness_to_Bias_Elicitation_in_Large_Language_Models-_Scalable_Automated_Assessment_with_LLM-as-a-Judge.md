...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据中的历史不平等、语言不平衡等因素，可能导致刻板印象和公平性问题。研究提出了一种可扩展的评估框架，通过多任务探测模型偏见、量化安全评分和使用越狱技术检测漏洞，系统评估LLMs对抗偏见诱导的鲁棒性。分析揭示了不同规模模型中的普遍偏见及其对安全性的影响，并评估了医疗等关键领域专用模型的安全性。研究发现模型规模与安全性之间存在关键权衡，研究还发布了CLEAR-Bias数据集以促进系统性漏洞评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T02:58:38Z
- **目录日期**: 2025-04-13
