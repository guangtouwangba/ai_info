...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用也引发了偏见问题。该研究提出可扩展的评估框架，通过多任务测试、自动安全评分和越狱技术，系统评估LLMs对抗偏见诱导的鲁棒性。研究发现：1)不同规模的前沿模型普遍存在偏见；2)模型大小与安全性存在权衡关系；3)医疗等专业领域微调模型也存在安全隐患。研究发布了CLEAR-Bias偏见提示数据集，为系统性漏洞评估提供支持。这些发现有助于开发更公平、更鲁棒的下一代语言模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T16:01:32Z
- **目录日期**: 2025-04-13
