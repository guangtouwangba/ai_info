...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用引发了关于偏见的担忧。这些偏见源于训练数据中的不平等、语言失衡和对抗性操纵。本研究提出可扩展的评估框架，通过多任务测试、安全评分和越狱技术来检测模型对抗偏见诱导的鲁棒性。研究发现模型大小与安全性之间存在关键权衡，并评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias偏见提示数据集，为系统化漏洞评估提供支持。这些发现有助于开发更公平、更鲁棒的未来语言模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T17:01:34Z
- **目录日期**: 2025-04-13
