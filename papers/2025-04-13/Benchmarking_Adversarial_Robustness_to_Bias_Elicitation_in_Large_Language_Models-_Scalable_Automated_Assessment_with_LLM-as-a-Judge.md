...
大型语言模型(LLMs)在推动AI进步的同时，其嵌入的偏见问题日益引发关注。这些偏见源于训练数据的不平等、语言失衡等因素，可能导致刻板印象和公平性问题。研究提出了一种可扩展的评估框架，通过多任务探测模型偏见、量化稳健性评分、利用越狱技术测试安全机制漏洞等方法，系统评估LLMs对抗偏见诱导的鲁棒性。分析揭示了模型规模与安全性之间的关键权衡，并评估了医疗等关键领域专用模型的安全性。研究发布了CLEAR-Bias偏见提示数据集，为开发更公平、更鲁棒的模型提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T20:01:23Z
- **目录日期**: 2025-04-13
