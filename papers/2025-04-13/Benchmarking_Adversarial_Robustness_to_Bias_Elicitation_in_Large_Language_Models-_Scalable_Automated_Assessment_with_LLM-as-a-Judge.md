...
大型语言模型(LLMs)在推动AI进步的同时，其社会应用引发了关于偏见问题的担忧。这些偏见源于训练数据的不平等、语言不平衡等因素，且模型易受对抗性攻击触发偏见响应。本研究提出可扩展的评估框架：(1)多任务测试社会文化维度偏见；(2)采用LLM-as-a-Judge自动评分量化稳健性；(3)利用越狱技术检测安全机制漏洞。分析揭示了模型规模与安全性间的关键权衡，并评估了医疗等专业领域模型的偏见问题。研究发布了CLEAR-Bias数据集以支持系统性漏洞评估，为开发更公平、稳健的模型提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T01:47:44Z
- **目录日期**: 2025-04-13
