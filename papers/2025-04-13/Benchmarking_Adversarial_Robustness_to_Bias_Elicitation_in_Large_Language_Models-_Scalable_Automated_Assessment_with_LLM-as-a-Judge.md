...
大型语言模型（LLMs）在推动AI进步的同时，其潜在偏见问题引发社会担忧。这些偏见源于训练数据中的历史不平等、语言不平衡和对抗性操纵。研究提出可扩展的评估框架，通过多任务测试、安全评分量化（采用LLM-as-a-Judge方法）和越狱技术，系统分析模型对抗偏见诱导的鲁棒性。研究发现模型大小与安全性存在关键权衡，并评估了医疗等关键领域微调模型的安全性。研究发布CLEAR-Bias数据集以促进系统性漏洞评估，为开发更公平、鲁棒的模型提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T03:52:56Z
- **目录日期**: 2025-04-13
