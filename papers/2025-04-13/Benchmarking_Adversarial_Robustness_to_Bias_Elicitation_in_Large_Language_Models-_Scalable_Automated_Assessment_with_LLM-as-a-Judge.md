...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。研究提出了一种可扩展的评估框架，通过多任务测试、安全评分量化及越狱技术检测，系统评估LLMs对抗偏见诱导的鲁棒性。分析发现，不同规模的先进模型均存在偏见问题，且模型大小与安全性之间存在权衡关系。研究还评估了医疗等关键领域专用模型的安全性，并发布了CLEAR-Bias数据集以支持系统性漏洞评估。这些发现为开发更公平、更鲁棒的语言模型提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T22:01:26Z
- **目录日期**: 2025-04-13
