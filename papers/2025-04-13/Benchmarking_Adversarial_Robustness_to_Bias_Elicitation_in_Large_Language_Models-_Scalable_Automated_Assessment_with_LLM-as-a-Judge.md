...
大型语言模型(LLMs)在推动AI进步的同时，其潜在偏见问题日益引发关注。这些偏见源于训练数据中的不平等、语言不平衡和对抗性操纵。研究提出可扩展的评估框架CLEAR-Bias，通过多任务方法系统探测模型在不同社会文化维度的偏见表现，采用LLM-as-a-Judge自动评分量化模型稳健性，并利用越狱技术检测安全机制漏洞。分析发现模型大小与安全性存在关键权衡，领域专用模型(如医疗)也存在安全隐患。该研究发布了包含偏见提示的CLEAR-Bias数据集，为开发更公平、稳健的语言模型提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T11:01:27Z
- **目录日期**: 2025-04-13
