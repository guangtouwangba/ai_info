...
Meta的Llama 4大模型因"作弊"事件引发争议，重新上线非特供版后排名从第2暴跌至第32名。此前Meta使用了未公开优化方式的"实验版"模型，替换为HuggingFace同款后引发社区信任危机。实测显示，Llama 4在带宽受限设备（如Mac Studio）和大型RAG任务中表现较好，但准确率和任务处理能力仍不如DeepSeek v3。后者在代码、推理、文本创作（尤其是解释和故事设计）方面更优。开发者批评Meta不尊重基准测试，导致社区对其信任度大幅下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T14:03:26Z
- **目录日期**: 2025-04-13
