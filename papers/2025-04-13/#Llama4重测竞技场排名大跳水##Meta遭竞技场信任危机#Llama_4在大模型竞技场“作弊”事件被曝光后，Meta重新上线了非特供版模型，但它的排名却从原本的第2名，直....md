...
Meta的Llama 4模型因被曝在竞技场测试中使用"特供优化版"而引发争议。重新上线标准版后，其排名从第2名暴跌至第32名，导致社区对Meta的信任危机。虽然Llama 4在特定硬件环境下表现良好（如内存充足但带宽受限的设备），并在大型RAG任务中速度占优，但其准确率和综合能力仍不如DeepSeek v3。实测显示，DeepSeek在代码、推理和文本创作（特别是解释能力和故事设计）方面表现更优。这一事件暴露了Meta在基准测试透明度方面的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T15:03:23Z
- **目录日期**: 2025-04-13
