...
这篇论文探讨了代码生成大模型（CodeLLMs）与人类偏好的对齐问题。研究发现现有基准测试过于关注代码正确性而忽视人类偏好，因此提出了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令数据集（近200亿标记）。实验显示执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显人类偏好对齐的重要性。研究验证了大规模合成指令微调的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T07:05:46Z
- **目录日期**: 2025-04-13
