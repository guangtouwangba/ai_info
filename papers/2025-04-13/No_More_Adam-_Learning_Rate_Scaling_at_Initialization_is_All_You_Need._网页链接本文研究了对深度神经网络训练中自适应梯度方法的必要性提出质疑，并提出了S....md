...
本文提出SGD-SaI优化方法，质疑自适应梯度方法(如Adam)的必要性。该方法改进带动量的SGD，通过在初始化时根据梯度信噪比(g-SNR)调整不同参数组的学习率，无需二阶动量即可避免训练不平衡。相比AdamW，内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，并显著节省内存(GPT-2节省5.93GB，Llama2-7B节省25.15GB)。在LoRA微调等任务中也优于现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T09:04:45Z
- **目录日期**: 2025-04-13
