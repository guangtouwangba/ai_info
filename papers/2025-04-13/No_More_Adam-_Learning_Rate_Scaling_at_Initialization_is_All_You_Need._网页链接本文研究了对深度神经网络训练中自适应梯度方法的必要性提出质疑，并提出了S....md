...
该研究提出SGD-SaI优化方法，通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，取代了自适应梯度方法(如Adam)。相比AdamW，SGD-SaI内存消耗减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，解决了传统SGD训练Transformer的难题。在完整精度训练中，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，且在LoRA微调等任务中优于现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T20:04:12Z
- **目录日期**: 2025-04-13
