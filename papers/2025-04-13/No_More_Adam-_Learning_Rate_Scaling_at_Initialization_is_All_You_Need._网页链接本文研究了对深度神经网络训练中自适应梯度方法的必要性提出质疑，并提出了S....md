...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI是对带动量的随机梯度下降(SGDM)的改进，通过初始化时依据梯度信噪比(g-SNR)调整不同参数组的学习率。相比AdamW，该方法无需二阶动量计算，内存使用减半，且在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现相当或更好。实验显示，SGD-SaI为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，具有超参数鲁棒性和广泛应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-13T12:04:34Z
- **目录日期**: 2025-04-13
