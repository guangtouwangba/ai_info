...
该研究提出了一种基于大型语言模型（LLM）的运动预测系统，旨在提高外骨骼在动态建筑环境中的辅助性能。系统通过智能眼镜的语音指令和视觉数据等多模态输入，结合感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块进行运动模式预测。实验显示，加入记忆系统后性能显著提升：加权F1分数从0.73（无记忆）升至0.81（仅STM）和0.90（STM+LTM），在模糊和安全关键指令中表现尤佳。校准指标（Brier分数从0.244降至0.090）验证了系统可靠性，为动态行业中人机协作提供了安全解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T03:15:08Z
- **目录日期**: 2025-04-22
