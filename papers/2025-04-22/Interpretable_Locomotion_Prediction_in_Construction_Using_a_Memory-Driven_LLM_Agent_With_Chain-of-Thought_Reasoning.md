...
该研究提出了一种基于大语言模型（LLM）的运动预测系统，用于提高外骨骼在动态施工环境中的辅助能力。系统通过智能眼镜的语音指令和视觉数据等多模态输入，结合感知模块、短期记忆（STM）、长期记忆（LTM）和优化模块，准确预测运动意图。测试显示，加入记忆系统后性能显著提升：加权F1分数从0.73（无记忆）升至0.81（仅STM）和0.90（STM+LTM），在模糊和安全关键指令中表现尤佳。校准指标（Brier分数从0.244降至0.090）证实了系统可靠性，为动态行业的人机协作提供了安全适配方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T08:01:14Z
- **目录日期**: 2025-04-22
