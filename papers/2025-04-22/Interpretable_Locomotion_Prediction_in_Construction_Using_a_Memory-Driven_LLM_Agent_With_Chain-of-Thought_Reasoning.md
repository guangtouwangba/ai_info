...
该研究提出了一种基于大型语言模型（LLM）的运动预测智能体，通过结合记忆系统提升外骨骼在动态施工环境中的辅助能力。系统采用多模态输入（语音指令和智能眼镜视觉数据），整合感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块进行运动模式预测。实验显示，加入STM后加权F1分数从0.73升至0.81，结合LTM后达0.90，尤其在模糊和安全关键指令中表现优异。校准指标显著改善（Brier分数从0.244降至0.090），验证了系统可靠性，为动态行业中的自适应辅助系统提供了安全高效的人机协作方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T22:01:12Z
- **目录日期**: 2025-04-22
