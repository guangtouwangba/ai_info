...
该研究提出了一种基于大语言模型（LLM）和记忆系统的运动预测智能体，用于提升外骨骼在动态施工环境中的辅助性能。通过整合语音指令和智能眼镜的视觉数据，该系统包含感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块。实验表明，加入记忆系统后性能显著提升：加权F1分数从基线0.73（无记忆）增至0.81（仅STM）和0.90（STM+LTM），在模糊和安全关键指令中表现尤佳。校准指标（Brier分数从0.244降至0.090，ECE从0.222降至0.044）证实了系统可靠性。该框架为动态行业中的自适应辅助系统提供了安全解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T04:01:11Z
- **目录日期**: 2025-04-22
