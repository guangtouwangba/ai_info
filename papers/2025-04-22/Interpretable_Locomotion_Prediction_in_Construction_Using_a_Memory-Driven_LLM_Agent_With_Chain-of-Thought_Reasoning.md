...
该研究提出了一种基于大语言模型（LLM）的运动预测智能体，通过结合记忆系统提升外骨骼在动态施工环境中的辅助能力。系统采用多模态输入（语音指令和智能眼镜视觉数据），整合感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块进行运动模式预测。实验显示，加入记忆系统后性能显著提升：加权F1分数从基线0.73（无记忆）升至0.81（仅STM）和0.90（STM+LTM），在模糊和安全关键指令中表现优异。校准指标（Brier分数从0.244降至0.090，ECE从0.222降至0.044）验证了系统可靠性，为动态行业的人机协作提供了安全解决方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T05:01:13Z
- **目录日期**: 2025-04-22
