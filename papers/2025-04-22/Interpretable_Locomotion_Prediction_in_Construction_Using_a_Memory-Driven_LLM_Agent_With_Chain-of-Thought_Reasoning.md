...
该研究提出了一种基于大语言模型（LLM）的运动预测智能体，用于提升外骨骼在动态建筑环境中的辅助性能。系统通过智能眼镜的语音指令和视觉数据等多模态输入，整合感知模块、短时记忆（STM）、长时记忆（LTM）和优化模块来预测运动模式。实验表明，加入记忆系统后性能显著提升：加权F1分数从0.73（无记忆）增至0.81（仅STM）和0.90（STM+LTM），在模糊和安全关键指令中表现尤佳。校准指标（Brier分数从0.244降至0.090，ECE从0.222降至0.044）验证了可靠性改进，为动态行业中人机协作提供了更安全的自适应辅助方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T19:01:15Z
- **目录日期**: 2025-04-22
