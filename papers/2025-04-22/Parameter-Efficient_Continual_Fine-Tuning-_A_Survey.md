...
这篇摘要探讨了大规模预训练模型在持续学习(CL)中的挑战与机遇。虽然预训练模型取得了突破性进展，但其依赖i.i.d.假设的特性限制了在动态环境中的适应性。作者指出，参数高效微调(PEFT)方法虽能有效适应特定数据，但仍面临灾难性遗忘问题。文章综述了CL算法、PEFT方法以及参数高效持续微调(PECFT)的最新进展，分析了不同方法、评估指标，并展望了未来研究方向。旨在促进CL与参数高效微调的协同发展，为研究者提供指导并开拓新的研究路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T00:01:53Z
- **目录日期**: 2025-04-22
