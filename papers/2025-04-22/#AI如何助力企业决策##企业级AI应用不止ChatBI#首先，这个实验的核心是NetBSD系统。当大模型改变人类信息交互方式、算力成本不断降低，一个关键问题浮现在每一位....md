...
数势科技在AIGC峰会提出企业级AI决策方案SwiftAgent平台，通过AI Agent技术让业务人员能像与分析师对话般获取数据洞见。谭李指出三大趋势：数据右移（实时可用）、决策下移（一线赋能）、管理后移（自动化）。强调基础模型成本下降（如DeepSeek）使企业AI应用成为可能，但需突破"Excel+Chat"简单模式，构建即时数据消费范式，使非技术人员快速获取决策支持。该方案旨在将AI转化为企业"智慧大脑"，实现业务决策升级。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T16:02:10Z
- **目录日期**: 2025-04-22
