...
数势科技联合创始人谭李在AIGC峰会上指出，企业级AI应用面临从数据到决策的转化挑战。AI Agent（如SwiftAgent平台）通过"数据分析与决策平台"实现：1）让非技术人员自然语言交互获取洞见；2）响应数据右移/决策下移/管理后移三大趋势；3）依托基础模型成本下降（如DeepSeek）实现普惠化。核心价值在于将专业分析能力转化为"即需即得"的决策支持，突破传统Excel+Chat的局限，构建企业智慧大脑。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T19:03:22Z
- **目录日期**: 2025-04-22
