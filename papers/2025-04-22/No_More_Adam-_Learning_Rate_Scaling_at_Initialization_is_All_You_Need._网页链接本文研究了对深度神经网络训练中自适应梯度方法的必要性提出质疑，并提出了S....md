...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。该方法基于带动量的随机梯度下降（SGDM），通过初始化时根据梯度信号噪声比（g-SNR）调整不同参数组的学习率，有效防止训练不平衡。相比AdamW，SGD-SaI内存使用减半，在Transformer、ImageNet分类、LLM预训练（如GPT-2、Llama2-7B）等任务中表现相当或更优，且对超参数变化更鲁棒。例如，它为GPT-2和Llama2-7B分别节省了5.93GB和25.15GB内存，在LoRA微调等场景也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T07:04:59Z
- **目录日期**: 2025-04-22
