...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI是基于带动量的随机梯度下降（SGDM）的改进，通过根据各参数组的梯度信号噪声比（g-SNR）在初始化时调整学习率，避免了训练不平衡问题。相比AdamW，SGD-SaI内存使用减半，在Transformer、ImageNet分类和大型语言模型（如GPT-2）训练中表现优异，甚至在某些任务中超越现有优化器。此外，它在LoRA微调等场景也表现突出，并显著节省内存（如为Llama2-7B节省25.15GB）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T02:32:10Z
- **目录日期**: 2025-04-22
