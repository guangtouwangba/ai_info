...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。与现有方法不同，它无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型的时间一致性问题，提出了时间表示对齐(TREPA)方法，利用自监督视频模型增强一致性。同时改进了SyncNet训练框架，解决了其收敛问题。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。创新点包括：1)端到端潜在扩散模型；2)TREPA时间对齐；3)SyncNet训练优化。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T14:06:01Z
- **目录日期**: 2025-04-22
