...
本研究提出了LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示，克服了传统两阶段方法的局限性。针对扩散模型的时间一致性问题，提出了TREPA（时间表示对齐）技术，利用自监督视频模型增强生成帧的时序连贯性。同时，研究深入分析了SyncNet的收敛问题，改进了其训练框架，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些创新不仅提升了唇同步效果，也为相关领域提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T13:13:17Z
- **目录日期**: 2025-04-22
