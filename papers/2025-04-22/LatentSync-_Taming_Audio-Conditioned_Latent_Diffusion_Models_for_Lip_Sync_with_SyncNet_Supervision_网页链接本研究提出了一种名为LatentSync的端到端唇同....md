...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强生成帧的一致性。同时，通过系统性研究改进了SyncNet训练框架，提升了其准确性。该方法在HDTF和VoxCeleb2数据集上表现优异，超越了现有唇同步技术。研究成果不仅提升了唇同步质量，其SyncNet优化经验也可推广至其他音频驱动动画领域。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T17:06:06Z
- **目录日期**: 2025-04-22
