...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。它通过四个案例研究（社会极化、信息传播、基本收入政策、飓风冲击）验证了模拟结果与现实数据的吻合性，证明其能有效捕捉人类行为机制。该系统为社会科学研究和政策制定提供了新工具，可辅助传统研究方法（如调查、访谈），帮助理解社会问题的模式和成因。研究展示了LLM驱动代理在模拟复杂社会现象方面的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T20:06:03Z
- **目录日期**: 2025-04-22
