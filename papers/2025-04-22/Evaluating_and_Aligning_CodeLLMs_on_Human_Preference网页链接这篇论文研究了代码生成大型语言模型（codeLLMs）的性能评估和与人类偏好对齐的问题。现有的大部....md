...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，关注模型输出与人类偏好的对齐问题。研究者从真实用户查询中精选397个样本(覆盖40类/44种语言)，构建更贴近实际编程场景的基准测试。同时开发了包含近200亿标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现：1)传统执行正确性基准与人类偏好评估存在差异；2)开源顶尖模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面仍有显著差距。研究凸显了代码生成模型优化中考虑人类偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T21:06:16Z
- **目录日期**: 2025-04-22
