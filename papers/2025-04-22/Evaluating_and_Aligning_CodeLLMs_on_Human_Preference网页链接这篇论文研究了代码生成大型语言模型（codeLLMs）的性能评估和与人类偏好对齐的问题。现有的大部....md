...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，指出当前基准测试过度关注代码正确性而忽视人类偏好。研究者构建了CodeArena基准（含397个多语言样本）和20B标记的SynCode-Instruct合成指令集。实验发现：1）传统执行测试与人类偏好存在偏差；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调代码生成模型需同时满足功能正确和人类编程习惯，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-22T07:06:27Z
- **目录日期**: 2025-04-22
