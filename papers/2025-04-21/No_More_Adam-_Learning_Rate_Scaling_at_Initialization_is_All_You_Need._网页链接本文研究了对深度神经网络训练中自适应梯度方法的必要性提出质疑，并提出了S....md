...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。SGD-SaI无需自适应二阶动量，从训练开始就避免不平衡，内存用量比AdamW减半。在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优异，甚至超越AdamW。内存效率显著，为GPT-2节省5.93GB，Llama2-7B节省25.15GB。该方法解决了用SGD训练Transformer的难题，具有超参数鲁棒性和广泛应用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T08:05:07Z
- **目录日期**: 2025-04-21
