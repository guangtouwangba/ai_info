...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法(如Adam)的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题。相比AdamW，SGD-SaI内存使用减半，在Transformer、ImageNet分类和LLM预训练等任务中表现相当或更好。实验显示，它为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，并在LoRA微调等任务中优于现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T12:05:01Z
- **目录日期**: 2025-04-21
