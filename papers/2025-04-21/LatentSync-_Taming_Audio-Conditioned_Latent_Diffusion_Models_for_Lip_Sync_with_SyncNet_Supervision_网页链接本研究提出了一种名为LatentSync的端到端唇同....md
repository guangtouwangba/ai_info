...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。针对扩散模型的时间一致性问题，提出时间表示对齐(TREPA)方法增强连贯性。同时优化SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)直接建模音频-视觉关系；2)TREPA保持同步精度；3)改进SyncNet训练方法。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T18:05:26Z
- **目录日期**: 2025-04-21
