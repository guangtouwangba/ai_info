...
评估一个行为的安全性需要考虑其发生的具体情境。对人类而言，这显而易见——比如删除邮件是否合适取决于邮件内容、目的（如删除敏感信息或清理垃圾）及邮箱类型（工作或个人）。但计算系统通常只能在有限情境中运作，因此依赖人工制定的策略和用户确认（如应用权限或访问控制列表）来防止危害。随着多功能智能代理（如自动个人助手）的出现，传统方法已无法应对其多样化的任务和情境。为此，本文提出"情境化代理安全框架"（Conseca），旨在生成即时、情境化且可人工验证的安全策略，以适应新一代智能系统的需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T01:30:02Z
- **目录日期**: 2025-04-21
