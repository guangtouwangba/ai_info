...
复旦大学周鹏、刘春森团队在《Nature》发表研究成果，成功研制全球最快闪存器件"破晓(PoX)"。该器件写入速度达400皮秒(比SRAM快两个数量级)，兼具非易失性存储特性，突破传统存储技术"速度与持久性不可兼得"的瓶颈。其创新在于：采用二维石墨烯实现电子弹道输运，引入"超注入"机制提升电荷效率，结合AI优化制造工艺。该技术特别适用于AI训练/推理、边缘计算等场景，有望替代现有SRAM+Flash架构。团队正推进产业化，或将重塑下一代AI硬件存储格局。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T09:03:59Z
- **目录日期**: 2025-04-21
