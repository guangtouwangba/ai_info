...
复旦大学和字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数就实现了1024分辨率图像生成。该模型通过统一处理文本和图像token实现跨模态对齐，无需依赖外部文本编码器。在GenEval评测中取得0.59分，超越同规模SDv2.1和LlamaGen模型。配合vLLM部署后，生成1024图像仅需13.55秒，在性能和速度上都展现出竞争力。研究指出当前局限主要来自训练数据和视觉tokenizer，仍有优化空间。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T00:03:54Z
- **目录日期**: 2025-04-21
