...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的训练效果。传统拒绝采样微调(RFT)方法主要依赖专家成功轨迹，但会忽略复杂任务。研究发现，专家失败轨迹中往往包含有价值的指导信息。EEF通过从失败轨迹中提取有益行动(如关键计划和动作)并排除有害行动，显著提高了智能体的探索效率和关键技能获取。在WebShop测试中，EEF以62%的胜率超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创下81分以上的新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T00:01:29Z
- **目录日期**: 2025-04-21
