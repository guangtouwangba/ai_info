...
该研究提出"探索专家失败轨迹"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有益行动(如关键计划和动作)来提升大语言模型的智能体能力。相比仅使用成功轨迹的拒绝采样微调(RFT)，EEF能有效解决复杂子任务，避免持续分布外问题。实验显示，EEF在WebShop任务中达到62%胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准上创下新纪录(首次超过0.81分和81分)，验证了利用专家失败经验的价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T02:30:09Z
- **目录日期**: 2025-04-21
