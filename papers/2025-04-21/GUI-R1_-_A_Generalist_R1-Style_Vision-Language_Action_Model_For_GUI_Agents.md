...
现有GUI智能体主要依赖大规模视觉语言模型（LVLMs）的监督微调训练，但这种方法需要大量数据且泛化能力有限。受强化微调（RFT）在大语言模型中的成功启发，研究者提出了首个基于强化学习的框架\name，通过统一动作空间规则建模提升LVLMs在真实GUI任务中的表现。该框架仅需0.02%数据（3K vs. 13M），利用跨平台高质量数据和GRPO等策略优化算法，在移动、桌面和网页8个基准测试中超越此前最佳方法OS-Atlas。结果表明基于强化学习的统一动作建模能显著提升LVLMs执行真实GUI任务的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T13:08:20Z
- **目录日期**: 2025-04-21
