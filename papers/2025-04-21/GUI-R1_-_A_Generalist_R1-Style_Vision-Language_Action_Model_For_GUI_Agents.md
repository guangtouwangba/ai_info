...
现有GUI智能体主要依赖大型视觉语言模型（LVLM）的监督微调，但存在数据需求大、泛化能力差等问题。受强化微调（RFT）启发，研究者提出首个强化学习框架，通过统一动作空间规则建模提升LVLM在跨平台（Windows/Linux/MacOS/Android/Web）GUI任务中的表现。仅用0.02%数据（3K vs 13M），该框架在8个跨平台基准测试中超越SOTA方法OS-Atlas，证实了强化学习在提升LVLM执行真实GUI任务能力方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T04:02:19Z
- **目录日期**: 2025-04-21
