...
现有GUI智能体主要依赖大规模视觉语言模型（LVLMs）的监督微调训练，但该方法需要大量数据且泛化能力有限。受强化微调（RFT）在大模型推理中的启发，研究者提出首个基于强化学习的框架\name，通过统一动作空间规则建模提升LVLMs在真实GUI任务中的表现。该框架仅需0.02%数据（3K vs. 13M），采用GRPO等算法优化模型，在跨平台（Windows/Linux/MacOS/Android/Web）8个基准测试中超越SOTA方法OS-Atlas，展现了强化学习在提升GUI任务执行能力方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T03:41:57Z
- **目录日期**: 2025-04-21
