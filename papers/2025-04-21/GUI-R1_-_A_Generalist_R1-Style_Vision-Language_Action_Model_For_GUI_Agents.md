...
现有基于监督微调大视觉语言模型（LVLM）的图形用户界面（GUI）智能体方法存在训练数据需求大、泛化能力不足的问题。为此，研究者提出首个强化学习框架**\name**，通过统一动作空间规则建模，结合少量跨平台高质量数据（仅需0.02%数据量）和GRPO策略优化算法，显著提升LVLM在真实GUI任务中的表现。实验表明，该框架在移动、桌面和网页三平台的8个基准测试中超越此前最佳方法（如OS-Atlas），验证了强化学习在提升GUI任务执行能力方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T16:02:07Z
- **目录日期**: 2025-04-21
