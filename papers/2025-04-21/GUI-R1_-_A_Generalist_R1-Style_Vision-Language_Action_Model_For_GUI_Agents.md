...
现有基于大视觉语言模型（LVLM）的图形用户界面（GUI）代理主要依赖监督微调训练，但存在数据需求大、泛化能力不足的问题。为此，研究者提出首个强化学习框架，通过统一动作空间规则建模，利用少量跨平台高质量数据（仅需0.02%数据量）和策略优化算法（如GRPO），在移动、桌面和网页等8个基准测试中超越此前最优方法。该框架显著提升了LVLM在真实GUI任务中的执行能力，展现了强化学习在GUI代理领域的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T20:02:42Z
- **目录日期**: 2025-04-21
