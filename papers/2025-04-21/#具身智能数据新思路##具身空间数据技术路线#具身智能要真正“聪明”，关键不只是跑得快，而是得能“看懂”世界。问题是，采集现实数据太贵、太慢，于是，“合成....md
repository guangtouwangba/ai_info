...
具身智能发展面临现实数据采集成本高的问题，合成数据成为新方向。当前主要有两种技术路线：一是视频合成+3D重建，但误差积累且控制难；二是端到端3D生成，但易产生不合理空间。关键挑战在于如何生成真实可控的三维空间。新解决方案如Sengine SimHub采用模态编码，将建筑规则转化为可学习结构，结合强化学习优化布局合理性。该领域仍需突破空间生成的常识性错误问题，如物体重叠等，才能实现真正可用的智能环境建模。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T02:31:45Z
- **目录日期**: 2025-04-21
