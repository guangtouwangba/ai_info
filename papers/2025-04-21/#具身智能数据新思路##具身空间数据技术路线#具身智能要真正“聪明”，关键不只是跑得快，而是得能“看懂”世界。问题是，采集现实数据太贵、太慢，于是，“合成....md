...
具身智能的发展面临现实数据采集成本高的问题，合成数据成为解决方案。目前主要有两种技术路线：一是视频合成后进行3D重建，但误差积累且控制困难；二是端到端3D生成，但容易产生不合理的空间结构。为解决这些问题，新方法如模态编码将建筑规则嵌入AI生成器，结合强化学习优化空间布局。例如Sengine SimHub通过编码常识性规则（如"餐桌靠近厨房"）提升生成合理性。该领域仍需突破真实可控3D空间生成的瓶颈，以实现更智能的具身系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T05:03:39Z
- **目录日期**: 2025-04-21
