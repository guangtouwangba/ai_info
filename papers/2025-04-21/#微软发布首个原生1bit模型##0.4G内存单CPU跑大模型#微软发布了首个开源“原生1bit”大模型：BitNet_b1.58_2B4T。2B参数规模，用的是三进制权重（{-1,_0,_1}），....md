...
微软发布首个开源原生1bit大模型BitNet b1.58 2B4T，采用三进制权重（-1,0,1），每个参数仅占1.58bit，内存需求仅0.4GB，可在单CPU上高效运行（每秒5-7个token，延迟29ms）。其能耗极低（0.028焦耳/生成），适配笔记本、树莓派等设备，在Mac M2上表现稳定。性能媲美全精度模型，数学推理（GSM8K）和常识推理（WinoGrande）任务超越Llama、Qwen等同类模型，实现高效与性能的突破。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T17:03:55Z
- **目录日期**: 2025-04-21
