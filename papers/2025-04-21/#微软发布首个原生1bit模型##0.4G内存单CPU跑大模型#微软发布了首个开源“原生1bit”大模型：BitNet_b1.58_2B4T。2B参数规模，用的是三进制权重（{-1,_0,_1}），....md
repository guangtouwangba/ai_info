...
微软发布首个开源原生1bit大模型BitNet b1.58 2B4T，采用三进制权重（-1,0,1），平均每个权重仅占1.58bit，内存需求降至0.4GB，可在单CPU上高效运行（5-7 token/秒）。其解码延迟仅29ms，能耗低至0.028焦耳/次，支持树莓派等边缘设备。性能表现优异，在数学推理（GSM8K）和常识推理（WinoGrande）任务上超越Llama等全精度模型，同时保持与16bit模型相当的精度。该技术大幅降低了大模型部署门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T19:03:43Z
- **目录日期**: 2025-04-21
