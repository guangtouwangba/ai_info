...
微软发布首个开源1bit大模型BitNet b1.58 2B4T，采用三进制权重（-1,0,1），每个权重仅占1.58bit，内存需求仅0.4GB，可在单CPU流畅运行（5-7 token/s）。其解码延迟29ms，能耗仅0.028焦耳/次，适用于笔记本、树莓派等设备。性能媲美全精度模型，在GSM8K数学推理和WinoGrande常识推理任务上超越Llama、Qwen等模型，实现高效低耗的大模型部署突破。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T14:04:02Z
- **目录日期**: 2025-04-21
