...
该研究提出通过人类演示增强移动GUI代理的能力，而非依赖大规模数据集实现通用化。团队开发了首个专注于演示学习的移动GUI数据集LearnGUI（含2,252离线任务和101在线任务），并设计了多智能体框架LearnAct，包含DemoParser（知识提取）、KnowSeeker（知识检索）和ActExecutor（任务执行）三个模块。实验表明：单次演示使Gemini-1.5-Pro准确率从19.3%提升至51.7%；在线测试中UI-TARS-7B-SFT任务成功率从18.1%提高到32.8%。该框架为开发更具适应性的移动GUI代理提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T06:01:30Z
- **目录日期**: 2025-04-21
