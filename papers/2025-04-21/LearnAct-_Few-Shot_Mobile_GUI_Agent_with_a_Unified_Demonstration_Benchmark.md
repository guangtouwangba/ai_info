...
该研究提出通过人类演示增强移动GUI代理能力的方案，以解决传统方法在大规模数据集预训练/微调中面临的泛化挑战。团队开发了首个专注于演示学习的移动GUI数据集LearnGUI（含2,252离线任务和101在线任务），并构建多智能体框架LearnAct，包含DemoParser（知识提取）、KnowSeeker（知识检索）和ActExecutor（任务执行）三个模块。实验显示：单次演示使Gemini-1.5-Pro离线准确率从19.3%提升至51.7%；在线任务中UI-TARS-7B-SFT成功率从18.1%提高到32.8%，证实演示学习能有效提升移动代理的适应性和个性化部署能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T12:01:54Z
- **目录日期**: 2025-04-21
