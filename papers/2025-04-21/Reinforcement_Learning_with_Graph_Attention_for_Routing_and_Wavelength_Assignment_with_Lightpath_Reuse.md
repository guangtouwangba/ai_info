...
本文研究了固定网格网络中结合灵活速率收发器的路由与波长分配问题（RWA-LR）。通过对比启发式算法，发现按跳数排序路径比按总长度排序能提升6%的网络吞吐量。作者采用图注意力网络构建强化学习（RL）智能体，其策略和价值函数能有效利用图结构数据。实验表明，该方法比现有最佳RL方案提升2.5%（17.4Tbps吞吐量），比最优启发式算法提升1.2%（8.5Tbps）。尽管边际增益有限，但凸显了在长周期资源分配任务中学习有效RL策略的挑战。研究代码已开源以供复现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T09:02:19Z
- **目录日期**: 2025-04-21
