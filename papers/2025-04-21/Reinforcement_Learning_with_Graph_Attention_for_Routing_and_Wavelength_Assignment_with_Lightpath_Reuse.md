...
本文研究了固定栅格网络中结合灵活速率收发器的路由与波长分配问题（RWA-LR），提出了一种基于图注意力网络的强化学习（RL）方法。通过优化候选路径排序（按跳数而非总长度），启发式算法吞吐量提升6%。所训练的RL智能体在策略和价值函数中利用图结构数据，相比现有最佳RL方法提升2.5%（17.4Tbps），较最优启发式算法提升1.2%（8.5Tbps）。研究开源了全部代码，结果表明在长周期资源分配任务中学习有效RL策略具有挑战性，边际收益有限但方法具有创新性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T22:02:06Z
- **目录日期**: 2025-04-21
