...
本文研究了固定网格网络中带可调谐光转发器的路由与波长分配问题（RWA-LR），提出了一种基于图注意力网络的强化学习（RL）方法。研究发现，按跳数排序候选路径的启发式算法比按总长度排序提高6%吞吐量。新RL方法在策略和价值函数中利用图结构数据，比现有最佳RL方法提升2.5%（17.4Tbps），比最佳启发式算法提升1.2%（8.5Tbps）。这些边际增益表明，在长周期资源分配任务中学习有效RL策略具有挑战性。研究开源了所有代码以便复现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T21:02:18Z
- **目录日期**: 2025-04-21
