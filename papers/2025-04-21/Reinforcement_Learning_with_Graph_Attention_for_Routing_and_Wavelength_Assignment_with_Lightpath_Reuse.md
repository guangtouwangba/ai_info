...
本文研究了固定网格网络中带灵活速率收发器的路由与波长分配问题（RWA-LR），提出了一种基于启发式算法和强化学习（RL）的解决方案。研究发现，按跳数排序候选路径可使吞吐量提高6%。作者采用图注意力网络构建RL智能体的策略和价值函数，以利用图结构数据。实验表明，该方法比现有最佳RL方法提升2.5%吞吐量（17.4Tbps），比最佳启发式算法提升1.2%（8.5Tbps）。这些边际增益表明，在长周期资源分配任务中学习有效RL策略具有挑战性。所有代码已开源以供复现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T16:02:18Z
- **目录日期**: 2025-04-21
