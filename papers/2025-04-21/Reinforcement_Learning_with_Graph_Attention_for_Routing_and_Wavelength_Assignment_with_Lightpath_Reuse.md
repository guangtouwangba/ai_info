...
本文研究了固定网格网络中带灵活速率收发器的路由与波长分配问题（RWA-LR），提出了一种新的强化学习（RL）方法。通过使用图注意力网络处理图结构数据，训练的RL智能体在吞吐量上比现有最佳RL方法提升2.5%（17.4 Tbps），比最优启发式算法提升1.2%（8.5 Tbps）。研究发现，按跳数排序候选路径的启发式算法比按总长度排序提升6%吞吐量。尽管边际增益有限，但凸显了在长周期资源分配任务中学习有效RL策略的挑战。研究开源了所有代码以便复现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T15:02:19Z
- **目录日期**: 2025-04-21
