...
该论文提出了一种新型的LLM智能体记忆系统，采用类似Zettelkasten方法的动态组织方式。系统会为每个新记忆创建包含上下文描述、关键词和标签的结构化笔记，并通过分析历史记忆建立关联连接。随着新记忆的加入，系统能触发对现有记忆的上下文表示和属性的更新，实现记忆网络的持续优化。这种结合Zettelkasten结构化原则与智能体驱动决策的方法，实现了更灵活、情境感知的记忆管理。在6个基础模型上的实验表明，其性能优于现有SOTA基线。相关代码已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T06:02:06Z
- **目录日期**: 2025-04-21
