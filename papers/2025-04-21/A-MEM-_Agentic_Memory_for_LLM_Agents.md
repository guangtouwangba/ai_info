...
本文提出了一种新型的LLM智能体记忆系统，能动态组织记忆并建立知识网络。受Zettelkasten方法启发，该系统通过动态索引和链接创建互联记忆网络：每新增记忆会生成包含上下文描述、关键词等结构化属性的笔记，并分析历史记忆建立关联。新记忆的加入还能触发已有记忆的更新，实现知识网络的持续优化。该系统结合了Zettelkasten的结构化原则与智能体驱动的灵活性，在6个基础模型上的实验表现优于现有方法。相关代码已开源（性能评估：GitHub/WujiangXu/AgenticMemory；系统实现：GitHub/agiresearch/A-mem）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T11:02:28Z
- **目录日期**: 2025-04-21
