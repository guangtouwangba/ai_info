...
这篇摘要探讨了当前大模型预训练方法的局限性，即对i.i.d.假设的依赖阻碍了其在动态学习场景中的适应性。作者认为AI的下一个突破在于实现持续学习(CL)，使模型能适应不断变化的环境。虽然参数高效微调(PEFT)方法能有效调整大模型，但仍面临灾难性遗忘问题。文章综述了CL算法、PEFT方法以及参数高效持续微调(PECFT)的最新进展，分析了不同方法、评估指标和未来研究方向，旨在促进CL与参数高效微调的协同发展，为该领域研究者提供指导并开拓新的研究路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T17:01:34Z
- **目录日期**: 2025-04-21
