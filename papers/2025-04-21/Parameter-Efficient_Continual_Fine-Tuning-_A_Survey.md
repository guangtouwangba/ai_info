...
这篇摘要探讨了大规模预训练网络在AI领域的革命性影响及其局限性，特别是对独立同分布(i.i.d.)假设的依赖阻碍了动态学习场景的适应能力。作者认为AI的下一个突破在于实现持续学习(CL)，使模型能适应不断变化的环境。参数高效微调(PEFT)是一种有效方法，通过小规模调整实现类似全微调的性能，但仍面临灾难性遗忘问题。文章综述了CL算法、PEFT方法及参数高效持续微调(PECFT)的最新进展，分析了不同方法、评估指标及未来研究方向，旨在促进CL与PEFT的协同发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T22:01:27Z
- **目录日期**: 2025-04-21
