...
这篇摘要探讨了大规模预训练网络在AI领域的革命性影响及其局限性，特别是对i.i.d.假设的依赖阻碍了模型在动态环境中的适应能力。作者认为AI的下一个突破在于实现持续学习(CL)，使模型能够适应不断变化的环境。虽然参数高效微调(PEFT)方法能有效调整模型以适应特定数据，但仍面临灾难性遗忘问题。文章综述了CL算法、PEFT方法以及参数高效持续微调(PECFT)的最新进展，分析了不同方法、评估指标及未来研究方向，旨在促进CL与PEFT的协同研究，为未来探索新方向铺路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T18:01:35Z
- **目录日期**: 2025-04-21
