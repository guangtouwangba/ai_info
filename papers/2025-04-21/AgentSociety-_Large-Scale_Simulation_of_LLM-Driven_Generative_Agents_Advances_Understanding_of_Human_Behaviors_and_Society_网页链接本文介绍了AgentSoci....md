...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟系统，它利用大型语言模型驱动上万个虚拟代理，在模拟环境中进行500万次互动。该系统被用于研究四个关键社会问题：群体极化、信息传播、基本收入政策效果和自然灾害影响。模拟结果与现实世界数据高度吻合，证明该系统能有效捕捉人类行为机制。研究表明，AgentSociety可作为计算社会科学的创新研究平台，为学者和政策制定者提供传统方法（如调查、访谈）之外的补充工具，帮助理解复杂社会现象的内在规律。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T20:06:09Z
- **目录日期**: 2025-04-21
