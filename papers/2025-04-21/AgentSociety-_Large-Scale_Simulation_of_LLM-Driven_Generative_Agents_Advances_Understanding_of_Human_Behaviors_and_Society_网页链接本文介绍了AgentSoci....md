...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个AI代理的社会互动（500万次）。该系统通过四个社会问题（社会极化、信息传播、基本收入政策、自然灾害影响）的实验，验证了其模拟结果与现实世界数据的一致性。研究表明，AgentSociety能有效捕捉人类行为机制，可作为社会科学研究和政策制定的新型工具，为传统研究方法（如调查、访谈）提供补充。其仿真能力有助于深入理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T14:05:47Z
- **目录日期**: 2025-04-21
