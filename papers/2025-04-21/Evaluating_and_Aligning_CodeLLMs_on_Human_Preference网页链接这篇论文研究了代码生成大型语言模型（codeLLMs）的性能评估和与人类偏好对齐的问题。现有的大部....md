...
该论文研究了代码生成大模型（codeLLMs）与人类偏好的对齐问题，指出现有基准测试过于关注代码正确性而忽视用户体验。为此，作者构建了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令数据集（近200亿标记）。实验发现执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著，凸显了人类偏好对齐的重要性。研究成果为提升代码LLMs的实际应用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T10:04:27Z
- **目录日期**: 2025-04-21
