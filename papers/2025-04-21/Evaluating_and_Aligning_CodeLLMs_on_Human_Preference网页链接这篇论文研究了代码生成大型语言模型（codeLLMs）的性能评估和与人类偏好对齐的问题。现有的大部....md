...
这篇论文探讨了代码生成大模型（CodeLLMs）如何更好地与人类偏好对齐。研究指出当前基准测试过于关注代码正确性，忽略了人类偏好因素，因此提出了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令数据集（20B标记）。实验发现执行基准与人类偏好评估存在差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显了人类偏好对齐的重要性。研究为提升代码生成模型的实用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T04:06:01Z
- **目录日期**: 2025-04-21
