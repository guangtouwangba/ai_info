...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记的合成指令数据集SynCode-Instruct验证指令微调效果。研究发现：现有基于执行的评测与人类偏好存在偏差，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面仍有显著差距。研究强调了代码生成模型除功能性外，还需重视开发者体验优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-21T21:06:13Z
- **目录日期**: 2025-04-21
