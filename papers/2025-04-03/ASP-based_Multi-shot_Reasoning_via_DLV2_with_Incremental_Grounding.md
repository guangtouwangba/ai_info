...
DLV2是一个基于知识表示与推理(KRR)的AI工具，支持回答集编程(ASP)。它采用"Ground & Solve"方法，先将逻辑程序转换为等价的基础程序，再通过命题算法寻找答案集。为适应流式推理等新兴应用需求，研究团队开发了DLV2的增量推理版本。该系统能在多次执行中保持运行状态，增量处理基础化过程，重用先前计算结果构建更通用的基础程序，并从中提取等效子集进行答案集计算。整个过程对用户完全透明。论文还探讨了该系统在实际领域中的应用和性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T00:01:36Z
- **目录日期**: 2025-04-03
