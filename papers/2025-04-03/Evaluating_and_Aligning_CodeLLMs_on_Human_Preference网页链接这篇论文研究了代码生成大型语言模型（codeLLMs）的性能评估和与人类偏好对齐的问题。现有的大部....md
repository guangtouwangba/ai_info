...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类任务和44种语言)来评估模型在真实编程场景中的表现，发现传统执行基准与人类偏好存在差异。同时开发了SynCode-Instruct语料库(近200亿标记)验证合成指令微调效果。实验显示开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)存在显著差距，强调了人类偏好对齐的重要性。研究突出了当前代码生成模型评估的局限性，为改进方向提供了新见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T07:06:33Z
- **目录日期**: 2025-04-03
