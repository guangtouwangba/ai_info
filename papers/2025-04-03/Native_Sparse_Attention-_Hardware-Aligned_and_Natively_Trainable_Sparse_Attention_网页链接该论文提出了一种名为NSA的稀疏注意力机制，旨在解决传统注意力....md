...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在长文本处理中的高计算成本问题。NSA通过动态分层稀疏策略，结合粗粒度令牌压缩和细粒度令牌选择，在保持全局上下文感知的同时优化局部精度。其创新点在于算法与硬件协同设计，显著提升了长文本建模效率。该方法通过硬件对齐和原生可训练性，实现了计算效率与模型性能的平衡，为大规模语言模型的高效注意力计算提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T22:04:53Z
- **目录日期**: 2025-04-03
