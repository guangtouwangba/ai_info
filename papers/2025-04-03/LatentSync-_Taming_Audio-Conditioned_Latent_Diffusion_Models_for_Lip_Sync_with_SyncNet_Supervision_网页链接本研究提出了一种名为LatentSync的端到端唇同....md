...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关联，并提出时间表示对齐(TREPA)方法解决扩散过程中时间一致性问题。研究还优化了SyncNet训练框架，通过调整模型架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)首个潜在扩散唇同步框架；2)TREPA增强时间一致性；3)SyncNet训练改进方案。这些发现可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T23:05:28Z
- **目录日期**: 2025-04-03
