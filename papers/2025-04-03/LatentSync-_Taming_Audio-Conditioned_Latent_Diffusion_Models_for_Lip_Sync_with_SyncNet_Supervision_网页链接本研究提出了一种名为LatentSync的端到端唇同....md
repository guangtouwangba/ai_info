...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧与真实帧。同时改进了SyncNet训练框架，解决了收敛问题并提升准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。这些改进可推广至其他基于SyncNet的唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T16:05:33Z
- **目录日期**: 2025-04-03
