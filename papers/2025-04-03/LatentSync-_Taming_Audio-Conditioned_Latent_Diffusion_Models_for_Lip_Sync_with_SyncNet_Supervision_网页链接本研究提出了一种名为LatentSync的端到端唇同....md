...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉相关性，解决了现有方法的时间一致性问题。研究提出时间表示对齐（TREPA）方法增强一致性，并优化SyncNet训练框架提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）直接潜在空间建模；2）TREPA保持同步精度；3）SyncNet训练改进方案。成果可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T20:05:20Z
- **目录日期**: 2025-04-03
