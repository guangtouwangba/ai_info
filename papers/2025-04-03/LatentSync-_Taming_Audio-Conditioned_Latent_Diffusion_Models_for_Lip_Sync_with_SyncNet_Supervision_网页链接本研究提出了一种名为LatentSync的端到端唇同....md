...
该研究提出LatentSync，一种基于音频条件的潜在扩散模型的端到端唇同步框架，无需中间运动表示。针对扩散模型的时间一致性问题，提出了时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时间一致性。研究还解决了SyncNet的收敛问题，通过改进模型架构、训练参数和数据预处理显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些发现对唇同步和音频驱动动画领域具有广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T19:05:27Z
- **目录日期**: 2025-04-03
