...
当前AI代理在复杂多模态任务、结构化生成和策略规划上仍有不足。由于通常依赖黑盒API而无法调整模型参数，传统微调方法效果有限。研究者提出迭代代理解码（IAD）方法，通过验证器引导的动态候选评估与选择机制实现迭代优化。实验显示，IAD在Sketch2Code、Text2SQL和Webshop任务上比基线方法提升3-10%，关键优势来自验证器引导的反馈而非采样多样性。研究还发现计算资源增加时IAD性能可扩展，但验证器质量对优化效果至关重要。该工作为推理时优化提供了重要设计原则。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T14:01:18Z
- **目录日期**: 2025-04-03
