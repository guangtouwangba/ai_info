...
当前AI代理在多模态应用、结构化生成和策略规划等复杂任务中仍有不足。由于通常依赖黑盒API且无法调整模型参数，传统微调方法效果有限。研究者提出迭代代理解码（IAD）方法，通过验证器引导的动态候选评估和选择进行迭代优化。相比最佳N采样（BON），IAD能更有效利用奖励信号。实验表明，IAD在Sketch2Code、Text2SQL和Webshop任务中性能提升3-10%，其优势主要来自验证器引导的优化而非采样多样性。研究还发现验证器质量对推理时优化至关重要，并分析了噪声/稀疏奖励对扩展性的影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T07:01:27Z
- **目录日期**: 2025-04-03
