...
当前AI代理在复杂多模态任务、结构化生成和战略规划方面仍有不足，而传统微调方法因依赖黑箱API难以优化。研究者提出迭代代理解码（IAD）方法，通过验证器引导的动态候选评估与选择机制实现迭代优化，相比最佳N采样（BON）能更有效利用奖励信号。实验显示IAD在Sketch2Code、Text2SQL和Webshop任务中性能提升3-10%，验证其改进主要源于验证器引导的优化而非采样多样性。研究还发现计算资源增加时IAD和BON均能扩展性能，但验证器质量对推理时优化效果起决定性作用，为推理时优化提供了重要设计原则。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T10:01:18Z
- **目录日期**: 2025-04-03
