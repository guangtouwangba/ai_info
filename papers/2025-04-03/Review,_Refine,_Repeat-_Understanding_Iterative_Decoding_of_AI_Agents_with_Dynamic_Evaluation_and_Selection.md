...
当前AI代理在复杂多模态任务、结构化生成和战略规划方面仍有不足。由于通常依赖黑盒API且无法调整模型参数，传统微调方法效果有限。研究提出迭代代理解码(IAD)方法，通过动态评估候选方案和验证器引导的迭代优化来提升性能。在Sketch2Code、Text2SQL和Webshop任务上，IAD比基线方法提升3-10%。实验表明，IAD的改进主要来自验证器引导的优化而非采样多样性。研究还发现，在优质验证器引导下，IAD和BON方法都能随计算资源增加而提升性能，验证器质量对推理时优化至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T04:01:20Z
- **目录日期**: 2025-04-03
