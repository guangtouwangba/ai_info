...
现有AI代理在多模态应用、结构化生成和战略规划等复杂任务上仍有不足。由于通常依赖黑盒API且无法调整模型参数，传统微调方法效果有限。研究者提出了迭代代理解码（IAD）方法，通过验证器引导的动态候选评估和选择机制实现迭代优化。实验表明，IAD在Sketch2Code、Text2SQL和Webshop任务上比基线方法提升3-10%，其优势主要来自验证器引导的优化而非采样多样性。研究还发现，在优质验证器指导下，IAD和BON方法都能通过增加计算资源获得更好表现，验证器质量对推理时优化至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T15:01:12Z
- **目录日期**: 2025-04-03
