...
当前AI代理在多模态应用、结构化生成和策略规划方面仍有不足，而传统微调方法因依赖黑盒API难以优化。研究者提出迭代代理解码（IAD），通过动态评估和验证器引导的候选选择实现迭代优化。相比最佳N采样（BON），IAD能更有效利用奖励信号。实验显示，IAD在Sketch2Code、Text2SQL和Webshop任务中性能提升3-10%，其优势主要来自验证器引导的优化而非采样多样性。研究还发现，在最优验证器指导下，IAD和BON均可通过增加计算资源提升效果，验证器质量对推理时优化至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T17:01:16Z
- **目录日期**: 2025-04-03
