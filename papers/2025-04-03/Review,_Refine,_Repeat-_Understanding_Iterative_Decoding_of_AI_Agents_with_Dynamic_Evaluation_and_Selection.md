...
当前AI代理在处理多模态应用、结构化生成和策略规划等复杂任务时仍有不足。由于通常依赖黑盒API且无法调整模型参数，传统微调方法效果有限。研究者提出迭代代理解码（IAD）方法，通过验证器引导的动态评估与选择机制实现迭代优化。实验表明，IAD在Sketch2Code、Text2SQL和Webshop任务上比基线方法提升3-10%，其优势主要来自验证器指导的反馈而非采样多样性。研究还发现，在优质验证器引导下，IAD和BON方法都能随计算资源增加而提升性能，验证器质量对推理时优化至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T13:07:59Z
- **目录日期**: 2025-04-03
