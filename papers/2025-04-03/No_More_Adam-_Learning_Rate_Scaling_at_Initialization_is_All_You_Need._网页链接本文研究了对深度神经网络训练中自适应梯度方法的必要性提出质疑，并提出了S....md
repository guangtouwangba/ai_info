...
该研究提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信噪比（g-SNR）对不同参数组进行学习率调整，有效解决了训练不平衡问题，并将内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及大模型（如GPT-2、Llama2-7B）预训练中表现优于或持平AdamW，同时显著节省内存（GPT-2节省5.93GB，Llama2节省25.15GB）。其鲁棒性和高效性使其成为替代自适应优化器的有力候选。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T22:04:25Z
- **目录日期**: 2025-04-03
