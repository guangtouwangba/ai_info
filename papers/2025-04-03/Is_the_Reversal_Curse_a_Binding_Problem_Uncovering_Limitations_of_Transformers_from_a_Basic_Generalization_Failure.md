...
大型语言模型（LLMs）存在"逆转诅咒"现象，即难以学习可逆的事实关联。研究表明，这与认知科学中的"绑定问题"相关，源于Transformer在概念绑定上的局限性：概念表征的不一致和纠缠。实验验证后，研究者提出基于JEPA（联合嵌入预测架构）的新模型设计，首次在不依赖数据增强或非因果掩码的情况下解决了该问题，并通过解耦概念表征的记忆层进一步提升泛化能力。这一突破使模型能通过参数化前向链解决大规模算术推理问题，性能超越依赖非参数化记忆和显式推理的前沿LLMs。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T07:01:38Z
- **目录日期**: 2025-04-03
