...
大型语言模型(LLMs)存在"逆转诅咒"(Reversal Curse)的泛化缺陷，即难以学习可逆事实关联。研究表明，这源于Transformer在概念绑定上的两个局限：概念表征的不一致性和纠缠性。受认知科学中"绑定问题"启发，研究者基于JEPA架构设计了新模型，首次在不依赖数据增强或非因果掩码的情况下突破了逆转诅咒。通过引入支持解耦概念表征的记忆层，进一步提升了泛化能力。这种逆转能力使模型能通过参数化前向链解决大规模算术推理问题，性能超越了依赖非参数记忆和显式推理的前沿LLMs。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T03:13:30Z
- **目录日期**: 2025-04-03
