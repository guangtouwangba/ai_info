...
大型语言模型(LLMs)存在"逆转诅咒"的泛化缺陷，即难以学习可逆事实关联。研究表明，这源于Transformer在概念绑定上的局限性：概念表征的不一致性和纠缠性。基于JEPA(联合嵌入预测架构)的新模型设计首次突破了这一限制，无需依赖数据增强或非因果掩码。通过引入支持解耦概念表征的特殊记忆层，模型的泛化能力得到进一步提升。这种逆转能力解锁了新型记忆整合机制，使模型能通过参数化前向链解决大规模算术推理问题，表现优于依赖非参数记忆和显式推理的前沿LLMs。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T20:01:16Z
- **目录日期**: 2025-04-03
