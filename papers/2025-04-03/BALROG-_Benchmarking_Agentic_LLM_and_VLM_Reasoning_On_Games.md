...
研究人员开发了名为BALROG的新基准测试，用于评估大型语言模型(LLMs)和视觉语言模型(VLMs)在复杂动态环境中的代理能力。该测试整合了从简单到极难(如NetHack)的各类强化学习环境，并设计了细粒度评估指标。研究发现，当前模型在简单任务中表现尚可，但在复杂任务中表现欠佳，特别是基于视觉的决策能力存在严重不足——某些模型在提供视觉环境时表现反而更差。该基准测试已开源，旨在促进智能代理领域的研究发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T02:28:15Z
- **目录日期**: 2025-04-03
