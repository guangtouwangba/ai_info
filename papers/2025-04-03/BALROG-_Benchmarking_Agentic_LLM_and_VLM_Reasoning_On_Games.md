...
大型语言模型(LLMs)和视觉语言模型(VLMs)虽然具备广泛知识和推理能力，但在复杂动态环境中表现欠佳。为评估这些模型的智能体能力，研究者开发了BALROG基准测试，包含从简单到极难(如NetHack)的多样化游戏环境。研究发现当前模型在简单任务中表现尚可，但在挑战性任务(尤其是基于视觉的决策)中严重不足，部分模型在提供视觉输入时表现反而更差。该基准测试已开源，旨在促进智能体研究发展。代码和排行榜见balrogai.com。(99字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T01:28:54Z
- **目录日期**: 2025-04-03
