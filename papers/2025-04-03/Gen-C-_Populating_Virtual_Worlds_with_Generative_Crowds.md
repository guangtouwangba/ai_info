...
过去20年，人群模拟研究主要聚焦于避碰、路径跟随等低层任务，缺乏对高层交互的建模。为此，研究者提出Gen-C生成模型，利用大语言模型生成初始场景，通过模拟构建时空扩展图来建模智能体行为与交互。该方法采用双变分图自编码器，分别学习图结构（交互）和节点特征（动作）的潜在空间，支持基于自然语言的条件生成。实验在大学校园和火车站场景中验证了模型能生成反映复杂决策的动态群体行为，为虚拟环境提供了多样化的智能体交互方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T14:01:39Z
- **目录日期**: 2025-04-03
