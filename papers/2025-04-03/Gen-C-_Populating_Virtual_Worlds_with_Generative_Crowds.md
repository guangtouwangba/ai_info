...
过去20年，人群模拟研究主要集中在避碰、路径跟随等底层任务，缺乏对高层交互的捕捉。为此，研究者提出Gen-C生成模型，通过大语言模型生成初始场景，经模拟扩展构建时空图来建模智能体行为与交互。该方法采用双变分图自编码器，分别学习图结构（交互关系）和节点特征（个体行为）的隐空间，支持基于自然语言描述生成动态人群交互。实验在大学校园和火车站场景中验证了其有效性，能合成反映复杂决策的多样化群体行为，为虚拟环境提供更真实的人群模拟方案。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T12:01:47Z
- **目录日期**: 2025-04-03
