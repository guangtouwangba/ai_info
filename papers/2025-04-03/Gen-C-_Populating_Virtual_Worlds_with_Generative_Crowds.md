...
过去20年，人群模拟研究主要集中在避碰等底层任务和路径跟随等有限行为上。为生成更真实的人群场景，我们提出Gen-C模型，利用大语言模型生成初始场景，通过模拟构建时空扩展图来建模智能体行为与交互。该方法采用双变分图自编码器，分别学习图结构（交互关系）和节点特征（个体行为）的潜在空间，支持基于自然语言的条件生成。实验证明，该模型能在大学校园和火车站等场景中生成具有复杂交互和高级决策模式的动态群体行为，显著提升了虚拟环境的真实性和多样性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T07:01:48Z
- **目录日期**: 2025-04-03
