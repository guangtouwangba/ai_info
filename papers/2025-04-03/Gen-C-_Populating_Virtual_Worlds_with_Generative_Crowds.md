...
过去20年，群体模拟研究主要集中在避碰等底层任务和简单行为上。为生成更真实的群体交互，本研究提出Gen-C模型，利用大语言模型生成初始场景，通过模拟构建时态扩展图来建模智能体行为与交互。该方法采用双变分图自编码器架构，分别学习图结构（交互关系）和节点特征（个体行为）的潜在空间，支持基于自然语言的条件生成。实验证明，该系统能根据文本描述自动生成多样化的动态群体行为，适用于校园、车站等复杂虚拟场景，显著提升了高级决策交互的模拟能力。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T08:01:41Z
- **目录日期**: 2025-04-03
