...
过去20年，人群模拟研究主要集中在避碰等底层任务和路径跟随等简单行为上。为生成更真实的人群场景，我们提出Gen-C模型，利用大语言模型生成初始场景，再通过模拟构建时态扩展图来建模智能体行为与交互。该方法采用双变分图自编码器，分别学习图结构（交互关系）和节点特征（个体行为）的潜在空间，支持基于自然语言生成动态人群交互。实验证明，该模型能合成反映复杂决策的高层行为，成功应用于大学校园和火车站场景，为虚拟环境创造多样化、动态化的人群行为。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T21:01:33Z
- **目录日期**: 2025-04-03
