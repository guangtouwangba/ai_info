...
过去20年，人群模拟研究主要集中在避碰等低层任务和路径跟随等简单行为。为生成更真实的人群场景，本研究提出Gen-C模型，利用大语言模型生成初始场景，通过模拟构建时空扩展图来建模智能体行为与交互。该方法采用双变分图自编码器，分别学习图结构（交互关系）和节点特征（个体行为）的潜在空间，支持基于自然语言的条件生成。实验在大学校园和火车站场景中验证了模型生成多样化、动态交互人群的能力，展示了复杂决策行为建模的潜力，为虚拟环境提供了高效的行为创作工具。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T20:01:27Z
- **目录日期**: 2025-04-03
