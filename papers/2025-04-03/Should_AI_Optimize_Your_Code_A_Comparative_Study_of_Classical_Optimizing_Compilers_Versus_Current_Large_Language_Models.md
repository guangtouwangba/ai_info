...
传统优化编译器在现代软件系统发展中至关重要，但并行编程的需求催生了更高效的优化技术。本研究探讨了大型语言模型（LLMs）能否革新代码优化方法，通过对比三个传统编译器与两个LLMs的性能。研究发现，LLMs（如CodeLlama-70B）在优化潜力上可超越传统编译器（如CETUS），最高提速达1.75倍，但大代码量时易生成错误结果，需自动化验证。提示策略显著影响性能，详细指令（DIP）和思维链（CoT）效果优于简单指令（IP）。研究还提出了优化模式测试集和自动化评估机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T21:02:33Z
- **目录日期**: 2025-04-03
