...
传统优化编译器在现代软件系统复杂性增长中发挥了重要作用。本研究探讨AI驱动的大型语言模型(LLMs)能否革新代码优化方法。通过比较三种经典编译器与两种LLMs的性能，并引入新的基准测试套件和自动评估机制，发现LLMs(特别是CodeLlama-70B)最高可实现1.75倍加速，优于最佳传统编译器CETUS的1.67倍。但LLMs在大规模代码上易产生错误，需自动化验证方法。研究还表明，将编译器策略融入提示能显著提升LLMs表现，且三种提示策略(简单指令、详细指令和思维链)效果差异显著。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T23:02:34Z
- **目录日期**: 2025-04-03
