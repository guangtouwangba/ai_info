...
传统优化编译器在现代软件系统复杂度提升中发挥了重要作用，而当前架构对高效并行编程的需求要求更强的优化技术。本研究探讨了大型语言模型（LLMs）能否革新代码优化方法，通过对比三个经典优化编译器与两个LLM的性能。研究发现：LLMs在优化潜力上可超越传统编译器（最高提速1.75倍 vs 编译器1.67倍），但大代码量时易生成错误结果，需自动化验证；将编译器策略融入提示词能显著提升LLM表现（CodeLlama-70B最优）；三种提示策略（简单指令、详细指令、思维链）效果差异显著。研究还提出了优化模式测试套件和自动评估机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T22:02:29Z
- **目录日期**: 2025-04-03
