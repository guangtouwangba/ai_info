...
传统优化编译器在现代软件系统的复杂性增长中发挥了重要作用。随着并行编程需求的增加，高效优化技术变得至关重要。大型语言模型（LLMs）的出现引发了关于AI能否革新代码优化方法的探讨。本研究比较了三种传统编译器与两种LLMs的优化能力，并引入了一套基准测试和自动评估机制。结果显示，LLMs（如CodeLlama-70B）在某些情况下能超越传统编译器（如CETUS），但存在生成错误代码的风险，需自动化验证。提示策略（如详细指令或思维链）显著影响LLMs性能，最佳情况下LLMs可实现1.75倍加速。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T03:14:42Z
- **目录日期**: 2025-04-03
