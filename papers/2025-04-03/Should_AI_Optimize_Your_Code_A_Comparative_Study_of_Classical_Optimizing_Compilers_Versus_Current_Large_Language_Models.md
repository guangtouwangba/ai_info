...
传统优化编译器在现代软件系统中发挥了重要作用，而当前架构对高效并行编程的需求进一步强化了优化技术的重要性。本研究探讨了大型语言模型（LLMs）是否能够革新代码优化方法，通过对比三种经典优化编译器与两种最新LLMs的性能。研究发现，LLMs在优化潜力上可能超越传统编译器（如CodeLlama-70B最高提速1.75倍），但大代码量时易生成错误结果，需自动化验证机制。将编译器策略融入提示词可显著提升LLMs表现，且不同提示策略（如思维链CoT）效果差异显著。最佳传统编译器CETUS最高提速1.67倍。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T06:02:34Z
- **目录日期**: 2025-04-03
