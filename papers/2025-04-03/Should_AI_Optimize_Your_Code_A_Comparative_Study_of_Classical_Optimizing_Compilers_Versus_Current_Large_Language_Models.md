...
传统优化编译器在现代软件系统的复杂性管理中扮演重要角色，而高效并行编程的需求推动了优化技术的发展。本研究探讨了大型语言模型（LLMs）能否革新代码优化方法，通过对比三种传统编译器与两种LLMs的性能。研究引入了一套优化模式基准测试和自动评估机制，测试了三种提示策略（简单指令、详细指令和思维链）。结果显示，LLMs（如CodeLlama-70B）在部分情况下性能优于传统编译器（如CETUS，最高加速1.67倍），但大代码量时易出错，需自动化验证。提示策略对LLMs性能影响显著，详细策略效果最佳（最高加速1.75倍）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T09:02:41Z
- **目录日期**: 2025-04-03
