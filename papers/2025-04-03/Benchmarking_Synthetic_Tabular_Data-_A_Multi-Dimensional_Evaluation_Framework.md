...
本文提出了一种评估合成数据质量的框架，通过量化合成数据对原始数据分布特性的复现程度，同时确保隐私保护。该框架采用基于保留集的基准测试策略，结合低维/高维分布比较、嵌入相似度测量和最近邻距离指标进行量化评估。支持多种数据类型（包括序列和上下文信息），并通过标准化指标提供可解释的质量诊断，旨在提升合成数据生成技术的可复现性和方法一致性。相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T21:01:51Z
- **目录日期**: 2025-04-03
