...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个代理及其500万次互动。它结合真实社会环境和大规模模拟引擎，用于研究社会现象，如极化现象、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界实验数据吻合，验证了其捕捉人类行为和社会机制的能力。该系统可作为社会科学研究和政策制定的工具，补充传统方法（如调查和访谈），帮助深入理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-03T07:05:51Z
- **目录日期**: 2025-04-03
