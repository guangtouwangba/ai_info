...
Ubiquant研究团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练，通过降低模型输出熵值来提升预测置信度。在数学推理任务中，EM表现媲美传统RL方法，甚至能超越使用海量数据训练的模型。关键创新在于选择模型回答不稳定的问题作为训练样本，例如仅用1道物理题就显著提升了Qwen2.5-Math-7B的数学能力。该方法解决了RL训练成本高的问题，为高效模型优化提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T15:02:30Z
- **目录日期**: 2025-06-01
