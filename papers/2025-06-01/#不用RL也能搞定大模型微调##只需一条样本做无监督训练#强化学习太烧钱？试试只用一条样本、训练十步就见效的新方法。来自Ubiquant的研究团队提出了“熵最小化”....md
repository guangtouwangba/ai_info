...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法仅需1条高质量样本进行10步无监督训练，通过降低模型输出熵值(提高答案置信度)提升性能。在数学推理任务中，EM表现媲美传统RL方法，且具备显著优势：无需标注数据/奖励模型、训练成本极低。关键创新在于选择"模型回答不稳定"的问题作为训练样本，例如仅用1道物理题就使Qwen2.5-Math-7B模型数学能力大幅提升。该方法为低成本优化大模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T12:02:30Z
- **目录日期**: 2025-06-01
