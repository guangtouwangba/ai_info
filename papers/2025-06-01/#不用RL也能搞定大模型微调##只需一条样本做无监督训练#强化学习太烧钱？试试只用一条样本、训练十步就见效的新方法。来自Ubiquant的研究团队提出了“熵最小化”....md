...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练，通过降低模型输出分布的熵值，迫使模型选择最自信的答案。实验显示，该方法在数学推理任务中表现优异，甚至超越传统RL方法。关键创新在于选择模型回答不稳定的问题作为训练样本，例如仅用1道物理题就显著提升了Qwen2.5-Math-7B的性能。EM方法具有数据需求极低、训练速度快、效果媲美RL等优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T10:02:35Z
- **目录日期**: 2025-06-01
