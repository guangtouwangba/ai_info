...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法通过降低模型输出分布的熵值，迫使模型选择最自信的答案，仅需1条高质量样本和10步训练就能显著提升性能。关键创新在于选择模型回答不稳定的问题作为训练样本。实验显示，Qwen2.5-Math-7B模型仅用1道物理题训练就大幅提升数学推理能力，效果媲美传统RL方法但成本极低。EM完全无监督，省去了数据标注和奖励模型构建环节，为高效模型优化提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T11:02:41Z
- **目录日期**: 2025-06-01
