...
Mamba核心作者Tri Dao团队发布两种新型注意力机制GTA和GLA，专为提升推理效率设计。实验显示，GTA在中大型模型质量优于GQA，GLA在保持精度的同时效率更高，尤其擅长处理长文本（32K/64K上下文）。新技术实现2倍推理加速，降低内存消耗，并增强并发处理能力，适用于不均衡负载场景。测试涵盖模型质量（困惑度、7项任务）和效率指标（延迟、吞吐量、KV缓存），证实其在不损失生成质量前提下显著提升性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T23:02:58Z
- **目录日期**: 2025-06-01
