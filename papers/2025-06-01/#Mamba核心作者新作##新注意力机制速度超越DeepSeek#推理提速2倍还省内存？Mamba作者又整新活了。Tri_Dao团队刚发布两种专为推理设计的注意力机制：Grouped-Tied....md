...
Tri Dao团队最新发布两种专为推理优化的注意力机制：GTA（分组绑定注意力）和GLA（分组潜在注意力）。这些新方法在保持模型生成质量的同时，显著提升了解码速度（最高2倍）和内存效率，尤其擅长处理长文本场景。测试显示，GTA在中大型模型中表现优于GQA，而GLA在保持精度的同时比MLA更高效，在32K/64K长文本处理时吞吐量优势明显。新机制还展现出更强的并发处理能力，能有效应对不均衡负载，为长上下文推理任务提供了更优解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T17:03:34Z
- **目录日期**: 2025-06-01
