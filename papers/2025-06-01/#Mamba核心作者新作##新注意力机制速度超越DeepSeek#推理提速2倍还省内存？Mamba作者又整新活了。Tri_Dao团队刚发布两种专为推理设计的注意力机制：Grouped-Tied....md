...
Mamba核心作者Tri Dao团队发布两种新型注意力机制GTA和GLA，专为提升推理效率设计。实验显示，GTA在中大型模型质量优于GQA，GLA与MLA精度相当但效率更高，在长文本处理（32K/64K）时吞吐量显著提升，同时优化内存使用和解码速度（提速2倍）。新机制在7个下游任务测试中保持生成质量，并增强并发处理能力，特别适合长上下文和不均衡负载场景。该成果有望替代现有方案如DeepSeek采用的注意力机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T16:03:15Z
- **目录日期**: 2025-06-01
