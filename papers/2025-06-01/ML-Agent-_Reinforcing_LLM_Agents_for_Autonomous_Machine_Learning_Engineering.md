...
本文提出了一种基于强化学习（RL）的自主机器学习（ML）代理框架ML-Agent，通过三个创新点实现高效学习：（1）探索增强微调，提升动作多样性；（2）单步RL训练，加速经验收集；（3）统一奖励模块整合ML反馈。实验表明，仅用7B参数的Qwen-2.5模型在9个任务上训练后，性能超越671B参数的DeepSeek-R1代理，并展现出持续改进和跨任务泛化能力。该研究首次实现了基于学习的ML代理范式，突破了传统依赖人工提示工程的局限，为自适应ML系统开辟了新方向。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T04:13:15Z
- **目录日期**: 2025-06-01
