...
本文提出了一种基于强化学习(RL)的机器学习代理(ML-Agent)训练框架，通过三个创新组件实现自主机器学习：1)探索增强微调，提升动作多样性；2)分步RL训练，加速经验收集；3)统一奖励模块，整合多源反馈。实验表明，仅用7B参数的Qwen-2.5模型在9个任务上训练的ML-Agent，性能超越671B参数的DeepSeek-R1代理，并展现出持续改进能力和优异的跨任务泛化性。该研究首次实现了基于学习的代理式机器学习范式，突破了传统依赖人工提示工程的局限。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T15:01:36Z
- **目录日期**: 2025-06-01
