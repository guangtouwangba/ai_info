...
本文提出了一种基于强化学习的自主机器学习（ML）代理训练框架，通过在线交互实验优化大语言模型（LLM）代理的ML能力。该框架包含三个关键组件：（1）探索增强微调，提升动作多样性；（2）分步强化学习，加速经验收集；（3）统一奖励模块，整合ML反馈信号。实验表明，使用7B参数的Qwen-2.5模型训练的ML-Agent，仅通过9个ML任务的训练，性能即超越671B参数的DeepSeek-R1代理，并展现出持续的改进能力和优异的跨任务泛化性能。这一成果为学习型代理ML开辟了新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T20:01:48Z
- **目录日期**: 2025-06-01
