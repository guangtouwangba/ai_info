...
本文提出了一种基于强化学习（RL）的机器学习代理（ML-Agent）框架，通过交互式实验优化自主机器学习任务。该框架包含三个关键部分：（1）探索增强微调，提升代理生成多样化动作的能力；（2）分步RL训练，加速经验收集；（3）统一的奖励模块，整合不同ML反馈信号。实验表明，仅用7B参数的Qwen-2.5模型在9个任务上训练的ML-Agent，性能超过671B参数的DeepSeek-R1代理，并展现出持续改进和跨任务泛化能力。这一方法突破了传统依赖人工提示工程的局限，实现了自适应的机器学习优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T14:02:21Z
- **目录日期**: 2025-06-01
