...
ZeroGUI是一个创新的在线学习框架，旨在零人工成本下自动化训练纯视觉GUI代理。它通过视觉语言模型（VLM）自动生成多样化任务目标，并利用VLM评估任务完成度，无需人工标注或定制评估函数。结合两阶段在线强化学习，ZeroGUI能持续适应动态交互环境。实验表明，该框架显著提升了UI-TARS和Aguvis等GUI代理在OSWorld和AndroidLab环境中的表现。ZeroGUI解决了现有离线方法依赖人工标注和环境适应性的局限，为GUI自动化提供了高效可扩展的解决方案。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T06:01:09Z
- **目录日期**: 2025-06-01
