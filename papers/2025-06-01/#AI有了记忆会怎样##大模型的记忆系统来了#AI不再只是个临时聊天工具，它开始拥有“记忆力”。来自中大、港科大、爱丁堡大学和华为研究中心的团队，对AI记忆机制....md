...
这项研究系统探讨了大语言模型(LLMs)的记忆机制。研究者从记忆表示(参数化记忆和上下文记忆)和记忆操作两个维度，提出了AI记忆系统的统一框架。参数化记忆指模型内部知识，响应快但难更新；上下文记忆则更灵活，包括非结构化(文本/图像等)和结构化(知识图谱)信息。研究定义了6种核心记忆操作：巩固、更新、索引、遗忘、检索和压缩，这些操作使AI能像人类一样管理记忆。该框架为大模型实现长期交互和个性化服务提供了理论基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T00:02:59Z
- **目录日期**: 2025-06-01
