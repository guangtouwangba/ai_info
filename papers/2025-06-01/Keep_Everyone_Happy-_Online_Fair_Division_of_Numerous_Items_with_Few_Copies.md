...
这篇论文提出了一种新的在线公平分配问题变体，其中学习者需顺序观察不可分割的项目，并将其不可撤销地分配给多个代理之一，同时满足公平性和效率约束。现有算法假设项目数量少且副本足够多，从而能从噪声反馈中准确估计效用。然而，现实场景（如在线平台）往往项目多而副本少，导致效用估计困难。为此，作者假设效用是项目-代理特征的未知函数，并将问题建模为上下文赌博机问题，提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T00:02:50Z
- **目录日期**: 2025-06-01
