...
这篇论文研究了一种新型在线公平分配问题，其中学习者需依次将不可分割的物品分配给多个代理，同时满足公平性和效率约束。现有算法假设物品数量少且副本充足，能通过噪声反馈准确估计效用，但这在现实中往往不成立（如平台用户多但使用频次低）。为此，作者提出将效用建模为物品-代理特征的未知函数，将问题转化为上下文赌博机任务，并设计了具有次线性遗憾保证的算法。实验验证了该方法的有效性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T07:02:30Z
- **目录日期**: 2025-06-01
