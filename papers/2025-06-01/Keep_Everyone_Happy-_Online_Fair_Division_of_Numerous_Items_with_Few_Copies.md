...
这篇论文提出了一种新的在线公平分配问题变体，其中学习者需顺序观察不可分割物品并将其不可撤销地分配给多个代理，同时满足公平和效率约束。现有算法假设物品数量少且有足够副本，但现实中常遇到物品多而副本少的情况（如平台用户多但使用服务次数少），导致难以准确估计效用。为此，作者将效用建模为物品-代理特征的未知函数，将问题转化为上下文赌博机问题，并提出具有次线性遗憾保证的算法。实验结果验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T16:02:31Z
- **目录日期**: 2025-06-01
