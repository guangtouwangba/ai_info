...
这篇论文研究了一种新型在线公平分配问题，其中学习者需依次将不可分割的物品分配给多个代理，同时满足公平性和效率约束。现有算法假设物品数量少且副本充足，能通过噪声反馈准确估计效用。然而，现实场景（如在线平台）中物品（用户）多而副本（使用次数）少，传统方法难以适用。为此，作者假设效用是物品-代理特征的未知函数，将问题建模为上下文赌博机问题，提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T15:02:19Z
- **目录日期**: 2025-06-01
