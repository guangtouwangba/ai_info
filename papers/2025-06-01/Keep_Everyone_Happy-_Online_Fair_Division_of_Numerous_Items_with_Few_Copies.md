...
这篇论文提出了一种新的在线公平分配问题变体，其中学习者需依次将不可分割的项目分配给多个代理，同时满足公平性和效率约束。现有算法假设项目数量少且有足够副本，但现实中项目（如平台用户）可能很多而使用次数（副本）很少，导致效用估计困难。为此，作者假设效用是项目-代理特征的未知函数，将问题建模为情境赌博机问题，并提出了具有次线性遗憾保证的算法。实验验证了算法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T04:13:49Z
- **目录日期**: 2025-06-01
