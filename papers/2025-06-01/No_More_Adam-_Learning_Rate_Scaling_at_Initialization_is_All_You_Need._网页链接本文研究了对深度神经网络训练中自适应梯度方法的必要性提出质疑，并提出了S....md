...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度学习中对自适应梯度方法（如Adam）的依赖。该方法基于带动量的随机梯度下降（SGDM），在初始化时根据各参数组的梯度信号噪声比（g-SNR）调整学习率，从而避免训练不平衡问题。相比AdamW，SGD-SaI内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，并显著节省内存（如为GPT-2节省5.93GB）。此外，它在LoRA微调等任务中也优于现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T01:32:08Z
- **目录日期**: 2025-06-01
