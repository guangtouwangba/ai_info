...
香港理工大学团队研究发现，大语言模型的"遗忘"可能只是表面现象（伪遗忘），而非真正的记忆擦除。他们开发了一套分析工具箱，通过评估模型内部结构变化（如参数敏感区域、表示空间漂移等）来区分可逆与不可逆遗忘。关键结论是：仅当多个网络层发生大规模参数扰动时才是真遗忘，否则模型仍能恢复"记忆"。该研究首次系统定义了遗忘标准，为AI安全领域提供了重要评估工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T08:02:44Z
- **目录日期**: 2025-06-01
