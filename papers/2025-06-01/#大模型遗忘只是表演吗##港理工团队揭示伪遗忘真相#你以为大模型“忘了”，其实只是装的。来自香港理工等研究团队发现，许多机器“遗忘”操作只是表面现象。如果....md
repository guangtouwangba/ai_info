...
香港理工大学团队研究发现，大语言模型的"遗忘"存在"伪遗忘"和"真遗忘"两种状态。仅输出表现变化但内部结构未改变时，模型只是"伪遗忘"（可逆）；当多个网络层发生大范围参数扰动导致表示方向、分布中心等根本性改变时，才是"真遗忘"（不可逆）。团队开发了包含PCA分析、表示漂移检测等工具的分析系统，能准确区分这两种状态。研究表明，仅凭准确率下降不能证明模型真正遗忘，关键要看内部结构的实质性变化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T16:03:06Z
- **目录日期**: 2025-06-01
