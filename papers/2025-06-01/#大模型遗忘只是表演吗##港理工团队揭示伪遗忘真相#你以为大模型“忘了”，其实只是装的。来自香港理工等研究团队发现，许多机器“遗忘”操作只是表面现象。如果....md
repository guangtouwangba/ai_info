...
香港理工团队研究发现，大模型的"遗忘"可能是伪装的。他们开发了表示空间分析工具箱，区分"伪遗忘"(可逆)和"真遗忘"(不可逆)。关键发现是：仅当多个网络层发生大范围扰动才算真正遗忘；若内部结构未变，即使输出表现下降，模型仍能恢复记忆。工具箱通过PCA方向变化、表示漂移、CKA相似性分析等方法精确评估遗忘性质。真遗忘会导致表示方向、分布中心等全方位改变且无法通过再学习恢复。该研究首次系统揭示了机器学习中遗忘现象的本质特征。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T11:02:50Z
- **目录日期**: 2025-06-01
