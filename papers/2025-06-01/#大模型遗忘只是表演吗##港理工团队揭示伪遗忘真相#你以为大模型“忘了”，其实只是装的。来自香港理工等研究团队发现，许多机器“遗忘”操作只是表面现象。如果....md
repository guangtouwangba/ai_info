...
香港理工大学团队研究发现，大语言模型的"遗忘"可能是表面现象（伪遗忘）。他们开发的分析工具表明，仅当模型多层结构发生显著扰动时才是真遗忘。若内部结构未变，即使输出性能下降，模型仍可恢复记忆。研究通过PCA、CKA相似性等多维度指标，首次系统区分了可逆与不可逆遗忘。真遗忘会导致表示方向、参数敏感区等根本性改变，而伪遗忘只是暂时性表现变化。该成果为评估AI记忆机制提供了新方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T14:03:30Z
- **目录日期**: 2025-06-01
