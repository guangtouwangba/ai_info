...
香港理工团队研究发现，大模型的"遗忘"可能只是表象。他们开发了分析工具箱，区分"伪遗忘"(可逆)和"真遗忘"(不可逆)。关键发现：仅当多个网络层发生大范围参数扰动时才是真正遗忘。伪遗忘表现为输出指标变化但内部结构未变，模型可轻易恢复；真遗忘则会导致表示方向、分布中心等根本性改变。该研究通过PCA、CKA等多维度分析方法，首次系统验证了模型遗忘的本质特征，为理解AI记忆机制提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T22:02:51Z
- **目录日期**: 2025-06-01
