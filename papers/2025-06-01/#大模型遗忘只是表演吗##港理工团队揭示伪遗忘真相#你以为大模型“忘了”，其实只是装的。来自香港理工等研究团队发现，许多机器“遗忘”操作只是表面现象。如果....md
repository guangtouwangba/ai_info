...
香港理工团队研究发现，大模型的"遗忘"可能是"伪遗忘"——仅输出表现变化而内部结构未变，仍可恢复记忆。团队开发了多维度分析工具箱（PCA、CKA相似性等），区分真伪遗忘：真遗忘需多层网络大范围扰动，导致表示方向、参数敏感区等不可逆变化；伪遗忘仅表面指标（准确率、困惑度）变化，结构未变时可逆。该研究首次系统定义了可逆与不可逆遗忘机制，为评估模型记忆擦除效果提供了量化标准。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T09:02:52Z
- **目录日期**: 2025-06-01
