...
香港理工大学团队研究发现，大语言模型的"遗忘"可能是表面现象（伪遗忘），而非真正的知识消除。他们开发了一套分析工具，通过检测模型内部结构变化（如参数敏感区域、表示方向等）来区分真伪遗忘。关键结论是：仅输出指标变化而内部结构未改变时，模型仍可恢复记忆；真正的遗忘需要多层网络发生不可逆的大范围扰动。该研究为评估AI模型的知识消除提供了系统性方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T17:03:24Z
- **目录日期**: 2025-06-01
