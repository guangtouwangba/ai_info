...
香港理工大学等研究团队发现，大语言模型的"遗忘"可能是伪装的。他们开发了表示空间分析工具箱，区分"伪遗忘"(可逆)和"真遗忘"(不可逆)。关键发现是：仅当多个网络层发生大范围参数扰动时才是真遗忘，否则模型仍能恢复记忆。工具箱通过PCA方向变化、表示漂移、CKA相似性等指标精确评估遗忘程度。研究表明，输出性能下降但内部结构未变时，模型只是"表演"遗忘，仍可通过微调恢复记忆。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T23:02:50Z
- **目录日期**: 2025-06-01
