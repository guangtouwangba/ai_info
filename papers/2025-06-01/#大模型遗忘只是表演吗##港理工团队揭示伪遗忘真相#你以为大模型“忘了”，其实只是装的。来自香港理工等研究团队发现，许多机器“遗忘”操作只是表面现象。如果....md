...
香港理工大学等研究团队发现，大语言模型的"遗忘"可能是表面现象（伪遗忘）。他们开发了表示空间分析工具箱，通过PCA、CKA相似性等多维度指标区分真/伪遗忘。关键结论是：仅当模型多层参数发生不可逆扰动（如表示方向漂移、Fisher信息变化）才属真实遗忘；若仅输出指标变化而结构未变，模型仍可恢复记忆。该研究首次系统定义了可逆与不可逆遗忘的判定标准，为评估AI记忆机制提供了量化工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T13:09:41Z
- **目录日期**: 2025-06-01
