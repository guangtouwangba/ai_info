...
香港理工大学团队研究发现，大语言模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发的分析工具表明，只有当模型多层结构发生显著参数扰动时，才属于不可逆的"真遗忘"。若仅输出指标变化而内部结构未变，模型仍能恢复记忆。该研究通过PCA、CKA等多维度分析方法，首次系统区分了可逆与不可逆遗忘，为评估模型记忆机制提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T21:02:41Z
- **目录日期**: 2025-06-01
