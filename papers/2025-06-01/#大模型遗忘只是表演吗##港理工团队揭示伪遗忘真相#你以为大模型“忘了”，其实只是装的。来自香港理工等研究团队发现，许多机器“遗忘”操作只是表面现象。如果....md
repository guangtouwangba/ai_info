...
香港理工团队研究发现，大模型的"遗忘"可能只是表面现象（伪遗忘）。通过开发表示空间分析工具箱，他们区分了可逆的伪遗忘和不可逆的真遗忘。关键发现是：只有当多个网络层发生大范围参数扰动时才是真遗忘；若仅输出指标变化而内部结构未变，模型仍可恢复记忆。研究采用PCA、CKA等多维度分析方法，证实真正的遗忘会导致表示空间全方位漂移且无法通过再学习恢复。该成果为评估模型遗忘机制提供了系统性方法论。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T19:03:06Z
- **目录日期**: 2025-06-01
