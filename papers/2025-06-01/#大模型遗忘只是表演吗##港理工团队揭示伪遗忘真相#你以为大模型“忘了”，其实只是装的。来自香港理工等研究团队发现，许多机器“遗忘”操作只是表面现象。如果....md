...
香港理工大学等研究团队发现，大语言模型的"遗忘"可能只是表面现象（伪遗忘）。他们开发了一套分析工具箱，通过评估模型内部结构变化（如表示方向、参数敏感区域等）来区分"伪遗忘"和"真遗忘"。关键发现是：只有当多个网络层发生大范围扰动时才是真正的不可逆遗忘；若仅输出表现变差而内部结构未变，模型仍能恢复记忆。该研究首次系统性地揭示了机器学习中的遗忘机制，为模型安全评估提供了新方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T18:03:49Z
- **目录日期**: 2025-06-01
