...
香港理工团队研究发现，大模型的"遗忘"可能是伪装的。他们开发了分析工具箱，区分"伪遗忘"（可逆）和"真遗忘"（不可逆）。关键发现是：仅当多个网络层发生大范围参数扰动时才是真遗忘，否则模型仍能恢复记忆。工具箱通过PCA方向变化、表示漂移、CKA相似性等多维度评估遗忘真实性。真遗忘会导致表示方向、分布中心等根本性改变，而伪遗忘只是输出表现变化但内部结构未变。该研究为评估模型遗忘机制提供了系统性方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T10:02:44Z
- **目录日期**: 2025-06-01
