...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时优化SyncNet训练框架，解决了其收敛问题，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可推广至其他依赖SyncNet的唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T14:06:24Z
- **目录日期**: 2025-06-01
