...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，但发现扩散过程中存在时间一致性问题。为此，作者提出时间表示对齐(TREPA)方法，通过自监督视频模型提取的时间特征增强生成帧的时序一致性。同时，针对SyncNet收敛问题进行了系统分析，改进了其训练框架，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进也可推广至其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T02:46:54Z
- **目录日期**: 2025-06-01
