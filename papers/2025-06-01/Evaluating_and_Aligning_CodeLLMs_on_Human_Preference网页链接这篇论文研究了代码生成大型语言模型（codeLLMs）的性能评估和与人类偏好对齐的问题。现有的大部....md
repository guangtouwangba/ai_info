...
这篇论文针对代码生成大模型(codeLLMs)提出创新评估方法，发现现有基准过于侧重代码正确性而忽略人类偏好。研究者构建了CodeArena基准(含397个高质量样本，覆盖40类任务和44种语言)和SynCode-Instruct语料库(近200亿标记)，首次系统评估了40多个模型。实验显示：1)执行基准与人类偏好存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距明显。研究强调代码生成模型需要同时满足功能正确性和人类编程习惯，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T03:33:21Z
- **目录日期**: 2025-06-01
