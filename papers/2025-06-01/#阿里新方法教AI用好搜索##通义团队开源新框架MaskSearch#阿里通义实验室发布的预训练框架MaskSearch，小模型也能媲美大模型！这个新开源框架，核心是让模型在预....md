...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让AI模型在预训练阶段学会结合搜索引擎进行推理。该框架创新性地融合了监督微调(SFT)和强化学习(RL)方法，采用多智能体生成思维链和混合奖励系统优化推理路径，并引入课程学习策略提升泛化能力。实验显示，采用MaskSearch的小模型在跨领域问答任务中表现优异，甚至能媲美大模型性能。研究还揭示了掩码策略和奖励函数对模型学习效果的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T05:03:35Z
- **目录日期**: 2025-06-01
