...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让AI模型在预训练时学会结合搜索引擎进行推理。该框架创新性地融合了监督微调(SFT)和强化学习(RL)方法，采用多智能体生成思维链和混合奖励机制优化推理路径，并引入课程学习策略提升泛化能力。实验显示，采用MaskSearch的小模型在问答任务中表现优异，跨领域性能甚至媲美大模型。研究还揭示了掩码策略和奖励函数对模型性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T19:04:02Z
- **目录日期**: 2025-06-01
