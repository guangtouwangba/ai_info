...
阿里通义实验室开源了预训练框架MaskSearch，通过"检索增强型掩码预测"(RAMP)技术，让AI模型在训练时学会结合搜索引擎进行推理。该框架创新性地融合了监督微调(SFT)和强化学习(RL)，采用多智能体生成思维链和混合奖励机制优化推理路径。实验显示，采用MaskSearch的小模型在问答任务中表现优异，甚至能媲美大模型，尤其在跨领域任务上展现出强大泛化能力。研究还发现掩码策略和奖励函数对模型性能有显著影响，验证了"教会模型如何学习"的技术价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T15:03:37Z
- **目录日期**: 2025-06-01
