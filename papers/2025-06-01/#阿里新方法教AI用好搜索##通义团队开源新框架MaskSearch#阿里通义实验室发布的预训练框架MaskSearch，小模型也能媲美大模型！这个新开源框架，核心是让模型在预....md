...
阿里通义实验室开源预训练框架MaskSearch，通过检索增强型掩码预测(RAMP)技术，使小模型性能媲美大模型。该框架创新性地让AI在预训练时学习结合搜索引擎进行推理，采用多智能体+教师模型生成思维链，并通过混合奖励系统优化推理路径。特别设计了课程学习策略逐步提升难度，显著增强模型泛化能力。实验显示其在跨领域任务中表现优异，不同掩码策略和奖励函数对效果影响显著。该研究为"让模型学会学习"提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T22:03:48Z
- **目录日期**: 2025-06-01
