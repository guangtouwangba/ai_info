...
阿里通义实验室开源了MaskSearch预训练框架，通过"检索增强型掩码预测"(RAMP)技术，让AI模型在预训练时学会结合搜索引擎进行推理。该框架创新性地融合了监督微调(SFT)和强化学习(RL)两种训练方式：SFT采用多智能体生成高质量思维链，RL则通过混合奖励机制优化推理路径。实验显示，采用课程学习策略的MaskSearch能显著提升小模型性能，在跨领域任务中可媲美大模型。研究还发现掩码策略和奖励函数对模型表现有重要影响，验证了训练方法设计的关键性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T01:30:51Z
- **目录日期**: 2025-06-01
