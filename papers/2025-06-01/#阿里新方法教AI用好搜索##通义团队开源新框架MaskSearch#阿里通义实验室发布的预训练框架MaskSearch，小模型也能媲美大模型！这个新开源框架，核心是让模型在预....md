...
阿里通义实验室开源了预训练框架MaskSearch，通过检索增强型掩码预测(RAMP)技术，让小模型也能媲美大模型性能。该框架创新性地结合了监督微调(SFT)和强化学习(RL)，采用多智能体生成思维链和混合奖励机制优化推理路径。通过课程学习策略逐步提升任务难度，显著增强了模型泛化能力。实验显示，在跨领域问答任务中，采用MaskSearch的小模型表现接近大模型。研究还发现掩码策略和奖励函数对性能有重要影响，验证了"教会模型如何学习"的技术价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T10:03:41Z
- **目录日期**: 2025-06-01
