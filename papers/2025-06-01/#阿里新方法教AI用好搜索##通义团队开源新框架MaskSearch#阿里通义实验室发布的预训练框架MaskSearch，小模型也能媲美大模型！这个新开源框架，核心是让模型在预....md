...
阿里通义实验室开源预训练框架MaskSearch，通过检索增强型掩码预测(RAMP)技术，使小模型能媲美大模型性能。该框架创新性地结合监督微调(SFT)和强化学习(RL)，采用多智能体生成思维链和混合奖励机制优化推理路径，并引入课程学习策略提升泛化能力。实验显示其在跨领域问答任务中表现优异，验证了不同掩码策略和奖励函数对模型性能的关键影响。该研究突破了传统预训练模式，通过"边推理边搜索"的机制显著提升了小模型的知识获取能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T20:03:53Z
- **目录日期**: 2025-06-01
