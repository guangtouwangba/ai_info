...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让模型学会结合搜索引擎进行推理。该框架创新性地兼容监督微调(SFT)和强化学习(RL)训练，采用多智能体生成思维链和混合奖励系统优化推理路径，并引入课程学习提升泛化能力。实验显示，该方法使小模型在问答任务上媲美大模型性能，尤其在跨领域表现突出。研究还揭示了掩码策略和奖励函数对模型学习效果的重要影响，为提升AI搜索推理能力提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T17:05:15Z
- **目录日期**: 2025-06-01
