...
阿里通义实验室开源了预训练框架MaskSearch，通过检索增强型掩码预测(RAMP)技术，让模型在预训练阶段学会结合搜索引擎进行推理。该框架创新性地兼容监督微调(SFT)和强化学习(RL)训练，采用多智能体生成思维链和混合奖励系统优化推理路径，并引入课程学习提升泛化能力。实验显示，采用MaskSearch的小模型在跨领域任务中表现媲美大模型，验证了该方法在提升模型推理效率方面的有效性。研究还揭示了不同掩码策略和奖励函数对性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T09:03:53Z
- **目录日期**: 2025-06-01
