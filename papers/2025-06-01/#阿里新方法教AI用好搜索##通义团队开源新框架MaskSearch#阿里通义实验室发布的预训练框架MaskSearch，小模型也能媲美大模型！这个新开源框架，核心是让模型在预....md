...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让AI模型学会在推理时主动使用搜索引擎获取外部知识。该框架创新性地结合了监督微调（多智能体+教师模型生成思维链）和强化学习（混合奖励系统优化推理路径），并采用课程学习策略提升泛化能力。实验显示，采用该方法的小模型在跨领域问答任务中表现媲美大模型，同时揭示了掩码策略和奖励函数对模型性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T07:03:09Z
- **目录日期**: 2025-06-01
