...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让AI模型在预训练阶段学会结合搜索引擎进行推理。该框架创新性地兼容监督微调(SFT)和强化学习(RL)训练：SFT采用多智能体生成思维链，RL通过混合奖励优化推理路径。结合课程学习策略，显著提升了小模型的泛化能力。实验表明，该方法使小模型在跨领域任务中表现媲美大模型，同时揭示了掩码策略和奖励函数对性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T12:03:49Z
- **目录日期**: 2025-06-01
