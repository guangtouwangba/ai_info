...
ThinkGeo是一个针对遥感领域的智能体基准测试，旨在评估大语言模型（LLM）通过结构化工具使用和多步规划解决复杂任务的能力。该基准包含436个涉及城市规划、灾害评估、环境监测等实际应用的查询任务，要求智能体基于卫星/航空影像进行空间推理。研究采用ReAct式交互循环，测试了GPT-4o、Qwen2.5等开源/闭源模型，发现不同模型在工具准确性和规划一致性上存在显著差异。ThinkGeo首次为评估工具增强型LLM的遥感空间推理能力提供了全面测试平台，相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-01T22:01:13Z
- **目录日期**: 2025-06-01
