...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过根据参数组的梯度信号噪声比（g-SNR）在初始化时调整学习率，无需自适应二阶动量即可避免训练不平衡。该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或等同于AdamW，同时将内存使用减半（例如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB），在LoRA微调等任务中也超越现有优化器，具有更强的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T16:04:50Z
- **目录日期**: 2025-05-19
