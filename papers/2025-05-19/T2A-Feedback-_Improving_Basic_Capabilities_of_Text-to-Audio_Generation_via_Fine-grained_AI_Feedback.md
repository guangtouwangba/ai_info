...
该研究提出了一种基于AI反馈学习的文本到音频(T2A)生成改进方法。针对现有模型在复杂多事件音频生成中存在的提示跟随和音质问题，研究者开发了三个细粒度评分管道：事件出现评分、事件顺序评分和音质评分。这些自动评分与人类偏好显著相关，优于现有评估指标。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并创建了专注于长文本、多事件场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调整，该方法能显著提升现有音频生成模型在简单和复杂场景下的表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T00:01:14Z
- **目录日期**: 2025-05-19
