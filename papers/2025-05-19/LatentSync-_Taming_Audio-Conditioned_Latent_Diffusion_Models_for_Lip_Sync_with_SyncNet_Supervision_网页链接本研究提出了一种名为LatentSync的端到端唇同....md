...
这篇论文提出了LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。与现有方法不同，它无需中间运动表示，直接利用Stable Diffusion建模音频-视觉关联。针对扩散模型的时间一致性问题，作者提出了TREPA方法，通过自监督视频模型的时间表示来对齐生成帧。研究还改进了SyncNet训练框架，解决了其收敛问题，并分析了架构、超参数和数据预处理的影响。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进也可推广到其他音频驱动的人像动画任务中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T23:05:24Z
- **目录日期**: 2025-05-19
