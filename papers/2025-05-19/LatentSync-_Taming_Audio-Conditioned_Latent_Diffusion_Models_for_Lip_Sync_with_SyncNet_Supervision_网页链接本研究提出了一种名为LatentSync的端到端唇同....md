...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂音视频关系，并提出时间表示对齐（TREPA）方法解决扩散过程中时间一致性问题。研究还改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。创新点包括：1）潜在扩散模型直接生成唇同步视频；2）TREPA增强时间一致性；3）SyncNet训练优化方案。成果可推广至其他音视频同步任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T14:05:45Z
- **目录日期**: 2025-05-19
