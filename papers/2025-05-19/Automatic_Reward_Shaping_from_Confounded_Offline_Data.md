...
本文提出了一种新型深度强化学习算法，针对复杂高维环境中存在未观测混杂因素导致的偏差数据问题。该算法基于深度Q网络(DQN)，通过寻找与观测数据兼容的最坏情况下安全策略，增强了对混杂偏差的鲁棒性。在12个存在混杂因素的Atari游戏测试中，该算法在行为策略与目标策略输入不匹配且存在未观测混杂因素的所有游戏中，表现均优于标准DQN。研究表明该方法能有效处理观测数据中的混杂偏差，提升决策性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T15:01:15Z
- **目录日期**: 2025-05-19
