...
这篇论文提出了一种新型深度强化学习算法，用于解决在存在未观测混杂因素的高维复杂环境中的离策略学习问题。基于深度Q网络(DQN)，该算法能够处理观测数据中的混杂偏差，通过寻找与观测数据兼容的最坏情况下环境的安全策略。研究在12个存在混杂因素的Atari游戏上测试了该算法，结果表明在行为策略和目标策略输入不匹配且存在未观测混杂因素的情况下，该算法始终优于标准DQN。这一成果为在现实世界复杂环境中开发稳健的AI决策系统提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T06:01:15Z
- **目录日期**: 2025-05-19
