...
过去十年，机器学习模型规模呈指数级增长，导致训练和运行成本高达数百万美元，阻碍了个人参与并推高了AI使用成本。现有剪枝方法（如迭代幅度剪枝）虽能保持精度，但需多次迭代训练，计算和环境代价高昂。为此提出的BINGO技术，能在单次训练中评估神经网络各权重的重要性，训练完成后直接生成权重重要性评分，实现一次性精准剪枝。相比现有方法，BINGO以更低计算成本保持模型精度，为"AI发展不必伴随模型膨胀"提供了可行方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T21:02:16Z
- **目录日期**: 2025-05-19
