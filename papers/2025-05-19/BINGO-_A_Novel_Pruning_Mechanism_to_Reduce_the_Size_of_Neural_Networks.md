...
过去十年，机器学习模型规模急剧扩大，导致训练和运行成本高达数百万美元，阻碍了非富裕群体参与AI发展。现有剪枝方法（如迭代幅度剪枝）虽能保持精度，但计算量和环境成本极高。为此，研究者提出BINGO技术：在训练过程中逐次分析神经网络子集，评估各权重对模型精度的重要性，最终为每个权重生成重要性评分，实现一次性高效剪枝。BINGO在降低计算强度的同时保持模型精度，为"AI发展不必伴随模型膨胀"提供了解决方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T10:02:11Z
- **目录日期**: 2025-05-19
