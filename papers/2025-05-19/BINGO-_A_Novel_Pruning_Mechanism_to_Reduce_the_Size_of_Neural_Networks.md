...
过去十年，机器学习模型变得极其庞大复杂，训练成本高昂，阻碍了资源不足者的参与。现有剪枝方法（如迭代幅度剪枝）虽能保持精度，但计算量和环境代价巨大。为此，研究者提出BINGO方法：在训练过程中逐次分析神经网络子集，评估各权重对模型精度的重要性，最终为每个权重生成重要性评分，实现一次性剪枝。BINGO能以较低计算成本完成高精度剪枝，打破"AI发展必须增大模型"的困境，降低技术门槛和资源消耗。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T08:02:12Z
- **目录日期**: 2025-05-19
