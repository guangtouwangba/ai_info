...
传统多智能体强化学习（MARL）需要全局状态可观测性，限制了算法的去中心化和可扩展性。近期研究表明，在智能体间影响衰减的假设下，全局观测可被局部邻域观测替代。本文以雷达网络中的目标检测功率分配为例，证明信号衰减特性（如无线通信中的路径损耗）能实现MARL的去中心化。作者提出两种约束多智能体马尔可夫决策过程模型，推导了全局价值函数和梯度的局部邻域近似及其误差界，并开发了去中心化鞍点策略梯度算法。该方法为无线通信和雷达网络的其他问题提供了扩展模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T03:23:30Z
- **目录日期**: 2025-05-19
