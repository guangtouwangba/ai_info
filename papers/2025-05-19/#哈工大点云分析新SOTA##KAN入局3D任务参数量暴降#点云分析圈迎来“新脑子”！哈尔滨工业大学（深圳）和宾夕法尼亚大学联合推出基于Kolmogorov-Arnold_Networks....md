...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs的新型点云分析模型PointKAN，通过可学习函数替代传统MLP的固定激活函数，显著提升几何特征学习能力。其核心模块包括几何仿射、局部特征处理（KAN Block+深度卷积）和全局特征提取（残差块）。升级版PointKAN-elite采用高效有理函数和参数共享技术，在降低参数量和计算开销的同时，于分类、分割等3D任务中全面超越MLP方案，实现"参数量暴降，精度反升"的突破。该成果在多个数据集上达到SOTA性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T21:03:26Z
- **目录日期**: 2025-05-19
