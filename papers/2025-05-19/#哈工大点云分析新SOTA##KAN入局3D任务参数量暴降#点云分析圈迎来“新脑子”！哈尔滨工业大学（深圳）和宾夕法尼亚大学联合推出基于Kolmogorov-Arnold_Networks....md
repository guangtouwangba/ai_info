...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs的PointKAN模型，革新3D点云分析。该模型通过可学习函数替代传统MLP固定激活函数，采用几何仿射模块、局部特征并行处理及残差全局特征提取结构，显著提升几何特征学习能力。升级版PointKAN-elite引入高效有理函数和参数共享，在降低80%参数量的同时保持精度。实验显示，该模型在分类、分割等任务中全面超越传统MLP方案，成为新SOTA。成果实现参数量暴降与精度提升的双突破。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T15:03:17Z
- **目录日期**: 2025-05-19
