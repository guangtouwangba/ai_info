...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs的PointKAN模型，革新3D点云分析。该模型通过可学习函数替代传统MLP固定激活函数，结合几何仿射模块、局部/全局特征处理结构，显著提升复杂几何特征提取能力。升级版PointKAN-elite引入高效有理函数和参数共享，参数量暴降80%的同时，在分类、分割等任务中性能超越MLP方案，实现精度与效率双突破。实验显示其在多个数据集上达到SOTA水平，为3D感知任务提供了轻量化新范式。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T16:03:20Z
- **目录日期**: 2025-05-19
