...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs（Kolmogorov-Arnold Networks）的点云分析新模型PointKAN，通过可学习函数替代传统MLP的固定激活函数，显著提升3D几何特征学习能力。其核心模块包括几何仿射、局部特征并行处理及全局残差提取，升级版PointKAN-elite进一步采用有理函数和参数共享技术，实现参数量暴降（最高减少98.8%）的同时，在分类、分割等任务中性能超越MLP方案，成为点云分析领域的新SOTA。相关成果已在多个基准数据集验证有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T10:03:19Z
- **目录日期**: 2025-05-19
