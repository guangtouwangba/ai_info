...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs的PointKAN模型，革新3D点云分析。该模型通过可学习函数替代传统MLP的固定激活函数，采用几何仿射模块、局部特征并行处理和残差全局特征提取等创新结构。升级版PointKAN-elite引入高效有理函数和参数共享技术，显著降低参数量。实验显示，该模型在分类、分割等任务中全面超越传统方法，实现"参数量暴降，精度反升"的突破，成为点云分析领域的新SOTA（最先进技术）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T19:03:03Z
- **目录日期**: 2025-05-19
