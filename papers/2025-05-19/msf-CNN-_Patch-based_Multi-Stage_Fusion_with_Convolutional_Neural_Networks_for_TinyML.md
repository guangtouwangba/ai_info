...
本文介绍了msf-CNN，一种针对微控制器（MCU）优化的新型卷积神经网络融合技术。该方法通过将融合解空间表示为有向无环图，高效搜索最优融合方案，显著扩展了可行解范围。相比现有方案（MCUNetV2和StreamNet），msf-CNN在多种MCU架构（ARM Cortex-M/RISC-V/ESP32）上实现运行时内存消耗降低50%，同时保持实时推理性能。该技术通过创新的分块融合策略优化神经网络层间数据流，为系统设计者在有限内存（如128kB RAM）条件下提供了更大的设计灵活性，有效平衡了内存占用与计算延迟的矛盾。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T17:01:04Z
- **目录日期**: 2025-05-19
