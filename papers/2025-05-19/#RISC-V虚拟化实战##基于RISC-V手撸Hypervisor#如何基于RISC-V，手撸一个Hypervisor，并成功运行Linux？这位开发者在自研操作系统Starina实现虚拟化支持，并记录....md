...
该开发者详细记录了在自研操作系统Starina上实现RISC-V虚拟化的全过程。通过RISC-V的H-extension进入Guest模式，逐步构建页表、处理系统调用、添加设备树支持，并解决时间读取和中断注入问题。最终模拟MMIO设备并接入virtio-fs文件系统，成功在Hypervisor上运行Linux内核。整个过程利用QEMU的RISC-V H-extension模拟和GDB调试工具，开发体验流畅。这一实践展示了从零构建轻量级虚拟机环境的关键步骤和技术细节。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T02:34:47Z
- **目录日期**: 2025-05-19
