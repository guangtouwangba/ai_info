...
阿里通义实验室提出"ZeroSearch"新方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习形成闭环。实验显示，仅用3B/7B参数的小模型，在单跳和多跳问答任务上就能媲美谷歌搜索效果，同时避免了真实API的高成本问题。这一创新解决了传统方法依赖搜索引擎导致的文档质量不稳定、训练成本高和扩展性差等痛点。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T01:32:14Z
- **目录日期**: 2025-05-19
