...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成文档数据，采用课程式抗噪训练（从高质量文档逐步加入噪声），并结合强化学习形成闭环训练。实验显示，仅用3B/7B参数的小模型就能在单跳/多跳问答任务上媲美甚至超越传统基于谷歌搜索的方法，同时实现零API成本。该方案有效解决了传统方法面临的文档质量差、成本高和扩展难三大问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T02:34:59Z
- **目录日期**: 2025-05-19
