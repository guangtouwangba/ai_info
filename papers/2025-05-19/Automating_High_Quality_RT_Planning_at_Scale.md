...
放疗（RT）规划复杂且耗时，人工智能（AI）可提升其精度和效率，但数据稀缺限制了进展。为此，我们开发了自动迭代放疗规划系统（AIRTP），通过AI集成放疗软件（如Varian Eclipse），自动化关键步骤（如器官轮廓勾画、射束设置和优化），生成高质量治疗计划。该系统提出的新方法可将剂量预测转化为可执行的计划，其质量与人工规划相当，但大幅节省时间。我们公开了头颈癌和肺癌的9组数据（规模超现有最大公共数据集10倍），以支持2025年AAPM挑战赛。详见GitHub仓库。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T14:02:32Z
- **目录日期**: 2025-05-19
