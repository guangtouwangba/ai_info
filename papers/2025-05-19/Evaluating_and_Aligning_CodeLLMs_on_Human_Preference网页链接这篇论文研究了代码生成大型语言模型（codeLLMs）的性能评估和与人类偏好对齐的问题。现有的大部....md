...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究者构建了包含397个高质量样本的评估集(覆盖40类任务和44种语言)，并开发了20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调了代码生成模型除功能性外，还需重视人类编程习惯和偏好的对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T06:06:17Z
- **目录日期**: 2025-05-19
