...
这篇论文针对代码生成大模型(codeLLMs)提出了两个重要贡献：1)创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2)构建了20B标记量的SynCode-Instruct合成指令数据集用于微调。研究发现现有基于执行的评估与人类偏好存在显著差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面存在明显差距，凸显了代码生成模型优化方向需要更多关注人类偏好因素。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T03:27:41Z
- **目录日期**: 2025-05-19
