...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，发现现有基准过于侧重代码正确性而忽视人类偏好。研究者创建了CodeArena基准（含397个高质量样本，覆盖40类/44种语言）和20B标记的SynCode-Instruct合成指令集。实验显示：1）传统执行测试与人类偏好存在偏差；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调了代码生成模型与人类偏好对齐的重要性，为改进模型评估提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T18:06:53Z
- **目录日期**: 2025-05-19
