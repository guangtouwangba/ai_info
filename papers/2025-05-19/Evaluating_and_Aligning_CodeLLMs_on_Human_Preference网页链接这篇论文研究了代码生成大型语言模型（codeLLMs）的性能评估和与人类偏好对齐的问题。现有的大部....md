...
这篇论文针对代码生成大型语言模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，专门评估模型输出与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令数据集用于模型微调。研究发现现有基于执行的评估与人类偏好存在显著差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显，凸显了人类偏好对齐在代码生成领域的重要性。研究为提升代码LLMs的实用性提供了新评估框架和数据支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T10:06:19Z
- **目录日期**: 2025-05-19
