...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)构建CodeArena基准测试(397个样本/40类/44种语言)，首次评估模型输出与人类偏好的匹配度；2)开发20B标记的SynCode-Instruct合成指令集验证微调效果。研究发现：传统基于执行的测试与人类偏好存在显著差异，开源模型(Qwen2.5-Coder)与商业模型(OpenAI o1)性能差距明显。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的契合度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T23:05:42Z
- **目录日期**: 2025-05-19
