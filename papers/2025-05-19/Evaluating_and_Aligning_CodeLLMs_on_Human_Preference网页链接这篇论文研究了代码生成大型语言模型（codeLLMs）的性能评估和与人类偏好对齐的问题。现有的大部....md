...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，以评估模型在真实场景中的表现。同时开发了SynCode-Instruct合成指令数据集(约20B标记)验证指令微调效果。实验发现：1)传统执行基准与CodeArena存在性能差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)差距显著。研究强调了代码LLMs与人类偏好对齐的重要性，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T11:06:14Z
- **目录日期**: 2025-05-19
