...
AgentSociety是一个基于大语言模型的大规模社会模拟器，能模拟1万多个代理的社会互动（500万次）。研究者用它分析四大社会问题：社会极化、信息传播、基本收入政策及自然灾害影响。结果显示其模拟结果与现实数据高度吻合，验证了该工具在揭示人类行为机制方面的有效性。这一平台为社会科学研究和政策制定提供了新方法，有望补充传统研究手段（如调查和实验）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T18:06:15Z
- **目录日期**: 2025-05-19
