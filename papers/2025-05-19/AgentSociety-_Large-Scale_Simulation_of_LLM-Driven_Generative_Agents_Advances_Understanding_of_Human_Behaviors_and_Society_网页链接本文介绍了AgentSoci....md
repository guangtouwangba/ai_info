...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟10,000+个AI代理的社会互动（500万次交互）。该系统通过四个社会问题案例（社会极化、信息传播、基本收入政策、自然灾害影响）验证其有效性，结果与现实数据高度吻合。研究表明，这种模拟能有效捕捉人类行为机制，为社会科学研究和政策制定提供新工具，补充传统研究方法（如调查和实验）。该系统展示了LLM在复杂社会现象建模方面的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T20:05:42Z
- **目录日期**: 2025-05-19
