...
本研究评估了大语言模型(LLM)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术的效果。实验发现：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型的基本反应能力，而大模型表现更稳健；3)复杂任务中高级提示技术对小模型帮助更大，但对高性能大模型提升有限。研究揭示当前LLM在规划、推理和空间协调等关键领域仍存在根本性局限，仅靠自我反思提示无法完全克服。动态基准测试凸显了通用推理能力的不足，表明需要超越静态基准来全面评估推理复杂性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T00:01:32Z
- **目录日期**: 2025-05-19
