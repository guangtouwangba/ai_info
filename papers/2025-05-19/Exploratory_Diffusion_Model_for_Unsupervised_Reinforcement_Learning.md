...
本文提出了一种名为探索性扩散模型（ExDM）的无监督强化学习方法，利用扩散模型强大的表达能力拟合探索数据，促进智能体在无奖励环境中的多样化探索。ExDM通过扩散模型准确估计回放缓冲区数据的分布，并引入基于分数的内在奖励机制，引导智能体探索访问较少的状态。该方法不仅能有效提升探索效率，还为下游任务提供了良好的初始化策略。理论分析和实验表明，ExDM在复杂结构环境中显著优于现有方法，解决了多步采样带来的训练不稳定和计算复杂度问题，实现了快速适应下游任务的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T05:02:26Z
- **目录日期**: 2025-05-19
