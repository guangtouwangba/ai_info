...
本文提出探索性扩散模型（ExDM），利用扩散模型的强大表达能力来拟合无监督强化学习中的探索数据，同时促进探索并为下游任务提供高效初始化。ExDM通过扩散模型准确估计回放缓冲区中收集数据的分布，并引入基于分数的内在奖励，鼓励智能体探索访问较少的状态。预训练后，ExDM能快速适应下游任务。研究还解决了扩散策略微调中的训练不稳定和多步采样计算复杂度等关键挑战。实验表明，ExDM在复杂结构环境中，在高效无监督探索和快速下游任务微调方面优于现有最优基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T19:02:19Z
- **目录日期**: 2025-05-19
