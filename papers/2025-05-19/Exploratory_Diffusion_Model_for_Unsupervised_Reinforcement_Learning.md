...
本文提出了一种无监督强化学习方法——探索性扩散模型（ExDM），利用扩散模型的强大表达能力来拟合探索数据，促进状态探索并为下游任务提供高效初始化。ExDM通过扩散模型准确估计回放缓冲区中数据的分布，并引入基于分数的内在奖励，鼓励智能体探索访问较少的状态。预训练后，ExDM能快速适应下游任务。研究提供了扩散策略微调的理论分析和实用算法，解决了多步采样导致的训练不稳定和计算复杂度问题。实验表明，ExDM在复杂结构环境中，在无监督探索和下游任务微调方面优于现有最优方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T07:02:29Z
- **目录日期**: 2025-05-19
