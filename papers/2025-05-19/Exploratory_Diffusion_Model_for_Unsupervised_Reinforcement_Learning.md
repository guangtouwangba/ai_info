...
本文提出了一种名为探索性扩散模型（ExDM）的无监督强化学习方法，利用扩散模型强大的表达能力来拟合探索数据，同时促进探索并为下游任务提供高效初始化。ExDM通过扩散模型准确估计回放缓冲区中收集数据的分布，并引入基于分数的内在奖励，鼓励智能体探索访问较少的状态。在预训练策略后，ExDM能快速适应下游任务。研究提供了扩散策略微调的理论分析和实用算法，解决了多步采样导致的训练不稳定和计算复杂度等关键挑战。实验表明，ExDM在复杂环境中显著优于现有方法，实现了高效探索和快速微调。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-19T17:02:32Z
- **目录日期**: 2025-05-19
