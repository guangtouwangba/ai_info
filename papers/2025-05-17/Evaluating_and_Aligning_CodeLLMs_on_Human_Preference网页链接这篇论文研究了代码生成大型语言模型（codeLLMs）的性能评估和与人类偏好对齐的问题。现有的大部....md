...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 构建CodeArena基准测试，包含397个真实编程任务样本(覆盖40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度；2) 创建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现：传统执行正确性基准与人类偏好存在显著差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类对齐方面仍有明显差距。研究强调了代码生成模型除功能性外，还需关注开发者体验和实用性的优化需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T02:34:27Z
- **目录日期**: 2025-05-17
