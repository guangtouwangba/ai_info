...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究收集了397个高质量编程样本(覆盖40类/44种语言)，并构建20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好存在偏差；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型除正确性外，还需重视开发者体验和实际需求匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T12:06:09Z
- **目录日期**: 2025-05-17
