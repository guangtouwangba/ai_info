...
这篇博客详细记录了如何在RISC-V架构上从零开始实现一个Hypervisor，并成功运行Linux系统的全过程。开发者基于RISC-V H-extension，通过9个关键步骤：包括进入Guest模式、处理系统调用、设备树支持、定时器中断、MMIO设备模拟等，最终在自研操作系统Starina上构建了轻量级虚拟机环境。该方案类似WSL2架构，但完全基于自主研发的内核。整个过程使用QEMU模拟器和GDB调试工具，开发体验流畅。最终实现了Linux内核在自研Hypervisor上的成功运行，展示了RISC-V虚拟化技术的实战应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T19:02:37Z
- **目录日期**: 2025-05-17
