...
这篇博客详细记录了基于RISC-V H-extension实现Hypervisor并运行Linux的全过程。开发者从零开始，通过9个关键步骤：进入Guest模式、处理系统调用、输出测试信息、加载Linux内核、补充设备树、支持时间读取、实现定时器中断、模拟MMIO设备，最终接入virtio-fs文件系统。整个过程在QEMU环境中完成，利用RISC-V的虚拟化扩展功能，成功让Linux运行在自研操作系统Starina上。开发中特别注意了权限控制和硬件模拟，并通过GDB调试确保稳定性。这为RISC-V平台的虚拟化开发提供了实用参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T23:02:20Z
- **目录日期**: 2025-05-17
