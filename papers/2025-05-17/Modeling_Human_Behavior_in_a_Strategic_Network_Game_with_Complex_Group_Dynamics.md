...
该研究探讨了如何通过小数据集建模人类在战略网络游戏（Junior High Game, JHG）中的行为，以更好地理解社会网络对财富、健康不平等等的影响。研究发现，最佳建模方法hCAB通过模拟人类行为的分布（而非均值）并假设其具有社区意识（而非简单行为匹配），能更准确地反映小规模群体（6-11人）的动态。实验还表明，人类参与者无法区分hCAB模型生成的个体行为与真实人类行为，验证了该模型在个体层面的拟真性。这一成果为研究社会网络中的行为模式提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T16:02:30Z
- **目录日期**: 2025-05-17
