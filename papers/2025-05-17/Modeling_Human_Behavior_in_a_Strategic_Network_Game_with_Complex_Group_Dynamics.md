...
人类网络对社会不平等、贫困和霸凌等关键问题有重大影响。为更好理解网络行为，研究对比了不同方法在小数据集上建模人类在“初中生游戏”（JHG）中的策略行为。最佳方法hCAB通过社区感知行为（而非简单行为匹配）建模行为分布（而非均值），在小群体（6-11人）中准确复现人类群体动态。用户研究显示，人类无法区分hCAB智能体与真实玩家，表明该模型能生成可信的个体行为。研究为理解网络行为提供了新工具，有助于优化社会结果。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T23:01:58Z
- **目录日期**: 2025-05-17
