...
该研究比较了多种从小数据集中学习人类在策略性网络游戏（初中生游戏，JHG）中行为模型的方法。最佳方法hCAB通过建模行为分布（而非均值）并假设人类具有社区意识行为（而非简单行为匹配），能准确反映小规模群体（6-11人）的动态特征。实验表明，人类参与者无法区分hCAB智能体与真实人类行为，验证了该模型在个体层面的拟真性。该成果为理解人类网络行为及其对社会不平等、贫困等议题的影响提供了新工具，凸显社区感知和分布建模在行为预测中的关键作用。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T14:02:35Z
- **目录日期**: 2025-05-17
