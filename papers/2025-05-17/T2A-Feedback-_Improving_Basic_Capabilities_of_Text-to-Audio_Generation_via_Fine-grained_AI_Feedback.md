...
本文提出了一种基于AI反馈学习的文本到音频(T2A)生成改进方法。研究团队开发了三个细粒度音频评分指标：事件发生评分、事件序列评分和音频质量评分，这些指标与人类偏好显著相关。基于此，构建了包含41k提示和249k音频的T2A-FeedBack数据集，并创建了针对长文本、多事件和叙事场景的T2A-EpicBench基准测试。实验表明，通过简单的偏好调整，该方法能显著提升现有音频生成模型在简单和复杂场景下的表现。这些评分指标既可作为反馈信号，也可作为评估标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T19:01:01Z
- **目录日期**: 2025-05-17
