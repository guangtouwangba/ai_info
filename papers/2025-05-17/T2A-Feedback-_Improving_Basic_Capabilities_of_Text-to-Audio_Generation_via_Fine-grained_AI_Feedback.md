...
当前文本到音频（T2A）生成模型在处理复杂多事件音频时仍难以满足人类对提示跟随和音质的要求。为此，研究者提出基于AI反馈学习的改进方法，开发了细粒度评分系统，包括事件存在性、事件顺序和音质评分。这些评分与人类偏好高度相关，优于传统指标。基于此，构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并推出专注于长文本、多事件和叙事场景的T2A-EpicBench基准测试。实验表明，通过偏好调优，模型在简单和复杂场景下的表现均显著提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T20:01:05Z
- **目录日期**: 2025-05-17
