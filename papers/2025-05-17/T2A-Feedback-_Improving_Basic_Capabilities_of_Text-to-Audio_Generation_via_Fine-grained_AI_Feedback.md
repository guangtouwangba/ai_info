...
该研究提出了一种基于AI反馈学习的文本到音频（T2A）生成改进方法。通过开发三个细粒度评分管道（事件发生评分、事件序列评分、声学和谐波质量评分），显著提升了模型对复杂多事件音频的生成质量。这些自动评分与人类偏好高度相关。研究构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并提出了针对长文本、多事件场景的T2A-EpicBench基准测试。实验表明，简单的偏好调整即可显著提升现有音频生成模型在简单和复杂场景下的表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T09:00:59Z
- **目录日期**: 2025-05-17
