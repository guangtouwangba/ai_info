...
当前文本转音频（T2A）模型在处理复杂多事件音频时仍难以满足人类对提示跟随和音质的需求。为此，研究提出通过AI反馈学习增强模型能力，开发了细粒度音频评分系统：事件存在性评分、事件顺序评分和音质评分。这些评分与人类偏好高度相关，优于传统指标。基于此构建了包含4.1万提示和24.9万音频的T2A-FeedBack数据集，并推出针对长文本、多事件场景的T2A-EpicBench基准测试。实验表明，通过简单偏好调优，模型在简单和复杂场景下的生成质量均显著提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T10:01:36Z
- **目录日期**: 2025-05-17
