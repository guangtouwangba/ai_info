...
本文提出SGD-SaI方法，质疑深度神经网络训练中自适应梯度方法（如Adam）的必要性。该方法改进带动量SGD，通过初始化时依据梯度信噪比(g-SNR)调整各参数组学习率，避免训练不平衡且内存用量减半。实验显示，在Transformer任务、ImageNet分类、GPT-2预训练及LoRA微调中，SGD-SaI性能匹配或超越AdamW，尤其在大模型训练中显著节省内存（如为Llama2-7B节省25.15GB）。其优势包括超参数鲁棒性、通用性及解决传统SGD训练Transformer的难题，成为高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T05:04:31Z
- **目录日期**: 2025-05-17
