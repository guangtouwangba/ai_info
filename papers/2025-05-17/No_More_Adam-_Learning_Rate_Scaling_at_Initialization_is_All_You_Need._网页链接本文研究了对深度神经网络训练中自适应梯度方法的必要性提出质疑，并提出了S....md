...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过根据梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，避免了训练不平衡问题。相比AdamW，它减少了50%的内存占用，并在Transformer任务、ImageNet分类和LLM预训练中表现更优。例如，在GPT-2和Llama2-7B训练中分别节省了5.93GB和25.15GB内存。该方法还展现出对超参数的鲁棒性，并在LoRA微调等任务中优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T15:04:38Z
- **目录日期**: 2025-05-17
