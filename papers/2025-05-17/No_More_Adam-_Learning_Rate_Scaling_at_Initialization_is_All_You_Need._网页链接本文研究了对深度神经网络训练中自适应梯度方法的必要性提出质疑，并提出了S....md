...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量就能避免训练不平衡。相比AdamW，它节省50%内存，在Transformer、ImageNet分类和LLM预训练等任务中表现相当或更好，并显著提升内存效率（如为Llama2-7B节省25GB内存）。该方法还展现出超参数鲁棒性，在LoRA微调等场景也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T03:21:19Z
- **目录日期**: 2025-05-17
