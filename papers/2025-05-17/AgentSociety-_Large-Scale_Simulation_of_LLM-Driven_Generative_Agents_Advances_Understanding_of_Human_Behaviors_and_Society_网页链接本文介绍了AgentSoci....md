...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可以模拟超过1万个代理的社会互动（500万次互动）。研究者用它研究了四个关键社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。模拟结果与现实世界数据吻合，验证了该系统捕捉人类行为机制的能力。这表明AgentSociety可作为社会科学研究和政策制定的有力工具，补充传统研究方法（如调查和访谈）。该技术为理解复杂社会现象提供了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T08:05:17Z
- **目录日期**: 2025-05-17
