...
阿里通义实验室提出"ZeroSearch"新方法，让大语言模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的伪检索结果，采用课程学习逐步增加噪声干扰，并利用强化学习形成训练闭环。实验显示，仅用3B/7B参数的小模型，在单跳/多跳问答任务上就能媲美传统基于真实搜索引擎的方法，同时实现零API成本。这项技术解决了传统方法存在的文档质量差、成本高和扩展难三大痛点，为高效训练检索增强型大模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T15:02:50Z
- **目录日期**: 2025-05-17
