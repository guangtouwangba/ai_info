...
阿里通义实验室提出"ZeroSearch"方法，通过大模型自建虚拟搜索引擎来优化训练：1）轻量微调生成含噪声的模拟检索结果；2）采用课程学习逐步增加噪声干扰；3）结合强化学习形成闭环训练。该方法在3B/7B小模型上即超越传统依赖真实搜索引擎的方案，显著降低API成本，在单跳/多跳问答任务中表现优异，效果媲美谷歌搜索。研究解决了传统方法存在的文档质量不稳定、成本高和扩展性差等问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T16:03:04Z
- **目录日期**: 2025-05-17
