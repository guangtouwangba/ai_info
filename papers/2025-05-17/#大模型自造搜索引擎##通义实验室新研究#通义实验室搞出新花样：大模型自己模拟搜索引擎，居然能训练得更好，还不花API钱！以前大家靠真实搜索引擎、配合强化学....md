...
阿里通义实验室提出"ZeroSearch"新方法，让大语言模型通过自生成检索结果进行训练，无需依赖真实搜索引擎API。该方法采用三步策略：1)轻量微调生成含噪声的模拟文档；2)课程学习逐步增加噪声强度；3)强化学习建立检索推理闭环。实验显示，仅用3B/7B参数模型就能在多跳问答任务上媲美谷歌搜索效果，同时解决了传统方法存在的文档质量差、API成本高和扩展性差等问题。这种自监督训练范式显著提升了模型检索能力，为搜索引擎优化提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T23:02:30Z
- **目录日期**: 2025-05-17
