...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的伪检索结果，采用课程学习逐步增加噪声干扰，并利用强化学习形成闭环训练。相比依赖真实搜索引擎的传统方法，该方案解决了数据质量差、API成本高和扩展性差等问题。实验表明，仅用3B/7B参数的小模型，在单跳/多跳问答任务上就能媲美甚至超越谷歌搜索效果，同时实现零API成本。这项研究为大模型的高效训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T09:02:54Z
- **目录日期**: 2025-05-17
