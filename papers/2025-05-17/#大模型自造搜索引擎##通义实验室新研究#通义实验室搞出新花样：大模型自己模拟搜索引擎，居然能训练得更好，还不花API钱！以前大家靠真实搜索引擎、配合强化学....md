...
通义实验室提出"ZeroSearch"新方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并利用强化学习形成闭环。实验显示，仅用3B/7B参数的小模型就能在单跳/多跳问答任务上媲美谷歌搜索效果，同时避免了真实搜索引擎API的高成本问题。该方案解决了传统方法中数据质量差、扩展难等痛点，为高效训练检索增强型大模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T12:02:53Z
- **目录日期**: 2025-05-17
