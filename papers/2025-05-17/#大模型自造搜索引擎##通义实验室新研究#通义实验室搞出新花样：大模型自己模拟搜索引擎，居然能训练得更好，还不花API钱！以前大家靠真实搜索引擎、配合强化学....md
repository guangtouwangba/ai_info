...
阿里通义实验室提出"ZeroSearch"方法，通过大模型自造搜索引擎来优化训练：1）轻量微调生成含噪声的模拟检索结果；2）采用课程学习逐步增加噪声难度；3）结合强化学习形成闭环训练。该方法无需真实搜索引擎API，在3B/7B小模型上即超越传统方法，单跳/多跳问答任务表现优异，成本显著降低。实验显示其效果可媲美谷歌搜索，解决了文档质量差、成本高和扩展难等问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T22:02:36Z
- **目录日期**: 2025-05-17
