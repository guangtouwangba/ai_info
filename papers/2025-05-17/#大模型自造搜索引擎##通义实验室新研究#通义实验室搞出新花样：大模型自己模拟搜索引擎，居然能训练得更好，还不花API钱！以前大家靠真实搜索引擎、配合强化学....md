...
阿里通义实验室提出"ZeroSearch"创新方法，通过大模型自模拟搜索引擎来优化训练：1）轻量微调生成含噪声的模拟文档；2）采用课程学习逐步增加噪声强度；3）结合强化学习形成闭环训练。该方法在3B/7B小模型上即实现优于传统搜索引擎的效果，显著降低API成本，在单跳/多跳问答任务中表现优异，甚至超越谷歌搜索性能。研究解决了真实搜索接口存在的质量不稳定、成本高和扩展性差等问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T18:02:55Z
- **目录日期**: 2025-05-17
