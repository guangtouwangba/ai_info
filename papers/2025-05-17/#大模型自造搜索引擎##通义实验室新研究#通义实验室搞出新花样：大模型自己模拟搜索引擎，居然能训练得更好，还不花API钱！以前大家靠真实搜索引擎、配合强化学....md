...
阿里通义实验室提出"ZeroSearch"方法，通过大模型自造搜索引擎来提升训练效果并降低成本。该方法采用三步策略：1)轻量微调模型生成含噪声的模拟文档；2)课程式抗噪训练，逐步增加噪声难度；3)强化学习闭环优化检索-推理流程。实验表明，仅用3B/7B小模型就能在单跳/多跳问答任务上媲美谷歌搜索，同时避免了真实API的高成本和扩展性问题。该研究为大模型检索训练提供了经济高效的新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T07:02:40Z
- **目录日期**: 2025-05-17
