...
阿里通义实验室提出"ZeroSearch"方法，通过大模型自建虚拟搜索引擎来优化训练。该方法采用三阶段策略：1)轻量微调生成含噪声的伪检索结果；2)课程学习逐步增加噪声比例；3)强化学习形成检索-推理闭环。实验显示，仅用3B/7B参数模型就能在单跳/多跳问答任务上媲美谷歌搜索，同时避免了真实API的高成本问题。这种自监督训练方式解决了传统方法面临的文档质量差、扩展性弱等痛点，为高效构建检索增强型语言模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T10:03:33Z
- **目录日期**: 2025-05-17
