...
阿里通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练，无需依赖真实搜索API。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升模型抗干扰能力，并利用强化学习算法优化检索-推理闭环。实验显示，仅用3B/7B参数的小模型在单跳和多跳问答任务上就能媲美谷歌搜索效果，同时解决了传统方法存在的文档质量差、成本高和扩展难等问题，实现了零API成本的训练优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T20:02:58Z
- **目录日期**: 2025-05-17
