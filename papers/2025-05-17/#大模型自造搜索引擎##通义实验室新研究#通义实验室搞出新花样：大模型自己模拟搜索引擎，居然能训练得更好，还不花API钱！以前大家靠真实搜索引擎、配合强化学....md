...
阿里通义实验室提出"ZeroSearch"创新方法，通过大模型自建虚拟搜索引擎来优化训练。该方法采用三阶段策略：1)轻量微调生成含噪声的模拟文档；2)课程学习逐步增加噪声训练抗干扰能力；3)强化学习闭环实现零API成本。实验显示，仅用3B/7B参数的小模型，在单跳和多跳问答任务上就能媲美甚至超越传统依赖真实搜索引擎的方法。该方案有效解决了传统方法面临的文档质量差、成本高和扩展难三大痛点，为高效训练检索增强型语言模型提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T06:03:02Z
- **目录日期**: 2025-05-17
