...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练（从高质量文档逐步过渡到噪声数据），并结合强化学习优化检索-推理流程。实验显示，仅用3B/7B参数的小模型，在单跳和多跳问答任务上就能媲美谷歌搜索效果，同时避免了传统方法依赖真实搜索引擎导致的高成本、文档质量差和扩展性差等问题。这一创新实现了零API成本的高效训练。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T14:03:02Z
- **目录日期**: 2025-05-17
