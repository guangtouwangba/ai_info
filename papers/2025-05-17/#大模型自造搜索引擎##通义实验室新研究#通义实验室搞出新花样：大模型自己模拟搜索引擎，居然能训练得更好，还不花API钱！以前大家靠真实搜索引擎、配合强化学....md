...
通义实验室提出"ZeroSearch"方法，让大模型自主模拟搜索引擎进行训练，无需依赖真实搜索API。该方法通过轻量微调生成含噪声的文档数据，采用课程式抗噪训练逐步提升难度，并结合强化学习形成闭环。实验表明，仅用3B/7B参数的小模型在单跳/多跳问答任务上就能媲美谷歌搜索效果，同时解决了传统方法存在的文档质量差、成本高和扩展难等问题。这种自造搜索引擎的范式显著降低了训练成本，同时提升了模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T11:02:58Z
- **目录日期**: 2025-05-17
