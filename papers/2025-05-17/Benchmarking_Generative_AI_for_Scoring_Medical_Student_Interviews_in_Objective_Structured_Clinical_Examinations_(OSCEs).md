...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生临床沟通技能（OSCE考试）的可行性。测试了GPT-4o、Claude 3.5等4种先进模型在零样本、思维链等不同提示策略下对28项MIRS量表的评分表现。结果显示，LLMs的精确准确率较低（0.27-0.44），但相邻误差准确率（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有可行性，为临床沟通技能的自动化评估奠定了基础，但需针对特定评估项目优化提示技术。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T23:02:10Z
- **目录日期**: 2025-05-17
