...
本研究探讨了使用大语言模型（LLMs）自动化评估医学生临床沟通技能（OSCE考试）的可行性。测试了GPT-4o、Claude 3.5等4种先进模型在28项MIRS评分标准下的表现，采用零样本、思维链等多种提示技术。结果显示：模型在严格匹配准确率（0.27-0.44）上表现一般，但在容错准确率（0.67-0.87）和阈值准确率（0.75-0.88）上表现良好，且评分者内部一致性高（GPT-4o α=0.98）。研究表明LLMs辅助评估具有潜力，但需针对特定评分项优化提示技术，为未来临床沟通技能自动化评估奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T19:02:25Z
- **目录日期**: 2025-05-17
