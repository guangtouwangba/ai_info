...
这项研究探索了使用大型语言模型（LLMs）自动评估医学临床技能考试（OSCE）的可行性，比较了GPT-4o、Claude 3.5等四种先进模型在Master Interview Rating Scale（MIRS）28项指标下的表现。实验采用零样本、思维链（CoT）等多种提示技术，基于174份专家共识评分进行基准测试。结果显示，LLMs的精确准确率较低（0.27-0.44），但相邻误差准确率（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好，且具有高评分者内信度（GPT-4o α=0.98）。研究表明AI辅助OSCE评估具备可行性，为临床沟通技能自动化评估奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T06:02:41Z
- **目录日期**: 2025-05-17
