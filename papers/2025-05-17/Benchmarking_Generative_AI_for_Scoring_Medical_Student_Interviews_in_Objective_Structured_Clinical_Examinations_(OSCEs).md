...
本研究探索了大型语言模型（LLMs）在自动化OSCE（客观结构化临床考试）评估中的应用潜力，使用Master Interview Rating Scale（MIRS）对四种先进模型（GPT-4o、Claude 3.5、Llama 3.1和Gemini 1.5 Pro）进行了测试。结果显示，LLMs在精确匹配准确度（0.27-0.44）上表现较低，但在相邻匹配（0.67-0.87）和阈值准确度（0.75-0.88）上表现中等至良好。零温度参数确保了高评分者内部信度（GPT-4o α=0.98）。链式思考、少量样本和多步提示技术对特定评估项有效。研究表明AI辅助OSCE评估可行，为临床沟通技能的自动化评估提供了基准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T08:02:34Z
- **目录日期**: 2025-05-17
