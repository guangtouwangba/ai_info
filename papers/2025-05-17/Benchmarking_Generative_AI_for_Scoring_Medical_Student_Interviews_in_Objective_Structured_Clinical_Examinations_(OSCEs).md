...
本研究探索了利用大型语言模型（LLM）自动化评估医学生临床技能考试（OSCE）的可行性，测试了GPT-4o等四种先进模型在Master面试评分量表（MIRS）28个项目中的表现。结果显示，LLM在精确匹配准确率（0.27-0.44）上表现较低，但在近似匹配（0.67-0.87）和阈值准确率（0.75-0.88）上表现中高等。零温度参数确保了高评分者内部信度（GPT-4o α=0.98）。特定提示技术（如思维链、少量示例）可提升特定项目的评估效果。研究表明AI辅助OSCE评估具有可行性，为临床沟通技能自动化评估奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T22:02:15Z
- **目录日期**: 2025-05-17
