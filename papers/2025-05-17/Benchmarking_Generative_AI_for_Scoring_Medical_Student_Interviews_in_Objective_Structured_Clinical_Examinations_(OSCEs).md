...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生OSCE考试（客观结构化临床考试）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在MIRS评分标准下的表现。通过零样本、思维链等提示技术，模型在174份专家评分数据上的表现显示：严格准确率较低（0.27-0.44），但容错准确率较高（0.67-0.88）。零温度参数确保了高评分者内信度（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有潜力，为临床沟通技能的自动化评估奠定了基础，但需针对特定评分项优化提示技术。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T01:29:54Z
- **目录日期**: 2025-05-17
