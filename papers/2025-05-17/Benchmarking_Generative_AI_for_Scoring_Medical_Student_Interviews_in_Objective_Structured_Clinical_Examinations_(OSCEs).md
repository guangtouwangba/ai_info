...
该研究探索了使用大型语言模型（LLMs）自动化评估医学临床技能考试（OSCE）的可行性，比较了GPT-4o、Claude 3.5等四种先进模型在零样本、思维链（CoT）等提示技术下的表现。结果显示，LLMs在精确匹配准确率（0.27-0.44）上较低，但在近似匹配（0.67-0.87）和阈值准确率（0.75-0.88）上表现中等至良好，且具有高评分者内信度（α=0.98）。研究表明，针对特定评估项目定制提示技术可提升效果，为临床沟通技能的自动化评估奠定了基础，并提供了多模型、多提示技术的基准性能数据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T16:02:42Z
- **目录日期**: 2025-05-17
