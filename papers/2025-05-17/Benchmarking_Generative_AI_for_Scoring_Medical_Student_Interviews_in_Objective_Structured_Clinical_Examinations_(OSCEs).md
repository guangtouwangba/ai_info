...
本研究探索了使用大型语言模型（LLMs）自动化评估医学临床技能考试（OSCE）的可行性，测试了GPT-4o、Claude 3.5等四种先进模型在Master评分量表（MIRS）28个项目中的表现。结果显示，LLMs的精确匹配准确率较低（0.27-0.44），但允许±1误差的准确率中等偏高（0.67-0.87）。零温度参数确保了评分者内部高度一致性（GPT-4o α=0.98）。思维链、小样本等提示技术对特定评估项目有效。研究表明AI辅助OSCE评估具有潜力，为临床沟通技能自动化评估奠定了基础，但精确度仍需提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T12:02:41Z
- **目录日期**: 2025-05-17
