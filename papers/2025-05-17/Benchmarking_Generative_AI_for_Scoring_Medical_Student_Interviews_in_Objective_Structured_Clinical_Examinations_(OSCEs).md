...
本研究探索了使用大型语言模型（LLMs）自动化评估医学生OSCE（客观结构化临床考试）中沟通技能的可行性。测试了GPT-4o、Claude 3.5等四种先进模型在零样本、思维链等不同提示策略下对MIRS量表的28项评分表现。结果显示，模型在严格准确率（0.27-0.44）上表现一般，但在容错准确率（0.67-0.87）和阈值准确率（0.75-0.88）上表现良好，且具有高评分者内信度（α=0.98）。研究表明，AI辅助OSCE评估具有潜力，为临床沟通技能的自动化评估提供了基准性能数据，但需针对特定评估项优化提示策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T07:02:30Z
- **目录日期**: 2025-05-17
