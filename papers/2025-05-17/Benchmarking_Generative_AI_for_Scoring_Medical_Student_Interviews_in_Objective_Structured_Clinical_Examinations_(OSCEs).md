...
本研究探索了使用大型语言模型（LLMs）自动评估医学OSCE考试中沟通技能的可行性。测试了GPT-4o、Claude 3.5等4种先进模型在MIRS量表的28个项目上的表现，采用零样本、思维链等多种提示技术。结果显示，LLMs在精确匹配准确率较低（0.27-0.44），但在近似匹配（0.67-0.87）和阈值准确率（0.75-0.88）表现中等至良好。模型间评分一致性高（GPT-4o α=0.98）。研究表明，AI辅助OSCE评估具有潜力，为未来临床沟通技能自动化评估研究奠定了基础，但需针对特定评估项目优化提示技术。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T21:02:20Z
- **目录日期**: 2025-05-17
