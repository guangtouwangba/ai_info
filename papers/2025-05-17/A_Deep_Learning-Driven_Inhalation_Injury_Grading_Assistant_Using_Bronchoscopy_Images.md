...
本研究开发了一种基于深度学习的吸入性损伤分级辅助工具，通过支气管镜图像克服传统主观评分方法的局限性。研究采用数据增强技术（包括图像变换、CUT和CycleGAN）解决医学影像数据不足问题，对比评估了GoogLeNet和ViT模型的性能。结果显示，GoogLeNet结合CUT增强效果最佳，分类准确率达97.8%。PCA和Grad-CAM分析表明CUT显著提升了特征空间的可分性和模型决策可靠性（热图平均强度119.6 vs 原始数据98.8）。该工具创新性地以机械通气时长作为分级标准，为临床诊断提供了客观支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T19:01:57Z
- **目录日期**: 2025-05-17
