...
该研究评估了大型语言模型(LLM)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术。研究发现：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型在基础任务的表现，而大模型更稳健；3)复杂任务中高级提示主要帮助小模型，对大模型提升有限。研究还发现推理方法效果不稳定，可能导致性能显著波动。与人类相比，LLM在规划、推理和空间协调等关键领域仍存在根本性局限，表明仅靠自我反思提示无法完全克服当前LLM的缺陷。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T13:06:33Z
- **目录日期**: 2025-05-17
