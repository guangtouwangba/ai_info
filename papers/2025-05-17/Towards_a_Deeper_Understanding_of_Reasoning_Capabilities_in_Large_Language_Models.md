...
该研究评估了大型语言模型(LLMs)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术。结果显示：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型的基础反应能力，而大模型表现更稳健；3)高级提示技术主要帮助小模型处理复杂任务，对大模型提升有限。研究发现，当前LLMs在规划、推理和空间协调等关键领域仍存在根本性缺陷，推理能力具有高度不稳定性。这表明仅靠自我反思提示无法完全克服现有局限，需要超越静态基准来评估真正的推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T23:00:56Z
- **目录日期**: 2025-05-17
