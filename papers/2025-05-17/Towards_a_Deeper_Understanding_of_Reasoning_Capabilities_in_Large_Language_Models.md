...
该研究评估了大型语言模型(LLMs)在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术的效果。实验发现：1)大模型普遍优于小模型，但策略性提示能缩小差距；2)过长提示会损害小模型在基础任务的表现，大模型更稳健；3)高级提示技术对小模型在复杂任务中帮助更大。研究显示当前LLMs在规划、推理和空间协调等关键领域仍存在根本性缺陷，推理能力具有高度不稳定性。虽然思维链等方法能提升数学推理，但动态基准测试揭示了通用推理能力的重大不足，表明需要超越静态基准来评估真实推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T11:01:22Z
- **目录日期**: 2025-05-17
