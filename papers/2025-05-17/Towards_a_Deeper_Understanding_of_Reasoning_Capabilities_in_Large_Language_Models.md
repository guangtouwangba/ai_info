...
该研究评估了大型语言模型(LLM)在动态环境中的自适应能力，测试了自我反思、启发式变异和规划等提示技术。结果显示：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型的基础反应能力，而大模型表现更稳健；3)高级提示技术对小模型在复杂任务中帮助更大，但对已表现优异的大模型提升有限。研究发现LLM在规划、推理和空间协调等关键领域仍存在根本性局限，当前推理方法虽能提升特定任务表现，但会引入不稳定性，表明静态基准无法全面反映推理复杂性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T18:01:19Z
- **目录日期**: 2025-05-17
