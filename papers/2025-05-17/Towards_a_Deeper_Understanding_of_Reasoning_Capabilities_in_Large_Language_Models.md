...
该研究评估了大型语言模型(LLMs)在动态环境中的自适应能力，发现：1)大模型普遍优于小模型，但策略性提示可缩小差距；2)过长提示会损害小模型在基础任务的表现，而大模型更稳健；3)高级提示技术主要帮助小模型处理复杂任务，对大模型提升有限。研究发现，尽管推理方法能提升特定场景(如数学问题)的表现，但在规划、推理和空间协调等关键领域仍存在根本性缺陷，且推理结果波动大。结果表明当前LLMs缺乏真正的涌现推理能力，需超越静态基准来全面评估推理复杂性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T09:01:21Z
- **目录日期**: 2025-05-17
