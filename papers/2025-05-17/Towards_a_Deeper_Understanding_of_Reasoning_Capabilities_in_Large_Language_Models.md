...
该研究评估了大语言模型（LLM）在动态环境中的自适应能力，测试了自我反思、启发式突变和规划等提示技术的作用。实验发现，大模型普遍优于小模型，但策略性提示能缩小差距；过长的提示会损害小模型在基础任务中的表现，而大模型更稳健；复杂任务中高级提示技术对小模型帮助更大，但可能导致性能不稳定。与人类相比，LLM在规划、推理和空间协调等关键领域仍存在明显局限，表明当前模型的推理能力存在根本缺陷，需超越静态基准以全面评估其复杂性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-17T07:01:12Z
- **目录日期**: 2025-05-17
