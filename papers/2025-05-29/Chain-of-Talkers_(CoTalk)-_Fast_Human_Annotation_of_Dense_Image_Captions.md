...
本文提出了一种名为Chain-of-Talkers (CoTalk)的AI辅助标注方法，旨在固定预算下最大化标注样本数量并提升标注全面性。该方法采用顺序标注策略，后续标注者只需补充前序遗漏的视觉信息，减少冗余工作量；同时通过多模态界面（阅读文本+语音输出）提升效率。实验表明，CoTalk在标注速度（0.42 vs. 0.30单位/秒）和检索性能（41.13% vs. 40.52%）上均优于传统并行标注方法。评估通过语义单元完整性分析和视觉-语言对齐效果验证其优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T22:01:56Z
- **目录日期**: 2025-05-29
