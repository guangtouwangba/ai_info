...
本文提出Chain-of-Talkers (CoTalk)方法，通过AI辅助优化图像标注流程，在固定预算下提升标注数量和全面性。该方法采用顺序标注减少冗余工作，后续标注者只需补充前序遗漏的视觉信息；同时结合多模态界面，通过语音输入提高效率。实验显示，CoTalk的标注速度(0.42单位/秒)优于并行方法(0.30单位/秒)，且在视觉-语言对齐任务中检索性能更优(41.13% vs 40.52%)。内在评估通过解析标注文本为对象-属性树分析语义完整性，外在评估验证了标注数据对模型训练的实际提升效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T09:02:12Z
- **目录日期**: 2025-05-29
