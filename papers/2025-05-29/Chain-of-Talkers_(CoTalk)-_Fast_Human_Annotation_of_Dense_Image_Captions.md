...
本文提出Chain-of-Talkers (CoTalk)方法，通过AI辅助优化图像标注流程，在固定预算下提升标注数量和质量。该方法采用顺序标注减少冗余工作，后续标注者只需补充前序遗漏信息；同时结合多模态界面（阅读输入+语音输出）提高效率。实验表明，相比并行标注，CoTalk标注速度提升40%（0.42单位/秒），检索性能提高0.61个百分点（41.13%）。通过语义单元完整性评估（对象-属性树分析）和实际应用测试（视觉-语言对齐），验证了该框架在提升标注全面性和实用性方面的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T13:10:35Z
- **目录日期**: 2025-05-29
