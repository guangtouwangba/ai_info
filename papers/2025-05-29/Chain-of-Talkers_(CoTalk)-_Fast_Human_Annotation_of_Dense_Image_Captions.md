...
本文提出Chain-of-Talkers (CoTalk)，一种AI辅助的序列标注方法，旨在固定预算下最大化标注样本数量并提升全面性。相比传统并行标注，CoTalk通过让后续标注者仅补充前序遗漏的"残差"信息来减少冗余工作。同时采用多模态界面（阅读输入+语音输出）优化效率。实验表明：CoTalk在语义单元完整性（通过对象-属性树分析）和实际应用（提升视觉-语言对齐）方面均优于并行方法，标注速度提高40%（0.42单位/秒），检索性能提升至41.13%。八人参与实验验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T17:01:58Z
- **目录日期**: 2025-05-29
