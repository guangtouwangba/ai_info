...
本文提出Chain-of-Talkers (CoTalk)方法，通过AI辅助优化图像标注流程。该方法采用顺序标注策略减少冗余工作，后续标注者只需补充前序遗漏的视觉信息。结合多模态界面（阅读输入+语音输出）提升效率。实验表明，相比并行标注，CoTalk将标注速度提高至0.42单位/秒（原0.30），检索性能提升至41.13%（原40.52%）。内在评估通过解析对象-属性树分析语义完整性，外在评估验证其在视觉-语言对齐任务中的实用性。8人参与实验证实该方法在固定预算下能最大化标注数量与质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T12:02:16Z
- **目录日期**: 2025-05-29
