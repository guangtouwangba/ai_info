...
本文提出Chain-of-Talkers（CoTalk），一种AI辅助的序列标注方法，旨在固定预算下最大化标注样本数量并提升标注完整性。相比传统并行标注，CoTalk通过让后续标注者仅补充前序遗漏的"残差"信息来减少冗余工作；同时采用多模态界面（阅读输入+语音输出）提升效率。实验表明，CoTalk的标注速度（0.42单位/秒）和检索性能（41.13%）均优于并行方法（0.30单位/秒，40.52%）。该方法通过解析标注为对象-属性树进行内在评估，并通过视觉-语言对齐任务验证实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T04:04:18Z
- **目录日期**: 2025-05-29
