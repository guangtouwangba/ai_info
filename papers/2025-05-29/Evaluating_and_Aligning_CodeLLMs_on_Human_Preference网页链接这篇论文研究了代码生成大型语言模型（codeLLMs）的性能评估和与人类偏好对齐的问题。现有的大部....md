...
该论文针对代码生成大模型(codeLLMs)提出创新评估框架CodeArena，包含397个真实编程任务样本，覆盖40类编程场景和44种语言，首次系统评估模型输出与人类偏好的匹配度。研究同时构建20B标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现：1)传统执行正确性指标与人类偏好存在显著差异；2)开源顶尖模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面差距明显。研究强调了代码生成模型除功能性外，还需重视开发者体验优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T08:17:09Z
- **目录日期**: 2025-05-29
