...
这篇论文研究了代码生成大模型(codeLLMs)如何与人类偏好对齐的问题。作者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发场景。同时构建了20B标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现，传统基于执行的评估与CodeArena结果存在差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面仍有明显差距。研究强调了代码生成模型不仅需要正确性，还需考虑开发者实际需求的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T04:08:13Z
- **目录日期**: 2025-05-29
