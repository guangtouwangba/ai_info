...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，重点关注模型输出与人类偏好的对齐问题。研究构建了包含397个高质量样本的评估集(覆盖40类任务和44种语言)，并开发了20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统基于执行的评估与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了对齐人类偏好在代码生成领域的重要性，为改进模型提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T15:06:16Z
- **目录日期**: 2025-05-29
