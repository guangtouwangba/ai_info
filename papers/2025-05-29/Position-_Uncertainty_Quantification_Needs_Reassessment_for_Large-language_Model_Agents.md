...
这篇立场论文指出，传统将不确定性分为偶然性和认知性的二分法在LLM智能体与用户的交互场景中存在局限。作者发现现有定义在交互环境中相互矛盾且失去意义，因此提出三个新研究方向：1) 未明确性不确定性（用户初始信息不完整时产生）；2) 交互学习（通过追问降低情境不确定性）；3) 输出不确定性（利用语言多样性表达不确定性）。这些方法旨在使LLM智能体的交互更透明、可信且直观，超越简单数值化表达。研究强调需要开发更适合开放式人机交互场景的不确定性量化框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T10:01:19Z
- **目录日期**: 2025-05-29
