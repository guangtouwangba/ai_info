...
这篇立场论文指出，传统对偶不确定性（偶然性与认知性）的划分在LLM智能体与用户的开放交互场景中存在局限。研究发现现有定义在交互环境中相互矛盾且失去意义，因此提出三个新研究方向：1) 未明确性不确定性（用户初始信息不完整）；2) 交互学习（通过追问降低情境不确定性）；3) 输出不确定性（利用丰富语言表达超越数字的不确定性）。这些新方法有望使LLM交互更透明、可信且直观。论文强调在开放交互场景中需要超越传统二分法的新型不确定性量化框架。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T21:01:10Z
- **目录日期**: 2025-05-29
