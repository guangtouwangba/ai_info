...
该立场论文指出，传统的不确定性二分法（认知不确定性和偶然不确定性）在LLM与用户交互的开放场景中存在局限。研究发现现有定义在交互环境中相互矛盾且失效，因此提出三个新研究方向：1) **未明确性不确定性**（用户未一次性提供完整信息）；2) **交互学习**（通过追问减少上下文不确定性）；3) **输出不确定性**（利用语言多样性表达更丰富的不确定性形式）。这些方向旨在提升LLM交互的透明度、可信度与直观性，突破传统数值化衡量的限制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T17:01:10Z
- **目录日期**: 2025-05-29
