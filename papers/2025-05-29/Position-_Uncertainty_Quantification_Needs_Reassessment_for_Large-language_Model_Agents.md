...
这篇立场论文指出，传统将不确定性分为偶然性和认知性的二分法在LLM智能体与用户交互的开放场景中存在局限。研究发现，现有定义在交互环境中相互矛盾且失去意义，因此提出三个新研究方向：1) 任务描述不足导致的不确定性；2) 通过交互式学习追问以降低情境不确定性；3) 利用语言多样性表达输出不确定性（而非简单数值）。这些新方法有望使LLM交互更透明、可信且直观。论文强调在开放交互场景中需要超越传统不确定性量化框架，开发更适应人机对话特性的不确定性表达方式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T11:01:10Z
- **目录日期**: 2025-05-29
