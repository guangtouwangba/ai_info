...
该研究提出了一种新型深度双泊松网络（DDPN），用于解决离散计数回归中的不确定性估计问题。DDPN通过输出双泊松分布参数，能够灵活表示计数数据的任意高低预测不确定性，并在集成时改进认知不确定性估计。研究证明DDPN具有类似异方差高斯模型的可学习损失衰减特性，并引入简单损失修改来控制该行为。实验表明，DDPN在准确性、校准性和分布外检测方面优于现有基线，为深度计数回归设立了新标准。这一成果填补了连续回归与计数回归在不确定性表示方法上的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T15:02:48Z
- **目录日期**: 2025-05-29
