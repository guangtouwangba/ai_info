...
基于视觉语言模型（VLM）的Web代理在模拟人类与网站交互方面取得进展，但其在开放网络环境中的部署存在安全漏洞。现有攻击方法通常依赖不现实的假设（如直接修改HTML或获取模型参数）。本文提出AdInject攻击方法，通过互联网广告投放注入恶意内容，无需知晓用户意图或代理参数，仅需静态恶意内容即可实施黑盒攻击。该方法结合误导性广告设计和基于VLM的内容优化，攻击成功率超60%，部分场景接近100%，揭示了广告投放作为真实攻击媒介的风险，凸显了开发防御机制的紧迫性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T00:01:25Z
- **目录日期**: 2025-05-29
