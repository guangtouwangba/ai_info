...
这篇论文提出了SGD-SaI方法，挑战了深度学习中对自适应梯度方法（如Adam）的依赖。该方法通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，优化了带动量的SGD（SGDM）。SGD-SaI无需二阶动量计算，内存占用仅为AdamW的一半，且在Transformer、ImageNet分类和GPT-2预训练等任务中表现相当或更优。实验显示，它对超参数变化鲁棒，并在LoRA微调中超越现有优化器。内存效率显著，例如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T04:06:53Z
- **目录日期**: 2025-05-29
