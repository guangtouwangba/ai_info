...
最新研究《LIFEBENCH》通过基准测试发现，大语言模型（LLMs）在遵循长度指令生成文本时表现不佳。当要求生成特定长度的内容（如1万字长文）时，多数模型会出现内容不足、重复或直接拒绝生成的问题。研究表明，许多模型宣传的最大输出长度存在过度承诺，实际能力与宣传不符。该研究提出了LIFEBENCH测试集，系统评估了26个模型的长文本生成能力，揭示了当前LLMs在长度指令遵循方面的显著缺陷。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T15:04:11Z
- **目录日期**: 2025-05-29
