...
最新研究《LIFEBENCH》发现，大语言模型（LLMs）在遵循长度指令生成特定字数文本时表现不佳。当要求生成长文本（如1万字）时，常出现内容不足、重复或直接拒绝生成的情况。该研究开发了LIFEBENCH基准测试，评估26个主流模型，结果显示多数模型存在"最大输出长度过度宣传"问题，实际能力与宣传不符。这一"字数危机"揭示了LLMs在长文本生成任务中的显著瓶颈，表明当前模型在精确控制输出长度方面仍有较大改进空间。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T12:04:08Z
- **目录日期**: 2025-05-29
