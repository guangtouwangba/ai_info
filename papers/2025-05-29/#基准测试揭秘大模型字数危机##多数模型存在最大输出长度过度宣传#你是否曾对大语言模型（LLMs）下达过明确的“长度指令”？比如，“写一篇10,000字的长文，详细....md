...
最新研究《LIFEBENCH》通过基准测试评估大语言模型（LLMs）遵循长度指令的能力，发现多数模型在生成特定长度文本时表现不佳。当要求生成长文本（如10,000字）时，模型常出现内容不足、重复或直接拒绝生成的问题。研究表明，许多模型存在最大输出长度过度宣传的情况，实际性能与宣传不符。该研究揭示了LLMs在长文本生成任务中的瓶颈，为未来模型优化提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T14:04:02Z
- **目录日期**: 2025-05-29
