...
最新研究《LIFEBENCH》揭示大语言模型在遵循长度指令方面存在显著缺陷。通过系统测试26个主流模型发现，当要求生成特定长度文本（如万字长文）时，多数模型表现不佳：或输出不足、内容重复，甚至直接拒绝执行。研究表明当前模型的最大输出长度存在过度宣传问题，长文本生成能力普遍未达宣称水平。该基准测试首次系统评估了模型对长度指令的遵循能力，暴露出大模型在精确控制输出规模方面的技术瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T20:03:51Z
- **目录日期**: 2025-05-29
