...
最新研究《LIFEBENCH》发现大语言模型(LLMs)在遵循长度指令方面存在显著缺陷。当要求生成特定长度文本时，多数模型表现不佳，出现内容不足、重复或直接拒绝生成的情况。该研究开发了LIFEBENCH基准测试，系统评估了26个模型的长文本生成能力，结果显示大多数模型存在最大输出长度过度宣传的问题，实际表现与宣传能力存在明显差距。这一发现揭示了当前大模型在长文本生成任务中的关键瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T22:03:52Z
- **目录日期**: 2025-05-29
