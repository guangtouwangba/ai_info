...
最新研究《LIFEBENCH》通过基准测试评估26个大语言模型（LLMs）在遵循长度指令方面的表现，发现多数模型存在显著缺陷。当要求生成特定长度文本（如1万字）时，模型常出现内容不足、重复或直接拒绝生成的问题。研究表明，当前模型对长文本生成存在"字数危机"，且厂商宣传的最大输出长度往往被高估。该测试揭示了LLMs在精确控制输出长度方面的技术瓶颈，对实际应用场景中的长文本生成任务提出了挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T13:12:36Z
- **目录日期**: 2025-05-29
