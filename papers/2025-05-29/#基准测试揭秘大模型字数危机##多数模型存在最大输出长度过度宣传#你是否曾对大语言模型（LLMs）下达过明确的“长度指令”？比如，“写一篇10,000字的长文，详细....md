...
最新研究《LIFEBENCH》发现主流大语言模型（LLM）在遵循长度指令方面存在显著缺陷。当要求生成特定长度文本（如万字长文）时，模型常出现内容不足、重复或直接拒绝生成的情况。该研究提出LIFEBENCH基准测试，系统评估26个模型后发现：多数模型的实际输出长度远低于宣称的最大长度，存在过度宣传问题。这一"字数危机"揭示了LLM在长文本生成任务中的核心瓶颈，表明当前模型对精确长度指令的遵循能力仍待提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T19:03:44Z
- **目录日期**: 2025-05-29
