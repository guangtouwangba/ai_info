...
最新研究《LIFEBENCH》发现，大语言模型（LLMs）在执行明确长度指令（如生成特定字数文本）时表现不佳。通过新基准测试评估26个模型发现，多数存在输出长度不足、内容重复或直接拒绝生成的问题，揭示其"最大输出长度"存在过度宣传现象。该研究系统性暴露了当前大模型在长文本生成任务中的技术瓶颈，表明模型实际能力与宣传存在显著差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T11:03:30Z
- **目录日期**: 2025-05-29
