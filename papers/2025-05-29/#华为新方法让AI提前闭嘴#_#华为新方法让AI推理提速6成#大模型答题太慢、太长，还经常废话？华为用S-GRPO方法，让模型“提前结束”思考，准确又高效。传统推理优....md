...
华为提出S-GRPO新方法解决大模型推理效率问题。该方法通过"串行分组+衰减奖励"机制，允许AI在推理过程中提前终止：先完整推理，再训练模型在不同深度中断作答，并根据"越早答对奖励越高"原则优化。实验显示，S-GRPO在数学和科学推理任务中可节省60%计算量，同时提升准确率，尤其在长度受限场景下，用一半计算资源即可达到更优效果。相比传统GRPO必须完整推理的机制，新方法实现了更短推理路径与更高答对率的双重突破。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T17:03:41Z
- **目录日期**: 2025-05-29
