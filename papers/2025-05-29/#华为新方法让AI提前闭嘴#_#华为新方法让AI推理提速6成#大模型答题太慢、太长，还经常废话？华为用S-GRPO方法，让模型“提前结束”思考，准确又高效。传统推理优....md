...
华为提出S-GRPO新方法优化大模型推理效率，通过"串行分组+衰减奖励"机制，允许AI在推理过程中提前终止。该方法分三步：完整推理→随时中断训练→按退出时间奖励（越早答对奖励越高）。实验显示，S-GRPO在数学/科学任务中节省60%计算量，同时提升准确率，尤其在长度受限时优势显著，仅需一半资源即可达到更好效果。相比传统GRPO的拖底计算，新方法使模型更早输出正确答案，实现推理速度与精度的双重突破。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T22:03:29Z
- **目录日期**: 2025-05-29
