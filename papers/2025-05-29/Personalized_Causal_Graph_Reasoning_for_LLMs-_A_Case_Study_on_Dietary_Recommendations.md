...
大语言模型（LLMs）擅长通用常识推理，但在处理涉及多因素个人数据的个性化推理时表现不足。本文提出"个性化因果图推理"框架，通过整合个人数据构建的因果图来增强LLM的推理能力。该框架在营养导向的饮食推荐案例中进行了验证，采用反事实评估方法衡量LLM推荐食物对血糖管理的效果。结果显示，该方法能有效降低三个时间窗口的平均血糖iAUC，表现优于传统方法。LLM作为评判者的评估也证实，该方法显著提升了推理过程的个性化程度。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T23:02:08Z
- **目录日期**: 2025-05-29
