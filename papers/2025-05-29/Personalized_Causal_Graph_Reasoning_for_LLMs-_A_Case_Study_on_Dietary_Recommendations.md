...
该研究提出了一种基于个性化因果图推理的智能框架，以增强大语言模型(LLMs)在个性化推理方面的能力。传统LLMs擅长常识推理，但在处理多因素个人数据时表现不足。该框架通过构建个人因果图来指导LLM的推理过程，并以营养导向的饮食推荐为案例进行验证。实验采用反事实评估方法，结果显示该方法能有效提供个性化饮食建议，在三个时间窗口内降低平均葡萄糖iAUC，表现优于传统方法。LLM作为评判者的评估也证实该方法显著提升了推理过程的个性化程度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T22:02:15Z
- **目录日期**: 2025-05-29
