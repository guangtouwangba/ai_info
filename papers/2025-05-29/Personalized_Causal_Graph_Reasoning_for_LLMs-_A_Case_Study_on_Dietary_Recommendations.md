...
该论文提出了一种基于个性化因果图推理的智能框架，以增强大语言模型（LLMs）在个性化推理方面的能力。传统LLMs擅长常识推理，但在处理多因素个人数据时表现不足。该框架通过构建个人因果图来指导LLM的推理过程，并以营养导向的饮食建议为案例进行研究。实验采用反事实评估方法，证明该方法能有效推荐个性化饮食方案，显著降低三个时间窗口内的平均血糖水平（iAUC），优于传统方法。LLM作为评判者的评估结果也表明，该方法显著提升了推理过程的个性化程度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T16:02:34Z
- **目录日期**: 2025-05-29
