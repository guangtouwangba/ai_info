...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该方法直接建模音频-视觉关联，利用Stable Diffusion的强大能力，并针对扩散模型的时间一致性问题提出TREPA技术（通过自监督视频特征对齐生成帧）。研究还改进了SyncNet训练框架，解决了其收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他依赖SyncNet的唇同步和音频驱动动画任务。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T14:05:42Z
- **目录日期**: 2025-05-29
