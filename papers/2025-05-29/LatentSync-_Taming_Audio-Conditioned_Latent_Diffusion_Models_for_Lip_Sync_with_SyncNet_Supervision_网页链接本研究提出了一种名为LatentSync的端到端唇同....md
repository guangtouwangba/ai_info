...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉相关性，并针对扩散模型的时间一致性问题提出了时间表示对齐（TREPA）方法。此外，研究改进了SyncNet的训练框架，提高了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有方法。这些创新不仅提升了唇同步效果，也为其他音频驱动的人像动画方法提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T19:05:07Z
- **目录日期**: 2025-05-29
