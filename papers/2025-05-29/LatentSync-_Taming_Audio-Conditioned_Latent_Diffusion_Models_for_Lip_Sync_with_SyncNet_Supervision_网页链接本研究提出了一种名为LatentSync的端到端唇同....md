...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案通过时间表示对齐（TREPA）方法增强生成帧的时间一致性，同时保持唇同步精度。研究还优化了SyncNet训练框架，解决了收敛问题，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）直接建模音频-视觉关系；2）TREPA保持时序连贯性；3）SyncNet训练改进方案。这些成果可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T06:05:43Z
- **目录日期**: 2025-05-29
