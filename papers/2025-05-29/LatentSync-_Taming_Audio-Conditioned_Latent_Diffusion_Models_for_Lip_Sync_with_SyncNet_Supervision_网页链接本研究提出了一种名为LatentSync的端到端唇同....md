...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。为解决扩散过程中时间一致性问题，提出时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时间一致性。同时，通过改进SyncNet的训练框架（包括架构、超参数和数据预处理），显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进也适用于其他唇同步和音频驱动的人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T16:05:33Z
- **目录日期**: 2025-05-29
