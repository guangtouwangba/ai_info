...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，提出TREPA方法增强时序对齐。同时优化SyncNet训练框架，通过改进模型架构、超参数和数据预处理提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供新思路。创新点包括直接建模音视频关联、解决扩散时序问题及SyncNet优化方案，这些经验可推广至其他唇同步研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T18:05:54Z
- **目录日期**: 2025-05-29
