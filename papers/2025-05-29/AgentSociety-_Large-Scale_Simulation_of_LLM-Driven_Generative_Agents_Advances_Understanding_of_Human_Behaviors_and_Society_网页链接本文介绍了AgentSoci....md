...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟10,000+智能代理的社会互动（500万次交互）。该系统通过四个社会实验验证其有效性：研究社会极化、信息传播、基本收入政策及自然灾害影响。模拟结果与现实数据高度吻合，证明其能准确捕捉人类行为机制。该系统为社会科学研究和政策制定提供了创新工具，可辅助传统研究方法（如调查和干预），帮助深入理解复杂社会问题的成因与模式。研究成果突显了大语言模型在社会模拟领域的应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-29T05:05:16Z
- **目录日期**: 2025-05-29
