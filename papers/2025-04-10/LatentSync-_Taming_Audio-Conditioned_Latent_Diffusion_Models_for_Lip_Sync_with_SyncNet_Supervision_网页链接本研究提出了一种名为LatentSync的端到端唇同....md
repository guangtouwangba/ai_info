...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该框架直接建模音频-视觉相关性，无需中间运动表示，克服了传统方法的时间一致性问题。创新点包括：1) 时间表示对齐(TREPA)方法，利用自监督视频模型增强生成帧的时间一致性；2) 对SyncNet收敛问题的系统分析，改进了其训练框架。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进也适用于其他基于SyncNet的唇同步和人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T22:05:07Z
- **目录日期**: 2025-04-10
