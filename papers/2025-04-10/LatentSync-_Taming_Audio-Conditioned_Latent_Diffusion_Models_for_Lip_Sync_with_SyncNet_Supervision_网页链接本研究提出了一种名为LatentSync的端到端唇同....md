...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型提取的时间特征对齐生成帧。同时改进了SyncNet训练框架，解决了收敛问题并提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验也可推广至其他音频驱动动画任务。该研究为基于扩散模型的唇同步提供了有效解决方案，兼具时间连贯性和口型准确性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T20:05:18Z
- **目录日期**: 2025-04-10
