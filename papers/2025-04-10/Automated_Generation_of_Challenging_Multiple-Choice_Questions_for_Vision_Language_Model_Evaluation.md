...
该研究提出了AutoConverter框架，将开放式的视觉问答(VQA)自动转换为多选题形式，以解决现有评测中因回答多样性导致的评估困难。实验表明，该方法能生成正确且具挑战性的多选题，且视觉语言模型(VLM)在其上的表现与人工编写题目相当或更低。基于此构建的VMCBench评测集整合了20个VQA数据集共9,018道多选题，并对33个前沿VLM进行全面评估，为可扩展、一致且可复现的VLM评测设立了新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T18:02:18Z
- **目录日期**: 2025-04-10
