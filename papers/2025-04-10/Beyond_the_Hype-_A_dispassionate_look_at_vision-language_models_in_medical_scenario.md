...
该研究提出了RadVUQA基准，用于全面评估大型视觉语言模型(LVLMs)在医学放射学领域的表现。该基准从五个维度测试模型能力：1)解剖结构识别；2)多模态理解；3)定量与空间推理；4)生理知识掌握；5)对抗不协调/合成数据的鲁棒性。结果显示，无论是通用还是医学专用LVLMs都存在明显缺陷，特别是在多模态理解和定量推理方面表现薄弱。研究表明现有LVLMs与临床医生水平存在巨大差距，亟需开发更强大、智能的模型。相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T10:02:34Z
- **目录日期**: 2025-04-10
