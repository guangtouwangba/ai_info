...
研究表明，在专业分类任务（如家居服务项目分类）中，基于嵌入的Softmax模型在准确性、校准性、延迟和成本上全面优于当前最先进的LLM提示方法。嵌入模型的准确率比提示方法高49.5%，且在纯文本、纯图像及图文混合场景中表现一致更优，同时生成校准良好的概率输出。此外，嵌入方法的处理速度比提示方法快14倍（图像）和81倍（文本），成本可降低至1/10。实际A/B测试验证了离线分析结果，证明基于专有数据集的嵌入方法在多分类任务中具有显著优势，为实际应用提供了更优选择。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T04:01:59Z
- **目录日期**: 2025-04-10
