...
人工智能（AI）正在深刻改变科研范式。本文提出的"Dolphin"是一个闭环的LLM驱动框架，旨在提升科研自动化水平。它通过分析先前实验反馈和相关论文生成新思路，利用经过调试的代码模板实现这些想法，并自动分析结果以优化下一轮研究。实验表明，Dolphin能持续提升输入主题的性能，在某些任务（如3D点分类）中提出的方法甚至可与最先进技术媲美。该框架通过改进数据分析、加速计算和促进创新构想，显著提高了科研效率，向自动化科研目标迈出了重要一步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T11:02:02Z
- **目录日期**: 2025-04-10
