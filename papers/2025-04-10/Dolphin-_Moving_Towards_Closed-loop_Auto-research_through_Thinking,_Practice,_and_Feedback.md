...
人工智能（AI）正在深刻改变科研范式。本文提出的Dolphin框架是一个基于大语言模型（LLM）的闭环系统，旨在提升科研自动化水平。它通过分析实验反馈和相关论文生成新思路，利用代码模板实现方案，并自动分析结果以优化下一轮研究。实验表明，Dolphin能在循环中持续提升性能，在某些任务（如3D点分类）中提出的方法甚至媲美最先进技术。该框架整合了从想法生成到结果反馈的全流程，推动了自动科研的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T09:02:23Z
- **目录日期**: 2025-04-10
