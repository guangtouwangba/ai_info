...
人工智能正在深刻改变科研范式。本文提出的"Dolphin"是一个闭环式大语言模型驱动框架，旨在提升科研自动化水平：1）基于实验反馈和相关文献生成新想法；2）通过异常回溯引导的代码结构实现方案；3）自动分析结果并反馈优化。在多个基准数据集上的实验表明，Dolphin能持续提升研究主题表现，在3D点分类等任务中提出的方法甚至媲美当前最优水平。该框架通过"生成-实现-反馈"的闭环机制，显著推进了自动化科研进程。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T08:02:15Z
- **目录日期**: 2025-04-10
