...
人工智能（AI）正在深刻改变科研范式。本文提出Dolphin框架，利用大语言模型（LLM）实现闭环自动化科研：通过分析实验反馈和相关文献生成新思路，用优化后的代码模板实现，并自动分析结果以指导下一轮研究。在多个基准数据集上的测试表明，Dolphin能持续提升研究性能，在3D点分类等任务中提出的方法甚至媲美最先进技术。该框架通过数据增强、计算加速和新思路生成，显著提高科研效率，向全自动科研目标迈进了一步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T04:02:20Z
- **目录日期**: 2025-04-10
