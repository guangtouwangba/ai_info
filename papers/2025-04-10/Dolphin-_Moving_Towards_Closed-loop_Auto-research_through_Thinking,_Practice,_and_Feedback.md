...
人工智能（AI）正在推动科研范式的变革。本文提出的Dolphin框架利用大型语言模型（LLM）实现闭环自动化科研：通过分析实验反馈和相关文献生成新思路，利用调试优化的代码模板实现方案，并自动分析结果以优化下一轮研究。实验表明，Dolphin能持续提升目标课题性能，在3D点分类等任务中提出的方法甚至媲美当前最优方案。该框架通过数据解析、计算加速和创新生成显著提升科研效率，向全自动科研目标迈出重要一步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T07:02:16Z
- **目录日期**: 2025-04-10
