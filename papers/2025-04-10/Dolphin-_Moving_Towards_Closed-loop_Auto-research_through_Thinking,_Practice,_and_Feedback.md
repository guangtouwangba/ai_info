...
人工智能（AI）正在推动科研范式的变革。本文提出的"Dolphin"是一个闭环的LLM驱动框架，旨在提升科研自动化水平。它通过分析先前实验反馈和相关论文生成新思路，利用调试优化的代码模板实现方案，并自动分析结果以优化下一轮研究。实验表明，Dolphin能在循环中持续提升输入课题的性能，在某些任务（如3D点分类）中提出的方法甚至媲美最先进技术。该框架通过改进数据分析、加速计算和促进创新，显著提高了研究效率，向自动化科研的终极目标迈进了一步。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T23:02:17Z
- **目录日期**: 2025-04-10
