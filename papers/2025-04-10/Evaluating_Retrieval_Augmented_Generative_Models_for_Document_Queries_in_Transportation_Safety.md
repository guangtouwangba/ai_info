...
本研究评估了ChatGPT、Google Vertex AI和ORNL检索增强生成（RAG）优化的LLaMA 2/LLaMA模型在美国危险品运输法规检索中的表现。通过40份法规文件和100个实际查询测试发现，RAG增强的LLaMA模型在准确性、细节和相关性上显著优于其他模型，尽管偶有不一致。这是RAG在运输安全领域的首次应用，凸显了高风险领域中模型专业调优和严格评估的重要性，以确保可靠性并减少错误风险。研究为专业领域的大语言模型应用提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T22:01:41Z
- **目录日期**: 2025-04-10
