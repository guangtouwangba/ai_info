...
FEABench是一个评估大语言模型(LLMs)和LLM代理能力的基准测试，用于通过有限元分析(FEA)模拟和解决物理、数学和工程问题。该研究开发了全面的评估方案，测试LLMs能否端到端地处理自然语言问题描述，并操作COMSOL Multiphysics软件进行计算。研究还设计了一个能通过API与软件交互、检查输出并通过多轮迭代改进解决方案的语言模型代理。最佳策略可生成88%可执行的API调用。这种能力将增强LLMs的数值求解精度，推动工程自动化发展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T00:01:53Z
- **目录日期**: 2025-04-10
