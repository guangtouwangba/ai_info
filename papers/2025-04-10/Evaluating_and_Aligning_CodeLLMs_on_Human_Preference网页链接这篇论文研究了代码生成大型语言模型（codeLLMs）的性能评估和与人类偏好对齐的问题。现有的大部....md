...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好的对齐问题。研究指出当前基准测试过于关注代码正确性而忽视人类偏好，为此提出了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令数据集（20B标记）。实验发现执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著，凸显人类偏好对齐的重要性。研究通过大规模实验验证了合成指令微调的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T09:06:17Z
- **目录日期**: 2025-04-10
