...
这篇论文针对代码生成大型语言模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个高质量编程任务样本(覆盖40类/44种语言)，首次关注代码生成与人类偏好的对齐问题；2) 构建近20B标记的SynCode-Instruct合成指令数据集验证微调效果。实验发现现有基于执行的评估与人类偏好存在差距，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著，凸显了人类偏好对齐在代码生成中的重要性。研究为提升代码LLMs的实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T08:06:04Z
- **目录日期**: 2025-04-10
