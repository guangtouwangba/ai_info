...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，指出当前基准测试过于关注代码正确性而忽视人类偏好。研究者创建了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好存在偏差；2)开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调代码LLMs需同时满足功能正确和人类编程习惯，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T15:06:23Z
- **目录日期**: 2025-04-10
