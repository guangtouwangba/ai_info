...
这篇论文提出了SGD-SaI优化方法，质疑了自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，有效解决了训练不平衡问题，并将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于AdamW，尤其在内存效率方面优势显著（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。同时，该方法对超参数变化鲁棒，在LoRA微调等任务中也超越现有优化器，展示了广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T10:04:40Z
- **目录日期**: 2025-04-10
