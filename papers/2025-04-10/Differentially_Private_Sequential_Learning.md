...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。连续信号采用新型平滑随机响应机制，根据与阈值的距离自适应添加噪声，相比传统均匀噪声，能更有效结合私有信号和历史观察，将渐近学习速度提升至Θε(log(n))（非隐私场景为Θ(√log(n))）。有限隐私预算ε∈(0,1)下，首次正确决策的期望停止时间和错误动作次数均有限，避免了非隐私场景中早期代理长期犯错的问题。连续信号在隐私学习中更高效，而二元信号因隐私噪声易形成信息级联前的恒定随机响应策略，削弱动作信息性，阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-10T01:29:26Z
- **目录日期**: 2025-04-10
