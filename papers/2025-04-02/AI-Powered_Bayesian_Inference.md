...
生成式人工智能(GAI)的出现改变了知识获取方式。虽然GAI不能完全信任，但其随机性可转化为优势：通过不同回答构建先验分布，反映AI预测的可信度。研究提出非参数贝叶斯框架，以GAI模型为基线设置Dirichlet过程先验，结合真实数据和AI生成的伪数据进行后验模拟。该方法通过优化而非采样快速获得独立同分布后验样本，实现可并行化的概率推断。关键创新在于利用AI预测进行不确定性量化，在保留概率一致性的同时，实现预测性推断。超参数可通过样本外评估调整，衡量AI先验的信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T10:01:20Z
- **目录日期**: 2025-04-02
