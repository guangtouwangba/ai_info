...
生成式人工智能(GAI)的出现改变了知识获取方式。虽然GAI决策不完全可靠，但其随机性可被转化为优势：通过不同回答构建先验分布，反映AI预测的确定性。本研究提出非参数贝叶斯框架，以GAI模型为基线设置Dirichlet过程先验分布，结合真实数据和AI生成的标记数据，通过优化而非采样快速获得后验独立同分布样本。该方法实现了基于AI预测的(预测性)推断和不确定性量化，将AI信息以概率方式整合到分析流程中。超参数可通过样本外评估调整，衡量AI先验信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T11:01:09Z
- **目录日期**: 2025-04-02
