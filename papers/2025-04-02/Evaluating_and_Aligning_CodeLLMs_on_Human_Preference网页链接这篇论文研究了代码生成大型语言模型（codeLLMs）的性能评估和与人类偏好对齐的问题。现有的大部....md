...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1) 创建CodeArena基准(397个样本/40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令集验证微调效果。实验发现：现有执行基准与人类偏好存在显著差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类对齐方面差距明显。研究强调了除代码正确性外，符合开发者习惯和需求的对齐能力对代码LLMs至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T21:05:20Z
- **目录日期**: 2025-04-02
