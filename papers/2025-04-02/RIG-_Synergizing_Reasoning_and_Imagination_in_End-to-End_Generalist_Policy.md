...
本文提出了一种结合推理与想象能力的端到端通用策略RIG，通过构建渐进式整合想象与推理内容的数据管道进行联合训练。该方法显式建模了推理、动作与环境动态间的内在关联，相比之前工作提升了17倍以上的样本效率与泛化能力。在推理阶段，RIG先推理潜在动作，再预测动作结果，使智能体能在真实行动前进行自我修正。实验表明，推理与想象的协同不仅提高了策略的鲁棒性、泛化性和互操作性，还能通过测试时扩展提升整体性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T01:28:25Z
- **目录日期**: 2025-04-02
