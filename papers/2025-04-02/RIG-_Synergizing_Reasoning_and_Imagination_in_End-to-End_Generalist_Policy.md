...
本文提出了一种结合推理与想象的端到端通用策略RIG，通过构建渐进式数据管道整合现有智能体的轨迹数据，联合训练推理与图像生成能力，显式建模推理、动作与环境动态的关联。相比之前方法，RIG展现出17倍以上的样本效率提升和泛化能力。在推理阶段，RIG先推理动作，生成潜在动作并预测结果，使智能体能在执行前自我修正。实验表明，这种协同作用不仅提升了策略的鲁棒性、泛化性和互操作性，还能通过测试时扩展增强整体性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T02:27:54Z
- **目录日期**: 2025-04-02
