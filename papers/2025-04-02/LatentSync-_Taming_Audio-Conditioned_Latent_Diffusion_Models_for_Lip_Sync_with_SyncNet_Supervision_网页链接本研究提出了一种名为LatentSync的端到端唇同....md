...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案创新性地采用时间表示对齐(TREPA)方法解决扩散模型的时间一致性问题，通过自监督视频模型对齐生成帧。研究还改进了SyncNet训练框架，解决了收敛问题并提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可应用于其他依赖SyncNet的唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T15:05:58Z
- **目录日期**: 2025-04-02
