...
这篇论文介绍了AgentSociety，这是一个基于大型语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（500万次）。研究者用它分析了四个关键社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界数据吻合，验证了该工具在理解人类行为和社会机制方面的有效性。AgentSociety展示了作为社会科学研究和政策制定辅助工具的潜力，能补充传统研究方法（如调查和访谈）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T20:04:58Z
- **目录日期**: 2025-04-02
