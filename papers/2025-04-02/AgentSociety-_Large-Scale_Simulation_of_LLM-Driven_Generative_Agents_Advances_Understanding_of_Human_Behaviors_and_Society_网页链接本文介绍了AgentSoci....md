...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个AI代理的社会互动（500万次）。研究者用它分析四大社会问题：社会极化、刺激性信息传播、基本收入政策影响和自然灾害（如飓风）冲击。模拟结果与现实世界数据高度吻合，验证了该系统捕捉人类行为机制的能力。该平台可作为计算社会科学的新型实验工具，辅助传统研究方法（调查、访谈等），帮助学者和政策制定者深入理解社会问题的成因与模式。研究展示了LLM驱动模拟器在社会研究中的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T18:05:06Z
- **目录日期**: 2025-04-02
