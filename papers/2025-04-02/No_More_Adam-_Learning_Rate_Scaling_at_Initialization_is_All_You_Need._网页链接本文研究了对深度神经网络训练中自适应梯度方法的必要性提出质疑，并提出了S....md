...
该论文提出了一种名为SGD-SaI的新优化方法，取代了传统自适应梯度方法（如Adam）。该方法基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信噪比（g-SNR）调整学习率，避免了训练不平衡问题。SGD-SaI显著减少了内存占用（相比AdamW节省5.93-25.15GB），在Transformer任务、ImageNet分类和LLM（如GPT-2）预训练中表现优异，甚至超越AdamW。此外，它在LoRA微调等任务中也优于现有优化器，同时具备超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T17:04:31Z
- **目录日期**: 2025-04-02
