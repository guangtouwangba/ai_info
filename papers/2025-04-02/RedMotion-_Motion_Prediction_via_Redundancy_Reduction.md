...
RedMotion是一种基于Transformer的自动驾驶运动预测模型，通过冗余减少学习环境表征。它采用两种冗余减少方法：1）内部Transformer解码器将可变大小的局部道路环境标记（道路图和智能体数据）压缩为固定大小的全局嵌入；2）自监督学习对增强道路环境视图的嵌入应用冗余减少原则。实验表明，在半监督设置下，RedMotion优于PreTraM、Traj-MAE和GraphDINO，并在Waymo运动预测挑战中与HPTR和MTR++性能相当。该模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T21:02:18Z
- **目录日期**: 2025-04-02
