...
RedMotion是一种基于Transformer的自动驾驶运动预测模型，通过冗余减少学习环境表征。它采用两种冗余减少方法：一是通过内部Transformer解码器将可变大小的局部道路环境标记（包含道路图和智能体数据）压缩为固定大小的全局嵌入；二是通过自监督学习对道路环境增强视图生成的嵌入应用冗余减少原则。实验表明，该模型在半监督设置下优于PreTraM、Traj-MAE和GraphDINO，并在Waymo运动预测挑战中取得与HPTR和MTR++相当的竞争力。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T19:02:21Z
- **目录日期**: 2025-04-02
