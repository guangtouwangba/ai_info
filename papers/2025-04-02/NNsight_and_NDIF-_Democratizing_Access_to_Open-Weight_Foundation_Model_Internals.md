...
NNsight和NDIF是两项协同工作的技术，用于大规模神经网络内部表征和计算的科学研究。NNsight是一个开源系统，扩展了PyTorch以实现延迟远程执行；NDIF是一个可扩展的推理服务，执行NNsight请求，支持共享GPU资源和预训练模型。它们基于干预图架构，将实验设计与模型运行时解耦。该框架透明高效地访问大型神经网络（如大语言模型）内部，无需单独托管定制模型。研究表明大规模AI内部研究存在日益扩大的差距，该框架通过支持多种研究方法填补这一空白。性能基准测试显示优于以往方法。详见nnsight.net。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T09:02:42Z
- **目录日期**: 2025-04-02
