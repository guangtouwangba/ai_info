...
NNsight和NDIF是两项协同工作的技术，旨在支持对大型神经网络内部表征和计算机制的科学研究。NNsight是一个开源系统，扩展了PyTorch以实现延迟远程执行；NDIF则是可扩展的推理服务，执行NNsight请求并共享GPU资源和预训练模型。它们基于"干预图"架构，将实验设计与模型运行解耦，为研究者提供透明高效的神经网络内部访问能力，特别是针对大语言模型(LLM)。该框架解决了当前AI研究中大规模模型内部机制研究不足的问题，支持多种研究方法，同时避免了单独托管定制模型的高成本和复杂性。相关资源已在nnsight.net发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T05:02:45Z
- **目录日期**: 2025-04-02
