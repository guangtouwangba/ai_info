...
NNsight和NDIF是两项协同工作的技术，旨在支持对大型神经网络内部表征和计算机制的科学分析。NNsight是一个开源系统，扩展了PyTorch以支持远程延迟执行；NDIF则是一个可扩展的推理服务，执行NNsight请求并共享GPU资源和预训练模型。该框架通过"干预图"架构解耦实验设计与模型运行时，为研究者提供透明高效的神经网络内部访问能力（包括大语言模型），而无需承担独立托管定制模型的高成本。研究显示当前大规模AI内部机制研究存在明显缺口，该框架为此缺口提供了多种研究方法支持。相关代码和文档见nnsight.net。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T07:02:15Z
- **目录日期**: 2025-04-02
