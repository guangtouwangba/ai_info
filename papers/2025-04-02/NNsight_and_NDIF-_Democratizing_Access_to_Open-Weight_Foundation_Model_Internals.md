...
NNsight和NDIF是两项协同工作的技术，旨在支持对大型神经网络内部表征和计算的科学研究。NNsight是一个开源系统，扩展了PyTorch以实现延迟远程执行；NDIF则是一个可扩展的推理服务，执行NNsight请求，允许用户共享GPU资源和预训练模型。它们通过"干预图"架构解耦实验设计与模型运行时，为研究大型语言模型(LLM)等深度神经网络内部机制提供了透明高效的访问方式，无需单独托管定制模型。该框架填补了大规模AI内部研究日益扩大的空白，支持多种研究方法，性能优于以往方案。详见nnsight.net。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T21:01:51Z
- **目录日期**: 2025-04-02
