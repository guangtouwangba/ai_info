...
NNsight和NDIF是两项协同工作的技术，旨在支持对大型神经网络内部表征和计算的科学研究。NNsight是一个开源系统，扩展了PyTorch以实现延迟远程执行；NDIF则是一个可扩展的推理服务，执行NNsight请求，允许用户共享GPU资源和预训练模型。通过干预图（Intervention Graph）架构，该框架将实验设计与模型运行时解耦，透明高效地访问大型语言模型等深度神经网络的内部机制，无需单独托管定制模型。研究展示了该框架支持多种研究方法的能力，并通过基准测试比较了与之前方法的性能差异。更多信息见nnsight.net。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T04:02:28Z
- **目录日期**: 2025-04-02
