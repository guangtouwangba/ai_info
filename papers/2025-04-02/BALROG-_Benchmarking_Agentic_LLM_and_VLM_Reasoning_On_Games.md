...
研究人员开发了名为BALROG的新基准测试，用于评估大语言模型(LLM)和视觉语言模型(VLM)在复杂动态环境中的智能体能力。该测试整合了从简单到极难(如NetHack)的多层次强化学习环境，设计了精细评估指标。研究发现当前模型在简单任务中表现尚可，但在复杂任务(尤其是需要视觉决策时)表现显著不足，部分模型在提供视觉信息后性能反而下降。BALROG作为开源基准平台(balrogai.com)发布，旨在促进智能体研究发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T14:01:47Z
- **目录日期**: 2025-04-02
