...
研究人员开发了名为BALROG的新基准测试，用于评估大语言模型(LLMs)和视觉语言模型(VLMs)在复杂动态环境中的代理能力。该测试包含从简单到极难(如NetHack)的各类游戏任务，设计了细粒度指标评估模型表现。研究发现当前模型在简单任务中表现尚可，但在复杂任务中表现欠佳，尤其是基于视觉的决策能力存在严重缺陷——部分模型在提供视觉环境时表现更差。该团队开源了BALROG基准测试，旨在促进智能代理领域的未来发展。代码和排行榜可在balrogai.com获取。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T05:02:17Z
- **目录日期**: 2025-04-02
