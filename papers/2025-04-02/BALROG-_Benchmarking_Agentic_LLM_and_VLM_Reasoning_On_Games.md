...
BALROG是一个新型基准测试，旨在评估大语言模型(LLM)和视觉语言模型(VLM)在复杂动态环境中的代理能力。该测试整合了多个难度级别的强化学习环境，从人类可快速解决的简单任务到需要多年掌握的困难任务(如NetHack)。研究发现，当前模型在简单游戏中表现尚可，但在复杂任务中表现欠佳，特别是基于视觉的决策能力存在严重缺陷——部分模型在提供视觉环境表征时表现更差。BALROG作为开源基准平台(balrogai.com)发布，以促进代理能力研究的进一步发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T13:08:45Z
- **目录日期**: 2025-04-02
