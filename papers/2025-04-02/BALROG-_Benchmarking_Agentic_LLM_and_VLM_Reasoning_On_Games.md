...
研究人员提出了BALROG基准测试，用于评估大型语言模型(LLMs)和视觉语言模型(VLMs)在复杂动态环境中的代理能力。该基准整合了多种难度级别的强化学习环境，从简单到极难(如NetHack)。测试发现，当前模型在简单任务中表现尚可，但在复杂任务中表现欠佳，特别是基于视觉的决策能力存在严重缺陷——部分模型在提供视觉环境时表现更差。BALROG作为一个开放、用户友好的基准测试平台，旨在推动代理能力研究的未来发展。相关代码和排行榜可在balrogai.com获取。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T17:01:45Z
- **目录日期**: 2025-04-02
