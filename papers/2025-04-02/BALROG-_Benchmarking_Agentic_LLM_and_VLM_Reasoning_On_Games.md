...
研究人员开发了名为BALROG的新基准测试，用于评估大型语言模型(LLMs)和视觉语言模型(VLMs)在复杂动态环境中的代理能力。该基准整合了从简单到极难(如NetHack)的多样化强化学习环境，并设计了精细评估指标。测试发现，当前模型在简单任务中表现尚可，但在复杂任务(尤其是基于视觉的决策)中表现欠佳，部分模型在提供视觉信息时表现反而更差。BALROG作为开源基准测试平台，旨在推动智能代理领域的研究发展。代码和排行榜可在balrogai.com获取。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T20:01:41Z
- **目录日期**: 2025-04-02
