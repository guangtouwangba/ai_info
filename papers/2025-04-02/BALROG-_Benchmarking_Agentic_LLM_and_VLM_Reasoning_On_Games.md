...
研究人员开发了名为BALROG的新基准测试，用于评估大语言模型(LLM)和视觉语言模型(VLM)在复杂动态环境中的代理能力。该测试整合了从简单到极难(如NetHack)的多种强化学习环境，设计了精细的评估指标。测试发现，当前模型在简单任务中表现尚可，但在高难度任务中表现不佳，尤其是基于视觉的决策能力存在严重缺陷——部分模型在提供视觉环境时表现反而更差。该基准测试已开源，旨在促进智能体研究发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T11:01:38Z
- **目录日期**: 2025-04-02
