...
BALROG是一个评估大语言模型(LLMs)和视觉语言模型(VLMs)在复杂环境中代理能力的新基准。该基准整合了多种难度级别的强化学习环境，从人类可快速解决的任务到极难任务(如NetHack)。研究发现，当前模型在简单任务中表现尚可，但在复杂任务(尤其是基于视觉的决策)中表现不佳，部分模型在提供视觉环境时表现反而更差。BALROG提供了细粒度评估指标，旨在促进代理能力研究的未来发展。相关代码和排行榜已在balrogai.com公开。(99字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T09:02:08Z
- **目录日期**: 2025-04-02
