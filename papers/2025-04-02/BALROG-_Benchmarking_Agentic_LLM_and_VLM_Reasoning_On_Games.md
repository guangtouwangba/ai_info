...
研究人员提出了BALROG基准测试，用于评估大语言模型(LLM)和视觉语言模型(VLM)在复杂动态环境中的代理能力。该基准整合了多种难度级别的强化学习环境，从人类可快速解决的任务到极难任务(如NetHack)。研究发现，当前模型在简单任务中表现尚可，但在复杂任务(尤其是基于视觉的决策)中表现欠佳，部分模型在提供视觉环境时表现反而更差。BALROG作为开放基准，旨在促进智能代理领域的研究发展。代码和排行榜可在balrogai.com获取。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T23:01:40Z
- **目录日期**: 2025-04-02
