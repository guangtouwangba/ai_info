...
PathOrchestra是一个基于自监督学习的大规模病理学基础模型，训练数据包含来自多中心的30万张病理切片，涵盖20种组织和器官类型。该模型在112项临床任务中进行了严格评估，包括癌症分类、病变识别、生物标志物评估和结构化报告生成等。在27,755张全切片图像和9,415,729个感兴趣区域上表现出色，47项任务准确率超过0.950。特别值得注意的是，它首次实现了高发结直肠癌和复杂淋巴瘤的结构化报告生成。PathOrchestra展示了大规模病理学基础模型的可行性和高效性，其高准确率和低标注依赖性使其具有显著的临床应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T02:28:17Z
- **目录日期**: 2025-04-02
