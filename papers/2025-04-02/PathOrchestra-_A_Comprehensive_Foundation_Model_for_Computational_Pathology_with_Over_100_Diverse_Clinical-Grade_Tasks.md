...
PathOrchestra是一种基于自监督学习的大规模病理基础模型，使用来自多个中心的20种组织器官的30万张病理切片训练而成。该模型在112项临床任务上进行了严格评估，涵盖癌症分类、病变识别、生物标志物评估、基因表达预测和结构化报告生成等。PathOrchestra在27,755张WSI和9,415,729个ROI上表现出色，47项任务准确率超过0.950，包括泛癌分类和淋巴瘤亚型诊断等。尤为突出的是，它首次实现了高发病率结直肠癌和复杂淋巴瘤的结构化报告生成。该模型展示了大规模病理基础模型的可行性，减少了对数据标注的依赖，具有临床整合潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T03:13:38Z
- **目录日期**: 2025-04-02
