...
DLV2是一个基于逻辑的知识表示与推理AI工具，支持回答集编程(ASP)。它采用"Ground & Solve"方法，先将逻辑程序转换为等价的基础程序，再通过命题算法求解。为适应流式推理等新兴需求，DLV2发展出了增量推理功能：系统在多次执行时保持活跃状态，增量处理基础程序，重用先前计算结果，自动维护一个更通用的基础程序，从中提取等效子集进行求解。这一过程对用户完全透明。论文介绍了该系统及其在实际领域的应用表现。该研究已被《逻辑编程理论与实践》(TPLP)期刊考虑发表。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T19:01:19Z
- **目录日期**: 2025-04-02
