...
DLV2是一个基于知识表示与推理的AI工具，支持答案集编程（ASP），广泛应用于学术和工业领域。它采用"Ground & Solve"方法，将逻辑程序转换为等价的基础程序并求解答案集。为适应流推理和事件处理等新兴需求，DLV2发展为支持增量推理的迭代系统。该系统在多次执行中保持活跃状态，复用先前计算，增量处理基础程序，仅计算等效部分以提升效率，且对用户完全透明。研究还展示了其在实际领域中的适用性和性能表现。该成果已被《逻辑编程理论与实践》(TPLP)期刊考虑发表。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T04:01:36Z
- **目录日期**: 2025-04-02
