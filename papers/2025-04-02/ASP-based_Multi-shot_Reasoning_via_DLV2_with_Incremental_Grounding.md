...
DLV2是一种基于知识表示与推理的AI工具，支持答案集编程（ASP）。它采用"Ground & Solve"方法，将逻辑程序转换为等价的基础程序并搜索答案集。针对流推理和事件处理等新兴需求，DLV2发展出增量推理功能，可在多次执行中保持系统活跃状态，复用先前计算结果，增量处理基础程序，而无需从头开始。这一过程对用户完全透明。研究展示了该系统在实践相关领域的应用和性能表现。该论文正在《逻辑编程理论与实践》(TPLP)期刊审稿中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T20:01:23Z
- **目录日期**: 2025-04-02
