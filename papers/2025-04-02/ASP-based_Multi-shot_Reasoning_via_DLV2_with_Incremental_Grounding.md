...
DLV2是一款基于逻辑的知识表示与推理AI工具，支持答案集编程（ASP）。它采用"Ground & Solve"方法，将输入程序转换为等价的基础程序，再通过命题算法搜索答案集。为适应流推理和事件处理等新兴需求，DLV2发展出增量推理功能：系统在多次执行中保持活跃状态，重用先前计算结果，增量处理基础程序，每次仅处理等效子集而非重新计算。这一过程对用户完全透明。研究展示了该系统在实践领域的适用性和性能表现。该论文正在《逻辑编程理论与实践》(TPLP)期刊审稿中。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T14:01:26Z
- **目录日期**: 2025-04-02
