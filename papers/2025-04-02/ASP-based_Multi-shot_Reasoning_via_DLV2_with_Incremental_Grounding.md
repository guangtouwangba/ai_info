...
DLV2是一个基于知识表示与推理(KRR)的AI工具，支持回答集编程(ASP)。它采用"Ground & Solve"方法，先将逻辑程序转换为等价的基础程序，再通过命题算法求解。为适应流式推理等新兴应用需求，DLV2发展出了增量推理功能：系统在多次执行时保持活跃状态，复用先前计算结果，增量处理基础化过程，每次仅处理变化部分而非重新计算。这种增量过程对用户完全透明。论文介绍了该系统在实践领域的应用和性能表现，目前正在《逻辑编程理论与实践》(TPLP)期刊审稿中。(100字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T18:01:27Z
- **目录日期**: 2025-04-02
