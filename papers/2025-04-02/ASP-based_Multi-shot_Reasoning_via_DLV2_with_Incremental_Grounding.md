...
DLV2是一个基于逻辑的知识表示与推理工具，支持答案集编程(ASP)，广泛应用于学术和工业领域。它采用"Ground & Solve"方法，将逻辑程序转换为等价的基础程序后求解答案集。为适应流推理和事件处理等新兴需求，DLV2演变为支持增量推理的迭代系统。该系统在多次执行时保持活跃状态，增量处理基础化过程，重用先前计算结果构建更通用的基础程序，从中提取等效子集进行求解，整个过程对用户透明。研究还探讨了该系统在实践领域的应用和性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T06:01:24Z
- **目录日期**: 2025-04-02
