...
DLV2是一个基于逻辑的知识表示与推理AI工具，支持回答集编程(ASP)。它采用"Ground & Solve"方法，将逻辑程序转换为等价的基础程序后搜索解集。为适应流推理等新兴需求，研究团队开发了增量推理版本，通过跨多次计算保持系统运行状态，复用先前结果来增量处理基础程序，避免重复计算。新系统能自动维护更通用的基础程序并提取等效子集，实现透明化的增量推理。论文介绍了该系统在实践领域的应用与性能表现，目前正由《理论与逻辑编程实践》(TPLP)期刊评审。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T12:01:33Z
- **目录日期**: 2025-04-02
