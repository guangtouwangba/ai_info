...
DLV2是一种基于逻辑的知识表示与推理AI工具，支持答案集编程(ASP)。它采用"Ground & Solve"方法，先将逻辑程序转换为等效基础程序，再搜索答案集。为适应流推理等新兴应用需求，研究团队开发了DLV2的增量推理版本，支持多轮迭代计算。该系统在多次执行中保持活跃状态，通过增量式处理重用先前计算结果，动态维护一个更通用的基础程序，并从中提取等效子集进行求解。整个过程对用户完全透明。论文还探讨了该系统在实际领域中的应用与性能表现。该研究已提交至《逻辑编程理论与实践》(TPLP)期刊。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T10:01:31Z
- **目录日期**: 2025-04-02
