...
DLV2是一个基于知识表示与推理(KRR)的AI工具，支持答案集编程(ASP)。它采用"Ground & Solve"方法，将逻辑程序转换为等价的基础程序并求解答案集。针对流式推理等新兴需求，研究团队开发了DLV2的增量推理版本，支持多轮迭代计算。该系统能跨轮次保持运行状态，增量处理基础化过程，重用先前计算结果构建更通用的基础程序，并自动提取等效子集进行求解，整个过程对用户透明。论文还探讨了该系统在实际应用领域的适用性和性能表现。该成果已提交至《理论与逻辑编程实践》(TPLP)期刊。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T07:01:32Z
- **目录日期**: 2025-04-02
