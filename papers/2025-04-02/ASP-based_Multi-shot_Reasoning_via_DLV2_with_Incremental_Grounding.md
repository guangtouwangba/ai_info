...
DLV2是一个基于知识表示与推理的AI工具，支持答案集编程（ASP）。它采用"Ground & Solve"方法，先将逻辑程序转换为等价的基础程序，再通过命题算法求解答案集。为适应流式推理等新兴需求，DLV2演变为支持增量推理的系统：在多次执行时保留计算状态，复用先前结果构建更通用的基础程序，仅处理变化部分来高效生成答案集。这一过程对用户完全透明。研究展示了该系统在实践领域的适用性和性能表现。该成果已被《逻辑编程理论与实践》(TPLP)期刊考虑发表。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T16:01:23Z
- **目录日期**: 2025-04-02
