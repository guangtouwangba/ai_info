...
DLV2是一个基于知识表示与推理(KRR)的AI工具，支持回答集编程(ASP)。它采用"Ground & Solve"方法，先将逻辑程序转换为等价的基础程序，再通过命题算法搜索答案集。为适应流式推理等新兴应用需求，DLV2已进化为支持增量推理的系统。该系统在多次执行时保持活跃状态，增量处理基础化过程，复用先前计算结果构建更通用的基础程序，仅使用等效子集计算答案集，且对用户完全透明。研究展示了该系统在实践相关领域的应用性和性能表现。该成果已被《理论与逻辑编程实践》(TPLP)期刊考虑发表。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T09:01:40Z
- **目录日期**: 2025-04-02
