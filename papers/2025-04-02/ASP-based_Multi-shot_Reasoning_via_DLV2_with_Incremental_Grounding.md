...
DLV2是一个基于逻辑的知识表示与推理AI工具，支持答案集编程（ASP）。它采用"Ground & Solve"方法，先将逻辑程序转换为等价的基础程序，再搜索答案集。针对流式推理等新兴需求，DLV2发展出了增量推理能力：系统在多次执行中保持活跃状态，复用先前计算结果，增量处理基础程序，而非每次都重新开始。这种增量过程对用户完全透明，每次仅处理必要的程序片段。研究展示了该系统在实践领域的应用和性能表现。该论文正在《逻辑编程理论与实践》(TPLP)期刊审稿中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-02T13:08:22Z
- **目录日期**: 2025-04-02
