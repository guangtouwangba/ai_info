...
本研究提出了一种新型的AI驱动质量工程平台，采用自适应差分进化量子变分自编码器-变压器模型（ADE-QVAET），以解决现有模型在处理噪声数据、不平衡问题、模式识别复杂性、特征提取无效和泛化能力弱等方面的挑战。ADE-QVAET结合量子变分自编码器-变压器（QVAET）来获取高维潜在特征并保持序列依赖性和上下文关系，从而提高缺陷预测的准确性。通过自适应差分进化（ADE）优化，模型实现了更好的收敛性和预测性能。实验结果显示，ADE-QVAET在训练百分比（TP）90时，准确率、精确率、召回率和F1分数分别达到98.08%、92.45%、94.67%和98.12%，表现出色。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T02:27:10Z
- **目录日期**: 2025-03-22
