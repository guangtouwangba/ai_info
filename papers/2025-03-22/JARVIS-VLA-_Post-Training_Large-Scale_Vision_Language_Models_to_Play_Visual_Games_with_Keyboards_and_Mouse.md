...
近年来，开放世界环境中的基于行动的决策引起了广泛关注。预训练于大规模网络数据集的视觉语言行动（VLA）模型在决策任务中表现出潜力。然而，先前的研究主要集中在行动后训练，常忽视基础模型的改进。为此，我们提出了一种新方法，通过视觉和语言指导以自监督方式改进视觉语言模型（VLMs），提升了模型在开放世界环境中的世界知识、视觉识别和空间定位能力。实验表明，后训练在非轨迹任务上带来了40%的性能提升，超越了传统模仿学习策略，并开源了代码、模型和数据集以促进进一步研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T05:02:10Z
- **目录日期**: 2025-03-22
