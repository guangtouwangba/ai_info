...
本研究提出了LatentSync，一种端到端的唇同步框架，利用音频条件的潜在扩散模型直接建模音频与视觉的复杂关系，无需中间运动表示。该框架通过时间表示对齐（TREPA）方法增强时间一致性，解决了扩散过程中时间一致性的问题。此外，研究还改进了SyncNet的训练框架，提高了其准确性。这些创新使LatentSync在HDTF和VoxCeleb2数据集上的表现超越了现有方法，为唇同步和音频驱动的人像动画提供了新的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T14:06:20Z
- **目录日期**: 2025-03-22
