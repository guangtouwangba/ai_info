...
本研究提出了一种名为LatentSync的端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模复杂的音频视觉相关性。研究解决了扩散方法中时间一致性不足的问题，提出了时间表示对齐（TREPA）方法，利用大规模自监督视频模型提取的时间表示来增强生成帧与真实帧的一致性。同时，研究改进了SyncNet的训练框架，提高了其准确性。该方法在HDTF和VoxCeleb2数据集上表现优异，超越了现有唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T23:05:08Z
- **目录日期**: 2025-03-22
