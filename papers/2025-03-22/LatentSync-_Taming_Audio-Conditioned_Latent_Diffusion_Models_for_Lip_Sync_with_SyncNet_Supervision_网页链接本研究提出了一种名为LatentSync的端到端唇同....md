...
本研究提出了一种名为LatentSync的端到端唇同步框架，利用音频条件的潜在扩散模型直接建模复杂的音频视觉相关性，无需中间运动表示。为解决时间一致性问题，提出了时间表示对齐（TREPA）方法，通过自监督视频模型提取的时间表示对齐生成帧与真实帧。研究还改进了SyncNet的训练框架，提高了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T21:04:58Z
- **目录日期**: 2025-03-22
