...
本研究提出了一种名为LatentSync的端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模复杂的音频视觉相关性，无需中间运动表示。为解决时间一致性问题，提出了时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时间一致性。研究还改进了SyncNet的训练框架，显著提高了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步方法，为音频驱动的人像动画提供了新的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T16:06:39Z
- **目录日期**: 2025-03-22
