...
UniSync是一种新颖的音频-视频同步评估方法，通过嵌入相似性来处理不同维度的音频（如梅尔频谱、HuBERT）和视觉（如RGB图像、面部解析图、面部标志、3DMM）表示。该方法在对比学习框架中引入了基于边界的损失组件和跨说话者非同步对，提升了判别能力。UniSync在标准数据集上优于现有方法，并展示了其在多种音频-视觉表示中的通用性。集成到说话人脸生成框架后，UniSync显著提高了自然和AI生成内容的同步质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T15:02:36Z
- **目录日期**: 2025-03-22
