...
西北大学的研究团队提出了一种名为MetaSpatial的新框架，旨在提升视觉语言模型（VLM）在三维空间布局任务中的推理能力。传统方法依赖于大规模标注数据和监督微调（SFT），但受限于空间任务的多样性和缺乏唯一标准答案。MetaSpatial采用基于规则奖励的强化学习（RFT）策略，通过程序化评估奖励函数和多轮布局优化机制，显著提升了模型的空间推理能力和布局生成质量。实验表明，MetaSpatial在仅使用50条无标注数据的情况下，优于传统SFT方法，展示了强化学习在三维空间智能建模中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-22T19:02:48Z
- **目录日期**: 2025-03-22
