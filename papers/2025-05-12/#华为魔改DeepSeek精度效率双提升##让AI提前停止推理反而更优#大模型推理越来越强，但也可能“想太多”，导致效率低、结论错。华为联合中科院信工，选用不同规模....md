...
华为联合中科院信工所提出DEER方法，通过动态判断思维链临界点让大模型提前终止推理。该方法在DeepSeek-R1-Distill-Qwen系列模型（1.5B-32B）上验证，可减少31%-43%生成长度同时提升1.7-5.7%准确率。DEER通过监测推理转折点、生成试验性答案并评估置信度实现动态退出，配合分支并行机制降低延迟。实验显示部分问题仅需20%推理步骤即可保持60.8%准确率，代码生成任务中长度减半且准确率提升。该研究证明适度缩短推理链可兼顾效率与精度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T04:05:15Z
- **目录日期**: 2025-05-12
