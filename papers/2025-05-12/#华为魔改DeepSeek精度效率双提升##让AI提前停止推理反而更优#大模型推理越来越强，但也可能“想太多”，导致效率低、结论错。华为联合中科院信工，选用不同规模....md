...
华为联合中科院信工所提出DEER方法，通过动态退出机制优化DeepSeek-R1-Distill-Qwen大模型推理效率。该方法能识别思维链中的关键临界点（Pearl Reasoning），在置信度高时提前终止推理，使生成长度减少31%-43%，准确率提升1.7-5.7个百分点。实验显示，部分问题仅需20%推理步骤即可保持60.8%准确率。DEER采用三步动态判断（监测转换点、生成试验答案、置信评估）和分支并行机制，在代码生成等任务中实现长度减半且准确率提升。研究成果表明适度缩短推理链反而能提升模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T03:22:26Z
- **目录日期**: 2025-05-12
