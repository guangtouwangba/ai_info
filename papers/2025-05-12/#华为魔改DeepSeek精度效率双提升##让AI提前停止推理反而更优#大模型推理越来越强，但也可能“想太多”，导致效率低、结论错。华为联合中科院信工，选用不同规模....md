...
华为联合中科院信工所提出DEER方法，通过动态判断推理链中的关键节点（Pearl Reasoning），让大模型在信息足够时提前停止推理。该方法在DeepSeek-R1-Distill-Qwen系列模型（1.5B-32B）上验证，通过监测思维转换点、生成试验性答案并评估置信度实现动态退出。实验显示DEER使准确率提升1.7-5.7%，生成长度减少31-43%，在代码生成任务中长度减半且准确率上升。研究还发现部分问题仅需20%推理步骤即可保持60.8%准确率，证明传统完整推理链并非必需。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T01:30:36Z
- **目录日期**: 2025-05-12
