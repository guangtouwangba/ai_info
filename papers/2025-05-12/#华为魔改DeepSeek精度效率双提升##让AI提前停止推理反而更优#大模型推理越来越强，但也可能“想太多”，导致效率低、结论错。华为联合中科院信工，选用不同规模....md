...
华为联合中科院信工所提出DEER方法，通过动态判断推理临界点（Pearl Reasoning）让大模型提前终止推理。该方法在DeepSeek系列模型（1.5B-32B）上验证：当检测到高置信度答案时强制退出，减少31%-43%生成长度，同时提升1.7-5.7%准确率。关键创新包括：1）监控思维转换点生成试验性答案；2）置信度评估与并行推理机制。实验显示，MATH-500数据集仅需20%推理步骤即可保持60.8%准确率，代码生成任务中长度减半且准确率提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T00:03:06Z
- **目录日期**: 2025-05-12
