...
该研究提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈。研究者用改良版Nim游戏验证框架后，分析了四种监督博弈（黑手党、辩论、后门代码、战争游戏），发现领域表现与AI系统能力间存在可量化的扩展规律。基于此，他们理论研究了嵌套可扩展监督（NSO）——即信任模型监督更强非信任模型并迭代的过程，确定了NSO成功的条件及最优监督层级数。实验显示，在400等级分差下，四种游戏的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且随系统变强进一步下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T11:02:08Z
- **目录日期**: 2025-05-12
