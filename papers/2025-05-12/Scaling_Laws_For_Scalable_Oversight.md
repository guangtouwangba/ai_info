...
本文提出了一个量化监督成功概率的框架，将监督建模为能力不匹配玩家间的博弈，监督能力是通用智力的分段线性函数。研究通过Nim游戏验证框架后，分析了四种监督博弈（Mafia、辩论、后门代码、战争游戏），发现领域表现与AI能力间的缩放规律。进一步理论研究了嵌套式可扩展监督（NSO），即信任模型监督更强非信任模型的迭代过程，确定了NSO成功条件及最优监督层级数。实验显示在400 Elo差距下，四种博弈的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且随系统变强成功率进一步下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T22:01:54Z
- **目录日期**: 2025-05-12
