...
本文提出一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈，监督能力表现为分段线性函数。研究通过改良Nim游戏验证框架，并应用于黑手党、辩论、后门代码和战争游戏四种监督场景，发现领域表现与AI能力间的缩放规律。进一步理论分析嵌套可扩展监督(NSO)机制，确定了监督成功的条件，并计算出最优监督层级数。实验显示在400 Elo差距下，四种场景的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且随被监督系统增强而下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T04:03:56Z
- **目录日期**: 2025-05-12
