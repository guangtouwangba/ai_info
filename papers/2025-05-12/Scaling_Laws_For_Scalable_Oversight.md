...
本文提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈。研究者通过Nim游戏和四个监督博弈（Mafia、辩论、后门代码、战争游戏）验证框架，发现领域表现与AI系统能力间存在缩放规律。基于此，理论研究了嵌套可扩展监督（NSO）——由可信模型监督更强不可信模型的迭代过程，确定了NSO成功条件及最优监督层级数。实验显示，在400 Elo差距下，四个游戏的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且成功率随被监督系统变强而进一步下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T20:02:09Z
- **目录日期**: 2025-05-12
