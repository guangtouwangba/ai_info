...
本文提出一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家间的博弈，监督能力被表示为分段线性函数，包含"任务无能"和"任务饱和"两个平台期。研究者通过改良Nim游戏验证框架后，应用于黑手党、辩论、后门代码和战争游戏四种监督场景，发现领域表现与AI系统能力存在比例关系。在嵌套可扩展监督(NSO)的理论研究中，确定了NSO成功的条件及最优监督层级数。实验显示，当Elo差距为400时，四种游戏的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且随系统变强而降低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T17:02:18Z
- **目录日期**: 2025-05-12
