...
本文提出了一个量化监督成功概率的框架，将监督建模为能力不匹配玩家之间的博弈，监督者的Elo评分是其通用智力的分段线性函数。研究通过Nim游戏验证框架后，分析了四种监督博弈（Mafia、Debate、Backdoor Code、Wargames）的扩展规律。进一步理论研究了嵌套可扩展监督（NSO），即信任模型监督更强非信任模型的迭代过程，确定了NSO成功的条件及最优监督层级数。实验显示，在400 Elo差距下，四种游戏的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且成功率随被监督系统变强而下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T07:02:08Z
- **目录日期**: 2025-05-12
