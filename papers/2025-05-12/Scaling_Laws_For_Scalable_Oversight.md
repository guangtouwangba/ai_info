...
该研究提出了一个量化监督成功概率的框架，将可扩展监督建模为能力不匹配玩家之间的博弈。监督者和被监督者的能力用分段线性Elo分数表示，包含"任务无能"和"任务饱和"两个平台期。研究通过改良Nim游戏验证框架后，分析了四种监督博弈（黑手党、辩论、后门代码、战争游戏），发现领域表现与AI能力间的缩放规律。进一步理论研究嵌套可扩展监督(NSO)时，确定了NSO成功的条件，并计算出最优监督层级数。实验显示在400Elo差距下，四种游戏的NSO成功率分别为13.5%、51.7%、10.0%和9.4%，且随系统变强而下降。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T03:20:55Z
- **目录日期**: 2025-05-12
