...
这篇论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过算法与硬件协同优化，采用动态分层稀疏策略：先进行粗粒度令牌压缩，再进行细粒度令牌选择，在保持全局上下文感知能力的同时确保局部精度。该方法在稀疏注意力设计上有两项关键创新，实现了高效的长文本建模能力。NSA既能降低计算开销，又保持了模型的表达能力，为大模型处理长序列任务提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T15:04:53Z
- **目录日期**: 2025-05-12
