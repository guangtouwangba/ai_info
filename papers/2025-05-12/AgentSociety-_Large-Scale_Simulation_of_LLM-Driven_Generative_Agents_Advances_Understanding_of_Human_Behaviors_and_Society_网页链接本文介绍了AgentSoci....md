...
AgentSociety是一个基于大型语言模型的大规模社会模拟器，可模拟超过1万个代理的社会互动（500万次）。研究者用它作为计算社会科学实验平台，研究了社会极化、信息传播、基本收入政策及自然灾害影响等社会问题。模拟结果与现实数据吻合，验证了该系统捕捉人类行为机制的能力，展示了其作为社会科学研究和政策制定工具的潜力。该系统结合了LLM代理、真实社会环境和大规模模拟引擎，为传统研究方法（如调查、访谈）提供了有力补充。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T15:05:04Z
- **目录日期**: 2025-05-12
