...
ProME v1.0是一个集成计算平台，用于极端条件下的材料性能研究，涵盖高温（数万开尔文）、高压（数百万大气压）和高应变率（每秒百万次）。该平台整合了多种软件工具，包括晶体结构搜索（ABC）、无序构型建模（SAE）、热力学性质预测（MFP²）、弹性建模（HTEM）、电热导率计算（TREX）、高应变率相场模拟（Hippos）和相图优化（AutoCalphad）。ProME已成功应用于航空航天发动机喷嘴用铂-铱-铝-铬四元合金设计，在保持高温强度的同时大幅降低成本。该平台为材料研发的基础科学和工业创新提供了关键支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T06:01:05Z
- **目录日期**: 2025-05-12
