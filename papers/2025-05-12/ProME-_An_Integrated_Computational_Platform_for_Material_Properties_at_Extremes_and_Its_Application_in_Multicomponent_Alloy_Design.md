...
ProME v1.0是一个集成计算平台，用于极端条件下的材料性能研究，涵盖高温（数万开尔文）、高压（数百万大气压）和高应变率（每秒数百万次）。该平台整合了多种软件工具，包括晶体结构搜索（ABC）、无序构型建模（SAE）、热力学性质计算（MFP²）、热弹性分析（HTEM）、电热导率模拟（TREX）、高应变率相场模拟（Hippos）和相图优化（AutoCalphad）。应用案例包括设计航天发动机喷嘴用Pt-Ir-Al-Cr四元合金，在保持高温强度的同时大幅降低成本。ProME为材料研发提供了重要的理论和工业创新支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T03:19:54Z
- **目录日期**: 2025-05-12
