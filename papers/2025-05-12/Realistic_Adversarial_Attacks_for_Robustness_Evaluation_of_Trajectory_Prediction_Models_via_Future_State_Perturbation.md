...
本文提出一种更全面的对抗性测试方法，通过扰动周围车辆的过去和未来状态来评估自动驾驶轨迹预测模型的鲁棒性。相比仅扰动历史位置的传统方法，新方法结合动态约束并保持战术行为，能生成更真实有效的对抗轨迹。实验表明，该方法能显著增加预测误差和碰撞率，并暴露出模型关键弱点（如无法识别潜在碰撞）。研究强调需要更全面的对抗测试来提升自动驾驶预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T15:01:20Z
- **目录日期**: 2025-05-12
