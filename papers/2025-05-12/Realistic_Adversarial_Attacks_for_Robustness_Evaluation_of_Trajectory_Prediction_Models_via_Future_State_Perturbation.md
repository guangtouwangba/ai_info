...
轨迹预测是自动驾驶系统的关键功能，用于预判其他道路使用者的行为。现有对抗性攻击方法通常仅扰动周围车辆的过去轨迹，可能导致不真实场景并遗漏关键漏洞。本研究提出通过扰动对抗智能体的过去和未来状态，结合动态约束和战术行为保持，实现更真实有效的攻击。新评估指标显示，该方法能显著增加先进预测模型的误差和碰撞率，暴露出模型在看似安全预测中无法识别潜在碰撞等弱点。结果表明，需要更全面的对抗性测试来提升自动驾驶轨迹预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T04:03:31Z
- **目录日期**: 2025-05-12
