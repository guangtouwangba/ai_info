...
当前轨迹预测模型的对抗攻击评估通常仅扰动周围智能体的历史位置，可能导致不现实场景并忽略关键漏洞，造成模型性能评估过于乐观。本研究提出一种新方法，通过扰动对抗智能体的历史和未来状态，结合动态约束并保持战术行为，实现更有效且真实的对抗攻击。新引入的性能指标可评估攻击轨迹的真实性和影响。测试表明，该方法能显著增加先进预测模型的误差和碰撞率，并暴露关键弱点（如无法检测看似安全预测中的潜在碰撞）。这凸显了需要更全面的对抗测试来提升自动驾驶轨迹预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T06:01:34Z
- **目录日期**: 2025-05-12
