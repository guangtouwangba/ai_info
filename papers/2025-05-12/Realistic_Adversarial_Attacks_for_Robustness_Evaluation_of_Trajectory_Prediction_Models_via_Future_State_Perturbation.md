...
本文提出了一种更全面的对抗性测试方法，通过扰动自动驾驶轨迹预测模型中周围车辆的过去和未来状态，来评估模型鲁棒性。新方法结合动态约束并保持战术行为，能生成更真实有效的对抗轨迹，揭示传统方法无法检测的模型缺陷。实验表明，该方法能显著增加预测误差和碰撞率，暴露模型在看似安全预测中无法识别潜在碰撞等关键弱点。研究强调需要更全面的对抗测试来提升自动驾驶轨迹预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T02:31:04Z
- **目录日期**: 2025-05-12
