...
当前自动驾驶轨迹预测模型的对抗攻击测试主要针对周围车辆的过去位置扰动，但这种方法可能产生不真实场景并忽略关键漏洞。本研究提出一种新方法，通过扰动对抗车辆的过去和未来状态，结合动态约束并保持战术行为，实现更有效且真实的攻击。新引入的性能指标评估了攻击轨迹的真实性和影响。测试表明，该方法能显著增加预测误差和碰撞率，并暴露出模型无法检测潜在碰撞等关键缺陷。研究强调需要更全面的对抗测试，以更好地评估和提高自动驾驶预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T23:01:27Z
- **目录日期**: 2025-05-12
