...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，重点关注模型输出与人类偏好的对齐问题。研究者构建了包含397个样本的基准测试(覆盖40类任务和44种语言)，并开发20B标记的合成指令数据集SynCode-Instruct。实验发现现有基于执行的评估与人类偏好存在差距，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著。研究强调代码生成模型不仅需要正确性，还需符合开发者实际偏好，为改进代码LLMs提供了重要方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T15:05:47Z
- **目录日期**: 2025-05-12
