...
这篇论文针对代码生成大模型(codeLLMs)提出新评估方法，指出当前基准测试过于侧重代码正确性而忽略人类偏好。研究者创建了CodeArena基准(包含397个高质量样本，覆盖40类44种语言)和SynCode-Instruct语料库(近20B标记)，发现执行基准与人类偏好存在差异。实验比较40多个模型显示，顶级开源模型(如Qwen2.5-Coder)与专有模型(如OpenAI o1)性能差距显著，凸显对齐人类偏好的重要性。研究证实大规模合成指令微调的有效性，为代码LLM评估提供了更全面的框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T07:06:25Z
- **目录日期**: 2025-05-12
