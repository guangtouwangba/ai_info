...
该研究提出SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统认知。该方法改进带动量SGD，通过在初始化阶段根据梯度信噪比(g-SNR)调整各组参数的学习率，无需自适应二阶动量即可避免训练失衡。相比AdamW，SGD-SAi内存用量减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，并显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。在LoRA微调等任务中也优于现有优化器，具有超参数鲁棒性和广泛应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T05:04:36Z
- **目录日期**: 2025-05-12
