...
这篇28页的综述总结了多智能体具身智能的前沿进展，重点包括：1）具身智能三要素（物理身体、环境交互、经验学习）；2）EMAPF框架实现组内集中控制与组间分布式协调；3）开放环境中采用图神经网络和自博弈机制实现动态适应；4）利用生成模型（如SMART-LLM）进行任务分解分配；5）分布式决策通过独立模型协作或混合评估机制优化。研究突破了单智能体局限，通过协同控制、动态适应和智能分配提升复杂环境下的多机器人协作效能。论文链接：https://arxiv.org/abs/2505.05108

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T14:03:30Z
- **目录日期**: 2025-05-12
