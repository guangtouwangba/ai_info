...
这篇28页的综述总结了多智能体具身智能的前沿进展。核心要素包括物理身体（具身性）、环境交互和适应性学习。研究重点包括：1）EMAPF框架实现组内集中控制与组间分布式协调；2）利用图神经网络和Transformer处理动态团队规模，通过自我博弈实现持续进化；3）生成模型（如SMART-LLM）进行任务分解分配，并优化子任务依赖关系；4）分布式决策采用独立模型协作或混合评估机制。这些技术推动多智能体在开放环境中的协作能力，弥补了传统单智能体研究的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T17:03:23Z
- **目录日期**: 2025-05-12
