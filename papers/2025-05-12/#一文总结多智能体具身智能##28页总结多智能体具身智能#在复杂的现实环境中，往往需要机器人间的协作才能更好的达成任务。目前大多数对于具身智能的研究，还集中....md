...
这篇28页综述总结了多智能体具身智能的前沿技术。核心要素包括物理身体、环境交互和经验学习。在控制规划方面，EMAPF框架通过动态聚类实现组内集中控制和组间分布式协调。针对开放环境，采用图神经网络处理团队规模变化，并通过自我博弈实现持续进化。任务分配方面，SMART-LLM等模型将大任务分解为并行子任务。分布式决策采用独立模型协作或混合评估机制。最新趋势包括闭环动态调整、子任务依赖图优化和集中式隐式分配，推动多智能体在复杂环境中的协作能力发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T19:02:55Z
- **目录日期**: 2025-05-12
