...
这篇28页的综述总结了多智能体具身智能领域的最新进展。核心要素包括物理身体（具身性）、环境交互和适应性学习。研究重点包括：1）EMAPF框架通过动态聚类实现组内集中控制与组间分布式协调；2）开放环境中利用图神经网络和自博弈机制实现动态团队适应与持续进化；3）SMART-LLM等任务分配方法通过生成模型分解任务并优化分配逻辑；4）分布式决策采用独立模型协作或混合评估机制。这些技术推动多智能体系统在复杂环境中实现高效协作与动态适应。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T12:03:33Z
- **目录日期**: 2025-05-12
