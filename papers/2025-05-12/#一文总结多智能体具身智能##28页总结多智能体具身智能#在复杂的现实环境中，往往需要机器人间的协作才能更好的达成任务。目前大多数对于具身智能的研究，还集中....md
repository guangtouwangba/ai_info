...
这篇28页的综述总结了多智能体具身智能的前沿技术。核心要素包括物理身体、环境交互和经验学习。在控制规划方面，EMAPF框架采用动态聚类实现组内集中控制和组间分布式协调。开放环境中的自进化机制利用图神经网络和Transformer处理团队规模变化，并通过自我博弈持续优化。任务分配方面，SMART-LLM等方法用语言模型分解任务并动态调整。分布式决策则通过独立模型协作或混合评估机制实现。这些技术推动了多智能体在复杂环境中的协作能力发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T10:03:21Z
- **目录日期**: 2025-05-12
