...
这篇28页的综述总结了多智能体具身智能的最新进展。核心要素包括物理身体（具身性）、环境交互和适应性学习。研究重点包括：1）EMAPF框架通过动态聚类实现组内集中控制与组间分布式协调；2）开放环境中采用图神经网络和自博弈机制实现动态团队适应与持续进化；3）任务分配方面，SMART-LLM等模型利用语言模型分解任务并优化分配逻辑；4）分布式决策采用独立模型协作或混合评估机制。这些方法通过生成模型增强多智能体在复杂环境中的协作能力。论文链接：https://arxiv.org/abs/2505.05108

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T15:03:07Z
- **目录日期**: 2025-05-12
