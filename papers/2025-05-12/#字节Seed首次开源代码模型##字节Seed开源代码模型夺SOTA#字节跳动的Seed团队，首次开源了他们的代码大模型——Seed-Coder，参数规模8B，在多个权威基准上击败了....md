...
字节跳动Seed团队首次开源代码大模型Seed-Coder（8B参数），在多项基准测试中超越Qwen3等模型达到SOTA水平。该模型包含基础版、指令优化版和推理强化版三个版本，创新性地采用"小模型自管数据"范式，通过四类代码数据源（文件/仓库级代码、Git提交、网络内容）和自动化质量筛选（双重去重、语法检查、LLM评分）构建训练集。特别设计了针对不同网络内容风格的定制化评分机制，显著减少人工干预。模型已完整开源，在IOI 2024等任务中展现优异表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T02:33:02Z
- **目录日期**: 2025-05-12
