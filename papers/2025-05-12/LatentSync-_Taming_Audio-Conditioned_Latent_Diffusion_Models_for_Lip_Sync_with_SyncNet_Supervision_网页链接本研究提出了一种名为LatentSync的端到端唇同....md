...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。针对扩散模型的时间一致性问题，提出TREPA方法增强帧间对齐，同时保持唇同步精度。研究还改进了SyncNet训练框架，通过优化模型架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：直接建模音频-视觉关联的扩散模型、解决时间一致性的TREPA方法，以及SyncNet训练的优化方案，这些经验可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T14:05:51Z
- **目录日期**: 2025-05-12
