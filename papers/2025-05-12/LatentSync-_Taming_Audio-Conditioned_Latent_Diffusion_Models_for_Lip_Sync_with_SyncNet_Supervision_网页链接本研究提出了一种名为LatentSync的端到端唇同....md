...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模复杂音视频关系，但发现扩散过程中存在时间一致性问题，因此提出TREPA方法增强一致性。同时针对SyncNet收敛问题进行了实证研究，改进了训练框架。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步技术。创新点包括：1)端到端潜在扩散模型；2)TREPA时间对齐方法；3)SyncNet训练优化方案。这些改进可推广至其他音视频同步任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T11:05:43Z
- **目录日期**: 2025-05-12
