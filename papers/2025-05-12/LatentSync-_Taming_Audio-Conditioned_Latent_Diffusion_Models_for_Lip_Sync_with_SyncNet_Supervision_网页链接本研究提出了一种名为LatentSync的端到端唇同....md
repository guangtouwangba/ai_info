...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，提出了TREPA方法增强帧间对齐；同时优化了SyncNet训练框架，解决了其收敛问题。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。创新点包括：1）直接建模音视频关联的扩散模型架构；2）提升时序一致性的TREPA技术；3）改进SyncNet训练的关键因素分析。该成果对音频驱动人像动画领域具有普适参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T04:06:51Z
- **目录日期**: 2025-05-12
