...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模音频-视觉关系而无需中间运动表示。针对扩散模型的时间一致性问题，开发了时间表示对齐(TREPA)方法，利用自监督视频模型增强生成帧的时间一致性。同时改进了SyncNet训练框架，解决了其收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些创新不仅提升了唇同步效果，也为其他音频驱动动画方法提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T16:05:38Z
- **目录日期**: 2025-05-12
