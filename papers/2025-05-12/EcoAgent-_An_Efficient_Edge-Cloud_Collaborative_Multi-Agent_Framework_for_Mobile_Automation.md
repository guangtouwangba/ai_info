...
EcoAgent是一个边缘-云协同的多智能体框架，旨在解决基于云的大语言模型（LLM）高延迟、高成本的问题，同时弥补边缘部署的小模型（SLM）通用能力不足的缺陷。该框架包含云端的规划智能体和边缘的执行、观察智能体，形成闭环协作：执行智能体操作设备，观察智能体通过图像压缩模块将屏幕信息转为精简文本以减少token消耗。若任务失败，规划智能体通过反射模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了token消耗，实现了高效的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T00:02:06Z
- **目录日期**: 2025-05-12
