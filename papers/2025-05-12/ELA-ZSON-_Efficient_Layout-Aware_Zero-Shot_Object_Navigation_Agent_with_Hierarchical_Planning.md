...
ELA-ZSON是一种高效的无样本目标导航方法，适用于复杂的多房间室内环境。它通过结合全局拓扑地图（含布局信息）和局部场景表示记忆，实现分层规划，由LLM驱动的智能体自主执行导航，无需人工干预、复杂奖励或昂贵训练。在MP3D基准测试中，该方法达到85%的目标导航成功率（SR）和79%的路径加权成功率（SPL），相比现有方法分别提升40%和60%。虚拟代理和真实机器人实验验证了其鲁棒性和实用性。（完整摘要：https://anonymous.4open.science/r/ELA-ZSON-C67E/）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T14:01:51Z
- **目录日期**: 2025-05-12
