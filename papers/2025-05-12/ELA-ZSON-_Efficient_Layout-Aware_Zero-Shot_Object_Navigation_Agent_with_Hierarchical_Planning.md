...
ELA-ZSON是一种高效的无训练目标导航方法，适用于复杂多房间室内环境。它通过结合全局拓扑地图布局信息和局部场景表示记忆进行分层规划，由LLM驱动的智能体自主管理导航过程，无需人工干预、复杂奖励或昂贵训练。在MP3D基准测试中，该方法实现了85%的目标导航成功率（SR）和79%的路径加权成功率（SPL），较现有方法分别提升40%和60%。虚拟代理和真实机器人实验验证了其鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T04:03:39Z
- **目录日期**: 2025-05-12
