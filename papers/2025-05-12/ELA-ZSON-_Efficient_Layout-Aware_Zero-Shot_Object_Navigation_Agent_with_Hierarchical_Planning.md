...
ELA-ZSON是一种高效的零样本物体导航方法，适用于复杂多房间室内环境。它通过结合全局拓扑地图（含布局信息）和局部场景表征记忆进行分层规划，实现高效导航。该方法由LLM驱动的智能体管理，无需人工交互、复杂奖励机制或昂贵训练。在MP3D基准测试中，其物体导航成功率（SR）达85%，路径加权成功率（SPL）达79%（比现有方法SR提升40%，SPL提升60%）。虚拟代理和真实机器人实验验证了其鲁棒性和实用性。详情见：https://anonymous.4open.science/r/ELA-ZSON-C67E/ （100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T09:02:00Z
- **目录日期**: 2025-05-12
