...
ELA-ZSON是一种高效的零样本物体导航方法，适用于复杂的多房间室内环境。它通过结合全局拓扑地图（含布局信息）和局部场景表示记忆，实现分层规划导航。该方法由LLM驱动的智能体管理，无需人工干预、复杂奖励机制或高成本训练。在MP3D基准测试中，ELA-ZSON达到85%的导航成功率（SR）和79%的路径加权成功率（SPL），相比现有方法分别提升40%和60%。虚拟代理和真实机器人实验验证了其鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T08:01:44Z
- **目录日期**: 2025-05-12
