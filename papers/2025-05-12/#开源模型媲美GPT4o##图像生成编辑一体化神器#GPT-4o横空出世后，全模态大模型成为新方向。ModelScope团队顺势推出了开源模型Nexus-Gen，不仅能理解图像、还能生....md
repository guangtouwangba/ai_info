...
ModelScope团队推出开源多模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer和扩散模型技术，实现图像理解、生成与编辑一体化。创新性地采用"预填充自回归"策略解决误差积累问题，并在高维特征空间建模提升图像质量。使用25M规模数据训练，特别依赖ImagePulse数据集处理各类编辑任务。模型支持多提示词输入和故事线式编辑，目前论文、代码、模型及数据集均已开源。团队计划进一步优化模型规模等方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T03:22:13Z
- **目录日期**: 2025-05-12
