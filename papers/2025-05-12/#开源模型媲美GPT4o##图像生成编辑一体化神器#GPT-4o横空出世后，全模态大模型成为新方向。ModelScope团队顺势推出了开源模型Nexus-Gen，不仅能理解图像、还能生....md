...
ModelScope团队推出开源多模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer和扩散模型技术，支持图像理解、生成和编辑三大功能，采用"预填充自回归"策略解决误差积累问题。训练使用25M规模数据，图像编辑依赖ImagePulse数据集。模型支持多prompt和故事线编辑，目前论文、代码、模型和数据集均已开源。未来计划在模型规模等方面继续优化。关键技术包括高维特征空间建模和分离训练自回归/扩散模块。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T02:32:40Z
- **目录日期**: 2025-05-12
