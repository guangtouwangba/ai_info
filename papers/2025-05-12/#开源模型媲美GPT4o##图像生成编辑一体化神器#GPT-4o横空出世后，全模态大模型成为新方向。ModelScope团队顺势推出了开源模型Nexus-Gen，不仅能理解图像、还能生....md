...
ModelScope团队推出开源多模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer和扩散模型技术，在高维特征空间进行建模，显著提升图像质量。采用"预填充自回归"策略解决误差积累问题，统一处理图像理解、生成和编辑任务，使用25M规模数据集训练。特别依赖ImagePulse数据集进行图像编辑训练，支持多prompt和故事线式编辑。模型代码、论文及数据集已全部开源，未来将在模型规模等方面持续优化。技术亮点包括语言理解与图像渲染能力的融合，以及训练推理一致性的提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-12T05:04:05Z
- **目录日期**: 2025-05-12
