...
上海AI实验室推出开源框架GraphGen，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架通过"知识图谱引导+双模型协同"机制，能自动生成专业QA数据，无需人工标注。用户只需在OpenXLab上传文本块并填写API Key，即可在线生成适配LLaMA-Factory等框架的训练数据，直接用于大模型SFT训练。项目已在GitHub开源并提供Web应用体验，支持海事等垂直领域的高质量数据合成，显著降低专业数据获取成本。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T01:30:17Z
- **目录日期**: 2025-04-27
