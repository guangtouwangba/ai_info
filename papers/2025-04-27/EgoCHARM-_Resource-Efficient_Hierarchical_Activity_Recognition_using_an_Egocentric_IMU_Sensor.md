...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于智能眼镜上的单IMU设备进行高低层次活动识别。该算法采用半监督分层学习策略，主要利用高层次活动标签训练，学习可泛化的低层次运动嵌入特征。实验在9种高层次和3种低层次活动上取得0.826和0.855的F1分数，模型参数仅63k（高层次）和22k（低层次），可直接部署在当前IMU芯片上。研究还分析了基于头戴式IMU的活动识别技术的机遇与限制，为轻量级可穿戴活动识别提供了有效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T19:01:12Z
- **目录日期**: 2025-04-27
