...
该研究提出了一种资源高效的机器学习算法EgoCHARM，用于智能眼镜上的单IMU设备进行人类活动识别。该分层算法采用半监督学习策略，主要利用高层活动标签训练，学习可泛化的低层运动嵌入特征，实现高低层活动识别。在9种高层和3种低层活动测试中，分别获得0.826和0.855的F1分数，模型参数仅63k(高层)和22k(低层)，可直接部署于现有IMU芯片。研究还通过敏感性分析揭示了头戴式IMU活动识别的优势与局限。该方法在资源占用和识别性能间取得了良好平衡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T00:01:31Z
- **目录日期**: 2025-04-27
