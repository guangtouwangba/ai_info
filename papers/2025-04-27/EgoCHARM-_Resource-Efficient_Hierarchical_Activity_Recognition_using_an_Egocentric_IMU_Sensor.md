...
本文提出EgoCHARM算法，一种基于头戴式惯性测量单元(IMU)的资源高效机器学习方法，用于识别高低层活动。该分层算法采用半监督学习策略，主要利用高层活动标签进行训练，学习可泛化的低层运动嵌入特征。实验在9种高层和3种低层活动上分别取得0.826和0.855的F1分数，模型参数仅63k(高层)和22k(低层)，可直接部署于现有IMU芯片。研究还分析了敏感度，探讨了头戴IMU活动识别的优势与局限，为健康监测和情境感知AI等应用提供了轻量级解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T10:01:07Z
- **目录日期**: 2025-04-27
