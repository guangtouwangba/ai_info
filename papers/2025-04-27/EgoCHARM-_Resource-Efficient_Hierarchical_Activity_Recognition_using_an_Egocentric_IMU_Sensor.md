...
本文提出EgoCHARM算法，通过单个头戴式惯性测量单元(IMU)高效识别高低层次活动。该分层算法采用半监督学习策略，主要利用高层次活动标签训练，学习可泛化的低层次运动嵌入特征。实验在9种高层次和3种低层次活动上分别达到0.826和0.855的F1分数，模型参数仅63k(高层次)和22k(低层次)，可直接部署于现有IMU芯片。研究还分析了基于头戴IMU的活动识别技术的敏感性和应用限制，为智能眼镜在健康追踪和情境感知等应用提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T01:28:50Z
- **目录日期**: 2025-04-27
