...
本文提出了一种名为EgoCHARM的资源高效机器学习算法，用于通过头戴式惯性测量单元(IMU)识别高低层级活动。该分层算法采用半监督学习策略，主要利用高层级活动标签进行训练，学习可泛化的低层级运动嵌入特征。实验在9种高层级和3种低层级活动上分别获得0.826和0.855的F1分数，模型参数仅63k(高层)和22k(低层)，可直接部署在当前IMU芯片上。研究还分析了使用头戴式IMU进行活动识别的机会与限制，为智能眼镜上的健康追踪和情境感知AI应用提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T22:01:16Z
- **目录日期**: 2025-04-27
