...
本文提出EgoCHARM算法，用于智能眼镜单IMU设备的高效人类活动识别。该分层算法采用半监督学习策略，仅需高级活动标签训练，即可学习可泛化的低级运动嵌入特征，同时识别高级和低级活动。在9种高级和3种低级活动测试中，分别取得0.826和0.855的F1分数，模型参数仅63k（高级）和22k（低级），可直接部署于现有IMU芯片。研究还分析了基于头戴式IMU的活动识别技术的敏感性和应用限制，为资源受限场景下的可穿戴活动识别提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T18:01:08Z
- **目录日期**: 2025-04-27
