...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于通过头戴式惯性测量单元(IMU)识别高级和低级活动。该分层算法采用半监督学习策略，主要使用高级活动标签进行训练，学习可泛化的低级运动嵌入，有效用于低级活动识别。实验在9种高级和3种低级活动上获得0.826和0.855的F1分数，模型参数仅63k(高级)和22k(低级)，可直接部署在当前IMU芯片上。研究还分析了灵敏度，探讨了头戴式IMU活动识别的机会与限制。该方法在性能和资源效率上优于现有技术。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T21:01:09Z
- **目录日期**: 2025-04-27
