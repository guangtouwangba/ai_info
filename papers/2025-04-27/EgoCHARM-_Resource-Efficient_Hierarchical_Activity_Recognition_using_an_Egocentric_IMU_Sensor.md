...
本文提出EgoCHARM算法，一种基于单目惯性测量单元(IMU)的资源高效机器学习方法，用于智能眼镜上的高/低层次活动识别。该分层算法采用半监督学习策略，主要利用高层次活动标签进行训练，学习可泛化的低层次运动嵌入特征。实验在9种高层次和3种低层次活动上分别获得0.826和0.855的F1分数，模型参数仅63k(高层次)和22k(低层次)，可直接部署在当前IMU芯片上。研究还分析了使用单目IMU进行活动识别的机会与限制，为资源受限场景下的穿戴式活动识别提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T07:01:10Z
- **目录日期**: 2025-04-27
