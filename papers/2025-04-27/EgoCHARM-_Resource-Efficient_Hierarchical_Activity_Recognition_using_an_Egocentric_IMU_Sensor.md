...
本文提出EgoCHARM算法，一种基于头戴式惯性测量单元(IMU)的资源高效活动识别方法。该分层算法采用半监督学习策略，主要利用高级活动标签进行训练，同时学习可泛化的低级运动嵌入特征。实验在9种高级和3种低级活动上分别达到0.826和0.855的F1分数，模型参数量仅63k(高级)和22k(低级)，可直接部署在当前IMU芯片上。研究还通过敏感性分析探讨了头戴IMU活动识别的机会与限制。该方法在保持高性能的同时显著降低了计算资源需求，适用于智能眼镜等可穿戴设备的活动识别应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T20:01:20Z
- **目录日期**: 2025-04-27
