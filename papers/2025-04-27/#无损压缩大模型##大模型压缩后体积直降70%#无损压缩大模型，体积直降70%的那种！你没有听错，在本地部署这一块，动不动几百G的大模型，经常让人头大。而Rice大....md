# #无损压缩大模型##大模型压缩后体积直降70%#无损压缩大模型，体积直降70%的那种！你没有听错，在本地部署这一块，动不动几百G的大模型，经常让人头大。而Rice大...
...
Rice大学与xMAD.ai团队开发了DFloat11无损压缩框架，可将大模型体积减少70%且保持精度无损。以Llama-3.1-405B为例，模型从811.7GB压缩至551.22GB，仅需8张80GB GPU即可运行。该技术通过挖掘BFloat16的数据稀疏性（仅使用2.6位有效信息）、动态长度编码和GPU优化设计（查找表、并行解压等），使推理速度提升最高38.8倍。相比传统压缩方法，DFloat11专为高效推理设计，显著降低CPU-GPU数据传输开销。代码已开源，适用于本地大模型部署场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T11:03:11Z
- **目录日期**: 2025-04-27
