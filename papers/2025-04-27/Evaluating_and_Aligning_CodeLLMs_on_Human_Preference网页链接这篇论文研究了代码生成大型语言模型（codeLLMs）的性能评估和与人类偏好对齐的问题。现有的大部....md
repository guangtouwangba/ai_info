...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令数据集，验证大规模指令微调效果。实验发现：传统基于执行的评估与人类偏好存在显著差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在明显差距。研究强调了代码生成模型除功能性外，还需重视人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T17:05:31Z
- **目录日期**: 2025-04-27
