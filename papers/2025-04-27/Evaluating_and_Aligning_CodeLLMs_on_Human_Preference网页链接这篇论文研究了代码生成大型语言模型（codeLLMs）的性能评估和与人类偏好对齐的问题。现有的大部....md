...
这篇论文研究了代码生成大模型（codeLLMs）与人类偏好的对齐问题，指出当前基准测试过于关注代码正确性而忽略用户体验。为此，作者构建了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和20B标记的合成指令数据集SynCode-Instruct。实验发现：1）执行基准与人类偏好存在差异；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调了代码LLMs与人类偏好对齐的重要性，为改进模型提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T20:06:15Z
- **目录日期**: 2025-04-27
