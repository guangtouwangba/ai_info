...
陈丹琦在ICLR 2025演讲中提出学术界训练语言模型的三大方向：1）开发高效小模型，如通过剪枝LLaMA-7B仅用3%计算成本实现更好性能；2）优化训练数据质量，其QuRater模型筛选300亿token数据训练的13亿参数模型验证质量与多样性平衡的重要性；3）发展后训练方法，如ProLong-8B模型通过微调技术以5%数据量实现512K长上下文处理能力。她证明仅需数百GPU小时即可开展前沿研究，呼吁学术界更积极参与语言模型研发，推动产学研新合作模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T15:03:18Z
- **目录日期**: 2025-04-27
