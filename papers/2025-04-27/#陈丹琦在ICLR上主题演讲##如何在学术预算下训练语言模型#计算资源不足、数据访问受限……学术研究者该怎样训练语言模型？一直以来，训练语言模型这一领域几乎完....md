...
陈丹琦在ICLR 2025主题演讲中提出学术界训练语言模型的三大方向：1）开发高效小模型，如通过剪枝LLaMA-7B仅用3%计算成本实现更好性能；2）优化训练数据，其QuRater系统筛选高质量数据训练的13亿参数模型验证了质量与多样性的平衡关键；3）后训练优化，如ProLong-8B模型以5%数据量实现512K长上下文处理能力。研究表明仅需数百GPU小时即可开展有效研究，呼吁学术界通过资源优化策略突破算力限制，推动产学研合作新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T10:03:07Z
- **目录日期**: 2025-04-27
