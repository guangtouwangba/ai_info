...
陈丹琦在ICLR 2025演讲中提出学术界训练语言模型的三大方向：1) 开发小而精的模型，如通过剪枝LLaMA-7B仅用3%计算成本实现更好性能；2) 优化训练数据，如QuRater模型筛选高质量数据训练13亿参数模型；3) 发展后训练方法，如ProLong-8B模型用5%数据量实现512K长上下文处理。她证明仅需数百GPU小时即可开展有效研究，呼吁学术界更积极参与语言模型研发，推动产学研合作新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T12:03:24Z
- **目录日期**: 2025-04-27
