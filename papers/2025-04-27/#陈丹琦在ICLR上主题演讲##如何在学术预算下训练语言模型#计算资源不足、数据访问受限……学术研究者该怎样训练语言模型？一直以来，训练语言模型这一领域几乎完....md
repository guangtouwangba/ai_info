...
陈丹琦在ICLR 2025演讲中指出，学术界可通过三个方向突破语言模型训练的资源限制：(1)开发高效小模型——其团队通过剪枝LLaMA-7B仅用3%计算成本实现更优性能；(2)优化数据质量——QuRater评分模型筛选300亿token训练出高性能13亿参数模型；(3)后训练创新——ProLong-8B以5%数据量在长上下文任务中超越同类。研究表明，数百GPU小时即可开展高质量研究，为学术机构参与大模型研发提供了可行路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T20:03:33Z
- **目录日期**: 2025-04-27
