...
陈丹琦在ICLR 2025演讲中指出，学术界可通过三个方向参与语言模型研究：1）开发高效小模型，如剪枝优化LLaMA-7B仅需3%计算成本；2）优化数据质量，其QuRater模型筛选300亿token数据训练出高性能13亿参数模型；3）发展后训练方法，如ProLong-8B模型用5%数据量实现512K长上下文处理。她证明仅需数百GPU小时即可开展研究，呼吁学术界突破资源限制，推动产学研合作新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T21:03:10Z
- **目录日期**: 2025-04-27
