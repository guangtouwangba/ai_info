...
DeepSeek R2 最新爆料显示，其训练和推理成本比GPT-4o下降97.3%，输入每百万tokens仅0.07美元，输出0.27美元。该模型参数量达1.2万亿（活跃参数78亿），采用混合专家架构（MoE），训练数据量5.2PB。值得注意的是，R2完全基于华为Ascend 910B集群训练，未使用英伟达芯片，FP16精度下算力达512 PetaFLOPS，利用率82%，性能约为A100集群的91%。在C-Eval 2.0和COCO测试中分别取得89.7%和92.4%的准确率，8bit量化压缩83%后精度损失不到2%。目前信息尚未官方证实。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T06:04:07Z
- **目录日期**: 2025-04-27
