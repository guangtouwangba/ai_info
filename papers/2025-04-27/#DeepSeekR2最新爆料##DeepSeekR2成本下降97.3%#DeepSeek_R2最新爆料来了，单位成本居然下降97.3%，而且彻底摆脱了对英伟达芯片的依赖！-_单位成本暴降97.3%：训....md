...
DeepSeek R2 最新爆料显示其单位成本下降97.3%，训练和推理成本远低于GPT-4o（输入每百万tokens 0.07美元，输出0.27美元）。该模型参数量达1.2万亿（活跃参数78亿），采用混合专家（MoE）架构，训练数据5.2PB。值得注意的是，R2完全基于华为Ascend 910B集群训练，未使用英伟达芯片，算力达512 PetaFLOPS，利用率82%，性能约为A100集群的91%。在C-Eval 2.0评测中得分89.7%，COCO图像测试准确率92.4%。8bit量化压缩模型体积83%，精度损失<2%，适合端侧部署。目前信息尚未官方证实。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T07:03:36Z
- **目录日期**: 2025-04-27
