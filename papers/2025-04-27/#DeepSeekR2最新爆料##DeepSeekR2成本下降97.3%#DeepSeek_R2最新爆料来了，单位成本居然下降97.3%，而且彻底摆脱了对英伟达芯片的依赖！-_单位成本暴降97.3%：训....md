...
DeepSeek R2 最新爆料显示其单位成本大幅下降97.3%，训练和推理成本远低于GPT-4o（输入每百万tokens 0.07美元，输出0.27美元）。该模型参数量达1.2万亿（78亿活跃参数），采用混合专家（MoE）架构，训练数据量5.2PB。值得注意的是，R2完全基于华为Ascend 910B集群训练，摆脱对英伟达芯片依赖，FP16精度下算力达512 PetaFLOPS，利用率82%，性能达A100集群的91%。在C-Eval 2.0和COCO测试中表现优异（89.7%和92.4%），8bit量化压缩83%体积且精度损失<2%。若属实，可能引发行业震动。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T04:03:55Z
- **目录日期**: 2025-04-27
