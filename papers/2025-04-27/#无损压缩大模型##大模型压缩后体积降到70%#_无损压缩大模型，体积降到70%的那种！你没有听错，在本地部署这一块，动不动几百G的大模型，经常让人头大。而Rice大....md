...
Rice大学和xMAD.ai团队开发的DFloat11框架实现了大模型无损压缩，可将模型体积缩减至70%（如Llama-3.1-405B从811.7GB压缩到551.22GB），且精度零损失。其核心技术包括：1）利用BFloat16权重的指数稀疏性（仅用2.6位）；2）动态长度编码优化存储；3）专为GPU推理设计的高效解压方案（查找表、并行解压等），使推理速度提升最高38.8倍。相比传统压缩方法，DFloat11支持直接高效推理，8张80GB GPU即可运行压缩后的大模型。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T19:02:49Z
- **目录日期**: 2025-04-27
