...
该研究提出SGD-SaI方法，质疑深度神经网络训练中自适应梯度方法（如Adam）的必要性。该方法改进带动量的随机梯度下降（SGDM），在初始化时根据梯度信号噪声比（g-SNR）调整不同参数组的学习率，避免训练不平衡，并将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或媲美AdamW，且对超参数更鲁棒。内存效率显著提升，例如为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，并在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T15:04:29Z
- **目录日期**: 2025-04-27
