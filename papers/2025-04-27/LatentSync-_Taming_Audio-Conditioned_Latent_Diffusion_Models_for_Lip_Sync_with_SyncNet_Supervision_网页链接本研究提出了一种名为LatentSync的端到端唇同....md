...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂音视频关系，通过时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题，同时保持同步精度。研究还优化了SyncNet训练框架，改进了模型架构、超参数和数据预处理，显著提升其准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。这些改进也可应用于其他音频驱动的人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T15:05:24Z
- **目录日期**: 2025-04-27
