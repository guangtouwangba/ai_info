...
这篇论文提出了一种在芯片设计前(pre-silicon)阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究以AI/ML应用中的时钟毛刺攻击为案例，发现两种新漏洞：1) 通过控制时钟毛刺参数导致指令跳过，阻止关键值从内存加载；2) 将合法指令转为非法指令，从而可被攻击者利用来控制流转向。该工作揭示了故障注入攻击的复杂性，强调了在物理层到系统软件层进行预防性安全分析的重要性，为保护网络基础设施安全提供了新见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T19:01:57Z
- **目录日期**: 2025-04-27
