...
本文提出了一种针对嵌入式系统的预硅级故障注入攻击综合分析方法，重点研究AI/ML应用中时钟毛刺攻击导致关键错误分类的问题。通过分析RISC-V指令集和流水线阶段的故障影响，研究发现了两个新漏洞：1）通过控制时钟毛刺参数实现指令跳过，阻止关键值从内存加载，破坏程序连续性和正确性；2）将合法指令转为非法指令，从而可被攻击者利用的控制流转向。该研究揭示了故障注入攻击的复杂性，强调了预先安全分析的重要性，为从电路层到软件层的故障传播提供了新的见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T11:02:11Z
- **目录日期**: 2025-04-27
