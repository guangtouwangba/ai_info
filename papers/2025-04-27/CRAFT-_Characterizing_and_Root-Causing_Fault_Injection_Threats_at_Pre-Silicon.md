...
这篇论文提出了一种在芯片设计前(pre-silicon)阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究通过时钟毛刺攻击对AI/ML应用关键错误分类进行测试，发现两个新漏洞：1) 导致指令跳过，阻止从内存加载关键值；2) 将合法指令转为非法指令以改变控制流。该工作揭示了故障注入攻击的复杂性，强调了从电路层到软件层的故障传播路径分析的重要性，以及进行预防性安全分析的关键价值。研究成果有助于理解多抽象层的系统漏洞根源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T17:02:05Z
- **目录日期**: 2025-04-27
