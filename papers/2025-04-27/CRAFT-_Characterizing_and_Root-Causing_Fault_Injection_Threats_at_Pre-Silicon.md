...
这篇论文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究以AI/ML应用中的时钟毛刺攻击为案例，揭示了两种新漏洞：一是导致指令跳过，阻止从内存加载关键值；二是将合法指令转为非法指令，从而可被攻击者利用控制流转向。该工作强调了故障注入攻击的复杂性，以及开展先发制人的安全分析的重要性，为从电路层到软件层的故障传播路径提供了深入见解，对保护网络基础设施安全具有关键意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T15:02:08Z
- **目录日期**: 2025-04-27
