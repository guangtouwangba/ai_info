...
该研究提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了AI/ML应用中时钟毛刺攻击导致关键错误分类的情况。通过追踪RISC-V指令集和流水线阶段的故障传播路径，研究发现了两种新漏洞：一是通过指令跳过阻止关键内存值加载，破坏程序连续性；二是将合法指令转为非法指令以劫持控制流。这项工作揭示了故障注入攻击的复杂性，强调了在物理层到系统软件各层面进行先发制人安全分析的重要性，为保护网络基础设施安全提供了新见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T16:02:09Z
- **目录日期**: 2025-04-27
