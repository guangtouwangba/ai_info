...
该研究提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。通过时钟毛刺攻击AI/ML应用导致关键错误分类，研究发现了两个新漏洞：1) 通过特定时钟参数实现指令跳过，阻止关键内存值加载；2) 将合法指令转为非法指令以劫持控制流。研究揭示了故障从电路层到应用软件的传播路径，强调了预先安全分析对保护网络基础设施的重要性。这些发现突显了故障注入攻击的复杂性及其在系统各抽象层的潜在危害。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T07:02:00Z
- **目录日期**: 2025-04-27
