...
这篇论文提出了一种在芯片设计前(pre-silicon)阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段中的故障影响。研究以AI/ML应用中的时钟毛刺攻击为例，揭示了两种新漏洞：1)导致指令跳过，阻止从内存加载关键值；2)将合法指令转为非法指令，从而可被攻击者利用控制流转移。该工作强调了故障注入攻击的复杂性，以及从电路层到应用软件层追踪故障传播的重要性，突显了先发制人的安全分析对保护网络基础设施的关键作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T04:02:42Z
- **目录日期**: 2025-04-27
