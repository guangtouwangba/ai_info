...
谷歌AI模型Gemini 2.5 Pro成功在《宝可梦蓝》游戏中收集到第八个道馆徽章，累计游戏时长近600小时。该项目由一位软件工程师发起，通过建立游戏模拟器接口让AI能获取游戏画面和状态数据，并基于网格分析进行决策。系统采用循环操作模式：截图分析→AI决策→执行操作，并通过定期摘要管理内存消耗。该实验是继Claude之后又一个AI玩游戏的突破性尝试，目前正在Twitch平台直播其挑战四天王的进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T22:02:49Z
- **目录日期**: 2025-04-27
