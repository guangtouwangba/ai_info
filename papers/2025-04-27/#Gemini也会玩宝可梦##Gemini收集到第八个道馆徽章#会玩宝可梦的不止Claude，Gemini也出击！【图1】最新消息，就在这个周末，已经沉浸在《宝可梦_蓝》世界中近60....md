...
谷歌AI模型Gemini 2.5 Pro成功在《宝可梦蓝》游戏中收集到第八个道馆徽章，累计游戏时长近600小时。该项目由软件工程师通过mGBA模拟器实现，系统架构包含：1）游戏接口实现截图捕捉和按键控制；2）决策系统通过网格化截图辅助AI分析；3）内存管理每100次动作进行摘要优化。采用实时循环机制处理游戏画面与AI决策，目前正向四天王发起挑战。该实验是继Claude之后第二个实现宝可梦通关的AI案例，可通过Twitch直播观看实况。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T12:02:50Z
- **目录日期**: 2025-04-27
