...
AgentSociety是一个基于大型语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次）。它通过真实社会环境和大规模模拟引擎，研究极化现象、信息传播、基本收入政策及外部冲击（如飓风）等社会问题。模拟结果与现实实验数据吻合，验证了其捕捉人类行为机制的能力。该系统可作为计算社会科学工具，辅助传统研究方法（如调查、访谈），帮助研究者和政策制定者分析社会问题的模式、原因及潜在机制，具有重要研究价值和应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T07:05:01Z
- **目录日期**: 2025-04-27
