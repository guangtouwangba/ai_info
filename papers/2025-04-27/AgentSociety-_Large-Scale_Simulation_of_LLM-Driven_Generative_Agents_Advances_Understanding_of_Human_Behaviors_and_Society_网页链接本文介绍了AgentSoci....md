...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。它被用于研究四个关键社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。模拟结果与现实世界数据吻合，验证了该系统捕捉人类行为机制的能力，展示了其作为社会科学研究和政策制定工具的潜力。该平台结合了LLM代理、真实社会环境和强大模拟引擎，能有效补充传统研究方法（如调查和访谈）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T13:10:26Z
- **目录日期**: 2025-04-27
