...
PaperCoder是一款多智能体系统，可将机器学习论文自动转换为可执行代码库。它通过三阶段流程实现：1）规划阶段制定开发路线图和架构；2）分析阶段确定模块功能与依赖关系；3）编码阶段生成完整代码。测试显示其代码改动率仅0.48%，77%原论文作者更偏好其输出，85%用户认为能加速研究复现。在Paper2Code和PaperBench基准测试中，表现优于ChatDev和MetaGPT。目前专注ML领域，未来计划扩展至物理、生物等学科。被评价为当前最接近"一键复现"的工具。项目已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T17:03:58Z
- **目录日期**: 2025-04-27
