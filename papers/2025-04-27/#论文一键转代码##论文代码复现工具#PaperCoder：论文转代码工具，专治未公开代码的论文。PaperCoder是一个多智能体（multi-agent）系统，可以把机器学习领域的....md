...
PaperCoder是一款多智能体系统，可将机器学习论文自动转换为可执行代码库。它通过三阶段流程实现：1) Planning阶段制定开发路线图并生成系统架构；2) Analyzing阶段深入分析各模块功能与依赖关系；3) Coding阶段按规划生成代码。在Paper2Code和PaperBench测试中，其代码改动率仅0.48%，77%原作者更偏好其生成结果。相比ChatDev和MetaGPT表现更优，85%用户认为能有效加速研究复现。目前专注于ML领域，未来计划扩展至物理、生物等学科。被评价为最接近"一键复现"的工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T13:09:25Z
- **目录日期**: 2025-04-27
