...
PaperCoder是一款多智能体系统，可将机器学习论文自动转换为可执行的完整代码库。其工作流程分为三个阶段：规划（生成架构图和配置文件）、分析（确定模块功能和依赖关系）和编码（按依赖顺序生成代码）。在Paper2Code和PaperBench基准测试中，PaperCoder表现优于ChatDev和MetaGPT，代码改动率仅0.48%，77%的原作者更偏好其生成结果。该工具显著加速研究复现，未来计划扩展到物理、生物等领域。目前被公认为最接近"一键复现"的解决方案，GitHub和Hugging Face已开放访问。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T10:03:58Z
- **目录日期**: 2025-04-27
