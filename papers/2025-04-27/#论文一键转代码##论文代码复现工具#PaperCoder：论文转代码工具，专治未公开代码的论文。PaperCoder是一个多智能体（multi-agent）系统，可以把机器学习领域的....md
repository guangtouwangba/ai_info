...
PaperCoder是一个多智能体系统，能将机器学习论文自动转换为可执行代码库。它通过三阶段流程实现：1）规划阶段生成系统架构和配置文件；2）分析阶段深入理解模块功能和依赖关系；3）编码阶段按规划生成可运行代码。测试显示其代码改动率仅0.48%，77%的原作者更偏好其输出，85%用户认为显著加速研究复现。在Paper2Code和PaperBench基准测试中，其表现优于ChatDev和MetaGPT。目前专注于机器学习领域，未来计划扩展至物理、生物等学科。被评价为当前最接近"一键复现"的工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-27T16:04:09Z
- **目录日期**: 2025-04-27
