...
香港大学、英伟达和图宾根大学联合团队提出Centaur方法，通过测试时在线学习（test-time training）动态优化自动驾驶决策。传统自动驾驶依赖预设规则，在变道、加塞等场景中表现过于保守，易引发急刹/避让等安全隐患。Centaur利用聚类熵和不确定性评估，实现边驾驶边学习的自适应能力，减少对固定规则的依赖。该方法显著提升了自动驾驶在不确定场景下的适应性和安全性，使AI驾驶更接近人类"安全驾驶思维"的灵活决策模式。技术首次实现了自动驾驶系统的实时策略优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T22:03:34Z
- **目录日期**: 2025-03-30
