...
香港大学、英伟达和图宾根大学联合团队提出Centaur方法，通过实时数据驱动优化自动驾驶决策。该方法突破传统依赖预设规则的局限，使AI能在变道、加塞等不确定性场景中动态调整驾驶策略，避免过度保守行为（如无故急刹）。这种"边开边学"的测试时计算技术显著提升了自动驾驶的适应性和安全性，解决了现有系统因过度谨慎反而引发安全隐患的问题。研究标志着自动驾驶首次实现实时策略优化，为复杂路况处理提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T20:03:22Z
- **目录日期**: 2025-03-30
