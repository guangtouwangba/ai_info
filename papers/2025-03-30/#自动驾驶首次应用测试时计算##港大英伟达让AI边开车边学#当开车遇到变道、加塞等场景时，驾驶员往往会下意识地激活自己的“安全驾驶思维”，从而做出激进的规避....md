...
香港大学、英伟达与图宾根大学联合团队提出Centaur方法，通过测试时计算技术让自动驾驶系统在行驶过程中动态学习。该方法利用数据驱动和不确定性评估，使AI能像人类驾驶员一样实时调整驾驶策略，减少因预设规则导致的过度保守行为（如无故急刹）。这种"边开边学"的机制显著提升了自动驾驶在变道、加塞等复杂场景下的适应性和安全性，突破了传统固定规则系统的局限性。相关技术首次实现自动驾驶在真实测试中的实时策略优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T23:03:45Z
- **目录日期**: 2025-03-30
