...
该论文提出了OVO-Bench（在线视频基准测试），用于评估视频大语言模型（LLMs）的时间感知能力。与依赖完整视频的离线模型不同，在线模型需动态处理视频流并根据提问时间戳调整响应。OVO-Bench聚焦三种场景：回溯过去事件、实时理解当前事件以及延迟响应以获取未来信息。该基准包含12项任务、644个独特视频及约2,800条精细标注。通过自动化生成与人工标注结合，构建了系统化的评估流程。测试显示，现有模型在在线视频理解上表现不佳，与人类水平差距显著，凸显了该领域的研究需求。代码与基准已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T16:02:16Z
- **目录日期**: 2025-03-30
