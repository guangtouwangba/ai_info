...
该论文提出了OVO-Bench基准测试，用于评估视频大语言模型（LLMs）的时间感知能力。与依赖完整视频的离线模型不同，在线模型需动态处理视频流并根据提问时间戳调整响应。OVO-Bench包含12项任务、644个视频和约2800条带精确时间戳的人工标注，测试模型在三种场景下的表现：回溯过去事件、实时理解当前事件以及延迟响应以获取未来信息。评估显示，现有模型在在线视频理解方面仍显著落后于人类。该基准旨在推动视频LLMs的发展，促进在线视频推理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T19:02:04Z
- **目录日期**: 2025-03-30
