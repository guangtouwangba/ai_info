...
OVO-Bench是一个专注于评估视频大语言模型（LLMs）时间感知能力的新基准，强调在线视频理解中时间戳的关键作用。与依赖完整视频的离线模型不同，在线模型需动态处理视频流并根据提问时间戳调整响应。该基准通过三种场景（回溯过去事件、实时理解当前事件、延迟响应以获取未来信息）评估模型能力，包含12项任务、644个视频及2800条精细标注。测试发现，现有模型在在线视频理解上表现不佳，与人类存在显著差距。OVO-Bench旨在推动视频LLMs发展，促进在线视频推理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T09:02:22Z
- **目录日期**: 2025-03-30
