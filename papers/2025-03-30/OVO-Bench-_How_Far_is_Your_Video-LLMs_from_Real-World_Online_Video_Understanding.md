...
本文介绍了OVO-Bench，一个专注于评估在线视频大语言模型（Video-LLMs）时间感知能力的新基准。传统离线模型依赖完整视频进行静态分析，而在线模型需动态处理视频流并根据提问时间戳调整响应。OVO-Bench通过12项任务（含644个视频和2,800条带精确时间戳的标注），测试模型在三种场景下的表现：回溯过去事件、实时理解当前画面、延迟响应以获取未来信息。评估显示，现有模型在在线视频理解上与人类存在显著差距。该基准旨在推动视频LLM的发展，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T17:02:06Z
- **目录日期**: 2025-03-30
