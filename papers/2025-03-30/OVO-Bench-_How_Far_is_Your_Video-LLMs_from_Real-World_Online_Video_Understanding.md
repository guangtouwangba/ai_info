...
OVO-Bench是一个新型视频基准测试，专注于评估在线视频大语言模型（LLMs）的时间感知能力。与依赖完整视频的离线模型不同，在线模型需动态处理视频流并根据提问时间戳调整响应。该基准测试通过三种场景评估模型能力：（1）回溯过去事件，（2）实时理解当前事件，（3）延迟响应以获取未来信息。包含12项任务、644个视频和约2800条人工标注的时间戳数据。测试发现，现有模型在在线视频理解上表现不佳，与人类存在显著差距。该基准旨在推动视频LLMs的发展，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T06:02:16Z
- **目录日期**: 2025-03-30
