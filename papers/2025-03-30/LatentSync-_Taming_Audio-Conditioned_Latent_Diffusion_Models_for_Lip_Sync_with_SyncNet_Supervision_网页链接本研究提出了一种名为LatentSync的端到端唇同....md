...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时解决了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理，显著提升准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新方案。研究成果对基于SyncNet的唇同步方法具有普适参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T15:05:16Z
- **目录日期**: 2025-03-30
