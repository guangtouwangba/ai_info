...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时解决了SyncNet收敛问题，优化了模型架构、训练参数等关键因素。该方法在HDTF和VoxCeleb2数据集上表现优异，超越了现有唇同步技术。研究成果不仅提升了唇同步效果，其SyncNet优化经验还可推广至其他人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T10:05:27Z
- **目录日期**: 2025-03-30
