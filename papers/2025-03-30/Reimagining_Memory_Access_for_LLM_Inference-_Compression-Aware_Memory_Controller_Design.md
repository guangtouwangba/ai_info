...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的解决方案。该方法采用无损块压缩技术(LZ4/ZSTD)压缩模型权重和KV缓存，在不影响推理质量的情况下，将模型权重内存占用减少25.2%，KV缓存减少46.9%。同时通过上下文相关的动态量化技术，使内存带宽和能耗按比例调整。硬件原型(7nm工艺)在4GHz频率下实现了8TB/s的吞吐量，面积开销仅3.8mm²。实验证明这种LLM感知的内存控制能有效解决大规模推理中的内存瓶颈问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T18:01:46Z
- **目录日期**: 2025-03-30
