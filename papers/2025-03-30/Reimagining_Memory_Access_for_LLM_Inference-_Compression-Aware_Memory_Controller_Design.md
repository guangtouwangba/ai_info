...
该论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的解决方案。主要采用两种技术：(1)利用无损块压缩(LZ4/ZSTD)压缩模型权重和KV缓存，在不影响推理质量的前提下，将内存占用分别减少25.2%和46.9%；(2)通过上下文相关的动态量化，使内存带宽和能耗按比例缩放。硬件原型在7nm工艺下实现4GHz频率和8TB/s吞吐量，面积开销仅3.8mm²。实验证明，这种LLM感知的内存控制能有效缓解内存瓶颈，支持高效的大规模推理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T17:01:49Z
- **目录日期**: 2025-03-30
