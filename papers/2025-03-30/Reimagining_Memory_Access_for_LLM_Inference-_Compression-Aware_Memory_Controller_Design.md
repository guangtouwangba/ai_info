...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型（LLM）推理效率的方法。该方法利用无损块压缩（如LZ4和ZSTD）压缩模型权重和键值缓存（KV cache），在不影响推理质量的情况下，显著减少了内存容量和带宽需求。实验结果显示，模型权重和KV缓存的内存占用分别降低了25.2%和46.9%。硬件原型在4GHz和32通道（7nm工艺）下实现了8TB/s的吞吐量，面积开销仅为3.8mm²，验证了该方案在大规模高效推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T22:01:50Z
- **目录日期**: 2025-03-30
