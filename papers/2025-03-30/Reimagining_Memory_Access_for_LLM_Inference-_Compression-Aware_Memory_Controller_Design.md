...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的解决方案。该方法采用无损块压缩技术(如LZ4和ZSTD)压缩模型权重和KV缓存，在不影响推理质量的前提下，显著降低了内存容量和带宽需求。实验结果显示，该方法使模型权重和KV缓存的内存占用分别减少了25.2%和46.9%。硬件原型在7nm工艺下实现了8TB/s的吞吐量，面积开销仅3.8mm²，验证了LLM感知内存控制作为高效大规模推理关键技术的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T01:29:11Z
- **目录日期**: 2025-03-30
