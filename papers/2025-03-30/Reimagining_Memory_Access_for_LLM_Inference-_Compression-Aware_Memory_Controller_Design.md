...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方案采用无损块压缩(如LZ4和ZSTD)来压缩模型权重和KV缓存，在不影响推理质量的情况下，显著减少了25.2%的权重内存占用和46.9%的KV缓存占用。同时，通过LLM感知的内存配置实现动态量化，使内存带宽和能耗按比例调整。实验显示，在7nm工艺下，4GHz、32通道的原型机可实现8TB/s吞吐量，面积开销仅3.8mm²，验证了该方案在大规模推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T21:01:47Z
- **目录日期**: 2025-03-30
