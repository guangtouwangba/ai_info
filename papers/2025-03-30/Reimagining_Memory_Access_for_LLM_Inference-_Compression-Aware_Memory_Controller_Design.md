...
该研究提出了一种优化大语言模型(LLM)推理效率的新方法，通过改进AI加速器的片上内存控制器来缓解内存瓶颈。该方法采用无损块压缩(LZ4/ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的情况下，显著减少了内存占用(权重减少25.2%，KV缓存减少46.9%)。同时支持基于上下文的动态量化，使内存带宽和能耗按比例调整。硬件原型在7nm工艺下实现了8TB/s吞吐量，面积开销仅3.8mm²。实验证明这种LLM感知的内存控制方案能有效提升大规模推理效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T16:01:57Z
- **目录日期**: 2025-03-30
