...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩(LZ4/ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的情况下，将模型权重和KV缓存的内存占用分别减少了25.2%和46.9%。同时通过上下文相关的动态量化，使内存带宽和能耗按比例调整。硬件原型(7nm工艺)在4GHz频率下实现了8TB/s的吞吐量，面积开销仅3.8mm²。实验证明这种LLM感知的内存控制方案能有效缓解内存瓶颈，提升大规模推理效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T12:02:06Z
- **目录日期**: 2025-03-30
