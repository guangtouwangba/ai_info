...
大语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而现有单智能体解码方法难以适应多样化任务。本文提出一种多智能体混合解码策略（Collab），通过动态选择预训练对齐模型的策略，在推理时实现高效协作。该方法基于长期效用指标，在解码时逐词选择最优模型，理论分析证明其目标奖励最优性。实验表明，Collab在开源对齐模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T18:01:30Z
- **目录日期**: 2025-03-30
