...
该研究提出了一种基于多智能体协作的解码方法（Collab），用于提升大语言模型（LLM）在推理阶段的对齐性能。传统强化学习人类反馈（RLHF）方法计算成本高，而单智能体解码策略难以适应多样化任务。Collab通过动态选择现有对齐LLM策略池中的最优模型进行token级决策，利用长期效用指标实现多模型高效协作。理论分析证明其能针对目标奖励函数实现最优性能。实验表明，Collab在开源对齐模型上的表现优于单智能体基线，最高提升1.56倍平均奖励和71.89%的GPT-4胜率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T07:01:35Z
- **目录日期**: 2025-03-30
