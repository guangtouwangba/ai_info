...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而单智能体解码方法难以适应多样化任务。本文提出一种基于多智能体协作的解码策略（Collab），通过动态选择最优模型进行token级决策，实现推理时对齐。该方法利用现成对齐模型作为智能体池，根据长期效用指标选择最佳模型，确保高效协作。理论分析证明其针对目标任务的优化性能。实验表明，Collab在开源模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4胜率提高71.89%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T11:01:37Z
- **目录日期**: 2025-03-30
