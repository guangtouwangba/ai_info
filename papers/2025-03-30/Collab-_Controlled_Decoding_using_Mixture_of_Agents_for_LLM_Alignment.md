...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而单智能体解码方法难以适应多样化任务。为此，研究者提出了一种基于多智能体协作的解码策略（Collab），动态选择最优LLM生成每个token，通过长时效用指标实现推理时对齐。理论分析证明其针对目标奖励的最优性，实验显示Collab在开源对齐模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T01:28:55Z
- **目录日期**: 2025-03-30
