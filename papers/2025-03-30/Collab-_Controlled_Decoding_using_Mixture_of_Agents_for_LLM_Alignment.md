...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）虽有效但计算成本高，而单智能体解码方法难以适应多样化任务。为此，研究者提出了一种基于多智能体协作的解码策略（Collab），在推理时动态选择最优模型进行token级决策。该方法通过长期效用指标从模型池中选择最适合当前任务的LLM，实现高效协作与对齐。理论分析证明其最优性，实验显示Collab在平均奖励上比当前最优方法提升1.56倍，GPT-4评估胜率提升71.89%，显著优于单智能体基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T22:01:29Z
- **目录日期**: 2025-03-30
