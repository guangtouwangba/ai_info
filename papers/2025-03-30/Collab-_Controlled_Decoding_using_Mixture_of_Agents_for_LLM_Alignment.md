...
该研究提出了一种多智能体协作的解码方法（Collab），用于提升大语言模型（LLM）在推理时的对齐性能。传统强化学习对齐（RLHF）计算成本高，而单智能体解码难以适应多样化任务。Collab通过动态选择多个预对齐LLM策略，基于长期效用指标在token级别切换最优模型，实现高效协作。理论分析证明其能针对目标奖励函数达到最优性能。实验表明，Collab在开源对齐模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T05:01:29Z
- **目录日期**: 2025-03-30
