...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而可控解码可在推理时对齐但单代理策略难以适应多样任务。本文提出多代理协作解码方法Collab，动态选择最优LLM生成每个token，通过长期效用指标实现模型间高效协作。理论分析证明其在目标奖励下的最优性，实验显示Collab超越当前最佳解码策略：平均奖励提升1.56倍，GPT-4胜率提高71.89%。该方法无需重训练，显著提升多任务对齐性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T04:01:25Z
- **目录日期**: 2025-03-30
