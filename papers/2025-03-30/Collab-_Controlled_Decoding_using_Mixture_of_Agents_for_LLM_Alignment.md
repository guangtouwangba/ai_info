...
大型语言模型（LLM）的对齐对安全可信的应用至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而单智能体解码方法难以适应多样化任务。本文提出一种基于多智能体协作的解码策略Collab，通过动态选择最优模型进行令牌级决策，实现推理时对齐。理论分析证明其针对目标任务的优化性能，实验显示Collab在开源对齐模型上表现优异：平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，显著优于当前最佳单智能体基线。该方法无需重新训练模型，实现了高效的多模型协作解码。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T06:01:38Z
- **目录日期**: 2025-03-30
