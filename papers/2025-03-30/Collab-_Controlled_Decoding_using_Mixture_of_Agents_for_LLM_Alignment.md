...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而单智能体解码方法难以适应多样化任务。本文提出一种混合智能体协作解码策略（Collab），通过动态选择多个预对齐LLM策略，在推理时基于长期效用指标进行词级模型切换，实现高效对齐。理论分析证明其针对目标奖励函数的最优性，实验表明Collab在开源模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T21:01:29Z
- **目录日期**: 2025-03-30
