...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习（RLHF）需调整数十亿参数，计算成本高。本文提出基于多智能体协作的解码方法Collab，通过动态选择最优模型进行token级策略切换，实现推理时对齐。理论分析证明其在目标奖励下性能最优。实验表明，Collab在开源对齐模型上表现优异，平均奖励提升1.56倍，GPT-4胜率提升71.89%，显著优于单智能体基线。该方法利用现成模型策略，在多样化任务中实现高效协作对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T08:01:28Z
- **目录日期**: 2025-03-30
