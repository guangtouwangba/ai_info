...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）需更新数十亿参数，计算成本高。本文提出基于多智能体协作的解码方法Collab，通过动态选择最优预对齐LLM策略进行推理时对齐。该方法在token级别评估长期效用指标，灵活切换策略，实现高效协作。理论分析证明其在目标奖励下的最优性。实验表明，Collab在开源对齐模型上超越单智能体基线，平均奖励提升1.56倍，GPT-4胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T16:01:38Z
- **目录日期**: 2025-03-30
