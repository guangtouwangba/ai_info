...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式，并生成优化的Python代码。相比传统方法，它显著提升了F1分数，能高效处理复杂和模糊查询。消融实验证实关系代数模块至关重要。工业评估显示，GateLens减少80%以上的分析时间，同时保持高准确性和可靠性。该工具无需依赖少量示例即实现高性能，在不同公司角色的多样化查询中展现出强泛化能力。其部署经验为AI集成到关键工作流（如发布验证）提供了实用指导，提升了汽车系统软件的扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T18:01:21Z
- **目录日期**: 2025-03-30
