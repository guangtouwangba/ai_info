...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数（RA）表达式，并生成优化的Python代码。该工具在基准数据集上表现优于基线系统，F1分数更高，并能更稳健地处理复杂和模糊查询。工业评估显示，GateLens将分析时间减少了80%以上，同时保持高准确性和可靠性。部署结果表明，GateLens无需依赖少量示例即可实现高性能，并能泛化处理不同公司角色的多样化查询类型，显著提升了汽车系统软件发布的效率和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T01:28:47Z
- **目录日期**: 2025-03-30
