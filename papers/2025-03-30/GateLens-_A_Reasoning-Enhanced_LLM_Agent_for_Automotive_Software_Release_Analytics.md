...
本文介绍了一种基于大语言模型(LLM)的工具GateLens，用于分析汽车领域中的表格数据。该工具能将自然语言查询转换为关系代数表达式并生成优化的Python代码，在基准测试中表现优于基线系统，F1分数更高且能更稳健地处理复杂模糊查询。工业评估表明，GateLens能减少80%以上的分析时间，同时保持高准确性和可靠性。该工具不依赖少量示例即实现高性能，展现了跨不同公司角色的强泛化能力。实际部署证明，GateLens能自动化测试结果分析，帮助做出更快、更明智的软件发布决策，提升汽车系统的可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T04:01:17Z
- **目录日期**: 2025-03-30
