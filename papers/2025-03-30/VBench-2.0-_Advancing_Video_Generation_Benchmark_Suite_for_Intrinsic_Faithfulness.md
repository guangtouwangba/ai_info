...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时间连贯的视频。现有评测基准（如VBench）主要评估表层忠实度（如帧美学、时序一致性和基础提示遵循），但忽略了内在真实性（如物理法则、常识推理等）。为填补这一空白，研究者提出了VBench-2.0，通过自动评估五个关键维度（人类逼真度、可控性、创造力、物理性和常识性）来推动视频生成模型向"世界模型"迈进。该框架结合通用视觉语言模型和专用异常检测方法，并通过人工标注验证，旨在为下一代视频生成模型建立内在真实性的新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T10:01:07Z
- **目录日期**: 2025-03-30
