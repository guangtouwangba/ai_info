...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表面真实性（如帧美学、时序一致性和基础提示匹配），但忽视了内在真实性（如物理规律、常识推理和结构完整性）。为此，研究者推出VBench-2.0基准，通过自动评估五大维度（人类拟真度、可控性、创造性、物理合理性和常识性）及其细分能力，结合通用视觉语言模型和专用异常检测方法，推动视频生成模型超越视觉可信度，实现内在真实性，为AI辅助影视和虚拟世界建模设定新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T13:05:17Z
- **目录日期**: 2025-03-30
