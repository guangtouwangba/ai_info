...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表面真实性（如帧美学、时序一致性和基础提示匹配），但忽略了更深层的现实世界原则（如物理定律、常识推理等）。为此，研究者提出VBench-2.0，通过自动化评估模型的内在真实性，涵盖人类逼真度、可控性、创造力、物理合理性和常识性五大维度，结合通用视觉语言模型和专用异常检测方法，并辅以人工标注验证。该基准旨在为下一代视频生成模型设定内在真实性的新标准，推动AI电影制作和虚拟世界建模的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T21:01:13Z
- **目录日期**: 2025-03-30
