...
视频生成技术已从早期不真实输出发展到能生成视觉逼真、时序连贯的视频。现有评测基准（如VBench）主要评估表面真实性，如帧美学、时序一致性和基础提示匹配，但忽略了是否符合现实世界规律。为实现真正的"世界模型"，需要关注内在真实性，即遵守物理定律、常识推理、解剖正确性和结构完整性。为此，研究团队推出VBench-2.0评测框架，从人类逼真度、可控性、创造性、物理性和常识性五个维度，结合通用视觉语言模型和专用异常检测方法，建立新一代视频生成模型的内在真实性评估标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T23:01:12Z
- **目录日期**: 2025-03-30
