...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表面真实性（如帧美学、时序一致性和基本提示遵循），但忽略了内在真实性（如物理规律、常识推理和结构合理性）。为此，研究团队推出VBench-2.0，通过自动化评估模型在五个关键维度（人类逼真度、可控性、创造性、物理性和常识性）的表现，结合先进视觉语言模型和异常检测方法，推动视频生成向内在真实性迈进，为AI辅助影视制作和虚拟世界建模设立新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T09:01:16Z
- **目录日期**: 2025-03-30
