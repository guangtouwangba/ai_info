...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时序连贯的视频。现有评测基准（如VBench）主要评估表面真实性（如帧美学、时序一致性和基础提示遵循），但忽略了内在真实性（如物理法则、常识推理、解剖正确性等）。为填补这一差距，研究者提出VBench-2.0，通过自动评估五个关键维度（人类逼真度、可控性、创造性、物理合理性和常识性）来推动视频生成模型的内在真实性发展。该框架结合通用视觉语言模型、专家方法及人工标注，旨在为下一代视频生成模型设立新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T16:01:17Z
- **目录日期**: 2025-03-30
