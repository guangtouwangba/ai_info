...
视频生成技术已取得显著进展，从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评测基准（如VBench）主要评估表面忠实度（如帧美学、时序一致性和基础提示匹配），但忽略了内在真实性（如物理规律、常识推理和结构合理性）。为此，研究团队推出VBench-2.0，通过自动化评估五个核心维度（人类拟真度、可控性、创造性、物理合理性和常识性），结合先进视觉语言模型和异常检测方法，建立新一代评测标准。该基准旨在推动视频生成模型从视觉可信迈向本质真实，为AI影视制作和虚拟世界建模奠定基础。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T08:01:08Z
- **目录日期**: 2025-03-30
