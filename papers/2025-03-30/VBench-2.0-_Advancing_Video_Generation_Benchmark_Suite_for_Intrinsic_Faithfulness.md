...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有基准（如VBench）主要评估表观真实性（如帧美学、时序一致性），但忽略了内在真实性（如物理规律、常识推理）。为此，研究团队推出VBench-2.0，通过五大维度（人体真实性、可控性、创造性、物理合理性、常识性）自动评估视频生成模型的内在真实性。该框架结合通用视觉语言模型和专用异常检测方法，并通过人工标注验证。VBench-2.0旨在为下一代视频生成模型设立内在真实性的新标准，推动AI电影制作和虚拟世界建模的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T18:01:12Z
- **目录日期**: 2025-03-30
