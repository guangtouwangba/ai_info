...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表面真实性（如单帧美学、时序一致性和基础提示匹配），但忽略了更深层的现实世界原则。为实现真正的"世界模型"，视频生成需满足物理法则、常识推理、解剖正确性和结构完整性等内在真实性要求。为此，研究者提出了VBench-2.0基准，从人类逼真度、可控性、创造性、物理性和常识性五个维度评估视频生成模型，结合通用视觉语言模型和专用异常检测方法，旨在为下一代视频生成模型设定内在真实性的新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T02:29:46Z
- **目录日期**: 2025-03-30
