...
近期直接偏好对齐算法（DPA，如DPO）在使大语言模型符合人类偏好方面展现出潜力，但缺乏统一的理论框架来分析其语义差异或设计新损失函数。本研究提出一种基于离散推理问题的形式化方法，将现有DPA损失转化为表征其语义的符号程序，并建立偏好损失的统一形式化体系，涵盖单模型和参考模型方法。该框架不仅揭示了DPA损失函数间的关联性，还支持从第一性原理系统探索损失函数空间并推导新变体。研究成果为人类-AI对齐研究提供了理论指导和实践工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T05:02:15Z
- **目录日期**: 2025-03-30
