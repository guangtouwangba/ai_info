...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个代理的互动行为。研究者用它分析了四大社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。结果显示，其模拟结果与现实世界数据高度吻合，验证了该工具在揭示人类行为机制方面的有效性。这一突破性平台为社会科学研究和政策制定提供了新方法，能有效补充传统调查和实验手段。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T07:05:03Z
- **目录日期**: 2025-03-30
