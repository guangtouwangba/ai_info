...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个AI代理的社会互动（500万次）。研究者用它分析四大社会问题：社会极化、信息传播、基本收入政策影响和自然灾害应对。模拟结果与现实数据吻合，验证了该工具在揭示人类行为机制和社会问题成因方面的有效性。该系统可作为社会科学研究和政策制定的创新平台，补充传统研究方法（如调查和访谈），帮助深入理解复杂社会现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T11:05:20Z
- **目录日期**: 2025-03-30
