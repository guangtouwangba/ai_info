...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个代理及其500万次互动。研究者用它研究四个关键社会问题：社会极化、信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界数据吻合，验证了其捕捉人类行为和社会机制的能力。该系统可作为社会科学研究和政策制定的有力工具，补充传统方法（如调查和访谈）。这项技术展示了LLM在模拟复杂社会现象方面的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T02:33:26Z
- **目录日期**: 2025-03-30
