...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它分析四大社会问题：社会极化、刺激性信息传播、基本收入政策影响和外部冲击（如飓风）。模拟结果与现实数据高度吻合，验证了该系统捕捉人类行为机制的能力。该工具可作为社会科学研究和政策制定的创新平台，补充传统调查和实验方法，帮助深入理解社会问题的成因与模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T01:32:01Z
- **目录日期**: 2025-03-30
