...
这篇论文提出了一种硬件无关的控制循环系统，用于优化生成式AI工作负载的推理部署。该系统能根据实时成本和容量信号，在异构加速器（如GPU和专用芯片）间动态分配请求，通过成本优化和容量优化模式的灵活切换，在保证低延迟和高吞吐的同时最大化资源利用率。基于Stable Diffusion模型的测试表明，该框架能稳定满足延迟要求，在资源不足时自动转移流量，并优先使用低成本加速器。这种基于反馈的全栈部署策略，可帮助企业在有限加速器资源下高效扩展生成式AI应用，同时保持系统韧性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T11:01:46Z
- **目录日期**: 2025-03-30
