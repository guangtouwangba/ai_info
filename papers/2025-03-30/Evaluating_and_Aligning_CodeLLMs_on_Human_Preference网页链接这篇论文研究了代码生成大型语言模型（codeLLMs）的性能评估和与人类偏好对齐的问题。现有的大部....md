...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本(覆盖40类/44种语言)，弥补现有基准忽略人类偏好的缺陷。同时构建20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现：1)传统执行基准与人类偏好评估存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调了代码生成模型与人类偏好对齐的重要性，为提升实用价值提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T12:06:15Z
- **目录日期**: 2025-03-30
