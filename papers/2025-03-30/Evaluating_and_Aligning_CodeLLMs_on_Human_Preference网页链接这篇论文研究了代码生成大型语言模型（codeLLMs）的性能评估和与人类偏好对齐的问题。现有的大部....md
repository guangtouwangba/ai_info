...
这篇论文探讨了代码生成大模型（codeLLMs）如何与人类偏好对齐。现有基准主要测试代码正确性，忽略了人类偏好匹配。为此，研究者构建了CodeArena基准（含397个样本，覆盖40类44种语言）和SynCode-Instruct语料库（20B标记）。实验发现：1）执行基准与CodeArena存在性能差异；2）开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著。研究强调了代码LLM与人类偏好对齐的重要性，为提升模型实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T19:05:19Z
- **目录日期**: 2025-03-30
