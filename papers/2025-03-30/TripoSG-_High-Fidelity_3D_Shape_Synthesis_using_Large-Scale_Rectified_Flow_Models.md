...
TripoSG是一种新型3D形状生成框架，通过扩散技术实现高保真3D网格生成。其核心创新包括：1）基于大规模数据训练的高效整流流变换器；2）结合SDF、法线和Eikonal损失的混合监督训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，该框架能生成细节丰富、与输入图像高度匹配的3D形状，并展现出优异的泛化能力，支持多种图像风格的3D转换。研究团队将公开模型以推动3D生成领域发展。该技术突破了当前3D生成在质量、泛化性和输入对齐方面的限制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T03:17:04Z
- **目录日期**: 2025-03-30
