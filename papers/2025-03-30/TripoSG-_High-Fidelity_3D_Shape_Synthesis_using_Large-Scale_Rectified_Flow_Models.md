...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型提出三大创新：1）基于大规模高质量数据训练的大规模整流流Transformer；2）结合SDF、法线和eikonal损失的混合监督3D VAE训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在3D形状生成上达到最先进水平，能生成细节丰富且与输入图像高度吻合的3D模型，并展现出对多样化图像风格的强泛化能力。该模型将被公开以推动3D生成领域的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T06:02:04Z
- **目录日期**: 2025-03-30
