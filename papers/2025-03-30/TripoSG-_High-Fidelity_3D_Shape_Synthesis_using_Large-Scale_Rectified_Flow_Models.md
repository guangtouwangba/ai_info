...
TripoSG是一种新型3D形状生成扩散模型，通过创新方法解决了当前3D生成技术在质量、泛化能力和输入对齐方面的挑战。该模型包含三大关键技术：1）基于大规模高质量数据训练的整流流transformer；2）结合SDF、法线和eikonal损失的混合监督3D VAE训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG能生成高保真3D网格，与输入图像精确对应，支持高分辨率细节，并适应多样化的图像风格和内容。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T09:02:12Z
- **目录日期**: 2025-03-30
