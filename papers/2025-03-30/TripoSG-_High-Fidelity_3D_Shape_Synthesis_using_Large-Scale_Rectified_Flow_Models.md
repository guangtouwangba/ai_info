...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型包含三大创新：1）基于大规模高质量数据训练的整流流变换器，实现最先进的生成质量；2）结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3）可生成200万高质量3D样本的数据处理流程，验证了数据规模和质量对3D生成的关键作用。实验表明，TripoSG在生成细节、输入图像对齐和多风格适应方面表现优异。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T19:01:55Z
- **目录日期**: 2025-03-30
