...
TripoSG是一种新型3D形状扩散范式，通过创新方法解决了当前3D生成在质量、泛化能力和输入对齐方面的挑战。该技术提出：1）基于大规模校正流变换器的3D生成框架；2）结合SDF、法线和eikonal损失的混合监督训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG实现了最先进的3D生成性能，能生成高保真网格并精确匹配输入图像，同时支持多种图像风格的3D转换。该模型将公开以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T15:01:56Z
- **目录日期**: 2025-03-30
