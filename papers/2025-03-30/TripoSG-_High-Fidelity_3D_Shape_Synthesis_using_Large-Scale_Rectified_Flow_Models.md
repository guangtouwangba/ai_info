...
TripoSG是一种新型3D形状生成扩散模型，通过三个创新点实现了当前最优性能：1）基于大规模数据训练的可整流流变压器，提升生成保真度；2）结合SDF、法线和eikonal损失的混合监督训练策略，优化3D重建质量；3）开发了可生成200万高质量3D样本的数据处理流程。实验表明，该框架能生成高分辨率、细节丰富的3D网格，并精准匹配输入图像，对不同风格内容展现出优异的泛化能力。研究团队将公开模型以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T07:02:04Z
- **目录日期**: 2025-03-30
