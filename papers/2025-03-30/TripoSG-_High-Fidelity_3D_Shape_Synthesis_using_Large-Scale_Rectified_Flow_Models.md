...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。它采用三个关键技术：1)基于大规模数据训练的整流流变压器，实现最先进的生成质量；2)结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3)构建200万高质量3D样本的数据处理流程。实验表明，该框架各组件协同工作，在生成细节、输入图像对齐和多风格适应性方面表现优异。TripoSG将公开模型以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T13:06:06Z
- **目录日期**: 2025-03-30
