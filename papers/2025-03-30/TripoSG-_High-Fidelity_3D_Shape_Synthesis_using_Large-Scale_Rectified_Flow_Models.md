...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型采用：1)基于大规模高质量数据训练的整流流变压器提升生成质量；2)结合SDF、法线和eikonal损失的混合监督训练策略优化3D重建；3)处理200万高质量3D样本的数据管道。实验表明，TripoSG在生成细节、输入图像对齐和多风格适应方面均达到最先进水平，展现了出色的泛化能力。该模型将公开以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T16:02:06Z
- **目录日期**: 2025-03-30
