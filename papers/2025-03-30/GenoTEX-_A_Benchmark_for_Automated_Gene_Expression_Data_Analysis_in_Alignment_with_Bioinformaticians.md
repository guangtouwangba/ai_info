...
近期机器学习显著提升了从基因表达数据中识别疾病相关基因的能力，但传统方法依赖专家知识且难以扩展。为此，研究者开发了GenoTEX基准数据集，包含符合计算基因组学标准的全流程标注数据（数据选择、预处理和统计分析），并引入基于大语言模型的GenoAgent多智能体系统，通过自校正编程流程实现自动化分析。实验验证了大语言模型在基因组数据分析中的潜力，同时揭示了改进方向。该数据集（GitHub可获取）为基因表达分析的自动化方法提供了标准化评估资源，有望推动该领域发展。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T13:06:37Z
- **目录日期**: 2025-03-30
