...
近期机器学习在基因表达数据疾病相关基因识别方面取得显著进展，但传统方法依赖专家经验且难以扩展。为此，研究者开发了GenoTEX基准数据集，包含符合计算基因组学标准的全流程标注数据（数据选择、预处理、统计分析），并配有生物信息学专家验证的注释。同时提出基于大语言模型（LLM）的GenoAgent系统，采用多智能体协作编程与自校正机制实现自动化分析。实验验证了LLM在基因组数据分析中的潜力，也揭示了改进方向。该资源（GitHub开源）为基因表达分析的自动化方法评估与优化提供了重要基准平台。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T09:02:42Z
- **目录日期**: 2025-03-30
