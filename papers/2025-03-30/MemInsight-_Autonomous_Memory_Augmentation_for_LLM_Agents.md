...
本文提出了一种名为MemInsight的自主记忆增强方法，旨在优化大型语言模型(LLM)代理的语义数据表示和检索机制。通过利用历史交互的自主增强，该方法显著提升了LLM代理的响应准确性和情境化程度。实验在对话推荐、问答和事件摘要三个任务场景中验证了其有效性：在LLM-REDIAL数据集上，推荐说服力提升达14%；在LoCoMo检索任务中，召回率比RAG基线高出34%。研究结果表明，MemInsight能有效增强LLM代理在多任务中的情境表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-30T04:00:57Z
- **目录日期**: 2025-03-30
