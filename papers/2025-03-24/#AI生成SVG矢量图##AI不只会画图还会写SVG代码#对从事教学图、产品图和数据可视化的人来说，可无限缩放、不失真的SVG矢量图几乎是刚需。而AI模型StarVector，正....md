...
StarVector是一款能直接生成高质量SVG矢量图代码的AI工具，可根据图像或文字描述输出结构简洁、语义清晰的SVG代码（如<circle>、<rect>等原生元素）。其基于包含210万组数据的SVG-Stack数据集训练，支持文本到SVG的精准转换（如描述"红色圆圈包矩形"即可生成对应代码）。采用自研评估指标DinoScore（基于DINOv2模型）替代传统像素比对，与人类评分高度一致。提供1B（轻量版）和8B（高精度版）两个版本，生成速度仅74秒（传统方法需20+分钟）。论文及代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T20:02:53Z
- **目录日期**: 2025-03-24
