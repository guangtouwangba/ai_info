...
StarVector是一款能直接生成高质量SVG矢量图代码的AI工具，支持通过图像或文字描述（如"红色圆圈包着写Start的矩形"）自动输出结构简洁、可直接使用的SVG代码。其核心是SVG-Stack数据集（含210万组图像-SVG配对数据），并创新采用DinoScore语义相似度评估指标。提供1B（轻量版）和8B（精准版）两个版本，生成速度仅74秒，远超传统方法的20分钟。该技术有望重塑矢量图创作方式，实现"AI写图"而非画图。项目已开源，相关论文发布于arXiv。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T17:03:09Z
- **目录日期**: 2025-03-24
