...
StarVector是一款能直接生成高质量SVG代码的AI工具，可根据图像或文字描述输出结构简洁、语义清晰的矢量图形代码（如<circle>、<rect>等原生元素）。其核心是基于包含210万组数据的SVG-Stack数据集，并创新采用DinoScore语义相似度评估指标。提供1B（轻量版）和8B（高精度版）两种模型，生成速度仅74秒，远超传统方法。支持文本到SVG的精准转换（如"红色圆圈包矩形"指令）。项目已开源，论文发布于arXiv，标志着AI驱动矢量图形生成的新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T16:03:24Z
- **目录日期**: 2025-03-24
