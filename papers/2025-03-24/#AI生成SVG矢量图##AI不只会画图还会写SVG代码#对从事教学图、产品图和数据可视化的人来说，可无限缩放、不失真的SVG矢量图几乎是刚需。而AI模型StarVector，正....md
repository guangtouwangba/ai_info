...
StarVector是一款AI工具，能直接生成高质量SVG矢量图代码（如<circle>、<polygon>等原生元素），支持图像或文本输入（如"红色圆圈包矩形"）。其核心是SVG-Stack数据集（210万组图像-SVG对），并创新使用DinoScore语义相似度评估。提供1B（快速原型）和8B（复杂图形）两个版本，生成仅需74秒，远超传统方法。未来SVG或将从"绘制"转向"AI编写"。论文及代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T14:03:19Z
- **目录日期**: 2025-03-24
