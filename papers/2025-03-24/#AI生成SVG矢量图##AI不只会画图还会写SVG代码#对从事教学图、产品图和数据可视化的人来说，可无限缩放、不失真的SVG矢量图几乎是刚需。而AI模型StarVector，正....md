...
StarVector是一款能直接生成高质量SVG矢量图代码的AI工具。它基于SVG-Stack数据集（含210万组图像-SVG代码对），支持通过图像或文字描述生成结构简洁、语义清晰的SVG代码（如使用<circle>、<rect>等原生元素）。创新性地采用DinoScore评估指标（基于DINOv2模型语义相似度），其生成速度（74秒/图）远超传统方法（20分钟）。提供1B（轻量版）和8B（精准版）两个版本，适用于教学图、产品图等场景。研究表明其输出质量与人类评分高度一致，预示AI生成矢量图的未来趋势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T19:02:44Z
- **目录日期**: 2025-03-24
