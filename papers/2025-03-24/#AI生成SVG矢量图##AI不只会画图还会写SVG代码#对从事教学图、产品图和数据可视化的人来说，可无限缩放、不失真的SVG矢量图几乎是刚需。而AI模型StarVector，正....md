...
StarVector是一款能直接生成高质量SVG矢量图代码的AI工具，支持通过图像或文本描述自动输出结构简洁、生产可用的SVG代码（如<circle>、<rect>等原生元素）。其基于包含210万组数据的SVG-Stack数据集训练，独创DinoScore语义评估指标，生成速度仅74秒（传统方法需20分钟）。提供轻量版（StarVector-1B）和旗舰版（StarVector-8B），后者能处理复杂图形。未来SVG设计或将从"绘制"转向AI"编写"。论文及代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T21:02:39Z
- **目录日期**: 2025-03-24
