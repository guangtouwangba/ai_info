...
StarVector是一款AI工具，能根据图像或文字描述直接生成高质量SVG矢量图代码，使用原生语义元素（如<circle>、<rect>等），结构简洁且可直接用于生产环境。其基于SVG-Stack数据集（含210万组图像-SVG-描述数据），支持文本到SVG的精准转换（如输入“红色圆圈包矩形”即可生成对应代码）。采用自创评估指标DinoScore（基于语义相似度），性能优于传统方法（74秒生成vs传统20分钟）。提供轻量版（1B参数）和旗舰版（8B参数），适用于快速原型或复杂图形需求。论文及代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T18:03:05Z
- **目录日期**: 2025-03-24
