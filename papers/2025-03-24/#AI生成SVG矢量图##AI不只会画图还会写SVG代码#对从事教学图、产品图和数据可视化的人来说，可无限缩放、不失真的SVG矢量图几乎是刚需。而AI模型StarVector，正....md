...
StarVector是一款能直接生成高质量SVG代码的AI工具，支持通过图像或文字描述（如"红色圆圈包着写'Start'的矩形"）自动输出结构简洁、生产级可用的SVG代码。其核心是基于含210万组数据的SVG-Stack数据集训练，采用自创的DinoScore语义评估指标。提供1B（轻量版）和8B（精准版）两个版本，生成速度仅74秒（传统方法需20分钟）。该技术突破显示AI可能重塑矢量图创作方式，未来SVG或将从"绘制"转向"代码生成"。项目已开源，相关论文发布于arXiv。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T11:03:14Z
- **目录日期**: 2025-03-24
