...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好的对齐问题。研究者发现现有基准测试过于关注代码正确性，忽视了人类偏好匹配，因此构建了CodeArena基准（包含397个高质量样本，覆盖40类任务和44种编程语言）和SynCode-Instruct合成指令集（近200亿标记）。实验显示执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著，凸显了人类偏好对齐的重要性。研究证实大规模合成指令微调能有效提升模型表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T14:06:21Z
- **目录日期**: 2025-03-24
