...
这篇论文针对代码生成大语言模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建近200亿token的SynCode-Instruct合成指令数据集用于微调。研究发现：现有执行基准与人类偏好存在显著差异；开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面仍有明显差距。研究突显了代码生成模型与人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T11:06:08Z
- **目录日期**: 2025-03-24
