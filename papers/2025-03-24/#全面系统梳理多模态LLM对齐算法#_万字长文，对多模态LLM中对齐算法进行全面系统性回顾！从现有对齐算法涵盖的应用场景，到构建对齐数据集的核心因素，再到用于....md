...
本文全面回顾了多模态大语言模型（MLLMs）中的对齐算法，涵盖了应用场景、数据集构建、评估方法和未来发展方向。文章指出，尽管MLLMs在处理复杂多模态数据方面取得进展，但在真实性、安全性和推理能力等方面仍面临挑战。通过对现有对齐算法的分类和评估基准的整理，研究为未来改进提供了参考，并提出了视觉信息整合和LLM对齐方法的经验启示等潜在发展方向。该研究由多个国内外知名机构的研究人员共同完成。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T06:04:15Z
- **目录日期**: 2025-03-24
