...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需二阶动量即可避免训练不平衡，并将内存占用减半。实验显示，在Transformer任务、ImageNet分类及GPT-2/Llama2预训练中，其性能匹配或优于AdamW，且超参数鲁棒性强。内存效率显著，如为GPT-2节省5.93GB，Llama2-7B节省25.15GB。在LoRA微调等场景也超越现有优化器，解决了SGD训练Transformer的难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T14:04:58Z
- **目录日期**: 2025-03-24
