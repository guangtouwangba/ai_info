...
本文提出了一种新的优化方法SGD-SaI，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI通过在不同参数组初始化时调整学习率，依据梯度信号噪声比（g-SNR）来指导，无需依赖自适应二阶动量，从而防止训练不平衡并减少内存使用。该方法在多种任务中表现优异，包括Transformer任务、ImageNet-1K分类和GPT-2预训练，且在LoRA微调等任务中优于现有优化器。内存效率方面，SGD-SaI在完整精度训练设置下显著节省内存，如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T07:05:17Z
- **目录日期**: 2025-03-24
