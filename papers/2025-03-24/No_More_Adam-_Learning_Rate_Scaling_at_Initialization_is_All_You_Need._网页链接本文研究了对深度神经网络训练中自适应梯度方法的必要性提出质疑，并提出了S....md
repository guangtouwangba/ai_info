...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量就能防止训练不平衡，并将优化器内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练中表现优于或媲美AdamW，特别在内存效率上优势显著：为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存。同时，该方法在LoRA微调等任务中也优于现有优化器，且具有超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T12:04:58Z
- **目录日期**: 2025-03-24
