...
本文质疑了深度神经网络训练中自适应梯度方法的必要性，并提出了SGD-SaI方法，这是对带动量的随机梯度下降（SGDM）的改进。SGD-SaI通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡，并减少了优化器的内存使用。该方法在多种任务中表现优异，包括Transformer任务、ImageNet-1K分类和GPT-2预训练，且在内存效率上显著优于AdamW，为GPT-2和Llama2-7B分别节省了5.93GB和25.15GB内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T04:05:09Z
- **目录日期**: 2025-03-24
