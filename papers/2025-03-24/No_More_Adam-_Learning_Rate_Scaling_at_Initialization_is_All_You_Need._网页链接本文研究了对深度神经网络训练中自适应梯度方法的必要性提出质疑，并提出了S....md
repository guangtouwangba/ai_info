...
本文提出了一种新的优化方法SGD-SaI，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI通过在不同参数组初始化时调整学习率，依据梯度信号噪声比（g-SNR）来指导，无需依赖自适应二阶动量，从而防止训练不平衡并减少内存使用。该方法在多种任务中表现优异，包括Transformer任务、ImageNet-1K分类和大型语言模型预训练，且在内存效率上显著优于AdamW。SGD-SaI还展示了在LoRA微调等任务中的优越性能，具备对超参数变化的鲁棒性和广泛的实际应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T06:05:28Z
- **目录日期**: 2025-03-24
