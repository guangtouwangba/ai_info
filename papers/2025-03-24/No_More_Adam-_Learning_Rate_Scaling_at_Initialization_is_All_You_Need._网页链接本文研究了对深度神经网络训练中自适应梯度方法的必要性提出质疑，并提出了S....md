...
本文提出了一种名为SGD-SaI的新方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题，并将优化器的内存使用量减半。该方法在多种任务中表现优异，包括Transformer任务、ImageNet-1K分类和GPT-2预训练，且在LoRA微调等任务中优于现有优化器。SGD-SaI在内存效率方面显著优于AdamW，为GPT-2和Llama2-7B节省了大量内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T05:05:07Z
- **目录日期**: 2025-03-24
