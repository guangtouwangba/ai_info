...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个代理的社会互动（500万次交互）。它作为计算社会科学实验平台，研究了社会极化、信息传播、基本收入政策及自然灾害影响等现实问题。模拟结果与现实数据高度吻合，验证了该系统捕捉人类行为机制的能力，展现了其在社会科学研究和政策制定中的实用价值。该系统能有效补充传统研究方法（如调查、访谈），帮助深入分析社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T20:04:40Z
- **目录日期**: 2025-03-24
