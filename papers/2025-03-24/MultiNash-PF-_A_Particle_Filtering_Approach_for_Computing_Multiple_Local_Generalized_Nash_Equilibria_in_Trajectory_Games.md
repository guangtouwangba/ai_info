...
该研究提出了一种高效算法MultiNash-PF，用于处理多模态多智能体交互问题。通过将交互建模为约束势轨迹博弈（CPTG），将不同交互模式对应为广义纳什均衡（GNE），并采用隐式粒子滤波高效识别势函数的多个局部极小值点，再通过优化求解器精修得到不同GNE。相比基线方法，计算时间减少达50%。实验验证了算法在实时人机交互场景中的有效性，能识别多模态交互并解决潜在冲突。该成果为机器人理解复杂多模态交互提供了新思路，具有实际应用价值。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T09:30:20Z
- **目录日期**: 2025-03-24
