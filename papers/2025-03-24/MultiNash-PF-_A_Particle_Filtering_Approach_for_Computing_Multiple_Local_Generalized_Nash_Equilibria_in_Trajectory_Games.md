...
本文提出了一种高效算法MultiNash-PF，用于处理多智能体交互中的多模态问题。该方法通过博弈论规划将交互结果建模为不同均衡点（对应不同交互模式），并利用约束势轨迹游戏(CPTG)和广义纳什均衡(GNE)进行建模。结合隐式粒子滤波和优化求解器，算法能高效识别多个局部GNE，相比基线方法减少50%计算时间。实验证明，该算法能有效处理人机交互中的多模态特性，实时解决潜在冲突。研究为机器人适应复杂多智能体交互提供了新思路，在实时性和准确性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T17:02:17Z
- **目录日期**: 2025-03-24
