...
本文提出了一种高效算法MultiNash-PF，用于处理多智能体交互中的多模态问题。该方法通过博弈论规划将交互结果建模为不同均衡点（对应不同交互模式），并利用约束势轨迹游戏（CPTG）和局部广义纳什均衡（GNE）进行描述。结合隐式粒子滤波和优化求解器，算法能快速识别多个局部GNE，相比基线方法计算时间减少达50%。实验验证了该算法在实时人机交互中有效处理多模态特性并解决潜在冲突的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T14:02:20Z
- **目录日期**: 2025-03-24
