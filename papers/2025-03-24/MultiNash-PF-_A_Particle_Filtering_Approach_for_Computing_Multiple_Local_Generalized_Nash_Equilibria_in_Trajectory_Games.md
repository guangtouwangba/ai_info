...
本文提出了一种高效算法MultiNash-PF，用于处理多智能体交互中的多模态问题。该方法通过博弈论规划将交互结果建模为不同均衡点（对应不同交互模式），并利用约束势轨迹游戏（CPTG）框架和广义纳什均衡（GNE）进行建模。结合隐式粒子滤波和优化求解器，算法能高效识别势函数的多个局部极小值点，从而找到不同GNE。实验表明，该算法比基线方法减少50%计算时间，并在人机交互场景中成功处理多模态交互，实时解决潜在冲突。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T16:02:23Z
- **目录日期**: 2025-03-24
