...
百度推出国内首个对话式应用开发平台"秒哒"，无需编写代码即可快速开发完整网站。用户只需输入需求，AI会自动完成架构设计、编码实现、前后端开发和测试，3分钟内生成包含多个页面和后台管理的完整网站，并自动配置域名。该平台支持H5、网站和轻应用开发，实现多智能体协作，所有内容均可二次编辑，同时适配移动端。这是百度去年发布的重点产品，通过自然语言交互实现分钟级应用开发，大幅降低技术门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T13:10:36Z
- **目录日期**: 2025-03-24
