...
百度推出的"秒哒"是国内首个对话式应用开发平台，用户无需编写代码，仅需描述需求即可在3分钟内生成完整网站。该平台通过AI自动完成架构设计、技术选型、前后端编码及测试，支持多页面生成（包括移动端和后台管理），并提供域名发布服务。演示显示，AI可同时处理多个智能体协作开发，实现H5、网站等应用的分钟级创建，所有内容均可二次编辑。该产品由李彦宏在2022年11月发布，特点是"会说需求比会写代码更重要"。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T14:04:04Z
- **目录日期**: 2025-03-24
