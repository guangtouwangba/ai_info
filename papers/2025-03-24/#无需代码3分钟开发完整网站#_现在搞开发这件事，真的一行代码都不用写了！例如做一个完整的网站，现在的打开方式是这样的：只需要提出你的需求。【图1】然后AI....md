...
百度推出的"秒哒"是国内首个对话式应用开发平台，用户无需编写代码，仅需提出需求，AI即可在3分钟内自动完成完整网站开发。该平台能自动处理架构设计、技术选型、编码实现等全流程，生成包含前端页面、后台管理及移动端适配的完整应用，并支持二次编辑和即时发布。演示显示，AI可一次性生成5个关联网页，甚至提供现成域名。该产品于2022年11月由李彦宏发布，主打"会说需求比会写代码更重要"的无代码开发理念，适用于H5、网站等轻应用快速搭建。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T22:03:15Z
- **目录日期**: 2025-03-24
