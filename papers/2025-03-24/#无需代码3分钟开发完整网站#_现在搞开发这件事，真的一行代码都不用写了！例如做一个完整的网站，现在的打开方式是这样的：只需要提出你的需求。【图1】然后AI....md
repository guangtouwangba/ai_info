...
百度推出的"秒哒"是国内首个对话式应用开发平台，用户无需编写代码即可快速创建完整网站。通过输入需求，AI会自动完成架构设计、技术选型、前后端编码及测试，3分钟内生成包含5个页面、后台管理和域名的网站，并支持移动端适配和二次编辑。该平台能开发H5、网站和轻应用，实现多智能体协作开发，大幅降低开发门槛。李彦宏在2022年11月曾亲自发布这一产品，其核心特点是"会说比会写重要"，通过自然语言交互实现分钟级应用开发。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T19:03:19Z
- **目录日期**: 2025-03-24
