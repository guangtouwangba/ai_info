...
大模型正在革新兴趣推荐系统，突破传统基于标签/ID匹配的局限。YouTube采用"语义ID"解决冷启动问题；快手M3CSR融合多模态数据捕捉用户真实兴趣；Meta通过知识蒸馏提升小模型性能；LinkedIn训练1500亿参数统一模型处理多任务；Netflix的UniCoRn实现搜索推荐一体化；华为FLIP实现文本与结构化数据互通；Spotify结合用户行为生成探索性推荐。通过知识蒸馏、模型优化等技术，新系统在精度、效率和冷启动方面显著提升，实现更精准、个性化的推荐体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T16:03:35Z
- **目录日期**: 2025-03-24
