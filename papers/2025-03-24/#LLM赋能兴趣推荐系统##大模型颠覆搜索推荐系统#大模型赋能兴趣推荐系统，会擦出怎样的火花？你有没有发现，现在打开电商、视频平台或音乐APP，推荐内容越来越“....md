...
大模型(LLM)正在革新兴趣推荐系统，突破传统基于标签匹配的局限。YouTube采用"语义ID"解决冷启动问题；快手的M3CSR多模态模型融合视频画面、文字和音乐；Meta通过知识蒸馏提升推荐精度48%；LinkedIn训练1500亿参数的统一推荐模型；Netflix的UniCoRn系统结合搜索与推荐；华为FLIP模型实现自然语言与表格数据互通；Spotify利用LLM生成探索性搜索建议。这些技术配合知识蒸馏、LoRA等优化方法，使系统更智能高效，能理解内容本质和用户深层需求，显著提升推荐质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T13:10:05Z
- **目录日期**: 2025-03-24
