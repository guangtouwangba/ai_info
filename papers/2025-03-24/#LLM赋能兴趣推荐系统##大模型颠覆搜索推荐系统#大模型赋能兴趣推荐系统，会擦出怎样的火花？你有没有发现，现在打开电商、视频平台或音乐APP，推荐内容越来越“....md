...
大模型(LLM)正在革新兴趣推荐系统，突破传统基于标签/ID匹配的局限。YouTube采用"语义ID"解决冷启动问题；快手的M3CSR融合多模态理解用户偏好；Meta通过知识蒸馏提升48%推荐精度；LinkedIn推出1500亿参数统一推荐模型；Netflix的UniCoRn系统结合搜索与推荐；华为FLIP模型实现文本与表格数据互通；Spotify利用LLM生成探索性搜索建议。这些技术配合知识蒸馏、LoRA等优化方法，使推荐系统更精准高效，能理解内容本质和用户深层意图，显著提升个性化推荐体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T19:02:53Z
- **目录日期**: 2025-03-24
