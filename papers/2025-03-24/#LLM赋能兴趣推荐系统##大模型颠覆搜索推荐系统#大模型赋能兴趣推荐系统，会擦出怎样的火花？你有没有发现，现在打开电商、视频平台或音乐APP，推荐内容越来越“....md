...
大模型正在革新兴趣推荐系统，突破传统基于标签/ID匹配的局限。YouTube采用"语义ID"理解视频内容本质，解决冷启动问题；快手M3CSR融合多模态数据捕捉用户真实兴趣；Meta通过知识蒸馏实现高效精准推荐；LinkedIn训练1500亿参数统一模型处理多任务；Netflix的UniCoRn系统结合搜索与推荐逻辑；华为FLIP模型实现文本与结构化数据互通；Spotify利用LLM生成探索性搜索建议。通过知识蒸馏、LoRA等技术优化，这些系统在保持高效的同时，展现出更强的语义理解、多任务处理和冷启动能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T22:02:53Z
- **目录日期**: 2025-03-24
