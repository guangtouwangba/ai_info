...
大模型正在革新兴趣推荐系统，通过语义理解突破传统标签匹配的局限。YouTube采用"语义ID"解决冷启动问题；快手M3CSR融合多模态数据捕捉用户偏好；Meta通过知识蒸馏提升小模型性能；LinkedIn训练1500亿参数统一模型处理多任务；Netflix的UniCoRn结合搜索与推荐；华为FLIP实现自然语言与结构化数据互通；Spotify用LLM生成探索性建议。这些技术配合模型优化方法（如知识蒸馏、LoRA），使系统更智能高效，能理解内容本质和用户意图，显著提升推荐精准度和响应速度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T09:31:16Z
- **目录日期**: 2025-03-24
