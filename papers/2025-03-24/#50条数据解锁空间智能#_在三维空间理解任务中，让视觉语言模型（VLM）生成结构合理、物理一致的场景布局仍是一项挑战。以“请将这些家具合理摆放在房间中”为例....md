...
西北大学的研究人员潘震宇和刘晗提出了MetaSpatial框架，通过规则驱动的强化学习策略（RFT）提升视觉语言模型（VLM）在三维空间布局任务中的推理能力。该方法利用约50条无标注数据，构建可程序化的奖励函数，评估布局的合理性、物理约束和用户偏好，并通过多轮优化机制逐步改进空间决策。实验表明，MetaSpatial在多个空间生成指标上显著优于传统的监督微调方法（SFT），展示了强化学习在三维空间智能建模中的有效性和通用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T06:04:48Z
- **目录日期**: 2025-03-24
