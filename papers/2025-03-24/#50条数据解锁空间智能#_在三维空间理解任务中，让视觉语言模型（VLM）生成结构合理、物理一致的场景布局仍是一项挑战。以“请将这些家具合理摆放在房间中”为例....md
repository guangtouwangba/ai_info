...
西北大学的研究人员潘震宇和刘晗提出了MetaSpatial框架，通过规则驱动的强化学习策略（RFT）提升视觉语言模型（VLM）在三维空间布局任务中的推理能力。传统方法如监督微调（SFT）受限于单一标准答案和标注数据的稀缺，难以生成多样化的合理布局。MetaSpatial通过构建可程序化评估的奖励函数和多轮优化机制，仅需约50条无标注数据，即可显著提升模型的空间推理和布局生成质量，验证了强化学习在三维空间智能建模中的有效性和通用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-24T00:03:45Z
- **目录日期**: 2025-03-24
