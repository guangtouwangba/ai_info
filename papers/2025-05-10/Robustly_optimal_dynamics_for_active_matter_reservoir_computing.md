...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟外部驱动的混沌信号预测模型，发现了一个此前被忽视的优异动力学机制。这一机制在多种物理参数和推断任务中均表现稳健且最优，揭示了系统信息处理能力主要取决于其内在弛豫特性。最优计算状态位于临界阻尼阈值之下，表现为多阶段微观动力学弛豫，并能在混沌驱动下通过准静态与高度非线性状态的快速切换实现自适应。相干与非相干动力学共同作用，且粒子动态相关性可标识最优性能区间。该模型为多体非平衡物理提供了新的计算解释框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T03:16:38Z
- **目录日期**: 2025-05-10
