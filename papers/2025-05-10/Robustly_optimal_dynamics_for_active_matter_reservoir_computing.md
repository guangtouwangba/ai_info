...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟外部驱动的混沌信号预测模型，发现了一个此前被忽视的优越动态机制。这一机制在多种物理参数和任务中表现稳健且最优，揭示了系统内在弛豫能力对信息处理的关键作用。最优计算状态位于临界阻尼阈值之下，表现为多阶段微观弛豫，并能通过准稳态与高度非线性状态的快速切换适应混沌驱动。相干与非相干动态共同作用，且动态相关性可标识最优性能区间。该模型为多体非平衡物理中的非常规计算提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T20:02:05Z
- **目录日期**: 2025-05-10
