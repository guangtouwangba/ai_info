...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过外部驱动模型预测混沌信号的未来状态。研究发现了一个此前被忽视的优越动力学状态，该状态在不同物理参数和推断任务中均表现出鲁棒的最优性。最优计算状态位于临界阻尼阈值以下，涉及多阶段的微观动力学弛豫过程。系统在混沌外部驱动下具有适应性，其响应机制多样，包括准静态与高度非线性动态状态之间的快速切换。相干与非相干动力学共同作用，且在不同时空尺度上运作。该模型为理解非平衡多体物理中的计算提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T19:01:58Z
- **目录日期**: 2025-05-10
