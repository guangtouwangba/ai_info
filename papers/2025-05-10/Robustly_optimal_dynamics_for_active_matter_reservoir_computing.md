...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟外部驱动的混沌信号预测模型，发现了一个此前被忽视的优异动力学机制。这一机制在多种物理参数和推断任务中均表现出稳健的最优性，揭示了物理系统计算与推断的普适性原理。其核心在于系统固有的多阶段弛豫能力——最优计算状态恰好位于临界阻尼阈值之下，表现为准静态与高度非线性动态间的快速切换。相干与非相干动力学的多尺度协同作用，以及粒子动态相关性，共同标记了最优计算区间。该模型为理解非平衡多体物理中的非常规计算提供了新视角。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T06:02:09Z
- **目录日期**: 2025-05-10
