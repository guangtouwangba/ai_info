...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟受外部驱动的系统预测混沌信号。研究发现了一个此前被忽视的优异动力学状态，该状态在不同物理参数和推断任务中均表现出稳健的最优性，揭示了物理系统计算与推断的通用机制。最优计算状态位于临界阻尼阈值以下，涉及多阶段微观动力学弛豫，其适应性源于准静态与高度非线性动态间的快速切换。相干与非相干动力学共同作用，且不同时空尺度上的相关性可标识最优性能状态。该模型为多体非平衡物理中的非传统计算提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T02:29:19Z
- **目录日期**: 2025-05-10
