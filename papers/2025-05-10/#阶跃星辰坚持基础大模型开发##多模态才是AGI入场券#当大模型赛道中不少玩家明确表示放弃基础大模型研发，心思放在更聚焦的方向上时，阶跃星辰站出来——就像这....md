# #阶跃星辰坚持基础大模型开发##多模态才是AGI入场券#当大模型赛道中不少玩家明确表示放弃基础大模型研发，心思放在更聚焦的方向上时，阶跃星辰站出来——就像这...
...
阶跃星辰坚持基础大模型研发，CEO姜大昕认为技术发展仍处快速迭代期（如GPT-4即将"过时"），放弃基础模型会错失主流趋势。同时强调模型与应用相辅相成——模型决定上限，应用提供场景和数据。其产品已完成从"跃问"ChatGPT形态向"阶跃AI"Agent平台的转型，主打端云一体，虽与头部企业合作但最终服务C端用户。姜大昕认为多模态是AGI关键，公司将持续探索多模态理解能力，保持基础模型与应用场景的双向推进。（99字）  

核心提炼：  
1. 战略定位：死磕基础大模型研发  
2. 技术判断：AI技术仍处陡峭增长期  
3. 产品逻辑：模型（上限）↔应用（场景）闭环  
4. 形态进化：ChatGPT→Agent平台（C端导向）  
5. 未来方向：多模态能力建设
