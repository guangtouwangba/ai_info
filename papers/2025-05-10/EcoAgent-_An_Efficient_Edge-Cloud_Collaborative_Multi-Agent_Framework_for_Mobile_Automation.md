...
EcoAgent是一个边缘-云协作的多智能体框架，旨在解决基于云端大语言模型（M）LLM的高延迟、高成本问题。它通过云端规划智能体和两个边缘智能体（执行智能体和观察智能体）形成闭环协作：执行智能体负责操作，观察智能体通过预理解模块压缩屏幕图像为精简文本以减少token消耗。若任务失败，规划智能体通过反射模块重新检索屏幕历史并调整计划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了大模型token消耗，实现了高效实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T22:01:35Z
- **目录日期**: 2025-05-10
