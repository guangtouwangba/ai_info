...
EcoAgent是一个边缘-云协作的多智能体框架，旨在提升移动自动化效率。它通过云端规划智能体与两个边缘智能体（执行智能体和观察智能体）的闭环协作，平衡了大型语言模型（LLM）的强推理能力与边缘部署的低延迟需求。观察智能体通过预理解模块压缩屏幕图像为简洁文本，减少令牌消耗；若任务失败，规划智能体通过反射模块重新规划。实验显示，EcoAgent在保持高任务成功率的同时显著降低了多模态LLM的令牌消耗，实现了高效实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T05:01:37Z
- **目录日期**: 2025-05-10
