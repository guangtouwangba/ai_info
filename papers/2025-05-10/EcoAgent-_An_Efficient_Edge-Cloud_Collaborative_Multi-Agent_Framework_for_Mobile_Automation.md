...
EcoAgent是一个边缘-云协作的多智能体框架，旨在解决基于云的大型语言模型（M）LLMs的高延迟和高成本问题，同时避免边缘部署的小型模型（M）SLMs在通用能力和复杂任务上的不足。该框架包含云端的规划智能体和边缘的执行与观察智能体，通过闭环协作实现移动自动化。观察智能体使用预理解模块压缩屏幕图像为简洁文本以减少令牌消耗，规划智能体通过反思模块在失败时重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了令牌消耗，实现了高效的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T11:01:43Z
- **目录日期**: 2025-05-10
