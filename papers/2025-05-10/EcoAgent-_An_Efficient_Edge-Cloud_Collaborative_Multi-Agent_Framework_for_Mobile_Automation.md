...
EcoAgent是一个边缘-云协作的多智能体框架，旨在解决基于云的大语言模型（LLM）在移动自动化中高延迟、高成本的问题。它通过云端规划智能体和两个边缘智能体（执行智能体和观察智能体）形成闭环协作。观察智能体利用预理解模块压缩屏幕图像为简洁文本，减少token消耗；若任务失败，规划智能体通过反射模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了LLM的token消耗，实现了高效实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T14:01:44Z
- **目录日期**: 2025-05-10
