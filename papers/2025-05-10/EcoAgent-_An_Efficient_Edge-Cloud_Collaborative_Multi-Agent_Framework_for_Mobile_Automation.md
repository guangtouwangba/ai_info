...
EcoAgent是一个边缘-云协同的多智能体框架，旨在解决基于云的大型语言模型（M）LLMs的高延迟和高成本问题，同时克服边缘部署的小型模型（M）SLMs通用能力不足的缺陷。该框架包含云端的规划智能体和边缘的执行与观察智能体，形成闭环协作。观察智能体通过预理解模块压缩屏幕图像为文本，减少令牌消耗；若任务失败，规划智能体通过反思模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了令牌消耗，实现了高效的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T06:01:47Z
- **目录日期**: 2025-05-10
