...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究者构建了包含397个高质量样本(覆盖40类任务和44种语言)的人工评测集，并开发了20B标记量的合成指令数据集SynCode-Instruct。实验发现：1)传统执行正确性评测与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)仍存在显著差距。研究强调了代码生成模型除功能性外，还需重视人类偏好对齐，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T20:05:49Z
- **目录日期**: 2025-05-10
