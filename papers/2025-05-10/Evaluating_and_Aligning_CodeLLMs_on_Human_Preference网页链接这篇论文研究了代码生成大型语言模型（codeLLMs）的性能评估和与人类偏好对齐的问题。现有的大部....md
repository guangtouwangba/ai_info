...
该论文针对代码生成大模型(codeLLMs)提出创新评估框架CodeArena，包含397个真实编程任务样本(覆盖40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度。研究同时构建20B标记的合成指令集SynCode-Instruct验证微调效果。实验发现：1)传统执行正确性基准与人类偏好存在偏差；2)开源顶级模型(如Qwen2.5-Coder)与商用模型(如OpenAI)性能差距显著。研究强调代码LLMs除功能性外，需重视开发者体验的对齐优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T05:05:32Z
- **目录日期**: 2025-05-10
