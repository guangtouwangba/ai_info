...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示，直接建模音频-视觉关联。针对扩散模型的时间一致性问题，创新性提出TREPA方法（时间表示对齐），利用自监督视频模型增强生成帧的时间连贯性。同时，通过系统研究改进了SyncNet训练框架，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验可推广至其他音频驱动动画任务。该成果为扩散模型在唇同步领域的应用提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T02:32:37Z
- **目录日期**: 2025-05-10
