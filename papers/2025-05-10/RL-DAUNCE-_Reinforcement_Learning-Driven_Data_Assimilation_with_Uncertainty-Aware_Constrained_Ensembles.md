...
研究人员开发了名为RL-DAUNCE的新型强化学习方法，用于改进数据同化过程。该方法通过三个创新点结合物理约束：1）采用类似传统同化方法中集合成员的代理结构；2）强调不确定性量化而非简单均值优化；3）通过集合代理设计在过程中实施物理约束。研究采用原始-对偶优化策略动态惩罚奖励函数，并限制动作空间来保证物理一致性。应用于具有强非高斯特征的Madden-Julian振荡现象时，RL-DAUNCE表现优于标准集合卡尔曼滤波（EnKF），与约束EnKF性能相当但计算量显著减少，能更好捕捉极端事件和量化不确定性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T07:01:26Z
- **目录日期**: 2025-05-10
