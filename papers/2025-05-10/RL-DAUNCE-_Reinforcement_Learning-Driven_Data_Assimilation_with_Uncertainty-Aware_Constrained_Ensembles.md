...
本文提出了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE。该方法通过三个创新点提升数据同化性能：1) 采用与集合成员对应的智能体设计，保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 引入对偶优化策略和动作空间约束确保物理一致性。在模拟Madden-Julian振荡现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，其性能与计算量更大的约束EnKF相当，能更好地恢复间歇信号、捕捉极端事件并量化不确定性，同时计算量大幅降低。该方法成功解决了传统方法在非高斯特征和物理约束下的失效问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T19:01:22Z
- **目录日期**: 2025-05-10
