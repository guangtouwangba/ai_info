...
研究人员开发了名为RL-DAUNCE的新型强化学习数据同化方法，通过三个创新点提升传统方法：1）采用类似集合成员的智能体设计保持计算效率；2）通过多集合成员推进实现不确定性量化；3）独特的"集合即智能体"设计可在同化过程中实施物理约束。该方法采用原始-对偶优化策略动态调整奖励函数，同时限制动作空间来确保物理一致性。在具有强非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波（EnKF），性能与计算量更大的约束EnKF相当，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出，且计算效率更高。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T05:01:19Z
- **目录日期**: 2025-05-10
