...
本文提出了RL-DAUNCE，一种基于强化学习的数据同化新方法。该方法通过三个创新点提升性能：1) 采用类似传统集合同化的"集合即智能体"设计保持计算效率；2) 通过推进多个集合成员实现不确定性量化；3) 引入原始对偶优化策略和动作空间约束确保物理一致性。在具有强非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，其性能与计算量更大的约束EnKF相当，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出，同时计算成本大幅降低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T20:01:25Z
- **目录日期**: 2025-05-10
