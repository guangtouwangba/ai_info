...
研究人员开发了名为RL-DAUNCE的新型强化学习方法，用于改进数据同化过程。该方法通过三个创新点：1) 采用类似传统同化方法中集合成员的代理结构；2) 强调不确定性量化而非简单均值优化；3) 在过程中强制实施物理约束。结合原始对偶优化策略和动作空间约束，确保物理一致性。应用于具有强非高斯特征的Madden-Julian振荡现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与约束EnKF相当但计算量更小，能更好恢复间歇信号、捕捉极端事件并量化不确定性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T23:01:21Z
- **目录日期**: 2025-05-10
