...
研究人员开发了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE，该方法通过三个关键创新提升性能：(1)采用类似传统同化方法中集合成员的智能体设计，保持计算效率；(2)通过推进多个集合成员进行不确定性量化，超越简单的均值优化；(3)独特的"集合即智能体"设计可在同化过程中实施物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数，确保约束满足。在具有强非高斯特征和多重物理约束的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与约束EnKF相当但计算量大幅降低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T18:01:23Z
- **目录日期**: 2025-05-10
