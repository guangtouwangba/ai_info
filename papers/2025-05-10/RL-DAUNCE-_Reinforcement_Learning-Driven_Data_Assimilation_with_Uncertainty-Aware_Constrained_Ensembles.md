...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE，通过三个创新点提升数据同化性能：1）采用类似传统集合同化的代理结构保持计算效率；2）通过多集合成员推进实现不确定性量化；3）利用集合代理设计在过程中实施物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数，并限制动作空间来保证约束满足。在具有强非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波，性能与约束集合卡尔曼滤波相当，但计算成本更低，能更好地恢复间歇信号、捕捉极端事件和量化不确定性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T11:01:25Z
- **目录日期**: 2025-05-10
