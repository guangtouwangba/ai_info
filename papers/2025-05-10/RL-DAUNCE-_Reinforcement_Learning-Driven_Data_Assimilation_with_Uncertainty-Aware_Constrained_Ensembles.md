...
研究人员开发了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE。该方法通过三个关键创新提升了传统数据同化：1)采用类似集合成员的智能体设计保持计算效率；2)通过推进多个集合成员实现不确定性量化；3)独特的"集合即智能体"设计可在同化过程中实施物理约束。该方法采用原始-对偶优化策略动态调整奖励函数，确保约束满足。在模拟马登-朱利安振荡(MJO)时，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与约束EnKF相当但计算量更少，尤其在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T13:06:41Z
- **目录日期**: 2025-05-10
