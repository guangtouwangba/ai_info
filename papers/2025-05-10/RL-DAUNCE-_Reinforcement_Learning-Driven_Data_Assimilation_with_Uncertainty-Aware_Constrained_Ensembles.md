...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE，通过三个创新点提升传统数据同化：1）采用类似集合成员的智能体结构保持计算效率；2）通过多集合成员推进实现不确定性量化；3）设计集合即智能体架构，在过程中强制物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数，确保约束满足。应用于具有强非高斯特征的MJO现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波（EnKF），性能媲美约束EnKF但计算量大幅降低，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-10T22:01:18Z
- **目录日期**: 2025-05-10
