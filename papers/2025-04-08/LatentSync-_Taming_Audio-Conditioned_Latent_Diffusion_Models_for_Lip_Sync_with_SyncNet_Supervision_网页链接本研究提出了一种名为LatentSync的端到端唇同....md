...
LatentSync提出了一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，但发现扩散过程存在时间不一致问题，因此提出时间表示对齐（TREPA）方法增强连贯性。研究还优化了SyncNet训练框架，解决了收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T18:05:37Z
- **目录日期**: 2025-04-08
