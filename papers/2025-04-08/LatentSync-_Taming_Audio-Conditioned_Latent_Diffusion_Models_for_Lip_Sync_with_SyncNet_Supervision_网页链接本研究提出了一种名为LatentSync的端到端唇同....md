...
本研究提出LatentSync框架，通过音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，并提出时间表示对齐(TREPA)技术解决扩散过程中的时间一致性问题。研究还改进了SyncNet训练框架，通过调整架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)首个基于潜在扩散的唇同步方法；2)TREPA增强时序一致性；3)SyncNet训练优化方案。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T12:05:28Z
- **目录日期**: 2025-04-08
