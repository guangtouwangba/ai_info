...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案创新性地采用时间表示对齐（TREPA）方法解决扩散过程中的时间一致性问题，利用自监督视频模型提取的时间特征对齐生成帧。同时，研究深入分析了SyncNet的收敛问题，改进了其训练框架，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动的人像动画提供了新思路。这些改进也可推广至其他依赖SyncNet的唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T08:05:36Z
- **目录日期**: 2025-04-08
