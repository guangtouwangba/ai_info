...
中国人民大学与蚂蚁集团研究团队提出首个个性化对齐大模型AlignXpert，突破传统AI价值观对齐的局限性。该模型通过90维心理偏好空间和16亿条交互数据构建130万用户画像数据集，采用ICA（上下文对齐）和PBA（偏好桥接）技术，实现17%的偏好预测准确率提升。研究解决了主流模型忽视少数群体偏好的问题，建立"用户画像-偏好方向"双向建模框架，相关数据和模型已开源。这一创新使AI能精准识别用户内在动机，在数据稀缺等复杂场景下保持稳定输出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T03:15:05Z
- **目录日期**: 2025-04-08
