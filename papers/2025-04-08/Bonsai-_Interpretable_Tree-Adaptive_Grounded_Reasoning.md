...
Bonsai是一种可组合的概率推理系统，旨在开发适应性强且透明的通用协作AI代理。它通过检索相关证据并计算子主张的似然概率，生成可调整的推理树。与不透明的黑盒模型不同，Bonsai具有领域适应性、可解释性和不确定性感知能力。其推理能力可通过证据缩放进行调整，并能可靠处理文本、图像、视频、音频和数据库等多种数据。实验表明，Bonsai在问答任务和人类对齐方面表现与领域专用黑盒方法相当，同时提供可解释、有依据且考虑不确定性的推理过程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T01:28:22Z
- **目录日期**: 2025-04-08
