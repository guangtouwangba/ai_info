...
基本对象系统（EOS）是一种带有内部Petri网（PN）标记的Petri网模型，用于分析多智能体系统的鲁棒性。虽然EOS的可达性不可判定，但其保守片段（cEOS，内部PN类型不能被完全删除）的可覆盖性已被证明是可判定的，但此前未研究其复杂度。本文首次通过将带数据的νPN编码为cEOS，证明了cEOS可覆盖性的复杂度下界为非原始递归的Fω2级别。这一结果为理解cEOS的计算复杂性提供了重要基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T03:13:53Z
- **目录日期**: 2025-04-08
