...
本文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信号噪声比（g-SNR）调整不同参数组的学习率，有效防止训练不平衡，并将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及大模型（如GPT-2、Llama2-7B）预训练中表现优于或媲美AdamW，同时显著节省内存（最高达25GB）。其鲁棒性和高效性使其成为替代自适应优化器的有力选择，尤其在资源受限场景下更具优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T03:16:20Z
- **目录日期**: 2025-04-08
