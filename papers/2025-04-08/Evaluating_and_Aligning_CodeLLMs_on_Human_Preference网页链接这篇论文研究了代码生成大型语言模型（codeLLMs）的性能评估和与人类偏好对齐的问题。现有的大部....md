...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本(覆盖40类/44种语言)，首次关注模型输出与人类偏好的对齐问题。研究同时构建20B标记的合成指令集SynCode-Instruct验证微调效果。实验发现：1)传统执行正确性基准与人类偏好存在偏差；2)开源最优模型(Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调代码LLMs需同时满足功能正确性和开发者使用偏好，为模型优化提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T04:05:40Z
- **目录日期**: 2025-04-08
