...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令数据集验证微调效果。研究发现：现有执行基准与人类偏好存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面存在显著差距。研究凸显了除代码正确性外，模型输出符合开发者习惯的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T06:07:10Z
- **目录日期**: 2025-04-08
