...
这篇论文针对代码生成大语言模型(codeLLMs)提出了两个关键贡献：1)创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，专门评估模型输出与人类偏好的匹配程度；2)构建了SynCode-Instruct合成指令数据集(约200亿token)验证大规模指令微调效果。研究发现：现有基于执行的评估与人类偏好存在差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面仍有显著差距。研究强调了代码生成模型与人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T03:17:31Z
- **目录日期**: 2025-04-08
