...
本文介绍了AgentSociety，这是一个基于大语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（共500万次）。该平台用于研究四个关键社会问题：社会极化、信息传播、基本收入政策影响和自然灾害冲击。研究发现模拟结果与现实数据高度吻合，验证了该工具在揭示人类行为机制方面的有效性。AgentSociety为社会科学研究和政策制定提供了新方法，能够补充传统研究手段（如调查和访谈），帮助深入理解复杂社会现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T21:05:33Z
- **目录日期**: 2025-04-08
