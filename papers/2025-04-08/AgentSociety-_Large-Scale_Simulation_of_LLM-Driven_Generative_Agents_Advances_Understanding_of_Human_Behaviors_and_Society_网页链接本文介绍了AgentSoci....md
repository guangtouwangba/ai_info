...
这篇论文介绍了AgentSociety，这是一个利用大型语言模型（LLM）驱动的大规模社会模拟器。该平台模拟了超过1万个代理的社会互动（共500万次），并研究了四个关键社会问题：社会极化、信息传播、基本收入政策影响和自然灾害应对。模拟结果与现实世界数据高度吻合，验证了其捕捉人类行为和社会机制的能力。研究表明，AgentSociety可作为社会科学研究和政策制定的有力工具，补充传统研究方法（如调查和实验），帮助深入理解社会问题的成因和潜在解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T08:05:15Z
- **目录日期**: 2025-04-08
