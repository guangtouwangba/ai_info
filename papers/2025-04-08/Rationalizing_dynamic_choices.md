...
本文探讨了分析师如何通过观察代理人的行为序列来判断其是否遵循理性贝叶斯模型。研究表明，当且仅当存在一个无论信息如何都能使代理人获益的单一偏离行为时，观察到的行为才无法被合理化。该结论进一步推广到可能行为序列的概率分布情形，并应用于四个场景：风险厌恶下的理性化单调性、贝叶斯模型的可观测数据拒绝、动态信息设计中的可行结果，以及无需信息假设的偏好部分识别。这些发现为行为理性分析提供了新的理论工具和实证应用方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T14:01:58Z
- **目录日期**: 2025-04-08
