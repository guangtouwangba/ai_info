...
Meta新发布的Llama 4大模型陷入争议。社区爆料称其实际表现与宣传的SOTA水平差距较大，有员工因不满"优化数据"要求而离职。TechCrunch指出Meta用于刷榜的是"实验性聊天特供版"，与开发者可下载的正式版存在明显差异（如频繁使用emoji、回答冗长）。虽然Meta官网标注了榜单模型为实验版，但引发对其真实能力的质疑。官方宣称Llama 4系列（含2万亿参数的Behemoth）在多任务上超越GPT-4o等竞品，但实际性能仍需验证。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T01:31:22Z
- **目录日期**: 2025-04-08
