...
Dion是一种高效的分布式AI训练优化器，通过正交化更新和本地动量缓冲技术大幅降低I/O开销，同时保持标准分布式训练(如DDP、FSDP)的同步语义。相比传统优化器需要同步完整梯度矩阵，Dion避免了全梯度交换，并采用高效的分片策略防止训练过程中重建大型矩阵，显著减少了多加速器间的通信开销。该方法在保持训练效果的同时提升了大规模AI模型训练的通信效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T15:01:59Z
- **目录日期**: 2025-04-08
