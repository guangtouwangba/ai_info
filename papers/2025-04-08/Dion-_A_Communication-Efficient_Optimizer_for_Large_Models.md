...
Dion是一种高效的分布式训练优化器，通过正交化更新和本地动量缓冲减少通信开销，避免全梯度同步。它在保持同步训练语义（如DDP、FSDP）的同时，显著降低I/O成本，并支持分片策略以避免重构大矩阵。相比传统优化器需同步完整梯度矩阵，Dion提升了大规模AI模型训练的加速器间计算效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-08T08:01:09Z
- **目录日期**: 2025-04-08
