...
视频生成技术已从早期不真实的输出发展到能生成视觉逼真、时序连贯的视频。现有评估基准（如VBench）主要关注表层真实性（如帧美学、时序一致性等），但忽略了内在真实性（如物理法则、常识推理等）。为此，研究者提出了VBench-2.0，新一代基准用于自动评估视频生成模型的内在真实性，涵盖五个关键维度：人类逼真度、可控性、创造性、物理合理性和常识性，每个维度细分为更精细的能力。该评估框架结合了通用视觉语言模型和专用异常检测方法，并通过大量标注确保与人类判断一致，旨在为下一代视频生成模型设定新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T05:01:09Z
- **目录日期**: 2025-03-28
