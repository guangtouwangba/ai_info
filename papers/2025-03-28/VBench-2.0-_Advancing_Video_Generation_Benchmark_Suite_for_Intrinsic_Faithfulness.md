...
视频生成技术已从早期的不真实输出发展到能生成视觉逼真且时序连贯的视频。现有评估基准（如VBench）主要关注表面忠实度（如帧美学、时序一致性和基础提示匹配），但忽略了内在真实性（如物理规律、常识推理等）。为此，研究者提出VBench-2.0，通过自动评估模型在人类逼真度、可控性、创造性、物理合理性和常识性五个维度的表现，推动视频生成向"世界模型"进阶。该框架结合通用视觉语言模型和专用异常检测方法，并通过人工标注验证，旨在为下一代视频生成模型建立内在真实性的新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T22:01:02Z
- **目录日期**: 2025-03-28
