...
视频生成技术已显著进步，从早期不真实的输出发展到能生成视觉逼真且时序连贯的视频。现有评测基准（如VBench）主要评估表观忠实度，如帧美学、时序一致性和基础提示匹配，但忽略了生成内容是否符合现实世界规则。为实现真正的"世界模型"，需关注内在忠实度，即视频是否遵守物理定律、常识推理、解剖正确性和结构完整性。为此，研究者推出VBench-2.0，通过五大维度（人类逼真度、可控性、创造性、物理性和常识性）自动评估视频生成模型的内在忠实度，整合先进视觉语言模型和异常检测方法，推动视频生成向更高真实性迈进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T10:00:56Z
- **目录日期**: 2025-03-28
