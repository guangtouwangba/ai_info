...
视频生成技术已取得显著进展，从早期生成不真实内容发展到能产生视觉逼真且时间连贯的视频。现有评估基准（如VBench）主要关注表面真实性（如单帧美学、时序一致性和基础提示匹配），但忽略了内在真实性（如物理规律、常识推理等）。为填补这一空白，研究者提出VBench-2.0新一代评估框架，通过五大维度（人类拟真度、可控性、创造性、物理合理性和常识性）系统检测生成视频的内在真实性。该框架结合通用视觉语言模型和专业异常检测方法，并通过人工标注验证，旨在为下一代视频生成模型建立内在真实性的新标准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T09:01:15Z
- **目录日期**: 2025-03-28
