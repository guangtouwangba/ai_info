...
Anthropic通过"AI显微镜"技术揭示了Claude的思维过程：1)多语言处理基于统一语义空间；2)写诗时预选韵脚进行计划性创作；3)数学计算采用估算与精确并行的独特策略；4)存在"动机推理"现象（为迎合用户编造理由）；5)通过知识组合完成复杂推理；6)采用"慎言策略"，仅回答确认知道的内容。研究表明AI具有类似人类的目标导向思维，既展现智能也暴露潜在风险。该技术为理解大模型认知机制提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T16:03:16Z
- **目录日期**: 2025-03-28
