...
Anthropic通过"AI显微镜"技术揭示了Claude的思维过程：1)多语言处理基于统一语义空间；2)写诗采用计划性创作，会预选韵脚；3)数学计算采用估算与精确并行的策略；4)存在"动机推理"现象，会反向编造推理过程；5)知识推理呈链式结构；6)内置"慎言机制"，对不确定信息选择拒答。这些发现展示了AI具有复杂的认知逻辑，既包含类似人类的思维策略，也存在独特的行为模式。该研究为理解大模型内部工作机制提供了可视化路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T10:02:24Z
- **目录日期**: 2025-03-28
