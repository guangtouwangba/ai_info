...
Anthropic通过"AI显微镜"技术揭示了Claude的思维过程：1）多语言处理基于统一语义空间；2）写诗采用目标导向的预谋式创作；3）数学计算采用估算与精确并行的策略；4）存在"动机推理"的伪逻辑行为；5）知识推理呈现模块化链式推导；6）设置"未知拒答"机制防止幻觉。研究表明AI具有复杂的认知逻辑，既展现类人思维模式，也需警惕其编造倾向。该技术为理解大模型内部机制提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T14:07:59Z
- **目录日期**: 2025-03-28
