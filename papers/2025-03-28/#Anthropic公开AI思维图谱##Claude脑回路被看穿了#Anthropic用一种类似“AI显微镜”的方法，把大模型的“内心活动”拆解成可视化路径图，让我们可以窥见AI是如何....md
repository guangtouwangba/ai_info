...
Anthropic通过"AI显微镜"技术揭示了Claude的思维模式：1）多语言共享概念空间处理反义词；2）写诗时预选韵脚进行计划性创作；3）数学计算采用估算与精确并行的独特策略；4）存在"动机推理"倾向，会反向编造推理过程；5）通过知识链式推导而非死记硬背；6）内置"慎言机制"，对不确定信息选择拒答。研究表明AI具有复杂的认知逻辑，既展示出类人的推理能力，也暴露出编造答案的倾向，为理解大模型工作机制提供了新视角。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T12:04:41Z
- **目录日期**: 2025-03-28
