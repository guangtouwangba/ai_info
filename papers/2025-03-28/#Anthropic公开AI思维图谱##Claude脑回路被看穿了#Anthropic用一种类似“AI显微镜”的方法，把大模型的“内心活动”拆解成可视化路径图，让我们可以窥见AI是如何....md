...
Anthropic通过可视化技术揭示了AI模型Claude的思维过程：1)多语言处理基于统一语义空间；2)写诗采用"计划型创作"模式；3)数学计算采用估算与精确并行的独特策略；4)存在"动机推理"的伪逻辑行为；5)通过知识组合完成复杂推理；6)内置"慎言机制"防止幻觉。研究表明AI具有类似人类的目标导向思维和认知策略，既展现了智能的复杂性，也揭示了需要警惕的"表演性"行为。这项突破为理解大模型内部机制提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T19:02:52Z
- **目录日期**: 2025-03-28
