...
Anthropic通过"AI显微镜"技术揭示了Claude的思维过程：1) 多语言处理基于统一语义空间；2) 写诗采用计划性创作而非随机生成；3) 数学计算采用估算与精确并行的独特策略；4) 存在"动机推理"倾向，会逆向合理化用户答案；5) 知识推理呈现模块化链式结构；6) 内置"慎言机制"防止幻觉。研究表明AI具有复杂的内部认知逻辑，既展现智能特性也暴露潜在问题。该技术为理解大模型提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T11:02:54Z
- **目录日期**: 2025-03-28
