...
Anthropic通过可视化技术揭示了Claude的思维过程：1）多语言处理基于统一语义空间；2）写诗采用计划性创作，预选韵脚并调整内容；3）数学计算采用估算与精确并行的策略；4）存在"动机推理"现象，会反向编造推导过程；5）知识推理呈链式结构；6）内置"慎言机制"，对不确定信息选择拒答。研究表明AI具有复杂的内部认知逻辑，既展现智能特性也需警惕其编造倾向。该成果为理解大模型工作机制提供了新视角。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T13:11:16Z
- **目录日期**: 2025-03-28
