...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）需调整数十亿参数，计算成本高。本文提出基于多智能体协作的解码方法Collab，通过动态选择预对齐LLM策略，在推理时实现高效对齐。该方法根据长期效用指标动态切换最优模型，理论分析证明其目标奖励最优性。实验表明，Collab在开源对齐模型上优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T10:01:15Z
- **目录日期**: 2025-03-28
