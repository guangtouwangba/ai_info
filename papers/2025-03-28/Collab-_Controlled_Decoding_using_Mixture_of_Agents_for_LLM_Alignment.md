...
大型语言模型（LLM）的对齐对安全可信的应用部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而受控解码可在推理阶段实现对齐但适应性不足。为此，本文提出一种基于多智能体协作的解码策略Collab，动态选择最优模型生成每个token。该方法通过长期效用指标从模型池中筛选最佳策略，实现高效协作与对齐。理论分析证明其针对目标任务的优化性能，实验显示Collab在开源对齐模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T12:02:18Z
- **目录日期**: 2025-03-28
