...
大型语言模型（LLM）的对齐对安全部署至关重要。传统强化学习（RLHF）需调整数十亿参数，计算成本高。本文提出一种基于多智能体协作的解码方法Collab，通过动态选择最优模型进行token级对齐，无需重新训练。该方法利用现成对齐策略，根据长期效用指标在解码时切换模型，理论分析证明其最优性。实验表明，Collab在开源模型上优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提高71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T21:01:25Z
- **目录日期**: 2025-03-28
