...
该研究提出了一种基于多智能体协作的解码方法（Collab），用于提升大语言模型（LLM）在推理阶段的对齐性能。传统强化学习人类反馈（RLHF）方法需重新训练数十亿参数，而现有单智能体解码策略难以适应多样化任务。Collab通过动态选择预训练对齐模型池中的最优策略，在token级别实现多模型协作：基于长期效用指标实时切换策略，确保每一步选择最佳模型。理论证明其相对目标奖励函数的最优性，实验表明其平均奖励提升1.56倍，GPT-4胜率提高71.89%，超越当前最优解码基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T07:01:28Z
- **目录日期**: 2025-03-28
