...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而现有单代理解码方法难以适应多样化任务。本文提出基于多代理协作的解码策略（Collab），通过动态选择最优LLM进行逐词生成，实现推理时对齐。该方法利用现成对齐模型，基于长期效用指标动态切换策略，理论分析证明其最优性。实验表明，Collab在开源模型上显著优于单代理基线，平均奖励提升1.56倍，GPT-4评估胜率提升71.89%，成为当前最佳解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T23:01:23Z
- **目录日期**: 2025-03-28
