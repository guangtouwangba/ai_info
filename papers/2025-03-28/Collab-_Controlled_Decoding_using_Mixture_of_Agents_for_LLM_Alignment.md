...
该研究提出了一种基于多智能体协作的解码方法Collab，用于提升大型语言模型（LLM）在推理阶段的对齐性能。不同于传统单智能体解码或需要重训练的RLHF方法，Collab通过动态选择最优模型进行token级生成：将现有对齐模型视为智能体池，根据长期效用指标实时切换策略。理论分析证明其能针对目标奖励函数实现最优性能。实验表明，Collab在开源对齐模型上显著优于单智能体基线，平均奖励提升1.56倍，GPT-4评估胜率提高71.89%，成为当前最先进的解码策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T20:01:31Z
- **目录日期**: 2025-03-28
