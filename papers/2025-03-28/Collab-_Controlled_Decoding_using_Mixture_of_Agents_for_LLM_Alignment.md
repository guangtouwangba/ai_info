...
大型语言模型（LLM）的对齐对安全可信部署至关重要。传统强化学习人类反馈（RLHF）方法计算成本高，而可控解码可在推理时对齐模型但单代理方法难以适应多样化任务。为此，研究者提出基于多代理协作的解码策略，动态选择最优LLM生成每个token，通过长时效用指标实现模型间高效协作。理论分析证明该方法在目标奖励下具有最优性能。实验表明，该策略（Collab）在开源对齐模型上显著优于单代理基线，平均奖励提升1.56倍，GPT-4胜率提高71.89%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T22:01:18Z
- **目录日期**: 2025-03-28
