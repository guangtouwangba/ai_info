...
TripoSG是一种新型3D形状生成扩散模型，通过创新方法解决了当前3D生成在质量、泛化能力和输入对齐方面的挑战。该模型包含三个关键创新：1)基于大规模数据训练的高保真3D流变换器；2)结合SDF、法线和eikonal损失的混合监督3D VAE训练策略；3)可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG能生成细节丰富、与输入图像高度匹配的3D网格，并展现出优异的跨风格生成能力。该模型将公开发布以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T18:02:00Z
- **目录日期**: 2025-03-28
