...
近期扩散模型在图像和视频生成方面取得突破，但3D形状生成仍面临数据规模、处理复杂度等技术瓶颈。研究者提出TripoSG框架：1)采用大规模整流流Transformer提升生成质量；2)结合SDF/法向量/Eikonal损失的混合训练策略优化3D重建；3)构建200万高质量3D样本的数据处理流程。实验表明，该框架能生成细节丰富、与输入图像高度匹配的高保真3D网格，并展现出色的跨风格泛化能力。研究同时揭示了数据质量与规模对3D生成模型的关键作用，相关模型将开源以推动领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T23:01:49Z
- **目录日期**: 2025-03-28
