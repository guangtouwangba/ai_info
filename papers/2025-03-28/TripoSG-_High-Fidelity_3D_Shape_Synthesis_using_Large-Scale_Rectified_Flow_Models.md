...
TripoSG是一种新型3D形状生成框架，通过扩散技术实现高保真3D网格生成，并与输入图像精确对应。该框架包含三个关键创新：1）基于大规模数据训练的高效3D形状生成模型；2）结合SDF、法线和eikonal损失的混合监督训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在3D形状生成上达到最先进水平，能产生高分辨率细节并保持与输入图像的高度一致性，同时展现出对多样化图像风格和内容的强大泛化能力。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T10:01:43Z
- **目录日期**: 2025-03-28
