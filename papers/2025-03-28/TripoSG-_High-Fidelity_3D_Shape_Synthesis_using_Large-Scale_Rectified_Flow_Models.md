...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。其创新包括：1)采用大规模整流流Transformer模型，通过高质量数据训练实现最优生成效果；2)结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建质量；3)开发数据处理流程生成200万高质量3D样本，验证了数据规模和质量对3D生成的关键作用。实验表明，TripoSG在生成细节、输入图像匹配度及多样化风格适应性方面均达到领先水平。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T05:01:55Z
- **目录日期**: 2025-03-28
