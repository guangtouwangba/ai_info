...
TripoSG提出了一种新型3D形状扩散范式，通过三个关键创新解决当前3D生成的质量和泛化能力不足问题：(1)采用大规模整流流变压器模型，利用高质量数据训练实现最优生成效果；(2)结合SDF、法线和eikonal损失的混合监督策略提升3D重建质量；(3)构建包含200万高质量样本的数据处理流程。实验表明，该框架能生成高保真3D网格，精确匹配输入图像，支持多种图像风格，并展现出色的细节表现和泛化能力。研究团队将公开模型以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T09:02:07Z
- **目录日期**: 2025-03-28
