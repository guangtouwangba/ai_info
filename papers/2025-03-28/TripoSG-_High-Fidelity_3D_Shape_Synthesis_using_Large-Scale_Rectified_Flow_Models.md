...
TripoSG是一种新型3D形状生成扩散模型，通过三个创新点实现高质量3D网格生成：1）基于大规模数据训练的可整流流transformer；2）结合SDF、法线和eikonal损失的混合监督训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，该框架能生成细节丰富、与输入图像高度一致的3D形状，并展现出优异的泛化能力，可处理多样化的图像风格和内容。该模型性能达到当前最优水平，其公开将推动3D生成领域的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T06:02:09Z
- **目录日期**: 2025-03-28
