...
TripoSG是一种新型3D形状生成扩散模型，通过创新方法解决了当前3D生成在质量、泛化能力和输入对齐方面的挑战。该模型提出：1）基于大规模校正流变换器的高保真生成架构；2）结合SDF、法线和eikonal损失的混合监督训练策略；3）可产生200万高质量3D样本的数据处理流程。实验表明，TripoSG实现了最先进的3D生成性能，能生成高分辨率细节丰富、与输入图像高度吻合的3D网格，并对多样图像风格展现出优异的泛化能力。该模型将被开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T19:01:44Z
- **目录日期**: 2025-03-28
