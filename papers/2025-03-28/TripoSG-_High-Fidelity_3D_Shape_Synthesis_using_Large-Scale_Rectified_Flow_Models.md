...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型采用：1）基于大规模高质量数据训练的整流流变压器提升生成质量；2）结合SDF、法线和eikonal损失的混合监督训练策略优化3D重建；3）处理200万高质量3D样本的数据管道。实验表明，TripoSG在生成细节和输入图像对齐方面达到最先进水平，并能处理多样图像风格，展现出色泛化能力。该模型将开源以推动3D生成领域发展。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T16:01:59Z
- **目录日期**: 2025-03-28
