...
TripoSG是一种新型3D形状生成扩散模型，通过创新的流变换器架构和混合监督训练策略（结合SDF、法线和Eikonal损失），实现了高保真3D网格生成。该模型利用200万高质量3D样本数据集训练，在生成质量、输入图像对齐和泛化能力方面取得突破。实验表明，TripoSG能生成细节丰富的3D形状，并兼容多种图像风格输入。研究强调了数据质量和数量对3D生成模型的关键作用。该模型性能达到当前最优水平，其开源将推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T13:10:02Z
- **目录日期**: 2025-03-28
