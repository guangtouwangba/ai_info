...
近期扩散技术在图像和视频生成方面取得了显著进展，但3D形状生成技术仍面临数据规模、处理复杂性和技术探索不足等限制。为解决这些问题，研究团队提出了TripoSG这一新型3D形状生成框架。该框架包含三个关键创新：1）基于大规模数据训练的高保真3D形状生成模型；2）结合SDF、法线和eikonal损失的混合监督训练策略；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG能生成与输入图像高度匹配的高分辨率3D网格，在保真度和泛化能力上均达到领先水平。该模型将公开以促进3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T03:14:02Z
- **目录日期**: 2025-03-28
