...
TripoSG是一种新型3D形状生成扩散模型，能根据输入图像生成高保真3D网格。该模型提出三大创新：1）基于大规模数据训练的可整流流变压器，实现最先进的生成质量；2）结合SDF、法线和eikonal损失的混合监督训练策略，提升3D重建性能；3）可生成200万高质量3D样本的数据处理流程。实验表明，TripoSG在细节呈现和输入图像匹配度上表现优异，并能处理多种图像风格，展现出强大的泛化能力。该模型将开源以推动3D生成领域发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T12:02:46Z
- **目录日期**: 2025-03-28
