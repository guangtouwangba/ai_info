...
这篇论文探讨了代码生成大模型（codeLLMs）如何与人类偏好对齐的问题。研究指出当前基准测试过于关注代码正确性，忽略了人类偏好匹配。为此，作者构建了CodeArena基准（含397个高质量样本，覆盖40类44种语言）和SynCode-Instruct语料库（近20B标记），用于评估模型在真实编程场景中的表现。实验发现执行基准与人类偏好评估存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显了人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T12:11:27Z
- **目录日期**: 2025-03-28
