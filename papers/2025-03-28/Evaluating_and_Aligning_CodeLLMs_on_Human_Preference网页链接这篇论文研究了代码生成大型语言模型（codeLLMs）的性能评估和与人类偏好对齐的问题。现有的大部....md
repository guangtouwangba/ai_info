...
这篇论文针对代码生成大模型(codeLLMs)提出新评估方法，指出当前基准测试过于关注代码正确性而忽视人类偏好。研究者构建了CodeArena基准测试(包含397个样本，覆盖40类任务和44种语言)和SynCode-Instruct指令数据集(近200亿标记)，用于评估模型与人类偏好的对齐程度。实验发现执行基准与CodeArena存在性能差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)表现差距显著，凸显了人类偏好对齐的重要性。该研究为代码LLM评估提供了更全面的框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T01:32:58Z
- **目录日期**: 2025-03-28
