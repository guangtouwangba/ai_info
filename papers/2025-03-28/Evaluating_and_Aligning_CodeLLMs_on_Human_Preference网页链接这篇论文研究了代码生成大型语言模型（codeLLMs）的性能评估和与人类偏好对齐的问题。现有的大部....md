...
这篇论文针对代码生成大型语言模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记的SynCode-Instruct合成指令数据集用于模型微调。研究发现现有基于执行的测试与人类偏好存在显著差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面差距明显。研究强调了在代码生成任务中考虑人类偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T22:05:00Z
- **目录日期**: 2025-03-28
