...
这篇论文针对代码生成大模型（CodeLLMs）提出新评估方法，发现现有基准过于侧重代码正确性而忽视人类偏好。研究者构建了CodeArena基准（含397个多语言样本）和SynCode-Instruct合成指令集（20B标记），发现执行基准与人类偏好存在差距。实验对比40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著，凸显对齐人类偏好的重要性。研究为代码生成模型的评估和优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T21:05:10Z
- **目录日期**: 2025-03-28
