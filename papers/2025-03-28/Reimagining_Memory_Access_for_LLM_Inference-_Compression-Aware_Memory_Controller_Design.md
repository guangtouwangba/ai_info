...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩技术(如LZ4和ZSTD)压缩模型权重和键值缓存(KV cache)，在不影响推理质量的情况下，显著减少了内存占用(权重减少25.2%，KV cache减少46.9%)。同时，通过动态量化和LLM感知的内存配置，实现了内存带宽和能耗的按需调节。硬件原型在7nm工艺下达到8TB/s吞吐量，面积开销仅3.8mm²，证明了该方案在大规模推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T09:01:57Z
- **目录日期**: 2025-03-28
