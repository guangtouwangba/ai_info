...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大型语言模型(LLM)推理效率的方法。该方法采用无损块压缩技术(LZ4/ZSTD)压缩模型权重和KV缓存，在不影响推理质量的前提下，显著降低了内存占用(权重减少25.2%，KV缓存减少46.9%)。同时通过动态量化使内存带宽和能耗随上下文自适应调整。硬件原型在7nm工艺下实现了8TB/s吞吐量，面积开销仅3.8mm²。实验证明这种LLM感知的内存控制方案能有效缓解内存瓶颈，是实现高效大规模推理的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T17:01:44Z
- **目录日期**: 2025-03-28
