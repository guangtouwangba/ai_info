...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩(如LZ4和ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的情况下，显著减少了内存占用和带宽消耗。实验结果显示，该方法能减少25.2%的模型权重内存占用和46.9%的KV缓存内存占用。硬件原型在7nm工艺下实现了8TB/s的吞吐量，面积开销仅3.8mm²，证明了LLM感知内存控制是实现高效大规模推理的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T11:01:44Z
- **目录日期**: 2025-03-28
