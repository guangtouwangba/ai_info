...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大型语言模型(LLM)推理效率的方法。该方案采用无损块压缩(如LZ4和ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的情况下，显著减少了内存占用(权重减少25.2%，KV缓存减少46.9%)。同时，通过LLM感知的内存配置实现了动态量化和带宽按需扩展。硬件原型在7nm工艺下实现了8TB/s的高吞吐量，面积开销仅3.8mm²。实验证明这种内存控制方案能有效解决LLM推理中的内存瓶颈问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T05:01:46Z
- **目录日期**: 2025-03-28
