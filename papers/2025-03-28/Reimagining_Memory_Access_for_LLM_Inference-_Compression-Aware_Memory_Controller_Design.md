...
本文提出一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩(如LZ4和ZSTD)技术压缩模型权重和KV缓存，在保持推理质量的同时，显著减少了内存占用(权重减少25.2%，KV缓存减少46.9%)。同时通过LLM感知的内存配置，实现了内存带宽和能耗随上下文动态量化按比例调整。硬件原型在7nm工艺下达到8TB/s吞吐量，面积开销仅3.8mm²，验证了该方案在大规模推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T19:01:36Z
- **目录日期**: 2025-03-28
