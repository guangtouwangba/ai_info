...
本文介绍了GateLens，一种基于大语言模型（LLM）的汽车领域表格数据分析工具。它通过将自然语言查询转换为关系代数表达式并生成优化Python代码，显著提升了分析效率和准确性。相比传统方法，GateLens在基准测试中表现更优，F1分数更高，能稳健处理复杂模糊查询。工业评估显示，该工具可减少80%以上的分析时间，同时保持高准确性和可靠性。实际部署表明，GateLens无需示例即可实现高性能，支持跨角色查询，为汽车软件发布验证提供了快速、可靠的自动化分析方案，提升了系统可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T09:01:26Z
- **目录日期**: 2025-03-28
