...
本文介绍了GateLens，一种基于大语言模型(LLM)的汽车领域表格数据分析工具。它通过将自然语言查询转换为关系代数表达式并生成优化Python代码，显著提升了分析效率。实验表明，GateLens在基准数据集上表现优于基线系统，F1分数更高，能稳健处理复杂模糊查询。工业评估显示其可减少80%以上的分析时间，同时保持高准确性和可靠性。该工具无需few-shot示例即可实现高性能，对不同公司角色的查询类型展现出强大泛化能力，能有效支持汽车系统软件发布验证决策，提升软件可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T07:01:18Z
- **目录日期**: 2025-03-28
