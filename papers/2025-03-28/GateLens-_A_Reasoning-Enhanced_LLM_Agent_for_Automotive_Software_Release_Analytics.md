...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式并生成优化的Python代码，显著优于基准系统，F1分数更高且能更稳健处理复杂模糊查询。消融实验证实关系代数模块的关键作用，工业评估显示其减少80%以上的分析时间，同时保持高准确性和可靠性。GateLens无需依赖少量示例即实现高性能，在不同公司角色的多样化查询中展现出强大泛化能力。实际部署表明，该工具可自动化测试结果分析，加快发布决策并提升汽车系统软件的可靠性和可扩展性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T12:02:08Z
- **目录日期**: 2025-03-28
