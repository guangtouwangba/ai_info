...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式并生成优化的Python代码，显著提升了复杂查询的处理能力。相比基线系统，它在基准测试中取得了更高的F1分数，工业评估显示其分析时间减少了80%以上，同时保持高准确性和可靠性。该工具无需依赖少量示例即表现出强大的泛化能力，支持跨部门多样化查询。实际部署证明，GateLens能自动化测试结果分析，帮助实现更快、更可靠的软件发布决策，提升汽车系统的可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T13:09:19Z
- **目录日期**: 2025-03-28
