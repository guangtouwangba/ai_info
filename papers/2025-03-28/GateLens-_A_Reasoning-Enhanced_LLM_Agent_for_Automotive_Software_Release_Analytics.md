...
本文介绍了GateLens，一种基于大语言模型（LLM）的工具，用于分析汽车领域的表格数据。GateLens将自然语言查询转换为关系代数表达式，并生成优化的Python代码，显著提升了复杂查询处理的准确性和鲁棒性。相比传统方法，它减少了80%以上的分析时间，同时保持高精度和可靠性。工业评估表明，GateLens无需依赖少量示例即可实现高性能，适用于不同角色的多样化查询。该工具通过自动化测试结果分析，帮助汽车系统实现更快、更可靠的软件发布决策，提升了软件的可扩展性和可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T05:01:19Z
- **目录日期**: 2025-03-28
