...
在线视频大语言模型（Video LLMs）的关键能力是时间感知（Temporal Awareness），即根据问题提出的时间戳动态推理。现有基准测试未能充分评估这一能力，为此研究者提出了OVO-Bench新基准，包含12项任务、644个视频和约2800个人工标注的精细时间戳元数据。该基准测试模型在三种场景下的表现：(1)回溯过去事件；(2)实时理解当前事件；(3)延迟响应以获取未来信息。评估9个现有模型发现，尽管在传统基准上有进步，但它们在在线视频理解方面仍显著落后于人类水平。该研究旨在推动视频LLMs的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T23:01:57Z
- **目录日期**: 2025-03-28
