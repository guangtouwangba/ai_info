...
OVO-Bench是一个新型在线视频基准测试，旨在评估视频大语言模型（LLMs）的时间感知能力。该能力使在线模型能根据提问时间戳动态处理视频流，而离线模型只能静态分析完整视频。OVO-Bench通过12项任务（含644个视频和2800条精细标注）测试模型在三种场景的表现：回溯过去事件、实时理解当前事件、延迟响应以获取未来信息。评估显示，现有模型在在线视频理解上仍显著落后于人类。该基准希望推动视频LLMs发展，促进在线视频推理研究。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T17:02:02Z
- **目录日期**: 2025-03-28
