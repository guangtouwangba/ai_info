...
OVO-Bench是一个新型视频基准测试，旨在评估在线视频大语言模型（LLMs）的时间感知能力。该能力使模型能根据提问时间戳动态调整响应，区分在线（实时处理视频流）和离线（静态分析完整视频）模型。OVO-Bench包含12项任务、644个视频和约2800条精细标注，测试模型在三种场景下的表现：回溯过去事件、实时理解当前事件、延迟响应以获取未来信息。评估显示，当前模型在在线视频理解上仍显著落后于人类。该基准希望推动视频LLMs发展，促进在线视频推理研究。项目代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T21:02:01Z
- **目录日期**: 2025-03-28
