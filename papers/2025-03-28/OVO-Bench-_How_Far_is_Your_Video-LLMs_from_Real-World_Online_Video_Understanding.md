...
OVO-Bench是一个新型在线视频基准测试，旨在评估视频大语言模型（Video-LLM）的时间感知能力。与依赖完整视频的离线模型不同，在线模型需根据提问时间戳动态处理视频流。该基准测试包含12项任务，覆盖644个独特视频和约2800条精细标注，评估模型在三种场景下的表现：（1）回溯过去事件；（2）实时理解当前事件；（3）延迟响应以获取未来信息。测试发现，现有模型在在线视频理解方面表现不佳，与人类水平差距显著。OVO-Bench希望推动视频LLM的发展，促进在线视频推理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T08:09:43Z
- **目录日期**: 2025-03-28
