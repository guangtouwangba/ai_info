...
OVO-Bench是一个新型视频基准测试，专注于评估在线视频大语言模型（LLMs）的时间感知能力——即根据问题时间戳动态推理的能力。该基准通过12项任务（含644个视频和2800条精细标注）测试模型在三种场景下的表现：回溯过去事件、实时理解当前事件、延迟响应以利用未来信息。研究发现，尽管现有模型在传统基准上表现良好，但在在线视频理解方面仍显著落后于人类。OVO-Bench旨在推动视频LLMs的发展，促进在线视频推理研究。数据和代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T06:02:18Z
- **目录日期**: 2025-03-28
