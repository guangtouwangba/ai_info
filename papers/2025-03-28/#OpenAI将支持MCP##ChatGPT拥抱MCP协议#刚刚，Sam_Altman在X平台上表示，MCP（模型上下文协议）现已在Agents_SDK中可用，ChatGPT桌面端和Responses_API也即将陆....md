...
OpenAI宣布将在ChatGPT桌面端和Responses API中支持MCP协议（模型上下文协议）。MCP由Anthropic推出，是AI模型与外部数据交互的开放标准，被称为"AI界的USB-C"。该协议允许AI直接访问实时数据源，提升响应准确性。此前Replit等公司已接入，OpenAI的加入进一步巩固了MCP作为AI集成标准的地位。Anthropic表示MCP已有数千个集成且持续增长。这一举措将使AI助手更好地理解用户实时数据，提供更精准的服务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T02:30:56Z
- **目录日期**: 2025-03-28
