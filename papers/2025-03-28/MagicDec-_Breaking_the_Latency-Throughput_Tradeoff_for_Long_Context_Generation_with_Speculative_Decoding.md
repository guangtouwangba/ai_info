...
MagicDec研究表明，推测解码（SD）技术能够显著提升大语言模型（LLMs）在长上下文场景下的推理效率。与传统认知不同，该技术不仅适用于小批量处理，在中长序列的高吞吐量推理中也能实现加速。通过采用稀疏KV缓存缓解计算瓶颈，并结合智能草拟策略优化批量处理，系统在Llama3.1-8B模型上实现了最高2.51倍的加速效果（批量32-256）。这项工作突破了推测解码的应用边界，证明其可在保持精度的同时，有效降低长上下文服务的延迟并提升吞吐量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T01:29:31Z
- **目录日期**: 2025-03-28
