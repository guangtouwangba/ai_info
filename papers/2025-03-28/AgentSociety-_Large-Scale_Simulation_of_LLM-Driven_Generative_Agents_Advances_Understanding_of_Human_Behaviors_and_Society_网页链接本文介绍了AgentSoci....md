...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，可模拟1万多个AI代理的社会互动（500万次交互）。研究者用它作为计算社会科学实验平台，研究了四大社会问题：社会极化、刺激性信息传播、基本收入政策影响和自然灾害（如飓风）冲击。模拟结果与现实世界数据高度吻合，证明该系统能有效捕捉人类行为机制。这显示了大语言模型驱动模拟器作为社会科学研究和政策制定新工具的潜力，可辅助传统研究方法（如调查和干预）分析社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T14:17:15Z
- **目录日期**: 2025-03-28
