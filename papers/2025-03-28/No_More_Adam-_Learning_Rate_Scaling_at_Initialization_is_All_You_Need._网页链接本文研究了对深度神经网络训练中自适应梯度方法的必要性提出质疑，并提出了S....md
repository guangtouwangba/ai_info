...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法的传统认知。该方法基于梯度信号噪声比(g-SNR)在初始化时调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡，并将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和LLM预训练(GPT-2/Llama2)中表现优于或持平AdamW，在LoRA微调等任务也超越现有优化器。内存效率显著提升，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，同时保持超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T17:04:18Z
- **目录日期**: 2025-03-28
