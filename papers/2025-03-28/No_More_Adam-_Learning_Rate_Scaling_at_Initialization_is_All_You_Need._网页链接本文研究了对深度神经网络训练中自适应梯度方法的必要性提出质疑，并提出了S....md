...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过基于梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，内存效率显著优于AdamW（节省5.93GB~25.15GB），且在LoRA微调等任务中超越现有优化器，同时具备超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T02:31:20Z
- **目录日期**: 2025-03-28
