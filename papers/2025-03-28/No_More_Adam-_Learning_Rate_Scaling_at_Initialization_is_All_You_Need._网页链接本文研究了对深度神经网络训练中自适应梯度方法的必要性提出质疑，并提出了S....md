...
该论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer、ImageNet分类和GPT-2预训练等任务中表现相当或更优，尤其在大模型训练中显著节省内存（如为Llama2-7B节省25.15GB）。同时，该方法对超参数变化鲁棒，在LoRA微调等场景也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T21:04:06Z
- **目录日期**: 2025-03-28
