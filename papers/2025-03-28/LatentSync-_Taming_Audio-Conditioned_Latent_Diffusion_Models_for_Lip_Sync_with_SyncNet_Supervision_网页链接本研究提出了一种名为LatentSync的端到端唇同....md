...
本研究提出LatentSync框架，通过音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。为解决时间一致性问题，开发了TREPA方法，利用自监督视频模型对齐生成帧。同时改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。创新点包括：1)直接建模音频-视觉关系；2)增强生成连贯性；3)优化SyncNet训练。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-28T22:04:43Z
- **目录日期**: 2025-03-28
