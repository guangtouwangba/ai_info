...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它研究四个社会问题：社会极化、信息传播、基本收入政策影响和飓风等外部冲击。模拟结果与现实数据吻合，验证了该工具捕捉人类行为机制的能力。该系统可作为计算社会科学实验平台，辅助传统研究方法（如调查、访谈），帮助研究者分析社会问题的模式和成因，为政策制定提供支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T22:04:07Z
- **目录日期**: 2025-05-31
