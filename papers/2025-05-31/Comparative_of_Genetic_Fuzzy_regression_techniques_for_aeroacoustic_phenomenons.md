...
本研究探讨了遗传模糊系统(GFS)在模拟翼型自噪声中的应用，该问题对航空航天、汽车和无人机领域具有重要意义。作者基于公开的翼型噪声数据集，比较了多种模糊回归方法，包括高规则密度的TSK模糊系统、级联遗传模糊树(GFT)架构，以及创新的基于模糊C均值(FCM)的聚类方法。结果表明，聚类辅助的模糊推理能有效降低模型复杂度，是处理复杂气动声学现象的有力工具。研究突出了模糊逻辑在气动声学回归建模中的实用价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T04:02:34Z
- **目录日期**: 2025-05-31
