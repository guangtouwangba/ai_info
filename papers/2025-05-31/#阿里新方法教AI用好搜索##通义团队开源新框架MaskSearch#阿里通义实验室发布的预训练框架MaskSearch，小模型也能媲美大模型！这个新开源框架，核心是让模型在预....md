...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让小模型也能媲美大模型性能。该框架创新性地将搜索引擎整合到预训练中，迫使模型通过搜索获取外部知识来补全信息。采用多智能体+教师模型的监督微调(SFT)和混合奖励强化学习(RL)双训练模式，配合课程学习策略，显著提升模型泛化能力。实验显示，在跨领域问答任务中，采用MaskSearch的小模型表现接近大模型水平。研究还揭示了掩码策略和奖励函数对性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T08:02:50Z
- **目录日期**: 2025-05-31
