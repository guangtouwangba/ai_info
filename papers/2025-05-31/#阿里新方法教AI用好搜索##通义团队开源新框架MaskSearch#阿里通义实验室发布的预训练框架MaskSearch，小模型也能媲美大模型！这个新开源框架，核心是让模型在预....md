...
阿里通义实验室开源了新型预训练框架MaskSearch，其核心创新是将检索增强训练融入预训练过程。该框架基于BERT的掩码预测思路，但强制模型通过搜索获取外部知识来补全信息（RAMP任务）。特色在于同时支持监督微调（采用多智能体+教师模型生成思维链）和强化学习（混合奖励系统优化推理路径），并引入课程学习策略提升泛化能力。实验显示，该方法使小模型在跨领域问答任务上达到媲美大模型的性能，同时验证了掩码策略和奖励函数设计对模型学习效果的关键影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T21:02:34Z
- **目录日期**: 2025-05-31
