...
阿里通义实验室开源了新型预训练框架MaskSearch，创新性地将搜索能力融入模型训练过程。该框架基于BERT的掩码预测思路，通过RAMP任务（检索增强型掩码预测）训练模型利用搜索引擎补全信息。其特色在于：1）同时支持监督微调和强化学习训练；2）采用多智能体+教师模型生成思维链；3）设计混合奖励系统优化推理路径；4）引入课程学习提升泛化能力。实验显示，采用MaskSearch的小模型在跨领域任务中可媲美大模型性能，且不同掩码策略和奖励函数对效果有显著影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T11:02:40Z
- **目录日期**: 2025-05-31
