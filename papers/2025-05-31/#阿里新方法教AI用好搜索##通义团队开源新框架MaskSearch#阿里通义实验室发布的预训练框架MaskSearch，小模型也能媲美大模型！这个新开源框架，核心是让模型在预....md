...
阿里通义实验室开源了MaskSearch预训练框架，通过"检索增强型掩码预测"(RAMP)技术，让AI模型在训练时学会结合搜索引擎进行推理。该框架创新性地融合了监督微调(SFT)和强化学习(RL)方法：SFT阶段采用多智能体生成思维链，RL阶段设计混合奖励机制优化推理路径。配合课程学习策略，使小模型在问答任务中表现媲美大模型，尤其在跨领域任务上展现出色泛化能力。不同掩码策略和奖励函数对性能影响显著，验证了"教会模型如何学习"的技术价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T20:02:40Z
- **目录日期**: 2025-05-31
