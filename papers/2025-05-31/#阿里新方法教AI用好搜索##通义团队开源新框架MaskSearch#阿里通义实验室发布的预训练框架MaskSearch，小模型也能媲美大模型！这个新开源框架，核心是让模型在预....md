...
阿里通义实验室开源了预训练框架MaskSearch，其核心创新是将搜索能力融入模型预训练过程。该框架基于BERT的掩码预测思路，但要求模型通过搜索获取外部知识来补全信息（RAMP任务）。MaskSearch兼容监督微调和强化学习训练，采用多智能体生成思维链和混合奖励机制优化推理路径，并引入课程学习策略提升泛化能力。实验显示，该方法使小模型在问答任务中达到媲美大模型的效果，尤其在跨领域任务表现突出。研究还揭示了掩码策略和奖励函数对性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T19:02:49Z
- **目录日期**: 2025-05-31
