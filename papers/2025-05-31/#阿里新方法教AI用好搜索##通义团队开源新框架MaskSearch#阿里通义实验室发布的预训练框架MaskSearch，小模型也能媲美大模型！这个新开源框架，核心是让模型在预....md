...
阿里通义实验室开源了MaskSearch预训练框架，创新性地将搜索能力融入模型训练。该框架基于BERT的掩码预测思路，但要求模型通过搜索获取外部知识来补全信息（RAMP任务）。其特点包括：1）兼容监督微调和强化学习训练；2）采用多智能体+教师模型生成思维链；3）设计混合奖励系统优化推理路径；4）引入课程学习提升泛化能力。实验显示，该方法使小模型在多个问答任务上媲美大模型性能，尤其在跨领域任务表现突出。研究表明掩码策略和奖励函数设计对模型学习效果影响显著。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T22:02:24Z
- **目录日期**: 2025-05-31
