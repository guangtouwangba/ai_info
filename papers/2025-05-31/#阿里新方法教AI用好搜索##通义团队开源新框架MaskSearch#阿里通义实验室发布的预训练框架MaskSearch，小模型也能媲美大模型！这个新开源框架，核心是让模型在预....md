...
阿里通义实验室开源了MaskSearch预训练框架，通过检索增强型掩码预测(RAMP)技术，让AI模型学会在推理过程中主动使用搜索引擎获取外部知识。该框架创新性地结合监督微调(SFT)和强化学习(RL)训练方式，采用多智能体+教师模型生成思维链，并设计混合奖励系统优化推理路径。实验显示，采用课程学习策略的MaskSearch能显著提升小模型性能，在跨领域任务上可媲美大模型。研究还揭示了不同掩码策略和奖励函数对模型性能的重要影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T09:02:46Z
- **目录日期**: 2025-05-31
