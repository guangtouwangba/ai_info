...
阿里通义实验室开源MaskSearch框架，通过检索增强型掩码预测(RAMP)技术，让AI模型在预训练阶段学会结合搜索引擎进行推理。该框架创新性地融合监督微调(SFT)和强化学习(RL)训练方式：SFT采用多智能体生成思维链，RL设计混合奖励机制优化推理路径，并引入课程学习提升泛化能力。实验表明，该方法使小模型在跨领域问答任务中表现媲美大模型，同时揭示了掩码策略和奖励函数对性能的关键影响。该研究为提升模型推理能力提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T14:02:47Z
- **目录日期**: 2025-05-31
