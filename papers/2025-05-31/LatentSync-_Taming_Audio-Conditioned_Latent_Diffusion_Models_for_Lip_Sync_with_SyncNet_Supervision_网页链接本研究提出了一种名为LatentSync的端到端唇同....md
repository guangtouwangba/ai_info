...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示，克服了传统方法的时间一致性问题。关键创新包括：1) 时间表示对齐(TREPA)技术，利用自监督视频模型增强生成帧的时间一致性；2) 对SyncNet训练框架的系统改进，提升了其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进不仅适用于唇同步任务，也可推广到其他音频驱动的人像动画应用中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T04:06:23Z
- **目录日期**: 2025-05-31
