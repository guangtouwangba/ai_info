...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示，克服了传统两阶段方法的局限。针对扩散模型的时间一致性问题，提出TREPA技术，利用自监督视频模型增强帧间一致性。同时改进了SyncNet训练框架，通过优化架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些创新不仅提升了唇同步效果，也为相关音频驱动动画研究提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T21:04:26Z
- **目录日期**: 2025-05-31
