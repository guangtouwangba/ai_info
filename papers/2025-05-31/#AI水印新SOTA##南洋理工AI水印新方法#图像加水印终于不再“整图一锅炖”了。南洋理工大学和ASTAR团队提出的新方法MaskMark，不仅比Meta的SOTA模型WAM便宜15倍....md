...
南洋理工大学与A*STAR团队提出新型AI水印方法MaskMark，通过"掩码机制"实现精准水印嵌入与提取。该方法分两种版本：MaskMark-D支持全图嵌入局部提取，MaskMark-ED专用于指定区域保护。相比Meta的WAM模型，训练成本降低15倍（单卡20小时完成），在高保真度下仍保持近100%提取准确率，尤其在小区域水印任务中表现突出。不仅能准确定位水印（IoU指标领先），还意外支持同时嵌入5个水印。具备快速微调能力，可灵活应对不同比特长度需求及新型攻击。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T20:02:31Z
- **目录日期**: 2025-05-31
