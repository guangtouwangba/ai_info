...
这项研究将神经网络转换为C语言逻辑电路，速度提升1744倍。开发者训练了一个使用逻辑门组合（如and/or/xor）而非ReLU激活函数的神经网络来学习康威生命游戏规则，随后将其编译为300多行支持bit级并行运算的C代码。该方法属于"可微分逻辑门网络(DLGN)"领域，通过连续松弛使逻辑门可训练，最终离散化为高效电路。用uint64_t类型实现64个单元并行计算，避免了浮点运算和GPU开销，实现"神经网络学习，C语言执行"的高效推理模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T02:32:11Z
- **目录日期**: 2025-05-31
