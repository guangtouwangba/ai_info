...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，有效解决了训练不平衡问题，同时将优化器内存占用减半。实验表明，该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或相当于AdamW，特别在大模型训练中可节省显著内存（如为Llama2-7B节省25.15GB）。该方法还具有超参数鲁棒性，在LoRA微调等任务中表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T14:04:27Z
- **目录日期**: 2025-05-31
