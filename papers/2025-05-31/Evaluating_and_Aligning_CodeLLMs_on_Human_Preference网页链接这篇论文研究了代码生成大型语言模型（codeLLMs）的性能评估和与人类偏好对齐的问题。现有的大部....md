...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1)创建CodeArena基准测试，包含397个真实编程任务样本(覆盖40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度；2)构建20B标记量的SynCode-Instruct合成指令集验证微调效果。研究发现传统执行测试与人类偏好存在显著差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类对齐方面差距明显。研究强调了在代码生成领域进行人类偏好对齐的重要性，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T23:04:42Z
- **目录日期**: 2025-05-31
