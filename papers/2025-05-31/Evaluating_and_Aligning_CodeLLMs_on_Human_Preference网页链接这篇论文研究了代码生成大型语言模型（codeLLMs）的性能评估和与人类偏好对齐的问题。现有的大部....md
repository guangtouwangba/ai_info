...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试(包含397个样本，覆盖40类任务和44种语言)来评估模型在真实编程场景中的表现，同时构建了20B标记的SynCode-Instruct合成指令集。实验发现：1)传统执行测试与人类偏好存在差距；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著。研究强调了代码LLMs不仅需要正确性，还需符合人类编程习惯的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T17:04:42Z
- **目录日期**: 2025-05-31
