...
这篇论文研究了代码生成大模型(codeLLMs)与人类偏好的对齐问题，提出CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种语言)来评估模型在真实编程场景中的表现。研究还构建了20B标记的合成指令数据集SynCode-Instruct用于微调验证。实验发现传统执行基准与CodeArena存在性能差异，并显示开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在显著差距，强调了优化这一维度的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T19:05:11Z
- **目录日期**: 2025-05-31
