...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究收集了397个涵盖40类编程任务和44种语言的真实样本，并构建20B标记的合成指令集SynCode-Instruct。实验发现：1)传统执行正确性测试与人类偏好存在偏差；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在显著差距。研究强调了大模型在代码生成中除正确性外，还需重视实际开发场景中的可用性需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T06:05:19Z
- **目录日期**: 2025-05-31
