...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，通过397个涵盖40类编程任务和44种语言的真实样本，首次系统评估模型输出与人类偏好的匹配度。研究同时构建20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现：1)传统执行正确性基准与人类偏好存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面差距明显。研究凸显了代码生成模型除功能性外，还需重视开发者体验和实际需求匹配的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T08:05:22Z
- **目录日期**: 2025-05-31
