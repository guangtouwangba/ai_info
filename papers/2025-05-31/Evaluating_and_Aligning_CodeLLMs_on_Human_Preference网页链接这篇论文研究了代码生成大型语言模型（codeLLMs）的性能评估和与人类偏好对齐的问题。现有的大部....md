...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究团队从真实用户查询中精选397个样本，覆盖40类任务和44种编程语言，比传统仅测试代码正确性的评估更全面。同时构建了20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现执行基准与人类偏好评估存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著，凸显对齐人类偏好的重要性。该研究为提升代码生成模型实用性提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T20:05:02Z
- **目录日期**: 2025-05-31
