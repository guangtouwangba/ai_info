...
这篇论文提出了一种新的在线公平分配问题变体，针对多个代理场景，学习者需顺序观察不可分割物品并将其不可撤销地分配给一个代理，同时满足公平性和效率约束。现有算法假设物品数量少且副本足够多，能通过嘈杂的强盗反馈准确估计所有物品-代理对的效用。然而，现实中许多应用（如在线平台拥有大量用户但每个用户仅使用少量服务）难以满足该假设。为此，作者假设效用是物品-代理特征的未知函数，将问题建模为上下文强盗问题，提出了具有次线性遗憾保证的算法，实验验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T00:02:38Z
- **目录日期**: 2025-05-31
