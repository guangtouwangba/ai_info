...
Nature报道了一种新型超薄电子纹身传感器，可贴在头部监测脑负荷状态。该设备结合脑电图(EEG)和眼动图(EOG)技术，通过分析θ/δ波(用脑强度)和α/β波(疲劳程度)的变化，能实时监测并预测大脑过载。整套系统约200美元，单次使用贴片20美元。实验显示其能准确识别"最优脑负荷"状态，并提前发出脑力预警。研究团队正开发适用于有头发区域的版本，目标是使脑疲劳监测像测心率一样便捷。这项技术有望应用于日常认知状态管理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T06:03:13Z
- **目录日期**: 2025-05-31
