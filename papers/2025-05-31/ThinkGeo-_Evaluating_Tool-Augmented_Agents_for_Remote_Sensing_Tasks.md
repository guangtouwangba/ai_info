...
ThinkGeo是一个专门评估大语言模型（LLM）在遥感任务中工具使用能力的基准测试平台。它涵盖城市规划、灾害评估、环境监测等实际应用场景，要求模型通过多步推理和结构化工具交互处理卫星/航空影像数据。该研究测试了开源和闭源模型（如GPT-4o、Qwen2.5）在436项任务中的表现，记录执行步骤和最终答案准确性。结果显示不同模型在工具精度和规划一致性方面存在显著差异。ThinkGeo首次为遥感领域的空间推理能力提供了系统评估框架，相关代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T19:01:17Z
- **目录日期**: 2025-05-31
