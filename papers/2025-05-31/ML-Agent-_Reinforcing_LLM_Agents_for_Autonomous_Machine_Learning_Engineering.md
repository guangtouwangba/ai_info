...
本文提出了一种基于强化学习（RL）的自主机器学习（ML）智能体训练框架ML-Agent，通过三个创新组件实现：1）探索增强微调，提升LLM智能体的动作多样性；2）分步RL训练，加速经验收集；3）统一的奖励模块。实验表明，仅用7B参数的Qwen-2.5模型在9个ML任务上训练后，性能超越671B参数的DeepSeek-R1智能体，并展现出持续的自我提升和优异的跨任务泛化能力。该研究首次实现了基于学习的ML智能体范式，突破了传统依赖人工提示工程的局限。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-31T01:29:06Z
- **目录日期**: 2025-05-31
