...
Ugo Pradère团队探索了利用Gemini模型实现生产级录音转写的方案。短音频转写效果良好，但长音频面临Token限制、输出质量下降和重复循环等问题。团队采用重叠分段策略（30秒重叠），通过LLM合并分段文本，显著提升了时间戳精度（1小时音频误差5-10秒）。最终方案将10分钟音频分三部分处理，单独合并重叠段后再重组，避免了循环错误。该方法在保持内容连贯性的同时，实现了高精度、可扩展的转写流程，但长音频仍需分段处理以克服模型限制。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T19:02:23Z
- **目录日期**: 2025-04-30
