...
Graphiti是由Zep团队开发的开源实时知识图谱系统，专为AI智能体设计。它能动态整合对话、企业数据和外部信息，支持秒级更新和双时间线追踪（事件发生/接收时间）。相比RAG技术，具有增量更新、混合检索（语义+关键词+图遍历）和可扩展关系等优势。应用场景包括电商客服（个性化推荐）、金融分析（市场追踪）和智能助手（实时决策）。项目已在GitHub开源，获7k+星标，文档完善。核心价值是为AI提供持续进化的"动态大脑"，解决传统知识库更新滞后问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T01:31:24Z
- **目录日期**: 2025-04-30
