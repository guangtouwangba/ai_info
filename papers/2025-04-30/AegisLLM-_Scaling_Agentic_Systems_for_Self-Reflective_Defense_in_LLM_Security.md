...
AegisLLM是一种基于多智能体协作的防御系统，用于对抗对抗性攻击和信息泄露。该系统由协调器、偏转器、响应器和评估器等自主智能体组成，通过协同工作确保大语言模型输出的安全性和合规性，并通过即时优化实现自我改进。实验表明，AegisLLM在无需重新训练模型的情况下，能有效应对多种威胁场景。在WMDP遗忘基准测试中，仅用20个训练样本和不到300次语言模型调用就实现了近乎完美的遗忘效果；在越狱测试中，性能比基线模型提升51%，误拒率仅为7.9%。该系统展现了动态防御相对于静态方法的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T04:01:49Z
- **目录日期**: 2025-04-30
