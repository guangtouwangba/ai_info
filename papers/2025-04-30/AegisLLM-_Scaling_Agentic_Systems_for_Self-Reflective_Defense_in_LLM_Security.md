...
AegisLLM是一个多智能体协作防御系统，用于对抗对抗性攻击和信息泄露。它通过四个自主智能体（协调器、偏转器、响应器和评估器）的协同工作流程，确保大模型输出的安全合规性，并通过即时优化实现自我改进。该系统在测试时扩展智能体推理能力，显著提升了鲁棒性且不影响模型效用，无需重新训练即可实时适应新型攻击。在WMDP遗忘基准测试中，仅需20个训练样本和不到300次调用即实现近乎完美的遗忘；在越狱测试中，误拒率仅为7.9%，优于同类方法（18-55%）。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T06:01:57Z
- **目录日期**: 2025-04-30
