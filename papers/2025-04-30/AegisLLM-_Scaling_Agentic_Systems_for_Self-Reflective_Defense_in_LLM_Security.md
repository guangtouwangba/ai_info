...
AegisLLM是一个多智能体协作防御系统，旨在对抗对抗性攻击和信息泄露。该系统通过协调器、偏转器、响应器和评估器等自主智能体的结构化工作流程，确保大语言模型输出的安全合规，并通过提示优化实现自我改进。研究表明，测试时扩展智能体推理系统（如增加角色或自动化提示优化）可显著提升鲁棒性而不影响模型效用。AegisLLM在WMDP遗忘基准测试中仅需20个训练样本和不到300次LM调用即实现近乎完美的遗忘，在越狱基准测试中性能提升51%，误拒率仅7.9%，优于传统静态防御方法。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T11:01:47Z
- **目录日期**: 2025-04-30
