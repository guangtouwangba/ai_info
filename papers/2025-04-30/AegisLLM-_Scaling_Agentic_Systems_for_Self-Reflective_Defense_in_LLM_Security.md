...
AegisLLM是一种基于多智能体协作的防御系统，用于对抗对抗性攻击和信息泄露。该系统包含四个自主智能体（协调器、偏转器、响应器和评估器），通过结构化工作流程确保大语言模型输出的安全合规，并利用提示优化实现自我改进。测试时扩展智能体推理系统（如增加角色或使用DSPy自动优化）显著提升了鲁棒性且不影响模型效用。实验表明，在WMDP遗忘基准测试中仅需20个样本和不到300次调用即实现近乎完美的遗忘；在越狱测试中比基线模型提升51%防御效果，误拒率仅7.9%。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T16:01:56Z
- **目录日期**: 2025-04-30
