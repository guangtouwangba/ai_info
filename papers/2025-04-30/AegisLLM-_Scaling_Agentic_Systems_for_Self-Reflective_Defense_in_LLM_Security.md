...
AegisLLM是一种基于多智能体协作的防御系统，用于对抗对抗性攻击和信息泄露。该系统通过四个自主智能体（协调器、偏转器、响应器和评估器）的协同工作流程，确保大语言模型输出的安全合规性，并通过即时优化实现自我改进。实验表明，AegisLLM在测试时扩展智能体推理系统能显著增强鲁棒性，且无需模型重新训练。在WMDP遗忘基准测试中，仅需20个训练样本和不到300次LM调用即实现近乎完美的遗忘效果；在越狱基准测试中，性能提升51%，误拒率仅为7.9%。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T05:01:42Z
- **目录日期**: 2025-04-30
