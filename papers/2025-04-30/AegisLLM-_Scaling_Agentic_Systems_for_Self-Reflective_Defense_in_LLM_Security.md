...
AegisLLM是一种基于多智能体协作的防御系统，用于对抗对抗性攻击和信息泄露。该系统由协调器、偏转器、响应器和评估器等自主智能体组成，通过协作确保大语言模型输出的安全性和合规性，并通过提示优化实现自我改进。实验表明，AegisLLM在不影响模型性能的前提下显著提升了鲁棒性，无需重新训练即可实时适应攻击。在WMDP遗忘基准测试中，仅需20个训练样本和不到300次模型调用即实现近乎完美的遗忘效果；在越狱测试中，误拒率仅为7.9%，优于同类方法。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T18:01:53Z
- **目录日期**: 2025-04-30
