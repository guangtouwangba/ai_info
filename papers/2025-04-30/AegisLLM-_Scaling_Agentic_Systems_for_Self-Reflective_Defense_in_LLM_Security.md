...
AegisLLM是一个多智能体协作防御系统，用于对抗对抗性攻击和信息泄露。该系统通过协调器、偏转器、响应器和评估器四个自主智能体的结构化工作流程，确保大语言模型输出的安全合规，并通过即时优化实现自我改进。研究表明，AegisLLM在不影响模型性能的前提下显著提升了鲁棒性，无需重新训练即可实时适应新型攻击。在WMDP遗忘基准测试中仅需20个训练样本和不到300次调用；在越狱测试中性能提升51%，误拒率仅7.9%。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T22:01:45Z
- **目录日期**: 2025-04-30
