...
AegisLLM是一个多智能体协作防御系统，用于对抗对抗性攻击和信息泄露。该系统通过四个自主智能体（协调器、偏转器、响应器和评估器）的协同工作，确保大语言模型输出的安全合规，并通过即时优化实现自我改进。研究表明，通过在测试时扩展智能体推理系统（增加角色或利用自动提示优化），AegisLLM显著提升了鲁棒性而不影响模型效用。在WMDP遗忘基准测试中，仅需20个训练样本和不到300次语言模型调用即实现近乎完美的遗忘效果；在越狱基准测试中，性能提升51%，误拒率仅7.9%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T07:01:47Z
- **目录日期**: 2025-04-30
