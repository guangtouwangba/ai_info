...
AegisLLM是一个多智能体协作防御系统，用于抵御对抗性攻击和信息泄露。它通过四个自主智能体（协调器、偏转器、响应器和评估器）的协同工作，确保大模型输出的安全合规，并通过提示优化实现自我改进。该系统在测试时扩展智能体推理能力，显著提升鲁棒性而不影响模型效用，且无需重新训练模型。在WMDP遗忘基准测试中，仅用20个训练样本和不到300次模型调用就实现近乎完美的遗忘；在越狱基准测试中，性能提升51%，误拒率仅为7.9%。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T03:16:39Z
- **目录日期**: 2025-04-30
