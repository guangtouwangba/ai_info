...
这篇论文提出了一种名为StreamFormer的新型流式视频处理框架，用于实时视频理解任务（如自动驾驶和具身AI）。该框架基于预训练视觉Transformer，通过引入因果时间注意力机制，实现了高效流式处理的同时保留图像表征能力。研究者采用多任务视觉-语言对齐框架统一训练，使模型能同时学习全局语义、时序动态和细粒度空间关系。实验表明，StreamFormer在在线动作检测、视频实例分割和视频问答等任务中表现优异且高效，展现了其在实时应用中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T01:28:22Z
- **目录日期**: 2025-04-30
