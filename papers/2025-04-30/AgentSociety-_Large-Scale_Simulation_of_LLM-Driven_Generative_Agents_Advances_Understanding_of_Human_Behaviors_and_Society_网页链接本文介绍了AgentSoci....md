...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。它作为计算社会科学实验平台，研究了四大社会问题：社会极化、刺激性信息传播、基本收入政策影响和飓风等外部冲击。模拟结果与现实世界数据高度吻合，验证了该系统捕捉人类行为机制的能力。该技术为社会科学研究和政策制定提供了新工具，能有效补充传统研究方法（如调查、访谈），帮助深入理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T08:05:26Z
- **目录日期**: 2025-04-30
