...
Graphiti是一个为AI智能体设计的实时知识图谱系统，能动态整合用户对话、企业数据和外部信息，形成持续演化的关系网络。相比传统RAG技术，它支持秒级增量更新、双时间线追踪（事件发生/接收时间）、混合检索（语义+关键词+图遍历）和自定义节点关系。适用于电商客服（个性化推荐）、金融分析（市场追踪）和智能助手（实时决策）等场景。由Zep团队开发，已开源并在GitHub获7k+星，文档完善社区活跃。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T00:03:47Z
- **目录日期**: 2025-04-30
