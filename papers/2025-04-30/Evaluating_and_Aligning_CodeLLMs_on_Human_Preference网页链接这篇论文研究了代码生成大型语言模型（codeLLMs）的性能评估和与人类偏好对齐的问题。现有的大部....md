...
该论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，聚焦模型输出与人类偏好的对齐问题。研究收集397个高质量编程任务样本(覆盖40类/44种语言)，并构建20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好评估存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调代码LLMs需同时保证功能正确性和符合开发者偏好，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T23:05:28Z
- **目录日期**: 2025-04-30
