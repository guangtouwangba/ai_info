...
本文提出TD-EVAL，一个面向任务型对话系统(TOD)的新型两阶段评估框架，通过细粒度的轮次级分析和整体对话级比较来提升评估效果。轮次级评估聚焦对话连贯性、知识一致性和策略合规性三个维度，而对话级评估采用TOD Agent Arena进行成对比较。实验表明，相比传统指标，TD-EVAL能更有效识别对话错误，且与人类判断更一致。该框架为TOD系统评估提供了即插即用的新范式，可同时评估轮次和系统层面性能。研究基于MultiWOZ 2.4和τ-Bench数据集验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T00:02:14Z
- **目录日期**: 2025-04-30
