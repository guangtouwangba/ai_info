...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该框架直接建模音频-视觉关系，无需中间运动表示，克服了传统方法的时间一致性问题。创新点包括：1）时间表示对齐(TREPA)方法增强生成帧的时间一致性；2）改进SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画应用。研究突破了扩散模型在时序任务中的限制，为相关领域提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T08:05:47Z
- **目录日期**: 2025-04-30
