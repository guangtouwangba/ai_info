...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法（时间表示对齐），利用自监督视频模型增强生成帧的时间连贯性。同时，研究深入分析了SyncNet收敛问题，优化了其训练框架，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T16:05:30Z
- **目录日期**: 2025-04-30
