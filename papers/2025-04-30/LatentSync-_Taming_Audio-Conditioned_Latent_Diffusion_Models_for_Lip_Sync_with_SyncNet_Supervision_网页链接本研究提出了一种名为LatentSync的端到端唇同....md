...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音视频关联，并提出时间表示对齐（TREPA）方法解决扩散过程中的时间一致性问题。同时，研究改进了SyncNet训练框架，提升了其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上性能优于现有技术。这些改进可推广至其他依赖SyncNet的唇同步和人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T15:05:41Z
- **目录日期**: 2025-04-30
