...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法。该框架直接建模音频-视觉关系，无需中间运动表示，克服了传统两阶段方法的局限。针对扩散模型的时间一致性问题，创新性提出TREPA方法，利用自监督视频模型的时间表示增强帧间一致性。同时，研究通过改进SyncNet训练框架（包括架构、超参数和数据预处理），显著提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验也可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T09:05:37Z
- **目录日期**: 2025-04-30
