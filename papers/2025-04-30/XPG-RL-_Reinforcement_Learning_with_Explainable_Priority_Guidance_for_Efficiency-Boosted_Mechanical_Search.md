...
XPG-RL是一个强化学习框架，用于解决机械搜索（MS）任务中的杂乱环境挑战。它通过可解释的优先级决策机制，结合原始感官输入，动态选择目标抓取、遮挡移除和视角调整等动作基元。该框架融合RGB-D输入与语义几何特征，生成结构化场景表征，并通过自适应阈值策略优化动作选择。实验表明，XPG-RL在任务成功率和运动效率上显著优于基线方法，长周期任务效率提升高达4.5倍，验证了结合领域知识与可学习策略对机器人操作的鲁棒性和高效性的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T08:01:50Z
- **目录日期**: 2025-04-30
