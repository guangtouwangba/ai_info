...
XPG-RL是一个基于强化学习的框架，用于解决杂乱环境中的机械搜索（MS）问题。它通过可解释的优先级决策机制，结合原始感官输入，动态选择目标抓取、遮挡移除和视角调整等动作基元。该框架整合了任务驱动的动作优先级和上下文感知切换策略，并优化策略以输出自适应阈值。感知模块融合RGB-D数据与语义几何特征，生成结构化场景表示。实验表明，XPG-RL在任务成功率和运动效率上显著优于基线方法，长时任务效率提升高达4.5倍，验证了结合领域知识与可学习策略的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T11:01:36Z
- **目录日期**: 2025-04-30
