...
本文提出了一种名为XPG-RL的强化学习框架，用于解决杂乱环境中的机械搜索（MS）问题。该框架通过可解释的优先级决策机制，结合原始感官输入，动态选择目标抓取、遮挡移除和视角调整等动作基元。XPG-RL整合了任务驱动的动作优先级机制和上下文感知切换策略，优化策略输出自适应阈值以选择动作基元。感知模块融合RGB-D输入与语义几何特征，生成结构化场景表示。实验表明，XPG-RL在任务成功率和运动效率上均优于基线方法，长时任务效率提升高达4.5倍，验证了其鲁棒性和高效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T17:01:27Z
- **目录日期**: 2025-04-30
