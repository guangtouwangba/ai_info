...
Prisma是一款针对视觉Transformer模型的可视化分析工具，能像"显微镜"一样观察模型内部激活机制。该工具支持视频模型（如ViViT/V-JEPA），提供注意力热图、特征重建等可视化功能，并内置小型ViT模型便于调试。研究发现：1）视觉模型比语言模型激活更密集（如CLIP-B/32单patch激活超500个latent）；2）稀疏自编码器(SAE)重建特征可降低损失；3）ViT中CLS token会吸收特征而patch token逐渐沉寂。Prisma为CV模型可解释性研究开辟了新途径，项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T14:03:45Z
- **目录日期**: 2025-04-30
