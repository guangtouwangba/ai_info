...
Prisma是一款专注于视觉Transformer模型内部机制的可视化分析工具，类似"显微镜"可观察、干预模型激活状态。它提供注意力热图、特征重建等可视化功能，支持视频模型分析，并内置小型ViT模型便于调试。研究发现：1)视觉模型激活特征比语言模型更密集；2)特征重建可能提升性能；3)ViT中CLS与patch token行为差异显著。该工具将NLP可解释性方法成功迁移到CV领域，为理解视觉模型工作机制提供新途径。项目已开源，包含论文和代码资源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T21:03:15Z
- **目录日期**: 2025-04-30
