...
Prisma是一款针对视觉Transformer模型的可视化分析工具，能像"显微镜"一样观察模型内部激活机制。它提供注意力热图、特征重建等可视化功能，并支持视频模型(ViViT/V-JEPA)的时间维度分析。研究发现：1)视觉模型特征比语言模型更密集(如CLIP-B/32每个patch激活500+特征)；2)稀疏自编码器(SAE)重建特征可降低损失，具有降噪效果；3)ViT中CLS token会吸收特征而patch token逐渐沉寂，揭示独特信息聚合机制。该工具开源，为CV模型可解释性研究提供了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T17:03:20Z
- **目录日期**: 2025-04-30
