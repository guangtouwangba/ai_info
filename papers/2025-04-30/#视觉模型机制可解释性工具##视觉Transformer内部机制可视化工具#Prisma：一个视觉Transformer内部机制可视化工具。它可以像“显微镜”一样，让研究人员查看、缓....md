...
Prisma是一款专注于视觉Transformer内部机制的可视化分析工具，可像"显微镜"一样观察模型激活状态并逆向分析学习算法。其特点包括：支持视频Transformer（如ViViT/V-JEPA）、提供注意力热图等可视化工具及机制分析功能，并内置小型ViT模型便于调试。研究发现：1）视觉模型激活特征比语言模型更密集；2）稀疏自编码器(SAE)重建特征可降低损失，可能具有降噪效果；3）ViT中CLS token会吸收更多特征，与语言模型不同。该工具为CV模型可解释性研究开辟了新途径，项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T10:03:30Z
- **目录日期**: 2025-04-30
