...
Prisma是一款专注于视觉Transformer模型内部机制分析的开源可视化工具，能够像"显微镜"一样观察、缓存和干预模型激活状态。该工具提供注意力热图、特征重建等可视化功能，支持ViViT等视频模型，并内置小型ViT模型用于快速验证。研究发现：1)视觉模型激活比语言模型更密集；2)特征重建可能具有降噪效果；3)ViT中CLS token会吸收更多特征。Prisma为计算机视觉模型的可解释性研究提供了新工具，复制了NLP领域的成功经验。项目已开源，包含论文和代码资源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T15:03:53Z
- **目录日期**: 2025-04-30
