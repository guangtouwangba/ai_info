...
Prisma是一款针对视觉Transformer模型的可视化分析工具，能够像"显微镜"一样观察模型内部激活机制。它支持ViViT等视频模型的时间维度分析，提供注意力热图、特征重建等可视化功能，并内置小型ViT模型便于调试。研究发现：1）视觉模型特征比语言模型更密集；2）特征重建可能提升性能；3）ViT中CLS token与patch token行为差异显著。该工具为CV模型可解释性研究开辟了新路径，已开源发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T18:03:33Z
- **目录日期**: 2025-04-30
