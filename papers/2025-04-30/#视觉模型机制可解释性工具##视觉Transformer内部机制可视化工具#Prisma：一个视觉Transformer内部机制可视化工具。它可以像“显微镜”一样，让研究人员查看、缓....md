...
Prisma是一个专为视觉Transformer设计的内部机制可视化工具，可像"显微镜"一样分析模型激活状态。它支持视频模型（如ViViT/V-JEPA），提供注意力热图、特征重建等分析工具，并内置小型ViT模型便于调试。研究发现：1）视觉模型激活特征比语言模型更密集（如CLIP每patch激活超500个latent）；2）稀疏自编码器(SAE)重建特征能降低loss，可能有降噪效果；3）ViT中CLS token会随层数增加吸收更多特征，与patch token行为差异显著。该工具开源，为CV模型可解释性研究提供了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T16:03:48Z
- **目录日期**: 2025-04-30
