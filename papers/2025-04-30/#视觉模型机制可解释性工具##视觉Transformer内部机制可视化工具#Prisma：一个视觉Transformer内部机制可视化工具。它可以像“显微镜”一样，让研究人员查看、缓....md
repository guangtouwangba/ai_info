...
Prisma是一款专注于视觉Transformer模型内部机制分析的开源可视化工具。它提供注意力热图、特征重建等可视化功能，支持视频模型分析，并内置小型ViT模型用于调试。研究发现：1)视觉模型激活比语言模型更密集；2)特征重建可降低模型损失；3)ViT中CLS token与patch token行为差异显著。该工具为理解视觉模型工作机制提供了新途径，类似NLP领域的TransformerLens，但专注于计算机视觉方向。项目已开源，包含论文和代码资源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T20:03:24Z
- **目录日期**: 2025-04-30
