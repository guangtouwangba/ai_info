...
Prisma是一款针对视觉Transformer模型的可视化分析工具，能像"显微镜"一样观察模型内部激活状态，逆向解析算法机制。它支持视频模型（如ViViT/V-JEPA）的时间维度分析，提供注意力热图、特征重建等可视化功能，并内置小型ViT模型便于调试。研究发现：1）视觉模型激活特征比语言模型更密集；2）特征重建可能提升性能；3）ViT中CLS token会吸收更多信息。该工具将NLP可解释性方法成功迁移到CV领域，为理解视觉模型提供了新途径。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T11:03:42Z
- **目录日期**: 2025-04-30
