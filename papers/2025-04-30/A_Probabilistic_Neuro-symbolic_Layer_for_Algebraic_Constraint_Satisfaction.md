...
该研究提出了一种可微的概率代数层（PAL），用于确保神经网络在连续环境中满足非凸代数约束条件（如自动驾驶避障）。PAL可无缝集成到任何神经架构中，通过最大似然训练且无需近似。它基于多项式参数化，定义了线性不等式组合的概率分布，支持高效的符号积分归一化，并可在GPU上并行计算。实验验证了PAL在代数约束集成和真实轨迹数据上的有效性。这一方法为安全关键应用（如自主代理）提供了严格的约束保障机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T00:02:23Z
- **目录日期**: 2025-04-30
