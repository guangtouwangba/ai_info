...
本文提出了一种可微概率层（PAL），用于确保神经网络在连续环境中满足非凸代数约束（如自动驾驶避障）。PAL可无缝集成到任何神经架构中，通过最大似然训练且无需近似。它基于多项式参数化，定义线性不等式合取/析取的分布，支持通过符号积分实现高效精确的归一化，并能在GPU上并行计算。实验验证了PAL在代数约束集成和真实轨迹数据上的有效性。该方法为安全关键应用（如自主代理）提供了严格的约束保障机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T01:29:48Z
- **目录日期**: 2025-04-30
