...
阿里妈妈推出URM"世界知识大模型"，解决大模型在电商推荐中的两大难题：缺乏领域知识和用户行为数据冗长。该模型融合大语言模型的通用能力与电商专业知识，能对用户兴趣进行深度推理。其特点是结合上下文实时生成精准推荐，例如通过"儿童新年衣服"识别用户身份（宝妈）和场景需求（新年），优先推荐相关商品而非近期浏览记录。这一创新使推荐系统具备场景化理解和动态响应能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T17:03:44Z
- **目录日期**: 2025-04-30
