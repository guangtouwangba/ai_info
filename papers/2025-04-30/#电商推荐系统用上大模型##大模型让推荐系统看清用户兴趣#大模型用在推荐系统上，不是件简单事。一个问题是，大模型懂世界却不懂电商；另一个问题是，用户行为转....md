...
阿里妈妈推出URM"世界知识大模型"，解决电商推荐系统两大难题：1)大模型缺乏电商领域知识；2)用户行为文本输入冗长。该模型融合大语言模型的通用知识与电商专业数据，能深度理解用户兴趣并实时推理。例如识别"儿童新年衣服"搜索背后是宝妈需求，结合"新年"关键词优先推荐节日童装而非用户常看的女裤。实现基于语境和历史的精准推荐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T19:03:32Z
- **目录日期**: 2025-04-30
