...
阿里妈妈推出URM"世界知识大模型"，解决电商推荐系统两大难题：1)大模型缺乏电商领域知识；2)用户行为数据冗长难处理。该模型融合大语言模型的通用知识与电商专业数据，能深度理解用户兴趣并实时推理。例如，当用户搜索"儿童新年衣服"时，系统能识别用户身份（宝妈）并结合语境（新年），优先推荐相关商品而非近期浏览记录（如女式牛仔裤），实现更精准的个性化推荐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T12:04:13Z
- **目录日期**: 2025-04-30
