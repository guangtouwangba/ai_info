...
阿里妈妈开发了URM（世界知识大模型），将大模型技术应用于电商推荐系统，解决了两大难题：1）大模型缺乏电商领域知识；2）用户行为数据过长。URM结合通用大模型能力与电商专业知识，能深度理解用户兴趣并实时推理推荐。例如，识别"儿童新年衣服"搜索背后的宝妈需求，结合"新年"关键词优先推荐节日款而非用户常浏览的女装。该系统通过语境分析和动态生成，实现更精准的个性化推荐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T20:03:50Z
- **目录日期**: 2025-04-30
