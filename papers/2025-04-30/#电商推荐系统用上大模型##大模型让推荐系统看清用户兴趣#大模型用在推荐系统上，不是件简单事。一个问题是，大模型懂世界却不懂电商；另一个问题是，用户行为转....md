...
阿里妈妈开发了URM"世界知识大模型"，解决大模型在电商推荐中的两大难题：缺乏领域知识和用户行为文本过长。该模型融合大语言模型的通用能力和电商专业知识，能对用户兴趣进行深度推理。它能结合用户历史行为和当前语境实时生成精准推荐，例如识别"儿童新年衣服"搜索背后的宝妈身份，优先推荐节日童装而非近期浏览的女裤。这一创新使推荐系统能更准确地理解用户真实需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T09:03:22Z
- **目录日期**: 2025-04-30
