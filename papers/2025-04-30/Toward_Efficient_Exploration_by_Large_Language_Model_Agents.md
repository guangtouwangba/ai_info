...
这篇摘要探讨了基于大语言模型(LLM)的强化学习(RL)智能体在探索效率方面的挑战。现有LLM智能体设计往往难以解决RL中的探索问题，而传统RL算法虽能有效探索但难以用纯自然语言实现。研究者没有采用微调或上下文学习让LLM隐式模仿RL算法，而是直接利用LLM显式实现一个已知的数据高效RL算法(后验采样强化学习)。实验表明，这种基于LLM的显式算法实现能在需要谨慎探索的自然语言任务中显著提高效率。该研究为LLM智能体的数据高效强化学习提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T10:01:06Z
- **目录日期**: 2025-04-30
