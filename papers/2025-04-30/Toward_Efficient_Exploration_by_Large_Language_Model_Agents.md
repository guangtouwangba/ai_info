...
这篇摘要探讨了基于大语言模型(LLM)的强化学习(RL)智能体在数据效率方面的挑战。研究发现，当前许多LLM智能体设计方案难以有效解决探索问题，而传统RL算法虽然能优雅处理探索，但在纯自然语言环境中难以实现。作者提出了一种创新方法：不是通过微调或上下文学习让LLM隐式模仿RL算法，而是直接利用LLM显式实现一个已知的数据高效RL算法(后验采样强化学习)。实验表明，这种基于LLM的实现方式在需要谨慎探索的自然语言任务中显著更有效。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T11:00:54Z
- **目录日期**: 2025-04-30
