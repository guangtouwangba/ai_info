...
这篇摘要探讨了基于大语言模型(LLM)的强化学习(RL)智能体的探索效率问题。研究发现，当前许多LLM智能体设计难以有效解决RL中的探索挑战，而传统RL算法虽能优雅处理探索问题，却难以在纯自然语言环境中实现。作者提出了一种创新方法：不依赖微调或上下文学习让LLM隐式模仿RL算法，而是直接利用LLM显式实现已有RL算法(后验采样强化学习)，该算法具有已知的统计高效探索能力。实验表明，这种基于LLM的实现方式在需要谨慎探索的自然语言任务中显著更有效。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T13:08:05Z
- **目录日期**: 2025-04-30
