...
这篇论文探讨了如何利用大语言模型(LLMs)实现数据高效的强化学习(RL)。研究发现，当前许多基于LLM的智能体设计在探索能力上存在不足，而传统RL算法虽然能有效解决探索问题，却难以直接应用于自然语言环境。作者提出了一种创新方法：不是通过微调或上下文学习让LLMs隐式模仿RL算法，而是直接让LLMs显式实现一个已知的数据高效RL算法(后验采样强化学习)。实验表明，这种基于LLM的实现方式在需要谨慎探索的自然语言任务中表现显著更优。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T03:15:48Z
- **目录日期**: 2025-04-30
