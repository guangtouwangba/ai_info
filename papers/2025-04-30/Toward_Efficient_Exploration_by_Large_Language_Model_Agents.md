...
这篇论文探讨了如何利用大语言模型(LLMs)实现高效的数据探索强化学习(RL)。研究表明，当前许多LLM智能体设计在探索能力上存在不足，而经典RL算法虽能有效解决探索问题，却难以在纯自然语言环境中实现。作者提出了一种创新方法：不是通过微调或上下文学习让LLMs隐式模仿RL算法，而是直接利用LLMs显式实现已知的"后验采样强化学习"算法。实验证明，这种基于LLM的实现方式在需要谨慎探索的自然语言任务中，能显著提高数据效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T14:00:58Z
- **目录日期**: 2025-04-30
