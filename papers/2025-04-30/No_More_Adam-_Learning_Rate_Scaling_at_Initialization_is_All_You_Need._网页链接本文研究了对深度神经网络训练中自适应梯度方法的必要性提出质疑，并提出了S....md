...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，无需自适应二阶动量即可避免训练不平衡。该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，内存效率显著优于AdamW（节省5.93-25.15GB），且在LoRA微调等任务中超越现有优化器，同时具备超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T06:04:36Z
- **目录日期**: 2025-04-30
