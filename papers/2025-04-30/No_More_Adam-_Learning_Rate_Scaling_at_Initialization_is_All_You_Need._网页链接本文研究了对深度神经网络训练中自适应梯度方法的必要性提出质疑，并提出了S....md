...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题，同时将优化器内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，在LLM训练中显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB），且具有超参数鲁棒性。该方法还提升了LoRA微调等任务的性能，为大型模型训练提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T02:32:29Z
- **目录日期**: 2025-04-30
