...
PathFinder是一种新型无地图导航系统，专为视障人士（BLV）设计，结合视觉语言模型（VLM）、大语言模型（LLM）和单目深度估计技术，通过深度优先搜索（DFS）算法识别最长无障碍路径。相比现有AI导航方案，它在户外环境中显著降低平均绝对误差（MAE）并提升决策速度，73%用户可在1分钟内掌握操作，80%认可其准确性、响应速度和便捷性。但系统在复杂室内和弱光环境下仍存在局限。研究显示其平衡了实时性、计算效率与导航精度，为BLV群体提供了实用辅助方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T08:01:41Z
- **目录日期**: 2025-04-30
