...
PathFinder是一种新型无地图导航系统，专为视障人士设计，通过结合视觉语言模型(VLM)、大语言模型(LLM)和单目深度估计技术，实现实时障碍物检测与路径规划。系统采用深度优先搜索(DFS)算法在深度图像中寻找最长无障碍路径，在计算效率和路线优化间取得平衡。测试表明，PathFinder在户外导航中显著降低平均绝对误差(MAE)并提升决策速度，73%用户能在1分钟内掌握操作，80%认可其准确性、响应速度和便捷性。但系统在复杂室内环境和弱光条件下仍存在局限。研究凸显了该技术在辅助视障人群独立出行方面的实用价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T16:01:36Z
- **目录日期**: 2025-04-30
