...
PathFinder是一种新型无地图导航系统，专为视障人士（BLV）设计，通过结合视觉语言模型（VLM）、大语言模型（LLM）和单目深度估计技术实现实时路径规划。系统采用深度优先搜索（DFS）算法分析深度图像，选择最长无障碍路径，兼顾计算效率与准确性。测试表明，PathFinder在户外导航中显著降低平均绝对误差（MAE），决策速度优于现有AI方案。用户研究显示，73%的参与者能快速掌握操作，80%认可其准确性、响应速度和便捷性，但在复杂室内和弱光环境下仍需改进。该系统为BLV群体提供了实用高效的导航解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T11:01:27Z
- **目录日期**: 2025-04-30
