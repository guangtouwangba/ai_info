...
PathFinder是一种新型无地图导航系统，专为视障人士(BLV)设计，通过结合视觉语言模型(VLM)、大语言模型(LLM)和单目深度估计技术，实现实时避障和路径规划。系统采用深度优先搜索(DFS)算法分析深度图像，找出最长无障碍路径，兼顾精度与计算效率。测试显示，PathFinder在户外导航中显著降低平均绝对误差(MAE)并提升决策速度，73%用户能在1分钟内掌握操作，80%认可其准确性、响应速度和便利性。但该系统在复杂室内环境和弱光条件下仍有改进空间。研究对比现有AI导航方案，证实PathFinder在实时性与适应性方面的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T09:01:36Z
- **目录日期**: 2025-04-30
