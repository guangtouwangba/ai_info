...
PathFinder是一种新型无地图导航系统，专为视障人士设计，通过结合视觉语言模型(VLM)、大语言模型(LLM)和单目深度估计技术实现实时路径规划。该系统采用深度优先搜索(DFS)算法分析深度图像，找出最长无障碍路径，在计算效率和实时性之间取得平衡。测试表明，PathFinder在户外导航中显著降低了平均绝对误差(MAE)，决策速度优于现有AI导航方案。用户研究显示，73%的视障用户能在1分钟内掌握使用方法，80%认可其准确性、响应速度和便利性，但在复杂室内环境和弱光条件下仍需改进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T19:01:17Z
- **目录日期**: 2025-04-30
