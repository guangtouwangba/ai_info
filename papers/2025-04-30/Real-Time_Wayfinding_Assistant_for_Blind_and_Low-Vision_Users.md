...
PathFinder是一种新型无地图导航系统，旨在帮助视障人士（BLV）解决户外导航难题。该系统结合视觉语言模型（VLM）、大语言模型（LLM）和单目深度估计技术，采用深度优先搜索（DFS）算法分析深度图像，找出最长无障碍路径。相比现有AI导航方案，PathFinder在精度、计算效率和实时性之间取得平衡，显著降低了平均绝对误差（MAE）并加快决策速度。用户测试显示，73%的参与者能快速掌握使用，80%认可其准确性、响应速度和便利性，但在复杂室内和弱光环境下仍需改进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T04:01:30Z
- **目录日期**: 2025-04-30
