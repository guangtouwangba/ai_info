...
PathFinder是一种新型无地图导航系统，专为视障人士设计，通过结合视觉语言模型(VLM)、大语言模型(LLM)和单目深度估计技术实现实时路径规划。系统采用深度优先搜索(DFS)算法分析深度图像，找出最长无障碍路径，在保证计算效率的同时优化路线选择。测试表明，PathFinder在户外导航中显著降低平均绝对误差(MAE)并提升决策速度，73%用户能在1分钟内掌握使用方法，80%认可其准确性、响应速度和便利性。但系统在复杂室内环境和弱光条件下仍存在局限。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T05:01:23Z
- **目录日期**: 2025-04-30
