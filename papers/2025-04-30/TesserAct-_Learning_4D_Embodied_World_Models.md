...
本文提出了一种学习新型4D具身世界模型的有效方法，通过训练RGB-DN（RGB、深度和法线）视频来预测3D场景随时间的动态演变。该方法超越了传统2D模型，将形状、配置和时间变化纳入预测，并能学习精确的逆向动态模型。研究首先利用现成模型扩展机器人操作视频数据集，添加深度和法线信息；然后微调视频生成模型以联合预测每帧的RGB-DN；最后提出算法将生成的RGB、深度和法线视频直接转换为高质量4D场景。该方法确保了时空一致性，支持新视角合成，并显著提升了基于视频的策略学习效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T13:08:16Z
- **目录日期**: 2025-04-30
