...
本文提出了一种学习新型4D具身世界模型的有效方法，该模型能预测3D场景在具身智能体动作影响下的动态演变，确保时空一致性。研究通过训练RGB-DN（RGB、深度和法线）视频来构建4D世界模型，不仅超越传统2D模型，还能准确学习智能体的逆向动力学模型。具体步骤包括：利用现成模型扩展机器人操作视频数据集；微调视频生成模型以联合预测每帧的RGB-DN；开发算法将生成的视频直接转换为高质量4D场景。该方法实现了具身场景的时空一致性预测、新视角合成，并显著提升了基于视频的策略学习效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T16:01:12Z
- **目录日期**: 2025-04-30
