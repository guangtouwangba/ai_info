...
这篇论文提出了一种学习新型4D具身世界模型的有效方法，该模型能预测3D场景随具身智能体动作的动态演变，确保时空一致性。研究通过RGB-DN（RGB、深度和法线）视频训练4D世界模型，不仅超越传统2D模型（整合了形状、配置和时间变化），还能准确学习具身智能体的逆向动力学模型。方法包括：1）利用现成模型扩展机器人操作视频数据集；2）微调视频生成模型以联合预测每帧RGB-DN；3）提出算法将生成视频直接转换为高质量4D场景。实验表明，该方法在时空一致性、新视角合成和策略学习方面显著优于现有视频世界模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T14:01:10Z
- **目录日期**: 2025-04-30
