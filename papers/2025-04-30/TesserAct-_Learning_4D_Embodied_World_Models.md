...
这篇论文提出了一种学习新型4D具身世界模型的有效方法，该模型能预测3D场景随具身智能体动作的动态演变，确保时空一致性。通过训练RGB-DN（RGB、深度和法线）视频数据，该方法超越了传统2D模型，将形状、配置和时间变化纳入预测，并有效学习精确的逆向动力学模型。具体步骤包括：利用现成模型扩展机器人操作视频数据集，添加深度和法线信息；微调视频生成模型以联合预测每帧的RGB-DN；开发算法将生成的RGB、深度和法线视频直接转换为高质量4D场景。该方法在4D场景预测、新视角合成和策略学习方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T11:01:06Z
- **目录日期**: 2025-04-30
