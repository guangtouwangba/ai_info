...
这篇论文提出了生成式AI（GenAI）应用安全的新视角，重点关注应用级安全而非通用威胁。作者首次定义了"安全可控性"（Security Steerability）概念，评估大语言模型（LLM）遵循系统提示中严格防护规则（如"避免讨论政治"）的能力，以防止恶意用户绕过应用防护。研究开发了两种新数据集（VeganRibs和ReverseText）来衡量LLM的安全可控性：前者测试模型在遭遇攻击（越狱和干扰）时执行非安全类防护规则的能力，后者评估模型将用户输入视为纯文本而非附加含义的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T23:02:07Z
- **目录日期**: 2025-04-30
