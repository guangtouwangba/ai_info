...
这篇论文提出了生成式AI（GenAI）应用安全的新视角，重点关注应用级安全威胁。作者首次定义了"安全可控性"（Security Steerability）概念，用于评估大语言模型（LLM）遵循系统提示中严格安全护栏（如"避免讨论政治"）的能力。研究开发了两种新数据集：VeganRibs测试模型在恶意用户攻击下执行非安全类护栏的能力，ReverseText则评估模型将用户输入视为纯文本的能力。研究表明，虽然LLM无法直接防御特定应用威胁，但可为应用提供自我保护框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T08:02:21Z
- **目录日期**: 2025-04-30
