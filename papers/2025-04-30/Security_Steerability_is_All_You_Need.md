...
本文提出了一种以应用为中心的生成式AI(GenAI)安全方法，重点研究应用层面的安全防护。作者定义了"安全可控性"(Security Steerability)这一新指标，用于评估大语言模型(LLM)遵循系统提示中严格防护规则的能力。研究开发了两种新数据集：VeganRibs测试LLM在恶意用户攻击下执行非安全相关防护规则的表现，ReverseText则评估LLM将用户输入视为纯文本处理的能力。研究表明，虽然LLM无法直接防御特定应用威胁，但可为应用程序提供自我保护的框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T22:02:01Z
- **目录日期**: 2025-04-30
