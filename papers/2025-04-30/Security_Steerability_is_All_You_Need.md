...
本文提出了生成式AI（GenAI）应用安全的新研究方向——"安全可控性"（Security Steerability），通过系统提示词设定严格防护规则（如"避免讨论政治"），评估大语言模型（LLM）抵御恶意用户攻击的能力。研究开发了两种测试方法：VeganRibs检测模型在遭遇越狱攻击时维持非安全类规则的能力，ReverseText则测试模型处理用户输入为纯文本的鲁棒性。该工作填补了当前GenAI安全研究在应用层防护的空白，为应用程序提供了基于LLM的自我防护框架，而非仅关注通用威胁防护。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T03:16:59Z
- **目录日期**: 2025-04-30
