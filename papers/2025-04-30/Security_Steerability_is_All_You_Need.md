...
本文提出了一种以应用为中心的生成式AI安全方法，重点研究应用级安全威胁的防护。作者定义了"安全可控性"这一新安全指标，用于评估大语言模型(LLM)遵循系统提示中严格防护规则的能力。研究开发了两种新数据集：VeganRibs测试模型在恶意用户攻击下执行非安全相关规则的能力，ReverseText则评估模型将用户输入视为纯文本处理的能力。研究表明，虽然LLM无法直接防御特定应用威胁，但可为应用提供自我防护框架。该方法填补了当前GenAI安全研究中应用级防护的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T14:02:11Z
- **目录日期**: 2025-04-30
