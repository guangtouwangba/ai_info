...
本文提出了一种以应用为中心的生成式AI（GenAI）安全方法，强调当前研究多关注通用威胁（如操纵LLM生成违禁内容），而较少探讨应用级安全防护。作者定义了"安全可控性"（Security Steerability）这一新指标，用于评估LLM通过系统提示（如"避免讨论政治"）设置严格防护栏的能力。研究开发了两种测试方法：VeganRibs测试LLM在恶意用户使用越狱攻击时的防护能力，ReverseText则评估LLM将用户输入视为纯文本的处理能力。研究表明，虽然LLM无法直接防御特定应用威胁，但可为应用提供自我保护框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T10:02:17Z
- **目录日期**: 2025-04-30
