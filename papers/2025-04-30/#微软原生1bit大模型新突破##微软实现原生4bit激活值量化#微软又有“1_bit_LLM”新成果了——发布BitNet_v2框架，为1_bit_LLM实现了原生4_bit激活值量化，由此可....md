...
微软最新推出BitNet v2框架，在1bit大模型(BitNet b1.58)基础上实现了原生4bit激活值量化，解决了此前激活值仍为8bit导致的硬件计算效率瓶颈。该框架引入H-BitLinear模块，通过Hadamard变换将激活分布转化为接近高斯分布，显著减少异常值，使4bit量化成为可能。这一突破可充分利用新一代GPU(如GB200)的4bit原生计算能力，同时降低内存带宽需求并提升计算效率，是继1.58-bit权重量化后的又一重要进展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-30T00:04:07Z
- **目录日期**: 2025-04-30
