...
PLAY2PROMPT是一种自动化框架，旨在解决大语言模型（LLMs）在零样本设置下使用外部工具时的挑战。该框架通过系统性地“玩”每个工具来探索其输入输出行为，从而在没有标注数据的情况下优化工具文档并生成使用示例。这些示例不仅指导LLM推理，还作为验证进一步提升工具利用。实验表明，PLAY2PROMPT显著提高了开放和封闭模型在真实任务中的零样本工具性能，为领域特定工具集成提供了可扩展且有效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T18:02:16Z
- **目录日期**: 2025-03-19
