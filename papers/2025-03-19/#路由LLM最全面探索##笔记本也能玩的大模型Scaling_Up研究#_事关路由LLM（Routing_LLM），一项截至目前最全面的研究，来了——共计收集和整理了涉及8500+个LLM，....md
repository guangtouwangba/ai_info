...
路由LLM（Routing LLM）是一种将多个大型语言模型（LLM）如ChatGPT、Qwen、DeepSeek等作为“专家”，通过路由器（Router）将输入分配给最合适的LLM处理的方法，旨在实现高性能、低计算消耗和减少幻觉。中山大学和普渡大学的研究人员基于8500多个LLM在12个基准测试上的2亿条性能记录，提出了Model-level Scaling Up现象，即随着LLM候选数量的增加，路由LLM的性能显著提升。他们还开发了RouterEval评测工具，使其他研究人员能在有限计算资源（如笔记本、单卡GPU）下参与路由LLM研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T21:04:16Z
- **目录日期**: 2025-03-19
