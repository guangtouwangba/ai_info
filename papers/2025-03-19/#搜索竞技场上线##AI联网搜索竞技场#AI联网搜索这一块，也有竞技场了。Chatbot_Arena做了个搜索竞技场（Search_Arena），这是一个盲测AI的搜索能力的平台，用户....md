...
Chatbot Arena推出了“搜索竞技场”（Search Arena），这是一个盲测AI搜索能力的平台，用户可以通过投票比较不同模型的搜索表现。例如，用户提问“下次月全食是什么时候？”，平台会展示两个模型的回答，并揭示答案来源。Search Arena还支持图片对话、文生图功能（如DALL-E 3、Flux、Ideogram），以及通过RepoChat与GitHub代码仓库对话，方便开发者查询代码。用户可以通过平台对比不同模型的表现，判断哪个模型更优。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T22:01:34Z
- **目录日期**: 2025-03-19
