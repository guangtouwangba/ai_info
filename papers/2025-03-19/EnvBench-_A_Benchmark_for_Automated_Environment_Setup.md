...
近期大型语言模型（LLMs）的进展使研究者能够专注于软件工程领域的实际仓库级任务。本文提出EnvBench，一个全面的环境设置基准，涵盖329个Python和665个基于JVM（Java、Kotlin）的仓库，专注于真实配置挑战。我们引入两种自动评估指标：Python的静态分析检查和JVM语言的编译检查。通过评估三种环境设置方法，包括简单零样本基线和两种代理工作流，测试了GPT-4o和GPT-4o-mini模型。最佳方法成功配置了6.69%的Python和29.47%的JVM仓库，表明EnvBench对当前方法仍具挑战性。基准套件和实验数据已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T19:01:57Z
- **目录日期**: 2025-03-19
