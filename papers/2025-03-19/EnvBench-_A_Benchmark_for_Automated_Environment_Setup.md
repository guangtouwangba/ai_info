...
近期大型语言模型（LLMs）的进展使得研究者能够专注于软件工程领域的实际仓库级任务。本文提出了一种自动化软件仓库环境配置的任务，并引入了EnvBench基准测试，涵盖329个Python和665个基于JVM的仓库，重点关注具有真实配置挑战的项目。EnvBench包含两种自动评估指标：Python的静态分析检查和JVM语言的编译检查。通过评估三种环境配置方法（包括零样本基线和两种代理工作流），最佳方法成功配置了6.69%的Python仓库和29.47%的JVM仓库，表明EnvBench对现有方法仍具挑战性。基准测试套件公开于GitHub。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T18:02:05Z
- **目录日期**: 2025-03-19
