...
近期大型语言模型（LLMs）的进展推动了软件工程领域中的实际仓库级任务研究。本文聚焦于自动化软件仓库环境设置任务，提出了EnvBench基准，涵盖329个Python和665个基于JVM（Java、Kotlin）的仓库，重点关注真实配置挑战。为支持基准扩展和模型调优，实现了两种自动评估指标：Python的静态导入缺失检查和JVM语言的编译检查。通过评估三种环境设置方法（包括零样本基线和两种代理工作流），使用GPT-4o和GPT-4o-mini进行测试，最佳方法成功配置了6.69%的Python和29.47%的JVM仓库，表明EnvBench对现有方法仍具挑战性。基准套件公开于https://github.com/JetBrains-Research/EnvBench。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T12:02:23Z
- **目录日期**: 2025-03-19
