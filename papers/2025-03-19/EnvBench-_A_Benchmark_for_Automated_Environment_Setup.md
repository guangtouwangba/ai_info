...
近期大型语言模型（LLMs）的进展推动了软件工程领域中的实际仓库级任务研究。本文聚焦于自动化软件仓库环境设置任务，提出了一个全面的环境设置基准EnvBench，涵盖329个Python和665个JVM（Java、Kotlin）仓库，重点关注具有真实配置挑战的项目。为支持基准扩展和模型调优，实现了两种自动评估指标：Python的静态导入缺失检查和JVM语言的编译检查。通过评估三种环境设置方法，包括简单零样本基线和两种代理工作流，使用GPT-4o和GPT-4o-mini进行测试，结果显示EnvBench对现有方法仍具挑战性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-19T21:01:46Z
- **目录日期**: 2025-03-19
