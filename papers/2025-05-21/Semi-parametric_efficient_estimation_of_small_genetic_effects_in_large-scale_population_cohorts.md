...
TarGene是一个用于群体遗传学研究的统一统计工作流，通过半参数高效且双重稳健的估计方法，分析遗传效应（包括k点交互作用）。它采用交叉验证/加权的TMLE和OSE估计器，能处理混杂因素和弱群体依赖性，并利用基于遗传相关性的方差估计器校正数据依赖性。该方法适用于大规模基因组数据库（如UK Biobank），能精准估计基因-基因/基因-环境交互作用，并通过模拟验证了统计功效和I类错误控制。相关算法已集成至Julia包TMLE.jl，并配套开源Nextflow流程TarGene，支持高性能与云计算平台。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:01:28Z
- **目录日期**: 2025-05-21
