...
本文提出ContextAgent，首个结合多维度感知情境（如可穿戴设备的视频、音频数据）来增强大语言模型（LLM）主动服务能力的智能体。它通过分析实时感知数据和历史用户画像，预测主动服务需求并自动调用工具。研究构建了首个情境感知主动服务评测基准ContextAgentBench，涵盖9类日常场景和20种工具。实验表明，ContextAgent在主动预测和工具调用准确率上分别比基线模型提升8.5%和6.0%。该研究为开发更先进、以人为中心的主动式AI助手提供了新思路。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T08:01:50Z
- **目录日期**: 2025-05-21
