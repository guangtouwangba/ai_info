...
这篇论文针对代码生成大型语言模型(codeLLMs)提出了两个关键贡献：1) 创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，专门评估模型输出与人类偏好的匹配程度；2) 构建了20B标记的SynCode-Instruct合成指令数据集用于模型微调。研究发现现有基于执行的基准测试无法充分反映人类偏好，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面存在显著差距，凸显了该研究方向的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T13:15:29Z
- **目录日期**: 2025-05-21
