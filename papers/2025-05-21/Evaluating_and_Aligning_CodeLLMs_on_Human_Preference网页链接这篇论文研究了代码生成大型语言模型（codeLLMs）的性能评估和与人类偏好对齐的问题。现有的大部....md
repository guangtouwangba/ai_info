...
该论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准，包含397个真实编程任务样本(覆盖40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记量的SynCode-Instruct合成指令集验证微调效果。实验发现传统执行正确性基准与人类偏好存在显著差异，开源顶尖模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显，揭示了现有评估体系在"人类对齐"维度的不足。研究强调代码生成模型需兼顾功能正确性和开发者体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:06:21Z
- **目录日期**: 2025-05-21
