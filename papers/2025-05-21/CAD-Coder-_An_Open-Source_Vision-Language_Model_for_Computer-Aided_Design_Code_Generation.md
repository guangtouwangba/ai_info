...
这篇论文介绍了CAD-Coder，一个开源视觉语言模型（VLM），专门用于从视觉输入直接生成可编辑的CAD代码（CadQuery Python）。通过使用新创建的GenCAD-Code数据集（包含超过16.3万张CAD模型图像和代码对），CAD-Coder在生成语法100%有效的代码和3D实体相似度方面超越了GPT-4.5等先进模型。该模型展现出一定的泛化能力，能够处理真实世界图像并执行未在微调阶段见过的CAD操作。CAD-Coder有望简化工程师和设计师的CAD工作流程，其代码已在GitHub公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T17:02:30Z
- **目录日期**: 2025-05-21
