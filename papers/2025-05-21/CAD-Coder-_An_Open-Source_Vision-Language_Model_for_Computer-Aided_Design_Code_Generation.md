...
本文介绍了CAD-Coder，一种基于视觉语言模型（VLM）的开源工具，能够直接从视觉输入生成可编辑的CAD代码（CadQuery Python）。通过使用新构建的GenCAD-Code数据集（包含超过16.3万张CAD模型图像与代码对），CAD-Coder在生成语法100%有效的代码和3D实体相似度方面超越了GPT-4.5和Qwen2.5-VL-72B等先进模型。该模型还展现出一定的泛化能力，能够处理真实世界图像和未在训练中见过的CAD操作。CAD-Coder有望显著提升工程设计的效率和准确性，其代码已公开在GitHub上。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T03:20:03Z
- **目录日期**: 2025-05-21
