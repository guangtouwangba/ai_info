...
这篇论文介绍了CAD-Coder，一个开源的视觉语言模型（VLM），专门用于从视觉输入直接生成可编辑的CAD代码（CadQuery Python）。该模型基于新构建的GenCAD-Code数据集（包含超过16.3万组CAD模型图像和代码对），在生成语法有效性（100%）和3D实体相似度准确性上超越了GPT-4.5等先进基线模型。CAD-Coder展现出一定的泛化能力，能处理真实世界图像并执行未在微调中见过的CAD操作。这一成果表明，基于代码微调的VLM有望显著优化工程设计的CAD工作流程。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T06:02:21Z
- **目录日期**: 2025-05-21
