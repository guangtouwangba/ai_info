...
本文介绍了CAD-Coder，一种开源视觉语言模型（VLM），专为从视觉输入直接生成可编辑的CAD代码（CadQuery Python）而优化。通过使用新创建的GenCAD-Code数据集（包含超过16.3万组CAD模型图像与代码对），CAD-Coder在生成语法有效性（100%）和3D实体相似度准确性上超越了GPT-4.5等先进模型。该模型还展现出一定的泛化能力，能够处理真实世界图像并执行未在训练中见过的CAD操作。CAD-Coder的优异表现凸显了基于代码微调的VLM在简化工程CAD流程中的潜力。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T22:02:10Z
- **目录日期**: 2025-05-21
