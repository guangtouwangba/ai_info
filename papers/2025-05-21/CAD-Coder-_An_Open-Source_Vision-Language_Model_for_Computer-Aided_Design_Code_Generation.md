...
这篇论文介绍了CAD-Coder，一个开源的视觉语言模型（VLM），专为从视觉输入直接生成可编辑的CAD代码（CadQuery Python）而优化。通过使用包含16.3万张CAD模型图像和代码对的新数据集GenCAD-Code，CAD-Coder在语法有效性（100%）和3D实体相似度准确性上超越了GPT-4.5和Qwen2.5-VL-72B等先进模型。该模型还展现出一定的泛化能力，能处理真实世界图像和未在训练中见过的CAD操作。CAD-Coder有望简化工程师和设计师的CAD工作流程。项目已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T16:02:24Z
- **目录日期**: 2025-05-21
