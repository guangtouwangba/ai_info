...
这篇论文介绍了CAD-Coder，一个开源的视觉语言模型（VLM），专门用于从视觉输入直接生成可编辑的CAD代码（CadQuery Python）。通过使用新构建的GenCAD-Code数据集（包含超过16.3万张CAD模型图像与代码对），CAD-Coder在语法有效性和3D实体相似性方面超越了GPT-4.5和Qwen2.5-VL-72B等先进模型，达到了100%的语法正确率。该模型还展现出一定的泛化能力，能够处理真实世界图像并执行未在训练中见过的CAD操作。CAD-Coder有望简化工程师和设计师的CAD工作流程。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T21:02:15Z
- **目录日期**: 2025-05-21
