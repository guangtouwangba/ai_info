...
这篇论文介绍了CAD-Coder，一个开源视觉语言模型（VLM），专门用于从视觉输入直接生成可编辑的CAD代码（CadQuery Python）。通过使用新构建的GenCAD-Code数据集（包含16.3万组CAD模型图像和代码对），CAD-Coder在生成语法100%有效且3D实体相似度最高的代码方面，超越了GPT-4.5等先进基线模型。该模型展现出一定的泛化能力，能处理真实世界图像和未在微调中见过的CAD操作。CAD-Coder的性能和适应性表明，基于代码微调的VLM有望优化工程设计的CAD工作流程。模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T14:02:31Z
- **目录日期**: 2025-05-21
