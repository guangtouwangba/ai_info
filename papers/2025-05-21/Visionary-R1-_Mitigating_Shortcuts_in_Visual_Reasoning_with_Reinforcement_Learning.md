...
本文探讨了如何通过强化学习训练视觉语言模型(VLM)进行图像推理。研究发现，直接应用强化学习会导致模型走捷径，降低泛化能力。为此，作者提出"描述-推理-回答"的输出格式：先生成详细图像描述，再进行推理。在27.3万无思维链监督的视觉问答对上训练的Visionary-R1模型，仅使用强化学习就在多个视觉推理基准上超越了GPT-4o、Claude3.5-Sonnet和Gemini-1.5-Pro等强大多模态模型。研究表明，先理解图像再进行推理是避免捷径学习的关键。(99字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T21:01:12Z
- **目录日期**: 2025-05-21
