...
生成式AI搜索通过提供端到端答案简化了复杂查询的信息检索，但破坏了传统搜索依赖用户反馈（如点击、停留时间）改进排序模型的闭环机制。由于反馈仅针对最终答案，难以映射到中间环节（查询分解、文档检索、答案生成），导致系统迭代困难。为此，研究提出NExT-Search新范式：1）用户调试模式，允许干预关键环节；2）影子用户模式，通过AI代理模拟用户偏好提供反馈。结合实时在线调整与离线模型更新，该系统能重建细粒度反馈循环，推动生成式搜索持续优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T03:18:42Z
- **目录日期**: 2025-05-21
