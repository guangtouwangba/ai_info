...
生成式AI搜索通过提供端到端的复杂查询答案，改变了传统信息检索模式，减少了用户手动浏览和总结的需求。然而，这种模式也破坏了传统搜索基于用户反馈（如点击、停留时间）的持续改进机制，因为AI搜索只能获得最终答案的粗粒度反馈，难以优化中间环节。为此，研究提出NExT-Search新范式，通过两种模式重新引入细粒度反馈：用户调试模式（允许干预关键环节）和影子用户模式（AI代理模拟用户偏好）。结合实时在线调整和离线模型更新，该系统有望实现基于人类反馈的持续进化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T18:01:00Z
- **目录日期**: 2025-05-21
