...
生成式AI搜索通过提供端到端答案简化了信息检索，但也破坏了传统搜索依赖用户反馈（如点击、停留时间）改进排序模型的闭环机制。由于AI搜索流程长（查询分解、文档检索、答案生成）而反馈粒度粗（仅针对最终答案），系统难以定位各环节的优化点。为此，研究提出NExT-Search新范式：1）用户调试模式允许介入关键环节；2）影子用户模式通过AI代理模拟偏好。反馈信号将用于实时调整（在线适应）和定期模型更新（离线训练），重建人机协同的改进闭环，推动AI搜索持续进化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T07:01:00Z
- **目录日期**: 2025-05-21
