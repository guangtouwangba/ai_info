...
生成式AI搜索通过提供端到端答案改变了信息检索方式，但削弱了传统搜索依赖用户反馈（如点击、停留时间）优化排序模型的闭环机制。由于AI搜索流程长（查询分解、文档检索、答案生成），而反馈仅针对最终结果，导致各环节难以针对性改进。为此，研究提出NExT-Search新范式：1）调试模式允许用户干预关键步骤；2）影子模式通过AI代理模拟用户偏好提供反馈。结合实时在线调整与离线模型更新，该系统能重建细粒度反馈循环，推动AI搜索持续进化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:01:04Z
- **目录日期**: 2025-05-21
