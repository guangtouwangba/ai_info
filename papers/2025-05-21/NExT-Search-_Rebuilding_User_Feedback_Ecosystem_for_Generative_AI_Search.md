...
生成式AI搜索通过提供端到端答案简化了信息检索，但也打破了传统搜索依赖用户反馈（如点击、停留时间）改进排名的闭环机制。由于反馈仅针对最终答案，难以优化中间环节（如查询分解、文档检索）。为此，NExT-Search提出新范式：1）"用户调试模式"允许干预关键步骤；2）"影子用户模式"通过AI代理模拟用户偏好。反馈信号将用于实时调整（在线适应）和周期性模型优化（离线更新）。该框架旨在重建细粒度反馈循环，使生成式AI搜索能持续演进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T20:00:58Z
- **目录日期**: 2025-05-21
