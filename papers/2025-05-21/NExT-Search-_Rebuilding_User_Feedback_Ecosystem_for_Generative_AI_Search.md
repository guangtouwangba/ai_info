...
生成式AI搜索通过提供端到端答案简化了信息检索，但也破坏了传统搜索基于用户反馈的改进机制。传统搜索依赖文档级细粒度反馈（如点击、停留时间）优化排名模型，而AI搜索的复杂流程（查询分解、检索、生成）仅获得最终答案的粗粒度反馈，导致改进困难。为此，论文提出NExT-Search新范式：1）用户调试模式允许干预关键步骤；2）影子用户模式通过AI代理模拟偏好。反馈信号可实时优化当前搜索（在线适应）或周期性更新模型（离线调整），重建人机协同的反馈闭环，推动AI搜索持续进化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T21:01:02Z
- **目录日期**: 2025-05-21
