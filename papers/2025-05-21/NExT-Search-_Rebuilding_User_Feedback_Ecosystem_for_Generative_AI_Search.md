...
生成式AI搜索通过提供端到端的复杂查询答案，改变了传统信息检索模式，减少了用户手动浏览和总结的需求。然而，这种模式削弱了传统搜索依赖的用户反馈改进机制，因为生成式搜索的反馈仅针对最终答案，难以映射到中间环节（如查询分解、文档检索和答案生成）。为解决这一问题，NExT-Search提出新范式：通过"用户调试模式"让用户干预关键步骤，以及"影子用户模式"用AI代理模拟用户偏好。结合实时在线调整和离线模型更新，该系统旨在重建细粒度反馈循环，推动AI搜索持续优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T06:00:57Z
- **目录日期**: 2025-05-21
