...
当前基于大语言模型(LLM)的聊天机器人在生成连贯回复方面表现优异，但在把握对话时机(特别是简短即时回应)上存在不足。研究团队构建了首个多模态对话数据集，包含视频、音频和文本的时序对齐数据，并提出MM-When2Speak模型。该模型通过整合视觉、听觉和文本信息，能自适应预测最佳回应时机和类型。实验显示，该模型在回应时机准确性上比主流商业LLM提升4倍，显著优于单模态基线。研究证明多模态输入对实现自然流畅的人机对话至关重要，为提升AI对话的及时性和交互性提供了新思路。(99字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T05:02:16Z
- **目录日期**: 2025-05-21
