...
当前基于大语言模型(LLM)的聊天机器人在生成连贯回复方面表现优异，但在把握对话时机(如简短即时反应)上仍有不足，主要因其仅依赖文本输入而缺乏真实对话中的多模态线索。为此，研究者构建了包含视觉、听觉和文本对齐数据的新型多模态对话数据集，并提出MM-When2Speak模型，通过整合多模态上下文来预测最佳回应时机和类型。实验表明，该模型在回应时机准确率上比主流商业LLM提升高达4倍，显著优于单模态基线，证实多模态输入对实现自然流畅的人机对话至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T06:02:11Z
- **目录日期**: 2025-05-21
