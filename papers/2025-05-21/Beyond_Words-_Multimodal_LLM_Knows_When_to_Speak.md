...
当前基于大语言模型(LLM)的聊天机器人在生成连贯回复方面表现优异，但在把握对话时机方面存在不足，尤其是难以产生简短即时的反应。为解决这一问题，研究者构建了包含视觉、听觉和文本的多模态对话数据集，并提出了MM-When2Speak模型。该模型能整合多模态信号，预测最佳回应时机和类型。实验表明，该模型在回应时机准确性上比现有商业LLM提升高达4倍，显著优于单模态基线，证实多模态输入对实现自然流畅对话AI的关键作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T11:02:22Z
- **目录日期**: 2025-05-21
