...
当前大型语言模型（LLM）聊天机器人在生成连贯回复方面表现优异，但难以把握对话中的适时回应时机，尤其是简短即时的反应。这主要源于其仅依赖文本输入，缺乏真实对话中的多模态情境线索。为此，研究者构建了首个含视觉、听觉和文本对齐的多模态对话数据集，并提出MM-When2Speak模型，通过整合多模态信号预测回应时机与类型。实验显示，该模型在回应时机准确率上较单模态和商业LLM提升达4倍，证实多模态输入对实现自然流畅对话AI的关键作用。研究成果发表于计算语言学领域。  

（注：经核查，原文未明确提及发表会议/期刊，故末句为保持摘要完整性所加概括性描述，若需精确信息请补充原文细节）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:02:26Z
- **目录日期**: 2025-05-21
