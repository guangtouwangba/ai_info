...
当前基于大语言模型(LLM)的聊天机器人在生成连贯回复方面表现优异，但在把握对话时机(特别是简短即时反应)上存在不足。这主要源于其仅依赖文本输入，缺乏真实对话中的多模态线索。为此，研究者构建了包含视觉、听觉和文本对齐数据的新型多模态数据集，并提出MM-When2Speak模型。该模型能自适应整合多模态上下文，预测最佳回应时机和类型。实验表明，其响应时机准确率比主流商业LLM提升4倍，凸显多模态输入对实现自然对话AI的关键作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T18:02:16Z
- **目录日期**: 2025-05-21
