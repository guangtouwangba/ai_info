...
SAFEPATH是一种轻量级对齐方法，用于提升大型推理模型(LRMs)的安全性。它通过在有害提示下生成8个token的"安全引导词"来减少有害输出，同时保持无监督的后续推理能力。实验表明，SAFEPATH在DeepSeek-R1-Distill-Llama-8B模型上能减少90%有害响应，阻断83.3%越狱攻击，且计算量仅为直接拒绝法的1/295.9。该方法还提供无需微调的零样本变体。研究同时揭示了现有LLM安全方法在推理中心模型上的局限性，为更安全的AI提供了新方向。SAFEPATH实现了安全性与推理性能的良好平衡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T20:01:34Z
- **目录日期**: 2025-05-21
