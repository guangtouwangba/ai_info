...
研究人员提出了SAFEPATH，一种轻量级对齐方法，通过微调大型推理模型（LRMs）使其在有害提示下生成简短的安全提示（8个token），同时保持后续推理不受监督。实验表明，SAFEPATH显著减少有害输出（降低90%），并阻挡83.3%的越狱攻击，计算成本比现有方法低数百倍。此外，还提出了无需微调的零样本变体。研究还分析了现有方法在推理中心模型中的适用性，揭示了关键差距，为更安全的AI提供了新方向。该方法在保持推理性能的同时，有效提升了安全性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T08:02:00Z
- **目录日期**: 2025-05-21
