...
研究人员提出了SAFEPATH，这是一种轻量级对齐方法，用于提升大型推理模型(LRM)的安全性。该方法通过微调模型，使其在面对有害提示时首先生成一个8个token的"安全提示"，而后续推理过程不受监督。实验表明，SAFEPATH在DeepSeek-R1-Distill-Llama-8B模型上能减少90%的有害输出，并阻挡83.3%的越狱攻击，同时计算量比现有方法低295-314倍。研究还提出了无需微调的零样本变体，并分析了现有LLM安全方法在推理模型上的适用性，为更安全的AI发展提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T17:01:42Z
- **目录日期**: 2025-05-21
