...
SAFEPATH是一种轻量级对齐方法，用于提升大型推理模型(LRMs)的安全性。它通过在有害提示下生成8个token的"安全前导词"来阻止不安全输出，同时保持后续推理不受监督。实验表明，SAFEPATH在DeepSeek-R1-Distill-Llama-8B模型上能减少90%的有害输出，并阻挡83.3%的越狱攻击，计算量比现有方法低295-314倍。该方法还提供了无需微调的零样本变体。研究还分析了现有LLM安全方法在推理模型上的适用性，揭示了关键差距，为更安全的AI提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T05:01:44Z
- **目录日期**: 2025-05-21
