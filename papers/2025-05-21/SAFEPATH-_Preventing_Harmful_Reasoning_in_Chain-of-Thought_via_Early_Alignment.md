...
SAFEPATH是一种轻量级对齐方法，通过微调大型推理模型（LRMs），使其在面对有害提示时生成8个token的"安全引导词"（Safety Primer），同时保持后续推理过程不受监督。实验表明，SAFEPATH在多个基准测试中显著减少有害输出（降幅达90%），并阻挡83.3%的越狱攻击，同时计算成本比现有方法低295-314倍。该方法还提供了无需微调的零样本变体。研究还分析了现有LLM安全方法在推理中心模型上的适用性，揭示了关键差距，为更安全的AI发展提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T21:01:39Z
- **目录日期**: 2025-05-21
