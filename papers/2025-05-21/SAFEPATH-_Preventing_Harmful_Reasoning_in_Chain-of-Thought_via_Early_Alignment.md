...
SAFEPATH是一种轻量级对齐方法，通过微调大型推理模型(LRMs)，使其在面对有害提示时首先生成一个8个token的"安全引导词"，而后续推理过程不受监督。实验表明，SAFEPATH在多个基准测试中能减少90%的有害输出，并阻挡83.3%的越狱攻击，同时保持推理性能，计算量比传统方法减少295-314倍。研究还提出了无需微调的零样本变体，并分析了现有LLM安全方法在推理模型中的适用性差距，为更安全的AI提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:01:49Z
- **目录日期**: 2025-05-21
