...
SAFEPATH是一种轻量级对齐方法，通过微调大型推理模型(LRMs)，使其在面对有害提示时生成8个token的安全提示(Safety Primer)，同时保持后续推理过程不受监督。实验表明，SAFEPATH在DeepSeek-R1-Distill-Llama-8B模型上能减少90%的有害输出，并拦截83.3%的越狱攻击，计算量比Direct Refusal和SafeChain分别减少295.9倍和314.1倍。该方法还推出了无需微调的零样本变体。研究同时揭示了现有LLM安全方法在推理中心模型上的局限性，为更安全的AI提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T22:01:34Z
- **目录日期**: 2025-05-21
