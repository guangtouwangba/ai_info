...
2025年独立开发者Noel Berry提出无需游戏引擎的手搓游戏方案：使用C#（支持热重载和实时调试）配合SDL3（跨平台视窗/输入/渲染）、FMOD（音频）等开源库。资源管理采用按需加载，关卡设计用LDtk/Tiled等工具+数据转换脚本，UI开发依赖Dear ImGui。C#的Native-AOT编译链和SDL3的跨平台支持简化移植流程。该方法强调轻量化工具链，适合小型团队快速迭代，通过组合专业库实现"从零开发"的高效工作流。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T21:04:00Z
- **目录日期**: 2025-05-21
