...
2025年独立游戏开发者Noel Berry提出"不用引擎手搓游戏"方法：1) 采用改进后的C#（支持热重载和实时调试）；2) 使用SDL3等开源库处理视窗/输入/渲染，FMOD处理音频；3) 直接管理资源加载/释放；4) 集成LDtk/Tiled等现成编辑器工具，配合ImGui快速开发UI；5) 通过C#的Native-AOT和SDL3实现跨平台移植。该方法适合小型团队，通过组合开源工具避免商业引擎限制，同时保持开发灵活性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T14:04:27Z
- **目录日期**: 2025-05-21
