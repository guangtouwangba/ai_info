...
2025年不用游戏引擎开发游戏的方法：资深开发者Noel Berry建议使用C#（支持热重载和实时调试）配合开源框架（如SDL3处理跨平台视窗/输入/渲染、FMOD管理音频）。资源管理采用按需加载，利用LDtk/Tiled等工具设计关卡，用Dear ImGui快速构建UI。C#的Native-AOT编译和SDL3的跨平台支持简化移植流程。该方法通过组合轻量级工具链，避免大型引擎的复杂性，特别适合小型团队开发2D/像素游戏。（注：图片内容涉及具体工具演示）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:04:13Z
- **目录日期**: 2025-05-21
