...
2025年独立开发者Noel Berry提出"不用游戏引擎手搓游戏"的方法：使用C#（支持热重载和实时调试）配合SDL3等开源库处理视窗/输入/渲染，FMOD处理音频。资源管理采用按需加载，利用LDtk/Tiled等现成工具做关卡设计，Dear ImGui创建UI。C#的Native-AOT编译和SDL3的跨平台支持简化移植流程。这种方法适合小团队，通过组合轻量级工具链避免商业引擎的复杂性，保持开发灵活性。关键优势在于直接控制代码、快速迭代和避免引擎限制。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T19:03:49Z
- **目录日期**: 2025-05-21
