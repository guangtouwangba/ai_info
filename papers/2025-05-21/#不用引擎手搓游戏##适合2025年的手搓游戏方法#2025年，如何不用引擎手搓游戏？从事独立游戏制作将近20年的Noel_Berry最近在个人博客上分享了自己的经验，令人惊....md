...
2025年独立开发者Noel Berry提出"不用游戏引擎手搓游戏"的方法：1) 使用C#语言，其热重载和反射功能简化开发；2) 采用SDL3等开源库处理视窗/输入/渲染，FMOD处理音频；3) 直接管理资源，按需加载；4) 利用LDtk/Tiled等现成工具做关卡设计，ImGui创建UI；5) 通过C#的Native-AOT实现跨平台移植。该方法适合小型团队，通过组合轻量级工具链替代重型引擎，保持开发灵活性和控制力。SDL3的跨平台支持和C#的演进使该方案在2025年更具可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T13:13:40Z
- **目录日期**: 2025-05-21
