...
2025年独立游戏开发者Noel Berry提出"不用游戏引擎"的开发方法，建议使用C#（支持热重载和实时调试）结合开源框架（如SDL3处理跨平台渲染/输入、FMOD处理音频）。资源管理采用按需加载，关卡设计用Tiled/LDtk等工具，UI开发用Dear ImGui。C#的Native-AOT编译和SDL3的跨平台支持使移植更便捷。该方法适合小型团队，通过组合轻量级工具实现高效开发，避免大型引擎的复杂性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T20:03:52Z
- **目录日期**: 2025-05-21
