...
大语言模型(LLMs)催生了一类新型的非结构化文档数据处理应用。虽然基于语义操作符(如LLM驱动的映射、过滤、连接等)的系统在基准测试中表现优异，但其优化仍具挑战性。研究者提出Abacus——一个可扩展的基于成本的优化器，它通过少量验证样例和先验知识评估操作符性能，在质量、成本和延迟维度上寻找最优实现方案。实验表明，在生物医学(BioDEX)、法律(CUAD)文档处理及多模态问答(MMQA)任务中，Abacus优化的系统比次优方案质量提升18.7%-39.2%，成本降低最高23.6倍，延迟减少最高4.2倍。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T15:02:06Z
- **目录日期**: 2025-05-21
