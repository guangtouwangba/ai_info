...
大语言模型(LLMs)催生了一类新型的非结构化文档数据处理应用。最新编程框架允许开发者通过组合语义操作符(如LLM驱动的映射、过滤、连接等)来构建这些应用。然而，现有优化器在优化系统质量、成本和延迟方面存在局限。本文提出Abacus，一个基于成本的可扩展优化器，通过少量验证样例和先验知识评估操作符性能，在生物医学(BioDEX)、法律(CUAD)和多模态问答(MMQA)任务中，优化后的系统质量提升18.7%-39.2%，成本降低23.6倍，延迟减少4.2倍。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T07:01:55Z
- **目录日期**: 2025-05-21
