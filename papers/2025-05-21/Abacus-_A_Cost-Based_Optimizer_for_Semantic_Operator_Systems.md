...
大型语言模型(LLMs)催生了一类新型数据处理应用，通过语义运算符(如LLM驱动的映射、过滤、连接等)处理非结构化文档。现有优化器存在局限性，无法在多维度约束下优化系统质量、成本或延迟。本文提出Abacus，一种基于成本的扩展优化器，利用少量验证示例和先验知识评估运算符性能，搜索最优实现方案。在生物医学(BioDEX)、法律(CUAD)文档处理和多模态问答(MMQA)任务中，Abacus优化后的系统质量提升18.7%-39.2%，成本降低23.6倍，延迟减少4.2倍，显著优于现有方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T04:03:00Z
- **目录日期**: 2025-05-21
