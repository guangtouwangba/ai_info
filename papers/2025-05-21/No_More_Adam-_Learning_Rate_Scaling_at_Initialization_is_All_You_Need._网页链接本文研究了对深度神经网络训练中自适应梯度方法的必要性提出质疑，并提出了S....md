...
该研究提出SGD-SaI方法，取代自适应梯度优化器（如Adam）。该方法基于梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，无需二阶动量计算。SGD-SaI显著减少内存占用（如为GPT-2节省5.93GB），在Transformer任务、ImageNet分类和LLM预训练中表现优于或持平AdamW，同时增强超参数鲁棒性。此外，在LoRA微调等任务中超越现有优化器，解决了传统SGD训练Transformer的难题，具有高效、通用和内存友好的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T03:22:19Z
- **目录日期**: 2025-05-21
