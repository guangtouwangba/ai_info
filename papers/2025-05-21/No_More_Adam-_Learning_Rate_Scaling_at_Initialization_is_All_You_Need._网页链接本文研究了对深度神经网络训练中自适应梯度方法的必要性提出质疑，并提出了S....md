...
该论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法通过初始化阶段基于梯度信噪比（g-SNR）对不同参数组进行学习率调整，无需自适应二阶动量，既解决了训练不平衡问题，又将内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及大模型（如GPT-2、Llama2-7B）预训练中性能媲美或超越AdamW，尤其在内存效率上优势显著（节省5.93GB~25.15GB）。其鲁棒性和广泛适用性使其成为自适应优化器的潜在替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T12:04:59Z
- **目录日期**: 2025-05-21
