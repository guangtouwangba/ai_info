...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，有效防止训练不平衡。相比AdamW，该方法内存使用减半，并在Transformer任务、ImageNet分类和GPT-2预训练等场景中表现相当或更优。特别在大型语言模型（如GPT-2、Llama2-7B）中，SGD-SaI显著节省内存（最高达25GB），同时保持优异性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T15:05:12Z
- **目录日期**: 2025-05-21
