...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI通过根据梯度信号噪声比(g-SNR)在初始化时调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。相比AdamW，该方法无需二阶动量计算，内存使用减半，且在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优。实验显示，SGD-SaI为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，并在LoRA微调等任务中优于现有优化器，具有更好的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T00:04:14Z
- **目录日期**: 2025-05-21
