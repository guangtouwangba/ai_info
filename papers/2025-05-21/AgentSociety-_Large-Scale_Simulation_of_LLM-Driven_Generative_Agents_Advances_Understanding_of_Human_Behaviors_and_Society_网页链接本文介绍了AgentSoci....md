...
AgentSociety是一个基于大型语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会生活及500万次互动。研究者利用该平台研究了极化现象、刺激性信息传播、无条件基本收入政策及外部冲击（如飓风）等社会问题。模拟结果与现实世界实验数据高度吻合，验证了其捕捉人类行为和社会机制的能力。这一工具为社会科学研究和政策制定提供了新方法，能够辅助传统研究手段（如调查和干预）深入分析社会问题的模式、原因和潜在机制，展现出重要的应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T22:04:48Z
- **目录日期**: 2025-05-21
