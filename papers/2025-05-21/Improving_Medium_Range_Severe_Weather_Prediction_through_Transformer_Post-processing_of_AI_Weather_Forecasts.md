...
本研究提出了一种基于解码器-仅Transformer网络的新方法，用于后处理AI天气预报模型（如盘古天气）的中期（1-8天）强天气预测。与传统使用密集神经网络处理离散预报样本不同，该方法将预报时间序列视为"标记"，使Transformer能学习大气状态的复杂时间关系。实验表明，基于Transformer的后处理显著优于传统神经网络，且盘古天气的高分辨率初始化预报即使不含对流参数，其表现也优于全球预报系统（GFS）。该方法提高了预测准确性、可靠性，并通过特征归因分析增强了可解释性，推动了中期强天气预报能力的发展。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-21T09:02:51Z
- **目录日期**: 2025-05-21
