...
TokenHSI提出了一种基于Transformer的统一策略模型，用于合成多样且物理合理的人-场景交互（HSI）。该方法通过将人体本体感知建模为共享标记，并结合任务特定的标记与掩码机制，实现了多技能的统一和灵活适应。相比现有专注于单一任务的控制器，TokenHSI支持跨技能知识共享和多任务训练，并能通过可变长度输入适应新场景。通过训练额外任务标记器，可调整交互目标的几何形状并协调多技能处理复杂任务。实验表明，该方法显著提升了HSI任务的通用性、适应性和可扩展性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T18:01:06Z
- **目录日期**: 2025-03-26
