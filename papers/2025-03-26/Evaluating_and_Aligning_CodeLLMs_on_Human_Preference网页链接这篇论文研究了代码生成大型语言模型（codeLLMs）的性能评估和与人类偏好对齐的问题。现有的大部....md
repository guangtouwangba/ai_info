...
这篇论文探讨了代码生成大模型（codeLLMs）如何更好地与人类偏好对齐。现有基准主要关注代码正确性，忽略了人类偏好匹配。为此，研究者构建了CodeArena基准（含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令集（近200亿标记）。实验发现执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显了人类偏好对齐的重要性。研究证实大规模合成指令微调能有效提升模型表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T16:06:24Z
- **目录日期**: 2025-03-26
