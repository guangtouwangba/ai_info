...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本(覆盖40类44种语言)，以评估模型在实际场景中的表现。同时开发了20B标记的合成指令数据集SynCode-Instruct用于微调验证。实验发现：1)传统执行基准与人类偏好评估存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型除正确性外，与人类编程习惯和偏好的对齐同样重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T10:06:16Z
- **目录日期**: 2025-03-26
