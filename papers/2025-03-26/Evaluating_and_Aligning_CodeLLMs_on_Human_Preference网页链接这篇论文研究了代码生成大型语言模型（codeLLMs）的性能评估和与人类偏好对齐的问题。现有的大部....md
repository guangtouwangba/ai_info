...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好对齐的问题。研究者创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，以评估模型在真实编程任务中的表现。同时开发了SynCode-Instruct合成指令数据集（近200亿标记）验证大规模微调效果。实验发现现有基准测试与人类偏好存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显了模型对齐人类偏好的重要性。研究强调了代码生成质量评估需超越单纯正确性考量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T02:36:52Z
- **目录日期**: 2025-03-26
