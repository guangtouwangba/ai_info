...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)建立CodeArena基准测试(397个样本/40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度；2)构建20B标记的SynCode-Instruct指令集验证合成数据微调效果。研究发现：现有基于执行的测试与人类偏好存在偏差；开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调，除代码正确性外，与开发者偏好对齐是提升实用性的关键维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T18:05:17Z
- **目录日期**: 2025-03-26
