...
这篇论文针对代码生成大模型（codeLLMs）提出两个关键贡献：1）创建CodeArena基准测试，包含397个真实编程场景样本（覆盖40类任务和44种语言），首次系统评估模型输出与人类偏好的匹配度；2）构建20B标记量的SynCode-Instruct合成指令集验证微调效果。研究发现：现有执行基准与人类偏好存在偏差，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调了代码生成模型除正确性外，还需关注开发者体验和实际需求对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T08:05:50Z
- **目录日期**: 2025-03-26
