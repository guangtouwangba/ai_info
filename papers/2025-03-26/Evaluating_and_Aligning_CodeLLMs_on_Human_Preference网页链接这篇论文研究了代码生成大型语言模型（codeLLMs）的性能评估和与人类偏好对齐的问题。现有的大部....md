...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1) 创建CodeArena基准测试，包含397个真实编程任务样本(覆盖40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记的SynCode-Instruct合成指令集验证微调效果。研究发现：传统执行测试与人类偏好存在偏差，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)存在显著差距。研究强调代码LLMs需同时关注正确性和人类偏好对齐，为模型优化提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T19:05:09Z
- **目录日期**: 2025-03-26
