...
AdaWorld是一种创新的世界模型学习方法，通过自监督方式从视频中提取潜在动作，捕捉帧间关键变化，并在预训练中融入动作信息。该方法构建了一个基于潜在动作的自回归世界模型，显著提升了模型对新环境和异构动作的适应能力。相比传统依赖大量标注数据和昂贵训练的方法，AdaWorld仅需有限交互和微调即可高效迁移和学习新动作。多环境实验表明，AdaWorld在仿真质量和视觉规划方面均表现出色，解决了现有世界模型跨领域应用的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T00:01:34Z
- **目录日期**: 2025-03-26
