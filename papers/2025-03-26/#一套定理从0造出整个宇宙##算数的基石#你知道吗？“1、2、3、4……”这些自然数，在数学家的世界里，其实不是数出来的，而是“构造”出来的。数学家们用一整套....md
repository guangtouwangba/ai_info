...
皮亚诺公理从零构建自然数体系：以0为起点，通过后继函数S(n)递归生成数列（S(0)=1,S(1)=2），并设定关键约束——0非任何数后继、不同数后继必异，确保数列无限不循环。这种形式化构造不仅奠定数学基石（自然数→整数→实数），更深刻影响计算机科学（运算逻辑/编程语言）和密码学等领域。公理系统通过严谨的"防作弊机制"（如排除非常规元素）建立了现代科技依赖的底层逻辑框架，使简单数数行为背后蕴含精妙的公理化思维创举。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T05:02:06Z
- **目录日期**: 2025-03-26
