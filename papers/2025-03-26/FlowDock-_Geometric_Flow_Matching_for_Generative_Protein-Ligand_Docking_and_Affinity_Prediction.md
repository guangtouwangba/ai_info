...
FlowDock是一种基于条件流匹配的新型深度几何生成模型，能够直接将未结合(apo)蛋白结构映射到任意数量配体结合(holo)的复合结构。它不仅能预测结构置信度，还能估计结合亲和力，支持多配体同时建模和虚拟筛选。在PoseBusters基准测试中，FlowDock以51%的成功率超越单序列AlphaFold 3；在DockGen-E数据集上表现与Chai-1相当；在CASP16竞赛中跻身配体结合亲和力预测前5名。该模型无需多序列比对信息，展示了在药物发现中的应用潜力。代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T03:12:25Z
- **目录日期**: 2025-03-26
