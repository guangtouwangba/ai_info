...
FlowDock是一种基于条件流匹配的深度几何生成模型，能直接将未结合（apo）结构映射到任意数量配体结合的（holo）结构，支持灵活的多配体对接和亲和力预测。相比单序列AlphaFold 3，FlowDock在未结合蛋白输入且无需多序列比对信息的情况下，盲对接成功率提高51%（PoseBusters数据集）。在DockGen-E数据集上，其性能与单序列Chai-1相当。在CASP16竞赛中，FlowDock位列前5（140个蛋白-配体复合物的亲和力预测）。该模型提供结构置信度和亲和力评分，可用于快速虚拟筛选。代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T02:32:14Z
- **目录日期**: 2025-03-26
