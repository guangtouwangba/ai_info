...
AgentSociety是一个基于大语言模型的大规模社会模拟器，能模拟超过1万个代理的社会互动（500万次）。研究者用它分析四大社会问题：社会极化、刺激性信息传播、无条件基本收入政策及飓风等外部冲击的影响。模拟结果与现实数据吻合，验证了其捕捉人类行为机制的能力。该系统可作为计算社会科学实验平台，辅助传统研究方法（如调查、访谈），帮助学者和政策制定者理解社会问题的成因与模式，展现了LLM在社会模拟中的实用价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T02:36:14Z
- **目录日期**: 2025-03-26
