...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，能模拟1万多个代理的社会互动（500万次交互）。研究者用它分析四大社会问题：社会极化、刺激性信息传播、基本收入政策影响和飓风等外部冲击。模拟结果与现实数据吻合，验证了该系统捕捉人类行为机制的能力，展现了其作为社会科学研究和政策制定工具的潜力。该系统结合LLM代理、真实社会环境和大规模模拟引擎，为传统研究方法（如调查、访谈）提供了补充支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T12:05:51Z
- **目录日期**: 2025-03-26
