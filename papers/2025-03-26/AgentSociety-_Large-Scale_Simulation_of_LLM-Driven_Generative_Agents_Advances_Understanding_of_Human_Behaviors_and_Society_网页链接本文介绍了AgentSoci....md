...
本文介绍了AgentSociety，这是一个基于大语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（共500万次）。该系统被用于研究四个关键社会问题：极化现象、刺激性信息传播、无条件基本收入政策影响以及外部冲击（如飓风）的影响。模拟结果与现实世界实验数据高度吻合，验证了其捕捉人类行为机制的能力。AgentSociety展示了作为社会科学研究和政策制定工具的潜力，能够辅助传统研究方法（如调查、访谈和干预），帮助理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T03:15:16Z
- **目录日期**: 2025-03-26
