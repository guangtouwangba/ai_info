...
谷歌与普林斯顿、NYU联合研究发现，Whisper语音模型的内部处理机制与人类大脑语言处理路径高度吻合。该模型未经大脑数据训练，却能精准预测大脑各区域在语音理解（STG→IFG）和语言生成（IFG→MC→STG）时的激活顺序与时间点，包括声音感知、语义解析、发音准备及自我监听等环节。研究表明，AI与大脑在预测性编码、上下文表征等底层计算逻辑上存在共性，这一发现可能推动脑机接口和认知神经科学发展，使AI成为解码人类思维的潜在工具。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T23:10:12Z
- **目录日期**: 2025-03-26
