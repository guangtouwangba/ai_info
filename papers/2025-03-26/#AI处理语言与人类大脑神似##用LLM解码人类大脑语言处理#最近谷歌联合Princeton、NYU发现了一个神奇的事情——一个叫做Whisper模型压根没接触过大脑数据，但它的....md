...
谷歌与普林斯顿、NYU合作研究发现，Whisper语音模型的内部处理机制与人类大脑语言处理路径高度吻合。模型各层嵌入能精准预测大脑不同区域（STG听觉区、IFG语义区、MC运动皮层）的激活时序：听语音时从声音到语义（STG→IFG），说话时从语义到发音（IFG→MC→STG），甚至包含自我监听环节。这种时空一致性表明AI与大脑可能共享底层计算逻辑，如上下文预测和嵌入表示。该发现为脑机接口和认知科学研究提供了新视角，或使AI成为解码人类思维的桥梁。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T21:03:00Z
- **目录日期**: 2025-03-26
