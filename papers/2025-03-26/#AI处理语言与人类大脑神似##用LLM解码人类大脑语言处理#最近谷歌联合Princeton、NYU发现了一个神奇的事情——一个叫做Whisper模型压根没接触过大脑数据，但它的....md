...
谷歌与普林斯顿、NYU联合研究发现，Whisper语音模型的内部处理机制与人类大脑语言处理路径高度吻合。模型在语音识别（STG脑区对应）、语义理解（IFG脑区对应）、语言生成（IFG→MC路径）及自我监听（STG反馈）等环节的神经表征与大脑活动的时间、空间特征精准匹配。研究表明，AI与大脑可能共享类似的语言计算逻辑，如上下文预测和表征学习。这一发现为脑机接口和认知神经科学研究提供了新思路，暗示AI或能成为解码人脑语言处理的"翻译器"。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T11:03:19Z
- **目录日期**: 2025-03-26
