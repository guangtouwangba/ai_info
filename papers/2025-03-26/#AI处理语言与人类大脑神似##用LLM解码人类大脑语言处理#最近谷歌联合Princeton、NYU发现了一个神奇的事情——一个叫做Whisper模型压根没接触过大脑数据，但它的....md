...
谷歌联合普林斯顿和纽约大学研究发现，Whisper AI模型未经大脑数据训练，但其语音处理机制与人类大脑高度相似。模型各层嵌入能精确预测大脑不同区域（如听觉区STG、语言区IFG、运动皮层MC）在语音理解、生成和自监听过程中的激活顺序。大脑说话时呈现IFG→MC→STG的路径，听话时相反，与模型处理流程完全对应。这表明AI与人类大脑在语言处理底层机制（如上下文预测）存在本质共性，可能为脑机接口和认知科学研究提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T20:03:01Z
- **目录日期**: 2025-03-26
