...
谷歌与普林斯顿、NYU联合研究发现，Whisper语音模型的内部处理机制与人类大脑处理语言的神经活动高度吻合。该模型虽未接触大脑数据，但其语音嵌入层能精准预测听觉皮层（STG）对声音信号的反应，语言嵌入层则对应语义理解阶段（IFG激活）。在语言产出时，模型与大脑同样遵循"构思→发音→自我监听"的流程，时间节点和脑区激活顺序完全同步。研究表明，AI与人类大脑在语言处理的底层计算逻辑上存在本质相似性，这一发现可能为脑机接口和认知神经科学研究提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T14:04:57Z
- **目录日期**: 2025-03-26
