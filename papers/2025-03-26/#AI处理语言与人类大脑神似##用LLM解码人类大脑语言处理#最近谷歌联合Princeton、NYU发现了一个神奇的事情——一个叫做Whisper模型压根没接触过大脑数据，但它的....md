...
谷歌与普林斯顿、NYU联合研究发现，Whisper语音模型的内部处理机制与人类大脑语言处理路径高度吻合。实验显示：1）模型语音嵌入层能精准预测大脑听觉区（STG）对声音信号的反应；2）语言嵌入层与语义理解区（IFG）的激活同步；3）说话时模型与大脑均遵循"语义构思→运动输出→听觉反馈"的流程。这种时空层面的高度对齐表明，AI与大脑可能共享类似的语言计算逻辑。该发现为脑机接口和认知科学研究提供了新方向，或使AI成为解码人类思维的桥梁。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T13:10:12Z
- **目录日期**: 2025-03-26
