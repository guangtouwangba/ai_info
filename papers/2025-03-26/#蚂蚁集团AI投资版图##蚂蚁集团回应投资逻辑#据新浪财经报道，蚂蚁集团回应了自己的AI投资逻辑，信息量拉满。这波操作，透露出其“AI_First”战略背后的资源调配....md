...
蚂蚁集团近期披露了其AI投资布局，展现出"AI First"战略。投资覆盖多个领域：大模型方向包括智谱AI（通用语言模型）和月之暗面（算法优化）；算力芯片领域投资墨芯科技（AI加速芯片）和清微智能（低功耗芯片）；AI Agent方向支持深度赋智（多智能体框架）；具身智能领域布局星海图（机器人AI融合）；应用层面投资秘塔科技（语言工具）、爱诗科技（视频生成）等。其逻辑认为AI正从概念转向产业化，重点关注大模型与具身智能的融合创新。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T06:03:03Z
- **目录日期**: 2025-03-26
