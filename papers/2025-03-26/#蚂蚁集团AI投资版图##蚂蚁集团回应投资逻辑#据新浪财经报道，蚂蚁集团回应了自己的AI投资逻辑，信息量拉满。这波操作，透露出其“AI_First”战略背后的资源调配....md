...
蚂蚁集团披露其AI投资战略布局，覆盖多个关键领域：大模型方向投资智谱AI（通用语言模型）和月之暗面（算法优化）；算力芯片方向布局墨芯科技（AI加速芯片）和清微智能（低功耗芯片）；AI Agent领域支持深度赋智（多智能体框架）；具身智能领域投资星海图（机器人融合技术）；应用层涉及秘塔科技（大模型工具）、爱诗科技（AI视频）等。其核心逻辑是推动AI技术从概念向产业化过渡，重点关注大模型与具身智能的融合创新。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T10:04:28Z
- **目录日期**: 2025-03-26
