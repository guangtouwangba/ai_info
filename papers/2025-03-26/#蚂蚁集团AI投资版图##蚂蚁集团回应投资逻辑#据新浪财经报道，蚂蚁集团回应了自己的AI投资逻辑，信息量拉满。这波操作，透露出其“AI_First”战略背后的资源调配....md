...
蚂蚁集团近期披露了其AI投资布局，展现出"AI First"战略。投资覆盖多个关键领域：大模型方向包括智谱AI（通用语言模型）和月之暗面（算法优化）；AI芯片领域投资了墨芯科技（计算加速）和清微智能（低功耗芯片）；智能体方向布局深度赋智（企业级多智能体框架）；具身智能领域投资星海图（机器人融合技术）；应用层涉及秘塔科技（AI工具）、爱诗科技（视频生成）等。蚂蚁认为AI正从概念走向产业化，特别看好大模型与具身智能的融合潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T15:22:09Z
- **目录日期**: 2025-03-26
