...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩技术(如LZ4和ZSTD)压缩模型权重和键值缓存(KV cache)，在不影响推理质量的情况下，显著减少了内存容量和带宽需求。实验显示，模型权重内存占用减少25.2%，KV缓存减少46.9%。硬件原型在7nm工艺下实现8TB/s吞吐量，面积开销仅3.8mm²。该方法还支持根据上下文动态调整量化精度，使内存带宽和能耗按比例变化，为大规模高效推理提供了可行方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T02:31:35Z
- **目录日期**: 2025-03-26
