...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方案。该方法采用无损块压缩(LZ4/ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的情况下，显著减少了25.2%的权重内存占用和46.9%的KV缓存占用。同时通过动态量化使内存带宽和能耗按比例调整。硬件原型(7nm工艺，4GHz，32通道)实现了8TB/s吞吐量，面积开销仅3.8mm²。实验证明这种LLM感知的内存控制能有效解决内存瓶颈问题，为大规模高效推理提供了可行方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T04:01:56Z
- **目录日期**: 2025-03-26
