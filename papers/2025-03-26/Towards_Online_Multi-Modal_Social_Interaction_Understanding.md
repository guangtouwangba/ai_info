...
本文提出了一种在线多模态社交交互理解框架Online-MMSI-VLM，用于解决实时人机交互中未来上下文缺失的问题。该框架采用两种互补策略：1）多轮对话预测，通过粗粒度（预测说话人轮次）到细粒度（生成对话内容）的方式模拟未来对话；2）社交感知视觉提示，利用边界框和人体关键点突出视频中的社交动态（如眼神和手势）。实验表明，该方法在三个任务和两个数据集上实现了最先进的性能，显著优于基线模型。代码和预训练模型将开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T06:02:14Z
- **目录日期**: 2025-03-26
