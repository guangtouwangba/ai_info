...
该研究提出了一种在线多模态社交交互理解框架（Online-MMSI-VLM），用于解决实时人机交互中未来上下文缺失的问题。该框架采用两种互补策略：1）通过粗粒度到细粒度的多轮对话预测模拟未来话语；2）利用社交感知视觉提示（如边界框和人体关键点）捕捉视频中的社交动态。实验表明，该方法在三个任务和两个数据集上实现了最先进性能，显著优于基线模型。该成果为实时社交交互系统提供了有效解决方案，代码和预训练模型将开源发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T14:03:48Z
- **目录日期**: 2025-03-26
