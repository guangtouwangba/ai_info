...
该论文研究多智能体系统中如何协作满足长期空间约束的问题。每个智能体受限于可能依赖其他智能体位置的空间不等式约束，这些约束需渐近或在有限时间内满足。研究首先将问题建模为集中式无约束优化，通过最大化反映约束满足度的目标函数获得最优配置，促使智能体相互协作。当约束不可行时，系统收敛至最小违规解。随后提出基于分布式共识的优化方案，近似集中式解，并开发了单积分器智能体的分布式控制器。仿真验证了方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T06:01:45Z
- **目录日期**: 2025-03-26
