...
这篇论文研究了多智能体系统中如何协作满足长期空间约束的问题。每个智能体都受到空间不等式约束的影响，这些约束可能取决于其他智能体的位置，无论是否存在直接通信。智能体需要渐近或在未知有限时间内满足约束，最终形成符合所有约束的集体构型。研究首先将其建模为集中式无约束优化问题，通过最大化反映约束满足度的目标函数来获得最优配置；当约束不可行时，智能体会收敛到最小违规解。随后提出了基于分布式共识的优化方案，开发了单积分器智能体的分布式控制器，并通过仿真验证了方法的有效性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T17:01:39Z
- **目录日期**: 2025-03-26
