...
本文研究了多智能体系统中长期空间约束的协同满足问题。每个智能体受限于可能依赖其他智能体位置的空间不等式约束（无论是否存在直接通信），需渐近或在未知有限时间内满足。研究首先将问题建模为集中式无约束优化，通过最大化反映约束满足度的目标函数获得最优配置，该函数促进智能体协作满足彼此约束；当约束不可行时，系统收敛至最小违规解。随后提出分布式共识优化方案逼近集中式解，并开发了单积分器智能体的分布式控制器。仿真验证了方法的有效性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T11:01:43Z
- **目录日期**: 2025-03-26
