...
这篇论文研究了多智能体系统中如何协作满足长期空间约束的问题。每个智能体都受到空间不等式约束的制约，这些约束可能依赖于其他智能体的位置，无论是否存在直接通信。研究目标是使智能体群体渐进或有限时间内满足所有约束条件。论文首先将问题建模为集中式无约束优化，通过最大化反映约束满足度的目标函数获得最优配置。当约束不可行时，系统会收敛到最小违规解。随后提出了基于分布式共识的优化方案，开发了单积分器智能体的分布式控制器。仿真结果验证了该方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T12:02:07Z
- **目录日期**: 2025-03-26
