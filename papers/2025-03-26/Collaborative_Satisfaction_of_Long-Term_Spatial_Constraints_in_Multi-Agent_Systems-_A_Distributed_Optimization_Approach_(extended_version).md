...
该研究提出了一种多智能体系统中协作满足长期空间约束的方法。每个智能体都受到空间不等式约束，这些约束可能依赖于其他智能体的位置，且智能体之间不一定有直接通信。研究首先将问题建模为集中式无约束优化，通过最大化反映约束满足度的目标函数获得最优配置；当约束不可行时，智能体会收敛到最小违规解。随后提出了基于分布式共识的优化方案，近似集中式解，并开发了单积分器智能体的分布式控制器。仿真验证了该方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T08:01:31Z
- **目录日期**: 2025-03-26
