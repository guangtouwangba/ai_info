...
这篇论文研究了多智能体系统中如何协作满足长期空间约束的问题。每个智能体受到空间不等式约束的影响，这些约束可能依赖于其他智能体的位置，无论是否存在直接通信。目标是渐进或在未知有限时间内满足所有约束，实现合规编队。研究首先提出集中式无约束优化框架，通过最大化约束满足度函数获得最优配置；当约束不可行时，智能体收敛至最小违规解。随后开发了基于分布式共识的优化方案，近似集中式解，并设计了单积分器智能体的分布式控制器。仿真验证了方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T10:01:48Z
- **目录日期**: 2025-03-26
