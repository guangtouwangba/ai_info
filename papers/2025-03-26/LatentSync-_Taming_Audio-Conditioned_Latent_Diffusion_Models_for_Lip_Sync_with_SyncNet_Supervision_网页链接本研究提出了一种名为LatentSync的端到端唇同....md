...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，提出TREPA方法利用自监督视频模型增强时序对齐。同时改进了SyncNet训练框架，解决了收敛问题并提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）直接建模音视频关联的潜在扩散架构；2）保持唇同步精度的时序对齐方案；3）优化的SyncNet训练策略。该成果对音频驱动人像动画领域具有推广价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T19:04:51Z
- **目录日期**: 2025-03-26
