...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型的时间表示增强帧间一致性。同时，研究深入分析了SyncNet收敛问题，改进其训练框架显著提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些发现不仅提升了唇同步效果，也为相关音频驱动动画研究提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T12:06:11Z
- **目录日期**: 2025-03-26
