...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示。针对扩散模型的时间一致性问题，提出了时间表示对齐(TREPA)技术，利用自监督视频模型增强生成帧的时序连贯性。同时，研究改进了SyncNet训练框架，解决了其收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进不仅适用于唇同步任务，也可推广到其他音频驱动的人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T08:05:29Z
- **目录日期**: 2025-03-26
