...
本文介绍了基于大语言模型（LLM）的个性化网页代理任务，旨在利用用户个性化数据（如个人资料和历史行为）来更好地理解用户指令并执行定制化操作。为解决该领域缺乏评估基准的问题，研究者构建了PersonalWAB基准测试，包含用户指令、个性化数据、网页功能和两种评估范式。同时，提出了PUMA框架，通过记忆库和任务特定检索策略筛选相关历史行为，并采用微调和直接偏好优化方法使LLM适应个性化网页代理任务。实验证明PUMA在PersonalWAB上优于现有网页代理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-26T04:02:33Z
- **目录日期**: 2025-03-26
