...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过初始化时根据梯度信号噪声比（g-SNR）调整不同参数组的学习率，避免训练不平衡，同时将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或媲美AdamW，并显著节省内存（如为GPT-2节省5.93GB）。此外，它在LoRA微调等任务中也优于现有优化器，且对超参数变化更鲁棒。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T03:22:44Z
- **目录日期**: 2025-05-13
