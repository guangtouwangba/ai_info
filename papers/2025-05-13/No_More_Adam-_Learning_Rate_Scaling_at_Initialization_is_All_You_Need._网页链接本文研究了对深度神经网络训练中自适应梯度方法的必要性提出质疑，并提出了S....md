...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化阶段根据不同参数组的梯度信噪比（g-SNR）调整学习率，解决了训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类、LLM预训练（如GPT-2）和LoRA微调等场景中表现更优或相当。实验显示，SGD-SaI为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，同时保持超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T00:04:44Z
- **目录日期**: 2025-05-13
