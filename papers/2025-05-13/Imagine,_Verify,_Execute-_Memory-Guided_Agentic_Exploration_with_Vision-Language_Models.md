...
IVE是一个受人类好奇心启发的机器人探索框架，利用视觉语言模型(VLMs)将RGB-D观测转换为语义场景图，想象新场景并预测其物理可行性，再通过动作工具生成可执行技能序列。该框架弥补了想象与执行之间的差距，在模拟和真实桌面环境中验证表明，IVE的探索多样性显著优于强化学习基线（状态访问熵提升4.1-7.8倍），且收集的经验能有效支持下游任务学习，其训练出的策略性能接近或超过基于人类示范数据训练的模型。该研究为开放环境中稀疏奖励场景的自主探索提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T20:00:55Z
- **目录日期**: 2025-05-13
