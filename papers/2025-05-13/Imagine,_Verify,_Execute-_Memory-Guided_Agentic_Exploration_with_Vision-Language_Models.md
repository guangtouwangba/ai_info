...
IVE是一个受人类好奇心启发的机器人探索框架，结合视觉语言模型（VLM）实现开放环境中的自主探索。它将RGB-D观测转化为语义场景图，预测新场景的物理可行性，并通过工具生成可执行动作序列。实验证明，IVE在模拟和真实桌面环境中比强化学习基线实现了4.1-7.8倍的探索状态多样性提升，且收集的经验能有效支持下游任务学习，其训练出的策略性能接近或超过基于人类示范数据训练的模型。该框架通过"想象-验证-执行"机制，弥合了语义推理与物理执行间的鸿沟。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T08:00:59Z
- **目录日期**: 2025-05-13
