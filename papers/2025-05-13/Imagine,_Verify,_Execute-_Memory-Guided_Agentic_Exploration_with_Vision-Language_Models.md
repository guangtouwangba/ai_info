...
IVE是一个受人类好奇心启发的机器人探索框架，通过视觉语言模型（VLM）将RGB-D观测抽象为语义场景图，想象新场景并预测其物理可行性，生成可执行动作序列。该框架弥补了想象与执行间的差距，在模拟和真实桌面环境中验证表明，IVE比强化学习基线实现了4.1至7.8倍的探索状态多样性提升，且收集的经验支持下游学习，其训练的策略性能接近或超过基于人类示范数据训练的模型。IVE通过语义推理驱动探索，解决了开放环境中稀疏奖励和任务监督不足的挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T18:01:06Z
- **目录日期**: 2025-05-13
