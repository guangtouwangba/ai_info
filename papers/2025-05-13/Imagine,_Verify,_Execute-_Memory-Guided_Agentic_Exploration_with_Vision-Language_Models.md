...
IVE是一个受人类好奇心启发的机器人探索框架，通过视觉语言模型(VLM)将RGB-D观测转化为语义场景图，想象新场景并预测其物理可行性，再通过工具生成可执行动作序列。该框架在模拟和真实桌面环境中验证，相比强化学习基线，IVE实现了4.1-7.8倍的探索状态熵增，展现出更丰富、有意义的探索能力。收集的经验还支持下游学习，训练出的策略性能接近或超过基于人类示范数据训练的模型。该研究解决了VLM输出缺乏物理基础的问题，弥合了想象与执行间的鸿沟。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T23:00:52Z
- **目录日期**: 2025-05-13
