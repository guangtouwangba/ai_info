...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时优化了SyncNet训练框架，解决了收敛问题并提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。改进后的SyncNet训练经验可推广至其他音频驱动动画任务。该研究突破了传统两阶段生成方法的局限，直接建模音频-视觉复杂关联，显著提升唇同步效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T21:05:12Z
- **目录日期**: 2025-05-13
