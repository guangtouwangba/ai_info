...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。它通过真实社会环境和大规模模拟引擎，研究社会极化、信息传播、基本收入政策及飓风等外部冲击的影响。实验结果显示其模拟结果与现实数据高度吻合，验证了该系统捕捉人类行为机制的能力。该平台可作为计算社会科学研究的工具，辅助传统方法（如调查、访谈），帮助研究者分析社会问题的成因与模式，并为政策制定提供参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T21:04:54Z
- **目录日期**: 2025-05-13
