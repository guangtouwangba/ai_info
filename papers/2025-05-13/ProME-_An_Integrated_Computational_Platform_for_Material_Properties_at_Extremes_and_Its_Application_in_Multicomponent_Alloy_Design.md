...
ProME v1.0是一个集成计算平台，用于极端条件下的材料性能研究，涵盖高温（数万开尔文）、高压（数百万大气压）和高应变率（每秒数百万次）。该平台整合了多种软件工具，包括晶体结构搜索（ABC）、无序结构建模（SAE）、热力学性质预测（MFP²）、弹性性质分析（HTEM）、电热传导模拟（TREX）、高应变率相场模拟（Hippos）和相图优化（AutoCalphad）。ProME已成功应用于航天发动机喷嘴用Pt-Ir-Al-Cr四元合金设计，在保持高温强度的同时显著降低成本，为材料研发提供了重要支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T02:30:22Z
- **目录日期**: 2025-05-13
