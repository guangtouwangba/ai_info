...
该研究提出ZeroTIR方法，通过基于结果的强化学习（RL）训练基础大语言模型（LLMs），使其能自主生成并执行Python代码来解决数学问题，而无需监督的工具使用示例。研究发现，随着RL训练的进行，关键指标（如代码执行频率、响应长度和任务准确率）呈可预测的正相关增长，表明训练计算量与工具增强推理策略的涌现存在量化关系。实验显示，ZeroTIR在数学基准测试上显著优于非工具强化学习基线。该研究为自主工具使用的获取和扩展提供了基础理解，并建立了可复现的基准框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T14:01:58Z
- **目录日期**: 2025-05-13
