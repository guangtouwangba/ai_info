...
该研究探索了基于结果奖励的强化学习（RL）如何让大语言模型（LLMs）自主学会使用外部工具（如Python代码执行）来解决数学推理问题。提出的ZeroTIR方法无需监督示例，训练基础LLM自发生成并执行代码。研究发现，随着RL训练推进，代码执行频率、响应长度和任务准确率呈可预测的正相关增长，表明训练计算投入与工具增强推理策略的涌现存在量化关系。实验显示，ZeroTIR在数学基准上显著优于非工具基线，为智能体自主工具使用的学习机制提供了可复现的基准框架。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T10:01:50Z
- **目录日期**: 2025-05-13
