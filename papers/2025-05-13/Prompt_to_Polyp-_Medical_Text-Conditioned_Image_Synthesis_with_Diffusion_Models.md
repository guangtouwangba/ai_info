...
本文研究了医疗领域的文本到图像生成技术，比较了两种方法：(1)微调预训练的大规模潜在扩散模型和(2)训练小型领域专用模型。作者提出了一种新型优化模型MSDM，基于Stable Diffusion架构，整合了临床文本编码器、变分自编码器和交叉注意力机制，能更好地对齐医学文本提示与生成图像。通过在结肠镜(MedVQA-GI)和放射学(ROCOv2)数据集上的评估发现，大规模模型生成质量更高，但MSDM能以更低计算成本达到相近效果。医学专家的定量指标和定性评估揭示了各方法的优缺点。该研究为医疗AI解决数据稀缺和隐私问题提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T22:02:25Z
- **目录日期**: 2025-05-13
