...
本文研究了医疗领域文本生成图像的两种方法：(1)微调大型预训练潜在扩散模型(如FLUX、Kandinsky)；(2)训练小型领域专用模型(如提出的MSDM)。MSDM基于Stable Diffusion优化架构，整合临床文本编码器、变分自编码器和交叉注意力机制，能更好对齐医学文本提示与生成图像。评估显示，在结肠镜(MedVQA-GI)和放射学(ROCOv2)数据上，大型模型保真度更高，但优化的MSDM能以更低计算成本达到相近质量。定量指标和专家评估揭示了各方法的优缺点，为解决医疗AI数据稀缺问题提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T11:02:23Z
- **目录日期**: 2025-05-13
