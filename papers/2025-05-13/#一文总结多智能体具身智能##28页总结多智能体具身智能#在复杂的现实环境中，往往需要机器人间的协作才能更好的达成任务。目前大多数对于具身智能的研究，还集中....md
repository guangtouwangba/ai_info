...
这篇28页综述总结了多智能体具身智能的前沿进展。核心要素包括物理身体、环境交互和适应性学习。研究重点包括：1）EMAPF框架实现组内集中控制与组间分布式协调；2）利用生成模型和图神经网络处理开放环境中的动态团队规模；3）SMART-LLM等任务分配方法将大任务分解为并行子任务；4）分布式决策采用独立模型协作或混合评估机制。最新趋势强调闭环任务分配、子任务依赖优化和集中式全局决策。该领域正从单智能体转向更复杂的多智能体协作系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T01:31:00Z
- **目录日期**: 2025-05-13
