...
这篇28页的综述系统梳理了多智能体具身智能领域的最新进展。核心突破包括：1）EMAPF框架通过动态聚类实现组内集中控制与组间分布式协调；2）开放环境中采用图神经网络和自博弈机制实现团队规模动态适应与持续进化；3）任务分配方面，SMART-LLM等模型通过语言模型分解任务并优化分配逻辑；4）分布式决策采用独立模型协作与混合评估机制。研究强调生成模型在任务分解、动态调整和隐式分配中的关键作用，为复杂环境下的多机器人协作提供了系统性解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T04:04:23Z
- **目录日期**: 2025-05-13
