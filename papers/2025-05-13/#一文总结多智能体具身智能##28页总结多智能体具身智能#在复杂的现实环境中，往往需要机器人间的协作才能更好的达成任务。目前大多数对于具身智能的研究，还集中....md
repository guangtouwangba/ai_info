...
这篇28页的综述总结了多智能体具身智能领域的前沿进展。核心要素包括具身性、交互性和适应性。研究重点包括：1）EMAPF框架实现分组控制与分布式协调；2）开放环境中利用生成模型和图神经网络实现动态团队适应与持续进化；3）任务分配方面采用SMART-LLM等范式进行任务分解与动态调整；4）分布式决策采用独立模型协作和混合评估机制。该领域正从单智能体转向多智能体协作，结合生成模型与强化学习，以应对复杂动态环境中的协同挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T03:21:42Z
- **目录日期**: 2025-05-13
