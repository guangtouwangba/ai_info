...
这篇28页的综述系统梳理了多智能体具身智能的前沿进展。核心突破包括：1）EMAPF框架通过动态聚类实现组内集中控制与组间分布式协调；2）开放环境中采用图神经网络和自博弈机制实现团队动态适应与持续进化；3）基于生成模型的任务分配技术（如SMART-LLM）实现任务分解与动态调整；4）分布式决策通过独立模型协作与混合评估机制优化协作效能。研究凸显了生成模型在多智能体控制、任务分配和分布式决策中的关键作用，为复杂环境下的机器人协作提供了系统性解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T02:32:46Z
- **目录日期**: 2025-05-13
