...
这篇28页综述总结了多智能体具身智能的前沿技术。核心要素包括具身性、交互性和适应性。研究重点包括：1）EMAPF框架实现组内集中控制与组间分布式协调；2）开放环境中采用图神经网络和自进化机制应对动态变化；3）利用生成模型（如SMART-LLM）进行任务分解与分配；4）分布式决策通过独立模型协作或混合评估机制实现。最新趋势强调闭环调整、任务依赖图优化和集中式全局决策。该领域正从单智能体向多智能体协作方向发展，提升复杂环境中的任务完成能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T05:03:57Z
- **目录日期**: 2025-05-13
