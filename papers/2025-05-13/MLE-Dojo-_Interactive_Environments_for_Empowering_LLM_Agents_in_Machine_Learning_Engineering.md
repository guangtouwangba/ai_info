...
MLE-Dojo是一个Gym风格的框架，用于在迭代式机器学习工程（MLE）工作流中训练、评估和改进基于大语言模型（LLM）的自主智能体。它基于200多个真实Kaggle挑战，提供交互式环境，支持监督微调和强化学习，涵盖数据处理、架构搜索、超参数调优等任务。与静态基准不同，MLE-Dojo允许智能体通过结构化反馈循环迭代优化解决方案。评估显示，当前LLM虽能逐步改进，但在生成长期解决方案和解决复杂错误方面仍有局限。该框架开源，支持灵活扩展，促进可复现性和社区创新。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T04:02:25Z
- **目录日期**: 2025-05-13
