...
MLE-Dojo是一个Gym风格的强化学习框架，用于在迭代式机器学习工程(MLE)流程中训练、评估和改进大型语言模型(LLM)智能体。基于200多个真实Kaggle挑战，它提供交互式环境，支持监督微调和强化学习，涵盖数据处理、架构搜索、超参数调优等现实任务。评估显示当前LLM能实现迭代改进，但在生成长期解决方案和解决复杂错误方面仍有局限。该框架具有灵活可扩展的架构，支持多种数据源、工具和评估协议，促进互操作性、可扩展性和可复现性。研究团队开源了框架和基准测试以推动社区创新。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T15:01:41Z
- **目录日期**: 2025-05-13
