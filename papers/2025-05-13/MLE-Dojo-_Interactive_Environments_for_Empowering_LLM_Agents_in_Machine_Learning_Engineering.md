...
MLE-Dojo是一个Gym风格的强化学习框架，用于系统化训练、评估和改进基于大语言模型(LLM)的自主代理。它通过200+真实Kaggle挑战构建交互式环境，支持数据预处理、架构搜索、超参数调优等机器学习工程任务。相比静态评估，该框架允许代理通过反馈循环迭代优化方案，支持监督微调和强化学习。评估显示当前LLM在长程解决方案和复杂错误处理上仍有局限。MLE-Dojo具有可扩展架构，能整合多种数据源和工具，支持模型调优并确保可复现性。该框架已开源以推动下一代MLE代理的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T06:01:38Z
- **目录日期**: 2025-05-13
