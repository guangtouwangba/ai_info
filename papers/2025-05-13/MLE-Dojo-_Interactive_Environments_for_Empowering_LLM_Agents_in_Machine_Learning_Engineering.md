...
MLE-Dojo是一个Gym风格的强化学习框架，用于评估和改进大型语言模型(LLM)代理在机器学习工程(MLE)工作流中的表现。它基于200多个真实Kaggle挑战，提供交互式环境支持监督微调和强化学习，覆盖数据处理、架构搜索等现实任务。评估显示当前LLM能实现迭代改进，但在生成长期解决方案和解决复杂错误方面仍有局限。该框架具有灵活可扩展的架构，支持多样化数据源和评估协议，促进模型代理调优和可复现性。研究团队开源了框架和基准测试以推动下一代MLE代理的发展。(99字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T07:01:42Z
- **目录日期**: 2025-05-13
