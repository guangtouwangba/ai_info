...
MLE-Dojo是一个Gym风格框架，用于在迭代式机器学习工程(MLE)工作流中系统化训练、评估和改进自主大型语言模型(LLM)智能体。基于200多个真实Kaggle挑战，该框架提供交互式环境，支持监督微调和强化学习，覆盖数据处理、架构搜索等现实工程场景。评估显示当前LLM虽能迭代改进，但在生成长期解决方案和解决复杂错误方面仍有局限。其可扩展架构支持多种数据源和工具集成，促进模型智能体调优和可复现性。该框架已开源以推动下一代MLE智能体的社区创新。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T09:01:31Z
- **目录日期**: 2025-05-13
