...
MLE-Dojo是一个Gym风格的强化学习框架，用于系统训练、评估和改进自主大型语言模型(LLM)代理在机器学习工程(MLE)工作流中的表现。基于200多个真实Kaggle挑战，它提供交互式环境，支持监督微调和强化学习，涵盖数据处理、架构搜索、超参数调优等任务。评估显示，当前LLM在迭代改进方面有效，但在生成长期解决方案和解决复杂错误方面仍有局限。该框架具有灵活可扩展的架构，支持多样化数据源、工具和评估协议，促进互操作性、可扩展性和可重复性。研究团队开源了框架和基准测试以推动社区创新。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T14:01:26Z
- **目录日期**: 2025-05-13
