...
在高风险场景中负责任地部署AI/ML系统不仅需要可靠性证明，还需要持续部署后监测以快速发现不安全行为。本文提出加权保形测试鞅(WCTM)方法，扩展了非参数变点检测的适用范围，可在线监测数据分布中的异常变化并控制误报。针对实际应用，设计了能适应轻度协变量偏移，同时快速检测严重概念偏移或极端协变量偏移的WCTM算法。相比现有方法，新方法突破了有限假设类限制，支持在线自适应调整，并能进行性能退化的根因分析。真实数据集实验表明，其性能优于当前最优基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T19:02:22Z
- **目录日期**: 2025-05-13
