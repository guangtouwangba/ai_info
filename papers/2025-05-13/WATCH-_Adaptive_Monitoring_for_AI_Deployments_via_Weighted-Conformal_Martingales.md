...
本文提出了一种加权泛化共形测试鞅(WCTM)方法，用于高风险AI/ML系统的持续部署后监测。该方法扩展了传统非参数变点检测的范围，能在线监测数据分布的任何异常变化并控制误报。针对实际应用，研究设计了特定WCTM算法，可适应轻度协变量偏移，同时快速检测更严重的概念偏移或极端协变量偏移。相比现有方法，WCTM突破了有限假设类别的限制，支持在线自适应调整，并能进行性能退化的根因分析。真实数据集实验表明，该方法优于当前最先进的基线方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T08:02:36Z
- **目录日期**: 2025-05-13
