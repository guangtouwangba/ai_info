...
在关键领域部署AI/ML系统时，不仅需要可靠性证明，还需持续监测以识别不安全行为。现有非参数变点检测方法（如符合检验鞅和随时有效推断）存在局限性，无法在线适应数据变化或进行根本原因分析。本文提出加权符合检验鞅（WCTM）方法，扩展了监测范围，可在线控制误报率并检测数据分布异常变化。针对实际应用，WCTM算法能适应轻度协变量偏移，快速识别概念漂移或极端协变量偏移等严重变化。实验表明，该方法在真实数据集上优于当前最优基线。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T06:02:42Z
- **目录日期**: 2025-05-13
