...
当前大语言模型（LLM）的扩展规律面临瓶颈，本文认为边际收益递减下信噪比变化引发的噪声效应是主因。为控制噪声，研究者通过对比优劣模型差异提出"相对过拟合"概念，并基于互补性开发了"接受-拒绝"（AR）框架。该框架使小模型（SLM）能普遍提升LLM决策效果，而非预期中的负面影响。实验表明，相比增加LLM参数量，该结构能以更低成本实现更优性能，且改进具有普适稳定性。研究还探讨了该框架在CV、AI科研等领域的潜力，有望帮助突破现有扩展瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T05:01:25Z
- **目录日期**: 2025-05-13
