...
当前大语言模型(LLM)的扩展规律面临瓶颈。本文认为边际收益递减下信噪比变化导致的噪声效应是根本原因。通过分析优劣模型差异，提出"相对过拟合"概念及"接受-拒绝"(AR)应用框架。在NLP领域，该框架使小模型(SLM)能普遍提升大模型决策输出，而非预期负面效果。基于主流架构和预训练模型的多数据集实验表明，相比增加LLM参数量，该结构能以更低成本获得更优性能提升，且效果稳定普适。研究还探讨了该框架在CV和AI科研等领域的潜力，有望帮助扩展律突破现有瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T10:01:16Z
- **目录日期**: 2025-05-13
