...
当前大语言模型（LLM）的缩放规律面临瓶颈。本文认为，边际收益递减下信噪比变化导致的噪声效应是根本原因。通过分析优劣模型差异，提出"相对过拟合"概念及"接受-拒绝"（AR）框架，利用小语言模型（SLM）对LLM决策产生普遍正向影响。实验表明，该结构能以更低参数量和计算成本，在多任务中稳定提升性能。研究还探讨了该方法在CV和科学AI等领域的潜力，有望帮助缩放规律突破现有瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T13:10:49Z
- **目录日期**: 2025-05-13
