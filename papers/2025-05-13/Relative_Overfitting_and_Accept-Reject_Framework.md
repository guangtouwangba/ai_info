...
当前大语言模型（LLM）的扩展规律面临瓶颈。本文认为，边际收益递减下信噪比变化导致的噪声效应是根本原因。通过分析性能优劣模型的差异，提出"相对过拟合"概念及"接受-拒绝"（AR）框架。在NLP任务中，该框架使小模型（SLM）能普遍提升LLM决策输出，而非预期负面效果。实验表明，相比增加LLM参数量，该结构能以更低成本实现更优性能提升，且效果稳定普适。研究还探讨了该方法在CV、AI科研等领域的潜力，有望帮助突破现有扩展瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T03:19:29Z
- **目录日期**: 2025-05-13
