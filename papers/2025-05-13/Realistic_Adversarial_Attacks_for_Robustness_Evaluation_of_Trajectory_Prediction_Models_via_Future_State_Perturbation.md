...
轨迹预测是自动驾驶系统的关键功能，用于预判其他道路使用者的行为。现有对抗性攻击方法通常仅扰动周围车辆的过去轨迹，可能导致不现实的测试场景并低估模型漏洞。本研究提出通过同时扰动对抗车辆的过去和未来状态，结合动态约束和行为保持策略，生成更真实有效的对抗攻击。新方法在先进预测模型上的测试显示，其能显著增加预测误差和碰撞率，并暴露出模型无法识别潜在碰撞等关键缺陷。结果表明，需要更全面的对抗性测试来提升自动驾驶轨迹预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T02:30:56Z
- **目录日期**: 2025-05-13
