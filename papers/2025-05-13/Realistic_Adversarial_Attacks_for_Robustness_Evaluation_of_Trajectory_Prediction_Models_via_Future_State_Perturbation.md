...
本文提出了一种更全面的对抗性测试方法，用于评估自动驾驶车辆轨迹预测模型的鲁棒性。不同于现有方法仅扰动周围车辆的过去位置，新方法同时扰动对抗车辆的过去和未来状态，结合动态约束并保持战术行为，从而生成更真实有效的对抗轨迹。实验表明，该方法能显著增加预测误差和碰撞率，并暴露出模型的关键弱点（如无法检测看似安全预测中的潜在碰撞）。研究强调需要更全面的对抗测试来提升预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T00:01:54Z
- **目录日期**: 2025-05-13
