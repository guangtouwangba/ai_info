...
这篇论文提出了一种更严格的轨迹预测模型鲁棒性评估方法，通过同时扰动对抗智能体的过去和未来状态来发现潜在漏洞。与传统仅扰动历史位置的方法相比，该方法结合动态约束并保持战术行为，能产生更真实有效的对抗攻击。研究引入了新的性能指标评估攻击效果，测试表明该方法能显著增加预测误差和碰撞率，暴露出模型将危险预测误判为安全等关键缺陷。这些发现强调了需要更全面的对抗测试来提升自动驾驶轨迹预测模型的可靠性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-13T01:29:12Z
- **目录日期**: 2025-05-13
