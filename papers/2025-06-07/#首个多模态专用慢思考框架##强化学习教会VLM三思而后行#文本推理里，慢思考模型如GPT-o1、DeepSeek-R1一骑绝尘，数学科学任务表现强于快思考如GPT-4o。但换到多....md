...
港科大等团队针对视觉语言模型(VLM)在多模态推理中"慢思考"失效问题，提出创新框架VL-Rethinker。该方案通过两大机制突破瓶颈：1）优势样本回放(SSR)聚焦关键训练样本；2）强制反思机制引导模型自主判断反思时机。基于39K高质量多模态数据(ViRL39K)的实验显示，该框架在MathVista等基准测试中超越GPT-o1，接近OpenAI-o1水平，不同规模模型均显著提升。研究证实了慢思考策略在多模态领域的适用性，为VLM的深度推理能力发展提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T13:09:56Z
- **目录日期**: 2025-06-07
