...
港科大团队针对视觉语言模型(VLM)在多模态推理中的"慢思考"失效问题，提出了VL-Rethinker框架。研究发现传统慢思考模型在图文任务中存在"优势消失"和"反思惰性"两大瓶颈。该框架通过两项创新机制：1）优势样本回放(SSR)聚焦高价值训练样本；2）强制反思提示引导选择性自我验证。在MathVista等基准测试中，该方法超越GPT-o1等模型，不同规模模型性能均显著提升，首次验证了慢思考策略在多模态领域的有效性。研究同时构建了39K高质量多模态推理数据集ViRL39K。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T20:01:56Z
- **目录日期**: 2025-06-07
