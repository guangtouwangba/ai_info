...
行为克隆（BC）在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，一旦偏离专家示范的状态，代理难以自主恢复。为解决这一问题，研究者提出从专家示范中学习搜索（L2S）的方法，通过构建世界模型和奖励模型，使代理能在测试时规划恢复路径，无需额外人工修正。该方法（SAILOR）在12项视觉操作任务中表现优于当前最佳的扩散策略（BC），即使BC数据量增加5-10倍仍存在差距。SAILOR能识别细微错误并避免奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T07:01:50Z
- **目录日期**: 2025-06-07
