...
行为克隆（BC）方法在模仿学习中的主要局限是只能模仿专家在特定状态下的行为，导致智能体在偏离专家示范状态时难以恢复。为此，研究者提出“学习搜索”（L2S）方法，通过从专家示范中学习世界模型和奖励模型，使智能体能在测试时自主规划以匹配专家结果，即使犯错后也能恢复。该方法（SAILOR）在12项视觉操作任务中表现优于当前最佳的扩散策略（BC），即使BC使用5-10倍数据仍存在差距。SAILOR能识别细微错误且抗奖励作弊。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T00:02:17Z
- **目录日期**: 2025-06-07
