...
行为克隆（BC）方法在模仿学习中的根本局限是只能模仿专家访问过的状态，导致智能体在偏离演示状态时无法自主恢复。为此，研究者提出学习搜索（L2S）方法，通过从专家演示中学习世界模型和奖励模型，使智能体能在测试时规划恢复路径。该方法（SAILOR）在12项视觉操作任务中均优于基于相同数据的扩散策略，即使BC使用5-10倍演示数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊，展现了更强的鲁棒性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T01:29:29Z
- **目录日期**: 2025-06-07
