...
行为克隆(BC)方法在模仿学习中的根本局限是只能模仿专家在特定状态下的行为，当智能体偏离专家示范状态时无法自主恢复。为此，研究者提出了基于专家示范的学习搜索(L2S)方法SAILOR，通过结合世界模型和奖励模型，使智能体能在测试时规划恢复路径。在12个视觉操作任务上的实验表明，SAILOR显著优于基于相同数据的扩散策略BC方法，即使BC使用5-10倍示范数据仍存在性能差距。SAILOR能识别细微错误并避免奖励欺骗，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T13:08:19Z
- **目录日期**: 2025-06-07
