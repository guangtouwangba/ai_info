...
行为克隆（BC）模仿学习的核心局限在于它只能教会智能体在专家访问过的状态下模仿行为，一旦偏离演示数据范围，智能体难以自主恢复。为此，研究者提出基于专家演示的"学习搜索"（L2S）方法SAILOR，通过构建世界模型和奖励模型，使智能体能在测试时规划恢复路径。实验表明，在12项视觉操作任务中，SAILOR显著优于当前最佳的扩散策略（Diffusion Policies），即使BC使用5-10倍演示数据仍存在性能差距。该方法能识别细微错误且抗奖励破解，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T10:02:05Z
- **目录日期**: 2025-06-07
