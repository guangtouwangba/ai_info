...
行为克隆（BC）模仿学习的核心局限在于它仅能教会智能体在专家访问过的状态下模仿行为，一旦偏离演示数据范围，智能体难以自主恢复。为此，研究者提出基于专家演示的"学习搜索"（L2S）方法，通过构建世界模型和奖励模型，使智能体能在测试时规划恢复路径。该方法（SAILOR）在12项视觉操作任务中稳定超越基于相同数据的扩散策略（Diffusion Policies），即使BC使用5-10倍演示数据仍存在性能差距。SAILOR能识别细微错误并抵抗奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T22:01:48Z
- **目录日期**: 2025-06-07
