...
行为克隆（BC）方法在模仿学习中的根本局限在于，它仅能教会智能体在专家访问过的状态下模仿行为，而一旦偏离演示状态，智能体往往无法自主恢复。为解决这一问题，研究者提出从专家演示中学习搜索（L2S）的方法，通过训练世界模型和奖励模型，使智能体能够在测试时规划恢复行为，而无需额外人工修正。实验表明，该方法（SAILOR）在多个视觉操作任务中显著优于基于BC的扩散策略，即使BC使用5-10倍演示数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T03:21:09Z
- **目录日期**: 2025-06-07
