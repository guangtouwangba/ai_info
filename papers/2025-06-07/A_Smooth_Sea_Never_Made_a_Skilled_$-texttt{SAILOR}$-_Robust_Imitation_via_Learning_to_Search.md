...
行为克隆（BC）在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，导致智能体一旦偏离专家轨迹就难以恢复。为解决这一问题，研究者提出了基于专家演示的学习搜索（L2S）方法SAILOR，通过构建世界模型和奖励模型，使智能体能在测试时自主规划恢复策略。实验表明，SAILOR在12项视觉操作任务中显著优于当前最佳的扩散策略（Diffusion Policies），即使BC使用5-10倍演示数据仍存在性能差距。该方法能识别细微错误并避免奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T05:02:09Z
- **目录日期**: 2025-06-07
