...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)建立CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次评估模型输出与人类偏好的匹配度；2)构建20B标记量的SynCode-Instruct合成指令集验证微调效果。实验发现：传统执行测试与人类偏好存在偏差，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著，凸显对齐人类偏好的重要性。研究填补了当前代码评估忽视用户体验的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T17:04:44Z
- **目录日期**: 2025-06-07
