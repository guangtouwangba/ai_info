...
这篇论文提出了DirectLayout框架，用于从文本描述直接生成3D室内场景布局。该方法利用大语言模型的空间推理能力，通过三个阶段生成布局：鸟瞰图生成、3D空间提升和物体位置优化。研究采用基于3D-Front数据集的思维链激活机制来增强空间推理，并设计了基于思维链的生成式布局奖励机制来提高泛化能力。在推理阶段，通过上下文学习实现迭代式资产-布局对齐。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，解决了现有方法在开放词汇和细粒度控制方面的不足。(99字)

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T10:01:08Z
- **目录日期**: 2025-06-07
