...
这篇论文提出了一种名为DirectLayout的框架，用于从文本描述直接生成3D室内场景布局。该框架利用大语言模型（LLMs）的空间推理能力，通过三个阶段生成布局：鸟瞰图（BEV）布局生成、3D空间提升和物体位置优化。研究采用基于3D-Front数据集的思维链（CoT）激活来增强空间推理，并设计了CoT-Grounded生成布局奖励以提高泛化能力。在推理阶段，通过上下文学习实现迭代式资产-布局对齐，解决不匹配问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T22:00:57Z
- **目录日期**: 2025-06-07
