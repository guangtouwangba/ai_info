...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该方法分三步：生成俯视图布局、提升至3D空间并优化物体摆放。通过3D-Front数据集激活思维链（CoT）增强空间推理，并设计CoT-Grounded奖励机制提高泛化能力。推理阶段采用上下文学习迭代调整布局与资产匹配。实验表明，DirectLayout在语义一致性、泛化性和物理合理性方面表现优异，解决了现有方法在开放词汇和细粒度控制上的不足，显著提升了3D场景合成的质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T08:01:01Z
- **目录日期**: 2025-06-07
