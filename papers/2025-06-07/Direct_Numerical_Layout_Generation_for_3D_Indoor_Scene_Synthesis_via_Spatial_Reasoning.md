...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该方法将生成过程分为三步：首先生成俯视图布局，然后将其提升到3D空间，最后优化物体摆放。通过3D-Front数据集激活链式思维（CoT），帮助模型掌握物体摆放原则，并设计了CoT-Grounded生成式布局奖励来增强泛化能力和空间规划。推理阶段通过上下文学习进行迭代式资产-布局对齐，解决资产与布局不匹配问题。实验表明，DirectLayout在语义一致性、泛化性和物理合理性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T01:28:38Z
- **目录日期**: 2025-06-07
