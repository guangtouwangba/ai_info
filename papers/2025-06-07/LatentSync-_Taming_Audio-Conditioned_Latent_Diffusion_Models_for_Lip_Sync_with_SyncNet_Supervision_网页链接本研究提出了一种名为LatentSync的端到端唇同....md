...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，并提出了时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题。同时，研究改进了SyncNet训练框架，通过优化模型架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进也可应用于其他音频驱动的人像动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T11:03:44Z
- **目录日期**: 2025-06-07
