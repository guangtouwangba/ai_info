...
这篇论文提出了LatentSync，一种端到端的唇同步框架，利用音频条件的潜在扩散模型直接建模音频-视觉关系，无需中间运动表示。针对扩散模型中时间一致性问题，作者提出了时间表示对齐(TREPA)方法，通过自监督视频模型增强生成帧的时间连续性。研究还改进了SyncNet训练框架，解决了其收敛问题，并分析了模型架构、超参数和数据预处理的影响。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进也可推广到其他基于SyncNet的唇同步和人像动画应用中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T13:11:58Z
- **目录日期**: 2025-06-07
