...
该研究提出了LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。它利用Stable Diffusion直接建模复杂的音频-视觉关系，解决了传统方法的时间一致性问题。研究者提出时间表示对齐(TREPA)方法，通过自监督视频模型增强生成帧的时间一致性。同时改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些改进也可应用于其他音频驱动的人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T07:05:07Z
- **目录日期**: 2025-06-07
