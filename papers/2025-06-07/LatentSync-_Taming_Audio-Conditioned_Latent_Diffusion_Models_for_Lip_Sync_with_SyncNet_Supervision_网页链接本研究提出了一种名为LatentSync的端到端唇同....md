...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该框架直接建模音频-视觉关系，克服了传统方法的时间一致性问题，并提出时间表示对齐（TREPA）方法增强生成帧的时序一致性。此外，研究优化了SyncNet训练框架，改进其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T14:04:14Z
- **目录日期**: 2025-06-07
