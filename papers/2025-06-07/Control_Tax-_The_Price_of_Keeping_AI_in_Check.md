...
本文提出"控制税"概念，即AI系统中集成控制措施所需的操作和财务成本。研究贡献包括：(1)建立量化控制税的理论框架，将分类器性能映射到安全保证；(2)在对抗环境下评估先进语言模型，研究攻击模型植入代码后门与监控模型检测漏洞的博弈；(3)提供控制协议的实际成本估算，开发平衡安全性与成本效益的优化监控策略。该框架通过系统关联安全保证与成本，帮助从业者在不同部署场景下做出基于经济可行性的决策，推动AI控制领域的实际应用发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T08:01:54Z
- **目录日期**: 2025-06-07
