...
本文提出了"控制税"(Control Tax)概念，用于衡量AI系统中集成安全控制措施的操作和财务成本。研究建立了量化控制税的理论框架，将分类器性能映射到安全保证；评估了先进语言模型在对抗性环境下的表现，攻击模型植入代码后门而监控模型尝试检测漏洞；并提供了控制协议的实际成本估算，开发了平衡安全性与成本效益的优化监控策略。该框架通过系统连接安全保证与其成本，帮助从业者做出明智决策，推动AI控制领域在不同部署场景中的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T03:20:58Z
- **目录日期**: 2025-06-07
