...
这篇论文重新审视了测试阶段的扩展规律，从实用效率角度揭示了小模型的效果被显著高估的问题。传统基于计算最优性的研究忽视了推理策略（如Best-of-N、长思维链）带来的内存访问瓶颈。作者提出了综合考虑计算和内存成本的"动力学扩展定律"，发现将资源分配给超过阈值的大模型更有效，因为注意力机制而非参数量成为主要成本因素。为此，作者提出以稀疏注意力为核心的新扩展范式，实验表明稀疏模型在各类资源条件下均优于密集模型，在AIME基准上最高可获得60分的准确率提升。研究表明稀疏注意力是实现测试阶段扩展潜力的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T19:00:59Z
- **目录日期**: 2025-06-07
