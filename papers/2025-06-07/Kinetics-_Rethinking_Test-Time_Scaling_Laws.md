...
该研究从实际效率角度重新审视测试阶段的扩展规律，发现小模型的效果被显著高估。传统基于计算最优性的方法忽视了推理策略（如Best-of-N、长链思维）引入的关键内存访问瓶颈。通过分析0.6B到32B参数的模型，研究提出了"动力学扩展定律"，综合考虑计算和内存访问成本，更有效地指导资源分配。研究表明，稀疏注意力机制能降低单token成本，在相同资源下支持更长生成和更多并行样本。实验显示，稀疏注意力模型在AIME问题求解准确率上比密集模型提升5-60分，证明其是实现测试阶段扩展潜力的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T03:20:20Z
- **目录日期**: 2025-06-07
