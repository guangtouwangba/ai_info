...
这篇论文重新审视了测试阶段的扩展规律，从实际效率角度发现小模型的效果被显著高估。传统基于计算最优性的方法忽略了推理策略（如Best-of-N、长思维链）引入的内存访问瓶颈。作者提出了综合考虑计算和内存成本的"动力学扩展定律"，表明在阈值以上的模型上分配计算资源更有效。研究发现注意力机制（而非参数量）是测试时主要成本因素，因此提出基于稀疏注意力的新扩展范式，可降低单token成本，支持更长生成和更多并行样本。实验显示稀疏注意力模型在AIME基准上显著优于密集模型，在低成本和高成本场景分别提升60分和5分。这表明稀疏注意力是实现测试扩展潜力的关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T02:30:29Z
- **目录日期**: 2025-06-07
