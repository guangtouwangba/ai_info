...
该研究从实际效率角度重新审视了测试阶段的扩展规律，发现小模型的有效性被显著高估。传统基于计算最优性的方法忽视了推理策略（如Best-of-N、长思维链）引入的内存访问瓶颈。通过分析0.6B到32B参数的模型，研究提出了"动力学扩展定律"，综合考虑计算和内存访问成本，表明资源应优先分配给达到阈值规模以上的模型，因为注意力机制（而非参数量）成为主要成本因素。基于此，研究提出稀疏注意力新范式，在相同资源下实现更长生成和更多并行样本，实验显示稀疏模型在各类成本区间均优于密集模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T18:01:04Z
- **目录日期**: 2025-06-07
