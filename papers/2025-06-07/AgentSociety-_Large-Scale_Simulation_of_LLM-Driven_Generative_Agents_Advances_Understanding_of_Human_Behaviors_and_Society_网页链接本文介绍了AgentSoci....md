...
AgentSociety是一个基于大语言模型的大规模社会模拟器，能够模拟1万多个代理的社会互动（500万次交互）。研究者用它研究了四个关键社会问题：社会极化、信息传播、基本收入政策影响和自然灾害（如飓风）的社会效应。模拟结果与现实世界数据高度吻合，验证了该工具在揭示人类行为机制方面的有效性。这一平台为社会科学研究和政策制定提供了新方法，能补充传统调查和实验的不足，展现出作为计算社会科学重要工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T10:05:14Z
- **目录日期**: 2025-06-07
