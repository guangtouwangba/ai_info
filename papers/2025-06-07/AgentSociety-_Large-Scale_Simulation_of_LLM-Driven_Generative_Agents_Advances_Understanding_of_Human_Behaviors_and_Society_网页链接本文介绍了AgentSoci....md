...
AgentSociety是一种基于大语言模型的大规模社会模拟系统，可模拟1万个代理的社交互动（500万次）。研究者用它分析四个社会问题：群体极化、信息传播、基本收入政策效果和自然灾害影响。结果显示其模拟结果与现实数据吻合，验证了该系统捕捉人类行为机制的能力。该系统可作为社会科学研究和政策制定的有效工具，补充传统研究方法（如调查、实验）。研究突显了大语言模型在复杂社会现象建模中的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T02:34:23Z
- **目录日期**: 2025-06-07
