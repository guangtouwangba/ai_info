...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。该方法基于带动量的随机梯度下降（SGDM），通过初始化时按梯度信噪比（g-SNR）调整不同参数组的学习率，无需二阶动量计算即可避免训练失衡。相比AdamW，SGD-SAi内存占用减半，在Transformer、ImageNet分类和LLM预训练（如GPT-2）中表现相当或更优，尤其节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB），且在超参数鲁棒性和LoRA微调中优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T22:04:12Z
- **目录日期**: 2025-06-07
