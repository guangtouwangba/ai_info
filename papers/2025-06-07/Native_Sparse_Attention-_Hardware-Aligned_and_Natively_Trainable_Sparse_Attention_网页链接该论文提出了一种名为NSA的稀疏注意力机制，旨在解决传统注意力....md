...
该论文提出了Native Sparse Attention（NSA）机制，用于解决传统注意力机制在处理长文本时的高计算成本问题。NSA结合算法创新与硬件优化，采用动态分层稀疏策略，通过粗粒度令牌压缩和细粒度令牌选择，在保持全局上下文感知能力的同时确保局部精度。这一设计实现了高效的长文本建模，并包含两个关键创新点，为大规模语言模型提供了更高效的注意力计算方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T14:03:44Z
- **目录日期**: 2025-06-07
