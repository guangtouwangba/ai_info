...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过算法与硬件协同优化，采用动态分层稀疏策略：先粗粒度压缩令牌，再细粒度选择关键令牌，在保持全局上下文感知能力的同时提升局部精度。该方法实现了高效的长文本建模，其核心创新在于将稀疏模式与硬件特性对齐，并保持端到端的可训练性。这一技术为大模型处理长序列任务提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T02:34:14Z
- **目录日期**: 2025-06-07
