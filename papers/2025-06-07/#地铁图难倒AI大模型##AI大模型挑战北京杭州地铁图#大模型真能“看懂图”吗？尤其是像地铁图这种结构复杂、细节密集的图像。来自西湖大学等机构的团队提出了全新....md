...
西湖大学等机构提出ReasonMap评测基准，测试多模态大模型对高分辨率地铁图(平均5839×5449像素)的空间理解和路径规划能力。研究发现，开源模型普遍表现不佳，存在混淆线路或遗漏站点等问题；闭源模型如GPT-4表现较好但仍显著低于人类水平。测试覆盖15个主流模型和多个城市地铁图，其中北京、杭州等复杂地图难度最高。结果表明当前AI在复杂图像理解和逻辑推理方面仍有明显局限，距离人类实际使用地图的能力存在差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T17:01:50Z
- **目录日期**: 2025-06-07
