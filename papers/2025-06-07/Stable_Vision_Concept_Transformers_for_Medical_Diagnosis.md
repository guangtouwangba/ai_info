...
本文提出两种可解释AI模型：视觉概念变换器(VCT)和稳定视觉概念变换器(SVCT)，以解决医学领域对透明AI的需求。VCT通过融合图像特征和概念特征提升模型性能，而SVCT基于VCT进一步整合去噪扩散平滑技术，增强模型在输入扰动下的解释稳定性。实验表明，这两种模型在四个医学数据集上均保持准确性且具有可解释性。SVCT尤其能在扰动情况下提供可靠解释，满足医学领域对模型鲁棒性和解释稳定性的要求，弥合了传统模型与基于概念模型之间的效用差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T10:02:16Z
- **目录日期**: 2025-06-07
