...
本文针对医疗领域对AI透明度的需求，提出两种可解释AI模型：Vision Concept Transformer (VCT)和其改进版Stable Vision Concept Transformer (SVCT)。传统概念瓶颈模型(CBMs)仅依赖概念特征，忽略了医学图像的内在特征，且对输入扰动敏感。VCT通过融合概念特征与图像特征提升性能；SVCT进一步采用视觉Transformer架构，结合去噪扩散平滑技术，确保模型在扰动下的解释稳定性。在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具有可解释性，SVCT尤其能提供稳定的解释，满足医疗领域需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T03:21:19Z
- **目录日期**: 2025-06-07
