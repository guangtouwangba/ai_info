...
本文针对医疗领域对AI可解释性的需求，提出两种改进的概念瓶颈模型：VCT（视觉概念Transformer）和SVCT（稳定视觉概念Transformer）。传统方法仅依赖概念特征而忽略医学图像固有特征，VCT通过融合概念特征与图像特征提升决策能力。SVCT进一步采用ViT架构，结合去噪扩散平滑技术增强模型抗干扰性，确保解释的稳定性。在四个医学数据集上的实验表明，VCT/SVCT在保持精度的同时优于基线模型，SVCT在输入扰动下仍能提供可靠解释，满足医疗领域对模型可解释性和鲁棒性的双重需求。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-07T13:08:29Z
- **目录日期**: 2025-06-07
