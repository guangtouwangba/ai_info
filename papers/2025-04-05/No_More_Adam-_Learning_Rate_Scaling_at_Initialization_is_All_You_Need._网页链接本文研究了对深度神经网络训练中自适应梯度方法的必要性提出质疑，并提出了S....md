...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度学习中自适应梯度优化器（如Adam）的必要性。该方法基于带动量的随机梯度下降（SGDM），通过初始化时按梯度信噪比（g-SNR）调整不同参数组的学习率，避免了训练不平衡问题。相比AdamW，SGD-SaI内存占用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，尤其在大模型（如Llama2-7B）上可节省显著内存（最高25GB）。该方法还具有超参数鲁棒性，在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T00:04:24Z
- **目录日期**: 2025-04-05
