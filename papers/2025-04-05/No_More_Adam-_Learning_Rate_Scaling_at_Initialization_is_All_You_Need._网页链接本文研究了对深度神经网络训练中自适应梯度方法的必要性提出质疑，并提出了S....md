...
这篇论文提出了一种名为SGD-SaI的新型优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。SGD-SaI基于带动量的随机梯度下降（SGDM），通过初始化时按梯度信噪比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。实验表明，该方法在Transformer任务、ImageNet分类和LLM预训练（如GPT-2、Llama2-7B）中性能媲美或超越AdamW，同时内存占用减半（最高节省25GB）。其优势还包括超参数鲁棒性，并在LoRA微调等场景表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T12:05:02Z
- **目录日期**: 2025-04-05
