...
该研究提出SGD-SaI方法，挑战了深度学习中自适应梯度方法（如Adam）的必要性。该方法改进带动量的SGD，通过在初始化时根据不同参数组的梯度信噪比（g-SNR）调整学习率，避免训练不平衡问题，且内存占用仅为AdamW的一半。实验表明，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中表现优于或持平AdamW，并显著节省内存（如为GPT-2节省5.93GB）。其鲁棒性强，在LoRA微调等任务中也优于现有优化器，为大规模模型训练提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T01:31:23Z
- **目录日期**: 2025-04-05
