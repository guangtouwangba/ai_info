...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法改进带动量随机梯度下降(SGDM)，通过初始化时依据梯度信噪比(g-SNR)调整各参数组学习率，无需自适应二阶动量即可避免训练失衡，并将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或媲美AdamW，且对超参数变化更鲁棒。内存效率显著提升，如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB。在LoRA微调等任务中也超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T11:05:09Z
- **目录日期**: 2025-04-05
