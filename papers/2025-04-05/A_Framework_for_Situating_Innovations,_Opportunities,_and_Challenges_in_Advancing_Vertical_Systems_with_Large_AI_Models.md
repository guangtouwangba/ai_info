...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，常暴露出脆弱性、缺乏情境理解、输出错误信息等问题。为此，研究者提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统。该框架帮助研究者：(1)定位创新点（如垂直领域知识如何赋能通用创新）；(2)发现被忽视的机会（如针对跨领域共性问题开发实用基础模型）；(3)促进跨学科交流（为AI开发者、领域专家等建立共同语言）。案例研究表明该框架能有效弥合模型能力与实际需求间的差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T13:05:27Z
- **目录日期**: 2025-04-05
