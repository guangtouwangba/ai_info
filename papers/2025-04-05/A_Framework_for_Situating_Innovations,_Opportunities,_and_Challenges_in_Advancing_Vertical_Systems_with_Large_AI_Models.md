...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，仍存在明显缺陷，如对输入数据细微变化敏感、关键决策缺乏情境理解、错误输出影响用户信任等。为此，研究者提出了一种分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架帮助研究者优化创新定位（如垂直领域洞察如何推动通用创新）、发现被忽视的机会（如跨领域共性问题指导基础模型开发），并促进跨学科交流（如建立AI开发者、领域专家与人机交互学者的共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T11:01:39Z
- **目录日期**: 2025-04-05
