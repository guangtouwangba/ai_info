...
大型AI模型在标准化测试中表现卓越，但在医疗、教育等高风险领域应用时，常暴露局限性，如输入数据敏感性、决策缺乏情境认知及错误输出等问题。为此，研究提出分层创新框架，通过模块化设计将大模型转化为实用垂直系统，满足实际需求。该框架帮助研究者定位创新方向（如垂直领域洞察转化为通用方案）、发现被忽视的机会（如开发实用基础模型而非追逐基准测试），并促进跨学科协作（如建立AI开发者、领域专家与人机交互学者的共同语言）。案例研究展示了框架的实际应用与动态性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T18:01:42Z
- **目录日期**: 2025-04-05
