...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活学习策略、动态预测和视频生成。UWM能同时处理带动作标注和无动作视频数据，在大规模多任务机器人数据集上预训练后，比模仿学习策略更泛化、鲁棒。实验表明，UWM通过独立控制不同模态的扩散步骤，有效利用无动作视频数据提升策略微调性能。该框架为整合模仿学习和世界建模提供了新思路，有望利用异构数据实现可扩展的机器人学习。详见项目网站：https://weirdlabuw.github.io/uwm/。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T19:01:29Z
- **目录日期**: 2025-04-05
