...
本文提出了一种名为统一世界模型（UWM）的新框架，旨在结合视频数据与动作数据来提升机器人策略学习。UWM通过整合动作扩散和视频扩散过程于统一的Transformer架构中，灵活支持策略、动态预测和视频生成等多种功能。实验表明，UWM不仅能在大规模多任务数据集上预训练，生成比传统模仿学习更泛化、稳健的策略，还能利用无动作标注的视频数据进一步提升微调策略性能。该研究为利用异构数据实现可扩展的机器人学习提供了新思路，并弥合了模仿学习与世界建模间的鸿沟。项目代码与视频已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T14:01:49Z
- **目录日期**: 2025-04-05
