...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM能同时处理含动作标注和无动作视频数据，在大规模多任务机器人数据集上预训练后，相比传统模仿学习表现出更强的泛化性和鲁棒性。实验表明，该方法不仅支持动作数据的策略学习，还能通过独立控制扩散步长有效利用无动作视频数据提升微调策略性能。UWM为整合异构机器人数据提供了统一框架，弥合了模仿学习与世界建模的鸿沟。代码和视频已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T05:01:52Z
- **目录日期**: 2025-04-05
