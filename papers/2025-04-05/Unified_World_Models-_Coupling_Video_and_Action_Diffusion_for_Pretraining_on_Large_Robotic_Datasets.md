...
本文提出统一世界模型(UWM)，通过整合动作扩散和视频扩散过程于统一Transformer架构，实现多模态机器人策略学习。UWM创新性地采用独立扩散时间步控制，能灵活表征策略、动态预测和视频生成。实验表明：(1)UWM在大规模多任务数据集上预训练后，比模仿学习获得更具泛化性的策略；(2)通过独立控制模态扩散时间步，可有效利用无动作标注视频数据提升策略性能。该框架为融合异构数据集提供了新思路，弥合了模仿学习与世界建模的范式差异，推动了通用机器人基础模型的发展。项目代码及视频已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T11:01:53Z
- **目录日期**: 2025-04-05
