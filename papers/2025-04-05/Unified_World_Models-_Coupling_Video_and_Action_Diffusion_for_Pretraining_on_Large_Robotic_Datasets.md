...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用多模态数据进行机器人策略学习。UWM采用统一Transformer架构，独立控制各模态的扩散时间步，灵活实现策略、动力学和视频生成功能。实验表明：（1）UWM能有效预训练多任务机器人数据集，比模仿学习获得更通用、鲁棒的策略；（2）通过独立控制模态扩散时间步，UWM可自然利用无动作视频数据提升微调策略性能。该框架为利用异构大数据进行可扩展的机器人学习提供了新思路，并统一了模仿学习与世界建模范式。项目详见：https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T12:02:00Z
- **目录日期**: 2025-04-05
