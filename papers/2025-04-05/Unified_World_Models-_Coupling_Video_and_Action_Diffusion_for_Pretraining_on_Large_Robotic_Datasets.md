...
模仿学习是构建通用机器人的有效方法，但依赖高质量专家演示限制了其扩展性。为解决这一问题，研究者提出了统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活表示策略、动态预测和视频生成。UWM支持大规模多任务机器人数据集预训练，相比单纯模仿学习能产生更通用和鲁棒的策略，并能利用无动作标注的视频数据进一步提升微调性能。实验表明，UWM为整合异构数据集提供了新思路，弥合了模仿学习与世界建模的鸿沟。详见项目页面：https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T16:01:43Z
- **目录日期**: 2025-04-05
