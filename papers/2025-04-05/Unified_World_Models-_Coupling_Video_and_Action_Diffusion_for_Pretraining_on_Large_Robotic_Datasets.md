...
模仿学习是构建通用机器人的一种有前景的方法，但依赖高质量专家演示限制了其在大型机器人基础模型中的扩展。为解决这一问题，研究者提出了统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用异构数据（含动作和无动作视频）进行策略学习。实验表明，UWM不仅能通过多模态数据预训练提升策略的泛化性和鲁棒性，还能利用无动作视频数据进一步优化策略性能。该框架为模仿学习与世界建模提供了统一方案，推动了大规模异构数据在机器人学习中的应用。详情见项目页面。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T15:01:46Z
- **目录日期**: 2025-04-05
