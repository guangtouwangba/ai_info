...
本文提出了一种名为“统一世界模型（UWM）”的框架，旨在结合视频数据与动作数据来提升机器人策略学习。UWM通过统一的Transformer架构整合动作扩散和视频扩散过程，独立控制各模态的扩散时间步，从而灵活实现策略、动态预测、逆向动态及视频生成功能。实验表明，UWM能有效利用大规模多任务数据集进行预训练，生成比模仿学习更鲁棒的策略，并支持从无动作标注视频中学习以优化策略性能。该框架为利用异构数据实现可扩展的机器人学习提供了新思路，并弥合了模仿学习与世界建模间的鸿沟。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T04:01:49Z
- **目录日期**: 2025-04-05
