...
本文提出了统一世界模型（UWM），一种整合动作扩散和视频扩散的框架，通过独立控制不同模态的扩散时间步，灵活实现策略学习、动态预测和视频生成。UWM能有效利用大规模多任务机器人数据集进行预训练，相比模仿学习产生更通用和鲁棒的策略，并能自然结合无动作标注的视频数据提升微调策略性能。实验表明，UWM为利用异构数据实现可扩展的机器人学习提供了新思路，同时统一了模仿学习和世界建模这两种范式。该研究推动了通用机器人基础模型的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T00:01:48Z
- **目录日期**: 2025-04-05
