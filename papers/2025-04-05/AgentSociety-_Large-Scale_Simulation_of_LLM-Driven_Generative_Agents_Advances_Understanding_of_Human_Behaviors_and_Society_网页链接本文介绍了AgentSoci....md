...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理及其500万次互动。研究者用它分析了社会极化、信息传播、基本收入政策及外部冲击（如飓风）等社会问题，结果与现实世界数据吻合。该系统结合了LLM代理、真实社会环境和大规模模拟引擎，能够捕捉人类行为的内在机制，为社会科学研究和政策制定提供了新工具。实验表明，AgentSociety能有效补充传统研究方法（如调查和访谈），帮助理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T06:05:42Z
- **目录日期**: 2025-04-05
