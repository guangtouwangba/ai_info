...
AgentSociety是一个基于大语言模型(LLM)的大规模社会模拟系统，能够模拟超过1万个AI代理及其500万次社会互动。该系统通过研究社会极化、信息传播、基本收入政策和自然灾害影响等四个关键问题，验证了其模拟结果与现实世界数据的高度一致性。研究表明，AgentSociety不仅能有效捕捉人类行为模式和社会机制，还可作为社会科学研究和政策制定的有力工具，为传统研究方法(如调查和访谈)提供重要补充。这一创新平台展示了LLM在理解复杂社会现象方面的巨大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T17:05:03Z
- **目录日期**: 2025-04-05
