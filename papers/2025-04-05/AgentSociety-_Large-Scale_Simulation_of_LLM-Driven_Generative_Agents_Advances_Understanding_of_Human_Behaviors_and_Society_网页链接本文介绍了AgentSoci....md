...
本文介绍了AgentSociety，这是一个利用大型语言模型（LLM）驱动的大规模社会模拟器，能够模拟超过1万个代理的社会生活及其500万次互动。研究者通过该平台研究了极化现象、刺激性信息传播、无条件基本收入政策及外部冲击（如飓风）等社会问题。模拟结果与现实世界实验吻合，验证了其捕捉人类行为和社会机制的能力。AgentSociety展现了作为社会科学研究和政策制定工具的潜力，能够辅助传统研究方法（如调查和干预），帮助理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T11:05:50Z
- **目录日期**: 2025-04-05
