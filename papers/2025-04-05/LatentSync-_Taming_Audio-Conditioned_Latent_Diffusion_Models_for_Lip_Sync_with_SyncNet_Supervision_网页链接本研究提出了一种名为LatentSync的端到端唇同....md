...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。为增强时间一致性，创新性地提出TREPA方法，利用自监督视频模型对齐生成帧与真实帧。同时，针对SyncNet收敛问题，通过改进模型架构、训练参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进不仅适用于唇同步任务，也可推广至其他音频驱动的人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T21:05:22Z
- **目录日期**: 2025-04-05
