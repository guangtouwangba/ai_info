...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉关系。针对扩散模型的时间一致性问题，提出TREPA方法，利用自监督视频模型增强生成帧的时间对齐。同时优化SyncNet训练框架，改进模型架构、超参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供新思路。创新点包括：1)端到端潜在扩散模型 2)TREPA时间对齐 3)SyncNet训练优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T01:32:17Z
- **目录日期**: 2025-04-05
