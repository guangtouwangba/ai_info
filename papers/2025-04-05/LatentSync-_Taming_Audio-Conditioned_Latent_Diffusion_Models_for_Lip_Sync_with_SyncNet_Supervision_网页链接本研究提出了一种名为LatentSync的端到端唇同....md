...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，但发现扩散过程中存在时间一致性问题。为此，研究者提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧与真实帧。同时，改进了SyncNet训练框架，解决了其收敛问题。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T10:05:55Z
- **目录日期**: 2025-04-05
