...
该论文提出Native Sparse Attention（NSA）稀疏注意力机制，通过动态分层稀疏策略（粗粒度令牌压缩+细粒度选择）解决传统注意力机制处理长文本时的高计算成本问题。NSA结合算法创新与硬件优化，在保持全局上下文感知能力的同时确保局部精度，实现了高效的长文本建模。其核心创新在于硬件对齐设计和原生可训练性，为大规模语言模型提供了更高效的注意力计算方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T20:04:55Z
- **目录日期**: 2025-04-05
