...
这篇论文针对代码生成大模型（CodeLLMs）提出了CodeArena基准测试，关注模型输出与人类偏好的对齐问题。研究收集了397个高质量编程任务样本，覆盖40类任务和44种语言，并构建了20B标记的合成指令数据集SynCode-Instruct。实验发现现有基于执行的评估与人类偏好存在差距，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI）性能差异显著，凸显了人类偏好对齐的重要性。研究为代码生成模型的实用化评估提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T20:05:51Z
- **目录日期**: 2025-04-05
