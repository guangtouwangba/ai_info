...
这篇论文探讨了代码生成大模型（CodeLLMs）如何与人类偏好对齐的问题。现有基准主要关注代码正确性，忽略了人类偏好匹配。为此，研究者构建了CodeArena基准（含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令集（近200亿标记）。实验发现，执行基准与CodeArena存在性能差异，开源顶级模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显了对齐人类偏好的重要性。研究强调需要更全面的评估标准来提升代码生成模型的实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T07:06:09Z
- **目录日期**: 2025-04-05
