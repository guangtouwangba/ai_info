...
该论文提出CodeArena基准测试，针对代码生成大模型（CodeLLMs）与人类偏好对齐的问题进行研究。通过397个高质量样本覆盖40类编程任务和44种语言，模拟真实编程场景。同时构建20B标记的合成指令数据集SynCode-Instruct验证指令微调效果。实验发现执行基准与人类偏好存在差距，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著，凸显对齐人类偏好的重要性。研究填补了现有评估体系忽视用户体验的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T08:06:21Z
- **目录日期**: 2025-04-05
