...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，通过397个真实编程任务样本(覆盖40类/44种语言)评估模型与人类偏好的对齐程度。研究发现传统执行基准与人类偏好存在差异，开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。同时构建20B标记量的SynCode-Instruct合成指令集验证大规模微调效果，强调代码生成模型除正确性外，还需关注实际开发场景中的可用性需求。研究突显了代码生成模型与人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T04:05:48Z
- **目录日期**: 2025-04-05
