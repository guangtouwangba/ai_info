...
该研究构建了一个描述异质性细胞群体生长的偏微分方程模型，将细胞划分为多个离散表型。模型中细胞倾向于向压缩较小的区域迁移，其运动方向沿细胞压力梯度下降方向。细胞压力定义为不同表型细胞密度的加权和，且细胞迁移能力和压力贡献权重随表型变化。研究从基于个体的晶格模型推导出该连续模型，分析了细胞表型空间分离的传播波解，并通过数值模拟验证了模型一致性。结果表明，细胞迁移能力的变异性可导致入侵细胞前沿出现空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T11:02:42Z
- **目录日期**: 2025-04-05
