...
该研究提出了一种偏微分方程模型，描述由多种不同表型组成的异质细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动方向遵循细胞压力的梯度。细胞压力定义为各表型细胞密度的加权和，且细胞的迁移能力和对压力的贡献因表型而异。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型依赖性和压力调节下的分裂、死亡和迁移行为。数值模拟显示，不同表型细胞在入侵前沿呈现空间隔离，表明细胞迁移能力的差异可导致入侵前沿的空间分布异质性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T15:02:34Z
- **目录日期**: 2025-04-05
