...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动方向遵循细胞压力的梯度。细胞压力由不同表型细胞的密度加权和决定，且细胞迁移能力和压力贡献权重随表型而异。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型依赖性分裂、死亡和迁移行为。数值模拟显示，不同表型细胞在入侵前沿呈现空间分离的波状传播，且离散与连续模型结果高度一致，表明细胞迁移能力的异质性可导致入侵前沿的空间分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T06:02:42Z
- **目录日期**: 2025-04-05
