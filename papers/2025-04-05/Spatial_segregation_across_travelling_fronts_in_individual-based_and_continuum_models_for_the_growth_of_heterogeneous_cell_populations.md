...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型假设细胞会向压缩较小的区域迁移，其运动方向取决于细胞压力梯度（由不同表型细胞的密度加权求和）。通过将个体层面的离散模型（细胞作为随机移动、分裂和死亡的代理）转化为连续极限，研究发现不同表型的细胞在侵袭前沿会出现空间分离，形成行波解。数值模拟验证了离散与连续模型的一致性，表明细胞迁移能力的差异可导致侵袭前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T22:02:28Z
- **目录日期**: 2025-04-05
