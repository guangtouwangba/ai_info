...
该研究提出了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力的梯度变化。细胞压力定义为不同表型细胞密度的加权和，且细胞的迁移能力和压力贡献权重随表型而异。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型依赖和压力调控的分裂、死亡及迁移行为。数值模拟和行波分析表明，细胞迁移能力的变异性可导致入侵前沿出现空间隔离现象，两种模型结果高度一致。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T20:02:21Z
- **目录日期**: 2025-04-05
