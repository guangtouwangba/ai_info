...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体的生长动态。模型中，细胞倾向于向压力较低的区域迁移，其运动方向取决于由不同表型细胞密度加权求和形成的细胞压力梯度。不同表型细胞具有不同的形态和力学特性，表现为细胞迁移能力和对压力的贡献权重存在差异。研究通过离散个体模型的连续极限推导出该方程，分析了表型依赖的细胞分裂、死亡和迁移行为。数值模拟显示，细胞迁移能力的异质性可导致入侵前沿出现空间表型隔离，与行波分析结果高度吻合。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T08:02:37Z
- **目录日期**: 2025-04-05
