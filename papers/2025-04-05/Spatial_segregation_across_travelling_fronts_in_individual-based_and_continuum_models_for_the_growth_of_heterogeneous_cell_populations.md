...
该研究构建了一个异质性细胞群体生长的偏微分方程模型，将细胞划分为多个离散表型。模型假设细胞会向压缩较小的区域迁移，其运动遵循细胞压力梯度（压力由不同表型细胞的密度加权和定义）。不同表型的细胞具有不同的形态和力学特性，表现为细胞迁移能力和对压力的贡献权重存在表型依赖性。研究从基于个体的离散模型推导出连续极限，分析了细胞表型依赖的增殖、死亡和迁移行为，并研究了表型空间分离的传播波解。数值模拟验证了模型一致性，表明细胞迁移能力的差异可导致侵袭前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T01:29:24Z
- **目录日期**: 2025-04-05
