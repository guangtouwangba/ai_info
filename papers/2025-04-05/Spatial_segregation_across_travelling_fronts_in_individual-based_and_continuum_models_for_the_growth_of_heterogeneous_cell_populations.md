...
该研究构建了一个异质性细胞群体生长的偏微分方程模型，将细胞划分为多个离散表型。模型假设细胞会向压缩较小的区域迁移，其运动遵循细胞压力梯度（压力为各表型细胞密度的加权和）。不同表型的细胞具有不同的形态和力学特性，表现为细胞迁移能力和压力贡献系数的差异。研究通过离散个体模型的连续极限推导该方程，分析了细胞表型依赖的压力调节分裂、死亡和迁移行为。数值模拟验证了模型与行波解的一致性，表明细胞迁移能力的差异可导致入侵前沿出现空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T23:02:30Z
- **目录日期**: 2025-04-05
