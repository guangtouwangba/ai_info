...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力梯度（压力为不同表型细胞密度的加权和）。不同表型的细胞具有不同的形态和力学特性，表现为细胞迁移能力和压力贡献权重的差异。研究通过离散个体模型的连续极限推导该方程，分析了细胞表型依赖和压力调节的分裂、死亡及迁移行为。数值模拟和行波分析表明，细胞迁移能力的差异可导致入侵前沿出现空间表型隔离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T07:02:21Z
- **目录日期**: 2025-04-05
