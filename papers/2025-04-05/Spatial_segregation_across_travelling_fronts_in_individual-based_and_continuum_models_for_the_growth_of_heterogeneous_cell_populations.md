...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体生长。模型中细胞倾向于向压力较低区域迁移，细胞压力定义为不同表型细胞密度的加权和。不同表型细胞具有不同的形态和力学特性，表现为细胞迁移能力和压力贡献的差异。研究通过离散个体模型推导出连续极限，并分析入侵前沿出现表型空间分离的行波解。数值模拟验证了两种模型的一致性，表明细胞迁移能力的差异可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T04:02:28Z
- **目录日期**: 2025-04-05
