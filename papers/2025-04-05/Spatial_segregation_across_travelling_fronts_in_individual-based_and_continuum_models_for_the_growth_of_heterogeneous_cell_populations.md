...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中细胞倾向于向压力较低区域迁移，细胞压力定义为不同表型细胞密度的加权和。不同表型的细胞具有不同的形态和力学特性，表现为细胞迁移能力和对压力的贡献权重存在差异。研究从基于个体的离散模型推导出该连续模型，并分析了细胞表型在入侵前沿空间分离的行波解。数值模拟验证了两种模型的一致性，表明细胞迁移能力的变异性可能导致入侵前沿出现空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T21:02:27Z
- **目录日期**: 2025-04-05
