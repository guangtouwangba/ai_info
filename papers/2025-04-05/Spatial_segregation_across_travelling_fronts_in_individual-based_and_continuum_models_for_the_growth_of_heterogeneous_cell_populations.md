...
该研究提出了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体生长。模型假设细胞倾向于向压缩较小的区域移动，其运动遵循细胞压力梯度（压力为不同表型细胞密度的加权和）。不同表型细胞的形态和力学特性差异通过表型依赖的迁移率和压力贡献权重体现。研究从基于个体的离散模型推导出连续极限，并分析了入侵波前中表型空间分离的行波解。数值模拟验证了离散与连续模型的一致性，表明细胞迁移率的异质性可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T03:12:37Z
- **目录日期**: 2025-04-05
