...
这篇论文研究了由多种表型组成的异质细胞群体的生长模型。通过偏微分方程描述细胞群体动态，其中细胞倾向于向压力较低的区域迁移，而压力由不同表型细胞的密度加权决定。模型考虑了表型差异导致的细胞运动性和压力贡献度的变化，并通过离散个体模型的连续极限推导得出。研究分析了细胞表型在入侵波前空间分离的行波解，并通过数值模拟验证了离散与连续模型的一致性。结果表明，细胞运动性的异质性可导致入侵波前的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-05T17:02:13Z
- **目录日期**: 2025-04-05
