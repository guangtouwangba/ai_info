...
GitHub上的legacy-cc项目公开了C语言之父Dennis Ritchie在1970年代编写的早期C编译器源码，堪称编程语言"考古现场"。该代码展现了C语言的原始形态（如无类型检查、默认int变量等），揭示了编译器核心逻辑（语法树处理、内存分配等）和优化雏形（如表达式折叠），成为现代编译器技术（如LLVM/GCC）的源头。项目包含prestruct和last1120c两个历史版本目录，采用宽松开源协议（2002年由Caldera授权）。开发者将其视为"数字文物"，展现了C语言从零演化的珍贵历史。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T22:03:46Z
- **目录日期**: 2025-03-25
