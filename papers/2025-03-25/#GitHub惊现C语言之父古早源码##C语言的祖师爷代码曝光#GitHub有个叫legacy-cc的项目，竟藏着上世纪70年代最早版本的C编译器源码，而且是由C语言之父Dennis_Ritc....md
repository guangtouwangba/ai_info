...
GitHub上出现了一个名为"legacy-cc"的项目，包含了C语言之父Dennis Ritchie在1970年代编写的最早版本C编译器源码。这个"数字文物"展示了C语言的原始形态，如没有类型检查、变量默认为int等特性，揭示了编译器处理语法树、内存分配等底层逻辑的雏形。项目包含早期(prestruct)和后期(last1120c)两个阶段的代码，Caldera公司在2002年就开放了这些源码。这份代码不仅是编程历史的重要见证，更为理解现代编译器技术(如LLVM/GCC)的起源提供了珍贵资料。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T19:03:35Z
- **目录日期**: 2025-03-25
