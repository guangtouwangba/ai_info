...
GitHub上的legacy-cc项目曝光了C语言之父Dennis Ritchie在70年代编写的早期C编译器源码，堪称编程语言"考古现场"。该代码展示了C语言的原始形态，如无类型检查、默认int变量等特性，并揭示了编译器核心逻辑（语法树处理、内存分配等）和早期优化技术（如表达式折叠），成为现代编译器（如LLVM/GCC）的技术雏形。项目包含prestruct和last1120c两个历史版本目录，2002年由Caldera公司以宽松开源协议发布。开发者认为这些代码不仅是技术遗产，更是Ritchie留给后人的珍贵手稿。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T14:04:09Z
- **目录日期**: 2025-03-25
