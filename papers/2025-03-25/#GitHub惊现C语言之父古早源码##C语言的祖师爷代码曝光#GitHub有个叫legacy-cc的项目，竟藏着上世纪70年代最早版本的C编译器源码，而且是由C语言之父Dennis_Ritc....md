...
GitHub上出现了一个名为legacy-cc的项目，其中包含了C语言之父Dennis Ritchie在上世纪70年代编写的最早版本C编译器源码。这段代码展示了C语言的原始形态，如缺乏类型检查、变量默认为int等特性，并揭示了编译器处理语法树、内存分配和语义分析的早期逻辑。项目分为prestruct和last1120c两个目录，对应不同开发阶段。2002年Caldera公司将其开源，允许自由使用和修改。这段代码不仅是技术考古的重要发现，更是编程语言发展史上的珍贵遗产。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T15:04:04Z
- **目录日期**: 2025-03-25
