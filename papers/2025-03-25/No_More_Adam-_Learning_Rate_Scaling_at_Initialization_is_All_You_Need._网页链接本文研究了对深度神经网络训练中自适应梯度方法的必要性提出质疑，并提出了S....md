...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统认知。该方法通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，有效解决了训练不平衡问题，同时将内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于AdamW，尤其在大模型训练中显著节省内存（如为Llama2-7B节省25.15GB）。该方法还展现出超参数鲁棒性，并在LoRA微调等任务中超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T13:11:16Z
- **目录日期**: 2025-03-25
