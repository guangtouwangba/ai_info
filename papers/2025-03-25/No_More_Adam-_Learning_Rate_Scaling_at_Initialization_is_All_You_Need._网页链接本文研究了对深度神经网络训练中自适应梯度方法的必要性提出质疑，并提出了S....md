...
该研究提出SGD-SaI方法，挑战了深度神经网络训练必须使用自适应梯度方法（如Adam）的传统认知。该方法基于梯度信号噪声比（g-SNR）在初始化时调整不同参数组的学习率，无需自适应二阶动量即可避免训练初期不平衡问题，并将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2/Llama2预训练中表现优于或持平AdamW，尤其显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB），同时展现超参数鲁棒性，在LoRA微调等任务中也超越现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T20:04:06Z
- **目录日期**: 2025-03-25
