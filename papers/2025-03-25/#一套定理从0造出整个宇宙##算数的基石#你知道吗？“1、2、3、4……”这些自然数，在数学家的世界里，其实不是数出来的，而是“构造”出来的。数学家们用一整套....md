...
皮亚诺公理从零构建自然数体系：以0为起点，通过后继函数S(n)递归生成序列（如S(0)=1），并设定关键约束——0非后继、后继唯一性、避免循环/重复。这一形式化构造不仅奠定了自然数的逻辑基础，还支撑了整数/实数等扩展体系，成为计算机科学（编程逻辑、密码学）和AI推理的数学根基。其"从空集创造无限"的思想，体现了数学公理化方法的强大，使自然数成为人类构建科技大厦最精密的"元工具"之一。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T21:02:55Z
- **目录日期**: 2025-03-25
