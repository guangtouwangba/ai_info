...
皮亚诺公理从零构建了整个自然数体系：以0为起点，通过后继函数S(n)递归生成后续数字（S(0)=1，S(1)=2），并设定关键约束——0非任何数后继、不同数后继必不同，确保数链无循环/重复。这种形式化构造不仅奠定了数学基石，还深刻影响了计算机科学（编程逻辑、密码学等）和人工智能的底层架构。公理系统通过严密的"防作弊机制"，使自然数成为兼具严谨性与扩展性的基础模型，堪称数学史上最精妙的"创世"发明之一。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T17:03:15Z
- **目录日期**: 2025-03-25
