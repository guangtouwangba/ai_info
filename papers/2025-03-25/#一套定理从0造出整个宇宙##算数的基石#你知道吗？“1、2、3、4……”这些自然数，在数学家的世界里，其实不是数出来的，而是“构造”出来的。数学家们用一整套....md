...
皮亚诺公理（Peano Axioms）从零构建了整个自然数体系：以0为起点，通过后继函数S(n)递归生成序列（如S(0)=1），并设定关键约束——0非任何数的后继、不同数后继必不同，确保序列无限且唯一。这一形式化构造不仅奠定了数学基础（延伸出整数、实数等），更深刻影响了计算机科学（运算逻辑、编程语言）和密码学等领域。公理系统通过严格的"防作弊机制"（如排除额外元素"！"）保障了数学体系的严谨性，成为现代科技大厦不可或缺的理论基石。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T13:09:39Z
- **目录日期**: 2025-03-25
