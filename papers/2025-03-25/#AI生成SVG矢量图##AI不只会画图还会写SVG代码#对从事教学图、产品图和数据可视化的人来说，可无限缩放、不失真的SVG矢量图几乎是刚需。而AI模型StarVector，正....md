...
StarVector是一款能直接生成高质量SVG矢量图代码的AI工具，支持通过图像或文字描述自动输出结构清晰、可直接使用的SVG代码（如<circle>、<rect>等原生元素）。其基于包含210万组数据的SVG-Stack数据集训练，独创DinoScore语义评估指标，生成速度仅74秒（传统方法需20分钟）。提供1B（轻量版）和8B（高精度版）两个模型版本，擅长教学图、产品图等场景。研究表明其输出与人类设计高度吻合，预示未来SVG可能由AI"编写"而非绘制。论文及代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T04:04:02Z
- **目录日期**: 2025-03-25
