...
StarVector是一款AI工具，能直接生成高质量SVG矢量图的代码，适用于教学图、产品图和数据可视化。它基于SVG-Stack数据集（含210万组图像和代码），支持图像或文本输入，输出简洁、语义化的SVG代码（如<circle>、<rect>等）。其独创的DinoScore评估指标与人类评分高度一致。提供轻量版（StarVector-1B）和旗舰版（StarVector-8B），生成速度仅74秒，远超传统方法。未来SVG图形或由AI“编写”而非绘制。论文和代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T03:15:26Z
- **目录日期**: 2025-03-25
