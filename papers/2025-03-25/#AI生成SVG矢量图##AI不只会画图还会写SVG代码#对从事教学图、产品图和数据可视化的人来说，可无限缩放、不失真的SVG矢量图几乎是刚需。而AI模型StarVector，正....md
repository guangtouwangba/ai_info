...
StarVector是一款AI工具，能直接生成高质量SVG矢量图的代码，适用于教学图、产品图和数据可视化。它基于SVG-Stack数据集（含210万组图像与代码），支持图像或文本输入，输出简洁标准的SVG代码（如<circle>、<rect>等）。采用自创的DinoScore评估语义相似度，性能远超传统方法（74秒/图）。提供1B（轻量）和8B（高精度）两个版本。未来SVG图形可能由AI"编写"而非绘制。论文和代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T01:30:58Z
- **目录日期**: 2025-03-25
