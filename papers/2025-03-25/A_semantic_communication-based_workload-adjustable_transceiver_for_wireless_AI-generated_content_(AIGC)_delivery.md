...
本文提出了一种面向动态无线网络的资源感知可调工作负载收发器（ROUTE），用于优化AI生成内容（AIGC）的传输。针对无线网络中的信道不稳定、带宽有限和计算资源分布不均等问题，ROUTE结合语义通信（SemCom）和基于扩散的生成AI模型，优先传输生成内容的关键语义信息以缓解通信资源瓶颈，并通过改进的扩散模型动态调整计算负载和语义密度，提升边缘与本地计算资源利用率，减少传输中的语义失真。仿真表明，相比传统AIGC方法，ROUTE在延迟和内容质量方面表现更优。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T20:01:31Z
- **目录日期**: 2025-03-25
