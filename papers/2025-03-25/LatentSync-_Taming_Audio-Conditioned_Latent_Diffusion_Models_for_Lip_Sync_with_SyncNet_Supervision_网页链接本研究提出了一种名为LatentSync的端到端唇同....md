...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强生成帧的连贯性。同时，研究深入分析了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理方案，显著提升了同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。该成果对基于SyncNet的唇同步技术发展具有普适参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T14:05:53Z
- **目录日期**: 2025-03-25
