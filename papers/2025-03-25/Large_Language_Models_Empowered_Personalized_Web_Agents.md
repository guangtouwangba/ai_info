...
这篇论文提出了个性化网络代理任务，利用大语言模型（LLM）整合用户个性化数据和指令，以提升网络任务完成的精准度。作者构建了PersonalWAB基准测试，包含用户指令、个性化数据、网络功能及两种评估范式。为解决现有LLM网络代理忽视个性化数据的问题，论文提出PUMA框架：通过记忆库和任务特定检索策略筛选相关历史行为，并采用微调和直接偏好优化方法对齐LLM，实现个性化操作执行。实验证明PUMA在PersonalWAB上优于现有网络代理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T20:02:18Z
- **目录日期**: 2025-03-25
