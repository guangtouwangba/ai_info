...
这篇论文提出了一种基于大语言模型(LLM)的个性化网络代理(PUMA)，通过整合用户档案和历史行为数据来更好地理解个性化指令并执行定制操作。研究团队构建了首个个性化网络代理评测基准PersonalWAB，包含用户指令、个性化数据、网络功能和两种评估范式。PUMA框架采用记忆库存储历史行为，通过特定任务检索策略筛选相关数据，并利用微调和直接偏好优化方法使LLM适应个性化任务执行。实验证明PUMA在PersonalWAB上优于现有网络代理，为个性化网络任务自动化提供了有效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T14:02:44Z
- **目录日期**: 2025-03-25
