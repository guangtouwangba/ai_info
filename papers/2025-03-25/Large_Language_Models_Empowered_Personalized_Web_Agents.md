...
这篇论文提出了个性化网络代理任务，通过整合用户个性化数据和指令来提升网络任务完成的准确性。作者构建了PersonalWAB基准测试，包含用户指令、个性化数据、网络功能和两种评估范式。为解决现有大语言模型（LLM）网络代理忽视个性化数据的问题，论文提出了PUMA框架：利用记忆库和任务特定检索策略筛选相关历史网络行为，并通过微调和偏好优化使LLM适应个性化操作执行。实验证明PUMA在PersonalWAB上优于现有网络代理。该研究突出了个性化数据在理解用户指令和执行定制操作中的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T05:02:27Z
- **目录日期**: 2025-03-25
