...
本文提出了一种基于大语言模型(LLM)的个性化网络代理(PUMA)，通过整合用户个性化数据(如个人资料和历史网络行为)来更好地理解用户指令并执行定制化操作。研究构建了首个个性化网络代理评测基准PersonalWAB，包含用户指令、个性化数据、网络功能和两种评估范式。PUMA框架采用记忆库和任务特定检索策略来筛选相关历史行为，并通过微调和直接偏好优化使LLM适应个性化操作执行。实验表明，PUMA在PersonalWAB上显著优于现有网络代理，验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T07:02:46Z
- **目录日期**: 2025-03-25
