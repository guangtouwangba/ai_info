...
这篇论文提出了一种基于大语言模型（LLM）的个性化网络代理（Web agent）框架PUMA，旨在通过整合用户个性化数据（如个人资料和历史网络行为）来更好地理解和执行用户指令。为解决缺乏评估基准的问题，研究者构建了PersonalWAB评测集，包含用户指令、个性化数据、网络功能和两种评测范式。PUMA框架利用记忆库和任务相关检索策略筛选历史行为数据，并通过微调和偏好优化使LLM适应个性化任务执行。实验表明，PUMA在PersonalWAB上显著优于现有网络代理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T21:02:16Z
- **目录日期**: 2025-03-25
