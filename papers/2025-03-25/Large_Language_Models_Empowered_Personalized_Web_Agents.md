...
该研究提出了一种基于大语言模型（LLM）的个性化网页代理（Personalized Web Agent），通过整合用户个性化数据（如个人资料和历史行为）来更好地理解用户指令并执行定制化操作。为解决缺乏评估基准的问题，研究者构建了PersonalWAB评测基准，包含用户指令、个性化数据、网页功能和两种评估范式。同时提出了PUMA框架，通过记忆库检索相关历史行为，并采用微调和偏好优化方法使LLM适应个性化任务。实验表明PUMA在PersonalWAB上优于现有网页代理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T13:09:02Z
- **目录日期**: 2025-03-25
