...
这篇论文提出了一种基于大语言模型（LLM）的个性化网络代理PUMA，旨在通过整合用户档案和历史行为数据来更好地理解个性化指令并执行定制化操作。研究者首先定义了LLM赋能的个性化网络代理任务，并构建了评估基准PersonalWAB，包含用户指令、个性化数据、网络功能及两种评估范式。PUMA框架采用记忆库和任务特定检索策略筛选相关历史行为，通过微调和直接偏好优化使LLM适应个性化网络代理任务。实验证明PUMA在PersonalWAB上优于现有网络代理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T15:02:38Z
- **目录日期**: 2025-03-25
