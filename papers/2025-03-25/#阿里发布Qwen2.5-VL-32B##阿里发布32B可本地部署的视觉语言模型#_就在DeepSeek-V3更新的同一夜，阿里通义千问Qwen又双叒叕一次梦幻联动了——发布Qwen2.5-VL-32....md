...
阿里发布了可本地部署的视觉语言模型Qwen2.5-VL-32B，填补了此前3B/7B/72B之间的空白。该模型通过强化学习优化，在人类偏好对齐、数学推理、图像解析等方面表现突出，其视觉逻辑推导能力甚至超过72B版本。在纯文本任务上，它超越了同规模的Mistral-24B和Gemma-27B等模型。典型应用场景包括：通过交通指示牌照片分析卡车行驶可行性，能结合限速、时间、距离进行多步骤推理。模型已开源，支持本地部署。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T02:29:41Z
- **目录日期**: 2025-03-25
