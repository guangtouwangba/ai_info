...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现：现有执行基准与人类偏好存在偏差，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著，表明对齐人类偏好的重要性。研究凸显了当前代码生成评估体系的不足，为提升模型实用性提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T06:06:12Z
- **目录日期**: 2025-03-25
