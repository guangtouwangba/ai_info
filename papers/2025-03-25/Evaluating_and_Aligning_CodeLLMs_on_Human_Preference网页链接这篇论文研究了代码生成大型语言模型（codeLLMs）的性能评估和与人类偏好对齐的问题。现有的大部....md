...
该研究针对代码生成大模型(codeLLMs)提出CodeArena基准测试，包含397个真实编程任务样本(覆盖40类/44种语言)，首次关注模型输出与人类偏好的对齐问题。同时构建20B标记的SynCode-Instruct合成指令集验证微调效果。实验发现：1)传统执行测试与人类偏好存在偏差；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著。研究强调代码LLMs需兼顾功能正确性和人类可接受性，为模型优化提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T07:06:17Z
- **目录日期**: 2025-03-25
