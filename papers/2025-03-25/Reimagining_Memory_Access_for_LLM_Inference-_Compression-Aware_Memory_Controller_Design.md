...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。主要创新点包括：(1)采用无损块压缩技术(LZ4/ZSTD)压缩模型权重和KV缓存，在不影响推理质量的情况下显著降低内存占用(权重减少25.2%，KV缓存减少46.9%)；(2)通过细粒度位级访问和LLM感知的内存配置，实现内存带宽和能耗随动态量化的比例调节。硬件原型在7nm工艺下达到8TB/s吞吐量，面积开销仅3.8mm²，证明了该方案在大规模推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T16:01:52Z
- **目录日期**: 2025-03-25
