...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的新方法。该方法采用无损块压缩技术(LZ4/ZSTD)压缩模型权重和KV缓存，在不影响推理质量的前提下，显著降低了25.2%的权重内存占用和46.9%的KV缓存占用。同时通过上下文相关的动态量化，使内存带宽和能耗实现按比例缩放。硬件原型在7nm工艺下实现了8TB/s的高吞吐量，面积开销仅3.8mm²。实验证明这种LLM感知的内存控制方案能有效解决大规模推理中的内存瓶颈问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T12:01:56Z
- **目录日期**: 2025-03-25
