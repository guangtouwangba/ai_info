...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型（LLM）推理效率的方法。该方法采用无损块压缩（如LZ4和ZSTD）技术，在不影响推理质量的情况下，显著减少了模型权重和键值（KV）缓存的内存占用和带宽需求。实验结果显示，模型权重和KV缓存的内存占用分别减少了25.2%和46.9%。硬件原型在7纳米工艺下实现了8 TB/s的吞吐量，面积开销仅为3.8平方毫米，验证了该方案在大规模推理中的高效性和可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T20:01:42Z
- **目录日期**: 2025-03-25
