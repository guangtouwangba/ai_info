...
这篇论文提出了一种通过改进AI加速器的片上内存控制器来提升大型语言模型(LLM)推理效率的解决方案。该方法采用无损块压缩(如LZ4和ZSTD)技术压缩模型权重和键值缓存(KV cache)，在不影响推理质量的情况下，显著减少了25.2%的权重内存占用和46.9%的KV缓存占用。同时通过LLM感知的内存配置，实现了内存带宽和能耗随动态量化的按比例调整。硬件原型在7nm工艺下达到8TB/s吞吐量，面积开销仅3.8mm²，证明了该方法在大规模高效推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T15:01:56Z
- **目录日期**: 2025-03-25
