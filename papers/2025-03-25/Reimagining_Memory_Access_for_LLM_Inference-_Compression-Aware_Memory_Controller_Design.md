...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩(如LZ4和ZSTD)技术压缩模型权重和键值缓存(KV cache)，在不影响推理质量的情况下，显著减少了25.2%的权重内存占用和46.9%的KV缓存占用。同时，通过动态量化使内存带宽和能耗随上下文需求按比例调整。硬件原型在7nm工艺下实现了8TB/s的吞吐量，面积开销仅3.8mm²，验证了LLM感知内存控制在提升大规模推理效率方面的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T18:01:42Z
- **目录日期**: 2025-03-25
