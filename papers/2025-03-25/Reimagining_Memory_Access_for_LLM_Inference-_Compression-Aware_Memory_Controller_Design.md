...
该研究提出了一种通过改进AI加速器的片上内存控制器来提升大语言模型(LLM)推理效率的方法。该方法采用无损块压缩(如LZ4和ZSTD)技术压缩模型权重和KV缓存，在不影响推理质量的情况下，显著降低了内存容量和带宽需求。实验显示，该方法可将模型权重内存占用减少25.2%，KV缓存减少46.9%。硬件原型在7nm工艺下实现了8TB/s的吞吐量，面积开销仅3.8mm²，证明LLM感知的内存控制是实现高效大规模推理的关键。该方法还支持根据上下文动态调整量化精度，使带宽和能耗按比例变化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T17:01:57Z
- **目录日期**: 2025-03-25
