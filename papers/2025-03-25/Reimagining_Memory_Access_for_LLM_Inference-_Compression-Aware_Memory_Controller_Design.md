...
该研究提出了一种通过改进AI加速器的片上内存控制器来缓解大语言模型(LLM)推理中内存瓶颈的新方法。该方法采用无损块压缩技术(LZ4/ZSTD)压缩模型权重和KV缓存，在不影响推理质量的情况下，将模型权重和KV缓存的内存占用分别减少了25.2%和46.9%。同时通过LLM感知的内存配置，实现了内存带宽和能耗随上下文动态量化按比例调整。硬件原型在7nm工艺下达到8TB/s吞吐量，面积开销仅3.8mm²，证明了该方法在大规模高效推理中的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T08:01:51Z
- **目录日期**: 2025-03-25
