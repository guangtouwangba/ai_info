...
大模型正在革新兴趣推荐系统，突破传统基于标签/ID匹配的局限。YouTube使用Transformer生成"语义ID"解决冷启动；快手的M3CSR多模态模型融合音视频理解用户偏好；Meta通过知识蒸馏将大模型能力迁移到小模型，精度提升48%；Netflix的UniCoRn统一处理搜索推荐，实现个性化结果；华为FLIP模型实现文本与表格数据互通。这些技术结合知识蒸馏、对比学习等方法，使推荐系统更智能高效，能理解内容语义和用户深层需求，同时降低计算成本，显著提升推荐准确性和用户体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T02:30:32Z
- **目录日期**: 2025-03-25
