...
大模型(LLM)正在革新兴趣推荐系统，突破传统基于标签匹配的局限。YouTube采用"语义ID"解决冷启动问题；快手M3CSR融合多模态数据捕捉用户兴趣；Meta通过知识蒸馏提升小模型性能；LinkedIn开发1500亿参数统一推荐模型；Netflix的UniCoRn系统结合搜索与推荐；华为FLIP模型实现文本与结构化数据互通；Spotify利用LLM生成探索性搜索建议。这些技术结合知识蒸馏、LoRA等优化方法，使推荐系统更智能高效，能理解内容本质和用户深层需求，显著提升推荐准确性和响应速度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T03:15:37Z
- **目录日期**: 2025-03-25
