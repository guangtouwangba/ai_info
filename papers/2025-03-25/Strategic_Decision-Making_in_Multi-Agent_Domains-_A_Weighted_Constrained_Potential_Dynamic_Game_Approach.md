...
本文提出利用约束动态势博弈（WCPDG）来解决多智能体交互决策问题。通过分析现实场景中的特定结构，将复杂交互转化为加权约束动态势博弈，使得广义纳什均衡（GNE）可通过求解单个约束最优控制问题获得。相比现有博弈求解器，该方法显著提高了计算效率。仿真实验验证了其有效性，并在双四旋翼无人机协同搬运物体并避让行人的导航任务中进行了实际验证。该方法为多目标耦合的交互决策提供了高效计算框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T00:02:24Z
- **目录日期**: 2025-03-25
