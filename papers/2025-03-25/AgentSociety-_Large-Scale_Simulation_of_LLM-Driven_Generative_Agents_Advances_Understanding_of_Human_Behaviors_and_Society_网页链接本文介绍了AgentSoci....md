...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它利用大型语言模型驱动超过1万个代理，模拟了500万次社会互动。研究者用它分析了四个关键社会问题：极化现象、刺激性信息传播、无条件基本收入政策影响和飓风等外部冲击。模拟结果与现实世界数据吻合，验证了该系统捕捉人类行为和社会机制的能力。AgentSociety展现了作为社会科学研究和政策制定工具的潜力，能够补充传统研究方法（如调查和访谈），帮助深入理解社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T05:05:00Z
- **目录日期**: 2025-03-25
