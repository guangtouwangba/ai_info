...
百度推出的"秒哒"是国内首个对话式应用开发平台，用户无需编写代码，仅需输入需求，AI即可在3分钟内自动完成完整网站开发。该平台能智能处理架构设计、技术选型、前后端编码、测试等全流程，并生成包含多个页面、后台管理及移动端适配的完整应用。用户可对生成内容进行二次编辑，支持一键发布并提供域名。该产品由李彦宏在2022年11月发布，主打"零代码"快速开发H5、网站和轻应用，显著降低开发门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T01:31:42Z
- **目录日期**: 2025-03-25
