...
FlowDock是一种基于条件流匹配的深度几何生成模型，首次实现了直接将未结合（apo）结构映射到任意数量配体结合（holo）结构的功能。它能预测结构置信度和结合亲和力，支持多配体虚拟筛选。在PoseBusters基准测试中，FlowDock以51%的盲对接成功率超越单序列AlphaFold 3；在DockGen-E数据集上，其表现与单序列Chai-1相当。在CASP16竞赛中，FlowDock位列前5名，展示了其在虚拟筛选中的有效性。该模型为药物发现提供了新工具，代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T14:02:34Z
- **目录日期**: 2025-03-25
