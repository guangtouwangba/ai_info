...
FlowDock是一种基于条件流匹配的深度几何生成模型，可直接将未结合（apo）蛋白质结构映射到任意数量配体结合的（holo）结构。它能同时预测多个配体的结合构象、结构置信度和结合亲和力，支持快速虚拟筛选。在PoseBusters基准测试中，FlowDock以51%的盲对接成功率超越单序列AlphaFold 3；在DockGen-E数据集上，其结合口袋泛化能力与单序列Chai-1相当。在CASP16竞赛中，FlowDock位列蛋白质-配体亲和力预测前5名。该模型无需多序列比对信息，显著提升了药物发现中的对接和筛选效率。代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T16:02:24Z
- **目录日期**: 2025-03-25
