...
FlowDock是一种基于条件流匹配的深度几何生成模型，能直接将未结合（apo）结构映射到任意数量配体的结合（holo）结构，支持灵活对接和亲和力预测。它提供结构置信度和结合亲和力值，可用于多配体药物靶点的快速虚拟筛选。在PoseBusters基准测试中，FlowDock以51%的盲对接成功率超越单序列AlphaFold 3；在DockGen-E数据集上，其性能与单序列Chai-1相当。在CASP16的140个蛋白-配体复合物评估中，FlowDock跻身亲和力预测前五名。代码、数据和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T17:02:27Z
- **目录日期**: 2025-03-25
