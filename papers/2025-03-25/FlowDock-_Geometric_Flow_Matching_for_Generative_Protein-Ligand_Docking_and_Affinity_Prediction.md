...
FlowDock是一种基于条件流匹配的深度几何生成模型，能够直接将未结合（apo）蛋白质结构映射到任意数量配体结合（holo）的复合结构。它不仅支持柔性蛋白-配体对接和亲和力预测，还能同时处理多个配体，并提供结构置信度和结合亲和力评分。在PoseBusters测试中，FlowDock以51%的成功率超越单序列AlphaFold 3；在DockGen-E数据集上表现与Chai-1相当。在CASP16竞赛中，其亲和力预测位列前5。该模型为药物发现提供了高效的虚拟筛选工具，代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T21:02:06Z
- **目录日期**: 2025-03-25
