...
FlowDock是一种基于条件流匹配的深度几何生成模型，能够直接将未结合（apo）结构映射到任意数量配体的结合（holo）结构，支持灵活蛋白-配体对接和亲和力预测。在PoseBusters基准测试中，FlowDock以51%的盲对接成功率超越单序列AlphaFold 3；在DockGen-E数据集上，其性能与单序列Chai-1相当。此外，在CASP16竞赛中，FlowDock在140个蛋白-配体复合物的亲和力预测中位列前五。该模型可生成结构置信度评分和结合亲和力值，助力新药靶点的快速虚拟筛选。源代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T07:02:36Z
- **目录日期**: 2025-03-25
