...
FlowDock是一种基于条件流匹配的新型深度几何生成模型，能直接将未结合(apo)结构映射到任意数量配体结合的(holo)结构，支持灵活蛋白质-配体对接和亲和力预测。在PoseBusters基准测试中，FlowDock以51%的成功率超越单序列AlphaFold 3；在DockGen-E数据集上表现与Chai-1相当；在CASP16竞赛中，其亲和力预测位列前五。该模型能同时处理多个配体，并提供结构置信度和结合亲和力评分，有助于新药靶点的快速虚拟筛选。代码和预训练模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-03-25T05:02:18Z
- **目录日期**: 2025-03-25
