...
随着大语言模型(LLM)推理流程日益复杂，传统模拟器已无法准确评估异构多阶段推理系统。为此，研究者提出了HERMES异构多阶段LLM推理模拟器。HERMES能精确建模检索增强生成(RAG)、KV缓存检索、推理、预填充和解码等多个阶段，支持异构客户端并发执行不同模型，并整合了先进批处理策略和多级内存架构。通过结合实际硬件追踪和分析建模，HERMES能评估内存带宽争用、集群间通信延迟等关键权衡。该工具为系统设计者优化软硬件协同设计提供了重要参考，助力下一代AI工作负载的高效处理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T00:02:11Z
- **目录日期**: 2025-04-18
