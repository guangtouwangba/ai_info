...
随着大语言模型(LLM)的快速发展，传统推理流程已无法满足多阶段任务(如检索增强生成、KV缓存检索等)的异构计算需求。为此，研究者提出了HERMES异构多阶段LLM推理模拟器，它能精确模拟包括RAG、KV检索、推理等各阶段在不同硬件架构(如GPU、CPU等)上的执行情况。HERMES支持多模型并发执行，整合了先进批处理策略和多级内存层次结构，通过结合真实硬件追踪和分析建模，可评估内存带宽争用、集群间通信延迟等关键因素。该工具为优化下一代AI工作负载的软硬件协同设计提供了重要支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T03:14:21Z
- **目录日期**: 2025-04-18
