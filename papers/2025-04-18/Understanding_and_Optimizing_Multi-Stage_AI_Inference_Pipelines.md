...
随着大语言模型(LLMs)的快速发展，传统推理流程已无法满足需求。现代LLM服务涉及检索增强生成(RAG)、KV缓存检索、动态模型路由等多阶段计算，需要整合GPU、ASIC、CPU等异构硬件。现有模拟器难以精确建模这种复杂工作流。为此，研究者提出HERMES异构多阶段LLM推理模拟器，支持RAG、KV检索、推理等全流程建模，能模拟多模型并发执行、高级批处理策略和多级内存架构。通过结合真实硬件追踪和分析建模，HERMES可评估内存带宽争用、集群通信延迟等关键指标，为下一代AI工作负载的软硬件协同设计提供优化指导。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T02:28:01Z
- **目录日期**: 2025-04-18
