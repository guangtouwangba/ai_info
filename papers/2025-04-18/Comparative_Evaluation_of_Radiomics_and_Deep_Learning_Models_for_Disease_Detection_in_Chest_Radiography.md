...
本研究系统评估了基于放射组学和深度学习的AI模型在胸片疾病检测（COVID-19、肺混浊和病毒性肺炎）中的表现。深度学习方法（如CNN和ViT）直接从图像学习，而放射组学方法提取定量特征，在数据有限时可能更具优势。研究比较了决策树、梯度提升、随机森林、SVM、MLP等放射组学模型与最先进计算机视觉架构的性能。结果显示不同模型在不同样本量下的诊断准确性和鲁棒性差异，为临床实践中AI工具的选择提供了依据，特别适用于需要快速可靠诊断的高通量自动化环境。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T00:01:55Z
- **目录日期**: 2025-04-18
