...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制处理长文本时计算成本高的问题。NSA通过动态分层稀疏策略结合算法与硬件优化，实现了高效的长文本建模。其核心创新包括：1）粗粒度令牌压缩保留全局上下文感知；2）细粒度令牌选择确保局部精度。该方法在保持模型性能的同时显著降低了计算复杂度，为大规模语言模型的高效训练提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T06:04:41Z
- **目录日期**: 2025-04-18
