...
在差分隐私的序贯学习框架中，代理通过内生噪声保护隐私。研究发现，连续信号与二元信号的学习效果存在显著差异：对于连续信号，采用基于阈值距离的自适应噪声机制（平滑随机响应）能使行动更准确反映私有信号和历史观察，将渐近学习速度提升至Θε(log(n))，优于非隐私场景的Θ(√log(n))。有限隐私预算ε∈(0,1)还能避免早期代理长期犯错的问题。然而对于二元信号，隐私噪声会阻碍学习，因代理在信息级联形成前倾向于使用固定随机响应策略，降低行动信息量。研究揭示了隐私机制对学习效率的双重影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T20:02:03Z
- **目录日期**: 2025-04-18
