...
在差分隐私的序列学习中，代理通过内生噪声保护隐私。针对连续信号，研究提出了一种基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声方法，能更好融合私有信号和历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（隐私预算有限时），优于非隐私场景的$\Theta(\sqrt{\log(n)})$。有限隐私预算下，首个正确决策的停止时间和错误动作次数均为有限值，避免了非隐私场景中早期代理长期犯错的问题。连续信号下隐私学习更高效，而二元信号则因信息级联前采用恒定随机响应策略，会阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T16:02:15Z
- **目录日期**: 2025-04-18
