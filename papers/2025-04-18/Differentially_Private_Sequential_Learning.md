...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。针对连续信号，本文提出基于阈值距离的自适应平滑随机响应机制，优于传统均匀噪声方法，将渐近学习速度提升至Θₑ(log(n))（非隐私场景为Θ(√log(n))）。有限隐私预算ϵ∈(0,1)时，首个正确决策的停止时间和错误动作次数均为有限值，而完全非隐私场景下二者发散。连续信号在隐私机制下学习更高效，因平滑响应能增强对数似然比；而二元信号则因固定随机响应策略易形成信息级联，降低动作信息量，阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T19:02:00Z
- **目录日期**: 2025-04-18
