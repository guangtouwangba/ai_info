...
在差分隐私的序贯学习中，连续信号和二元信号表现出显著差异。针对连续信号，研究者提出了一种基于阈值距离的自适应噪声机制（平滑随机响应），相比传统均匀噪声方法，能更高效融合私有信号和观测历史，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算下，首个正确决策的停止时间和错误动作次数均保持有限，而传统场景则发散。但对于二元信号，隐私噪声会阻碍学习，因智能体在信息级联形成前倾向于使用固定随机响应策略，降低动作信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T12:02:16Z
- **目录日期**: 2025-04-18
