...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。针对连续信号，研究提出基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声方法，能更有效结合私有信号与历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（隐私预算有限时），优于非隐私场景的$\Theta(\sqrt{\log(n)})$。有限隐私预算还能避免早期代理长期错误决策，使首次正确决策时间和错误次数保持有限。然而，对于二元信号，隐私噪声会阻碍学习，因代理在信息级联前倾向于恒定随机响应策略，降低动作信息量。连续信号在隐私机制下学习效率更高。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T07:02:15Z
- **目录日期**: 2025-04-18
