...
本文提出了一种基于序贯蒙特卡罗采样的贝叶斯不确定性量化框架，用于解决基于代理的传染病传播模型缺乏实时数据同化和不确定性量化的问题。该框架通过将代理模型与贝叶斯建模相结合，能够在计算资源有限的情况下快速生成疾病传播参数的分布估计，为公共卫生决策提供支持。相比传统多次运行取统计值的方法，新方法显著减少了模型误差，提供更真实的可能结果范围，并解决了代理模型无法同化数据的关键限制。研究还展示了该框架在高性能计算系统中的良好扩展性，并通过模拟疫情数据验证了其有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T02:27:53Z
- **目录日期**: 2025-04-18
