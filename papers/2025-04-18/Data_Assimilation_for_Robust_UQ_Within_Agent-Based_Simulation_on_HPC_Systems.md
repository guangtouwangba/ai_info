...
该研究提出了一种基于序贯蒙特卡罗采样的贝叶斯不确定性量化框架，用于解决基于主体的模拟模型(ABMs)缺乏不确定性量化(UQ)和实时数据同化能力的问题。该框架将ABMs与贝叶斯建模相结合，能够在静态或实时数据流条件下，提供疾病传播参数的分布估计，帮助公共卫生机构快速应对新发传染病威胁。该方法降低了模型误差，提供了可能的现实结果范围，同时保持了ABMs的灵活性，并能很好地扩展到高性能计算系统。研究还展示了在模拟疫情爆发中处理静态和流式数据的算法细节与结果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T01:28:57Z
- **目录日期**: 2025-04-18
