...
该研究提出了"睡眠时间计算"方法，通过让大型语言模型在查询前预先处理上下文并预测可能的问题，显著降低了测试时的计算需求。实验表明，该方法在Stateful GSM-Symbolic和Stateful AIME任务上能减少约5倍测试计算量，同时提升准确率13%-18%。通过Multi-Query GSM-Symbolic任务分摊计算成本，单查询平均成本降低2.5倍。分析发现用户查询的可预测性与方法效果正相关。最后，研究还将该方法应用于实际软件工程任务进行验证。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T15:01:03Z
- **目录日期**: 2025-04-18
