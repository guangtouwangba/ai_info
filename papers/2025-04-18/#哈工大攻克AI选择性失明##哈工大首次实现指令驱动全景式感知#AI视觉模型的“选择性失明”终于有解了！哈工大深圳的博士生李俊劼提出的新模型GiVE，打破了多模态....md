...
哈工大深圳博士生李俊劼团队提出GiVE模型，解决了AI视觉"选择性失明"问题。该模型通过AG-Adapter模块实现指令驱动的全景感知，能根据文本指令聚焦图像细节（如角落物体）。创新点包括三个训练目标：图文细节对齐、目标关系提取、潜在目标识别，并构建了含8万图像的多目标指令数据集MOInst。GiVE可作为插件集成现有系统，显著提升图文检索和分类准确率。这项突破为医疗、安防等场景提供新可能，代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T17:03:52Z
- **目录日期**: 2025-04-18
