...
哈工大深圳博士生李俊劼团队提出GiVE模型，通过AG-Adapter模块实现指令驱动的全景式感知，解决了AI视觉模型的"选择性失明"问题。该模型能根据文本指令聚焦图像细节（如角落物体），并设计了三个训练目标优化细节感知。团队构建了含8万图像的多目标指令数据集MOInst进行训练。GiVE可作为插件集成到现有系统，实验显示其显著提升了图文检索和图像分类准确率。该技术有望应用于医疗、安防等领域，推动AI从"粗略看图"向"按需识图"进化。代码和数据集已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T16:03:50Z
- **目录日期**: 2025-04-18
