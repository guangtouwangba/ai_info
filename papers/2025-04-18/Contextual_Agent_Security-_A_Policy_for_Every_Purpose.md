...
评估一个行为的安全性需要考虑其发生的具体情境。对人类而言，这显而易见——比如删除邮件是否恰当，取决于邮件内容、目的（如删除敏感信息或清理垃圾）和邮箱类型（工作或个人）。但计算系统通常只能在有限情境中运作，依赖手动设置策略和用户确认（如应用权限或访问控制列表）来防止危害。然而，随着多功能智能代理（如自动化个人助手）的兴起，其广泛的任务和情境要求重新设计安全机制。本文提出"情境化智能代理安全"（Conseca）框架，旨在生成即时、情境化且可人工验证的安全策略，作为适应新挑战的初步探索。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T22:02:25Z
- **目录日期**: 2025-04-18
