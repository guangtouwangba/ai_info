...
DeepSeek开源了自研的分布式文件系统3FS（Fire-Flyer File System），该系统已用于V3和R1模型训练。3FS在180节点集群中可实现6.6TB/s的聚合读取吞吐量，并利用SSD带宽优势提升性能。系统采用四类节点架构（Client、Meta、Mgmtd、Storage），通过CRAQ协议实现强一致性：写操作通过链式复制从头节点传递到尾节点，确保数据一致后才可读。尽管CRAQ可能在高并发写入时出现延迟，但提供了良好的读取性能。3FS对用户透明，分布式存储的文件仍显示为统一路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T05:03:03Z
- **目录日期**: 2025-04-18
