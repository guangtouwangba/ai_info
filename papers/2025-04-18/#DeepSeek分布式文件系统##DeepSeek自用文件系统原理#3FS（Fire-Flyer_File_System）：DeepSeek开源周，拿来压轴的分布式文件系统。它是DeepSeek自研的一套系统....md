...
DeepSeek 开源了自研的分布式文件系统 3FS（Fire-Flyer File System），该系统被用于 V3 和 R1 训练，能高效利用固态硬盘带宽，在大规模集群中实现高达 6.6TB/s 的聚合读取吞吐量。3FS 采用四类节点（Client、Meta、Mgmtd、Storage）架构，并通过 CRAQ（链式复制）协议保证强一致性：写入数据从头节点传递到尾节点，标记为 Clean 后才可读取。虽然 CRAQ 提供高读性能，但写入延迟可能受最慢节点影响。项目仍在完善中，已开源在 GitHub。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T08:03:37Z
- **目录日期**: 2025-04-18
