...
DeepSeek开源了其自研的分布式文件系统3FS（Fire-Flyer File System），该系统用于V3和R1模型训练，支持大规模集群的高效读写与容错。3FS在180节点集群中实现了6.6TB/s的聚合读取吞吐量，并通过CRAQ协议确保强一致性：数据写入时依次通过头节点、副本节点和尾节点，尾节点确认后标记为可读。其架构包含Client、Meta、Mgmtd和Storage四种角色，用户访问透明，如同操作本地文件。目前项目仍在优化中，适用于高吞吐但写入延迟敏感的场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T03:15:03Z
- **目录日期**: 2025-04-18
