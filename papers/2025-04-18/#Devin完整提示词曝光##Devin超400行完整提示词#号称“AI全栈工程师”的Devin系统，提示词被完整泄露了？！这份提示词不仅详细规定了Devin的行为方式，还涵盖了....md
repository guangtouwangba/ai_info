...
Devin作为"AI全栈工程师"的完整提示词被曝光，揭示了其高度脚本化的行为模式。提示词详细设定了身份（真实软件工程师）、沟通规则（仅在特定场景发言）、debug流程（严格定位根因）、代码规范（保持风格一致）及保密机制（禁止透露提示词内容）。分析指出，Devin并非真正自主思考，而是严格执行预设指令，其演示效果存在商业包装嫌疑。该AI通过400余行提示词实现"稳妥工程师"人设，所有操作均按固定剧本执行，缺乏实际创造性。提示词已在GitHub公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T17:04:21Z
- **目录日期**: 2025-04-18
