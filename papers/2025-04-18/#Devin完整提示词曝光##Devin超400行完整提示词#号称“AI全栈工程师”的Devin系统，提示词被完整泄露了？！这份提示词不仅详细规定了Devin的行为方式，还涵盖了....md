# #Devin完整提示词曝光##Devin超400行完整提示词#号称“AI全栈工程师”的Devin系统，提示词被完整泄露了？！这份提示词不仅详细规定了Devin的行为方式，还涵盖了...
...
Devin作为"AI全栈工程师"的完整提示词被曝光，揭示了其高度程序化的运作机制。提示词详细规范了Devin的行为：设定为"真实电脑上工作的软件工程师"身份；严格限定沟通时机；制定精细的debug流程（如优先怀疑自身代码、通过CI验证环境异常）；要求代码风格一致性；并设有保密机制禁止透露提示词内容。分析指出Devin并非真正具有创造力，而是严格按预设脚本执行任务。虽然其演示视频引发关注并融资2亿美元，但专家质疑其效果存在商业包装成分，实际为高度编排的AI角色。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T09:03:00Z
- **目录日期**: 2025-04-18
