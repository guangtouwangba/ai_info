...
Devin作为"AI全栈工程师"的完整提示词被曝光，揭示其高度编排的行为模式。提示词详细规定了身份设定（真实软件工程师）、沟通时机（仅在特定场景发声）、debug流程（严格遵循测试优先原则）、代码风格（保持一致性）及保密机制（禁止透露提示词内容）。分析指出，Devin并非具备创造力，而是严格按预设脚本执行任务。尽管其演示视频引发热议并获2亿美元融资，但专家质疑其效果存在商业包装成分，实际运作流程高度刻意。提示词泄露证实了该AI系统的行为完全基于精细控制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T13:09:56Z
- **目录日期**: 2025-04-18
