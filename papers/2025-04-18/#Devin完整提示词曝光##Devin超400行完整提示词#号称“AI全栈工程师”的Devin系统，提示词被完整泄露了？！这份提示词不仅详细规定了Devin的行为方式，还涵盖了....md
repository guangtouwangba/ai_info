...
Devin作为"AI全栈工程师"的完整提示词被曝光，揭示其行为被严格规范。提示词设定了Devin作为"真实软件工程师"的人设，详细规定了沟通时机（如环境错误时）、debug流程（需定位根因）、代码风格（保持一致性）等操作准则，强调谨慎执行而非创造性。分析指出Devin实际是按固定剧本运行的AI，演示效果存在商业包装成分。此次泄露显示其"自主性"实为预设流程，并非真正具备程序员创造力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T23:03:48Z
- **目录日期**: 2025-04-18
