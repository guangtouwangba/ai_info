...
当前自监督学习（SSL）研究快速发展，但缺乏理论解释。研究发现，根据可识别性理论（IT），SSL可能符合柏拉图表征假说（PRH），即不同方法会收敛到同一理想表征。然而，现有IT无法完全解释SSL的实际成功。为此，研究者提出"奇异可识别性理论"（SITh），作为涵盖整个SSL流程的扩展理论框架，旨在揭示SSL隐含的数据假设，推动学习更具解释性和泛化性的表征。未来研究需关注三个方向：1）SSL训练动态与收敛性；2）有限样本、批量和数据多样性的影响；3）架构、增强策略和优化器的归纳偏置作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T10:01:41Z
- **目录日期**: 2025-04-18
