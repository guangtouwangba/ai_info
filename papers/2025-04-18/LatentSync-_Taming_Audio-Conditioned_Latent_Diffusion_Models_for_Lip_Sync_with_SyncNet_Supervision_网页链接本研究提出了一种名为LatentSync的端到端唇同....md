...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂音视频关系，并通过时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题。研究还优化了SyncNet训练框架，提升其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)端到端潜在扩散模型；2)TREPA增强时间一致性；3)SyncNet训练改进方案。这些发现对唇同步和音频驱动动画领域具有广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T22:05:04Z
- **目录日期**: 2025-04-18
