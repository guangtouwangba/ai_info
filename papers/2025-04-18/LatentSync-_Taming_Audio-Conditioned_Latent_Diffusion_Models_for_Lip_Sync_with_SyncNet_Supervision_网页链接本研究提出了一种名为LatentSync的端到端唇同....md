...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法。相比传统两阶段方法，它能直接建模音视频关联，利用Stable Diffusion的强大能力。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型提取的时间特征对齐生成帧。研究还解决了SyncNet收敛问题，通过优化架构、超参数和数据预处理显著提升其精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet改进经验可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T15:05:38Z
- **目录日期**: 2025-04-18
