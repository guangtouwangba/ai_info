...
本研究提出LatentSync端到端唇同步框架，利用音频条件的潜在扩散模型直接建模音视频关系，无需中间运动表示。针对扩散模型的时间一致性问题，创新性提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时深入研究了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理方法，显著提升其准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验也可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T09:05:38Z
- **目录日期**: 2025-04-18
