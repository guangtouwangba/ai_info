...
本研究提出LatentSync，一种基于音频条件的潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉关系。针对扩散模型的时间一致性问题，开发了TREPA方法，利用自监督视频模型的时间表示增强生成帧的时序一致性。同时，通过系统研究改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。这些创新不仅提升了唇同步效果，也为相关音频驱动动画方法提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T20:05:22Z
- **目录日期**: 2025-04-18
