...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发场景。同时构建了20B标记的SynCode-Instruct合成指令数据集用于模型微调。实验发现现有基于执行的测试与人类偏好存在差距，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差异显著，凸显了模型对齐人类偏好的重要性。研究为提升代码生成模型的实用价值提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T02:30:56Z
- **目录日期**: 2025-04-18
