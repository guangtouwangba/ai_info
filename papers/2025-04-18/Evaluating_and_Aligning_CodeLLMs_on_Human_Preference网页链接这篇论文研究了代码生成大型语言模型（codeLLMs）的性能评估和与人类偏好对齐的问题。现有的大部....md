...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，聚焦人类偏好对齐问题。研究者构建了包含397个样本的基准数据集(覆盖40类任务和44种语言)，并开发20B标记的合成指令集SynCode-Instruct。实验发现：1)传统执行基准与人类偏好评估存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。该工作为提升代码LLMs的实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T08:05:49Z
- **目录日期**: 2025-04-18
