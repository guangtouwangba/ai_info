...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发需求。同时构建了SynCode-Instruct合成指令数据集(约20B标记)验证大规模微调效果。实验发现：1)传统基于执行的评估与人类偏好存在差距；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著。研究强调了代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T21:05:27Z
- **目录日期**: 2025-04-18
