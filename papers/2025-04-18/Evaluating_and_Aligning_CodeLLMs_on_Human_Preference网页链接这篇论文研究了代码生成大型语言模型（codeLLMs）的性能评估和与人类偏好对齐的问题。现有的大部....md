...
该论文针对代码生成大模型(codeLLMs)提出创新评估框架CodeArena，通过397个涵盖40类编程任务和44种语言的高质量样本，首次系统评估模型输出与人类偏好的对齐程度。研究同时构建20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验揭示：1)传统执行基准与人类偏好存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显。研究强调代码生成模型除正确性外，还需重视开发者体验和实际需求匹配。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T16:05:56Z
- **目录日期**: 2025-04-18
