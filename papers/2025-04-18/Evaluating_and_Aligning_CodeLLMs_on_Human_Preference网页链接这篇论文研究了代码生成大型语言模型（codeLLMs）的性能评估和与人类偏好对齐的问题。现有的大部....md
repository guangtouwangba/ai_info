...
这篇论文针对代码生成大型语言模型（codeLLMs）提出了CodeArena基准测试，强调评估模型输出与人类偏好的匹配度，而不仅是代码正确性。CodeArena包含397个高质量样本，覆盖40个类别和44种编程语言。研究还构建了SynCode-Instruct语料库（近20B标记）验证合成指令微调效果。实验发现执行基准与CodeArena存在性能差异，开源顶级模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显对齐人类偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T01:32:06Z
- **目录日期**: 2025-04-18
