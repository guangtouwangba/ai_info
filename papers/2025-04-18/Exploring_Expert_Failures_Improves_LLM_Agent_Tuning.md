...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)作为智能体的性能。传统拒绝采样微调(RFT)方法仅利用专家成功轨迹，而EEF创新性地从专家失败轨迹中提取有价值的行为(如关键计划和动作)，同时过滤有害行为。实验表明，EEF能解决RFT无法处理的复杂子任务，在WebShop任务中达到62%的胜率(优于RFT的53.6%和GPT-4的35.6%)，并在WebShop和SciWorld任务中创下0.81分和81分的新SOTA记录，显著提升了智能体的探索效率和关键技能获取能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T15:01:23Z
- **目录日期**: 2025-04-18
