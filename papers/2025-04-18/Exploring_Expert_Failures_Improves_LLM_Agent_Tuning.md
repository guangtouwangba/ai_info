...
该研究提出了一种名为"探索专家失败案例"(EEF)的新方法，用于提升大型语言模型(LLM)作为智能体的性能。研究发现，专家模型(如GPT-4)在复杂子任务上失败的轨迹中往往包含有价值的信息(如关键行动和计划)。EEF通过从这些失败轨迹中提取有益行动并整合到训练数据中，同时排除有害行动，显著提高了模型的学习效率。实验表明，EEF在WebShop任务中达到62%的胜率，优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创造了新的最高分记录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T14:01:18Z
- **目录日期**: 2025-04-18
