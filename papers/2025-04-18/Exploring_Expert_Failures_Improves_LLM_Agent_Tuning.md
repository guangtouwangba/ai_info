...
研究发现大型语言模型(LLMs)作为智能体在复杂任务中表现优异，但传统拒绝采样微调(RFT)方法偏向简单场景，导致许多复杂子任务难以解决。研究者提出"探索专家失败"(EEF)方法，通过分析专家失败轨迹中有价值的行动(如关键计划和动作)，将其整合到训练数据中，同时排除有害动作以避免干扰学习。EEF成功解决了部分传统方法无法处理的子任务，在WebShop测试中获得62%胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T10:01:19Z
- **目录日期**: 2025-04-18
