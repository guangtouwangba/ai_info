...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)的代理能力。研究发现，专家模型(如GPT-4)在复杂子任务中失败的轨迹往往包含有价值的指导信息。EEF通过从失败的专家轨迹中提取有益行动(如关键计划和动作)，同时排除有害行动，将这些信息整合到训练数据中。实验表明，EEF成功解决了一些之前无法完成的子任务，在WebShop任务中达到62%的胜率，优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下了新的最先进水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T18:01:28Z
- **目录日期**: 2025-04-18
