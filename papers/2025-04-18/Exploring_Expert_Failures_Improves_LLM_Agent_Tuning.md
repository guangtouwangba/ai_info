...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有益行动来提升大语言模型的代理能力。传统拒绝采样微调(RFT)方法主要依赖成功轨迹，难以解决复杂子任务。EEF从失败轨迹中提取有价值的行为(如关键计划和动作)，同时排除有害行为，显著提高了模型探索效率和关键技能获取。在WebShop任务中，EEF以62%胜率超越RFT(53.6%)和GPT-4(35.6%)，并创下WebShop(>0.81)和SciWorld(>81)的新SOTA表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T08:01:19Z
- **目录日期**: 2025-04-18
