...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)在复杂子任务中的失败轨迹，提取有价值的关键行动和计划指导，同时排除有害动作，将其整合到训练数据中。相比传统的拒绝采样微调(RFT)方法，EEF能更高效地提升智能体探索能力，解决RFT难以处理的分布外任务。实验显示，EEF在WebShop任务中达到62%的胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准上创下超过81分的新SOTA表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T11:01:24Z
- **目录日期**: 2025-04-18
