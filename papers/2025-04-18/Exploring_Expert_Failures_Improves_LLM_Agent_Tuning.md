...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的训练效果。研究发现，专家模型(如GPT-4)在复杂子任务上的失败轨迹往往包含有价值的行动指导。EEF通过从这些失败轨迹中提取有益行动并剔除有害行动，显著提升了智能体的探索效率和关键技能获取。实验表明，EEF在WebShop任务中取得了62%的胜率，优于拒绝采样微调(RFT)的53.6%和GPT-4的35.6%，并在WebShop和SciWorld任务上创造了新的最先进水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T19:01:14Z
- **目录日期**: 2025-04-18
