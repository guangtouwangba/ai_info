...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)方法主要依赖专家成功的轨迹，但会忽略复杂任务。研究发现，专家失败的轨迹中往往包含有价值的指导信息(如计划和关键动作)。EEF通过从失败轨迹中提取有益动作并排除有害动作，有效解决了部分RFT无法处理的复杂子任务。实验表明，EEF在WebShop任务中达到62%的胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创造了新的最优性能记录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T13:06:57Z
- **目录日期**: 2025-04-18
