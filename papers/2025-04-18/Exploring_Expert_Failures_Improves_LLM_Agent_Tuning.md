...
该研究提出"探索专家失败"(EEF)方法，通过分析大型语言模型(LLM)专家(如GPT-4)在复杂子任务中的失败轨迹，提取有价值的行动指导(如关键计划和动作)，同时排除有害动作以避免污染学习过程。相比传统的拒绝采样微调(RFT)方法，EEF能显著提升智能体在困难任务中的表现。实验显示，EEF在WebShop任务中达到62%的胜率，超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下超过81分的新纪录，成为首个突破该指标的方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T21:01:21Z
- **目录日期**: 2025-04-18
