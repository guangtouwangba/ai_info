...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的性能。传统拒绝采样微调(RFT)方法主要依赖专家成功的轨迹，但忽略了失败轨迹中的有价值信息。EEF通过分析专家失败的轨迹，提取其中有用的行动(如计划和关键操作)并排除有害行动，将这些有益信息整合到训练数据中。实验表明，EEF能解决RFT无法处理的复杂子任务，在WebShop任务中达到62%的胜率，超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创造了新的最优成绩。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T09:01:20Z
- **目录日期**: 2025-04-18
