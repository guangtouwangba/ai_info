...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)方法主要依赖专家成功轨迹，但难以处理复杂子任务。研究发现，专家失败轨迹中的有益行动(如计划和关键步骤)能显著提升代理探索效率和关键技能获取。EEF通过筛选专家失败轨迹中的有益行动并排除有害行动，成功解决了部分RFT无法处理的子任务。实验表明，EEF在WebShop任务中达到62%的胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T06:01:19Z
- **目录日期**: 2025-04-18
