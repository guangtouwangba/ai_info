...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI是对带动量随机梯度下降(SGDM)的改进，通过初始化时依据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，它内存用量减半，在Transformer任务、ImageNet分类、LLM预训练和LoRA微调中表现更优。例如，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，同时保持或超越AdamW的性能，且对超参数变化更具鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T05:04:25Z
- **目录日期**: 2025-04-18
