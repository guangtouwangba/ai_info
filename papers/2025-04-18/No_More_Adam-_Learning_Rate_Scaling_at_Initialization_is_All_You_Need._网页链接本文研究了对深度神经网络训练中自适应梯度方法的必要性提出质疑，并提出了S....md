...
该论文提出SGD-SaI方法，质疑深度神经网络训练中自适应梯度方法(如Adam)的必要性。该方法改进带动量的随机梯度下降(SGDM)，通过初始化时依据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题。相比AdamW，SGD-SAi内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，尤其在大模型训练中显著节省内存(GPT-2省5.93GB，Llama2-7B省25.15GB)，且在LoRA微调等任务优于现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-18T15:04:41Z
- **目录日期**: 2025-04-18
