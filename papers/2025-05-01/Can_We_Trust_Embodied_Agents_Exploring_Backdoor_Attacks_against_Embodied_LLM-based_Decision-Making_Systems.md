...
本文提出了首个针对基于大语言模型（LLM）的具身AI决策系统的后门攻击框架（BALD），系统研究了攻击面和触发机制。作者设计了三种攻击方式：词注入、场景操纵和知识注入，针对LLM决策流程的不同环节。实验在自动驾驶和家庭机器人任务中测试了GPT-3.5、LLaMA2和PaLM2等主流模型，结果显示攻击成功率极高（词/知识注入达100%，场景操纵最高90%），且具有隐蔽性。这些攻击仅需有限系统访问权限，并能抵抗现有防御措施，揭示了具身LLM系统的严重安全漏洞，亟需防护措施。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T03:22:05Z
- **目录日期**: 2025-05-01
