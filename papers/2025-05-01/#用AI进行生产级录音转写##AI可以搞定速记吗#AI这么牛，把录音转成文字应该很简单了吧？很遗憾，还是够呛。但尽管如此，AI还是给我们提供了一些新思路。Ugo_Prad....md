...
该文介绍了使用Gemini AI模型进行生产级录音转写的实践探索。短音频转写效果理想，但长音频面临Token限制、输出质量下降等问题。团队采用分段处理方案，通过30秒重叠切割保证内容连贯性，并用LLM合并分段文本。最终方案将音频按10分钟分段，单独合并重叠部分，显著提升了时间戳精度（1小时音频误差仅5-10秒）和输出可靠性。虽然仍需循环调用LLM，但分段策略有效解决了长音频处理的三大核心难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T08:03:29Z
- **目录日期**: 2025-05-01
