...
该文介绍了使用Gemini模型进行生产级录音转写的实践经验。短音频转写效果理想，但长音频面临Token限制、输出质量下降等问题。团队采用分段处理方案，通过30秒重叠切割和LLM合并，解决了时间戳偏移和内容连贯性问题。最终方案将10分钟音频分三段处理，单独合并重叠部分后再重组，显著提升了转写质量和可靠性。虽然仍需循环调用LLM，但实现了1小时音频仅5-10秒时间戳偏移的精度。分段策略有效平衡了处理成本与输出质量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T09:03:32Z
- **目录日期**: 2025-05-01
