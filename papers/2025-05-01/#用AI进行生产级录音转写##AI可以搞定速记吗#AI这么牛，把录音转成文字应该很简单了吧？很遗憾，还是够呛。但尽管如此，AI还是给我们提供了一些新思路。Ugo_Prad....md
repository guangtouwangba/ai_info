...
这篇微博介绍了Ugo Pradère团队使用Gemini模型进行生产级录音转写的实践。短音频转写效果理想，但长音频面临Token限制、输出质量下降等问题。团队采用分段转写方案，通过30秒重叠切割保证内容连贯性，并用LLM合并分段文本，最终实现每小时转写仅5-10秒时间戳偏移的高精度结果。分段处理既提升了质量又降低了成本，但合并过程仍需应对Token限制的挑战。该方案为长音频AI转写提供了可行的技术路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T02:33:30Z
- **目录日期**: 2025-05-01
