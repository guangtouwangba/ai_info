...
该文介绍了使用Gemini模型进行生产级录音转写的实践经验。短音频转写效果理想，但长音频处理遇到令牌限制、输出质量下降等问题。团队采用分段转写方案，通过30秒重叠切割和LLM合并，解决了时间戳偏移问题。最终方案将10分钟音频分为三部分，单独合并重叠段后再重组，显著提升了转写质量和可靠性。虽然仍需循环调用LLM，但实现了每小时转写仅5-10秒的时间戳误差，证明了分段处理长音频的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T03:22:46Z
- **目录日期**: 2025-05-01
