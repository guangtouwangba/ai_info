...
这篇微博介绍了Ugo Pradère团队使用Gemini模型进行生产级录音转写的实践经验。短音频转写效果理想，但长音频处理遇到Token限制、输出质量下降等问题。团队最终采用重叠分段转写方案（30秒重叠），通过LLM合并分段文本，并校正时间戳偏移，实现了高质量转写。该方法解决了长音频处理的可靠性问题，使1小时以上转写的时间戳误差控制在5-10秒内。虽然仍需循环调用LLM，但分段方案显著提升了转写精度和成本效益。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T07:03:24Z
- **目录日期**: 2025-05-01
