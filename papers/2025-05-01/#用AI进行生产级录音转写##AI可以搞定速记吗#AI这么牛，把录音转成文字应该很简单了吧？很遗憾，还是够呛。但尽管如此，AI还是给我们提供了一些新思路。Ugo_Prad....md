...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。短音频转写效果良好，但长音频处理遇到Token限制、输出质量下降等问题。团队采用分段转写方案，通过30秒重叠切割和LLM合并，解决了时间戳偏移和内容连贯性问题。最终方案将10分钟音频分为三段，单独合并重叠部分后重组，显著提升了转写质量和可靠性，使1小时以上转写的最大时间偏移控制在5-10秒。这一方法有效规避了长音频处理的固有缺陷，为AI录音转写提供了实用解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T23:03:13Z
- **目录日期**: 2025-05-01
