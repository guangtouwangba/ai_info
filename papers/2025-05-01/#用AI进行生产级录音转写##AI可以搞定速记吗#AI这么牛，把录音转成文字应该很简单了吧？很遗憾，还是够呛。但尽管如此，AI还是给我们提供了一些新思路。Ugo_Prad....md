...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。团队发现虽然短音频转写效果理想，但长音频处理面临token限制、输出质量下降等问题。通过采用分段转写方案（30秒重叠切割）、LLM合并分段文本、时间戳校正等方法，最终实现了高质量的长音频转写，使1小时音频的时间戳误差控制在5-10秒内。分段处理显著提升了转写可靠性，并通过智能拼接算法完成最终文本重构。这一方案有效解决了大模型在长音频处理中的技术瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T15:03:33Z
- **目录日期**: 2025-05-01
