...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。短音频转写效果良好，但长音频面临Token限制、输出质量下降等问题。团队采用分段处理方案：将音频切割为重叠片段分别转写，再用LLM合并重叠部分，最后拼接完整文本。该方法解决了时间戳偏移问题（1小时音频误差5-10秒），通过"结束分段"时间戳基准和逐句校正实现精准衔接。最终方案显著提升了长音频转写的可靠性和准确性，但处理流程仍需多次调用LLM并优化分段策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T21:03:24Z
- **目录日期**: 2025-05-01
