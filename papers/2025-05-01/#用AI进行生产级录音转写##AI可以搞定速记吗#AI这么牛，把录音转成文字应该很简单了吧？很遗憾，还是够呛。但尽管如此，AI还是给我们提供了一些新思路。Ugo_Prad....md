...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。短音频转写效果理想，但长音频面临Token限制、输出质量下降等问题。团队采用分段转写方案，通过30秒重叠切割和LLM合并，解决了时间戳偏移和内容连贯性问题。最终方案将音频分段转写后单独合并，再重组完整文本，显著提升了转写精度（1小时音频最大偏移5-10秒）和可靠性，避免了循环调用导致的错误。这一方法为AI音频转写提供了可扩展的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T20:03:21Z
- **目录日期**: 2025-05-01
