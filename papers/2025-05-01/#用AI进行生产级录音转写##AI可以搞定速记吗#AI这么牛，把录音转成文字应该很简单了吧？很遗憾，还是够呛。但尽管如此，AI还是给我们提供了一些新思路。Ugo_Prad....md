...
该文介绍了使用Gemini模型进行生产级录音转写的实践探索。团队发现长音频转写面临Token限制、输出质量下降等问题，于是采用分段处理方案：将音频切割为重叠片段（30秒重叠），分别转写后通过LLM合并。这种方法显著提升了时间戳精度（1小时音频误差仅5-10秒），并通过分段重组实现完整转写。虽然仍需循环调用LLM，但分段策略有效解决了长文本合并的连贯性问题，最终构建出可扩展的精准转写流程。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T01:30:25Z
- **目录日期**: 2025-05-01
