...
本文提出GIT-STORM模型，通过将传统MLP先验替换为掩码生成先验（如MaskGIT），改进了基于Transformer的高效随机世界模型（STORM）。该模型在Atari 100k基准测试中显著提升了强化学习性能，并首次将Transformer世界模型应用于连续动作环境。通过状态混合函数整合潜在状态表征与动作，模型能够处理连续控制任务，在DeepMind Control Suite上验证了有效性。结果表明，掩码生成先验具有多功能性和高效性，为构建更精确的世界模型和强化学习策略开辟了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T04:04:50Z
- **目录日期**: 2025-05-01
