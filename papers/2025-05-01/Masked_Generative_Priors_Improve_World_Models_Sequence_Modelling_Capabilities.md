...
深度强化学习（RL）已成为复杂环境中构建智能体的主流方法。基于模型的RL方法通过世界模型预测环境动态，显著提升数据效率，推动研究向实际应用转化。本文提出GIT-STORM模型，用掩码生成先验（如MaskGIT）替代传统MLP先验，在Atari 100k基准测试中表现出色。首次将基于Transformer的世界模型应用于连续动作环境，通过状态混合函数整合潜在状态与动作，在DeepMind Control Suite中验证了有效性。研究证明了掩码生成先验的通用性，为构建更精确的世界模型和高效RL策略开辟了新途径。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T13:08:42Z
- **目录日期**: 2025-05-01
