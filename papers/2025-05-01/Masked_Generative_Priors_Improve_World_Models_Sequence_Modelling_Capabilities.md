...
本文提出GIT-STORM模型，将掩码生成先验（MaskGIT Prior）引入基于Transformer的世界模型架构（STORM），取代传统MLP先验。该模型在Atari 100k基准测试中显著提升了强化学习任务的性能，并首次将基于Transformer的世界模型成功应用于连续动作环境。通过状态混合函数整合潜在状态表征与动作，模型能有效处理连续控制任务，在DeepMind Control Suite上验证了其有效性。研究表明，掩码生成动力学先验具有多功能性和高效性，为构建更精确的世界模型和强化学习策略开辟了新途径，推动了研究向实际部署的转化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T06:01:53Z
- **目录日期**: 2025-05-01
