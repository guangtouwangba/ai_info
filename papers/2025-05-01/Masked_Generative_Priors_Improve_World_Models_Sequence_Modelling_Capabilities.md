...
本文提出了GIT-STORM模型，通过将传统MLP先验替换为掩码生成先验（如MaskGIT），改进了基于Transformer的世界模型架构。该模型在Atari 100k基准测试中显著提升了强化学习任务的性能，并首次将Transformer世界模型应用于连续动作环境。通过状态混合函数整合潜在状态表征与动作，模型成功处理了连续控制任务，在DeepMind Control Suite上验证了有效性。研究表明，掩码生成动态先验具有多功能性和高效性，为构建更精确的世界模型和有效强化学习策略开辟了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T08:01:59Z
- **目录日期**: 2025-05-01
