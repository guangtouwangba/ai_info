...
这篇论文针对代码生成大语言模型(codeLLMs)提出两个关键贡献：1) 创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建了20B标记的SynCode-Instruct合成指令数据集，验证了大规模指令微调的有效性。实验发现，传统执行基准与人类偏好评估存在差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面仍有明显差距，凸显了该研究方向的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T11:05:47Z
- **目录日期**: 2025-05-01
