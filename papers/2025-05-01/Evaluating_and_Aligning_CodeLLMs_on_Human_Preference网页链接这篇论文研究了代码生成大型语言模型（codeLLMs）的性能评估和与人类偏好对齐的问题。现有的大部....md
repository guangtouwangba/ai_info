...
这篇论文针对代码生成大模型(codeLLMs)与人类偏好对齐的问题，提出了CodeArena基准测试(包含397个高质量样本，覆盖40类任务和44种语言)和SynCode-Instruct指令数据集(近200亿标记)。研究发现现有基于执行的评测与人类偏好存在差异，实验显示开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐方面存在显著差距。研究突出了代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T06:05:19Z
- **目录日期**: 2025-05-01
