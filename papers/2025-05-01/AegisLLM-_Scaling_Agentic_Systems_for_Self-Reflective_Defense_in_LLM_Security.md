...
AegisLLM是一个多智能体协作防御系统，用于对抗对抗性攻击和信息泄露。它通过四个自治智能体（协调器、偏转器、响应器和评估器）的协同工作，确保大语言模型输出的安全合规，并通过即时优化实现自我改进。该系统在测试时扩展智能体推理，显著提升鲁棒性而不影响模型效用，无需重新训练即可实时适应攻击。在WMDP遗忘基准测试中，仅用20个训练样本和不到300次调用就实现近乎完美的遗忘；在越狱测试中，性能提升51%，误拒率仅为7.9%。AegisLLM展现了优于静态防御的适应性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T01:29:32Z
- **目录日期**: 2025-05-01
