...
该研究提出SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法的必要性。该方法改进SGDM，通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可防止训练不平衡，并将优化器内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或媲美AdamW，解决了SGD训练Transformer的难题，且在LoRA微调中超越现有优化器。内存效率方面，相比AdamW节省显著（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T05:04:06Z
- **目录日期**: 2025-05-01
