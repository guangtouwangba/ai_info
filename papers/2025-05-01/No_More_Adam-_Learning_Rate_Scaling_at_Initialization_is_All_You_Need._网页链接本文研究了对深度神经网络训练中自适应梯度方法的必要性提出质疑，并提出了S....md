...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的必要性。SGD-SaI基于带动量的随机梯度下降（SGDM），通过在初始化时根据各参数组的梯度信噪比（g-SNR）调整学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，该方法内存效率提升50%，在Transformer、ImageNet分类和LLM（如GPT-2、Llama2-7B）预训练中表现相当或更优，并显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB），同时展现超参数鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T10:04:24Z
- **目录日期**: 2025-05-01
