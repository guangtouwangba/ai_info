...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时依据梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题。相比AdamW，SGD-SaI内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，甚至超越现有最优优化器。特别在内存效率上，为GPT-2节省5.93GB，为Llama2-7B节省25.15GB内存。该方法还展现出超参数鲁棒性和广泛适用性，包括在LoRA微调中的优势表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T14:04:26Z
- **目录日期**: 2025-05-01
