...
本研究提出LatentSync，一种端到端唇同步框架，基于音频条件的潜在扩散模型，直接建模音频-视觉关联而无需中间运动表示。针对扩散模型的时间一致性问题，提出了时间表示对齐（TREPA）方法，利用自监督视频模型增强生成帧的时序连贯性。同时改进了SyncNet训练框架，通过优化架构、超参数和数据预处理提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动的人像动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T13:12:22Z
- **目录日期**: 2025-05-01
