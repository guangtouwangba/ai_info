...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强帧间一致性。同时，论文系统研究了SyncNet收敛问题，优化了模型架构、训练参数和数据预处理方法，显著提升了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可推广至其他依赖SyncNet的唇同步方法中。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T19:04:49Z
- **目录日期**: 2025-05-01
