...
Prisma是一款专注于视觉Transformer模型内部机制分析的开源可视化工具。它能像显微镜一样观察、缓存和干预模型内部激活状态，支持ViViT等视频模型的时间维度分析。工具提供注意力热图、特征重建等可视化功能，以及activation patching等分析手段。研究发现：1)视觉模型激活特征比语言模型更密集；2)特征重建可能具有降噪效果；3)ViT中CLS token与patch token行为差异显著。该工具为视觉模型的"可解释性"研究提供了新方法，已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T02:34:21Z
- **目录日期**: 2025-05-01
