...
PathFinder是一种新型无地图导航系统，专为视障人士（BLV）设计。它结合视觉语言模型（VLM）、大语言模型（LLM）和单目深度估计技术，通过深度优先搜索（DFS）算法识别最长无障碍路径，在实时性、精度和计算效率之间取得平衡。测试显示，PathFinder在户外导航中降低了平均绝对误差（MAE），决策速度优于现有AI方案。73%的用户能在1分钟内掌握操作，80%认可其准确性、响应速度和便捷性，但在复杂室内和弱光环境下仍需改进。研究突显了该系统在动态环境中的实用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T00:01:36Z
- **目录日期**: 2025-05-01
