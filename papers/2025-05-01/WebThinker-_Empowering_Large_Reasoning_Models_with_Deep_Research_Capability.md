...
WebThinker是一种增强大型推理模型（LRM）能力的深度研究代理，通过自主网络搜索、页面导航和实时报告撰写来解决静态知识限制问题。它包含深度网络探索模块，动态填补知识空白，并采用自主"思考-搜索-撰写"策略，实现推理、信息收集和报告写作的无缝结合。通过基于强化学习的迭代在线直接偏好优化（DPO）训练策略，进一步提升了研究工具利用率。在复杂推理基准测试（GPQA、GAIA等）和科学报告生成任务中，WebThinker显著优于现有方法和商业系统，提高了LRM在复杂场景下的可靠性和适用性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T19:01:36Z
- **目录日期**: 2025-05-01
