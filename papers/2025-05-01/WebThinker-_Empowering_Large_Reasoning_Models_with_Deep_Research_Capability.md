...
WebThinker是一种增强大型推理模型（LRM）能力的深度研究代理，通过动态网络搜索和实时信息整合解决静态知识限制问题。它包含深度网络探索模块和自主“思考-搜索-起草”策略，使模型能在推理过程中无缝切换网络搜索与报告撰写。此外，采用基于强化学习的迭代在线直接偏好优化（DPO）训练策略提升工具使用能力。实验表明，WebThinker在复杂推理基准（GPQA、GAIA等）和科研报告生成任务上显著优于现有方法及商业系统，提高了LRM在复杂场景下的可靠性和适用性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T22:01:47Z
- **目录日期**: 2025-05-01
