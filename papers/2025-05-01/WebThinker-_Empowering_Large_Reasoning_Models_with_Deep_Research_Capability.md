...
WebThinker是一种增强大型推理模型（LRM）能力的深度研究代理，通过动态网络搜索和实时报告撰写解决静态知识局限问题。它包含深度网络探索模块，允许模型在遇到知识缺口时自主搜索、导航和提取网络信息，并采用自主思考-搜索-草拟策略，无缝结合推理、信息收集和报告撰写。通过基于强化学习的迭代在线直接偏好优化（DPO）训练策略，进一步优化研究工具使用。实验表明，WebThinker在复杂推理基准（GPQA、GAIA等）和科学报告生成任务（Glaive）上显著优于现有方法和专有系统，提升了LRM在复杂场景中的可靠性和适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T13:08:29Z
- **目录日期**: 2025-05-01
