...
该研究提出**WebThinker**，一种增强大型推理模型（LRM）动态知识获取能力的深度研究代理。通过**深度网络探索模块**，模型能在推理过程中自主搜索、导航网页并填补知识缺口，结合**“思考-搜索-撰写”策略**实时整合推理、信息收集与报告生成。采用基于强化学习的**在线直接偏好优化（DPO）**训练策略提升工具使用效率。实验表明，WebThinker在复杂推理基准（GPQA、GAIA等）和科研报告生成任务（Glaive）上显著优于现有方法及商业系统，为复杂场景下的可靠研究系统提供了新思路。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T16:02:02Z
- **目录日期**: 2025-05-01
