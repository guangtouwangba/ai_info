...
该研究提出两种新型自动编码器方案（VO-MASD-3D和VO-MASD-Hier），首次解决了多智能体任务中自动提取子群协调模式的关键挑战。通过动态分组函数自动检测智能体交互形成的潜在子群，同时捕获子群和时间层面的抽象特征，形成可迁移的多智能体技能。该方法可应用于离线多任务数据，发现的子群技能无需重新训练即可跨任务迁移。在《星际争霸》任务中的实验表明，其显著优于现有分层多智能体强化学习方法，并能有效缓解延迟稀疏奖励场景下的学习难度。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T14:02:12Z
- **目录日期**: 2025-05-01
