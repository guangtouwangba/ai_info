...
这篇论文提出了一种学习新型4D具身世界模型的有效方法，通过训练RGB-DN（RGB、深度和法线）视频来预测3D场景随时间变化的动态演化，确保时空一致性。该方法超越了传统2D模型，将形状、配置和时间变化纳入预测，并能准确学习具身智能体的逆向动态模型。具体步骤包括：利用现成模型扩展机器人操作视频数据集；微调视频生成模型以联合预测每帧的RGB-DN；开发算法将生成的视频直接转换为高质量4D场景。该方法保证了4D场景预测的时空连贯性，支持新视角合成，并显著提升了基于视频世界模型的策略学习效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T00:01:15Z
- **目录日期**: 2025-05-01
