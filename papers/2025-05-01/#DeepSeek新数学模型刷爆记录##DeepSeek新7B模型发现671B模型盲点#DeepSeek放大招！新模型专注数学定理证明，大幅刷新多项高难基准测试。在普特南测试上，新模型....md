...
DeepSeek发布新数学模型DeepSeek-Prover-V2，在普特南测试中以49道题刷新纪录，远超当前最佳模型（10道）。值得注意的是，7B参数的小模型通过强化学习发现了671B大模型的盲点，在非CoT模式下解决了13个大模型无法处理的问题，尤其在有限基数问题中展现出独特推理模式。该模型基于V1.5-Base微调后，通过大模型收集的数据进行强化学习，最终获得超越大模型的新能力。这一突破展示了小模型通过特定训练方法实现超越规模限制的可能性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T18:02:38Z
- **目录日期**: 2025-05-01
