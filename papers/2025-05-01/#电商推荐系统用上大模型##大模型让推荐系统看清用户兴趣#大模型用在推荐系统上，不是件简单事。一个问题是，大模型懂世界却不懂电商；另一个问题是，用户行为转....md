...
阿里妈妈推出URM世界知识大模型，解决电商推荐系统两大难题：1）大模型缺乏垂直领域知识；2）用户行为数据冗长难处理。该模型融合通用大语言模型能力与电商专业知识，能深度推理用户兴趣，结合语境实时生成精准推荐。例如识别"儿童新年衣服"搜索隐含的宝妈身份和节日需求，优先推荐相关商品而非用户近期浏览记录。这一技术实现了对用户意图的细粒度理解，提升电商推荐效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-01T01:31:35Z
- **目录日期**: 2025-05-01
