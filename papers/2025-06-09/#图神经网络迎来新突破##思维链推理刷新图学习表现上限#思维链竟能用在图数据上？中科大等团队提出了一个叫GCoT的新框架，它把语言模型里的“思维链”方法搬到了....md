...
中科大团队提出GCoT框架，首次将语言模型的思维链推理应用于无文本的图数据。该方法通过分步处理：先由预训练模型初步推断，再融合多层隐藏表示生成中间"思维"，最后用该思维生成节点专属提示进行下一步推理。在8个数据集测试中，GCoT仅用1-5个样本就超越现有最优模型，显著提升了图学习表现。这一突破解决了传统方法在图数据上因结构复杂和缺乏文本而效果不佳的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T03:29:07Z
- **目录日期**: 2025-06-09
