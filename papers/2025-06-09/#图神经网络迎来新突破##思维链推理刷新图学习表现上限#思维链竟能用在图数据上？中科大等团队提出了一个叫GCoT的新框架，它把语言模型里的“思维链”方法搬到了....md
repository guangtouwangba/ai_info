...
中科大团队提出GCoT框架，首次将语言模型的"思维链"方法应用于无文本的图数据。该框架通过三步推理：1）预训练模型初步推断；2）融合多层隐藏表示生成中间"思维"；3）利用思维生成节点专属提示进行下一步推理。在8个数据集测试中，GCoT仅用1-5个样本就超越现有最优模型，刷新了图学习表现上限。这一突破解决了传统方法在图数据（结构复杂、缺乏文本）上难以有效应用的难题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T06:02:37Z
- **目录日期**: 2025-06-09
