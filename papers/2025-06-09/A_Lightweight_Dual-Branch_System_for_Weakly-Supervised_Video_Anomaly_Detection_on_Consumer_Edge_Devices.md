...
本文提出RuleVAD，一种轻量级视频异常检测系统，用于解决智能安防在消费电子设备上部署的算力限制问题。该系统采用解耦双分支架构：隐式分支通过视觉特征快速二分类过滤正常活动；显式分支结合YOLO-World目标检测和数据挖掘生成可解释的文本关联规则，实现细粒度分类，降低误报率。在XD-Violence和UCF-Crime数据集测试中，其性能优于现有方法，且可部署于NVIDIA Jetson Nano等低功耗设备，为消费电子产品提供实时高效的安防监控方案。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T16:02:18Z
- **目录日期**: 2025-06-09
