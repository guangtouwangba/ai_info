...
本文提出"心理图像检索"(MIR)任务，研究用户通过多轮交互搜索心中所想图像的现实场景。针对现有方法依赖抽象语言反馈的局限，作者开发了GenIR生成式检索框架，利用扩散模型在每轮交互中生成可视化反馈，帮助用户直观调整查询。该方法显著优于现有交互式检索技术。研究还建立了自动化流程生成高质量多轮MIR数据集。这项工作开创性地定义了新任务，提供了数据集和有效的生成式检索方法，为未来研究奠定了基础。实验证明GenIR能有效提升用户通过多轮交互精准定位心理图像的能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T20:01:31Z
- **目录日期**: 2025-06-09
