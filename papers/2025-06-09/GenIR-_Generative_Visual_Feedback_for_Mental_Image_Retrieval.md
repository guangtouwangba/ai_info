...
本文提出"心理图像检索"(MIR)任务，研究用户通过多轮交互从心理图像中检索目标图像的现实场景。针对现有方法依赖抽象语言反馈的局限，作者开发了GenIR生成式检索框架，利用扩散模型在每轮交互中生成可视化反馈，帮助用户直观调整查询。研究还建立了自动化生成高质量多轮MIR数据集的方法。实验表明，GenIR显著优于现有交互式检索方法。该工作通过定义新任务、构建数据集和开发有效方法，为心理图像检索领域奠定了基础。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T05:02:05Z
- **目录日期**: 2025-06-09
