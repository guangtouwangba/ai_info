...
PersonaAgent是一个创新的个性化大语言模型（LLM）代理框架，通过整合个性化记忆模块（情景与语义记忆）和个性化行动模块（定制化工具操作），动态适应用户需求。其核心是用户专属的系统提示（persona），协调记忆与行动的交互：记忆指导行动，行动结果反过来优化记忆。该框架采用测试时用户偏好对齐策略，通过模拟最近n次交互并基于文本损失反馈优化persona提示，实现实时偏好校准。实验表明，PersonaAgent在个性化行动空间和实际应用扩展性上显著优于基线方法，展现了动态定制用户体验的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T20:00:54Z
- **目录日期**: 2025-06-09
