...
PersonaAgent是首个个性化LLM智能体框架，通过个性化记忆模块（情景与语义记忆）和个性化行动模块（定制化工具操作）实现用户需求适配。其核心"角色"系统作为中介，利用记忆信息指导行动，行动结果反过来优化记忆。该框架采用测试时用户偏好对齐策略，通过模拟最近n次交互优化角色提示，利用文本损失反馈实现实时偏好校准。实验表明，PersonaAgent在行动空间个性化及实际应用扩展性方面显著优于基线方法，验证了其提供动态定制用户体验的可行性与潜力。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T03:26:02Z
- **目录日期**: 2025-06-09
