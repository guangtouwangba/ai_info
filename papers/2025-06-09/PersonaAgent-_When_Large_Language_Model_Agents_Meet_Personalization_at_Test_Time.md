...
PersonaAgent是一个创新的个性化大语言模型（LLM）代理框架，旨在解决多样化个性化任务。它包含两个核心模块：个性化记忆模块（情景和语义记忆机制）和个性化行动模块（定制化工具操作）。通过用户专属的"角色"（系统提示）作为中介，该框架动态整合记忆信息指导行动，同时用行动结果优化记忆。测试阶段采用用户偏好对齐策略，通过模拟最近n次交互优化角色提示，利用文本损失反馈实现实时偏好匹配。实验表明，PersonaAgent在个性化行动空间和实际应用扩展性上显著优于基线方法，展现了动态定制用户体验的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T05:01:29Z
- **目录日期**: 2025-06-09
