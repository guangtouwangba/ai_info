...
PersonaAgent是一个创新的个性化LLM代理框架，旨在解决多样化个性化任务。它包含两个核心模块：个性化记忆模块（情景和语义记忆）和个性化行动模块（定制化工具操作）。通过用户专属的"角色"系统提示作为中介，该框架动态协调记忆与行动，形成闭环优化。研究还提出测试时用户偏好对齐策略，通过模拟最近n次交互优化角色提示，利用文本损失反馈实现实时偏好校准。实验表明，PersonaAgent在个性化行动空间和实际应用扩展性上显著优于基线方法，展现了提供动态定制用户体验的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T21:01:08Z
- **目录日期**: 2025-06-09
