...
PersonaAgent是一个创新的个性化LLM代理框架，通过整合个性化记忆模块（情景与语义记忆）和个性化行动模块（定制工具操作）来满足用户多样化需求。其核心是"角色"系统提示，作为中介协调记忆与行动的动态交互：记忆指导行动，行动结果又更新记忆。该框架采用测试时用户偏好对齐策略，通过模拟最近n次交互优化角色提示，利用文本损失反馈实现实时偏好校准。实验表明，PersonaAgent在动作空间个性化及实际应用扩展性上显著优于基线方法，展现了提供动态定制用户体验的可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T13:10:35Z
- **目录日期**: 2025-06-09
