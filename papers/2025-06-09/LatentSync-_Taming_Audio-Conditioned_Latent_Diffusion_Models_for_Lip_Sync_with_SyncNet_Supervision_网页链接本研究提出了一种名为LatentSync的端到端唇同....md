...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案通过时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题，并优化SyncNet训练框架提升准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。创新点包括：直接建模音视频相关性、增强生成连贯性、改进SyncNet收敛性，这些发现可推广至其他唇同步研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T02:39:24Z
- **目录日期**: 2025-06-09
