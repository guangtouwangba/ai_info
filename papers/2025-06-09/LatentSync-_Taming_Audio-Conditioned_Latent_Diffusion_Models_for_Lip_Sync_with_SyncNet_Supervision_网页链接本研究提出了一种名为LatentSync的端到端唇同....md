...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法，利用自监督视频模型增强帧间对齐。同时优化了SyncNet训练框架，显著提升其准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供新方案。创新点包括：1)直接建模音频-视觉关联；2)解决扩散模型时序问题；3)改进SyncNet训练方法。这些改进可推广至其他唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T15:04:17Z
- **目录日期**: 2025-06-09
