...
该研究针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的对齐程度。同时构建近200亿标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现：1)传统执行测试与人类偏好存在偏差；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)存在显著差距。研究强调代码生成质量需兼顾功能正确性和人类偏好，为模型优化提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T15:04:39Z
- **目录日期**: 2025-06-09
