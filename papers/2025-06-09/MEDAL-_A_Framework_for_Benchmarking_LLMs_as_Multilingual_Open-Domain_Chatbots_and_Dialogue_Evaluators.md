...
本文介绍了MEDAL框架，这是一个自动化多智能体系统，用于生成、评估和优化开放域对话评估基准。该框架利用先进的大语言模型（LLM）生成多语言对话，并使用GPT-4.1进行多维性能分析，发现显著的跨语言差异。基于大规模评估结果，研究团队构建了一个新的多语言元评估基准，并进行了细致的人工标注。实验表明，现有LLM在检测细微对话问题（如共情和推理）方面仍有不足。该研究旨在解决当前评估数据集静态、过时且缺乏多语言覆盖的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T19:01:48Z
- **目录日期**: 2025-06-09
