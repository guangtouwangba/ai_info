...
本文介绍了MEDAL框架，一个用于生成、评估和筛选更具代表性和多样性的开放域对话评估基准的多智能体自动化系统。该框架利用多个先进大语言模型（LLM）生成基于多样化种子语境的用户-聊天机器人多语言对话，并采用GPT-4.1进行多维度性能分析，发现显著的跨语言表现差异。通过大规模评估，研究团队构建了一个新的多语言元评估基准，并人工标注了包含细微质量判断的样本。实验表明，现有LLM在检测涉及同理心和推理等细微问题时仍存在困难。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T21:02:06Z
- **目录日期**: 2025-06-09
