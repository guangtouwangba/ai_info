...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时依据梯度信噪比（g-SNR）调整不同参数组的学习率，解决了传统SGD训练不平衡的问题，同时将优化器内存占用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，尤其在大模型（如Llama2-7B）中节省显著内存（最高25.15GB）。该方法对超参数鲁棒，在LoRA微调等场景也优于现有优化器。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T04:09:29Z
- **目录日期**: 2025-06-09
