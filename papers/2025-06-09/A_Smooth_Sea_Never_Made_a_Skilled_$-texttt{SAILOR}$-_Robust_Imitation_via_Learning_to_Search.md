...
行为克隆（BC）方法在模仿学习中的根本局限是只能模仿专家在特定状态下的行为，一旦偏离专家示范的状态，代理往往无法恢复。为此，研究者提出基于专家示范的搜索学习（L2S）方法，通过结合世界模型和奖励模型，使代理能在测试时自主规划恢复行为，而无需额外人工修正。在多个视觉操作任务上，该方法（SAILOR）显著优于当前最优的扩散策略（Diffusion Policies），即使BC使用5-10倍数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T00:01:59Z
- **目录日期**: 2025-06-09
