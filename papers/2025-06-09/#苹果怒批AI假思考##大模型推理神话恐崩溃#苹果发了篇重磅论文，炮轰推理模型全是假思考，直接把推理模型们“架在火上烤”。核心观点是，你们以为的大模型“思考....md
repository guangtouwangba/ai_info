...
苹果最新论文《The Illusion of Thinking》指出当前大语言模型（如DeepSeek、Claude等）的"推理能力"本质是模式匹配而非真正思考。研究通过汉诺塔等4类复杂度可调的谜题测试发现：低难度时基础模型更高效；中等难度思维链（CoT）有效；但高复杂度下所有模型准确率归零，且会主动减少"思考"。论文揭示大模型缺乏泛化逻辑能力，无法可靠执行既定步骤，质疑其"深度思考"宣称。研究团队认为现有推理机制存在根本局限，更多算力无法解决本质问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T01:30:41Z
- **目录日期**: 2025-06-09
