...
苹果发表论文《The Illusion of Thinking》，质疑当前大语言模型的"思考"能力本质是模板匹配而非真正推理。研究通过汉诺塔等4种复杂度可调的谜题测试发现：低难度时简单模型更高效；中等难度思维链机制有效；但高复杂度时所有模型（包括DeepSeek、Claude等）准确率归零。关键发现是模型在复杂任务中会主动"罢工"，暴露出缺乏泛化逻辑能力的本质缺陷，连基础步骤执行都困难，更无法实现深度思考。论文直指当前AI推理存在根本性局限。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T03:28:35Z
- **目录日期**: 2025-06-09
