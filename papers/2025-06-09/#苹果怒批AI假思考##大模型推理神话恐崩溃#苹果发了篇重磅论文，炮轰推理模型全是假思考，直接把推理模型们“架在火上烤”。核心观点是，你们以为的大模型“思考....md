...
苹果发表论文《The Illusion of Thinking》，批评当前大语言模型的推理能力只是"假思考"。研究通过汉诺塔等4种复杂度可调的谜题测试发现：低难度时无推理模型表现更优；中等难度思维链机制有效；但高复杂度下所有模型（包括DeepSeek、Claude等）准确率归零。关键发现是模型在困难时会主动"罢工"，而非资源不足，证明其缺乏真正的逻辑泛化能力，连基础步骤执行都存在问题。论文质疑大模型所谓"思考"本质是高级模板匹配，无法实现深度推理。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T00:02:37Z
- **目录日期**: 2025-06-09
