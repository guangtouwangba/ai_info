...
稀疏自编码器（SAEs）被用于提升视觉语言模型（VLMs，如CLIP）的可解释性和可控性。本研究提出了一种评估视觉表征中神经元单义性的框架，并通过大规模用户研究验证其与人类感知的一致性。实验表明，SAEs能显著增强神经元的单义性，其中稀疏性和宽隐变量是关键影响因素。此外，在CLIP视觉编码器上应用SAE干预可直接引导多模态大语言模型（如LLaVA）的输出，无需修改原模型。这些发现证明了SAEs作为无监督工具在提升VLM可解释性和控制方面的实用性与有效性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T13:11:57Z
- **目录日期**: 2025-06-09
