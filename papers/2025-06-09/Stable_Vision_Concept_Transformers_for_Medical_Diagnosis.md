...
本文提出两种可解释AI模型：视觉概念转换器(VCT)和稳定视觉概念转换器(SVCT)，用于解决医疗领域AI的可解释性问题。VCT通过融合图像特征和概念特征提升模型性能，SVCT在此基础上加入去噪扩散平滑技术，增强模型抗干扰能力。实验表明，这两种模型在四个医疗数据集上保持准确性的同时具备良好的可解释性，SVCT尤其能在输入扰动情况下提供稳定的解释，满足医疗领域对AI模型透明度和可靠性的双重需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T00:02:09Z
- **目录日期**: 2025-06-09
