...
这篇论文提出了DirectLayout框架，利用大语言模型(LLM)的空间推理能力直接从文本描述生成3D室内场景布局。该方法通过三阶段流程：首先生成鸟瞰图布局，然后提升到3D空间，最后优化物体摆放。研究者采用基于3D-Front数据集的思维链(CoT)激活来增强空间推理，并设计了CoT-Grounded生成式布局奖励来提高泛化能力。推理阶段通过上下文学习进行迭代式资产-布局对齐，解决了资源与布局不匹配问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，显著优于现有方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-09T00:01:09Z
- **目录日期**: 2025-06-09
