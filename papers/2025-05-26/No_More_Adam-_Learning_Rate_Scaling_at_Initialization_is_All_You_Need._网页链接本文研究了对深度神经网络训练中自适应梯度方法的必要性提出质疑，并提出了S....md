...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统认知。SGD-SaI通过初始化时基于梯度信号噪声比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类和GPT-2预训练中表现相当或更优，且在LoRA微调中超越现有优化器。内存效率显著，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存，展现出强大的实用性和鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T11:04:46Z
- **目录日期**: 2025-05-26
