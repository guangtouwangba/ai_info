...
这篇LLM入门指南系统性地介绍了大语言模型的核心概念体系：1) 智能体由模型、指令编排和工具集三要素构成；2) 模型分为参数量级不同的大/小语言模型；3) 指令编排包含目标设定、记忆管理（短期/长期）和推理规划（CoT/ReAct）；4) 工具调用通过函数和RAG技术扩展能力；5) 性能增强方法包括上下文学习、检索增强和微调技术；6) LangChain框架为LLM应用开发提供模块化支持。全文构建了从基础概念到实践应用的完整认知框架，适合初学者快速建立知识体系。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T17:03:34Z
- **目录日期**: 2025-05-26
