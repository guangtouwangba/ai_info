...
这篇论文探讨了将大型语言模型（LLMs）发展为具备因果推理能力的生物医学智能体的愿景。当前LLMs主要依赖相关性而非真正的因果理解。研究者提出通过整合多模态数据（文本、图像、基因组等）和干预式推理来构建因果LLM智能体，需解决四大挑战：设计安全可控的智能体框架、建立严格的因果评估基准、融合异构数据源，以及结合结构化知识图谱和形式化因果推理工具。这种智能体有望加速药物发现（通过自动化假设生成）和实现个性化医疗（建立患者特异性因果模型），需要跨学科合作来推动这一研究方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T00:02:04Z
- **目录日期**: 2025-05-26
