...
研究发现，大语言模型(LLM)在处理"大海捞针"任务时面临重大挑战，当关键信息(针)埋藏在大量无关内容(干草堆)中时表现不佳。实验表明，关键信息片段越短，模型性能下降越明显，且会加剧位置敏感性。这一现象在三个不同领域(常识、生物医学和数学推理)和七种最先进的大模型中普遍存在。研究揭示了关键信息长度对模型性能的重要影响，为设计更健壮的上下文感知系统提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T12:01:03Z
- **目录日期**: 2025-05-26
