...
研究发现，大语言模型(LLMs)在处理"大海捞针"任务时面临重大挑战。当需要从大量无关信息中提取少量关键内容时，关键信息的长度会显著影响模型表现。实验表明，关键信息越短，模型性能下降越明显，且位置敏感性增强。这一现象在三个不同领域(常识、生物医学和数学推理)和七种最先进的LLMs中都普遍存在。研究揭示了关键信息长度对模型性能的重要影响，为设计更稳健的上下文感知LLM系统提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T10:05:05Z
- **目录日期**: 2025-05-26
