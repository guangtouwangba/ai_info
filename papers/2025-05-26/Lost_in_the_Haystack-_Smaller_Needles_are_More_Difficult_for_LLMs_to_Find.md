...
大型语言模型(LLMs)在处理"大海捞针"任务时面临重大挑战，即从大量无关信息中提取关键内容。研究发现，当相关上下文("金内容")较短时，模型性能会急剧下降，且位置敏感性增强。这一现象在三个不同领域(常识、生物医学和数学推理)和七种最先进的LLM模型中均得到验证。研究表明，较小的金内容会持续降低模型性能，这对需要整合分散细粒度信息的智能系统构成主要挑战。该发现为设计稳健的上下文感知LLM系统提供了重要指导。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T20:01:03Z
- **目录日期**: 2025-05-26
