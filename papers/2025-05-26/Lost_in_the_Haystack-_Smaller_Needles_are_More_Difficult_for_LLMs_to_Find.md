...
研究发现，大语言模型(LLM)在处理"大海捞针"任务时面临显著挑战。当需要从大量无关信息中提取少量关键内容时，关键信息的长度会显著影响模型表现：关键信息越短，模型性能下降越明显，且对信息位置更加敏感。这一现象在三个不同领域(常识问答、生物医学推理和数学推理)和七种最先进的大语言模型中都普遍存在。研究揭示了当前LLM在整合分散的细粒度信息方面的重大局限，为设计更健壮的上下文感知系统提供了重要参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T02:30:50Z
- **目录日期**: 2025-05-26
