...
研究发现，大语言模型(LLM)在处理"大海捞针"任务时，当关键信息上下文较短时表现显著下降。实验表明，较短的关键上下文不仅降低模型性能，还会加剧位置敏感性，这对需要整合分散细粒度信息的智能系统构成挑战。该现象在通用知识、生物医学和数学推理三个领域，以及七种不同规模和架构的先进LLM中均存在。研究填补了此前对关键信息长度影响认知的空白，为设计健壮的上下文感知LLM系统提供了重要指导。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T22:00:50Z
- **目录日期**: 2025-05-26
