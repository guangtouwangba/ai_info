...
大型语言模型（LLMs）在"大海捞针"任务中存在显著挑战，即需要从大量无关信息中提取关键内容。研究发现，当关键上下文（"针"）较短时，模型性能会急剧下降，并加剧位置敏感性。这一现象在三个不同领域（常识、生物医学和数学推理）及七种最先进的LLM中均得到验证。研究表明，较小的关键上下文会持续降低模型表现，这对需要整合分散细粒度信息的智能系统构成重大挑战。该发现为设计鲁棒的上下文感知LLM系统提供了重要指导。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T08:02:22Z
- **目录日期**: 2025-05-26
