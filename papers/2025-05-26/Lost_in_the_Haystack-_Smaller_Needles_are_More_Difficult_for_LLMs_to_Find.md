...
研究表明，大语言模型（LLM）在"大海捞针"任务中面临关键挑战：当有效信息（"针"）的上下文较短时，模型性能会显著下降。实验发现，较小的有效上下文会降低模型表现并放大位置敏感性，这一现象在7种不同架构的先进LLM和三个领域（常识、生物医学和数学推理）中均成立。该发现对需要整合零散细粒度信息的智能系统构成重大挑战，为设计鲁棒的上下文感知LLM系统提供了重要参考。先前研究多关注位置偏差和干扰信息数量，而有效上下文长度的影响长期被忽视。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T14:01:06Z
- **目录日期**: 2025-05-26
