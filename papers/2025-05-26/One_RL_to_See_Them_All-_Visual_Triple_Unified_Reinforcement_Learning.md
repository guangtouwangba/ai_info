...
该研究提出了V-Triune系统，通过强化学习（RL）统一训练视觉语言模型（VLMs）完成视觉推理和感知任务。系统包含三个核心组件：样本级数据格式化统一输入、验证器级奖励计算提供定制反馈、源级指标监控诊断问题。研究还设计了动态IoU奖励机制，为感知任务提供自适应反馈。基于开源7B和32B模型构建的Orsta模型，在数学、解谜等4类推理任务和定位、检测等4类感知任务上表现优异，在MEGA-Bench Core基准上提升2.1-14.1分。结果表明该方法能有效扩展VLMs的多任务能力。代码和模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T14:01:51Z
- **目录日期**: 2025-05-26
