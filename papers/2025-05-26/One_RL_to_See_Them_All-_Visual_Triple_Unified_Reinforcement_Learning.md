...
V-Triune是一个视觉三重统一强化学习系统，通过整合样本级数据格式化、验证器级奖励计算和源级指标监控，使视觉语言模型（VLM）能在一个训练流程中同时学习视觉推理和感知任务（如目标检测和定位）。该系统创新地引入动态IoU奖励机制，为感知任务提供自适应反馈。基于开源7B和32B骨干模型开发的Orsta模型，在数学、拼图等4类推理任务和定位、计数等4类感知任务上表现全面提升，在MEGA-Bench Core基准上提升幅度达+2.1至+14.1。该方法展示了统一强化学习框架在VLM中的有效性和扩展性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T05:01:52Z
- **目录日期**: 2025-05-26
