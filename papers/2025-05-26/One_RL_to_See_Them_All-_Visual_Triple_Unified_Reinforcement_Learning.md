...
V-Triune是一个视觉三重统一强化学习系统，通过整合视觉推理和感知任务（如目标检测和定位）实现VLMs的联合训练。该系统包含三个核心组件：样本级数据格式化统一输入、验证器级奖励计算提供定制化反馈，以及源级指标监控诊断数据问题。创新性地提出了动态IoU奖励机制，为感知任务提供自适应反馈。基于开源7B和32B模型的Orsta系统在多样化数据集（含4类推理和4类感知任务）上训练后，在MEGA-Bench Core基准上取得显著提升（+2.1至+14.1），验证了该统一RL框架的有效性和扩展性。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T20:01:46Z
- **目录日期**: 2025-05-26
