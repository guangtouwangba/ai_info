...
本文提出了V-Triune系统，这是一个通过强化学习(RL)统一训练视觉语言模型(VLMs)完成视觉推理和感知任务的框架。该系统包含三个核心组件：样本级数据格式化统一任务输入，验证器级奖励计算提供定制化反馈，以及源级指标监控诊断数据问题。研究者还开发了动态IoU奖励机制，为感知任务提供自适应反馈。基于开源7B和32B模型实现的Orsta模型，在数学、拼图等4类推理任务和定位、检测等4类感知任务上表现优异，在MEGA-Bench Core基准上获得2.1-14.1的性能提升，验证了该统一RL方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T06:01:45Z
- **目录日期**: 2025-05-26
