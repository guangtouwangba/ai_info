...
北航刘偲教授团队提出创新框架"Flying-on-a-Word"(Flow)，实现通过自然语言指令控制无人机飞行。该框架整合语言指令、六自由度状态和第一视角视觉，采用模仿学习方法学习人类飞行员操作策略。团队构建了覆盖5.02平方公里的大规模真实数据集及仿真测试环境，并开发了地面站-无人机协作策略解决机载计算限制问题。真机测试中，Pi-0-UAV模型成功实现语言实时控制，标志着无人机交互方式的重大突破。研究通过成功率(SR)和轨迹质量(NDTW)等指标验证了系统有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T12:04:04Z
- **目录日期**: 2025-05-26
