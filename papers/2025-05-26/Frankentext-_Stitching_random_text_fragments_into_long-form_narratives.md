...
研究人员提出"Frankentexts"——一种由大语言模型在严格约束下（如90%内容需直接复制人类文本）生成长篇叙事的新方法。该方法测试模型在满足写作提示、整合不同文本片段并保持连贯性的可控生成能力。实验显示，Gemini-2.5-Pro表现优异：81%的生成文本连贯且100%符合提示要求，59%甚至被检测器误判为人类写作。人类可通过语气突变和语法不一致识别这类文本。该研究不仅提出了新的生成挑战，还为混合写作检测提供了数据，并探索了人机协作写作的新模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T16:01:45Z
- **目录日期**: 2025-05-26
