...
基于大型语言模型（LLM）的GUI代理可显著改变日常生活，但目前面临高质量训练数据不足的问题。现有方法利用LLM收集轨迹进行模仿学习或提供在线强化学习的奖励信号，但结果奖励模型（ORM）无法提供细粒度反馈，且可能过度惩罚最终失败轨迹中的有价值步骤。为此，本文提出进度奖励模型（ProgRM），通过预测任务完成进度为在线训练提供密集的中间奖励。为解决进度奖励标注难题，设计了基于最长公共子序列（LCS）的自标注算法，自动识别关键步骤并分配进度标签。实验表明，ProgRM训练的表现优于主流LLM和ORM模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T12:02:16Z
- **目录日期**: 2025-05-26
