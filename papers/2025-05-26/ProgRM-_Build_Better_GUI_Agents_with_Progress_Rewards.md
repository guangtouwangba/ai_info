...
基于大语言模型(LLM)的GUI智能体面临高质量训练数据稀缺的问题。现有方法使用结果奖励模型(ORM)，但无法提供细粒度反馈且容易惩罚失败轨迹中的有用步骤。为此，研究者提出进度奖励模型(ProgRM)，通过预测任务完成进度来提供密集的中间奖励。为应对进度标注挑战，设计了基于最长公共子序列(LCS)的自标注算法来自动识别关键步骤并分配进度标签。实验表明，使用ProgRM训练的智能体表现优于主流商业LLM和ORM训练的智能体，验证了该方法的有效性。代码将在论文接受后公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T15:02:02Z
- **目录日期**: 2025-05-26
