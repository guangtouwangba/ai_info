...
当前基于大语言模型（LLM）的GUI智能体因高质量训练数据稀缺而受限，主要源于轨迹收集和奖励标注的困难。现有研究利用LLM生成模仿学习轨迹或提供在线强化学习（RL）的奖励信号，但其结果奖励模型（ORM）无法提供细粒度反馈，且可能过度惩罚最终失败轨迹中的关键步骤。为此，本文提出进度奖励模型（ProgRM），通过预测任务完成进度为在线训练提供密集的中间奖励。为标注进度奖励标签，设计了一种基于最长公共子序列（LCS）的高效自标注算法。实验表明，ProgRM训练的智能体性能优于主流LLM和ORM方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T06:02:08Z
- **目录日期**: 2025-05-26
