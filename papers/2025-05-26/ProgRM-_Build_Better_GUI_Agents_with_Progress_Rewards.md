...
基于大语言模型(LLM)的GUI代理能显著改变日常生活，但高质量训练数据稀缺。现有方法使用LLM收集轨迹进行模仿学习或提供在线强化学习的奖励信号，但结果奖励模型(ORM)无法提供细粒度反馈，且会过度惩罚失败轨迹中的有价值步骤。为此，研究者提出进度奖励模型(ProgRM)，通过预测任务完成进度为在线训练提供密集中间奖励。为应对进度标签标注难题，设计了基于最长公共子序列(LCS)的自标注算法来识别关键步骤并分配进度标签。实验表明，ProgRM训练的代理性能优于主流专有LLM和ORM方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T13:09:52Z
- **目录日期**: 2025-05-26
