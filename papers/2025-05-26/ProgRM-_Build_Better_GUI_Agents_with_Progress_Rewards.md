...
基于大语言模型（LLM）的GUI智能体面临高质量训练数据不足的问题，现有方法依赖结果奖励模型（ORM），但无法提供细粒度反馈且可能惩罚失败轨迹中的有效步骤。为此，研究提出进度奖励模型（ProgRM），通过预测任务完成进度为在线训练提供密集中间奖励。为解决进度标注难题，设计了基于最长公共子序列（LCS）的自标注算法，自动识别轨迹关键步骤并分配进度标签。实验表明，ProgRM训练的智能体性能超越主流私有LLM和ORM基线，验证了其有效性。代码将在论文录用后开源。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T19:01:55Z
- **目录日期**: 2025-05-26
