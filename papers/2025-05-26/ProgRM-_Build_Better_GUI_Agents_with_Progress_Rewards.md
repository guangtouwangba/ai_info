...
基于大语言模型(LLM)的GUI智能体能显著改变日常生活，但当前面临高质量训练数据不足的挑战。现有方法使用结果奖励模型(ORM)，但无法提供细粒度反馈，且会过度惩罚失败轨迹中的有价值步骤。为此，研究者提出进度奖励模型(ProgRM)，通过预测任务完成进度来提供密集的中间奖励。为解决进度标签标注难题，设计了基于最长公共子序列(LCS)的自标注算法来自动识别关键步骤并分配进度标签。实验表明，ProgRM训练的智能体性能优于主流私有LLM和ORM训练模型，验证了其有效性。代码将在论文录用后开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T16:01:56Z
- **目录日期**: 2025-05-26
