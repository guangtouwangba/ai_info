...
这篇论文针对代码生成大模型(codeLLMs)提出两个创新：1)构建CodeArena基准测试(397个样本/44种语言)，首次评估模型输出与人类偏好的匹配度；2)创建20B标记的SynCode-Instruct合成指令集验证微调效果。研究发现：传统基于执行的测试与人类偏好存在显著差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类偏好对齐方面差距明显。研究强调在代码生成领域，除功能性正确外，与开发者实际需求的对齐同样关键。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T11:06:07Z
- **目录日期**: 2025-05-26
