...
Know-The-Ropes (KtR) 提出了一种多智能体框架，通过将领域先验转化为分层算法蓝图来解决单一大语言模型（LLM）的局限性（如有限上下文、角色过载和领域迁移脆弱性）。KtR 将任务递归分解为类型化的子任务，由控制器协调，并采用零样本或轻量级增强（如思维链、微调或自检）解决。实验显示，在背包问题（3-8项）中，3个GPT-4o-mini智能体将准确率从3%提升至95%；在任务分配问题（6-15项）中，6个o3-mini智能体实现100%（10项内）和84%（13-15项）的准确率，远超零样本的11%。该方法通过结构化分解和针对性增强，使轻量模型高效协作，无需依赖庞大单体模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T00:02:17Z
- **目录日期**: 2025-05-26
