...
这篇立场论文主张，负责任人工智能（RAI）指标间的理论不一致性（如公平性定义差异或准确性与隐私的权衡）应被视为优势而非缺陷。作者提出，通过将指标视为不同目标来协调矛盾可带来三大益处：(1)保持道德立场多样性（规范性多元主义）；(2)更完整捕捉多维度伦理概念（认识论完整性）；(3)防止模型对单一指标的过拟合（隐式正则化）。相比强行统一指标导致的价值窄化，应转而研究可接受的不一致阈值，在实践中实现稳健的近似一致性。该观点呼吁重新审视RAI理论与实践框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T15:01:19Z
- **目录日期**: 2025-05-26
