...
研究人员开发了名为OneProt的多模态AI系统，用于整合蛋白质的结构、序列、文本和结合位点数据。该系统基于ImageBind框架，采用图神经网络和Transformer架构，通过轻量级微调方案对齐不同模态的潜在空间，重点关注与序列数据的成对对齐而非完全匹配。OneProt在检索任务中表现优异，并能在酶功能预测和结合位点分析等下游任务中有效应用。该系统还能将专业编码器的表征信息转移到序列编码器，增强进化相关序列的区分能力。结合位点编码器的引入是此前类似模型未采用的重要创新。这一进展为药物发现和蛋白质工程等应用开辟了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T08:04:01Z
- **目录日期**: 2025-05-26
