...
本文介绍了OneProt，一种多模态蛋白质AI模型，整合了结构、序列、文本和结合位点数据。基于ImageBind框架，OneProt采用轻量级微调方案，通过图神经网络和Transformer架构对齐蛋白质模态的潜在空间，重点实现与序列数据的成对对齐而非完全匹配。该模型在检索任务中表现优异，并在酶功能预测和结合位点分析等下游任务中验证了多模态系统的有效性。OneProt还能将专业编码器的表征信息转移到序列编码器，增强进化相关序列的区分能力。研究还发现结合位点编码器对预测性能贡献显著，为药物发现和蛋白质工程开辟了新途径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T07:02:43Z
- **目录日期**: 2025-05-26
