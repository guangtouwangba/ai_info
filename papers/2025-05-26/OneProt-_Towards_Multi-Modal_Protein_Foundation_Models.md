...
研究人员开发了名为OneProt的多模态蛋白质AI系统，整合结构、序列、文本和结合位点数据。基于ImageBind框架，该系统采用轻量级微调方案，通过图神经网络和Transformer架构对齐蛋白质模态编码器的潜在空间。相比传统方法，它仅需成对序列对齐而非完全匹配，在检索任务和下游应用（如酶功能预测）中表现优异。该系统还能将专业编码器的表征信息转移到序列编码器，增强进化相关序列的区分能力。实验表明结合位点编码器对预测性能贡献显著。这项研究拓展了多模态蛋白质模型的应用前景，为药物发现和蛋白质工程开辟了新途径。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T16:02:24Z
- **目录日期**: 2025-05-26
