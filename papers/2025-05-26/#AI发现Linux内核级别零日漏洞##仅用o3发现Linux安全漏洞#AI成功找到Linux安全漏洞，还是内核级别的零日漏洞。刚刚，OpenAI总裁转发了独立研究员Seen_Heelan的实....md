...
AI首次独立发现Linux内核级零日漏洞（CVE-2025-37899），该漏洞位于SMB注销命令处理程序中，属于释放后使用（use-after-free）类型。独立研究员Seen Heelan仅使用o3模型API（无需复杂工具）通过分析12000行代码成功识别该漏洞。OpenAI总裁转发了这一突破性成果，标志着大语言模型首次公开发现此类安全漏洞。该发现展示了AI在代码审计方面的潜力，仅依靠基础模型即可完成传统需要专业工具的安全检测工作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T00:03:14Z
- **目录日期**: 2025-05-26
