...
该研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，提出TREPA方法增强时序对齐。同时优化SyncNet训练框架，改进其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1) 直接建模音频-视觉关系的潜在扩散架构；2) 提升生成连续性的时序对齐技术；3) SyncNet训练方案的改进。这些发现对音频驱动的人像动画具有普适价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T18:05:25Z
- **目录日期**: 2025-05-26
