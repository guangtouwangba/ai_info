...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案利用Stable Diffusion直接建模音视频关联，并针对扩散模型的时间一致性问题提出TREPA方法，通过自监督视频模型对齐生成帧。研究还优化了SyncNet训练框架，改进其收敛性和准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1)首个潜在扩散唇同步方案 2)TREPA时间一致性增强 3)SyncNet训练优化。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-26T20:05:23Z
- **目录日期**: 2025-05-26
