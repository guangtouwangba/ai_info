...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。连续信号下，本文提出的平滑随机响应机制（根据与阈值的距离自适应添加噪声）相比传统均匀噪声方法，能更高效融合私有信号和历史观测，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算$\epsilon \in (0,1)$还能避免早期代理长期错误决策。然而，二元信号下隐私噪声会阻碍学习，因代理在信息级联形成前倾向于使用恒定随机响应策略，降低动作信息量。研究揭示了隐私机制对连续/离散信号学习效率的相反影响。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T21:01:56Z
- **目录日期**: 2025-04-09
