...
在差分隐私的序贯学习框架中，代理通过内生噪声保护隐私。研究发现，连续信号与二元信号的学习效果存在显著差异：对于连续信号，提出的平滑随机响应机制（根据与阈值的距离自适应添加噪声）使学习速度提升至Θε(log(n))，优于非隐私场景的Θ(√log(n))。有限隐私预算ε∈(0,1)还能避免早期代理长期犯错的问题。然而，二元信号在隐私保护下学习效率降低，因为代理在信息级联形成前倾向于使用恒定随机响应策略，削弱了行动信息量。这表明隐私机制对学习效果的影响高度依赖于信号类型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T15:02:54Z
- **目录日期**: 2025-04-09
