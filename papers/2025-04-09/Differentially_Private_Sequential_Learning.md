...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。研究发现连续信号与二元信号的学习效果存在显著差异：对于连续信号，采用基于阈值距离的自适应噪声机制（平滑随机响应）能使行动更准确反映私有信号和历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$），且首个正确决策的预期停止时间和错误行动次数保持有限。而二元信号中，隐私噪声会阻碍学习，代理在信息级联形成前倾向于使用固定随机响应策略，降低行动信息量。隐私预算$\epsilon \in (0,1)$下连续信号的学习效率更高。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T13:10:01Z
- **目录日期**: 2025-04-09
