...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。针对连续信号，本文提出基于阈值的平滑随机响应机制，使动作更准确反映私有信号和历史观测，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（隐私预算有限时），优于非隐私场景的$\Theta(\sqrt{\log(n)})$。有限隐私预算下，首次正确决策的期望停止时间和错误动作次数均有限，避免了非隐私场景中早期代理长期犯错的问题。连续信号下隐私学习更高效，而二元信号因固定随机响应策略易形成信息级联，削弱动作信息量，阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T22:01:52Z
- **目录日期**: 2025-04-09
