...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。对于连续信号，本文提出基于阈值的自适应平滑随机响应机制，使动作更准确反映私有信号和历史观测，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（隐私预算$\epsilon>0$），优于非隐私场景的$\Theta(\sqrt{\log(n)})$。有限隐私预算还能避免早期代理长期错误决策的问题。然而，二元信号下隐私噪声会阻碍学习，因代理在信息级联前倾向于使用固定随机响应策略，降低动作信息量。连续信号场景中隐私学习更高效，而二元信号则受抑制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T12:02:10Z
- **目录日期**: 2025-04-09
