...
本研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法。该框架直接建模音频-视觉关系，无需中间运动表示，利用Stable Diffusion的强大能力。针对扩散模型的时间一致性问题，研究者开发了TREPA方法（时间表示对齐），通过自监督视频模型提取的时间特征来增强生成帧的连贯性。同时，论文系统研究了SyncNet的收敛问题，改进了其训练框架。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些创新对音频驱动的人像动画领域具有广泛适用性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T05:06:26Z
- **目录日期**: 2025-04-09
