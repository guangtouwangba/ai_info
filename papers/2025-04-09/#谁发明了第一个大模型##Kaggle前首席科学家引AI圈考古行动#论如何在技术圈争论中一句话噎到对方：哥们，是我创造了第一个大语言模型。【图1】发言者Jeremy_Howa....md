...
Jeremy Howard（fast.ai创始人、Kaggle前首席科学家）声称自己创造了第一个大语言模型，引发AI圈热议。经考证，其2018年提出的ULMFiT模型采用"无监督预训练-微调"范式，被GPT-1团队承认是重要灵感来源。技术分析显示ULMFiT已具备现代LLM的核心特征（语言建模、自监督训练、任务通用性），被学界称为"现代大模型的最后共同祖先"。虽然未使用Transformer架构，但该研究确实为后续大模型发展奠定了基础，被认为是NLP领域被低估的关键转折点。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T03:16:15Z
- **目录日期**: 2025-04-09
