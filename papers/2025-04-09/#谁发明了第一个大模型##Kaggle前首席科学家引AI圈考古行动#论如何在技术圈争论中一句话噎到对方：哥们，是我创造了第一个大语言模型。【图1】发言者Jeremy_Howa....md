...
这篇微博讨论了AI领域关于"谁发明了第一个大语言模型"的争议。Kaggle前首席科学家Jeremy Howard声称自己创造了第一个大语言模型，指的是他在2018年提出的ULMFiT模型。该模型采用非监督预训练-微调范式，被GPT-1作者承认是其灵感来源之一。考据显示ULMFiT已具备现代大语言模型的关键特征，被认为是所有现代大模型的"共同祖先"。虽然ULMFiT未使用Transformer架构，但它在技术路径上确实开创了大语言模型的发展方向，是一个被低估的重要转折点。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T07:04:23Z
- **目录日期**: 2025-04-09
