...
GenoTEX是一个用于自动化分析基因表达数据的基准数据集，包含基因-性状关联问题的分析代码和结果，涵盖数据集选择、预处理和统计分析流程，并遵循计算基因组学标准。该数据集由生物信息学专家标注以确保准确性。研究者还提出了GenoAgent，一个基于大语言模型（LLM）的多智能体系统，采用多步编程工作流和自我修正机制来协作分析基因表达数据。实验表明LLM方法在基因组数据分析中具有潜力，但也揭示了未来改进的挑战。GenoTEX可作为评估和提升基因表达数据自动化分析方法的资源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T14:02:43Z
- **目录日期**: 2025-04-09
