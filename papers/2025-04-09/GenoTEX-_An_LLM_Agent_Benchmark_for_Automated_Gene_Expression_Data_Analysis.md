...
近期机器学习在基因表达数据疾病相关基因识别方面取得显著进展，但传统方法依赖专家经验且难以扩展。为此，研究者开发了GenoTEX基准数据集，包含符合计算基因组学标准的完整分析流程（数据选择、预处理、统计分析）及专家标注结果。同时提出基于大语言模型的多智能体系统GenoAgent，通过多步骤编程和自校正机制实现自动化分析。实验验证了大语言模型在基因组数据分析中的潜力，也揭示了改进方向。GenoTEX为基因表达分析的自动化方法评估与优化提供了新资源，数据集已在GitHub开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T04:02:41Z
- **目录日期**: 2025-04-09
