...
APIGen-MT是一个两阶段框架，用于生成可验证且多样化的多轮AI智能体交互数据。第一阶段通过LLM评审委员会和迭代反馈创建带真实动作的任务蓝图，第二阶段模拟人机交互生成完整轨迹。基于该数据训练的xLAM-2-fc-r模型系列（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5，其中小模型在多轮场景表现尤为突出，且具有更稳定的跨试验一致性。该方法通过验证的"蓝图到细节"流程生成高质量训练数据，相关模型和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T22:01:31Z
- **目录日期**: 2025-04-09
