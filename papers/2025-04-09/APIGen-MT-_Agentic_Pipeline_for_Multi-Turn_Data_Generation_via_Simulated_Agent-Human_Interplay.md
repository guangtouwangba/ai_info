...
APIGen-MT是一个两阶段框架，用于生成可验证且多样化的多轮AI代理训练数据。第一阶段通过LLM评审委员会和迭代反馈创建详细任务蓝图，第二阶段将其转化为完整交互轨迹。基于该数据训练的xLAM-2-fc-r模型系列（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5，其中较小模型在多轮交互中表现更优且更稳定。该方法通过验证的"蓝图到细节"流程产生高质量数据，推动了更可靠、高效的AI代理发展。相关数据和模型已在HuggingFace开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T16:01:46Z
- **目录日期**: 2025-04-09
