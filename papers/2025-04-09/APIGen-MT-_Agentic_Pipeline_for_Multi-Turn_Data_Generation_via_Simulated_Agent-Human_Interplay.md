...
APIGen-MT是一个两阶段框架，用于生成可验证且多样化的多轮AI代理训练数据。第一阶段通过LLM评审委员会和迭代反馈创建详细任务蓝图；第二阶段将这些蓝图转化为完整的人机交互轨迹。基于此训练的xLAM-2-fc-r系列模型（1B-70B参数）在τ-bench和BFCL基准测试中超越GPT-4o和Claude 3.5，其中小模型在多轮交互中表现尤为出色且保持稳定性。该验证性蓝图到细节的方法能生成高质量训练数据，推动开发更可靠高效的AI代理。模型和数据已在HuggingFace开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T04:01:49Z
- **目录日期**: 2025-04-09
