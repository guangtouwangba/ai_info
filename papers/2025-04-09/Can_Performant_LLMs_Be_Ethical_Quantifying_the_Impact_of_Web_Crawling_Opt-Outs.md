...
这项研究探讨了网络爬虫退出机制对大型语言模型(LLM)性能的影响，提出了"数据合规差距"(DCG)概念来衡量合规与非合规训练数据间的性能差异。实验使用15亿参数模型发现，截至2025年1月，遵守网络数据退出机制基本不影响模型的一般知识获取(DCG接近0%)，但在生物医学等专业领域，排除主要出版商会导致性能下降。结果表明，通用LLM可以使用完全开放数据进行同等训练，而专业领域模型可能在训练后期需要访问高质量版权资源。研究为AI训练实践和政策制定提供了实证依据。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T22:01:04Z
- **目录日期**: 2025-04-09
