...
这项研究探讨了网络爬虫退出机制对大型语言模型(LLM)性能的影响，提出了"数据合规差距"(DCG)概念来衡量合规数据集与非合规数据集训练模型的性能差异。实验使用15亿参数模型发现，截至2025年1月，遵守网络数据退出机制不会显著影响模型的一般知识获取(DCG接近0%)，但在生物医学等专业领域，排除主要出版商会降低模型性能。研究表明，通用LLM可以使用完全开放数据训练，而专业领域模型可能需要在训练后期接入高质量版权数据才能获得更好表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T13:08:36Z
- **目录日期**: 2025-04-09
