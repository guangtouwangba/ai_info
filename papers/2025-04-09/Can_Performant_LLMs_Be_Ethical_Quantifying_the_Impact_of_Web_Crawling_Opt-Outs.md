...
这项研究探讨了网络爬虫退出机制对大型语言模型(LLM)性能的影响，提出了"数据合规差距"(DCG)概念。实验表明，截至2025年1月，遵守网络数据退出机制不会显著影响模型的一般知识获取(DCG接近0%)，但在生物医学等专业领域，排除主要出版商会导致性能下降。研究发现，通用LLM使用完全开放数据训练效果相当，而专业领域模型在训练后期可能需要访问高质量版权内容。该研究为数据合规与模型性能之间的权衡提供了实证依据，对AI训练实践和政策制定具有参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T07:01:10Z
- **目录日期**: 2025-04-09
