...
TxGemma是一套高效、通用的治疗性大语言模型（LLMs），专为药物开发中的属性预测、交互推理和解释性设计。该套件包含2B、9B和27B参数模型，基于Gemma-2微调，覆盖小分子、蛋白质、核酸、疾病和细胞系数据。在66项任务中，TxGemma在64项上表现优于或媲美现有通用模型（45项更优），并在50项上超越专用模型（26项更优）。其微调所需数据更少，适合数据有限场景。此外，TxGemma支持自然语言交互和机理推理。基于Gemini 2.5的Agentic-Tx系统进一步提升了化学与生物学任务表现，显著超越基准模型。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T15:02:19Z
- **目录日期**: 2025-04-09
