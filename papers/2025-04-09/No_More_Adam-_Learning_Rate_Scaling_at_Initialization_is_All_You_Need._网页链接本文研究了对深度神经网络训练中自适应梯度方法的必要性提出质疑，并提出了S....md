...
该研究提出SGD-SaI方法，质疑自适应梯度方法(如Adam)的必要性。该方法改进SGDM，通过初始化时基于梯度信噪比(g-SNR)调整各参数组学习率，无需二阶动量即可避免训练失衡，内存消耗比AdamW减少50%。实验显示，在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优于或持平AdamW，尤其在大模型训练中显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法解决了SGD训练Transformer的难题，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T01:31:41Z
- **目录日期**: 2025-04-09
