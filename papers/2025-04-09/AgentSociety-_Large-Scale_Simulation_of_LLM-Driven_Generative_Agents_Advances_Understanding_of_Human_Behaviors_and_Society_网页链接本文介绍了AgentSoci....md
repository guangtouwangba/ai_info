...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟超过1万个AI代理的社会互动（500万次交互）。研究者用它分析了四大社会问题：社会极化、刺激性信息传播、基本收入政策影响和自然灾害（如飓风）的社会效应。模拟结果与现实世界数据高度吻合，验证了该系统捕捉人类行为机制的能力。该平台可作为计算社会科学的实验工具，辅助传统研究方法（如调查、访谈），帮助研究者和政策制定者理解社会问题的成因与模式，展现了AI在社会模拟领域的重大潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T10:06:03Z
- **目录日期**: 2025-04-09
