...
FEABench是一个评估大语言模型（LLM）和LLM代理通过有限元分析（FEA）模拟和解决物理、数学及工程问题的基准测试。该研究提出了一种综合评估方案，测试LLM能否端到端地处理自然语言问题描述，并操作COMSOL Multiphysics软件进行计算。研究者还设计了一个能通过API与软件交互、检查输出并迭代改进解决方案的智能代理。最佳策略实现了88%的API调用成功率。这种能力将增强LLM的数值求解精度，推动工程自动化发展。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T13:08:25Z
- **目录日期**: 2025-04-09
