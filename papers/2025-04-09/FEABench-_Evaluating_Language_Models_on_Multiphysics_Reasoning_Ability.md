...
FEABench是一个用于评估大语言模型(LLMs)和LLM代理使用有限元分析(FEA)解决物理、数学和工程问题的基准测试。该研究开发了全面的评估方案，测试LLMs通过自然语言理解问题描述并操作COMSOL Multiphysics软件进行端到端求解的能力。研究还设计了一个能通过API与软件交互、检查输出并迭代改进解决方案的智能代理系统，其最佳表现能达到88%的API调用成功率。这种结合LLMs推理能力和数值求解器精度的技术，将推动工程自动化发展。相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T08:01:11Z
- **目录日期**: 2025-04-09
