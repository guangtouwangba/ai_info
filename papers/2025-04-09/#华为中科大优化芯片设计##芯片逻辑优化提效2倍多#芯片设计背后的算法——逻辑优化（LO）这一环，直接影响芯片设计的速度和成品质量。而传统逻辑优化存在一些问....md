...
中科大与华为合作开发的CMO框架创新性地结合图神经网络(GNN)和蒙特卡洛树搜索(MCTS)，通过GESD方案实现芯片逻辑优化。该技术采用"教师-学生"知识蒸馏模式，分离结构/语义特征，优化奖励机制，并应用焦点损失函数处理数据不平衡问题。实验显示在EPFL/IWLS等数据集上，优化效率最高提升2.5倍，电路深度显著降低(如Hyp电路从8259层减至5762层)。该技术已集成至华为EMU工具，支持国产EDA发展，在仅用CPU环境下实现高效优化，摆脱GPU依赖。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-09T23:03:29Z
- **目录日期**: 2025-04-09
