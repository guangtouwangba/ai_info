...
本文介绍了Academy，一种模块化、可扩展的中间件，旨在将自主智能体部署到联邦研究生态系统中，包括高性能计算（HPC）系统、实验设施和数据存储库。Academy支持异步执行、异构资源、高吞吐量数据流和动态资源可用性，满足科学计算需求。它提供了状态智能体表达、智能体间协调管理以及计算与实验控制集成的抽象层。微基准测试显示其在HPC环境中具有高性能和可扩展性。案例研究展示了Academy在材料发现、去中心化学习和信息提取等领域的广泛应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T03:20:47Z
- **目录日期**: 2025-05-11
