...
华为与中科院信工所合作开发了DEER方法，通过动态判断推理临界点（Pearl Reasoning）让大模型提前终止推理。该方法在DeepSeek-R1-Distill-Qwen系列模型（1.5B-32B）测试中，实现准确率提升1.7-5.7%，生成长度减少31-43%。关键技术包括：监测思维转换点、并行评估试验性答案置信度、动态退出机制。实验显示，部分问题仅需20%推理步骤即可保持60.8%准确率。在代码生成任务中，生成长度减半的同时准确率反而提升。该研究突破了传统固定退出策略，实现精度与效率的双重优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T16:02:46Z
- **目录日期**: 2025-05-11
