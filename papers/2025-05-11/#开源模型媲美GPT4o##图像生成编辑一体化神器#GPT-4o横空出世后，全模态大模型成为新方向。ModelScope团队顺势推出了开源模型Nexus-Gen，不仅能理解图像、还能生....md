...
ModelScope团队推出开源多模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer与扩散模型技术，在高维特征空间进行建模，显著提升图像生成质量。采用"预填充自回归"策略解决误差积累问题，统一处理图像理解、生成和编辑三类任务。训练使用25M规模数据，其中编辑任务依赖ImagePulse数据集。模型支持多prompt操作和故事线式编辑，目前论文、代码、模型及数据集均已开源。未来计划在模型规模、图像Token等方面继续优化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T15:02:37Z
- **目录日期**: 2025-05-11
