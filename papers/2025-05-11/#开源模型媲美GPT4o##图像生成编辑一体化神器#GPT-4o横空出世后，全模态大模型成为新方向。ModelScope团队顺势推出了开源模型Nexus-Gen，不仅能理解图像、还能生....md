...
ModelScope团队推出开源多模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer和扩散模型技术，支持图像理解、生成和编辑三大功能，通过"预填充自回归"策略优化图像质量。使用25M规模数据训练，特别依赖ImagePulse数据集处理编辑任务。模型采用模块化训练，支持多提示词输入和故事线编辑，已开源论文、代码、模型及数据集。未来计划在模型规模等方面继续优化。论文和资源链接已公布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T16:02:34Z
- **目录日期**: 2025-05-11
