...
ModelScope团队推出开源多模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer和扩散模型技术，在高维特征空间建模，显著提升图像质量。通过"预填充自回归"策略优化训练一致性，支持图像理解、生成和编辑三大功能。使用25M规模数据训练，特别依赖ImagePulse数据集处理图像编辑任务。模型支持多prompt和故事线编辑，目前论文、代码、模型和数据集均已开源。未来计划在模型规模、图像Token等方面继续优化。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T21:02:38Z
- **目录日期**: 2025-05-11
