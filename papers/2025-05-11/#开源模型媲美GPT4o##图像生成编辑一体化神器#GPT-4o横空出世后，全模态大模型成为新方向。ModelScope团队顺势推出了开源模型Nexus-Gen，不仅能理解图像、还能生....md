...
ModelScope团队推出开源全模态模型Nexus-Gen，性能媲美GPT-4o。该模型结合Transformer和扩散模型技术，实现图像理解、生成和编辑一体化。创新性地采用"预填充自回归"策略解决误差积累问题，并在高维特征空间建模提升图像质量。训练使用25M规模数据，支持多prompt和故事线编辑。模型、代码及ImagePulse数据集均已开源。未来计划在模型规模等方面继续优化，展现出强大的多模态处理能力和应用潜力。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T18:02:24Z
- **目录日期**: 2025-05-11
