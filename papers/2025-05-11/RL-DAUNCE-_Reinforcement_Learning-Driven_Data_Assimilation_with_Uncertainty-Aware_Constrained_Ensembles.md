...
研究人员开发了RL-DAUNCE，一种基于强化学习的数据同化新方法。该方法通过三个创新点提升性能：1）采用类似传统集合成员的智能体结构保持计算效率；2）通过多集合成员推进实现不确定性量化；3）利用"集合即智能体"设计在过程中实施物理约束。研究者采用原始-对偶优化策略动态惩罚奖励函数确保约束满足，并通过限制动作空间保证状态变量边界。在具有强非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波，性能媲美约束型EnKF但计算量大幅降低，尤其在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T16:01:24Z
- **目录日期**: 2025-05-11
