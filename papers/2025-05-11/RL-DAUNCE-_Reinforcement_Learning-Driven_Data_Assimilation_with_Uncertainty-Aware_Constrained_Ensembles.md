...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE。该方法通过三个创新点提升数据同化性能：1) 采用类似传统集合成员结构的智能体设计保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 创新性地在训练过程中实施物理约束。研究者开发了对偶优化策略约束奖励函数，并限制动作空间来确保物理一致性。在模拟马登-朱利安振荡的实验中，RL-DAUNCE显著优于标准集合卡尔曼滤波，其性能与约束集合卡尔曼滤波相当，但计算成本更低，尤其在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T07:01:26Z
- **目录日期**: 2025-05-11
