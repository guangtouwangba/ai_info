...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE，通过三个关键创新提升传统数据同化：1）采用类似集合成员的智能体设计保持计算效率；2）通过多集合成员推进实现不确定性量化；3）利用集合智能体设计在过程中实施物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数，确保约束满足。在具有强非高斯特征的Madden-Julian振荡案例中，RL-DAUNCE显著优于标准集合卡尔曼滤波（EnKF），性能与约束EnKF相当，能更好恢复间歇信号、捕捉极端事件并量化不确定性，同时计算成本大幅降低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T11:01:21Z
- **目录日期**: 2025-05-11
