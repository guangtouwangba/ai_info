...
研究人员开发了一种名为RL-DAUNCE的新型强化学习数据同化方法，该方法通过三个创新点提升性能：1）采用类似传统集合成员的智能体结构保持计算效率；2）通过多集合成员推进实现不确定性量化；3）利用"集合即智能体"设计在过程中实施物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数以确保约束满足。在模拟Madden-Julian振荡（具有强非高斯特征）时，RL-DAUNCE显著优于标准集合卡尔曼滤波，性能媲美约束版集合卡尔曼滤波，但计算量大幅降低，特别在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T06:01:28Z
- **目录日期**: 2025-05-11
