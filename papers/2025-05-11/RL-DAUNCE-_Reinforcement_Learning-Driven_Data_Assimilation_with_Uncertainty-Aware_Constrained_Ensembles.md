...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE。该方法通过三个关键创新：1) 采用类似传统集合同化的"集合即智能体"设计保持计算效率；2) 通过推进多个集合成员实现不确定性量化；3) 采用对偶优化策略和动作空间约束确保物理一致性。在具有强非高斯特征的MJO现象测试中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与计算量更大的约束EnKF相当，能更好地恢复间歇信号、捕捉极端事件和量化不确定性，同时计算量大幅降低。该方法成功解决了传统方法因违反物理约束导致的失效问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T14:01:35Z
- **目录日期**: 2025-05-11
