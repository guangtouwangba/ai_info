...
研究人员提出了一种基于强化学习的数据同化新方法RL-DAUNCE，该方法通过三个创新点提升了传统数据同化：1) 采用类似集合成员的智能体设计保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 利用对偶优化策略和动作空间约束确保物理一致性。在模拟马登-朱利安振荡（具有强非高斯特征）时，RL-DAUNCE显著优于标准集合卡尔曼滤波（EnKF），其性能与计算量更大的约束EnKF相当，能更好地恢复间歇信号、捕捉极端事件并量化不确定性，同时计算成本大幅降低。该方法成功解决了物理约束违反问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T21:01:36Z
- **目录日期**: 2025-05-11
