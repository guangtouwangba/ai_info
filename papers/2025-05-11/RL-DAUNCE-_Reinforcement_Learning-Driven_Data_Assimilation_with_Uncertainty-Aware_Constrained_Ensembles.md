...
研究人员开发了一种基于强化学习的数据同化新方法RL-DAUNCE，通过三个创新点提升性能：(1)采用类似传统集合成员的结构设计智能体，保持机器学习的高效性；(2)通过推进多个集合成员进行不确定性量化，超越简单的均值优化；(3)独特的"集合即智能体"设计可在同化过程中实施物理约束。该方法采用原始-对偶优化策略动态调整奖励函数，确保约束满足。应用于具有强非高斯特征的马登-朱利安振荡现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波，性能与约束型EnKF相当但计算量更少，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现突出。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T13:06:39Z
- **目录日期**: 2025-05-11
