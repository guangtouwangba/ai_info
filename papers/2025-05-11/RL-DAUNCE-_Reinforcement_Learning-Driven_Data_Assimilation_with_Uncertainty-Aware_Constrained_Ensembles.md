...
研究人员开发了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE。该方法通过三个创新点提升数据同化性能：(1)采用类似传统集合成员设计的智能体结构保持计算效率；(2)通过多集合成员推进实现不确定性量化；(3)在训练过程中通过原始-对偶优化策略和动作空间约束确保物理一致性。应用于具有强非高斯特征的Madden-Julian振荡现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，其性能与计算成本更高的约束EnKF相当，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现优异，同时计算量大幅降低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T08:01:36Z
- **目录日期**: 2025-05-11
