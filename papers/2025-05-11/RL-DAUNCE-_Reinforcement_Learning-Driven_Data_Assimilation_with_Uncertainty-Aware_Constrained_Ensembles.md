...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE。该方法通过三个创新点提升数据同化性能：1) 采用类似传统集合数据同化的"集合即智能体"设计；2) 强调不确定性量化而非仅优化平均状态；3) 通过原始-对偶优化策略和动作空间约束确保物理一致性。在具有强非高斯特征的MJO现象测试中，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与计算量更大的约束EnKF相当，能更好恢复间歇信号、捕捉极端事件并量化不确定性，同时保持计算高效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T12:01:27Z
- **目录日期**: 2025-05-11
