...
研究人员开发了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE，通过三个创新点提升传统方法：1)采用类似集合成员的智能体设计保持计算效率；2)通过多集合成员推进实现不确定性量化；3)利用"集合即智能体"设计在过程中实施物理约束。该方法采用原始-对偶优化策略动态惩罚奖励函数确保约束满足。应用于具有强非高斯特征的Madden-Julian振荡现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波(EnKF)，性能与约束EnKF相当但计算量大幅减少，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T04:02:52Z
- **目录日期**: 2025-05-11
