...
本文提出了一种基于强化学习(RL)的新型数据同化方法RL-DAUNCE，通过三个创新点提升传统数据同化：1) 采用类似集合成员的智能体设计保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 采用对偶优化策略和动作空间约束确保物理一致性。该方法应用于具有强非高斯特征的Madden-Julian振荡现象时，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现优异，计算量显著低于传统集合卡尔曼滤波(EnKF)，且避免了物理约束违反导致的灾难性失败，性能与约束EnKF相当。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T18:01:22Z
- **目录日期**: 2025-05-11
