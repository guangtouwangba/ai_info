...
研究人员开发了RL-DAUNCE，一种基于强化学习的数据同化新方法。该方法通过三个关键创新提升性能：1) 采用类似传统集合方法的智能体设计保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 独特的"集合即智能体"设计可在同化过程中实施物理约束。研究采用原始-对偶优化策略动态调整奖励函数，确保约束满足。应用于具有强非高斯特征的Madden-Julian振荡现象时，RL-DAUNCE显著优于标准集合卡尔曼滤波，在恢复间歇信号、捕捉极端事件和量化不确定性方面表现优异，同时计算成本大幅降低。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T23:01:19Z
- **目录日期**: 2025-05-11
