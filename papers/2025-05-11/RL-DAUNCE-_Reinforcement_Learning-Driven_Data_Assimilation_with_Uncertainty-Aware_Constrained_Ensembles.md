...
本文提出了一种基于强化学习的数据同化新方法RL-DAUNCE，通过三个创新点提升数据同化性能：1) 采用类似传统集合成员的智能体结构保持计算效率；2) 通过多集合成员推进实现不确定性量化；3) 设计对偶优化策略和动作空间约束确保物理一致性。该方法应用于具有强非高斯特征的MJO现象时，表现优于标准集合卡尔曼滤波(EnKF)，在恢复间歇信号、捕捉极端事件和量化不确定性方面与约束EnKF相当，但计算量显著降低。RL-DAUNCE成功解决了物理约束违反问题，展示了强化学习在数据同化中的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T10:01:23Z
- **目录日期**: 2025-05-11
