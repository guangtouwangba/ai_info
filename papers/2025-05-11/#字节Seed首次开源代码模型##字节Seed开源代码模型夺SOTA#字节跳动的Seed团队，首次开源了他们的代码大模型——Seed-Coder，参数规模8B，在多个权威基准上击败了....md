...
字节跳动Seed团队首次开源代码大模型Seed-Coder（8B参数），在多项基准测试中超越Qwen3等模型达到SOTA水平。该模型推出三个版本：基础版、指令优化版和推理强化版，其中推理版在IOI 2024表现优异。创新点在于采用"小模型自管数据"范式，通过四类高质量代码数据源（文件/仓库级代码、Git提交、网络内容），结合双重去重、语法检查和LLM评分机制实现自动化数据筛选。模型已完整开源，展示了在代码生成和理解任务上的先进性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T18:02:44Z
- **目录日期**: 2025-05-11
