...
字节跳动Seed团队首次开源8B参数代码大模型Seed-Coder，包含Base、Instruct和Reasoning三个版本，在多项基准测试中超越Qwen3等模型达到SOTA水平。该模型创新性地采用"小模型自管数据"范式，通过四类高质量代码数据源（文件/仓库级代码、Git提交记录、网络代码内容），结合双重去重、语法检查和LLM质量评估进行自动化数据筛选。特别针对不同网络内容定制评分标准，显著减少人工干预。Reasoning版本通过强化学习在IOI 2024竞赛中超越32B参数模型，展现强大推理能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T17:02:54Z
- **目录日期**: 2025-05-11
