...
字节跳动Seed团队首次开源代码大模型Seed-Coder（8B参数），在多项基准测试中超越Qwen3和DeepSeek-R1，达到SOTA水平。该模型包含基础版、指令优化版和推理强化版三个版本，采用"小模型自管数据"的创新范式，通过模型自主生成和筛选训练数据（含文件/仓库级代码、Git提交记录等四类数据源），结合双重去重、语法检查和LLM质量评估。特别设计了针对不同网络内容的定制化评分机制，显著减少人工干预。完整代码和模型已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T23:02:40Z
- **目录日期**: 2025-05-11
