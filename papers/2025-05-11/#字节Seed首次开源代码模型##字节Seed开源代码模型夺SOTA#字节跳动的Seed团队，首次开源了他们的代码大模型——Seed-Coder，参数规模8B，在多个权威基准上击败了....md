...
字节跳动Seed团队首次开源代码大模型Seed-Coder（8B参数），在多个基准测试中超越Qwen3和DeepSeek-R1，达到SOTA水平。模型包含Base、Instruct（强化指令理解）和Reasoning（优化复杂推理）三个版本，其中推理版在IOI 2024表现优于QwQ-32B。创新点在于采用"小模型自管数据"范式，通过四类高质量代码数据源（文件/仓库级代码、Git提交、网络内容）和自动化筛选流程（去重、语法检查、LLM评分），显著减少人工干预。项目已完整开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T21:02:57Z
- **目录日期**: 2025-05-11
