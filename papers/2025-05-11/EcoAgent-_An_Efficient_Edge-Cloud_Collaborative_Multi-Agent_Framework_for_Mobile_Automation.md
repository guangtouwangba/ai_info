...
EcoAgent是一种边缘-云协作的多智能体框架，旨在解决基于云的大型语言模型（LLM）的高延迟和高成本问题，同时弥补边缘部署的小型模型（SLM）在通用能力和复杂任务上的不足。该框架包含云端的规划智能体和边缘的执行与观察智能体，形成闭环协作。观察智能体通过预理解模块压缩屏幕图像为文本，减少令牌消耗；若任务失败，规划智能体通过反思模块重新规划。实验显示，EcoAgent在保持高任务成功率的同时显著降低了令牌消耗，实现了高效的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T18:01:40Z
- **目录日期**: 2025-05-11
