...
EcoAgent是一个边缘-云协作的多智能体框架，旨在解决基于云的大语言模型（LLM）高延迟、高成本的问题，同时避免边缘部署的小模型（SLM）通用能力不足的缺陷。该框架包含云端规划智能体和两个边缘智能体：执行智能体负责操作，观察智能体通过预理解模块压缩屏幕图像为文本以减少token消耗。若任务失败，规划智能体通过反射模块重新规划。实验显示，EcoAgent在保持高任务成功率的同时显著降低了大模型的token消耗，实现了高效实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T17:01:44Z
- **目录日期**: 2025-05-11
