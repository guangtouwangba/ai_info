...
EcoAgent是一个边缘-云协作的多智能体框架，旨在解决基于云的大型语言模型（LLM）在移动自动化中高延迟和高成本的问题。它通过云端规划智能体与两个边缘智能体（执行智能体和观察智能体）的闭环协作实现高效任务处理。观察智能体通过预理解模块压缩屏幕图像为文本以减少令牌消耗，而规划智能体在失败时通过反射模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了令牌使用量，实现了高效且实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T11:01:38Z
- **目录日期**: 2025-05-11
