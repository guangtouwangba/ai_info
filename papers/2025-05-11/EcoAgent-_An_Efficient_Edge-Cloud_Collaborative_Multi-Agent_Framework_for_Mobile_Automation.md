...
EcoAgent是一个边缘-云协作的多智能体框架，旨在提升移动自动化的效率。它通过云端规划智能体和两个边缘智能体（执行智能体和观察智能体）形成闭环协作。观察智能体使用预理解模块压缩屏幕图像为简洁文本，减少令牌消耗；若任务失败，规划智能体通过反射模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时，显著减少了多模态大语言模型（MLLM）的令牌消耗，实现了高效且实用的移动自动化，解决了现有方案延迟高、成本高或能力不足的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T20:01:46Z
- **目录日期**: 2025-05-11
