...
EcoAgent是一个边缘-云协同的多智能体框架，旨在解决基于云的大语言模型（M）LLMs高延迟、高成本的问题，同时克服边缘部署的小模型（M）SLMs通用能力不足的缺陷。该框架包含云端的规划智能体和边缘端的执行与观察智能体，形成闭环协作。观察智能体通过预理解模块压缩屏幕图像为文本，减少令牌消耗；规划智能体在失败时通过反思模块重新规划。实验表明，EcoAgent在保持高任务成功率的同时显著降低了令牌消耗，实现了高效实用的移动自动化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T08:01:54Z
- **目录日期**: 2025-05-11
