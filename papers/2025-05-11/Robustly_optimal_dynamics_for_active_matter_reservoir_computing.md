...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟外部驱动的混沌信号预测模型，发现了一个此前被忽视的优异动力学机制。这一机制在多种物理参数和推断任务中均表现出稳健的最优性，其核心在于系统固有的多阶段弛豫能力。最优计算状态出现在临界阻尼阈值以下，表现为准静态与高度非线性动态的快速切换，且相干与非相干动力学共同作用。该模型从物理角度阐释了非平衡多体系统中的信息处理机制，为非常规计算提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T03:20:58Z
- **目录日期**: 2025-05-11
