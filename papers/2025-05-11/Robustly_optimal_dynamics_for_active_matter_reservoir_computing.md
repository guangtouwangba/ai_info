...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟系统预测混沌信号未来状态。研究发现了一个此前被忽视的优越动力学机制，该机制在不同物理参数和推断任务中均表现稳健且最优。系统信息处理能力主要取决于其固有弛豫特性，在无特定推断目标时即可识别，并在单粒子储层测试中显现。最优计算状态位于临界阻尼阈值以下，涉及多阶段微观动力学弛豫。该系统在混沌外部驱动下具有适应性，表现为准静态与高度非线性动态间的快速切换，相干与非相干动力学共同作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T01:29:42Z
- **目录日期**: 2025-05-11
