...
该研究探讨了活性物质在储层计算（RC）范式中的信息处理能力，通过模拟受外部驱动的模型预测混沌信号。研究发现了一种此前被忽视的优越动力学机制，该机制在不同物理参数和推断任务中均表现出稳健的最优性。系统信息处理能力主要由其内在弛豫特性决定，最优计算状态位于临界阻尼阈值之下，表现为多阶段的微观动力学弛豫。在混沌驱动下，系统通过准静态与高度非线性状态的快速切换实现适应性。相关动力学特征可标识最优性能区间。该模型为理解非平衡多体物理中的学习和非常规计算提供了新视角。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T20:02:05Z
- **目录日期**: 2025-05-11
