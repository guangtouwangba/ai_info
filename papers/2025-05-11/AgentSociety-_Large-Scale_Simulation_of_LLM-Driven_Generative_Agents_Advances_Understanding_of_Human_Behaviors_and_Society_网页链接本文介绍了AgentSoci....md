...
AgentSociety是一个基于大语言模型的大规模社会模拟系统，能够模拟1万多个AI代理的社会互动（500万次交互）。该系统通过四个社会问题案例研究（社会极化、信息传播、基本收入政策、自然灾害影响）验证了其有效性，模拟结果与现实世界数据高度吻合。研究表明，这种LLM驱动的社会模拟器能有效捕捉人类行为模式和社会机制，可作为计算社会科学研究和政策制定的有力工具，弥补传统研究方法（如调查、访谈）的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T10:05:03Z
- **目录日期**: 2025-05-11
