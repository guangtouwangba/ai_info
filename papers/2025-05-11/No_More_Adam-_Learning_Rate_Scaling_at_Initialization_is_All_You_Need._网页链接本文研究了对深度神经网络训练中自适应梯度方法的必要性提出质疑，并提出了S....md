...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的传统观念。该方法基于带动量的随机梯度下降（SGDM），通过初始化时根据梯度信号噪声比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优异，内存效率显著优于AdamW（如为Llama2-7B节省25.15GB内存），同时在LoRA微调等任务中也超越现有优化器，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T10:04:28Z
- **目录日期**: 2025-05-11
