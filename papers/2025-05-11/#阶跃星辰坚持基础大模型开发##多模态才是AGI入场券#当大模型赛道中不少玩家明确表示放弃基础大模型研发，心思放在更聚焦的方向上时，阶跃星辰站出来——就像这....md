...
阶跃星辰坚持基础大模型研发，认为行业技术仍处于快速发展阶段，基础模型决定应用上限。CEO姜大昕表示，尽管部分企业转向聚焦领域，阶跃仍将保持对主流技术趋势的投入，并推动模型与应用的协同发展。其C端产品从"跃问"升级为"阶跃AI"，转型为端云一体Agent平台，目标是通过智能终端服务普通用户。公司认为多模态是AGI的关键方向，同时强调基础大模型的核心地位。产品形态虽动态调整，但"模型驱动应用"的逻辑保持不变。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-11T12:03:01Z
- **目录日期**: 2025-05-11
