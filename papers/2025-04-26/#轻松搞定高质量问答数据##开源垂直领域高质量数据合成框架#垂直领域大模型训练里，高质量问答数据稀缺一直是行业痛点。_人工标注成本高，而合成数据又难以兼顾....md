...
上海AI实验室推出开源框架GraphGen，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架采用"知识图谱引导+双模型协同"机制，能自动生成专业QA数据，无需人工标注。用户只需在OpenXLab上传文本块并配置API Key，即可在线生成适配LLaMA-Factory等训练框架的数据，直接用于大模型SFT训练。目前已上线Web应用支持快速试用，适用于海事等专业领域。项目资源包括GitHub代码库和OpenXLab体验平台，显著降低了垂域模型训练的数据获取门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T13:07:16Z
- **目录日期**: 2025-04-26
