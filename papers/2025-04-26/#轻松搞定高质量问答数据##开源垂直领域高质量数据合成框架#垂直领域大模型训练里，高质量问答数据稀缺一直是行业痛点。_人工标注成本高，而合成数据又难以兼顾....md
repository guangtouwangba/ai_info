...
上海AI实验室推出开源框架GraphGen，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架采用"知识图谱引导+双模型协同"机制，用户只需上传文本块并填写API Key，即可在线生成适配LLaMA-Factory等训练的专业QA数据，无需人工标注。生成的问答数据可直接用于大模型SFT训练，已在OpenXLab上线Web应用供快速试用。项目资源包括GitHub代码库和在线体验平台，显著降低了垂域专业数据的获取门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T15:02:39Z
- **目录日期**: 2025-04-26
