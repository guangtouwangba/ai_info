...
上海AI实验室推出开源框架GraphGen，解决垂直领域大模型训练中高质量问答数据稀缺问题。该框架采用"知识图谱引导+双模型协同"机制，能自动生成专业QA数据，无需人工标注。用户只需在OpenXLab上传文本（如海事知识）和API Key，即可生成适配LLaMA-Factory等框架的训练数据，直接用于模型微调。项目已在GitHub开源，并提供在线体验平台，显著降低垂域模型开发门槛。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T17:02:34Z
- **目录日期**: 2025-04-26
