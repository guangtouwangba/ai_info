...
上海AI实验室推出GraphGen框架，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架采用"知识图谱引导+双模型协同"机制，提升模型对专业领域的理解能力。用户只需在OpenXLab平台上传文本（如海事知识）并填写API密钥，即可在线生成适配LLaMA-Factory等框架的训练数据，且可直接用于大模型SFT训练。项目已开源并提供Web应用体验，显著降低人工标注成本，实现专业QA数据的自动化生成。GitHub和OpenXLab均已提供相关资源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T06:02:44Z
- **目录日期**: 2025-04-26
