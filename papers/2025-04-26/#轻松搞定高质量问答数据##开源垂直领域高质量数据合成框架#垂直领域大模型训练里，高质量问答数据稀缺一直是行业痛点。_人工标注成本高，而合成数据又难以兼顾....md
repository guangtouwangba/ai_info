...
上海AI实验室推出开源框架GraphGen，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架采用"知识图谱引导+双模型协同"机制，提升模型对专业领域的理解能力。用户只需在OpenXLab平台上传文本（如海事知识）并填写API密钥，即可在线生成适配LLaMA-Factory等框架的训练数据，直接用于大模型SFT训练。相比人工标注，该方法显著降低成本，同时保证数据质量。项目已在GitHub开源，并提供Web应用体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T21:02:45Z
- **目录日期**: 2025-04-26
