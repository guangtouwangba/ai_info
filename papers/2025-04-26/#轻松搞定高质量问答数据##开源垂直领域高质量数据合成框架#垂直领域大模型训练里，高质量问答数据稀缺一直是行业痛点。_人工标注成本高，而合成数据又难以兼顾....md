...
上海AI实验室推出开源框架GraphGen，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架通过"知识图谱引导+双模型协同"机制，自动生成专业QA数据，无需人工标注。用户只需在OpenXLab上传领域文本（如海事知识）并配置API，即可生成适配LLaMA-Factory等框架的训练数据，直接用于大模型SFT训练。项目已在GitHub开源并提供在线体验，显著降低垂域数据获取成本。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T08:02:40Z
- **目录日期**: 2025-04-26
