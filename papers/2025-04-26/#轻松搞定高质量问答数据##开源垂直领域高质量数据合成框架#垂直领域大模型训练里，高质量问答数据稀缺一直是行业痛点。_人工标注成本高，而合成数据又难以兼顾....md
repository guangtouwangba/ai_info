...
上海AI实验室推出GraphGen框架，解决垂直领域大模型训练中高质量问答数据稀缺的痛点。该框架通过"知识图谱引导+双模型协同"机制，自动生成专业QA数据，无需人工标注。用户只需在OpenXLab上传文本块并配置API Key，即可在线生成适配LLaMA-Factory等训练框架的数据，直接用于大模型SFT训练。项目已在GitHub开源，并提供Web应用体验。该方法显著降低了垂域数据获取成本，同时保证了生成数据的质量与专业性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T23:02:43Z
- **目录日期**: 2025-04-26
