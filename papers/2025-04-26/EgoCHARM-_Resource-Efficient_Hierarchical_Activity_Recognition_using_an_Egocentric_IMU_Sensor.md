...
本文提出了一种名为EgoCHARM的资源高效机器学习算法，用于通过头戴式惯性测量单元(IMU)识别高级和低级活动。该分层算法采用半监督学习策略，主要使用高级活动标签进行训练，同时学习可泛化的低级运动嵌入特征。实验在9种高级和3种低级活动上分别获得0.826和0.855的F1分数，模型参数仅需63k(高级)和22k(低级)，可直接部署在当前IMU芯片上。研究还进行了敏感性分析，探讨了头戴式IMU活动识别的机会与限制。该方法在性能与资源效率间取得了良好平衡。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T23:01:16Z
- **目录日期**: 2025-04-26
