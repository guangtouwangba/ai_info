...
本文提出EgoCHARM算法，一种基于头戴式惯性测量单元(IMU)的高效机器学习方法，用于智能眼镜上的人类活动识别。该分层算法采用半监督学习策略，主要利用高级活动标签训练，可学习通用低级运动嵌入，实现高级和低级活动识别。在9种高级和3种低级活动测试中，分别获得0.826和0.855的F1分数，模型参数仅63k(高级)和22k(低级)，可直接部署在当前IMU芯片上。研究还分析了使用头戴式IMU进行活动识别的机会和限制。该方法在资源(内存、计算、功耗、样本)效率方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T06:01:15Z
- **目录日期**: 2025-04-26
