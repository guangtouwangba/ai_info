...
本文提出了一种名为EgoCHARM的高效机器学习算法，用于通过头戴式惯性测量单元(IMU)识别高低层活动。该分层算法采用半监督学习策略，主要利用高层活动标签进行训练，同时学习可泛化的低层运动嵌入特征。实验在9种高层和3种低层活动上取得了0.826和0.855的F1分数，模型参数仅63k(高层)和22k(低层)，可直接部署在当前IMU芯片上。研究还分析了使用头戴式IMU进行活动识别的机会与限制，展示了其在资源(内存、计算、功耗、样本)效率方面的优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T08:01:14Z
- **目录日期**: 2025-04-26
