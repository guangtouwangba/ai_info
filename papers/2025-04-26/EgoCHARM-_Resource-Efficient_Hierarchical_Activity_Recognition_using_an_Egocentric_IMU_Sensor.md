...
本文提出了一种名为EgoCHARM的高效机器学习算法，用于通过头戴式惯性测量单元(IMU)识别高低层次活动。该分层算法采用半监督学习策略，主要依赖高层次活动标签进行训练，同时学习可泛化的低层次运动嵌入。实验在9种高层次和3种低层次活动上取得了0.826和0.855的F1分数，模型参数仅63k(高层次)和22k(低层次)，可直接部署在当前IMU芯片上。研究还分析了使用头戴式IMU进行活动识别的机会与限制。该方法在资源(内存、计算、功耗、样本)效率方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T09:01:15Z
- **目录日期**: 2025-04-26
