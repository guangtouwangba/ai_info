...
本文提出EgoCHARM算法，用于智能眼镜上的单IMU人体活动识别。该算法采用分层半监督学习策略，仅需高级活动标签训练，即可学习可泛化的低级运动嵌入特征。在9种高级和3种低级活动识别中分别取得0.826和0.855的F1分数，模型参数量仅63k（高级）和22k（低级），可直接部署于现有IMU芯片。研究还分析了基于头戴式IMU的活动识别的机会与限制。该方法在资源效率（内存、计算、功耗、样本）方面表现优异，适用于健康监测和情境感知AI助手等场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T15:01:07Z
- **目录日期**: 2025-04-26
