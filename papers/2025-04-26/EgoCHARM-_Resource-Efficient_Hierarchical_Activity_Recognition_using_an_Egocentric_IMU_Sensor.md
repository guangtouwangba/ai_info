...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于智能眼镜上的单IMU设备进行高低层活动识别。该分层算法采用半监督学习策略，主要依赖高层活动标签训练，学习可泛化的低层运动嵌入特征。实验在9种高层和3种低层活动上分别取得0.826和0.855的F1分数，模型参数仅63k（高层）和22k（低层），可直接部署在当前IMU芯片上。研究还分析了基于第一视角IMU的活动识别技术的机遇与限制，为资源受限场景下的活动识别提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T00:01:25Z
- **目录日期**: 2025-04-26
