...
本文提出了一种资源高效的机器学习算法EgoCHARM，用于通过头戴式惯性测量单元(IMU)识别高低层次活动。该分层算法采用半监督学习策略，主要利用高层次活动标签进行训练，学习可泛化的低层次运动嵌入特征。实验在9种高层次和3种低层次活动上取得了0.826和0.855的F1分数，模型参数仅63k(高层次)和22k(低层次)，使低层次编码器可直接部署于现有IMU芯片。研究还分析了使用头戴式IMU进行活动识别的机会与限制。该方法在保持高性能的同时显著降低了计算资源需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T03:13:08Z
- **目录日期**: 2025-04-26
