...
本文提出EgoCHARM算法，用于智能眼镜单IMU设备的高效活动识别。该分层算法采用半监督学习策略，主要利用高层活动标签训练，同时学习可泛化的低层运动嵌入特征。在9种高层和3种低层活动识别中分别取得0.826和0.855的F1分数，模型参数量仅63k（高层）和22k（低层），可直接部署在当前IMU芯片上。研究还通过敏感性分析揭示了头戴式IMU活动识别的优势与局限，为健康监测和情境感知AI等应用提供了资源（内存、算力、功耗、样本）高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T12:01:23Z
- **目录日期**: 2025-04-26
