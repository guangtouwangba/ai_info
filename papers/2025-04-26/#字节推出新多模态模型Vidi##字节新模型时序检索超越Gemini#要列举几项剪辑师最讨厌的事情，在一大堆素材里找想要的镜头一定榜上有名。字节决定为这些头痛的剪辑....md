...
字节跳动推出全新多模态模型Vidi，专注于视频理解与编辑。该模型能快速定位1分钟至1小时视频中的目标内容，在VUE-TR基准测试中表现优于Gemini和GPT-4。Vidi采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并通过两阶段训练（跨模态对齐+应用后训练）提升时序检索能力。团队还推出了优化5个维度的VUE-TR评估基准。目前发布技术报告，即将推出7B模型demo。未来将拓展视频理解任务和交互式编辑功能，填补大模型与视频应用的差距。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T03:15:10Z
- **目录日期**: 2025-04-26
