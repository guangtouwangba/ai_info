...
字节跳动推出全新多模态模型Vidi，专注于视频理解与编辑。该模型能在1分钟至1小时以上的视频中精确定位内容，在VUE-TR基准测试中表现优于Gemini和GPT-4。Vidi采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并采用两阶段训练：跨模态对齐（视觉/音频与文本/时间戳对齐）和应用后微调。团队还推出了优化5个关键维度的VUE-TR评估基准。即将发布7B模型demo和权重，未来计划扩展视频理解任务并开发交互式编辑功能。论文和项目已公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T02:29:28Z
- **目录日期**: 2025-04-26
