...
字节跳动推出新型多模态模型Vidi，专注于视频理解与编辑。该模型能快速定位长视频中的目标内容，在VUE-TR基准测试中表现优于Gemini和GPT-4。Vidi采用分解注意力架构，将计算复杂度从O(N²)降至O(N)，并通过两阶段训练（跨模态对齐和应用后训练）提升时序检索能力。团队还推出了优化视频评估维度的VUE-TR基准。目前发布技术报告和评估基准，即将推出7B模型demo。未来计划扩展视频理解任务并开发交互式编辑功能，推动大模型在视频领域的应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T04:03:20Z
- **目录日期**: 2025-04-26
