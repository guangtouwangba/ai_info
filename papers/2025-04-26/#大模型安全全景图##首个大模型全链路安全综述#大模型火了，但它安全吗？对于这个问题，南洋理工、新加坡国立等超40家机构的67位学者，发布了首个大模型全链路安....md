...
这篇由67位学者联合发布的大模型安全综述，系统梳理了843篇研究，首次构建了从数据采集到商业应用的全链路安全知识图谱。研究指出大模型在各阶段面临的关键风险：数据污染（0.1%即可破坏模型）、微调阶段的指令注入、对齐阶段的奖励机制操控、模型遗忘引发的偏见、部署阶段的提示注入攻击，以及医疗/金融等场景的幻觉与隐私风险。防御策略包括差分隐私、多模态审核、事实验证等。该研究为构建可信AI提供了系统性参考框架，强调技术治理需协同推进。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T01:30:40Z
- **目录日期**: 2025-04-26
