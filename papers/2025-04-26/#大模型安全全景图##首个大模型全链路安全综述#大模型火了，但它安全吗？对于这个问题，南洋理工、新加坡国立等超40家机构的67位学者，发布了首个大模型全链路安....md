...
这篇由67位学者联合发布的大模型安全综述，系统梳理了843篇研究，首次构建了从数据采集到商业应用的全链路安全知识图谱。研究指出大模型在各阶段面临不同风险：数据污染（0.1%即可破坏模型）、微调攻击（指令注入/后门）、对齐偏差（奖励机制被操控）、模型遗忘（可能引入偏见）、部署威胁（提示注入/越狱攻击）以及商业应用风险（医疗/金融领域的幻觉和偏见）。防御需结合技术（差分隐私/对抗训练）与治理措施，为构建可信AI提供了系统性参考框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T02:29:47Z
- **目录日期**: 2025-04-26
