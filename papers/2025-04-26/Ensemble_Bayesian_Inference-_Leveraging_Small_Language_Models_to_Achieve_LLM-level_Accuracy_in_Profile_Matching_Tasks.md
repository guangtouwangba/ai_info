...
本研究提出了一种名为"集成贝叶斯推理(EBI)"的新方法，通过贝叶斯估计整合多个小型语言模型(SLM)的判断，使其性能超越单个模型。实验表明，该方法在多种任务(如日语和英语的能力评估与用户画像分析)中表现优异，甚至纳入性能较差的模型也能提升整体表现。这一发现为在有限计算资源下构建高性能AI系统提供了新思路，同时为有效利用性能较低的模型开辟了途径。研究基于现有的大模型评估、集成方法和开源模型应用成果，展现了该方法的新颖性和重要意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T07:01:36Z
- **目录日期**: 2025-04-26
