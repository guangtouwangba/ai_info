...
这项研究提出了一种名为"集成贝叶斯推理"(EBI)的新方法，通过贝叶斯估计整合多个小型语言模型(SLM)的判断，使其性能超越单个模型的限制。实验表明，即使在包含表现较差的模型(负提升值)时，EBI仍能提升整体性能，且在不同语言(日语和英语)任务中都有效。该方法为在有限计算资源下构建高性能AI系统提供了新思路，并能有效利用性能较低的模型。研究建立在现有大型语言模型评估、集成方法和开源模型应用的基础上，具有重要创新意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T14:01:38Z
- **目录日期**: 2025-04-26
