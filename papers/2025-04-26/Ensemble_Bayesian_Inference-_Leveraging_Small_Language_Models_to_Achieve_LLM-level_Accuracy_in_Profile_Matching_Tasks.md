...
本研究提出了一种新型集成贝叶斯推理方法(EBI)，通过贝叶斯估计整合多个小型语言模型(SLM)的输出，使其性能超越单个模型。实验表明，即使在包含负提升值模型的情况下，EBI仍能提升整体表现，且在多语言任务(日语/英语的能力评估和用户画像分析)中均有效。这一发现为在有限计算资源下构建高性能AI系统提供了新思路，同时为有效利用性能较低模型开辟了途径。研究基于现有的大模型评估、集成方法和开源模型应用成果，展示了该方法的新颖性和重要意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T08:01:44Z
- **目录日期**: 2025-04-26
