...
本研究提出一种名为"集成贝叶斯推理"(EBI)的新方法，通过贝叶斯估计整合多个小型语言模型(SLM)的预测结果，使其性能超越单个模型。实验表明，即使在包含负提升值模型的情况下，该集成方法仍能提升整体表现，且适用于日英双语环境。这一发现为在有限计算资源下构建高性能AI系统提供了新思路，同时为有效利用性能较低的模型开辟了途径。该方法基于现有的大模型评估、集成学习等研究，在提升模型性能的同时降低了计算成本。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T22:01:29Z
- **目录日期**: 2025-04-26
