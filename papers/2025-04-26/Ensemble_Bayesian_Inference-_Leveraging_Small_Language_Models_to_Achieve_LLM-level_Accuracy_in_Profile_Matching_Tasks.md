...
本研究提出了一种名为"集成贝叶斯推理(EBI)"的新方法，通过贝叶斯估计整合多个小型语言模型(SLM)的预测结果，使其性能可媲美专有大型语言模型(LLM)。实验表明，EBI在多种任务(包括日英双语的能力评估和消费者画像分析)中表现优异，甚至能有效利用性能较差的模型提升整体表现。该方法为在有限计算资源下构建高性能AI系统提供了新思路，并拓展了低性能模型的有效利用途径。研究建立在LLM性能评估、集成方法和开源LLM应用等现有成果基础上，具有重要创新意义。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T06:01:46Z
- **目录日期**: 2025-04-26
