...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。SGD-SaI无需自适应二阶动量，从训练开始就防止不平衡，内存使用比AdamW减少一半。实验显示，在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，解决了用SGD训练Transformer的难题。在LoRA微调等任务中也优于现有优化器，并为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T22:03:46Z
- **目录日期**: 2025-04-26
