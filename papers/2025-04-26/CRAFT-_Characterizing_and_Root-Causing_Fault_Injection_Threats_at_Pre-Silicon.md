...
这篇论文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V架构中时钟毛刺攻击对AI/ML应用的危害。研究发现两种新漏洞：一是通过指令跳过阻止关键内存值加载，影响程序正确性；二是将合法指令转为非法指令以劫持控制流。研究揭示了故障从电路层到软件层的传播路径，强调了在芯片设计阶段进行安全分析的重要性，以保护网络基础设施免受这类跨抽象层（从物理实现到系统软件）的攻击威胁。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T06:02:04Z
- **目录日期**: 2025-04-26
