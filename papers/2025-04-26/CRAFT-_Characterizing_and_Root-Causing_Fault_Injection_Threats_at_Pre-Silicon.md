...
本文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了AI/ML应用中时钟毛刺攻击导致的严重误分类问题。研究揭示了RISC-V指令集和流水线阶段的故障影响，并追踪了从电路层到AI/ML软件的故障传播路径。通过控制时钟毛刺参数，发现了两个新漏洞：一是导致指令跳转，阻止关键值从内存加载；二是将合法指令转为非法指令，从而可被攻击者利用控制流转向。研究强调了故障注入攻击的复杂性及预先安全分析的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T19:01:54Z
- **目录日期**: 2025-04-26
