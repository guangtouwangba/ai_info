...
本文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析RISC-V指令集和流水线阶段的故障影响，并追踪从电路层到AI/ML软件的故障传播。研究发现了两种新型漏洞：1) 通过时钟毛刺导致指令跳过，阻止关键值从内存加载，破坏程序连续性和正确性；2) 将合法指令转为非法指令，从而可被攻击者利用控制流转向。该工作揭示了故障注入攻击的复杂性，强调了在关键网络基础设施中进行预先安全分析的重要性，特别是在AI/ML应用关键错误分类场景下的时钟毛刺攻击。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T09:02:05Z
- **目录日期**: 2025-04-26
