...
这篇论文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响。研究以AI/ML应用中的时钟毛刺攻击为案例，揭示了两种新漏洞：一是通过指令跳过阻止关键内存值加载，破坏程序连续性；二是将合法指令转为非法指令以劫持控制流。该工作追踪了故障从电路层到应用软件的传播路径，强调了预判性安全分析的重要性，为保护网络基础设施提供了关键见解。研究结果凸显了故障注入攻击的复杂性及其对嵌入式系统多层次的威胁。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T16:02:07Z
- **目录日期**: 2025-04-26
