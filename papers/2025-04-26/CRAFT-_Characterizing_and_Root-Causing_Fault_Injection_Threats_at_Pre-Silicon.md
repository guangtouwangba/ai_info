...
本文提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析了RISC-V指令集和流水线阶段的故障影响，并追踪从电路层到AI/ML应用软件的故障传播。研究发现通过控制时钟毛刺参数可引发两种新漏洞：一是导致指令跳过，阻止从内存加载关键值，影响程序连续性和正确性；二是将合法指令转为非法指令，从而可被攻击者利用来控制流转向。该研究揭示了故障注入攻击的复杂性，强调了先发制人的安全分析对保护网络基础设施的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T02:28:08Z
- **目录日期**: 2025-04-26
