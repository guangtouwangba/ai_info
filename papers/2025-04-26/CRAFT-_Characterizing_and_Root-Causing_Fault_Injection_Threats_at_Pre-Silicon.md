...
该研究提出了一种在芯片设计前阶段进行可控故障注入攻击的综合方法，重点分析RISC-V指令集和流水线阶段的故障影响。通过时钟毛刺攻击在AI/ML应用中引发关键错误分类，研究发现了两个新漏洞：一是导致指令跳过，阻止从内存加载关键值，影响程序连续性和正确性；二是将合法指令转为非法指令，从而可被攻击者利用控制流转向。该工作揭示了故障注入攻击的复杂性，强调了在芯片设计前进行安全分析的重要性，以追踪故障从电路层到应用软件的传播路径。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T04:02:03Z
- **目录日期**: 2025-04-26
