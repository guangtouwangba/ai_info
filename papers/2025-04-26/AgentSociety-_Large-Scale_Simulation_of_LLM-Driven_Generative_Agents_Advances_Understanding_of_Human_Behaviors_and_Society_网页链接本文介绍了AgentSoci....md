...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个AI代理的社会互动（500万次交互）。研究者用它研究四大社会问题：社会极化、信息传播、基本收入政策效果和自然灾害影响。模拟结果与现实数据高度吻合，验证了该系统在揭示人类行为机制方面的可靠性。该平台为社会科学研究和政策制定提供了创新工具，能有效补充传统研究方法（如调查和实验），帮助深入理解复杂社会现象的成因和规律。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T13:09:33Z
- **目录日期**: 2025-04-26
