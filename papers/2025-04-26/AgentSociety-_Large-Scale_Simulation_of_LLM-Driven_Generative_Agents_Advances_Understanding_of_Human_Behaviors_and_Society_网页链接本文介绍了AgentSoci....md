...
这篇论文介绍了AgentSociety，这是一个基于大型语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（500万次）。研究者利用该平台研究了四个关键社会问题：社会极化、信息传播、基本收入政策影响和自然灾害冲击。模拟结果与现实世界数据高度吻合，验证了该工具在分析人类行为和社会机制方面的有效性。该平台可作为计算社会科学的新研究手段，为学者和政策制定者提供重要支持，弥补传统研究方法（如调查和访谈）的局限性，有助于深入理解复杂社会现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T05:04:54Z
- **目录日期**: 2025-04-26
