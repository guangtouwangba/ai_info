...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，关注模型输出与人类偏好的对齐问题。研究构建了包含397个高质量样本的基准测试(覆盖40类任务和44种语言)，并开发20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行基准与人类偏好存在差异；2)开源模型(Qwen2.5-Coder)与商业模型(OpenAI o1)性能差距显著。研究强调在代码生成任务中，除正确性外还需重视开发者体验和实际需求匹配，为提升代码LLMs的实用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T01:32:40Z
- **目录日期**: 2025-04-26
