...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，发现现有基准过于侧重代码正确性而忽视人类偏好。研究者构建了CodeArena基准（397个高质量样本，覆盖40类44种语言）和SynCode-Instruct指令集（20B标记）来评估模型表现。实验显示执行基准与人类偏好存在差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显对齐人类偏好的重要性。研究证实大规模合成指令微调的有效性，为改进代码生成模型提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T07:05:33Z
- **目录日期**: 2025-04-26
