...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，强调评估模型输出与人类偏好的匹配度。研究者收集了397个高质量编程任务样本(覆盖40类/44种语言)，并构建20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行基准与人类偏好存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究凸显了代码生成模型与人类偏好对齐的重要性，为改进模型提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T08:05:32Z
- **目录日期**: 2025-04-26
