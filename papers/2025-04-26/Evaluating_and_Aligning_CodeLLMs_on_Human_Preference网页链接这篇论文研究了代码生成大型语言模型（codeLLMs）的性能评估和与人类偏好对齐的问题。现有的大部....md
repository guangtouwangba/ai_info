...
这篇论文提出了CodeArena基准测试，用于评估代码生成大型语言模型（codeLLMs）与人类偏好的对齐程度。该基准包含397个高质量样本，覆盖40个类别和44种编程语言，模拟真实编程任务的复杂性。研究还构建了SynCode-Instruct合成指令语料库（近20B标记）验证大规模指令微调效果。实验显示，现有基于执行的基准与CodeArena存在性能差异，开源顶级模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显了人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T03:17:43Z
- **目录日期**: 2025-04-26
