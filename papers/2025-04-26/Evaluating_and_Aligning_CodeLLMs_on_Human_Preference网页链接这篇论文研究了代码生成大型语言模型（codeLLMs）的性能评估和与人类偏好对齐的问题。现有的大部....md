...
这篇论文针对代码生成大型语言模型（codeLLMs）提出了新评估方法，指出当前基准测试过于关注代码正确性而忽略人类偏好。研究者创建了CodeArena基准（含397个高质量样本，覆盖40类44种语言）和SynCode-Instruct合成指令库（近20B标记）。实验发现执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显人类偏好对齐的重要性。研究验证了大规模合成指令微调的有效性，为代码LLM评估提供了新维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T09:05:29Z
- **目录日期**: 2025-04-26
