...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案通过时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题，利用自监督视频模型提取的时间特征对齐生成帧。同时针对SyncNet收敛问题，通过调整模型架构、训练参数和数据预处理优化性能。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动的人像动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T19:04:41Z
- **目录日期**: 2025-04-26
