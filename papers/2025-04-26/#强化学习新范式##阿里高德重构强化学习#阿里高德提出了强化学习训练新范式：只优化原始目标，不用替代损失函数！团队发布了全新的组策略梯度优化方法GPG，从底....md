...
阿里高德提出强化学习新范式GPG，直接优化原始目标而无需替代损失函数。该方法革新训练框架，具有三大特点：(1)直接目标优化提升效率；(2)简化架构，去除评论模型和KL约束；(3)采用精准梯度估计(AGE)降低偏差。GPG通过组内奖励归一化和动态梯度校正机制增强稳定性，在数学推理和视觉理解等任务中表现优异，超越现有方法。该技术在多模态任务中展现出强通用性和鲁棒性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T10:03:40Z
- **目录日期**: 2025-04-26
