...
阿里高德提出强化学习新范式GPG（组策略梯度优化），突破性地摒弃传统替代损失函数，直接优化原始目标。该方法具有三大创新：1）消除评论模型和参考模型，简化架构；2）采用精准梯度估计技术（AGE）揭示奖励偏差；3）通过组内平均奖励归一化和动态梯度校正提升稳定性。实验显示，GPG在数学推理、视觉理解等单/多模态任务中表现优异，展现出更强的通用性和鲁棒性。该研究从底层重构了强化学习训练框架，显著提升了训练效率和策略稳定性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T18:03:32Z
- **目录日期**: 2025-04-26
