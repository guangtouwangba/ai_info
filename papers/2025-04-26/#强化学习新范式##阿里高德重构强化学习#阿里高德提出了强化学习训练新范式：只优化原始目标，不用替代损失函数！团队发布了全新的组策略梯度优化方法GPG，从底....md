...
阿里高德提出强化学习新范式GPG，直接优化原始目标而无需替代损失函数。该方法革新训练框架，消除评论模型和KL约束，引入精准梯度估计(AGE)降低方差并校正偏差。GPG在单/多模态任务中表现优异，尤其在数学推理和视觉理解等复杂任务展现强通用性。核心突破包括：1)直接目标优化提升效率；2)极简架构去除冗余组件；3)动态梯度校正增强稳定性。实验表明其性能超越现有方法，为强化学习训练提供了新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-26T17:03:23Z
- **目录日期**: 2025-04-26
