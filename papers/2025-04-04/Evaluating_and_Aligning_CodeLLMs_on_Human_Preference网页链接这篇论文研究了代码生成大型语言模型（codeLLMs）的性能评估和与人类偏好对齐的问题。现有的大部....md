...
这篇论文研究了代码生成大模型（codeLLMs）与人类偏好的对齐问题。作者提出了CodeArena基准，包含397个高质量编程任务样本，覆盖40个类别和44种语言，更贴近实际开发需求。同时构建了20B标记的合成指令数据集SynCode-Instruct用于模型微调。实验发现现有基于执行的评测与人类偏好存在差距，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著。研究强调了代码生成模型不仅需要正确性，还需符合人类开发者偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T01:32:25Z
- **目录日期**: 2025-04-04
