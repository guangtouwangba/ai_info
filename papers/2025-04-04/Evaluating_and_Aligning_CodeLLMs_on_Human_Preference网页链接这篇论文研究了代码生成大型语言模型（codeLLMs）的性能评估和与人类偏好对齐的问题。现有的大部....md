...
这篇论文探讨了代码生成大模型（CodeLLMs）与人类偏好对齐的问题，指出当前基准测试过于关注代码正确性而忽略实际偏好。研究者开发了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，模拟真实编程任务。同时构建了SynCode-Instruct合成指令数据集（近200亿标记）验证大规模微调效果。实验显示，执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与专有模型（如OpenAI o1）差距显著，凸显人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T10:06:40Z
- **目录日期**: 2025-04-04
