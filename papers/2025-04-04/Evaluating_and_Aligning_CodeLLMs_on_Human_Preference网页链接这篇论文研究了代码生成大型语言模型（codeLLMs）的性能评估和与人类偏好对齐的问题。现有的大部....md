...
该论文研究代码生成大模型（CodeLLMs）与人类偏好的对齐问题。现有基准主要评估代码正确性，而忽略了人类偏好匹配。为此，作者构建了CodeArena基准（含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令数据集（近200亿标记）。实验发现：1)执行基准与CodeArena存在性能差异；2)开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI）差距显著。研究凸显了代码模型与人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T14:06:23Z
- **目录日期**: 2025-04-04
