...
该论文针对代码生成大模型(codeLLMs)提出新评估方法CodeArena，包含397个真实编程任务样本(覆盖40类/44种语言)，重点考察模型输出与人类偏好的匹配度。研究同时构建20B标记的合成指令数据集SynCode-Instruct用于微调验证。实验发现：1)传统执行基准与人类偏好评估存在差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调代码生成模型除正确性外，还需重视人类偏好对齐。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T21:05:29Z
- **目录日期**: 2025-04-04
