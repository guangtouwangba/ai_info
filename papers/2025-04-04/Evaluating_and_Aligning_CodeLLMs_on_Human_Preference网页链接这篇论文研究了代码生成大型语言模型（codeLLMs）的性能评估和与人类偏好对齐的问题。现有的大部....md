...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好的对齐问题。研究者创建了CodeArena基准测试（包含397个高质量样本，覆盖40类任务和44种编程语言）和SynCode-Instruct合成指令集（近200亿标记），以评估模型在真实编程场景中的表现。实验发现现有基于执行的测试与人类偏好存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显了模型对齐人类偏好的必要性。研究强调了代码生成质量评估需超越单纯正确性，关注实际开发需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T03:17:29Z
- **目录日期**: 2025-04-04
