...
大型AI模型在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，暴露出脆弱性（如对输入数据微小变化敏感）、缺乏情境判断、输出错误信息等问题。为此，研究者提出分层创新框架，通过模块化方法将大模型转化为实用垂直系统，并强调各层级的动态协同。该框架帮助研究者：（1）定位创新方向（如垂直领域知识赋能通用创新）；（2）发现被忽视的机会（如跨领域共性问题驱动实用基础模型开发）；（3）促进跨学科协作（建立开发者、领域专家与人机交互学者的共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T10:01:44Z
- **目录日期**: 2025-04-04
