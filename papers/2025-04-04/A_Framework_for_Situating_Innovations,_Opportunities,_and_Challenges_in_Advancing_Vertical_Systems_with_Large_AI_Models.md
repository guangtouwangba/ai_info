...
大型AI模型在标准化测试中表现优异，但在医疗、教育等高风险领域应用时，常暴露出脆弱性、缺乏情境理解及输出错误等问题。为此，研究提出一个分层创新框架，通过模块化方法将大模型转化为实用"垂直系统"，并强调各层间的动态交互。该框架帮助研究者：(1)定位创新方向（如垂直领域知识赋能通用创新）；(2)发现被忽视的机会（如开发实用基础模型而非追逐基准测试）；(3)促进跨学科沟通（建立开发者、领域专家与人机交互学者的共同语言）。案例研究表明该框架可指导实际应用创新。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T22:01:28Z
- **目录日期**: 2025-04-04
