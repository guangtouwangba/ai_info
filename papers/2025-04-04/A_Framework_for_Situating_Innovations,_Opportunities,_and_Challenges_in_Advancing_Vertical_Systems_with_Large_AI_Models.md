...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，仍存在明显缺陷，如对输入数据细微变化敏感、关键决策缺乏情境理解、错误输出影响用户信任等。为此，研究者提出分层创新框架，通过模块化方法将大模型转化为实用垂直系统，并强调各层间的动态交互。该框架可帮助研究人员：(1)定位创新点（如垂直领域知识如何赋能通用创新）；(2)发现被忽视的机会（如跨领域共性问题）；(3)促进跨学科协作（为开发者、领域专家建立共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T21:01:27Z
- **目录日期**: 2025-04-04
