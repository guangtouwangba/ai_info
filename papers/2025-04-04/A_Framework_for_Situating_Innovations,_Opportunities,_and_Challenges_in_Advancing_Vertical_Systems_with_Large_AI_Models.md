...
大型AI模型虽然在标准化测试中表现优异，但在医疗、教育、法律等高风险领域应用时，暴露出明显缺陷，如对输入数据的微小变化敏感、关键决策缺乏情境认知、自信地输出错误信息等。为此，研究者提出一个分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态交互。该框架帮助研究者：(1)定位创新点（如垂直领域洞察可推动通用创新）；(2)发现被忽视的机会（如跨领域共性问题可指导实用基础模型开发）；(3)促进跨学科协作（为AI开发者、领域专家等建立共同语言）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T03:12:53Z
- **目录日期**: 2025-04-04
