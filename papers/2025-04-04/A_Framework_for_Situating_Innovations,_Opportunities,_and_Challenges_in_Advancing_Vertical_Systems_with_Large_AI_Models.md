...
大型AI模型在标准化测试中表现优异，但在医疗、教育等关键领域应用时暴露出明显缺陷，如对输入数据变化敏感、决策缺乏情境理解、错误输出等问题。为此，研究者提出了一个分层创新框架，通过模块化方法将大模型转化为实用的垂直系统，并强调各层间的动态协作。该框架能帮助研究者：(1)定位创新方向（如垂直领域洞察赋能通用创新）；(2)发现被忽视的机会（如跨领域共性问题建模）；(3)促进跨学科交流（建立开发者、领域专家与人机交互学者的共同语言）。案例研究验证了其可行性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T23:01:38Z
- **目录日期**: 2025-04-04
