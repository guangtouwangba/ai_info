...
该研究构建了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型假设细胞会向压力较低区域迁移，细胞压力定义为不同表型细胞密度的加权和。通过将个体细胞建模为经历表型依赖、压力调控的分裂、死亡和迁移的随机游走代理，从离散模型推导出连续极限。研究发现不同表型细胞在入侵前沿会形成空间分离的行波解，数值模拟验证了离散与连续模型的一致性。结果表明，细胞迁移能力的异质性可能导致入侵前沿出现空间隔离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T15:02:37Z
- **目录日期**: 2025-04-04
