...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体生长。模型中，细胞倾向于向压缩较小的区域移动，其运动方向沿细胞压力梯度下降。细胞压力定义为不同表型细胞密度的加权和，且细胞的移动性和压力贡献权重随表型而变化。研究通过离散个体模型推导出连续极限，并分析了细胞表型依赖和压力调节的分裂、死亡和运动。结果显示，不同表型的细胞在入侵前沿呈现空间隔离，数值模拟验证了模型与行波分析的一致性，表明细胞移动性差异可导致入侵前沿的空间隔离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T17:02:32Z
- **目录日期**: 2025-04-04
