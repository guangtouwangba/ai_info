...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型假设细胞会向压缩力较小的区域迁移，其运动方向取决于细胞压力梯度（由不同表型细胞的加权密度决定）。不同表型细胞具有不同的形态和力学特性，表现为迁移能力和压力贡献的差异。研究通过离散个体模型的连续极限推导出该方程，分析了细胞表型空间分离的传播波解，并通过数值模拟验证了模型一致性。结果表明，细胞迁移能力的异质性可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T14:02:34Z
- **目录日期**: 2025-04-04
