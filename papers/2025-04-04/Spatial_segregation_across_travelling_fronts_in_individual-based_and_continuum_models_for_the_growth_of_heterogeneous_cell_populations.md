...
该研究建立了一个偏微分方程模型，描述由多个离散表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域迁移，其运动沿细胞压力梯度方向进行，而细胞压力由不同表型细胞的密度加权和定义。不同表型的细胞具有不同的形态和力学特性，表现为细胞迁移率和压力贡献权重的表型依赖性。研究通过离散个体模型的连续极限推导该模型，分析了细胞表型空间分离的传播波解，并通过数值模拟验证了模型一致性。结果表明，细胞迁移率的异质性可导致入侵前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T13:08:52Z
- **目录日期**: 2025-04-04
