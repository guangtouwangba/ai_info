...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压缩较小的区域移动，其运动方向沿细胞压力梯度下降方向。细胞压力定义为不同表型细胞密度的加权和，且细胞迁移率和压力贡献权重随表型而异。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型依赖性和压力调节的分裂、死亡及运动。数值模拟显示，不同表型细胞在入侵前沿呈现空间分离的波状传播，且细胞迁移率的变异性是这种空间分离的基础机制。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T19:02:28Z
- **目录日期**: 2025-04-04
