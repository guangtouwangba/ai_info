...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体的生长动态。模型中，细胞倾向于向压力较低的区域迁移，其运动方向由细胞压力梯度决定，而细胞压力是不同表型细胞密度的加权总和。不同表型的细胞具有不同的形态和力学特性，因此其迁移能力和对压力的贡献权重也不同。研究从基于个体的离散模型推导出连续极限模型，并分析了细胞表型在入侵前沿空间分离的行波解。数值模拟验证了离散与连续模型的一致性，表明细胞迁移能力的差异可导致入侵前沿的空间分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T22:02:23Z
- **目录日期**: 2025-04-04
