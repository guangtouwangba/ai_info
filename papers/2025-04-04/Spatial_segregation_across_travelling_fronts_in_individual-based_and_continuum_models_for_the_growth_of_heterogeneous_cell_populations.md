...
该研究建立了一个偏微分方程模型，描述由多种不同表型组成的异质性细胞群体的生长动态。模型中，细胞倾向于向压力较低的区域迁移，其运动方向沿细胞压力梯度下降。细胞压力定义为不同表型细胞密度的加权总和，且细胞的迁移能力和对压力的贡献权重随表型变化。研究通过离散个体模型的连续极限推导出该模型，分析了细胞表型在入侵前沿空间分离的行波解，并通过数值模拟验证了模型一致性。结果表明，细胞迁移能力的变异性可导致入侵前沿出现空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T04:02:17Z
- **目录日期**: 2025-04-04
