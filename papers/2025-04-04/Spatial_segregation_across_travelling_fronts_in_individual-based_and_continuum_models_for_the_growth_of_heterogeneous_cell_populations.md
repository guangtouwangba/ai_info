...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质性细胞群体的生长动态。模型假设细胞会向压力较低区域迁移，细胞压力定义为不同表型细胞密度的加权总和。不同表型的细胞具有不同的运动能力和压力贡献权重。研究通过离散个体模型推导出连续极限，并分析了细胞表型依赖的迁移、分裂和死亡行为。重点研究了入侵前沿中不同表型细胞空间分离的行波解，数值模拟验证了离散与连续模型的一致性。结果表明，细胞运动能力的差异可导致入侵前沿出现空间隔离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T07:02:28Z
- **目录日期**: 2025-04-04
