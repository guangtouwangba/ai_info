...
该研究构建了一个偏微分方程模型，描述由多种表型组成的异质细胞群体生长。模型中细胞倾向于向压缩较小的区域迁移，其运动遵循细胞压力梯度（压力为不同表型细胞密度的加权和）。不同表型细胞具有不同的形态和力学特性，表现为细胞迁移能力和压力贡献权重的差异。研究通过离散个体模型的连续极限推导该方程，分析了细胞表型依赖和压力调控的分裂、死亡及迁移行为。数值模拟验证了模型与行波解的一致性，表明细胞迁移能力的差异可导致侵袭前沿的空间表型分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T16:02:25Z
- **目录日期**: 2025-04-04
