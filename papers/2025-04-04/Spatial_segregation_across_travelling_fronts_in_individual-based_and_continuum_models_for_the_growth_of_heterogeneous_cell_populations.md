...
该研究建立了一个偏微分方程模型，描述由多种表型组成的异质细胞群体生长。模型假设细胞会向压缩较小的区域迁移，其运动方向沿细胞压力梯度下降方向，而细胞压力由不同表型细胞的密度加权总和决定。通过将个体层面的离散模型（细胞作为单代理进行表型依赖、压力调节的分裂、死亡和迁移）转化为连续极限，推导出该模型。研究分析了细胞表型在入侵前沿空间分离的行波解，并通过数值模拟验证了离散与连续模型的一致性，表明细胞迁移能力的差异可导致入侵前沿的空间分离。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T08:02:44Z
- **目录日期**: 2025-04-04
