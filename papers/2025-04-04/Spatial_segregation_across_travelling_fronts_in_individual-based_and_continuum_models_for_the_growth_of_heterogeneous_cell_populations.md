...
该研究提出了一种偏微分方程模型，用于描述由多个不同表型组成的异质性细胞群体生长。模型假设细胞倾向于向压缩较小的区域移动，其运动方向沿细胞压力梯度下降方向。细胞压力定义为不同表型细胞密度的加权和，且细胞移动性和压力贡献权重随表型变化。研究通过离散个体模型推导出该连续模型，分析了细胞表型空间分离的行波解，并通过数值模拟验证了模型一致性。结果表明，细胞移动性的差异可导致入侵细胞前沿出现空间分离现象。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T05:02:32Z
- **目录日期**: 2025-04-04
