...
过去20年，人群模拟研究主要集中在避碰等低层任务和路径跟随等有限行为上。为生成更真实的人群场景，我们提出Gen-C模型，通过大语言模型生成初始场景，再经模拟扩展构建时空交互图。该模型采用双变分图自编码器，分别学习图结构（交互关系）和节点特征（个体行为）的潜在空间，支持基于自然语言的条件生成。实验证明，该方法能在大学校园和火车站等场景中，生成反映复杂决策的多样化动态群体行为，显著提升了虚拟环境中人群交互的真实性和可控性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T02:27:47Z
- **目录日期**: 2025-04-04
