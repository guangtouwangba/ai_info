...
过去20年，人群模拟研究多集中于避碰等底层任务和路径跟随等简单行为。为生成更真实的人群场景，本文提出Gen-C模型，利用大语言模型生成初始场景，通过模拟构建时空扩展图来建模智能体行为与交互。该方法采用双变分图自编码器，分别学习图结构（交互关系）和节点特征（个体行为）的潜在空间，支持基于自然语言的条件生成。实验在大学校园和火车站场景中验证了模型生成多样化动态行为的能力，展现了模拟复杂高层决策交互的潜力，避免了真实视频数据标注的高成本问题。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T00:02:10Z
- **目录日期**: 2025-04-04
