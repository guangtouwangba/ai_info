...
传统优化编译器在现代软件系统的复杂性应对中发挥重要作用。随着并行编程需求增长，研究者探索大型语言模型（LLMs）能否革新代码优化方法。本研究对比了三种传统编译器与两种LLM的优化能力，并引入包含挑战性优化模式的基准测试套件及自动评估机制。结果表明：LLM（如CodeLlama-70B）最高可实现1.75倍加速，优于最佳传统编译器CETUS（1.67倍），但大代码量时易生成错误结果，需自动化验证。将编译器策略融入提示词能显著提升LLM表现，且不同提示策略（简单指令、详细指令、思维链）效果差异显著。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T00:03:36Z
- **目录日期**: 2025-04-04
