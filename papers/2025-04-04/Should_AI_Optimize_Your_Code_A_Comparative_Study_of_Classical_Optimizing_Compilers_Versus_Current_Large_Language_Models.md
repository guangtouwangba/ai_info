...
传统优化编译器在现代软件系统发展中至关重要，但并行编程的需求和大型语言模型（LLMs）的兴起引发了AI是否能够革新代码优化方法的探讨。本研究对比了三种传统编译器与两种LLMs在代码优化中的表现，并提出了一个基准测试套件和自动验证机制。结果显示，LLMs（如CodeLlama-70B）潜力显著，最高提速达1.75倍，优于传统编译器（如CETUS的1.67倍），但大代码量时易出错，需自动化验证。提示策略中，将编译器策略融入提示可显著提升性能，且三种策略效果差异明显。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T01:29:37Z
- **目录日期**: 2025-04-04
