...
当前AI代理在多模态应用、结构化生成和策略规划等复杂任务中仍面临挑战。传统微调方法因依赖黑箱API而受限，而最佳N采样(BON)等推理时优化方法虽有效但缺乏迭代反馈机制。为此，研究者提出迭代代理解码(IAD)，通过验证器指导的动态评估与选择实现迭代优化。实验表明，IAD在Sketch2Code、Text2SQL和Webshop任务中性能提升3-10%，关键改进源于验证器引导的优化而非采样多样性。研究还发现，在优质验证器指导下，IAD和BON都能通过增加计算资源提升性能，验证器质量对推理时优化至关重要。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T02:27:27Z
- **目录日期**: 2025-04-04
