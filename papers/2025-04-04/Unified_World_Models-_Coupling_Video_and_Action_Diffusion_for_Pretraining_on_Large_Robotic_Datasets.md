...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用多模态数据提升机器人策略学习。UWM基于统一Transformer架构，独立控制各模态扩散时间步，灵活实现策略、动态预测和视频生成功能。实验表明：（1）UWM在大规模多任务数据集预训练中优于模仿学习，生成更鲁棒策略；（2）通过独立调控模态时间步，可有效利用无动作视频数据提升微调性能。该框架为整合异构数据、桥接模仿学习与世界建模提供了新思路，推动了通用机器人基础模型的发展。项目详见https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T10:01:55Z
- **目录日期**: 2025-04-04
