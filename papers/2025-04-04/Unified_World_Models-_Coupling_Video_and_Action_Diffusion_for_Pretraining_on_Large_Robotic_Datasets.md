...
模仿学习是构建通用机器人的有效方法，但依赖高质量专家演示限制了其扩展性。为解决这一问题，研究者提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用大量无标注视频数据提升策略学习。UWM采用统一Transformer架构，独立控制各模态扩散步骤，灵活实现策略、动态预测和视频生成。实验表明，UWM在大规模多任务数据集上预训练后，策略泛化性和鲁棒性优于传统模仿学习，并能通过无动作视频数据进一步提升性能。该框架为利用异构数据实现可扩展的机器人学习提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T09:01:52Z
- **目录日期**: 2025-04-04
