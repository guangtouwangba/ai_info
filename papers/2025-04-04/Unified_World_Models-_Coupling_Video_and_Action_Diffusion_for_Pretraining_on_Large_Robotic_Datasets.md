...
本文提出了统一世界模型（UWM），一种结合视频和动作数据的机器人策略学习框架。UWM通过统一Transformer架构整合动作扩散和视频扩散过程，能灵活表示策略、动态预测和视频生成。实验表明：（1）UWM在大规模多任务数据集上的预训练比模仿学习产生更泛化、鲁棒的策略；（2）通过独立控制模态扩散步长，UWM能有效利用无动作标注视频数据提升策略性能。该框架为利用异构数据实现可扩展的机器人学习提供了新思路，并统一了模仿学习与世界建模范式。代码和视频见项目网页。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T20:01:40Z
- **目录日期**: 2025-04-04
