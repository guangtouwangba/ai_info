...
本文提出统一世界模型（UWM），一种结合视频与动作数据的机器人策略学习框架。UWM通过统一Transformer架构整合动作扩散和视频扩散过程，利用独立扩散时间步控制不同模态，可灵活实现策略、动态预测、逆向动态和视频生成功能。实验表明：1）UWM能有效利用多任务机器人数据集进行预训练，比模仿学习获得更具泛化性和鲁棒性的策略；2）通过独立控制模态扩散时间步，UWM可直接利用无动作标注视频数据提升策略微调效果。该框架为利用异构大数据进行可扩展机器人学习提供了新思路，统一了模仿学习与世界建模范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T07:01:43Z
- **目录日期**: 2025-04-04
