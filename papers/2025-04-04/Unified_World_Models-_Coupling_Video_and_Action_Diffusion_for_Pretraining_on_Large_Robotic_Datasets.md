...
本文提出了一种名为"统一世界模型（UWM）"的新框架，用于机器人策略学习。UWM通过统一架构整合了动作扩散和视频扩散过程，能灵活表示策略、前向/逆向动态和视频生成。研究表明：1）UWM支持大规模多任务数据集预训练，比模仿学习产生更通用稳健的策略；2）通过独立控制模态扩散时间步，UWM能有效利用无动作视频数据提升策略性能。该框架为利用异构数据集进行可扩展机器人学习提供了新思路，并统一了模仿学习和世界建模两种范式。实验验证了UWM在仿真和真实场景中的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T19:01:47Z
- **目录日期**: 2025-04-04
