...
模仿学习是构建通用机器人的有效方法，但依赖高质量专家示范，难以扩展。为解决这一问题，研究者提出统一世界模型（UWM）框架，通过整合动作扩散和视频扩散过程，利用未标注动作的视频数据提升策略学习。UWM能灵活表示策略、动态预测和视频生成，在大规模多任务数据集上预训练后，比单纯模仿学习更具泛化性和鲁棒性。实验表明，UWM通过独立控制扩散步骤，能有效利用无动作视频数据，进一步提升策略性能。该框架为利用异构数据实现可扩展机器人学习提供了新思路。详见项目页面。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T06:01:47Z
- **目录日期**: 2025-04-04
