...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用多模态数据提升机器人策略学习。UWM采用统一Transformer架构，独立控制各模态扩散时间步，灵活实现策略、动态预测和视频生成。实验表明：（1）UWM在大规模多任务数据集预训练中，比模仿学习产生更泛化、鲁棒的策略；（2）通过独立调控模态扩散步，UWM能有效利用无动作标注视频数据提升策略微调效果。该框架为整合异构数据、桥接模仿学习与世界建模提供了新思路，推动了机器人基础模型的可扩展学习。项目页面见文末链接。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T13:08:03Z
- **目录日期**: 2025-04-04
