...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用大规模异构数据提升机器人策略学习。UWM采用统一Transformer架构，独立控制各模态的扩散时间步，灵活实现策略、动态预测和视频生成。实验表明：1）UWM在多任务机器人数据集预训练中，比模仿学习获得更泛化、鲁棒的策略；2）通过独立控制模态扩散时间步，可有效利用无动作标注视频数据提升策略微调性能。该框架为整合模仿学习与世界建模提供了简单统一的解决方案，代码见项目网站。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T04:01:39Z
- **目录日期**: 2025-04-04
