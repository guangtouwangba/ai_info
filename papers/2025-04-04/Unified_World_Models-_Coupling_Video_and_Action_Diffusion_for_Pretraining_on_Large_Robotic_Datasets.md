...
模仿学习是构建通用机器人的重要方法，但依赖高质量专家演示限制了其扩展性。本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM支持同时利用带动作标注和无动作视频数据，在大规模多任务数据集预训练后，比单纯模仿学习获得更鲁棒的策略，并能通过控制扩散时间步从无动作视频中学习。实验表明，UWM为整合模仿学习和世界建模提供了统一框架，有助于利用异构数据实现可扩展的机器人学习。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T14:01:48Z
- **目录日期**: 2025-04-04
