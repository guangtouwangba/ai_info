...
模仿学习是构建通用机器人的重要方法，但依赖高质量专家示范限制了其扩展性。本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM能同时处理含动作标注和无动作视频数据，在大规模多任务机器人数据集上预训练后，比单纯模仿学习获得更具泛化性和鲁棒性的策略。实验表明，UWM通过独立控制模态扩散步骤，有效利用无动作视频数据提升策略性能，为整合模仿学习和世界建模提供了统一框架。详见项目页面：https://weirdlabuw.github.io/uwm/。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T08:01:56Z
- **目录日期**: 2025-04-04
