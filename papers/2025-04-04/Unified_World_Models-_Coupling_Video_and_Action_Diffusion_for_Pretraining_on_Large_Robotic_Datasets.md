...
本文提出统一世界模型(UWM)，通过整合动作扩散和视频扩散过程，利用Transformer架构灵活实现策略学习、动态预测和视频生成。UWM突破传统模仿学习对专家演示的依赖，能同时利用含动作标注和无动作的视频数据进行预训练。实验表明：1)UWM在大规模多任务机器人数据集上预训练后，比单纯模仿学习获得更通用、鲁棒的策略；2)通过独立控制模态扩散步长，可有效利用无动作视频数据提升微调策略性能。该框架为整合模仿学习与世界建模提供了统一方案，推动了利用异构数据实现可扩展机器人学习的研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T22:01:39Z
- **目录日期**: 2025-04-04
