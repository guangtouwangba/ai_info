...
本文提出统一世界模型（UWM），通过整合动作扩散和视频扩散过程，利用Transformer架构实现多模态学习。UWM能灵活表示策略、动态预测和视频生成，支持大规模多任务机器人数据集预训练，比模仿学习更具泛化性和鲁棒性。该框架还能利用无动作标注的视频数据，通过独立控制扩散步长提升策略性能。实验表明，UWM为利用异构数据进行可扩展机器人学习提供了新思路，并统一了模仿学习和世界建模范式。代码和视频见项目网站。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T18:01:43Z
- **目录日期**: 2025-04-04
