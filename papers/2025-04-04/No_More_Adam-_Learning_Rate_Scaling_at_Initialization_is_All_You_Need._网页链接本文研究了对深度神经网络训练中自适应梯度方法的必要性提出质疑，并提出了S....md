...
该研究提出SGD-SaI方法，挑战了自适应梯度方法(如Adam)的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整各参数组学习率，解决了传统SGD训练不平衡问题，同时将优化器内存占用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类及GPT-2预训练中表现优于或持平AdamW，在LoRA微调中超越现有优化器。内存效率显著，为GPT-2和Llama2-7B分别节省5.93GB和25.15GB内存。该方法兼具超参数鲁棒性和广泛适用性，为大规模模型训练提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T12:05:05Z
- **目录日期**: 2025-04-04
