...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉关联。针对扩散模型的时间一致性问题，提出TREPA方法，利用自监督视频模型增强帧间一致性。同时优化SyncNet训练框架，改进其收敛性和准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。创新点包括：1）端到端潜在扩散架构；2）TREPA时间对齐机制；3）SyncNet训练优化方案。该成果对音频驱动人像动画领域具有普适参考价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T14:06:02Z
- **目录日期**: 2025-04-04
