...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉相关性，解决了传统方法的时间一致性问题。创新点包括：1）时间表示对齐（TREPA）方法增强生成帧的时间一致性；2）改进SyncNet训练框架，提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-04T09:06:00Z
- **目录日期**: 2025-04-04
