...
视觉语言模型（VLMs）在直接多模态任务中表现优异，但在游戏等交互式视觉环境中的决策能力较弱。为解决这一“知而不行”的差距，研究者提出了VLM-Gym——一个专为多游戏并行训练设计的强化学习（RL）环境，包含多样化的视觉游戏和可调节难度。通过纯RL驱动的自进化训练，G0模型展现出感知与推理能力。进一步优化的G1模型结合了感知增强的冷启动和RL微调，性能超越Claude-3.7等领先专有模型。研究发现，RL训练中感知与推理能力相互促进。代码已开源以推动VLM交互代理研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T06:00:57Z
- **目录日期**: 2025-05-20
