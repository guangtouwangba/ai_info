...
视觉语言模型（VLMs）在直接多模态任务中表现出色，但在游戏等交互式视觉环境中的决策能力较弱。为解决这一“知行差距”，研究者提出了VLM-Gym——一个强化学习（RL）环境，包含多样化的视觉游戏，支持并行训练。通过VLM-Gym，纯RL训练的G0模型展现出感知与推理能力。进一步改进的G1模型结合了感知增强的冷启动策略，在所有游戏中超越基准模型（如Claude-3.7-Sonnet-Thinking）。研究发现，RL训练中感知与推理能力相互促进。相关代码已开源，以推动VLMs作为交互式智能体的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T09:01:15Z
- **目录日期**: 2025-05-20
