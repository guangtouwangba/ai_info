...
视觉语言模型（VLMs）在多模态任务中表现优异，但在交互式视觉环境（如游戏）中的决策能力较弱。为此，研究者提出了VLM-Gym——一个强化学习（RL）环境，包含多样化的视觉游戏，支持并行训练。通过VLM-Gym，他们训练了纯RL驱动的G0模型，发现其涌现出感知与推理能力。进一步改进的G1模型结合了感知增强的冷启动策略，在所有游戏中表现均超越G0和Claude-3.7等领先模型。研究发现，RL训练中感知与推理能力相互促进。相关代码已开源以推动VLM作为交互代理的研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T10:08:15Z
- **目录日期**: 2025-05-20
