...
视觉语言模型（VLMs）在直接多模态任务中表现优异，但在游戏等交互式视觉环境中的决策能力较弱，形成“知而不行”的差距。为此，研究者提出VLM-Gym，一个强化学习（RL）环境，包含多样化的视觉游戏，支持并行训练。通过纯RL驱动的自我进化训练G0模型，展现出感知和推理能力。进一步改进的G1模型在RL微调前加入感知增强的冷启动，性能超越教师模型和Claude-3.7-Sonnet-Thinking等领先专有模型。研究发现，感知和推理能力在RL训练中相互促进。相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T11:00:55Z
- **目录日期**: 2025-05-20
