...
视觉语言模型（VLMs）在直接多模态任务中表现优异，但在游戏等交互式视觉环境中的决策能力较弱。为解决这一“知而不行”的差距，研究者开发了VLM-Gym——一个支持多游戏并行训练的强化学习（RL）环境。通过纯RL驱动的自我进化训练G0模型后，进一步开发了结合感知增强冷启动的G1模型。G1在所有游戏中均超越其教师模型及Claude-3.7等领先专有模型。研究发现，RL训练中感知与推理能力会相互促进。相关代码已开源以推动VLM作为交互代理的研究。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T20:00:55Z
- **目录日期**: 2025-05-20
