...
本文介绍了VLM-Gym，一个专为视觉语言模型（VLMs）设计的强化学习环境，旨在解决VLMs在视觉丰富交互环境（如游戏）中决策能力不足的问题。通过VLM-Gym，研究者训练了G0和G1模型，其中G1模型在强化学习微调前加入了感知增强的冷启动步骤。实验表明，G1模型在所有游戏中均超越其教师模型及领先的专有模型（如Claude-3.7-Sonnet-Thinking）。研究发现，感知和推理能力在强化学习过程中相互促进。相关代码已开源，以推动VLMs作为交互代理的发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T18:00:56Z
- **目录日期**: 2025-05-20
