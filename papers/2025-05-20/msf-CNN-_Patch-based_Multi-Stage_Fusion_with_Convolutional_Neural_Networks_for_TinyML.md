...
本文介绍了msf-CNN，一种针对微控制器（MCU）优化的卷积神经网络融合技术。该技术通过将融合解空间建模为有向无环图，高效搜索最优融合方案，显著扩展了可行解范围。相比现有方法（MCUNetV2和StreamNet），msf-CNN在ARM Cortex-M、RISC-V和ESP32等MCU上运行时，可减少50%的RAM占用，同时保持低延迟以满足实时性要求。这种基于分块融合的方法优化了神经网络层间数据流，为系统设计者提供了更大的灵活性，使AI模型能适配MCU的极小内存预算（如128kB RAM）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T00:01:22Z
- **目录日期**: 2025-05-20
