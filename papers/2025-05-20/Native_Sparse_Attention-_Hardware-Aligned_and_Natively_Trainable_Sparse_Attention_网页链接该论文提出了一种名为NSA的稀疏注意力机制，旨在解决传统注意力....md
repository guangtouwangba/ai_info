...
这篇论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过算法与硬件协同优化，采用动态分层稀疏策略：先进行粗粒度令牌压缩，再进行细粒度令牌选择，在保持全局上下文感知能力的同时确保局部精度。该方法的两个关键创新点在于：1）硬件对齐设计提升计算效率；2）原生可训练性使模型能端到端学习稀疏模式。实验表明NSA能有效降低长文本建模的计算开销，同时维持模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T04:06:16Z
- **目录日期**: 2025-05-20
