...
这篇论文提出了SGD-SaI方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了训练不平衡问题，同时将优化器内存使用减半。实验表明，该方法在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优于或等同AdamW，尤其在大模型训练中显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB）。该方法具有超参数鲁棒性和广泛适用性，为替代自适应优化器提供了高效方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T04:05:46Z
- **目录日期**: 2025-05-20
