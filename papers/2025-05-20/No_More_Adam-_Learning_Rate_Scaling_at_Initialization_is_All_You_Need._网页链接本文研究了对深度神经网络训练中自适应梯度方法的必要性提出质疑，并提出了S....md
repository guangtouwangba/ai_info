...
该论文提出SGD-SaI方法，质疑自适应梯度方法(如Adam)的必要性。该方法改进带动量SGD，通过在初始化时依据梯度信噪比(g-SNR)调整各参数组学习率，避免训练不平衡且减少50%内存占用。实验显示，SGD-SaI在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优于或持平AdamW，尤其在大模型训练中显著节省内存（GPT-2省5.93GB，Llama2-7B省25.15GB），同时保持超参数鲁棒性。该方法解决了SGD训练Transformer的难题，成为高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T02:34:05Z
- **目录日期**: 2025-05-20
