...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理及其500万次互动。研究者用它分析四大社会问题：社会极化、刺激性信息传播、无条件基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界数据高度吻合，验证了该工具在揭示人类行为机制方面的有效性。这一平台为社会科学研究和政策制定提供了新方法，能补充传统调查和实验的不足，帮助深入理解复杂社会现象背后的模式和原因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T04:06:26Z
- **目录日期**: 2025-05-20
