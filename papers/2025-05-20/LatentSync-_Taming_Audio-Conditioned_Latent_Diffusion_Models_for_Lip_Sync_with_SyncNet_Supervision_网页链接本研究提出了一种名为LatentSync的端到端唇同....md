...
本研究提出LatentSync，一种端到端唇同步框架，利用音频条件的潜在扩散模型直接建模音频-视觉关系，无需中间运动表示。为解决扩散过程中时间一致性问题，开发了TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时改进了SyncNet训练框架，提升了其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上优于现有技术。创新点包括：1) 首个基于潜在扩散的唇同步模型；2) TREPA时间一致性增强；3) SyncNet训练优化方案。这些改进可推广至其他音频驱动动画任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T04:06:43Z
- **目录日期**: 2025-05-20
