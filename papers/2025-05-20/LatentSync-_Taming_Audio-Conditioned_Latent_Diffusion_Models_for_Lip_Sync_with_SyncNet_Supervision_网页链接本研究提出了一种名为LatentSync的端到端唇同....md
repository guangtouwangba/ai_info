...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，并针对扩散模型的时间一致性问题提出TREPA（时间表示对齐）技术，通过自监督视频模型增强生成帧的时序一致性。研究还优化了SyncNet训练框架，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他依赖SyncNet的唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T02:35:13Z
- **目录日期**: 2025-05-20
