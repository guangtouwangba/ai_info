...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该框架直接建模音频-视觉关系，基于Stable Diffusion能力。针对扩散模型的时间一致性问题，提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时改进了SyncNet训练框架，解决了收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括潜在扩散模型应用、TREPA时间一致性增强及SyncNet训练优化，为唇同步和音频驱动动画提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T19:05:41Z
- **目录日期**: 2025-05-20
