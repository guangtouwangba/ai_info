...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示，直接建模音频-视觉相关性。针对扩散模型的时间一致性问题，提出时间表示对齐(TREPA)方法增强一致性。同时优化SyncNet训练框架，改进其收敛性和准确性。在HDTF和VoxCeleb2数据集上，该方法在多项指标上超越现有技术。创新点包括：1)直接利用Stable Diffusion能力；2)TREPA保持同步精度；3)SyncNet训练优化可推广至其他唇同步方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T12:05:59Z
- **目录日期**: 2025-05-20
