...
该研究提出了Cosmos-Reason1模型，旨在通过长链思维推理理解物理世界并生成具身决策。模型基于分层本体表示物理常识（空间、时间、物理定律）和二维本体实现跨具身泛化，开发了70亿和560亿参数的两种多模态大语言模型。采用两阶段训练：物理AI监督微调（SFT）和强化学习（RL）。评估显示该方法显著提升了物理常识与具身推理能力。相关代码和预训练模型已通过NVIDIA开放许可发布。研究为物理AI系统建立了包含感知、理解与行动执行的综合基准框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T19:02:12Z
- **目录日期**: 2025-05-20
