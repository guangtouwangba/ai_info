...
本文提出了一种基于强化学习的无训练数据方法，用于改进最大割问题（MaxCut）的近似解。传统Goemans-Williamson（GW）算法通过随机超平面对半定规划松弛解进行舍入，而该方法利用马尔可夫决策过程，让智能体学习选择更优的舍入超平面。相比GW算法，该方法能在大规模不同密度和度分布的图上持续获得更好的割解，避免了现有学习方法的泛化和扩展性问题，且无需依赖大量标记数据或领域专业知识。该工作为NP难问题提供了新的启发式求解思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T10:08:25Z
- **目录日期**: 2025-05-20
