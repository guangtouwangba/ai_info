...
本文提出了一种基于强化学习的无训练数据方法，用于改进最大割问题（MaxCut）的近似解。传统Goemans-Williamson（GW）算法通过随机超平面划分获得解，而新方法利用马尔可夫决策过程（MDP）让智能体学习选择更优的划分超平面。相比GW算法，该方法无需大规模标注数据，避免了现有学习方法的泛化性和可扩展性问题。实验表明，该方法在不同密度和度分布的大规模图上能持续获得更好的切割结果，优于GW算法的随机超平面划分方式。这一工作为NP难问题提供了新的数据无关学习解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T14:01:15Z
- **目录日期**: 2025-05-20
