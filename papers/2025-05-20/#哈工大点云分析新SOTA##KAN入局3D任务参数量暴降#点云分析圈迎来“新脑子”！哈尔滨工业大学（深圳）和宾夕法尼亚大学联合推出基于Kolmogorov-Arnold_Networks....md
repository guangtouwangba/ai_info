...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs（Kolmogorov-Arnold Networks）的PointKAN模型，革新3D点云分析任务。该模型通过可学习函数替代传统MLP的固定激活函数，显著提升几何特征学习能力。核心模块包括几何仿射、局部特征并行处理（KAN Block+深度卷积）和全局残差特征提取。升级版PointKAN-elite引入高效有理函数结构，实现参数量暴降且精度提升。实验显示，其在分类、分割及小样本任务中全面超越MLP方案，成为点云分析新SOTA（State-of-the-Art）。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T00:03:34Z
- **目录日期**: 2025-05-20
