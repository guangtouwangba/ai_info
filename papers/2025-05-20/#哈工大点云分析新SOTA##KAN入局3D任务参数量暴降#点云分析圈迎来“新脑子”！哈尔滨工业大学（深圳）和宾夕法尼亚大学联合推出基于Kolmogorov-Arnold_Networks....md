...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs（Kolmogorov-Arnold Networks）的新型点云分析模型PointKAN，显著提升3D任务性能。通过可学习函数替代传统MLP的固定激活函数，PointKAN能更灵活捕捉几何特征，其核心模块包括几何仿射、局部特征处理（KAN Block+深度卷积）和全局特征提取（残差块）。升级版PointKAN-elite引入高效有理函数和参数共享，参数量暴降但精度反升，在分类、分割等任务中全面超越现有方法，成为点云分析新SOTA。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T02:33:20Z
- **目录日期**: 2025-05-20
