...
哈尔滨工业大学（深圳）与宾夕法尼亚大学联合提出基于KANs的PointKAN模型，革新点云分析领域。该模型通过可学习函数替代传统MLP的固定激活函数，结合几何仿射模块、局部/全局特征处理结构，显著提升3D任务性能。升级版PointKAN-elite采用高效有理函数和参数共享技术，在参数量暴降的同时，于分类、分割等任务中全面超越现有方法，实现精度与效率双突破。相关成果已在多个数据集验证其SOTA表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T03:22:15Z
- **目录日期**: 2025-05-20
