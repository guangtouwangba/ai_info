...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本(覆盖40类/44种语言)，重点关注模型输出与人类偏好的对齐性。研究发现传统基于执行的评估与人类偏好存在显著差异，并通过40多个模型实验揭示开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)的性能差距。同时构建20B标记的合成指令数据集SynCode-Instruct验证了大规模指令微调的有效性，强调代码生成模型除正确性外还需优化人类偏好匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T09:06:24Z
- **目录日期**: 2025-05-20
