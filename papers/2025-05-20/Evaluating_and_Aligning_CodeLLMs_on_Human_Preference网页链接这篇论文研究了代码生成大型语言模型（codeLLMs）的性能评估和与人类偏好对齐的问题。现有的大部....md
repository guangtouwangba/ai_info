...
这篇论文聚焦代码生成大模型(codeLLMs)与人类偏好的对齐问题，提出CodeArena基准测试（含397个跨40类/44种语言的样本）来评估模型在真实编程场景中的表现。研究同时构建了20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现：1)传统执行测试与人类偏好评估存在差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调了代码生成模型除功能性外，还需重视与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T16:06:14Z
- **目录日期**: 2025-05-20
