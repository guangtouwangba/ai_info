...
该论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，重点评估模型输出与人类偏好的匹配度。研究收集了397个涵盖40类编程任务和44种语言的真实样本，并构建近200亿标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好存在差距；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差异显著。研究强调代码生成模型除正确性外，还需重视与开发者实际需求的契合度，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-20T05:06:20Z
- **目录日期**: 2025-05-20
