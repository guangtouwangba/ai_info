...
Dec-POMDPs是难解的NEXP完全问题，但局部可见性和依赖假设可简化某些多智能体任务。DeWeese和Qu(2024)提出局部互依赖多智能体MDP框架，给出了三种易计算且接近最优的闭式策略，但在有限可见度下会出现"惩罚抖动"问题。本研究提出扩展截断策略类，首次实现对任意局部互依赖MDP都保持指数级接近最优的闭式部分可观测策略，能记忆视野外智能体，显著改善有限可见度表现，消除抖动现象，并在特定条件下实现完全可观测的最优行为。同时扩展了原框架，支持转移依赖和扩展奖励依赖。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T02:30:51Z
- **目录日期**: 2025-06-05
