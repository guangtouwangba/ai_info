...
本文研究了局部相互依赖多智能体MDP（LIM-MDP）中的可计算最优策略问题。作者提出了"扩展截断策略类"，这是首个针对任意LIM-MDP的非平凡闭式近优部分可观测策略类，其性能随能见度呈指数级接近最优。相比之前易受"惩罚抖动"现象影响的策略，新策略能记忆超出能见范围的智能体，显著提升了小固定能见度下的性能，消除了抖动问题，并在特定条件下保证完全可观测的最优联合行为。研究还扩展了LIM-MDP框架，允许转移依赖和扩展奖励依赖，并在新设定中复现了理论结果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T16:01:11Z
- **目录日期**: 2025-06-05
