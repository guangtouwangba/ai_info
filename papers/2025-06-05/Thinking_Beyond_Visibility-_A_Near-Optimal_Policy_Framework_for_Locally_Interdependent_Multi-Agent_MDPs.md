...
Dec-POMDPs因计算复杂度高而难以求解。针对合作导航等问题，DeWeese和Qu（2024）提出局部依赖多智能体MDP框架，给出三种可计算且接近最优的策略，但在有限视野下易陷入"惩罚抖动"。本研究提出扩展截断策略类，首次实现任意局部依赖MDP下接近最优的闭式部分可观测策略，通过记忆视野外智能体显著提升小视野性能，消除抖动现象，并在特定条件下实现完全可观测的最优行为。同时扩展了原框架，支持转移依赖和扩展奖励依赖，并验证了理论结果的普适性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T15:01:18Z
- **目录日期**: 2025-06-05
