...
该研究针对局部可观测多智能体MDP问题，提出了"扩展截断策略类"——首个非平凡的近最优闭式策略类。相比此前工作中易受"惩罚抖动"现象影响的三种策略，新策略能记忆超出当前视野范围的智能体状态，在小视野固定场景中表现更优，能有效解决抖动问题，并在特定条件下实现完全可观测的最优联合行为。研究还扩展了局部依赖多智能体MDP模型，使其支持转移依赖和扩展奖励依赖，并在新框架下复现了理论结果。这些策略计算可行，且最优性随视野范围呈指数级逼近。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T06:01:25Z
- **目录日期**: 2025-06-05
