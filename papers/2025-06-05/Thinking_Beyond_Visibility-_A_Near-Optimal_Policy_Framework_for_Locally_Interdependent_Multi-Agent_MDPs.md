...
本文介绍了针对局部可观测多智能体MDP问题的扩展截断策略类（Extended Cutoff Policy Class）。该策略类能在智能体视野有限的情况下，记住视野范围外的其他智能体，显著提升性能并解决"惩罚抖动"问题。这些闭式策略计算高效，在视野范围内接近最优，某些情况下甚至能实现完全可观测的最优行为。研究还扩展了局部依赖多智能体MDP模型，允许状态转移和奖励的额外依赖关系，并在新模型中验证了理论结果。该工作为部分可观测环境下的多智能体协作提供了首个非平凡的近优闭式策略解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T22:01:01Z
- **目录日期**: 2025-06-05
