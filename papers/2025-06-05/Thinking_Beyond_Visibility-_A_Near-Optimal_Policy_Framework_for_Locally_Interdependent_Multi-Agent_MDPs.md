...
本文介绍了针对局部可观测多智能体MDP问题的扩展截断策略类（Extended Cutoff Policy Class）。该策略类首次实现了在局部依赖多智能体MDP中接近最优的闭式部分可观测策略，其性能随可视范围呈指数级接近最优。新策略能记忆超出可视范围的智能体，显著提升了小固定可视范围下的表现，解决了"惩罚抖动"现象，并在特定条件下保证完全可观测的最优联合行为。研究还提出了允许转移依赖和扩展奖励依赖的广义局部依赖多智能体MDP模型，并在该框架下复现了理论结果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T23:01:16Z
- **目录日期**: 2025-06-05
