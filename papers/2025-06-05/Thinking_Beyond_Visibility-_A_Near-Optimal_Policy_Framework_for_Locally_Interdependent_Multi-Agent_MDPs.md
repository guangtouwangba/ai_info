...
本文介绍了针对局部可观测多智能体MDP（LI-MAMDP）的扩展截断策略类，这是首个非平凡的近最优闭式策略类。在DeWeese和Qu（2024）提出的LI-MAMDP框架基础上，新策略通过记忆超出可视范围的智能体，显著提升了小固定可视范围下的性能，解决了"惩罚抖动"问题，并在特定条件下实现完全可观测的最优行为。研究还将LI-MAMDP推广到包含转移依赖和扩展奖励依赖的情形，并在此设定下复现了理论结果。这些策略计算可行且随可视范围呈指数级接近最优。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T14:01:12Z
- **目录日期**: 2025-06-05
