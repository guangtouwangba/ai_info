...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇部同步，无需中间运动表示。该方案创新性地引入时间表示对齐(TREPA)方法解决扩散过程中的时间一致性问题，同时保持同步精度。研究还改进了SyncNet训练框架，通过优化模型架构、超参数和数据预处理显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为音频驱动人像动画提供了新思路。这些改进也可推广至其他依赖SyncNet的唇同步应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T09:07:11Z
- **目录日期**: 2025-06-05
