...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案利用Stable Diffusion直接建模音视频关联，但发现传统扩散方法存在时间一致性问题。为此提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时针对SyncNet收敛问题，系统研究了模型架构、超参数和数据预处理的影响，改进了训练框架。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，相关SyncNet优化经验可推广至其他人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T01:33:07Z
- **目录日期**: 2025-06-05
