...
Quarkdown是一款基于Markdown的轻量化排版工具，兼具简单易用与专业灵活性。它支持函数化书写（含变量、判断、循环）、多格式导出（HTML/PDF/幻灯片等），并内置可扩展标准库（数学/布局等组件）。相比LaTeX，其语法更简洁、生成更快且支持实时预览，适用于论文、电子书、技术文档等场景。全流程仅需Markdown语法即可完成复杂排版，实现从轻量到专业的无缝过渡。项目地址：github.com/iamgio/quarkdown。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T01:31:03Z
- **目录日期**: 2025-06-05
