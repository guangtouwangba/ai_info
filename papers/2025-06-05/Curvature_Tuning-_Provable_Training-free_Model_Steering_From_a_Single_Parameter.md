...
这篇论文提出了一种新的微调方法——曲率调谐(CT)，通过调整激活函数的曲率来调节模型的决策边界，而非传统的权重调整。CT具有可解释性，仅需引入一个超参数，就能显著提升模型性能。实验表明，CT在12个数据集上使ResNet-50/152的下游准确率分别提升7.14%/8.46%(相比线性探测)和4.64%/1.70%(相比LoRA)，并在鲁棒性基准测试中使准确率提升超过10倍。该方法将模型投影到平滑函数空间，与现有特征适应方法形成互补，提供了一种参数高效的微调新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T00:02:41Z
- **目录日期**: 2025-06-05
