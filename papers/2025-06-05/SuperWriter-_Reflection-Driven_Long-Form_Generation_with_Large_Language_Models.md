...
为解决大语言模型生成长文本时存在的连贯性、逻辑一致性和质量下降问题，研究者提出了SuperWriter-Agent框架。该框架通过引入结构化思维（规划与优化阶段）来模拟专业作者的写作流程，并基于此构建监督微调数据集训练7B参数的SuperWriter-LM模型。采用分层直接偏好优化（DPO）结合蒙特卡洛树搜索（MCTS）技术，将最终质量评估反馈至每个生成步骤进行优化。实验表明，该模型在多项基准测试中超越更大规模的基线模型，验证了分层DPO和结构化思维对提升长文本生成质量的有效性。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T09:02:20Z
- **目录日期**: 2025-06-05
