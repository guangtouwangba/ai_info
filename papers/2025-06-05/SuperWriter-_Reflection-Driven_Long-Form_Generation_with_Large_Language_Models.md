...
为解决大语言模型在生成长文本时面临的连贯性、逻辑一致性和质量保持等挑战，研究者提出了基于代理的框架SuperWriter-Agent。该框架通过引入结构化思维（规划与优化阶段）来模拟专业作者的写作流程，并构建监督微调数据集训练7B参数的SuperWriter-LM模型。结合分层直接偏好优化（DPO）和蒙特卡洛树搜索（MCTS）技术，该模型能根据最终质量评估优化每个生成步骤。实验表明，SuperWriter-LM在多项基准测试中超越更大规模的基线模型，验证了分层DPO和结构化思维步骤对提升长文本生成质量的有效性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T14:01:49Z
- **目录日期**: 2025-06-05
