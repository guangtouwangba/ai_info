...
这篇论文提出了TracLLM框架，用于追踪长上下文大语言模型(LLM)生成输出时依赖的关键文本片段(如句子、段落)。该技术可应用于LLM系统调试、攻击后取证分析(如提示注入攻击)和增强用户信任等场景。相比现有特征归因方法(如Shapley)，TracLLM通过智能搜索算法提高了效率，并采用贡献分数集成/去噪技术提升准确性。实验表明，TracLLM能有效识别长上下文中影响LLM输出的关键文本。该框架为长上下文LLM的可解释性提供了新解决方案，代码和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T02:31:01Z
- **目录日期**: 2025-06-05
