...
这篇论文提出了TracLLM，首个针对长上下文大语言模型(LLMs)的通用上下文回溯框架。该框架旨在高效准确地识别影响LLM输出的关键文本片段(如句子、段落)，解决现有特征归因方法(如Shapley)在长上下文场景下性能不足或计算成本高的问题。TracLLM通过基于信息搜索的算法提高效率，并采用贡献分数集成/去噪技术提升准确性。该技术在调试LLM系统、攻击取证分析和增强用户信任等方面具有应用价值。实验表明，TracLLM能有效识别长上下文中导致LLM输出的关键文本。代码和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T10:01:04Z
- **目录日期**: 2025-06-05
