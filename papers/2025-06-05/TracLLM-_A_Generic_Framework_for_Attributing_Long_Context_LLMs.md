...
这篇论文提出了TracLLM框架，旨在解决长上下文大语言模型(LLM)的"上下文回溯"问题——即识别生成输出所依赖的具体文本片段。该框架通过改进现有特征归因方法(如Shapley)，开发了基于知情搜索的算法提高效率，并采用贡献分数集成/去噪技术提升准确性。TracLLM可应用于LLM系统调试、攻击后取证分析(如提示注入攻击)和增强用户信任等场景。评估表明，该框架能有效识别长上下文中影响LLM输出的关键文本。代码和数据已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T20:01:16Z
- **目录日期**: 2025-06-05
