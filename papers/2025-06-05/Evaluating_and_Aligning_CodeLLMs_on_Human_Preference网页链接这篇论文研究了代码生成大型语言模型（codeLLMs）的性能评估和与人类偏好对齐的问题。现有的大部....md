...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，指出当前基准测试过于侧重代码正确性而忽视人类偏好。研究者构建了CodeArena基准（含397个跨40类/44种编程语言的样本）和20B标记的SynCode-Instruct合成指令集。实验发现：1）执行基准与人类偏好评估存在差异；2）开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著。研究强调代码LLMs需同时满足功能正确和人类偏好对齐，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T17:06:20Z
- **目录日期**: 2025-06-05
