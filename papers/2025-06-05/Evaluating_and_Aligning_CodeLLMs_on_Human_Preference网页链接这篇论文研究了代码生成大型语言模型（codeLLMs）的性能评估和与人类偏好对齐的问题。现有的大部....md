...
这篇论文研究了代码生成大模型（CodeLLMs）与人类偏好的对齐问题，指出当前基准测试过于关注代码正确性而忽略用户体验。作者提出CodeArena基准（包含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令集（近200亿标记），用于评估模型在真实编程场景中的表现。实验发现执行基准与人类偏好存在差异，且开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差距显著，凸显了人机对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T11:06:08Z
- **目录日期**: 2025-06-05
