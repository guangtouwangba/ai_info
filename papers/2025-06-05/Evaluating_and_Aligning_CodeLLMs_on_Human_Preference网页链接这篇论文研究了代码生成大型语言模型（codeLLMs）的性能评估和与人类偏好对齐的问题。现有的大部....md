...
这篇论文针对代码生成大模型（codeLLMs）提出新评估方法，重点关注与人类偏好的对齐。研究创建了CodeArena基准测试（含397个跨40类别/44语言的高质量样本）和SynCode-Instruct合成指令集（20B标记），发现传统执行测试与人类偏好存在差距。实验比较40多个模型显示，开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）性能差异显著，强调对齐人类偏好的重要性。研究为提升代码LLMs的实际适用性提供了新评估框架和数据支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T23:05:44Z
- **目录日期**: 2025-06-05
