...
该论文针对代码生成大模型(codeLLMs)提出CodeArena基准测试，通过397个涵盖40类编程任务和44种语言的真实样本，评估模型与人类偏好的对齐程度。研究发现传统基于执行的测试与人类偏好存在差距，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差异显著。同时构建了20B标记的合成指令数据集SynCode-Instruct验证微调效果。研究强调了代码生成模型除正确性外，还需重视人类编程习惯和需求匹配，为模型优化提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T10:05:51Z
- **目录日期**: 2025-06-05
