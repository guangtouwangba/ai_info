...
AI专家Karpathy提出，在AI协同时代，仅依赖复杂UI界面（如Adobe、CAD）而缺乏文本交互能力的软件将被淘汰。他认为软件需支持"氛围式编程"，并划分了四类应用风险等级：高风险的纯UI软件（如Photoshop）、中风险的混合型（如Excel）、低风险的文本主导型（如VSCode）和安全的纯文本工具（如终端）。支持者认为封闭UI会阻碍AI整合，反对者则强调专业软件需要复杂UI。共识是未来软件需实现UI与文本指令的无缝转换，并建立标准化AI交互框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T21:03:38Z
- **目录日期**: 2025-06-05
