# #大神Karpathy炮轰复杂UI应用##无法被模型调用的软件将失去未来#在人与AI高度协同的时代，只有大量复杂UI界面的应用将会被淘汰。大神Karpathy给出了对于应用程序...
...
AI专家Karpathy提出，在AI协同时代，仅依赖复杂UI界面（如Adobe、CAD）的软件将面临淘汰风险，因其缺乏文本交互能力，难以实现"氛围式编程"。他将应用按UI/文本比例分为四个风险等级，认为软件需支持AI调用。支持者指出封闭UI会阻碍AI整合，反对者则认为专业软件（如Photoshop）需要专门培训，AI应适应人类操作方式。争议焦点在于UI的未来角色——建议UI应构建在文本交互基础上，实现双向转换。双方都认同需要更规范的软件框架来促进人机协作。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T20:03:36Z
- **目录日期**: 2025-06-05
