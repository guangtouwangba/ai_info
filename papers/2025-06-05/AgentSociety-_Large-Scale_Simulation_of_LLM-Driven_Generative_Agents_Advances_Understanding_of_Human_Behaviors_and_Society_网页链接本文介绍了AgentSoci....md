...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它结合了大型语言模型（LLM）驱动的智能代理、真实社会环境和强大的模拟引擎。研究者模拟了超过1万个代理的社会生活，记录了500万次互动。该系统被用于研究四个关键社会问题：社会极化、信息传播、无条件基本收入政策以及外部冲击（如飓风）的影响。模拟结果与现实世界实验数据吻合，证明了该系统在捕捉人类行为和社会机制方面的有效性，展示了其作为社会科学研究和政策制定工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T08:06:11Z
- **目录日期**: 2025-06-05
