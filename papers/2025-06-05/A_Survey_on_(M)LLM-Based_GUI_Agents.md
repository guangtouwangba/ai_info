...
这篇综述系统分析了基于大语言模型（LLM）的图形用户界面（GUI）代理的最新进展。现代GUI代理包含四大核心组件：(1)结合文本解析和多模态理解的感知系统；(2)通过内部建模和历史经验构建的知识探索机制；(3)利用高级推理的任务规划框架；(4)具备安全控制的交互系统。研究揭示了LLM和多模态学习如何革新跨平台GUI自动化，同时指出现有评估框架的局限性。关键挑战包括元素精确定位、知识检索、长程规划和执行安全控制。该综述为研究者提供了该领域的全面概述，并展望了未来发展方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T20:02:21Z
- **目录日期**: 2025-06-05
