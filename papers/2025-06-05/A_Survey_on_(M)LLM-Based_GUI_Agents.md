...
这篇综述全面探讨了基于大语言模型（LLM）的图形用户界面（GUI）智能代理的发展现状。文章系统分析了GUI代理的四大核心组件：（1）结合文本解析和多模态理解的感知系统；（2）通过内部建模和历史经验构建的知识库探索机制；（3）利用高级推理方法进行任务分解的规划框架；（4）具备安全控制的交互系统。研究揭示了LLM和多模态学习如何革新跨平台GUI自动化，同时指出了当前评估框架的局限性，并探讨了元素定位、知识检索、长程规划和执行安全等关键技术挑战及未来研究方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T15:02:54Z
- **目录日期**: 2025-06-05
