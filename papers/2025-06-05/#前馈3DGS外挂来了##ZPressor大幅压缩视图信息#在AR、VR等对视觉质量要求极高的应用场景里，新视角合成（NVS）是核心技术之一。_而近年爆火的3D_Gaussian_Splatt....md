...
Monash大学与ZIP Lab团队开发了轻量模块ZPressor，用于优化前馈式3D高斯泼溅（3DGS）模型。该模块显著提升了新视角合成（NVS）性能：在36张输入视图下，PSNR提升4.65dB，推理时间减少70%，显存占用降低80%，并支持输入视图从8张扩展到近500张。3DGS因快速、高质量的渲染成为NVS领域的明星方案，但现有模型在多视图输入时面临信息过载问题。ZPressor作为即插即用解决方案，有效解决了这一瓶颈，适用于AR/VR等高要求场景。论文和代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T20:03:14Z
- **目录日期**: 2025-06-05
