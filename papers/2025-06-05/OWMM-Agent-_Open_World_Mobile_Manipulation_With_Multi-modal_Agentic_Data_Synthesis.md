...
这篇论文提出了一种新型多模态智能体架构，用于解决开放世界移动操作(OWMM)任务的挑战。该架构通过维护多视角场景帧和智能体状态进行决策，并通过函数调用控制机器人。针对领域偏移导致的幻觉问题，作者开发了一个数据合成流程来微调视觉语言模型(VLM)，使其适应任务领域。最终提出的OWMM-VLM模型是首个专为移动操作器设计的统一基础模型，具备全局场景理解、机器人状态跟踪和多模态动作生成能力。实验表明，该模型性能优于GPT-4o等基础模型，并在真实世界中展现出强大的零样本泛化能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T13:10:11Z
- **目录日期**: 2025-06-05
