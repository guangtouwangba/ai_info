...
这篇论文提出了一种新型多模态智能体架构OWMM-VLM，用于解决开放世界移动操作（OWMM）任务的挑战。该架构通过维护多视角场景帧和智能体状态进行决策，并采用函数调用控制机器人。针对领域偏移导致的幻觉问题，研究者开发了数据合成流程来微调视觉语言模型（VLM），使其适应任务领域。实验表明，OWMM-VLM作为首个专为移动操作设计的统一基础模型，在全局场景理解、状态跟踪和多模态动作生成方面表现优异，性能超越GPT-4o等模型，并展现出强大的零样本泛化能力。项目页面见GitHub链接。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T12:01:10Z
- **目录日期**: 2025-06-05
