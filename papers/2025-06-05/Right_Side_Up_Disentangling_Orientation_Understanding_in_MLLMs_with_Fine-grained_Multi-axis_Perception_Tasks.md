...
DORI是一个专门评估视觉语言模型物体方向理解能力的基准测试，包含四个维度：正面对齐、旋转变换、相对方向关系和标准方向理解。通过11个数据集的67个物体类别测试，发现当前最先进的15个模型存在明显局限：最佳模型在粗粒度任务中仅达54.2%准确率，细粒度方向判断降至33.0%，且在参考系转换或复合旋转任务中表现更差。结果表明现有模型缺乏精确角度估计、视角变化跟踪和复合旋转理解能力，揭示了其内部3D空间表征的不足。DORI为改进机器人控制、3D场景重建和人机交互提供了诊断框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T12:02:37Z
- **目录日期**: 2025-06-05
