...
DORI是一个专门评估视觉语言模型物体方向感知能力的基准测试，包含四个维度：正面对齐、旋转变换、相对方向关系和标准方向理解。该测试基于11个数据集的67个物体类别，涵盖合成和真实场景。评估15个最先进的视觉语言模型后发现，即使在粗粒度任务中最佳模型准确率仅54.2%，细粒度判断降至33.0%，且在参考系转换和复合旋转任务中表现更差。结果表明现有模型缺乏精确角度估计、视角变化跟踪和复合旋转理解能力，突显了改进3D空间表征机制的必要性。DORI为提升机器人控制、3D场景重建和人机交互提供了诊断框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T08:02:46Z
- **目录日期**: 2025-06-05
