...
视觉感知中的物体朝向理解是机器人操控和增强现实等应用的核心挑战。现有视觉语言基准测试未能独立评估该能力，常与位置关系和场景理解混淆。研究者提出DORI基准，专门评估物体朝向感知，包含四个维度：正面对齐、旋转变换、相对方向关系和标准朝向理解。通过11个数据集的67类物体测试发现，15个先进模型的最高准确率仅54.2%（粗粒度任务）和33.0%（细粒度判断），在参考系转换和复合旋转任务中表现更差。这表明现有模型缺乏精确角度估计和3D空间表征能力，凸显了开发专用朝向表征机制的必要性。DORI为提升机器人控制等应用提供了诊断框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T20:02:01Z
- **目录日期**: 2025-06-05
