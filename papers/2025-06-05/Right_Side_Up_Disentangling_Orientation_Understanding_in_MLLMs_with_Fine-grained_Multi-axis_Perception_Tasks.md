...
DORI是一个专注于评估多模态系统物体方向感知能力的基准测试，包含四个维度：正面对齐、旋转变换、相对方向关系和标准方向理解。通过11个数据集的67个物体类别，DORI评估了15个先进视觉语言模型，发现其表现有限：最佳模型在粗粒度任务中仅达54.2%准确率，细粒度方向判断中为33.0%，且在参考系转换或复合旋转任务中表现更差。结果表明现有模型缺乏精确角度估计、视角变化跟踪和复合旋转理解能力，突显了改进3D空间表征机制的必要性。DORI为机器人控制、3D场景重建等应用提供了诊断框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T06:02:23Z
- **目录日期**: 2025-06-05
