...
研究者提出了DORI基准测试，专门评估视觉语言模型对物体方向的理解能力。DORI从四个维度(正面对齐、旋转变换、相对方向关系、标准方向理解)测试模型表现，涵盖11个数据集的67个物体类别。测试15个最先进模型后发现：即使在粗粒度任务中最高准确率仅54.2%，细粒度判断降至33.0%；模型在参考系转换和复合旋转任务中表现更差。这表明现有模型缺乏精确角度估计、视角变化跟踪和复合旋转理解能力，反映了其内部3D空间表征的局限性。DORI为改进机器人控制、3D场景重建和人机交互提供了诊断框架。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T05:02:33Z
- **目录日期**: 2025-06-05
