...
DORI是一个专门评估多模态系统物体方向感知能力的基准测试，包含四个维度：正面对齐、旋转变换、相对方向关系和标准方向理解。通过11个数据集、67个物体类别的测试，发现当前最先进的视觉语言模型在方向理解上存在显著局限：最佳模型在粗粒度任务中准确率仅54.2%，细粒度判断中降至33.0%，且在参考系转换和复合旋转任务中表现更差。研究表明现有模型缺乏精确角度估计、视角变化跟踪和复合旋转理解能力，揭示了其内部3D空间表征的不足。该成果对机器人控制、3D场景重建和人机交互有重要启示。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T10:01:56Z
- **目录日期**: 2025-06-05
