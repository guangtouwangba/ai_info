...
该研究提出SGD-SaI方法，质疑深度学习中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，无需二阶动量即可避免训练不平衡，并将内存占用减半。实验表明，在Transformer任务、ImageNet分类及GPT-2/Llama2预训练中，其性能匹配或超越AdamW，且对超参数更鲁棒。内存效率显著，如为Llama2-7B节省25.15GB。在LoRA微调等场景也优于现有优化器，为训练大模型提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-05T23:04:33Z
- **目录日期**: 2025-06-05
