...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡问题，同时将优化器内存占用减半。实验表明，该方法在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或媲美AdamW，特别在LLM训练中显著节省内存（GPT-2节省5.93GB，Llama2-7B节省25.15GB），且在LoRA微调等任务中也优于现有优化器，展现出优异的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T07:05:05Z
- **目录日期**: 2025-06-10
