...
该研究提出GUI-Reflection框架，通过预训练、监督微调和在线反思调优三阶段训练，赋予多模态大语言模型(GUI模型)自我反思与纠错能力。创新点包括：1)自动从成功轨迹生成反思数据，构建专门任务套件；2)搭建移动端在线训练环境；3)开发迭代式在线调优算法。该方法无需人工标注，实现了全自动数据生成和学习，显著提升了GUI代理的容错性和适应性。所有数据、模型和环境工具将开源，为智能GUI自动化提供新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T09:01:25Z
- **目录日期**: 2025-06-10
