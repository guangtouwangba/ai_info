...
本文提出GUI-Reflection框架，通过三个训练阶段（预训练、监督微调、在线反思调优）赋予多模态大语言模型自我反思和纠错能力，以提升图形界面(GUI)自动化性能。该框架创新性地采用全自动数据生成流程：1)从成功轨迹自动构建反思数据集；2)搭建移动端在线训练环境；3)设计迭代式在线调优算法。相比现有仅关注基础理解的GUI模型，该方案首次系统性地解决了错误恢复问题，无需人工标注即可实现反思能力的涌现。所有数据、模型和环境工具将开源，推动更鲁棒、自适应的GUI自动化发展。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T10:00:59Z
- **目录日期**: 2025-06-10
