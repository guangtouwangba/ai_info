...
该研究提出了GUI-Reflection框架，通过多阶段训练（预训练、监督微调和在线反思调优）增强多模态大语言模型在GUI自动化中的自我反思和纠错能力。该框架采用全自动数据生成流程：1）从成功轨迹自动构建反思数据，并设计专门任务组评估反思能力；2）搭建移动设备在线训练环境；3）开发迭代式在线调优算法持续提升模型性能。该方法无需人工标注，赋予GUI代理自主纠错能力，推动更鲁棒、自适应的界面自动化，所有数据、模型和环境工具将开源发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T06:04:44Z
- **目录日期**: 2025-06-10
