...
该研究提出了GUI-Reflection框架，通过三个阶段（预训练、监督微调和在线反思调优）赋予多模态大语言模型自我反思和纠错能力，以提升GUI自动化性能。该框架创新性地采用全自动数据生成方法：1) 从成功轨迹自动构建反思数据集；2) 开发移动设备在线训练环境；3) 提出迭代式在线调优算法。相比依赖完美离线轨迹的传统方法，该方案无需人工标注即可实现错误恢复能力，并设计了专门评估反思能力的任务集。研究将公开所有数据、模型和环境，推动更鲁棒、自适应的GUI自动化发展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T21:01:13Z
- **目录日期**: 2025-06-10
