...
本文提出了GUI-Reflection框架，旨在增强多模态大语言模型（MLLMs）在图形用户界面（GUI）自动化中的自我反思和纠错能力。该框架通过三个阶段（预训练、监督微调、在线反思调优）实现全自动数据生成和学习，无需人工标注。研究者设计了可扩展的数据管道自动生成反思与纠错数据，开发了移动设备在线训练环境，并提出迭代式在线调优算法持续提升模型能力。该框架显著提升了GUI代理的鲁棒性和适应性，相关数据、模型、环境及工具将全部开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T20:01:00Z
- **目录日期**: 2025-06-10
