...
本文提出GUI-Reflection框架，通过三个阶段（预训练、监督微调、在线反思调优）赋予多模态GUI模型自我反思与纠错能力。该框架采用全自动数据生成流程：1）从成功轨迹自动构建反思数据，设计专门任务集评估反思能力；2）搭建移动设备在线训练环境；3）提出迭代式在线调优算法持续提升模型能力。相比依赖无错误离线轨迹的传统方法，该方案无需人工标注即可实现自主错误恢复，推动GUI自动化向更鲁棒、自适应的方向发展。所有数据、模型及工具将开源发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T18:01:00Z
- **目录日期**: 2025-06-10
