...
该研究提出了GUI-Reflection框架，通过三个阶段（预训练、监督微调、在线反思调优）为多模态GUI模型赋予自我反思和纠错能力。该框架完全自动化生成反思数据，无需人工标注：1）从成功轨迹自动构建反思数据集，并设计专门任务套件评估反思能力；2）建立移动设备在线训练环境；3）开发迭代式在线调优算法持续提升模型性能。该技术使GUI代理具备错误恢复能力，推动更鲁棒、自适应的界面自动化发展。所有数据、模型和环境工具将开源发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T23:00:52Z
- **目录日期**: 2025-06-10
