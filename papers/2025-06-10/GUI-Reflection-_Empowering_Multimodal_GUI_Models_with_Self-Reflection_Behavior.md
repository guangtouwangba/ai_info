...
该研究提出了GUI-Reflection框架，通过三个阶段（预训练、监督微调和在线反思调优）将自我反思和纠错能力集成到多模态GUI模型中。该框架采用全自动数据生成流程，无需人工标注：1) 从成功轨迹自动构建反思数据，并开发GUI-Reflection任务套件专门评估反思能力；2) 建立移动设备在线训练环境；3) 提出迭代式在线反思调优算法持续提升模型能力。该方案使GUI代理具备自我修正功能，推动更鲁棒、自适应的GUI自动化发展，所有数据、模型和环境工具将开源发布。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T17:01:01Z
- **目录日期**: 2025-06-10
