...
该研究提出GUI-Reflection框架，通过三阶段训练（预训练、监督微调、在线反思调优）赋予多模态大语言模型自我反思和纠错能力。框架特点包括：1) 自动从成功轨迹生成反思数据，无需人工标注；2) 创建移动端在线训练环境；3) 开发迭代式在线调优算法。该方法突破了现有GUI模型依赖无错离线轨迹的局限，显著提升了界面自动化的鲁棒性和适应性。所有数据、模型和环境工具将开源发布，为智能GUI自动化开辟新路径。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T14:01:02Z
- **目录日期**: 2025-06-10
