...
这项研究评估了大语言模型（LLMs）在动态对抗辩论中的置信度调整能力。研究发现LLMs存在系统性过度自信（初始平均72.9% vs 理性基准50%）、辩论中置信度不降反升（最终达83%）、61.7%的辩论出现双方同时高估胜率（≥75%）等五大问题。即使明确告知胜率为50%，模型置信度仍从50%升至57.1%。此外，模型的私下推理与公开置信度评分存在偏差。结果表明，LLMs在多轮动态任务中缺乏准确自我评估和更新信念的能力，这对LLMs在助理和代理角色中的应用提出了重要警示。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T11:02:22Z
- **目录日期**: 2025-06-10
