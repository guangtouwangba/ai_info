...
该研究评估了大语言模型(LLMs)在动态对抗辩论中的置信度校准问题。通过组织60场三轮政策辩论，发现五大问题：(1)系统性过度自信(初始平均72.9% vs 理性基准50%)；(2)置信度随辩论升级(最终达83%)；(3)61.7%辩论双方同时声称≥75%胜率；(4)自我辩论偏见(相同模型互辩时置信度从64.1%升至75.2%)；(5)私密推理与公开评分不一致。结果表明LLMs在动态多轮任务中缺乏准确自我评估和更新信念的能力，这对部署为助手或代理构成重大隐患。实验代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T18:02:14Z
- **目录日期**: 2025-06-10
