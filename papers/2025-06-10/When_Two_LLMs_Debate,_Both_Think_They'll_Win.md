...
这项研究评估了大语言模型(LLMs)在对抗性辩论中的置信度调整能力。通过60场三轮政策辩论实验发现：1)模型初始平均置信度72.9%，显著高于理性基准50%；2)随着辩论进行，模型置信度不降反升，最终达83%；3)61.7%的辩论中出现双方同时声称≥75%胜率的逻辑矛盾；4)即使明确告知胜率应为50%，模型仍会提升自信；5)模型的内部推理与公开置信度存在偏差。结果表明LLMs在动态多轮任务中缺乏准确自我评估和信念更新的能力，这对部署为智能助手构成重大隐患。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T15:02:29Z
- **目录日期**: 2025-06-10
