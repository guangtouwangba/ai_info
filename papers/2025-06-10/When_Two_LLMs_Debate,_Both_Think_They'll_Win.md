...
这项研究评估了大语言模型(LLMs)在动态对抗辩论中的置信度调整能力。研究发现五个主要问题：(1)系统性过度自信，初始平均置信度72.9%远高于合理的50%基准；(2)随着辩论进行，模型反而提高胜率评估至83%；(3)61.7%的辩论中双方同时声称≥75%的胜率；(4)即使明确告知胜率为50%，模型仍会提高自信度；(5)模型的私有推理与公开置信度评级不一致。结果表明LLMs在动态多轮任务中缺乏准确自我评估和更新信念的能力，这对LLMs在助理和代理角色中的应用提出了重要警示。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T14:02:20Z
- **目录日期**: 2025-06-10
