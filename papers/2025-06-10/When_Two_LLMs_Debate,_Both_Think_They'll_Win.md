...
该研究评估了大语言模型（LLMs）在动态对抗辩论中的置信度调整能力。实验发现五大问题：(1) 系统性过度自信，初始平均置信度72.9%（理性基线应为50%）；(2) 随辩论进行，模型反而提高胜率评估（最终达83%）；(3) 61.7%的辩论中出现双方同时声称≥75%胜率的逻辑矛盾；(4) 即使明确告知胜率为50%，模型自我辩论时置信度仍从50%升至57.1%；(5) 私有推理与公开置信度存在偏差。结果表明LLMs在多轮动态任务中缺乏准确自我评估和信念更新的能力，这对AI助手的实际部署构成重大隐患。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T04:07:16Z
- **目录日期**: 2025-06-10
