...
香港大学与英伟达合作提出广义空间传播网络（GSPN），革新视觉注意力机制。传统Transformer自注意力处理高分辨率图像时存在计算复杂度高（O(N²)）和破坏空间结构的问题。GSPN通过二维线性传播和稳定性-上下文条件机制，将计算量降至√N级别，同时保留图像空间连贯性。该方法在效率上实现重大突破（生成加速超84倍），并在多项视觉任务中刷新性能纪录，解决了高分辨率图像处理的关键瓶颈。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T18:03:06Z
- **目录日期**: 2025-06-10
