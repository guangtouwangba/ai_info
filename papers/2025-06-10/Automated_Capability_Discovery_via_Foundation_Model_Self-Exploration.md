...
本文提出了自动化能力发现（ACD）框架，利用一个基础模型作为“科学家”，系统地生成开放式任务来评估目标模型（可以是自身）的能力和潜在风险。ACD结合前沿模型和开放式研究思想，自动发现模型多样化的能力与缺陷。实验覆盖GPT、Claude和Llama系列模型，自动生成数千个任务并聚类分析，揭示了人工难以发现的能力范围和失败模式。模型评分与人类评估高度一致，验证了方法的有效性。ACD通过模型自生成任务和自评估，为新型AI系统的自动化评估提供了可扩展方案。相关代码和评估日志已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T18:02:25Z
- **目录日期**: 2025-06-10
