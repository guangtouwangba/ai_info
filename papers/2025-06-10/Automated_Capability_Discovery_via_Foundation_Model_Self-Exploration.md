...
本文提出了自动化能力发现（ACD）框架，利用一个基础模型作为“科学家”，系统地设计开放式任务来评估目标模型的能力和潜在风险。ACD结合前沿模型和开放性思想，自动发现多样化的模型能力与缺陷。实验覆盖GPT、Claude和Llama系列模型，自动生成数千个任务，聚类后揭示数十种能力领域和失败模式，远超人工评估范围。模型评分与人类评估高度一致。ACD通过模型自生成任务和自评估，为新型AI系统的可扩展自动化评估提供了重要进展。代码和评估日志已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T09:03:01Z
- **目录日期**: 2025-06-10
