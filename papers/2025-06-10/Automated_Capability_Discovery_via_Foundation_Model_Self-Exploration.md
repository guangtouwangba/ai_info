...
本文提出自动能力发现框架（ACD），利用一个基础模型作为"科学家"来系统性地生成开放式任务，以探测目标模型（可以是其自身）的能力边界。该方法结合前沿模型和开放式研究思想，能自动发现模型多样化的潜在能力和缺陷。实验覆盖GPT、Claude和Llama系列模型，自动生成数千个任务并聚类分析，揭示了人工难以发现的数十种能力领域和失败模式。人工验证表明模型评分与人类评估高度一致。ACD通过利用基础模型的自主任务生成和评估能力，为AI系统的可扩展自动化评估提供了新途径。所有代码和评估日志已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T08:02:38Z
- **目录日期**: 2025-06-10
