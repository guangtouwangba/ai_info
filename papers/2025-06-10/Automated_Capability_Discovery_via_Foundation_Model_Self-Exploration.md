...
本文提出了自动能力发现（ACD）框架，利用一个基础模型作为“科学家”来系统地生成开放式任务，以探测目标模型（可能是其自身）的能力范围。ACD结合前沿模型和开放式研究思想，自动发现目标模型多样化的能力与缺陷。实验覆盖GPT、Claude和Llama等系列模型，自动生成数千个任务并聚类分析，揭示了单团队难以发现的广泛能力领域和失败模式。人工验证表明模型评分与人类评估高度一致。ACD通过模型自主创建任务和评估，为新型AI系统的可扩展自动化评估迈出重要一步，相关代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T06:06:11Z
- **目录日期**: 2025-06-10
