...
本文提出了自动能力发现（ACD）框架，利用一个基础模型作为“科学家”，系统地生成开放式任务来测试目标模型（可能是自身）的能力。ACD结合前沿模型和开放式方法，自动发现模型多样化的能力和缺陷。实验涵盖GPT、Claude和Llama系列模型，自动生成数千个任务，聚类后揭示数十种能力领域和失败模式，远超人工评估范围。模型评分与人类评估高度一致。ACD通过模型自生成任务和自评估，为新型AI系统的可扩展自动化评估迈出重要一步。代码和评估日志已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T03:24:22Z
- **目录日期**: 2025-06-10
