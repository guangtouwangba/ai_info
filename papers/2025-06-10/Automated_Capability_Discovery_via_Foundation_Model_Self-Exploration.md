...
本文提出自动化能力发现（ACD）框架，利用基础模型作为"科学家"自动生成开放式任务，系统性地评估目标模型（包括自身）的能力和潜在风险。ACD结合前沿模型和开放式方法，能自动发现模型多样化的能力与缺陷。实验涵盖GPT、Claude和Llama系列模型，自动生成数千个任务并聚类分析，揭示传统方法难以发现的数十种能力范围和失败模式。人工验证显示模型评分与人类评估高度一致。ACD通过模型自生成任务和自评估，为AI系统自动化评估提供了新方向。相关代码和评估日志已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T05:02:09Z
- **目录日期**: 2025-06-10
