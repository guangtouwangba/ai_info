...
这篇论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，通过397个涵盖40类编程任务和44种语言的高质量样本，首次系统评估模型输出与人类偏好的匹配度。研究同时构建了20B标记的合成指令数据集SynCode-Instruct用于微调验证。实验发现：1)传统执行基准与人类偏好评估存在显著差异；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显。研究强调了代码生成模型与人类偏好对齐的重要性，为提升实用价值提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T19:05:24Z
- **目录日期**: 2025-06-10
