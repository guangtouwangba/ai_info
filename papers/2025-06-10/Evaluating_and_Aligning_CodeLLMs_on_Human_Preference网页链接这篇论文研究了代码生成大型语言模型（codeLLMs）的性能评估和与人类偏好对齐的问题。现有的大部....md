...
该研究针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，包含397个真实编程任务样本(覆盖40类/44种语言)，首次系统评估模型输出与人类偏好的匹配度。研究同时构建20B标记的合成指令数据集SynCode-Instruct验证微调效果。实验发现：1)传统执行正确性基准与人类偏好存在偏差；2)开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人性化输出方面仍有显著差距。该工作凸显了代码LLMs与人类偏好对齐的重要性，为后续优化提供新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T22:05:43Z
- **目录日期**: 2025-06-10
