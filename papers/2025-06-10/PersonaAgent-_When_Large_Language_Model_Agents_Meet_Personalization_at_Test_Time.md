...
PersonaAgent是首个个性化LLM智能体框架，通过整合个性化记忆模块（情景与语义记忆）和个性化行动模块（定制工具操作），动态适应用户需求。其核心机制是以用户专属的"角色提示"为中介，利用记忆信息控制行动，并通过行动结果优化记忆。该框架采用测试时用户偏好对齐策略，通过模拟最近n次交互并基于文本损失反馈优化角色提示，实现实时偏好匹配。实验表明，PersonaAgent在动作空间个性化及实际应用扩展性方面显著优于基线方法，验证了提供动态定制用户体验的可行性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T01:28:36Z
- **目录日期**: 2025-06-10
