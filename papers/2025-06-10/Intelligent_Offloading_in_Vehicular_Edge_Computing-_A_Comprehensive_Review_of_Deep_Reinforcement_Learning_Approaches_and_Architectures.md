...
本文综述了基于深度强化学习（DRL）的车载边缘计算（VEC）卸载策略的最新进展。随着智能交通系统（ITS）日益复杂，传统卸载方法难以应对动态异构环境，DRL因其自适应决策能力受到关注。文章从学习范式（单/多智能体）、系统架构（集中式/分布式/分层）和优化目标（延迟/能耗/公平性）等维度对现有研究进行分类比较，分析了马尔可夫决策过程（MDP）的应用，并探讨了奖励设计、协调机制和可扩展性等新兴趋势。最后指出了开放挑战和未来研究方向，以推动下一代ITS智能卸载策略的发展。（100字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T06:06:21Z
- **目录日期**: 2025-06-10
