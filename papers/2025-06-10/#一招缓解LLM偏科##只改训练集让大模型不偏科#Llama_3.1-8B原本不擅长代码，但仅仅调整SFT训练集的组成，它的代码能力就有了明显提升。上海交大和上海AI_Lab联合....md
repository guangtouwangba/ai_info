...
上海交大与上海AI Lab联合提出IDEAL方法，通过科学调整SFT训练集的数据配比（而非单纯增加数据量）显著提升模型能力。实验显示，Llama 3.1-8B在代码任务上的表现因数据比例优化而明显改善。研究发现：1）数据量过多可能损害性能；2）不当配比会导致能力退化。该方法通过理论建模评估领域数据影响，利用K-FAC技术高效计算Hessian逆矩阵，并通过超参数m动态调整数据比例，实现更均衡的训练效果。核心突破在于"重质不重量"的数据组成优化策略。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T13:15:04Z
- **目录日期**: 2025-06-10
