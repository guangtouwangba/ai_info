...
上海交大与上海AI Lab联合提出IDEAL方法，通过科学调整SFT训练集的数据配比（而非单纯增加数据量）显著提升模型能力。实验显示，Llama 3.1-8B在代码任务上的表现因优化数据比例而明显改善。该方法通过理论建模评估领域数据影响，采用K-FAC技术高效计算Hessian逆矩阵，并引入超参数m动态调整数据权重，避免传统SFT中数据过量导致的性能退化问题，实现更均衡的多领域能力提升。核心发现：数据质量配比优于数量堆砌。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T21:04:00Z
- **目录日期**: 2025-06-10
