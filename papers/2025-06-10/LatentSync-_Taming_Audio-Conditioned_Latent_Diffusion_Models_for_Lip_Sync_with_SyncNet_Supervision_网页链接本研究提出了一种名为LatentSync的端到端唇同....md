...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。为解决扩散模型的时间一致性问题，创新性地提出TREPA方法，通过自监督视频模型的时间表示对齐生成帧。同时针对SyncNet收敛问题，系统分析了模型架构、超参数和数据预处理等关键因素，改进训练框架提升了准确性。该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验可推广至其他音频驱动动画任务。研究突破了传统两阶段生成方法的限制，直接建模复杂的音视频关联。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T07:06:07Z
- **目录日期**: 2025-06-10
