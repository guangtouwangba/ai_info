...
当前测试时扩展范式主要依赖生成长推理轨迹（“多思考”）再行动，但这种方法无法让智能体从环境中获取新信息或动态调整行为。本文提出测试时交互扩展这一新维度，通过延长交互时间实现探索、回溯和动态重规划等行为。研究以网页智能体为例，发现仅通过提示式交互扩展就能显著提升任务成功率。进一步提出TTI（测试时交互）方法，采用基于课程的在线强化学习自适应调整交互长度。实验显示，基于Gemma 3 12B模型的TTI在WebVoyager和WebArena基准上达到开源最优水平，能自适应平衡探索与利用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T08:02:03Z
- **目录日期**: 2025-06-10
