...
当前测试时扩展范式主要依赖生成长推理轨迹（"多思考"）再行动，但无法让智能体从环境获取新信息或动态调整行为。本文提出测试时交互扩展这一新维度，通过延长交互时间来实现探索、回溯和动态重规划等复杂行为。研究聚焦网页智能体领域，发现仅通过提示式交互扩展就能显著提升任务成功率。基于此，作者开发了TTI（测试时交互）方法，采用课程式在线强化学习自适应调整行动轨迹。实验表明，Gemma 3 12B模型配合TTI在WebVoyager和WebArena基准测试中创造了开源最优性能，验证了交互扩展作为计算扩展补充维度的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T19:01:41Z
- **目录日期**: 2025-06-10
