...
当前测试时扩展方法主要依赖生成长推理链（“多思考”）再行动，但无法在交互中获取新信息或动态调整行为。本文提出测试时交互扩展（TTI），通过延长交互时间实现探索、回溯和动态重规划。研究以网页代理为例，发现仅通过提示扩展交互即可提升任务成功率。进一步提出基于课程的在线强化学习TTI方法，自适应调整交互长度。使用Gemma 3 12B模型，TTI在WebVoyager和WebArena基准上创造了开源最佳表现。结果表明，交互扩展是计算扩展的有效补充维度，为训练自适应代理开辟了新途径。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T04:06:57Z
- **目录日期**: 2025-06-10
