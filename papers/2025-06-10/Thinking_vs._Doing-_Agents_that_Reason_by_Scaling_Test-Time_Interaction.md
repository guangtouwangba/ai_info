...
当前测试时扩展方法主要依赖生成长推理轨迹（“多思考”）来提升表现，但在需要交互的智能体任务中，这种方式无法获取环境新信息或动态调整行为。本研究提出测试时交互扩展（TTI），通过延长交互时间轴支持探索、回溯和动态重规划等行为。以网页智能体为例，实验表明仅通过提示式交互扩展即可显著提升任务成功率。进一步提出的TTI方法采用课程式在线强化学习，自适应调整推演长度，基于Gemma 3 12B模型在WebVoyager和WebArena基准上取得开源最优效果，证明了交互扩展是计算扩展的有效补充维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T21:01:55Z
- **目录日期**: 2025-06-10
