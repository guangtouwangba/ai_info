...
当前测试时扩展范式主要依赖生成长推理轨迹（"多思考"）再行动，但这种方法无法让智能体从环境中获取新信息或动态调整行为。本研究提出"测试时交互扩展"新维度，通过延长交互时间实现探索、回溯和动态重规划等行为。在网页智能体领域，实验显示仅通过提示式交互扩展就能显著提升任务成功率。进一步提出的TTI（测试时交互）方法采用课程式在线强化学习，自适应调整行动轨迹长度。基于Gemma 3 12B模型的TTI在WebVoyager和WebArena基准测试中创造了开源开放数据的SOTA表现，证明了交互扩展是计算扩展的有效补充维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T06:05:41Z
- **目录日期**: 2025-06-10
