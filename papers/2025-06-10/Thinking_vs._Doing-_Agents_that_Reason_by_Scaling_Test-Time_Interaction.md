...
当前测试时扩展范式主要依赖生成长推理链（"多思考"）再行动，但这种方法无法让智能体从环境中获取新信息或动态调整行为。本研究提出"测试时交互扩展"新维度，通过延长交互时间实现探索、回溯和动态重规划等复杂行为。在网页智能体领域，实验表明仅通过提示式交互扩展就能显著提升任务成功率。进一步提出的TTI方法（基于课程学习的在线强化学习）能自适应调整交互长度，使用Gemma 3 12B模型在WebVoyager和WebArena基准上达到开源最优性能，证明交互扩展是计算扩展的有效补充维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T18:01:52Z
- **目录日期**: 2025-06-10
