...
当前测试时扩展范式主要依赖生成长推理轨迹（"多思考"）再行动，但无法让智能体从环境中获取新信息或动态调整行为。本文提出测试时交互扩展这一新维度，通过延长交互时间实现探索、回溯和动态重规划等复杂行为。研究聚焦网页智能体领域，发现仅通过提示驱动的交互扩展即可显著提升任务成功率。进一步提出TTI（测试时交互）方法，采用课程式在线强化学习自适应调整交互长度。基于Gemma 3 12B模型的TTI在WebVoyager和WebArena基准测试中达到开源最优水平，证明交互扩展是计算扩展的有效补充维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T07:01:53Z
- **目录日期**: 2025-06-10
