...
本文提出了一种名为DesRUTGe的新型城市交通模拟框架，通过结合深度强化学习(DRL)和SUMO模拟器，生成高保真的24小时交通模式。该框架采用去中心化联邦学习(DFL)方法，每个交通检测器及其对应区域作为独立学习节点，仅需少量历史数据进行本地DRL模型训练，并通过与相邻区域交换模型参数进行协作优化，无需中央协调器。基于巴塞罗那真实数据的测试表明，DesRUTGe在生成准确性、可扩展性和隐私保护方面优于传统SUMO工具和集中式学习方法，为城市规划和智能交通系统提供了更优解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T06:05:32Z
- **目录日期**: 2025-06-10
