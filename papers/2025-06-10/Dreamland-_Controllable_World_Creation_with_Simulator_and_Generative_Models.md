...
Dreamland是一个结合物理模拟器精确控制和大规模生成模型真实输出的混合世界生成框架。它通过分层世界抽象编码像素级和物体级的语义与几何信息，作为连接两者的中间表示，从而提升可控性并降低适配成本。该框架支持直接使用现有及未来的预训练生成模型。研究还构建了D3Sim数据集用于训练和评估混合生成流程。实验表明，Dreamland在图像质量上提升50.8%，可控性增强17.9%，并能有效促进具身智能体训练。代码和数据将公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-10T03:23:12Z
- **目录日期**: 2025-06-10
