...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。针对扩散模型中时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示增强帧间一致性。同时，通过系统研究改进了SyncNet训练框架，提升了其收敛性和准确性。该方法在HDTF和VoxCeleb2数据集上表现优异，超越了现有唇同步技术。这些改进不仅适用于本框架，也可推广至其他基于SyncNet的音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T03:22:33Z
- **目录日期**: 2025-04-20
