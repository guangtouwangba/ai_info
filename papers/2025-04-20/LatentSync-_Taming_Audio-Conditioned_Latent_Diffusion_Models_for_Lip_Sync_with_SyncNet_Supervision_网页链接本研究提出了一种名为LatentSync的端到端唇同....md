...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模复杂音视频关系，但面临时间一致性问题。为此，研究者开发了TREPA方法，通过自监督视频模型的时间表征对齐生成帧，提升一致性。同时，针对SyncNet收敛问题进行了系统研究，改进了模型架构、训练参数和数据预处理，显著提高了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进也可推广至其他基于SyncNet的唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T22:05:15Z
- **目录日期**: 2025-04-20
