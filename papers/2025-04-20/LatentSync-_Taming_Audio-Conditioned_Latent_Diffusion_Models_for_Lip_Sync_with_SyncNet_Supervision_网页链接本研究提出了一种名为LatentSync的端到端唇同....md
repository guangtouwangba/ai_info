...
本研究提出LatentSync框架，通过音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。针对扩散模型的时间一致性问题，开发了TREPA方法，利用自监督视频模型的时间表示增强连贯性。同时优化了SyncNet训练框架，解决了收敛问题并提升准确性。在HDTF和VoxCeleb2数据集上的实验表明，该方法在多项指标上超越现有技术。创新点包括：1）直接建模音视频关联的扩散模型；2）保持同步精度的时序对齐技术；3）改进SyncNet训练的关键因素分析。成果可推广至其他人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T19:05:13Z
- **目录日期**: 2025-04-20
