...
在差分隐私的序列学习中，代理通过内生噪声保护隐私。对于连续信号，本文提出一种基于阈值距离的自适应噪声机制（平滑随机响应），相比传统均匀噪声，能更好结合私有信号和历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算下，首次正确决策的停止时间和错误动作次数均保持有限，而传统场景下二者发散。连续信号下隐私学习更高效，因平滑响应增强了对数似然比；但二进制信号中隐私噪声会阻碍学习，因代理在信息级联前倾向恒定随机响应策略，降低动作信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T17:02:21Z
- **目录日期**: 2025-04-20
