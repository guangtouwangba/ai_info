...
在差分隐私的序贯学习中，代理人通过内生噪声保护隐私。针对连续信号，研究提出了一种基于阈值距离的自适应噪声机制（平滑随机响应），相比传统均匀噪声方法，能更高效聚合信息，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$）。有限隐私预算下，首个正确决策的停止时间和错误动作次数均有限，而完全非隐私场景下二者发散。连续信号在隐私机制下学习效率更高，但二元信号因隐私噪声易形成信息级联，导致动作信息量下降，阻碍学习进程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T12:02:24Z
- **目录日期**: 2025-04-20
