...
在差分隐私的序贯学习中，代理通过内生噪声保护隐私。针对连续信号，本文提出基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声，能更高效结合私有信号与历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（隐私预算有限时），优于非隐私场景的$\Theta(\sqrt{\log(n)})$。隐私预算$\epsilon \in (0,1)$还能避免早期代理长期错误决策，使首正确决策时间和错误次数保持有限。连续信号下，平滑机制通过增强对数似然比提升信息聚合效率；而二元信号中，隐私噪声会阻碍学习，因代理在信息级联前倾向恒定随机响应策略，降低动作信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T00:02:15Z
- **目录日期**: 2025-04-20
