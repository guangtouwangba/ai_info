...
在差分隐私的序贯学习场景中，智能体通过内生噪声保护隐私。研究发现，连续信号和二元信号的学习效果存在显著差异：对于连续信号，提出的平滑随机响应机制能根据阈值距离自适应添加噪声，使动作更准确反映私有信号和历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（非隐私场景为$\Theta(\sqrt{\log(n)})$），且首个正确决策的停止时间和错误动作次数保持有限。而二元信号下，隐私噪声会阻碍学习，因智能体在信息级联形成前倾向于使用恒定随机响应策略，降低动作信息量，拖累整体学习效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T03:17:49Z
- **目录日期**: 2025-04-20
