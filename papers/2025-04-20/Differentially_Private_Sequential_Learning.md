...
在差分隐私的序贯学习中，代理通过添加噪声保护隐私。针对连续信号，研究提出基于阈值距离的自适应噪声机制（平滑随机响应），相比传统均匀噪声，能更有效结合私有信号和历史观察，将渐近学习速度提升至Θε(log(n))（非隐私场景为Θ(√log(n))）。有限隐私预算ε∈(0,1)下，首次正确决策时间和错误动作次数保持有限，避免了非隐私场景的长期早期错误。连续信号下隐私学习更高效，因平滑响应增强了对数似然比；而二元信号中隐私噪声会阻碍学习，因代理在信息级联前采用恒定随机响应策略，降低动作信息量。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T15:02:24Z
- **目录日期**: 2025-04-20
