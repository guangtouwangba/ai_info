...
在差分隐私的序贯学习模型中，代理通过内生噪声保护隐私。对于连续信号，本文提出一种基于阈值距离的自适应平滑随机响应机制，相比传统均匀噪声方法，能更高效结合私有信号和历史观察，将渐近学习速度提升至$\Theta_{\epsilon}(\log(n))$（隐私预算$\epsilon>0$时），优于非隐私场景的$\Theta(\sqrt{\log(n)})$。有限隐私预算还能避免早期代理长期错误决策的问题。然而，对于二元信号，隐私噪声会阻碍学习，因代理在信息级联形成前倾向于使用恒定随机响应策略，降低动作信息量。连续信号在隐私场景下展现出更高学习效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T11:02:09Z
- **目录日期**: 2025-04-20
