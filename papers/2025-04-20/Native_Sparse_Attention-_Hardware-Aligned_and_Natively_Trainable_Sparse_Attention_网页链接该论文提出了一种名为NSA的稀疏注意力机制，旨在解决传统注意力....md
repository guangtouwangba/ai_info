...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在降低传统注意力机制处理长文本时的高计算成本。NSA结合算法创新与硬件优化，采用动态分层稀疏策略，通过粗粒度令牌压缩和细粒度令牌选择，兼顾全局上下文感知与局部精度。其核心创新点包括硬件对齐设计和原生可训练性，显著提升了长文本建模效率。这一方法为大规模语言模型的高效计算提供了新思路，同时保持了模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T23:04:44Z
- **目录日期**: 2025-04-20
