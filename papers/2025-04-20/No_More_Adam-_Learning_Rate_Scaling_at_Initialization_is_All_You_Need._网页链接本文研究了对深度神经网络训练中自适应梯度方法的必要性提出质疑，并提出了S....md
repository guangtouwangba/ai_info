...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI通过初始化时基于梯度信号噪声比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡的问题。相比AdamW，它不仅将内存使用减半，还在Transformer任务、ImageNet分类和LLM预训练中表现相当或更优。实验显示，该方法为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，且在LoRA微调等任务中超越现有优化器，具有更强的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T03:21:05Z
- **目录日期**: 2025-04-20
