...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。SGD-SaI是对带动量随机梯度下降(SGDM)的改进，通过初始化时依据梯度信号噪声比(g-SNR)调整不同参数组的学习率，无需自适应二阶动量即可避免训练不平衡。相比AdamW，它节省50%内存，在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优异。具体节省：GPT-2节省5.93GB，Llama2-7B节省25.15GB内存。该方法解决了SGD训练Transformer的难题，具有超参数鲁棒性和广泛应用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T12:05:02Z
- **目录日期**: 2025-04-20
