...
该研究介绍了AgentSociety，这是一个基于大型语言模型的大规模社会模拟器，能够模拟超过1万个代理的社会互动（共500万次）。研究者利用该平台研究了四个关键社会问题：社会极化、信息传播、基本收入政策影响和自然灾害冲击。模拟结果与现实世界数据高度吻合，验证了该工具在捕捉人类行为机制方面的有效性。这一创新方法为社会科学研究和政策制定提供了新的实验平台，能够补充传统研究方法（如调查和访谈），帮助深入理解复杂社会问题的模式和成因。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T05:05:35Z
- **目录日期**: 2025-04-20
