...
复旦大学与字节Seed团队合作推出SimpleAR模型，采用纯自回归架构（0.5B参数）实现1024分辨率图像生成，无需外挂文本编码器。该模型通过decoder-only Transformer统一处理图文token，在GenEval评测达0.59分，超越同规模SDv2.1和LlamaGen。生成速度优化后仅需13.55秒/图，效率优于部分扩散模型。研究指出性能差距主要源于训练数据与视觉分词器限制，仍有提升空间。成果展示了自回归路径在跨模态生成任务中的潜力。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T00:02:48Z
- **目录日期**: 2025-04-20
