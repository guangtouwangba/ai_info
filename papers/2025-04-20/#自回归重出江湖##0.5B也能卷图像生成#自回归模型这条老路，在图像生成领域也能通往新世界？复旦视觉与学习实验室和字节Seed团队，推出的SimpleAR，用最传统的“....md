...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构（仅0.5B参数），通过统一处理文本和图像token实现跨模态对齐，在1024分辨率图像生成中表现突出：GenEval得分0.59，超越同规模SDv2.1和LlamaGen，生成耗时仅13.55秒。该模型不依赖外部文本编码器，通过优化采样步数平衡性能与速度，虽在1.5B规模下与顶尖模型仍有差距，但凸显了训练数据和视觉分词器的改进潜力。研究证明传统自回归方法在图像生成领域仍具竞争力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T06:03:04Z
- **目录日期**: 2025-04-20
