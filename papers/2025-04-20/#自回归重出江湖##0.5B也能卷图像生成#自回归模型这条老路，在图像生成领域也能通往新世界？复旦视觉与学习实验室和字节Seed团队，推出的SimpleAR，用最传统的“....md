...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数即可生成1024分辨率图像，在GenEval评测中取得0.59分，超越同规模SDv2.1和LlamaGen。该模型通过统一处理文本/图像token实现跨模态对齐，无需外挂文本编码器。部署后生成时间仅13.55秒，通过采样优化减少生成步数，性能接近MaskGIT。研究指出当前限制主要来自训练数据和视觉分词器，仍有提升空间。这项工作证明了自回归模型在图像生成领域的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T17:03:54Z
- **目录日期**: 2025-04-20
