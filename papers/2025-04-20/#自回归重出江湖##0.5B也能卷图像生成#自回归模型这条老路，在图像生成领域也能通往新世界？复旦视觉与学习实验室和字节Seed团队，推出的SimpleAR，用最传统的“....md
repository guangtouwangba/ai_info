...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，在仅0.5B参数规模下实现1024分辨率图像生成，性能超越同规模SDv2.1和LlamaGen模型。该模型通过统一处理文本/图像token实现跨模态对齐，无需外挂文本编码器，在GenEval评测中获得0.59高分。部署后生成1024图像仅需13.55秒，通过采样优化兼顾速度与质量。研究指出当前限制主要来自训练数据和视觉tokenizer，仍有提升空间。这项工作证明了自回归方法在图像生成领域的持续竞争力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T10:03:09Z
- **目录日期**: 2025-04-20
