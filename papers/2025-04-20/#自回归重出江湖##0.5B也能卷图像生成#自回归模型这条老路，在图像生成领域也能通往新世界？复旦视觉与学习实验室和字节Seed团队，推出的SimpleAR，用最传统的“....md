...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数就实现了1024分辨率图像生成。该模型通过统一处理文本和图像token实现跨模态对齐，在GenEval评测中获得0.59高分，超越同规模SDv2.1和LlamaGen。部署后生成1024图像仅需13.55秒，通过采样优化进一步提升了效率。研究表明当前性能瓶颈主要来自训练数据和视觉tokenizer限制，仍有提升空间。这项成果证明了自回归方法在图像生成领域的潜力。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T03:18:46Z
- **目录日期**: 2025-04-20
