...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数就实现了1024分辨率图像生成。该模型通过统一的decoder处理文本和图像token，在GenEval基准上取得0.59的高分，超越同规模SDv2.1和LlamaGen。配合vLLM部署后，生成时间仅需13.55秒，并通过采样优化提升效率。研究指出当前限制主要来自训练数据和视觉tokenizer，仍有改进空间。这项工作证明传统自回归方法在跨模态生成任务中仍具竞争力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T04:02:56Z
- **目录日期**: 2025-04-20
