...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，仅用0.5B参数即可生成1024分辨率图像。该模型通过统一处理文本和图像token实现跨模态对齐，无需依赖外部文本编码器。在GenEval基准测试中取得0.59分，超越同规模SDv2.1和LlamaGen。部署后生成1024图像仅需13.55秒，通过采样优化实现高效推理。研究指出当前性能差距主要源于训练数据和视觉tokenizer限制，仍有提升空间。相关成果已开源，展示了自回归模型在图像生成领域的新潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T08:03:10Z
- **目录日期**: 2025-04-20
