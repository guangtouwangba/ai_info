...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用传统自回归方法（Next-token prediction），仅用0.5B参数实现1024分辨率图像生成，并在文生图任务中表现优异。该模型通过纯decoder-only Transformer统一处理文本与图像token，跨模态对齐高效。在GenEval评测中得分0.59，超越同规模SDv2.1和LlamaGen，生成1024图像仅需13.55秒。虽1.5B规模下性能暂不及Infinity，但研究者指出数据与视觉tokenizer仍有优化空间。模型兼顾速度与质量，部分表现优于MaskGIT。论文与代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T02:32:39Z
- **目录日期**: 2025-04-20
