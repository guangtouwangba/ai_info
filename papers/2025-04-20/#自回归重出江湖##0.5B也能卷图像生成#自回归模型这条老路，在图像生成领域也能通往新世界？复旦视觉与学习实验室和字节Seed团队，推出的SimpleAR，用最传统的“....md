...
复旦大学与字节Seed团队合作开发的SimpleAR模型，采用纯自回归Transformer架构，在仅0.5B参数规模下实现了1024分辨率图像生成。该模型通过统一处理文本和图像token实现跨模态对齐，不依赖外部文本编码器。在GenEval评测中取得0.59分，超越同规模SDv2.1和LlamaGen。经vLLM优化后生成时间仅需13.55秒，并通过采样优化减少生成步数。研究指出当前性能差距主要源于训练数据和视觉tokenizer限制，显示自回归方法在图像生成领域仍具潜力。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T13:08:36Z
- **目录日期**: 2025-04-20
