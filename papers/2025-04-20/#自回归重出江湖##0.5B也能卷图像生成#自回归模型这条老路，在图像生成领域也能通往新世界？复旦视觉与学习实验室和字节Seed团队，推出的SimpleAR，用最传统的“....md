...
复旦大学与字节Seed团队开发的SimpleAR模型，采用纯自回归Transformer架构，在仅0.5B参数规模下实现1024分辨率图像生成。该模型通过统一处理文本/图像token实现跨模态对齐，无需外挂文本编码器，在GenEval评测达到0.59分，超越同规模SDv2.1和LlamaGen。配合vLLM部署后生成仅需13.55秒，通过采样优化在速度上优于MaskGIT。研究指出当前1.5B规模与SOTA的差距主要源于训练数据与视觉分词器限制，但已证明自回归路径在图像生成领域的潜力。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T15:03:52Z
- **目录日期**: 2025-04-20
