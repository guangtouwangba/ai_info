...
具身智能的发展面临现实数据采集成本高的问题，合成数据成为新方向。目前有两种技术路线：一是视频合成+3D重建，但误差积累且难以控制；二是端到端3D生成，但缺乏常识导致空间不合理。为解决这些问题，Sengine SimHub提出模态编码方法，将建筑规则编码进AI生成器，结合强化学习优化布局合理性。该技术旨在生成既真实又可用的三维空间，突破当前具身智能数据瓶颈。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T16:03:07Z
- **目录日期**: 2025-04-20
