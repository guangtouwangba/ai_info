...
具身智能发展面临现实数据采集成本高的问题，合成数据成为解决方案。当前主要有两种技术路线：1）视频合成+3D重建，但存在误差累积问题；2）端到端3D生成，但缺乏常识导致空间不合理。Sengine SimHub提出新思路，通过模态编码将建筑规则（如"餐桌靠近厨房"）融入AI生成过程，结合强化学习优化空间结构。该方案旨在解决传统方法中语义缺失和布局不合理的问题，推动具身智能在复杂室内环境中的应用发展。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T17:03:06Z
- **目录日期**: 2025-04-20
