...
具身智能发展面临现实数据采集难题，合成数据成为关键解决方案。当前主要有两种技术路线：1）视频合成+3D重建，存在误差积累和控制精度问题；2）端到端3D生成，易产生不合理空间。室内场景生成仍是瓶颈，现有方法缺乏常识导致布局不合理。新思路采用模态编码，将专业规则（如"餐桌靠近厨房"）转化为可学习结构，结合强化学习优化空间合理性。Sengine SimHub等系统正探索这一方向，旨在生成既真实又实用的三维环境。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T21:02:52Z
- **目录日期**: 2025-04-20
