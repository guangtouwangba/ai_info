...
具身智能发展面临现实数据采集难题，合成数据成为关键解决方案。当前主要有两种技术路线：1）视频合成+3D重建，存在误差累积和控制精度问题；2）端到端3D生成，易产生不合理空间。室内场景尤其缺乏有效仿真平台。新思路采用模态编码，将专业规则（如"餐桌靠近厨房"）融入生成器，结合强化学习优化布局合理性。代表系统Sengine SimHub通过编码建筑师经验，提升生成空间的实用性和稳定性，解决了传统方法缺乏常识的问题。这一创新为具身智能数据生成提供了更可靠的技术路径。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T22:02:42Z
- **目录日期**: 2025-04-20
