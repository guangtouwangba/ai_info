...
具身智能发展面临现实数据采集难题，合成数据成为关键解决方案。目前存在两条技术路线：1）视频合成+3D重建，但存在误差累积和控制精度问题；2）端到端3D生成，易产生不合理空间。核心挑战在于如何生成真实可控的三维空间。新思路采用模态编码，将专业规则（如"餐桌靠近厨房"）编码进生成器，结合强化学习优化空间结构。代表方案Sengine SimHub通过融合建筑师经验和AI学习，提升空间合理性，解决传统方法缺乏常识的问题。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T18:02:54Z
- **目录日期**: 2025-04-20
