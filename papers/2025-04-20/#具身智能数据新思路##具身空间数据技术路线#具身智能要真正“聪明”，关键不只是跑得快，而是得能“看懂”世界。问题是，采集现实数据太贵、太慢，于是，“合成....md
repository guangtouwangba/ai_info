...
具身智能发展面临现实数据采集难题，合成数据成为关键解决方案。目前主要有两种技术路线：一是视频合成+3D重建，但存在误差累积和控制精度问题；二是端到端3D生成，虽先进但缺乏常识易产生不合理空间。突破方向是模态编码，如Sengine SimHub将建筑规则编码进AI，结合强化学习优化空间结构。当前挑战在于如何生成真实可控的三维环境，尤其是复杂室内场景仍缺乏互联网规模的数据支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T20:02:48Z
- **目录日期**: 2025-04-20
