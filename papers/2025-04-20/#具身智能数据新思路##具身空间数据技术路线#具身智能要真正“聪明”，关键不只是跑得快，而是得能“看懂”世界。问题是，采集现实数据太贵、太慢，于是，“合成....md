...
具身智能发展面临现实数据采集难题，合成数据成为关键解决方案。目前主要有两种技术路线：1）视频合成+3D重建，存在误差累积和控制精度问题；2）端到端3D生成，易产生不合理空间。核心挑战在于如何生成真实可控的三维空间。新思路采用模态编码，将专业规则（如"餐桌靠近厨房"）转化为可学习结构，结合强化学习优化空间合理性。代表性方案Sengine SimHub通过编码建筑师经验，提升生成空间的实用性和稳定性，为解决具身智能数据问题提供了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T15:03:01Z
- **目录日期**: 2025-04-20
