...
研究表明，大型语言模型（LLMs）作为智能体在多轮推理任务中表现优异。尽管拒绝采样微调（RFT）能通过模仿专家成功轨迹提升性能，但其倾向于简单场景，导致复杂子任务难以解决。新方法EEF通过分析专家失败轨迹，提取有价值的指导信息（如关键行动和计划），并剔除有害动作以避免干扰学习。实验显示，EEF成功解决了部分RFT无法处理的子任务，在WebShop任务中以62%胜率超越RFT（53.6%）和GPT-4（35.6%），并在SciWorld中创下81分的新纪录，成为首个在WebShop突破0.81分的方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T14:01:19Z
- **目录日期**: 2025-04-20
