...
该研究提出了一种名为"探索专家失败案例"(EEF)的新方法，用于改进大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)方法主要关注成功轨迹，而EEF创新性地从专家(如GPT-4)的失败轨迹中提取有价值的指导信息(如关键行动和计划)，同时排除有害动作以避免污染模型学习过程。实验表明，EEF能解决RFT无法处理的复杂子任务，在WebShop任务中达到62%的胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下新的最高水平。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T15:01:24Z
- **目录日期**: 2025-04-20
