...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)主要依赖成功轨迹，但专家(如GPT-4)往往在复杂子任务上失败。EEF创新性地从失败的专家轨迹中提取有价值的行为(如关键行动计划)，同时过滤有害动作，将这些有益信息整合到训练数据中。实验表明，EEF能解决RFT无法处理的子任务，在WebShop任务中达到62%的胜率(优于RFT的53.6%和GPT-4的35.6%)，并在WebShop和SciWorld任务中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T05:01:23Z
- **目录日期**: 2025-04-20
