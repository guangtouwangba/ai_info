...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有效动作来提升大语言模型的代理能力。传统拒绝采样微调(RFT)方法仅关注成功轨迹，难以处理复杂子任务。EEF则从失败轨迹中提取有价值的指导信息(如关键行动和计划)，同时剔除有害动作，显著提高了模型探索效率和关键技能获取。实验表明，EEF在WebShop任务中达到62%胜率，超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下新纪录，首次突破0.81和81分。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T13:06:37Z
- **目录日期**: 2025-04-20
