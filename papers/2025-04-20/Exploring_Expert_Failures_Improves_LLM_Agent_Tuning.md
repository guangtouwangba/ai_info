...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)的代理能力。传统拒绝采样微调(RFT)主要依赖专家成功轨迹，但会忽略复杂子任务。EEF通过分析专家失败轨迹中的有益行动(如关键计划和动作)，将其整合到训练数据中，同时排除有害行动以避免负面影响。实验表明，EEF能解决RFT无法处理的子任务，在WebShop任务中达到62%的胜率，超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T17:01:26Z
- **目录日期**: 2025-04-20
