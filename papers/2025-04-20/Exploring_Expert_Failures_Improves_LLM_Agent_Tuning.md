...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)的代理能力。传统基于成功轨迹的拒绝采样微调(RFT)方法难以处理复杂子任务。EEF创新性地从专家失败的轨迹中提取有价值的指导信息(如关键行动和计划)，同时剔除有害动作以避免干扰模型学习。实验表明，EEF成功解决了部分RFT无法处理的子任务，在WebShop任务中达到62%的胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下了超过81分的新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T18:01:20Z
- **目录日期**: 2025-04-20
