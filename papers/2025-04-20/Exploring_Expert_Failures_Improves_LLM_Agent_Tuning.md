...
该研究提出"探索专家失败案例"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有效行动(如关键计划和动作)来提升大语言模型的代理能力。传统拒绝采样微调(RFT)仅关注成功轨迹，难以处理复杂子任务。EEF筛选失败轨迹中有益信息加入训练数据，同时排除有害内容，显著提高了模型探索效率和关键技能学习。实验表明，EEF在WebShop任务中以62%胜率超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop(>0.81)和SciWorld(>81)任务中创下新纪录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T10:01:29Z
- **目录日期**: 2025-04-20
