...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有益行动(如关键计划和动作)来提升大型语言模型的代理能力。与仅使用成功轨迹的拒绝采样微调(RFT)相比，EEF能有效解决复杂子任务，避免持续分布外问题。实验显示，EEF在WebShop任务中达到62%胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并创下WebShop(>0.81)和SciWorld(>81)的新SOTA。该方法证明了失败轨迹中的有价值信息对提升模型探索效率和关键技能获取的重要作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T01:29:15Z
- **目录日期**: 2025-04-20
