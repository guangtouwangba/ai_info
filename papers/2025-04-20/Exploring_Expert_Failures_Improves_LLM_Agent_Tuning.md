...
该研究提出"探索专家失败"(EEF)方法，通过分析专家模型(如GPT-4)失败轨迹中的有效动作来提升大语言模型的代理能力。研究发现，专家失败轨迹中的关键行动和计划能显著提高探索效率和技能获取。EEF精选这些有益动作加入训练数据，同时排除有害动作以避免干扰。实验表明，EEF成功解决了部分之前无法完成的子任务，在WebShop任务中以62%胜率超越RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创下超过81分的新纪录，成为首个突破该阈值的方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T20:01:16Z
- **目录日期**: 2025-04-20
