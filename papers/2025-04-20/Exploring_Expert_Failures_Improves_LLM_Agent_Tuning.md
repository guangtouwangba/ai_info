...
这篇论文提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的训练效果。研究发现，专家模型(如GPT-4)在复杂子任务上的失败轨迹往往包含有价值的信息，如关键行动和计划。EEF通过从专家失败轨迹中提取有益行动并剔除有害行动，将这些信息整合到训练数据中。实验表明，EEF能解决之前无法完成的子任务，在WebShop测试中达到62%的胜率，优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld基准测试中创造了新的最高记录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T02:30:22Z
- **目录日期**: 2025-04-20
