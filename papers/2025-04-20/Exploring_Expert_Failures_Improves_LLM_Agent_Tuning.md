...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于提升大型语言模型(LLM)作为智能体的性能。研究发现，专家模型(如GPT-4)在复杂子任务中失败的历史轨迹往往包含有价值的指导信息。EEF通过从这些失败轨迹中提取有益动作(如关键计划和行动)，同时剔除有害动作，显著提高了智能体的探索效率和关键技能获取能力。实验表明，EEF在WebShop任务中取得了62%的胜率，超越了RFT方法(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创造了新的最佳性能记录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T21:01:18Z
- **目录日期**: 2025-04-20
