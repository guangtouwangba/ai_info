...
该研究提出了一种名为"探索专家失败"(EEF)的新方法，用于改进大型语言模型(LLM)作为智能体的性能。传统拒绝采样微调(RFT)方法主要依赖专家成功的轨迹，但难以解决复杂子任务。研究发现，专家失败的轨迹中往往包含有价值的指导信息(如关键行动和计划)。EEF通过从失败轨迹中提取有益行动并排除有害行动来增强训练数据，成功解决了部分RFT无法处理的子任务。实验表明，EEF在WebShop任务中达到62%的胜率，显著优于RFT(53.6%)和GPT-4(35.6%)，并在WebShop和SciWorld任务中创下了新的最佳性能记录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T09:01:18Z
- **目录日期**: 2025-04-20
