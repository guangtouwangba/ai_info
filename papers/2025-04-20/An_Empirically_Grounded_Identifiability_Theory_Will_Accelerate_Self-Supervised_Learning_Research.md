...
当前自监督学习（SSL）研究快速发展，但缺乏理论解释。基于可识别性理论（IT）分析发现，尽管不同SSL方法可能收敛到相同的"柏拉图式理想表示"（PRH），但现有IT无法完全解释SSL的实际成功。为此，研究者提出扩展的"奇异可识别性理论"（SITh），旨在建立覆盖完整SSL流程的理论框架，以深入理解数据假设并推动学习更具可解释性和泛化能力的表示。未来研究应聚焦三个方向：1)SSL训练动态与收敛性；2)有限样本、批次大小和数据多样性的影响；3)架构、数据增强、初始化和优化器中的归纳偏置作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T13:07:00Z
- **目录日期**: 2025-04-20
