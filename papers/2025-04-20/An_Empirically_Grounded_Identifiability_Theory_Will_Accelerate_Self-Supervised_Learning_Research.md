...
自监督学习（SSL）是当前AI系统的核心技术。根据柏拉图表示假说（PRH），尽管方法和工程实现不同，但所有表示最终会收敛到同一个理想状态。本文结合可识别性理论（IT）证明PRH可能在SSL中出现，但现有理论无法完全解释SSL的实际成功。为此，作者提出扩展的"奇异可识别性理论"（SITh），以涵盖整个SSL流程，帮助理解其隐含的数据假设，推动学习更具解释性和泛化性的表示。未来研究应关注：1）SSL训练动态与收敛性；2）有限样本、批次大小和数据多样性的影响；3）架构、增强、初始化和优化器的归纳偏置作用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T06:01:40Z
- **目录日期**: 2025-04-20
