...
评估一个行为的安全性需要考虑其发生的具体情境。对人类来说，这显而易见（如删除邮件是否恰当取决于邮件内容、目的和邮箱类型），但计算系统通常只能在有限情境中运作，依赖人工制定的策略和用户确认（如应用权限）。然而，随着多功能智能代理（如自动化个人助手）的出现，其广泛的任务和情境要求重新设计安全机制。为此，本文提出"情境化代理安全"（Conseca）框架，旨在生成即时、情境相关且可人工验证的安全策略，以适配未来通用智能系统的复杂需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T07:02:43Z
- **目录日期**: 2025-04-20
