...
评估一个行为的安全性需要结合具体情境。对人类而言，这显而易见（如删除邮件是否合适取决于邮件内容、目的和邮箱类型），但计算系统通常只能在有限情境中运作，依赖手动制定的策略和用户确认（如应用权限）。随着通用智能代理（如自动个人助手）的出现，其多任务能力要求安全设计适应更广泛的情境。本文提出"情境化代理安全"（Conseca）框架，旨在生成即时、情境化且可人工验证的安全策略，作为适应这类系统规模与能力的第一步解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T23:02:12Z
- **目录日期**: 2025-04-20
