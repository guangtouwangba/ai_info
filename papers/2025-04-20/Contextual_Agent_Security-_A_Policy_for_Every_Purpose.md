...
评估一个行为的安全性需要考虑其发生的具体情境。对人类而言，这显而易见，比如删除邮件是否合适取决于邮件内容、目的（如删除敏感信息或清理垃圾）和邮箱类型（工作或个人）。传统计算系统由于功能有限，依靠手动制定的策略和用户确认（如应用权限或访问控制列表）即可防范风险。但随着多功能通用智能体（如自动个人助手）的出现，其广泛的应用场景和能力要求重新设计安全机制。本文提出Conseca框架，旨在生成即时、情境化且可人工验证的安全策略，作为面向智能体的情境安全研究的初步探索。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-04-20T08:02:35Z
- **目录日期**: 2025-04-20
