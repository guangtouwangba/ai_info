...
这篇论文探讨了代码生成大模型（codeLLMs）与人类偏好对齐的问题。研究者创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，以评估模型在真实编程任务中的表现。同时开发了SynCode-Instruct合成指令数据集（约200亿标记）验证大规模指令微调效果。实验发现执行基准与CodeArena存在性能差异，开源模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著，强调了人类偏好对齐的重要性。研究突出了现有评估体系在衡量代码实用性方面的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T20:05:21Z
- **目录日期**: 2025-05-05
