...
本研究提出了Insight-V框架，旨在提升多模态大语言模型(MLLM)的推理能力。通过两步自动化流程生成高质量的长链推理数据：1)渐进式策略生成多样化长推理路径；2)多粒度评估确保数据质量。为解决直接训练效果不佳的问题，设计了包含推理代理(执行长链推理)和总结代理(评估结果)的多智能体系统，并采用迭代DPO算法提升生成稳定性。基于LLaVA-NeXT模型和更强基础MLLM的实验表明，该方法在视觉推理任务中表现显著提升，同时保持感知任务的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T20:02:18Z
- **目录日期**: 2025-05-05
