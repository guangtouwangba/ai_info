...
本文介绍了Insight-V系统，旨在提升多模态大语言模型（MLLM）的推理能力。通过两步流程自动生成长链、高质量的视觉语言推理数据，并采用多粒度评估确保数据质量。为解决直接训练效果不佳的问题，设计了包含推理代理（执行长链推理）和总结代理（评估结果）的多智能体系统，结合迭代DPO算法提升稳定性。基于LLaVA-NeXT和更强基础模型，Insight-V在视觉推理任务中表现显著提升，同时保持感知任务的性能。该研究填补了多模态任务中长链推理数据与训练优化的空白。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T16:02:32Z
- **目录日期**: 2025-05-05
