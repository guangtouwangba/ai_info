...
本文提出Insight-V框架，旨在提升多模态大语言模型(MLLM)的推理能力。该框架包含两个创新：1) 通过渐进式策略自动生成高质量的长链视觉-语言推理数据，采用多粒度评估确保数据质量；2)设计多代理系统(推理代理+总结代理)配合迭代DPO算法，有效处理复杂推理数据。基于LLaVA-NeXT模型，Insight-V在需要视觉推理的多模态任务中表现显著提升，同时保持感知任务的性能。该方法无需人工标注，为多模态推理提供了可扩展的数据生成和训练方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T05:02:31Z
- **目录日期**: 2025-05-05
