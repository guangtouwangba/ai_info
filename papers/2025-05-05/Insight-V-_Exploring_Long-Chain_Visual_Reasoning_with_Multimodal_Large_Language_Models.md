...
本文介绍了Insight-V系统，旨在提升多模态大语言模型(MLLM)的推理能力。该系统通过两步流程自动生成长链、高质量的视觉语言推理数据：首先生成多样化长推理路径，再通过多粒度评估确保数据质量。为解决直接训练效果不佳的问题，Insight-V设计了包含推理代理（执行长链推理）和总结代理（评估推理结果）的多智能体系统，并采用迭代DPO算法提升生成稳定性。基于LLaVA-NeXT模型和更强基础MLLM的实验表明，该系统在视觉推理任务中表现显著提升，同时保持感知任务的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T11:02:36Z
- **目录日期**: 2025-05-05
