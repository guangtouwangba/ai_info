...
本文介绍了Insight-V，一个旨在提升多模态大语言模型（MLLMs）推理能力的框架。它通过两步流程自动生成长且结构化的多模态推理数据：首先生成多样化的长推理路径，再通过多粒度评估确保数据质量。为解决直接监督训练效果不佳的问题，Insight-V设计了多智能体系统，包含负责长链推理的推理智能体和评估推理结果的总结智能体，并结合迭代DPO算法提升生成稳定性。实验表明，基于LLaVA-NeXT改进的Insight-V在视觉推理任务中表现显著提升，同时保持感知任务的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T15:02:39Z
- **目录日期**: 2025-05-05
