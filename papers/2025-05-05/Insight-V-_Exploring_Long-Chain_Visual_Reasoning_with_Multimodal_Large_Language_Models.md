...
本文提出Insight-V框架，旨在提升多模态大语言模型(MLLM)的推理能力。该框架包含两个关键创新：1) 通过渐进式策略自动生成长链视觉推理数据，并采用多粒度评估确保质量；2) 设计由推理代理和总结代理组成的多智能体系统，结合迭代DPO算法优化训练流程。基于LLaVA-NeXT模型，该方法在需要视觉推理的多模态基准测试中表现显著提升，同时不影响感知任务的性能。研究解决了当前MLLM在长链推理数据和训练流程优化方面的不足，为复杂多模态任务提供了有效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T03:20:27Z
- **目录日期**: 2025-05-05
