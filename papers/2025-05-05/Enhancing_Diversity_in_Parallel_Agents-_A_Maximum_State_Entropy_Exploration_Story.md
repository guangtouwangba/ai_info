...
并行数据收集通过让N个智能体在相同环境模拟器中同时运行，将数据收集速度提升N倍。本文提出了一种新型学习框架，通过最大化并行设置中收集数据的熵来优化效率。该方法平衡了单个智能体的熵与智能体间的多样性，减少冗余数据，并采用集中式策略梯度方法。实验表明，该方法优于相同智能体系统，并能与利用数据多样性的批量强化学习技术协同工作。理论分析也表明，专业化并行采样分布能实现更快的收敛速度，验证了该方法的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T21:01:18Z
- **目录日期**: 2025-05-05
