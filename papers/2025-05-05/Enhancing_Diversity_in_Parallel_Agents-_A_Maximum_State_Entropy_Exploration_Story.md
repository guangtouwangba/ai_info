...
并行数据收集通过多个代理在相同环境模拟器中运行，显著提升了强化学习效率。本文提出一种新框架，通过最大化数据熵来优化并行代理的策略，平衡个体熵与代理间多样性以减少冗余。采用集中式策略梯度方法，实验表明其优于同质代理系统，并能与批量强化学习技术协同利用数据多样性。理论分析证明，专业化并行采样分布可加速收敛，验证了方法的有效性。该研究为大规模实际应用中的高效数据收集提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T17:01:24Z
- **目录日期**: 2025-05-05
