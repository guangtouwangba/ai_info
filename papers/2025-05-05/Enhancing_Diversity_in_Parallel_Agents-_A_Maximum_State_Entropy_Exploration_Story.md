...
并行数据收集通过多个相同智能体在模拟环境中同时运行，将数据收集效率提升N倍。本文提出了一种新框架，通过最大化并行环境中的数据熵来优化策略，平衡个体熵与智能体间多样性以减少冗余。采用集中式策略梯度方法，实验表明该方法优于同质智能体系统，并能与批量强化学习技术协同利用数据多样性。理论分析表明，专业化并行采样分布能加快收敛速度，验证了该方法的有效性。这一成果为大规模强化学习应用提供了更高效的数据收集方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T06:01:22Z
- **目录日期**: 2025-05-05
