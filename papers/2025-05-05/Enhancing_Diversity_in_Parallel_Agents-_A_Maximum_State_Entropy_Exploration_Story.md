...
并行数据收集通过让N个相同智能体在环境模拟器中并行运行，将数据收集效率提升N倍。本文提出一种新框架，通过最大化并行环境中的数据熵来优化策略：既保持单个智能体的探索性，又减少智能体间的冗余。该方法采用集中式策略梯度，实验表明其优于同质智能体系统，并能与批量强化学习技术结合以利用数据多样性。理论分析表明，专用并行采样分布具有更快的收敛速度，验证了方法的有效性。这一成果为突破N倍加速瓶颈提供了新思路，其理论分析也具有独立价值。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T05:01:31Z
- **目录日期**: 2025-05-05
