...
该研究提出CATransformers框架，用于全面评估和优化机器学习系统的碳足迹，包括运行碳排放（训练与推理）和隐含碳排放（硬件制造与全生命周期）。该框架通过碳感知架构搜索，在早期设计阶段同时优化模型和硬件架构。实验表明，碳优化策略与传统延迟或能效优化方案存在显著差异。应用于多模态CLIP模型时，新方法CarbonCLIP在保持准确性和延迟的同时，总碳排放量比现有边缘小CLIP基线降低达17%。研究强调需要整体优化方法来构建高性能且环境可持续的AI系统。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T10:01:21Z
- **目录日期**: 2025-05-05
