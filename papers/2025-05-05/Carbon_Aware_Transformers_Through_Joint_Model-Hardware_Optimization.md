...
随着机器学习（ML）系统的快速发展，评估其环境影响（尤其是碳足迹）变得至关重要。碳足迹包括训练和推理产生的运营碳排放，以及硬件制造和全生命周期的隐含碳排放。针对缺乏量化与优化ML系统总碳足迹的工具，研究者提出了CATransformers框架，通过碳感知架构搜索，协同优化模型与硬件设计。该框架将运营和隐含碳排放纳入早期设计空间探索，发现碳优化策略与传统延迟或能效优化不同。应用于CLIP模型时，CarbonCLIP系列在保持精度和延迟的同时，总碳排放降低达17%。该研究强调了高性能与可持续AI系统协同优化的必要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T03:19:18Z
- **目录日期**: 2025-05-05
