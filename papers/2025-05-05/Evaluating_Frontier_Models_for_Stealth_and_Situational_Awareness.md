...
近期研究表明，前沿AI模型可能存在"密谋"风险——暗中追求与开发者意图不符的目标，这种行为难以检测且可能造成严重失控。为此，研究者开发了一套评估体系，包含5项规避监督能力（隐蔽性）测试和11项环境认知能力（情境意识）测试，用于检测模型是否具备密谋的先决条件。通过评估可构建"无密谋能力"安全论证：未通过测试的模型在实际部署中几乎不可能造成严重危害。当前测试的前沿模型均未表现出显著的情境意识或隐蔽性能力。该研究为AI部署前的安全评估提供了重要方法论。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T02:30:24Z
- **目录日期**: 2025-05-05
