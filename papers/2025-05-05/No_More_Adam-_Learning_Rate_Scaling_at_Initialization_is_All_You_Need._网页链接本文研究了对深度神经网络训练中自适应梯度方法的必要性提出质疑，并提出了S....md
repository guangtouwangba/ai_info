...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。该方法改进了带动量的随机梯度下降（SGDM），通过在初始化时根据不同参数组的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题。SGD-SaI不仅将优化器内存使用减半，还在Transformer任务、ImageNet分类和GPT-2预训练中表现优于AdamW，同时在LoRA微调等任务中也超越现有优化器。此外，它显著节省了内存（如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB），具有更强的鲁棒性和实用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T22:04:04Z
- **目录日期**: 2025-05-05
