...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观点。SGD-SaI通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，解决了传统SGD训练不平衡的问题。相比AdamW，它不仅内存效率更高（节省5.93-25.15GB），在Transformer、ImageNet分类和LLM预训练等任务中表现相当或更好，还展现出更强的超参数鲁棒性。该方法特别适合大模型训练，在LoRA微调等场景也优于现有优化器，为深度学习优化提供了更高效的替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T05:04:35Z
- **目录日期**: 2025-05-05
