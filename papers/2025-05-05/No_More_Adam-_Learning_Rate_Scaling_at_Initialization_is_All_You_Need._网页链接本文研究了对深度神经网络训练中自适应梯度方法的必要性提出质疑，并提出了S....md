...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据梯度信号噪声比(g-SNR)调整不同参数组的学习率，改进了带动量的随机梯度下降(SGDM)。相比AdamW，SGD-SaI内存使用减半，在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现更优。实验显示，它为GPT-2节省5.93GB内存，为Llama2-7B节省25.15GB内存，同时保持或超越AdamW的性能，解决了传统SGD训练Transformer的难题，具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T09:04:35Z
- **目录日期**: 2025-05-05
