...
本文提出了一种基于有限演示数据来学习人类感知与控制模型的一般性估计方法。该模型结构包含：i) 主体对环境动态的内部表征；ii) 主体对可观测结果的偏好。模型构建符合认知科学中的主动推理理论，认为人类行为旨在最小化"惊讶度"（实际观察与偏好观察的差异）。作者采用双层优化方法进行估计，并基于先验分布的结构假设。通过在自然驾驶数据中建立跟车行为模型验证，结果表明从数据中学习主动推理模型是替代传统黑箱驾驶模型的有前景的方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T01:30:05Z
- **目录日期**: 2025-05-05
