...
该研究提出LatentSync框架，一种基于音频条件潜在扩散模型的端到端唇同步方法，无需中间运动表示。针对扩散模型的时间一致性问题，创新性地提出TREPA方法，利用自监督视频模型的时间表示对齐生成帧。同时，研究解决了SyncNet收敛问题，通过优化架构、超参数和数据预处理显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验可推广至其他音频驱动动画任务。研究突破了传统两阶段生成限制，直接建模音频-视觉复杂关联。（98字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T09:05:36Z
- **目录日期**: 2025-05-05
