...
该研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。为解决扩散模型的时间一致性问题，研究者开发了TREPA方法，利用自监督视频模型的时间表示来对齐生成帧。同时，针对SyncNet的收敛问题，研究通过调整模型架构、训练参数和数据预处理显著提升了其准确性。该方法在HDTF和VoxCeleb2数据集上超越了现有技术。这些改进不仅提升了唇同步效果，也为其他音频驱动动画方法提供了参考。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T13:13:54Z
- **目录日期**: 2025-05-05
