...
本研究提出LatentSync框架，利用音频条件潜在扩散模型实现端到端唇同步，无需中间运动表示。该框架直接建模音频-视觉关系，但发现扩散过程存在时间一致性问题，因此提出TREPA方法（时间表示对齐）来增强连贯性。同时，研究改进了SyncNet训练框架，解决了收敛问题，提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。创新点包括：1）首个基于潜在扩散的唇同步模型；2）TREPA时间对齐技术；3）SyncNet训练优化方案。这些改进可推广至其他人像动画应用。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T20:05:03Z
- **目录日期**: 2025-05-05
