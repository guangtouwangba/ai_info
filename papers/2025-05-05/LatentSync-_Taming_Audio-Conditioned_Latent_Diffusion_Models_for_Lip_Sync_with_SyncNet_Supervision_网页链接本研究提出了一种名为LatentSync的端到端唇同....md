...
本研究提出LatentSync框架，通过音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方法利用Stable Diffusion直接建模音频-视觉关系，并针对扩散模型的时间一致性问题提出TREPA技术，使用自监督视频模型对齐生成帧。研究还优化了SyncNet训练框架，改进了模型架构、超参数和数据预处理，显著提升其准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术。这些改进可推广至其他基于SyncNet的唇同步和音频驱动动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T03:28:15Z
- **目录日期**: 2025-05-05
