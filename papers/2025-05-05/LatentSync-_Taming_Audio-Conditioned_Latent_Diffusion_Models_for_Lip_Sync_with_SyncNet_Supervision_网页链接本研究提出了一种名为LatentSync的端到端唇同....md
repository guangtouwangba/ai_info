...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架。该方法直接建模音频-视觉关系，无需中间运动表示，克服了传统两阶段方法的局限。针对扩散模型的时间一致性问题，创新性地提出TREPA（时间表示对齐）方法，利用自监督视频模型增强帧间一致性。同时，研究深入分析了SyncNet的收敛问题，优化了其训练框架，显著提升了准确性。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，为唇同步和音频驱动动画提供了新思路。这些改进也可应用于其他相关领域。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T10:05:45Z
- **目录日期**: 2025-05-05
