...
本研究提出LatentSync，一种基于音频条件潜在扩散模型的端到端唇同步框架，无需中间运动表示。该框架利用Stable Diffusion直接建模音频-视觉关系，但面临时间一致性问题。为此，研究者开发了TREPA方法，通过自监督视频模型的时间表示对齐生成帧，提升一致性。同时，针对SyncNet收敛问题进行了实证研究，优化了模型架构、训练参数和数据预处理，显著提高了SyncNet精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越了现有唇同步技术。这些改进也可应用于其他音频驱动的人像动画方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T11:05:44Z
- **目录日期**: 2025-05-05
