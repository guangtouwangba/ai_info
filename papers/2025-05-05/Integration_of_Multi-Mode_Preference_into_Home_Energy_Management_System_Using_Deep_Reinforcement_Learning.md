...
本文提出了一种基于深度强化学习(DRL)的多模式家庭能源管理系统(HEMS)框架，旨在动态优化用户偏好下的家庭能源消耗。与现有使用静态权重因子的方法不同，该框架能根据用户对不同电器的动态偏好进行智能调节。研究采用无模型单智能体DRL算法，利用15分钟间隔的真实数据（电价、环境温度、电器功耗）进行验证。结果显示，该模型在不同偏好模式下均能出色优化能耗，其计算效率优于传统的混合整数线性规划(MILP)方法，同时保持近乎最优的性能，有效提升了用户在需求响应(DR)项目中的参与度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T14:01:41Z
- **目录日期**: 2025-05-05
