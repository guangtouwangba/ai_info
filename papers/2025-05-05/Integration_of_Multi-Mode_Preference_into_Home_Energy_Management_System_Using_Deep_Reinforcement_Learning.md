...
本文提出了一种基于深度强化学习(DRL)的智能家庭能源管理系统(HEMS)框架，通过动态多模式偏好优化家庭能源使用。传统方法使用静态权重因子难以反映用户行为的动态特性，而该DRL-HEMS框架能根据用户定义的实时偏好进行优化，提升用户参与需求响应(DR)的积极性。采用无模型单智能体DRL算法，系统在保持用户友好性的同时实现动态优化。使用15分钟间隔的真实数据验证表明，该模型在不同偏好模式下均能出色优化能耗，计算效率优于传统混合整数线性规划(MILP)方法，同时保持近乎最优的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T05:01:42Z
- **目录日期**: 2025-05-05
