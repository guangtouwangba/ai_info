...
本文提出了一种基于深度强化学习(DRL)的智能家庭能源管理系统(HEMS)框架DRL-HEMS，通过动态多模式偏好优化家庭能源使用。与现有采用静态权重因子的方法不同，该框架能根据用户对各类电器的动态偏好进行优化，提升用户参与需求响应(DR)的积极性。研究采用无模型单智能体DRL算法，使用15分钟间隔的真实数据(电价、环境温度、电器功耗等)验证。结果显示，该模型在不同偏好模式下均能出色优化能耗，计算效率优于传统的混合整数线性规划(MILP)方法，同时保持近乎最优的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T02:30:59Z
- **目录日期**: 2025-05-05
