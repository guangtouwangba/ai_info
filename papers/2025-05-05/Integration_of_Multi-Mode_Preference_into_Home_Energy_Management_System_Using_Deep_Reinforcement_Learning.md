...
家庭能源管理系统（HEMS）是智能家居生态中的关键工具，旨在提升能效、降低成本和优化用户舒适度。传统方法常将用户舒适度视为固定偏差，但忽略了动态偏好。本文提出基于深度强化学习（DRL）的多模式HEMS框架（DRL-HEMS），通过动态学习用户偏好优化能源使用。该模型采用无模型单智能体DRL算法，结合电价、环境温度和电器功耗等实时数据，显著提升了需求响应（DR）的参与度。实验表明，相比传统混合整数线性规划（MILP）算法，DRL-HEMS在保持近最优性能的同时，计算效率更高。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T23:01:34Z
- **目录日期**: 2025-05-05
