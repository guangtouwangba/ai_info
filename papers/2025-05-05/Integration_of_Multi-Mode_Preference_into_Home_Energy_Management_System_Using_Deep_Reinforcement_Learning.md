...
家庭能源管理系统（HEMS）是智能家居中提升能效、降低成本及改善舒适度的关键工具。传统方法常将用户舒适度简单视为设备设置的静态偏差，忽略了行为动态性。本研究提出基于深度强化学习（DRL）的多模式HEMS框架，通过动态学习用户偏好优化能源使用。采用无模型单智能体DRL算法，结合电价、环境温度等实时数据验证，该模型在不同偏好模式下均表现优异。相比传统混合整数线性规划（MILP）算法，其计算效率更高且接近最优性能，显著提升了用户在需求响应中的参与度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T03:19:43Z
- **目录日期**: 2025-05-05
