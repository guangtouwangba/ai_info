...
本文提出了一种基于深度强化学习(DRL)的智能家庭能源管理系统(HEMS)框架DRL-HEMS。与现有采用静态权重因子的方法不同，该框架能根据用户动态定义的多模式偏好进行优化，提升用户在需求响应(DR)项目中的参与度。研究采用无模型单智能体DRL算法，利用15分钟间隔的真实数据(电价、环境温度、设备功耗)进行验证。结果表明，该模型在不同偏好模式下都能出色优化能耗，相比传统混合整数线性规划(MILP)算法，在保持近乎最优性能的同时显著提高了计算效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T13:09:48Z
- **目录日期**: 2025-05-05
