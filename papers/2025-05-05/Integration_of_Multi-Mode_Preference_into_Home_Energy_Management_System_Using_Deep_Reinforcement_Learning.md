...
家庭能源管理系统（HEMS）是智能家居生态中的关键工具，旨在提升能源效率、降低成本并优化用户舒适度。传统方法常将用户舒适度视为固定权重因素，忽略动态偏好。本文提出一种基于深度强化学习（DRL）的多模式HEMS框架（DRL-HEMS），通过动态学习用户偏好实现个性化优化，提升用户在需求响应（DR）中的参与度。该模型采用无模型单智能体DRL算法，结合电价、环境温度等实时数据验证。结果显示，相比传统混合整数线性规划（MILP）算法，DRL-HEMS在保持近似最优性能的同时显著提升计算效率。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T21:01:26Z
- **目录日期**: 2025-05-05
