...
家庭能源管理系统（HEMS）是智能家居生态中的关键工具，旨在提升能效、降低成本并优化用户舒适度。传统研究常将用户舒适度简单视为设备设置的静态偏差，而本文提出了一种基于深度强化学习（DRL）的多模式HEMS框架（DRL-HEMS），动态适应用户偏好。该模型通过无模型单智能体DRL算法，结合电价、环境温度等实时数据，显著优化了不同偏好模式下的能耗。相比传统混合整数线性规划（MILP）算法，DRL-HEMS在保持近最优性能的同时，计算效率更高，提升了用户对需求响应（DR）的参与度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T20:01:34Z
- **目录日期**: 2025-05-05
