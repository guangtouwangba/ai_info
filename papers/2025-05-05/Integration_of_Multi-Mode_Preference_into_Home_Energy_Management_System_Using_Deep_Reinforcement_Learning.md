...
本文提出了一种基于深度强化学习(DRL)的多模式家庭能源管理系统(HEMS)框架，旨在通过动态优化用户偏好来提高能源效率。传统HEMS常使用静态权重因子，难以反映用户行为的动态变化。该研究采用无模型单智能体DRL算法，整合了针对不同家电的动态多模式偏好设置，显著提升了用户在需求响应(DR)项目中的参与度。使用15分钟间隔的真实数据(电价、环境温度、设备功耗)验证表明，该模型在不同偏好模式下均能出色优化能耗，计算效率优于传统的混合整数线性规划(MILP)方法，同时保持近乎最优的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T22:01:35Z
- **目录日期**: 2025-05-05
