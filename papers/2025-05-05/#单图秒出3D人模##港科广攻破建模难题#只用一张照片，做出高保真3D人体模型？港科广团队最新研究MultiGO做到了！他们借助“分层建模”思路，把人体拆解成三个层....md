...
港科广团队提出MultiGO方法，通过分层建模（骨架、关节、衣物褶皱三层次）实现单张照片生成高保真3D人体模型。该技术突破传统模板限制，采用骨架增强（3D傅里叶特征投影）、关节扰动（抗误差训练）和皱纹去噪（扩散模型优化细节）三大策略，在CustomHuman和THuman3.0测试集中精度领先。研究入选CVPR 2025，可应用于虚拟试衣、影视特效等领域。代码即将开源，细节达亚毫米级，显著提升3D建模效率与真实感。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T13:11:01Z
- **目录日期**: 2025-05-05
