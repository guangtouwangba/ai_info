...
当前大型语言模型（LLMs）的偏好对齐方法主要关注相对偏好，却忽略了响应质量，导致模型可能过度拟合或错误学习。本研究提出了一种基于奖励条件的LLM策略，通过分析数据集中的完整响应质量范围，帮助模型推断更优结果。方法包括对偏好数据进行质量评分重标注，构建奖励增强数据集。实验表明，该方法显著提升了DPO（Direct Preference Optimization）性能，有效利用偏好数据并减少错误学习问题。研究证明了该方法在数据扩展之外的广泛有效性。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T00:02:18Z
- **目录日期**: 2025-05-05
