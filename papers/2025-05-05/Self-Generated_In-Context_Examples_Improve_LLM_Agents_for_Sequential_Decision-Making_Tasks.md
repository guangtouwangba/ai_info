...
该研究探讨了如何通过自动学习自身成功经验来提升大语言模型(LLM)在序列决策任务中的表现，而无需依赖特定任务的知识工程。研究者构建并优化了一个自生成示例数据库，结果表明：仅简单积累训练任务中的成功轨迹就能显著提升在ALFWorld(73%→89%)、Wordcraft(55%→64%)和InterCode-SQL(75%→79%)三个基准测试的表现。进一步引入基于群体训练的数据库级选择和基于效用的示例级选择后，ALFWorld任务准确率达到91%，媲美需要复杂任务特定组件的方法。这为替代费时费力的人工知识工程提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T04:03:12Z
- **目录日期**: 2025-05-05
