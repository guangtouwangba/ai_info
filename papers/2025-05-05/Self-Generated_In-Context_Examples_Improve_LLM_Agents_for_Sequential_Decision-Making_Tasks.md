...
该研究探讨了如何让大语言模型（LLM）智能体通过自动学习自身在类似任务中的成功经验来提升决策性能，而非依赖任务特定的知识工程。通过构建和优化自生成的成功轨迹数据库，研究者在ALFWorld、Wordcraft和InterCode-SQL三个基准测试中显著提升了性能（如ALFWorld从73%升至89%）。进一步引入基于群体训练的数据库级筛选和基于效用的轨迹级筛选后，性能进一步提升（ALFWorld达91%），与复杂任务特定方法相当。结果表明，自动轨迹数据库构建可有效替代人工密集的知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T16:02:22Z
- **目录日期**: 2025-05-05
