...
该研究探讨了如何通过自动学习自身成功经验来提升大语言模型（LLM）在序列决策任务中的表现，而无需依赖特定任务的知识工程。通过构建和优化自生成案例数据库，研究者在ALFWorld、Wordcraft和InterCode-SQL三个基准测试中显著提升了模型性能（如ALFWorld从73%提升至89%）。进一步引入基于群体训练的数据库级筛选和基于实用性的案例级筛选后，ALFWorld性能达到91%，媲美复杂的手工优化方法。结果表明，自动轨迹数据库构建可有效替代人工密集的知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T18:02:15Z
- **目录日期**: 2025-05-05
