...
该研究探讨了如何让大语言模型(LLM)智能体通过自动学习自身在类似任务中的成功经验来提升决策能力，而非依赖特定任务的知识工程。通过构建和优化自生成案例数据库，即使简单积累成功轨迹也能显著提高ALFWorld(73%→89%)、Wordcraft(55%→64%)和InterCode-SQL(75%→79%)三个基准测试的表现。研究还引入了基于群体训练的数据库级筛选和基于实证效用的案例级筛选两种扩展方法，使ALFWorld性能达到91%，媲美复杂定制化方案。这表明自动轨迹数据库构建可有效替代费时的人工知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T19:02:03Z
- **目录日期**: 2025-05-05
