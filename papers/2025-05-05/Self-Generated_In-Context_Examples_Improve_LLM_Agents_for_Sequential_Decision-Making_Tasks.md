...
该研究探讨了如何通过自动学习提升大语言模型(LLM)在序列决策任务中的表现，而不依赖特定任务的知识工程。通过构建和优化自生成的成功案例数据库，研究者在ALFWorld(73%→89%)、Wordcraft(55%→64%)和InterCode-SQL(75%→79%)三个基准测试上取得了显著提升。进一步引入两种扩展方法：基于群体训练的数据库级选择和基于经验效用的案例级选择，最终在ALFWorld上达到91%的准确率，与需要复杂任务特定组件的方法相当。研究表明，自动构建轨迹数据库可有效替代人工密集的知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T07:02:27Z
- **目录日期**: 2025-05-05
