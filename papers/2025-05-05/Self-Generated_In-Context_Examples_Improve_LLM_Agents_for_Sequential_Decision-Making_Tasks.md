...
该研究提出了一种通过自动学习自身成功经验来提升大语言模型(LLM)代理性能的方法，而无需依赖特定任务的知识工程。通过构建和优化自生成示例数据库，研究显示仅简单积累成功轨迹就能显著提升三个基准测试的性能(ALFWorld 73%→89%，Wordcraft 55%→64%，InterCode-SQL 75%→79%)。进一步引入的两种扩展方法——基于群体训练的数据库级选择和基于经验效用的示例级选择，使ALFWorld性能达到91%，媲美更复杂的任务特定方法。结果表明，自动轨迹数据库构建可有效替代人工密集的知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T22:02:11Z
- **目录日期**: 2025-05-05
