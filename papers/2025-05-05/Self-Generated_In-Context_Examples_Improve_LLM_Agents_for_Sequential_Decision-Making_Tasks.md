...
该研究探讨了如何通过自动学习LLM智能体在类似任务中的成功经验来提升其决策性能，而无需依赖任务特定的知识工程。通过构建和优化自生成示例数据库，研究发现仅简单累积成功轨迹就能显著提升ALFWorld（73%到89%）、Wordcraft（55%到64%）和InterCode-SQL（75%到79%）的测试性能。进一步引入基于群体训练的数据库级选择和基于经验效用的示例级选择后，ALFWorld性能提升至91%，与复杂任务特定方法相当。结果表明，自动轨迹数据库构建可有效替代人工密集型知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T14:02:22Z
- **目录日期**: 2025-05-05
