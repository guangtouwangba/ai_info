...
该研究提出了一种通过自动学习自身成功经验来提升大语言模型（LLM）代理在序列决策任务中表现的方法，而无需依赖特定任务的知识工程。通过构建和优化自生成示例数据库，代理在ALFWorld、Wordcraft和InterCode-SQL三个基准测试中的性能显著提升（如ALFWorld从73%提升至89%）。进一步引入基于群体训练的数据库级选择和基于经验效用的示例级选择后，性能进一步提升（ALFWorld达到91%），媲美依赖复杂任务特定组件的方法。结果表明，自动轨迹数据库构建是替代人工密集型知识工程的有效方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T10:02:31Z
- **目录日期**: 2025-05-05
