...
该研究探讨了如何通过自动学习自身成功经验来提升大语言模型(LLM)在序列决策任务中的表现，而无需依赖特定任务的知识工程。通过构建和优化自生成案例数据库，研究发现仅简单积累成功轨迹就能显著提升在ALFWorld(73%→89%)、Wordcraft(55%→64%)和InterCode-SQL(75%→79%)三个基准测试中的表现。进一步引入基于群体训练的数据库级筛选和基于实用性的轨迹级筛选后，ALFWorld任务准确率达到91%，媲美更复杂的任务特定方法。研究表明自动轨迹数据库构建可有效替代人工密集的知识工程。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T15:02:27Z
- **目录日期**: 2025-05-05
