...
TARL是一种创新方法，用于通过心率数据早期检测患者病情恶化。它通过建模心率时间序列中的关键子序列（shapelets）及其动态变化，构建知识图谱来追踪病情进展。该方法引入转移感知知识嵌入，增强子序列间关联并量化缺失值影响，从而生成全面的心率表征，用于预测未来趋势和病情预警。研究团队联合医护人员收集ICU患者的心率数据和诊断指标，验证显示TARL兼具高可靠性和早期检测能力。案例研究证实其可解释性，有望成为辅助临床识别患者恶化的AI工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T02:31:09Z
- **目录日期**: 2025-05-05
