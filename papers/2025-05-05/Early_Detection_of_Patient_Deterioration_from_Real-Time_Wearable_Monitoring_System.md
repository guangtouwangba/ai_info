...
本文提出了一种名为TARL的创新方法，用于通过可穿戴设备监测心率数据，实现患者病情恶化的早期检测。TARL通过建模心率时间序列中关键子序列（shapelets）的结构关系，构建动态知识图谱来追踪病情进展。该方法采用转移感知知识嵌入技术，强化shapelet间关联并量化缺失值影响，从而形成全面的心率表征。这些表征能解释病情结构并预测未来趋势。通过ICU患者实际数据验证，TARL展现出高可靠性和早期检测能力。案例研究证实其可解释性，表明该AI工具能有效辅助临床识别患者恶化早期征兆。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T22:01:46Z
- **目录日期**: 2025-05-05
