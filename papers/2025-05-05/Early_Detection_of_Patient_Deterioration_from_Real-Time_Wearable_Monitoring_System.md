...
TARL是一种创新方法，通过建模心率时间序列中代表性子序列（shapelets）的结构关系来早期检测患者病情恶化。它构建了一个shapelet转移知识图谱，捕捉心率动态变化以预测病情发展，并采用转移感知知识嵌入来强化shapelet间关联及量化缺失值影响，从而生成全面的心率表征。该方法结合可穿戴设备采集的ICU患者心率数据及临床诊断指标，能可靠地实现早期病情预警。实验证明TARL兼具高可靠性和早期检测能力，其可解释的检测流程有望成为辅助临床识别患者恶化迹象的AI工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T07:01:56Z
- **目录日期**: 2025-05-05
