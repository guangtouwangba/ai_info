...
本文提出了一种名为TARL的创新方法，用于通过心率数据早期检测患者病情恶化。TARL通过建模心率时间序列中的代表性子序列（shapelets）及其动态变化，构建知识图谱来捕捉病情进展趋势。该方法采用转移感知知识嵌入技术，强化shapelet间关联并量化缺失值影响，从而生成全面的心率表征。这些表征能解释病情结构并预测未来心率变化，帮助实现早期预警。通过ICU患者可穿戴设备数据的实验验证，TARL展现出高可靠性和早期检测能力。案例研究还证明了其可解释性，表明其作为AI辅助工具的临床潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T11:01:55Z
- **目录日期**: 2025-05-05
