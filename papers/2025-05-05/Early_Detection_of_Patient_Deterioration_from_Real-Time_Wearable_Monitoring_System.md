...
TARL是一种创新方法，通过分析心率时间序列中的关键子序列（shapelets）来早期检测患者病情恶化。它构建了一个shapelet转移知识图谱，模拟心率动态变化，预测病情发展。该方法还引入转移感知知识嵌入，强化shapelet间关系并量化缺失值影响，形成全面的心率表征。这些表征能解释病情结构并预测未来趋势。通过ICU患者可穿戴设备数据和临床诊断指标验证，TARL在真实数据中表现出高可靠性和早期检测能力。案例研究展示了其可解释的检测过程，证实其作为AI辅助工具帮助临床识别早期恶化迹象的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-05T14:01:52Z
- **目录日期**: 2025-05-05
