...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它分析四大社会问题：社会极化、刺激性信息传播、无条件基本收入政策及飓风等外部冲击的影响。其模拟结果与现实数据高度吻合，验证了该工具在揭示人类行为机制方面的有效性。该系统可作为计算社会科学的新平台，辅助传统研究方法（如调查、实验），为研究者和政策制定者提供重要支持。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T11:05:23Z
- **目录日期**: 2025-06-03
