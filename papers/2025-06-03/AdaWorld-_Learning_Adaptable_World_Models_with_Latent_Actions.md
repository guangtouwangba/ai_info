...
AdaWorld是一种创新的世界模型学习方法，通过自监督方式从视频中提取潜在动作，捕捉帧间关键变化，并在预训练中融入动作信息。该方法构建了一个基于潜在动作的自回归世界模型，显著减少了对大量标注数据和昂贵训练的需求，从而能通过有限交互快速适应新环境中的异构动作。实验表明，AdaWorld在模拟质量和视觉规划方面表现优异，支持高效迁移和有限微调下的新动作学习，突破了传统世界模型在跨领域应用中的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T03:21:51Z
- **目录日期**: 2025-06-03
