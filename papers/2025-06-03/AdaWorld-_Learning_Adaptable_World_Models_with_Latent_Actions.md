...
AdaWorld是一种创新的世界模型学习方法，通过自监督方式从视频中提取潜在动作，捕捉帧间关键变化，并在预训练中融入动作信息。该方法构建了一个基于潜在动作的自回归世界模型，显著提升了模型在新环境中的适应能力，即使交互和微调数据有限。相比依赖大量标注动作数据的传统方法，AdaWorld实现了更高效的迁移和新动作学习。多环境实验表明，AdaWorld在模拟质量和视觉规划方面均表现出色，解决了现有世界模型跨领域应用的局限性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T06:01:33Z
- **目录日期**: 2025-06-03
