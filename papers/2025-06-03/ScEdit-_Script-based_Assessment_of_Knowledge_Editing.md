...
该研究指出当前知识编辑（KE）任务过于简单，许多方法在评估中近乎完美，但缺乏实际应用验证。为此，作者提出基于脚本的评估基准ScEdit，涵盖反事实和时序编辑，结合词级和文本级评估方法，将传统事实型（"What"类问题）评估扩展到行动型（"How"类问题）。实验显示所有KE方法在现有指标上表现下降，文本级评估面临挑战，凸显任务难度。该基准旨在推动KE技术在实际场景（如LLM智能体）中的应用验证。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T11:00:57Z
- **目录日期**: 2025-06-03
