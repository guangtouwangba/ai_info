...
这篇论文探讨了代码生成大模型（codeLLMs）如何更好地与人类偏好对齐。现有基准主要测试代码正确性，而忽略了人类偏好匹配。为此，研究者构建了CodeArena基准（含397个高质量样本，覆盖40类任务和44种语言）和SynCode-Instruct合成指令集（近200亿标记）。实验发现：1)传统执行基准与CodeArena存在性能差异；2)开源顶级模型（如Qwen2.5-Coder）与商业模型（如OpenAI o1）差距显著。研究强调了代码模型对齐人类偏好的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T06:05:57Z
- **目录日期**: 2025-06-03
