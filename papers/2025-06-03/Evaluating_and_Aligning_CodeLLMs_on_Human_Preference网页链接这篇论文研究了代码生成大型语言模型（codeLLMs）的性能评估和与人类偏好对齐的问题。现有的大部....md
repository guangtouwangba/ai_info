...
这篇论文针对代码生成大模型(codeLLMs)提出CodeArena基准，从397个真实编程任务(覆盖40类/44种语言)评估模型与人类偏好的对齐程度，弥补了传统基准仅关注代码正确性的不足。研究同时构建20B标记的合成指令集SynCode-Instruct验证微调效果。实验发现执行基准与人类偏好存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)性能差距显著，强调对齐人类偏好的重要性。研究为提升代码LLMs的实用价值提供了新评估维度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T10:06:21Z
- **目录日期**: 2025-06-03
