...
这篇论文针对代码生成大型语言模型(codeLLMs)提出两个关键贡献：1) 创建了CodeArena基准测试，包含397个高质量编程样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建了20B标记的SynCode-Instruct合成指令数据集用于微调。研究发现：现有执行基准与人类偏好存在显著差异，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)在人类对齐方面差距明显。研究强调了代码生成模型除正确性外，还需重视实际开发场景中的用户体验。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T01:33:40Z
- **目录日期**: 2025-06-03
