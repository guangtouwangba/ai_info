...
这篇论文针对代码生成大模型(codeLLMs)提出了两个关键贡献：1) 创建了CodeArena基准测试，包含397个高质量编程任务样本，覆盖40个类别和44种语言，专门评估模型输出与人类偏好的匹配程度；2) 构建了20B标记的SynCode-Instruct合成指令数据集用于模型微调。实验发现现有基于执行的评估与人类偏好存在差距，且开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差异显著，凸显了代码生成模型与人类偏好对齐的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T14:07:43Z
- **目录日期**: 2025-06-03
