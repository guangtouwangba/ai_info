...
本文揭示了生成流网络(GFlowNet)与强化学习(RL)中策略评估的基本联系。研究发现，在特定结构条件下，评估均匀策略获得的价值函数与GFlowNets中的流函数密切相关。基于此，作者提出了修正随机策略评估(RPE)算法，仅需评估固定随机策略即可实现与GFlowNets相同的奖励匹配效果。实验表明，RPE在多个基准测试中取得了与现有方法相当的结果。这一发现为理解(非最大熵)RL与GFlowNets之间被忽视的联系提供了新视角，简化了GFlowNets的实现方式，同时增强了RL的多样性解决方案发现能力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T09:02:57Z
- **目录日期**: 2025-06-03
