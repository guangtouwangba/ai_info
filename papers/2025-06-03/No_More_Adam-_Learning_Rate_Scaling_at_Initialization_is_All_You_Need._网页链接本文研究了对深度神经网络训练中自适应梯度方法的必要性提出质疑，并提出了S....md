...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了传统深度学习中必须使用自适应梯度方法（如Adam）的观点。SGD-SaI基于带动量的随机梯度下降（SGDM），通过初始化时按不同参数组的梯度信号噪声比（g-SNR）调整学习率，无需自适应二阶动量即可避免训练不平衡问题。相比AdamW，该方法内存使用减半，在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优异或相当，尤其在大模型训练中显著节省内存（如为Llama2-7B节省25.15GB）。该方法对超参数变化鲁棒，具有广泛应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T01:32:18Z
- **目录日期**: 2025-06-03
