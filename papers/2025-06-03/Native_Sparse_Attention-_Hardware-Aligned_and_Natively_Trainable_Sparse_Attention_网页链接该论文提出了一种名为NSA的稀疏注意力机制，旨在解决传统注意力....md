...
这篇论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过算法与硬件协同优化，采用动态分层稀疏策略：先进行粗粒度令牌压缩，再进行细粒度令牌选择，在保持全局上下文感知能力的同时确保局部精度。该方法在稀疏注意力设计上有两个关键创新：硬件对齐性（提升计算效率）和原生可训练性（保持模型性能）。实验表明NSA能显著降低长文本建模的计算开销，同时维持模型效果。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T20:04:53Z
- **目录日期**: 2025-06-03
