...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在降低传统注意力机制处理长文本时的高计算成本。NSA通过动态分层稀疏策略（粗粒度令牌压缩+细粒度令牌选择），在保持全局上下文感知能力的同时提高局部精度。其核心创新在于算法与硬件协同优化，使稀疏注意力计算更高效且可直接训练。该方法有效解决了大模型处理长序列时的计算瓶颈问题，为提升Transformer架构的扩展性提供了新思路。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T11:05:15Z
- **目录日期**: 2025-06-03
