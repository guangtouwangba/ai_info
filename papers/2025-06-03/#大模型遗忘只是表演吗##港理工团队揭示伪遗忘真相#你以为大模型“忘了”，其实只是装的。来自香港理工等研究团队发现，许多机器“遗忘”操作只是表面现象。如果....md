...
香港理工大学等团队研究发现，大语言模型的"遗忘"可能是伪遗忘。他们开发了一套分析工具箱，通过PCA、CKA相似性等方法区分真/伪遗忘。关键结论是：仅输出指标变化而内部结构未变（如参数敏感区、表示方向稳定）属于可逆的伪遗忘；真正的遗忘需要多层网络发生大规模参数扰动，导致表示空间不可逆漂移。该研究为评估模型记忆机制提供了量化标准，表明单纯性能下降不等于彻底遗忘。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T02:33:21Z
- **目录日期**: 2025-06-03
