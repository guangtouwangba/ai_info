...
香港理工大学等团队研究发现，大语言模型的"遗忘"可能是"伪遗忘"——仅表面输出改变而内部结构未变，仍可恢复记忆。他们开发了表示空间分析工具箱，通过PCA、CKA相似性等多维度指标区分真伪遗忘。真遗忘需多层网络发生大规模参数扰动，导致表示方向、分布中心等不可逆变化；而伪遗忘仅输出指标变化时，模型仍能通过参数微调恢复原状。该研究为评估AI记忆机制提供了系统性方法论。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T00:03:36Z
- **目录日期**: 2025-06-03
