...
香港理工大学团队研究发现，大语言模型的"遗忘"可能只是表面现象。他们开发了一套分析工具，区分"伪遗忘"（可逆）和"真遗忘"（不可逆）。关键发现是：仅当多个网络层发生大范围参数扰动时才是真正遗忘，而仅输出指标变化但内部结构未变的"遗忘"可以轻易恢复。研究团队通过PCA方向变化、表示漂移、CKA相似性等多维度指标，建立了系统性评估方法，证明只有模型表示方向和参数敏感区域发生根本性改变才算真正遗忘。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T01:31:17Z
- **目录日期**: 2025-06-03
