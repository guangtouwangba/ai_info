...
这篇论文研究了大型语言模型(LLM)在连续交互过程中信息传递的演变。通过"传话游戏"实验设计，研究者发现LLM在多次传递文本时会产生系统性偏差和吸引子效应(attractor states)。实验追踪了文本毒性、积极性、难度和长度等属性的变化，发现开放性指令会增强吸引效应，不同文本属性对吸引效应的敏感度不同(如毒性比长度更易受影响)。这些发现揭示了多步传递动态的重要性，为理解LLM文化动态提供了初步见解。研究表明，单个LLM输出中可忽略的小偏差在连续交互中可能被放大，导致内容向特定状态演化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T16:03:21Z
- **目录日期**: 2025-06-03
