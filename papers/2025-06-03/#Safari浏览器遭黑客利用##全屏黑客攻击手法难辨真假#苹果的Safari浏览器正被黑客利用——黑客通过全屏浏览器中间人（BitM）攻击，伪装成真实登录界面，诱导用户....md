...
Safari浏览器正遭受新型全屏钓鱼攻击。黑客利用Fullscreen API隐藏地址栏，伪造登录页面诱导用户输入账号密码。攻击通过社交媒体广告等渠道传播，特点是全屏模式下Safari仅显示易被忽略的滑动动画提示（其他浏览器有明确弹窗）。苹果以"动画已足够提示"为由拒绝修复。安全团队建议用户在全屏登录时务必核对网址，警惕虚假界面。该攻击手法在Chrome/Firefox也可实施，但因提示更明显相对安全。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T17:04:21Z
- **目录日期**: 2025-06-03
