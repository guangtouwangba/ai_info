...
黑客正利用Safari浏览器的全屏漏洞实施中间人攻击。攻击者伪造登录页面，通过Fullscreen API隐藏地址栏，仅显示滑动动画提示（易被忽略），诱导用户输入账号密码。该手法在社交媒体广告/评论区传播，Chrome/Firefox会弹出全屏提示，但Safari防御薄弱。苹果以"动画已足够提示"为由拒绝修复。安全建议：全屏登录时务必手动核对网址，勿轻信界面真实性。（99字）  

核心要点：  
1. 攻击手法：全屏隐藏地址栏+伪造登录页  
2. 漏洞特性：Safari仅有滑动动画提示（对比其他浏览器的弹窗提示）  
3. 现状：苹果认定当前设计合理，暂无修复计划  
4. 防御措施：人工核对网址是主要应对方式

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T19:03:26Z
- **目录日期**: 2025-06-03
