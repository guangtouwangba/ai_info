...
Safari浏览器正面临一种新型全屏中间人攻击（BitM）。黑客伪造登录页面诱导用户点击，利用Fullscreen API隐藏地址栏，通过全屏模式掩盖虚假网址。相比Chrome/Firefox的明确提示，Safari仅以滑动动画警示，极易被忽略。该攻击已通过社交媒体广告传播，能绕过多数安全软件检测。尽管安全团队SquareX已向苹果通报，但苹果以"动画已足够警示"为由拒绝修复。专家建议用户在全屏登录时务必手动核对网址，警惕任何看似正常的登录界面。该漏洞同样存在于其他浏览器，但Safari的防护最薄弱。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T12:03:54Z
- **目录日期**: 2025-06-03
