...
黑客正利用Safari浏览器的全屏漏洞实施中间人攻击。攻击者伪造登录页面诱导用户点击，通过Fullscreen API隐藏地址栏，由于Safari全屏时仅显示易被忽略的滑动动画（其他浏览器会有明显提示），用户难以辨别真假登录界面。该攻击手法已出现在社交媒体广告等渠道，能绕过多数安全软件检测。尽管安全团队已向苹果报告，苹果以"动画已足够提示"为由拒绝修复。建议用户在全屏登录时务必手动核对网址，警惕虚假页面。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T18:03:59Z
- **目录日期**: 2025-06-03
