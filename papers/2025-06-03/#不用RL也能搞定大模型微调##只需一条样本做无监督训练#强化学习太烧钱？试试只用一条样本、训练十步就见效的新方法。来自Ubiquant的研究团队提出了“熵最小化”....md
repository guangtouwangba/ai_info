...
Ubiquant团队提出"熵最小化"(EM)方法，无需强化学习(RL)即可高效微调大模型。该方法完全无监督，仅需1条高质量样本和10步训练，通过降低模型输出分布的熵值，强制模型选择最自信的答案。实验显示，EM在数学推理任务中表现优异，甚至超越需要大量数据的RL方法。关键创新在于选择"模型回答不稳定"的问题作为训练样本。例如仅用1道物理题就显著提升了Qwen2.5-Math-7B的数学能力，大幅降低了微调成本和复杂度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T00:03:27Z
- **目录日期**: 2025-06-03
