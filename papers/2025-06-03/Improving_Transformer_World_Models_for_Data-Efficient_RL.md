...
该研究提出了一种基于模型的强化学习（MBRL）方法，在Craftax-classic基准测试中实现了69.66%的奖励表现，超越DreamerV3（53.2%）和人类水平（65%）。该方法结合了CNN和RNN的新型策略架构作为无模型基线，并引入三项改进：(1) "Dyna预热"策略，混合真实和模拟数据训练；(2) 基于图像块的"最近邻标记器"，优化Transformer世界模型(TWM)输入；(3) "块式教师强制"技术，使TWM能联合推理未来时间步的标记。这些设计显著提升了样本效率，仅用100万环境步就达到当前最佳性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T04:05:45Z
- **目录日期**: 2025-06-03
