...
该研究提出了一种基于模型的强化学习（MBRL）新方法，在Craftax-classic开放世界生存游戏中取得69.66%的奖励分数（100万步内），首次超越人类表现（65.0%）并显著优于DreamerV3（53.2%）。方法创新包括：1）结合CNN和RNN的新型策略架构；2）"预热Dyna"混合真实与想象数据训练；3）图像块的"最近邻标记化"改进Transformer世界模型输入；4）"块式教师强制"使模型能联合推理未来标记。这些设计显著提升了样本效率，解决了泛化、深度探索和长期推理等挑战。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T14:02:54Z
- **目录日期**: 2025-06-03
