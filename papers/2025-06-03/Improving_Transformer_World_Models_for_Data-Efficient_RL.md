...
该研究提出了一种基于模型的强化学习（MBRL）方法，在Craftax-classic基准测试中实现了69.66%的奖励表现，超越了DreamerV3（53.2%）和人类水平（65%）。该方法结合了CNN和RNN的新型策略架构，并引入了三项改进：(a) "Dyna预热"策略，混合真实与模拟数据训练；(b) 基于图像块的"最近邻标记化"技术，优化Transformer世界模型（TWM）输入；(c) "块式教师强制"机制，使TWM能联合推理未来时间步的标记。这些设计显著提升了样本效率，仅用100万环境步就实现了当前最佳性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T15:02:17Z
- **目录日期**: 2025-06-03
