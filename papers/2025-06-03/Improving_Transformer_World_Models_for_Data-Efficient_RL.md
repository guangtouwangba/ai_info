...
该研究提出了一种基于模型的强化学习（MBRL）方法，在开放世界2D生存游戏Craftax-classic上取得了69.66%的奖励表现，超越DreamerV3（53.2%）和人类水平（65%）。该方法首先构建了一个结合CNN和RNN的新型策略架构作为无模型基线，然后通过三项改进提升标准MBRL框架：(1) "预热Dyna"策略混合真实与虚拟数据训练；(2) 基于图像块的"最近邻标记器"优化Transformer世界模型(TWM)输入；(3) "块式教师强制"使TWM能联合推理未来标记。这些设计显著提升了样本效率，仅用100万步就实现SOTA性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-03T19:02:09Z
- **目录日期**: 2025-06-03
