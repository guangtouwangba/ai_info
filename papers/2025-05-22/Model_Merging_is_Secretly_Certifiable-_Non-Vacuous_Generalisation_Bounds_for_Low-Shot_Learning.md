...
这篇论文探讨了深度网络在IID（独立同分布）泛化能力认证方面的突破，这对医疗、安全等高风险AI应用至关重要。研究发现，通过模型融合而非微调的方法学习下游任务，可获得非平凡的泛化保证，且认证的泛化差距极小且与基础网络规模无关。实验首次展示了仅用100个样本即可获得有效泛化保证，适用于VIT-B视觉模型和mistral-7B语言模型等大型模型。这一发现为现有系统的可信认证提供了新思路，并开辟了理论与实践结合的研究方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T08:01:35Z
- **目录日期**: 2025-05-22
