...
这篇论文提出了一种新方法，通过模型融合而非微调来认证深度网络的独立同分布(IID)泛化能力。研究发现，只需对现有学习策略稍作调整，就能为小规模数据(低至100个样本)提供非平凡的泛化保证，且与基础网络规模无关。该方法成功应用于VIT-B视觉模型和mistral-7B语言模型，首次实现了使用极少量数据时的可认证泛化性能。这一发现对医学、安全等高风险领域AI系统的可信认证具有重要意义，并为实践与理论的交叉研究开辟了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T09:01:31Z
- **目录日期**: 2025-05-22
