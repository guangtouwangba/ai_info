...
这篇论文提出了一种新方法，通过模型融合而非微调来验证深度网络在独立同分布(IID)数据上的泛化能力。研究发现，只需对现有学习策略稍作调整，就能为小规模高价值数据(如医学、安全领域)提供非平凡的泛化保证。关键突破在于：当使用数据驱动的下游任务融合学习时，认证的泛化差距变得极小且与基础网络规模无关。实验首次展示了在仅100个样本的情况下，视觉模型(VIT-B)和语言模型(mistral-7B)仍能保持有效的泛化保证。这一发现对构建可信AI系统具有直接意义，为理论与实践的结合开辟了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T14:01:35Z
- **目录日期**: 2025-05-22
