...
这篇论文提出了一种新方法，通过模型融合而非微调来验证深度网络在独立同分布(IID)数据上的泛化能力。研究发现，只需对现有学习策略稍作调整，就能为小规模高价值数据(如医疗、安全领域)提供非平凡的泛化保证。关键突破在于：1) 泛化差距与基础网络规模无关；2) 首次实现了仅用100个样本即可验证视觉(VIT-B)和语言模型(mistral-7B)的泛化性能。这一发现不仅为现有系统提供了可信认证路径，也为理论与实践的结合开辟了新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T17:01:30Z
- **目录日期**: 2025-05-22
