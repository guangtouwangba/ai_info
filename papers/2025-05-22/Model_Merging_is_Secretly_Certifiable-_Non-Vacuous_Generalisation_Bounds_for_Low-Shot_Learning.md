...
该研究提出了一种新方法，通过模型融合而非微调来认证深度网络在独立同分布（IID）数据上的泛化能力。研究发现，只需对现有学习策略稍作调整，即可为小规模数据（如100个样本）提供非平凡的泛化保证，且该保证与基础网络规模无关。实验证明，该方法适用于视觉模型（如VIT-B）和语言模型（如mistral-7B）。这一发现对医疗、安全等高风险领域具有重要意义，为现有系统的可信认证提供了新途径，并开辟了理论与实践结合的新研究方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T07:01:40Z
- **目录日期**: 2025-05-22
