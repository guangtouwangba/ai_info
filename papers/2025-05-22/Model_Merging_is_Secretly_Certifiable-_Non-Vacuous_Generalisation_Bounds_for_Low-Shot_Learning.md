...
这篇论文提出了一种新方法，通过模型融合而非微调来认证深度网络在独立同分布(IID)数据上的泛化能力。研究发现，只需对现有学习策略稍作调整，就能获得非平凡的泛化保证，且认证的泛化差距与基础网络规模无关。实验首次展示了在仅使用100个样本的情况下，对VIT-B视觉模型和mistral-7B语言模型仍能获得有效的泛化保证。这一发现对医疗、安全等高风险领域应用可信AI具有重要意义，为现有系统认证提供了新思路，并开辟了理论与实践交叉研究的新方向。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T03:19:31Z
- **目录日期**: 2025-05-22
