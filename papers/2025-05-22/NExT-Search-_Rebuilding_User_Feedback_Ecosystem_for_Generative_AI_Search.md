...
生成式AI搜索通过提供端到端答案简化了信息检索，但也中断了传统搜索依赖用户反馈（如点击、停留时间）改进排名的闭环机制。由于用户仅对最终答案给出粗粒度反馈，难以优化中间环节（查询分解、文档检索、答案生成）。为此，研究提出NExT-Search新范式：1）调试模式允许用户干预关键步骤；2）影子用户模式通过AI代理模拟偏好提供细粒度反馈。这些反馈可实时优化搜索结果（在线适应）或周期性更新模型（离线调整），重建人机协同的改进循环，推动AI搜索持续进化。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T01:28:28Z
- **目录日期**: 2025-05-22
