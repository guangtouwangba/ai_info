...
2025年独立开发者Noel Berry提出"不用游戏引擎手搓游戏"的方法：使用C#（支持热重载和实时调试）配合SDL3等开源库处理视窗/输入/渲染，FMOD处理音频。资源管理采用按需加载，利用LDtk/Tiled等现成工具做关卡设计，Dear ImGui创建UI。C#的Native-AOT编译链和SDL3的跨平台支持简化移植流程。该方法适合小型团队，通过组合轻量级工具链实现高效开发，避免商业引擎的复杂性。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T00:03:51Z
- **目录日期**: 2025-05-22
