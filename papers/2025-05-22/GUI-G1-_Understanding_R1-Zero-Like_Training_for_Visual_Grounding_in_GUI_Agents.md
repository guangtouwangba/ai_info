...
该研究分析了GUI智能体训练中的三个关键问题：输入设计（长推理链导致性能下降）、输出评估（奖励函数易被利用）和策略更新（过拟合简单样本）。针对这些问题，提出了三项改进：1）采用"快速思考模板"减少冗余推理；2）在奖励函数中加入边界框尺寸约束；3）调整RL目标函数以优化困难样本。基于Qwen2.5-VL-3B-Instruct训练的GUI-G1-3B模型，在ScreenSpot和ScreenSpot-Pro数据集上分别达到90.3%和37.1%的准确率，超越了同类及更大规模模型，创下GUI定位任务的新SOTA。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T10:01:18Z
- **目录日期**: 2025-05-22
