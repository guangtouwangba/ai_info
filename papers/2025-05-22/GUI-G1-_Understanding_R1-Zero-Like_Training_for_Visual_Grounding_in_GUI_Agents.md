...
该研究分析了GUI智能体训练中的三个关键问题：输入设计（长推理链导致性能下降）、输出评估（奖励函数易被利用）和策略更新（容易过拟合简单样本）。针对这些问题，研究者提出了快速思考模板（减少冗余推理）、带约束的奖励函数（防止奖励作弊）和难度感知优化策略。基于Qwen2.5-VL-3B-Instruct训练的GUI-G1-3B模型，在17K公开数据上取得了突破性表现：ScreenSpot准确率90.3%，ScreenSpot-Pro 37.1%，超越了同类模型甚至更大的UI-TARS-7B，创造了GUI定位任务的新标杆。项目已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T03:19:05Z
- **目录日期**: 2025-05-22
