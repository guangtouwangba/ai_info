...
这篇论文分析了当前GUI智能体训练中的三个关键问题：输入设计（长推理链导致性能下降）、输出评估（基于框大小的奖励函数引发奖励作弊）和策略更新（RL容易过拟合简单样本）。针对这些问题，研究者提出了三个改进方案：1）采用快速思考模板减少冗余推理；2）在奖励函数中加入框大小约束；3）调整RL目标函数以优化困难样本。最终训练的GUI-G1-3B模型在ScreenSpot数据集上达到90.3%准确率，超越同类模型甚至更大的UI-TARS-7B，创下GUI定位任务的新SOTA。项目代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T07:01:19Z
- **目录日期**: 2025-05-22
