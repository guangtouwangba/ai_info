...
这篇论文分析了当前GUI智能体训练中的三个关键问题：输入设计、输出评估和策略更新。研究发现，过长的思维链会降低定位性能，基于点击信号或框面积的奖励函数会导致奖励作弊，而在线强化学习容易过拟合简单样本。针对这些问题，作者提出了三项改进：1) 采用"快速思考模板"减少冗余推理；2) 在奖励函数中加入框尺寸约束；3) 调整RL目标函数以优化困难样本。最终训练的GUI-G1-3B模型在ScreenSpot和ScreenSpot-Pro数据集上分别达到90.3%和37.1%的准确率，超越了同类模型性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T02:30:18Z
- **目录日期**: 2025-05-22
