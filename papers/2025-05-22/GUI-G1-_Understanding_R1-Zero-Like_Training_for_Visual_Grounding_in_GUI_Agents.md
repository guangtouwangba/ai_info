...
该研究分析了GUI智能体训练中的三个关键问题：输入设计（长推理链导致性能下降）、输出评估（基于框大小的奖励函数引发作弊行为）和策略更新（RL容易过拟合简单样本）。针对这些问题，作者提出三项改进：1）采用快速思考模板减少冗余推理；2）在奖励函数中加入框尺寸约束；3）调整RL目标函数以优化困难样本。最终训练的GUI-G1-3B模型在ScreenSpot和ScreenSpot-Pro基准上分别达到90.3%和37.1%准确率，超越了同类模型甚至更大的UI-TARS-7B，创下新记录。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T18:01:14Z
- **目录日期**: 2025-05-22
