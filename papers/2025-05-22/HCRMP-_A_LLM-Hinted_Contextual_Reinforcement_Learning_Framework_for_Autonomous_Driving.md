...
该研究提出了一种新型LLM-Hinted RL范式，通过将大型语言模型（LLM）与强化学习（RL）结合来提升自动驾驶性能。针对现有方法过度依赖易产生幻觉的LLM输出（非幻觉率仅57.95%）的问题，该方案保持LLM与RL的相对独立性：LLM提供语义提示进行状态增强和策略优化，RL则通过策略学习抵消潜在错误。基于此开发的HCRMP架构包含语义增强、上下文稳定锚和语义缓存三大模块，在CARLA测试中任务成功率高达80.3%，关键场景碰撞率降低11.4%，显著提升了复杂场景下的驾驶表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T02:30:50Z
- **目录日期**: 2025-05-22
