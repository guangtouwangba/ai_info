...
该研究提出了一种新型LLM-Hinted RL范式，通过将大型语言模型（LLMs）与强化学习（RL）相结合来提升自动驾驶性能。针对现有方法过度依赖易产生幻觉的LLM输出（关键驾驶任务中非幻觉率仅57.95%）的问题，该范式保持LLM与RL的相对独立性：LLM生成语义提示辅助RL进行运动规划，而RL通过策略学习纠正潜在错误。基于此开发的HCRMP架构包含状态增强、上下文稳定锚和语义缓存三大模块，在CARLA测试中任务成功率高达80.3%，安全关键场景碰撞率降低11.4%，显著提升了复杂场景下的驾驶表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T07:01:52Z
- **目录日期**: 2025-05-22
