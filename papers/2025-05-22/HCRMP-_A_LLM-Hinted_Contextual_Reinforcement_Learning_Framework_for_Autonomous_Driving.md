...
该研究提出了一种新型LLM-Hinted RL范式，通过结合大语言模型(LLM)和强化学习(RL)来提升自动驾驶性能。针对现有方法过度依赖易产生幻觉的LLM输出(非幻觉率仅57.95%)的问题，该方案保持LLM与RL的相对独立性：LLM生成语义提示辅助RL进行运动规划，而RL通过策略学习纠正潜在错误。基于此提出的HCRMP架构包含状态增强、稳定性锚定和语义缓存三大模块，在CARLA测试中表现出色：任务成功率最高达80.3%，关键安全场景碰撞率降低11.4%，显著提升了复杂场景下的驾驶性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T14:01:46Z
- **目录日期**: 2025-05-22
