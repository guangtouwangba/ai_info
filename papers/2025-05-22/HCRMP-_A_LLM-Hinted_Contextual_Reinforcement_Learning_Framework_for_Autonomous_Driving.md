...
该研究提出了一种新型LLM-Hinted RL范式，通过将大型语言模型（LLM）与强化学习（RL）结合来提升自动驾驶性能。针对当前LLM主导方法易产生幻觉（非幻觉率仅57.95%）的问题，该范式保持LLM与RL的相对独立性：LLM生成语义提示辅助运动规划，RL则通过策略学习纠正潜在错误。基于此设计的HCRMP架构包含状态增强、知识库稳定性锚点和语义缓存三大模块，在CARLA实验中实现80.3%的任务成功率，并在安全关键场景下使碰撞率降低11.4%，显著提升复杂场景下的驾驶表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T06:01:40Z
- **目录日期**: 2025-05-22
