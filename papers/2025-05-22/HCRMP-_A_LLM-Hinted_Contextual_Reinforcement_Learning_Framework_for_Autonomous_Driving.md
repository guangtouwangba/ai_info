...
该研究提出了一种新型LLM-Hinted RL范式，以解决大型语言模型(LLM)在自动驾驶中因幻觉问题(非幻觉率仅57.95%)导致的性能下降。通过保持LLM与强化学习(RL)的相对独立性，利用LLM生成语义提示来辅助状态增强和策略优化，同时RL通过策略学习抵消可能的错误指示。基于此提出的HCRMP架构包含三个模块：增强语义表示模块扩展状态空间，上下文稳定锚模块提升多评价权重提示的可靠性，语义缓存模块整合LLM低频指导与RL高频控制。在CARLA的测试中，HCRMP任务成功率最高达80.3%，碰撞率降低11.4%。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T15:01:46Z
- **目录日期**: 2025-05-22
