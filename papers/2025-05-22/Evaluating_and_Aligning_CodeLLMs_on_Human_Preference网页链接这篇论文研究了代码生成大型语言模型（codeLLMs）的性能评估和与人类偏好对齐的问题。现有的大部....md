...
该论文针对代码生成大模型(codeLLMs)提出新评估框架CodeArena，聚焦人类偏好对齐问题。研究创建了包含397个高质量样本的基准测试(覆盖40类任务和44种语言)，并构建近200亿标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行测试与人类偏好存在偏差；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著。研究强调代码生成模型除正确性外，还需重视与开发者实际需求的匹配度。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T06:06:00Z
- **目录日期**: 2025-05-22
