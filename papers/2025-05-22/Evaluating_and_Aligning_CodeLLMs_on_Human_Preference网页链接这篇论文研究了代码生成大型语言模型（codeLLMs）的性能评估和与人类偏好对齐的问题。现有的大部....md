...
这篇论文针对代码生成大模型(codeLLMs)提出两个关键贡献：1) 创建CodeArena基准测试，包含397个真实编程任务样本，覆盖40类任务和44种语言，首次系统评估模型输出与人类偏好的匹配度；2) 构建20B标记的SynCode-Instruct合成指令集，验证大规模指令微调效果。实验发现：现有基于执行的评估与人类偏好存在显著差异；开源顶级模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距明显。研究强调了代码生成模型与人类偏好对齐的重要性，为改进方向提供了新基准。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T01:33:38Z
- **目录日期**: 2025-05-22
