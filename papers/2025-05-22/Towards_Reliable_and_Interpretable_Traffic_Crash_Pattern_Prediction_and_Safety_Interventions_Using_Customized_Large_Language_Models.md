...
本文提出TrafficSafe框架，利用大语言模型（LLMs）将交通事故预测重构为基于文本的推理任务。该研究整合了58,903份包含基础设施、环境、驾驶员和车辆信息的多模态事故报告，构建TrafficSafe事件数据集。经微调的LLM模型F1分数比基线提升42%。通过提出的TrafficSafe Attribution特征归因框架发现：酒驾是严重事故的首要因素，其风险贡献是其他驾驶行为的两倍。该框架还能识别关键特征，指导数据收集以持续优化模型。TrafficSafe为交通安全研究提供了可解释、可行动的AI解决方案，具有转化应用潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T07:02:33Z
- **目录日期**: 2025-05-22
