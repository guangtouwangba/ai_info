...
本文提出TrafficSafe框架，利用大语言模型（LLMs）将交通事故预测重构为基于文本的推理任务。该研究整合了58,903份包含基础设施、环境、驾驶员和车辆信息的多模态事故报告，构建TrafficSafe事件数据集。实验显示，定制化微调的TrafficSafe LLM模型F1分数比基线方法平均提升42%。通过创新的句子级特征归因框架TrafficSafe Attribution，研究发现酒驾是严重事故的首要因素，其风险贡献度是其他驾驶行为的两倍。该框架还能识别关键特征，指导事故数据收集策略，为交通安全研究提供了可解释、可操作的AI解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T09:02:21Z
- **目录日期**: 2025-05-22
