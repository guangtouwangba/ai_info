...
本文提出TrafficSafe框架，利用大语言模型（LLMs）将交通事故预测重构为基于文本的推理任务。该研究构建了一个包含58,903份真实事故报告的多模态数据集，整合了基础设施、环境、驾驶员和车辆信息。经微调的TrafficSafe模型F1分数比基线方法平均提升42%。通过创新的句子级特征归因框架TrafficSafe Attribution发现：酒驾是严重事故的首要因素，其风险贡献是其他驾驶行为的两倍。该框架还能识别关键特征，指导数据收集策略优化。研究为将AI技术转化为可行动的交通安全解决方案提供了创新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T06:02:20Z
- **目录日期**: 2025-05-22
