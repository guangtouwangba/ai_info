...
本文提出TrafficSafe框架，利用大语言模型（LLMs）将交通事故预测重构为基于文本的推理任务。该研究收集了58,903份包含基础设施、环境、驾驶员和车辆信息的多模态事故报告，构建TrafficSafe事件数据集。实验表明，定制化微调的LLM模型F1分数比基线方法平均提高42%。通过提出的句子级特征归因框架TrafficSafe Attribution发现：酒驾是严重事故的首要因素，其风险贡献度是其他驾驶行为的两倍。该框架还能识别关键特征，指导事故数据收集策略，为将AI技术转化为可行动的交通安全解决方案提供了创新范式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T18:02:25Z
- **目录日期**: 2025-05-22
