...
这篇论文介绍了一个名为AgentSociety的大规模社会模拟器，它结合了大型语言模型（LLM）驱动的智能代理、真实社会环境和强大的模拟引擎。研究者模拟了超过1万个代理的社会生活及500万次互动，并利用该平台研究了四个关键社会问题：极化现象、刺激性信息传播、无条件基本收入政策影响和外部冲击（如飓风）。模拟结果与现实世界实验数据吻合，验证了该系统捕捉人类行为机制的能力，展示了其作为社会科学研究和政策制定工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T18:05:12Z
- **目录日期**: 2025-05-22
