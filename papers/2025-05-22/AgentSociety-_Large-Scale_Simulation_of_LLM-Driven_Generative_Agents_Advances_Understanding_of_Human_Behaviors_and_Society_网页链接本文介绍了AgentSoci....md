...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它分析了四个关键社会问题：群体极化、刺激性信息传播、基本收入政策影响和自然灾害（如飓风）的社会效应。模拟结果与现实世界数据高度吻合，验证了该系统捕捉人类行为机制的能力。该平台可作为传统社会科学研究方法（调查、访谈等）的补充工具，帮助研究者和政策制定者理解复杂社会问题的成因与模式。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T23:04:44Z
- **目录日期**: 2025-05-22
