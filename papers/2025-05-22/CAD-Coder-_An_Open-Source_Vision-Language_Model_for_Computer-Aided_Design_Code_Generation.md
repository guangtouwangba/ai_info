...
本文介绍了CAD-Coder，一个基于视觉语言模型（VLM）的开源工具，能够直接从视觉输入生成可编辑的CAD代码（CadQuery Python）。通过使用包含16.3万张CAD模型图像和代码对的新数据集GenCAD-Code，CAD-Coder在语法正确率和3D实体相似度上超越了GPT-4.5等先进模型，达到100%的语法有效性。该模型还展现出一定的泛化能力，能够处理真实世界图像和未在训练中见过的CAD操作。CAD-Coder有望显著提升工程设计的效率，减少成本和时间。代码已在GitHub公开。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T00:02:21Z
- **目录日期**: 2025-05-22
