...
该研究探讨了开发高效、安全的窄域AI系统的两个关键挑战。首先，实验发现某些窄域技能需要基于广泛数据训练才能掌握，特别是当技能存在层级依赖时，宽分布数据能形成有效学习路径。其次，研究尝试将大模型的特定技能迁移到小模型中，发现技能通常无法完全定位到特定可剪枝组件，但剪枝方法仍优于蒸馏法。作者提出使用正则化目标来对齐目标技能与可剪枝组件，同时遗忘不必要技能。这些发现为开发专业化的高效AI系统提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T22:01:00Z
- **目录日期**: 2025-05-22
