...
该研究探讨了构建高效、安全的专用窄AI系统的两大挑战。首先，实验发现某些窄技能需要基于广泛数据训练才能掌握，因为技能间存在层级依赖关系，宽分布数据能形成加速学习的内在课程。其次，研究发现模型技能通常无法完全定位到特定可剪枝组件中，但基于剪枝的方法仍优于蒸馏法。研究者提出通过正则化目标使目标技能与可剪枝组件对齐，同时消除不必要技能。这些发现为开发专业化小型模型提供了重要见解，在效率和安全性方面具有应用价值。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T04:02:37Z
- **目录日期**: 2025-05-22
