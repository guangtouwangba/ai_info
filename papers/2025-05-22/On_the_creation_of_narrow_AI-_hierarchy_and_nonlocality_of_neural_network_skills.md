...
该研究探讨了开发高效、安全的专用窄AI系统的两大挑战。首先，实验发现某些窄技能需要基于广泛数据训练才能掌握，尤其是当技能存在层级依赖时，宽分布数据能形成学习梯度从而加速训练。其次，研究发现从大模型迁移特定技能到小模型时，技能往往无法完全定位到特定可剪枝组件，但剪枝方法仍优于蒸馏法。研究者提出通过正则化目标来对齐目标技能与可剪枝组件，同时遗忘不必要技能。这项工作为开发专业化窄AI系统提供了重要见解。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T17:00:57Z
- **目录日期**: 2025-05-22
