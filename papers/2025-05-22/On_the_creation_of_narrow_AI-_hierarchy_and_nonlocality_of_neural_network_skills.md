...
该研究探讨了构建高效、安全的专用窄AI系统的挑战。研究发现：1) 训练窄模型有时需要宽分布数据，因为技能之间存在层级依赖，宽分布能形成加速学习的内在课程；2) 从通用大模型迁移特定技能到小模型时，技能往往无法完全定位到特定可剪枝组件，但剪枝方法仍优于蒸馏。研究提出使用正则化目标来对齐目标技能与可剪枝组件，同时遗忘不必要技能。这些发现为开发专业窄AI系统提供了重要见解，涉及训练策略和模型压缩方法。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T18:01:04Z
- **目录日期**: 2025-05-22
