...
该研究探讨了开发高效、安全的专用窄AI系统的两个关键挑战。首先，实验发现某些窄技能必须通过广泛数据分布训练才能掌握，因为技能间存在层级依赖关系，广泛数据能提供加速学习的内在课程。其次，研究发现从通用大模型向专用小模型迁移技能时，技能往往无法完全定位到特定可剪枝组件，但剪枝方法仍优于蒸馏法。研究提出通过正则化目标使所需技能与可剪枝组件对齐，同时遗忘不必要技能。这些发现为开发高效窄AI系统提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T14:01:02Z
- **目录日期**: 2025-05-22
