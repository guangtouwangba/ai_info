...
该研究探讨了创建高效、安全的专用窄AI系统面临的挑战。研究发现：1) 训练窄模型时，有时需要基于广泛数据分布才能学习其中的特定窄技能，尤其是当技能存在层级依赖时，宽分布能形成加速学习的内在课程；2) 从通用大模型迁移特定技能到小模型时，技能往往无法完美定位到特定可剪枝组件，但剪枝方法仍优于蒸馏法。研究提出通过正则化目标使所需技能与可剪枝组件对齐，同时遗忘不必要技能。这项工作为开发专业化AI系统提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T15:00:59Z
- **目录日期**: 2025-05-22
