...
该研究探讨了创建高效、安全的窄域AI系统的两大挑战。首先，实验发现某些窄域技能需要基于广泛数据训练才能掌握，尤其是当技能存在层级依赖时，宽分布数据能形成有效学习路径。其次，研究发现从通用大模型向专用小模型迁移技能时，技能往往无法完全定位到特定可剪枝组件，但剪枝方法仍优于蒸馏法。研究还探索了通过正则化目标对齐技能与可剪枝组件，同时消除不必要技能的方法。这些发现为开发专业化小型AI系统提供了重要见解。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T10:01:05Z
- **目录日期**: 2025-05-22
