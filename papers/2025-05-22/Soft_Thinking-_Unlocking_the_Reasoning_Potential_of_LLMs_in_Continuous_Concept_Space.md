...
本文提出"软思维"(Soft Thinking)方法，突破传统基于离散语言符号的推理限制。该方法通过生成连续概念空间中的抽象概念标记，实现类似人类的"软"推理。这些概念标记由概率加权的词嵌入混合构成，能同时包含多个相关离散标记的含义，隐式探索多种推理路径。实验表明，在数学和编程基准测试中，该方法将pass@1准确率最高提升2.48个百分点，同时减少22.4%的标记使用量。定性分析显示其输出仍保持高度可解释性，有望突破离散语言推理的固有瓶颈。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T04:03:46Z
- **目录日期**: 2025-05-22
