...
本文提出"软思考"(Soft Thinking)方法，突破传统基于离散语言标记的推理限制。该方法通过生成连续概念空间中的抽象概念标记，模拟人类"软"推理过程。这些概念标记由概率加权的标记嵌入混合而成，能实现语义平滑过渡和更丰富的表示，同时隐含探索多种推理路径。实验表明，在数学和编程基准测试中，该方法相比标准思维链(CoT)可将pass@1准确率提升2.48分，同时减少22.4%的标记使用。定性分析显示其输出仍保持高度可解释性，有望突破基于离散语言的推理瓶颈。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T02:31:21Z
- **目录日期**: 2025-05-22
