...
该研究提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中依赖自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，解决了传统SGD训练Transformer时的平衡性问题，同时将内存占用减半。实验表明，该方法在Transformer任务、ImageNet分类和LLM预训练（如GPT-2、Llama2-7B）中性能媲美或超越AdamW，尤其在内存效率上优势显著（节省5.93GB~25.15GB），且对超参数变化更鲁棒，适用于LoRA微调等场景。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T16:04:35Z
- **目录日期**: 2025-05-22
