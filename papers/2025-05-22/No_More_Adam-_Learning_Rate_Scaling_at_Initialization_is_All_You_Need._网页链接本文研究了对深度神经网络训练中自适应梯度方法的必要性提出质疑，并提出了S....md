...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了深度神经网络训练中必须使用自适应梯度方法（如Adam）的传统观念。SGD-SaI是对带动量的随机梯度下降（SGDM）的改进，通过根据各参数组在初始化时的梯度信号噪声比（g-SNR）调整学习率，避免了训练不平衡问题。相比AdamW，SGD-SaI内存使用减半，在Transformer任务、ImageNet分类和LLM预训练中表现优异，甚至在LoRA微调中超越现有优化器。此外，它在GPT-2和Llama2-7B训练中分别节省了5.93GB和25.15GB内存，具有显著的效率优势。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T02:33:54Z
- **目录日期**: 2025-05-22
