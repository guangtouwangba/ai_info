...
这篇论文提出了SGD-SaI方法，质疑了深度神经网络训练中自适应梯度方法的必要性。该方法通过初始化时根据不同参数组的梯度信号噪声比(g-SNR)调整学习率，无需自适应二阶动量即可防止训练不平衡，并将优化器内存使用减半。实验表明，SGD-SaI在Transformer任务、ImageNet分类和GPT-2预训练中表现优于或持平AdamW，且在LoRA微调中超越现有优化器。内存效率方面，相比AdamW节省显著：GPT-2节省5.93GB，Llama2-7B节省25.15GB。该方法具有超参数鲁棒性和广泛适用性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T08:04:54Z
- **目录日期**: 2025-05-22
