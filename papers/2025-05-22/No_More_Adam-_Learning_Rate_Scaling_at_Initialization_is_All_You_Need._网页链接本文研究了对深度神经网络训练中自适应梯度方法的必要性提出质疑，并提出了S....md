...
该研究提出SGD-SaI方法，质疑自适应梯度方法（如Adam）的必要性。SGD-SaI通过初始化时基于梯度信噪比（g-SNR）调整不同参数组的学习率，无需自适应二阶动量即可解决训练不平衡问题，并将内存使用减半。实验显示，在Transformer任务、ImageNet分类、GPT-2预训练及LoRA微调中，其性能匹配或优于AdamW，且对超参数更鲁棒。内存效率显著，如为GPT-2节省5.93GB，为Llama2-7B节省25.15GB。该方法为训练大模型提供了高效替代方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T20:04:26Z
- **目录日期**: 2025-05-22
