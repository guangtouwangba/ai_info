...
知识蒸馏技术通过大模型(教师模型)指导小模型(学生模型)提升性能，主要包括三种方法：(1)软标签蒸馏：学生模型学习教师模型输出的概率分布；(2)硬标签蒸馏：学生模型直接模仿教师模型的最终分类结果；(3)协同蒸馏：多个模型互为师生，通过相互学习软标签实现共同提升。这些方法使小模型能高效继承大模型的知识，在保持较小体积的同时获得接近大模型的性能表现。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T14:03:40Z
- **目录日期**: 2025-05-22
