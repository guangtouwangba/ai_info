...
知识蒸馏技术使小型AI模型(学生)能向大型模型(教师)高效学习。主要分为三种方法：(1)软标签蒸馏：学生模型学习教师输出的概率分布；(2)硬标签蒸馏：学生直接模仿教师的最终分类结果；(3)协同蒸馏：多个模型互为师生，通过交换软标签相互提升。这些技术通过不同方式传递知识，帮助小模型获得接近大模型的性能，同时保持较小的计算开销。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T09:02:53Z
- **目录日期**: 2025-05-22
