...
知识蒸馏技术使小型AI模型(学生模型)能向大型AI模型(教师模型)高效学习，主要分为三种方法：(1)软标签蒸馏：学生模型匹配教师模型的概率输出分布；(2)硬标签蒸馏：学生模型直接学习教师模型的最终预测类别；(3)协同蒸馏：多个模型互为师生，通过交换软标签和接收真实标签共同提升。这些技术通过不同方式传递大模型的知识，帮助小模型获得接近大模型的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T19:03:30Z
- **目录日期**: 2025-05-22
