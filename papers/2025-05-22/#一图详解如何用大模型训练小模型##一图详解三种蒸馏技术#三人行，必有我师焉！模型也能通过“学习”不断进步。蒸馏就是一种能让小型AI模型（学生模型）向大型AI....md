...
知识蒸馏技术使小型AI模型(学生)能向大型模型(教师)高效学习，主要包括三种方法：(1)软标签蒸馏：学生模型通过匹配教师输出的概率分布学习；(2)硬标签蒸馏：学生直接学习教师的最终预测类别(one-hot标签)；(3)协同蒸馏：多个模型互为师生，通过交换软标签和真实标签共同提升。这些技术通过不同方式传递知识，实现模型压缩和性能提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T22:03:34Z
- **目录日期**: 2025-05-22
