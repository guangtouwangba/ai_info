...
微软研究院开发的AI天气预报系统Aurora在多个关键领域超越传统数值模型：5天全球空气污染预报（74%指标更优）、10天海浪预报（86%更优）、热带气旋路径预报（100%更优）及10天全球天气预报（92%指标领先）。该系统采用3D Swin Transformer架构，通过百万小时地球数据预训练，能以极低成本适配新任务。相比传统方法，Aurora计算成本降低数个数量级，在分辨率和极端天气预测方面表现突出，未来可扩展至海洋环流、植被监测等领域。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T12:03:49Z
- **目录日期**: 2025-05-22
