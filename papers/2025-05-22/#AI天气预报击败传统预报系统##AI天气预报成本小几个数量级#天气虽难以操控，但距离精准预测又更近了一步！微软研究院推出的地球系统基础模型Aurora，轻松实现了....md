...
微软研究院开发的AI天气预测模型Aurora在多个关键领域超越传统数值预报系统：5天全球空气污染预报(74%指标领先)、10天海浪预报(86%领先)、热带气旋路径预报(100%领先)及10天全球天气预报(92%指标占优)。该模型采用3D Swin Transformer架构，通过百万小时地球系统数据预训练，具备低成本微调优势，计算成本比传统方法低数个数量级。Aurora的三大核心组件(编码器-处理器-解码器)实现了多源数据统一处理，可扩展应用于海洋环流、植被监测等地球系统预测任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T13:12:58Z
- **目录日期**: 2025-05-22
