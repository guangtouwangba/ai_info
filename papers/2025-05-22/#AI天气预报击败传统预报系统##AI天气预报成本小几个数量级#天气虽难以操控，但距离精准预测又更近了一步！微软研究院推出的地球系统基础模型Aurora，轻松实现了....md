...
微软研究院开发的AI天气预报系统Aurora在多领域超越传统数值模型：5天全球空气污染预报（74%指标更优）、10天海浪预报（86%更优）、热带气旋路径预报（100%领先）、10天全球天气预报（92%指标更优）。其核心采用3D Swin Transformer U-Net架构，通过百万小时地球数据预训练，支持低成本微调适配新任务。该系统显著降低计算成本（小几个数量级），并具备高分辨率预测优势，未来可扩展至海洋环流、植被监测等领域。论文详见arXiv:2405.13063。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T11:03:44Z
- **目录日期**: 2025-05-22
