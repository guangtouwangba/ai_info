...
微软研究院开发的AI天气预测模型Aurora在多领域超越传统数值模型：5天全球空气污染预报(74%指标领先)、10天海浪预报(86%领先)、热带气旋路径预报(100%领先)及10天全球天气预报(92%指标占优)。该模型采用3D Swin Transformer架构，通过百万小时地球系统数据预训练，具备低成本微调优势，计算成本比传统方法低数个数量级。其核心由编码器(数据统一处理)、处理器(时序演化)和解码器(结果输出)组成，可扩展应用于海洋环流、植被监测等地球系统预测任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T18:03:59Z
- **目录日期**: 2025-05-22
