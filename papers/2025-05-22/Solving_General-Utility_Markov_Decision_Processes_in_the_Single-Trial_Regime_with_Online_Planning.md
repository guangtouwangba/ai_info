...
本文首次提出了在单次试验场景下解决无限时域折扣通用效用马尔可夫决策过程(GUMDP)的方法。研究首先分析了单次试验条件下的策略优化基础问题，包括最优策略类别、问题转化等效MDP及计算复杂度；其次提出利用在线规划技术(特别是蒙特卡洛树搜索算法)求解该场景下的GUMDP；最后实验表明该方法优于现有基线模型。该工作为单轨迹评估场景下的决策优化提供了理论框架和实用算法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T14:02:05Z
- **目录日期**: 2025-05-22
