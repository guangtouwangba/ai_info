...
本文提出了首个解决无限时域折扣通用效用马尔可夫决策过程（GUMDP）单次试验优化的方法。研究首先分析了单次试验场景下的策略优化基础问题，包括最优策略类别、等价MDP转换以及计算复杂度。其次，通过在线规划技术（特别是蒙特卡洛树搜索算法）实现了单次试验GUMDP求解。实验结果表明，该方法在性能上显著优于基线模型。该研究为单次评估场景下的决策优化提供了理论框架和实用算法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-05-22T03:20:04Z
- **目录日期**: 2025-05-22
