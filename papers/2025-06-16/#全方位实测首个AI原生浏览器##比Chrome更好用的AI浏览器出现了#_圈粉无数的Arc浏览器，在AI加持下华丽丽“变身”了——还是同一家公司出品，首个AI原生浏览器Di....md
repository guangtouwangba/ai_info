...
Dia浏览器是Arc团队推出的首个AI原生浏览器，已启动内测。其核心功能包括：直接与任意网页对话（如电商比价、视频总结）、智能比较不同网页内容、自动获取标签页上下文等。目前最受欢迎的是与YouTube、谷歌日历等网页的交互功能。测试显示，该浏览器能实现商品比价、视频定位答疑等实用场景，用户体验优于传统浏览器。现有Arc会员可优先体验，虽处测试阶段但已获积极反馈，被部分用户视为Chrome的潜在替代品。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-16T00:02:33Z
- **目录日期**: 2025-06-16
