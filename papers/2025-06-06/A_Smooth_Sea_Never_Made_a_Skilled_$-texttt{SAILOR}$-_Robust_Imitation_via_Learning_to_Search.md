...
行为克隆（BC）方法在模仿学习中的根本局限是只能模仿专家在特定状态下的行为，一旦偏离专家示范的状态，代理往往无法恢复。为解决这一问题，研究者提出从专家示范中学习搜索（L2S）的方法，通过训练世界模型和奖励模型，使代理能在测试时自主规划以匹配专家目标，即使犯错后也能恢复。该方法（SAILOR）在12项视觉操作任务中表现优于当前最先进的扩散策略（BC），即使BC使用5-10倍数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊。代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T10:02:13Z
- **目录日期**: 2025-06-06
