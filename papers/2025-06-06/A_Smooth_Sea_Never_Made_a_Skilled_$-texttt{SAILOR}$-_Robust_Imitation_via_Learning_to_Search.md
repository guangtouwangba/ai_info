...
行为克隆（BC）方法在模仿学习中的根本局限在于，它只能让智能体模仿专家在特定状态下的行为。一旦智能体犯错并偏离专家示范的状态，它往往无法自主恢复。为此，研究者提出了基于专家示范的"学习搜索"（L2S）方法，通过训练世界模型和奖励模型，使智能体能在测试时规划恢复策略。新方法SAILOR在12项视觉操作任务中均优于当前最先进的扩散策略（Diffusion Policies），即使BC使用5-10倍示范数据仍存在性能差距。SAILOR能识别细微错误并避免奖励作弊，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T11:02:00Z
- **目录日期**: 2025-06-06
