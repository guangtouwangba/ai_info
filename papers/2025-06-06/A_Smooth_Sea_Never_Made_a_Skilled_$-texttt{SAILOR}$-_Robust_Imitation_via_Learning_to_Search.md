...
行为克隆(BC)模仿学习存在根本局限：它仅教会智能体在专家访问过的状态下模仿专家行为。当BC智能体犯错并偏离演示数据范围时，往往无法自主恢复。为此，研究者提出从专家演示中学习搜索(L2S)的方法$\texttt{SAILOR}$，通过学习世界模型和奖励模型，使智能体能在测试时规划恢复策略。在12项视觉操作任务上的实验表明，$\texttt{SAILOR}$显著优于基于相同数据的扩散策略BC方法，即使BC使用5-10倍演示数据仍存在性能差距。该方法能识别细微错误并避免奖励欺骗，代码已开源。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T18:01:57Z
- **目录日期**: 2025-06-06
