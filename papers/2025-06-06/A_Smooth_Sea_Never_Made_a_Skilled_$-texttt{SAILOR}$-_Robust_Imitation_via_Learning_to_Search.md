...
行为克隆（BC）方法在模仿学习中的核心局限是只能模仿专家在特定状态下的行为，一旦偏离专家示范状态，智能体难以自主恢复。为此，研究者提出"学习搜索"（L2S）框架SAILOR，通过构建世界模型和奖励模型，使智能体能在测试时自主规划恢复路径，无需额外人工修正。在12项视觉操作任务测试中，SAILOR显著优于基于相同数据的扩散策略BC方法，即使BC数据量扩大5-10倍仍存在性能差距。该方法能识别细微错误并抵抗奖励作弊，代码已开源。（99字）

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T13:10:32Z
- **目录日期**: 2025-06-06
