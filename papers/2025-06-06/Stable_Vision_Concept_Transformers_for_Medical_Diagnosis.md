...
本文针对医疗领域对AI透明性的需求，提出两种改进的概念瓶颈模型（CBM）。Vision Concept Transformer（VCT）通过融合概念特征与图像特征，解决了现有CBM仅依赖概念特征导致性能下降的问题。进一步提出的Stable Vision Concept Transformer（SVCT）基于VCT框架，结合视觉Transformer架构和去噪扩散平滑技术，在保持可解释性的同时提升了模型鲁棒性。在四个医疗数据集上的实验表明，VCT/SVCT在保持准确性的同时提供了稳定的解释能力，特别是SVCT能在输入扰动下保持解释可靠性，满足了医疗领域对AI系统的关键需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T07:02:26Z
- **目录日期**: 2025-06-06
