...
本文提出首个基于AI的准粒子干涉(QPI)核提取框架，通过两步学习策略解决多散射源图像的反演问题。首先使用变分自编码器学习散射核的紧凑潜在空间表示，随后通过专用编码器将QPI观测数据与预学习核的潜在表示对齐。这种设计使模型能在复杂纠缠散射条件下稳健推断核函数。研究构建了包含100种独特核的物理真实QPI数据集，实验表明该方法相比直接单步基线具有显著更高的提取精度，对未见核的泛化能力也更强。该成果为量子材料电子结构研究提供了新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T21:01:26Z
- **目录日期**: 2025-06-06
