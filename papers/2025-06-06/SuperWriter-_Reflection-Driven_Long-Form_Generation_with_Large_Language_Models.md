...
为了解决大语言模型（LLM）生成长文本时存在的连贯性、逻辑一致性和质量下降问题，研究者提出了SuperWriter-Agent框架。该框架通过结构化思维（规划与优化阶段）模拟专业作者的写作流程，并基于此训练了一个7B参数的SuperWriter-LM模型。研究采用分层直接偏好优化（DPO）结合蒙特卡洛树搜索（MCTS）来优化生成质量。实验表明，该模型在多项基准测试中表现优异，超越更大规模的基线模型，验证了分层DPO和结构化思维步骤对提升长文本生成质量的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T03:22:16Z
- **目录日期**: 2025-06-06
