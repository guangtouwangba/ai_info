...
为了解决大语言模型在生成长文本时面临的一致性、逻辑性和质量下降问题，研究者提出了**SuperWriter-Agent**框架。该框架通过结构化思维（规划与优化阶段）模拟专业作者的写作流程，并基于此训练了一个7B参数的**SuperWriter-LM**模型。结合分层直接偏好优化（DPO）和蒙特卡洛树搜索（MCTS）技术，模型能根据最终质量反馈优化生成步骤。实验表明，该模型在多项基准测试中表现最优，超越更大规模的基线模型，验证了分层DPO和结构化思维对提升长文本生成质量的有效性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T04:05:05Z
- **目录日期**: 2025-06-06
