...
这篇论文探讨了代码生成大模型(codeLLMs)与人类偏好对齐的问题。研究者创建了CodeArena基准测试，包含397个高质量样本，覆盖40个类别和44种编程语言，以评估模型在真实编程任务中的表现。同时开发了SynCode-Instruct合成指令数据集(约20B标记)验证大规模微调效果。实验发现传统执行基准与CodeArena存在差异，开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI o1)性能差距显著，凸显了人类偏好对齐的重要性。研究强调了代码生成模型不仅需要正确性，还需符合开发者实际需求。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T07:06:23Z
- **目录日期**: 2025-06-06
