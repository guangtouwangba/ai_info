...
这篇论文探讨了代码生成大语言模型(codeLLMs)与人类偏好的对齐问题。研究者创建了CodeArena基准测试，包含397个高质量编程任务样本(覆盖40类44种语言)，并构建了20B标记的合成指令数据集SynCode-Instruct。实验发现：1)传统执行基准与人类偏好基准存在性能差异；2)开源模型(如Qwen2.5-Coder)与商业模型(如OpenAI)在人类偏好对齐上差距显著。研究强调了除代码正确性外，模型输出与开发者实际需求匹配的重要性。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T03:26:35Z
- **目录日期**: 2025-06-06
