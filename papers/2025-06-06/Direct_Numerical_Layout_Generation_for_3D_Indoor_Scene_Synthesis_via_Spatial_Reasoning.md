...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该框架通过三阶段流程实现：首先生成俯视图布局，再将其提升至3D空间，最后优化物体摆放。研究采用基于3D-Front数据集的思维链（CoT）激活方法来增强空间推理能力，并设计了CoT-Grounded生成式布局奖励机制以提高泛化性。实验表明，DirectLayout能有效处理开放词汇场景，在语义一致性、泛化能力和物理合理性方面表现优异，解决了现有方法在灵活性和用户指令对齐方面的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T21:01:05Z
- **目录日期**: 2025-06-06
