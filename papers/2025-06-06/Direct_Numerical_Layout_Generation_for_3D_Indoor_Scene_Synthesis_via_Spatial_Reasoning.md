...
这篇论文提出了DirectLayout框架，用于从文本描述直接生成3D室内场景布局。该方法利用大语言模型的空间推理能力，通过三个步骤生成布局：鸟瞰图生成、3D空间提升和物体位置优化。研究采用基于3D-Front数据集的思维链激活技术来增强空间推理，并设计了生成式布局奖励机制来提高泛化能力。推理阶段通过上下文学习迭代调整资产与布局的匹配。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，解决了现有方法在开放词汇和细粒度控制方面的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T14:00:54Z
- **目录日期**: 2025-06-06
