...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该框架通过三阶段流程：首先生成鸟瞰图布局，然后提升到3D空间，最后优化物体摆放。研究采用基于3D-Front数据集的思维链（CoT）激活来增强空间推理能力，并设计了CoT-Grounded生成式布局奖励机制以提高泛化性。在推理阶段，通过上下文学习实现迭代式资产-布局对齐，解决了资产与布局不匹配的问题。实验表明，该方法在语义一致性、泛化能力和物理合理性方面表现优异。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T13:09:31Z
- **目录日期**: 2025-06-06
