...
这篇论文提出了DirectLayout框架，用于根据文本描述直接生成3D室内场景布局。该方法利用大语言模型的空间推理能力，将生成过程分为三个步骤：首先生成俯视图布局，然后提升到3D空间，最后优化物体摆放。研究采用基于3D-Front数据集的思维链激活技术来增强空间推理，并设计了奖励机制来提高泛化能力。在推理阶段，通过上下文学习进行迭代优化以解决资产与布局的匹配问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，克服了现有方法灵活性不足的问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T11:00:59Z
- **目录日期**: 2025-06-06
