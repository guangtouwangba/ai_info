...
这篇论文提出了DirectLayout框架，利用大语言模型(LLMs)的空间推理能力直接从文本描述生成3D室内场景布局。该方法将生成过程分为三个阶段：首先生成俯视图布局，然后提升到3D空间，最后优化物体摆放。通过基于3D-Front数据集的思维链激活和生成式布局奖励机制，增强了模型的空间规划和泛化能力。在推理阶段，采用上下文学习进行迭代式资产-布局对齐，解决不匹配问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，优于现有方法。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T20:01:05Z
- **目录日期**: 2025-06-06
