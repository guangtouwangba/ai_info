...
这篇论文提出了DirectLayout框架，利用大语言模型（LLMs）的空间推理能力直接从文本描述生成3D室内场景布局。该方法分三个阶段：首先生成俯视图布局，再将其提升到3D空间，最后优化物体摆放。通过3D-Front数据集激活思维链（CoT）机制，增强空间推理能力，并设计了基于CoT的生成式布局奖励机制来提升泛化性。推理时采用上下文学习进行迭代式资产-布局对齐，解决不匹配问题。实验表明，DirectLayout在语义一致性、泛化能力和物理合理性方面表现优异，克服了现有方法在开放词汇和细粒度控制上的不足。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T22:01:01Z
- **目录日期**: 2025-06-06
