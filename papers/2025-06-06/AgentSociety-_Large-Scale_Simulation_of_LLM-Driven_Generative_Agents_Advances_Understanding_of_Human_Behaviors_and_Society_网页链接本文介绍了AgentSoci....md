...
AgentSociety是一个基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次交互）。研究者用它分析四个关键社会问题：极化现象、刺激性信息传播、无条件基本收入政策影响和飓风等外部冲击。模拟结果与现实世界数据吻合，验证了该工具在揭示人类行为机制方面的有效性。该系统可作为计算社会科学实验平台，辅助传统研究方法（如调查、访谈），帮助研究者探索社会问题的成因与模式，为政策制定提供新工具。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T13:13:57Z
- **目录日期**: 2025-06-06
