...
AgentSociety是一种基于大语言模型的大规模社会模拟器，可模拟1万多个代理的社会互动（500万次）。它能研究社会极化、信息传播、基本收入政策及外部冲击（如飓风）等社会问题，结果与现实数据吻合。该系统结合代理模型、真实环境和大规模模拟引擎，可作为计算社会科学实验平台，辅助传统研究方法（如调查、访谈），帮助理解社会问题的模式和机制。其准确性验证了模拟人类行为的能力，展现了作为社会科学研究和政策制定工具的潜力。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T04:08:41Z
- **目录日期**: 2025-06-06
