...
本文提出了"控制税"(Control Tax)的概念，用于衡量将AI控制措施整合到AI流程中的操作和财务成本。研究主要贡献包括：(1)建立量化控制税的理论框架，将分类器性能映射到安全保证；(2)在对抗环境下评估最先进语言模型，研究攻击模型植入代码后门与监控模型检测漏洞的能力；(3)提供控制协议的实际财务成本估算，并开发优化监控策略，在审计预算等实际约束下平衡安全性与成本效益。该框架通过系统连接安全保证与成本，帮助从业者做出明智决策，推动AI控制领域的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T18:01:46Z
- **目录日期**: 2025-06-06
