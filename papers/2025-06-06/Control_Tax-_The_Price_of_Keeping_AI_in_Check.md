...
本文提出"控制税"概念，量化AI控制措施的操作和财务成本。研究贡献包括：(1)建立理论框架，将分类器性能映射到安全保证；(2)在对抗环境下评估先进语言模型，测试模型检测代码后门的能力；(3)估算控制协议的实际成本，开发兼顾安全与成本效益的优化监控策略。该框架通过系统连接安全保证与其成本，帮助从业者在不同部署场景下做出基于经济可行性的决策，推动AI控制领域的实际应用。研究重点关注高风险应用中AI代理的监管机制及其实现成本问题。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T07:02:06Z
- **目录日期**: 2025-06-06
