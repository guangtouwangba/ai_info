...
本文提出"控制税"(Control Tax)概念,用于衡量AI系统中集成安全控制措施的操作和财务成本。研究构建了量化控制税的理论框架,将分类器性能映射到安全保障;评估了先进语言模型在对抗性攻击(如代码后门插入)中的表现;并提供了控制协议的实际成本估算。通过开发优化监控策略,研究在审计预算等实际约束下平衡安全性和成本效益。该框架系统性地连接安全保障与其成本,帮助从业者做出明智决策,推动AI控制领域(AIC)在不同部署场景中的经济可行性评估。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T16:01:58Z
- **目录日期**: 2025-06-06
