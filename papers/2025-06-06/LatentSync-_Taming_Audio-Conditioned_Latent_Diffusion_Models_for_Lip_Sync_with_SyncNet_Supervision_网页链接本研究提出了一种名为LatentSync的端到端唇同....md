...
本研究提出LatentSync框架，利用音频条件的潜在扩散模型实现端到端唇同步，无需中间运动表示。该方案通过Stable Diffusion直接建模音视频关联，并针对扩散模型的时间一致性问题提出TREPA方法，利用自监督视频模型的时间表示增强生成帧的时序一致性。研究还优化了SyncNet训练框架，改进了模型架构、超参数等关键因素，显著提升了唇同步精度。实验表明，该方法在HDTF和VoxCeleb2数据集上超越现有技术，其SyncNet优化经验可推广至其他音视频同步任务。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T21:05:16Z
- **目录日期**: 2025-06-06
