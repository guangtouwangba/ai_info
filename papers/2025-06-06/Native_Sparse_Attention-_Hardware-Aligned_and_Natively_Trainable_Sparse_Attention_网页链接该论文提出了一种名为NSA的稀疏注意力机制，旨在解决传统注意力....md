...
该论文提出了一种名为NSA（Native Sparse Attention）的新型稀疏注意力机制，旨在解决传统注意力机制在处理长文本时计算成本过高的问题。NSA通过动态分层稀疏策略，结合粗粒度令牌压缩和细粒度令牌选择，在保持全局上下文感知能力的同时提升局部精度。该方法创新性地将算法设计与硬件优化相结合，显著提高了长文本上下文建模的效率。NSA的核心贡献在于提出了两种关键性创新设计，使稀疏注意力机制更适配硬件特性且保持可训练性，为大模型处理长序列任务提供了高效解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T20:04:47Z
- **目录日期**: 2025-06-06
