...
这篇论文提出了一种名为SGD-SaI的新优化方法，挑战了传统自适应梯度方法（如Adam）的必要性。该方法通过初始化时基于梯度信噪比(g-SNR)调整不同参数组的学习率，有效解决了训练不平衡问题，并将内存使用减半。实验显示，SGD-SaI在Transformer任务、ImageNet分类、GPT-2预训练和LoRA微调中表现优于或持平AdamW，尤其在大模型训练中节省显著内存（如为Llama2-7B节省25.15GB）。该方法具有超参数鲁棒性和广泛适用性，为深度学习优化提供了更高效的解决方案。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T03:25:17Z
- **目录日期**: 2025-06-06
