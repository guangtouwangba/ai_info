...
该研究针对局部可观测多智能体MDP问题，提出了"扩展截断策略类"——首个非平凡的近最优闭式策略类。新策略能记忆超出当前视野范围的智能体状态，在有限视野条件下显著提升性能，解决了原有策略存在的"惩罚抖动"现象，并在特定条件下实现完全可观测的最优行为。研究还扩展了局部依赖多智能体MDP模型，允许状态转移依赖和扩展奖励依赖，并在新模型中验证了理论结果。相比前人工作，新策略在保持计算可行性的同时，对视野范围的优化呈指数级接近最优。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T03:21:48Z
- **目录日期**: 2025-06-06
