...
Dec-POMDPs是NEXP完全问题，通常难以求解。DeWeese和Qu（2024）针对局部可见性和依赖性问题提出了局部相互依赖多智能体MDP框架，给出了三种可计算且接近最优的闭式策略，但在有限可见性下可能因"惩罚抖动"现象表现不佳。本研究提出了扩展截断策略类，首次实现了对任意局部相互依赖MDP都接近最优的闭式部分可观测策略，能记忆视野外的智能体，有效解决惩罚抖动问题，并在特定条件下保证完全可观测的最优行为。同时扩展了原框架，支持转移依赖和扩展奖励依赖。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T01:28:45Z
- **目录日期**: 2025-06-06
