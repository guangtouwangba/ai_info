...
本文介绍了针对局部依赖多智能体MDP问题的扩展截断策略类（Extended Cutoff Policy Class），这是首个在部分可观测条件下仍能保持指数级接近最优的非平凡闭式策略类。该策略通过记忆超出可视范围的智能体信息，显著提升了小固定可视范围下的性能，解决了"惩罚抖动"现象，并在特定条件下实现完全可观测的最优行为。研究还扩展了局部依赖多智能体MDP模型，使其包含转移依赖和扩展奖励依赖，并在新框架下复现了理论结果。这些策略在合作导航、避障等任务中具有计算可行性，同时保持接近最优的性能。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T00:01:23Z
- **目录日期**: 2025-06-06
