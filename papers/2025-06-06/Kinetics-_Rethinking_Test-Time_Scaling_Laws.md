...
该研究重新审视了测试阶段的模型扩展规律，发现传统基于计算最优性的方法高估了小模型的实际效率，忽视了推理时的内存访问瓶颈。通过分析0.6B到32B参数的模型，提出了综合考虑计算和内存成本的"动力学扩展定律"，表明资源应优先分配给超过特定规模阈值的模型，因为注意力机制而非参数量成为主要成本因素。研究提出采用稀疏注意力机制的新扩展范式，能在相同资源下支持更长文本生成和更多并行样本。实验显示，稀疏注意力模型在AIME问题求解任务上比密集模型提升5-60个准确率点，特别适合测试阶段的持续扩展。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T19:01:09Z
- **目录日期**: 2025-06-06
