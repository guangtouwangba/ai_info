...
该研究从实际效率角度重新审视了测试阶段的模型缩放规律，发现小模型的效果被显著高估。传统基于计算最优的方法忽视了推理策略（如Best-of-N、长链思维）引入的内存访问瓶颈。通过分析0.6B到32B参数模型，作者提出了综合考虑计算和内存成本的"动力学缩放定律"，表明资源应优先分配给超过特定阈值的大模型。研究发现注意力机制（而非参数量）是测试时主要成本来源，因此提出基于稀疏注意力的新缩放范式，实验显示稀疏模型在各类资源条件下均优于稠密模型，最高可获得60+准确率提升。

## 元数据

- **来源**: ArXiv
- **类型**: 论文
- **保存时间**: 2025-06-06T08:01:21Z
- **目录日期**: 2025-06-06
